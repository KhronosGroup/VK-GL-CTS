    ...
    virtual long Run()
    {
        ...
        if (glGetError() != GL_INVALID_VALUE)
        {
            m_context.getTestContext().getLog() << tcu::TestLog::Message
                                                << "glGetActiveAtomicCounterBufferiv should generate INAVLID_VALUE when"
        ...
        if (glGetError() != GL_INVALID_VALUE)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message
                << "glBindBufferBase should generate INAVLID_VALUE when"
        ...
        if (glGetError() != GL_INVALID_VALUE)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message
                << "glBindBufferRange should generate INAVLID_VALUE when"
                   " index is greater than or equal GL_MAX_ATOMIC_COUNTER_BUFFER_BINDINGS."
                << tcu::TestLog::EndMessage;
            error = ERROR;
        }
...
class NegativeLargeOffset : public SACSubcaseBase
{
    ...
    virtual long Run()
    {
        ...
        std::ostringstream os;
        os << "#version 420 core" NL "layout(location = 0) out uvec4 o_color;" NL "layout(binding = 0, offset = "
           << maxSize
           << ") uniform atomic_uint ac_counter_fs;" NL "void main() {" NL
              "  o_color = uvec4(atomicCounterIncrement(ac_counter_fs)); " NL " }";
