            ...
            if (GL_NO_ERROR == (error = gl.getError()))
            {
                ...
                if (GL_TIMEOUT_EXPIRED == wait_result)
                {
                    m_context.getTestContext().getLog()
                        ...
                        << TEST_SYNC_WAIT_TIMEOUT
                        << " nanoseconds. Potentially test may not be done in finite time "
                ...
                else if ((GL_CONDITION_SATISFIED == wait_result) || (GL_ALREADY_SIGNALED == wait_result))
                {
                    ...
                    {
                        m_context.getTestContext().getLog()
                            << tcu::TestLog::Message << "Result data [" << data_dst[0] << ", " << data_dst[1]
                            << "is not equal to the reference [" << reference[0] << ", " << reference[1]
                            << "]. Tests fails." << tcu::TestLog::EndMessage;
                    }
                }
            }
            else
