...
tcu::TestNode::IterateResult PageSizeGetterTest::iterate()
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Invalid page size reported by glGetDoublev()"
                              " (reported value: "
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Invalid page size reported by glGetFloatv()"
                              " (reported value: "
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Invalid page size reported by glGetInteger64v()"
                              " (reported value: "
    ...
    for (unsigned int n_iteration = 0; n_iteration < 2; ++n_iteration)
    {
        ...
        for (int n_entry_point = 0; n_entry_point < (m_is_texture_buffer_range_supported ? 2 : 1); ++n_entry_point)
        {
                 ...
                 ++n_texel, current_tb_offset += 4 /* rgba */)
            {
                ...
                {
                    m_testCtx.getLog() << tcu::TestLog::Message
                                       << "A texel read from the texture buffer at index "
                                          "["
                                       << n_texel
                                       << "]"
...
bool IndirectDispatchBufferStorageTestCase::execute(glw::GLuint sparse_bo_storage_flags)
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Invalid atomic counter value encountered. "
                              "Expected value: ["
                           << m_expected_ac_value
                           << "]"
                              ", found:"
                    ...
                    if (memcmp(expected_color, found_color, sizeof(expected_color)) != 0)
                    {
                        m_testCtx.getLog()
                            ...
                            << "), "
                               "Found:"
                               "("
                            << (int)found_color[0] << ", " << (int)found_color[1] << ", " << (int)found_color[2] << ", "
                            << (int)found_color[3] << "), " << tcu::TestLog::EndMessage;
    ...
    for (unsigned int n_iteration = 0; n_iteration < 3; ++n_iteration)
    {
                                 ...
                                 static_cast<unsigned int>(current_ssbo_offset + (sizeof(int) * 4 /* std140 */)))
        {
                                     ...
                                     current_ssbo_offset < (ssbo_commit_start_offset + ssbo_commit_size))))
            {
                ...
                {
                    m_testCtx.getLog() << tcu::TestLog::Message
                                       << "Value written to the SSBO at byte "
                                          "["
