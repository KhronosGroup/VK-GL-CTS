    ...
    try
    {
        ...
        for (glw::GLuint i = 0; i < xfb_count; ++i)
        {
            if (!gl.isTransformFeedback(xfb_dsa[i]))
            {
                ...
                /* Log. */
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "CreateTransformFeedbacks has not created defualt objects."
                    << tcu::TestLog::EndMessage;
            }
        }
...
void BuffersTest::prepareObjects()
{
    ...
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Buffer object has not been generated and no error has been triggered."
            << tcu::TestLog::EndMessage;
    ...
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Buffer allocation failed." << tcu::TestLog::EndMessage;
                                   ...
                                   glw::GLchar const *const tested_function_name)
{
    ...
    if (-1 == buffer_binding)
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message
            << "glGetTransformFeedbacki_v with parameter GL_TRANSFORM_FEEDBACK_BUFFER_BINDING "
               "has not returned anything and error has not been generated."
            << tcu::TestLog::EndMessage;
                            ...
                            glw::GLchar const *const tested_function_name)
{
    ...
    if (-1 == buffer_start)
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message
            << "glGetTransformFeedbacki_v with parameter GL_TRANSFORM_FEEDBACK_BUFFER_START "
               "has not returned anything and error has not been generated."
            << tcu::TestLog::EndMessage;
                           ...
                           glw::GLchar const *const tested_function_name)
{
    ...
    if (-1 == buffer_size)
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message
            << "glGetTransformFeedbacki_v with parameter GL_TRANSFORM_FEEDBACK_BUFFER_SIZE "
               "has not returned anything and error has not been generated."
            << tcu::TestLog::EndMessage;
