 ...
 *
 * Specification:
 ** If geometry shading is available, use both a vertex shader and a
 * geometry shader and capture the varyings processed by the geometry
 * shader. Verify it produced the expected result. Also emmit extra
 * primitives from the geometry shader and make sure the
 * extra primitives are captures and are correctly reflected by
 * PRIMITIVES_GENERATED and TRANSFORM_FEEDBACK_PRIMITIVES_WRITTEN. This is
 * to test the new primitives generated by the geometry shader affect the
 * counters correctly.
 *
 * Procedure:
 * A single triangle is drawn with geometry shader. Geometry shader adds
 * one vertex to the middle of triangle. Total of two triangles are
 * generated from triangle strip.
 */
class Geometry : public deqp::TestCase
{
public:
    Geometry(deqp::Context &context, const char *test_name, const char *test_description);
    ~Geometry(void);
    virtual IterateResult iterate(void);
...
protected:
    ...
    std::vector<glw::GLfloat> m_initial_data;
    std::vector<glw::GLfloat> m_reference_data;
    std::vector<glw::GLfloat> m_result_data;
    glw::GLuint m_bo_size;
