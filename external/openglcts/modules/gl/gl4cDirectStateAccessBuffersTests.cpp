    ...
    try
    {
        ...
        for (glw::GLuint i = 0; i < buffers_count; ++i)
        {
            if (!gl.isBuffer(buffers_dsa[i]))
            {
                ...
                /* Log. */
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "CreateBuffers has not created default objects."
                    << tcu::TestLog::EndMessage;
            }
        }
    }
...
tcu::TestNode::IterateResult DataTest::iterate()
{
    ...
    try
    {
        ...
        /* BufferStorage Tests */
        static const glw::GLenum bits[] = {GL_MAP_READ_BIT | GL_DYNAMIC_STORAGE_BIT, GL_MAP_READ_BIT | GL_MAP_WRITE_BIT,
...
const glw::GLuint DataTest::s_reference[]     = {0,   1,   2,   4,    8,    16,  64,
                                                 128, 256, 512, 1024, 2048, 4096}; //!< Reference data.
...
tcu::TestNode::IterateResult ClearTest::iterate()
{
    ...
    try
    {
...

        {
            ...
            is_ok &=
                TestClearNamedBufferData<glw::GLubyte, false>(GL_R8UI, 1, GL_RED_INTEGER, GL_UNSIGNED_BYTE, reference);
            is_ok &=
                TestClearNamedBufferData<glw::GLubyte, false>(GL_RG8UI, 2, GL_RG_INTEGER, GL_UNSIGNED_BYTE, reference);
            is_ok &= TestClearNamedBufferData<glw::GLubyte, false>(GL_RGBA8UI, 4, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE,
                                                                   reference);

            /* unsigned byte component ClearNamedBufferSubData tests */
            is_ok &=
                TestClearNamedBufferData<glw::GLubyte, true>(GL_R8UI, 1, GL_RED_INTEGER, GL_UNSIGNED_BYTE, reference);
            is_ok &=
                TestClearNamedBufferData<glw::GLubyte, true>(GL_RG8UI, 2, GL_RG_INTEGER, GL_UNSIGNED_BYTE, reference);
            is_ok &= TestClearNamedBufferData<glw::GLubyte, true>(GL_RGBA8UI, 4, GL_RGBA_INTEGER, GL_UNSIGNED_BYTE,
                                                                  reference);
        }
...

        {
            ...
            is_ok &= TestClearNamedBufferData<glw::GLushort, false>(GL_R16UI, 1, GL_RED_INTEGER, GL_UNSIGNED_SHORT,
                                                                    reference);
            is_ok &= TestClearNamedBufferData<glw::GLushort, false>(GL_RG16UI, 2, GL_RG_INTEGER, GL_UNSIGNED_SHORT,
                                                                    reference);
            is_ok &= TestClearNamedBufferData<glw::GLushort, false>(GL_RGBA16UI, 4, GL_RGBA_INTEGER, GL_UNSIGNED_SHORT,
                                                                    reference);

            /* unsigned short component ClearNamedBufferSubData tests */
            is_ok &= TestClearNamedBufferData<glw::GLushort, true>(GL_R16UI, 1, GL_RED_INTEGER, GL_UNSIGNED_SHORT,
                                                                   reference);
            is_ok &= TestClearNamedBufferData<glw::GLushort, true>(GL_RG16UI, 2, GL_RG_INTEGER, GL_UNSIGNED_SHORT,
                                                                   reference);
            is_ok &= TestClearNamedBufferData<glw::GLushort, true>(GL_RGBA16UI, 4, GL_RGBA_INTEGER, GL_UNSIGNED_SHORT,
                                                                   reference);
        }
...

        {
            ...
            is_ok &=
                TestClearNamedBufferData<glw::GLuint, false>(GL_R32UI, 1, GL_RED_INTEGER, GL_UNSIGNED_INT, reference);
            is_ok &=
                TestClearNamedBufferData<glw::GLuint, false>(GL_RG32UI, 2, GL_RG_INTEGER, GL_UNSIGNED_INT, reference);
            is_ok &=
                TestClearNamedBufferData<glw::GLuint, false>(GL_RGB32UI, 3, GL_RGB_INTEGER, GL_UNSIGNED_INT, reference);
            is_ok &= TestClearNamedBufferData<glw::GLuint, false>(GL_RGBA32UI, 4, GL_RGBA_INTEGER, GL_UNSIGNED_INT,
                                                                  reference);

            /* unsigned int component ClearNamedBufferSubData tests */
            is_ok &=
                TestClearNamedBufferData<glw::GLuint, true>(GL_R32UI, 1, GL_RED_INTEGER, GL_UNSIGNED_INT, reference);
            is_ok &=
                TestClearNamedBufferData<glw::GLuint, true>(GL_RG32UI, 2, GL_RG_INTEGER, GL_UNSIGNED_INT, reference);
            is_ok &=
                TestClearNamedBufferData<glw::GLuint, true>(GL_RGB32UI, 3, GL_RGB_INTEGER, GL_UNSIGNED_INT, reference);
            is_ok &= TestClearNamedBufferData<glw::GLuint, true>(GL_RGBA32UI, 4, GL_RGBA_INTEGER, GL_UNSIGNED_INT,
                                                                 reference);
        }
...
const glw::GLuint MapReadWriteTest::s_reference[]     = {0,   1,   2,   4,    8,    16,  64,
                                                         128, 256, 512, 1024, 2048, 4096}; //!< Reference data.
...
const glw::GLuint MapWriteOnlyTest::s_reference[]     = {0,   1,   2,   4,    8,    16,  64,
                                                         128, 256, 512, 1024, 2048, 4096}; //!< Reference data.
...
const glw::GLuint MapRangeReadBitTest::s_reference[]     = {1,   2,   4,   8,    16,   64,
                                                            128, 256, 512, 1024, 2048, 4096}; //!< Reference data.
...
const glw::GLuint MapRangeWriteBitTest::s_reference[]     = {1,   2,   4,   8,    16,   64,
                                                             128, 256, 512, 1024, 2048, 4096}; //!< Reference data.
...
const glw::GLuint SubDataQueryTest::s_reference[]     = {1,   2,   4,   8,    16,   64,
                                                         128, 256, 512, 1024, 2048, 4096}; //!< Reference data.
                                  ...
                                  const glw::GLchar *when_shall_be_generated)
{
    ...
    if (expected_error != (error = gl.getError()))
    {
        /* Log. */
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << function_name << " does not generate " << glu::getErrorStr(expected_error)
            << when_shall_be_generated << "The error value of " << glu::getErrorStr(error) << " was observed."
            << tcu::TestLog::EndMessage;
...
bool ErrorsTest::TestErrorsOfClearNamedBufferData()
{
    ...
    try
    {
        ...
        {
            (void)(glw::GLbyte *)m_pMapNamedBufferRange(buffer, 0, sizeof(unused_data),
                                                        GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT);
            GLU_EXPECT_NO_ERROR(gl.getError(), "glMapNamedBuffer failed.");
...
bool ErrorsTest::TestErrorsOfClearNamedBufferSubData()
{
    ...
    try
    {
        ...
        {
            /* Test. */
            m_pClearNamedBufferSubData(buffer, GL_R8, 0, -((glw::GLsizei)sizeof(unused_data)), GL_RGBA,
                                       GL_UNSIGNED_BYTE, &unused_data);
...
bool ErrorsTest::TestErrorsOfCopyNamedBufferSubData()
{
    ...
    try
    {
        ...
        {
            (void)(glw::GLbyte *)m_pMapNamedBufferRange(buffer_r, 0, sizeof(unused_data),
                                                        GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT);
            GLU_EXPECT_NO_ERROR(gl.getError(), "glMapNamedBuffer failed.");
        ...
        {
            (void)(glw::GLbyte *)m_pMapNamedBufferRange(buffer_w, 0, sizeof(unused_data),
                                                        GL_MAP_READ_BIT | GL_MAP_PERSISTENT_BIT);
            GLU_EXPECT_NO_ERROR(gl.getError(), "glMapNamedBuffer failed.");
...
bool ErrorsTest::TestErrorsOfNamedBufferData()
{
    ...
    try
    {
        ...
        {
            /* Prepare for invalid type error behavior verification. */
            static const glw::GLenum valid_usages[] = {
                GL_STREAM_DRAW, GL_STREAM_READ,  GL_STREAM_COPY,  GL_STATIC_DRAW,  GL_STATIC_READ,
                GL_STATIC_COPY, GL_DYNAMIC_DRAW, GL_DYNAMIC_READ, GL_DYNAMIC_COPY, GL_NONE};
...
bool ErrorsTest::TestErrorsOfNamedBufferStorage()
{
    ...
    try
    {
        ...
        /* Test improper persistent bit behavior error behavior. */
        {
            ...
            is_ok &= ErrorCheckAndLog("glNamedBufferStorage", GL_INVALID_VALUE,
                                      " if flags contains MAP_PERSISTENT_BIT "
...
const glw::GLint FunctionalTest::s_initial_data_a[] = {1, 2, 3,
                                                       4, 5, 5}; //!< Initial data to be uploaded for the input buffer.

const glw::GLint FunctionalTest::s_initial_data_b[] = {
    0, 0, 0, 0, 0, 0, 36}; //!< Initial data to be uploaded for the output buffer.

const glw::GLint FunctionalTest::s_expected_data[] = {
    0, 1, 4, 9, 16, 25, 36}; //!< Expected result which shall be read from output buffer.
        ...
        if (GL_NO_ERROR == gl.getError())
        {
            ...
            if (GL_NO_ERROR == gl.getError())
            {
                ...
                if (GL_NO_ERROR == gl.getError())
                {
                    ...
                    if ((GL_NO_ERROR == gl.getError()) || (DE_NULL == p))
                    {
                        ...
                        if (GL_NO_ERROR == gl.getError())
                        {
                            ...
                            if (GL_NO_ERROR == gl.getError())
                            {
                                    ...
                                    *p = 3;
                                    ...
                                    if (GL_NO_ERROR == gl.getError())
                                    {
                                        ...
                                        else
                                        {
                                            m_pUnmapNamedBuffer(m_bo_in);
                                            m_context.getTestContext().getLog()
                                                << tcu::TestLog::Message << "GetNamedBufferPointerv has failed."
                                                << tcu::TestLog::EndMessage;
                                        }
                                    }
                                    else
                                    {
                                        m_pUnmapNamedBuffer(m_bo_in);
                                        m_context.getTestContext().getLog()
                                            << tcu::TestLog::Message << "FlushMappedNamedBufferRange has failed."
                                            << tcu::TestLog::EndMessage;
                                    }
                                }
                                else
                                {
                                    m_context.getTestContext().getLog()
                                        << tcu::TestLog::Message << "MapNamedBufferRange has failed."
                                        << tcu::TestLog::EndMessage;
                                }
                            }
                            else
                            {
                                m_context.getTestContext().getLog()
                                    << tcu::TestLog::Message << "CopyNamedBufferSubData has failed."
                                    << tcu::TestLog::EndMessage;
                            }
                        }
                        else
                    ...
                    {
                        m_context.getTestContext().getLog()
                            << tcu::TestLog::Message << "MapNamedBuffer has failed." << tcu::TestLog::EndMessage;
                    }
                }
                else
                {
                    m_context.getTestContext().getLog()
                        << tcu::TestLog::Message << "NamedBufferSubData has failed." << tcu::TestLog::EndMessage;
                }
            }
            else
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "ClearNamedBufferSubData has failed." << tcu::TestLog::EndMessage;
            }
        }
        else
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "NamedBufferStorage has failed." << tcu::TestLog::EndMessage;
        }
    }
    else
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "CreateBuffers has failed." << tcu::TestLog::EndMessage;
    }
...
bool FunctionalTest::PrepareOutputBuffer()
{
    ...
    if (GL_NO_ERROR == gl.getError())
    {
        ...
        if (GL_NO_ERROR == gl.getError())
        {
            ...
            if (GL_NO_ERROR == gl.getError())
            {
                ...
                {
                    m_context.getTestContext().getLog()
                        << tcu::TestLog::Message << "BindBufferRange has failed." << tcu::TestLog::EndMessage;
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "NamedBufferData has failed." << tcu::TestLog::EndMessage;
            }
        }
        else
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "BindBuffer has failed." << tcu::TestLog::EndMessage;
            throw 0; /* This function is not being tested, throw test internal error */
        }
    }
    else
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "GenBuffers has failed." << tcu::TestLog::EndMessage;
        throw 0; /* This function is not being tested, throw test internal error */
    }
...
bool FunctionalTest::CheckArrayBufferImmutableFlag()
{
    ...
    if (GL_NO_ERROR == gl.getError())
    {
        ...
        if (-1 != is_storage_immutable)
        {
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "Input buffer storage is unexpectedly mutable."
                    << tcu::TestLog::EndMessage;
            }
        }
        else
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "GetNamedBufferParameteriv has not returned a data."
                << tcu::TestLog::EndMessage;
        }
    }
    else
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "GetNamedBufferParameteriv has failed." << tcu::TestLog::EndMessage;
    }
...
bool FunctionalTest::CheckTransformFeedbackBufferSize()
{
    ...
    if (GL_NO_ERROR == gl.getError())
    {
        ...
        if (-1 != size)
        {
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "Output buffer size is " << size << ", but " << sizeof(s_initial_data_b)
        ...
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "GetNamedBufferParameteri64v has not returned a data."
                << tcu::TestLog::EndMessage;
        }
    }
    else
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "GetNamedBufferParameteri64v has failed." << tcu::TestLog::EndMessage;
    }
...
bool FunctionalTest::CheckTransformFeedbackResult()
{
    ...
    if (GL_NO_ERROR == gl.getError())
    {
        for (glw::GLuint i = 0; i < sizeof(s_initial_data_b) / sizeof(s_initial_data_b[0]); ++i)
        {
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "Expected data is not equal to results." << tcu::TestLog::EndMessage;
                return false;
            }
        }
    ...
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "GetNamedBufferSubData has failed." << tcu::TestLog::EndMessage;
    }
    return false;
}
