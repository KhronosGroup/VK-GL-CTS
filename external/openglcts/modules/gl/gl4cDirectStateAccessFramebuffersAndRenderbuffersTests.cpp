    ...
    try
    {
        ...
        for (glw::GLuint i = 0; i < framebuffers_count; ++i)
        {
            if (!gl.isFramebuffer(framebuffers_dsa[i]))
            {
                ...
                /* Log. */
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "CreateFramebuffers has not created default objects."
                    << tcu::TestLog::EndMessage;
            }
        }
    }
...
bool RenderbufferAttachmentTest::Test(glw::GLenum attachment, glw::GLenum internalformat)
{
    ...
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "NamedFramebufferRenderbuffer for "
            << glu::getFramebufferAttachmentStr(attachment) << " attachment failed with error value "
                                      ...
                                      glw::GLenum internalformat, glw::GLuint levels, glw::GLint layers)
{
    ...
    layers = deMax32(1, (glw::GLint)((uint64_t)layers / (1ull << (uint64_t)(2 * (levels - 1)))));
...
const glw::GLint TextureLayerAttachmentTest::s_reference_color_integer[4] = {1, 2, 3, 4}; //!< Reference integer color.
...
tcu::TestNode::IterateResult DrawReadBufferTest::iterate()
{
    ...
    try
    {
        ...
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "Framebuffer is unexpectedly incomplete." << tcu::TestLog::EndMessage;
...
bool InvalidateDataTest::CheckErrorAndLog(const glw::GLenum attachment)
{
    ...
    if (glw::GLenum error = gl.getError())
    {
        ...
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "InvalidateDataTest unexpectedly generated " << glu::getErrorStr(error)
            << " error for attachment " << glu::getFramebufferAttachmentStr(attachment) << ". Test fails."
            << tcu::TestLog::EndMessage;
...
bool InvalidateSubDataTest::CheckErrorAndLog(const glw::GLenum attachment)
{
    ...
    if (glw::GLenum error = gl.getError())
    {
        /* There is an error. Log. */
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "InvalidateSubDataTest unexpectedly generated " << glu::getErrorStr(error)
            << " error for attachment " << glu::getFramebufferAttachmentStr(attachment) << ". Test fails."
            << tcu::TestLog::EndMessage;
...
bool BlitTest::CheckColor()
{
    ...
    /* Reference values. */
    static const glw::GLfloat reference[2][3][4] = {{{1.f, 0.f, 0.f, 1.f}, {0.f, 1.f, 0.f, 1.f}, {0.f, 0.f, 1.f, 1.f}},
...
void GetParametersTest::PrepareFramebuffer()
{
    ...
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Unexpectedly framebuffer was incomplete." << tcu::TestLog::EndMessage;
...
bool GetParametersTest::TestDefaultFramebuffer()
{
    ...
    for (glw::GLuint i = 0; i < pnames_count; ++i)
    {
        ...
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "GetNamedFramebufferParameteriv returned " << parameter_dsa << ", but "
                << parameter_legacy << " was expected for " << pnames_strings[i] << " parameter name of default object."
                << tcu::TestLog::EndMessage;
...
bool GetParametersTest::TestCustomFramebuffer()
{
    ...
    for (glw::GLuint i = 0; i < pnames_count; ++i)
    {
        ...
        if (parameter_legacy != parameter_dsa)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "GetNamedFramebufferParameteriv returned " << parameter_dsa << ", but "
                << parameter_legacy << " was expected for " << pnames_strings[i]
                << " parameter name of framebuffer object." << tcu::TestLog::EndMessage;
    ...
    for (glw::GLuint j = 0; j < attachments_count; ++j)
    {
        ...
        if (parameter_legacy != parameter_dsa)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "GetNamedFramebufferAttachmentParameteriv returned " << parameter_dsa
                << ", but " << parameter_legacy
                << " was expected for GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE parameter "
...
tcu::TestNode::IterateResult GetAttachmentParameterErrorsTest::iterate()
{
    ...
    try
    {
        ...
        gl.getNamedFramebufferAttachmentParameteriv(m_fbo_invalid, GL_COLOR_ATTACHMENT0,
                                                    GL_FRAMEBUFFER_ATTACHMENT_COMPONENT_TYPE,
...
bool FunctionalTest::PrepareFirstFramebuffer()
{
    ...
    try
    {
        ...
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "CheckNamedFramebufferStatus is incomplete." << tcu::TestLog::EndMessage;
    ...
    try
    {
        ...
        for (glw::GLuint i = 0; i < renderbuffers_count; ++i)
        {
            if (!gl.isRenderbuffer(renderbuffers_dsa[i]))
            {
                ...
                /* Log. */
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "CreateRenderbuffers has not created default objects."
                    << tcu::TestLog::EndMessage;
            }
        }
    }
...
StorageTest::StorageTest(deqp::Context &context)
    : deqp::TestCase(context, "renderbuffers_storage", "Renderbuffer Objects Storage Test")
    , m_fbo(0)
    , m_rbo(0)
{
    /* Intentionally left blank. */
                ...
                if (hasComponent[i % 4 /* color components count*/])
                {
                        ...
                        2 /* Precision */)
                    {
                        m_context.getTestContext().getLog()
                            ...
                            << s_reference_color_integer[i % 4 /* color components count*/] << ", but fetched value "
...
tcu::TestNode::IterateResult StorageMultisampleTest::iterate()
{
    ...
    try
    {
        ...
        const struct
        {
            ...
            {1, 1}, {(glw::GLuint)max_renderbuffer_size / 2, 1}, {1, (glw::GLuint)max_renderbuffer_size / 2}};
    ...
    for (glw::GLuint i = 0; i < 2; ++i)
    {
        ...
        if (i)
        {
            ...
            if (glw::GLenum error = gl.getError())
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message
                    << "Renderbuffer storage multisample test failed because "
...
const glw::GLint StorageMultisampleTest::s_reference_color_integer[4] = {
    1, 2, 3, 4}; //!< Reference color value for integral color internal formats.
...
tcu::TestNode::IterateResult StorageErrorsTest::iterate()
{
    ...
    try
    {
        ...
        is_ok &= ExpectError(GL_INVALID_ENUM, "NamedRenderbufferStorage",
                             "internalformat is not a color-renderable, "
