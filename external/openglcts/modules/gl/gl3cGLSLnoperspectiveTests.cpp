...
void compile_shader(deqp::Context &context, glw::GLuint shader_id, const glw::GLchar *shader_code)
{
    ...
    if (GL_TRUE != status)
    {
        ...
        context.getTestContext().getLog()
            << tcu::TestLog::Message << "Failed to compile shader:\n"
            << &message[0] << tcu::TestLog::EndMessage << tcu::TestLog::KernelSource(shader_code);
...
tcu::TestNode::IterateResult FunctionalTest::iterate()
{
    ...
    static const char *shaders[][2] = {
        {vs_noperspective, fs_noperspective}, {vs_default, fs_default}, {vs_flat, fs_flat}, {vs_smooth, fs_smooth}};
