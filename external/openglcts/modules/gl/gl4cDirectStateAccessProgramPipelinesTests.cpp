    ...
    try
    {
        ...
        for (glw::GLuint i = 0; i < program_pipelines_count; ++i)
        {
            if (!gl.isProgramPipeline(program_pipelines_dsa[i]))
            {
                ...
                /* Log. */
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "CreateProgramPipelines has not created default objects."
                    << tcu::TestLog::EndMessage;
            }
        }
    }
...
bool DefaultsTest::testProgramPipelineParameter(glw::GLenum pname, glw::GLint expected_value)
{
    ...
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "glGetProgramPipelineiv with parameter " << pname
    ...
    else
    {
        ...
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "glGetProgramPipelineiv with parameter " << pname << " has returned "
                << value << ", however " << expected_value << " was expected." << tcu::TestLog::EndMessage;
...
tcu::TestNode::IterateResult ErrorsTest::iterate()
{
    ...
    try
    {
        ...
        if (GL_INVALID_VALUE != (error = gl.getError()))
        {
            ...
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message
                << "CreateProgramPipelines has not generated INVALID_VALUE error when callded "
...
void FunctionalTest::prepareShaderPrograms()
{
    ...
    if (GL_FALSE == status)
    {
        ...
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Vertex shader program building failed with log: " << log
            << tcu::TestLog::EndMessage;
    ...
    if (GL_FALSE == status)
    {
        ...
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Fragment shader program building failed with log: " << log
            << tcu::TestLog::EndMessage;
