...
TextureLodSelectionTestCase::TextureLodSelectionTestCase(deqp::Context& context) :
	...
	, m_texture(0), m_vao(0), m_vbo(0), m_isContextES(false)
	{
	}
	...
	for (size_t n = 0; n < textures.size(); ++n)
	{
		...
		else
		{
			...
			*/
			...
			if (!drawAndVerify(locScale, -1.0f, 1, 2, 4, 0.0f, -1000.0f, 1000.0f, GL_LINEAR,
			...
			*/
			...
			GLU_EXPECT_NO_ERROR(gl.getError(), "texParameterf");
			if (!drawAndVerify(locScale, 0.0f, 1, 2, 4, 0.0f, 0.5f, 1000.0f, GL_LINEAR, GL_LINEAR_MIPMAP_LINEAR,
							   true))
			{
				ret = false;
			}
			...
			*/
			...
			GLU_EXPECT_NO_ERROR(gl.getError(), "texParameterf");
			if (!drawAndVerify(locScale, 1.0f, 1, 2, 4, 0.0f, -1000.0f, 0.5f, GL_LINEAR, GL_LINEAR_MIPMAP_LINEAR,
							   true))
			{
				ret = false;
			}
...
bool TextureLodSelectionTestCase::doComparison(const int size, const GLubyte* const expectedcolor)
{
	...
	GLint col_bits[] = { m_context.getRenderTarget().getPixelFormat().redBits,
