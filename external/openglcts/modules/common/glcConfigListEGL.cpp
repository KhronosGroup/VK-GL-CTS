...
{
    uint32_t renderableMask = 0;
    uint32_t conformantMask = 0;
    ...
    for (std::vector<eglw::EGLConfig>::iterator cfgIter = configs.begin(); cfgIter != configs.end(); cfgIter++)
    {
        ...
        uint32_t renderableBits = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_RENDERABLE_TYPE);
        uint32_t conformantBits = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_CONFORMANT);
        int32_t redSize         = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_RED_SIZE);
        int32_t greenSize       = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_GREEN_SIZE);
        int32_t blueSize        = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_BLUE_SIZE);
        int32_t alphaSize       = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_ALPHA_SIZE);
        int32_t depthSize       = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_DEPTH_SIZE);
        int32_t stencilSize     = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_STENCIL_SIZE);
        int32_t numSamples      = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_SAMPLES);
        ...
        uint32_t surfaceBits  = eglu::getConfigAttribInt(library, display, *cfgIter, EGL_SURFACE_TYPE);
        uint32_t surfaceTypes = ((surfaceBits & EGL_WINDOW_BIT) ? SURFACETYPE_WINDOW : 0) |
        ...
        else
        {
            ...
            configList.excludedConfigs.push_back(ExcludedConfig(CONFIGTYPE_EGL, id,
                                                                !isRenderable     ? EXCLUDEREASON_NOT_COMPATIBLE :
                                                                !isConformant     ? EXCLUDEREASON_NOT_CONFORMANT :
                                                                (numSamples != 0) ? EXCLUDEREASON_MSAA :
                                                                isFloatType       ? EXCLUDEREASON_FLOAT :
                                                                                    EXCLUDEREASON_YUV));
        }
    }
