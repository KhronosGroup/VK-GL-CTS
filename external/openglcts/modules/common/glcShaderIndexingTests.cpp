...
private:
    ...
    virtual void setupUniforms(uint32_t programID, const Vec4 &constCoords);
...
void ShaderIndexingCase::setup(uint32_t programID)
{
    DE_UNREF(programID);
}

void ShaderIndexingCase::setupUniforms(uint32_t programID, const Vec4 &constCoords)
{
                                                      ...
                                                      glu::GLSLVersion glslVersion)
{
    ...
    std::string vtx =
        glu::getGLSLVersionDeclaration(glslVersion) + std::string("\n"
    ...
    std::string frag = glu::getGLSLVersionDeclaration(glslVersion) +
                       std::string("\n"
                                   "precision highp float;\n"
                                   "in float color[4];\n"
    ...
    return new ShaderIndexingCase(context, caseName, description, false, TYPE_FLOAT, evalGreenColor, vtx.c_str(),
                                  frag.c_str());
}
