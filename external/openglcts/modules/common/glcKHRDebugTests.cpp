...
#define CHECK_ERROR(expected_error, error_message)                                                                     \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            m_testCtx.getLog() << tcu::TestLog::Message << "File: " << __FILE__ << ", line: " << __LINE__              \
...
APIErrorsTest::APIErrorsTest(tcu::TestContext &testCtx, glu::ApiType apiType, bool is_debug, const GLchar *name)
    : TestBase(testCtx, apiType, is_debug)
    , TestCase(testCtx, name, "Verifies that errors are generated as expected")
{
    /* Nothing to be done */
}
...
LabelsTest::LabelsTest(tcu::TestContext &testCtx, glu::ApiType apiType, bool is_debug, const GLchar *name)
    : TestCase(testCtx, name, "Verifies that labels can be assigned and queried")
    , TestBase(testCtx, apiType, is_debug)
{
    /* Nothing to be done */
}
...
tcu::TestNode::IterateResult LabelsTest::iterate()
{
    ...
    for (size_t test_case_index = 0; test_case_index < n_test_cases; ++test_case_index)
    {
        ...
        catch (const std::exception &exc)
        {
            ...
            m_testCtx.getLog() << tcu::TestLog::Message << "Error during test case: " << test_case.m_name
                               << tcu::TestLog::EndMessage;
...
tcu::TestNode::IterateResult ReceivingMessagesTest::iterate()
{
     ...
     */
    {
        ...
        if (1 != ret)
        {
            m_testCtx.getLog() << tcu::TestLog::Message
                               << "GetDebugMessageLog returned invalid number of messages: " << ret << ", expected 1"
                               << tcu::TestLog::EndMessage;
     ...
     */
    {
        ...
        {
            m_testCtx.getLog() << tcu::TestLog::Message << "State of DEBUG_LOGGED_MESSAGES: " << n_debug_messages
                               << ", expected " << max_debug_messages << tcu::TestLog::EndMessage;
    ...
    if (1 != max_debug_messages)
    {
        ...
        {
            m_testCtx.getLog() << tcu::TestLog::Message
                               << "GetDebugMessageLog returned unexpected number of messages: " << ret << ", expected "
                               << half_count << tcu::TestLog::EndMessage;
        ...
        {
            m_testCtx.getLog() << tcu::TestLog::Message << "State of DEBUG_LOGGED_MESSAGES: " << n_debug_messages
                               << ", expected " << rest_count << tcu::TestLog::EndMessage;
        ...
        {
            m_testCtx.getLog() << tcu::TestLog::Message
                               << "GetDebugMessageLog returned unexpected number of messages: " << ret << ", expected 1"
                               << tcu::TestLog::EndMessage;
                                              ...
                                              GLvoid *expected_user_info) const
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "State of DEBUG_OUTPUT: " << debug_state << ", expected "
                           << expected_state << tcu::TestLog::EndMessage;
...
void ReceivingMessagesTest::inspectCallbackCounter(GLuint &callback_counter, GLuint expected_number_of_messages) const
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Debug callback was executed invalid number of times: " << callback_counter
...
void ReceivingMessagesTest::inspectMessageLog(GLuint expected_number_of_messages) const
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "GetDebugMessageLog returned invalid number of messages: " << ret
                           << ", expected " << expected_number_of_messages << tcu::TestLog::EndMessage;
...
void GroupsTest::inspectGroupStack(GLuint expected_depth) const
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "State of DEBUG_GROUP_STACK_DEPTH: " << stack_depth
                           << ", expected " << expected_depth << tcu::TestLog::EndMessage;
                                   ...
                                   const glw::GLchar *expected_label) const
{
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Got message with invalid source: " << source << ", expected "
                           << expected_source << tcu::TestLog::EndMessage;
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Got message with invalid type: " << type << ", expected "
                           << expected_type << tcu::TestLog::EndMessage;
    ...
    if (expected_id != id)
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Got message with invalid id: " << id << ", expected "
                           << expected_id << tcu::TestLog::EndMessage;
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Got message with invalid severity: " << severity
                           << ", expected " << expected_severity << tcu::TestLog::EndMessage;
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Got message with invalid length: " << length << ", expected "
                           << expected_length << tcu::TestLog::EndMessage;
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Got message with invalid message: " << messageLog
                           << ", expected " << expected_label << tcu::TestLog::EndMessage;
...
tcu::TestNode::IterateResult SynchronousCallsTest::iterate()
{
    ...
    m_tls.set((void *)(uintptr_t)m_uid);
