...
FramebufferBlitMultiToSingleSampledTestCase::FramebufferBlitMultiToSingleSampledTestCase(deqp::Context& context) :
	TestCase(context, "framebuffer_blit_functionality_multisampled_to_singlesampled_blit",
...
void FramebufferBlitMultiToSingleSampledTestCase::init()
{
	...
	m_setup.ul_rect = { 0, bufHeight / 2, bufWidth / 2, bufHeight - bufHeight / 2 }; /* upper left  (x, y, width, height) */
	m_setup.ur_rect = { bufWidth / 2, bufHeight / 2, bufWidth - bufWidth / 2, bufHeight - bufHeight / 2 };			/* upper right (x, y, width, height) */
...
bool FramebufferBlitMultiToSingleSampledTestCase::init_gl_objs(F f, const GLuint count, const GLuint* buf, const GLint format)
{
 ...
 */
bool FramebufferBlitMultiToSingleSampledTestCase::getColor(const Coord & coord, Color* color, const bool bFloatInternalFormat)
{
	bool result = true;
 ...
 */
bool FramebufferBlitMultiToSingleSampledTestCase::checkColor(const Color & actual, const Color & expect, const GLuint channels)
{
	...
	for (GLuint i = 0; i < m_bufferCfg.size(); i++)
	{
		...
		for (GLint j = 0; j < max_color_attachments; j++)
		{
			...
			for (GLuint k = 0; k < m_multisampleColorCfg.size(); k++)
			{
					...
					if (m_isContextES)
					{
						...
						if (GL_SRGB8_ALPHA8 == color_config.internal_format)
						{
							...
							gl.getFramebufferAttachmentParameteriv(
								GL_DRAW_FRAMEBUFFER, m_defaultFBO ? GL_COLOR_ATTACHMENT0 : GL_BACK,
						...
						{
							m_testCtx.getLog() << tcu::TestLog::Message
											   << "Required channel for "
											   << getEnumName(color_config.internal_format)
											   << " not present in default framebuffer. Skipping."
											   << tcu::TestLog::EndMessage;
							continue;
						}
					}
				...
				if (m_msTbosSupported && buf_config.src_type == GL_TEXTURE_2D)
				{
					result &= clearColorBuffer(m_fbos[0], GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D_MULTISAMPLE, m_color_tbos[0],
				...
				else if (buf_config.src_type == GL_RENDERBUFFER)
				{
					result &= clearColorBuffer(m_fbos[0], GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_color_rbos[0],
											   ul_color, m_setup.ul_rect, ul_center,
											   color_config.color_channel_bits, color_config.bIsFloat);
					result &= clearColorBuffer(m_fbos[0], GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_color_rbos[0],
											   ur_color, m_setup.ur_rect, ur_center,
											   color_config.color_channel_bits, color_config.bIsFloat);
					result &= clearColorBuffer(m_fbos[0], GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_color_rbos[0],
											   ll_color, m_setup.ll_rect, ll_center,
											   color_config.color_channel_bits, color_config.bIsFloat);
					result &= clearColorBuffer(m_fbos[0], GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_color_rbos[0],
											   lr_color, m_setup.lr_rect, lr_center,
											   color_config.color_channel_bits, color_config.bIsFloat);
				}
				else
				{
					result &= clearColorBuffer(m_defaultFBO, GL_NONE, 0, 0, ul_color, m_setup.ul_rect, ul_center,
											   color_config.color_channel_bits,
											   color_config.bIsFloat); /* default buffer */
					result &= clearColorBuffer(m_defaultFBO, GL_NONE, 0, 0, ur_color, m_setup.ur_rect, ur_center,
											   color_config.color_channel_bits,
											   color_config.bIsFloat); /* default buffer */
					result &= clearColorBuffer(m_defaultFBO, GL_NONE, 0, 0, ll_color, m_setup.ll_rect, ll_center,
											   color_config.color_channel_bits,
											   color_config.bIsFloat); /* default buffer */
					result &= clearColorBuffer(m_defaultFBO, GL_NONE, 0, 0, lr_color, m_setup.lr_rect, lr_center,
											   color_config.color_channel_bits,
											   color_config.bIsFloat); /* default buffer */
				}
				...
				{
					result &= clearColorBuffer(m_fbos[1], GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_color_tbos[1], DST_COLOR,
										  m_fullRect, m_defaultCoord, color_config.color_channel_bits,
										  color_config.bIsFloat);
				}
				else if (buf_config.dst_type == GL_RENDERBUFFER)
				{
					result &= clearColorBuffer(m_fbos[1], GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, m_color_rbos[1], DST_COLOR,
										  m_fullRect, m_defaultCoord, color_config.color_channel_bits,
										  color_config.bIsFloat);
				}
				else
				...
				gl.bindFramebuffer(GL_READ_FRAMEBUFFER,
										 buf_config.src_type == 0 ? m_defaultFBO : *buf_config.src_fbo);
				GLU_EXPECT_NO_ERROR(gl.getError(), "bindFramebuffer");

				gl.bindFramebuffer(GL_DRAW_FRAMEBUFFER,
										 buf_config.dst_type == 0 ? m_defaultFBO : *buf_config.dst_fbo);
				GLU_EXPECT_NO_ERROR(gl.getError(), "bindFramebuffer");
																	  ...
																	  const tcu::IVec2& lr_center)
{
	...
	for (GLuint i = 0; i < m_bufferCfg.size(); i++)
	{
		...
		for (GLuint j = 0; j < m_depthCfg.size(); j++)
		{
			...
			result &=
				init_gl_objs<GL_RENDERBUFFER, 0>(gl.bindRenderbuffer, 1, &m_depth_rbos[1], depth_config.internal_format);
			...
			if (m_msTbosSupported && buf_config.src_type == GL_TEXTURE_2D)
			{
				result &= clearDepthBuffer(m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
									  m_depth_tbos[0], depth_config.internal_format, ul_depth, m_setup.ul_rect, ul_center);
				result &= clearDepthBuffer(m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
									  m_depth_tbos[0], depth_config.internal_format, ur_depth, m_setup.ur_rect, ur_center);
				result &= clearDepthBuffer( m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
									  m_depth_tbos[0], depth_config.internal_format, ll_depth, m_setup.ll_rect, ll_center);
				result &= clearDepthBuffer( m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
									  m_depth_tbos[0], depth_config.internal_format, lr_depth, m_setup.lr_rect, lr_center);
			}
			else if (buf_config.src_type == GL_RENDERBUFFER)
			{
			...
			if (depth_config.attachment == GL_DEPTH_STENCIL_ATTACHMENT)
			{
				if (m_msTbosSupported && buf_config.src_type == GL_TEXTURE_2D)
				{
					result &= clearStencilBuffer( m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
											m_depth_tbos[0], depth_config.internal_format, ul_stcil, m_setup.ul_rect,
											ul_center);
					result &= clearStencilBuffer( m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
											m_depth_tbos[0], depth_config.internal_format, ur_stcil, m_setup.ur_rect,
											ur_center);
					result &= clearStencilBuffer( m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
											m_depth_tbos[0], depth_config.internal_format, ll_stcil, m_setup.ll_rect,
											ll_center);
					result &= clearStencilBuffer( m_fbos[0], depth_config.attachment, GL_TEXTURE_2D_MULTISAMPLE,
											m_depth_tbos[0], depth_config.internal_format, lr_stcil, m_setup.lr_rect,
											lr_center);
				}
				else if (buf_config.src_type == GL_RENDERBUFFER)
				{
...
FramebufferBlitTests::FramebufferBlitTests(deqp::Context& context)
	: TestCaseGroup(context, "framebuffer_blit",
					"Verify conformance of framebuffer blit implementation")
{
}
