					...
					GLubyte* colors, GLubyte* r)
{
	...
	auto copy_pixel = [](GLubyte* dst, GLubyte* src) {dst[0]=src[0]; dst[1]=src[1]; dst[2]=src[2]; dst[3]=src[3];};
...
tcu::TestNode::IterateResult TextureLodBiasAllTestCase::iterate()
{
	...
	if (failedbiases > 0)
	{
		m_testCtx.getLog() << tcu::TestLog::Message << "Total failed lod bias combinations (vertex shader): "
						   << failedbiases << tcu::TestLog::EndMessage;
	}
...
TextureLodBiasTests::TextureLodBiasTests(deqp::Context& context)
	: TestCaseGroup(context, "texture_lod_bias",
					"Verify conformance of texture lod bias functionality")
{
}
