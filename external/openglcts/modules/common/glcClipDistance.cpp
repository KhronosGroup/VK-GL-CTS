...
glcts::ClipDistance::CoverageTest::CoverageTest(deqp::Context &context)
    : ClipDistanceTestBase(context, "coverage", "Clip Distance API Coverage Test")
    , m_gl_max_clip_distances_value(0)
{
    /* Intentionally left blank. */
}
            ...
            for (glw::GLuint i_redeclaration = 0; i_redeclaration < 2; ++i_redeclaration)
            {
                     ...
                     ++i_clip_count)
                {
                    ...
                    if (!checkResults(primitive_type, i_clip_function, results))
                    {
                        /* Result's setup. */
                        m_testCtx.getLog()
                            << tcu::TestLog::Message << "Functional test have failed when drawing "
                            << glu::getPrimitiveTypeStr(primitive_type) << ((redeclaration) ? " with " : " without ")
...
const glw::GLchar *glcts::ClipDistance::FunctionalTest::m_vertex_shader_code = "${VERSION}\n"
                                                                               "${EXTENSION}\n"
                                                                               "\n"
...
const glw::GLchar *glcts::ClipDistance::NegativeTest::m_fragment_shader_code = "${VERSION}\n"
                                                                               "${PRECISION}\n"
                                                                               "\n"
                                                                               "out vec4 color;\n"
                                                                               "\n"
 ...
 */
const glcts::ClipDistance::Utility::Program::LinkageStatus &glcts::ClipDistance::Utility::Program::ProgramStatus() const
{
