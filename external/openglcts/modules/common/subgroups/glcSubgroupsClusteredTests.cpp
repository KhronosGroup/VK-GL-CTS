...
std::string getBodySource(CaseDefinition caseDef)
{
    ...
    for (uint32_t i = 1; i <= subgroups::maxSupportedSubgroupSize(); i *= 2)
    {
        bdy << "  {\n"
            ...
            << " op = " << getOpTypeName(caseDef.opType) + "(data[gl_SubgroupInvocationID], clusterSize);\n"
...
deqp::TestCaseGroup *createSubgroupsClusteredTests(deqp::Context &testCtx)
{
    de::MovePtr<deqp::TestCaseGroup> graphicGroup(
        new deqp::TestCaseGroup(testCtx, "graphics", "Subgroup clustered category tests: graphics"));
    de::MovePtr<deqp::TestCaseGroup> computeGroup(
        new deqp::TestCaseGroup(testCtx, "compute", "Subgroup clustered category tests: compute"));
    de::MovePtr<deqp::TestCaseGroup> framebufferGroup(
        new deqp::TestCaseGroup(testCtx, "framebuffer", "Subgroup clustered category tests: framebuffer"));

    const ShaderStageFlags stages[] = {SHADER_STAGE_VERTEX_BIT, SHADER_STAGE_TESS_EVALUATION_BIT,
    ...
    de::MovePtr<deqp::TestCaseGroup> group(
        new deqp::TestCaseGroup(testCtx, "clustered", "Subgroup clustered category tests"));
