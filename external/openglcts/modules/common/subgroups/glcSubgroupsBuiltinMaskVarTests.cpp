...
void initPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    else
    {
        ...
        {
            const string geometry = "#extension GL_KHR_shader_subgroup_ballot: enable\n"
                                    "layout(${TOPOLOGY}) in;\n"
                                    "layout(points, max_vertices = 1) out;\n"
...
deqp::TestCaseGroup *createSubgroupsBuiltinMaskVarTests(deqp::Context &testCtx)
{
    de::MovePtr<deqp::TestCaseGroup> graphicGroup(
        new deqp::TestCaseGroup(testCtx, "graphics", "Subgroup builtin mask category    tests: graphics"));
    de::MovePtr<deqp::TestCaseGroup> computeGroup(
        new deqp::TestCaseGroup(testCtx, "compute", "Subgroup builtin mask category tests: compute"));
    de::MovePtr<deqp::TestCaseGroup> framebufferGroup(
        new deqp::TestCaseGroup(testCtx, "framebuffer", "Subgroup builtin mask category tests: framebuffer"));
    ...
    for (int a = 0; a < DE_LENGTH_OF_ARRAY(all_stages_vars); ++a)
    {
...

        {
            ...
            SubgroupFactory<CaseDefinition>::addFunctionCaseWithPrograms(computeGroup.get(), varLower, "",
                                                                         supportedCheck, initPrograms, test, caseDef);
        }
    ...
    de::MovePtr<deqp::TestCaseGroup> group(
        new deqp::TestCaseGroup(testCtx, "builtin_mask_var", "Subgroup builtin mask variable tests"));
