...
static bool checkFragmentPipelineStages(std::vector<const void *> datas, uint32_t width, uint32_t height, uint32_t)
{
...
{
    const bool formatIsBoolean = FORMAT_R32_BOOL == caseDef.format || FORMAT_R32G32_BOOL == caseDef.format ||
                                 FORMAT_R32G32B32_BOOL == caseDef.format || FORMAT_R32G32B32A32_BOOL == caseDef.format;
    ...
    const string source =
        ...
        (OPTYPE_ALLEQUAL == caseDef.opType) ?
            "  " + subgroups::getFormatNameForGLSL(caseDef.format) +
                " valueEqual = " + subgroups::getFormatNameForGLSL(caseDef.format) +
                "(1.25 * float(data[gl_SubgroupInvocationID]) + 5.0);\n" + "  " +
                subgroups::getFormatNameForGLSL(caseDef.format) +
                " valueNoEqual = " + subgroups::getFormatNameForGLSL(caseDef.format) +
                (formatIsBoolean ?
                     "(subgroupElect())\n;" :
                     "(12.0 * float(data[gl_SubgroupInvocationID]) + float(gl_SubgroupInvocationID));\n") +
                "  result = " + getOpTypeName(caseDef.opType) + "(" + subgroups::getFormatNameForGLSL(caseDef.format) +
                "(1)) ? 0x1u : 0u;\n"
                "  result |= " +
                getOpTypeName(caseDef.opType) +
    ...
    else if (SHADER_STAGE_TESS_CONTROL_BIT == caseDef.shaderStage)
    {
        ...
        controlSource << "${VERSION_DECL}\n"
                      ...
                      << "  }\n"
                      << source << "  out_color[gl_InvocationID] = float(result);"
    ...
    else if (SHADER_STAGE_FRAGMENT_BIT == caseDef.shaderStage)
    {
        const string sourceFragment =
            (OPTYPE_ALL == caseDef.opType) ? "  result |= " + getOpTypeName(caseDef.opType) +
                                                 "(!gl_HelperInvocation) ? 0x0u : 0x1u;\n"
                                                 "  result |= " +
                                                 getOpTypeName(caseDef.opType) +
                                                 "(false) ? 0u : 0x1Au;\n"
                                                 "  result |= 0x4u;\n" :
            (OPTYPE_ANY == caseDef.opType) ? "  result |= " + getOpTypeName(caseDef.opType) +
            ...
            (OPTYPE_ALLEQUAL == caseDef.opType) ?
                                             "  " + subgroups::getFormatNameForGLSL(caseDef.format) +
                                                 ...
                                                 (formatIsBoolean ?
                                                      "(subgroupElect());\n" :
                                                      "(12.0 * float(data[gl_SubgroupInvocationID]) + gl_FragCoord.x * "
                                                      "float(gl_SubgroupInvocationID));\n") +
                                                 "  result |= " + getOpTypeName(caseDef.opType) + "(" +
                                                 subgroups::getFormatNameForGLSL(caseDef.format) +
                                                 "(1)) ? 0x10u : 0u;\n"
                                                 "  result |= " +
        ...
        fragmentSource << "${VERSION_DECL}\n"
                       ...
                       << "{\n"
                       << "  uint result = 0u;\n"
                       << "  if (dFdx(float(gl_SubgroupInvocationID) * gl_FragCoord.x * gl_FragCoord.y) - "
                          "dFdy(float(gl_SubgroupInvocationID) * gl_FragCoord.x * gl_FragCoord.y) > 0.0f)\n"
                       << "  {\n"
...
void initPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    if (SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        {
            src << "  result[offset] = " << getOpTypeName(caseDef.opType) << "(true) ? 0x1u : 0u;\n"
        ...
        {
            src << "  " << subgroups::getFormatNameForGLSL(caseDef.format)
                << " valueEqual = " << subgroups::getFormatNameForGLSL(caseDef.format)
                << "(1.25 * float(data[gl_SubgroupInvocationID]) + 5.0);\n"
                << "  " << subgroups::getFormatNameForGLSL(caseDef.format)
                << " valueNoEqual = " << subgroups::getFormatNameForGLSL(caseDef.format)
...
tcu::TestStatus noSSBOtest(Context &context, const CaseDefinition caseDef)
{
    ...
    inputData.initializeType = OPTYPE_ALLEQUAL == caseDef.opType ? subgroups::SSBOData::InitializeZero :
    ...
    if (SHADER_STAGE_VERTEX_BIT == caseDef.shaderStage)
        ...
        return subgroups::makeFragmentFrameBufferTest(context, FORMAT_R32_UINT, &inputData, 1,
...
deqp::TestCaseGroup *createSubgroupsVoteTests(deqp::Context &testCtx)
{
        ...
        for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
        {
...

            {
                ...
                SubgroupFactory<CaseDefinition>::addFunctionCaseWithPrograms(
                    computeGroup.get(), op + "_" + subgroups::getFormatNameForGLSL(format), "", supportedCheck,
...

            {
                ...
                SubgroupFactory<CaseDefinition>::addFunctionCaseWithPrograms(
                    graphicGroup.get(), op + "_" + subgroups::getFormatNameForGLSL(format), "", supportedCheck,
                    initPrograms, test, caseDef);
            }
    ...
    de::MovePtr<deqp::TestCaseGroup> group(new deqp::TestCaseGroup(testCtx, "vote", "Subgroup vote category tests"));
