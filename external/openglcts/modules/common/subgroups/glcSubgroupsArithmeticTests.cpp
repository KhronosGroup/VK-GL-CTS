...
void initFrameBufferPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    bdy << indexVars << "  " << subgroups::getFormatNameForGLSL(caseDef.format)
        ...
        << "  }\n"
        << "  tempResult = "
        << getCompare(caseDef.opType, caseDef.format, "ref",
...
void initPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    const string bdy = indexVars + "  " + subgroups::getFormatNameForGLSL(caseDef.format) +
                       " ref = " + getIdentity(caseDef.opType, caseDef.format) +
                       ";\n"
                       "  uint tempResult = 0u;\n"
                       "  for (uint index = start; index < end; index++)\n"
                       ...
                       "    {\n"
                       "      ref = " +
                       getOpTypeOperation(caseDef.opType, caseDef.format, "ref", "data[index]") +
                       ";\n"
                       "    }\n"
                       "  }\n"
                       "  tempResult = " +
...
deqp::TestCaseGroup *createSubgroupsArithmeticTests(deqp::Context &testCtx)
{
        ...
        for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
        {
...

            {
                ...
                SubgroupFactory<CaseDefinition>::addFunctionCaseWithPrograms(
                    computeGroup.get(), de::toLower(op) + "_" + subgroups::getFormatNameForGLSL(format), "",
...

            {
                ...
                SubgroupFactory<CaseDefinition>::addFunctionCaseWithPrograms(
                    graphicGroup.get(), de::toLower(op) + "_" + subgroups::getFormatNameForGLSL(format), "",
                    supportedCheck, initPrograms, test, caseDef);
            }
            ...
            for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
            {
                ...
                SubgroupFactory<CaseDefinition>::addFunctionCaseWithPrograms(
                    ...
                    de::toLower(op) + "_" + subgroups::getFormatNameForGLSL(format) + "_" +
    ...
    de::MovePtr<deqp::TestCaseGroup> group(
        new deqp::TestCaseGroup(testCtx, "arithmetic", "Subgroup arithmetic category tests"));
