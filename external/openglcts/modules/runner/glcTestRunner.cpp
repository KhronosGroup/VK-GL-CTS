...
static void getTestRunsForNoContext(glu::ApiType type, vector<TestRunParams> &runs, const ConfigList &configs,
                                    const RunParams *runParams, const int numRunParams, const char *mustpassDir)
{
...
static void getTestRunsForSingleConfig(glu::ApiType type, vector<TestRunParams> &runs, const ConfigList &configs,
                                       const RunParams *runParams, const int numRunParams, const char *mustpassDir)
{
    vector<Config>::const_iterator cfgIter = configs.configs.begin();
...
static void writeRunSummary(const TestRunSummary &summary, const char *filename)
{
    ...
    de::UniquePtr<qpXmlWriter, XmlWriterDeleter> writer(qpXmlWriter_createFileWriter(out.get(), false, false));
    if (!writer)
        throw std::bad_alloc();
...
TestRunner::TestRunner(tcu::Platform &platform, tcu::Archive &archive, const char *waiverPath, const char *logDirPath,
...
void TestRunner::deinitSession(void)
{
    ...
    DE_ASSERT(result.numExecuted ==
              result.numPassed + result.numFailed + result.numNotSupported + result.numWarnings + result.numWaived);
