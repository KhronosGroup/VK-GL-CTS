...
public:
    ...
    ComputeShaderBase()
        : renderTarget(m_context.getRenderContext().getRenderTarget())
...
static decltype(glw::Functions::bufferStorage) getBufferStorageFunction(glu::RenderContext &renderContext)
{
...
class LongRunningPersistentSSBOComputeTest : public ComputeShaderBase
{
    ...
    long RunComputePersistent()
    {
        ...
        m_dataLoadStore =
            static_cast<int *>(glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, sizeof(int) * 2, buffer_flags));
        m_dataLoadStore[0] = 0;
...
class BasicResourceUniform : public ComputeShaderBase
{
    ...
    virtual long Run()
    {
        ...
        /* mat4x3 */
        {
            const GLfloat value[12] = {63.0f, 64.0f, 65.0f, 66.0f, 67.0f, 68.0f, 69.0f, 70.0f, 71.0f, 27.0f, 73, 74.0f};
...
class AdvancedPipelineGenDrawCommands : public ComputeShaderBase
{
    ...
    virtual long Run()
    {
        ...
        /* object buffer */
        {
            ...
            } data = {{tcu::translationMatrix(vec3(-1.5f, -0.5f, 0.0f)),
                       tcu::translationMatrix(vec3(0.5f, -0.5f, 0.0f)), tcu::translationMatrix(vec3(-0.5f, 0.5f, 0.0f)),
                       tcu::translationMatrix(vec3(0.5f, 0.5f, 0.0f))},
                      {4, 4, 4, 4},
                      {0, 4, 8, 12}};
...
class AdvancedPipelineComputeChain : public ComputeShaderBase
{
    ...
    virtual long Run()
    {
        ...
        /* validate storage buffer 4 */
        {
            ...
            data =
                static_cast<mat4 *>(glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, sizeof(mat4) * 2, GL_MAP_READ_BIT));
...
class AdvancedPipelinePostXFB : public ComputeShaderBase
{
    ...
    virtual long Run()
    {
        ...
        /* validate xfb buffer */
        {
            const float ref_data[3 * 8] = {-1, -1, 0, 1, 1, 1, 0, 1, 3, -1, 0, 1, 0, 1, 1, 1, -1, 3, 0, 1, 1, 0, 0, 1};
            float *data;
            data = static_cast<float *>(
...
class AdvancedSharedMax : public ComputeShaderBase
{
    ...
    virtual long Run()
    {
        ...
        /* validate buffer */
        {
            vec4 *data;
            data = static_cast<vec4 *>(
                glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, sizeof(vec4) * 1024, GL_MAP_READ_BIT));
...
class AdvancedResourcesMax : public ComputeShaderBase
{
    ...
    bool RunIteration(GLuint index)
    {
        ...
        /* validate buffer */
        {
            ...
            data =
                static_cast<GLuint *>(glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, sizeof(GLuint), GL_MAP_READ_BIT));
            if (data[0] != (index + 1) * 4)
            {
...
class NegativeAPIProgram : public ComputeShaderBase
{
    ...
    virtual long Run()
    {
        ...
        if (glGetError() != GL_INVALID_OPERATION)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message
                << "INVALID_OPERATION is generated by GetProgramiv if <pname> is\n"
...
void ComputeShaderTests::init()
{
    ...
    addChild(
        new TestSubcase(m_context, "simple-compute-shared_context", TestSubcase::Create<LongRunningComputeFenceTest>));
    addChild(new TestSubcase(m_context, "simple-compute-shared_context-persistent-buffer",
                             TestSubcase::Create<LongRunningPersistentSSBOComputeTest>));
