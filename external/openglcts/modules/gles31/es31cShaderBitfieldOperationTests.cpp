...
{
    return ((int(deFloatFloor(x * range + 0.5f)) & 0xFF) << 0) | ((int(deFloatFloor(y * range + 0.5f)) & 0xFF) << 8) |
           ((int(deFloatFloor(z * range + 0.5f)) & 0xFF) << 16) | ((int(deFloatFloor(w * range + 0.5f)) & 0xFF) << 24);
}
...
static bool checkOutData(GLuint result, const GLfloat input[4], float range)
{
    ...
    GLuint expected_mp = pack(reduce32PrecisionTo16(input[0]), reduce32PrecisionTo16(input[1]),
                              reduce32PrecisionTo16(input[2]), reduce32PrecisionTo16(input[3]), range);
...
ShaderBitfieldOperationTests::ShaderBitfieldOperationTests(Context &context, glu::GLSLVersion glslVersion)
    : TestCaseGroup(context, "shader_bitfield_operation", "Shader Bitfield Operation tests")
    , m_glslVersion(glslVersion)
{
}
