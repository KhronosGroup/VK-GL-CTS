...
class BasicAtomicCase4VSFS : public ShaderStorageBufferObjectBase
{
    ...
    virtual long Run()
    {
        if (!IsVSFSAvailable(2, 2))
            return NOT_SUPPORTED;
        const char *const glsl_vs = NL
            "layout(location = 0) in vec4 g_in_position;" NL "layout(std430, binding = 0) coherent buffer Counters {" NL
            "  uint g_uint_counter;" NL "  int g_int_counter;" NL "};" NL
            "layout(std430, binding = 1) buffer OutputU {" NL "  uint udata[8];" NL "} g_outputU;" NL
            "layout(std430, binding = 2) buffer OutputI {" NL "  int idata[8];" NL "} g_outputI;" NL "void main() {" NL
            "  gl_Position = g_in_position;" NL "#ifdef GL_ES" NL "  gl_PointSize = 1.0f;" NL "#endif" NL
        ...
        const char *const glsl_fs =
            NL "layout(location = 0) out vec4 g_fs_out;" NL "layout(std430, binding = 0) coherent buffer Counters {" NL
               ...
               "void main() {" NL "  g_fs_out = vec4(0, 1, 0, 1);" NL "  uint uidx = atomicAdd(g_uint_counter, 1u);" NL
               "  int iidx = atomicAdd(g_int_counter, -1);" NL "  g_outputU.udata[uidx] = uidx;" NL
               "  g_outputI.idata[iidx] = iidx;" NL "}";
        m_program = CreateProgram(glsl_vs, glsl_fs);
        glLinkProgram(m_program);
        if (!CheckProgram(m_program))
            return ERROR;
...
class AdvancedSwitchBuffersVS : public ShaderStorageBufferObjectBase
{
    ...
    virtual long Run()
    {
        ...
        glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_SHADER_STORAGE_BUFFER, 0, 2 * offset, sizeof(float) * 32);
        glBindBuffer(GL_COPY_READ_BUFFER, m_storage_buffer[3]);
        glCopyBufferSubData(GL_COPY_READ_BUFFER, GL_SHADER_STORAGE_BUFFER, 0, 3 * offset, sizeof(float) * 32);
        ...
        for (int i = 0; i < 4; ++i)
        {
            glBindBufferRange(GL_SHADER_STORAGE_BUFFER, 0, m_storage_buffer[4], i * offset, sizeof(float) * 32);
...
class AdvancedUsageSyncCS : public ShaderStorageBufferObjectBase
{
    ...
    virtual long Run()
    {
        ...
        /* Buffer1 */
        {
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "[Buffer1] Data at index 0 is " << data[0] << " should be "
                    << ref_data[0] << tcu::TestLog::EndMessage;
                return ERROR;
            }
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "[Buffer1] Data at index 1 is " << data[1] << " should be at least"
                    << ref_data[1] << tcu::TestLog::EndMessage;
                return ERROR;
            }
            glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
        }
        /* Buffer2 */
        {
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "[Buffer2] Data at index 0 is " << data[0] << " should be "
                    << ref_data[0] << tcu::TestLog::EndMessage;
                return ERROR;
            }
            /* data incremented in VS which can be invoked multiple times per vertex */
            ...
            {
                m_context.getTestContext().getLog()
                    << tcu::TestLog::Message << "[Buffer2] Data at index 1 is " << data[1] << " should be at least"
                    << ref_data[1] << tcu::TestLog::EndMessage;
                return ERROR;
            }
            glUnmapBuffer(GL_SHADER_STORAGE_BUFFER);
