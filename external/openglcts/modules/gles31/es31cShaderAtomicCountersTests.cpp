        ...
        for (int y = 0; y < viewportH; ++y)
        {
            for (int x = 0; x < viewportW; ++x)
            {
                referenceFrame.setPixel(
                    x, y,
                    tcu::RGBA(static_cast<int>(expected[0] * 255), static_cast<int>(expected[1] * 255),
...
public:
    ...
    long Verify()
    {
        ...
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message << "BUFFER_MAP_POINTER is "
                << reinterpret_cast<uintptr_t>(static_cast<int *>(ptr)) << " should be "
                << reinterpret_cast<uintptr_t>(static_cast<int *>(map_pointer_)) << tcu::TestLog::EndMessage;
            return ERROR;
        }
        return NO_ERROR;
    }
        ...
        for (int x = 0; x < dim_x_; x++)
            for (int y = 0; y < dim_y_; y++)
                for (int z = 0; z < dim_z_; z++)
                    if (!CheckFinalCounterValue(counter_buffer_,
                                                (x * dim_y_ * dim_z_ + y * dim_z_ + z) * sizeof(GLuint), 1064))
                        error = ERROR;
...
class AdvancedUsageMultiDimArrayLarge : public AdvancedUsageMultiDimArray
{
    virtual long Setup()
    {
        ...
        glsl_cs_ =
            NL "layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;" NL
               ...
               "void incrementCounterArray1D(in atomic_uint arr[6]) {" NL "  for (int i = 1; i < 5; i++)" NL
               "    atomicCounterIncrement(arr[i]);" NL "  atomicCounterIncrement(arr[0]);" NL
               "  atomicCounterIncrement(arr[5]);" NL "}" NL
               "// Increments all the elements in a two dimensional counter array using different methods." NL
               "void incrementCounterArray2D(in atomic_uint arr[5][6]) {" NL "  incrementCounterArray1D(arr[1]);" NL
               "  for (int i = 2; i < 5; i++) {" NL "    for (int j = 0; j < 5; j++) {" NL
               "      atomicCounterIncrement(arr[i][j]);" NL "    }" NL "    atomicCounterIncrement(arr[i][5]);" NL
               "  }" NL "  for (int i = 0; i < 4; i++)" NL "     atomicCounterIncrement(arr[0][i]);" NL
               "  atomicCounterIncrement(arr[0][4]);" NL "  atomicCounterIncrement(arr[0][5]);" NL "}" NL
               "// Increments all the atomic counters once." NL "void main() {" NL "  for (int i = 0; i < 2; i++)" NL
               "    incrementCounterArray2D(ac_counter[i]);" NL "  for (int i = 0; i < 5; i++)" NL
    ...
    virtual long Setup()
    {
        ...
        glsl_cs_ =
            NL "layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;" NL
               ...
               "// Increments all the elements in a one dimensional array using different methods." NL
               "void incrementCounterArray1D(in atomic_uint arr[3]) {" NL "  for (int i = 1; i < 3; i++)" NL
               "    atomicCounterIncrement(arr[i]);" NL "  atomicCounterIncrement(arr[0]);" NL "}" NL
               "// Increments all the elements in a two dimensional counter array using different methods." NL
               "void incrementCounterArray2D(in atomic_uint arr[5][3]) {" NL "  incrementCounterArray1D(arr[1]);" NL
               "  for (int i = 2; i < 5; i++) {" NL "    for (int j = 0; j < 2; j++) {" NL
               "      atomicCounterIncrement(arr[i][j]);" NL "    }" NL "    atomicCounterIncrement(arr[i][2]);" NL
               "  }" NL "  for (int i = 0; i < 2; i++)" NL "     atomicCounterIncrement(arr[0][i]);" NL
               "  atomicCounterIncrement(arr[0][2]);" NL "}" NL "// Increments all the atomic counters once." NL
               "void main() {" NL "  for (int i = 0; i < 2; i++)" NL "    incrementCounterArray2D(ac_counter[i]);" NL
               "  for (int i = 0; i < 5; i++)" NL "    incrementCounterArray1D(ac_counter[2][i]);" NL "}";
    ...
    virtual long Setup()
    {
        ...
        glsl_cs_ =
            NL "layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;" NL
               ...
               "void incrementCounterArray1D(in atomic_uint arr[2]) {" NL "  atomicCounterIncrement(arr[0]);" NL
               "  atomicCounterIncrement(arr[1]);" NL "}" NL
               "// Increments all the elements in a two dimensional counter array using different methods." NL
               "void incrementCounterArray2D(in atomic_uint arr[2][2]) {" NL "  incrementCounterArray1D(arr[1]);" NL
               "  for (int i = 0; i < 2; i++) {" NL "    atomicCounterIncrement(arr[0][i]);" NL "  }" NL "}" NL
               "// Increments all the atomic counters once." NL "void main() {" NL
               "  incrementCounterArray2D(ac_counter[0]);" NL "  for (int i = 0; i < 2; i++)" NL
...
class AdvancedUsageSwitchPrograms : public SACSubcaseBase
{
    ...
    std::string GenFSSrc(int binding, int offset)
    {
        ...
        os << "#version 310 es" NL "layout(location = 0) out uvec4 o_color;" NL "layout(binding = " << binding
           << ", offset = " << offset
           << ") uniform atomic_uint ac_counter_fs;" NL "void main() {" NL
...
class AdvancedUsageUBO : public BasicUsageCS
{
    ...
    virtual long Run()
    {
        ...
        glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_ssbo);
        data =
            static_cast<UVec4 *>(glMapBufferRange(GL_SHADER_STORAGE_BUFFER, 0, 256 * sizeof(UVec4), GL_MAP_READ_BIT));
...
class NegativeAPI : public SACSubcaseBase
{
    ...
    virtual long Run()
    {
        ...
        if (glGetError() != GL_INVALID_VALUE)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message
                << "glBindBufferBase should generate INVALID_VALUE when"
        ...
        if (glGetError() != GL_INVALID_VALUE)
        {
            m_context.getTestContext().getLog()
                << tcu::TestLog::Message
                << "glBindBufferRange should generate INVALID_VALUE when"
                   " index is greater than or equal GL_MAX_ATOMIC_COUNTER_BUFFER_BINDINGS."
                << tcu::TestLog::EndMessage;
            error = ERROR;
        }
...
class NegativeLargeOffset : public SACSubcaseBase
{
    ...
    virtual long Run()
    {
        ...
        std::ostringstream os;
        os << "#version 310 es" NL "layout(location = 0) out uvec4 o_color;" NL "layout(binding = 0, offset = "
           << maxSize
           << ") uniform atomic_uint ac_counter_fs;" NL "void main() {" NL
              "  o_color = uvec4(atomicCounterIncrement(ac_counter_fs)); " NL " }";
...
void ShaderAtomicCountersTests::init()
{
    ...
    addChild(new TestSubcase(m_context, "advanced-usage-multidim-array-small",
                             TestSubcase::Create<AdvancedUsageMultiDimArraySmall>));
    addChild(new TestSubcase(m_context, "advanced-usage-multidim-array-medium",
                             TestSubcase::Create<AdvancedUsageMultiDimArrayMedium>));
    addChild(new TestSubcase(m_context, "advanced-usage-multidim-array-large",
                             TestSubcase::Create<AdvancedUsageMultiDimArrayLarge>));
    addChild(
