...
void DrawIndirectBase::ReadPixelsFloat<test_api::ES3>(int x, int y, int width, int height, void *data)
{
    ...
    GLenum type = ((pixelFormat.redBits == 10) && (pixelFormat.greenBits == 10) && (pixelFormat.blueBits == 10) &&
...
public:
    ...
    long TemplatedRun()
    {
        ...
        glBufferData(GL_ARRAY_BUFFER,
                     (GLsizeiptr)(coords.size() * sizeof(coords[0]) +
...
public:
    ...
    virtual long Run()
    {
        ...
        ReadPixelsFloat<api>(0, (getWindowHeight() + 3) / 4, getWindowWidth(), 2 * (getWindowHeight() / 4),
                             &bufferTest2[0]);
...
struct CNegativeUnalignedOffset : public DrawIndirectBase
{
    ...
    virtual long Run()
    {
        ...
        {
            glDrawElementsIndirect(GL_TRIANGLES, GL_UNSIGNED_INT, reinterpret_cast<void *>((uintptr_t)offsets[i]));
            if (glGetError() != GL_INVALID_VALUE)
            {
                result.error() << "Invalid error code returned by a driver for GL_INVALID_VALUE type";
            }
            glDrawArraysIndirect(GL_TRIANGLES, reinterpret_cast<void *>((uintptr_t)offsets[i]));
            if (glGetError() != GL_INVALID_VALUE)
            {
                result.error() << "Invalid error code returned by a driver for GL_INVALID_VALUE type";
            }
...
void DrawIndirectTestsES31::init()
{
    ...
    addChild(new TestSubcase(m_context, "basic-binding-multi", TestSubcase::Create<CMultiBindingPoint<test_api::ES3>>));
    addChild(
    ...
    addChild(
        new TestSubcase(m_context, "basic-buffer-getPointerv", TestSubcase::Create<CBufferGetPointerv<test_api::ES3>>));
    ...
    addChild(
        new TestSubcase(m_context, "basic-buffer-copySubData", TestSubcase::Create<CBufferCopySubData<test_api::ES3>>));
