...
def parseExtensions (src, versions, allFunctions, allCompositeTypes, allEnums, allBitfields, allHandles, allDefinitions):
	...
	def getExtensionDataDict():
		...
		extensionsDataFile	= readFile(os.path.join(SCRIPTS_SRC_DIR, "extensions_data.txt"))
		...
		for item in extensionList:
			versionList = []
			if len(item[2]):
				versionList = [int(number) for number in item[2].split('_')[:4]]
...
def writeSupportedExtensions(apiName, api, filename):
	...
	if apiName=='SC':
		extensionsData			= readFile(os.path.join(SCRIPTS_SRC_DIR, "extensions_data.txt"))
...
def writeMandatoryFeatures(api, filename):
	...
	mandatoryFeatures = readFile(os.path.join(SCRIPTS_SRC_DIR, "mandatory_features.txt"))
