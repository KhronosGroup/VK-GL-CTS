...
def genExtDepArray(extDepsName, extDepsDict):
    yield 'static const std::tuple<uint32_t, uint32_t, const char*, const char*>\t{}[]\t='.format(extDepsName)
    yield '{'
...
def genApiVersions(name, apiVersions):
    yield 'static const std::tuple<uint32_t, uint32_t, uint32_t>\t{}[]\t='.format(name)
    yield '{'
