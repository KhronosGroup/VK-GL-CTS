...
{
    {
        ...
        Except("StoreContentResponse::status", response.status, true,
               "After requesting to store data on a server we should received true");
    }

    {
        ...
        Except("StoreContentResponse::status", response.status, true,
               "After requesting to store data with a name that is already in use we should received false");
    }
}
...
{
    {
        ...
        Except("StoreContentResponse::status", response.status, true,
               "After requesting to get data from server store we should received true");
...

    {
        ...
        Except("StoreContentResponse::status", response.status, false,
               "Requesting to get data from server memory that no longer exist should result in false");
    }
}
...
{
    {
        ...
        Except("StoreContentResponse::status", response.status, true,
               "After requesting server to compile glsl shader we should get true as a result");
        Except("StoreContentResponse::binary.empty()", response.binary.empty(), false,
               "Received data must be not empty");
    }
}
