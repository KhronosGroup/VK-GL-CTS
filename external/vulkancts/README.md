...
Following command line options MUST be used when running CTS:
	...
	--deqp-caselist-file=<vulkancts>/external/vulkancts/mustpass/1.0.3/vk-default.txt
...
additional `PRODUCT:` lines to the conformance statement. For example:
	...
	CONFORM_VERSION:         vulkan-cts-1.0.3.0
	PRODUCT:                 Product A
	PRODUCT:                 Product B
	...
...
and path to current mustpass list. For example:
	...
	python external/vulkancts/scripts/verify_submission.py VK_10_Khronos_1/ external/vulkancts/mustpass/1.0.3/vk-default.txt
