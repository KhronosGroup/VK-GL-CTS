...
public:
	...
	// ISlangUnknown
	SLANG_NO_THROW SlangResult SLANG_MCALL queryInterface(SlangUUID const& guid,
		void** outObject) {
		return SLANG_OK;
	};
...
typedef void(CALLBACK *PFNSPSETDIAGONSTICCB)(SlangCompileRequest *,
...
public:
	...
	SlangResult read(void* buffer, size_t length, size_t& outReadBytes) {
		...
		if (m_isPipe) {
			...
			SLANG_RETURN_ON_FAIL(_updateState(
				PeekNamedPipe(m_streamHandle, nullptr, DWORD(0), &pipeBytesRead,
			...
			SLANG_RETURN_ON_FAIL(_updateState(ReadFile(
				m_streamHandle, buffer, DWORD(length), &bytesRead, nullptr)));
		}
		else {
			SLANG_RETURN_ON_FAIL(_updateState(ReadFile(
				m_streamHandle, buffer, DWORD(length), &bytesRead, nullptr)));
	...
	SlangResult write(const void* buffer, size_t length) {
		...
		DWORD numWritten = 0;
		BOOL writeResult =
			WriteFile(m_streamHandle, buffer, DWORD(length), &numWritten, nullptr);
	...
	SlangResult flush() {
		if ((Index(m_access) & Index(FileAccess::Write)) == 0 ||
			m_streamHandle.isNull()) {
			return SLANG_E_NOT_AVAILABLE;
		}
	...
	WinPipeStream(HANDLE handle, FileAccess access, bool isOwned = true) {
		...
		if (m_isPipe) {
			...
			if (GetNamedPipeInfo(handle, &flags, &outBufferSize, &inBufferSize,
				&maxInstances)) {
			}
		}
	}
...
public:
	// Process
	bool isTerminated() { return waitForTermination(0); }
	bool waitForTermination(int timeInMs) {
		...
		SLANG_LOG(std::cout << "#1 waitForTermination: start terminating process"
			<< m_processHandle << "\n";)
		...
		if (res == WAIT_TIMEOUT) {
			SLANG_LOG(
				std::cout << "#2 waitForTermination: Process FAILED TO terminated"
				<< m_processHandle << "\n";)
				return false;
		}
		SLANG_LOG(std::cout << "#2 waitForTermination: Process terminated"
			<< m_processHandle << "\n";)
			_hasTerminated();
		return true;
	}
...
protected:
	...
	HANDLE m_streams[Index(
		StdStreamType::CountOf)]; ///< Streams to communicate with the process
};
...
public:
	...
	int SetupSlangDLL() {
		if (!handle) {
			char lpBuffer[128];
			DWORD ret =
				GetEnvironmentVariable("SLANG_DLL_PATH_OVERRIDE", lpBuffer, 128);
			if (ret > 0) {
	...
	void getSlangFunctionHandles() {
		...
		m_sfn.pfnspProcessCommandLineArguments =
			(PFNSPPROCESSCOMMANDLINEARG)GetProcAddress(
				handle, "spProcessCommandLineArguments");
	}
	...
	SlangResult CreateProcess(std::string& exename, std::string& cmdline,
		DWORD flags, WinProcess*& outProcess) {
		...
		WinHandle processHandle;
		{
...

			{
				...
				SLANG_RETURN_FAIL_ON_FALSE(CreatePipe(childStdOutReadTmp.writeRef(),
				...
				if ((flags & Process::Flag::DisableStdErrRedirection) == 0) {
					...
					SLANG_RETURN_FAIL_ON_FALSE(CreatePipe(
						childStdErrReadTmp.writeRef(), childStdErrWrite.writeRef(),
						&securityAttributes, bufferSize));
				}
				// create stdin pipe for child process
				SLANG_RETURN_FAIL_ON_FALSE(CreatePipe(childStdInRead.writeRef(),
				...
				SLANG_RETURN_FAIL_ON_FALSE(DuplicateHandle(
					currentProcess, childStdOutReadTmp, currentProcess,
					childStdOutRead.writeRef(), 0, FALSE, DUPLICATE_SAME_ACCESS));
				// create a non-inheritable duplicate of the stderr reader
				if (childStdErrReadTmp)
					SLANG_RETURN_FAIL_ON_FALSE(DuplicateHandle(
						currentProcess, childStdErrReadTmp, currentProcess,
						childStdErrRead.writeRef(), 0, FALSE, DUPLICATE_SAME_ACCESS));
				// create a non-inheritable duplicate of the stdin writer
				SLANG_RETURN_FAIL_ON_FALSE(DuplicateHandle(
					currentProcess, childStdInWriteTmp, currentProcess,
					childStdInWrite.writeRef(), 0, FALSE, DUPLICATE_SAME_ACCESS));
			}
			...
			BOOL success = CreateProcessW(
				lppath, (LPWSTR)lpcmdline, nullptr, nullptr, true, createFlags,
		...
		streams[Index(StdStreamType::Out)] =
			childStdOutRead.detach(); // new WinPipeStream(childStdOutRead.detach(),
		// FileAccess::Read);
		streams[Index(StdStreamType::In)] =
			childStdInWrite.detach(); // new WinPipeStream(childStdInWrite.detach(),
		// FileAccess::Write);
	...
	void createJSONCompileCommand(std::ostringstream& jsonCmd,
		std::string& filename, std::string& stage) {
		jsonCmd << "{\n"
			<< "    \"jsonrpc\" : \"2.0\", \n"
			<< "    \"method\" : \"tool\", \n"
			<< "    \"params\" : \n"
	...
	SlangResult sendCommand(std::string& filename, std::string& stage) {
		...
		commandSize =
			"Content-Length: " + std::to_string(jsonCmd.str().size()) + "\r\n\r\n";
		if (!getMutexInfinite(false)) {
			SLANG_LOG(std::cout << "#5: Failed to acquire mutex\n";)
				return SLANG_FAIL;
		}
		if (!m_writeStream) {
			SLANG_LOG(std::cout << "read stream is NULL which means test-server has "
				"closed unexpectedly\n";)
				releaseMutex();
			return SLANG_FAIL;
		}
...
#define HEADER_BUFF_MAX_SIZE 1024
	SlangResult readResult(std::string& output) {
		...
		while (state != ReadState::Done && timeoutCount <= MAX_TIMEOUT_ITER_COUNT) {
			...
			if (!m_readStream) {
				SLANG_LOG(std::cout << "read stream is NULL which means test-server "
			...
			case ReadState::Header: {
				...
				// Read the header
				SlangResult readres =
					m_readStream->read(content, HEADER_BUFF_MAX_SIZE, contentSize);
				releaseMutex();
				if (readres != SLANG_OK) {
				...
				if (contentSize > 0) {
					...
					std::copy(content, content + HEADER_BUFF_MAX_SIZE,
						std::back_inserter(contentstr));
					...
					bufferSize =
						std::stoi(contentstr.substr(pos_start, pos_end - pos_start));
			...
			case ReadState::Content: {
				if (bufferSize - alreadyReadBuffer > 0) {
					...
					SlangResult res =
						m_readStream->read(readBuff, tobeRead, readStreamSize);
					releaseMutex();
					if (res != SLANG_OK) {
						state = ReadState::Error;
						break;
		...
		if (timeoutCount >= MAX_TIMEOUT_ITER_COUNT) {
			...
			SLANG_LOG(std::cout << "waiting for spwaned thread to be killed:"
				<< hProcessMgmtThread << " threadID:" << dwThreadId
				<< "\n";)
				DWORD wres = WaitForSingleObject(hProcessMgmtThread,
					threadDiedWaitMS); // wait for the thread
			SLANG_LOG(std::cout << "Waited for thread id" << dwThreadId
				<< "singleobject wait result =" << wres << "\n";)
	...
	SlangResult spawinAndWaitTestServer() {
		...
		result = CreateProcess(exename, cmdline,
			Process::Flag::DisableStdErrRedirection, m_process);
		if (result != SLANG_OK) {
			SLANG_LOG(std::cout << "Failed to laucnh the test-server\n";)
				return SLANG_FAIL;
		}
		m_readStream = new WinPipeStream(m_process->getStream(StdStreamType::Out),
			FileAccess::Read);
		if (m_process->getStream(StdStreamType::ErrorOut))
			m_readErrStream = new WinPipeStream(
				m_process->getStream(StdStreamType::ErrorOut), FileAccess::Read);
		m_writeStream = new WinPipeStream(m_process->getStream(StdStreamType::In),
			FileAccess::Write);
	...
	SlangResult parseSpirvAsm(std::string& output, std::vector<uint32_t>* dst) {
		...
		std::string spvasm =
			output.substr(spirv_start_pos, spirv_end_pos - spirv_start_pos);
	...
	int setupSlangLikeSlangc(const std::vector<std::string>* sources,
		...
		glu::ShaderProgramInfo* buildInfo)
	{
		...
		DWORD ret =
			GetEnvironmentVariable("DISABLE_CTS_SLANG_SERVER_MODE", lpBuffer, 128);
		...
		do {
			...
			if (!enableServerMode) {
				...
				if (result != SLANG_OK) {
					SLANG_LOG(std::cout << "Failed to create CompileRequest: " << std::hex
						<< result << "\n";)
						break;
				}
			}
			for (int shaderType = 0; shaderType < glu::SHADERTYPE_LAST;
				shaderType++) {
				if (!sources[shaderType].empty()) {
					const std::string& srcText = Slang::getShaderStageSource(
						sources, buildOptions, (glu::ShaderType)shaderType);
					glu::ShaderType shaderstage = glu::ShaderType(shaderType);
					std::string slangShaderStage =
						findSlangShaderStage((glu::ShaderType)shaderType);
					...
					if (enableServerMode) {
						...
						if (result != SLANG_OK) {
							SLANG_LOG(std::cout << "Failed to spawn test sever: " << std::hex
								<< result << "\n";)
								break;
						}
						result = sendCommand(temp_fname, slangShaderStage);
						if (result != SLANG_OK) {
							SLANG_LOG(std::cout << "Failed to send command to test sever: "
								<< std::hex << result << "\n";)
								break;
						}
						...
						if (result != SLANG_OK) {
							SLANG_LOG(std::cout << "Failed to read results from test sever: "
								<< std::hex << result << "\n";)
								break;
						}
						...
						if (result != SLANG_OK) {
							SLANG_LOG(std::cout << "Failed to generate SPIRV ouptput from "
								"test-server results: "
								<< std::hex << result << "\n";)
								break;
						}
						...
						compileRequest->setDiagnosticCallback(&_diagnosticCallback,
							nullptr);
						compileRequest->setCommandLineCompilerMode();
						const char* args[] = {
							"-target",          "spirv",
							"-stage",           slangShaderStage.c_str(),
							"-entry",           "main",
							"-allow-glsl",      "-matrix-layout-row-major",
							temp_fname.c_str(), "-o",
							"temp.spv" };
						int argCount = sizeof(args) / sizeof(char*); // 8;
						result =
							compileRequest->processCommandLineArguments(args, argCount);
						if (result != SLANG_OK) {
							SLANG_LOG(std::cout << "Failed to proces command line arguments: "
								<< std::hex << result << "\n";)
								break;
						}
						...
						shaderBuildInfo.infoLog =
							""; // shader.getInfoLog(); // \todo [2015-07-13 pyry] Include
						// debug log?
						shaderBuildInfo.compileTimeUs =
							deGetMicroseconds() - compileStartTime;
						...
						buildInfo->program.infoLog =
							""; // glslangProgram.getInfoLog(); // \todo [2015-11-05 scygan]
						// Include debug log?
						buildInfo->program.linkOk = true;
	...
	int setupSlang(const std::vector<std::string>* sources,
		const ShaderBuildOptions& buildOptions,
		const ShaderLanguage shaderLanguage,
		std::vector<uint32_t>* dst,
		glu::ShaderProgramInfo* buildInfo)
	{
		...
		for (int shaderType = 0; shaderType < glu::SHADERTYPE_LAST; shaderType++) {
			if (!sources[shaderType].empty()) {
				...
				if (result != SLANG_OK) {
					SLANG_LOG(std::cout << "Failed to create local session: " << std::hex
						<< result << "\n";)
						// return SLANG_FAIL;
						break;
				}
				...
				if (result != SLANG_OK) {
					SLANG_LOG(std::cout << "Failed to find the entry point: " << std::hex
						<< result << "\n";)
						// return SLANG_FAIL;
						// break;
				}
				...
				if (result == SLANG_OK) {
					...
					SlangResult result = session->createCompositeComponentType(
						&componentTypes[0], 2, composedProgram.writeRef(),
						diagnosticsBlob.writeRef());
					if (result != SLANG_OK) {
						SLANG_LOG(std::cout << "Failed to create composite component type: "
							<< std::hex << result << "\n";)
				...
				if (buildInfo->shaders[0].compileOk) {
					...
					{
						ComPtr<slang::IBlob> diagnosticsBlob;
						result = composedProgram->link(linkedProgram.writeRef(),
							diagnosticsBlob.writeRef());
						if (result != SLANG_OK) {
				...
				if (buildInfo->program.linkOk) {
					...
					ComPtr<slang::IBlob> spirvCode;
					{
						...
						if (result != SLANG_OK) {
							SLANG_LOG(std::cout << "Failed to generate SPIRV Code: "
								<< std::hex << result << "\n";)
								diagnoseIfNeeded(diagnosticsBlob);
							return SLANG_FAIL;
						}
...
inline bool getMutexInfinite(bool sleepThread, int timeout_thresh) {
	...
	while (true) { //&& timeout >= 0
		...
		if (dwWaitResult == WAIT_ABANDONED || dwWaitResult == WAIT_FAILED) {
			SLANG_LOG(std::cout << "getMutexInfinite: dWaitResult:" << dwWaitResult
				<< std::endl;)
				return false;
		}
		if (sleepThread)
...
inline DWORD getMutexState() {
	// int timeout = 50;
	SLANG_LOG(std::cout << "Inside infinited mutex thread id:"
		<< GetCurrentThreadId() << "for mutex" << ghMutex
		<< "\n";)
		assert(ghMutex);
...
DWORD WINAPI spawinAndWaitTestServerThread(LPVOID lpParam) {
	...
	do {
		...
		SLANG_LOG(
			std::cout << "spawinAndWaitTestServerThread: #1 thread is active\n";);
		if (!getMutexInfinite(false)) {
			SLANG_LOG(
				std::cout
				<< "#1 spawinAndWaitTestServerThread: Failed to acquire mutex\n";)
				tstate = ethread_state_exit;
			return -1;
		}
		SLANG_LOG(std::cout << "#2 spawinAndWaitTestServerThread: Thread launching "
			"test-server \n";)
			if (g_slangContext.spawinAndWaitTestServer() == SLANG_OK) {
				SLANG_LOG(std::cout << "#3 spawinAndWaitTestServerThread: thread "
				...
				SLANG_LOG(std::cout << "#4 spawinAndWaitTestServerThread: thread failed "
		...
		SLANG_LOG(std::cout << "#5 spawinAndWaitTestServerThread: thread state "
			"before is getting killed:"
			<< tstate << " thread id:" << GetCurrentThreadId()
			<< " threadHandle:" << GetCurrentThread() << "\n";)
			if (getMutexState() == WAIT_TIMEOUT) {
				SLANG_LOG(std::cout << "#6 spawinAndWaitTestServerThread: spwaned "
		...
		SLANG_LOG(std::cout << "#7 spawinAndWaitTestServerThread: thread state "
			"before is getting killed:"
			<< tstate << " thread id:" << GetCurrentThreadId()
			<< " threadHandle:" << GetCurrentThread() << "\n";)
			if (!getMutexInfinite(false)) {
				...
				SLANG_LOG(
					std::cout
					<< "#8 spawinAndWaitTestServerThread: Failed to acquire mutex\n";)
					return -1;
			}
		SLANG_LOG(
			std::cout
			<< "#9 spawinAndWaitTestServerThread: thread after taking mutex"
			<< GetCurrentThreadId() << "\n";)
		...
		SLANG_LOG(
			std::cout
			<< "#10 spawinAndWaitTestServerThread: thread after exiting mutex"
			<< GetCurrentThreadId() << "\n";)
	} while (false);
	return 0;
}
...
#endif
		if (tstate == ethread_state_exit) {
			SLANG_LOG(std::cout << "#1 spawnThreadForTestServer: Failed to spawn "
			...
			while (tstate != ethread_state_alive) {
				if (!getMutexInfinite(true)) {
					SLANG_LOG(std::cout << "#2 spawnThreadForTestServer: : Failed to "
						"acquire mutex\n";)
						return -1;
				}
				if (tstate == ethread_state_start) {
					...
					hProcessMgmtThread = NULL;
					releaseMutex(); // and poll again
					std::cout << "#3 spawnThreadForTestServer: Worker thread failed to "
...
bool compileShaderToSpirV(
	...
	glu::ShaderProgramInfo* buildInfo)
{
	return SLANG_OK == Slang::g_slangContext.setupSlangLikeSlangc(sources, buildOptions, shaderLanguage, dst, buildInfo);
}
