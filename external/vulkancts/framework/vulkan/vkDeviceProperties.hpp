...
struct PropertyDesc
{
    PropertyDesc(VkStructureType sType_, const char *name_, uint32_t specVersion_, uint32_t typeId_)
        : name(name_)
        , sType(sType_)
...
struct PropertyStructWrapper : PropertyStruct
{
    ...
    PropertyStructWrapper(void) : m_propertyDesc(VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2, DE_NULL, ~0u, 0u)
    {
