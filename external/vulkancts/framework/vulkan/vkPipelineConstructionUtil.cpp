...
#endif
{
    ...
    {
#ifndef CTS_USES_VULKANSC
        VkRenderPassMultiviewCreateInfo *multiView = (VkRenderPassMultiviewCreateInfo *)findPNext(
            pCreateInfo->pNext, VK_STRUCTURE_TYPE_RENDER_PASS_MULTIVIEW_CREATE_INFO);
        if (multiView)
        {
    ...
    else
    {
        ...
        for (uint32_t s = 0; s < pCreateInfo->subpassCount; ++s)
        {
            ...
            VkMultisampledRenderToSingleSampledInfoEXT *msrtss =
                (VkMultisampledRenderToSingleSampledInfoEXT *)findPNext(
                    pCreateInfo->pSubpasses[s].pNext, VK_STRUCTURE_TYPE_MULTISAMPLED_RENDER_TO_SINGLE_SAMPLED_INFO_EXT);
            if (msrtss)
                subpass.m_msrtss = *msrtss;

            VkSubpassDescriptionDepthStencilResolve *dsr = (VkSubpassDescriptionDepthStencilResolve *)findPNext(
                pCreateInfo->pSubpasses[s].pNext, VK_STRUCTURE_TYPE_SUBPASS_DESCRIPTION_DEPTH_STENCIL_RESOLVE);
            if (dsr)
            {
                subpass.m_dsr = *dsr;
        ...
        if (m_images[i] != VK_NULL_HANDLE)
        {
            for (uint32_t j = 0; j < (uint32_t)subpass.m_colorAttachments.size(); ++j)
            {
                if (subpass.m_colorAttachments[j].index == i)
                {
                    ...
                    vk.cmdPipelineBarrier(commandBuffer, vk::VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                          vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0u, 0u, DE_NULL, 0u,
                                          DE_NULL, 1, &imageMemoryBarrier);
            ...
            if (subpass.m_dsr.sType == vk::VK_STRUCTURE_TYPE_SUBPASS_DESCRIPTION_DEPTH_STENCIL_RESOLVE)
            {
                    ...
                    i == subpass.m_dsr.pDepthStencilResolveAttachment->attachment)
                {
                    ...
                    vk::VkImageMemoryBarrier imageMemoryBarrier = vk::initVulkanStructure();
                    imageMemoryBarrier.srcAccessMask            = vk::VK_ACCESS_NONE_KHR;
                    imageMemoryBarrier.dstAccessMask            = vk::VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT |
...
void RenderPassWrapper::end(const DeviceInterface &vk, const VkCommandBuffer commandBuffer) const
{
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < (uint32_t)m_attachments.size(); ++i)
        {
            ...
            vk::VkPipelineStageFlags srcStageMask = color ? vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT :
                                                            vk::VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT;
            vk.cmdPipelineBarrier(commandBuffer, srcStageMask, vk::VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, 0u, 0u,
...
void GraphicsPipelineWrapper::createShaders(bool linked, bool binary)
{
    ...
    if (m_internalData->vertexShader.isSet())
        createInfos.push_back(
            makeShaderCreateInfo(vk::VK_SHADER_STAGE_VERTEX_BIT, m_internalData->vertexShader, linked, binary));
    if (m_internalData->tessellationControlShader.isSet())
    ...
    if (m_internalData->geometryShader.isSet())
        createInfos.push_back(
            makeShaderCreateInfo(vk::VK_SHADER_STAGE_GEOMETRY_BIT, m_internalData->geometryShader, linked, binary));
    if (m_internalData->fragmentShader.isSet())
        createInfos.push_back(
            makeShaderCreateInfo(vk::VK_SHADER_STAGE_FRAGMENT_BIT, m_internalData->fragmentShader, linked, binary));
    if (m_internalData->taskShader.isSet())
        createInfos.push_back(
            makeShaderCreateInfo(vk::VK_SHADER_STAGE_TASK_BIT_EXT, m_internalData->taskShader, linked, binary));
    if (m_internalData->meshShader.isSet())
    {
        createInfos.push_back(
            makeShaderCreateInfo(vk::VK_SHADER_STAGE_MESH_BIT_EXT, m_internalData->meshShader, linked, binary));
        if (!m_internalData->taskShader.isSet())
    ...
    if (isConstructionTypeShaderObject(m_internalData->pipelineConstructionType))
    {
        ...
        if (pointerToCreateInfo->pViewportState)
        {
            ...
            VkPipelineViewportDepthClipControlCreateInfoEXT *depthClipControl =
                (VkPipelineViewportDepthClipControlCreateInfoEXT *)findPNext(
                    pointerToCreateInfo->pViewportState->pNext,
                    VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_DEPTH_CLIP_CONTROL_CREATE_INFO_EXT);
            if (depthClipControl)
                state->negativeOneToOne = depthClipControl->negativeOneToOne;
            VkPipelineViewportShadingRateImageStateCreateInfoNV *viewportShadingRate =
                (VkPipelineViewportShadingRateImageStateCreateInfoNV *)findPNext(
                    pointerToCreateInfo->pViewportState->pNext,
            ...
            VkPipelineViewportSwizzleStateCreateInfoNV *viewportSwizzle =
                (VkPipelineViewportSwizzleStateCreateInfoNV *)findPNext(
                    pointerToCreateInfo->pViewportState->pNext,
            ...
            VkPipelineViewportWScalingStateCreateInfoNV *viewportWScaling =
                (VkPipelineViewportWScalingStateCreateInfoNV *)findPNext(
                    pointerToCreateInfo->pViewportState->pNext,
            ...
            VkPipelineViewportCoarseSampleOrderStateCreateInfoNV *coarseSampleOrder =
                (VkPipelineViewportCoarseSampleOrderStateCreateInfoNV *)findPNext(
                    pointerToCreateInfo->pViewportState->pNext,
        ...
        if (pointerToCreateInfo->pRasterizationState)
        {
            ...
            VkPipelineRasterizationConservativeStateCreateInfoEXT *conservative =
                (VkPipelineRasterizationConservativeStateCreateInfoEXT *)findPNext(
            ...
            VkPipelineRasterizationDepthClipStateCreateInfoEXT *depthClip =
                (VkPipelineRasterizationDepthClipStateCreateInfoEXT *)findPNext(
                    pointerToCreateInfo->pRasterizationState->pNext,
            ...
            VkPipelineRasterizationLineStateCreateInfoEXT *rasterizationLine =
                (VkPipelineRasterizationLineStateCreateInfoEXT *)findPNext(
                    pointerToCreateInfo->pRasterizationState->pNext,
            ...
            VkPipelineRasterizationStateStreamCreateInfoEXT *rasterizationStream =
                (VkPipelineRasterizationStateStreamCreateInfoEXT *)findPNext(
                    pointerToCreateInfo->pRasterizationState->pNext,
            ...
            VkPipelineRasterizationProvokingVertexStateCreateInfoEXT *provokingVertex =
                (VkPipelineRasterizationProvokingVertexStateCreateInfoEXT *)findPNext(
                    pointerToCreateInfo->pRasterizationState->pNext,
                    VK_STRUCTURE_TYPE_PIPELINE_RASTERIZATION_PROVOKING_VERTEX_STATE_CREATE_INFO_EXT);
            if (provokingVertex)
                state->provokingVertexMode = provokingVertex->provokingVertexMode;
            VkDepthBiasRepresentationInfoEXT *depthBiasRepresentationInfo =
                (VkDepthBiasRepresentationInfoEXT *)findPNext(pointerToCreateInfo->pRasterizationState->pNext,
        ...
        if (pointerToCreateInfo->pColorBlendState)
        {
            ...
            VkPipelineColorBlendAdvancedStateCreateInfoEXT *blendAdvancedState =
                (VkPipelineColorBlendAdvancedStateCreateInfoEXT *)findPNext(
            ...
            VkPipelineColorWriteCreateInfoEXT *colorWrite = (VkPipelineColorWriteCreateInfoEXT *)findPNext(
                pointerToCreateInfo->pColorBlendState->pNext, VK_STRUCTURE_TYPE_PIPELINE_COLOR_WRITE_CREATE_INFO_EXT);
            if (colorWrite)
            {
        ...
        if (pointerToCreateInfo->pVertexInputState)
        {
            ...
            VkPipelineVertexInputDivisorStateCreateInfoEXT *divisorInfo =
                (VkPipelineVertexInputDivisorStateCreateInfoEXT *)findPNext(
        ...
        if (pointerToCreateInfo->pTessellationState)
        {
            ...
            VkPipelineTessellationDomainOriginStateCreateInfo *tessellationDomainOrigin =
                (VkPipelineTessellationDomainOriginStateCreateInfo *)findPNext(
        ...
        if (pointerToCreateInfo->pMultisampleState)
        {
            ...
            VkPipelineCoverageModulationStateCreateInfoNV *coverageModulation =
                (VkPipelineCoverageModulationStateCreateInfoNV *)findPNext(
            ...
            VkPipelineCoverageReductionStateCreateInfoNV *coverageReduction =
                (VkPipelineCoverageReductionStateCreateInfoNV *)findPNext(
                    pointerToCreateInfo->pMultisampleState->pNext,
                    VK_STRUCTURE_TYPE_PIPELINE_COVERAGE_REDUCTION_STATE_CREATE_INFO_NV);
            if (coverageReduction)
                state->coverageReductionMode = coverageReduction->coverageReductionMode;
            VkPipelineCoverageToColorStateCreateInfoNV *coverageToColor =
                (VkPipelineCoverageToColorStateCreateInfoNV *)findPNext(
                    pointerToCreateInfo->pMultisampleState->pNext,
            ...
            VkPipelineSampleLocationsStateCreateInfoEXT *sampleLocations =
                (VkPipelineSampleLocationsStateCreateInfoEXT *)findPNext(
                    pointerToCreateInfo->pMultisampleState->pNext,
        ...
        VkPipelineRepresentativeFragmentTestStateCreateInfoNV *representativeFragment =
            (VkPipelineRepresentativeFragmentTestStateCreateInfoNV *)findPNext(
                pointerToCreateInfo->pNext,
        ...
        VkPipelineFragmentShadingRateStateCreateInfoKHR *fragmentShadingRate =
            (VkPipelineFragmentShadingRateStateCreateInfoKHR *)findPNext(
                pointerToCreateInfo->pNext, VK_STRUCTURE_TYPE_PIPELINE_FRAGMENT_SHADING_RATE_STATE_CREATE_INFO_KHR);
        ...
        VkPipelineViewportExclusiveScissorStateCreateInfoNV *exclusiveScissor =
            (VkPipelineViewportExclusiveScissorStateCreateInfoNV *)findPNext(
                pointerToCreateInfo->pNext, VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_EXCLUSIVE_SCISSOR_STATE_CREATE_INFO_NV);
        if (exclusiveScissor)
        {
        ...
        VkPipelineDiscardRectangleStateCreateInfoEXT *discardRectangle =
            (VkPipelineDiscardRectangleStateCreateInfoEXT *)findPNext(
                pointerToCreateInfo->pNext, VK_STRUCTURE_TYPE_PIPELINE_DISCARD_RECTANGLE_STATE_CREATE_INFO_EXT);
        if (discardRectangle)
        {
