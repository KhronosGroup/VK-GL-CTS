...
tcu::TestStatus testPhysicalDeviceFeaturePipelineBinariesFeaturesKHR (Context& context)
{
	const VkPhysicalDevice		physicalDevice	= context.getPhysicalDevice();
	const CustomInstance		instance		(createCustomInstanceWithExtension(context, "VK_KHR_get_physical_device_properties2"));
	const InstanceDriver&		vki				(instance.getDriver());
	...
	VkPhysicalDevicePipelineBinariesFeaturesKHR	devicePipelineBinariesFeaturesKHR[count];
	const bool									isPipelineBinariesFeaturesKHR = checkExtension(properties, "VK_KHR_pipeline_binaries");

	for (int ndx = 0; ndx < count; ++ndx)
	{
		deMemset(&devicePipelineBinariesFeaturesKHR[ndx], 0xFF * ndx, sizeof(VkPhysicalDevicePipelineBinariesFeaturesKHR));
		devicePipelineBinariesFeaturesKHR[ndx].sType = isPipelineBinariesFeaturesKHR ? VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PIPELINE_BINARIES_FEATURES_KHR : VK_STRUCTURE_TYPE_MAX_ENUM;
		devicePipelineBinariesFeaturesKHR[ndx].pNext = DE_NULL;

		deMemset(&extFeatures.features, 0xcd, sizeof(extFeatures.features));
		extFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2;
		extFeatures.pNext = &devicePipelineBinariesFeaturesKHR[ndx];

		vki.getPhysicalDeviceFeatures2(physicalDevice, &extFeatures);
	}

	if (isPipelineBinariesFeaturesKHR)
		log << TestLog::Message << devicePipelineBinariesFeaturesKHR[0] << TestLog::EndMessage;

	if (isPipelineBinariesFeaturesKHR &&
		(devicePipelineBinariesFeaturesKHR[0].pipelineBinaries != devicePipelineBinariesFeaturesKHR[1].pipelineBinaries))
	{
		TCU_FAIL("Mismatch between VkPhysicalDevicePipelineBinariesFeaturesKHR");
	}
	return tcu::TestStatus::pass("Querying succeeded");
}
...
void addSeparateFeatureTests (tcu::TestCaseGroup* testGroup)
{
	...
	addFunctionCase(testGroup, "pipeline_binaries_features_khr", "VkPhysicalDevicePipelineBinariesFeaturesKHR", testPhysicalDeviceFeaturePipelineBinariesFeaturesKHR);
