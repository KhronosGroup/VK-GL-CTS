...
tcu::TestStatus createDeviceWithUnsupportedFeaturesTestUniformBufferUnsizedArrayFeaturesEXT (Context& context)
{
    const PlatformInterface&                vkp = context.getPlatformInterface();
    tcu::TestLog&                            log = context.getTestContext().getLog();
    tcu::ResultCollector                    resultCollector            (log);
    ...
    if (const void* featuresStruct = findStructureInChain(const_cast<const void*>(deviceFeatures2.pNext), getStructureType<VkPhysicalDeviceUniformBufferUnsizedArrayFeaturesEXT>()))
    {
        static const Feature features[] =
        {
        FEATURE_ITEM (VkPhysicalDeviceUniformBufferUnsizedArrayFeaturesEXT, uniformBufferUnsizedArray),
        };
        auto* supportedFeatures = reinterpret_cast<const VkPhysicalDeviceUniformBufferUnsizedArrayFeaturesEXT*>(featuresStruct);
        checkFeatures(vkp, instance, instanceDriver, physicalDevice, 1, features, supportedFeatures, queueFamilyIndex, queueCount, queuePriority, numErrors, resultCollector, &extensionNames, emptyDeviceFeatures, isSubProcess, context.getUsedApiVersion(), commandLine);
    }
...
void addSeparateUnsupportedFeatureTests (tcu::TestCaseGroup* testGroup)
{
	...
	addFunctionCase(testGroup, "uniform_buffer_unsized_array_features_ext", createDeviceWithUnsupportedFeaturesTestUniformBufferUnsizedArrayFeaturesEXT);
}
