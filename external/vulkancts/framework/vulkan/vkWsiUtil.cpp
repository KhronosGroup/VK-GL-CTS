...
const char *getName(Type wsiType)
{
    static const char *const s_names[] = {
        "xlib", "xcb", "wayland", "android", "win32", "macos",
    };
...
void WsiTriangleRenderer::recordFrame(VkCommandBuffer cmdBuffer, uint32_t imageNdx, uint32_t frameNdx) const
{
    ...
    if (m_explicitLayoutTransitions || m_attachmentLayouts[imageNdx] == VK_IMAGE_LAYOUT_UNDEFINED)
    {
        ...
        const VkImageMemoryBarrier barrier =
            makeImageMemoryBarrier(0, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, m_attachmentLayouts[imageNdx],
                                   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, m_aliasImages[imageNdx], range);
        m_vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
