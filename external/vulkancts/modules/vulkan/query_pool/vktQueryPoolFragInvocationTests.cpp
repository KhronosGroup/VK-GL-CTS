...
void recordRenderPassCommands	(const DeviceInterface&		vkd,
...
tcu::TestStatus testInvocations (Context& context, const TestParams params)
{
	...
	{
		// position								// color
		tcu::Vec4(-1.0f, -1.0f, 0.0f, 1.0f),	geometryColor,
		tcu::Vec4( 3.0f, -1.0f, 0.0f, 1.0f),	geometryColor,
	...
	if (isAtomic)
	{
		...
		atomicBuffer.reset(new BufferWithMemory(ctx.vkd, ctx.device, ctx.allocator, atomicBufferInfo, MemoryRequirement::HostVisible));
		...
		descriptorPool = descPoolBuilder.build(ctx.vkd, ctx.device, VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT, 1u);
		...
		updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u), atomicBufferType, &atomicBufferDescriptorInfo);
	...
	if (params.secondary)
	{
		...
		recordRenderPassCommands(ctx.vkd, secCmdBuffer, bindPoint, pipeline.get(), *pipelineLayout, *descriptorSet, *vertexBuffer, vbOffset);
	...
	if (!params.secondary)
		recordRenderPassCommands(ctx.vkd, primaryCmdBuffer, bindPoint, pipeline.get(), *pipelineLayout, *descriptorSet, *vertexBuffer, vbOffset);
	else
	...
	if (isAtomic)
	{
		...
		{
			std::ostringstream msg;
			msg << "Framebuffer size: " << vkExtent.width << "x" << vkExtent.height << "; expected atomic counter to be " << pixelCount << " but found " << atomicCounter;
			return tcu::TestStatus::fail(msg.str());
		}
	}
	...
	if (!tcu::floatThresholdCompare(log, "Result", "", geometryColor, resultAccess, colorThreshold, tcu::COMPARE_LOG_ON_ERROR))
