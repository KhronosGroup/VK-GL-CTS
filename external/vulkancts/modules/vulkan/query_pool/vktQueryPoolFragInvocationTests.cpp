...
tcu::TestStatus testInvocations(Context &context, const TestParams params)
{
    ...
    if (needsExact)
    {
        if (expectedResult != queryResult)
        {
            ...
            msg << "Framebuffer size: " << vkExtent.width << "x" << vkExtent.height << "; expected query result to be "
                << expectedResult << " but found " << queryResult;
    ...
    else
    {
        if (queryResult < expectedResult)
        {
            ...
            msg << "Framebuffer size: " << vkExtent.width << "x" << vkExtent.height
                << "; expected query result to be at least " << expectedResult << " but found " << queryResult;
            return tcu::TestStatus::fail(msg.str());
        }
    }
    ...
    if (!tcu::floatThresholdCompare(log, "Result", "", getFlatColor(), resultAccess, colorThreshold,
