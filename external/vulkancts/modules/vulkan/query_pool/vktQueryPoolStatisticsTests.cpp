...
{
public:
    QueryPoolStatisticsTest(tcu::TestContext &context, const std::string &name, const ResetType resetType,
                            const CopyType copyType, bool query64Bits, bool dstOffset = false,
                            const StrideType strideType = STRIDE_TYPE_VALID)
        ...
        for (uint32_t i = 0; i < 4; ++i)
        {
            ...
            computeShaderInvocationsGroupHostQueryReset->addChild(
                new QueryPoolStatisticsTest<ComputeInvocationsTestInstance>(
                    m_testCtx, prefix + copyTypeStr[copyTypeIdx] + "primary", RESET_TYPE_HOST, copyType[copyTypeIdx],
                    query64Bits, dstOffset));
            computeShaderInvocationsGroupHostQueryReset->addChild(
                new QueryPoolStatisticsTest<ComputeInvocationsSecondaryTestInstance>(
                    m_testCtx, prefix + copyTypeStr[copyTypeIdx] + "secondary", RESET_TYPE_HOST, copyType[copyTypeIdx],
                    query64Bits, dstOffset));
            computeShaderInvocationsGroupHostQueryReset->addChild(
                new QueryPoolStatisticsTest<ComputeInvocationsSecondaryInheritedTestInstance>(
