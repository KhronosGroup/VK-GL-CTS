                                                            ...
                                                            const VkDeviceSize bufferSizeBytes)
{
    ...
    for (size_t parametersNdx = 0u; parametersNdx < m_parameters.size(); ++parametersNdx)
    {
        ...
        if (m_parameters[0].resetType == RESET_TYPE_BEFORE_COPY || m_parameters[0].copyType == COPY_TYPE_CMD)
        {
            ...
            vk.cmdCopyQueryPoolResults(*cmdBuffer, *queryPool, 0, 1u, m_resetBuffer->object(), dstOffsetQuery, stride,
                                       flags);
...
public:
    ...
    struct ParametersGraphic : public GenericParameters
    {
        ParametersGraphic(const VkQueryPipelineStatisticFlags queryStatisticFlags_,
                          ...
                          const CopyType copyType_, const uint32_t dstOffset_)
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        if (m_parametersGraphic.copyType == COPY_TYPE_CMD)
        {
            vk.cmdCopyQueryPoolResults(*cmdBuffer, *queryPool, 0, m_parametersGraphic.queryCount,
                                       m_queryBuffer->object(), m_parametersGraphic.dstOffset,
                                       NUM_QUERY_STATISTICS * sizeof(uint64_t), m_parametersGraphic.queryFlags);
    ...
    for (uint32_t copyTypeIdx = 0; copyTypeIdx < DE_LENGTH_OF_ARRAY(copyType); copyTypeIdx++)
    {
        for (uint32_t i = 0; i < 4; ++i)
        {
            ...
            computeShaderInvocationsGroup->addChild(new QueryPoolStatisticsTest<ComputeInvocationsTestInstance>(
                m_testCtx, prefix + copyTypeStr[copyTypeIdx] + "primary", "", RESET_TYPE_NORMAL, copyType[copyTypeIdx],
                query64Bits, dstOffset));
            computeShaderInvocationsGroup->addChild(
                new QueryPoolStatisticsTest<ComputeInvocationsSecondaryTestInstance>(
                    m_testCtx, prefix + copyTypeStr[copyTypeIdx] + "secondary", "", RESET_TYPE_NORMAL,
                    copyType[copyTypeIdx], query64Bits, dstOffset));
            computeShaderInvocationsGroup->addChild(
                new QueryPoolStatisticsTest<ComputeInvocationsSecondaryInheritedTestInstance>(
                    m_testCtx, prefix + copyTypeStr[copyTypeIdx] + "secondary_inherited", "", RESET_TYPE_NORMAL,
                    copyType[copyTypeIdx], query64Bits, dstOffset));
    ...
    // Multiple statistics query flags enabled
    {
        ...
        for (uint32_t partialFlagsIdx = 0u; partialFlagsIdx < DE_LENGTH_OF_ARRAY(partialFlags); partialFlagsIdx++)
        {
            for (uint32_t waitFlagsIdx = 0u; waitFlagsIdx < DE_LENGTH_OF_ARRAY(waitFlags); waitFlagsIdx++)
            {
                ...
                {
                    uint32_t dstOffset = copyTypesIdx == 2u ? uint32_t(NUM_QUERY_STATISTICS * sizeof(uint64_t)) : 0u;
                    ...
                    uint32_t queryCount = partialFlagsIdx ? 2u : 1u;
                    {
                        ...
                        GraphicBasicMultipleQueryTestInstance::ParametersGraphic param(
                            statisticsFlags | VK_QUERY_PIPELINE_STATISTIC_FRAGMENT_SHADER_INVOCATIONS_BIT, queryFlags,
                            queryCount, false, copyTypes[copyTypesIdx], dstOffset);
...

                    {
                        ...
                        GraphicBasicMultipleQueryTestInstance::ParametersGraphic param(
                            ...
                            queryCount, true, copyTypes[copyTypesIdx], dstOffset);
                        vertexShaderMultipleQueries->addChild(
                            new QueryPoolGraphicMultipleQueryStatisticsTest<VertexShaderMultipleQueryTestInstance>(
                                m_testCtx, testName.str().c_str(), "", param));
                    }
