...
protected:
    ...
    uint32_t getRequiredNumerOfPasses(void);
...
void QueryTestBase::setupCounters()
{
    ...
    uint32_t queueFamilyIndex                 = cmdPoolCreateInfo.queueFamilyIndex;
    ...
    VK_CHECK(vki.enumeratePhysicalDeviceQueueFamilyPerformanceQueryCountersKHR(physicalDevice, queueFamilyIndex,
                                                                               &counterCount, &m_counters[0], DE_NULL));
}
...
Move<VkQueryPool> QueryTestBase::createQueryPool(uint32_t enabledCounterOffset, uint32_t enabledCounterStride)
{
    ...
    uint32_t enabledCountersCount = static_cast<uint32_t>(enabledCounters.size());
...
bool QueryTestBase::acquireProfilingLock()
{
    ...
    VkAcquireProfilingLockInfoKHR lockInfo = {
        VK_STRUCTURE_TYPE_ACQUIRE_PROFILING_LOCK_INFO_KHR, NULL, 0,
    ...
    if (result == VK_TIMEOUT)
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Timeout reached, profiling lock wasn't acquired - test had to end earlier"
...
bool QueryTestBase::verifyQueryResults(VkQueryPool queryPool)
{
    ...
    uint32_t enabledCounterCount = m_enabledCountersCountMap[queryPool.getInternal()];
    std::vector<VkPerformanceCounterResultKHR> recordedCounters(enabledCounterCount);

    // verify that query result can be retrieved
    VkResult result =
        vkd.getQueryPoolResults(device, queryPool, 0, 1, sizeof(VkPerformanceCounterResultKHR) * enabledCounterCount,
                                &recordedCounters[0], sizeof(VkPerformanceCounterResultKHR), VK_QUERY_RESULT_WAIT_BIT);
    if (result == VK_NOT_READY)
    {
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Pass but result is not ready" << tcu::TestLog::EndMessage;
        return true;
    }
    return (result == VK_SUCCESS);
}

uint32_t QueryTestBase::getRequiredNumerOfPasses()
{
...
GraphicQueryTest::GraphicQueryTest(vkt::Context &context) : GraphicQueryTestBase(context)
{
}
...
tcu::TestStatus GraphicQueryTest::iterate(void)
{
    ...
    for (uint32_t passIndex = 0; passIndex < getRequiredNumerOfPasses(); passIndex++)
    {
        const Unique<VkFence> fence(createFence(vkd, device));
...
GraphicMultiplePoolsTest::GraphicMultiplePoolsTest(vkt::Context &context) : GraphicQueryTestBase(context)
{
}
...
ComputeQueryTestBase::ComputeQueryTestBase(vkt::Context &context) : QueryTestBase(context)
{
}
...
ComputeQueryTest::ComputeQueryTest(vkt::Context &context) : ComputeQueryTestBase(context)
{
}
...
tcu::TestStatus ComputeQueryTest::iterate(void)
{
    ...
    vkd.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT,
                           (VkDependencyFlags)0u, 0u, (const VkMemoryBarrier *)DE_NULL, 1u, &m_computeFinishBarrier, 0u,
...
ComputeMultiplePoolsTest::ComputeMultiplePoolsTest(vkt::Context &context) : ComputeQueryTestBase(context)
{
}
...
void QueryPoolPerformanceTests::init(void)
{
    ...
    addChild(
        new QueryPoolPerformanceTest(m_testCtx, TT_MULTIPLE_POOLS, VK_QUEUE_GRAPHICS_BIT, "multiple_pools_graphic"));
    addChild(
        new QueryPoolPerformanceTest(m_testCtx, TT_MULTIPLE_POOLS, VK_QUEUE_COMPUTE_BIT, "multiple_pools_compute"));
}
