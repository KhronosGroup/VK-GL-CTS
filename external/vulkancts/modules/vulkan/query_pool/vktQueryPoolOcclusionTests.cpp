    ...
    , m_testVector(testVector)
{
    ...
    m_stateObjects = new StateObjects(m_context.getDeviceInterface(), m_context, NUM_VERTICES_IN_DRAWCALL,
                                      m_testVector.primitiveTopology);

    const vk::VkDevice device     = m_context.getDevice();
    const vk::DeviceInterface &vk = m_context.getDeviceInterface();

    const vk::VkQueryPoolCreateInfo queryPoolCreateInfo = {
        vk::VK_STRUCTURE_TYPE_QUERY_POOL_CREATE_INFO, DE_NULL, 0u, vk::VK_QUERY_TYPE_OCCLUSION, NUM_QUERIES_IN_POOL, 0};
...
tcu::TestStatus BasicOcclusionQueryTestInstance::iterate(void)
{
    ...
    vk::VkResult queryResult =
        vk.getQueryPoolResults(device, m_queryPool, 0, NUM_QUERIES_IN_POOL, queryResultsSize, queryResults,
                               sizeof(queryResults[0]), vk::VK_QUERY_RESULT_64_BIT);
          ...
          (m_testVector.queryResultsAvailability ? vk::VK_QUERY_RESULT_WITH_AVAILABILITY_BIT : 0))
{
    ...
    m_stateObjects = new StateObjects(m_context.getDeviceInterface(), m_context,
...
vk::Move<vk::VkCommandBuffer> OcclusionQueryTestInstance::recordRender(vk::VkCommandPool cmdPool)
{
        ...
        !hasSeparateCopyCmdBuf())
    {
        vk.cmdCopyQueryPoolResults(*cmdBuffer, m_queryPool, 0, NUM_QUERIES_IN_POOL, m_queryPoolResultsBuffer->object(),
                                   /*dstOffset*/ 0, m_testVector.queryResultsStride, m_queryResultFlags);
...
void OcclusionQueryTestInstance::captureResults(uint64_t *retResults, uint64_t *retAvailAbility, bool allowNotReady)
{
    ...
    const vk::VkDevice device     = m_context.getDevice();
    const vk::DeviceInterface &vk = m_context.getDeviceInterface();
    std::vector<uint8_t> resultsBuffer(static_cast<size_t>(m_testVector.queryResultsStride) * NUM_QUERIES_IN_POOL);

    if (m_testVector.queryResultsMode == RESULTS_MODE_GET || m_testVector.queryResultsMode == RESULTS_MODE_GET_RESET)
    {
        vk::VkResult queryResult =
            vk.getQueryPoolResults(device, m_queryPool, 0, NUM_QUERIES_IN_POOL, resultsBuffer.size(), &resultsBuffer[0],
...
{
public:
    QueryPoolOcclusionTest(tcu::TestContext &context, const char *name, const char *description,
                           const OcclusionQueryTestVector &testVector)
...
void QueryPoolOcclusionTests::init(void)
{
    ...
    //Basic tests
    {
        ...
        addChild(new QueryPoolOcclusionTest<BasicOcclusionQueryTestInstance>(
            m_testCtx, "basic_conservative", "draw with conservative occlusion query", testVector));
        testVector.queryControlFlags = vk::VK_QUERY_CONTROL_PRECISE_BIT;
        addChild(new QueryPoolOcclusionTest<BasicOcclusionQueryTestInstance>(
            m_testCtx, "basic_precise", "draw with precise occlusion query", testVector));
    }
                                 ...
                                 testAvailabilityIdx < DE_LENGTH_OF_ARRAY(testAvailability); ++testAvailabilityIdx)
                            {
                                     ...
                                     ++discardHalfIdx)
                                {
                                    ...
                                    addChild(new QueryPoolOcclusionTest<OcclusionQueryTestInstance>(
                                        m_testCtx, testName.str().c_str(), testDescr.str().c_str(), testVector));
                                }
                            }
                        }
                    }
        ...
        for (int resultsModeIdx = 0; resultsModeIdx < DE_LENGTH_OF_ARRAY(resultsMode); ++resultsModeIdx)
        {
                 ...
                 ++testAvailabilityIdx)
            {
                ...
                for (int resultSizeIdx = 0; resultSizeIdx < DE_LENGTH_OF_ARRAY(resultSizes); ++resultSizeIdx)
                {
                    ...
                    const vk::VkDeviceSize strides[] = {1 * resultSize,   2 * resultSize, 3 * resultSize,
                                                        4 * resultSize,   5 * resultSize, 13 * resultSize,
                                                        1024 * resultSize};
                    ...
                    for (int strideIdx = 0; strideIdx < DE_LENGTH_OF_ARRAY(strides); strideIdx++)
                    {
                        ...
                        testName << resultsModeStr[resultsModeIdx] << "_results_size_" << resultSizeStr[resultSizeIdx]
                                 << "_stride_" << strides[strideIdx] << "_" << testAvailabilityStr[testAvailabilityIdx]
                                 << "_availability";
