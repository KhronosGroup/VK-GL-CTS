                                                        ...
                                                        const vk::VkCommandBuffer commandBuffer)
{
    ...
    const vk::VkClearAttachment attachment = {
        m_testVector.clearOp == CLEAR_COLOR ? vk::VK_IMAGE_ASPECT_COLOR_BIT :
                                              vk::VK_IMAGE_ASPECT_DEPTH_BIT, // VkImageAspectFlags aspectMask;
                ...
                for (int resultSizeIdx = 0; resultSizeIdx < DE_LENGTH_OF_ARRAY(resultSize); ++resultSizeIdx)
                {
                    ...
                    for (int waitIdx = 0; waitIdx < DE_LENGTH_OF_ARRAY(wait); ++waitIdx)
                    {
                        ...
                        for (int clearOpIdx = 0; clearOpIdx < DE_LENGTH_OF_ARRAY(clearOp); ++clearOpIdx)
                        {
                            ...
                            testDescr << "draw occluded " << primitiveTopologyStr[primitiveTopologyIdx] << "with "
                                      << controlFlagsStr[controlFlagIdx] << ", "
                                      << "get results without availability bit "
                                      << "with " << clearOpStr[clearOpIdx] << " as " << resultSizeStr[resultSizeIdx]
                                      << "bit variables,"
                                      << "wait for results on" << waitStr[waitIdx];
                    ...
                    for (int dstOffsetIdx = 0; dstOffsetIdx < DE_LENGTH_OF_ARRAY(copyQueryDstOffset); dstOffsetIdx++)
                    {
                        for (int strideIdx = 0; strideIdx < DE_LENGTH_OF_ARRAY(strides); strideIdx++)
                        {
                            ...
                            testDescr << resultsModeStr[resultsModeIdx] << " results "
                                      << testAvailabilityStr[testAvailabilityIdx] << " availability bit as "
                                      << resultSizeStr[resultSizeIdx] << "bit variables, with stride"
                                      << strides[strideIdx];
