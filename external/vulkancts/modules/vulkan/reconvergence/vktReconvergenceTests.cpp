...
bitset128 bitsetFromU64(uint64_t mask, uint32_t subgroupSize)
{
    mask &= subgroupSizeToMask(subgroupSize);
    bitset128 result(mask);
    for (uint32_t i = 0; i < 128 / subgroupSize - 1; ++i)
    {
...
uint64_t bitsetToU64(const bitset128 &bitset, uint32_t subgroupSize, uint32_t invocationID)
{
...
ReconvergenceTestCase::ReconvergenceTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                             const CaseDef data)
...
{
public:
    OP(OPType _type, uint64_t _value, uint32_t _caseValue = 0) : type(_type), value(_value), caseValue(_caseValue)
    {
    }
...
static bitset128 bitsetElect(const bitset128 &value, int32_t subgroupSize)
{
    bitset128 ret; // zero initialized

    for (int32_t i = 0; i < 128; i += subgroupSize)
    {
...
public:
    ...
    void genIf(IFType ifType)
    {
        ...
        uint32_t localIndexCmp = deRandom_getUint32(&rnd) % 128;
    ...
    void generateRandomProgram()
    {
        do
        {
            ...
            if (caseDef.isUCF())
            {
                const uint32_t invocationStride = 128;
                // Simulate for all subgroup sizes, to determine whether OP_BALLOTs are nonuniform
                for (int32_t subgroupSize = 4; subgroupSize <= 64; subgroupSize *= 2)
                {
        ...
        for (int32_t i = 0; i < (int32_t)ops.size(); ++i)
        {
            ...
            case OP_IF_MASK:
                ...
                if (ops[i].value == ~0ULL)
                {
                    ...
                    *css << "if (testBit(uvec2(0x" << std::hex << (ops[i].value & 0xFFFFFFFF) << ", 0x"
                         << (ops[i].value >> 32) << "), gl_SubgroupInvocationID)) {\n";
            ...
            case OP_IF_LOCAL_INVOCATION_INDEX:
                printIndent(*css);
                *css << "if (gl_LocalInvocationIndex >= inputA.a[0x" << std::hex << ops[i].value << "]) {\n";
                indent += 4;
                break;
            case OP_ELSE_MASK:
    ...
    // values to ref
    uint32_t simulate(bool countOnly, uint32_t subgroupSize, uint32_t invocationStride, uint64_t *ref)
    {
        ...
        const uint64_t fullSubgroupMask = subgroupSizeToMask(subgroupSize);

        // Per-invocation output location counters
        uint32_t outLoc[128] = {0};
        ...
        while (i < (int32_t)ops.size())
        {
            ...
            case OP_BALLOT:
                ...
                if (caseDef.isSUCF())
                {
                    for (uint32_t id = 0; id < 128; id += subgroupSize)
                    {
                        uint64_t subgroupMask = bitsetToU64(stateStack[nesting].activeMask, subgroupSize, id);
            ...
            case OP_IF_LOCAL_INVOCATION_INDEX:
            {
                // all bits >= N
                bitset128 mask(0);
                for (int32_t j = (int32_t)ops[i].value; j < 128; ++j)
                    mask.set(j);
            ...
            case OP_ELSE_LOCAL_INVOCATION_INDEX:
            {
                // all bits < N
                bitset128 mask(0);
                for (int32_t j = 0; j < (int32_t)ops[i].value; ++j)
                    mask.set(j);
        ...
        uint32_t maxLoc = 0;
        for (uint32_t id = 0; id < ARRAYSIZE(outLoc); ++id)
...
void ReconvergenceTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    css << "bool testBit(uvec2 mask, uint bit) { return (bit < 32) ? ((mask.x >> bit) & 1) != 0 : ((mask.y >> "
           "(bit-32)) & 1) != 0; }\n";
...
tcu::TestStatus ReconvergenceTestInstance::iterate(void)
{
    ...
    const uint32_t subgroupSize     = subgroupProperties.subgroupSize;
    const uint32_t invocationStride = 128;
    ...
    uint32_t maxLoc = program.simulate(true, subgroupSize, invocationStride, DE_NULL);
    ...
    const VkBool32 computeFullSubgroups =
        subgroupProperties.subgroupSize <= 64 && m_context.getSubgroupSizeControlFeaturesEXT().computeFullSubgroups;
    ...
    int32_t pcinvocationStride = 0;
    vk.cmdPushConstants(*cmdBuffer, *pipelineLayout, allShaderStages, 0, sizeof(pcinvocationStride),
                        &pcinvocationStride);
    ...
    deMemset(ptrs[1], 0, invocationStride * sizeof(uint64_t));
    ...
    {
        maxLoc   = newMaxLoc;
        sizes[1] = maxLoc * sizeof(uint64_t);
        ...
        ptrs[1]              = (uint32_t *)buffers[1]->getAllocation().getHostPtr();
        deMemset(ptrs[1], 0, (size_t)sizes[1]);
    ...
    uint64_t *ref = new uint64_t[maxLoc];
    deMemset(ref, 0, maxLoc * sizeof(uint64_t));
    program.simulate(false, subgroupSize, invocationStride, ref);

    const uint64_t *result = (const uint64_t *)ptrs[1];
    ...
    {
        uint64_t fullMask = subgroupSizeToMask(subgroupSize);
        ...
        for (uint32_t lane = 0; lane < invocationStride; ++lane)
        {
            ...
            while (refLoc < maxLoc)
            {
                ...
                // the previous result.
                uint64_t expectedResult = m_data.isElect() ? ((lane % subgroupSize) == 0 ? 2 : 1) : fullMask;
...
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(testCtx, "reconvergence", "reconvergence tests"));
