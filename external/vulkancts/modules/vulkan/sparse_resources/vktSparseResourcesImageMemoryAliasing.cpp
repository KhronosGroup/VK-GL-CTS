...
const uint32_t MODULO_DIVISOR = 127;
...
{
public:
    ImageSparseMemoryAliasingCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
ImageSparseMemoryAliasingCase::ImageSparseMemoryAliasingCase(tcu::TestContext &testCtx, const std::string &name,
...
{
public:
    ImageSparseMemoryAliasingInstance(Context &context, const ImageType imageType, const tcu::UVec3 &imageSize,
...
ImageSparseMemoryAliasingInstance::ImageSparseMemoryAliasingInstance(Context &context, const ImageType imageType,
                                                                     const tcu::UVec3 &imageSize, const VkFormat format,
                                                                     const bool useDeviceGroups)
...
tcu::TestStatus ImageSparseMemoryAliasingInstance::iterate(void)
{
    ...
    for (uint32_t physDevID = 0; physDevID < m_numPhysicalDevices; physDevID++)
    {
        ...
        imageSparseInfo.flags = VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT | VK_IMAGE_CREATE_SPARSE_ALIASED_BIT |
        ...
        const VkSemaphore imageMemoryBindSemaphores[] = {memoryBindSemaphoreTransfer.get(),
                                                         memoryBindSemaphoreCompute.get()};
...

        {
            ...
            const VkMemoryRequirements imageMemoryRequirements =
                getImageMemoryRequirements(deviceInterface, getDevice(), *imageRead);
            ...
            for (uint32_t planeNdx = 0; planeNdx < formatDescription.numPlanes; ++planeNdx)
            {
                ...
                for (uint32_t layerNdx = 0; layerNdx < imageSparseInfo.arrayLayers; ++layerNdx)
                {
                    ...
                    {
                        const VkSparseMemoryBind imageReadMipTailMemoryBind = makeSparseMemoryBind(
                        ...
                        deviceMemUniquePtrVec.push_back(makeVkSharedPtr(
                            Move<VkDeviceMemory>(check<VkDeviceMemory>(imageReadMipTailMemoryBind.memory),
                                                 Deleter<VkDeviceMemory>(deviceInterface, getDevice(), DE_NULL))));

                        imageReadMipTailBinds.push_back(imageReadMipTailMemoryBind);

                        const VkSparseMemoryBind imageWriteMipTailMemoryBind = makeSparseMemoryBind(
                            deviceInterface, getDevice(), aspectRequirements.imageMipTailSize, memoryType,
                            aspectRequirements.imageMipTailOffset + layerNdx * aspectRequirements.imageMipTailStride);

                        deviceMemUniquePtrVec.push_back(makeVkSharedPtr(
                            Move<VkDeviceMemory>(check<VkDeviceMemory>(imageWriteMipTailMemoryBind.memory),
                                                 Deleter<VkDeviceMemory>(deviceInterface, getDevice(), DE_NULL))));
                ...
                if ((aspectRequirements.formatProperties.flags & VK_SPARSE_IMAGE_FORMAT_SINGLE_MIPTAIL_BIT) &&
                    aspectRequirements.imageMipTailFirstLod < imageSparseInfo.mipLevels)
                {
                    const VkSparseMemoryBind imageReadMipTailMemoryBind =
                    ...
                    const VkSparseMemoryBind imageWriteMipTailMemoryBind =
                        makeSparseMemoryBind(deviceInterface, getDevice(), aspectRequirements.imageMipTailSize,
                                             memoryType, aspectRequirements.imageMipTailOffset);
            ...
            VkBindSparseInfo bindSparseInfo = {
                ...
                2u,                       //uint32_t signalSemaphoreCount;
            ...
            if (imageResidencyMemoryBinds.size() > 0)
            {
                ...
                imageResidencyBindInfo[0].bindCount = static_cast<uint32_t>(imageResidencyMemoryBinds.size());
                imageResidencyBindInfo[0].pBinds    = imageResidencyMemoryBinds.data();

                imageResidencyBindInfo[1].image     = *imageWrite;
                imageResidencyBindInfo[1].bindCount = static_cast<uint32_t>(imageResidencyMemoryBinds.size());
            ...
            if (imageReadMipTailBinds.size() > 0)
            {
                ...
                imageMipTailBindInfo[0].bindCount = static_cast<uint32_t>(imageReadMipTailBinds.size());
                imageMipTailBindInfo[0].pBinds    = imageReadMipTailBinds.data();

                imageMipTailBindInfo[1].image     = *imageWrite;
                imageMipTailBindInfo[1].bindCount = static_cast<uint32_t>(imageWriteMipTailBinds.size());
        ...
        std::vector<std::vector<uint32_t>> planeOffsets(imageSparseInfo.mipLevels);
        std::vector<std::vector<uint32_t>> planeRowPitches(imageSparseInfo.mipLevels);
        ...
        deviceInterface.cmdCopyBufferToImage(*commandBuffer, *inputBuffer, *imageRead,
...

        {
            ...
            for (uint32_t planeNdx = 0; planeNdx < formatDescription.numPlanes; ++planeNdx)
            {
                ...
                imageSparseTransferSrcBarriers.emplace_back(makeImageMemoryBarrier(
                    ...
                    VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *imageRead,
                    makeImageSubresourceRange(aspect, 0u, imageSparseInfo.mipLevels, 0u, imageSparseInfo.arrayLayers)));
            }
...

        {
            ...
            for (uint32_t planeNdx = 0; planeNdx < formatDescription.numPlanes; ++planeNdx)
            {
                ...
                imageSparseShaderStorageBarriers.emplace_back(makeImageMemoryBarrier(
                    0u, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, *imageWrite,
                    makeImageSubresourceRange(aspect, 0u, imageSparseInfo.mipLevels, 0u, imageSparseInfo.arrayLayers)));
            }

            deviceInterface.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                               VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL,
                                               static_cast<uint32_t>(imageSparseShaderStorageBarriers.size()),
                                               imageSparseShaderStorageBarriers.data());
        }
...

        {
            ...
            deviceInterface.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                               VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 1u, &memoryBarrier, 0u, DE_NULL, 0u,
                                               DE_NULL);
        }
        ...
        submitCommandsAndWait(deviceInterface, getDevice(), computeQueue.queueHandle, *commandBuffer, 2u,
                              imageMemoryBindSemaphores, stageBits, 0, DE_NULL, m_useDeviceGroups, firstDeviceID);
        ...
        for (uint32_t planeNdx = 0; planeNdx < formatDescription.numPlanes; ++planeNdx)
            for (uint32_t mipmapNdx = 0; mipmapNdx < imageSparseInfo.mipLevels; ++mipmapNdx)
                planePointers[mipmapNdx][planeNdx] =
                    outputData + static_cast<size_t>(planeOffsets[mipmapNdx][planeNdx]);
        ...
        for (uint32_t channelNdx = 0; channelNdx < 4; ++channelNdx)
        {
            ...
            float fixedPointError =
                tcu::TexVerifierUtil::computeFixedPointError(formatDescription.channels[channelNdx].sizeBits);
            ;
            ...
            for (uint32_t mipmapNdx = 0; mipmapNdx < aspectRequirements.imageMipTailFirstLod; ++mipmapNdx)
            {
                ...
                const tcu::ConstPixelBufferAccess pixelBuffer =
                    vk::getChannelAccess(formatDescription, gridSize, planeRowPitches[mipmapNdx].data(),
                    ...
                    for (uint32_t offsetY = 0u; offsetY < gridSize.y(); ++offsetY)
                        for (uint32_t offsetX = 0u; offsetX < gridSize.x(); ++offsetX)
                        {
                            ...
                            case 2:
                                ...
                                fReferenceValue =
                                    static_cast<float>(iReferenceValue) / static_cast<float>(MODULO_DIVISOR);
                                break;
                            case 3:
                                iReferenceValue = 1u;
                                fReferenceValue = 1.f;
            ...
            for (uint32_t mipmapNdx = aspectRequirements.imageMipTailFirstLod; mipmapNdx < imageSparseInfo.mipLevels;
                 ++mipmapNdx)
            {
...
void ImageSparseMemoryAliasingCase::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    for (uint32_t mipLevelNdx = 0; mipLevelNdx < mipLevels; ++mipLevelNdx)
    {
        ...
        src << versionDecl << "\n"
            << "layout (local_size_x = " << xWorkGroupSize << ", local_size_y = " << yWorkGroupSize
...
tcu::TestCaseGroup *createImageSparseMemoryAliasingTestsCommon(tcu::TestContext &testCtx,
                                                               ...
                                                               const bool useDeviceGroup = false)
{
    ...
    const std::vector<TestImageParameters> imageParameters{
        {IMAGE_TYPE_2D,
         {tcu::UVec3(512u, 256u, 1u), tcu::UVec3(128u, 128u, 1u), tcu::UVec3(503u, 137u, 1u), tcu::UVec3(11u, 37u, 1u)},
         getTestFormats(IMAGE_TYPE_2D)},
        {IMAGE_TYPE_2D_ARRAY,
         {tcu::UVec3(512u, 256u, 6u), tcu::UVec3(128u, 128u, 8u), tcu::UVec3(503u, 137u, 3u), tcu::UVec3(11u, 37u, 3u)},
         getTestFormats(IMAGE_TYPE_2D_ARRAY)},
        {IMAGE_TYPE_CUBE,
         {tcu::UVec3(256u, 256u, 1u), tcu::UVec3(128u, 128u, 1u), tcu::UVec3(137u, 137u, 1u), tcu::UVec3(11u, 11u, 1u)},
         getTestFormats(IMAGE_TYPE_CUBE)},
        {IMAGE_TYPE_CUBE_ARRAY,
         {tcu::UVec3(256u, 256u, 6u), tcu::UVec3(128u, 128u, 8u), tcu::UVec3(137u, 137u, 3u), tcu::UVec3(11u, 11u, 3u)},
         getTestFormats(IMAGE_TYPE_CUBE_ARRAY)},
        {IMAGE_TYPE_3D,
         {tcu::UVec3(256u, 256u, 16u), tcu::UVec3(128u, 128u, 8u), tcu::UVec3(503u, 137u, 3u),
          tcu::UVec3(11u, 37u, 3u)},
    ...
    for (size_t imageTypeNdx = 0; imageTypeNdx < imageParameters.size(); ++imageTypeNdx)
    {
        ...
        for (size_t formatNdx = 0; formatNdx < imageParameters[imageTypeNdx].formats.size(); ++formatNdx)
        {
                 ...
                 ++imageSizeNdx)
            {
                ...
                formatGroup->addChild(new ImageSparseMemoryAliasingCase(testCtx, stream.str(), "", imageType, imageSize,
                                                                        format, glu::GLSL_VERSION_440, useDeviceGroup));
            }
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "image_sparse_memory_aliasing", "Sparse Image Memory Aliasing"));
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "device_group_image_sparse_memory_aliasing", "Sparse Image Memory Aliasing"));
