...
{
    group->addChild(new SparseCase(testCtx, params.name, params.function, params.imageType, params.imageSize,
                                   params.format, params.operand));
}
...
tcu::TestCaseGroup *createSparseResourcesShaderIntrinsicsTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "shader_intrinsics", "Sparse Resources Shader Intrinsics"));
    ...
    static const std::string functions[SPARSE_SPIRV_FUNCTION_TYPE_LAST]{
        "_sparse_fetch", "_sparse_read", "_sparse_sample_explicit_lod", "_sparse_sample_implicit_lod", "_sparse_gather",
    };
    ...
    for (uint32_t functionNdx = 0; functionNdx < SPARSE_SPIRV_FUNCTION_TYPE_LAST; ++functionNdx)
    {
        ...
        for (const auto &imageParams : imageParameters)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> imageTypeGroup(new tcu::TestCaseGroup(
                testCtx, (getImageTypeName(caseParams.imageType) + functions[functionNdx]).c_str(), ""));
            ...
            for (const auto &testFormat : imageParams.formats)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> formatGroup(
                    new tcu::TestCaseGroup(testCtx, getImageFormatID(caseParams.format).c_str(), ""));
                ...
                for (size_t imageSizeNdx = 0; imageSizeNdx < imageParams.imageSizes.size(); ++imageSizeNdx)
                {
                    ...
                    {
                    case SPARSE_FETCH:
                        if ((caseParams.imageType == IMAGE_TYPE_CUBE) ||
                            (caseParams.imageType == IMAGE_TYPE_CUBE_ARRAY))
                            continue;
                        break;
                    ...
                    case SPARSE_GATHER:
                        if ((caseParams.imageType == IMAGE_TYPE_CUBE) ||
                            (caseParams.imageType == IMAGE_TYPE_CUBE_ARRAY) || (caseParams.imageType == IMAGE_TYPE_3D))
                            continue;
                        break;
                    default:
                        break;
                    ...
                    nameStream << caseParams.imageSize.x() << "_" << caseParams.imageSize.y() << "_"
                               << caseParams.imageSize.z();
