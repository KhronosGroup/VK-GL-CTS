...
constexpr deUint32 kMemoryPatterns[] = { 0xAAAAAAAAu, 0x55555555u };

class BufferSparseRebindCase : public TestCase
{
public:
					BufferSparseRebindCase	(tcu::TestContext&		testCtx,
...
BufferSparseRebindCase::BufferSparseRebindCase (tcu::TestContext&		testCtx,
												const std::string&		name,
												const deUint32			bufferSize,
												const bool				useDeviceGroups)
...
{
public:
					BufferSparseRebindInstance	(Context&					context,
												 const deUint32				bufferSize,
												 const bool					useDeviceGroups);
...
BufferSparseRebindInstance::BufferSparseRebindInstance (Context&		context,
														const deUint32	bufferSize,
														const bool		useDeviceGroups)
...
tcu::TestStatus BufferSparseRebindInstance::iterate (void)
{
	...
	for (deUint32 physDevID = 0; physDevID < m_numPhysicalDevices; physDevID++)
	{
		...
		const deUint32 queueFamilyIndices[] = { sparseQueue.queueFamilyIndex, transferQueue.queueFamilyIndex };
		...
		deviceMemories.push_back(Move<VkDeviceMemory> (check<VkDeviceMemory>(sparseMemoryBindFull[0].memory), Deleter<VkDeviceMemory>(deviceInterface, getDevice(), DE_NULL)));
		deviceMemories.push_back(Move<VkDeviceMemory> (check<VkDeviceMemory>(sparseMemoryBindFull[1].memory), Deleter<VkDeviceMemory>(deviceInterface, getDevice(), DE_NULL)));
		...
		for (deUint32 memoryIdx = 0; memoryIdx < 2; memoryIdx++)
		{
			...
			const Unique<VkCommandBuffer> commandBufferFill(allocateCommandBuffer(deviceInterface, getDevice(), *commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));

			beginCommandBuffer(deviceInterface, *commandBufferFill);
			deviceInterface.cmdFillBuffer(*commandBufferFill, *sparseBuffer, 0, VK_WHOLE_SIZE, kMemoryPatterns[memoryIdx]);
		...
		const Unique<VkCommandBuffer>	commandBufferCopy(allocateCommandBuffer(deviceInterface, getDevice(), *commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
		...
		// Make the changes visible to the host
		{
			...
			deviceInterface.cmdPipelineBarrier(*commandBufferCopy, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 0u, DE_NULL, 1u, &outputBufferHostBarrier, 0u, DE_NULL);
		}
		...
		// In case of device groups, submit on the physical device with the resource
		submitCommandsAndWait(deviceInterface, getDevice(), transferQueue.queueHandle, *commandBufferCopy, 1u, &bufferMemoryBindSemaphore.get(),
			waitStageBits, 0, DE_NULL, m_useDeviceGroups, firstDeviceID);

		// Retrieve data from output buffer to host memory
		invalidateAlloc(deviceInterface, getDevice(), *outputBufferAlloc);

		const deUint32* outputData = static_cast<const deUint32*>(outputBufferAlloc->getHostPtr());

		// Wait for sparse queue to become idle
		deviceInterface.queueWaitIdle(sparseQueue.queueHandle);

		// Prepare reference data
		std::vector<deUint32> referenceData;
		referenceData.resize(m_bufferSize / sizeof(deUint32));

		for (deUint32 i = 0; i < referenceData.size(); i++)
		{
			if (i * sizeof(deUint32) >= partialBindOffset &&
				i * sizeof(deUint32) < (partialBindOffset + partialBindSize))
			{
