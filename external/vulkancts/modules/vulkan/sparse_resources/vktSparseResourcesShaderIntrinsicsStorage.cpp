...
void SparseShaderIntrinsicsCaseStorage::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    src << "%type_uniformconst_image_residency = OpTypePointer UniformConstant " << typeImageResidencyName
        ...
        << "%constant_uint_resident = OpConstant " << (formatIsR64(m_format) ? "%type_uint64" : "%type_uint") << " "
        ...
        << "%constant_uint_not_resident = OpConstant " << (formatIsR64(m_format) ? "%type_uint64" : "%type_uint") << " "
        << MEMORY_BLOCK_NOT_BOUND_VALUE << "\n"
                                                           ...
                                                           const VkImage imageResidency)
{
    ...
    for (uint32_t mipLevelNdx = 0u; mipLevelNdx < imageSparseInfo.mipLevels; ++mipLevelNdx)
    {
        ...
        pipelines[mipLevelNdx] = makeVkSharedPtr(
            makeComputePipeline(deviceInterface, getDevice(), *pipelineLayout, (VkPipelineCreateFlags)0u, *shaderModule,
                                (VkPipelineShaderStageCreateFlags)0u, &specializationInfo));
