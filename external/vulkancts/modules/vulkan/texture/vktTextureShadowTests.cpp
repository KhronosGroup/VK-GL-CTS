...
tcu::TestStatus Texture2DShadowTestInstance::iterate(void)
{
...

    {
        ...
        const bool isHighQuality =
            verifyTexCompareResult(m_context.getTestContext(), rendered.getAccess(), texture.getTexture(), &texCoord[0],
...
void populateTextureShadowTests(tcu::TestCaseGroup *textureShadowTests)
{
    ...
    } formats[] = {{"d16_unorm", VK_FORMAT_D16_UNORM},
    ...
    // 2D cases.
    {
        de::MovePtr<tcu::TestCaseGroup> group2D(
            new tcu::TestCaseGroup(testCtx, "2d", "2D texture shadow lookup tests"));
            ...
            for (int compareNdx = 0; compareNdx < DE_LENGTH_OF_ARRAY(compareOp); compareNdx++)
            {
                for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                {
                    for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
                    {
                        ...
                        filterGroup->addChild(new TextureTestCase<Texture2DShadowTestInstance>(testCtx, name.c_str(),
                                                                                               "", testParameters));
                    }
                }
            }
    ...
    // Cubemap cases.
    {
        de::MovePtr<tcu::TestCaseGroup> groupCube(
            new tcu::TestCaseGroup(testCtx, "cube", "Cube map texture shadow lookup tests"));
            ...
            for (int compareNdx = 0; compareNdx < DE_LENGTH_OF_ARRAY(compareOp); compareNdx++)
            {
                for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                {
                    for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
                    {
                        ...
                        filterGroup->addChild(new TextureTestCase<TextureCubeShadowTestInstance>(testCtx, name.c_str(),
                                                                                                 "", testParameters));
                    }
                }
            }
            ...
            for (int compareNdx = 0; compareNdx < DE_LENGTH_OF_ARRAY(compareOp); compareNdx++)
            {
                for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                {
                    for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
                    {
                        ...
                        filterGroup->addChild(new TextureTestCase<Texture1DShadowTestInstance>(testCtx, name.c_str(),
                                                                                               "", testParameters));
                    }
                }
            }
