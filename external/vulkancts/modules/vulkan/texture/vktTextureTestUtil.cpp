                        ...
                        const std::vector<Program> &programs, const char *texCoordSwizzle)
{
    ...
    for (std::vector<Program>::const_iterator programIt = programs.begin(); programIt != programs.end(); ++programIt)
    {
        ...
        case PROGRAM_1D_SHADOW:
            ...
            lookup  = "vec4(texture(u_sampler, vec3(texCoord, 0.0, u_ref)), 0.0, 0.0, 1.0)";
            break;
        case PROGRAM_1D_FLOAT_BIAS:
            sampler = "sampler1D";
            lookup  = "texture(u_sampler, texCoord, u_bias)";
...
TextureBinding::TextureBinding(Context &context) : m_context(context)
{
}
...
void TextureBinding::updateTextureData(const TestTextureSp &textureData, const TextureBinding::Type textureType)
{
    ...
    if (sparse)
    {
        pipeline::uploadTestTextureSparse(vkd, vkDevice, m_context.getPhysicalDevice(),
                                          m_context.getInstanceInterface(), imageParams, m_context.getUniversalQueue(),
                                          m_context.getUniversalQueueFamilyIndex(), m_context.getSparseQueue(),
                                          allocator, m_allocations, *m_textureData, *m_textureImage);
    }
    else
    {
        m_textureImageMemory =
            allocator.allocate(getImageMemoryRequirements(vkd, vkDevice, *m_textureImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(vkDevice, *m_textureImage, m_textureImageMemory->getMemory(),
                                     m_textureImageMemory->getOffset()));

        pipeline::uploadTestTexture(vkd, vkDevice, m_context.getUniversalQueue(),
                                    m_context.getUniversalQueueFamilyIndex(), allocator, *m_textureData,
                                    *m_textureImage);
    }
...
void TextureBinding::updateTextureViewMipLevels(uint32_t baseLevel, uint32_t maxLevel)
{
    const DeviceInterface &vkd              = m_context.getDeviceInterface();
    const VkDevice vkDevice                 = m_context.getDevice();
...
TextureRenderer::TextureRenderer(Context &context, VkSampleCountFlagBits sampleCount, uint32_t renderWidth,
                                 uint32_t renderHeight, VkComponentMapping componentMapping)
    : m_context(context)
    ...
    , m_componentMapping(componentMapping)
{
    ...
    // Vertex index buffer
    {
        ...
        m_vertexIndexBufferMemory = allocator.allocate(getBufferMemoryRequirements(vkd, vkDevice, *m_vertexIndexBuffer),
...
{
    m_textureBindings.push_back(TextureBindingSp(
        new TextureBinding(m_context, texture, TextureBinding::TYPE_2D, aspectMask, backingMode, m_componentMapping)));
}
...
{
    m_textureBindings.push_back(TextureBindingSp(
        new TextureBinding(m_context, texture, TextureBinding::TYPE_3D, aspectMask, backingMode, m_componentMapping)));
}
                                 ...
                                 const glu::TextureTestUtil::ReferenceParams &params, const float maxAnisotropy)
{
    ...
    // Create Graphics Pipeline
    {
        ...
        if (samplerCreateInfo.magFilter == VK_FILTER_LINEAR || samplerCreateInfo.minFilter == VK_FILTER_LINEAR ||
...

        {
            ...
            DescriptorSetUpdateBuilder()
                .writeSingle(*descriptorSet[0], DescriptorSetUpdateBuilder::Location::binding(0),
...

        {
            ...
            DescriptorSetUpdateBuilder()
                .writeSingle(*descriptorSet[1], DescriptorSetUpdateBuilder::Location::binding(0),
        ...
        // Pipeline Layout
        {
            VkDescriptorSetLayout descriptorSetLayouts[2] = {*descriptorSetLayout[0], *descriptorSetLayout[1]};
    ...
    vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *pipelineLayout, 0u, 1,
                              &descriptorSet[0].get(), 0u, DE_NULL);
    vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *pipelineLayout, 1u, 1,
                              &descriptorSet[1].get(), 0u, DE_NULL);
    ...
    tcu::copy(result.getAccess(),
              tcu::ConstPixelBufferAccess(m_textureFormat, tcu::IVec3(m_renderWidth, m_renderHeight, 1u),
...
Texture2DTestCaseParameters::Texture2DTestCaseParameters(void) : width(64), height(64)
{
}

TextureCubeTestCaseParameters::TextureCubeTestCaseParameters(void) : size(64)
{
}

Texture2DArrayTestCaseParameters::Texture2DArrayTestCaseParameters(void) : numLayers(8)
{
}
