...
Compressed3DTestInstance::Compressed3DTestInstance(Context &context, const ParameterType &testParameters)
    ...
    , m_renderer3D(context, testParameters.sampleCount, testParameters.width, testParameters.height,
                   testParameters.depth, makeComponentMappingRGBA(), VK_IMAGE_TYPE_3D, VK_IMAGE_VIEW_TYPE_3D)
{
...
tcu::TestStatus Compressed3DTestInstance::iterate(void)
{
    ...
    for (uint32_t s = 0; s < slices; ++s)
    {
        ...
        sampleTexture(tcu::SurfaceAccess(referenceFrame, pixelFormat), m_texture3D->getTexture(), &texCoord[0],
                      sampleParams);
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
            for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
            {
                ...
                compressedTextureTests->addChild(new TextureTestCase<Compressed2DTestInstance>(
                    ...
                    (formatStr + ", TEXTURETYPE_2D").c_str(), testParameters));
            }
}
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); ++formatNdx)
            for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); ++backingNdx)
            {
                ...
                compressedTextureTests->addChild(new TextureTestCase<Compressed3DTestInstance>(
                    ...
                    (formatStr + ", TEXTURETYPE_3D").c_str(), testParameters));
            }
}

tcu::TestCaseGroup *createTextureCompressedFormatTests(tcu::TestContext &testCtx)
{
    return createTestGroup(testCtx, "compressed", "Texture compressed format tests.",
                           populateTextureCompressedFormatTests);
}

tcu::TestCaseGroup *create3DTextureCompressedFormatTests(tcu::TestContext &testCtx)
{
    return createTestGroup(testCtx, "compressed_3D", "3D texture compressed format tests.",
                           populate3DTextureCompressedFormatTests);
}
