...
tcu::TestStatus Texture2DArrayFilteringTestInstance::iterate(void)
{
...

    {
        ...
        if (!isHighQuality)
        {
            ...
            const bool isOk = verifyTextureResult(m_context.getTestContext(), rendered.getAccess(),
                                                  (tcu::Texture2DArrayView)texture.getTexture(), texCoordPtr, refParams,
                                                  lookupPrecision, lodPrecision, pixelFormat);
...
void populateTextureFilteringTests(tcu::TestCaseGroup *textureFilteringTests)
{
    ...
    } minFilterModes[] = {{"nearest", Sampler::NEAREST},
    ...
    // 2D texture filtering.
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> combinationsGroup(
            new tcu::TestCaseGroup(testCtx, "combinations", "Filter and wrap mode combinations"));
            ...
            for (int magFilterNdx = 0; magFilterNdx < DE_LENGTH_OF_ARRAY(magFilterModes); magFilterNdx++)
            {
                ...
                for (int wrapSNdx = 0; wrapSNdx < DE_LENGTH_OF_ARRAY(wrapModes); wrapSNdx++)
                {
                    ...
                    for (int wrapTNdx = 0; wrapTNdx < DE_LENGTH_OF_ARRAY(wrapModes); wrapTNdx++)
                    {
                        ...
                        wrapSGroup->addChild(new TextureTestCase<Texture2DFilteringTestInstance>(testCtx, name.c_str(),
                                                                                                 "", testParameters));
                    }
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> groupUnnormal(
            new tcu::TestCaseGroup(testCtx, "unnormal", "Unnormalized Texture Filtering"));
    ...
    // Cube map texture filtering.
    {
        de::MovePtr<tcu::TestCaseGroup> groupCube(
            new tcu::TestCaseGroup(testCtx, "cube", "Cube Map Texture Filtering"));

        de::MovePtr<tcu::TestCaseGroup> formatsGroup(new tcu::TestCaseGroup(testCtx, "formats", "2D Texture Formats"));
        de::MovePtr<tcu::TestCaseGroup> sizesGroup(new tcu::TestCaseGroup(testCtx, "sizes", "Texture Sizes"));
        de::MovePtr<tcu::TestCaseGroup> combinationsGroup(
            new tcu::TestCaseGroup(testCtx, "combinations", "Filter and wrap mode combinations"));
        de::MovePtr<tcu::TestCaseGroup> onlyFaceInteriorGroup(
            new tcu::TestCaseGroup(testCtx, "no_edges_visible", "Don't sample anywhere near a face's edges"));
        ...
        for (int fmtNdx = 0; fmtNdx < DE_LENGTH_OF_ARRAY(filterableFormatsByType); fmtNdx++)
        {
            ...
            for (int filterNdx = 0; filterNdx < DE_LENGTH_OF_ARRAY(minFilterModes); filterNdx++)
            {
                ...
                if (verifierCanBeUsed(testParameters.format, testParameters.minFilter, testParameters.magFilter))
                {
                    filterGroup->addChild(new TextureTestCase<TextureCubeFilteringTestInstance>(testCtx, name.c_str(),
                                                                                                "", testParameters));
                }
            }
            formatsGroup->addChild(filterGroup.release());
    ...
    // 2D array texture filtering.
    {
        de::MovePtr<tcu::TestCaseGroup> group2DArray(
            new tcu::TestCaseGroup(testCtx, "2d_array", "2D Array Texture Filtering"));

        de::MovePtr<tcu::TestCaseGroup> formatsGroup(
            new tcu::TestCaseGroup(testCtx, "formats", "2D Array Texture Formats"));
        de::MovePtr<tcu::TestCaseGroup> sizesGroup(new tcu::TestCaseGroup(testCtx, "sizes", "Texture Sizes"));
        de::MovePtr<tcu::TestCaseGroup> combinationsGroup(
            new tcu::TestCaseGroup(testCtx, "combinations", "Filter and wrap mode combinations"));
        ...
        for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(sizes2DArray); sizeNdx++)
        {
            ...
            for (int filterNdx = 0; filterNdx < DE_LENGTH_OF_ARRAY(minFilterModes); filterNdx++)
            {
                ...
                filterGroup->addChild(new TextureTestCase<Texture2DArrayFilteringTestInstance>(testCtx, name.c_str(),
                                                                                               "", testParameters));
            }
            sizesGroup->addChild(filterGroup.release());
        }
