...
Texture2DImageViewMinLodIntTexCoordTest::Texture2DImageViewMinLodIntTexCoordTest(
    tcu::TestContext &testContext, const string &name, const Texture2DMipmapTestCaseParameters &params)
...
Texture3DImageViewMinLodIntTexCoordTest::Texture3DImageViewMinLodIntTexCoordTest(
    tcu::TestContext &testContext, const string &name, const Texture3DMipmapTestCaseParameters &params)
    : vkt::TestCase(testContext, name)
    , m_params(params)
{
}
...
void populateTextureMipmappingTests(tcu::TestCaseGroup *textureMipmappingTests)
{
    ...
    static const struct
    {
        ...
        const char *name;
    } coordTypes[] = {// Mipmapping with translated and scaled coordinates
    ...
    static const struct
    {
        const CoordType type;
        const char *name;
    } cubeCoordTypes[] = {// Mipmapping with translated and scaled coordinates
                          {COORDTYPE_BASIC, "basic"},
                          // Mipmapping with perspective projection
                          {COORDTYPE_PROJECTED, "projected"},
    ...
    // Cubemap cases.
    {
        ...
        for (int coordType = 0; coordType < DE_LENGTH_OF_ARRAY(cubeCoordTypes); coordType++)
        {
            de::MovePtr<tcu::TestCaseGroup> coordTypeGroup(
                new tcu::TestCaseGroup(testCtx, cubeCoordTypes[coordType].name));
