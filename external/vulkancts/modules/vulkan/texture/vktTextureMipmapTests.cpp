...
struct Texture2DMipmapTestCaseParameters : public Texture2DTestCaseParameters,
                                           public TextureMipmapCommonTestCaseParameters
{
};

struct TextureCubeMipmapTestCaseParameters : public TextureCubeTestCaseParameters,
                                             public TextureMipmapCommonTestCaseParameters
{
};

struct Texture3DMipmapTestCaseParameters : public Texture3DTestCaseParameters,
                                           public TextureMipmapCommonTestCaseParameters
{
};
...
Texture2DMipmapTestInstance::Texture2DMipmapTestInstance(Context &context,
    ...
    , m_renderer(context, testParameters.sampleCount, testParameters.width * 4, testParameters.height * 4)
{
    ...
    m_texture = TestTexture2DSp(new pipeline::TestTexture2D(vk::mapVkFormat(m_testParameters.format),
                                                            m_testParameters.width, m_testParameters.height));
    ...
    {
        const uint32_t step  = 0xff / (numLevels - 1);
        const uint32_t inc   = deClamp32(step * levelNdx, 0x00, 0xff);
        const uint32_t dec   = 0xff - inc;
        const uint32_t rgb   = (inc << 16) | (dec << 8) | 0xff;
        const uint32_t color = 0xff000000 | rgb;
...
tcu::TestStatus Texture2DMipmapTestInstance::iterate(void)
{
    ...
    // Sampling parameters.
    refParams.sampler =
        util::createSampler(m_testParameters.wrapS, m_testParameters.wrapT, m_testParameters.minFilter, magFilter);
    ...
    // Compare and log.
    {
        ...
        const bool isTrilinear = m_testParameters.minFilter == Sampler::NEAREST_MIPMAP_LINEAR ||
                                 m_testParameters.minFilter == Sampler::LINEAR_MIPMAP_LINEAR;
        ...
        for (int gridY = 0; gridY < gridHeight; gridY++)
        {
            for (int gridX = 0; gridX < gridWidth; gridX++)
            {
                ...
                numFailedPixels += computeTextureLookupDiff(
                    ...
                    tcu::getSubregion(errorMask.getAccess(), curX, curY, curW, curH), m_texture->getTexture(),
                    &texCoord[0], refParams, lookupPrec, lodPrec, m_context.getTestContext().getWatchDog());
            }
        }
...
TextureCubeMipmapTestInstance::TextureCubeMipmapTestInstance(Context &context,
...
tcu::TestStatus TextureCubeMipmapTestInstance::iterate(void)
{
    ...
    // Render reference and compare
    {
        ...
        refParams.sampler                 = util::createSampler(m_testParameters.wrapS, m_testParameters.wrapT,
        ...
        for (int cellNdx = 0; cellNdx < (int)gridLayout.size(); cellNdx++)
        {
            ...
            numFailedPixels += computeTextureLookupDiff(
                tcu::getSubregion(renderedFrame.getAccess(), curX, curY, curW, curH),
                tcu::getSubregion(referenceFrame.getAccess(), curX, curY, curW, curH),
                tcu::getSubregion(errorMask.getAccess(), curX, curY, curW, curH), m_texture->getTexture(), &texCoord[0],
...
Texture3DMipmapTestInstance::Texture3DMipmapTestInstance(Context &context,
    ...
    , m_renderer(context, testParameters.sampleCount, testParameters.width * 4, testParameters.height * 4)
{
    ...
    const int numLevels =
        deLog2Floor32(de::max(de::max(testParameters.width, testParameters.height), testParameters.depth)) + 1;

    m_texture =
        TestTexture3DSp(new pipeline::TestTexture3D(vk::mapVkFormat(m_testParameters.format), m_testParameters.width,
    ...
    for (int levelNdx = 0; levelNdx < numLevels; levelNdx++)
    {
        ...
        const uint32_t inc   = deClamp32(step * levelNdx, 0x00, 0xff);
        const uint32_t dec   = 0xff - inc;
        const uint32_t rgb   = (0xff << 16) | (dec << 8) | inc;
        const uint32_t color = 0xff000000 | rgb;
...
Texture2DLodControlTestInstance::Texture2DLodControlTestInstance(
...
tcu::TestStatus Texture2DLodControlTestInstance::iterate(void)
{
    ...
    // Compare and log.
    {
        const tcu::IVec4 formatBitDepth = getTextureFormatBitDepth(vk::mapVkFormat(VK_FORMAT_R8G8B8A8_UNORM));
        const tcu::PixelFormat pixelFormat(formatBitDepth[0], formatBitDepth[1], formatBitDepth[2], formatBitDepth[3]);
        const bool isTrilinear =
            m_minFilter == Sampler::NEAREST_MIPMAP_LINEAR || m_minFilter == Sampler::LINEAR_MIPMAP_LINEAR;
        tcu::LookupPrecision lookupPrec;
        tcu::LodPrecision lodPrec;
...
TextureCubeLodControlTestInstance::TextureCubeLodControlTestInstance(
    Context &context, const TextureCubeMipmapTestCaseParameters &testParameters)
    : TestInstance(context)
    , m_texSize(64)
    , m_testParameters(testParameters)
    , m_minFilter(testParameters.minFilter)
...
Texture3DLodControlTestInstance::Texture3DLodControlTestInstance(
    Context &context, const Texture3DMipmapTestCaseParameters &testParameters)
    : TestInstance(context)
    , m_texWidth(32)
    , m_texHeight(32)
    , m_texDepth(32)
                 ...
                 testParameters.testType >= util::TextureCommonTestCaseParameters::TEST_IMAGE_VIEW_MINLOD)
{
    ...
    m_texture =
        TestTexture3DSp(new pipeline::TestTexture3D(vk::mapVkFormat(format), m_texWidth, m_texHeight, m_texDepth));
...
{
public:
    Texture2DImageViewMinLodBaseLevelTestInstance(Context &context,
                                                  const Texture2DMipmapTestCaseParameters &testParameters)
        : Texture2DLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    Texture3DImageViewMinLodBaseLevelTestInstance(Context &context,
                                                  const Texture3DMipmapTestCaseParameters &testParameters)
        : Texture3DLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    TextureCubeImageViewMinLodBaseLevelTestInstance(Context &context,
                                                    const TextureCubeMipmapTestCaseParameters &testParameters)
        : TextureCubeLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    Texture2DImageViewMinLodIntTexCoordTestInstance(Context &context,
                                                    const Texture2DMipmapTestCaseParameters &testParameters)
        : Texture2DLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    Texture2DImageViewMinLodBaseLevelIntTexCoordTestInstance(Context &context,
                                                             const Texture2DMipmapTestCaseParameters &testParameters)
        : Texture2DLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    Texture2DImageViewMinLodIntTexCoordTest(tcu::TestContext &testContext, const string &name,
                                            const string &description, const Texture2DMipmapTestCaseParameters &params);
...
Texture2DImageViewMinLodIntTexCoordTest::Texture2DImageViewMinLodIntTexCoordTest(
    tcu::TestContext &testContext, const string &name, const string &description,
...
void Texture2DImageViewMinLodIntTexCoordTest::checkSupport(Context &context) const
{
    ...
    if (imageViewMinLodFeatures.minLod == false)
...
{
public:
    Texture3DImageViewMinLodIntTexCoordTestInstance(Context &context,
                                                    const Texture3DMipmapTestCaseParameters &testParameters)
        : Texture3DLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    Texture3DImageViewMinLodBaseLevelIntTexCoordTestInstance(Context &context,
                                                             const Texture3DMipmapTestCaseParameters &testParameters)
        : Texture3DLodControlTestInstance(context, testParameters)
        , m_testParam(testParameters)
    {
...
{
public:
    Texture3DImageViewMinLodIntTexCoordTest(tcu::TestContext &testContext, const string &name,
                                            const string &description, const Texture3DMipmapTestCaseParameters &params);
...
Texture3DImageViewMinLodIntTexCoordTest::Texture3DImageViewMinLodIntTexCoordTest(
    tcu::TestContext &testContext, const string &name, const string &description,
...
struct GatherParams
{
    ...
    float getNumericMinLod(void) const
    {
        ...
        switch (minLod)
        {
        case GatherMinLod::MINLOD_0_1:
            lod = 0.1f;
            break;
        case GatherMinLod::MINLOD_1_1:
            lod = 1.1f;
            break;
        default:
            DE_ASSERT(false);
...
{
public:
    TextureGatherMinLodTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
tcu::TestStatus TextureGatherMinLodInstance::iterate(void)
{
    ...
    const tcu::IVec3 iColorExtent(static_cast<int>(colorExtent.width), static_cast<int>(colorExtent.height),
    ...
    const auto verifBufferSize = static_cast<VkDeviceSize>(iColorExtent.x() * iColorExtent.y() * iColorExtent.z() *
                                                           tcu::getPixelSize(tcuFormat));
    ...
    const auto combinedSamplerInfo =
        makeDescriptorImageInfo(sampler.get(), texView.get(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    setUpdateBuilder.writeSingle(descriptorSet.get(), DescriptorSetUpdateBuilder::Location::binding(0u),
                                 VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &combinedSamplerInfo);
    ...
    const auto preClearBarrier = makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, texture.get(), texSRR);
    ...
    for (uint32_t level = 0u; level < texMipLevels; ++level)
    {
        ...
        vkd.cmdClearColorImage(cmdBuffer, texture.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &levelColor, 1u,
                               &levelRange);
    }
    ...
    const auto postClearBarrier = makeImageMemoryBarrier(
        ...
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, texture.get(), texSRR);
    cmdPipelineImageMemoryBarrier(vkd, cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                  &postClearBarrier);
    ...
    cmdPipelineImageMemoryBarrier(vkd, cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                  VK_PIPELINE_STAGE_TRANSFER_BIT, &postColorBarier);
    ...
    const auto threshold     = (m_params.needsRobustness2() ? 0.0f : 0.005f); // 1/255 < 0.005 < 2/255
    ...
    if (!tcu::boolAll(thresholdMet))
    {
        ...
        msg << "Unexpected output buffer color: expected " << expectedColor << " but found " << resultColor
            << " [diff=" << diff << "]";
        TCU_FAIL(msg.str());
    }
...
void populateTextureMipmappingTests(tcu::TestCaseGroup *textureMipmappingTests)
{
    ...
    static const struct
    {
        ...
        {"clamp", Sampler::CLAMP_TO_EDGE}, {"repeat", Sampler::REPEAT_GL}, {"mirror", Sampler::MIRRORED_REPEAT_GL}};

    static const struct
    {
        const char *name;
        const Sampler::FilterMode mode;
    } minFilterModes[] = {{"nearest_nearest", Sampler::NEAREST_MIPMAP_NEAREST},
    ...
    static const struct
    {
        const char *name;
        const Sampler::FilterMode mode;
    } magFilterModes[] = {{"nearest", Sampler::NEAREST}, {"linear", Sampler::LINEAR}};
    ...
    // 2D cases.
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> biasGroup2D(
            new tcu::TestCaseGroup(testCtx, "bias", "User-supplied bias value"));
        de::MovePtr<tcu::TestCaseGroup> minLodGroup2D(
            new tcu::TestCaseGroup(testCtx, "min_lod", "Lod control: min lod"));
        de::MovePtr<tcu::TestCaseGroup> maxLodGroup2D(
            new tcu::TestCaseGroup(testCtx, "max_lod", "Lod control: max lod"));
...
#ifndef CTS_USES_VULKANSC
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodExtGroup2D(
            new tcu::TestCaseGroup(testCtx, "image_view_min_lod", "VK_EXT_image_view_min_lod tests"));
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodGroup2D(
            new tcu::TestCaseGroup(testCtx, "min_lod", "ImageView's minLod"));
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodBaseLevelGroup2D(
            new tcu::TestCaseGroup(testCtx, "base_level", "ImageView's minLod with base level different than one"));
#endif // CTS_USES_VULKANSC

        for (int coordType = 0; coordType < DE_LENGTH_OF_ARRAY(coordTypes); coordType++)
        {
            de::MovePtr<tcu::TestCaseGroup> coordTypeGroup(
                new tcu::TestCaseGroup(testCtx, coordTypes[coordType].name, coordTypes[coordType].desc));
    ...
    // Cubemap cases.
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> minLodGroupCube(
            new tcu::TestCaseGroup(testCtx, "min_lod", "Lod control: min lod"));
        de::MovePtr<tcu::TestCaseGroup> maxLodGroupCube(
            new tcu::TestCaseGroup(testCtx, "max_lod", "Lod control: max lod"));
        ...
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodExtGroupCube(
            new tcu::TestCaseGroup(testCtx, "image_view_min_lod", "VK_EXT_image_view_min_lod tests"));
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodGroupCube(
            new tcu::TestCaseGroup(testCtx, "min_lod", "ImageView's minLod"));
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodBaseLevelGroupCube(
            new tcu::TestCaseGroup(testCtx, "base_level", "ImageView's minLod with base level different than one"));
#endif // CTS_USES_VULKANSC
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> coordTypeGroup(
                new tcu::TestCaseGroup(testCtx, cubeCoordTypes[coordType].name, cubeCoordTypes[coordType].desc));
    ...
    // 3D cases.
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> biasGroup3D(
            new tcu::TestCaseGroup(testCtx, "bias", "User-supplied bias value"));
        de::MovePtr<tcu::TestCaseGroup> minLodGroup3D(
            new tcu::TestCaseGroup(testCtx, "min_lod", "Lod control: min lod"));
        de::MovePtr<tcu::TestCaseGroup> maxLodGroup3D(
            new tcu::TestCaseGroup(testCtx, "max_lod", "Lod control: max lod"));
...
#ifndef CTS_USES_VULKANSC
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodExtGroup3D(
            new tcu::TestCaseGroup(testCtx, "image_view_min_lod", "VK_EXT_image_view_min_lod tests"));
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodGroup3D(
            new tcu::TestCaseGroup(testCtx, "min_lod", "ImageView's minLod"));
        de::MovePtr<tcu::TestCaseGroup> imageViewMinLodBaseLevelGroup3D(
            new tcu::TestCaseGroup(testCtx, "base_level", "ImageView's minLod with base level different than one"));
#endif // CTS_USES_VULKANSC
...
#ifndef CTS_USES_VULKANSC
    {
        const auto minLodGatherGroup =
            createTestGroup(testCtx, "min_lod_gather", "Test minLod with textureGather operations",
                            populateMinLodGatherGroup, destroyDeviceHelpers);
