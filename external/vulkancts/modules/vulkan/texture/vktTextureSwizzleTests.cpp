...
{
public:
    SwizzleTestCase(tcu::TestContext &context, const std::string &name, const std::string &description,
...
void populateTextureSwizzleTests(tcu::TestCaseGroup *textureSwizzleTests)
{
    ...
    } formats2D[] = {{VK_FORMAT_R4G4_UNORM_PACK8, PROGRAM_2D_FLOAT},
    ...
    de::MovePtr<tcu::TestCaseGroup> groupCompMap(
        new tcu::TestCaseGroup(testCtx, "component_mapping", "Component mapping swizzles"));
    de::MovePtr<tcu::TestCaseGroup> groupTexCoord(
        new tcu::TestCaseGroup(testCtx, "texture_coordinate", "Texture coordinate swizzles"));
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats2D); formatNdx++)
            for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
                for (int mappingNdx = 0; mappingNdx < DE_LENGTH_OF_ARRAY(componentMappings); mappingNdx++)
                {
                    ...
                    groupCompMap->addChild(new SwizzleTestCase<Swizzle2DTestInstance>(
                        testCtx, caseName.c_str(), caseDesc.c_str(), testParameters));
                }
