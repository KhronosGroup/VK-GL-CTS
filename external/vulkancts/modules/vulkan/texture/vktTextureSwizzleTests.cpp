...
{
public:
    SwizzleTestCase(tcu::TestContext &context, const std::string &name,
    ...
    {
        initializePrograms(programCollection, this->m_testsParameters.texCoordPrecision,
                           this->m_testsParameters.programs, this->m_testsParameters.texCoordSwizzle);
    }
...
Swizzle2DTestParameters::Swizzle2DTestParameters(void)
    ...
    , texCoordSwizzle(nullptr)
    , texCoordMapping(nullptr)
{
}
...
public:
    ...
    Swizzle2DTestInstance(Context &context, const ParameterType &testParameters);
...
Swizzle2DTestInstance::Swizzle2DTestInstance(Context &context, const ParameterType &testParameters)
    : TestInstance(context)
    , m_testParameters(testParameters)
    , m_format(isCompressedFormat(m_testParameters.format) ? tcu::TextureFormat() // Unused
                                                             :
                                                             mapVkFormat(testParameters.format))
    , m_compressedFormat(isCompressedFormat(m_testParameters.format) ? mapVkCompressedFormat(testParameters.format) :
                                                                       tcu::CompressedTexFormat()) // Unused
    , m_texture(TestTexture2DSp(
          isCompressedFormat(m_testParameters.format) ?
...
tcu::TestStatus Swizzle2DTestInstance::iterate(void)
{
    ...
    {
        sampleParams.samplerType =
            isCompressedFormat(m_testParameters.format) ? SAMPLERTYPE_FLOAT : getSamplerType(m_format);
    }
    ...
    // Render reference (based on sampleTextureNonProjected in gluTextureTestUtil.cpp)
    {
        ...
        tcu::Vec3 tri[2][2] = {{sq.swizzle(0, 1, 2), sq.swizzle(3, 2, 1)}, {tq.swizzle(0, 1, 2), tq.swizzle(3, 2, 1)}};
        ...
        const float triLod[2]   = {
            de::clamp(computeNonProjectedTriLod(sampleParams.lodMode, dstSize, srcSize, triS[0], triT[0]) + lodBias,
                        sampleParams.minLod, sampleParams.maxLod),
            de::clamp(computeNonProjectedTriLod(sampleParams.lodMode, dstSize, srcSize, triS[1], triT[1]) + lodBias,
                        sampleParams.minLod, sampleParams.maxLod)};
        ...
        for (int y = 0; y < dstSize.y(); y++)
            for (int x = 0; x < dstSize.x(); x++)
            {
                ...
                dst.setPixel(src.sample(sampleParams.sampler, s, t, lod) * sampleParams.colorScale +
                                 sampleParams.colorBias,
                             x, y);
            }
    }
    ...
    if (!isDefaultComponentMapping(m_testParameters.componentMapping))
    {
        ...
        const tcu::IVec4 bitDepth = getTextureFormatBitDepth(
            isCompressedFormat(m_testParameters.format) ? getUncompressedFormat(m_compressedFormat) : m_format);
        const uint8_t one  = uint8_t(255.0f * (lookupScale + lookupBias));
        const uint8_t zero = uint8_t(255.0f * lookupBias);

        const VkComponentSwizzle swizzle[] = {m_testParameters.componentMapping.r, m_testParameters.componentMapping.g,
                                              m_testParameters.componentMapping.b, m_testParameters.componentMapping.a};
...
void populateTextureSwizzleTests(tcu::TestCaseGroup *textureSwizzleTests)
{
    ...
    static const struct
    {
        ...
        const Program program;
    } colorFormats2D[] = {{VK_FORMAT_R4G4_UNORM_PACK8, PROGRAM_2D_FLOAT},
                          ...
                          {VK_FORMAT_R8_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R8_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_R8G8_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R8G8_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_R8G8B8_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R8G8B8_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_R8G8B8A8_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R8G8B8A8_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_A2B10G10R10_USCALED_PACK32, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R16_UNORM, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R16_SNORM, PROGRAM_2D_FLOAT_BIAS},
                          {VK_FORMAT_R16_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R16_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_R16G16_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R16G16_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_R16G16B16_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R16G16B16_SSCALED, PROGRAM_2D_FLOAT_BIAS},
                          ...
                          {VK_FORMAT_R16G16B16A16_USCALED, PROGRAM_2D_FLOAT},
                          {VK_FORMAT_R16G16B16A16_SSCALED, PROGRAM_2D_FLOAT_BIAS},
    ...
    static const struct
    {
        ...
        {VK_FORMAT_D16_UNORM, PROGRAM_2D_FLOAT},         {VK_FORMAT_X8_D24_UNORM_PACK32, PROGRAM_2D_FLOAT},
        {VK_FORMAT_D32_SFLOAT, PROGRAM_2D_FLOAT},        {VK_FORMAT_D16_UNORM_S8_UINT, PROGRAM_2D_FLOAT},
        {VK_FORMAT_D24_UNORM_S8_UINT, PROGRAM_2D_FLOAT}, {VK_FORMAT_D32_SFLOAT_S8_UINT, PROGRAM_2D_FLOAT}};

    static const struct
    {
        const VkFormat format;
        const Program program;
    } stencilFormats2D[] = {{VK_FORMAT_S8_UINT, PROGRAM_2D_UINT},
    ...
    static const struct
    {
        ...
        {"oooo",
         {VK_COMPONENT_SWIZZLE_ONE, VK_COMPONENT_SWIZZLE_ONE, VK_COMPONENT_SWIZZLE_ONE, VK_COMPONENT_SWIZZLE_ONE}},
    ...
    } texCoordSwizzles2d[] = {{"yx", {1, 0}}, {"xx", {0, 0}}, {"yy", {1, 1}}};
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(colorFormats2D); formatNdx++)
            for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
                for (int mappingNdx = 0; mappingNdx < DE_LENGTH_OF_ARRAY(componentMappings); mappingNdx++)
                {
                    ...
                    const string caseName  = de::toLower(formatStr.substr(10)) + "_2d" + "_" + sizes2D[sizeNdx].name +
                                            backingModes[backingNdx].name + "_" + componentMappings[mappingNdx].name;
    ...
    for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(sizes2D); sizeNdx++)
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(depthFormats2D); formatNdx++)
            for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
            {
                ...
                const string caseName  = de::toLower(formatStr.substr(10)) + "_2d" + "_" + sizes2D[sizeNdx].name +
                                        backingModes[backingNdx].name + "_" + componentMappings[1].name;
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(colorFormats2D); formatNdx++)
            for (int backingNdx = 0; backingNdx < DE_LENGTH_OF_ARRAY(backingModes); backingNdx++)
                for (int swizzleNdx = 0; swizzleNdx < DE_LENGTH_OF_ARRAY(texCoordSwizzles2d); swizzleNdx++)
                {
                    const string formatStr = de::toString(getFormatStr(colorFormats2D[formatNdx].format));
                    const string caseName  = de::toLower(formatStr.substr(10)) + "_2d" + "_" + sizes2D[sizeNdx].name +
                                            backingModes[backingNdx].name + "_" +
                                            texCoordSwizzles2d[swizzleNdx].swizzle;
