                                            ...
                                            const std::vector<float> &texCoords) const
{
    ...
    lookupPrec.colorThreshold = tcu::Vec4(0.9f / float(-lim(m_inFormat, 0)), 0.9f / float(-lim(m_inFormat, 1)),
                                          0.9f / float(-lim(m_inFormat, 2)), 0.9f / float(-lim(m_inFormat, 3)));
...
public:
    ...
    SnormLinearClampTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             ParamsSp params)
...
void populateUfloatNegativeValuesTests(tcu::TestCaseGroup *group)
{
    ...
    group->addChild(cts_amber::createAmberTestCase(
        testCtx, "b10g11r11", "", "texture/conversion/ufloat_negative_values", "b10g11r11-ufloat-pack32.amber",
...
void populateTextureConversionTests(tcu::TestCaseGroup *group)
{
    ...
    group->addChild(createTestGroup(testCtx, "ufloat_negative_values",
                                    "Tests for converting negative floats to unsigned floats",
                                    populateUfloatNegativeValuesTests));
    group->addChild(createTestGroup(testCtx, "snorm_clamp",
                                    "Tests for SNORM corner cases when smallest negative number gets clamped to -1",
                                    populateSnormClampTests));
    group->addChild(createTestGroup(
        testCtx, "snorm_clamp_linear",
        "Tests for SNORM corner cases when negative number gets clamped to -1 after applying linear filtering",
        populateSnormLinearClampTests));
}
