...
protected:
    ...
    void getExplicitFilteringRefData(const MultiPlaneImageData &imageData, vector<uint8_t> &refData);
    void getImplicitFilteringRefData(const MultiPlaneImageData &imageData, vector<uint8_t> &refData);
...
VkSamplerCreateInfo LinearFilteringTestInstance::getSamplerInfo(
    VkFilter minMagFilter, const VkSamplerYcbcrConversionInfo *samplerConversionInfo)
{
    return {
...
void LinearFilteringTestInstance::getExplicitFilteringRefData(const MultiPlaneImageData &imageData,
                                                              vector<uint8_t> &refData)
{
    ...
    const uint8_t *lumaData                                    = static_cast<const uint8_t *>(imageData.getPlanePtr(0));
    const uint8_t *chromaBData                                 = static_cast<const uint8_t *>(imageData.getPlanePtr(1));
    const uint8_t *chromaRData                                 = chromaBData; // assuming 2 planes
    uint32_t chromaStride                                      = 2;
    uint32_t chromaOffset                                      = 1;

    if (planarFormatDescription.numPlanes == 3)
    {
        chromaRData  = static_cast<const uint8_t *>(imageData.getPlanePtr(2));
        chromaStride = 1;
        chromaOffset = 0;
    }

    // associate nearest chroma sample with each luma sample
    vector<uint8_t> intermediateImageData(imageSize.x() * imageSize.y() * 4, 255);
    for (uint32_t y = 0; y < imageSize.y(); ++y)
    {
        for (uint32_t x = 0; x < imageSize.x(); ++x)
        {
            uint32_t component                   = x * 4 + imageSize.x() * y * 4;
            uint32_t chromaIndex                 = x / 2 + (imageSize.x() / 2) * (y / 2);
    ...
    tcu::ConstPixelBufferAccess intermediateImage(vk::mapVkFormat(VK_FORMAT_R8G8B8A8_UNORM), imageSize.x(),
                                                  imageSize.y(), 1, intermediateImageData.data());
    ...
    for (uint32_t y = 0; y < renderSize.y(); ++y)
    {
        ...
        for (uint32_t x = 0; x < renderSize.x(); ++x)
        {
            ...
            uint32_t texelIndex     = x * 4 + renderSize.x() * y * 4;
            refData[texelIndex + 1] = static_cast<uint8_t>(255 * color[0]); // g
            refData[texelIndex + 2] = static_cast<uint8_t>(255 * color[1]); // b
            refData[texelIndex]     = static_cast<uint8_t>(255 * color[2]); // r
        }
    }
}

void LinearFilteringTestInstance::getImplicitFilteringRefData(const MultiPlaneImageData &imageData,
                                                              vector<uint8_t> &refData)
{
    const tcu::UVec2 renderSize(m_cases[m_caseIndex].renderSize);
    ...
    const uint32_t channelRemap[]    = {1, 0, 2}; // remap to have channels in order: Y Cr Cb
    const tcu::Sampler *refSampler[] = {&refSamplerLinear, &refSamplerNearest, &refSamplerNearest};

    for (uint32_t channelNdx = 0; channelNdx < 3; channelNdx++)
    {
        ...
        const uint32_t orderedChannelNdx(channelRemap[channelNdx]);
        ...
        for (uint32_t y = 0; y < renderSize.y(); ++y)
        {
            ...
            for (uint32_t x = 0; x < renderSize.x(); ++x)
            {
                uint32_t texelIndex = x * 4 + renderSize.x() * y * 4 + channelNdx;
                float xCoord        = ((float)x + 0.5f) / (float)renderSize.x();
                refData[texelIndex] = static_cast<uint8_t>(
                    255.0f * refTexView.sample(*refSampler[orderedChannelNdx], xCoord, yCoord, 0.0f)[0]);
            }
        }
    }
}
...
tcu::TestStatus LinearFilteringTestInstance::iterate(void)
{
    ...
    vector<uint8_t> refData(renderSize.x() * renderSize.y() * 4, 255);
    ...
    const bool explicitFiltering(featureFlags &
    ...
    if (!tcu::floatThresholdCompare(m_context.getTestContext().getLog(), "Compare", "", refImage, resImage,
                                    thresholdVec, tcu::COMPARE_LOG_RESULT))
...
LinearFilteringTestCase::LinearFilteringTestCase(tcu::TestContext &context, const char *name, const char *description,
                                                 VkFormat format)
...
tcu::TestCaseGroup *createFilteringTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> filteringTests(
        new tcu::TestCaseGroup(testCtx, "filtering", "YCbCr filtering tests"));

    for (const auto &ycbcrFormat : ycbcrFormats)
    {
        const std::string name = std::string("linear_sampler_") + ycbcrFormat.name;
        filteringTests->addChild(
            new LinearFilteringTestCase(filteringTests->getTestContext(), name.c_str(), "", ycbcrFormat.format));
    }
