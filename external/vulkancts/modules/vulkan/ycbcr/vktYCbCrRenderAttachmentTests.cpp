...
{
    const VkAttachmentReference dstAttachmentRef = {0u, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL};

    const VkAttachmentDescription dstAttachment = {(VkAttachmentDescriptionFlags)0u,
                                                   aspectFormat,
                                                   VK_SAMPLE_COUNT_1_BIT,
    ...
    const VkSubpassDescription subpass = {(VkSubpassDescriptionFlags)0,
                                                                    ...
                                                                    tcu::UVec2 size)
{
    const VkFramebufferCreateInfo pCreateInfo = {
        VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO, nullptr, 0u, renderPass, 1, &imageView, size.x(), size.y(), 1};
...
{
    const VkPipelineLayoutCreateInfo createInfo = initVulkanStructure();
                                                                    ...
                                                                    tcu::UVec2 size)
{
    ...
    const Unique<VkShaderModule> vertexShaderModule(createShaderModule(m_vkd, m_device, bc.get("vert")));
    const Unique<VkShaderModule> fragmentShaderModule(createShaderModule(m_vkd, m_device, bc.get("frag")));
    ...
    return ::makeGraphicsPipeline(m_vkd, m_device, layout, *vertexShaderModule, VK_NULL_HANDLE, VK_NULL_HANDLE,
                                  VK_NULL_HANDLE, *fragmentShaderModule, renderPass, viewports, scissors,
                                  VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0u, 0u, &vertexInputState, nullptr, nullptr,
...
tcu::TestStatus RenderAttachmentTestInstance::iterate(void)
{
    ...
    const Unique<VkCommandPool> commandPool(createCommandPool(
        m_vkd, m_context.getDevice(), VK_COMMAND_POOL_CREATE_TRANSIENT_BIT, m_context.getUniversalQueueFamilyIndex()));
...

    {
        const VkRenderPassBeginInfo beginInfo = {
            ...
            {{0, 0}, {renderSize.x(), renderSize.y()}}, 0u,      nullptr};
    ...
    downloadImage(m_vkd, m_device, m_context.getUniversalQueueFamilyIndex(), m_context.getDefaultAllocator(),
                  *testImage, &imageData, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
...
tcu::TestCaseGroup *createRenderAttachmentTests(tcu::TestContext &testCtx)
{
    ...
    auto &tc = renderAttachmentTests->getTestContext();
    ...
    for (const auto &ycbcrFormat : ycbcrFormats)
    {
        ...
        for (const auto &ycbcrPlane : ycbcrFormat.planes)
        {
            ...
            jointGroup->addChild(new RenderAttachmentTestCase(tc, plane, ycbcrFormat.format, ycbcrPlane, false));
            disjointGroup->addChild(new RenderAttachmentTestCase(tc, plane, ycbcrFormat.format, ycbcrPlane, true));
        }
