...
tcu::TestStatus testStorageImageWrite(Context &context, TestParameters params)
{
...

    {
        ...
        for (uint32_t planeNdx = 0; planeNdx < formatDescription.numPlanes; ++planeNdx)
        {
            ...
            auto computePipeline = makeVkSharedPtr(makeComputePipeline(vkd, device, *pipelineLayout,
                                                                       (VkPipelineCreateFlags)0u, shaderModule->get(),
                                                                       (VkPipelineShaderStageCreateFlags)0u, DE_NULL));
            ...
            auto imageView = makeVkSharedPtr(
                makeImageView(vkd, device, *image, VK_IMAGE_VIEW_TYPE_2D, planeCompatibleFormat, subresourceRange));
    ...
    auto addTests = [&](int formatNdx)
    {
        ...
        for (size_t sizeNdx = 0; sizeNdx < availableSizes.size(); sizeNdx++)
        {
            ...
            addFunctionCaseWithPrograms(
                sizeGroup.get(), "disjoint", "", checkSupport, initPrograms, testStorageImageWrite,
                TestParameters(format, imageSize, (VkImageCreateFlags)VK_IMAGE_CREATE_DISJOINT_BIT));
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "storage_image_write", "Writing to YCbCr storage images"));
