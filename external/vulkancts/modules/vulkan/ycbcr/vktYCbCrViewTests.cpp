...
struct TestParameters
{
    ...
    TestParameters(ViewType viewType_, VkFormat format_, const UVec2 &size_, VkImageCreateFlags createFlags_,
                   uint32_t planeNdx_, glu::ShaderType shaderType_)
        : viewType(viewType_)
        , format(format_)
        , size(size_)
...
void checkImageUsageSupport(Context &context, VkFormat format, VkImageUsageFlags usage)
{
    {
        ...
        if ((usage & VK_IMAGE_USAGE_SAMPLED_BIT) != 0 && (featureFlags & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) == 0)
        {
            TCU_THROW(NotSupportedError, "Format doesn't support sampling");
        }

        // Other image usages are not handled currently
        DE_ASSERT((usage & ~(VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT |
...
tcu::TestStatus testPlaneView(Context &context, TestParameters params)
{
    ...
    if (imageAlias)
    {
        ...
        {
            VkBindImagePlaneMemoryInfo planeInfo = {VK_STRUCTURE_TYPE_BIND_IMAGE_PLANE_MEMORY_INFO_KHR, DE_NULL,
    ...
    const Unique<VkImageView> planeView(
        createImageView(vkd, device, !imageAlias ? *image : *imageAlias, planeViewFormat,
            ...
            for (size_t ndx = 0; ndx < numValues; ++ndx)
            {
                if (boolAny(greaterThanEqual(abs(result[ndx] - reference[ndx]), threshold)))
                {
                    context.getTestContext().getLog()
                        ...
                        << ": got " << result[ndx] << ", expected " << reference[ndx] << TestLog::EndMessage;
                    allOk = false;
                }
            }
        }
...
{
    addTestGroup(group, "image_view", "Plane View via VkImageView", populateViewTypeGroup,
                 TestParameters::VIEWTYPE_IMAGE_VIEW);
    addTestGroup(group, "memory_alias", "Plane View via Memory Aliasing", populateViewTypeGroup,
                 TestParameters::VIEWTYPE_MEMORY_ALIAS);
}
