...
struct TestConfig
{
    TestConfig(const ImageConfig &src_, const ImageConfig &dst_) : src(src_), dst(dst_)
    {
    }
...
void checkSupport(Context &context, const TestConfig config)
{
    ...
    if (!de::contains(context.getDeviceExtensions().begin(), context.getDeviceExtensions().end(),
                      string("VK_KHR_sampler_ycbcr_conversion")))
...
void genCopies(de::Random &rng, size_t copyCount, vk::VkFormat srcFormat, const UVec2 &srcSize, vk::VkFormat dstFormat,
               const UVec2 &dstSize, vector<vk::VkImageCopy> *copies)
{
    ...
    {
        const pair<uint32_t, uint32_t> planes(rng.choose<pair<uint32_t, uint32_t>>(pairs.begin(), pairs.end()));
...
uint32_t buildSeed(const TestConfig &config)
{
...
void logTestCaseInfo(TestLog &log, const TestConfig &config, const vector<vk::VkImageCopy> &copies)
{
    {
...
{
    const vk::VkFormat ycbcrFormats[] = {vk::VK_FORMAT_R4G4_UNORM_PACK8,
    ...
    for (size_t srcFormatNdx = 0; srcFormatNdx < DE_LENGTH_OF_ARRAY(ycbcrFormats); srcFormatNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> srcFormatGroup(new tcu::TestCaseGroup(
            testCtx, srcFormatName.c_str(), ("Tests for copies using format " + srcFormatName).c_str()));
        ...
        for (size_t dstFormatNdx = 0; dstFormatNdx < DE_LENGTH_OF_ARRAY(ycbcrFormats); dstFormatNdx++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> dstFormatGroup(new tcu::TestCaseGroup(
                testCtx, dstFormatName.c_str(), ("Tests for copies using format " + dstFormatName).c_str()));
                    ...
                    for (size_t srcDisjointNdx = 0; srcDisjointNdx < 2; srcDisjointNdx++)
                        for (size_t dstDisjointNdx = 0; dstDisjointNdx < 2; dstDisjointNdx++)
                        {
                            ...
                            addFunctionCase(dstFormatGroup.get(),
                                            string(srcTilingName) + (srcDisjoint ? "_disjoint_" : "_") +
                                                string(dstTilingName) + (dstDisjoint ? "_disjoint" : ""),
                                            "", checkSupport, imageCopyTest, config);
                        }
                }
            }
...
void initYcbcrDimensionsCopyTests(tcu::TestCaseGroup *testGroup)
{
    ...
    for (size_t imageDimensionNdx = 0; imageDimensionNdx < DE_LENGTH_OF_ARRAY(imageDimensions); imageDimensionNdx++)
    {
        ...
        const string dimensionsName("src" + de::toString(srcSize.x()) + "x" + de::toString(srcSize.y()) + "_dst" +
                                    de::toString(dstSize.x()) + "x" + de::toString(dstSize.y()));

        de::MovePtr<tcu::TestCaseGroup> dimensionGroup(
            new tcu::TestCaseGroup(testCtx, dimensionsName.c_str(), ("Image dimensions " + dimensionsName).c_str()));
...
{
    return createTestGroup(testCtx, "copy_dimensions", "YCbCr format copy tests between different image dimensions",
                           initYcbcrDimensionsCopyTests);
}
