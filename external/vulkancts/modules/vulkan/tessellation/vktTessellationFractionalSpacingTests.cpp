...
std::vector<float> readFloatArray(const int count, const void *memory, const int offset)
{
    ...
    if (count != 0)
    {
        const float *pFloatData = reinterpret_cast<const float *>(static_cast<const uint8_t *>(memory) + offset);
...
tcu::TestStatus test(Context &context, TestParams testParams)
{
    ...
    const Buffer tessLevelsBuffer(vk, device, allocator,
                                  makeBufferCreateInfo(tessLevelsBufferSizeBytes, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT),
                                  MemoryRequirement::HostVisible);
    ...
    for (uint32_t tessLevelCaseNdx = 0; tessLevelCaseNdx < tessLevelCases.size(); ++tessLevelCaseNdx)
    {
        ...
        // Verify the result.
        {
            ...
            const std::vector<float> resultTessCoords =
                readFloatArray(numResults, resultAlloc.getHostPtr(), sizeof(int32_t));
...
tcu::TestCaseGroup *createFractionalSpacingTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "fractional_spacing", "Test fractional spacing modes"));

    addFunctionCaseWithPrograms(group.get(), "glsl_odd", "", initPrograms, test,
                                TestParams(SHADER_LANGUAGE_GLSL, SPACINGMODE_FRACTIONAL_ODD));
    addFunctionCaseWithPrograms(group.get(), "glsl_even", "", initPrograms, test,
                                TestParams(SHADER_LANGUAGE_GLSL, SPACINGMODE_FRACTIONAL_EVEN));
    addFunctionCaseWithPrograms(group.get(), "hlsl_odd", "", initPrograms, test,
                                TestParams(SHADER_LANGUAGE_HLSL, SPACINGMODE_FRACTIONAL_ODD));
    addFunctionCaseWithPrograms(group.get(), "hlsl_even", "", initPrograms, test,
