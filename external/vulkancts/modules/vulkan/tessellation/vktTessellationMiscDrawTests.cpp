...
tcu::TestCaseGroup *createMiscDrawTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "misc_draw", "Miscellaneous draw-result-verifying cases"));
    ...
    for (int primitiveTypeNdx = 0; primitiveTypeNdx < DE_LENGTH_OF_ARRAY(primitivesNoIsolines); ++primitiveTypeNdx)
        for (int spacingModeNdx = 0; spacingModeNdx < SPACINGMODE_LAST; ++spacingModeNdx)
        {
            ...
            addFunctionCaseWithPrograms(
                ...
                initProgramsFillCoverCase, runTest,
    ...
    for (int primitiveTypeNdx = 0; primitiveTypeNdx < DE_LENGTH_OF_ARRAY(primitivesNoIsolines); ++primitiveTypeNdx)
        for (int spacingModeNdx = 0; spacingModeNdx < SPACINGMODE_LAST; ++spacingModeNdx)
        {
            ...
            addFunctionCaseWithPrograms(
                ...
                initProgramsFillNonOverlapCase, runTest,
                makeCaseDefinition(primitiveType, spacingMode, getReferenceImagePathPrefix(caseName)));
        }
    ...
    for (int spacingModeNdx = 0; spacingModeNdx < SPACINGMODE_LAST; ++spacingModeNdx)
    {
        ...
        addFunctionCaseWithPrograms(
            group.get(), caseName, "Basic isolines render test", checkSupportCase, initProgramsIsolinesCase, runTest,
