...
public:
    ...
    GridRenderTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       const Flags flags);
...
GridRenderTestCase::GridRenderTestCase(tcu::TestContext &testCtx, const std::string &name,
                                       const std::string &description, const Flags flags)
    ...
    , m_numLayers((m_flags & FLAG_GEOMETRY_SCATTER_LAYERS) ? 8 : 1)
{
    ...
    m_testCtx.getLog() << tcu::TestLog::Message << "Tessellation level: " << m_tessGenLevel << ", mode = quad.\n"
                       << "\tEach input patch produces " << (m_tessGenLevel * m_tessGenLevel) << " ("
                       << (m_tessGenLevel * m_tessGenLevel * 2) << " triangles)\n"
                       << tcu::TestLog::EndMessage;
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message << "Using geometry shader minimum maximum output limits."
                           << tcu::TestLog::EndMessage;
    ...
    m_testCtx.getLog() << tcu::TestLog::Message << "Program:\n"
                       ...
                       << (m_tessGenLevel * m_tessGenLevel * 2 * geometryVerticesPerPrimitive) << "\n"
                       << "\tTotal program output primitive count per input patch: "
                       << (m_tessGenLevel * m_tessGenLevel * 2 * geometryPrimitivesOutPerPrimitive) << "\n"
                       << tcu::TestLog::EndMessage;
}
...
tcu::TestCaseGroup *createGeometryGridRenderLimitsTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "limits", "Render with properties near their limits"));

    static const TestCaseDescription cases[] = {
        {"output_required_max_tessellation", "Minimum maximum tessellation level", FLAG_TESSELLATION_MAX_SPEC},
...
tcu::TestCaseGroup *createGeometryGridRenderScatterTests(tcu::TestContext &testCtx)
{
    ...
    static const TestCaseDescription cases[] = {
        ...
        {"geometry_scatter_layers",
         "Each geometry shader instance outputs its primitives to multiple layers and far from other primitives of the "
         "same instance",
