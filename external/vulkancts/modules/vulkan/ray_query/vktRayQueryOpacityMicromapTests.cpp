...
{
public:
    OpacityMicromapCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
OpacityMicromapCase::OpacityMicromapCase(tcu::TestContext &testCtx, const std::string &name,
...
tcu::TestStatus OpacityMicromapInstance::iterate(void)
{
    ...
    if (m_params.copyType != CT_NONE)
    {
        copyMicromapBackingBuffer = de::MovePtr<BufferWithMemory>(
            new BufferWithMemory(vkd, device, alloc, micromapBackingBufferCreateInfo, MemoryRequirement::Local));
...
void addCopyTests(tcu::TestCaseGroup *group)
{
    ...
    for (size_t copyTypeNdx = CT_FIRST_ACTIVE; copyTypeNdx < CT_NUM_COPY_TYPES; ++copyTypeNdx)
    {
        ...
        for (uint32_t modeNdx = 0; modeNdx < DE_LENGTH_OF_ARRAY(modes); ++modeNdx)
        {
            ...
            {
                TestParams testParams{
                    SST_COMPUTE_SHADER,    SSP_COMPUTE_PIPELINE, false, 0, level, modes[modeNdx].mode, seed++,
...
tcu::TestCaseGroup *createOpacityMicromapTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
        testCtx, "opacity_micromap", "Test acceleration structures using opacity micromap with ray query"));
