...
{
public:
							RayQueryStressCase		(tcu::TestContext& testCtx, const std::string& name, const RayQueryTestParams& rayQueryParams, const StressTestParams& stressParams);
...
{
public:
								RayQueryStressInstance	(Context& context, const RayQueryTestParams& rayQueryParams, const StressTestParams& stressParams);
...
RayQueryStressCase::RayQueryStressCase (tcu::TestContext& testCtx, const std::string& name, const RayQueryTestParams& rayQueryParams, const StressTestParams& stressParams)
...
void RayQueryStressCase::initPrograms (vk::SourceCollections& programCollection) const
{
	...
	if (m_rayQueryParams.shaderSourceType != RayQueryShaderSourceType::RAY_GENERATION_RT)
	{
		src <<
			"   Ray ray = rays[index];\n"
			"   float x = " << MAX_T_VALUE * 2 << ";\n"
			"   float y = " << MAX_T_VALUE * 2 << ";\n"
			"   float z = index;\n"
			"   float w = ray.pos.z;\n"
			"   rayQueryEXT rayQuery;\n"
			"   rayQueryInitializeEXT(rayQuery, scene, " << m_rayQueryParams.rayFlags << ", 0xFF, ray.pos, ray.tmin, ray.dir, ray.tmax);\n"
...
RayQueryStressInstance::RayQueryStressInstance(Context& context, const RayQueryTestParams& rayQueryParams, const StressTestParams& stressParams)
...
tcu::TestStatus RayQueryStressInstance::iterate (void)
{
	...
	*/
	...
	std::vector<tcu::Vec3> instance1(m_stressParams.testType == TestType::TRIANGLES ?
		totalNumPrims * 3 : totalNumPrims * 2);
	...
	for (uint32_t idx=0; idx < totalNumPrims; ++idx)
	{
		...
		m_rayQueryParams.rays[idx] = Ray{ tcu::Vec3( center[0], center[1], z-epsilon), 0.0f, tcu::Vec3(0.0f, 0.0f, 1.0f), MAX_T_VALUE };
	...
	switch (m_rayQueryParams.pipelineType)
	{
		...
		{
			resultData = rayQueryComputeTestSetup<ResultData>(m_context.getDeviceInterface(),
															  m_context.getDevice(),
		...
		{
			resultData = rayQueryRayTracingTestSetup<ResultData>(m_context.getDeviceInterface(),
																 m_context.getDevice(),
																 m_context.getDefaultAllocator(),
																 m_context.getInstanceInterface(),
																 m_context.getPhysicalDevice(),
		...
		{
			resultData = rayQueryGraphicsTestSetup<ResultData>(m_context.getDeviceInterface(),
															   m_context.getDevice(),
															   m_context.getUniversalQueueFamilyIndex(),
															   m_context.getDefaultAllocator(),
															   m_context.getBinaryCollection(),
...
tcu::TestCaseGroup* createRayQueryStressTests(tcu::TestContext& testCtx)
{
	...
	} shaderSourceTypes[] =
	{
		...
		{ RayQueryShaderSourceType::RAY_GENERATION_RT,		RayQueryShaderSourcePipeline::RAYTRACING,	"rgen_rt_shader",			},
	...
	for (size_t shaderSourceNdx = 0; shaderSourceNdx < DE_LENGTH_OF_ARRAY(shaderSourceTypes); ++shaderSourceNdx)
	{
		...
		for (size_t bottomTestNdx = 0; bottomTestNdx < DE_LENGTH_OF_ARRAY(bottomTestTypes); ++bottomTestNdx)
		{
			...
			sourceTypeGroup->addChild(new RayQueryStressCase(group->getTestContext(), bottomTestTypes[bottomTestNdx].name, rayQueryTestParams, testParams));
		}
