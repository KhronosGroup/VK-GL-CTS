...
const uint32_t TEST_WIDTH            = 256u;
const uint32_t TEST_HEIGHT           = 256u;
...
{
public:
    TestConfiguration() : m_bottomAccelerationStructures(), m_topAccelerationStructure(), m_expected()
    {
    }
    ...
    virtual const VkAccelerationStructureKHR *initAccelerationStructures(Context &context, TestParams &testParams,
                                                                         VkCommandBuffer cmdBuffer) = 0;
    virtual bool verify(BufferWithMemory *resultBuffer, Context &context, TestParams &testParams)   = 0;
...
public:
    ...
    virtual bool verify(BufferWithMemory *resultBuffer, Context &context, TestParams &testParams) override;

private:
    uint32_t chooseAABB(de::Random &rng, const std::vector<tcu::Vec2> &vertices, const std::vector<tcu::UVec2> &aabbs);
    uint32_t chooseTriangle(de::Random &rng, const std::vector<tcu::Vec2> &vertices,
                            const std::vector<tcu::UVec3> &triangles);
};

uint32_t TestConfigurationNoMiss::chooseAABB(de::Random &rng, const std::vector<tcu::Vec2> &vertices,
                                             const std::vector<tcu::UVec2> &aabbs)
{
    while (true)
    {
        const uint32_t n    = (uint32_t)rng.getInt(0, (uint32_t)aabbs.size() - 1);
...
uint32_t TestConfigurationNoMiss::chooseTriangle(de::Random &rng, const std::vector<tcu::Vec2> &vertices,
                                                 const std::vector<tcu::UVec3> &triangles)
{
    while (true)
    {
        ...
        if (ab < MIN_TRIANGLE_EDGE_LENGTH || bc < MIN_TRIANGLE_EDGE_LENGTH || ca < MIN_TRIANGLE_EDGE_LENGTH ||
            triangleArea(ab, bc, ca) < MIN_TRIANGLE_AREA_SIZE)
            continue;
                                                                                      ...
                                                                                      VkCommandBuffer cmdBuffer)
{
    ...
    const VkDevice device      = context.getDevice();
    Allocator &allocator       = context.getDefaultAllocator();
    const tcu::Vec2 centerPixelCenter =
        tcu::Vec2(0.5f - 0.5f / float(testParams.width), 0.5f - 0.5f / float(testParams.height));
    ...
    if (testParams.geomType == GEOM_TYPE_AABBS)
    {
        ...
        while (aabbs.size() < testParams.squaresGroupCount)
        {
            ...
            const uint32_t n   = chooseAABB(rng, vertices, aabbs);
            tcu::UVec2 &t      = aabbs[n];
            const tcu::Vec2 &a = vertices[t.x()];
            const tcu::Vec2 &b = vertices[t.y()];
            ...
            const tcu::Vec2 g  = tcu::Vec2(d.x(), a.y());
            const tcu::Vec2 h  = tcu::Vec2(b.x(), d.y());
            const uint32_t B   = t.y();
            const uint32_t D   = (uint32_t)vertices.size();
            const uint32_t E   = D + 1;
            const uint32_t F   = D + 2;
            const uint32_t G   = D + 3;
            const uint32_t H   = D + 4;
    ...
    else if (testParams.geomType == GEOM_TYPE_TRIANGLES)
    {
        ...
        {
            const uint32_t n   = chooseTriangle(rng, vertices, triangles);
        ...
        {
            const uint32_t a = triangles[i].x();
            const uint32_t b = triangles[i].y();
            const uint32_t c = triangles[i].z();
...
bool TestConfigurationNoMiss::verify(BufferWithMemory *resultBuffer, Context &context, TestParams &testParams)
{
...
bool TestConfigurationSingleHit::verify(BufferWithMemory *resultBuffer, Context &context, TestParams &testParams)
{
    tcu::TestLog &log           = context.getTestContext().getLog();
    const uint32_t width        = testParams.width;
    const uint32_t height       = testParams.height;
    ...
    , m_data(data)
{
    ...
    {
    case TEST_TYPE_NO_MISS:
        m_testConfig = de::MovePtr<TestConfiguration>(new TestConfigurationNoMiss());
        break;
    case TEST_TYPE_SINGLE_HIT:
        m_testConfig = de::MovePtr<TestConfiguration>(new TestConfigurationSingleHit());
        break;
    default:
...
{
    const uint32_t seed = (uint32_t)(testCtx.getCommandLine().getBaseSeed());
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "watertightness", "Tests watertightness of ray query"));
