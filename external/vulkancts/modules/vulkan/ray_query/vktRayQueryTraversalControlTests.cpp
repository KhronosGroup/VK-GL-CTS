...
void GraphicsConfiguration::initConfiguration(Context &context, TestParams &testParams)
{
    ...
    registerShaderModule(vkd, device, context, shaderModules, shaderCreateInfos, VK_SHADER_STAGE_VERTEX_BIT,
                         shaderNameIt->second[0], rayQueryTestName[testParams.bottomType][testParams.shaderTestType]);
    tescX = registerShaderModule(vkd, device, context, shaderModules, shaderCreateInfos,
                                 ...
                                 rayQueryTestName[testParams.bottomType][testParams.shaderTestType]);
    ...
    registerShaderModule(vkd, device, context, shaderModules, shaderCreateInfos, VK_SHADER_STAGE_GEOMETRY_BIT,
                         shaderNameIt->second[3], rayQueryTestName[testParams.bottomType][testParams.shaderTestType]);
    fragX = registerShaderModule(vkd, device, context, shaderModules, shaderCreateInfos, VK_SHADER_STAGE_FRAGMENT_BIT,
                                 shaderNameIt->second[4],
                                 rayQueryTestName[testParams.bottomType][testParams.shaderTestType]);
...
bool GraphicsConfiguration::verifyImage(BufferWithMemory *resultBuffer, Context &context, TestParams &testParams)
{
    ...
    case SST_FRAGMENT_SHADER:
        ...
        for (uint32_t y = 1; y < testParams.height - 1; ++y)
            for (uint32_t x = 1; x < testParams.width - 1; ++x)
            {
...
void RayTracingConfiguration::initConfiguration(Context &context, TestParams &testParams)
{
    ...
    rgenX = registerShaderModule(vkd, device, context, *rayTracingPipeline, VK_SHADER_STAGE_RAYGEN_BIT_KHR,
                                 shaderNameIt->second[0],
                                 rayQueryTestName[testParams.bottomType][testParams.shaderTestType], 0);
...
bool RayTracingConfiguration::verifyImage(BufferWithMemory *resultBuffer, Context &context, TestParams &testParams)
{
    ...
    for (uint32_t y = 0; y < testParams.height / 2; ++y)
        for (uint32_t x = 0; x < testParams.width; ++x)
        {
            referenceAccess.setPixel(hitMissValue[0], x, y, 0);
            referenceAccess.setPixel(hitMissValue[1], x, y, 1);
        }
    for (uint32_t y = testParams.height / 2; y < testParams.height; ++y)
        for (uint32_t x = 0; x < testParams.width; ++x)
        {
    ...
    for (uint32_t y = 1; y < testParams.height / 2; ++y)
        for (uint32_t x = 1; x < testParams.width - 1; ++x)
        {
    ...
    for (uint32_t y = testParams.height / 2; y < testParams.height - 1; ++y)
        for (uint32_t x = 1; x < testParams.width - 1; ++x)
        {
...
{
public:
    RayQueryTraversalControlTestCase(tcu::TestContext &context, const char *name, const char *desc,
...
RayQueryTraversalControlTestCase::RayQueryTraversalControlTestCase(tcu::TestContext &context, const char *name,
...
tcu::TestCaseGroup *createTraversalControlTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "traversal_control", "Tests verifying traversal control in RT hit shaders"));
    ...
    for (size_t shaderSourceNdx = 0; shaderSourceNdx < DE_LENGTH_OF_ARRAY(shaderSourceTypes); ++shaderSourceNdx)
    {
        ...
        for (size_t shaderTestNdx = 0; shaderTestNdx < DE_LENGTH_OF_ARRAY(shaderTestTypes); ++shaderTestNdx)
        {
            ...
            for (size_t testTypeNdx = 0; testTypeNdx < DE_LENGTH_OF_ARRAY(bottomTestTypes); ++testTypeNdx)
            {
                ...
                testTypeGroup->addChild(new RayQueryTraversalControlTestCase(
