...
Move<VkDevice> createDefaultDevice(const PlatformInterface &vkp, VkInstance instance, const InstanceInterface &vki,
                                   VkPhysicalDevice physicalDevice, uint32_t queueIndex, uint32_t sparseQueueIndex,
...
DefaultDevice::DefaultDevice(const PlatformInterface &vkPlatform, const tcu::CommandLine &cmdLine,
    ...
    , m_device(createDefaultDevice(vkPlatform, *m_instance, m_instanceInterface, m_physicalDevice,
                                   ...
                                   m_deviceFeatures.getCoreFeatures2(), m_creationExtensions, cmdLine,
                                   resourceInterface))
#ifndef CTS_USES_VULKANSC
    , m_deviceInterface(de::MovePtr<DeviceDriver>(new DeviceDriver(vkPlatform, *m_instance, *m_device)))
#else
    , m_deviceInterface(
          de::MovePtr<DeviceDriverSC>(new DeviceDriverSC(vkPlatform, *m_instance, *m_device, cmdLine, resourceInterface,
                                                         getDeviceVulkanSC10Properties(), getDeviceProperties())))
#endif // CTS_USES_VULKANSC
{
#ifndef CTS_USES_VULKANSC
    DE_UNREF(resourceInterface);
