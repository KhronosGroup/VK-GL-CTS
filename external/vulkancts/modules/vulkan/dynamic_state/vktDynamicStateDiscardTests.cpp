...
tcu::TestStatus DiscardTestInstance::iterate(void)
{
    ...
    vk::Move<vk::VkDescriptorSet> descriptorSet =
        makeDescriptorSet(m_vk, device, *descriptorPool, *m_descriptorSetLayout);
...
{
public:
    DiscardTestCase(tcu::TestContext &context, const char *name, const char *description,
                    vk::PipelineConstructionType pipelineConstructionType, TestDynamicStateDiscard testCase)
...
{
    addChild(new DiscardTestCase(m_testCtx, "stencil", "Use dynamic stencil with discard", m_pipelineConstructionType,
                                 TestDynamicStateDiscard::TEST_STENCIL));
    addChild(new DiscardTestCase(m_testCtx, "viewport", "Use dynamic viewport with discard", m_pipelineConstructionType,
                                 TestDynamicStateDiscard::TEST_VIEWPORT));
    addChild(new DiscardTestCase(m_testCtx, "scissor", "Use dynamic scissor with discard", m_pipelineConstructionType,
                                 TestDynamicStateDiscard::TEST_SCISSOR));
    addChild(new DiscardTestCase(m_testCtx, "depth", "Use dynamic depth with discard", m_pipelineConstructionType,
                                 TestDynamicStateDiscard::TEST_DEPTH));
    addChild(new DiscardTestCase(m_testCtx, "blend", "Use dynamic blend constants with discard",
                                 m_pipelineConstructionType, TestDynamicStateDiscard::TEST_BLEND_CONSTANTS));
    addChild(new DiscardTestCase(m_testCtx, "line", "Use dynamic line width with discard", m_pipelineConstructionType,
                                 TestDynamicStateDiscard::TEST_LINE_WIDTH));
}
