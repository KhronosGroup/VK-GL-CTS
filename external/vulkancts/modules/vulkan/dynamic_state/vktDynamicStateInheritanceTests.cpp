...
static const VkPipelineInputAssemblyStateCreateInfo assembly{
    VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO, NULL, 0, VK_PRIMITIVE_TOPOLOGY_POINT_LIST, VK_FALSE};

static const VkPipelineViewportStateCreateInfo viewportTemplate = {
    VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO, NULL, 0, 0, NULL, 0, NULL};
...
static void fill(VkRenderPass renderPass, VkPipelineLayout layout, uint32_t staticViewportScissorCount,
                 uint32_t stageCount, const VkPipelineShaderStageCreateInfo *pStages,
                 VkGraphicsPipelineCreateInfo *outCreateInfo, VkPipelineViewportStateCreateInfo *outViewportState)
{
...
{
    return uint8_t(roundf(clamp(x, 0.0f, 1.0f) * 255.0f));
}
...
void InheritanceTestInstance::startRenderCmds(const TestGeometry &geometry)
{
    ...
    VkCommandBufferInheritanceInfo inheritanceInfo{
        VK_STRUCTURE_TYPE_COMMAND_BUFFER_INHERITANCE_INFO, NULL, m_renderPass.get(), 0, m_framebuffer.get(), 0, 0, 0};
    ...
    case kInheritFromSecondary:
        ...
        m_vk.cmdSetViewport(m_setStateCmdBuffer, 0, uint32_t(geometry.viewports.size()), &geometry.viewports[0]);
        m_vk.cmdSetScissor(m_setStateCmdBuffer, 0, uint32_t(geometry.scissors.size()), &geometry.scissors[0]);
        break;
    ...
    case kInheritFromSecondaryWithCount:
        m_vk.cmdSetViewportWithCountEXT(m_setStateCmdBuffer, uint32_t(geometry.viewports.size()),
                                        &geometry.viewports[0]);
        m_vk.cmdSetScissorWithCountEXT(m_setStateCmdBuffer, uint32_t(geometry.scissors.size()), &geometry.scissors[0]);
        break;
    }
    ...
    if (m_inheritanceMode != kInheritanceDisabled)
    {
        ...
        VkCommandBufferInheritanceViewportScissorInfoNV inheritViewportInfo{
            VK_STRUCTURE_TYPE_COMMAND_BUFFER_INHERITANCE_VIEWPORT_SCISSOR_INFO_NV, NULL, VK_TRUE,
    ...
    if (m_inheritanceMode == kInheritanceDisabled)
    {
        m_vk.cmdSetViewport(m_subpassCmdBuffer, 0, uint32_t(geometry.viewports.size()), &geometry.viewports[0]);
        m_vk.cmdSetScissor(m_subpassCmdBuffer, 0, uint32_t(geometry.scissors.size()), &geometry.scissors[0]);
    }
    ...
    if (!graphicsPipeline)
    {
        ...
        pipelinestate::fill(m_renderPass.get(), m_rectanglePipelineLayout.get(), staticViewportCount, 3, m_stages,
                            &pipelineInfo, &viewportInfo);
        m_rectanglePipelines[staticViewportCount] =
            createGraphicsPipeline(m_vk, m_context.getDevice(), 0, &pipelineInfo, NULL);
    ...
    m_vk.cmdDraw(m_subpassCmdBuffer, uint32_t(geometry.rectangles.size()), 1, 0, 0);
    ...
    case kInheritFromPrimary:
        ...
        m_vk.cmdSetViewport(m_primaryCmdBuffer, 0, uint32_t(geometry.viewports.size()), &geometry.viewports[0]);
        m_vk.cmdSetScissor(m_primaryCmdBuffer, 0, uint32_t(geometry.scissors.size()), &geometry.scissors[0]);
        break;
    case kInheritFromPrimaryWithCount:
        // Same but with count inherited.
        m_vk.cmdSetViewportWithCountEXT(m_primaryCmdBuffer, uint32_t(geometry.viewports.size()),
                                        &geometry.viewports[0]);
        m_vk.cmdSetScissorWithCountEXT(m_primaryCmdBuffer, uint32_t(geometry.scissors.size()), &geometry.scissors[0]);
        break;
    case kSplitInheritance:
        ...
        {
            m_vk.cmdSetViewport(m_primaryCmdBuffer, 1, uint32_t(geometry.viewports.size() - 1), &geometry.viewports[1]);
            m_vk.cmdSetScissor(m_primaryCmdBuffer, 1, uint32_t(geometry.scissors.size() - 1), &geometry.scissors[1]);
        }
        /* FALLTHROUGH */
    case kInheritanceDisabled:
    case kInheritFromSecondary:
    ...
    m_vk.cmdBeginRenderPass(m_primaryCmdBuffer, &renderPassBeginInfo, VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS);
    VkCommandBuffer secondaryCmdBuffers[2] = {m_setStateCmdBuffer, m_subpassCmdBuffer};
    ...
    VkImageMemoryBarrier imageBarrier{VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                                      ...
                                      m_colorImage.get(),
                                      {VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1}};
    m_vk.cmdPipelineBarrier(m_primaryCmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
    ...
    m_vk.cmdCopyImageToBuffer(m_primaryCmdBuffer, m_colorImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    VkBufferMemoryBarrier bufferBarrier{VK_STRUCTURE_TYPE_BUFFER_MEMORY_BARRIER,
                                        ...
                                        0,
                                        VK_WHOLE_SIZE};
    m_vk.cmdPipelineBarrier(m_primaryCmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0, NULL,
                            1, &bufferBarrier, 0, NULL);

    // End and submit primary command buffer.
    VK_CHECK(m_vk.endCommandBuffer(m_primaryCmdBuffer));
    VkSubmitInfo submitInfo{VK_STRUCTURE_TYPE_SUBMIT_INFO, NULL, 0, NULL, NULL, 1, &m_primaryCmdBuffer, 0, NULL};
...
{
public:
    InheritanceTestCase(tcu::TestContext &testCtx, InheritanceMode inheritanceMode, const char *name,
