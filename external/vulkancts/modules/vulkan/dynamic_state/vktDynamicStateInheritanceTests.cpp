...
void InheritanceTestInstance::startRenderCmds(const TestGeometry &geometry)
{
    ...
    case kInheritFromSecondary:
        ...
        m_vk.cmdSetViewport(m_setStateCmdBuffer.get(), 0, uint32_t(geometry.viewports.size()), &geometry.viewports[0]);
    ...
    if (m_inheritanceMode == kInheritanceDisabled)
    {
        m_vk.cmdSetViewport(m_subpassCmdBuffer.get(), 0, uint32_t(geometry.viewports.size()), &geometry.viewports[0]);
    ...
    if (!m_rectanglePipelines[staticViewportCount].wasBuild())
    {
        ...
        m_rectanglePipelines[staticViewportCount]
            ...
            .setupPreRasterizationShaderState(viewports, scissors, *m_rectanglePipelineLayout, *m_renderPass, 0u,
                                              *m_vertModule, &pipelinestate::rasterization, DE_NULL, DE_NULL,
                                              *m_geomModule)
    ...
    case kInheritFromPrimary:
        ...
        m_vk.cmdSetViewport(m_primaryCmdBuffer.get(), 0, uint32_t(geometry.viewports.size()), &geometry.viewports[0]);
...
public:
    InheritanceTestCase(tcu::TestContext &testCtx, vk::PipelineConstructionType pipelineConstructionType,
                        InheritanceMode inheritanceMode, const char *name, const char *description)
