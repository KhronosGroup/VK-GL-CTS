...
void DynamicStateInheritanceTests::init (void)
{
	...
	addChild(new InheritanceTestCase(m_testCtx, m_pipelineConstructionType, kInheritFromSecondaryNested,
			 "nested", "Inherit viewport/scissor from another secondary command buffer"));
	...
	addChild(new InheritanceTestCase(m_testCtx, m_pipelineConstructionType, kInheritFromSecondaryNestedWithCount,
			 "nested_with_count", "Inherit viewport/scissor with count from another secondary command buffer within a secondary buffer"));
#endif // CTS_USES_VULKANSC
}
