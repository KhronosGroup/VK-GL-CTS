...
{
    vkd->cmdSetStencilOpEXT(cmdBuffer, VK_STENCIL_FRONT_AND_BACK, VK_STENCIL_OP_ZERO, VK_STENCIL_OP_INCREMENT_AND_CLAMP,
...
void setCoarseSamplingOrder(const DeviceInterface *vkd, VkCommandBuffer cmdBuffer, const DynamicStateData *)
{
    ...
    const VkCoarseSampleOrderCustomNV order = {
        ...
        1u,                                                      // uint32_t sampleLocationCount;
...
{
public:
    DynamicStateComputeCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
