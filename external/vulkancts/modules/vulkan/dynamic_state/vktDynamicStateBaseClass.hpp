...
protected:
    ...
    void beginRenderPassWithClearColor(const vk::VkClearColorValue &clearColor, const bool skipBeginCmdBuffer = false,
                                       const bool previousTransfer = false);

    void setDynamicViewportState(const uint32_t width, const uint32_t height);

    void setDynamicViewportState(uint32_t viewportCount, const vk::VkViewport *pViewports,
    ...
    void pushVertexOffset(const uint32_t vertexOffset, const vk::VkPipelineLayout pipelineLayout,
