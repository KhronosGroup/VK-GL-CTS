...
public:
    ...
    virtual void initPipeline(const vk::VkDevice device)
    {
        ...
        const vk::Move<vk::VkShaderModule> vs(
            m_isMesh ? vk::Move<vk::VkShaderModule>() :
                       createShaderModule(m_vk, device, binaries.get(m_vertexShaderName), 0));
        const vk::Move<vk::VkShaderModule> gs(
            m_isMesh ? vk::Move<vk::VkShaderModule>() :
                       createShaderModule(m_vk, device, binaries.get(m_geometryShaderName), 0));
        const vk::Move<vk::VkShaderModule> ms(m_isMesh ?
                                                  vk::createShaderModule(m_vk, device, binaries.get(m_meshShaderName)) :
        ...
        if (m_isMesh)
        {
            m_pipeline.setupPreRasterizationMeshShaderState(
                viewports, scissors, *m_pipelineLayout, *m_renderPass, 0u, DE_NULL, *ms,
...
void DynamicStateVPTests::init(void)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        addChild(new InstanceFactory<ViewportParamTestInstance, FunctionSupport0>(
            m_testCtx, "viewport" + nameSuffix, "Set viewport which is twice bigger than screen size" + descSuffix,
            m_pipelineConstructionType, shaderPaths, checkSupportFunc));
        addChild(new InstanceFactory<ScissorParamTestInstance, FunctionSupport0>(
            m_testCtx, "scissor" + nameSuffix,
            "Perform a scissor test on 1/4 bottom-left part of the surface" + descSuffix, m_pipelineConstructionType,
            shaderPaths, checkSupportFunc));
        ...
        addChild(new InstanceFactory<ViewportArrayTestInstance, FunctionSupport0>(
            m_testCtx, "viewport_array" + nameSuffix, "Multiple viewports and scissors" + descSuffix,
            m_pipelineConstructionType, shaderPaths, checkSupportFunc));
    }
}
