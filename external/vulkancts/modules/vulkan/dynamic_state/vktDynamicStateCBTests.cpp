...
public:
    ...
    virtual void initPipeline(const vk::VkDevice device)
    {
        ...
        const vk::Move<vk::VkShaderModule> ms(m_isMesh ?
                                                  createShaderModule(m_vk, device, binaries.get(m_meshShaderName), 0) :
...
void DynamicStateCBTests::init(void)
{
...
#ifndef CTS_USES_VULKANSC
    {
        ...
        addChild(new InstanceFactory<BlendConstantsTestInstance, FunctionSupport0>(
            m_testCtx, "blend_constants_mesh", "Check if blend constants are working properly in mesh shaders",
            m_pipelineConstructionType, shaderPaths, checkMeshShaderSupport));
    }
#endif // CTS_USES_VULKANSC
}
