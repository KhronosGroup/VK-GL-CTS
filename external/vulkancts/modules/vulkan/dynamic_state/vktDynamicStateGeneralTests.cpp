...
void DynamicStateGeneralTests::init(void)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        addChild(new InstanceFactory<StateSwitchTestInstance, FunctionSupport0>(
            m_testCtx, "state_switch" + nameSuffix,
            "Perform multiple draws with different VP states (scissor test)" + descSuffix, m_pipelineConstructionType,
            shaderPaths, checkSupportFunc));
        addChild(new InstanceFactory<BindOrderTestInstance, FunctionSupport0>(
            m_testCtx, "bind_order" + nameSuffix,
            "Check if binding order is not important for pipeline configuration" + descSuffix,
            m_pipelineConstructionType, shaderPaths, checkSupportFunc));
        if (!isMesh)
            addChild(new InstanceFactory<StatePersistenceTestInstance>(
                m_testCtx, "state_persistence" + nameSuffix,
                "Check if bound states are persistent across pipelines" + descSuffix, m_pipelineConstructionType,
                shaderPaths));
    }
}
