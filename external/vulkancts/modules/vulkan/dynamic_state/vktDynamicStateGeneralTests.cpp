...
public:
    ...
    virtual void initPipeline(const vk::VkDevice device)
    {
        ...
        pipelineCreateInfo_1.addShader(
        ...
        pipelineCreateInfo_1.addState(
            PipelineCreateInfo::InputAssemblerState(vk::VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP));
        ...
        pipelineCreateInfo_2.addShader(
            PipelineCreateInfo::PipelineShaderStage(*vs, "main", vk::VK_SHADER_STAGE_VERTEX_BIT));
        pipelineCreateInfo_2.addShader(
            PipelineCreateInfo::PipelineShaderStage(*fs, "main", vk::VK_SHADER_STAGE_FRAGMENT_BIT));
...
void DynamicStateGeneralTests::init(void)
{
    ...
    addChild(new InstanceFactory<StateSwitchTestInstance>(
        m_testCtx, "state_switch", "Perform multiple draws with different VP states (scissor test)", shaderPaths));
    addChild(new InstanceFactory<BindOrderTestInstance>(
        m_testCtx, "bind_order", "Check if binding order is not important for pipeline configuration", shaderPaths));
    addChild(new InstanceFactory<StatePersistenceTestInstance>(
        m_testCtx, "state_persistence", "Check if bound states are persistent across pipelines", shaderPaths));
}
