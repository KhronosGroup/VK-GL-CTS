    ...
    void initialize(void)
    {
        ...
        const vk::Move<vk::VkShaderModule> vs =
            (m_isMesh ? vk::Move<vk::VkShaderModule>() :
                        vk::createShaderModule(m_vk, device, binaries.get(m_vertexShaderName)));
        const vk::Move<vk::VkShaderModule> ms =
            (m_isMesh ? vk::createShaderModule(m_vk, device, binaries.get(m_meshShaderName)) :
                        vk::Move<vk::VkShaderModule>());
        const vk::Move<vk::VkShaderModule> fs =
            vk::createShaderModule(m_vk, device, binaries.get(m_fragmentShaderName));
...
public:
    DepthBiasNonZeroCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
DepthBiasNonZeroCase::DepthBiasNonZeroCase(tcu::TestContext &testCtx, const std::string &name,
                                           const std::string &description, const DepthBiasNonZeroParams &params)
...
tcu::TestStatus DepthBiasNonZeroInstance::iterate(void)
{
    ...
    const auto renderPass =
        vk::makeRenderPass(vkd, device, colorFormat, depthFormat, vk::VK_ATTACHMENT_LOAD_OP_CLEAR,
                           vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
    ...
    const auto framebuffer =
        vk::makeFramebuffer(vkd, device, renderPass.get(), static_cast<uint32_t>(attachments.size()),
    ...
    if (m_params.useMeshShaders)
    {
        pipeline = vk::makeGraphicsPipeline(
            vkd, device, pipelineLayout.get(), DE_NULL, meshModule.get(), fragModule.get(), renderPass.get(), viewports,
            scissors, 0u /*subpass*/, &rasterizationInfo, nullptr, &depthStencilStateInfo, nullptr, &dynamicStateInfo);
    }
    else
#endif // CTS_USES_VULKANSC
    {
        pipeline = vk::makeGraphicsPipeline(
            vkd, device, pipelineLayout.get(), vertModule.get(), DE_NULL, DE_NULL, DE_NULL, fragModule.get(), // shaders
            renderPass.get(), viewports, scissors, vk::VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0u /*subpass*/,
            0u /*patchControlPoints*/, &vertexInputInfo, &rasterizationInfo, nullptr, &depthStencilStateInfo, nullptr,
            &dynamicStateInfo);
    }
    ...
    vk::beginRenderPass(vkd, cmdBuffer, renderPass.get(), framebuffer.get(), scissors.at(0),
                        static_cast<uint32_t>(clearColors.size()), de::dataOrNull(clearColors));
...
void DynamicStateRSTests::init(void)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        addChild(new InstanceFactory<DepthBiasParamTestInstance, FunctionSupport0>(
            m_testCtx, "depth_bias" + nameSuffix, "Test depth bias functionality" + descSuffix,
            m_pipelineConstructionType, shaderPaths, (isMesh ? checkMeshShaderSupport : checkNothing)));
        addChild(new InstanceFactory<DepthBiasClampParamTestInstance, FunctionSupport0>(
            m_testCtx, "depth_bias_clamp" + nameSuffix, "Test depth bias clamp functionality" + descSuffix,
        ...
        addChild(new InstanceFactory<LineWidthParamTestInstance, FunctionSupport0>(
            m_testCtx, "line_width" + nameSuffix,
            "Draw a line with width set to max defined by physical device" + descSuffix, m_pipelineConstructionType,
            shaderPaths, (isMesh ? checkMeshAndWideLinesSupport : checkWideLinesSupport)));
