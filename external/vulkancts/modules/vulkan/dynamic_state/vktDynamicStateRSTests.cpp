...
public:
    DepthBiasParamTestInstance(Context &context, vk::PipelineConstructionType pipelineConstructionType,
                               ShaderMap shaders)
        : DepthBiasBaseCase(context, pipelineConstructionType, shaders[glu::SHADERTYPE_VERTEX],
...
void DynamicStateRSTests::init(void)
{
    ...
    addChild(new InstanceFactory<DepthBiasParamTestInstance>(m_testCtx, "depth_bias", "Test depth bias functionality",
                                                             m_pipelineConstructionType, shaderPaths));
    addChild(new InstanceFactory<DepthBiasClampParamTestInstance, FunctionSupport0>(
        m_testCtx, "depth_bias_clamp", "Test depth bias clamp functionality", m_pipelineConstructionType, shaderPaths,
        checkDepthBiasClampSupport));
    addChild(new InstanceFactory<LineWidthParamTestInstance, FunctionSupport0>(
        m_testCtx, "line_width", "Draw a line with width set to max defined by physical device",
        m_pipelineConstructionType, shaderPaths, checkWideLinesSupport));
