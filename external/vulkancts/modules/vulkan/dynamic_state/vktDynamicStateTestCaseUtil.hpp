...
public:
    InstanceFactory(tcu::TestContext &testCtx, const std::string &name,
                    const vk::PipelineConstructionType pipelineConstructionType, const ShaderMap &shaderPaths)
        ...
        for (ShaderMap::const_iterator i = m_shaderPaths.begin(); i != m_shaderPaths.end(); ++i)
        {
            if (i->second)
            {
                programCollection.glslSources.add(i->second)
                    ...
                    << ((i->first == glu::SHADERTYPE_TASK || i->first == glu::SHADERTYPE_MESH) ? spv14Options :
                                                                                                 defaultOptions);
            }
        }
    }
