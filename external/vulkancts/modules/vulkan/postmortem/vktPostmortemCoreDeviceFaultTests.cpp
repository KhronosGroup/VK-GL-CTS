...
static Move<VkDevice> createDeviceFaultDevice(Context &context, VkPhysicalDeviceFaultFeaturesKHR deviceFaultFeatures,
                                              TestFlags testOptions)
{
    ...
    const CustomInstance instance =
        createCustomInstanceWithExtensions(context, getRequiredExtensions(useValidation, context.getUsedApiVersion()));
    ...
    return createCustomDevice(useValidation, platformInterface, instance, instanceInterface, physicalDevice,
                              &deviceCreateInfo);
}
...
public:
    ContextWrapper(Context &ctx, TestFlags testOptions)
        ...
        , m_customDevice(createDeviceFaultDevice(ctx, m_faultFeatures, testOptions))
        , m_deviceInterface(m_context.getPlatformInterface(), m_context.getInstance(), *m_customDevice,
    ...
    {
        return m_deviceInterface.getDeviceFaultReportsKHR(*m_customDevice, timeout, pFaultCounts, pFaultInfo);
    }

    [[nodiscard]] VkResult getDeviceFaultDebugInfoKHR(VkDeviceFaultDebugInfoKHR *pDebugInfo) const
    {
        return m_deviceInterface.getDeviceFaultDebugInfoKHR(*m_customDevice, pDebugInfo);
    }
...
VkResult ContextWrapper::submitShaderAbortDeviceFault() const
{
    ...
    vk::VkQueue queue(getDeviceQueue(m_deviceInterface, *m_customDevice, queueFamilyIndex, 0));
    vk::SimpleAllocator allocator(m_deviceInterface, *m_customDevice,
    ...
    de::MovePtr<BufferWithMemory> outBuffer = de::MovePtr<BufferWithMemory>(new BufferWithMemory(
        m_deviceInterface, *m_customDevice, allocator, outBufferInfo, MemoryRequirement::HostVisible));

    // create descriptor set layout
    auto descriptorSetLayout = DescriptorSetLayoutBuilder()
                                   .addSingleBinding(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_COMPUTE_BIT)
                                   .build(m_deviceInterface, *m_customDevice);

    // create descriptor pool
    auto descriptorPool =
        DescriptorPoolBuilder()
            .addType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1u)
            .build(m_deviceInterface, *m_customDevice, VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT, 1u);
    ...
    auto descriptorSet = allocateDescriptorSet(m_deviceInterface, *m_customDevice, &allocInfo);
    ...
    DescriptorSetUpdateBuilder()
        ...
        .update(m_deviceInterface, *m_customDevice);

    // create compute pipeline
    const Unique<VkShaderModule> shaderModule(
        createShaderModule(m_deviceInterface, *m_customDevice, m_context.getBinaryCollection().get("comp"), 0u));
    const Unique<VkPipelineLayout> pipelineLayout(
        makePipelineLayout(m_deviceInterface, *m_customDevice, 1u, &(*descriptorSetLayout), 0, nullptr));
    const Unique<VkPipeline> pipeline(
        makeComputePipeline(m_deviceInterface, *m_customDevice, *pipelineLayout, *shaderModule));

    // create command buffer
    const Unique<VkCommandPool> cmdPool(makeCommandPool(m_deviceInterface, *m_customDevice, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(m_deviceInterface, *m_customDevice, *cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));

    beginCommandBuffer(m_deviceInterface, *cmdBuffer, VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT);

    m_deviceInterface.cmdBindPipeline(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *pipeline);
    m_deviceInterface.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0, 1,
                                            &descriptorSet.get(), 0, 0);
    m_deviceInterface.cmdDispatch(*cmdBuffer, 1, 1, 1);

    endCommandBuffer(m_deviceInterface, *cmdBuffer);
    ...
    const auto fence = createFence(m_deviceInterface, *m_customDevice);
    VK_CHECK(m_deviceInterface.queueSubmit(queue, 1u, &submitInfo, *fence));
    m_deviceInterface.waitForFences(*m_customDevice, 1, &*fence, VK_TRUE, (~0ull));

    if ((m_testOptions & USE_DEVICE_WAIT_IDLE) != 0)
        return m_deviceInterface.deviceWaitIdle(*m_customDevice);
    else
