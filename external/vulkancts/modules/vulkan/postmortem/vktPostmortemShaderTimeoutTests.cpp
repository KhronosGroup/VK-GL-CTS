...
public:
    ShaderTimeoutCase(tcu::TestContext &testCtx, const std::string &name, uint32_t iterations)
        : TestCase(testCtx, name, "Long-running compute shader")
...
{
    de::MovePtr<tcu::TestCaseGroup> timeoutGroup(
        new tcu::TestCaseGroup(testCtx, "shader_timeout", "Shader timeout tests."));
    for (int i = 0; i < 16; ++i)
    {
