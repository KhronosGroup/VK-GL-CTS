...
public:
    ...
    static UseAfterFreeTestCase *UBOToSSBOInvertCase(tcu::TestContext &testCtx, const std::string &name,
                                                     const std::string &description, const uint32_t numValues,
...
private:
    UseAfterFreeTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         const uint32_t numValues, const tcu::IVec3 &localSize, const tcu::IVec3 &workSize,
...
UseAfterFreeTestCase::UseAfterFreeTestCase(tcu::TestContext &testCtx, const std::string &name,
                                           const std::string &description, const uint32_t numValues,
                                           const tcu::IVec3 &localSize, const tcu::IVec3 &workSize,
                                           const BufferType bufferType)
...
{
    return makeComputePipeline(vk, device, pipelineLayout, static_cast<VkPipelineCreateFlags>(0u), shaderModule,
...
{
    de::MovePtr<tcu::TestCaseGroup> useAfterFreeGroup(
        new tcu::TestCaseGroup(testCtx, "use_after_free", "Use buffer after free."));

    useAfterFreeGroup->addChild(UseAfterFreeTestCase::UBOToSSBOInvertCase(testCtx, "ubo_to_ssbo_single_invocation",
                                                                          "Copy from UBO to SSBO, inverting bits", 256,
                                                                          tcu::IVec3(1, 1, 1), tcu::IVec3(1, 1, 1)));
    useAfterFreeGroup->addChild(UseAfterFreeTestCase::CopyInvertSSBOCase(testCtx, "ssbo_to_ssbo_single_invocation",
                                                                         "Copy from SSBO to SSBO, inverting bits", 256,
                                                                         tcu::IVec3(1, 1, 1), tcu::IVec3(1, 1, 1)));
