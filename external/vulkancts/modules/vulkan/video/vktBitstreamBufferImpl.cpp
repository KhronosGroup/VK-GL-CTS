...
VkResult BitstreamBufferImpl::Create(DeviceContext *devctx, uint32_t queueFamilyIndex, VkDeviceSize bufferSize,
                                     ...
                                     const VkVideoProfileListInfoKHR *profileList, bool resourcesWithoutProfiles)
{
    VkSharedBaseObj<BitstreamBufferImpl> vkBitstreamBuffer(new BitstreamBufferImpl(
        devctx, queueFamilyIndex, bufferOffsetAlignment, bufferSizeAlignment, profileList, resourcesWithoutProfiles));
                                                ...
                                                VkDeviceSize srcOffset, VkDeviceSize dstOffset, VkDeviceSize size)
{
    ...
    const uint8_t *readData = sourceBuffer->GetReadOnlyDataPtr(srcOffset, maxSize);
    DE_ASSERT(readData);
    size                   = std::min(size, maxSize);
    auto *bitstreamBasePtr = static_cast<uint8_t *>(m_bitstreamBuffer->getAllocation().getHostPtr());
    deMemcpy(bitstreamBasePtr + dstOffset, readData, size);
    return size;
}
