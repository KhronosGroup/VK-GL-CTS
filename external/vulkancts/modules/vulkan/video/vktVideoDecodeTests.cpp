...
public:
    ...
    VideoDevice::VideoDeviceFlags requiredDeviceFlags() const
    {
        return VideoDevice::VIDEO_DEVICE_FLAG_REQUIRE_SYNC2_OR_NOT_SUPPORTED |
               (hasOption(DecoderOption::UseStatusQueries) ?
...
void createParser(const TestDefinition *params, VideoBaseDecoder *decoder,
...
static MovePtr<VideoBaseDecoder> decoderFromTestDefinition(DeviceContext *devctx, const TestDefinition &test)
{
    ...
    VK_CHECK(
        VulkanVideoFrameBuffer::Create(devctx, test.hasOption(DecoderOption::UseStatusQueries), vkVideoFrameBuffer));
    ...
    const DecodedFrame *decodeFrame()
    {
        ...
        if (videoLoggingEnabled())
            std::cout << "<= Wait on picIdx: " << pLastDecodedFrame->pictureIndex
                      ...
                      << "\tdecodeOrder: " << pLastDecodedFrame->decodeOrder << "\ttimestamp "
                      << pLastDecodedFrame->timestamp << "\tdstImageView "
                      << (pLastDecodedFrame->outputImageView ?
                              pLastDecodedFrame->outputImageView->GetImageResource()->GetImage() :
                              VK_NULL_HANDLE)
                      << std::endl;
...
tcu::TestStatus InterleavingDecodeTestInstance::iterate(void)
{
    ...
    for (int i = 0; i < m_testDefinitions.size(); i++)
    {
        processors.push_back(MovePtr<FrameProcessor>(new FrameProcessor(
            &m_deviceContext, m_testDefinitions[i].get(), m_decoders[i].get(), m_context.getTestContext().getLog())));
    }
