...
public:
    ...
    MovePtr<vector<uint8_t>> loadTestVideoData(void);
...
MovePtr<vector<uint8_t>> VideoDecodeTestInstance::loadTestVideoData(void)
{
...
tcu::TestStatus VideoDecodeTestInstance::iterateSingleFrame(void)
{
    ...
    MovePtr<vector<uint8_t>> videoData      = loadTestVideoData();
...
public:
    ...
    MovePtr<vector<uint8_t>> loadTestVideoData(bool primary);
...
MovePtr<vector<uint8_t>> DualVideoDecodeTestInstance::loadTestVideoData(bool primary)
{
    switch (m_caseDef.testType)
    {
    case TEST_TYPE_H264_DECODE_INTERLEAVED:
        return primary ? loadVideoDataClipA() : loadVideoDataClipB();
    case TEST_TYPE_H264_BOTH_DECODE_ENCODE_INTERLEAVED:
        return loadVideoDataClipA();
    case TEST_TYPE_H264_H265_DECODE_INTERLEAVED:
        return primary ? loadVideoDataClipA() : loadVideoDataClipD();
    default:
        TCU_THROW(InternalError, "Unknown testType");
    }
}
...
tcu::TestStatus DualVideoDecodeTestInstance::iterate(void)
{
    ...
    const uint32_t queueFamilyIndexTransfer = getQueueFamilyIndexTransfer();
    Allocator &allocator                    = getAllocator();
    MovePtr<vector<uint8_t>> videoData1     = loadTestVideoData(true);
    MovePtr<vector<uint8_t>> videoData2     = loadTestVideoData(false);
