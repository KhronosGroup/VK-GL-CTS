...
struct DecodeTestParam
{
    ...
    {TEST_TYPE_H264_DECODE_I, {CLIP_A, 1, Option::Default}},
    {TEST_TYPE_H264_DECODE_I_P, {CLIP_A, 2, Option::Default}},
    {TEST_TYPE_H264_DECODE_I_P_NOT_MATCHING_ORDER, {CLIP_A, 2, Option::CachedDecoding}},
    {TEST_TYPE_H264_DECODE_CLIP_A, {CLIP_A, ALL_FRAMES, Option::Default}},
    {TEST_TYPE_H264_DECODE_I_P_B_13, {CLIP_H264_4K_26_IBP_MAIN, ALL_FRAMES, Option::Default}},
    {TEST_TYPE_H264_DECODE_I_P_B_13_NOT_MATCHING_ORDER, {CLIP_H264_4K_26_IBP_MAIN, ALL_FRAMES, Option::CachedDecoding}},
    {TEST_TYPE_H264_DECODE_QUERY_RESULT_WITH_STATUS, {CLIP_A, ALL_FRAMES, Option::UseStatusQueries}},
    {TEST_TYPE_H264_DECODE_RESOLUTION_CHANGE, {CLIP_C, ALL_FRAMES, Option::Default}},
    {TEST_TYPE_H264_DECODE_RESOLUTION_CHANGE_DPB, {CLIP_C, ALL_FRAMES, Option::RecreateDPBImages}},

    {TEST_TYPE_H265_DECODE_I, {CLIP_D, 1, Option::Default}},
    {TEST_TYPE_H265_DECODE_I_P, {CLIP_D, 2, Option::Default}},
    {TEST_TYPE_H265_DECODE_I_P_NOT_MATCHING_ORDER, {CLIP_D, 2, Option::CachedDecoding}},
    {TEST_TYPE_H265_DECODE_I_P_B_13, {CLIP_JELLY_HEVC, ALL_FRAMES, Option::Default}},
    {TEST_TYPE_H265_DECODE_I_P_B_13_NOT_MATCHING_ORDER, {CLIP_JELLY_HEVC, ALL_FRAMES, Option::CachedDecoding}},
    {TEST_TYPE_H265_DECODE_CLIP_D, {CLIP_D, ALL_FRAMES, Option::Default}},
};
...
struct InterleavingDecodeTestParams
{
    ...
    {TEST_TYPE_H264_DECODE_INTERLEAVED,
     {CLIP_A, ALL_FRAMES, Option::CachedDecoding},
     {CLIP_A, ALL_FRAMES, Option::CachedDecoding}},
    {TEST_TYPE_H264_H265_DECODE_INTERLEAVED,
     {CLIP_A, ALL_FRAMES, Option::CachedDecoding},
     {CLIP_D, ALL_FRAMES, Option::CachedDecoding}},
};

class TestDefinition
{
public:
    static MovePtr<TestDefinition> create(DecodeTestParam params, uint32_t baseSeed)
    {
        return MovePtr<TestDefinition>(new TestDefinition(params, baseSeed));
    }

    TestDefinition(DecodeTestParam params, uint32_t baseSeed)
        : m_params(params)
        , m_info(clipInfo(params.stream.clip))
        , m_hash(baseSeed)
    {
        m_profile = VkVideoCoreProfile(m_info->profile.codecOperation, m_info->profile.subsamplingFlags,
                                       m_info->profile.lumaBitDepth, m_info->profile.chromaBitDepth,
                                       m_info->profile.profileIDC);
    ...
    VideoDevice::VideoDeviceFlags requiredDeviceFlags() const
    {
        return VideoDevice::VIDEO_DEVICE_FLAG_REQUIRE_SYNC2_OR_NOT_SUPPORTED |
               (hasOption(Option::UseStatusQueries) ?
                    VideoDevice::VIDEO_DEVICE_FLAG_QUERY_WITH_STATUS_FOR_DECODE_SUPPORT :
                    VideoDevice::VIDEO_DEVICE_FLAG_NONE);
    }
...
static MovePtr<VideoBaseDecoder> decoderFromTestDefinition(DeviceContext *devctx, const TestDefinition &test)
{
    ...
    VK_CHECK(VulkanVideoFrameBuffer::Create(devctx, test.hasOption(Option::UseStatusQueries), vkVideoFrameBuffer));
    ...
    params.queryDecodeStatus                 = test.hasOption(Option::UseStatusQueries);
    params.outOfOrderDecoding                = test.hasOption(Option::CachedDecoding);
    params.alwaysRecreateDPB                 = test.hasOption(Option::RecreateDPBImages);
...
de::MovePtr<vkt::ycbcr::MultiPlaneImageData> getDecodedImage(DeviceContext &devctx, VkImageLayout layout,
                                                             const DecodedFrame *frame)
{
    ...
    const VkImageSubresourceRange imageSubresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, videoImageLayerIndex, 1);
    ...
    vkt::ycbcr::downloadImage(vkd, device, queueFamilyIndexTransfer, devctx.allocator(), image,
                              multiPlaneImageData.get(), 0, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, videoImageLayerIndex);
    ...
    , m_deviceContext(&m_context, &m_videoDevice)
{
    VkDevice device =
        getDeviceSupportingQueue(VK_QUEUE_VIDEO_DECODE_BIT_KHR | VK_QUEUE_TRANSFER_BIT,
                                 m_testDefinition->getCodecOperation(), m_testDefinition->requiredDeviceFlags());
...
tcu::TestStatus VideoDecodeTestInstance::iterate()
{
    ...
    FrameProcessor processor(&m_deviceContext, m_testDefinition->getClipFilename(),
                             m_testDefinition->getCodecOperation(), m_testDefinition->extensionProperties(),
                             m_decoder.get(), m_context.getTestContext().getLog());
    std::vector<int> incorrectFrames;
    std::vector<int> correctFrames;

    if (m_testDefinition->hasOption(Option::CachedDecoding))
    {
    ...
    for (int frameNumber = 0; frameNumber < m_testDefinition->framesToCheck(); frameNumber++)
    {
        ...
        auto resultImage = getDecodedImage(m_deviceContext,
                                           m_decoder->dpbAndOutputCoincide() ? VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR :
...
tcu::TestStatus InterleavingDecodeTestInstance::iterate(void)
{
    ...
    for (int i = 0; i < m_testDefinitions.size(); i++)
    {
        processors.push_back(MovePtr<FrameProcessor>(new FrameProcessor(
            &m_deviceContext, m_testDefinitions[i].get()->getClipFilename(),
            m_testDefinitions[i].get()->getCodecOperation(), m_testDefinitions[i].get()->extensionProperties(),
            m_decoders[i].get(), m_context.getTestContext().getLog())));
    }
    ...
    for (int i = 0; i < interleaveCacheSize; i++)
    {
        for (int decoderIdx = 0; decoderIdx < m_decoders.size(); decoderIdx++)
        {
            ...
            if (test->hasOption(Option::UseStatusQueries))
            {
    ...
    for (int i = 0; i < m_testDefinitions.size(); i++)
    {
        ...
        for (int frameNumber = 0; frameNumber < m_testDefinitions[i]->framesToCheck(); frameNumber++)
        {
            ...
            auto resultImage          = getDecodedImage(m_deviceContext,
                                               decoder->dpbAndOutputCoincide() ? VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR :
                                                                                          VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR,
                                                        frame);
#if FRAME_DUMP_DEBUG
            auto bytes = semiplanarToYV12(*resultImage);
...
tcu::TestCaseGroup *createVideoDecodeTests(tcu::TestContext &testCtx)
{
    ...
    for (const auto &decodeTest : g_DecodeTests)
    {
        ...
        uint32_t rngSeed     = baseSeed ^ deStringHash(testName);
