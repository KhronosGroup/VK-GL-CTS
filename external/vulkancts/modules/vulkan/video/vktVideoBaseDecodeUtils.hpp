...
public:
    ...
    VkResult CreateImage(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex, Allocator &allocator,
                         const VkImageCreateInfo *pImageCreateInfo, const MemoryRequirement memoryRequirement);
    VkResult StageImage(const DeviceInterface &vkd, VkDevice device, VkImageUsageFlags usage,
...
public:
    ...
    void deinit(void);
    void init(const DeviceInterface &vkd, VkDevice device);
...
public:
    ...
    int32_t init(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex, Allocator &allocator,
                 uint32_t numImages, const VkImageCreateInfo *pOutImageCreateInfo,
...
public:
    ...
    virtual int32_t InitImagePool(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex,
    ...
    virtual int32_t QueuePictureForDecode(int8_t picId, VulkanParserDecodePictureInfo *pDecodePictureInfo,
    ...
    virtual void GetImageResourcesByIndex(int32_t numResources, const int8_t *referenceSlotIndexes,
                                          VkVideoPictureResourceInfoKHR *videoPictureResources,
                                          PictureResourceInfo *pictureResourcesInfo, VkImageLayout newImageLayout);
    ...
    Move<VkQueryPool> CreateVideoQueries(const DeviceInterface &vkd, VkDevice device, uint32_t numSlots,
                                         const VkVideoProfileInfoKHR *pDecodeProfile);
    int32_t QueueDecodedPictureForDisplay(int8_t picId, DisplayPictureInfo *pDispInfo);
...
public:
    ...
    void MapPictureResource(NvidiaVulkanPictureBase *pPic, int32_t dpbSlot);
...
public:
    ...
    static NvidiaParserVideoPictureParameters *Create(const DeviceInterface &vkd, VkDevice device,
    ...
    VkResult Update(const DeviceInterface &vkd, const StdVideoPictureParametersSet *pSpsStdPictureParametersSet,
                    const StdVideoPictureParametersSet *pPpsStdPictureParametersSet);
...
public:
    ...
    ~VulkanVideoBitstreamBuffer();
    VkResult CreateVideoBitstreamBuffer(const DeviceInterface &vkd, VkDevice device, Allocator &allocator,
                                        VkDeviceSize bufferSize, VkDeviceSize bufferOffsetAlignment,
                                        VkDeviceSize bufferSizeAlignment, void *pNext,
                                        const unsigned char *pBitstreamData = DE_NULL,
                                        VkDeviceSize bitstreamDataSize      = 0);
    VkResult CopyVideoBitstreamToBuffer(const DeviceInterface &vkd, VkDevice device,
                                        const unsigned char *pBitstreamData, VkDeviceSize bitstreamDataSize);
...
typedef vector<de::MovePtr<vector<uint8_t>>> HeapType;

#define ALLOC_HEAP_OBJECT_ARRAY(H, T, N) \
    ((T *)heap.emplace(H.end(), de::MovePtr<vector<uint8_t>>(new vector<uint8_t>(sizeof(T) * N)))->get()->data())
...
public:
    ...
    void initialize(const VkVideoCodecOperationFlagBitsKHR videoCodecOperation, const DeviceInterface &vkd,
                    const VkDevice device, const uint32_t queueFamilyIndexTransfer,
                    const uint32_t queueFamilyIndexDecode, Allocator &allocator);

    VkDevice getDevice(void);
    const DeviceInterface &getDeviceDriver(void);
    uint32_t getQueueFamilyIndexTransfer(void);
    uint32_t getQueueFamilyIndexDecode(void);
    VkQueue getQueueTransfer(void);
    VkQueue getQueueDecode(void);
    Allocator &getAllocator(void);

    void setDecodeParameters(bool randomOrSwapped, bool queryResultWithStatus, uint32_t frameCountTrigger,
                             bool submitAfter = true, uint32_t gopSize = 0, uint32_t dpbCount = 1);
    int32_t DecodeCachedPictures(VideoBaseDecoder *friendDecoder = DE_NULL, bool waitSubmitted = true);

protected:
    // NvidiaVulkanParserVideoDecodeClient callbacks
    virtual int32_t BeginSequence(const NvidiaVulkanParserSequenceInfo *pnvsi)
        override; // Returns max number of reference frames (always at least 2 for MPEG-2)
    virtual bool AllocPictureBuffer(
        INvidiaVulkanPicture **ppNvidiaVulkanPicture) override; // Returns a new INvidiaVulkanPicture interface
    virtual bool DecodePicture(NvidiaVulkanParserPictureData *pNvidiaVulkanParserPictureData)
        override; // Called when a picture is ready to be decoded
    ...
    uint32_t FillDpbH264State(const NvidiaVulkanParserPictureData *pNvidiaVulkanParserPictureData,
                              ...
                              StdVideoDecodeH264PictureInfoFlags currPicFlags, int32_t *pCurrAllocatedSlotIndex);
    ...
    int8_t AllocateDpbSlotForCurrentH265(NvidiaVulkanPictureBase *pNvidiaVulkanPictureBase, bool isReference);
    ...
    int8_t SetPicDpbSlot(NvidiaVulkanPictureBase *pNvidiaVulkanPictureBase, int8_t dpbSlot);
    ...
    void SubmitQueue(VkCommandBufferSubmitInfoKHR *commandBufferSubmitInfo, VkSubmitInfo2KHR *submitInfo,
    ...
    void SubmitQueue(vector<VkCommandBufferSubmitInfoKHR> &commandBufferSubmitInfos, VkSubmitInfo2KHR *submitInfo,
                     const VkFence frameCompleteFence, const vector<VkFence> &frameConsumerDoneFence,
    ...
    virtual bool UpdatePictureParametersHandler(
        NvidiaVulkanPictureParameters *pNvidiaVulkanPictureParameters,
    ...
    virtual int32_t DecodePictureWithParameters(PerFrameDecodeParameters *pPicParams,
                                                VulkanParserDecodePictureInfo *pDecodePictureInfo, HeapType &heap);

    // Client methods
    virtual void Deinitialize(void);
    uint32_t GetNumDecodeSurfaces(VkVideoCodecOperationFlagBitsKHR codec, uint32_t minNumDecodeSurfaces, uint32_t width,
                                  uint32_t height);
    ...
    NvidiaParserVideoPictureParameters *CheckStdObjectAfterUpdate(
    ...
    NvidiaParserVideoPictureParameters *AddPictureParameters(
...
protected:
    ...
    VkDevice m_device;
    uint32_t m_queueFamilyIndexTransfer;
    uint32_t m_queueFamilyIndexDecode;
