...
public:
    ...
    VkResult CreateImage(DeviceContext &vkDevCtx, const VkImageCreateInfo *pDpbImageCreateInfo,
                         const VkImageCreateInfo *pOutImageCreateInfo, uint32_t imageIndex,
    ...
    bool GetImageSetNewLayout(VkImageLayout newDpbImageLayout, VkVideoPictureResourceInfoKHR *pDpbPictureResource,
...
public:
    ...
    int32_t init(DeviceContext &vkDevCtx, const VkVideoProfileInfoKHR *pDecodeProfile, uint32_t numImages,
                 ...
                 bool useImageArray = false, bool useImageViewArray = false, bool useSeparateOutputImages = false,
                 bool useLinearOutput = false, bool resourcesWithoutProfiles = false);
    ...
    VkResult GetImageSetNewLayout(DeviceContext &vkDevCtx, uint32_t imageIndex, VkImageLayout newDpbImageLayout,
                                  ...
                                  VulkanVideoFrameBuffer::PictureResourceInfo *pOutputPictureResourceInfo = nullptr)
    {
        ...
        bool validImage = m_perFrameDecodeResources[imageIndex].GetImageSetNewLayout(
            newDpbImageLayout, pDpbPictureResource, pDpbPictureResourceInfo, newOutputImageLayout,
            pOutputPictureResource, pOutputPictureResourceInfo);

        if (!validImage)
        {
            result = m_perFrameDecodeResources[imageIndex].CreateImage(
                vkDevCtx, &m_dpbImageCreateInfo, &m_outImageCreateInfo, imageIndex, m_imageArray, m_imageViewArray,
                m_usesSeparateOutputImage, m_usesLinearOutput);
...
public:
    ...
    VkResult CreateVideoQueries(uint32_t numSlots, DeviceContext &vkDevCtx, const VkVideoProfileInfoKHR *pDecodeProfile)
    {
    ...
    uint32_t FlushDisplayQueue()
    {
        ...
        {
            uint8_t pictureIndex = m_displayFrames.front();
    ...
    int32_t InitImagePool(const VkVideoProfileInfoKHR *pDecodeProfile, uint32_t numImages, VkFormat dpbImageFormat,
                          VkFormat outImageFormat, const VkExtent2D &maxImageExtent, VkImageUsageFlags dpbImageUsage,
                          VkImageUsageFlags outImageUsage, uint32_t queueFamilyIndex, bool useImageArray,
                          bool useImageViewArray, bool useSeparateOutputImage, bool useLinearOutput) override;
    ...
    size_t GetDisplayedFrameCount() const override
    {
        return m_displayFrames.size();
    }

    // dequeue
    int32_t DequeueDecodedPicture(DecodedFrame *pDecodedFrame) override;

    int32_t ReleaseDisplayedPicture(DecodedFrameRelease **pDecodedFramesRelease, uint32_t numFramesToRelease) override;

    int32_t GetDpbImageResourcesByIndex(
        uint32_t numResources, const int8_t *referenceSlotIndexes, VkVideoPictureResourceInfoKHR *dpbPictureResources,
    ...
    int32_t ReleaseImageResources(uint32_t numResources, const uint32_t *indexes) override
    {
...
VkResult VulkanVideoFrameBuffer::Create(DeviceContext *vkDevCtx, bool supportsQueries, bool resourcesWithoutProfiles,
                                        VkSharedBaseObj<VulkanVideoFrameBuffer> &vkVideoFrameBuffer)
{
    VkSharedBaseObj<VkVideoFrameBuffer> videoFrameBuffer(
        new VkVideoFrameBuffer(*vkDevCtx, supportsQueries, resourcesWithoutProfiles));
    if (videoFrameBuffer)
    {
...
int32_t VkVideoFrameBuffer::QueuePictureForDecode(
    int8_t picId, VkParserDecodePictureInfo *pDecodePictureInfo,
    VulkanVideoFrameBuffer::ReferencedObjectsInfo *pReferencedObjectsInfo,
    VulkanVideoFrameBuffer::FrameSynchronizationInfo *pFrameSynchronizationInfo)
{
    ...
    if (videoLoggingEnabled())
    {
        ...
        tcu::print(";;; %d queue decode: %d: decoderOrder=%d displayOrder=%d completeFence=%d consumerDoneFence=%d "
                   "completeSem=%d consumerDoneSem=%d\n",
                   counter++, picId, (int)m_perFrameDecodeImageSet[picId].m_decodeOrder,
                   m_perFrameDecodeImageSet[picId].m_displayOrder,
                   (uint32_t)m_perFrameDecodeImageSet[picId].m_frameCompleteFence.getInternal(),
                   (uint32_t)m_perFrameDecodeImageSet[picId].m_frameConsumerDoneFence.getInternal(),
                   (uint32_t)m_perFrameDecodeImageSet[picId].m_frameCompleteSemaphore.getInternal(),
                   (uint32_t)m_perFrameDecodeImageSet[picId].m_frameConsumerDoneSemaphore.getInternal());
    }
...
int32_t VkVideoFrameBuffer::ReleaseDisplayedPicture(DecodedFrameRelease **pDecodedFramesRelease,
...
int32_t VkVideoFrameBuffer::GetDpbImageResourcesByIndex(
    uint32_t numResources, const int8_t *referenceSlotIndexes, VkVideoPictureResourceInfoKHR *dpbPictureResources,
    VulkanVideoFrameBuffer::PictureResourceInfo *dpbPictureResourcesInfo, VkImageLayout newDpbImageLayerLayout)
{
    ...
    for (unsigned int resId = 0; resId < numResources; resId++)
    {
        if ((uint32_t)referenceSlotIndexes[resId] < m_perFrameDecodeImageSet.size())
        {
            VkResult result = m_perFrameDecodeImageSet.GetImageSetNewLayout(
                m_vkDevCtx, referenceSlotIndexes[resId], newDpbImageLayerLayout, &dpbPictureResources[resId],
...
int32_t VkVideoFrameBuffer::GetCurrentImageResourceByIndex(
    int8_t picIdx, VkVideoPictureResourceInfoKHR *dpbPictureResource,
    VulkanVideoFrameBuffer::PictureResourceInfo *dpbPictureResourceInfo, VkImageLayout newDpbImageLayerLayout,
    VkVideoPictureResourceInfoKHR *outputPictureResource,
    VulkanVideoFrameBuffer::PictureResourceInfo *outputPictureResourceInfo, VkImageLayout newOutputImageLayerLayout)
{
    DE_ASSERT(dpbPictureResource);
    if ((uint32_t)picIdx < m_perFrameDecodeImageSet.size())
    {
        VkResult result = m_perFrameDecodeImageSet.GetImageSetNewLayout(
...
int32_t VkVideoFrameBuffer::InitImagePool(const VkVideoProfileInfoKHR *pDecodeProfile, uint32_t numImages,
                                          ...
                                          bool useImageArray, bool useImageViewArray, bool useSeparateOutputImage,
                                          bool useLinearOutput)
{
    ...
    int32_t imageSetCreateResult = m_perFrameDecodeImageSet.init(
        m_vkDevCtx, pDecodeProfile, numImages, dpbImageFormat, outImageFormat, maxImageExtent, dpbImageUsage,
        outImageUsage, queueFamilyIndex, useImageArray, useImageViewArray, useSeparateOutputImage, useLinearOutput,
        m_resourcesWithoutProfiles);
    m_numberParameterUpdates++;

    return imageSetCreateResult;
}

VkResult NvPerFrameDecodeResources::CreateImage(DeviceContext &vkDevCtx, const VkImageCreateInfo *pDpbImageCreateInfo,
                                                const VkImageCreateInfo *pOutImageCreateInfo, uint32_t imageIndex,
                                                VkSharedBaseObj<VkImageResource> &imageArrayParent,
                                                VkSharedBaseObj<VkImageResourceView> &imageViewArrayParent,
                                                bool useSeparateOutputImage, bool useLinearOutput)
{
    VkResult result = VK_SUCCESS;
    ...
    if (!ImageExist() || m_recreateImage)
    {
        ...
        if (!imageArrayParent)
        {
            result = VkImageResource::Create(vkDevCtx, pDpbImageCreateInfo, imageResource);
            if (result != VK_SUCCESS)
            {
                return result;
            }
        ...
        if (useSeparateOutputImage || useLinearOutput)
        {
            ...
            result = VkImageResource::Create(vkDevCtx, pOutImageCreateInfo, displayImageResource);
            if (result != VK_SUCCESS)
            {
                return result;
            }
...
void NvPerFrameDecodeResources::Deinit()
{
    ...
    {
        assert((m_frameCompleteFence == VK_NULL_HANDLE) && (m_frameConsumerDoneFence == VK_NULL_HANDLE) &&
               (m_frameCompleteSemaphore == VK_NULL_HANDLE) && (m_frameConsumerDoneSemaphore == VK_NULL_HANDLE) &&
               !m_frameDpbImageView && !m_outImageView);
        return;
    }
...
int32_t NvPerFrameDecodeImageSet::init(DeviceContext &vkDevCtx, const VkVideoProfileInfoKHR *pDecodeProfile,
                                       uint32_t numImages, VkFormat dpbImageFormat, VkFormat outImageFormat,
                                       const VkExtent2D &maxImageExtent, VkImageUsageFlags dpbImageUsage,
                                       VkImageUsageFlags outImageUsage, uint32_t queueFamilyIndex, bool useImageArray,
                                       ...
                                       bool resourcesWithoutProfiles)
{
    ...
    const bool reconfigureImages =
        (m_numImages && (m_dpbImageCreateInfo.sType == VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO)) &&
