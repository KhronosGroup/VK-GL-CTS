...
const char *CreateVulkanVideoDecodeParserFuncName =
    "?CreateVulkanVideoDecodeParser@@YA_NPEAPEAVVulkanVideoDecodeParser@@W4VkVideoCodecOperationFlagBitsKHR@@"
    "PEBUVkExtensionProperties@@P6AXPEBDZZH@Z";
#else
const char *CreateVulkanVideoDecodeParserFuncName = "_Z29CreateVulkanVideoDecodeParserPP23VulkanVideoDecodeParser32VkVi"
                                                    "deoCodecOperationFlagBitsKHRPK21VkExtensionPropertiesPFvPKczEi";
#endif

typedef void (*NvidiaParserLogFuncType)(const char *format, ...);
typedef bool (*CreateVulkanVideoDecodeParserFunc)(NvidiaVulkanVideoDecodeParser **ppobj,
                                                  vk::VkVideoCodecOperationFlagBitsKHR eCompression,
                                                  const vk::VkExtensionProperties *extensionProperty,
                                                  NvidiaParserLogFuncType pParserLogFunc, int logLevel);
...
public:
    ...
    virtual bool parseByteStream(uint8_t *pData, int64_t size);
...
bool ClsVulkanVideoDecodeParser::initialize(NvidiaVulkanParserVideoDecodeClient *nvidiaVulkanParserVideoDecodeClient)
{
    ...
    NvidiaVulkanParserInitDecodeParameters parameters = {
        NV_VULKAN_VIDEO_PARSER_API_VERSION, nvidiaVulkanParserVideoDecodeClient, 0, 0, DE_NULL, true};
...
bool ClsVulkanVideoDecodeParser::parseByteStream(uint8_t *pData, int64_t size)
{
    DE_ASSERT(m_vulkanVideoDecodeParser != DE_NULL);
...
public:
    ...
    virtual IfcVulkanVideoDecodeParser *createIfcVulkanVideoDecodeParser(
        VkVideoCodecOperationFlagBitsKHR codecOperation, const VkExtensionProperties *stdExtensionVersion);
...
NvFunctions::NvFunctions(const vk::Platform &platform)
    ...
    : m_library(de::MovePtr<vk::Library>(
          platform.createLibrary(vk::Platform::LIBRARY_TYPE_VULKAN_VIDEO_DECODE_PARSER, DE_NULL)))
#else
...
#endif
{
    ...
    m_createVulkanVideoDecodeParserFunc = reinterpret_cast<CreateVulkanVideoDecodeParserFunc>(
        funcsLibrary.getFunction(CreateVulkanVideoDecodeParserFuncName));
...
IfcVulkanVideoDecodeParser *NvFunctions::createIfcVulkanVideoDecodeParser(
    VkVideoCodecOperationFlagBitsKHR codecOperation, const VkExtensionProperties *stdExtensionVersion)
{
    ...
    if (!m_createVulkanVideoDecodeParserFunc(&pobj, codecOperation, stdExtensionVersion, NvidiaParserLogFunc, 0) ||
        pobj == DE_NULL)
    {
        return DE_NULL;
    }
