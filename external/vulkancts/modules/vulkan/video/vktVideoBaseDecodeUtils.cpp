...
void createParser(VkVideoCodecOperationFlagBitsKHR codecOperation, const VkExtensionProperties *extensionProperties,
                  VideoBaseDecoder *decoder, VkSharedBaseObj<VulkanVideoDecodeParser> &parser)
{
    ...
    const VkParserInitDecodeParameters pdParams = {
        ...
        static_cast<uint32_t>(2 * 1024 * 1024), // 2MiB is the default bitstream buffer size
    ...
    {
        if (strcmp(pStdExtensionVersion->extensionName, VK_STD_VULKAN_VIDEO_CODEC_H264_DECODE_EXTENSION_NAME) ||
    ...
    {
        if (strcmp(pStdExtensionVersion->extensionName, VK_STD_VULKAN_VIDEO_CODEC_H265_DECODE_EXTENSION_NAME) ||
...
VideoBaseDecoder::VideoBaseDecoder(Parameters &&params)
    ...
    , m_framesToCheck((uint32_t)params.framesToCheck)
    , m_dpb(3)
    ...
    , m_resetPictureParametersFrameTriggerHack((int32_t)params.pictureParameterUpdateTriggerHack)
    ...
    , m_alwaysRecreateDPB(params.alwaysRecreateDPB)
{
    ...
    VK_CHECK(util::getSupportedVideoFormats(*m_deviceContext, m_profile, m_decodeCaps.flags, m_outImageFormat,
                                            m_dpbImageFormat));
...
int32_t VideoBaseDecoder::StartVideoSequence(const VkParserDetectedVideoFormat *pVideoFormat)
{
    ...
    VkExtent2D imageExtent =
        VkExtent2D{std::max((uint32_t)(pVideoFormat->display_area.right - pVideoFormat->display_area.left),
                            pVideoFormat->coded_width),
                   std::max((uint32_t)(pVideoFormat->display_area.bottom - pVideoFormat->display_area.top),
    ...
    // reason)
    m_numDecodeSurfaces =
        std::max(4u, m_framesToCheck); // N.B. pVideoFormat->minNumDecodeSurfaces is NOT advertised correctly!
    VkResult result = VK_SUCCESS;

    if (videoLoggingEnabled())
    {
        std::cout << "\t" << std::hex << m_supportedVideoCodecs << " HW codec types are available: " << std::dec
                  << std::endl;
    }
    ...
    int32_t ret          = m_videoFrameBuffer->InitImagePool(
        videoProfile.GetProfile(), m_numDecodeSurfaces, m_dpbImageFormat, m_outImageFormat, codedExtent, imageExtent,
    ...
    DE_ASSERT((uint32_t)ret >= m_numDecodeSurfaces);
    ...
    if (videoLoggingEnabled())
    {
        std::cout << "Allocating Video Device Memory" << std::endl
                  << "Allocating " << m_numDecodeSurfaces << " Num Decode Surfaces and " << maxDpbSlotCount
                  << " Video Device Memory Images for DPB " << std::endl
    ...
    {
        uint32_t allocateNumBuffers = std::min<uint32_t>(m_decodeFramesData.GetBitstreamBuffersQueue().GetMaxNodes(),
                                                         (m_numBitstreamBuffersToPreallocate - availableBuffers));

        allocateNumBuffers =
            std::min<uint32_t>(allocateNumBuffers, m_decodeFramesData.GetBitstreamBuffersQueue().GetFreeNodesNumber());
        ...
        for (uint32_t i = 0; i < 1; i++)
        {
            ...
            result = BitstreamBufferImpl::Create(m_deviceContext, m_deviceContext->decodeQueueFamilyIdx(), allocSize,
                                                 m_videoCaps.minBitstreamBufferOffsetAlignment,
                                                 m_videoCaps.minBitstreamBufferSizeAlignment, bitstreamBuffer,
            ...
            int32_t nodeAddedWithIndex =
                m_decodeFramesData.GetBitstreamBuffersQueue().AddNodeToPool(bitstreamBuffer, false);
...
int32_t VideoBaseDecoder::BeginSequence(const VkParserSequenceInfo *pnvsi)
{
    ...
    const uint32_t maxDpbSlots = (pnvsi->eCodec == VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR) ?
                                     VkParserPerFrameDecodeParameters::MAX_DPB_REF_AND_SETUP_SLOTS :
                                     VkParserPerFrameDecodeParameters::MAX_DPB_REF_SLOTS;
...
bool VideoBaseDecoder::DecodePicture(VkParserPictureData *pd)
{
    ...
    if (videoLoggingEnabled())
        std::cout << "\t ==> VulkanVideoParser::DecodePicture " << picIdx << std::endl
                  << "\t\t progressive: " << (bool)pd->progressive_frame << // Frame is progressive
            ...
            "\t\t\t second_field: " << (bool)pd->second_field << // Second field of a complementary field pair
            ...
            "\t\t ref_pic: " << (bool)pd->ref_pic_flag << std::endl; // Frame is a reference frame
                                     ...
                                     VkParserDecodePictureInfo *pDecodePictureInfo)
{
    ...
    if (pd->sideDataLen > 0)
    {
        cachedParameters->pd.pSideData = new uint8_t[pd->sideDataLen];
...
int32_t VideoBaseDecoder::DecodePictureWithParameters(MovePtr<CachedDecodeParameters> &cachedParameters)
{
    ...
    int32_t retPicIdx = GetCurrentFrameData((uint32_t)currPicIdx, cachedParameters->frameDataSlot);
    ...
    if (pPicParams->decodeFrameInfo.pSetupReferenceSlot != nullptr)
    {
        ...
        setupActivationSlot.pPictureResource =
            &pPicParams
                 ->dpbSetupPictureResource; //dpbAndOutputCoincide() ? &pPicParams->decodeFrameInfo.dstPictureResource : &pPicParams->pictureResources[pPicParams->numGopReferenceSlots];
    ...
    int picIdx =
        m_videoFrameBuffer->QueuePictureForDecode(currPicIdx, &cachedParameters->decodedPictureInfo,
...
void VideoBaseDecoder::ApplyPictureParameters(de::MovePtr<CachedDecodeParameters> &cachedParameters)
{
    ...
    if (videoLoggingEnabled())
    {
        std::cout << "ApplyPictureParameters object " << cachedParameters->decodeBeginInfo.videoSessionParameters
...
void VideoBaseDecoder::SubmitQueue(de::MovePtr<CachedDecodeParameters> &cachedParameters)
{
    ...
    {
        VK_CHECK(vk.waitForFences(device, 1, &cachedParameters->frameSynchronizationInfo.frameConsumerDoneFence, true,
                                  TIMEOUT_100ms));
    ...
    if (videoLoggingEnabled())
    {
        std::cout << "\t +++++++++++++++++++++++++++< " << cachedParameters->pictureParams.currPicIdx
                  << " >++++++++++++++++++++++++++++++" << std::endl;
        std::cout << std::dec << "\t => Decode Submitted for CurrPicIdx: " << cachedParameters->pictureParams.currPicIdx
                  << std::endl
                  << "\t\tm_nPicNumInDecodeOrder: " << cachedParameters->picNumInDecodeOrder
                  << "\t\tframeCompleteFence " << cachedParameters->frameSynchronizationInfo.frameCompleteFence
                  << "\t\tframeCompleteSemaphore " << cachedParameters->frameSynchronizationInfo.frameCompleteSemaphore
                  << "\t\tdstImageView "
    ...
    { // For fence/sync debugging
        ...
        else
        {
            ...
            {
                VK_CHECK(vk.waitForFences(device, 1, &cachedParameters->frameSynchronizationInfo.frameCompleteFence,
                                          true, TIMEOUT_100ms));
...
VkDeviceSize VideoBaseDecoder::GetBitstreamBuffer(VkDeviceSize size, VkDeviceSize minBitstreamBufferOffsetAlignment,
                                                  ...
                                                  VkSharedBaseObj<VulkanBitstreamBuffer> &bitstreamBuffer)
{
    ...
    if (videoLoggingEnabled())
    {
        std::cout << "\t\tFrom bitstream buffer pool with size " << newSize << " B, " << newSize / 1024 << " KB, "
                  << newSize / 1024 / 1024 << " MB" << std::endl;
    ...
    if (videoLoggingEnabled() && newSize > m_maxStreamBufferSize)
    {
        std::cout << "\tAllocated bitstream buffer with size " << newSize << " B, " << newSize / 1024 << " KB, "
                  << newSize / 1024 / 1024 << " MB" << std::endl;
        m_maxStreamBufferSize = (uint32_t)newSize;
    }
