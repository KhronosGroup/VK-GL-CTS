...
void VideoBaseDecoder::StartVideoSequence (const VkParserDetectedVideoFormat* pVideoFormat)
{
	...
	VkExtent2D imageExtent = { pVideoFormat->max_session_width, pVideoFormat->max_session_height };
...
int32_t VideoBaseDecoder::BeginSequence (const VkParserSequenceInfo* pnvsi)
{
	...
	detectedFormat.coded_width = m_nvsi.nDisplayWidth;
									  ...
									  VkParserDecodePictureInfo* pDecodePictureInfo)
{
	...
	if (m_profile.GetCodecType() == VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR)
	{
		...
		pDecodePictureInfo->displayWidth	= m_nvsi.nCodedWidth;
		pDecodePictureInfo->displayHeight	= m_nvsi.nCodedHeight;
	}
	else if (m_profile.GetCodecType() == VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_KHR)
	{
		...
		// TODO: Hack around a ref-counting issue in the frame buffer. A better approach would be to modify the frame buffer not to drop displayed frames in cached decoding mode.
