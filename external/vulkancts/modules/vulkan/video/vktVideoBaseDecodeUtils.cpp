...
VkFormat codecGetVkFormat(VkVideoChromaSubsamplingFlagBitsKHR chromaFormatIdc, int bitDepthLuma, bool isSemiPlanar)
{
    ...
    case VK_VIDEO_CHROMA_SUBSAMPLING_420_BIT_KHR:
    {
        ...
        case VK_VIDEO_COMPONENT_BIT_DEPTH_10_BIT_KHR:
            return (isSemiPlanar ? VK_FORMAT_G10X6_B10X6R10X6_2PLANE_420_UNORM_3PACK16 :
                                   VK_FORMAT_G10X6_B10X6_R10X6_3PLANE_420_UNORM_3PACK16);
        case VK_VIDEO_COMPONENT_BIT_DEPTH_12_BIT_KHR:
            return (isSemiPlanar ? VK_FORMAT_G12X4_B12X4R12X4_2PLANE_420_UNORM_3PACK16 :
    ...
    case VK_VIDEO_CHROMA_SUBSAMPLING_422_BIT_KHR:
    {
        ...
        case VK_VIDEO_COMPONENT_BIT_DEPTH_10_BIT_KHR:
            return (isSemiPlanar ? VK_FORMAT_G10X6_B10X6R10X6_2PLANE_422_UNORM_3PACK16 :
                                   VK_FORMAT_G10X6_B10X6_R10X6_3PLANE_422_UNORM_3PACK16);
        case VK_VIDEO_COMPONENT_BIT_DEPTH_12_BIT_KHR:
            return (isSemiPlanar ? VK_FORMAT_G12X4_B12X4R12X4_2PLANE_422_UNORM_3PACK16 :
    ...
    case VK_VIDEO_CHROMA_SUBSAMPLING_444_BIT_KHR:
    {
        ...
        case VK_VIDEO_COMPONENT_BIT_DEPTH_10_BIT_KHR:
            return (isSemiPlanar ? VK_FORMAT_G10X6_B10X6R10X6_2PLANE_444_UNORM_3PACK16_EXT :
                                   VK_FORMAT_G10X6_B10X6_R10X6_3PLANE_444_UNORM_3PACK16);
        case VK_VIDEO_COMPONENT_BIT_DEPTH_12_BIT_KHR:
            return (isSemiPlanar ? VK_FORMAT_G12X4_B12X4R12X4_2PLANE_444_UNORM_3PACK16_EXT :
...
VkVideoComponentBitDepthFlagsKHR getLumaBitDepth(uint8_t lumaBitDepthMinus8)
{
...
VkVideoComponentBitDepthFlagsKHR getChromaBitDepth(uint8_t chromaBitDepthMinus8)
{
...
void setImageLayout(const DeviceInterface &vkd, VkCommandBuffer cmdBuffer, VkImage image, VkImageLayout oldImageLayout,
                    VkImageLayout newImageLayout, VkPipelineStageFlags2KHR srcStages,
                    VkPipelineStageFlags2KHR dstStages, VkImageAspectFlags aspectMask = VK_IMAGE_ASPECT_COLOR_BIT)
{
...
NvidiaVideoDecodeH264DpbSlotInfo::NvidiaVideoDecodeH264DpbSlotInfo() : dpbSlotInfo(), stdReferenceInfo()
{
}
...
NvidiaVideoDecodeH265DpbSlotInfo::NvidiaVideoDecodeH265DpbSlotInfo() : dpbSlotInfo(), stdReferenceInfo()
{
}
...
typedef struct dpbEntry
{
    ...
    void setReferenceAndTopBoottomField(bool isReference, bool nonExisting, bool isLongTerm, bool isFieldRef,
                                        bool topFieldIsReference, bool bottomFieldIsReference, int16_t frameIdx,
                                        const int16_t fieldOrderCntList[2], NvidiaVulkanPictureBase *picBuff)
    {
        ...
        {
            used_for_reference = (unsigned char)(3 & ((bottomFieldIsReference << bottomFieldShift) |
                                                      (topFieldIsReference << topFieldShift)));
        }
        else
        {
        ...
        FrameIdx         = frameIdx;
        FieldOrderCnt[0] = fieldOrderCntList[used_for_reference ==
                                             2]; // 0: for progressive and top reference; 1: for bottom reference only.
        FieldOrderCnt[1] = fieldOrderCntList[used_for_reference !=
                                             1]; // 0: for top reference only;  1: for bottom reference and progressive.
        dpbSlot          = -1;
        m_picBuff        = picBuff;
    }

    void setReference(bool isLongTerm, int32_t picOrderCnt, NvidiaVulkanPictureBase *picBuff)
    {
    ...
    void setH264PictureData(NvidiaVideoDecodeH264DpbSlotInfo *pDpbRefList, VkVideoReferenceSlotInfoKHR *pReferenceSlots,
    ...
    void setH265PictureData(NvidiaVideoDecodeH265DpbSlotInfo *pDpbSlotInfo,
                            VkVideoReferenceSlotInfoKHR *pReferenceSlots, uint32_t dpbEntryIdx, uint32_t dpbSlotIndex)
    {
...
void VideoBaseDecoder::initialize(const VkVideoCodecOperationFlagBitsKHR videoCodecOperation,
                                  ...
                                  const uint32_t queueFamilyIndexTransfer, const uint32_t queueFamilyIndexDecode,
                                  Allocator &allocator)
{
...
uint32_t VideoBaseDecoder::getQueueFamilyIndexTransfer(void)
{
...
uint32_t VideoBaseDecoder::getQueueFamilyIndexDecode(void)
{
...
void VideoBaseDecoder::setDecodeParameters(bool randomOrSwapped, bool queryResultWithStatus, uint32_t frameCountTrigger,
                                           bool submitAfter, uint32_t gopSize, uint32_t dpbCount)

{
    ...
    DEBUGLOG(std::cout << m_randomOrSwapped << " " << m_queryResultWithStatus << " " << m_frameCountTrigger << " "
                       << m_submitAfter << " " << m_gopSize << " " << m_dpbCount << std::endl);
...
{
    DEBUGLOG(std::cout << "VideoBaseDecoder::BeginSequence " << std::dec << pnvsi->nCodedWidth << "x"
                       << pnvsi->nCodedHeight << std::endl);
    DEBUGLOG(std::cout << "VideoBaseDecoder::BeginSequence nMinNumDecodeSurfaces=" << pnvsi->nMinNumDecodeSurfaces
                       << " pnvsi->isSVC=" << pnvsi->isSVC << std::endl);

    const int32_t maxDbpSlots =
        MAX_DPB_SLOTS_PLUS_1 - ((pnvsi->eCodec == VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR) ? 0 : EXTRA_DPB_SLOTS);
    const int32_t configDpbSlotsPre =
        (pnvsi->nMinNumDecodeSurfaces > 0) ? (pnvsi->nMinNumDecodeSurfaces - (pnvsi->isSVC ? 3 : 1)) : 0;
    ...
    DEBUGLOG(std::cout << "VideoBaseDecoder::BeginSequence configDpbSlots=" << configDpbSlots
                       << " configDpbSlotsPlus1=" << configDpbSlotsPlus1 << std::endl);

    const bool sequenceUpdate =
        (m_nvidiaVulkanParserSequenceInfo.nMaxWidth != 0) && (m_nvidiaVulkanParserSequenceInfo.nMaxHeight != 0);
    const bool formatChange =
        (pnvsi->eCodec != m_nvidiaVulkanParserSequenceInfo.eCodec) ||
    ...
    const VkVideoChromaSubsamplingFlagBitsKHR chromaSubsampling =
        ConvertStdH264ChromaFormatToVulkan((StdVideoH264ChromaFormatIdc)pnvsi->nChromaFormat);
...
bool VideoBaseDecoder::DecodePicture(NvidiaVulkanParserPictureData *pNvidiaVulkanParserPictureData)
{
    ...
    decodePictureInfo.flags.fieldPic =
        pNvidiaVulkanParserPictureData->field_pic_flag ? 1 : 0; // 0 = frame picture, 1 = field picture
    decodePictureInfo.flags.repeatFirstField =
        3 &
        (uint32_t)pNvidiaVulkanParserPictureData
            ->repeat_first_field; // For 3:2 pulldown (number of additional fields, 2 = frame doubling, 4 = frame tripling)
    ...
    {
        decodePictureInfo.flags.bottomField = pNvidiaVulkanParserPictureData->bottom_field_flag ?
                                                  1 :
                                                  0; // 0 = top field, 1 = bottom field (ignored if field_pic_flag=0)
        decodePictureInfo.flags.secondField =
            pNvidiaVulkanParserPictureData->second_field ? 1 : 0; // Second field of a complementary field pair
        decodePictureInfo.flags.topFieldFirst =
            pNvidiaVulkanParserPictureData->top_field_first ? 1 : 0; // Frame pictures only
...
bool VideoBaseDecoder::UpdatePictureParameters(
    NvidiaVulkanPictureParameters *pNvidiaVulkanPictureParameters,
    NvidiaSharedBaseObj<NvidiaParserVideoRefCountBase> &pictureParametersObject, uint64_t updateSequenceCount)
{
    DEBUGLOG(std::cout << "VideoBaseDecoder::UpdatePictureParameters " << (void *)pNvidiaVulkanPictureParameters << " "
                       << updateSequenceCount << std::endl);
...
bool VideoBaseDecoder::DisplayPicture(INvidiaVulkanPicture *pNvidiaVulkanPicture, int64_t llPTS)
{
...
void VideoBaseDecoder::UnhandledNALU(const uint8_t *pbData, int32_t cbData)
{
...
{
    DEBUGLOG(std::cout << "\tDecodePicture sps_sid:"
                       << (uint32_t)pNvidiaVulkanParserPictureData->CodecSpecific.h264.pStdSps->seq_parameter_set_id
                       << " pps_sid:"
                       << (uint32_t)pNvidiaVulkanParserPictureData->CodecSpecific.h264.pStdPps->seq_parameter_set_id
                       << " pps_pid:"
                       << (uint32_t)pNvidiaVulkanParserPictureData->CodecSpecific.h264.pStdPps->pic_parameter_set_id
                       << std::endl);
    bool bRet = false;
    ...
    VkVideoReferenceSlotInfoKHR *pReferenceSlots =
        ALLOC_HEAP_OBJECT_ARRAY(heap, VkVideoReferenceSlotInfoKHR, PerFrameDecodeParameters::MAX_DPB_REF_SLOTS);
    ...
    *pSetupReferenceSlot = {
    ...
    pPerFrameDecodeParameters->decodeFrameInfo.dstPictureResource.sType =
    ...
    if (m_videoCodecOperation == VK_VIDEO_CODEC_OPERATION_DECODE_H264_BIT_KHR)
    {
        ...
        *pH264 = nvVideoH264PicParameters();
        ...
        pPerFrameDecodeParameters->pCurrentPictureParameters =
            StdVideoPictureParametersSet::StdVideoPictureParametersSetFromBase(pin->pPpsClientObject);
        DEBUGLOG(std::cout << "\tDecodePicture SPS:" << (void *)pin->pSpsClientObject
                           << " PPS:" << (void *)pin->pPpsClientObject << std::endl);
        ...
        pPictureInfo->pSliceOffsets =
            static_cast<uint32_t *>(copyToHeap(heap, pNvidiaVulkanParserPictureData->pSliceDataOffsets,
                                               sizeof(uint32_t) * pNvidiaVulkanParserPictureData->nNumSlices));
        ...
        {
            pStdPictureInfo->PicOrderCnt[pNvidiaVulkanParserPictureData->bottom_field_flag] =
                pin->CurrFieldOrderCnt[pNvidiaVulkanParserPictureData->bottom_field_flag];
        }

        pPerFrameDecodeParameters->numGopReferenceSlots =
            FillDpbH264State(pNvidiaVulkanParserPictureData, pin->dpb, DE_LENGTH_OF_ARRAY(pin->dpb), pDpbRefList,
                             pReferenceSlots, pPerFrameDecodeParameters->pGopReferenceImagesIndexes,
                             pH264->stdPictureInfo.flags, &pSetupReferenceSlot->slotIndex);
        ...
        if (pSetupReferenceSlot->slotIndex >= 0)
        {
            if (m_distinctDstDpbImages)
            {
                ...
                pPerFrameDecodeParameters->pictureResources[setupSlotNdx] =
                    pPerFrameDecodeParameters->decodeFrameInfo.dstPictureResource;
        ...
        if (pPerFrameDecodeParameters->numGopReferenceSlots)
        {
            ...
            {
                pPerFrameDecodeParameters->pictureResources[dpbEntryIdx].sType =
                    VK_STRUCTURE_TYPE_VIDEO_PICTURE_RESOURCE_INFO_KHR;
                pReferenceSlots[dpbEntryIdx].pPictureResource =
                    &pPerFrameDecodeParameters->pictureResources[dpbEntryIdx];
            ...
            pPerFrameDecodeParameters->decodeFrameInfo.referenceSlotCount =
                pPerFrameDecodeParameters->numGopReferenceSlots;
        }
        else
        {
    ...
    else if (m_videoCodecOperation == VK_VIDEO_CODEC_OPERATION_DECODE_H265_BIT_KHR)
    {
        ...
        *pHevc = nvVideoH265PicParameters();
        ...
        pPictureInfo->pSliceSegmentOffsets =
            static_cast<uint32_t *>(copyToHeap(heap, pNvidiaVulkanParserPictureData->pSliceDataOffsets,
                                               sizeof(uint32_t) * pNvidiaVulkanParserPictureData->nNumSlices));

        pStdPictureInfo->pps_pic_parameter_set_id = pin->pic_parameter_set_id; // PPS ID
        pStdPictureInfo->flags.IrapPicFlag =
            (pin->IrapPicFlag ? 1 : 0); // Intra Random Access Point for current picture.
        pStdPictureInfo->flags.IdrPicFlag =
            (pin->IdrPicFlag ? 1 : 0); // Instantaneous Decoding Refresh for current picture.
        ...
        pPerFrameDecodeParameters->numGopReferenceSlots =
            FillDpbH265State(pNvidiaVulkanParserPictureData, pin, pDpbRefList, pStdPictureInfo, pReferenceSlots,
...
uint32_t VideoBaseDecoder::FillDpbH264State(const NvidiaVulkanParserPictureData *pNvidiaVulkanParserPictureData,
                                            const NvidiaVulkanParserH264DpbEntry *dpbIn, uint32_t maxDpbInSlotsInUse,
                                            ...
                                            int32_t *pCurrAllocatedSlotIndex)
{
    ...
    for (uint32_t inIdx = 0; inIdx < maxDpbInSlotsInUse; inIdx++)
    {
        ...
        if (used_for_reference)
        {
            ...
            refOnlyDpbIn[numUsedRef].setReferenceAndTopBoottomField(
                ...
                !!(used_for_reference & bottomFieldMask), (int16_t)dpbIn[inIdx].FrameIdx, fieldOrderCntList,
    ...
    if (refDpbUsedAndValidMask)
    {
        ...
        for (int32_t dpbIdx = 0; (uint32_t)dpbIdx < numUsedRef; dpbIdx++)
        {
            if (!refOnlyDpbIn[dpbIdx].is_non_existing)
            {
                ...
                int32_t frameNumDiff = ((int32_t)pNvidiaVulkanParserPictureData->CodecSpecific.h264.frame_num -
                                        refOnlyDpbIn[dpbIdx].FrameIdx);
    ...
    int8_t currPicDpbSlot =
        AllocateDpbSlotForCurrentH264(GetPic(pNvidiaVulkanParserPictureData->pCurrPic), currPicFlags);
...
vector<pair<VkFormat, VkImageUsageFlags>> getImageFormatAndUsageForOutputAndDPB(
    const InstanceInterface &vk, const VkPhysicalDevice physicalDevice,
    const VkVideoProfileListInfoKHR *videoProfileList, const VkFormat recommendedFormat,
    const bool distinctDstDpbImages)
{
    const VkImageUsageFlags dstFormatUsages = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR;
    ...
    {
        const MovePtr<vector<VkFormat>> bothUsageFormats =
            getSupportedFormats(vk, physicalDevice, bothImageUsages, videoProfileList);
    ...
    {
        {
            const MovePtr<vector<VkFormat>> dstUsageFormats =
                getSupportedFormats(vk, physicalDevice, dstFormatUsages, videoProfileList);
        ...
        {
            const MovePtr<vector<VkFormat>> dpbUsageFormats =
                getSupportedFormats(vk, physicalDevice, dpbFormatUsages, videoProfileList);
...
int32_t VideoBaseDecoder::StartVideoSequence(const VulkanParserDetectedVideoFormat *pVideoFormat)
{
    ...
    const uint32_t queueFamilyIndex   = getQueueFamilyIndexDecode();
    Allocator &allocator              = getAllocator();
    const VkDeviceSize bufferSize     = 4 * 1024 * 1024;

    DE_ASSERT(m_videoFrameBuffer != DE_NULL);
    DE_ASSERT(m_videoCodecOperation ==
              pVideoFormat->codec); // Make sure video have same format the queue was created for
    ...
    const uint32_t maxDpbSlotCount =
        pVideoFormat
            ->maxNumDpbSlots; // This is currently configured by the parser to maxNumDpbSlots from the stream plus 1 for the current slot on the fly
    ...
    void *videoProfileExtention = h264 ? (void *)videoProfileExtentionH264.get() :
                                  h265 ? (void *)videoProfileExtentionH265.get() :
                                         DE_NULL;
    const MovePtr<VkVideoProfileInfoKHR> videoProfile =
        getVideoProfile(videoCodec, videoProfileExtention, chromaSubsampling, lumaBitDepth, chromaBitDepth);
    const MovePtr<VkVideoProfileListInfoKHR> videoProfileList = getVideoProfileList(videoProfile.get());

    const MovePtr<VkVideoDecodeH264CapabilitiesKHR> videoCapabilitiesExtension264D =
        getVideoCapabilitiesExtensionH264D();
    const MovePtr<VkVideoDecodeH265CapabilitiesKHR> videoCapabilitiesExtension265D =
        getVideoCapabilitiesExtensionH265D();
    void *videoCapabilitiesExtension = h264 ? (void *)videoCapabilitiesExtension264D.get() :
                                       h265 ? (void *)videoCapabilitiesExtension265D.get() :
                                              DE_NULL;
    MovePtr<VkVideoDecodeCapabilitiesKHR> videoDecodeCapabilities =
        getVideoDecodeCapabilities(videoCapabilitiesExtension);
    const MovePtr<VkVideoCapabilitiesKHR> videoCapabilites =
        getVideoCapabilities(vki, physDevice, videoProfile.get(), videoDecodeCapabilities.get());

    DEBUGLOG(std::cout << "StartVideoSequence: maxDpbSlots=" << videoCapabilites->maxDpbSlots
                       << " maxActiveReferencePictures=" << videoCapabilites->maxActiveReferencePictures << std::endl);

    const bool videoExtentSupported = validateVideoExtent(codedExtent, *videoCapabilites);

    m_distinctDstDpbImages =
        (videoDecodeCapabilities->flags & VK_VIDEO_DECODE_CAPABILITY_DPB_AND_OUTPUT_DISTINCT_BIT_KHR) ? true : false;

    const vector<pair<VkFormat, VkImageUsageFlags>> imageFormatAndUsageForOutputAndDPB =
        getImageFormatAndUsageForOutputAndDPB(vki, physDevice, videoProfileList.get(), videoVkFormat,
                                              m_distinctDstDpbImages);

    const bool outFormatValidate =
        validateFormatSupport(vki, physDevice, imageFormatAndUsageForOutputAndDPB[0].second, videoProfileList.get(),
                              imageFormatAndUsageForOutputAndDPB[0].first);
    const bool isVideoProfileSutable =
        validateVideoProfileList(vki, physDevice, videoProfileList.get(), imageFormatAndUsageForOutputAndDPB[0].first,
                                 imageFormatAndUsageForOutputAndDPB[0].second);
    ...
    const uint32_t dpbPictureImageLayers =
        (videoCapabilites->flags & VK_VIDEO_CAPABILITY_SEPARATE_REFERENCE_IMAGES_BIT_KHR) ? 1 : m_numDecodeSurfaces;
    const VkImageCreateInfo outImageCreateInfo =
        makeImageCreateInfo(outPictureFormat, codedExtent, &queueFamilyIndex, outPictureUsage, videoProfileList.get());
    const VkImageCreateInfo dpbImageCreateInfo =
        makeImageCreateInfo(dpbPictureFormat, codedExtent, &queueFamilyIndex, dpbPictureUsage, videoProfileList.get(),
                            dpbPictureImageLayers);
    const VkImageCreateInfo *pDpbImageCreateInfo =
        dpbPictureUsage == static_cast<VkImageUsageFlags>(0) ? DE_NULL : &dpbImageCreateInfo;

    if (m_width == 0 || m_height == 0)
    {
        const MovePtr<VkVideoSessionCreateInfoKHR> videoSessionCreateInfo =
            getVideoSessionCreateInfo(queueFamilyIndex, videoProfile.get(), codedExtent, outPictureFormat,
                                      dpbPictureFormat, std::min(maxDpbSlotCount, videoCapabilites->maxDpbSlots),
        ...
        m_videoFrameBuffer->InitImagePool(vkd, device, queueFamilyIndex, allocator, m_maxDecodeFramesCount,
                                          m_maxDecodeFramesAllocated, &outImageCreateInfo, pDpbImageCreateInfo,
                                          videoProfile.get());
        ...
        {
            m_decodeFramesData[decodeFrameId].bitstreamBuffer.CreateVideoBitstreamBuffer(
                vkd, device, allocator, bufferSize, m_minBitstreamBufferOffsetAlignment,
                m_minBitstreamBufferSizeAlignment, videoProfileList.get());
            m_decodeFramesData[decodeFrameId].commandBuffer =
                allocateCommandBuffer(vkd, device, *m_videoCommandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
        }
    }
...
bool VideoBaseDecoder::UpdatePictureParametersHandler(
    NvidiaVulkanPictureParameters *pNvidiaVulkanPictureParameters,
    NvidiaSharedBaseObj<NvidiaParserVideoRefCountBase> &pictureParametersObject, uint64_t updateSequenceCount)
{
    NvidiaSharedBaseObj<StdVideoPictureParametersSet> pictureParametersSet(
        StdVideoPictureParametersSet::Create(pNvidiaVulkanPictureParameters, updateSequenceCount));
...
bool VideoBaseDecoder::AddPictureParametersToQueue(
    NvidiaSharedBaseObj<StdVideoPictureParametersSet> &pictureParametersSet)
{
    ...
    if ((!!m_lastSpsPictureParametersQueue &&
         !!m_lastPpsPictureParametersQueue) ||           // the last slots are already occupied
...
bool VideoBaseDecoder::CheckStdObjectBeforeUpdate(
    NvidiaSharedBaseObj<StdVideoPictureParametersSet> &stdPictureParametersSet)
{
...
NvidiaParserVideoPictureParameters *NvidiaParserVideoPictureParameters::VideoPictureParametersFromBase(
    NvidiaParserVideoRefCountBase *pBase)
{
    if (!pBase)
        return DE_NULL;
...
VkResult VulkanVideoBitstreamBuffer::CreateVideoBitstreamBuffer(const DeviceInterface &vkd, VkDevice device,
                                                                Allocator &allocator, VkDeviceSize bufferSize,
                                                                VkDeviceSize bufferOffsetAlignment,
                                                                VkDeviceSize bufferSizeAlignment, void *pNext,
...
VkResult VulkanVideoBitstreamBuffer::CopyVideoBitstreamToBuffer(const DeviceInterface &vkd, VkDevice device,
                                                                const unsigned char *pBitstreamData,
                                                                VkDeviceSize bitstreamDataSize)
{
...
NvidiaParserVideoPictureParameters *VideoBaseDecoder::CheckStdObjectAfterUpdate(
    NvidiaSharedBaseObj<StdVideoPictureParametersSet> &stdPictureParametersSet,
    NvidiaParserVideoPictureParameters *pNewPictureParametersObject)
{
    ...
    if (pNewPictureParametersObject)
    {
        ...
        else
        {
            ...
            const NvidiaParserVideoPictureParameters *pOwnerPictureParameters =
                NvidiaParserVideoPictureParameters::VideoPictureParametersFromBase(
                    stdPictureParametersSet->m_vkObjectOwner);
...
NvidiaParserVideoPictureParameters *VideoBaseDecoder::AddPictureParameters(
...
#endif
    {
        pPictureParametersObject = NvidiaParserVideoPictureParameters::Create(
            vkd, device, m_videoDecodeSession.get(), spsStdPictureParametersSet, ppsStdPictureParametersSet,
            m_currentPictureParameters);
...
void *copyToHeap(HeapType &heap, const void *p, size_t size)
{
    ...
    heap.push_back(de::MovePtr<vector<uint8_t>>(new vector<uint8_t>(size)));
...
void appendHeap(HeapType &heapTo, HeapType &heapFrom)
{
    ...
    for (auto &item : heapFrom)
        heapTo.push_back(de::MovePtr<vector<uint8_t>>(item.release()));

    heapFrom.clear();
}

void appendPerFrameDecodeParameters(PerFrameDecodeParameters *pPerFrameDecodeParameters,
                                    vector<PerFrameDecodeParameters *> &perFrameDecodeParameters, HeapType &heap)
{
    ...
    if (pPerFrameDecodeParameters->bitstreamDataLen > 0)
        pPerFrameDecodeParameters->pBitstreamData = static_cast<unsigned char *>(
            copyToHeap(heap, pPerFrameDecodeParameters->pBitstreamData,
                       static_cast<size_t>(deAlign64(pPerFrameDecodeParameters->bitstreamDataLen, 16))));
}
...
int32_t VideoBaseDecoder::DecodePictureWithParameters(PerFrameDecodeParameters *pPerFrameDecodeParameters,
                                                      ...
                                                      HeapType &heap)
{
    DEBUGLOG(std::cout << "\tDecodePictureWithParameters:" << std::dec << m_pPerFrameDecodeParameters.size()
                       << std::endl);
    ...
    m_pVulkanParserDecodePictureInfo.push_back((VulkanParserDecodePictureInfo *)copyToHeap(
        m_heaps[ndx], pVulkanParserDecodePictureInfo, sizeof(*pVulkanParserDecodePictureInfo)));
...
int32_t VideoBaseDecoder::DecodeCachedPictures(VideoBaseDecoder *friendDecoder, bool waitSubmitted)
{
    ...
    for (size_t ndx = 0; ndx < ndxMax; ++ndx)
    {
        ...
        pFrameData->bitstreamBuffer.CopyVideoBitstreamToBuffer(vkd, device, pPicParams->pBitstreamData,
        ...
        pPicParams->decodeFrameInfo.srcBufferRange =
            deAlign64((VkDeviceSize)pPicParams->bitstreamDataLen, m_minBitstreamBufferSizeAlignment);
...

        {
            const VkImageLayout newLayout =
                m_distinctDstDpbImages ? VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR : VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR;
            ...
            m_videoFrameBuffer->GetImageResourcesByIndex((int8_t)pPicParams->currPicIdx, &pictureResource,
                                                         &currentPictureResource, newLayout);

            DEBUGLOG(std::cout << "PicNdx: " << std::dec << (int32_t)picNdx << " " << currentPictureResource.image
                               << std::endl);
            ...
            if (currentPictureResource.currentImageLayout == VK_IMAGE_LAYOUT_UNDEFINED)
            {
                ...
                DEBUGLOG(std::cout << "\tTransit DST: " << imageBarrier.image << " from " << imageBarrier.oldLayout
                                   << std::endl);
        ...
        if (m_distinctDstDpbImages)
        {
            ...
            VkVideoPictureResourceInfoKHR &pictureResource =
            ...
            if (currentPictureResource.currentImageLayout == VK_IMAGE_LAYOUT_UNDEFINED)
            {
                ...
                imageBarrier.subresourceRange.baseArrayLayer =
                    pPicParams->decodeFrameInfo.dstPictureResource.baseArrayLayer;

                DEBUGLOG(std::cout << "\tTransit DPB: " << imageBarrier.image << ":"
                                   << imageBarrier.subresourceRange.baseArrayLayer << " from " << imageBarrier.oldLayout
                                   << std::endl);
        ...
        if (pPicParams->numGopReferenceSlots)
        {
            ...
            m_videoFrameBuffer->GetImageResourcesByIndex(
                pPicParams->numGopReferenceSlots, &pPicParams->pGopReferenceImagesIndexes[0],
                pPicParams->pictureResources, pictureResourcesInfo.data(), newLayout);

            for (int32_t resId = 0; resId < pPicParams->numGopReferenceSlots; resId++)
            {
                s << std::dec << (int32_t)pPicParams->pGopReferenceImagesIndexes[resId] << ":"
                  << pictureResourcesInfo[resId].image << " ";
                ...
                if (pictureResourcesInfo[resId].image != DE_NULL &&
                    pictureResourcesInfo[resId].currentImageLayout != newLayout)
                {
                    VkImageMemoryBarrier2KHR &imageBarrier = incSizeSafe(imageBarriers);
                    ...
                    DEBUGLOG(std::cout << "\tTransit DPB: " << imageBarrier.image << " from " << imageBarrier.oldLayout
                                       << std::endl);
        ...
        int32_t retVal = m_videoFrameBuffer->QueuePictureForDecode(
            (int8_t)currPicIdx, pDecodePictureInfo, pPicParams->pCurrentPictureParameters->m_vkObjectOwner,
    ...
    if (m_randomOrSwapped)
    {
        ...
        else
        {
            ...
            for (vector<size_t>::iterator it = recordOrderIndices.begin(); it != recordOrderIndices.end();
                 it += m_gopSize)
            {
    ...
    for (size_t ndx = 0; ndx < ndxMax; ++ndx)
    {
        ...
        DEBUGLOG(std::cout << "beginCommandBuffer " << commandBuffer << " VkVideoSessionParametersKHR:"
                           << pOwnerPictureParameters->GetVideoSessionParametersKHR() << std::endl);
        ...
        decodeBeginInfo = {
            ...
            pOwnerPictureParameters
                ->GetVideoSessionParametersKHR(),        //  VkVideoSessionParametersKHR videoSessionParameters;
        ...
        dependencyInfo = {
            ...
            0,                                     //  uint32_t memoryBarrierCount;
            DE_NULL,                               //  const VkMemoryBarrier2KHR* pMemoryBarriers;
            1,                                     //  uint32_t bufferMemoryBarrierCount;
            &bitstreamBufferMemoryBarrier,         //  const VkBufferMemoryBarrier2KHR* pBufferMemoryBarriers;
            (uint32_t)imageBarriers.size(),        //  uint32_t imageMemoryBarrierCount;
        ...
        if (m_queryResultWithStatus)
            vkd.cmdResetQueryPool(commandBuffer, frameSynchronizationInfo.queryPool,
        ...
        if (pPicParams->decodeFrameInfo.pSetupReferenceSlot != nullptr)
        {
            ...
            dstPictureSlot.pPictureResource             = m_distinctDstDpbImages ?
                                                              &pPicParams->pictureResources[pPicParams->numGopReferenceSlots] :
        ...
        if (m_queryResultWithStatus)
            vkd.cmdBeginQuery(commandBuffer, frameSynchronizationInfo.queryPool, frameSynchronizationInfo.startQueryId,
                              0u);
        ...
        if (!m_submitAfter)
            SubmitQueue(&commandBufferSubmitInfo, &m_submitInfos[picNdx], &m_frameSynchronizationInfos[picNdx],
                        &m_frameConsumerDoneSemaphoreSubmitInfos[picNdx], &m_frameCompleteSemaphoreSubmitInfos[picNdx]);
    }

    if (m_submitAfter && !interleaved)
    {
        for (size_t ndx = 0; ndx < recordOrderIndices.size(); ++ndx)
            SubmitQueue(&m_commandBufferSubmitInfos[ndx], &m_submitInfos[ndx], &m_frameSynchronizationInfos[ndx],
                        &m_frameConsumerDoneSemaphoreSubmitInfos[ndx], &m_frameCompleteSemaphoreSubmitInfos[ndx]);
    }
    ...
    if (interleaved)
    {
        for (uint32_t ndx = 0; ndx < ndxMax; ++ndx)
        {
            ...
            if (m_frameSynchronizationInfos[ndx].frameCompleteSemaphore != DE_NULL)
                m_frameCompleteSemaphoreSubmitInfos.push_back(
                    makeSemaphoreSubmitInfo(m_frameSynchronizationInfos[ndx].frameCompleteSemaphore,
                                            VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT_KHR));

            if (m_frameSynchronizationInfos[ndx].frameConsumerDoneSemaphore != DE_NULL)
                m_frameConsumerDoneSemaphoreSubmitInfos.push_back(
                    makeSemaphoreSubmitInfo(m_frameSynchronizationInfos[ndx].frameConsumerDoneSemaphore,
        ...
        if (friendDecoder != DE_NULL)
        {
            ...
            for (uint32_t ndx = 0; ndx < ndxMax; ++ndx)
            {
                if (friendDecoder->m_frameSynchronizationInfos[ndx].frameConsumerDoneFence != DE_NULL)
                    m_frameConsumerDoneFences.push_back(
                        friendDecoder->m_frameSynchronizationInfos[ndx].frameConsumerDoneFence);

                if (friendDecoder->m_frameSynchronizationInfos[ndx].frameCompleteSemaphore != DE_NULL)
                    m_frameCompleteSemaphoreSubmitInfos.push_back(
                        makeSemaphoreSubmitInfo(friendDecoder->m_frameSynchronizationInfos[ndx].frameCompleteSemaphore,
                                                VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT_KHR));

                if (friendDecoder->m_frameSynchronizationInfos[ndx].frameConsumerDoneSemaphore != DE_NULL)
                    m_frameConsumerDoneSemaphoreSubmitInfos.push_back(makeSemaphoreSubmitInfo(
                        friendDecoder->m_frameSynchronizationInfos[ndx].frameConsumerDoneSemaphore,
                        VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT_KHR));
            }
        }
        ...
        if (waitSubmitted)
        {
            ...
            commandBufferSubmitInfos.reserve(m_commandBufferSubmitInfos.size() +
                                             friendDecoder->m_commandBufferSubmitInfos.size());
            ...
            SubmitQueue(commandBufferSubmitInfos, &m_submitInfos[ndxMax - 1], m_frameCompleteFences[ndxMax - 1],
                        m_frameConsumerDoneFences, m_frameCompleteSemaphoreSubmitInfos,
                        m_frameConsumerDoneSemaphoreSubmitInfos);
        }
    }
    ...
    if (waitSubmitted)
    {
        VK_CHECK(vkd.waitForFences(device, (uint32_t)m_frameCompleteFences.size(), m_frameCompleteFences.data(), true,
                                   ~0ull));
    ...
    if (m_queryResultWithStatus)
    {
        for (size_t ndx = 0; ndx < ndxMax; ++ndx)
        {
            ...
            VkResult result = vkd.getQueryPoolResults(device, m_frameSynchronizationInfos[ndx].queryPool,
                                                      m_frameSynchronizationInfos[ndx].startQueryId, 1,
                                                      sizeof(queryResult), &queryResult, sizeof(queryResult),
...
void VideoBaseDecoder::SubmitQueue(VkCommandBufferSubmitInfoKHR *commandBufferSubmitInfo, VkSubmitInfo2KHR *submitInfo,
                                   ...
                                   VkSemaphoreSubmitInfoKHR *frameCompleteSemaphore)
{
    ...
    const uint32_t waitSemaphoreCount = (frameSynchronizationInfo->frameConsumerDoneSemaphore == DE_NULL) ? 0u : 1u;
    const uint32_t signalSemaphoreInfoCount = (frameSynchronizationInfo->frameCompleteSemaphore == DE_NULL) ? 0u : 1u;

    *frameConsumerDoneSemaphore = makeSemaphoreSubmitInfo(frameSynchronizationInfo->frameConsumerDoneSemaphore,
                                                          VK_PIPELINE_STAGE_2_TOP_OF_PIPE_BIT_KHR);
    *frameCompleteSemaphore     = makeSemaphoreSubmitInfo(frameSynchronizationInfo->frameCompleteSemaphore,
                                                          VK_PIPELINE_STAGE_2_BOTTOM_OF_PIPE_BIT_KHR);
    ...
    *submitInfo = {
    ...
    if ((frameSynchronizationInfo->frameConsumerDoneSemaphore == DE_NULL) &&
        (frameSynchronizationInfo->frameConsumerDoneFence != DE_NULL))
...
void VideoBaseDecoder::SubmitQueue(vector<VkCommandBufferSubmitInfoKHR> &commandBufferSubmitInfos,
                                   VkSubmitInfo2KHR *submitInfo, const VkFence frameCompleteFence,
                                   ...
                                   const vector<VkSemaphoreSubmitInfoKHR> &frameCompleteSemaphores)
{
    ...
    *submitInfo = {
        ...
        (uint32_t)frameCompleteSemaphores.size(),     //  uint32_t waitSemaphoreInfoCount;
        ...
        (uint32_t)frameConsumerDoneSemaphores.size(), //  uint32_t signalSemaphoreInfoCount;
...
int32_t NvidiaParserVideoPictureParameters::PopulateH264UpdateFields(
    ...
    vk::VkVideoDecodeH264SessionParametersAddInfoKHR &h264SessionParametersAddInfo)
{
    ...
    DE_ASSERT((pStdPictureParametersSet->m_updateType == VK_PICTURE_PARAMETERS_UPDATE_H264_SPS) ||
...
int32_t NvidiaParserVideoPictureParameters::PopulateH265UpdateFields(
    const StdVideoPictureParametersSet *pStdPictureParametersSet,
    vk::VkVideoDecodeH265SessionParametersAddInfoKHR &h265SessionParametersAddInfo)
{
    ...
    DE_ASSERT((pStdPictureParametersSet->m_updateType == VK_PICTURE_PARAMETERS_UPDATE_H265_SPS) ||
...
NvidiaParserVideoPictureParameters *NvidiaParserVideoPictureParameters::Create(
    const DeviceInterface &vkd, VkDevice device, VkVideoSessionKHR videoSession,
    const StdVideoPictureParametersSet *pSpsStdPictureParametersSet,
    const StdVideoPictureParametersSet *pPpsStdPictureParametersSet, NvidiaParserVideoPictureParameters *pTemplate)
{
                                                    ...
                                                    const StdVideoPictureParametersSet *pPpsStdPictureParametersSet)
{
    ...
    {
        updateInfo.updateSequenceCount =
            std::max(pSpsStdPictureParametersSet->m_updateSequenceCount, updateInfo.updateSequenceCount);
    }

    if (pPpsStdPictureParametersSet)
    {
        updateInfo.updateSequenceCount =
            std::max(pPpsStdPictureParametersSet->m_updateSequenceCount, updateInfo.updateSequenceCount);
    }
...
VkResult ImageObject::CreateImage(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex,
                                  Allocator &allocator, const VkImageCreateInfo *pImageCreateInfo,
                                  const MemoryRequirement memoryRequirement)
{
    ...
    m_image =
        de::MovePtr<ImageWithMemory>(new ImageWithMemory(vkd, device, allocator, *pImageCreateInfo, memoryRequirement));
...
VkResult ImageObject::StageImage(const DeviceInterface &vkd, VkDevice device, VkImageUsageFlags usage,
                                 const MemoryRequirement memoryRequirement, uint32_t queueFamilyIndex)
{
    ...
    const VkImageLayout layout =
        (usage & VK_IMAGE_USAGE_VIDEO_DECODE_DST_BIT_KHR) ? VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR :
    ...
    setImageLayout(vkd, *cmdBuffer, m_image->get(), VK_IMAGE_LAYOUT_UNDEFINED, layout, VK_PIPELINE_STAGE_HOST_BIT,
...
void NvidiaPerFrameDecodeImage::init(const DeviceInterface &vkd, VkDevice device)
{
...
VkResult NvidiaPerFrameDecodeImage::CreateImage(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex,
                                                Allocator &allocator, const VkImageCreateInfo *pImageCreateInfo,
                                                const MemoryRequirement memoryRequirement)
{
...
NvidiaPerFrameDecodeImageSet::NvidiaPerFrameDecodeImageSet() : m_size(0), m_frameDecodeImages()
{
}

int32_t NvidiaPerFrameDecodeImageSet::init(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex,
                                           Allocator &allocator, uint32_t numImages,
                                           ...
                                           MemoryRequirement memoryRequirement)
{
    ...
    for (uint32_t imageIndex = firstIndex; imageIndex < numImages; imageIndex++)
    {
        ...
        VK_CHECK(m_frameDecodeImages[imageIndex].CreateImage(vkd, device, queueFamilyIndex, allocator,
                                                             pOutImageCreateInfo, memoryRequirement));

        DEBUGLOG(std::cout << "CreateImg: " << m_frameDecodeImages[imageIndex].m_frameImage.getImage() << " "
                           << std::dec << pOutImageCreateInfo->extent.width << "x" << pOutImageCreateInfo->extent.height
                           << " " << m_frameDecodeImages[imageIndex].m_frameImageCurrentLayout << std::endl);
        ...
        if (pDpbImageCreateInfo != DE_NULL)
        {
            ...
            if (pDpbImageCreateInfo->arrayLayers == 1 || imageIndex == firstIndex)
            {
                ...
                VK_CHECK(m_frameDecodeImages[imageIndex].m_dpbImage->CreateImage(
                    vkd, device, queueFamilyIndex, allocator, pDpbImageCreateInfo, memoryRequirement));

                DEBUGLOG(std::cout << "CreateDPB: " << m_frameDecodeImages[imageIndex].m_dpbImage->getImage() << " "
                                   << std::dec << pOutImageCreateInfo->extent.width << "x"
                                   << pOutImageCreateInfo->extent.height << " "
                                   << m_frameDecodeImages[imageIndex].m_dpbImageCurrentLayout << std::endl);
            }
            else
            {
...
Move<VkQueryPool> VideoFrameBuffer::CreateVideoQueries(const DeviceInterface &vkd, VkDevice device, uint32_t numSlots,
...
int32_t VideoFrameBuffer::InitImagePool(const DeviceInterface &vkd, VkDevice device, int32_t queueFamilyIndex,
                                        Allocator &allocator, uint32_t numImages, uint32_t maxNumImages,
                                        const VkImageCreateInfo *pOutImageCreateInfo,
                                        const VkImageCreateInfo *pDpbImageCreateInfo,
                                        const VkVideoProfileInfoKHR *pDecodeProfile)
{
    ...
    if (numImages && pOutImageCreateInfo)
    {
        ...
        return m_perFrameDecodeImageSet.init(vkd, device, queueFamilyIndex, allocator, numImages, pOutImageCreateInfo,
                                             pDpbImageCreateInfo, MemoryRequirement::Local);
    }
...
int32_t VideoFrameBuffer::QueuePictureForDecode(int8_t picId, VulkanParserDecodePictureInfo *pDecodePictureInfo,
                                                ...
                                                FrameSynchronizationInfo *pFrameSynchronizationInfo)
{
    ...
    if (m_perFrameDecodeImageSet[picId].m_hasConsummerSignalFence)
    {
        pFrameSynchronizationInfo->frameConsumerDoneFence =
            m_perFrameDecodeImageSet[picId].m_frameConsumerDoneFence.get();
    ...
    {
        pFrameSynchronizationInfo->frameCompleteSemaphore =
            m_perFrameDecodeImageSet[picId].m_frameCompleteSemaphore.get();
    ...
    if (m_perFrameDecodeImageSet[picId].m_hasConsummerSignalSemaphore)
    {
        pFrameSynchronizationInfo->frameConsumerDoneSemaphore =
            m_perFrameDecodeImageSet[picId].m_frameConsumerDoneSemaphore.get();
...
int32_t VideoFrameBuffer::DequeueDecodedPicture(DecodedFrame *pDecodedFrame)
{
    ...
    if ((uint32_t)pictureIndex < m_perFrameDecodeImageSet.size())
    {
        ...
        {
            pDecodedFrame->frameCompleteSemaphore =
                m_perFrameDecodeImageSet[pictureIndex].m_frameCompleteSemaphore.get();
        ...
        pDecodedFrame->frameConsumerDoneSemaphore =
            m_perFrameDecodeImageSet[pictureIndex].m_frameConsumerDoneSemaphore.get();
                                                  ...
                                                  uint32_t numFramesToRelease)
{
    for (uint32_t i = 0; i < numFramesToRelease; i++)
    {
        ...
        m_perFrameDecodeImageSet[picId].m_hasConsummerSignalSemaphore =
            pDecodedFrameRelease->hasConsummerSignalSemaphore;
    }

    return 0;
}

void VideoFrameBuffer::GetImageResourcesByIndex(int32_t numResources, const int8_t *referenceSlotIndexes,
                                                VkVideoPictureResourceInfoKHR *videoPictureResources,
                                                PictureResourceInfo *pictureResourcesInfos,
                                                VkImageLayout newImageLayout)
{
    DE_ASSERT(newImageLayout == VK_IMAGE_LAYOUT_VIDEO_DECODE_DST_KHR ||
              newImageLayout == VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR);
    ...
    for (int32_t resId = 0; resId < numResources; resId++)
    {
        ...
        if (de::inBounds(referenceSlotIndex, 0, perFrameDecodeImageSetSize))
        {
            ...
            {
                videoPictureResource.codedOffset = {
            ...
            else if (newImageLayout == VK_IMAGE_LAYOUT_VIDEO_DECODE_DPB_KHR)
            {
                videoPictureResource.codedOffset = {
                    0, 0}; // FIXME: This parameter must to be adjusted based on the interlaced mode.
                videoPictureResource.codedExtent = m_extent;
                videoPictureResource.baseArrayLayer =
                    perFrameDecodeImage.m_dpbImage->isArray() ? referenceSlotIndex : 0;
