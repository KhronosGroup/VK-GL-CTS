...
public:
    ...
    explicit NvidiaSharedBaseObj(NvidiaBaseObjType *const newObjectPtr = nullptr) : m_sharedObject(nullptr)
    {
        Reset(newObjectPtr);
    }

    NvidiaSharedBaseObj(const NvidiaSharedBaseObj<NvidiaBaseObjType> &newObject) : m_sharedObject(nullptr)
    {
        Reset(newObject.Get());
    }
    ...
    const NvidiaSharedBaseObj<NvidiaBaseObjType> &operator=(
        const NvidiaSharedBaseObj<VkBaseObjType2> &otherSharedObject)
    {
...
public:
    ...
    virtual int32_t BeginSequence(
        const NvidiaVulkanParserSequenceInfo
            *pnvsi) = 0; // Returns max number of reference frames (always at least 2 for MPEG-2)
    virtual bool AllocPictureBuffer(
        INvidiaVulkanPicture **ppNvidiaVulkanPicture)               = 0; // Returns a new INvidiaVulkanPicture interface
    virtual bool DecodePicture(NvidiaVulkanParserPictureData
                                   *pNvidiaVulkanParserPictureData) = 0; // Called when a picture is ready to be decoded
    ...
    virtual bool DisplayPicture(INvidiaVulkanPicture *pNvidiaVulkanPicture, int64_t llPTS) = 0;
    ...
    virtual int32_t GetOperatingPoint(void * /* pOPInfo */)
    {
        return 0;
...
typedef struct NvidiaVulkanParserInitDecodeParameters
{
    ...
    bool
        bOutOfBandPictureParameters; // If set, Picture Parameters are going to be provided via UpdatePictureParameters callback
...
public:
    ...
    virtual bool DecodeSliceInfo(void *pVulkanParserSliceInfo, const void *pVulkanParserPictureData,
                                 int32_t iSlice)                                                           = 0;
...
public:
    ...
    static StdVideoPictureParametersSet *Create(NvidiaVulkanPictureParameters *pPictureParameters,
...
union VulkanParserFieldFlags
{
    struct
    {
        ...
        uint32_t
            repeatFirstField : 3; // For 3:2 pulldown (number of additional fields, 2 = frame doubling, 4 = frame tripling)
