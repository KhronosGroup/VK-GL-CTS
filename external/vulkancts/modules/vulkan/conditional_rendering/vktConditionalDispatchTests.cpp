...
{
public:
    ConditionalDispatchTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
ConditionalDispatchTest::ConditionalDispatchTest(tcu::TestContext &testCtx, const std::string &name,
                                                 const std::string &description, const ConditionalTestSpec &testSpec)
...
tcu::TestStatus ConditionalDispatchTestInstance::iterate(void)
{
    ...
    const uint32_t expectedResult = m_conditionalData.expectCommandExecution ? m_numCalls : 0u;
...
void ConditionalDispatchTests::init(void)
{
    for (int conditionNdx = 0; conditionNdx < DE_LENGTH_OF_ARRAY(conditional::s_testsData); conditionNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> conditionalDrawRootGroup(new tcu::TestCaseGroup(
            m_testCtx, de::toString(conditionData).c_str(), "Conditionaly execute dispatch calls"));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> conditionSizeGroup(new tcu::TestCaseGroup(
            m_testCtx, "condition_size", "Tests verifying the condition is being read as a 32-bit value"));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> allocOffsetGroup(new tcu::TestCaseGroup(
            m_testCtx, "alloc_offset", "Tests verifying the buffer allocation offset is applied correctly"));
