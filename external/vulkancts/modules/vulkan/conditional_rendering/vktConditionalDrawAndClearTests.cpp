...
const DrawTestParams drawTestGrid[] = {
    {false, false, false, b0001, b1000, b1111, false, NONE}, {true, false, false, b0001, b1000, b0000, false, NONE},
    {true, false, false, b0001, b0001, b1110, false, NONE},  {true, false, false, b1111, b1111, b0000, false, NONE},
    {true, false, false, b0010, b0010, b1101, false, NONE},  {true, true, false, b1010, b1010, b0101, false, NONE},
    {false, true, true, b1010, b1010, b1111, false, NONE},   {true, true, true, b0010, b1000, b0001, false, NONE},
    {true, true, true, b1001, b1001, b0110, false, NONE},    {true, true, true, b0010, b1000, b1111, true, FILL},
    {true, true, true, b1001, b1001, b1111, true, FILL},     {false, true, true, b1001, b1001, b0110, true, FILL},
    {true, true, true, b0010, b1000, b1111, true, COPY},     {true, true, true, b1001, b1001, b1111, true, COPY},
...
protected:
    ...
    void createInitBufferWithPredicate(bool discard, bool invert, uint32_t offsetMultiplier,
                                       VkBufferUsageFlagBits extraUsage);
...
void ConditionalRenderingBaseTestInstance::createInitBufferWithPredicate(
    bool discard, bool invert, uint32_t offsetMultiplier = 0,
    VkBufferUsageFlagBits extraUsage = (VkBufferUsageFlagBits)0)
{
    ...
    m_conditionalRenderingBuffer = Buffer::createAndAlloc(
        m_vkd, m_device, BufferCreateInfo(dataSize, VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT | extraUsage),
    ...
    flushMappedMemoryRange(m_vkd, m_device, m_conditionalRenderingBuffer->getBoundMemory().getMemory(),
                           m_conditionalRenderingBuffer->getBoundMemory().getOffset(), VK_WHOLE_SIZE);
}
...
ConditionalRenderingUpdateBufferWithDrawTestInstance::ConditionalRenderingUpdateBufferWithDrawTestInstance(
    Context &context, bool testParams)
    : ConditionalRenderingBaseTestInstance(context)
    , m_testParams(testParams)
{
}
...
tcu::TestStatus ConditionalRenderingUpdateBufferWithDrawTestInstance::iterate(void)
{
    ...
    m_testParams ? prepareReferenceImageOneColor(reference, tcu::Vec4(0, 1, 0, 1)) :
...
void ConditionalRenderingDrawAndClearTests::init(void)
{
    ...
    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(clearColorTestGrid); testNdx++)
        color->addChild(new InstanceFactory1WithSupport<ConditionalRenderingClearAttachmentsTestInstance,
            ...
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE, generateClearTestName(clearColorTestGrid[testNdx]),
            "Color clear test.", clearColorTestGrid[testNdx], checkSupport));
    ...
    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(clearDepthTestGrid); testNdx++)
        depth->addChild(new InstanceFactory1WithSupport<ConditionalRenderingClearAttachmentsTestInstance,
                                                        ClearTestParams, FunctionSupport0>(
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE, generateClearTestName(clearDepthTestGrid[testNdx]),
            "Depth clear test.", clearDepthTestGrid[testNdx], checkSupport));

    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(clearColorTwiceGrid); testNdx++)
        color->addChild(new InstanceFactory1WithSupport<ConditionalRenderingClearAttachmentsTestInstance,
                                                        ClearTestParams, FunctionSupport0>(
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE,
            "clear_attachment_twice_" + generateClearTestName(clearColorTwiceGrid[testNdx]), "Color clear test.",
    ...
    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(clearDepthTwiceGrid); testNdx++)
        depth->addChild(new InstanceFactory1WithSupport<ConditionalRenderingClearAttachmentsTestInstance,
                                                        ClearTestParams, FunctionSupport0>(
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE,
            "clear_attachment_twice_" + generateClearTestName(clearDepthTwiceGrid[testNdx]), "Depth clear test.",
    ...
    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(drawTestGrid); testNdx++)
        draw->addChild(new InstanceFactory1WithSupport<ConditionalRenderingDrawTestInstance, DrawTestParams,
            ...
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE, "case_" + de::toString(testNdx), "Draw test.", AddProgramsDraw(),
            drawTestGrid[testNdx], checkFan));

    draw->addChild(new InstanceFactory1WithSupport<ConditionalRenderingUpdateBufferWithDrawTestInstance, bool,
                                                   FunctionSupport0, AddProgramsUpdateBufferUsingRendering>(
        m_testCtx, tcu::NODETYPE_SELF_VALIDATE, "update_with_rendering_no_discard", "Draw test.",
        AddProgramsUpdateBufferUsingRendering(), true, checkFanAndVertexStores));
    draw->addChild(new InstanceFactory1WithSupport<ConditionalRenderingUpdateBufferWithDrawTestInstance, bool,
                                                   FunctionSupport0, AddProgramsUpdateBufferUsingRendering>(
        m_testCtx, tcu::NODETYPE_SELF_VALIDATE, "update_with_rendering_discard", "Draw test.",
        AddProgramsUpdateBufferUsingRendering(), false, checkFanAndVertexStores));
