...
protected:
    ...
    void createInitBufferWithPredicate(ConditionalBufferMemory memoryType, bool discard, bool invert,
                                       uint32_t offsetMultiplier, VkBufferUsageFlagBits extraUsage);
    ...
    VkBufferUsageFlagBits extraUsage = (VkBufferUsageFlagBits)0)
{
    ...
    de::SharedPtr<Draw::Buffer> buffer =
        Buffer::createAndAlloc(m_vkd, m_device, BufferCreateInfo(dataSize, usage), m_context.getDefaultAllocator(),
    ...
    if (memoryType == ConditionalBufferMemory::LOCAL)
    {
        m_conditionalRenderingBuffer =
            Buffer::createAndAlloc(m_vkd, m_device,
                                   BufferCreateInfo(dataSize, VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT |
                                                                  VK_BUFFER_USAGE_TRANSFER_DST_BIT | extraUsage),
                                   m_context.getDefaultAllocator(), MemoryRequirement::Local);
...
tcu::TestStatus ConditionalRenderingDrawTestInstance::iterate(void)
{
    ...
    createInitBufferWithPredicate(m_testParams.m_memoryType, m_testParams.m_discard, m_testParams.m_invert,
                                  offsetMultiplier, bufferUsageExtraFlags);
...
void ConditionalRenderingDrawAndClearTests::init(void)
{
    ...
    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(drawTestGrid); testNdx++)
        draw->addChild(new InstanceFactory1WithSupport<ConditionalRenderingDrawTestInstance, DrawTestParams,
            ...
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE, generateDrawTestName(testNdx, drawTestGrid[testNdx]), "Draw test.",
    ...
    for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(UpdateBufferWithDrawTestGrind); testNdx++)
        draw->addChild(new InstanceFactory1WithSupport<ConditionalRenderingUpdateBufferWithDrawTestInstance,
            ...
            m_testCtx, tcu::NODETYPE_SELF_VALIDATE,
            generateUpdateBufferWithDrawTestName(UpdateBufferWithDrawTestGrind[testNdx]), "Draw test.",
