...
const char* getDrawCommandTypeName (DrawCommandType command)
{
	switch (command)
	{
		...
		case DRAW_COMMAND_TYPE_DRAW_MULTI_EXT:				return "draw_multi_ext";
		case DRAW_COMMAND_TYPE_DRAW_MULTI_INDEXED_EXT:		return "draw_multi_indexed_ext";
		case DRAW_COMMAND_TYPE_INDIRECT_BYTE_COUNT_EXT:		return "draw_indirect_byte_count_ext";
...
public:
	...
	virtual		tcu::TestStatus iterate								(void);
				...
				void			recordDraw							(vk::VkCommandBuffer cmdBuffer, deUint32 index);
...
void checkSupport (Context& context, ConditionalTestSpec testSpec)
{
	...
	if (testSpec.command == DRAW_COMMAND_TYPE_DRAW_MULTI_EXT || testSpec.command == DRAW_COMMAND_TYPE_DRAW_MULTI_INDEXED_EXT)
...
void ConditionalTransformFeedbackDraw::createIndirectBuffer (void)
{
	...
	m_indirectBuffer = Draw::Buffer::createAndAlloc(m_vk,
													m_context.getDevice(),
													Draw::BufferCreateInfo(sizeof(drawCommands),
...
void ConditionalTransformFeedbackDraw::createIndexedIndirectBuffer (void)
{
	...
	m_indirectBuffer = Draw::Buffer::createAndAlloc(m_vk,
													m_context.getDevice(),
													Draw::BufferCreateInfo(sizeof(drawCommand),
																	vk::VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT),
													m_context.getDefaultAllocator(),
													vk::MemoryRequirement::HostVisible);
...
{
	m_countBuffer = Draw::Buffer::createAndAlloc(m_vk,
												 m_context.getDevice(),
												 Draw::BufferCreateInfo(sizeof(deUint32) * 2,
																	vk::VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT),
												 m_context.getDefaultAllocator(),
												 vk::MemoryRequirement::HostVisible);

	deUint32* countBufferPtr = reinterpret_cast<deUint32*>(m_countBuffer->getBoundMemory().getHostPtr());
...
void ConditionalTransformFeedbackDraw::createXfbBuffer (void)
{
	...
	m_xfbBuffer = Draw::Buffer::createAndAlloc(m_vk,
												 m_context.getDevice(),
												 Draw::BufferCreateInfo(sizeof(float) * output_count,
...
void ConditionalTransformFeedbackDraw::createStreamPipeline (void)
{
	...
	const vk::Unique<vk::VkShaderModule> vs(createShaderModule(m_vk, m_context.getDevice(), m_context.getBinaryCollection().get(m_vertexShaderName), 0));
	std::string geom_name = m_context.getDeviceFeatures().shaderTessellationAndGeometryPointSize ? "VertexFetchWritePoint.geom" : "VertexFetch.geom";
	const vk::Unique<vk::VkShaderModule> gs(createShaderModule(m_vk, m_context.getDevice(), m_context.getBinaryCollection().get(geom_name), 0));
	...
	const vk::VkVertexInputBindingDescription vertexInputBindingDescription = { 0, sizeof(VertexElementData), vk::VK_VERTEX_INPUT_RATE_VERTEX };

	const vk::VkVertexInputAttributeDescription vertexInputAttributeDescriptions[] = {
		{ 0u, 0u, vk::VK_FORMAT_R32G32B32A32_SFLOAT, 0u },
		{ 1u, 0u, vk::VK_FORMAT_R32G32B32A32_SFLOAT, static_cast<deUint32>(sizeof(tcu::Vec4)) },
		{ 2u, 0u, vk::VK_FORMAT_R32_SINT, static_cast<deUint32>(sizeof(tcu::Vec4)) * 2 }
	};

	const auto vertexInputState = PipelineCreateInfo::VertexInputState(1,
...
void ConditionalTransformFeedbackDraw::recordDraw (vk::VkCommandBuffer cmdBuffer, deUint32 index)
{
	const deUint32 firstVertex = 6u * index;
	const deUint32 firstIndex = 6u * index;
	...
	switch (m_command)
	{
		...
		{
			m_vk.cmdDrawIndirect(cmdBuffer, m_indirectBuffer->object(), indirectOffset, 1, sizeof(vk::VkDrawIndirectCommand));
			break;
		}
		...
		{
			m_vk.cmdDrawMultiIndexedEXT(cmdBuffer, 1u, &multiDrawIndexedInfo, 1, 0, sizeof(vk::VkMultiDrawIndexedInfoEXT), &vertexOffset);
			break;
		}
		...
		{
			m_vk.cmdDrawIndirectCount(cmdBuffer, m_indirectBuffer->object(), indirectOffset, m_indirectCountBuffer->object(), 0, 1, sizeof(vk::VkDrawIndirectCommand));
			break;
		}
		case DRAW_COMMAND_TYPE_DRAW_INDEXED_INDIRECT_COUNT:
		{
			m_vk.cmdDrawIndexedIndirectCount(cmdBuffer, m_indirectBuffer->object(), indexedIndirectOffset, m_indirectCountBuffer->object(), 0, 1, sizeof(vk::VkDrawIndexedIndirectCommand));
			break;
		}
		default: DE_ASSERT(false);
	}
...
tcu::TestStatus ConditionalTransformFeedbackDraw::iterate (void)
{
	...
	m_queryBuffer = Draw::Buffer::createAndAlloc(m_vk,
												 m_context.getDevice(),
												 Draw::BufferCreateInfo(sizeof(deUint32) * 2u, vk::VK_BUFFER_USAGE_TRANSFER_DST_BIT | vk::VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT),
												 m_context.getDefaultAllocator(),
												 vk::MemoryRequirement::HostVisible);
	...
	m_vk.cmdCopyQueryPoolResults(*m_cmdBuffer, *queryPool, 0u, 2u, m_queryBuffer->object(), 0u, sizeof(deUint32), vk::VK_QUERY_RESULT_WAIT_BIT);

	vk::VkBufferMemoryBarrier bufferMemoryBarrier = vk::makeBufferMemoryBarrier(vk::VK_ACCESS_TRANSFER_WRITE_BIT, vk::VK_ACCESS_CONDITIONAL_RENDERING_READ_BIT_EXT, m_queryBuffer->object(), 0u, sizeof(deUint32) * 2);

	m_vk.cmdPipelineBarrier(*m_cmdBuffer, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, vk::VK_PIPELINE_STAGE_CONDITIONAL_RENDERING_BIT_EXT, 0, 0, DE_NULL, 1, &bufferMemoryBarrier, 0, DE_NULL);
	...
	conditionalRenderingBeginInfo.offset = sizeof(deUint32);
	...
	for (uint32_t stream = 0u; stream < 4u; ++stream)
	{
		...
		m_vk.cmdPushConstants(*m_cmdBuffer, *m_streamPipelineLayout, vk::VK_SHADER_STAGE_GEOMETRY_BIT, 0u, sizeof(int), &stream);

		conditionalRenderingBeginInfo.offset = sizeof(deUint32) * (stream % 2);
	...
	m_vk.cmdPipelineBarrier(*m_cmdBuffer, vk::VK_PIPELINE_STAGE_TRANSFORM_FEEDBACK_BIT_EXT, vk::VK_PIPELINE_STAGE_HOST_BIT, 0u, 1u, &tfMemoryBarrier, 0u, DE_NULL, 0u, DE_NULL);
	endCommandBuffer(m_vk, *m_cmdBuffer);
	submitCommandsAndWait(m_vk, device, queue, m_cmdBuffer.get());

	invalidateAlloc(m_vk, device, m_xfbBuffer->getBoundMemory());

	deUint32* queryBufferPtr = reinterpret_cast<deUint32*>(m_queryBuffer->getBoundMemory().getHostPtr());
	float* xfbBufferPtr = reinterpret_cast<float*>(m_xfbBuffer.get()->getHostPtr());

	if (queryBufferPtr[0] != 0) {
		log << tcu::TestLog::Message << "Occlusion query 0 expected result was 0, but query reported " << queryBufferPtr[0] << tcu::TestLog::EndMessage;
	} else if (queryBufferPtr[1] == 0) {
		log << tcu::TestLog::Message << "Occlusion query 1 expected result was not 0, but query reported " << queryBufferPtr[1] << tcu::TestLog::EndMessage;
	}
	for (uint32_t i = 0; i < 24; ++i) {
		...
		{
			log << tcu::TestLog::Message << "Expected value at index " << i << "was " << expected <<", but actual value was " << xfbBufferPtr[i] << tcu::TestLog::EndMessage;
...
void ConditionalTransformFeedbackTests::init (void)
{
	for (deUint32 commandTypeIdx = 0; commandTypeIdx < DRAW_COMMAND_TYPE_DRAW_LAST; ++commandTypeIdx)
	{
		...
		addChild(new InstanceFactory1WithSupport<ConditionalTransformFeedbackDraw, ConditionalTestSpec, FunctionSupport1<ConditionalTestSpec>, AddProgramsDraw>(m_testCtx, std::string(getDrawCommandTypeName(command)), AddProgramsDraw(), testSpec, FunctionSupport1<ConditionalTestSpec>::Args(checkSupport, testSpec)));
	}
}
