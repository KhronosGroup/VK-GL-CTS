...
ConditionalDraw::ConditionalDraw(Context &context, ConditionalTestSpec testSpec)
    : Draw::DrawTestsBaseClass(
          context, testSpec.shaders[glu::SHADERTYPE_VERTEX], testSpec.shaders[glu::SHADERTYPE_FRAGMENT],
          Draw::SharedGroupParams(new Draw::GroupParams{false, false, false}), vk::VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST)
...
void ConditionalDrawTests::init(void)
{
    for (int conditionNdx = 0; conditionNdx < DE_LENGTH_OF_ARRAY(conditional::s_testsData); conditionNdx++)
    {
        ...
        tcu::TestCaseGroup *conditionalDrawRootGroup =
            new tcu::TestCaseGroup(m_testCtx, de::toString(conditionData).c_str(), "Conditionaly execute draw calls");
