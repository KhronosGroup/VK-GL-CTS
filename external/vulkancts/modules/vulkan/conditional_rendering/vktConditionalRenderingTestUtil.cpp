...
void checkConditionalRenderingCapabilities(vkt::Context &context, const ConditionalData &data)
{
    ...
    {
        const vk::VkPhysicalDeviceConditionalRenderingFeaturesEXT &conditionalRenderingFeatures =
            context.getConditionalRenderingFeaturesEXT();
...
de::SharedPtr<Draw::Buffer> createConditionalRenderingBuffer(vkt::Context &context, const ConditionalData &data)
{
    ...
    de::SharedPtr<Draw::Buffer> buffer = Draw::Buffer::createAndAlloc(
        ...
        Draw::BufferCreateInfo(sizeof(uint32_t), vk::VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT),
        context.getDefaultAllocator(), vk::MemoryRequirement::HostVisible);

    uint8_t *conditionBufferPtr       = reinterpret_cast<uint8_t *>(buffer->getBoundMemory().getHostPtr());
    *(uint32_t *)(conditionBufferPtr) = data.conditionValue;
