                                                             ...
                                                             bool computeQueue)
{
    ...
    const auto allocOffset =
        static_cast<vk::VkDeviceSize>(sizeof(data.conditionValue) * (data.allocationOffset ? 4u : 0u));

    // Create host-visible buffer. This may be the final buffer or only a staging buffer.
    const auto hostUsage = ((data.memoryType == HOST) ? vk::VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT :
                                                        vk::VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
    de::SharedPtr<Draw::Buffer> hostBuffer =
        Draw::Buffer::createAndAlloc(vk, device, Draw::BufferCreateInfo(bufferSize, hostUsage), alloc,
                                     vk::MemoryRequirement::HostVisible, allocOffset);

    // Copy data to host buffer.
    uint8_t *conditionBufferPtr = reinterpret_cast<uint8_t *>(hostBuffer->getHostPtr());
    ...
    const auto deviceLocalUsage =
        (vk::VK_BUFFER_USAGE_CONDITIONAL_RENDERING_BIT_EXT | vk::VK_BUFFER_USAGE_TRANSFER_DST_BIT);
    de::SharedPtr<Draw::Buffer> deviceLocalBuffer =
        Draw::Buffer::createAndAlloc(vk, device, Draw::BufferCreateInfo(bufferSize, deviceLocalUsage), alloc,
                                     vk::MemoryRequirement::Local, allocOffset);
