    ...
    if (interlock == INT_SHADING_RATE_ORDERED || interlock == INT_SHADING_RATE_UNORDERED)
    {
        if (!g_singletonDevice)
        {
            ...
            VkPhysicalDeviceShadingRateImageFeaturesNV shadingRateImageFeatures =
                initVulkanStructure(&fragmentShaderInterlockFeatures);
            VkPhysicalDeviceFeatures2 features2 = initVulkanStructure(&shadingRateImageFeatures);

            const VkPhysicalDevice physicalDevice =
                chooseDevice(g_instanceWrapper->instance.getDriver(), g_instanceWrapper->instance,
            ...
            const VkDeviceCreateInfo deviceCreateInfo = {
                ...
                DE_LENGTH_OF_ARRAY(extensions),       // uint32_t enabledExtensionCount;
            ...
            Move<VkDevice> device =
                createCustomDevice(context.getTestContext().getCommandLine().isValidationEnabled(),
                                   context.getPlatformInterface(), g_instanceWrapper->instance,
                                   g_instanceWrapper->instance.getDriver(), physicalDevice, &deviceCreateInfo);
...
tcu::TestStatus FSITestInstance::iterate(void)
{
    ...
    buffer = de::MovePtr<BufferWithMemory>(new BufferWithMemory(
        vk, device, allocator,
        makeBufferCreateInfo(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_STORAGE_BUFFER_BIT),
...
tcu::TestCaseGroup *createBasicTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "basic", "Test VK_EXT_fragment_shader_interlock"));
    ...
    TestGroupCase dimCases[] = {
        {8, "8x8", "8x8"},           {16, "16x16", "16x16"},           {32, "32x32", "32x32"},
        {64, "64x64", "64x64"},      {128, "128x128", "128x128"},      {256, "256x256", "256x256"},
        {512, "512x512", "512x512"}, {1024, "1024x1024", "1024x1024"},
    };
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> killGroup(
            new tcu::TestCaseGroup(testCtx, killCases[killNdx].name, killCases[killNdx].description));
        for (int resNdx = 0; resNdx < DE_LENGTH_OF_ARRAY(resCases); resNdx++)
        {
            de::MovePtr<tcu::TestCaseGroup> resGroup(
                new tcu::TestCaseGroup(testCtx, resCases[resNdx].name, resCases[resNdx].description));
            for (int intNdx = 0; intNdx < DE_LENGTH_OF_ARRAY(intCases); intNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> intGroup(
                    new tcu::TestCaseGroup(testCtx, intCases[intNdx].name, intCases[intNdx].description));
                for (int sampNdx = 0; sampNdx < DE_LENGTH_OF_ARRAY(sampCases); sampNdx++)
                {
                    ...
                    {
                        de::MovePtr<tcu::TestCaseGroup> ssGroup(
                            new tcu::TestCaseGroup(testCtx, ssCases[ssNdx].name, ssCases[ssNdx].description));
                        for (int dimNdx = 0; dimNdx < DE_LENGTH_OF_ARRAY(dimCases); dimNdx++)
                        {
                            ...
                            ssGroup->addChild(
                                new FSITestCase(testCtx, dimCases[dimNdx].name, dimCases[dimNdx].description, c));
                        }
