...
tcu::TestStatus queryRefreshableObjects(Context &context)
{
    ...
    uint32_t refreshableObjectsMaxCount(countReported + 2);
    std::vector<vk::VkObjectType> refreshableObjects(refreshableObjectsMaxCount);

    for (uint32_t countRequested = 0; countRequested < refreshableObjectsMaxCount; ++countRequested)
    {
        // get refreshable objects
        uint32_t countRetrieved = countRequested;
        std::fill(refreshableObjects.begin(), refreshableObjects.end(), vk::VK_OBJECT_TYPE_UNKNOWN);
        result =
            vki.getPhysicalDeviceRefreshableObjectTypesKHR(physicalDevice, &countRetrieved, refreshableObjects.data());
        ...
        if (countRetrieved != std::min(countRequested, countReported))
            TCU_THROW(NotSupportedError,
                      "vkGetPhysicalDeviceRefreshableObjectTypesKHR returned invalid number of retrieved objects");

        // verify retrieved objects
        for (uint32_t i = 0; i < countRetrieved; ++i)
        {
            if (refreshableObjects[i] == vk::VK_OBJECT_TYPE_UNKNOWN)
                TCU_THROW(NotSupportedError,
                          "vkGetPhysicalDeviceRefreshableObjectTypesKHR returned invalid object type");
        }
    }
...
tcu::TestStatus refreshObjects(Context &context, bool individualRefresh)
{
    ...
    result =
        vki.getPhysicalDeviceRefreshableObjectTypesKHR(physicalDevice, &countReported, refreshableObjectTypes.data());
    if (result != vk::VK_SUCCESS)
        TCU_THROW(NotSupportedError, "vkGetPhysicalDeviceRefreshableObjectTypesKHR failed");
    ...
    de::MovePtr<vk::Allocation> bufferAllocation =
        allocator.allocate(bufferRequirements, vk::MemoryRequirement::HostVisible);
    ...
    const vk::VkSamplerYcbcrConversionCreateInfo conversionInfo{
        ...
        false                            // VkBool32 forceExplicitReconstruction;
    };
    vk::Move<vk::VkSamplerYcbcrConversion> ycbcrConversion =
    ...
    vk::Move<vk::VkImageView> imageView =
        makeImageView(vkd, device, *image, vk::VK_IMAGE_VIEW_TYPE_2D, vk::VK_FORMAT_R8G8B8A8_UNORM,
                      makeImageSubresourceRange(vk::VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, 1u));
    vk::Move<vk::VkShaderModule> shaderModule =
        createShaderModule(vkd, device, context.getBinaryCollection().get("comp"), 0u);
    ...
    vk::Move<vk::VkPipeline> pipeline =
        makeComputePipeline(vkd, device, *pipelineLayout, 0u, *shaderModule, 0u, DE_NULL);
    ...
    std::map<vk::VkObjectType, uint64_t> objectHandlesMap{
    ...
    if (individualRefresh)
    {
        for (const auto &object : objectHandlesMap)
        {
            ...
            uint64_t objectHandle       = object.second;

            // skip object type if it can't be refreshed on current implementation
            if (std::find(refreshableObjectTypes.begin(), refreshableObjectTypes.end(), objectType) ==
                refreshableObjectTypes.end())
                continue;
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < countReported; ++i)
        {
            vk::VkObjectType objectType = refreshableObjectTypes[i];
            uint64_t objectHandle       = objectHandlesMap.at(objectType);
...
tcu::TestCaseGroup *createObjectRefreshTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "object_refresh", "Tests VK_KHR_object_refresh"));

    addFunctionCase(group.get(), "query_refreshable_objects", "Test VK_KHR_object_refresh extension",
                    checkRefreshSupport, queryRefreshableObjects);
    addFunctionCaseWithPrograms(group.get(), "refresh_individual_objects", "Test VK_KHR_object_refresh extension",
                                checkRefreshSupport, createComputeSource, refreshIndividualObjects);
    addFunctionCaseWithPrograms(group.get(), "refresh_all_objects", "Test VK_KHR_object_refresh extension",
                                checkRefreshSupport, createComputeSource, refreshAllObjects);
