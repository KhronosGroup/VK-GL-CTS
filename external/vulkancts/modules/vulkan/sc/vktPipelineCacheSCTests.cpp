...
tcu::TestStatus createPipelineCacheTest(Context &context, TestParams testParams)
{
    ...
    if (!context.getTestContext().getCommandLine().isSubProcess())
    {
        ...
        // graphics pipeline
        {
            ...
            VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo = prepareSimpleGraphicsPipelineCI(
                vertexInputStateCreateInfo, shaderStageCreateInfos, inputAssemblyStateCreateInfo,
                viewPortStateCreateInfo, rasterizationStateCreateInfo, multisampleStateCreateInfo,
                colorBlendAttachmentState, colorBlendStateCreateInfo, dynamicStateCreateInfo, dynamicStates,
        ...
        // compute pipeline
        {
            ...
            VkComputePipelineCreateInfo computePipelineCreateInfo =
                prepareSimpleComputePipelineCI(shaderStageCreateInfo, *pipelineLayout);
    ...
    std::vector<uint8_t> customCacheData(context.getResourceInterface()->getCacheDataSize());
    deMemcpy(customCacheData.data(), context.getResourceInterface()->getCacheData(),
             context.getResourceInterface()->getCacheDataSize());
    uintptr_t initialDataSize = customCacheData.size();
    VkPipelineCacheHeaderVersionSafetyCriticalOne *pcHeader =
        (VkPipelineCacheHeaderVersionSafetyCriticalOne *)customCacheData.data();

    switch (testParams.type)
    {
    case PCTT_WRONG_VENDOR_ID:
    {
        pcHeader->headerVersionOne.vendorID = uint32_t(VK_VENDOR_ID_MAX_ENUM);
        break;
    }
    ...
    CreatePipelineCacheFunc createPipelineCacheFunc =
        (CreatePipelineCacheFunc)getDeviceProcAddrFunc(*device, "vkCreatePipelineCache");
    DestroyPipelineCacheFunc destroyPipelineCacheFunc =
...
tcu::TestCaseGroup *createPipelineCacheTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "pipeline_cache", "Tests verifying Vulkan SC pipeline cache"));
    ...
    for (int testIdx = 0; testIdx < DE_LENGTH_OF_ARRAY(tests); ++testIdx)
    {
        ...
        addFunctionCaseWithPrograms(group.get(), tests[testIdx].name, "", createShaders, createPipelineCacheTest,
                                    params);
    }
