...
tcu::TestStatus testGetFaultData(Context &context, TestParams testParams)
{
    ...
    uint32_t faultCount       = maxQueryFaultCount;
    ...
    case FHF_NULL:
        ...
        if (result != VK_SUCCESS)
        {
            context.getTestContext().getLog()
                << tcu::TestLog::Message << "Result is not VK_SUCCESS" << tcu::TestLog::EndMessage;
            isOK = false;
        }
        if (unrecordedFaults != VK_FALSE)
        {
            context.getTestContext().getLog()
                << tcu::TestLog::Message << "unrecordedFaults is not VK_FALSE" << tcu::TestLog::EndMessage;
            isOK = false;
        }
        if (faultCount != 0u)
        {
            context.getTestContext().getLog()
                << tcu::TestLog::Message << "faultCount is not 0" << tcu::TestLog::EndMessage;
            isOK = false;
        }
        break;
    case FHF_ARRAY:
        ...
        {
            context.getTestContext().getLog()
                << tcu::TestLog::Message << "pFaults have been modified" << tcu::TestLog::EndMessage;
            isOK = false;
        }
        break;
    default:
...
void testFaultCallback(VkBool32 incompleteFaultData, uint32_t faultCount, VkFaultData *pFaultData)
{
...
tcu::TestCaseGroup *createFaultHandlingTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "fault_handling", "Tests verifying Vulkan SC fault handling"));

    // add tests for vkGetFaultData function
    {
        de::MovePtr<tcu::TestCaseGroup> getFaultDataGroup(
            new tcu::TestCaseGroup(testCtx, "get_fault_data", "Testing vkGetFaultData results"));
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> behaviourGroup(
                new tcu::TestCaseGroup(testCtx, behaviours[behaviourIdx].name, ""));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> faultCallbackInfoGroup(
            new tcu::TestCaseGroup(testCtx, "fault_callback_info", "Testing vkGetFaultData results"));

        {
            FaultCallbackInfoTestParams testParams{true};
            addFunctionCase(faultCallbackInfoGroup.get(), "create_device_with_callback_with_fault_data", "",
                            testCreateDeviceWithFaultCallbackInfo, testParams);
        }
        {
            FaultCallbackInfoTestParams testParams{false};
            addFunctionCase(faultCallbackInfoGroup.get(), "create_device_with_callback_without_fault_data", "",
                            testCreateDeviceWithFaultCallbackInfo, testParams);
        }
