...
tcu::TestCaseGroup *createCommandPoolMemoryReservationTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "command_pool_memory_reservation",
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> memConGroup(
            new tcu::TestCaseGroup(testCtx, "memory_consumption", "Testing vkGetCommandPoolMemoryConsumption"));
        ...
        for (int cbIdx = 0; cbIdx < DE_LENGTH_OF_ARRAY(maxCommandBuffers); ++cbIdx)
        {
            ...
            for (int sizeIdx = 0; sizeIdx < DE_LENGTH_OF_ARRAY(reservedSizes); ++sizeIdx)
            {
                de::MovePtr<tcu::TestCaseGroup> sizeGroup(
                    new tcu::TestCaseGroup(testCtx, reservedSizes[sizeIdx].name, ""));

                for (int simIdx = 0; simIdx < DE_LENGTH_OF_ARRAY(recording); ++simIdx)
                {
                    de::MovePtr<tcu::TestCaseGroup> simGroup(
                        new tcu::TestCaseGroup(testCtx, recording[simIdx].name, ""));
