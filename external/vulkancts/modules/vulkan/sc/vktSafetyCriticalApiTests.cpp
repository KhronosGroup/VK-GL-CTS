...
tcu::TestStatus forbiddenCoreExtensions(Context &context)
{
    ...
    const std::vector<vk::VkExtensionProperties> deviceExtensions =
        vk::enumerateDeviceExtensionProperties(vki, physicalDevice, DE_NULL);
...
tcu::TestCaseGroup *createSafetyCriticalAPITests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "api", "Tests verifying Vulkan SC specific API"));

    addFunctionCase(group.get(), "forbidden_core_commands", "Verify existence of functions removed from Vulkan",
                    forbiddenCoreCommands);
    addFunctionCase(group.get(), "forbidden_core_extensions", "Verify existence of extensions removed from Vulkan",
                    forbiddenCoreExtensions);
    addFunctionCase(group.get(), "forbidden_promoted_commands",
                    "Verify existence of promoted functions removed from Vulkan", forbiddenPromotedCommands);
    addFunctionCase(group.get(), "forbidden_features", "Verify if specific device features are forbidden for Vulkan SC",
                    forbiddenDeviceFeatures);
    addFunctionCase(group.get(), "forbidden_properties",
                    "Verify if specific device properties are forbidden for Vulkan SC", forbiddenDeviceProperties);
    addFunctionCase(group.get(), "allowed_extensions", "Verify if extensions are allowed for Vulkan SC",
                    allowedExtensions);
