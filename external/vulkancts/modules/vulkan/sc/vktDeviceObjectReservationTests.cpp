...
const uint32_t VERIFYMAXVALUES_OBJECT_COUNT = 5U;
const uint32_t VERIFYMAXVALUES_ARRAYLAYERS  = 8U;
const uint32_t VERIFYMAXVALUES_MIPLEVELS    = 5U;
...
struct TestParams
{
    TestParams(const TestMaxValues &testMaxValues_         = TMV_UNDEFINED,
               const TestRequestCounts &testRequestCounts_ = TRC_UNDEFINED,
               const TestPoolSizes &testPoolSizeType_      = PST_UNDEFINED)
...
void createSemaphores(const DeviceInterface &vkd, const VkDevice device, std::vector<SemaphoreSp>::iterator begin,
...
void createCommandBuffers(const DeviceInterface &vkd, const VkDevice device, const VkCommandPool commandPool,
                          std::vector<CommandBufferSp>::iterator begin, std::vector<CommandBufferSp>::iterator end)
{
...
void createFences(const DeviceInterface &vkd, const VkDevice device, std::vector<FenceSp>::iterator begin,
...
void allocateDeviceMemory(const DeviceInterface &vkd, const VkDevice device, VkDeviceSize size,
                          std::vector<DeviceMemorySp>::iterator begin, std::vector<DeviceMemorySp>::iterator end)
{
...
void createBuffers(const DeviceInterface &vkd, const VkDevice device, VkDeviceSize size,
                   std::vector<BufferSp>::iterator begin, std::vector<BufferSp>::iterator end)
{
...
void createImages(const DeviceInterface &vkd, const VkDevice device, uint32_t size,
                  std::vector<ImageSp>::iterator begin, std::vector<ImageSp>::iterator end)
{
    uint32_t queueFamilyIndex = 0u;
...
void createEvents(const DeviceInterface &vkd, const VkDevice device, std::vector<EventSp>::iterator begin,
...
void createQueryPools(const DeviceInterface &vkd, const VkDevice device, std::vector<QueryPoolSp>::iterator begin,
...
void createBufferViews(const DeviceInterface &vkd, const VkDevice device, const VkBuffer buffer,
                       const VkDeviceSize size, std::vector<BufferViewSp>::iterator begin,
...
void createImageViews(const DeviceInterface &vkd, const VkDevice device, const VkImage image, const VkFormat format,
                      std::vector<ImageViewSp>::iterator begin, std::vector<ImageViewSp>::iterator end)
{
    for (std::vector<ImageViewSp>::iterator it = begin; it != end; ++it)
    {
        VkComponentMapping componentMapping{VK_COMPONENT_SWIZZLE_IDENTITY, VK_COMPONENT_SWIZZLE_IDENTITY,
...
void createPipelineLayouts(const DeviceInterface &vkd, const VkDevice device,
                           std::vector<PipelineLayoutSp>::iterator begin, std::vector<PipelineLayoutSp>::iterator end)
{
...
void createRenderPasses(const DeviceInterface &vkd, const VkDevice device, VkAttachmentDescription *colorAttachment,
                        std::vector<RenderPassSp>::iterator begin, std::vector<RenderPassSp>::iterator end)
{
...
void createGraphicsPipelines(const DeviceInterface &vkd, const VkDevice device, VkShaderModule vertexShaderModule,
                             VkShaderModule fragmentShaderModule, VkRenderPass renderPass,
                             VkPipelineLayout pipelineLayout, VkDeviceSize poolEntrySize,
                             de::SharedPtr<vk::ResourceInterface> resourceInterface,
                             std::vector<PipelineSp>::iterator begin, std::vector<PipelineSp>::iterator end)
{
    ...
    for (std::vector<PipelineSp>::iterator it = begin; it != end; ++it)
    {
        ...
        if (poolEntrySize != 0u)
        {
            ...
            std::size_t hashValue =
                calculateGraphicsPipelineHash(graphicsPipelineCI, resourceInterface->getObjectHashes());
...
void createComputePipelines(const DeviceInterface &vkd, const VkDevice device, VkShaderModule shaderModule,
                            VkPipelineLayout pipelineLayout, VkDeviceSize poolEntrySize,
                            de::SharedPtr<vk::ResourceInterface> resourceInterface,
                            std::vector<PipelineSp>::iterator begin, std::vector<PipelineSp>::iterator end)
{
    for (std::vector<PipelineSp>::iterator it = begin; it != end; ++it)
    {
        ...
        if (poolEntrySize != 0u)
        {
            pipelineOfflineCreateInfo = resetPipelineOfflineCreateInfo();
            std::size_t hashValue =
                calculateComputePipelineHash(computePipelineCI, resourceInterface->getObjectHashes());
            memcpy(pipelineOfflineCreateInfo.pipelineIdentifier, &hashValue, sizeof(std::size_t));
            pipelineOfflineCreateInfo.poolEntrySize = poolEntrySize;
...
void createDescriptorSetLayouts(const DeviceInterface &vkd, const VkDevice device,
                                ...
                                std::vector<DescriptorSetLayoutSp>::iterator end)
{
    for (std::vector<DescriptorSetLayoutSp>::iterator it = begin; it != end; ++it)
    {
        ...
        *it = DescriptorSetLayoutSp(
            new Unique<VkDescriptorSetLayout>(createDescriptorSetLayout(vkd, device, &descriptorSetLayoutCI)));
    }
}

void createSamplers(const DeviceInterface &vkd, const VkDevice device, std::vector<SamplerSp>::iterator begin,
...
void createDescriptorPools(const DeviceInterface &vkd, const VkDevice device, uint32_t maxSets,
                           std::vector<DescriptorPoolSp>::iterator begin, std::vector<DescriptorPoolSp>::iterator end)
{
...
void createDescriptorSets(const DeviceInterface &vkd, const VkDevice device, const VkDescriptorPool descriptorPool,
                          const VkDescriptorSetLayout setLayout, std::vector<DescriptorSetSp>::iterator begin,
...
void createFramebuffers(const DeviceInterface &vkd, const VkDevice device, const VkRenderPass renderPass,
                        const VkImageView imageView, std::vector<FramebufferSp>::iterator begin,
...
void createCommandPools(const DeviceInterface &vkd, const VkDevice device, std::vector<CommandPoolSp>::iterator begin,
                                   ...
                                   std::vector<SamplerYcbcrConversionSp>::iterator end)
{
    for (std::vector<SamplerYcbcrConversionSp>::iterator it = begin; it != end; ++it)
    {
        ...
        *it = SamplerYcbcrConversionSp(
            new Unique<VkSamplerYcbcrConversion>(createSamplerYcbcrConversion(vkd, device, &ycbcrConversionCI)));
    }
}
...
DeviceObjectReservationInstance::DeviceObjectReservationInstance(Context &context, const TestParams &testParams_)
...
{
public:
    MultipleReservation(Context &context, const TestParams &testParams_)
...
{
    if (testParams.testMaxValues == TMV_MAX_PIPELINESTATISTICS_QUERIES_PER_POOL &&
        context.getDeviceFeatures().pipelineStatisticsQuery == VK_FALSE)
...
{
public:
    VerifyMaxValues(Context &context, const TestParams &testParams_)
        : DeviceObjectReservationInstance(context, testParams_)
    {
    }
    Move<VkDevice> createTestDevice(VkDeviceCreateInfo &deviceCreateInfo,
    ...
    void performTest(const DeviceInterface &vkd, VkDevice device) override
    {
        ...
        case TMV_MAX_IMAGEVIEW_MIPLEVELS:
        {
            const VkImageCreateInfo imageCreateInfo = {
                ...
                VK_FORMAT_R8_UNORM,                  // VkFormat format;
                {
                    1 << VERIFYMAXVALUES_MIPLEVELS, // uint32_t width;
                    1 << VERIFYMAXVALUES_MIPLEVELS, // uint32_t height;
                    1u                              // uint32_t depth;
                },                                  // VkExtent3D extent;
                VERIFYMAXVALUES_MIPLEVELS,          // uint32_t mipLevels;
                1u,                                 // uint32_t arrayLayers;
                VK_SAMPLE_COUNT_1_BIT,              // VkSampleCountFlagBits samples;
                VK_IMAGE_TILING_OPTIMAL,            // VkImageTiling tiling;
                VK_IMAGE_USAGE_SAMPLED_BIT,         // VkImageUsageFlags usage;
        ...
        case TMV_MAX_IMAGEVIEW_ARRAYLAYERS:
        {
            const VkImageCreateInfo imageCreateInfo = {
                ...
                1u,                          // uint32_t mipLevels;
                VERIFYMAXVALUES_ARRAYLAYERS, // uint32_t arrayLayers;
                VK_SAMPLE_COUNT_1_BIT,       // VkSampleCountFlagBits samples;
                VK_IMAGE_TILING_OPTIMAL,     // VkImageTiling tiling;
                VK_IMAGE_USAGE_SAMPLED_BIT,  // VkImageUsageFlags usage;
                VK_SHARING_MODE_EXCLUSIVE,   // VkSharingMode sharingMode;
...
{
    if (testParams.testRequestCounts == TRC_SAMPLERYCBCRCONVERSION &&
        context.getDeviceVulkan11Features().samplerYcbcrConversion == VK_FALSE)
...
{
public:
    VerifyRequestCounts(Context &context, const TestParams &testParams_)
        : DeviceObjectReservationInstance(context, testParams_)
    {
    }
                                    ...
                                    VkPhysicalDeviceVulkanSC10Features &sc10Features) override
    {
        ...
        case TRC_COMMAND_BUFFER:
            ...
            objectInfo.commandBufferRequestCount =
                2 * VERIFYMAXVALUES_OBJECT_COUNT + (VERIFYMAXVALUES_OBJECT_COUNT - VERIFYMAXVALUES_OBJECT_COUNT / 2);
            break;
        case TRC_FENCE:
    ...
    void performTest(const DeviceInterface &vkd, VkDevice device) override
    {
        ...
        case TRC_COMMAND_BUFFER:
        {
            ...
            createCommandBuffers(vkd, device, commandPools[0]->get(),
                                 begin(commandBuffers) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(commandBuffers));
        ...
        case TRC_BUFFER_VIEW:
        {
            ...
            createBufferViews(vkd, device, buffer.get(), 128ull, begin(bufferViews) + VERIFYMAXVALUES_OBJECT_COUNT / 2,
                              end(bufferViews));
        ...
        case TRC_IMAGE_VIEW:
        {
            ...
            createImageViews(vkd, device, image.get(), VK_FORMAT_R8_UNORM,
                             begin(imageViews) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(imageViews));
        ...
        case TRC_PIPELINE_LAYOUT:
        {
            ...
            std::fill(begin(pipelineLayouts) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(pipelineLayouts),
                      PipelineLayoutSp());
            createPipelineLayouts(vkd, device, begin(pipelineLayouts) + VERIFYMAXVALUES_OBJECT_COUNT / 2,
                                  end(pipelineLayouts));
        ...
        case TRC_RENDER_PASS:
        {
            ...
            createRenderPasses(vkd, device, &attachmentDescription,
                               begin(renderPasses) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(renderPasses));
        ...
        case TRC_GRAPHICS_PIPELINE:
        {
            ...
            Move<VkShaderModule> vertexShaderModule =
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("vertex"), 0u);
            Move<VkShaderModule> fragmentShaderModule =
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("fragment"), 0u);

            std::vector<PipelineSp> pipelines(VERIFYMAXVALUES_OBJECT_COUNT);
            createGraphicsPipelines(vkd, device, vertexShaderModule.get(), fragmentShaderModule.get(),
                                    renderPasses[0]->get(), pipelineLayouts[0]->get(), pipelineDefaultSize,
                                    m_context.getResourceInterface(), begin(pipelines), end(pipelines));
            ...
            if (m_context.getDeviceVulkanSC10Properties().recyclePipelineMemory)
            {
                ...
                createGraphicsPipelines(vkd, device, vertexShaderModule.get(), fragmentShaderModule.get(),
                                        renderPasses[0]->get(), pipelineLayouts[0]->get(), pipelineDefaultSize,
                                        m_context.getResourceInterface(),
                                        begin(pipelines) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(pipelines));
        ...
        case TRC_COMPUTE_PIPELINE:
        {
            ...
            Move<VkShaderModule> shaderModule =
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("compute"), 0u);

            std::vector<PipelineSp> pipelines(VERIFYMAXVALUES_OBJECT_COUNT);
            createComputePipelines(vkd, device, shaderModule.get(), pipelineLayouts[0]->get(), pipelineDefaultSize,
                                   m_context.getResourceInterface(), begin(pipelines), end(pipelines));
        ...
        case TRC_DESCRIPTORSET_LAYOUT:
        {
            ...
            std::fill(begin(descriptorSetLayouts) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(descriptorSetLayouts),
                      DescriptorSetLayoutSp());
            createDescriptorSetLayouts(vkd, device, begin(descriptorSetLayouts) + VERIFYMAXVALUES_OBJECT_COUNT / 2,
                                       end(descriptorSetLayouts));
        ...
        case TRC_DESCRIPTORSET:
        {
            ...
            createDescriptorPools(vkd, device, VERIFYMAXVALUES_OBJECT_COUNT, begin(descriptorPools),
                                  end(descriptorPools));

            std::vector<DescriptorSetSp> descriptorSets(VERIFYMAXVALUES_OBJECT_COUNT);
            createDescriptorSets(vkd, device, descriptorPools[0]->get(), descriptorSetLayouts[0]->get(),
                                 begin(descriptorSets), end(descriptorSets));
            if (m_context.getDeviceVulkanSC10Properties().recycleDescriptorSetMemory)
            {
                std::fill(begin(descriptorSets) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(descriptorSets),
                          DescriptorSetSp());
                createDescriptorSets(vkd, device, descriptorPools[0]->get(), descriptorSetLayouts[0]->get(),
                                     begin(descriptorSets) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(descriptorSets));
        ...
        case TRC_FRAMEBUFFER:
        {
            ...
            createFramebuffers(vkd, device, renderPasses[0]->get(), imageViews[0]->get(), begin(framebuffers),
                               end(framebuffers));
            std::fill(begin(framebuffers) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(framebuffers), FramebufferSp());
            createFramebuffers(vkd, device, renderPasses[0]->get(), imageViews[0]->get(),
                               begin(framebuffers) + VERIFYMAXVALUES_OBJECT_COUNT / 2, end(framebuffers));
...
{
public:
    VerifyPipelinePoolSizes(Context &context, const TestParams &testParams_)
        : DeviceObjectReservationInstance(context, testParams_)
    {
    }
    ...
    void performTest(const DeviceInterface &vk, VkDevice device) override
    {
        ...
        Move<VkShaderModule> fragmentShader =
            createShaderModule(vk, device, m_context.getBinaryCollection().get("fragment"), 0);
        ...
        VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo = prepareSimpleGraphicsPipelineCI(
            vertexInputStateCreateInfo, shaderStageCreateInfos, inputAssemblyStateCreateInfo, viewPortStateCreateInfo,
...
tcu::TestCaseGroup *createDeviceObjectReservationTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
        testCtx, "device_object_reservation", "Tests verifying VkDeviceObjectReservationCreateInfo"));

    // add basic tests
    {
        de::MovePtr<tcu::TestCaseGroup> basicGroup(new tcu::TestCaseGroup(group->getTestContext(), "basic", ""));

        basicGroup->addChild(new InstanceFactory1<DeviceObjectReservationInstance, TestParams>(
            testCtx, tcu::NODETYPE_SELF_VALIDATE, "create_device", "", TestParams()));
        basicGroup->addChild(new InstanceFactory1<MultipleReservation, TestParams>(
            testCtx, tcu::NODETYPE_SELF_VALIDATE, "multiple_device_object_reservation", "", TestParams()));
    ...
    // add tests verifying device limits
    {
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> maxValGroup(
                new tcu::TestCaseGroup(group->getTestContext(), "max_values", ""));

            for (int32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testMaxValues); ndx++)
            {
                TestParams testParams{testMaxValues[ndx].testMaxValues, TRC_UNDEFINED};
                maxValGroup->addChild(
                    new InstanceFactory1WithSupport<VerifyMaxValues, TestParams, FunctionSupport1<TestParams>>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, testMaxValues[ndx].name, "", testParams,
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> requestCountGroup(
                new tcu::TestCaseGroup(group->getTestContext(), "request_count", ""));

            for (int32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testRequestCounts); ndx++)
            {
                TestParams testParams{TMV_UNDEFINED, testRequestCounts[ndx].requestCount};
                requestCountGroup->addChild(
                    new InstanceFactory1WithSupport<VerifyRequestCounts, TestParams, FunctionSupport1<TestParams>,
                                                    ProgramsVerifyLimits>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, testRequestCounts[ndx].name, "", ProgramsVerifyLimits(),
                        testParams,
                        typename FunctionSupport1<TestParams>::Args(checkSupportVerifyRequestCounts, testParams)));
            }
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> ppsGroup(
            new tcu::TestCaseGroup(group->getTestContext(), "pipeline_pool_size", ""));
        ...
        for (int32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(poolSizes); ndx++)
        {
            TestParams testParams(TMV_UNDEFINED, TRC_UNDEFINED, poolSizes[ndx].type);

            ppsGroup->addChild(new InstanceFactory1<VerifyPipelinePoolSizes, TestParams, ProgramsVerifyLimits>(
                testCtx, tcu::NODETYPE_SELF_VALIDATE, poolSizes[ndx].name, "", ProgramsVerifyLimits(), testParams));
        }
