...
public:
    RobustBufferReadTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                         VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
                         bool testPipelineRobustness, VkDeviceSize readAccessRange, bool readFromStorage,
...
public:
    RobustBufferWriteTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                          VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
                          bool testPipelineRobustness, VkDeviceSize writeAccessRange, bool accessOutOfBackingMemory);
...
{
public:
    BufferAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
...
{
public:
    BufferReadInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper, Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                       de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
                       de::MovePtr<vk::DeviceDriverSC, vk::DeinitDeviceDeleter> deviceDriver,
...
{
public:
    BufferWriteInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper, Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                        de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
                        de::MovePtr<vk::DeviceDriverSC, vk::DeinitDeviceDeleter> deviceDriver,
...
TestInstance *RobustBufferReadTest::createInstance(Context &context) const
{
    ...
    Move<VkDevice> device =
        createRobustBufferAccessDevice(context, instanceWrapper->instance, instanceWrapper->instance.getDriver(),
                                       m_testPipelineRobustness ? &features2 : DE_NULL);
#ifndef CTS_USES_VULKANSC
    de::MovePtr<vk::DeviceDriver> deviceDriver =
    ...
    return new BufferReadInstance(context, instanceWrapper, device, deviceDriver, m_shaderType, m_shaderStage,
                                  m_bufferFormat, m_readFromStorage, m_readAccessRange, m_accessOutOfBackingMemory,
    ...
    de::MovePtr<vk::DeviceDriverSC, vk::DeinitDeviceDeleter> deviceDriver =
        de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(context.getPlatformInterface(), instanceWrapper->instance, *device,
                               context.getTestContext().getCommandLine(), context.getResourceInterface(),
                               context.getDeviceVulkanSC10Properties(), context.getDeviceProperties()),
            DeinitDeviceDeleter(context.getResourceInterface().get(), *device));
#endif // CTS_USES_VULKANSC

    return new BufferWriteInstance(context, instanceWrapper, device, deviceDriver, m_shaderType, m_shaderStage,
                                   m_bufferFormat, m_writeAccessRange, m_accessOutOfBackingMemory,
                                   m_testPipelineRobustness);
}

// BufferAccessInstance

BufferAccessInstance::BufferAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
                                           Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                                           de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
    ...
    , m_testPipelineRobustness(testPipelineRobustness)
{
    ...
    // Check format support
    {
        ...
        const VkFormatProperties formatProperties =
            getPhysicalDeviceFormatProperties(instanceWrapper->instance.getDriver(), physicalDevice, m_bufferFormat);
    ...
    if (m_shaderStage == VK_SHADER_STAGE_COMPUTE_BIT)
    {
        m_testEnvironment = de::MovePtr<TestEnvironment>(
            new ComputeEnvironment(m_context, m_instanceWrapper->instance, m_instanceWrapper->instance.getDriver(),
                                   *m_device, *m_descriptorSetLayout, *m_descriptorSet, m_testPipelineRobustness));
    }
    else
    {
        using tcu::Vec4;
        ...
        m_testEnvironment = de::MovePtr<TestEnvironment>(
            new GraphicsEnvironment(m_context, m_instanceWrapper->instance, m_instanceWrapper->instance.getDriver(),
                                    *m_device, *m_descriptorSetLayout, *m_descriptorSet,
...
BufferReadInstance::BufferReadInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
                                       Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                                       de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
...
BufferWriteInstance::BufferWriteInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
                                         Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                                         de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
