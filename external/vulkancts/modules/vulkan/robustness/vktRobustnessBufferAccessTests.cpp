...
public:
    ...
    RobustBufferAccessTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
{
public:
    RobustBufferReadTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                         VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
...
{
public:
    RobustBufferWriteTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                          VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
...
{
public:
    BufferAccessInstance(Context &context, Move<VkDevice> device, ShaderType shaderType, VkShaderStageFlags shaderStage,
...
{
public:
    BufferReadInstance(Context &context, Move<VkDevice> device, ShaderType shaderType, VkShaderStageFlags shaderStage,
                       VkFormat bufferFormat, bool readFromStorage, VkDeviceSize inBufferAccessRange,
...
{
public:
    BufferWriteInstance(Context &context, Move<VkDevice> device, ShaderType shaderType, VkShaderStageFlags shaderStage,
...
const uint32_t RobustBufferAccessTest::s_testArraySize         = 1024;
...
RobustBufferAccessTest::RobustBufferAccessTest(tcu::TestContext &testContext, const std::string &name,
                                               const std::string &description, VkShaderStageFlags shaderStage,
...
TestInstance *RobustBufferReadTest::createInstance(Context &context) const
{
    ...
    return new BufferReadInstance(context, device, m_shaderType, m_shaderStage, m_bufferFormat, m_readFromStorage,
                                  m_readAccessRange, m_accessOutOfBackingMemory);
}
...
TestInstance *RobustBufferWriteTest::createInstance(Context &context) const
{
    ...
    return new BufferWriteInstance(context, device, m_shaderType, m_shaderStage, m_bufferFormat, m_writeAccessRange,
                                   m_accessOutOfBackingMemory);
}

// BufferAccessInstance

BufferAccessInstance::BufferAccessInstance(Context &context, Move<VkDevice> device, ShaderType shaderType,
    ...
    , m_accessOutOfBackingMemory(accessOutOfBackingMemory)
{
    ...
    DE_ASSERT(RobustBufferAccessTest::s_numberOfBytesAccessed % sizeof(uint32_t) == 0);
    ...
    // Check format support
    {
        ...
        const VkFormatProperties formatProperties   = getPhysicalDeviceFormatProperties(
            context.getInstanceInterface(), context.getPhysicalDevice(), m_bufferFormat);
    ...
    // Create buffer to write data into
    {
        ...
        {
            if (m_outBufferAllocSize >=
                ((RobustBufferAccessTest::s_testArraySize + 1) * RobustBufferAccessTest::s_numberOfBytesAccessed))
            {
...
BufferReadInstance::BufferReadInstance(Context &context, Move<VkDevice> device, ShaderType shaderType,
                                       VkShaderStageFlags shaderStage, VkFormat bufferFormat, bool readFromStorage,
...
BufferWriteInstance::BufferWriteInstance(Context &context, Move<VkDevice> device, ShaderType shaderType,
                                         VkShaderStageFlags shaderStage, VkFormat bufferFormat,
                                         VkDeviceSize writeBufferAccessRange, bool accessOutOfBackingMemory)

    : BufferAccessInstance(context, device, shaderType, shaderStage, bufferFormat, BUFFER_ACCESS_TYPE_WRITE,
                           RobustBufferAccessTest::s_numberOfBytesAccessed, // inBufferAccessRange
                           writeBufferAccessRange, accessOutOfBackingMemory)
{
}
...
static void addBufferAccessTests(tcu::TestContext &testCtx, tcu::TestCaseGroup *parentNode)
{
    ...
    const VkFormat bufferFormats[] = {VK_FORMAT_R32_SINT, VK_FORMAT_R32_UINT, VK_FORMAT_R32_SFLOAT};
    ...
    for (int stageNdx = 0; stageNdx < DE_LENGTH_OF_ARRAY(bufferAccessStages); stageNdx++)
    {
        ...
        for (int shaderTypeNdx = 0; shaderTypeNdx < SHADER_TYPE_COUNT; shaderTypeNdx++)
        {
            ...
            for (size_t formatNdx = 0; formatNdx < formatsLength; formatNdx++)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> ssboWriteTests(
                    new tcu::TestCaseGroup(testCtx, "oob_storage_write", ""));
                ...
                for (size_t rangeNdx = 0; rangeNdx < rangesLength; rangeNdx++)
                {
                    ...
                    uboReadTests->addChild(new RobustBufferReadTest(testCtx, rangeConfig.name, "", stage,
                                                                    (ShaderType)shaderTypeNdx, bufferFormat,
                                                                    rangeInBytes, false, false));
                    ssboReadTests->addChild(new RobustBufferReadTest(testCtx, rangeConfig.name, "", stage,
                                                                     (ShaderType)shaderTypeNdx, bufferFormat,
                                                                     rangeInBytes, true, false));
            ...
            // Read/write out of the memory that backs the buffer
            {
                ...
                outOfAllocTests->addChild(new RobustBufferReadTest(testCtx, "oob_uniform_read", "", stage,
                                                                   (ShaderType)shaderTypeNdx, format, 16, false, true));
                outOfAllocTests->addChild(new RobustBufferReadTest(testCtx, "oob_storage_read", "", stage,
                                                                   (ShaderType)shaderTypeNdx, format, 16, true, true));
                outOfAllocTests->addChild(new RobustBufferWriteTest(testCtx, "oob_storage_write", "", stage,
                                                                    (ShaderType)shaderTypeNdx, format, 16, true));
