...
void createTestBuffer(const vk::DeviceInterface &deviceInterface, const VkDevice &device, VkDeviceSize accessRange,
                      VkBufferUsageFlags usage, SimpleAllocator &allocator, Move<VkBuffer> &buffer,
                      de::MovePtr<Allocation> &bufferAlloc, AccessRangesData &data, FillBufferProcPtr fillBufferProc,
...
void populateBufferWithDummy(void *buffer, VkDeviceSize size, const void *const blob)
{
    DE_UNREF(blob);
...
public:
    ...
    RobustAccessWithPointersTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
{
public:
    RobustReadTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                   VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
                   VkDeviceSize readAccessRange, bool accessOutOfBackingMemory);
...
{
public:
    RobustWriteTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                    VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
...
{
public:
    AccessInstance(Context &context, Move<VkDevice> device, ShaderType shaderType, VkShaderStageFlags shaderStage,
                   VkFormat bufferFormat, BufferAccessType bufferAccessType, VkDeviceSize inBufferAccessRange,
...
{
public:
    ReadInstance(Context &context, Move<VkDevice> device, ShaderType shaderType, VkShaderStageFlags shaderStage,
                 VkFormat bufferFormat, VkDeviceSize inBufferAccessRange, bool accessOutOfBackingMemory);
...
{
public:
    WriteInstance(Context &context, Move<VkDevice> device, ShaderType shaderType, VkShaderStageFlags shaderStage,
                  VkFormat bufferFormat, VkDeviceSize writeBufferAccessRange, bool accessOutOfBackingMemory);
                       ...
                       bool dummy)
{
    ...
    shaderSource(op::Capability, "Shader")(op::Capability, "VariablePointersStorageBuffer")(
        op::Extension, "\"SPV_KHR_storage_buffer_storage_class\"")(op::Extension, "\"SPV_KHR_variable_pointers\"")(
        var.version, is, op::ExtInstImport, "\"GLSL.std.450\"")(op::MemoryModel, "Logical", "GLSL450");
    ...
    else // this is a start of actual shader that tests variable pointers
    {
        ...
        shaderSource(op::Decorate, var.dataArrayType, "ArrayStride", 64)
            ...
            // an output block
            (op::MemberDecorate, var.dataOutputType, 0, "Offset", 0)(op::Decorate, var.dataOutputType, "Block")
            ...
            (var.voidId, is, op::TypeVoid)(var.voidFuncVoid, is, op::TypeFunction, var.voidId)
                ...
                (var.boolean, is, op::TypeBool)
                    ...
                    (var.f32, is, op::TypeFloat, 32)(var.s32, is, op::TypeInt, 32, 1)(var.u32, is, op::TypeInt, 32, 0)
                        ...
                        (var.v4f32, is, op::TypeVector, var.f32, 4)(var.v4s32, is, op::TypeVector, var.s32,
...
RobustReadTest::RobustReadTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                               VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
                               VkDeviceSize readAccessRange, bool accessOutOfBackingMemory)
...
TestInstance *RobustReadTest::createInstance(Context &context) const
{
    ...
    if (pointerFeatures.variablePointersStorageBuffer != true)
        return new NotSupportedInstance(
            context, std::string("VariablePointersStorageBuffer support is required for this test."));
    ...
    return new ReadInstance(context, device, m_shaderType, m_shaderStage, m_bufferFormat, m_readAccessRange,
...
RobustWriteTest::RobustWriteTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                                 VkShaderStageFlags shaderStage, ShaderType shaderType, VkFormat bufferFormat,
                                 VkDeviceSize writeAccessRange, bool accessOutOfBackingMemory)
...
TestInstance *RobustWriteTest::createInstance(Context &context) const
{
    ...
    // We need a device with enabled robust buffer access feature (it is disabled in default device)
    Move<VkDevice> device = createRobustBufferAccessDevice(context);
    return new WriteInstance(context, device, m_shaderType, m_shaderStage, m_bufferFormat, m_writeAccessRange,
                             m_accessOutOfBackingMemory);
}
...
AccessInstance::AccessInstance(Context &context, Move<VkDevice> device, ShaderType shaderType,
                               VkShaderStageFlags shaderStage, VkFormat bufferFormat, BufferAccessType bufferAccessType,
                               VkDeviceSize inBufferAccessRange, VkDeviceSize outBufferAccessRange,
    ...
    , m_accessOutOfBackingMemory(accessOutOfBackingMemory)
{
    ...
    createTestBuffer(vk, *m_device, inBufferAccessRange, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, memAlloc, m_inBuffer,
                     m_inBufferAlloc, m_inBufferAccess, &populateBufferWithValues, &m_bufferFormat);
    createTestBuffer(vk, *m_device, outBufferAccessRange, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, memAlloc, m_outBuffer,
                     m_outBufferAlloc, m_outBufferAccess, &populateBufferWithDummy, DE_NULL);
    ...
    createTestBuffer(vk, *m_device, 3 * sizeof(int32_t), VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, memAlloc, m_indicesBuffer,
                     m_indicesBufferAlloc, indicesAccess, &populateBufferWithCopy, &indices);
    ...
    if (m_shaderStage == VK_SHADER_STAGE_COMPUTE_BIT)
    {
        m_testEnvironment = de::MovePtr<TestEnvironment>(
            new ComputeEnvironment(m_context, *m_device, *m_descriptorSetLayout, *m_descriptorSet));
    }
    else
    {
        using tcu::Vec4;
        ...
        createTestBuffer(vk, *m_device, vertexBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, memAlloc, m_vertexBuffer,
                         m_vertexBufferAlloc, vertexAccess, &populateBufferWithCopy, &vertices);
...
bool AccessInstance::isOutBufferValueUnchanged(VkDeviceSize offsetInBytes, VkDeviceSize valueSize)
{
    ...
    const uint32_t defaultValue      = 0xBABABABAu;
        ...
        else
        {
            ...
            if (isOutOfBoundsAccess)
            {
                ...
                if (isValuePartiallyOutOfBounds && !m_accessOutOfBackingMemory)
                {
                    ...
                    if (isReadAccess)
                    {
                        isWithinBoundsPartOk = isValueWithinBufferOrZero(inDataPtr, m_inBufferAccess.allocSize,
                                                                         outValuePtr, distanceToOutOfBounds);
                ...
                if (!isValidValue)
                {
                    ...
                    {
                        matchesVec4Pattern = verifyOutOfBoundsVec4(
                            static_cast<const uint32_t *>(static_cast<const void *>(outValuePtr)) - 3, m_bufferFormat);
                    }
...
ReadInstance::ReadInstance(Context &context, Move<VkDevice> device, ShaderType shaderType,
                           VkShaderStageFlags shaderStage, VkFormat bufferFormat,
                           //bool                    readFromStorage,
                           VkDeviceSize inBufferAccessRange, bool accessOutOfBackingMemory)

    : AccessInstance(context, device, shaderType, shaderStage, bufferFormat, BUFFER_ACCESS_TYPE_READ_FROM_STORAGE,
...
WriteInstance::WriteInstance(Context &context, Move<VkDevice> device, ShaderType shaderType,
                             VkShaderStageFlags shaderStage, VkFormat bufferFormat, VkDeviceSize writeBufferAccessRange,
                             bool accessOutOfBackingMemory)

    : AccessInstance(context, device, shaderType, shaderStage, bufferFormat, BUFFER_ACCESS_TYPE_WRITE_TO_STORAGE,
