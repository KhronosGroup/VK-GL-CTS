...
class SingletonDevice
{
    SingletonDevice(Context &context)
        : m_context(context)
        , m_instanceWrapper(new CustomInstanceWrapper(context, context.getInstanceExtensions()))
        , m_logicalDevice()
    {
        ...
        const VkPhysicalDevice physicalDevice =
            chooseDevice(m_instanceWrapper->instance.getDriver(), m_instanceWrapper->instance,
                         context.getTestContext().getCommandLine());
        m_instanceWrapper->instance.getDriver().getPhysicalDeviceFeatures2(physicalDevice, &features2);
        m_logicalDevice = createRobustBufferAccessDevice(context, m_instanceWrapper->instance,
                                                         m_instanceWrapper->instance.getDriver(), &features2);

#ifndef CTS_USES_VULKANSC
        m_deviceDriver = de::MovePtr<DeviceDriver>(
            new DeviceDriver(context.getPlatformInterface(), m_instanceWrapper->instance, *m_logicalDevice));
#else
        m_deviceDriver = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(context.getPlatformInterface(), m_instanceWrapper->instance, *m_logicalDevice,
...
using PipelineRobustnessImageRobustnessScalarSingleton =
    SingletonDevice<kImageRobustness | kPipelineRobustness | kScalarBlockLayout>;
using PipelineRobustnessRobustness2ScalarSingleton =
    SingletonDevice<kRobustness2 | kPipelineRobustness | kScalarBlockLayout>;
...
using ImageRobustnessInt64AtomicsScalarSingleton =
    SingletonDevice<kImageRobustness | kShaderImageInt64Atomics | kScalarBlockLayout>;
using Robustness2Int64AtomicsScalarSingleton =
    SingletonDevice<kRobustness2 | kShaderImageInt64Atomics | kScalarBlockLayout>;

using PipelineRobustnessImageRobustnessInt64AtomicsSingleton =
    SingletonDevice<kImageRobustness | kPipelineRobustness | kShaderImageInt64Atomics>;
using PipelineRobustnessRobustness2Int64AtomicsSingleton =
    SingletonDevice<kRobustness2 | kPipelineRobustness | kShaderImageInt64Atomics>;

using PipelineRobustnessImageRobustnessInt64AtomicsScalarSingleton =
    SingletonDevice<kImageRobustness | kPipelineRobustness | kShaderImageInt64Atomics | kScalarBlockLayout>;
using PipelineRobustnessRobustness2Int64AtomicsScalarSingleton =
    SingletonDevice<kRobustness2 | kPipelineRobustness | kShaderImageInt64Atomics | kScalarBlockLayout>;
...
RobustnessExtsTestCase::RobustnessExtsTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                               const CaseDef data)
...
tcu::TestStatus RobustnessExtsTestInstance::iterate(void)
{
    ...
    else if (m_data.stage == STAGE_RAYGEN)
    {
        ...
        sbtBuffer = de::MovePtr<BufferWithMemory>(new BufferWithMemory(
            vk, device, allocator,
            makeBufferCreateInfo(rayTracingProperties.shaderGroupHandleSize,
                                 VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_RAY_TRACING_BIT_NV),
            MemoryRequirement::HostVisible));
...
static void createTests(tcu::TestCaseGroup *group, bool robustness2, bool pipelineRobustness)
{
    ...
    TestGroupCase fullLenCases32Bit[] = {
        {~0U, "null_descriptor", ""}, {0, "img", ""},     {4, "len_4", ""},     {8, "len_8", ""},
        {12, "len_12", ""},           {16, "len_16", ""}, {20, "len_20", ""},   {31, "len_31", ""},
        {32, "len_32", ""},           {33, "len_33", ""}, {35, "len_35", ""},   {36, "len_36", ""},
        {39, "len_39", ""},           {40, "len_41", ""}, {252, "len_252", ""}, {256, "len_256", ""},
        {260, "len_260", ""},
    };

    TestGroupCase fullLenCases64Bit[] = {
        {~0U, "null_descriptor", ""}, {0, "img", ""},     {8, "len_8", ""},     {16, "len_16", ""},
        {24, "len_24", ""},           {32, "len_32", ""}, {40, "len_40", ""},   {62, "len_62", ""},
        {64, "len_64", ""},           {66, "len_66", ""}, {70, "len_70", ""},   {72, "len_72", ""},
        {78, "len_78", ""},           {80, "len_80", ""}, {504, "len_504", ""}, {512, "len_512", ""},
        {520, "len_520", ""},
    };
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> pushGroup(
            new tcu::TestCaseGroup(testCtx, pushCases[pushNdx].name, pushCases[pushNdx].name));
        for (int tempNdx = 0; tempNdx < DE_LENGTH_OF_ARRAY(tempCases); tempNdx++)
        {
            de::MovePtr<tcu::TestCaseGroup> tempGroup(
                new tcu::TestCaseGroup(testCtx, tempCases[tempNdx].name, tempCases[tempNdx].name));
            for (int fmtNdx = 0; fmtNdx < DE_LENGTH_OF_ARRAY(fmtCases); fmtNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> fmtGroup(
                    new tcu::TestCaseGroup(testCtx, fmtCases[fmtNdx].name, fmtCases[fmtNdx].name));
                ...
                for (int unrollNdx = 0; unrollNdx < DE_LENGTH_OF_ARRAY(unrollCases); unrollNdx++)
                {
                    de::MovePtr<tcu::TestCaseGroup> unrollGroup(
                        new tcu::TestCaseGroup(testCtx, unrollCases[unrollNdx].name, unrollCases[unrollNdx].name));
                    ...
                    for (int volNdx = 0; volNdx < DE_LENGTH_OF_ARRAY(volCases); volNdx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> volGroup(
                            new tcu::TestCaseGroup(testCtx, volCases[volNdx].name, volCases[volNdx].name));
                        ...
                        for (int descNdx = 0; descNdx < numDescCases; descNdx++)
                        {
                            de::MovePtr<tcu::TestCaseGroup> descGroup(
                                new tcu::TestCaseGroup(testCtx, descCases[descNdx].name, descCases[descNdx].name));
                            ...
                            {
                                de::MovePtr<tcu::TestCaseGroup> rwGroup(new tcu::TestCaseGroup(
                                    testCtx, readOnlyCases[roNdx].name, readOnlyCases[roNdx].name));
                                ...
                                {
                                    de::MovePtr<tcu::TestCaseGroup> fmtQualGroup(new tcu::TestCaseGroup(
                                        testCtx, fmtQualCases[fmtQualNdx].name, fmtQualCases[fmtQualNdx].name));
                                    ...
                                    for (int lenNdx = 0; lenNdx < numLenCases; lenNdx++)
                                    {
                                        ...
                                        de::MovePtr<tcu::TestCaseGroup> lenGroup(new tcu::TestCaseGroup(
                                            testCtx, lenCases[lenNdx].name, lenCases[lenNdx].name));
                                        for (int sampNdx = 0; sampNdx < DE_LENGTH_OF_ARRAY(sampCases); sampNdx++)
                                        {
                                            de::MovePtr<tcu::TestCaseGroup> sampGroup(new tcu::TestCaseGroup(
                                                testCtx, sampCases[sampNdx].name, sampCases[sampNdx].name));
                                            ...
                                            for (int viewNdx = 0; viewNdx < DE_LENGTH_OF_ARRAY(viewCases); viewNdx++)
                                            {
                                                ...
                                                de::MovePtr<tcu::TestCaseGroup> viewGroup(new tcu::TestCaseGroup(
                                                    testCtx, viewCases[viewNdx].name, viewCases[viewNdx].name));
                                                     ...
                                                     stageNdx++)
                                                {
                                                    ...
                                                    viewGroup->addChild(
                                                        new RobustnessExtsTestCase(testCtx, stageCases[stageNdx].name,
                                                                                   stageCases[stageNdx].name, c));
                                                }
...
{
    return createTestGroup(testCtx, "robustness2", "VK_EXT_robustness2 tests", createRobustness2Tests, cleanupGroup);
}

tcu::TestCaseGroup *createImageRobustnessTests(tcu::TestContext &testCtx)
{
    return createTestGroup(testCtx, "image_robustness", "VK_EXT_image_robustness tests", createImageRobustnessTests,
                           cleanupGroup);
}
