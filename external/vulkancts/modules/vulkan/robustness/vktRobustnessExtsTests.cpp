...
{
    SingletonDevice(Context &context) : m_logicalDevice()
    {
...
void RobustnessExtsTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER:
        decls << "layout(scalar, set = 0, binding = 1) " << vol << "buffer sbodef0_1 { " << bufType
              << " val[]; } ssbo0_1;\n";
        decls << "layout(scalar, set = 0, binding = 1) " << vol << "buffer sbodef0_1_pad { vec4 pad; " << bufType
              << " val[]; } ssbo0_1_pad;\n";
        break;
    case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
        switch (format)
...
tcu::TestStatus RobustnessExtsTestInstance::iterate(void)
{
    ...
    const bool featureSampledImage = ((getPhysicalDeviceFormatProperties(m_context.getInstanceInterface(),
                                                                         m_context.getPhysicalDevice(), m_data.format)
                    ...
                    for (int volNdx = 0; volNdx < DE_LENGTH_OF_ARRAY(volCases); volNdx++)
                    {
                        ...
                        for (int descNdx = 0; descNdx < numDescCases; descNdx++)
                        {
                            ...
                            for (int fmtQualNdx = 0; fmtQualNdx < DE_LENGTH_OF_ARRAY(fmtQualCases); fmtQualNdx++)
                            {
                                ...
                                int numLenCases =
                                    robustness2 ? DE_LENGTH_OF_ARRAY((isR64 ? fullLenCases64Bit : fullLenCases32Bit)) :
                                ...
                                for (int lenNdx = 0; lenNdx < numLenCases; lenNdx++)
                                {
                                    ...
                                    de::MovePtr<tcu::TestCaseGroup> lenGroup(
                                        new tcu::TestCaseGroup(testCtx, lenCases[lenNdx].name, lenCases[lenNdx].name));
                                        ...
                                        for (int viewNdx = 0; viewNdx < DE_LENGTH_OF_ARRAY(viewCases); viewNdx++)
                                        {
                                                 ...
                                                 stageNdx++)
                                            {
                                                ...
                                                CaseDef c = {
                                                    ...
                                                    {imageDim[0], imageDim[1], imageDim[2]}, // uint32_t imageDim[3];
                                                };

                                                viewGroup->addChild(new RobustnessExtsTestCase(
                                                    testCtx, stageCases[stageNdx].name, stageCases[stageNdx].name, c));
                                            }
