        ...
        , m_logicalDevice()
    {
        ...
        m_deviceDriver = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(context.getPlatformInterface(), m_instanceWrapper->instance, *m_logicalDevice,
                               ...
                               m_context.getDeviceVulkanSC10Properties()),
...
void RobustnessExtsTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    std::string SupportR64 =
        (is64BitFormat ? std::string("#extension GL_EXT_shader_explicit_arithmetic_types_int64 : require\n"
    ...
    case STAGE_COMPUTE:
    {
        ...
        css << "#version 450 core\n"
            ...
            << SupportR64 << decls.str()
            << "layout(local_size_x = 1, local_size_y = 1) in;\n"
               "void main()\n"
               "{\n"
               "  "
            << vecType << " accum = " << vecType
            << "(0);\n"
               "  "
            << vecType
            << " temp;\n"
               "  "
            << qLevelType << " temp_ql;\n"
            << checks.str() << "  " << vecType << " color = (accum != " << vecType << "(0)) ? " << vecType
            << "(0,0,0,0) : " << vecType
            << "(1,0,0,1);\n"
...
tcu::TestStatus RobustnessExtsTestInstance::iterate(void)
{
    ...
    if (!m_data.nullDescriptor)
    {
        ...
        buffer    = de::MovePtr<BufferWithMemory>(new BufferWithMemory(
            vk, device, allocator,
            makeBufferCreateInfo(size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT |
