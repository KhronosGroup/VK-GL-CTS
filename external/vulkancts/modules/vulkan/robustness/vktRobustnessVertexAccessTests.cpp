...
TestInstance *DrawAccessTest::createInstance(Context &context) const
{
...
#ifndef CTS_USES_VULKANSC
    de::MovePtr<vk::DeviceDriver> deviceDriver =
        de::MovePtr<DeviceDriver>(new DeviceDriver(context.getPlatformInterface(), context.getInstance(), *device));
#else
    de::MovePtr<vk::DeviceDriverSC, vk::DeinitDeviceDeleter> deviceDriver =
    ...
    return new DrawAccessInstance(context, device, deviceDriver, m_inputFormat, m_numVertexValues, m_numInstanceValues,
                                  m_numVertices, m_numInstances);
}
...
TestInstance *DrawIndexedAccessTest::createInstance(Context &context) const
{
    ...
    return new DrawIndexedAccessInstance(context, device, deviceDriver, m_inputFormat, m_numVertexValues,
...
DrawAccessInstance::DrawAccessInstance(Context &context, Move<VkDevice> device,
                                       ...
                                       VkFormat inputFormat, uint32_t numVertexValues, uint32_t numInstanceValues,
                                       uint32_t numVertices, uint32_t numInstances)
    : VertexAccessInstance(context, device, deviceDriver, inputFormat, numVertexValues, numInstanceValues, numVertices,
                           numInstances,
