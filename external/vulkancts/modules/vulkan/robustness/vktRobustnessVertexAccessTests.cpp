...
{
public:
    VertexAccessTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
{
public:
    DrawAccessTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                   VkFormat inputFormat, uint32_t numVertexValues, uint32_t numInstanceValues, uint32_t numVertices,
                   uint32_t numInstances);
...
public:
    ...
    DrawIndexedAccessTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
{
public:
    VertexAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
...
{
public:
    DrawAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper, Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                       de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
                       de::MovePtr<vk::DeviceDriverSC, vk::DeinitDeviceDeleter> deviceDriver,
...
{
public:
    DrawIndexedAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
                              Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                              de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
...
VertexAccessTest::VertexAccessTest(tcu::TestContext &testContext, const std::string &name,
                                   const std::string &description, VkFormat inputFormat, uint32_t numVertexValues,
                                   uint32_t numInstanceValues, uint32_t numVertices, uint32_t numInstances)
...
DrawAccessTest::DrawAccessTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                               VkFormat inputFormat, uint32_t numVertexValues, uint32_t numInstanceValues,
                               uint32_t numVertices, uint32_t numInstances)

    : VertexAccessTest(testContext, name, description, inputFormat, numVertexValues, numInstanceValues, numVertices,
                       numInstances)
{
}
...
TestInstance *DrawAccessTest::createInstance(Context &context) const
{
...
#ifndef CTS_USES_VULKANSC
    de::MovePtr<vk::DeviceDriver> deviceDriver =
        de::MovePtr<DeviceDriver>(new DeviceDriver(context.getPlatformInterface(), instanceWrapper->instance, *device));
#else
    de::MovePtr<vk::DeviceDriverSC, vk::DeinitDeviceDeleter> deviceDriver =
        de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(context.getPlatformInterface(), instanceWrapper->instance, *device,
    ...
    return new DrawAccessInstance(context, instanceWrapper, device, deviceDriver, m_inputFormat, m_numVertexValues,
                                  m_numInstanceValues, m_numVertices, m_numInstances);
}
...
DrawIndexedAccessTest::DrawIndexedAccessTest(tcu::TestContext &testContext, const std::string &name,
                                             const std::string &description, VkFormat inputFormat,
                                             IndexConfig indexConfig)

    : VertexAccessTest(testContext, name, description, inputFormat,
...
TestInstance *DrawIndexedAccessTest::createInstance(Context &context) const
{
    ...
    return new DrawIndexedAccessInstance(context, instanceWrapper, device, deviceDriver, m_inputFormat,
                                         m_numVertexValues, m_numInstanceValues, m_numVertices, m_numInstances,
                                         s_indexConfigs[m_indexConfig]);
}

// VertexAccessInstance

VertexAccessInstance::VertexAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
                                           Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                                           de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
    ...
    , m_numInstances(numInstances)
{
    ...
    const VkPhysicalDevice physicalDevice = chooseDevice(
        instanceWrapper->instance.getDriver(), instanceWrapper->instance, context.getTestContext().getCommandLine());
    SimpleAllocator memAlloc(vk, *m_device,
                             getPhysicalDeviceMemoryProperties(instanceWrapper->instance.getDriver(), physicalDevice));
    ...
    if (m_inputFormat == VK_FORMAT_R64_UINT || m_inputFormat == VK_FORMAT_R64_SINT)
    {
        const VkFormatProperties formatProperties =
            getPhysicalDeviceFormatProperties(instanceWrapper->instance.getDriver(), physicalDevice, m_inputFormat);
    ...
    // Create result ssbo
    {
        ...
        m_outBufferAlloc =
            memAlloc.allocate(getBufferMemoryRequirements(vk, *m_device, *m_outBuffer), MemoryRequirement::HostVisible);
    ...
    // Setup graphics test environment
    {
        ...
        m_graphicsTestEnvironment = de::MovePtr<GraphicsEnvironment>(new GraphicsEnvironment(
            m_context, m_instanceWrapper->instance, m_instanceWrapper->instance.getDriver(), *m_device,
...
DrawAccessInstance::DrawAccessInstance(Context &context, std::shared_ptr<CustomInstanceWrapper> instanceWrapper,
                                       Move<VkDevice> device,
#ifndef CTS_USES_VULKANSC
                                       de::MovePtr<vk::DeviceDriver> deviceDriver,
#else
                                       ...
                                       VkFormat inputFormat, uint32_t numVertexValues, uint32_t numInstanceValues,
                                       uint32_t numVertices, uint32_t numInstances)
    : VertexAccessInstance(context, instanceWrapper, device, deviceDriver, inputFormat, numVertexValues,
                           numInstanceValues, numVertices, numInstances,
...
DrawIndexedAccessInstance::DrawIndexedAccessInstance(Context &context,
    ...
    : VertexAccessInstance(context, instanceWrapper, device, deviceDriver, inputFormat, numVertexValues,
                           numInstanceValues, numVertices, numInstances, indices)
    , m_indices(indices)
{
}
...
static tcu::TestCaseGroup *createDrawTests(tcu::TestContext &testCtx, VkFormat format)
{
    ...
    const TestConfig testConfigs[] = {
        ...
        {"vertex_out_of_bounds", "Create data for 6 vertices, draw 9 vertices", format, numChannels * 2 * 6,
         numChannels, 9, 1},
        {"vertex_incomplete", "Create data for half a vertex, draw 3 vertices", format, numChannels, numChannels, 3, 1},
        {"instance_out_of_bounds", "Create data for 1 instance, draw 3 instances", format, numChannels * 2 * 9,
         numChannels, 3, 3},
    };
    ...
    for (int i = 0; i < DE_LENGTH_OF_ARRAY(testConfigs); i++)
    {
        ...
        drawTests->addChild(new DrawAccessTest(testCtx, config.name, config.description, config.inputFormat,
                                               config.numVertexValues, config.numInstanceValues, config.numVertices,
                                               config.numInstances));
    }
...
static tcu::TestCaseGroup *createDrawIndexedTests(tcu::TestContext &testCtx, VkFormat format)
{
    ...
    const TestConfig testConfigs[] = {
        ...
        {"last_index_out_of_bounds", "Only last index is out of bounds", format,
         DrawIndexedAccessTest::INDEX_CONFIG_LAST_INDEX_OUT_OF_BOUNDS},
        {"indices_out_of_bounds", "Random indices out of bounds", format,
         DrawIndexedAccessTest::INDEX_CONFIG_INDICES_OUT_OF_BOUNDS},
        {"triangle_out_of_bounds", "First triangle is out of bounds", format,
         DrawIndexedAccessTest::INDEX_CONFIG_TRIANGLE_OUT_OF_BOUNDS},
    };
    ...
    for (int i = 0; i < DE_LENGTH_OF_ARRAY(testConfigs); i++)
    {
        ...
        drawTests->addChild(new DrawIndexedAccessTest(testCtx, config.name, config.description, config.inputFormat,
                                                      config.indexConfig));
    }
