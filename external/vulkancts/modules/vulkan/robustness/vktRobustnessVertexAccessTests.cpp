...
{
public:
    VertexAccessInstance(Context &context, Move<VkDevice> device, VkFormat inputFormat, uint32_t numVertexValues,
...
{
public:
    DrawAccessInstance(Context &context, Move<VkDevice> device, VkFormat inputFormat, uint32_t numVertexValues,
                       uint32_t numInstanceValues, uint32_t numVertices, uint32_t numInstances);
...
{
public:
    DrawIndexedAccessInstance(Context &context, Move<VkDevice> device, VkFormat inputFormat, uint32_t numVertexValues,
                              uint32_t numInstanceValues, uint32_t numVertices, uint32_t numInstances,
                              const std::vector<uint32_t> &indices);
...
TestInstance *DrawAccessTest::createInstance(Context &context) const
{
    ...
    return new DrawAccessInstance(context, device, m_inputFormat, m_numVertexValues, m_numInstanceValues, m_numVertices,
                                  m_numInstances);
}
...
TestInstance *DrawIndexedAccessTest::createInstance(Context &context) const
{
    ...
    return new DrawIndexedAccessInstance(context, device, m_inputFormat, m_numVertexValues, m_numInstanceValues,
                                         m_numVertices, m_numInstances, s_indexConfigs[m_indexConfig]);
}

// VertexAccessInstance

VertexAccessInstance::VertexAccessInstance(Context &context, Move<VkDevice> device, VkFormat inputFormat,
    ...
    , m_numInstances(numInstances)
{
    ...
    // Setup graphics test environment
    {
        ...
        m_graphicsTestEnvironment = de::MovePtr<GraphicsEnvironment>(new GraphicsEnvironment(
            m_context, *m_device, *m_descriptorSetLayout, *m_descriptorSet,
...
bool VertexAccessInstance::verifyResult(void)
{
    ...
    const uint32_t outValueSize        = sizeof(uint32_t);
    bool allOk                         = true;
    ...
    for (uint32_t valueNdx = 0; valueNdx < m_outBufferSize / outValueSize; valueNdx++)
    {
        ...
        const uint32_t *outValuePtr   = (uint32_t *)outDataPtr + valueNdx;
...
DrawAccessInstance::DrawAccessInstance(Context &context, Move<VkDevice> device, VkFormat inputFormat,
                                       uint32_t numVertexValues, uint32_t numInstanceValues, uint32_t numVertices,
                                       uint32_t numInstances)
    : VertexAccessInstance(context, device, inputFormat, numVertexValues, numInstanceValues, numVertices, numInstances,
...
DrawIndexedAccessInstance::DrawIndexedAccessInstance(Context &context, Move<VkDevice> device, VkFormat inputFormat,
                                                     uint32_t numVertexValues, uint32_t numInstanceValues,
                                                     uint32_t numVertices, uint32_t numInstances,
                                                     const std::vector<uint32_t> &indices)
    : VertexAccessInstance(context, device, inputFormat, numVertexValues, numInstanceValues, numVertices, numInstances,
                           indices)
