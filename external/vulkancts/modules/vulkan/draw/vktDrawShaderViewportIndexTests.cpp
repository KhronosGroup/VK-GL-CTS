...
struct FragmentTestParams
{
    ...
    FragmentTestParams(int nvp, bool write) : numViewports(nvp), writeFromVertex(write)
    {
        if (!write)
            DE_ASSERT(nvp == 1);
    }
...
public:
    ...
    Renderer(Context &context, const UVec2 &renderSize, const int numViewports, const std::vector<UVec4> &cells,
...
tcu::TestCaseGroup *createShaderViewportIndexTests(tcu::TestContext &testCtx)
{
    ...
    for (int numViewports = 1; numViewports <= MIN_MAX_VIEWPORTS; ++numViewports)
        addFunctionCaseWithPrograms(group.get(), "vertex_shader_" + de::toString(numViewports), "", checkSupportVertex,
                                    initVertexTestPrograms, testVertexShader, numViewports);

    addFunctionCaseWithPrograms(group.get(), "fragment_shader_implicit", "", checkSupportFragment,
                                initFragmentTestPrograms, testFragmentShader, FragmentTestParams(1, false));
    for (int numViewports = 1; numViewports <= MIN_MAX_VIEWPORTS; ++numViewports)
        addFunctionCaseWithPrograms(group.get(), "fragment_shader_" + de::toString(numViewports), "",
                                    checkSupportFragment, initFragmentTestPrograms, testFragmentShader,
                                    FragmentTestParams(numViewports, true));

    for (int numViewports = 1; numViewports <= MIN_MAX_VIEWPORTS; ++numViewports)
        addFunctionCaseWithPrograms(group.get(), "tessellation_shader_" + de::toString(numViewports), "",
                                    checkSupportTessellation, initTessellationTestPrograms, testTessellationShader,
                                    numViewports);
