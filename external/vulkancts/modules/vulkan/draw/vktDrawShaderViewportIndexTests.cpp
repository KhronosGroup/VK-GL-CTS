...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const int numViewports, const std::vector<UVec4> cells)
{
    ...
    for (std::vector<UVec4>::const_iterator it = cells.begin(); it != cells.end(); ++it)
    {
        const VkViewport viewport =
            makeViewport(float(it->x()), float(it->y()), float(it->z()), float(it->w()), 0.0f, 1.0f);
    ...
    const VkPipelineViewportStateCreateInfo pipelineViewportStateInfo = {
        ...
        static_cast<uint32_t>(numViewports), // uint32_t                                    viewportCount;
        &viewports[0],                       // const VkViewport*                           pViewports;
        static_cast<uint32_t>(numViewports), // uint32_t                                    scissorCount;
...
tcu::TextureLevel generateReferenceImage(const tcu::TextureFormat format, const UVec2 &renderSize,
                                         const Vec4 &clearColor, const std::vector<UVec4> &cells,
                                         const std::vector<Vec4> &cellColors)
{
    ...
    for (std::size_t i = 0; i < cells.size(); ++i)
    {
        ...
        tcu::clear(tcu::getSubregion(image.getAccess(), cell.x(), cell.y(), cell.z(), cell.w()), cellColors[i]);
    }
...
public:
    ...
    Renderer(Context &context, const UVec2 &renderSize, const TestParams &testParams, const std::vector<UVec4> &cells,
             const VkFormat colorFormat, const Vec4 &clearColor, const std::vector<Vec4> &colors, const Shader shader)
        ...
        , m_shader(shader)
    {
        ...
        if (shader == TESSELLATION)
        {
            ...
            m_tessellationEvaluationModule = createShaderModule(
                vk, device,
                context.getBinaryCollection().get(context.contextSupports(VK_API_VERSION_1_2) ? "tese_1_2" : "tese"),
                0u);
        }

        m_vertexModule = createShaderModule(
            vk, device,
            context.getBinaryCollection().get(context.contextSupports(VK_API_VERSION_1_2) ? "vert_1_2" : "vert"), 0u);
...
protected:
    ...
    void drawCommands(Context &context, VkCommandBuffer cmdBuffer)
    {
        ...
        if (m_shader == FRAGMENT)
        {
            ...
            const auto colorsBufferCreateInfo = vk::makeBufferCreateInfo(static_cast<VkDeviceSize>(colorsBufferSize),
                                                                         vk::VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);
            m_colorsBuffer                    = SharedPtr<BufferWithMemory>(new vk::BufferWithMemory{
                vk, device, allocator, colorsBufferCreateInfo, MemoryRequirement::HostVisible});
            ...
            const auto colorsBufferDescriptorInfo =
                vk::makeDescriptorBufferInfo(m_colorsBuffer->get(), 0ull, VK_WHOLE_SIZE);
            vk::DescriptorSetUpdateBuilder updateBuilder;
            updateBuilder.writeSingle(m_descriptorSet.get(), vk::DescriptorSetUpdateBuilder::Location::binding(0u),
                                      vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, &colorsBufferDescriptorInfo);
            updateBuilder.update(vk, device);

            vk.cmdBindDescriptorSets(cmdBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout.get(), 0u, 1u,
                                     &m_descriptorSet.get(), 0u, nullptr);
        }

        vk.cmdDraw(cmdBuffer, static_cast<uint32_t>(m_numViewports * 6), 1u, 0u, 0u); // two triangles per viewport
    }
...
tcu::TestStatus testVertexFragmentShader(Context &context, const TestParams &testParams, Renderer::Shader shader)
{
...

    {
        context.getTestContext().getLog()
            << tcu::TestLog::Message << "Rendering a colorful grid of " << testParams.numViewports << " rectangle(s)."
    ...
    // Log image
    {
        ...
        const tcu::TextureLevel referenceImage =
            generateReferenceImage(mapVkFormat(colorFormat), renderSize, clearColor, cells, colors);
...
tcu::TestStatus testTessellationShader(Context &context, const TestParams testParams)
{
    ...
    // Draw
    {
        Renderer renderer(context, renderSize, testParams, cells, colorFormat, clearColor, colors,
...
tcu::TestCaseGroup *createShaderViewportIndexTests(tcu::TestContext &testCtx, const SharedGroupParams groupParams)
{
    ...
    for (testParams.numViewports = 1; testParams.numViewports <= MIN_MAX_VIEWPORTS; ++testParams.numViewports)
        addFunctionCaseWithPrograms(group.get(), "vertex_shader_" + de::toString(testParams.numViewports), checkSupport,
                                    initVertexTestPrograms, testVertexShader, testParams);

    testParams.numViewports = 1;
    addFunctionCaseWithPrograms(group.get(), "fragment_shader_implicit", checkSupport, initFragmentTestPrograms,
                                testFragmentShader, testParams);
    testParams.writeFromVertex = true;
    for (testParams.numViewports = 1; testParams.numViewports <= MIN_MAX_VIEWPORTS; ++testParams.numViewports)
        addFunctionCaseWithPrograms(group.get(), "fragment_shader_" + de::toString(testParams.numViewports),
    ...
    for (testParams.numViewports = 1; testParams.numViewports <= MIN_MAX_VIEWPORTS; ++testParams.numViewports)
        addFunctionCaseWithPrograms(group.get(), "tessellation_shader_" + de::toString(testParams.numViewports),
                                    checkSupport, initTessellationTestPrograms, testTessellationShader, testParams);
