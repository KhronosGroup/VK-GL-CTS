...
public:
    ...
    void draw(Context &context, const VkBuffer colorBuffer) const
    {
        ...
        if (m_shader == FRAGMENT)
        {
            ...
            colorsBuffer.reset(new vk::BufferWithMemory{vk, device, allocator, colorsBufferCreateInfo,
                                                        MemoryRequirement::HostVisible});
            ...
            const auto colorsBufferDescriptorInfo =
                vk::makeDescriptorBufferInfo(colorsBuffer->get(), 0ull, VK_WHOLE_SIZE);
            ...
            vk.cmdBindDescriptorSets(*m_cmdBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayout.get(), 0u, 1u,
                                     &descriptorSet.get(), 0u, nullptr);
        }

        vk.cmdDraw(*m_cmdBuffer, static_cast<uint32_t>(m_numViewports * 6), 1u, 0u, 0u); // two triangles per viewport
...
tcu::TestStatus testTessellationShader(Context &context, const TestParams testParams)
{
    ...
    // Draw
    {
        const Renderer renderer(context, renderSize, testParams, cells, colorFormat, clearColor, colors,
