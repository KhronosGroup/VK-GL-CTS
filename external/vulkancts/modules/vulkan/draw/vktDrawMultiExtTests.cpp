...
tcu::TestStatus MultiDrawInstance::iterate(void)
{
    ...
    const auto framebuffer = makeFramebuffer(vkd, device, renderPass.get(), static_cast<uint32_t>(attachments.size()),
    ...
    for (uint32_t subpassIdx = 0u; subpassIdx < imageLayers; ++subpassIdx)
    {
        pipelines.emplace_back(makeGraphicsPipeline(
            ...
            fragModule.get(), renderPass.get(), viewports, scissors, primitiveTopology, subpassIdx, patchControlPoints,
    ...
    for (uint32_t layerIdx = 0u; layerIdx < imageLayers; ++layerIdx)
    {
        ...
        if (isIndexed)
        {
            ...
            vkd.cmdDrawMultiIndexedEXT(cmdBuffer, drawInfos.drawInfoCount(), drawInfoPtr, m_params.instanceCount,
        ...
        else
        {
            ...
            vkd.cmdDrawMultiEXT(cmdBuffer, drawInfos.drawInfoCount(), drawInfoPtr, m_params.instanceCount,
