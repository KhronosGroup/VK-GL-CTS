...
{
public:
    MultiDrawTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
protected:
    void beginSecondaryCmdBuffer(VkCommandBuffer cmdBuffer, VkFormat colorFormat, VkFormat depthStencilFormat,
                                 VkRenderingFlagsKHR renderingFlags, uint32_t viewMask) const;
    void preRenderingCommands(VkCommandBuffer cmdBuffer, VkImage colorImage,
    ...
    void drawCommands(VkCommandBuffer cmdBuffer, VkPipeline pipeline, VkBuffer vertexBuffer,
...
MultiDrawTest::MultiDrawTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             const TestParams &params)
...
void MultiDrawInstance::preRenderingCommands(VkCommandBuffer cmdBuffer, VkImage colorImage,
                                             const VkImageSubresourceRange colorSubresourceRange, VkImage dsImage,
                                             const VkImageSubresourceRange dsSubresourceRange) const
{
...
void MultiDrawInstance::drawCommands(VkCommandBuffer cmdBuffer, VkPipeline pipeline, VkBuffer vertexBuffer,
                                     VkDeviceSize vertexBufferOffset, int32_t vertexOffset, VkBuffer indexBuffer,
                                     VkDeviceSize indexBufferOffset, bool isMixedMode,
...
tcu::TestStatus MultiDrawInstance::iterate(void)
{
    ...
    if (m_params.groupParams->useSecondaryCmdBuffer)
    {
        ...
        for (uint32_t layerIdx = 0u; layerIdx < imageLayers; ++layerIdx)
        {
            ...
            if (m_params.groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
            {
                beginSecondaryCmdBuffer(secCmdBuffer, colorFormat, dsFormat,
        ...
        for (int y = 0; y < iHeight; ++y)
            for (int x = 0; x < iWidth; ++x)
            {
                ...
                else
                {
                    ...
                    const auto drawIndex = (m_params.stride == 0u ? (isMosaic ? (drawInfos.drawInfoCount() - 1u) : 0u) :
                                                                    (isMosaic ? (triangleIndex / trianglesPerDraw) :
                                                                                (drawInfos.drawInfoCount() - 1u)));
                    referenceColor =
                        tcu::IVec4(static_cast<int>((drawIndex >> 8) & 0xFFu), static_cast<int>((drawIndex)&0xFFu),
