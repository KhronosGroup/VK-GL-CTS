...
#ifndef CTS_USES_VULKANSC
void NegativeViewportHeightTestInstance::beginSecondaryCmdBuffer(VkCommandBuffer cmdBuffer,
...
tcu::TestStatus NegativeViewportHeightTestInstance::iterate(void)
{
    ...
    if (m_params.groupParams->useSecondaryCmdBuffer)
    {
        ...
        if (m_params.groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
        {
            ...
            beginRendering(vk, *secCmdBuffer, *m_colorTargetView, rect, clearColor, VK_IMAGE_LAYOUT_GENERAL,
        ...
        if (!m_params.groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
            beginRendering(vk, *cmdBuffer, *m_colorTargetView, rect, clearColor, VK_IMAGE_LAYOUT_GENERAL,
...
{
public:
    NegativeViewportHeightTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
tcu::TestCaseGroup *createNegativeViewportHeightTests(tcu::TestContext &testCtx, const SharedGroupParams groupParams)
{
    ...
    return createTestGroup(testCtx, "negative_viewport_height", "Negative viewport height (VK_KHR_maintenance1)",
                           populateTestGroup, subGroupParams);
}

tcu::TestCaseGroup *createZeroViewportHeightTests(tcu::TestContext &testCtx, const SharedGroupParams groupParams)
{
    SubGroupParams subGroupParams{false, groupParams};
    return createTestGroup(testCtx, "zero_viewport_height", "Zero viewport height (VK_KHR_maintenance1)",
                           populateTestGroup, subGroupParams);
}
