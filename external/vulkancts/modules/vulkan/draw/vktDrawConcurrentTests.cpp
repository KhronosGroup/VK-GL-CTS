...
tcu::TestStatus ConcurrentDraw::iterate(void)
{
    ...
    de::MovePtr<vk::DeviceDriver> deviceDriver =
        de::MovePtr<vk::DeviceDriver>(new vk::DeviceDriver(m_context.getPlatformInterface(), instance, *computeDevice));
#else
    ...
    const vkt::compute::Buffer buffer(vk, *computeDevice, *allocator,
    ...
    const Unique<VkPipeline> pipeline(
        vkt::compute::makeComputePipeline(vk, *computeDevice, *pipelineLayout, *shaderModule));
...
{
    {
        ...
        addChild(new InstanceFactory<ConcurrentDraw>(m_testCtx, "compute_and_triangle_list",
                                                     "Draws triangle list while running a compute shader", testSpec));
    }
}
