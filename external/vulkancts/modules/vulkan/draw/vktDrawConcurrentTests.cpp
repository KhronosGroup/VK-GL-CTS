...
tcu::TestStatus ConcurrentDraw::iterate(void)
{
    ...
    computeDevice = createCustomDevice(validation, m_context.getPlatformInterface(), m_context.getInstance(), instance,
...
void ConcurrentDrawTests::init(void)
{
    ...
    addChild(new InstanceFactory<ConcurrentDraw, FunctionSupport1<ConcurrentDraw::TestSpec>>(
        m_testCtx, "compute_and_triangle_list", "Draws triangle list while running a compute shader", testSpec,
