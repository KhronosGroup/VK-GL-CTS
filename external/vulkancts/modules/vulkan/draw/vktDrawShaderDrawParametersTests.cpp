...
tcu::TestStatus DrawTest::iterate(void)
{
    // Draw
    {
        ...
        if (isIndirect())
        {
            ...
            if (isIndexed())
                m_vk.cmdDrawIndexedIndirect(*m_cmdBuffer, m_indirectBuffer->object(), 0ull, numIndirectDraws,
                                            sizeof(vk::VkDrawIndexedIndirectCommand));
            else
                m_vk.cmdDrawIndirect(*m_cmdBuffer, m_indirectBuffer->object(), 0ull, numIndirectDraws,
        ...
        else
        {
            ...
            if (isIndexed())
                m_vk.cmdDrawIndexed(*m_cmdBuffer, NUM_VERTICES, numInstances, NDX_FIRST_INDEX, OFFSET_FIRST_INDEX,
                                    firstInstance);
            else
...
void addDrawCase(tcu::TestCaseGroup *group, const DrawTest::TestSpec testSpec, const TestFlags flags)
{
    ...
    group->addChild(new InstanceFactory<DrawTest, FunctionSupport1<TestFlags>>(
        group->getTestContext(), name.str(), "", addFlags(testSpec, flags),
        FunctionSupport1<TestFlags>::Args(checkSupport, testSpec.flags | flags)));
}
