...
struct DrawParamsBase
{
    ...
    DrawParamsBase(const vk::VkPrimitiveTopology top) : topology(top)
    {
    }
};
...
struct DrawParams : DrawParamsBase
{
    ...
    DrawParams(const vk::VkPrimitiveTopology top, const uint32_t vertexC, const uint32_t instanceC,
               const uint32_t firstV, const uint32_t firstI)
        : DrawParamsBase(top)
    {
...
struct DrawIndexedParams : DrawParamsBase, IndexedParamsBase
{
    ...
    DrawIndexedParams(const vk::VkPrimitiveTopology top, const vk::VkIndexType indexT, const uint32_t indexC,
                      const uint32_t instanceC, const uint32_t firstIdx, const int32_t vertexO, const uint32_t firstIns)
        : DrawParamsBase(top)
...
struct DrawIndirectParams : DrawParamsBase
{
    ...
    DrawIndirectParams(const vk::VkPrimitiveTopology top) : DrawParamsBase(top)
    {
    }
...
tcu::TestStatus DrawTestInstance<DrawParams>::iterate(void)
{
    ...
    m_vk.cmdDraw(*m_cmdBuffer, m_data.params.vertexCount, m_data.params.instanceCount, m_data.params.firstVertex,
...
tcu::TestStatus DrawTestInstance<DrawIndexedParams>::iterate(void)
{
    ...
    m_vk.cmdDrawIndexed(*m_cmdBuffer, m_data.params.indexCount, m_data.params.instanceCount, m_data.params.firstIndex,
...
tcu::TestStatus DrawTestInstance<DrawIndirectParams>::iterate(void)
{
    ...
    if (m_data.commands.size() > 1 && !(features.multiDrawIndirect))
    {
        ...
        {
            const uint32_t offset = (uint32_t)(indirectAlloc->getOffset() + cmdIdx * sizeof(vk::VkDrawIndirectCommand));
            m_vk.cmdDrawIndirect(*m_cmdBuffer, *indirectBuffer, offset, 1, sizeof(vk::VkDrawIndirectCommand));
        }
    }
    else
    {
        m_vk.cmdDrawIndirect(*m_cmdBuffer, *indirectBuffer, indirectAlloc->getOffset(),
                             (uint32_t)m_data.commands.size(), sizeof(vk::VkDrawIndirectCommand));
    }
    ...
    if (m_data.commands.size() > 1 && !(features.multiDrawIndirect))
    {
        for (uint32_t cmdIdx = 0; cmdIdx < m_data.commands.size(); ++cmdIdx)
        {
            const uint32_t offset =
                (uint32_t)(indirectAlloc->getOffset() + cmdIdx * sizeof(vk::VkDrawIndexedIndirectCommand));
            m_vk.cmdDrawIndexedIndirect(*m_cmdBuffer, *indirectBuffer, offset, 1,
                                        sizeof(vk::VkDrawIndexedIndirectCommand));
        }
    }
    else
    {
        m_vk.cmdDrawIndexedIndirect(*m_cmdBuffer, *indirectBuffer, indirectAlloc->getOffset(),
                                    (uint32_t)m_data.commands.size(), sizeof(vk::VkDrawIndexedIndirectCommand));
    }
...
struct TestCaseParams
{
    ...
    TestCaseParams(const DrawCommandType cmd, const vk::VkPrimitiveTopology top) : command(cmd), topology(top)
    {
    }
};
...
void populateSubGroup(tcu::TestCaseGroup *testGroup, const TestCaseParams caseParams)
{
    ...
    for (uint32_t primitiveCountIdx = 0; primitiveCountIdx < DE_LENGTH_OF_ARRAY(PRIMITIVE_COUNT); ++primitiveCountIdx)
    {
        ...
        case DRAW_COMMAND_TYPE_DRAW_INDEXED:
        {
            ...
            testGroup->addChild(new IndexedCase(
                testCtx, name.c_str(), "vkCmdDrawIndexed testcase.",
        ...
        case DRAW_COMMAND_TYPE_DRAW_INDIRECT:
        {
            ...
            testGroup->addChild(
                new IndirectCase(testCtx, (name + "_single_command").c_str(), "vkCmdDrawIndirect testcase.", params));

            params.addCommand(vertexCount, 1, firstVertex, 0);
            testGroup->addChild(
                new IndirectCase(testCtx, (name + "_multi_command").c_str(), "vkCmdDrawIndirect testcase.", params));
            break;
        }
        case DRAW_COMMAND_TYPE_DRAW_INDEXED_INDIRECT:
        {
            ...
            testGroup->addChild(new IndexedIndirectCase(testCtx, (name + "_single_command").c_str(),
                                                        "vkCmdDrawIndexedIndirect testcase.", params));

            params.addCommand(vertexCount, 1, firstIndex, vertexOffset, 0);
            testGroup->addChild(new IndexedIndirectCase(testCtx, (name + "_multi_command").c_str(),
                                                        "vkCmdDrawIndexedIndirect testcase.", params));
            break;
        }
        default:
...
{
    for (uint32_t idx = 0; idx != vk::VK_PRIMITIVE_TOPOLOGY_PATCH_LIST; ++idx)
    {
        ...
        addTestGroup(testGroup, groupName, "Testcases with a specific topology.", populateSubGroup,
                     TestCaseParams(cmdType, topology));
    }
}

void createDrawTests(tcu::TestCaseGroup *testGroup)
{
    for (uint32_t idx = 0; idx < DRAW_COMMAND_TYPE_DRAW_LAST; ++idx)
    {
        const DrawCommandType command = DrawCommandType(idx);
        addTestGroup(testGroup, getDrawCommandTypeName(command), "Group for testing a specific draw command.",
                     createTopologyGroups, command);
    }
}
