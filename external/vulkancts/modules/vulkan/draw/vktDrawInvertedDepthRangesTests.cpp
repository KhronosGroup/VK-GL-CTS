...
constexpr float kDepthThreshold =
    0.0025f; // Used when checking depth buffer values. Less than depth delta in each pixel (~= 1.4/205).
    ...
    , m_depthAttachmentFormat(VK_FORMAT_D16_UNORM)
{
    ...
    // Render pass
    {
        ...
        const VkImageUsageFlags depthTargeUsageFlags = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
    ...
    pipelineCreateInfo.addState(PipelineCreateInfo::RasterizerState(m_params.depthClampEnable, // depthClampEnable
...
InvertedDepthRangesTestInstance::ColorAndDepth InvertedDepthRangesTestInstance::draw(const VkViewport viewport)
{
...

    {
        ...
        initialTransitionDepth2DImage(vk, *cmdBuffer, m_depthTargetImage->object(), VK_IMAGE_LAYOUT_GENERAL,
                                      VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);
        vk.cmdClearColorImage(*cmdBuffer, m_colorTargetImage->object(), VK_IMAGE_LAYOUT_GENERAL, &clearColor, 1,
                              &subresourceRange);
        vk.cmdClearDepthStencilImage(*cmdBuffer, m_depthTargetImage->object(), VK_IMAGE_LAYOUT_GENERAL, &clearDepth, 1u,
    ...
    }
    {
        ...
        vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                              ...
                              0, 1, &depthBarrier, 0, DE_NULL, 0, DE_NULL);
    }
    ...
    // Get result
    {
        ...
        const auto colorPixels = m_colorTargetImage->readSurface(queue, alloc, VK_IMAGE_LAYOUT_GENERAL, zeroOffset,
                                                                 iWidth, iHeight, VK_IMAGE_ASPECT_COLOR_BIT);
        const auto depthPixels = m_depthTargetImage->readSurface(queue, alloc, VK_IMAGE_LAYOUT_GENERAL, zeroOffset,
                                                                 iWidth, iHeight, VK_IMAGE_ASPECT_DEPTH_BIT);
...
{
public:
    InvertedDepthRangesTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
void populateTestGroup(tcu::TestCaseGroup *testGroup)
{
    ...
    for (int ndxDepthClamp = 0; ndxDepthClamp < DE_LENGTH_OF_ARRAY(depthClamp); ++ndxDepthClamp)
        for (int ndxDelta = 0; ndxDelta < DE_LENGTH_OF_ARRAY(delta); ++ndxDelta)
        {
            ...
            const TestParams params = {depthClamp[ndxDepthClamp].depthClamp, minDepth, maxDepth};
