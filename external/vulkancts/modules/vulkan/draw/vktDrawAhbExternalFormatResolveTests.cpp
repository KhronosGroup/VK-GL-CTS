...
struct DrawResources
{
	...
	Move<VkPipeline>				m_inputAttachmentPipeline;			// Reads from input attachment (external image) and renders to vulkan image
	...
	de::MovePtr<ImageWithMemory>	m_resultAttachmentImage;			// Used as render target when reading from external image as input attachment
...
protected:
	...
	void				doRenderPass						(const vk::DeviceInterface& vk, const vk::VkDevice device, const VkQueue queue, const deUint32 queueFamilyIndex, bool renderInputAttachment);
	...
	void				buildReferenceImage					(tcu::TextureLevel& reference, bool performDownsample, bool ahbFormatVulkanFormatAlphaMismatch) const;
	...
	deUint64									m_retrievedInternalFormat	= 0u;
...
tcu::TestStatus AhbExternalFormatResolveTestInstance::iterate (void)
{
	...
	if (!androidBuffer.allocate(m_format, m_width, m_height, m_layers, m_usage))
	{
		...
		log << tcu::TestLog::Message << "Skipping format " << formatName << ". Reason: " << skipReason << tcu::TestLog::EndMessage;
	...
	if (m_isInputAttachment)
	{
		...
		resultAccess	= tcu::ConstPixelBufferAccess(mapVkFormat(m_colorAttachmentFormat), m_width, m_height, 1u, allocColor.getHostPtr());
	}
	else
	{
		...
		cpuTexture	= tcu::TextureLevel(AndroidHardwareBufferInstance::formatToTextureFormat(m_format), m_width, m_height, m_layers);
		...
		{
			tcu::CompressedTexFormat	compressedFormat	= m_format == AndroidHardwareBufferInstance::Format::RAW10 ? tcu::COMPRESSEDTEXFORMAT_AHB_RAW10
	...
	// Validate output
	{
		tcu::TextureFormat					textureFormat	= m_isInputAttachment ? mapVkFormat(m_colorAttachmentFormat) : AndroidHardwareBufferInstance::formatToTextureFormat(m_format);
		tcu::TextureLevel					reference		(textureFormat, m_width, m_height, m_layers);
		const bool							alphaMismatch	= !AndroidHardwareBufferInstance::hasFormatAlpha(m_format) && hasAlphaChannel(textureFormat.order);
		...
		const char*							description		= "Validate output image was rendered according to expectation (if YUV and input test, a follow up test is done for no downsample)";
		...
		const tcu::UVec4					threshold		= (m_format == AndroidHardwareBufferInstance::Format::YCbCr_P010) ? tcu::UVec4(4u) : tcu::UVec4(0u);

		if (!tcu::intThresholdCompare(log, name, description, referenceAccess, resultAccess, threshold, tcu::COMPARE_LOG_ON_ERROR))
...
void AhbExternalFormatResolveTestInstance::renderToExternalFormat (AndroidHardwareBufferInstance& androidBuffer)
{
	...
	m_resources.m_fragmentShaderBase				= vk::createShaderModule(vk, device, m_context.getBinaryCollection().get("frag_" + shaderType));
	if (m_isInputAttachment)
	{
		...
		m_resources.m_fragmentShaderInput	= vk::createShaderModule(vk, device, m_context.getBinaryCollection().get(shaderName));
	}
	createDescriptors();
	...
	createPipelines();
	{
		...
		m_resources.m_vertexBuffer	= de::MovePtr<BufferWithMemory>(new BufferWithMemory(vk, device, m_context.getDefaultAllocator(), makeBufferCreateInfo(sizeof(vertices), vk::VK_BUFFER_USAGE_VERTEX_BUFFER_BIT), vk::MemoryRequirement::HostVisible));
...

	{
		...
		Move<VkCommandBuffer>	commandBuffer	= vk::allocateCommandBuffer(vk, device, commandPool.get(), vk::VK_COMMAND_BUFFER_LEVEL_PRIMARY);
...
void AhbExternalFormatResolveTestInstance::clearAttachments(const vk::DeviceInterface& vk, vk::VkCommandBuffer commandBuffer)
{
...
void AhbExternalFormatResolveTestInstance::doRenderPass (const vk::DeviceInterface& vk, const vk::VkDevice device, const VkQueue queue, const deUint32 queueFamilyIndex, bool renderInputAttachment)
{
	Move<VkCommandPool>			commandPool					= vk::createCommandPool(vk, device, 0u, queueFamilyIndex);
	Move<VkCommandBuffer>		primaryCommandBuffer		= vk::allocateCommandBuffer(vk, device, commandPool.get(), vk::VK_COMMAND_BUFFER_LEVEL_PRIMARY);
	Move<VkCommandBuffer>		secondaryCommandBuffer		= vk::allocateCommandBuffer(vk, device, commandPool.get(), vk::VK_COMMAND_BUFFER_LEVEL_SECONDARY);
	...
	if (m_groupParams.useSecondaryCmdBuffer)
	{
		...
		const vk::VkFormat									colorAttachmentFormat	= m_nullColorAttachment ? vk::VK_FORMAT_UNDEFINED : m_colorAttachmentFormat;
		...
		const vk::VkCommandBufferUsageFlags					commandBufferBeginFlags	= m_groupParams.secondaryCmdBufferCompletelyContainsDynamicRenderpass
...
void AhbExternalFormatResolveTestInstance::copyImageToBuffer (const vk::DeviceInterface& vk, vk::VkCommandBuffer commandBuffer)
{
	...
	if (m_isInputAttachment)
	{
		...
		vk.cmdPipelineBarrier(commandBuffer, vk::VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, 0u,
		...
		vk.cmdCopyImageToBuffer(commandBuffer, m_resources.m_resultAttachmentImage->get(), vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
								m_resources.m_resultBuffer->get(), 1u, &region);
	}
}

void AhbExternalFormatResolveTestInstance::initialAttachmentTransition (const vk::DeviceInterface& vk, vk::VkCommandBuffer commandBuffer)
{
	const vk::VkImage					resultImage			= m_isInputAttachment ? m_resources.m_resultAttachmentImage->get() : VK_NULL_HANDLE;
	const vk::VkImageSubresourceRange	subresourceRange	= vk::makeImageSubresourceRange(vk::VK_IMAGE_ASPECT_COLOR_BIT, 0u, VK_REMAINING_MIP_LEVELS, 0u, VK_REMAINING_ARRAY_LAYERS);
	...
	vk.cmdPipelineBarrier(commandBuffer, vk::VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0u,
							0u, DE_NULL, 0u, DE_NULL, barrierCount, imageBarriers);
}

void AhbExternalFormatResolveTestInstance::transitionInputAttachmentToOutput(const vk::DeviceInterface& vk, vk::VkCommandBuffer commandBuffer)
{
	const vk::VkImageSubresourceRange	subresourceRange	= vk::makeImageSubresourceRange(vk::VK_IMAGE_ASPECT_COLOR_BIT, 0u, VK_REMAINING_MIP_LEVELS, 0u, VK_REMAINING_ARRAY_LAYERS);
	const vk::VkImageMemoryBarrier		imageBarrier		=
	{
	...
	vk.cmdPipelineBarrier(commandBuffer, vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, vk::VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, 0u,
						  0u, DE_NULL, 0u, DE_NULL, 1u, &imageBarrier);
}

bool AhbExternalFormatResolveTestInstance::checkExternalFormatTestingRequired (const AndroidHardwareBufferInstance& androidBuffer)
{
	const vk::DeviceInterface&	vk					= m_context.getDeviceInterface();
	const vk::VkDevice			device				= m_context.getDevice();
	...
	if (m_isInputAttachment && (m_nullColorAttachment == VK_FALSE) && (formatProperties.formatFeatures & vk::VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT) == 0u)
		TCU_THROW(NotSupportedError, "Format lacks input attachment usage: nullColorAttachment is VK_FALSE and format does not support VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT");
...
void AhbExternalFormatResolveTestInstance::buildReferenceImage(tcu::TextureLevel& texture, bool performDownsample, bool ahbFormatVulkanFormatAlphaMismatch) const
{
	...
	tcu::IVec2	renderAreaEnd	(renderAreaStart.x() + m_renderArea.extent.width, renderAreaStart.y() + m_renderArea.extent.height);

	deUint32	colorIndex		= 0u;
	for (deUint32 y = 0u; y < m_height; ++y, colorIndex ^= 2u)
	{
		for (deUint32 x = 0u; x < m_width; ++x, colorIndex ^= 1u)
		{
			if (m_isClearOnly)
				access.setPixel(m_clearColor, x, y);
			else
			{
				bool	isInsideRenderArea	= ((renderAreaStart.x() <= static_cast<int32_t>(x)) && (static_cast<int32_t>(x) < renderAreaEnd.x())) &&
	...
	if (performDownsample)
	{
		...
		AndroidHardwareBufferInstance::ChromaLocation	xLocation	= AndroidHardwareBufferInstance::vkChromaLocationToChromaLocation(m_xChromaLocation);
		AndroidHardwareBufferInstance::ChromaLocation	yLocation	= AndroidHardwareBufferInstance::vkChromaLocationToChromaLocation(m_yChromaLocation);
...
void AhbExternalFormatResolveTestInstance::createImagesAndViews (AndroidHardwareBufferInstance& androidBuffer)
{
	...
	// Import android hardware buffer to Vulkan
	{
		// Create VkImage
		{
			...
			vk::VkImageUsageFlags						usage				= (m_nullColorAttachment && m_isInputAttachment) ?
		...
		// Bind
		VK_CHECK(vk.bindImageMemory(device, m_resources.m_androidExternalImage.get(), m_resources.m_androidExternalImageMemory.get(), 0u));

		// Create view
		m_resources.m_androidExternalImageView	= createImageView(m_resources.m_androidExternalImage.get(), vk::VK_FORMAT_UNDEFINED);
	}
	...
	if (m_isInputAttachment)
	{
		...
		m_resources.m_resultAttachmentImage			= de::MovePtr<ImageWithMemory>(new ImageWithMemory(vk, device, m_context.getDefaultAllocator(), imageCreateInfo, vk::MemoryRequirement::Any));
		m_resources.m_resultAttachmentImageView		= createImageView(m_resources.m_resultAttachmentImage->get(), m_colorAttachmentFormat);
		...
		m_resources.m_resultBuffer					= de::MovePtr<BufferWithMemory>(new BufferWithMemory(vk, device, m_context.getDefaultAllocator(), bufferCreateInfo, vk::MemoryRequirement::HostVisible));
	}

	if (m_nullColorAttachment == VK_FALSE)
	{
		imageCreateInfo.usage	|= m_isInputAttachment ? (vk::VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | vk::VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT) : vk::VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
		m_resources.m_androidColorAttachmentImage		= de::MovePtr<ImageWithMemory>(new ImageWithMemory(vk, device, m_context.getDefaultAllocator(), imageCreateInfo, vk::MemoryRequirement::Any));
		m_resources.m_androidColorAttachmentImageView	= createImageView(m_resources.m_androidColorAttachmentImage->get(), m_colorAttachmentFormat);
	}
}
...
void AhbExternalFormatResolveTestInstance::createRenderPass (void)
{
	...
	const vk::VkSubpassDescription2		subpassDescriptions[]		=
	{
		...
		// Subpass 1
		{
			...
			m_isInputAttachment ? &inputAttachmentReference : nullptr,	// const VkAttachmentReference2*	pInputAttachments
			1u,															// uint32_t							colorAttachmentCount
...
void AhbExternalFormatResolveTestInstance::createPipelines (void)
{
	...
	const vk::VkColorComponentFlags						colorFlags				= vk::VK_COLOR_COMPONENT_R_BIT
	...
	vk::VkGraphicsPipelineCreateInfo					createInfo				=
	{
		...
		m_groupParams.useDynamicRendering ? &pipelineRenderingInfo : nullptr,	// const void*										pNext
...
void AhbExternalFormatResolveTestInstance::beginRender (vk::VkCommandBuffer cmd, const vk::VkRect2D& renderArea, bool clearPass) const
{
	const vk::DeviceInterface&	vk						= m_context.getDeviceInterface();

	if (m_groupParams.useDynamicRendering)
	{
		vk::VkRenderingFlags				renderingFlags	= m_groupParams.useSecondaryCmdBuffer && !m_groupParams.secondaryCmdBufferCompletelyContainsDynamicRenderpass
		...
		vk::VkClearValue					clearValue		= makeClearValueColorF32(m_clearColor.x(), m_clearColor.y(), m_clearColor.z(), m_clearColor.w());
		vk::VkRenderingAttachmentInfoKHR	colorAttachment	=
		{
			vk::VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO_KHR,											// VkStructureType						sType
			nullptr,																						// const void*							pNext
			m_nullColorAttachment ? VK_NULL_HANDLE : m_resources.m_androidColorAttachmentImageView.get(),	// VkImageView							imageView
	...
	else
	{
		...
		vector<vk::VkClearValue>		clearColors				(m_nullColorAttachment ? 1u : 2u,
																	makeClearValueColorF32(m_clearColor.x(), m_clearColor.y(), m_clearColor.z(), m_clearColor.w()));
		if (m_isInputAttachment)
			clearColors.emplace_back(clearColors.front()); // All images have the same maximums, so we can reuse clear colors

		const vk::VkRenderPassBeginInfo	renderPassBeginInfo		=
		{
			vk::VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,										// VkStructureType		sType
			DE_NULL,																			// const void*			pNext
			clearPass ? m_resources.m_renderPassClear.get() : m_resources.m_renderPass.get(),	// VkRenderPass			renderPass
			clearPass ? m_resources.m_framebufferClear.get() : m_resources.m_framebuffer.get(),	// VkFramebuffer		framebuffer
			renderArea,																			// VkRect2D				renderArea
...
void AhbExternalFormatResolveTestInstance::drawCommands (vk::VkCommandBuffer cmd, bool drawFromInputAttachment) const
{
	...
	if (!m_isClearOnly)
	{
		...
		if (m_isInputAttachment)
		{
			...
			if (drawFromInputAttachment)
			{
				vk.cmdBindPipeline(cmd, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, m_resources.m_inputAttachmentPipeline.get());
				vk.cmdBindDescriptorSets(cmd, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, m_resources.m_inputAttachmentPipelineLayout.get(),
											0u, 1u, &m_resources.m_descriptorSet.get(), 0u, nullptr);
				vk.cmdDraw(cmd, 4u, 1u, 0u, 0u);
			}
		}
	}
...
void AhbExternalFormatResolveTestCase::initPrograms (SourceCollections& programCollection) const
{
	...
	const deUint32		typeCount	= sizeof(possibleTypes) / sizeof(possibleTypes[0]);

	for (deUint32 i = 0; i < typeCount; ++i)
	{
		...
		source	<< "#version 430\n"
				...
				<< "    " << possibleTypes[i].first << "vec4(" << possibleTypes[i].second << ", 0.0f, 0.0f, " << possibleTypes[i].second << "),\n"
				<< "    " << possibleTypes[i].first << "vec4(0.0f, " << possibleTypes[i].second << ", 0.0f, " << possibleTypes[i].second << "),\n"
				<< "    " << possibleTypes[i].first << "vec4(0.0f, 0.0f, " << possibleTypes[i].second << ", " << possibleTypes[i].second << "),\n"
				<< "};\n"
				<< "void main()\n"
				<< "{\n"
	...
	for (deUint32 i = 0; i < typeCount; ++i)
	{
		uint32_t	swizzleCount	= AndroidHardwareBufferInstance::isFormatYuv(m_params.m_format) ? DE_LENGTH_OF_ARRAY(swizzleOrder) : 1u;
		for (uint32_t swizzleIndex = 0u; swizzleIndex < swizzleCount; ++swizzleIndex)
		{
			const uint32_t* swizzle = swizzleOrder[swizzleIndex];
			std::string			shaderName	= "frag_input_" + std::string(possibleTypes[i].first) + "vec4_" + shaderIndex[swizzle[0]] + shaderIndex[swizzle[1]] + shaderIndex[swizzle[2]];
			std::stringstream	source;
			source	<< "#version 430\n"
					<< "layout(location = 0) out " << possibleTypes[i].first << "vec4 out_color;\n"
					<< "layout(input_attachment_index=0, set=0, binding=0) uniform " << possibleTypes[i].first << "subpassInput input_attachment;"
					...
					<< "    out_color = " << possibleTypes[i].first << "vec4(input_color." << shaderIndex[swizzle[0]] << ","
																			"input_color." << shaderIndex[swizzle[1]] << ","
																			"input_color." << shaderIndex[swizzle[2]] << ","
																			"input_color.w);\n"
					<< "}\n";
...
void createAhbExternalFormatResolveDrawTests (tcu::TestCaseGroup* testGroup, const SharedGroupParams groupParams)
{
	...
	for (deUint32 i = 0; i < AndroidHardwareBufferInstance::Format::COUNT; ++i)
	{
		...
		if (!params.m_groupParams.useSecondaryCmdBuffer || params.m_groupParams.secondaryCmdBufferCompletelyContainsDynamicRenderpass)
		{
			// CPU side validation requires valid tcu::TextureFormat
			if (hasValidTextureFormat)
			{
