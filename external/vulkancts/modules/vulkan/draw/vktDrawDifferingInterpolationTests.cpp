...
tcu::TestStatus DrawTestInstance::iterate(void)
{
    ...
    for (uint32_t frameIdx = 0; frameIdx < DE_LENGTH_OF_ARRAY(frames); frameIdx++)
    {
        ...
        // Create color buffer image.
        {
            ...
            const ImageCreateInfo targetImageCreateInfo(VK_IMAGE_TYPE_2D, VK_FORMAT_R8G8B8A8_UNORM, targetImageExtent,
        ...
        // Create render pass and frame buffer.
        {
            ...
            renderPassCreateInfo.addAttachment(
                AttachmentDescription(VK_FORMAT_R8G8B8A8_UNORM, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_LOAD,
        ...
        // Queue draw and read results.
        {
            ...
            initialTransitionColor2DImage(vk, *cmdBuffer, colorTargetImages[frameIdx]->object(),
                                          VK_IMAGE_LAYOUT_GENERAL, vk::VK_ACCESS_TRANSFER_WRITE_BIT,
                                          vk::VK_PIPELINE_STAGE_TRANSFER_BIT);

            vk.cmdClearColorImage(*cmdBuffer, colorTargetImages[frameIdx]->object(), VK_IMAGE_LAYOUT_GENERAL,
                                  &clearColor, 1, &subresourceRange);

            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 1, &memBarrier, 0, DE_NULL, 0,
                                  DE_NULL);
            ...
            frames[frameIdx] = colorTargetImages[frameIdx]->readSurface(queue, m_context.getDefaultAllocator(),
                                                                        VK_IMAGE_LAYOUT_GENERAL, zeroOffset, WIDTH,
...
void createTests(tcu::TestCaseGroup *testGroup)
{
    ...
    testGroup->addChild(new DrawTestCase(testCtx, "noperspective_0",
                                         "Mismatching noperspective interpolation testcase 0.", paramsNoPerspective0));
    testGroup->addChild(new DrawTestCase(testCtx, "noperspective_1",
                                         "Mismatching noperspective interpolation testcase 1.", paramsNoPerspective1));
}
...
{
    return createTestGroup(testCtx, "differing_interpolation", "Tests for mismatched interpolation decorations.",
                           createTests);
}
