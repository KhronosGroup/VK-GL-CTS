...
DepthClampTestInstance::DepthClampTestInstance(Context &context, const TestParams &params, const VkFormat format,
                                               const float epsilon, bool useDynamicRendering)
    ...
    , m_useDynamicRendering(useDynamicRendering)
{
    ...
    VkPipelineRenderingCreateInfoKHR renderingCreateInfo{
        VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR, DE_NULL, 0u, 0u, DE_NULL, m_format, m_format};
...
tcu::ConstPixelBufferAccess DepthClampTestInstance::draw()
{
    ...
    vk.cmdClearDepthStencilImage(*cmdBuffer, m_depthTargetImage->object(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                 &clearDepth.depthStencil, 1, &subresourceRange);
...
public:
    DepthClampTest(tcu::TestContext &testCtx, const string &name, const string &description, const TestParams &params,
                   const VkFormat format, const float epsilon, bool useDynamicRendering)
...
void createTests(tcu::TestCaseGroup *testGroup, bool useDynamicRendering)
{
    for (int i = 0; i < DE_LENGTH_OF_ARRAY(depthStencilImageFormatsToTest); ++i)
    {
        ...
        for (const auto &params : depthClearValuesToTest)
        {
            ...
            testGroup->addChild(new DepthClampTest(testGroup->getTestContext(), testCaseName, "Depth clamp", params,
                                                   format, epsilon, useDynamicRendering));
        }
    }
}
} // namespace
