...
tcu::ConstPixelBufferAccess DepthClampTestInstance::draw(const VkViewport viewport)
{
    ...
    if (isCombinedType)
        ...
        initialTransitionDepth2DImage(vk, *cmdBuffer, m_depthTargetImage->object(),
                                      VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_ACCESS_TRANSFER_WRITE_BIT,
                                      VK_PIPELINE_STAGE_TRANSFER_BIT);
        ...
        for (int y = 0; y < resultImage.getHeight(); ++y)
            for (int x = 0; x < resultImage.getWidth(); ++x)
            {
                if (std::abs(m_params.expectedValue - resultImage.getPixDepth(x, y, z)) >= m_epsilon)
                {
                    ...
                    msg << "Depth value mismatch, expected: " << m_params.expectedValue
                        << ", got: " << resultImage.getPixDepth(x, y, z) << " at "
                        << "(" << x << ", " << y << ", " << z << ")";
...
{
public:
    DepthClampTest(tcu::TestContext &testCtx, const string &name, const string &description, const TestParams &params,
                   const VkFormat format, const float epsilon)
    ...
    virtual void checkSupport(Context &context) const
    {
        ...
        const auto usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT |
                           VK_IMAGE_USAGE_TRANSFER_DST_BIT;
...
void createTests(tcu::TestCaseGroup *testGroup)
{
    for (int i = 0; i < DE_LENGTH_OF_ARRAY(depthStencilImageFormatsToTest); ++i)
    {
        ...
        for (const auto &params : depthClearValuesToTest)
        {
            ...
            testGroup->addChild(
                new DepthClampTest(testGroup->getTestContext(), testCaseName, "Depth clamp", params, format, epsilon));
        }
    }
}
} // namespace
