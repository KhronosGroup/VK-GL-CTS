...
tcu::TestStatus renderPointSizeClampTest (Context& context)
{
	...
	SimpleAllocator							memAlloc				(vk, vkDevice, getPhysicalDeviceMemoryProperties(context.getInstanceInterface(), phyDevice));
	...
	{
		{ tcu::Vec4(testPointXCoord, 0.0f, 0.0f, 1.0f), pointColor }
	};
	...
	const VkVertexInputAttributeDescription			vertexInputAttributeDescriptionPos =
	{
		...
		offsetof(Vertex, pos)			// deUint32    offset
	};
	...
	const VkPipelineVertexInputStateCreateInfo vertexInputStateCreateInfo =
	{
		...
		(uint32_t) vertexInputAttributeDescriptions.size(),			// deUint32                                    vertexAttributeDescriptionCount
	...
	vk.cmdPushConstants(*cmdBuf, *pipelineLayout, pcPointSize.stageFlags, pcPointSize.offset, pcPointSize.size, &testPointSize);
...
tcu::TestCaseGroup*	createDrawPointClampTests (tcu::TestContext& testCtx)
{
	...
	addFunctionCaseWithPrograms	(pointClampTests.get(), "point_size_clamp_max",	createPointSizeClampProgs,	renderPointSizeClampTest);
