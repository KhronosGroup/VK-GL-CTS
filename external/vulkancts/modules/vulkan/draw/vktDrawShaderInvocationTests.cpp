...
void checkExtTestSupport(Context &context, std::string testName)
{
    DE_UNREF(testName);
    checkSupport(context, EXT);
}
void checkCoreTestSupport(Context &context, std::string testName)
{
    DE_UNREF(testName);
    checkSupport(context, CORE);
}
void checkMemModelTestSupport(Context &context, std::string testName)
{
    DE_UNREF(testName);
    checkSupport(context, CORE_MEM_MODEL);
}
...
void createTests(tcu::TestCaseGroup *testGroup)
{
    ...
    struct caseDef
    {
        ...
        {"helper_invocation_volatile_mem_model", "helper_invocation_volatile_mem_model.amber",
         checkMemModelTestSupport},
    };

    for (unsigned i = 0; i < sizeof(cases) / sizeof(caseDef); i++)
    {
        cts_amber::AmberTestCase *testCase =
            cts_amber::createAmberTestCase(testCtx, cases[i].name, "", dataDir, cases[i].file);
