...
tcu::TestStatus DiscardRectanglesTestInstance::iterate(void)
{
    ...
    // Write command buffer and submit it
    {
        ...
        vk.cmdDraw(*m_cmdBuffer, static_cast<uint32_t>(m_vertices.size()), 1u, 0u, 0u); // two triangles
...
{
public:
    DiscardRectanglesTestCase(tcu::TestContext &context, const char *name, const char *description, TestParams params);
...
DiscardRectanglesTestCase::DiscardRectanglesTestCase(tcu::TestContext &context, const char *name,
                                                     const char *description, TestParams params)
        ...
        for (uint32_t scissor = 0; scissor < TEST_SCISSOR_MODE_COUNT; scissor++)
        {
            for (uint32_t mode = 0; mode < TEST_MODE_COUNT; mode++)
            {
                for (uint32_t rect = 0; rect < NUM_RECT_TESTS; rect++)
                {
                    ...
                    params.dynamicDiscardRectangles = dynamic ? true : false;
