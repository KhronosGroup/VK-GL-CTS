...
tcu::TestStatus DrawTestInstance::iterate(void)
{
    ...
    if (!m_data.useDynamicRendering)
    {
        ...
        renderPassCreateInfo.addAttachment(
            AttachmentDescription(imageFormat, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_LOAD,
    ...
    // Queue draw and read results.
    {
        ...
        if (m_data.useDynamicRendering)
        {
            VkRenderingAttachmentInfoKHR colorAttachment{
                ...
                useMultisampling ? VK_RESOLVE_MODE_AVERAGE_BIT :
                ...
                useMultisampling ? VK_ATTACHMENT_LOAD_OP_CLEAR :
    ...
    for (uint32_t sampleNdx = 0; sampleNdx < DE_LENGTH_OF_ARRAY(samples); sampleNdx++)
        for (uint32_t auxNdx = 0; auxNdx < DE_LENGTH_OF_ARRAY(auxQualifiers); auxNdx++)
            for (uint32_t interNdx = 0; interNdx < DE_LENGTH_OF_ARRAY(interTypes); interNdx++)
            {
                ...
                const DrawParams params = {interTypes[interNdx], samples[sampleNdx], auxQualifiers[auxNdx],
                                           useDynamicRendering};
...
{
    return createTestGroup(testCtx, "explicit_vertex_parameter", "Tests for VK_AMD_shader_explicit_vertex_parameter.",
