...
const Topology topologiesToTest[] = {Topology::TRIANGLE_STRIP, Topology::TRIANGLES, Topology::TRIANGLE};
...
const FormatPair formatsToTest[] = {
    {VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_UNDEFINED},  {VK_FORMAT_R8G8B8A8_SNORM, VK_FORMAT_UNDEFINED},
    {VK_FORMAT_UNDEFINED, VK_FORMAT_D32_SFLOAT},      {VK_FORMAT_UNDEFINED, VK_FORMAT_D16_UNORM},
    {VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_D32_SFLOAT}, {VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_D16_UNORM},
    {VK_FORMAT_R8G8B8A8_SNORM, VK_FORMAT_D32_SFLOAT}, {VK_FORMAT_R8G8B8A8_SNORM, VK_FORMAT_D16_UNORM},
};
...
const uint32_t TOPOLOGY_MAX_VERTICES_COUNT = 6;
const uint32_t TEST_MAX_STEPS_COUNT        = 3;
...
const Vertices verticesByTopology[] = {
    {"_triangle_strip", VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, DE_LENGTH_OF_ARRAY(verticesTriangleStrip),
     verticesTriangleStrip},
    {"_triangles", VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, DE_LENGTH_OF_ARRAY(verticesTriangles), verticesTriangles},
    {"_big_triangle", VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, DE_LENGTH_OF_ARRAY(verticesBigTriangle),
     verticesBigTriangle}};
...
private:
    void clearAttachments(const DeviceInterface &vk, VkCommandBuffer cmdBuffer, const ClearOp clearOp,
    ...
    , m_params(params)
{
    ...
    const auto &vertexData = verticesByTopology[(size_t)m_params.topology];
    {
        ...
        m_vertexBuffer =
            Buffer::createAndAlloc(vk, device, BufferCreateInfo(totalDataSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT),
    ...
    std::vector<VkImageView> attachments;
    {
        ...
        if (hasColor)
        {
            ...
            const ImageCreateInfo targetImageCreateInfo(VK_IMAGE_TYPE_2D, m_params.colorFormat, {WIDTH, HEIGHT, 1u}, 1u,
                                                        1u, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_OPTIMAL,
                                                        targetImageUsageFlags);
            m_colorTargetImage = Image::createAndAlloc(vk, device, targetImageCreateInfo,
                                                       m_context.getDefaultAllocator(), queueFamilyIndex);

            renderPassCreateInfo.addAttachment(
                AttachmentDescription(m_params.colorFormat,                       // format
        ...
        if (hasDepth)
        {
            const VkImageUsageFlags depthImageUsageFlags = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT |
                                                           VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
                                                           VK_IMAGE_USAGE_TRANSFER_DST_BIT;
            const ImageCreateInfo depthImageCreateInfo(VK_IMAGE_TYPE_2D, m_params.depthFormat, {WIDTH, HEIGHT, 1u}, 1u,
                                                       1u, VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_TILING_OPTIMAL,
                                                       depthImageUsageFlags);
            m_depthTargetImage = Image::createAndAlloc(vk, device, depthImageCreateInfo,
                                                       m_context.getDefaultAllocator(), queueFamilyIndex);

            renderPassCreateInfo.addAttachment(
                AttachmentDescription(m_params.depthFormat,                               // format
                                      VK_SAMPLE_COUNT_1_BIT,                              // samples
                                      VK_ATTACHMENT_LOAD_OP_LOAD,                         // loadOp
                                      VK_ATTACHMENT_STORE_OP_STORE,                       // storeOp
                                      VK_ATTACHMENT_LOAD_OP_DONT_CARE,                    // stencilLoadOp
        ...
        const VkAttachmentReference colorAttachmentReference =
            hasColor ? makeAttachmentReference(0u, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL) : AttachmentReference();
        const VkAttachmentReference depthAttachmentReference =
            hasDepth ? makeAttachmentReference(hasColor ? 1u : 0u, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL) :
                       AttachmentReference();
    ...
    const Unique<VkShaderModule> fragmentModule(
        createShaderModule(vk, device, m_context.getBinaryCollection().get(hasColor ? "frag" : "frag_depthonly"), 0));
    ...
    pipelineCreateInfo.addState(
        PipelineCreateInfo::DepthStencilState(hasDepth, hasDepth, VK_COMPARE_OP_ALWAYS, VK_FALSE, VK_FALSE));
    pipelineCreateInfo.addState(PipelineCreateInfo::RasterizerState());
    pipelineCreateInfo.addState(PipelineCreateInfo::MultiSampleState());
    m_pipeline = createGraphicsPipeline(vk, device, DE_NULL, &pipelineCreateInfo);
}

void MultipleClearsTest::clearAttachments(const DeviceInterface &vk, vk::VkCommandBuffer cmdBuffer,
                                          const ClearOp clearOp, const size_t stepIndex)
{
    ...
    {
        const auto &vertexData       = verticesByTopology[(size_t)m_params.topology];
        const uint32_t verticesCount = vertexData.verticesCount;
        vk.cmdPushConstants(cmdBuffer, *m_pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(color),
                            color.getPtr());
        vk.cmdDraw(cmdBuffer, verticesCount, 1, static_cast<uint32_t>(verticesCount * stepIndex), 0);
    }
    break;
    case ClearOp::CLEAR:
    {
        ...
        if (m_params.colorFormat != VK_FORMAT_UNDEFINED)
        {
            const VkClearAttachment clearAttachment = {
                ...
                static_cast<uint32_t>(clearAttachments.size()), // uint32_t                 colorAttachment
        ...
        vk.cmdClearAttachments(cmdBuffer, static_cast<uint32_t>(clearAttachments.size()), clearAttachments.data(), 1,
                               &clearRect);
    }
    break;
    default:
...
tcu::TestStatus MultipleClearsTest::iterate(void)
{
    ...
    if (hasColor)
        initialTransitionColor2DImage(vk, *cmdBuffer, m_colorTargetImage->object(),
                                      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                                      VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
    if (hasDepth)
        initialTransitionDepth2DImage(
            vk, *cmdBuffer, m_depthTargetImage->object(), VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
    ...
    if (!m_params.steps.empty() && m_params.steps[0].clearOp == ClearOp::LOAD)
        beginRenderPass(vk, *cmdBuffer, *m_renderPass, *m_framebuffer, makeRect2D(0, 0, WIDTH, HEIGHT),
                        m_params.steps[0].color, m_params.steps[0].depth, 0);
    else
        beginRenderPass(vk, *cmdBuffer, *m_renderPass, *m_framebuffer, makeRect2D(0, 0, WIDTH, HEIGHT));
    ...
    for (uint32_t i = 0; i < m_params.repeatCount; ++i)
    ...
    if (hasColor)
    {
        const auto resultImage = m_colorTargetImage->readSurface(queue, m_context.getDefaultAllocator(),
                                                                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, {0, 0, 0}, WIDTH,
        ...
        for (int z = 0; z < resultImage.getDepth(); ++z)
            for (int y = 0; y < resultImage.getHeight(); ++y)
                for (int x = 0; x < resultImage.getWidth(); ++x)
                {
                    ...
                    if (abs(difference.x()) >= m_params.colorEpsilon || abs(difference.y()) >= m_params.colorEpsilon ||
                        abs(difference.z()) >= m_params.colorEpsilon)
                    {
                        ostringstream msg;
                        msg << "Color value mismatch, expected: " << m_params.expectedColor
                            << ", got: " << resultImage.getPixel(x, y, z) << " at "
                            << "(" << x << ", " << y << ", " << z << ")";
    ...
    if (hasDepth)
    {
        const auto resultImage = m_depthTargetImage->readSurface(queue, m_context.getDefaultAllocator(),
                                                                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, {0, 0, 0}, WIDTH,
                                                                 HEIGHT, VK_IMAGE_ASPECT_DEPTH_BIT);
        ...
        for (int z = 0; z < resultImage.getDepth(); ++z)
            for (int y = 0; y < resultImage.getHeight(); ++y)
                for (int x = 0; x < resultImage.getWidth(); ++x)
                {
                    ...
                    {
                        ostringstream msg;
                        msg << "Depth value mismatch, expected: " << m_params.expectedDepth
...
{
public:
    MultipleClearsWithinRenderPassTest(tcu::TestContext &testCtx, const string &name, const string &description,
    ...
    virtual void checkSupport(Context &context) const
    {
        ...
        if (m_params.colorFormat != VK_FORMAT_UNDEFINED)
        {
            const auto colorUsage =
                VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
            if (vki.getPhysicalDeviceImageFormatProperties(vkd, m_params.colorFormat, VK_IMAGE_TYPE_2D,
                                                           VK_IMAGE_TILING_OPTIMAL, colorUsage, 0u,
        ...
        if (m_params.depthFormat != VK_FORMAT_UNDEFINED)
        {
            const auto depthUsage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT |
                                    VK_IMAGE_USAGE_TRANSFER_DST_BIT;
            if (vki.getPhysicalDeviceImageFormatProperties(vkd, m_params.depthFormat, VK_IMAGE_TYPE_2D,
                                                           VK_IMAGE_TILING_OPTIMAL, depthUsage, 0u,
                                                           &imageFormatProperties) != VK_SUCCESS)
...
void MultipleClearsWithinRenderPassTests::init()
{
    for (const auto &formatPair : formatsToTest)
    {
        ...
        {
            const string testNameSuffix = formatSuffix.str() + verticesByTopology[(uint32_t)topology].testNameSuffix;
            {
                const TestParams params = {formatPair.colorFormat,       // VkFormat colorFormat;
                ...
                addChild(new MultipleClearsWithinRenderPassTest(
                    m_testCtx, "load_clear_draw" + testNameSuffix,
                    "Multiple clears within same render pass, methods: load, clear, draw", params));
            }
            {
                const TestParams params = {formatPair.colorFormat,       // VkFormat format;
                                           formatPair.depthFormat,       // VkFormat depthFormat;
                                           topology,                     // Topology topology;
                                           Vec4(0.0f, 0.5f, 0.5f, 1.0f), // Vec4 expectedColor;
                                           0.01f,                        // float colorEpsilon;
                                           ...
                                           {                             // vector<ClearStep> steps;
                                            ...
                                            {ClearOp::DRAW, Vec4(0.0f, 0.0f, 1.0f, 0.5f), 0.9f}}};
                addChild(new MultipleClearsWithinRenderPassTest(
                    m_testCtx, "draw_clear_draw" + testNameSuffix,
                    "Multiple clears within same render pass, methods: draw, clear, draw", params));
            }
            {
                const TestParams params = {formatPair.colorFormat,       // VkFormat format;
                                           formatPair.depthFormat,       // VkFormat depthFormat;
                                           ...
                                           {                             // vector<ClearStep> steps;
                                            ...
                                            {ClearOp::DRAW, Vec4(0.0f, 0.0f, 1.0f, 0.5f), 0.9f}}};
                addChild(new MultipleClearsWithinRenderPassTest(
                    m_testCtx, "clear_clear_draw" + testNameSuffix,
                    "Multiple clears within same render pass, methods: clear, clear, draw", params));
            }
            {
                const TestParams params = {formatPair.colorFormat,       // VkFormat format;
                                           formatPair.depthFormat,       // VkFormat depthFormat;
                ...
                addChild(new MultipleClearsWithinRenderPassTest(
                    m_testCtx, "load_clear" + testNameSuffix,
                    "Multiple clears within same render pass, methods: load, clear", params));
            }
            {
                const TestParams params = {formatPair.colorFormat,       // VkFormat format;
                                           formatPair.depthFormat,       // VkFormat depthFormat;
                                           ...
                                           {                             // vector<ClearStep> steps;
                                            ...
                                            {ClearOp::CLEAR, Vec4(0.0f, 1.0f, 0.0f, 1.0f), 0.9f}}};
                addChild(new MultipleClearsWithinRenderPassTest(
                    m_testCtx, "draw_clear" + testNameSuffix,
                    "Multiple clears within same render pass, methods: draw, clear", params));
            }
            {
                const TestParams params = {formatPair.colorFormat,       // VkFormat format;
                                           formatPair.depthFormat,       // VkFormat depthFormat;
                                           ...
                                           {                             // vector<ClearStep> steps;
                                            ...
                                            {ClearOp::CLEAR, Vec4(0.0f, 1.0f, 0.0f, 1.0f), 0.9f}}};
                addChild(new MultipleClearsWithinRenderPassTest(
                    m_testCtx, "clear_clear" + testNameSuffix,
                    "Multiple clears within same render pass, methods: clear, clear", params));
            }
        }
    }
}
