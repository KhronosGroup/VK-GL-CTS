    ...
    for (auto dataFromCompute : {false, true})
        for (int drawTypeIdx = 0; drawTypeIdx < DRAWTYPE_LAST; drawTypeIdx++)
        {
                ...
                m_testCtx, drawTypeStr.c_str(), ("Draws geometry using " + drawTypeStr + "draw call").c_str());
            {
                ...
                tcu::TestCaseGroup *indirectDrawMultiviewGroup = new tcu::TestCaseGroup(
                    m_testCtx, "indirect_draw_multiview", "Draws geometry with indirect draws and multiview");
                {
                        ...
                        !m_groupParams->useSecondaryCmdBuffer)
                    {
                        ...
                        indirectDrawGroup->addChild(
                            new InstanceFactory<IndirectDraw, FunctionSupport1<IndirectDraw::TestSpec>>(
                                m_testCtx, "triangle_strip_memory_access", "Draws triangle strip", testSpec,
                                FunctionSupport1<IndirectDraw::TestSpec>::Args(checkSupport, testSpec)));
