...
{
public:
    InstancedDrawCase(tcu::TestContext &testCtx, const std::string &name, const std::string &desc, TestParams params)
    ...
    virtual void checkSupport(Context &context) const
    {
        if (m_params.testAttribDivisor)
        {
            ...
            const vk::VkPhysicalDeviceVertexAttributeDivisorFeaturesEXT &vertexAttributeDivisorFeatures =
                context.getVertexAttributeDivisorFeaturesEXT();
...
tcu::TestStatus InstancedDrawInstance::iterate()
{
    ...
    if (m_params.function == TestParams::FUNCTION_DRAW_INDIRECT &&
        !m_context.getDeviceFeatures().drawIndirectFirstInstance)
    {
    ...
    for (int instanceCountNdx = 0; instanceCountNdx < DE_LENGTH_OF_ARRAY(instanceCounts); instanceCountNdx++)
    {
        ...
        for (int firstInstanceIndexNdx = 0; firstInstanceIndexNdx < firstInstanceIndicesCount; firstInstanceIndexNdx++)
        {
            ...
            if (m_params.testMultiview)
            {
                ...
                m_vk.cmdPipelineBarrier(*m_cmdBuffer, vk::VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                        vk::VK_PIPELINE_STAGE_TRANSFER_BIT, (vk::VkDependencyFlags)0, 0,
                                        (const vk::VkMemoryBarrier *)DE_NULL, 0,
                                        (const vk::VkBufferMemoryBarrier *)DE_NULL, 1, &barrier);
            }
            else
            {
                initialTransitionColor2DImage(m_vk, *m_cmdBuffer, m_colorTargetImage->object(),
            ...
            m_vk.cmdClearColorImage(*m_cmdBuffer, m_colorTargetImage->object(), vk::VK_IMAGE_LAYOUT_GENERAL,
                                    &clearColor.color, 1, &subresourceRange);
            ...
            if (m_params.useDynamicRendering)
                beginRendering(m_vk, *m_cmdBuffer, *m_colorTargetView, renderArea, clearColor,
                               ...
                               (m_params.testMultiview) ? 2u : 1u, (m_params.testMultiview) ? 3u : 0u);
            else
            ...
            m_vk.cmdBindVertexBuffers(*m_cmdBuffer, 0, DE_LENGTH_OF_ARRAY(vertexBuffers), vertexBuffers,
                                      vertexBufferOffsets);

            const float pushConstants[] = {(float)firstInstance, (float)instanceCount};
            m_vk.cmdPushConstants(*m_cmdBuffer, *m_pipelineLayout, vk::VK_SHADER_STAGE_VERTEX_BIT, 0u,
            ...
            {
            case TestParams::FUNCTION_DRAW:
                m_vk.cmdDraw(*m_cmdBuffer, (uint32_t)m_data.size(), instanceCount, 0u, firstInstance);
                break;

            case TestParams::FUNCTION_DRAW_INDEXED:
                m_vk.cmdDrawIndexed(*m_cmdBuffer, (uint32_t)m_indexes.size(), instanceCount, 0u, 0u, firstInstance);
                break;
            ...
            const rr::VertexAttrib vertexAttribs[] = {
                ...
                rr::VertexAttrib(
                    rr::VERTEXATTRIBTYPE_FLOAT, 4, sizeof(tcu::Vec4),
                    m_params.testAttribDivisor ? (m_params.attribDivisor == 0 ? INT_MAX : m_params.attribDivisor) : 1,
                    &m_instancedColor[0])};
            ...
            for (int testAttribDivisor = 0; testAttribDivisor < 2; testAttribDivisor++)
            {
                for (int divisorNdx = 0; divisorNdx < DE_LENGTH_OF_ARRAY(divisors); divisorNdx++)
                {
                    for (int multiviewNdx = 0; multiviewNdx < DE_LENGTH_OF_ARRAY(multiviews); multiviewNdx++)
                    {
                        ...
                        param.testAttribDivisor   = testAttribDivisor ? true : false;
                        ...
                        addChild(
                            new InstancedDrawCase(m_testCtx, de::toLower(testName), "Instanced drawing test", param));
                    }
                }
            }
        }
