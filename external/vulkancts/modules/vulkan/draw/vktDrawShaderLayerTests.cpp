...
public:
    ...
    Renderer(Context &context, const UVec2 &renderSize, const int numLayers, const VkFormat colorFormat,
             const Vec4 &clearColor, const std::vector<PositionColorVertex> &vertices, const Shader shader)
    ...
    void draw(Context &context, const VkBuffer colorBuffer) const
    {
        ...
        vk.cmdDraw(*m_cmdBuffer, static_cast<uint32_t>(m_numLayers * 6), 1u, 0u, 0u); // two triangles per layer
...
tcu::TestStatus testVertexShader(Context &context, const int numLayers)
{
    ...
    const VkDeviceSize colorBufferSize =
        renderSize.x() * renderSize.y() * tcu::getPixelSize(mapVkFormat(colorFormat)) * numLayers;
...

    {
        context.getTestContext().getLog()
            << tcu::TestLog::Message << "Rendering a rectangle in each of the " << numLayers << " layer(s)."
    ...
    // Verify layers.
    {
        ...
        for (int i = 0; i < numLayers; i++)
        {
            const tcu::ConstPixelBufferAccess resultImage(mapVkFormat(colorFormat), renderSize.x(), renderSize.y(), 1u,
                                                          resultMem + ((colorBufferSize / numLayers) * i));
...
tcu::TestStatus testTessellationShader(Context &context, const int numLayers)
{
    ...
    // Draw.
    {
        const Renderer renderer(context, renderSize, numLayers, colorFormat, clearColor, vertices,
...
tcu::TestCaseGroup *createShaderLayerTests(tcu::TestContext &testCtx)
{
    ...
    for (int i = 0; i < DE_LENGTH_OF_ARRAY(numLayersToTest); ++i)
    {
        ...
        addFunctionCaseWithPrograms(group.get(), "vertex_shader_" + de::toString(numLayers), "", checkRequirements,
                                    initVertexTestPrograms, testVertexShader, numLayers);
    }

    for (int i = 0; i < DE_LENGTH_OF_ARRAY(numLayersToTest); ++i)
    {
        int numLayers = numLayersToTest[i];
        addFunctionCaseWithPrograms(group.get(), "tessellation_shader_" + de::toString(numLayers), "",
                                    checkRequirements, initTessellationTestPrograms, testTessellationShader, numLayers);
    }
