...
public:
    ...
    Renderer(Context &context, const bool useDynamicRendering, const UVec2 &renderSize, const int numLayers,
    ...
    void draw(Context &context, const VkBuffer colorBuffer) const
    {
        ...
        if (m_useDynamicRendering)
        {
            ...
            beginRendering(vk, *m_cmdBuffer, *m_colorAttachment, renderArea, clearValue,
                           VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_ATTACHMENT_LOAD_OP_CLEAR, 0, m_numLayers);
        }
        else
        {
...
tcu::TestStatus testVertexShader(Context &context, const TestParams params)
{
    ...
    // Draw.
    {
        const Renderer renderer(context, params.useDynamicRendering, renderSize, params.numLayers, colorFormat,
                                clearColor, vertices, Renderer::VERTEX);
...
tcu::TestStatus testTessellationShader(Context &context, const TestParams params)
{
    ...
    {
        const Renderer renderer(context, params.useDynamicRendering, renderSize, params.numLayers, colorFormat,
                                clearColor, vertices, Renderer::TESSELLATION);
        renderer.draw(context, colorBuffer->object());
    }
...
tcu::TestCaseGroup *createShaderLayerTests(tcu::TestContext &testCtx, bool useDynamicRendering)
{
    ...
    TestParams parmas{1, useDynamicRendering};
    ...
    for (int i = 0; i < DE_LENGTH_OF_ARRAY(numLayersToTest); ++i)
    {
        ...
        addFunctionCaseWithPrograms<TestParams>(group.get(), "tessellation_shader_" + de::toString(parmas.numLayers),
                                                "", checkRequirements, initTessellationTestPrograms,
                                                testTessellationShader, parmas);
    }
