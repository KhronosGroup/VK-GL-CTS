...
tcu::TestStatus FragmentShadingBarycentricDataTestInstance::iterate(void)
{
    ...
    const auto pipelineWrapper =
        makeGraphicsPipeline(m_testParams.pipelineConstructionType, vki, vkd, physicalDevice, device, deviceExtensions,
                             pipelineLayout, *renderPass, vertModule, fragModule, width, height, topology,
                             VK_SAMPLE_COUNT_1_BIT, withColor, provokingVertexLast);
...
tcu::TestCaseGroup *createTests(tcu::TestContext &testCtx, const std::string &name)
{
    ...
    MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, name.c_str(), "Tests fragment shading barycentric extension"));
    // Tests using graphics pipeline libraries
    MovePtr<tcu::TestCaseGroup> libGroup(new tcu::TestCaseGroup(testCtx, "pipeline_library"));
    MovePtr<tcu::TestCaseGroup> fastLinkGroup(new tcu::TestCaseGroup(
        testCtx, "fast_linked_library", "Tests using graphics pipeline libraries with fast linking"));
