                                             ...
                                             const bool dynamicTopology = false)
{
    ...
    const VkPipelineMultisampleStateCreateInfo *multisampleStateInfoPtr =
        (rasterizationSamples > VK_SAMPLE_COUNT_1_BIT) ? &multisampleStateInfo : DE_NULL;
...
tcu::TestStatus FragmentShadingBarycentricDataTestInstance::iterate(void)
{
    ...
    const Move<VkPipeline> pipeline =
        makeGraphicsPipeline(vkd, device, *pipelineLayout, *renderPass, *vertModule, *fragModule, width, height,
                             topology, VK_SAMPLE_COUNT_1_BIT, withColor, provokingVertexLast);
...
tcu::TestStatus FragmentShadingBarycentricWeightTestInstance::iterate(void)
{
    ...
    for (size_t ndx = 0; ndx < 2; ndx++)
    {
        ...
        const Move<VkPipeline> pipeline = makeGraphicsPipeline(
            vkd, device, *pipelineLayout, *renderPass, *vertModule, *fragModule, width, height, pipelineTopology,
