...
BufferAddressTestCase::BufferAddressTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                             const CaseDef data)
...
tcu::TestStatus BufferAddressTestInstance::iterate(void)
{
    ...
    bindings[1].descriptorType =
        m_data.base == BASE_UBO ? VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER : VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    ...
    VkBufferCreateInfo bufferCreateInfo =
        makeBufferCreateInfo(DE_NULL, bufferSize,
    ...
    for (uint32_t i = 0; i < numBindings; ++i)
    {
        ...
        if (useKHR)
            ...
            gpuAddrs[i] = vk.getBufferDeviceAddressEXT(device, &bufferDeviceAddressInfo);
        cpuAddrs[i] = (uint8_t *)allocations[multiBuffer ? i : 0]->getHostPtr();
        if (!multiBuffer)
        {
    ...
    copyBuffer = de::MovePtr<BufferWithMemory>(new BufferWithMemory(
        vk, device, allocator,
        makeBufferCreateInfo(DE_NULL, DIM * DIM * sizeof(uint32_t), VK_BUFFER_USAGE_TRANSFER_DST_BIT, 0),
    ...
    else if (m_data.stage == STAGE_RAYGEN)
    {
        ...
        sbtBuffer     = de::MovePtr<BufferWithMemory>(new BufferWithMemory(
            vk, device, allocator,
            makeBufferCreateInfo(DE_NULL, rayTracingProperties.shaderGroupHandleSize,
...
{
public:
    CaptureReplayTestCase(tcu::TestContext &context, const char *name, const char *desc, uint32_t seed);
...
CaptureReplayTestCase::CaptureReplayTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                             uint32_t seed)
    : vkt::TestCase(context, name, desc)
    , m_seed(seed)
{
...
tcu::TestStatus CaptureReplayTestInstance::iterate(void)
{
    ...
    VkBufferCreateInfo bufferCreateInfo =
        makeBufferCreateInfo(DE_NULL, 0,
                             VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT |
                                 VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
...
tcu::TestCaseGroup *createBufferDeviceAddressTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "buffer_device_address", "Test VK_EXT_buffer_device_address"));
    ...
    TestGroupCase setCases[] = {
        {0, "set0", "set 0"},    {3, "set3", "set 3"},    {7, "set7", "set 7"},
        {15, "set15", "set 15"}, {31, "set31", "set 31"},
    };
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> setGroup(
            new tcu::TestCaseGroup(testCtx, setCases[setNdx].name, setCases[setNdx].description));
        for (int depthNdx = 0; depthNdx < DE_LENGTH_OF_ARRAY(depthCases); depthNdx++)
        {
            de::MovePtr<tcu::TestCaseGroup> depthGroup(
                new tcu::TestCaseGroup(testCtx, depthCases[depthNdx].name, depthCases[depthNdx].description));
            for (int baseNdx = 0; baseNdx < DE_LENGTH_OF_ARRAY(baseCases); baseNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> baseGroup(
                    new tcu::TestCaseGroup(testCtx, baseCases[baseNdx].name, baseCases[baseNdx].description));
                for (int cvtNdx = 0; cvtNdx < DE_LENGTH_OF_ARRAY(cvtCases); cvtNdx++)
                {
                    de::MovePtr<tcu::TestCaseGroup> cvtGroup(
                        new tcu::TestCaseGroup(testCtx, cvtCases[cvtNdx].name, cvtCases[cvtNdx].description));
                    for (int storeNdx = 0; storeNdx < DE_LENGTH_OF_ARRAY(storeCases); storeNdx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> storeGroup(new tcu::TestCaseGroup(
                            testCtx, storeCases[storeNdx].name, storeCases[storeNdx].description));
                        for (int btNdx = 0; btNdx < DE_LENGTH_OF_ARRAY(btCases); btNdx++)
                        {
                            de::MovePtr<tcu::TestCaseGroup> btGroup(
                                new tcu::TestCaseGroup(testCtx, btCases[btNdx].name, btCases[btNdx].description));
                            for (int layoutNdx = 0; layoutNdx < DE_LENGTH_OF_ARRAY(layoutCases); layoutNdx++)
                            {
                                de::MovePtr<tcu::TestCaseGroup> layoutGroup(new tcu::TestCaseGroup(
                                    testCtx, layoutCases[layoutNdx].name, layoutCases[layoutNdx].description));
                                for (int stageNdx = 0; stageNdx < DE_LENGTH_OF_ARRAY(stageCases); stageNdx++)
                                {
                                    ...
                                    layoutGroup->addChild(new BufferAddressTestCase(
                                        testCtx, stageCases[stageNdx].name, stageCases[stageNdx].description, c));
                                }
    ...
    de::MovePtr<tcu::TestCaseGroup> capGroup(new tcu::TestCaseGroup(
        testCtx, "capture_replay_stress", "Test VK_BUFFER_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT_EXT"));
