...
std::vector<VkDescriptorType> toDescriptorTypeVector(DescriptorTypeFlags bitfield)
{
    ...
    if (bitfield & DTFB_MUTABLE_VALVE)
        result.push_back(VK_DESCRIPTOR_TYPE_MUTABLE_VALVE);
...
public:
    ...
    de::MovePtr<BindingInterface> toMutable(uint32_t iteration) const override
    {
        ...
        return de::MovePtr<BindingInterface>(
            new SingleBinding(VK_DESCRIPTOR_TYPE_MUTABLE_VALVE, toDescriptorTypeVector(descFlags)));
    }
        ...
        for (const auto &b : bindings)
        {
            ...
            if (strategy == PoolMutableStrategy::KEEP_TYPES || strategy == PoolMutableStrategy::EXPAND_TYPES)
            {
                if (mainType == VK_DESCRIPTOR_TYPE_MUTABLE_VALVE)
                {
                    ...
                    const VkMutableDescriptorTypeListVALVE typeList = {static_cast<uint32_t>(lastVec.size()),
...
{
public:
    MutableTypesTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
    ...
    for (uint32_t iter = 0u; iter < numIterations; ++iter)
    {
        ...
        for (size_t bindingIdx = 0; bindingIdx < numBindings; ++bindingIdx)
        {
            ...
            shader << binding->glslCheckStatements(iter, 0u, idx32, getDescriptorNumericValue(iter, idx32),
                                                   tcu::nothing<uint32_t>(), usePushConstants);
        }
        ...
        for (const auto &descType : usedDescriptorTypes)
        {
            ...
            case VK_DESCRIPTOR_TYPE_MUTABLE_VALVE:
                TCU_THROW(InternalError,
                          "Found VK_DESCRIPTOR_TYPE_MUTABLE_VALVE in list of used array descriptor types");
                break;
...
{
    GroupPtr mainGroup(
        new tcu::TestCaseGroup(testCtx, "mutable_descriptor", "Tests for VK_VALVE_mutable_descriptor_type"));
    ...
    // Basic tests with a single mutable descriptor.
    {
        GroupPtr singleCases(new tcu::TestCaseGroup(testCtx, "single",
                                                    "Basic mutable descriptor tests with a single mutable descriptor"));
        ...
        {
            GroupPtr subGroup(new tcu::TestCaseGroup(
                testCtx, "switches", "Test switching from one to another descriptor type works as expected"));
    ...
    {
        GroupPtr singleNonMutableGroup(
            new tcu::TestCaseGroup(testCtx, "single_nonmutable", "Tests using a single non-mutable descriptor"));
    ...
    {
        GroupPtr singleAndArrayGroup(new tcu::TestCaseGroup(
            testCtx, "single_and_array", "Tests using a single mutable binding followed by a mutable array binding"));
    ...
    // Cases with several mutable non-array bindings.
    {
        ...
        GroupPtr mutableOnlyGroup(
            new tcu::TestCaseGroup(testCtx, "mutable_only", "Tests using only mutable descriptors"));
        GroupPtr mixedGroup(
            new tcu::TestCaseGroup(testCtx, "mixed", "Tests mixing mutable descriptors an non-mutable descriptors"));
