...
DescriptorSetRandomTestCase::DescriptorSetRandomTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                                         const CaseDef &data)
                ...
                if (caseDef.maxInlineUniformBlocks > 0)
                {
                    if (numInlineUniformBlocks < caseDef.maxInlineUniformBlocks)
                    {
                        ...
                        binding.descriptorCount =
                            (arraySizes[b] ? arraySizes[b] : 1) * 16 + 16; // add 16 for "ivec4 dummy"
    ...
    for (uint32_t s = 0; s < m_data.numDescriptorSets; ++s)
    {
        ...
        for (size_t b = 0; b < bindings.size(); ++b)
        {
            ...
            uint32_t descriptorIncrement =
                (binding.descriptorType == VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK_EXT) ? 16 : 1;
            ...
            if (binding.descriptorCount > 0)
            {
                ...
                case VK_DESCRIPTOR_TYPE_INLINE_UNIFORM_BLOCK_EXT:
                    decls << "layout(set = " << s << ", binding = " << b << ") uniform inlineubodef" << s << "_" << b
                          << " { ivec4 dummy; int val" << array.str() << "; } inlineubo" << s << "_" << b << ";\n";
                    break;
                case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
                case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
...
tcu::TestStatus DescriptorSetRandomTestInstance::iterate(void)
{
    ...
    for (uint32_t s = 0; s < m_data.numDescriptorSets; ++s)
    {
        ...
        descriptorPools[s] =
            poolBuilder.build(vk, device, poolCreateFlags, 1u,
            ...
            else if (binding.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
            {
                ...
                for (uint32_t d = descriptor; d < descriptor + binding.descriptorCount; ++d)
                {
                    ...
                    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, m_data.allPipelineStages,
                                          ...
                                          (const VkBufferMemoryBarrier *)DE_NULL, 1, &postStorageImageBarrier);
    ...
    for (uint32_t s = 0; s < m_data.numDescriptorSets; ++s)
    {
        ...
        if (randRange(&rnd, 1, 2) == 1 && m_context.contextSupports(vk::ApiVersion(1, 1, 0)) &&
    ...
    else if (m_data.stage == STAGE_RAYGEN_NV)
    {
        ...
        VkRayTracingShaderGroupCreateInfoNV group = {
            ...
            VK_SHADER_UNUSED_NV,                                       //  uint32_t closestHitShader;
            VK_SHADER_UNUSED_NV,                                       //  uint32_t anyHitShader;
            VK_SHADER_UNUSED_NV,                                       //  uint32_t intersectionShader;
        };
        ...
        vk.getRayTracingShaderGroupHandlesNV(device, *pipeline, 0, 1, static_cast<uintptr_t>(allocSize), ptr);
    }
    ...
    else
    {
        ...
        const VkSubpassDescription subpassDesc = {
            ...
            (attachmentReferences.empty() ?
                 DE_NULL :
                 attachmentReferences.data()), // const VkAttachmentReference*    pInputAttachments
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0u, DE_NULL, 1u,
                          &copyBufferBarrier, 0u, DE_NULL);
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0u, DE_NULL, 1u,
                          &storageImgBufferBarrier, 0u, DE_NULL);
...
tcu::TestCaseGroup *createDescriptorSetRandomTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "descriptorset_random", "Randomly-generated desciptor set layouts"));
    ...
    TestGroupCase stageCases[] = {
        {STAGE_COMPUTE, "comp", "compute"}, {STAGE_FRAGMENT, "frag", "fragment"},
        {STAGE_VERTEX, "vert", "vertex"},   {STAGE_RAYGEN_NV, "rgnv", "raygen_nv"},
        {STAGE_RAYGEN, "rgen", "raygen"},   {STAGE_INTERSECT, "sect", "intersect"},
        {STAGE_ANY_HIT, "ahit", "any_hit"}, {STAGE_CLOSEST_HIT, "chit", "closest_hit"},
        {STAGE_MISS, "miss", "miss"},       {STAGE_CALLABLE, "call", "callable"},
    };
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> setsGroup(
            new tcu::TestCaseGroup(testCtx, setsCases[setsNdx].name, setsCases[setsNdx].description));
        for (int indexNdx = 0; indexNdx < DE_LENGTH_OF_ARRAY(indexCases); indexNdx++)
        {
            de::MovePtr<tcu::TestCaseGroup> indexGroup(
                new tcu::TestCaseGroup(testCtx, indexCases[indexNdx].name, indexCases[indexNdx].description));
            for (int uboNdx = 0; uboNdx < DE_LENGTH_OF_ARRAY(uboCases); uboNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> uboGroup(
                    new tcu::TestCaseGroup(testCtx, uboCases[uboNdx].name, uboCases[uboNdx].description));
                for (int sboNdx = 0; sboNdx < DE_LENGTH_OF_ARRAY(sboCases); sboNdx++)
                {
                    de::MovePtr<tcu::TestCaseGroup> sboGroup(
                        new tcu::TestCaseGroup(testCtx, sboCases[sboNdx].name, sboCases[sboNdx].description));
                    for (int sampledImgNdx = 0; sampledImgNdx < DE_LENGTH_OF_ARRAY(sampledImgCases); sampledImgNdx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> sampledImgGroup(new tcu::TestCaseGroup(
                            testCtx, sampledImgCases[sampledImgNdx].name, sampledImgCases[sampledImgNdx].description));
                        for (int storageImgNdx = 0; storageImgNdx < DE_LENGTH_OF_ARRAY(sImgTexCases); ++storageImgNdx)
                        {
                            de::MovePtr<tcu::TestCaseGroup> storageImgGroup(new tcu::TestCaseGroup(
                                testCtx, sImgTexCases[storageImgNdx].name, sImgTexCases[storageImgNdx].description));
                            for (int iubNdx = 0; iubNdx < DE_LENGTH_OF_ARRAY(iubCases); iubNdx++)
                            {
                                de::MovePtr<tcu::TestCaseGroup> iubGroup(new tcu::TestCaseGroup(
                                    testCtx, iubCases[iubNdx].name, iubCases[iubNdx].description));
                                for (int uabNdx = 0; uabNdx < DE_LENGTH_OF_ARRAY(uabCases); uabNdx++)
                                {
                                    de::MovePtr<tcu::TestCaseGroup> uabGroup(new tcu::TestCaseGroup(
                                        testCtx, uabCases[uabNdx].name, uabCases[uabNdx].description));
                                    for (int stageNdx = 0; stageNdx < DE_LENGTH_OF_ARRAY(stageCases); stageNdx++)
                                    {
                                        ...
                                        const VkFlags rtShaderStagesNV =
                                            currentStage == STAGE_RAYGEN_NV ? VK_SHADER_STAGE_RAYGEN_BIT_NV : 0;
                                        const VkFlags rtPipelineStagesNV =
                                            currentStage == STAGE_RAYGEN_NV ?
                                                VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_NV :
                                                0;
                                        const VkFlags rtShaderStagesKHR =
                                            isRayTracingStageKHR(currentStage) ? ALL_RAY_TRACING_STAGES : 0;
                                        const VkFlags rtPipelineStagesKHR =
                                            isRayTracingStageKHR(currentStage) ?
                                        ...
                                        de::MovePtr<tcu::TestCaseGroup> stageGroup(new tcu::TestCaseGroup(
                                            testCtx, stageCases[stageNdx].name, stageCases[stageNdx].description));
                                        for (int iaNdx = 0; iaNdx < DE_LENGTH_OF_ARRAY(iaCases); ++iaNdx)
                                        {
                                            ...
                                            de::MovePtr<tcu::TestCaseGroup> iaGroup(new tcu::TestCaseGroup(
                                                testCtx, iaCases[iaNdx].name, iaCases[iaNdx].description));
                                            ...
                                            for (uint32_t rnd = 0; rnd < numSeeds; ++rnd)
                                            {
                                                ...
                                                const VkFlags allPipelineStages = VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT |
                                                ...
                                                CaseDef c = {
                                                    ...
                                                    seed++,                                  // uint32_t seed;
                                                    rtShaderStages ? rtShaderStages :
                                                                     allShaderStages, // VkFlags allShaderStages;
                                                    rtPipelineStages ? rtPipelineStages :
                                                ...
                                                iaGroup->addChild(
                                                    new DescriptorSetRandomTestCase(testCtx, name.c_str(), "test", c));
                                            }
