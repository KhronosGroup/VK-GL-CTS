...
tcu::TestStatus EmptyDescriptorUpdateCase(Context &context)
{
    ...
    vk::Unique<vk::VkDescriptorPool> descriptorPool(
        vk::DescriptorPoolBuilder()
            .addType(vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 2)
...
tcu::TestCaseGroup *createEmptyDescriptorUpdateTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
        testCtx, "empty_descriptor", "Update last descriptor in a set that includes an empty binding"));
...
{
public:
    SamplerlessDescriptorWriteTestCase(tcu::TestContext &testCtx, const std::string &name,
                                       const std::string &description, const SamplerlessParams &params);
...
SamplerlessDescriptorWriteTestCase::SamplerlessDescriptorWriteTestCase(tcu::TestContext &testCtx,
...
void SamplerlessDescriptorWriteTestCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    case vk::VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        descriptorDecl =
            "layout(rgba8, set=" + std::to_string(m_params.descriptorSet) + ", binding=0) uniform image2D img;";
        readOp = "imageLoad(img, ivec2(0, 0))";
        break;
    case vk::VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
        descriptorDecl = "layout(input_attachment_index=0, set=" + std::to_string(m_params.descriptorSet) +
                         ", binding=0) uniform subpassInput img;";
...
vk::VkFormatFeatureFlagBits SamplerlessDescriptorWriteTestCase::getMainImageFeature(void) const
{
    ...
    {
    case vk::VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
        feature = vk::VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT;
        break;
    case vk::VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        feature = vk::VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT;
        break;
    case vk::VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
        feature = vk::VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT;
        break;
    default:
        DE_ASSERT(false);
        break;
...
vk::VkExtent3D SamplerlessDescriptorWriteTestInstance::getMainImageExtent(void) const
{
    ...
    case vk::VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        extent = &kMinimumExtent;
        break;
    case vk::VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
        extent = &kFramebufferExtent;
        break;
    default:
        DE_ASSERT(false);
        break;
...
vk::VkImageUsageFlags SamplerlessDescriptorWriteTestInstance::getMainImageUsage(void) const
{
    ...
    case vk::VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        ...
        break;
    case vk::VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
        usage |= vk::VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
        break;
    default:
        DE_ASSERT(false);
        break;
...
tcu::TestStatus SamplerlessDescriptorWriteTestInstance::iterate(void)
{
    ...
    const auto mainView =
        vk::makeImageView(vkd, device, mainImage.get(), vk::VK_IMAGE_VIEW_TYPE_2D, kImageFormat, colorSubresourceRange);
    const auto fbView =
        vk::makeImageView(vkd, device, fbImage.get(), vk::VK_IMAGE_VIEW_TYPE_2D, kImageFormat, colorSubresourceRange);

    // Buffer to copy rendering result to.
    const vk::VkDeviceSize resultsBufferSize =
        static_cast<vk::VkDeviceSize>(static_cast<uint32_t>(tcu::getPixelSize(tcuFormat)) * kFramebufferExtent.width *
                                      kFramebufferExtent.height * kFramebufferExtent.depth);
...
tcu::TestCaseGroup *createSamplerlessWriteTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "samplerless", "Verify sampler unused with some descriptor image types"));
...
public:
    ...
    static const uint32_t kNumBuffers;
    static const uint32_t kNumOffsets;
...
const uint32_t RandomDescriptorUpdateTestInstance::kNumBuffers              = 3u;
...
tcu::TestStatus RandomDescriptorUpdateTestInstance::iterate()
{
    ...
    const uint32_t bufferSize = 256u * kNumOffsets;

    float bufferContents[kNumBuffers][bufferSize / 4];
    float counter            = 1.0f;
    float sign               = 1.0f;
    uint32_t offset          = 0;
    uint32_t channelSelector = 0;
    ...
    for (uint32_t b = 0; b < kNumBuffers; b++)
    {
        ...
        for (uint32_t o = 0; o < kNumOffsets; o++)
        {
            offset = o * 64;
    ...
    {
        buffers.emplace_back(std::make_shared<vk::BufferWithMemory>(vkd, device, allocator, bufferInfo,
                                                                    vk::MemoryRequirement::HostVisible));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "random", "Update descriptors randomly between draws"));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "descriptor_update", "Update descriptor sets"));
