...
void bindDescriptorSets(const vk::DeviceInterface &m_vki, vk::VkCommandBuffer cmdBuffer,
                        vk::VkShaderStageFlags stageFlags, vk::VkPipelineBindPoint bindPoint,
                        vk::VkPipelineLayout layout, uint32_t first, uint32_t count,
...
BufferRenderInstance::BufferRenderInstance(Context &context, DescriptorUpdateMethod updateMethod, bool isPrimaryCmdBuf,
                                           ...
                                           bool viewOffset, bool dynamicOffset, bool dynamicOffsetNonZero, bool bind2)
...
void BufferRenderInstance::writeDrawCmdBuffer(vk::VkCommandBuffer cmd) const
{
        ...
        m_updateMethod != DESCRIPTOR_UPDATE_METHOD_WITH_PUSH)
    {
        ...
        case DESCRIPTOR_SET_COUNT_MULTIPLE:
        {
            ...
            bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(), 0,
                               (int)sets.size(), &sets.front(), numOffsets, dynamicOffsetPtr, m_bind2);
            break;
        }
        case DESCRIPTOR_SET_COUNT_MULTIPLE_DISCONTIGUOUS:
        {
            ...
            for (uint32_t setNdx = 0; setNdx < getDescriptorSetCount(m_descriptorSetCount); setNdx++)
            {
                ...
                bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(),
                                   descriptorSetNdx, 1, &sets[setNdx], numOffsets, dynamicOffsetPtr, m_bind2);
...
) const
{
...
#endif
    {
        ...
        case DESCRIPTOR_SET_COUNT_MULTIPLE:
        {
            bindDescriptorSets(m_vki, *cmd, vk::VK_SHADER_STAGE_COMPUTE_BIT, vk::VK_PIPELINE_BIND_POINT_COMPUTE,
                               m_pipelineLayout, 0, getDescriptorSetCount(m_descriptorSetCount), m_descriptorSets,
                               m_numDynamicOffsets, m_dynamicOffsets, m_bind2);
            break;
        }
        case DESCRIPTOR_SET_COUNT_MULTIPLE_DISCONTIGUOUS:
        {
            ...
            for (uint32_t setNdx = 0; setNdx < getDescriptorSetCount(m_descriptorSetCount); setNdx++)
            {
                ...
                bindDescriptorSets(m_vki, *cmd, vk::VK_SHADER_STAGE_COMPUTE_BIT, vk::VK_PIPELINE_BIND_POINT_COMPUTE,
                                   m_pipelineLayout, descriptorSetNdx, 1u, &m_descriptorSets[setNdx], numOffsets,
...
vkt::TestInstance *BufferDescriptorCase::createInstance(vkt::Context &context) const
{
    ...
    if (m_exitingStages == vk::VK_SHADER_STAGE_COMPUTE_BIT)
    {
        ...
        return new BufferRenderInstance(context, m_updateMethod, m_isPrimaryCmdBuf, m_descriptorType,
                                        ...
                                        m_dynamicOffsetSet, m_dynamicOffsetNonZero, m_bind2);
}
...
void ImageFetchRenderInstance::writeDrawCmdBuffer(vk::VkCommandBuffer cmd) const
{
        ...
        m_updateMethod != DESCRIPTOR_UPDATE_METHOD_WITH_PUSH)
    {
        ...
        {
            bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(), 0,
                               (int)sets.size(), &sets.front(), 0, DE_NULL, m_bind2);
            break;
        }
        case DESCRIPTOR_SET_COUNT_MULTIPLE_DISCONTIGUOUS:
        {
            for (uint32_t setNdx = 0; setNdx < getDescriptorSetCount(m_descriptorSetCount); setNdx++)
            {
                const uint32_t descriptorSetNdx = getDescriptorSetNdx(m_descriptorSetCount, setNdx);
                bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(),
                                   descriptorSetNdx, 1, &sets[setNdx], 0, DE_NULL, m_bind2);
            }
            break;
        }
        default:
...
public:
    ImageSampleRenderInstance(vkt::Context &context, DescriptorUpdateMethod updateMethod, bool isPrimaryCmdBuf,
                              ...
                              vk::VkShaderStageFlags stageFlags, ShaderInputInterface shaderInterface,
                              vk::VkImageViewType viewType, uint32_t baseMipLevel, uint32_t baseArraySlice,
                              bool isImmutable, const bool bind2);
...
void ImageSampleRenderInstance::writeDrawCmdBuffer(vk::VkCommandBuffer cmd) const
{
        ...
        m_updateMethod != DESCRIPTOR_UPDATE_METHOD_WITH_PUSH)
    {
        ...
        case DESCRIPTOR_SET_COUNT_MULTIPLE:
        {
            bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(), 0u,
                               (int)setHandles.size(), &setHandles.front(), 0u, DE_NULL, m_bind2);
            break;
        }
        case DESCRIPTOR_SET_COUNT_MULTIPLE_DISCONTIGUOUS:
        {
            for (uint32_t setNdx = 0; setNdx < getDescriptorSetCount(m_descriptorSetCount); setNdx++)
            {
                const uint32_t descriptorSetNdx = getDescriptorSetNdx(m_descriptorSetCount, setNdx);
                bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(),
                                   descriptorSetNdx, 1u, &setHandles[setNdx], 0u, DE_NULL, m_bind2);
            }
            break;
        }
        default:
    ...
    case vk::VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
        if (m_exitingStages == vk::VK_SHADER_STAGE_COMPUTE_BIT)
        {
            ...
            return new ImageSampleRenderInstance(context, m_updateMethod, m_isPrimaryCmdBuf, m_descriptorType,
                                                 ...
                                                 m_baseMipLevel, m_baseArraySlice, m_isImmutableSampler, m_bind2);
    ...
    case vk::VK_DESCRIPTOR_TYPE_STORAGE_IMAGE:
        if (m_exitingStages == vk::VK_SHADER_STAGE_COMPUTE_BIT)
        {
            ...
            return new ImageFetchRenderInstance(context, m_updateMethod, m_isPrimaryCmdBuf, m_descriptorType,
                                                m_descriptorSetCount, m_activeStages, m_shaderInterface, m_viewType,
                                                m_baseMipLevel, m_baseArraySlice, m_bind2);
...
public:
    TexelBufferRenderInstance(vkt::Context &context, DescriptorUpdateMethod updateMethod, bool isPrimaryCmdBuf,
                              vk::VkDescriptorType descriptorType, DescriptorSetCount descriptorSetCount,
                              vk::VkShaderStageFlags stageFlags, ShaderInputInterface shaderInterface,
                              bool nonzeroViewOffset, const bool bind2);
...
void TexelBufferRenderInstance::writeDrawCmdBuffer(vk::VkCommandBuffer cmd) const
{
        ...
        m_updateMethod != DESCRIPTOR_UPDATE_METHOD_WITH_PUSH)
    {
        ...
        {
            bindDescriptorSets(m_vki, cmd, m_stageFlags, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, getPipelineLayout(), 0,
                               (uint32_t)sets.size(), &sets.front(), 0, DE_NULL, m_bind2);
            break;
        }
        case DESCRIPTOR_SET_COUNT_MULTIPLE_DISCONTIGUOUS:
        {
