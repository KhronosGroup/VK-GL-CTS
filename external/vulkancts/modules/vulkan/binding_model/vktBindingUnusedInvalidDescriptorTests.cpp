...
public:
        ...
        , m_invalid(invalid)
    {
        ...
        if (requireImage(m_type))
        {
            ...
            m_image                                 = vk::createImage(vk, device, &imageCreateInfo);
            const VkMemoryRequirements requirements = getImageMemoryRequirements(vk, device, m_image.get());
            m_allocation                            = allocator.allocate(requirements, MemoryRequirement::Any);

            VK_CHECK(vk.bindImageMemory(device, m_image.get(), m_allocation->getMemory(), m_allocation->getOffset()));
            ...
            m_imageView = makeImageView(vk, device, m_image.get(), VK_IMAGE_VIEW_TYPE_2D, format,
            ...
            {
                m_sampler = Move<VkSampler>();
            }

            m_imageInfo.imageView   = m_imageView.get();
            m_imageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
            m_imageInfo.sampler     = m_sampler.get();
        }
        else // reqireBuffer
        {
            ...
            m_buffer                                = vk::createBuffer(vk, device, &bufferCreateInfo);
            const VkMemoryRequirements requirements = getBufferMemoryRequirements(vk, device, m_buffer.get());
            m_allocation                            = allocator.allocate(requirements, MemoryRequirement::HostVisible);

            VK_CHECK(vk.bindBufferMemory(device, m_buffer.get(), m_allocation->getMemory(), m_allocation->getOffset()));

            m_bufferInfo.buffer = m_buffer.get();
    ...
    void update(const DeviceInterface &vk, const VkDevice device, const QueueData &queueData)
    {
        ...
        if (requireImage(m_type))
        {
            ...
            vk.cmdClearColorImage(cmdBuffer.get(), m_image.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                  &clearColorValue, 1u, &colorSubresourceRange);
