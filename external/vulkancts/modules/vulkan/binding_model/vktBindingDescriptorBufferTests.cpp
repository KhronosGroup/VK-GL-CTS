...
std::string glslOutputVerification(const TestParams &params, const std::vector<SimpleBinding> &simpleBindings, bool)
{
    ...
    if ((params.variant == TestVariant::SINGLE) || (params.variant == TestVariant::MULTIPLE) ||
        ...
        (params.variant == TestVariant::ROBUST_NULL_DESCRIPTOR) || (params.variant == TestVariant::CAPTURE_REPLAY))
    {
...
{
public:
    DescriptorBufferTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
void DescriptorBufferTestCase::checkSupport(Context &context) const
{
    ...
    if (m_params.variant == TestVariant::MULTIPLE)
    {
        const VkPhysicalDeviceVulkan13Properties &vulkan13properties =
            *findStructure<VkPhysicalDeviceVulkan13Properties>(&context.getDeviceProperties2());
    ...
    for (uint32_t arrayIndex = 0; arrayIndex < arrayCount; ++arrayIndex)
    {
                 ...
                 (binding.descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER))
        {
            ...
            }
            {
                ...
                uint32_t *pBufferData = static_cast<uint32_t *>(stagingBuffer.alloc->getHostPtr());
...
tcu::TestStatus DescriptorBufferTestInstance::iterate()
{
    ...
    if (m_params.isCompute())
    {
        ...
        m_pipeline = makeComputePipeline(vk, *m_device, *m_pipelineLayout, VK_PIPELINE_CREATE_DESCRIPTOR_BUFFER_BIT_EXT,
                                         *shaderModule, (VkPipelineShaderStageCreateFlags)0, pSpecializationInfo);
    }
...
void populateDescriptorBufferTests(tcu::TestCaseGroup *topGroup)
{
            ...
            for (auto pStage = choiceStages; pStage < DE_ARRAY_END(choiceStages); ++pStage)
                for (auto pOptions = caseOptions; pOptions < DE_ARRAY_END(caseOptions); ++pOptions)
                {
                    ...
                    if ((pOptions->setsPerBuffer != 1) && (pOptions->bufferBindingCount < 4))
                    {
                        ...
                        subGroup->addChild(new DescriptorBufferTestCase(
                            testCtx, getCaseNameUpdateHash(params, subGroupHash), "", params));
                    }
                }
...

    {
        ...
        MovePtr<tcu::TestCaseGroup> subGroup(
            new tcu::TestCaseGroup(testCtx, "max", "Max sampler/resource bindings tests"));
        const uint32_t subGroupHash = baseSeed ^ deStringHash(subGroup->getName());
...

    {
        ...
        //
        MovePtr<tcu::TestCaseGroup> subGroup(
            new tcu::TestCaseGroup(testCtx, "embedded_imm_samplers", "Max embedded immutable samplers tests"));
        const uint32_t subGroupHash = baseSeed ^ deStringHash(subGroup->getName());
...

    {
        ...
        //
        MovePtr<tcu::TestCaseGroup> subGroupPush(new tcu::TestCaseGroup(
            testCtx, "push_descriptor", "Use push descriptors in addition to descriptor buffer"));
        MovePtr<tcu::TestCaseGroup> subGroupPushTemplate(new tcu::TestCaseGroup(
            testCtx, "push_template",
            "Use descriptor update template with push descriptors in addition to descriptor buffer"));
...

    {
        ...
        MovePtr<tcu::TestCaseGroup> subGroupBuffer(
            new tcu::TestCaseGroup(testCtx, "buffer_access", "Robust buffer access"));
        MovePtr<tcu::TestCaseGroup> subGroupNullDescriptor(
            new tcu::TestCaseGroup(testCtx, "null_descriptor", "Null descriptor"));
        ...
        for (auto pQueue = choiceQueues; pQueue < DE_ARRAY_END(choiceQueues); ++pQueue)
            for (auto pStage = choiceStages; pStage < DE_ARRAY_END(choiceStages); ++pStage)
            {
                ...
                subGroupBuffer->addChild(new DescriptorBufferTestCase(
                    testCtx, getCaseNameUpdateHash(params, subGroupBufferHash), "", params));
...

    {
        ...
        //
        MovePtr<tcu::TestCaseGroup> subGroup(
            new tcu::TestCaseGroup(testCtx, "capture_replay", "Capture and replay tests"));
        const uint32_t subGroupHash = baseSeed ^ deStringHash(subGroup->getName());
