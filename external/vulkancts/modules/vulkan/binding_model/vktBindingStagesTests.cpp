...
vk::VkImageCreateInfo makeImageCreateInfo (const vk::VkFormat		format,
...
tcu::TestStatus	StagesTestInstance::iterate (void)
{
	...
	const auto			readDescriptorSetLayout(vk::DescriptorSetLayoutBuilder()
		.addSingleBinding(m_descriptorType, vk::VK_SHADER_STAGE_FRAGMENT_BIT | vk::VK_SHADER_STAGE_COMPUTE_BIT)
		.build(vk, device));
	const auto			writeDescriptorSetLayout(vk::DescriptorSetLayoutBuilder()
		.addSingleBinding(vk::VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, vk::VK_SHADER_STAGE_FRAGMENT_BIT | vk::VK_SHADER_STAGE_COMPUTE_BIT)
		.build(vk, device));
	const auto			pipelineLayout		= vk::makePipelineLayout(vk, device, { *readDescriptorSetLayout, *writeDescriptorSetLayout });
	...
	if (m_descriptorType == vk::VK_DESCRIPTOR_TYPE_STORAGE_BUFFER || m_descriptorType == vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
	{
		vk::VkBufferUsageFlags usage = m_descriptorType == vk::VK_DESCRIPTOR_TYPE_STORAGE_BUFFER ? vk::VK_BUFFER_USAGE_STORAGE_BUFFER_BIT : vk::VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT;
		...
		readBuffer = de::MovePtr<vk::BufferWithMemory>(new vk::BufferWithMemory(vk, device, allocator, readBufferCreateInfo, vk::MemoryRequirement::HostVisible));
	...
	else
	{
		...
		readImage = de::MovePtr<vk::ImageWithMemory>(new vk::ImageWithMemory(vk, device, allocator, imageCreateInfo, vk::MemoryRequirement::Any));
		...
		const auto& readAlloc = readBuffer->getAllocation();
		const auto	readDataPtr = reinterpret_cast<deUint8*>(readAlloc.getHostPtr()) + readAlloc.getOffset();
		for (deUint32 i = 0; i < 4 * 4 * 4; ++i)
			readDataPtr[i] = (deUint8)(((i + 1) % 4) * 64 - 1);
		vk::flushAlloc(vk, device, readAlloc);

		const auto			copyCmdBuffer = allocateCommandBuffer(vk, device, cmdPool.get(), vk::VK_COMMAND_BUFFER_LEVEL_PRIMARY);
		...
		vk::VkImageMemoryBarrier preImageMemoryBarrier = vk::makeImageMemoryBarrier(vk::VK_ACCESS_NONE, vk::VK_ACCESS_TRANSFER_WRITE_BIT, vk::VK_IMAGE_LAYOUT_UNDEFINED, vk::VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, **readImage, subresourceRange);
		vk::VkImageMemoryBarrier postImageMemoryBarrier = vk::makeImageMemoryBarrier(vk::VK_ACCESS_TRANSFER_WRITE_BIT, vk::VK_ACCESS_SHADER_READ_BIT, vk::VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, vk::VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, **readImage, subresourceRange);
		vk.cmdPipelineBarrier(*copyCmdBuffer, vk::VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &preImageMemoryBarrier);
		vk.cmdCopyBufferToImage(*copyCmdBuffer, **readBuffer, **readImage, vk::VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1u, &region);
		vk.cmdPipelineBarrier(*copyCmdBuffer, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, vk::VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &postImageMemoryBarrier);
	...
	const auto			writeBuffer	= de::MovePtr<vk::BufferWithMemory>(new vk::BufferWithMemory(vk, device, allocator, writeBufferCreateInfo, vk::MemoryRequirement::HostVisible));
	...
	const auto			colorImage			(makeImage(vk, device, makeImageCreateInfo(colorFormat, renderSize, vk::VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | vk::VK_IMAGE_USAGE_TRANSFER_SRC_BIT)));
	const auto			colorImageAlloc		(bindImage(vk, device, allocator, *colorImage, vk::MemoryRequirement::Any));
	const auto			colorImageView		(makeImageView(vk, device, *colorImage, vk::VK_IMAGE_VIEW_TYPE_2D, colorFormat, colorSubresourceRange));

	const auto			renderPass			(makeRenderPass(vk, device, colorFormat));
	const auto			framebuffer			(vk::makeFramebuffer(vk, device, *renderPass, *colorImageView, renderSize.x(), renderSize.y()));
	...
	std::vector<vk::VkViewport> viewports	= { vk::VkViewport{0.0f, 0.0f, (float)renderSize.x(), (float)renderSize.y(),0.0f, 1.0f} };
	std::vector<vk::VkRect2D>	scissors	= { vk::VkRect2D{{0, 0}, {(deUint32)renderSize.x(), (deUint32)renderSize.y()} } };
	...
	const auto			pipeline			= vk::makeGraphicsPipeline(vk, device, *pipelineLayout, *vertexModule, DE_NULL, DE_NULL, DE_NULL, *fragmentModule, *renderPass, 0u, &vertexInputState);
	...
	const vk::VkDeviceSize colorOutputBufferSize = renderSize.x() * renderSize.y() * tcu::getPixelSize(vk::mapVkFormat(vk::VK_FORMAT_R8G8B8A8_UNORM));
	de::MovePtr<vk::BufferWithMemory> colorOutputBuffer = de::MovePtr<vk::BufferWithMemory>(new vk::BufferWithMemory(
		vk, device, allocator, vk::makeBufferCreateInfo(colorOutputBufferSize, vk::VK_BUFFER_USAGE_TRANSFER_DST_BIT), vk::MemoryRequirement::HostVisible));
	...
	vk::VkRenderPassBeginInfo renderPassBegin = {
		...
		*framebuffer,													// VkFramebuffer		framebuffer;
		{{0,0},{(deUint32)renderSize.x(), (deUint32)renderSize.y()}},	// VkRect2D				renderArea;
	...
	vk::VkImageMemoryBarrier imageMemoryBarrier = vk::makeImageMemoryBarrier(vk::VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, vk::VK_ACCESS_TRANSFER_READ_BIT, vk::VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *colorImage, subresourceRange);
	vk.cmdPipelineBarrier(*cmdBuffer, vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &imageMemoryBarrier);
	...
	vk::VkBufferImageCopy region = {
		...
		{(deUint32)renderSize.x(), (deUint32)renderSize.y(), 1u},	// VkExtent3D					imageExtent;
	};
	vk.cmdCopyImageToBuffer(*cmdBuffer, *colorImage, vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, **colorOutputBuffer, 1u, &region);
	...
	tcu::ConstPixelBufferAccess resultBuffer = tcu::ConstPixelBufferAccess(
		...
		renderSize.x(), renderSize.y(), 1, (const void*)colorOutputBuffer.get()->getAllocation().getHostPtr());
...
class StagesTestCase : public TestCase
{
	...
	virtual TestInstance*	createInstance			(Context& context) const { return new StagesTestInstance(context, m_descriptorType); }
...
void StagesTestCase::initPrograms (vk::SourceCollections& programCollection) const
{
	...
	{
		comp
			<< "layout(set = 0, binding = 0) uniform readBuffer{\n"
			<< "    vec4 readValues;\n"
			<< "};\n";
	}
	...
	{
		comp
			<< "    writeValues[gl_GlobalInvocationID.x] = texture(readImage, vec2(0.0f))[gl_GlobalInvocationID.x] * 4.0f;\n";
	}
	comp
		<< "}\n"
		;
	...
	vert
		...
		<< "\n"
		<< "void main (void) {\n"
		<< "    gl_Position = vec4(float(gl_VertexIndex & 1) * 2.0f - 1.0f, float((gl_VertexIndex >> 1) & 1) * 2.0f - 1.0f, 0.0f, 1.0f);\n"
		<< "}\n"
		;
	...
	{
		frag
			<< "    color = vec4(readValues[0] / 4.0f, readValues[1] / 4.0f, readValues[2] / 4.0f, readValues[3] / 4.0f);\n";
	}
	else
	{
		frag
...
tcu::TestCaseGroup* createStagesTests (tcu::TestContext& testCtx)
{
	...
	de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(testCtx, "stages", "Update stages from different pipeline bind points with the same call"));
	...
	} descriptorTypeTests[] =
	{
		...
		{ vk::VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,	"combined_image_sampler",	},
	};
