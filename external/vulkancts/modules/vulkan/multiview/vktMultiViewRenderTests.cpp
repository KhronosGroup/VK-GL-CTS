...
vk::Move<vk::VkRenderPass> makeRenderPass(const DeviceInterface &vk, const VkDevice device, const VkFormat colorFormat,
                                          const vector<uint32_t> &viewMasks, RenderPassType renderPassType,
                                                         ...
                                                         RenderPassType renderPassType)
{
    ...
    case RENDERPASS_TYPE_LEGACY:
        return MultiView::makeRenderPassWithAttachments<AttachmentDescription1, AttachmentReference1,
            ...
            vk, device, colorFormat, viewMasks, renderPassType == RENDERPASS_TYPE_RENDERPASS2);
    case RENDERPASS_TYPE_RENDERPASS2:
...
vk::Move<vk::VkRenderPass> makeRenderPassWithDepth(const DeviceInterface &vk, const VkDevice device,
                                                   const VkFormat colorFormat, const vector<uint32_t> &viewMasks,
                                                   const VkFormat dsFormat, RenderPassType renderPassType)
{
    switch (renderPassType)
    {
    case RENDERPASS_TYPE_LEGACY:
...
void cmdBeginRenderPass(DeviceInterface &vkd, VkCommandBuffer cmdBuffer, const VkRenderPassBeginInfo *pRenderPassBegin,
                        const VkSubpassContents contents, RenderPassType renderPassType)
{
    switch (renderPassType)
    {
    case RENDERPASS_TYPE_LEGACY:
...
protected:
    ...
    const tcu::Vec4 getQuarterRefColor(const uint32_t quarterNdx, const int colorNdx, const int layerNdx,
                                       const bool background = true, const uint32_t subpassNdx = 0u);
    ...
    void fillQuarter(const tcu::PixelBufferAccess &pixelBuffer, const tcu::Vec4 &color, const int layerNdx,
                     const uint32_t quarter, const uint32_t subpassNdx);
...
MultiViewRenderTestInstance::MultiViewRenderTestInstance(Context &context, const TestParameters &parameters)
    ...
    , m_extensionSupported((parameters.renderPassType == RENDERPASS_TYPE_RENDERPASS2) &&
...
tcu::TestStatus MultiViewRenderTestInstance::iterate(void)
{
    ...
    Unique<VkRenderPass> renderPass(makeRenderPass(*m_device, *m_logicalDevice, m_parameters.colorFormat,
                                                   m_parameters.viewMasks, m_parameters.renderPassType));

    Unique<VkFramebuffer> frameBuffer(makeFramebuffer(*m_device, *m_logicalDevice, *renderPass,
                                                      m_colorAttachment->getImageView(), m_parameters.extent.width,
...
void MultiViewRenderTestInstance::createMultiViewDevices(void)
{
...

    {
        ...
        m_device        = MovePtr<DeviceDriver>(
            new DeviceDriver(m_context.getPlatformInterface(), m_context.getInstance(), *m_logicalDevice));
        m_allocator = MovePtr<Allocator>(new SimpleAllocator(
            *m_device, *m_logicalDevice, getPhysicalDeviceMemoryProperties(instance, physicalDevice)));
...
Move<VkPipeline> MultiViewRenderTestInstance::makeGraphicsPipeline(
    ...
    const VkVertexInputRate vertexInputRate, const bool useDepthTest, const bool useStencilTest)
{
...
void MultiViewRenderTestInstance::setPoint(const tcu::PixelBufferAccess &pixelBuffer, const tcu::Vec4 &pointColor,
                                           const int pointSize, const int layerNdx, const uint32_t quarter)
{
...
void MultiViewRenderTestInstance::fillQuarter(const tcu::PixelBufferAccess &pixelBuffer, const tcu::Vec4 &color,
                                              const int layerNdx, const uint32_t quarter, const uint32_t subpassNdx)
{
        ...
        while (mask > 0u)
        {
            ...
            if (mask & 1u)
            {
                ...
                if (TEST_TYPE_POINT_SIZE == m_parameters.viewIndex)
                {
                    ...
                    const uint32_t dummyQuarterNdx    = 0u;
                    const int pointSize =
                    ...
                    if (subpassCount == 1)
                        for (uint32_t drawNdx = 0u; drawNdx < m_squareCount; ++drawNdx)
                            setPoint(referenceFrame->getLevel(0),
                                     getQuarterRefColor(dummyQuarterNdx, vertexPerPrimitive * drawNdx, layerNdx, false),
                                     pointSize, layerNdx, drawNdx);
                    else
                        setPoint(referenceFrame->getLevel(0),
                                 getQuarterRefColor(dummyQuarterNdx, vertexPerPrimitive * subpassQuarterNdx, layerNdx,
                                                    false),
                ...
                if (TEST_TYPE_MULTISAMPLE == m_parameters.viewIndex)
                {
                    ...
                    if (subpassCount == 1)
                        ...
                        fillTriangle(referenceFrame->getLevel(0),
                                     getQuarterRefColor(dummyQuarterNdx, vertexPerPrimitive * subpassQuarterNdx,
                                                        layerNdx, false),
...
tcu::TestStatus MultiViewAttachmentsTestInstance::iterate(void)
{
    ...
    // FrameBuffer & renderPass
    Unique<VkRenderPass> renderPass(makeRenderPassWithAttachments(*m_device, *m_logicalDevice, m_parameters.colorFormat,
                                                                  m_parameters.viewMasks, m_parameters.renderPassType));
                                                       ...
                                                       VkFramebuffer frameBuffer, vector<PipelineSp> &pipelines)
{
    ...
    for (uint32_t subpassNdx = 0u; subpassNdx < subpassCount; subpassNdx++)
    {
        ...
        beginSecondaryCommandBuffer(*m_device, cmdBufferSecondary.back().get()->get(), renderPass, subpassNdx,
                                    frameBuffer);
                                        ...
                                        vector<PipelineSp> &pipelines)
{
    ...
    m_device->getQueryPoolResults(*m_logicalDevice, *occlusionQueryPool, 0u, queryCountersNumber,
                                  sizeof(uint64_t) * queryCountersNumber, (void *)&m_occlusionValues[0],
    ...
    m_device->getQueryPoolResults(*m_logicalDevice, *timestampStartQueryPool, 0u, queryCountersNumber,
                                  sizeof(uint64_t) * queryCountersNumber, (void *)&m_timestampStartValues[0],
                                  sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
    ...
    m_device->getQueryPoolResults(*m_logicalDevice, *timestampEndQueryPool, 0u, queryCountersNumber,
                                  sizeof(uint64_t) * queryCountersNumber, (void *)&m_timestampEndValues[0],
                                  sizeof(uint64_t), VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
...
{
    const uint32_t subpassCount = static_cast<uint32_t>(m_parameters.viewMasks.size());
    Unique<VkRenderPass> renderPass(makeRenderPassWithDepth(*m_device, *m_logicalDevice, m_parameters.colorFormat,
                                                            m_parameters.viewMasks, m_dsFormat,
                                                            m_parameters.renderPassType));
...
void MultiViewDepthStencilTestInstance::beforeDraw(void)
{
    ...
    imageBarrier(*m_device, *m_cmdBuffer, m_dsAttachment->getImage(), subresourceRange, VK_IMAGE_LAYOUT_UNDEFINED,
                 VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0, 0, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
...
{
public:
    MultiViewRenderTestsCase(tcu::TestContext &context, const char *name, const char *description,
                             const TestParameters &parameters)
...
void multiViewRenderCreateTests(tcu::TestCaseGroup *group)
{
    ...
    for (int renderPassTypeNdx = 0; renderPassTypeNdx < 2; ++renderPassTypeNdx)
    {
        ...
        MovePtr<tcu::TestCaseGroup> groupRenderPass2(
            (renderPassTypeNdx == 0) ?
                DE_NULL :
                new tcu::TestCaseGroup(group->getTestContext(), "renderpass2", "RenderPass2 index tests"));
        ...
        MovePtr<tcu::TestCaseGroup> groupViewIndex(
            new tcu::TestCaseGroup(testCtx, "index", "ViewIndex rendering tests."));

        for (int testTypeNdx = TEST_TYPE_VIEW_MASK; testTypeNdx < TEST_TYPE_LAST; ++testTypeNdx)
        {
            MovePtr<tcu::TestCaseGroup> groupShader(
                new tcu::TestCaseGroup(testCtx, shaderName[testTypeNdx].c_str(), ""));
            ...
            const VkFormat colorFormat =
                (testType == TEST_TYPE_MULTISAMPLE) ? VK_FORMAT_R32G32B32A32_SFLOAT : VK_FORMAT_R8G8B8A8_UNORM;
            ...
            else
            {
                ...
                // maxMultiviewViewCount case
                {
                    ...
                    const vector<uint32_t> dummyMasks;
                    const TestParameters parameters = {incompleteExtent3D, dummyMasks,  testType,
                                                       sampleCountFlags,   colorFormat, renderPassType};

                    groupShader->addChild(
