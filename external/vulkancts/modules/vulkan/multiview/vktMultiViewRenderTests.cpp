...
TestParameters MultiViewRenderTestInstance::fillMissingParameters(const TestParameters &parameters)
{
    ...
    else
    {
        ...
        const VkPhysicalDevice physicalDevice =
            chooseDevice(instanceDriver, m_instanceWrapper->instance, m_context.getTestContext().getCommandLine());
...
void MultiViewRenderTestInstance::createMultiViewDevices(void)
{
...

    {
        ...
        m_logicalDevice = createCustomDevice(m_context.getTestContext().getCommandLine().isValidationEnabled(),
                                             ...
                                             instanceDriver, physicalDevice, &deviceInfo);
#ifndef CTS_USES_VULKANSC
        m_device = de::MovePtr<DeviceDriver>(
            new DeviceDriver(m_context.getPlatformInterface(), m_instanceWrapper->instance, *m_logicalDevice));
#else
        m_device = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(m_context.getPlatformInterface(), m_instanceWrapper->instance, *m_logicalDevice,
    ...
    : MultiViewRenderTestInstance(context, parameters)
{
    ...
    const VkPhysicalDevice physDevice =
        chooseDevice(vki, m_instanceWrapper->instance, context.getTestContext().getCommandLine());
    ...
    , m_occlusionObjectsOffset(0)
{
    ...
    const std::vector<VkQueueFamilyProperties> queueProperties =
        vk::getPhysicalDeviceQueueFamilyProperties(m_instanceWrapper->instance.getDriver(), physicalDevice);
    ...
    , m_stencilTest(m_parameters.viewIndex == TEST_TYPE_STENCIL)
{
    ...
    for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(formats); ++ndx)
    {
        ...
        const VkFormatProperties formatProperties =
            getPhysicalDeviceFormatProperties(m_instanceWrapper->instance.getDriver(), physicalDevice, format);
