...
public:
    BlockBasicTypeCase(tcu::TestContext &testCtx, const std::string &name, const VarType &type, uint32_t layoutFlags,
                       int numInstances, MatrixLoadFlags matrixLoadFlag)
...
{
public:
    BlockSingleStructCase(tcu::TestContext &testCtx, const std::string &name, uint32_t layoutFlags,
...
{
public:
    Block2LevelStructArrayCase(tcu::TestContext &testCtx, const std::string &name, uint32_t layoutFlags,
                               BufferMode bufferMode, int numInstances, MatrixLoadFlags matrixLoadFlag)
        : UniformBlockCase(testCtx, name, bufferMode, matrixLoadFlag)
...
void createRandomCaseGroup(tcu::TestCaseGroup *parentGroup, tcu::TestContext &testCtx, const char *groupName,
                           UniformBlockCase::BufferMode bufferMode, uint32_t features, int numCases, uint32_t baseSeed)
{
...
void UniformBlockTests::init(void)
{
        ...
        for (int modeNdx = 0; modeNdx < DE_LENGTH_OF_ARRAY(bufferModes); modeNdx++)
        {
            ...
            for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
            {
                for (int isArray = 0; isArray < 2; isArray++)
                {
                    ...
                    modeGroup->addChild(
                        new Block2LevelStructArrayCase(m_testCtx, (baseName + "_vertex"), baseFlags | DECLARE_VERTEX,
    ...
    // ubo.random
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            createRandomCaseGroup(group, m_testCtx, "all_per_block_buffers", UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allFeatures, 50, 200);
            // All random features, shared buffer
            createRandomCaseGroup(group, m_testCtx, "all_shared_buffer", UniformBlockCase::BUFFERMODE_SINGLE,
                                  use8BitStorage | use16BitStorage | allFeatures, 50, 250);

            // All random features, out of order member offsets
            createRandomCaseGroup(group, m_testCtx, "all_out_of_order_offsets", UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allFeatures | FEATURE_OUT_OF_ORDER_OFFSETS, 50,
                                  300);
            // VK_EXT_scalar_block_layout
            createRandomCaseGroup(group, m_testCtx, "scalar", UniformBlockCase::BUFFERMODE_SINGLE,
                                  use8BitStorage | use16BitStorage | allScalar, 100, deInt32Hash(313));
            // VK_EXT_descriptor_indexing
            createRandomCaseGroup(group, m_testCtx, "descriptor_indexing", UniformBlockCase::BUFFERMODE_SINGLE,
