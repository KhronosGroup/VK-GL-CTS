...
{
public:
    BlockBasicTypeCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       const VarType &type, uint32_t layoutFlags, int numInstances, MatrixLoadFlags matrixLoadFlag)
                               ...
                               const VarType &type, uint32_t layoutFlags, int numInstances = 0)
{
    ...
    typeGroup->addChild(new BlockBasicTypeCase(testCtx, "vertex", "", type, layoutFlags | DECLARE_VERTEX, numInstances,
                                               LOAD_FULL_MATRIX));
    typeGroup->addChild(new BlockBasicTypeCase(testCtx, "fragment", "", type, layoutFlags | DECLARE_FRAGMENT,
                                               numInstances, LOAD_FULL_MATRIX));
    typeGroup->addChild(new BlockBasicTypeCase(
        testCtx, "both", "", type, layoutFlags | DECLARE_VERTEX | DECLARE_FRAGMENT, numInstances, LOAD_FULL_MATRIX));
    ...
    typeGroup->addChild(new BlockBasicTypeCase(testCtx, "fragment_comp_access", "", type,
                                               layoutFlags | DECLARE_FRAGMENT, numInstances, LOAD_MATRIX_COMPONENTS));
...
{
public:
    BlockSingleStructCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                          uint32_t layoutFlags, BufferMode bufferMode, int numInstances, MatrixLoadFlags matrixLoadFlag)
...
{
public:
    BlockSingleStructArrayCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
{
public:
    BlockSingleNestedStructCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                MatrixLoadFlags matrixLoadFlag)
        : UniformBlockCase(testCtx, name, description, bufferMode, matrixLoadFlag)
    {
...
{
public:
    BlockSingleNestedStructArrayCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                     MatrixLoadFlags matrixLoadFlag)
        : UniformBlockCase(testCtx, name, description, bufferMode, matrixLoadFlag)
    {
...
{
public:
    BlockMultiBasicTypesCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
{
public:
    BlockMultiNestedStructCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                               uint32_t flagsA, uint32_t flagsB, BufferMode bufferMode, int numInstances,
                               MatrixLoadFlags matrixLoadFlag)
        : UniformBlockCase(testCtx, name, description, bufferMode, matrixLoadFlag)
    {
...
{
public:
    Block2LevelStructArrayCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                               uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                               MatrixLoadFlags matrixLoadFlag)
        : UniformBlockCase(testCtx, name, description, bufferMode, matrixLoadFlag)
...
{
public:
    LinkByBindingCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                           ...
                           int numCases, uint32_t baseSeed)
{
    ...
    for (int ndx = 0; ndx < numCases; ndx++)
        group->addChild(
            new RandomUniformBlockCase(testCtx, de::toString(ndx), "", bufferMode, features, (uint32_t)ndx + baseSeed));
}
...
UniformBlockTests::UniformBlockTests(tcu::TestContext &testCtx) : TestCaseGroup(testCtx, "ubo", "Uniform Block tests")
{
}
...
void UniformBlockTests::init(void)
{
    ...
    static const struct
    {
        const char *name;
        uint32_t flags;
    } layoutFlags[] = {{"std140", LAYOUT_STD140}, {"std430", LAYOUT_STD430}, {"scalar", LAYOUT_SCALAR}};
    ...
    // ubo.2_level_array
    {
        ...
        for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
        {
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                createBlockBasicTypeCases(*layoutGroup, m_testCtx, typeName, parentType,
                ...
                if (glu::isDataTypeMatrix(type))
                {
                    for (int matFlagNdx = 0; matFlagNdx < DE_LENGTH_OF_ARRAY(matrixFlags); matFlagNdx++)
                        createBlockBasicTypeCases(
                            *layoutGroup, m_testCtx, (std::string(matrixFlags[matFlagNdx].name) + "_" + typeName),
            ...
            for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
            {
                for (int isArray = 0; isArray < 2; isArray++)
                {
                    ...
                    modeGroup->addChild(new Block2LevelStructArrayCase(
                        m_testCtx, (baseName + "_vertex"), "", baseFlags | DECLARE_VERTEX, bufferModes[modeNdx].mode,
                        isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new Block2LevelStructArrayCase(
                        m_testCtx, (baseName + "_fragment"), "", baseFlags | DECLARE_FRAGMENT,
                        bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new Block2LevelStructArrayCase(
                        m_testCtx, (baseName + "_both"), "", baseFlags | DECLARE_VERTEX | DECLARE_FRAGMENT,
                        bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new Block2LevelStructArrayCase(
                        m_testCtx, (baseName + "_vertex_comp_access"), "", baseFlags | DECLARE_VERTEX,
        ...
        for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
        {
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                if (!dataTypeSupportsPrecisionModifier(type))
                    createBlockBasicTypeCases(*layoutGroup, m_testCtx, typeName, VarType(type, 0),
                                              layoutFlags[layoutFlagNdx].flags);
            }
            ...
            for (int precNdx = 0; precNdx < DE_LENGTH_OF_ARRAY(precisionFlags); precNdx++)
            {
                ...
                for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
                {
                    ...
                    if (glu::isDataTypeMatrix(type))
                    {
                        for (int matFlagNdx = 0; matFlagNdx < DE_LENGTH_OF_ARRAY(matrixFlags); matFlagNdx++)
                        {
                            createBlockBasicTypeCases(*precGroup, m_testCtx,
                                                      matrixFlags[matFlagNdx].name + "_" + typeName,
                                                      VarType(type, precisionFlags[precNdx].flags),
    ...
    // ubo.single_basic_array
    {
        ...
        for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
        {
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                createBlockBasicTypeCases(
                    *layoutGroup, m_testCtx, typeName,
                ...
                {
                    for (int matFlagNdx = 0; matFlagNdx < DE_LENGTH_OF_ARRAY(matrixFlags); matFlagNdx++)
                        createBlockBasicTypeCases(*layoutGroup, m_testCtx,
                                                  matrixFlags[matFlagNdx].name + "_" + typeName,
        ...
        for (int modeNdx = 0; modeNdx < DE_LENGTH_OF_ARRAY(bufferModes); modeNdx++)
        {
            ...
            for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
            {
                for (int isArray = 0; isArray < 2; isArray++)
                {
                    ...
                    modeGroup->addChild(new BlockSingleStructCase(m_testCtx, baseName + "_vertex", "",
                    ...
                    modeGroup->addChild(new BlockSingleStructCase(
                        m_testCtx, baseName + "_both", "", baseFlags | DECLARE_VERTEX | DECLARE_FRAGMENT,
                        bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new BlockSingleStructCase(m_testCtx, baseName + "_vertex_comp_access", "",
                                                                  baseFlags | DECLARE_VERTEX, bufferModes[modeNdx].mode,
        ...
        for (int modeNdx = 0; modeNdx < DE_LENGTH_OF_ARRAY(bufferModes); modeNdx++)
        {
            ...
            for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
            {
                for (int isArray = 0; isArray < 2; isArray++)
                {
                    ...
                    modeGroup->addChild(
                        new BlockSingleStructArrayCase(m_testCtx, baseName + "_vertex", "", baseFlags | DECLARE_VERTEX,
                                                       bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
        ...
        for (int modeNdx = 0; modeNdx < DE_LENGTH_OF_ARRAY(bufferModes); modeNdx++)
        {
            ...
            for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
            {
                for (int isArray = 0; isArray < 2; isArray++)
                {
                    ...
                    modeGroup->addChild(
                        new BlockSingleNestedStructCase(m_testCtx, baseName + "_vertex", "", baseFlags | DECLARE_VERTEX,
                                                        bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
        ...
        for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
        {
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                if (glu::isDataTypeMatrix(type))
                {
                    for (int matFlagNdx = 0; matFlagNdx < DE_LENGTH_OF_ARRAY(matrixFlags); matFlagNdx++)
                        createBlockBasicTypeCases(
                            *layoutGroup, m_testCtx, matrixFlags[matFlagNdx].name + "_" + typeName,
            ...
            for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
            {
                for (int isArray = 0; isArray < 2; isArray++)
                {
                    ...
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        m_testCtx, baseName + "_vertex", "", baseFlags | DECLARE_VERTEX, baseFlags | DECLARE_VERTEX,
                        bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        m_testCtx, baseName + "_fragment", "", baseFlags | DECLARE_FRAGMENT,
                        baseFlags | DECLARE_FRAGMENT, bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        m_testCtx, baseName + "_both", "", baseFlags | DECLARE_VERTEX | DECLARE_FRAGMENT,
                        baseFlags | DECLARE_VERTEX | DECLARE_FRAGMENT, bufferModes[modeNdx].mode, isArray ? 3 : 0,
                        LOAD_FULL_MATRIX));
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        m_testCtx, baseName + "_mixed", "", baseFlags | DECLARE_VERTEX, baseFlags | DECLARE_FRAGMENT,
                        bufferModes[modeNdx].mode, isArray ? 3 : 0, LOAD_FULL_MATRIX));
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        m_testCtx, baseName + "_vertex_comp_access", "", baseFlags | DECLARE_VERTEX,
                    ...
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        ...
                        LOAD_MATRIX_COMPONENTS));
                    modeGroup->addChild(new BlockMultiBasicTypesCase(
                        m_testCtx, baseName + "_mixed_comp_access", "", baseFlags | DECLARE_VERTEX,
                        baseFlags | DECLARE_FRAGMENT, bufferModes[modeNdx].mode, isArray ? 3 : 0,
                        LOAD_MATRIX_COMPONENTS));
                }
            }
    ...
    // .link_by_binding
    {
        tcu::TestCaseGroup *linkByBindingGroup =
            new tcu::TestCaseGroup(m_testCtx, "link_by_binding", "Blocks with same name but different binding");
        addChild(linkByBindingGroup);

        linkByBindingGroup->addChild(
            new LinkByBindingCase(m_testCtx, "single_buf_single_instance", "", UniformBlockCase::BUFFERMODE_SINGLE, 0));
        linkByBindingGroup->addChild(
            new LinkByBindingCase(m_testCtx, "single_buf_instance_array", "", UniformBlockCase::BUFFERMODE_SINGLE, 2));
        linkByBindingGroup->addChild(new LinkByBindingCase(m_testCtx, "per_block_buf_single_instance", "",
                                                           UniformBlockCase::BUFFERMODE_PER_BLOCK, 0));
        linkByBindingGroup->addChild(new LinkByBindingCase(m_testCtx, "per_block_buf_instance_array", "",
                                                           UniformBlockCase::BUFFERMODE_PER_BLOCK, 2));
    }
    ...
    // ubo.random
    {
        ...
        const uint32_t allScalar =
            ~allLayouts & ~FEATURE_16BIT_STORAGE & ~FEATURE_8BIT_STORAGE & ~FEATURE_DESCRIPTOR_INDEXING;
        const uint32_t descriptorIndexing = FEATURE_STD140_LAYOUT | FEATURE_STD430_LAYOUT | FEATURE_SCALAR_LAYOUT |
                                            FEATURE_DESCRIPTOR_INDEXING | allShaders | allBasicTypes | unused |
                                            matFlags;
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            // Basic types.
            createRandomCaseGroup(group, m_testCtx, "scalar_types", "Scalar types only, per-block buffers",
                                  UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allShaders | allLayouts | unused, 25, 0);
            createRandomCaseGroup(group, m_testCtx, "vector_types", "Scalar and vector types only, per-block buffers",
                                  UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allShaders | allLayouts | unused | FEATURE_VECTORS,
                                  25, 25);
            createRandomCaseGroup(
                group, m_testCtx, "basic_types", "All basic types, per-block buffers",
                UniformBlockCase::BUFFERMODE_PER_BLOCK,
                use8BitStorage | use16BitStorage | allShaders | allLayouts | unused | allBasicTypes | matFlags, 25, 50);
            createRandomCaseGroup(group, m_testCtx, "basic_arrays", "Arrays, per-block buffers",
                                  UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allShaders | allLayouts | unused | allBasicTypes |
                                      matFlags | FEATURE_ARRAYS,
                                  25, 50);

            createRandomCaseGroup(group, m_testCtx, "basic_instance_arrays", "Basic instance arrays, per-block buffers",
                                  UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allShaders | allLayouts | unused | allBasicTypes |
                                      matFlags | FEATURE_INSTANCE_ARRAYS,
                                  25, 75);
            createRandomCaseGroup(group, m_testCtx, "nested_structs", "Nested structs, per-block buffers",
                                  UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allShaders | allLayouts | unused | allBasicTypes |
                                      matFlags | FEATURE_STRUCTS,
                                  25, 100);
            createRandomCaseGroup(group, m_testCtx, "nested_structs_arrays",
                                  "Nested structs, arrays, per-block buffers", UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allShaders | allLayouts | unused | allBasicTypes |
            ...
            createRandomCaseGroup(group, m_testCtx, "all_per_block_buffers", "All random features, per-block buffers",
                                  UniformBlockCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allFeatures, 50, 200);
            createRandomCaseGroup(group, m_testCtx, "all_shared_buffer", "All random features, shared buffer",
                                  UniformBlockCase::BUFFERMODE_SINGLE, use8BitStorage | use16BitStorage | allFeatures,
                                  50, 250);

            createRandomCaseGroup(
                group, m_testCtx, "all_out_of_order_offsets", "All random features, out of order member offsets",
                UniformBlockCase::BUFFERMODE_PER_BLOCK,
                use8BitStorage | use16BitStorage | allFeatures | FEATURE_OUT_OF_ORDER_OFFSETS, 50, 300);
            createRandomCaseGroup(group, m_testCtx, "scalar", "VK_EXT_scalar_block_layout",
                                  UniformBlockCase::BUFFERMODE_SINGLE, use8BitStorage | use16BitStorage | allScalar,
                                  100, deInt32Hash(313));
            createRandomCaseGroup(group, m_testCtx, "descriptor_indexing", "VK_EXT_descriptor_indexing",
                                  UniformBlockCase::BUFFERMODE_SINGLE,
