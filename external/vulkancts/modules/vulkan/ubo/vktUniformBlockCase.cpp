            ...
            for (int compNdx = 0; compNdx < vecSize; compNdx++)
            {
                ...
                case glu::TYPE_UINT16:
                    *((uint16_t *)compPtr) = (uint16_t)rnd.getInt(0, 9);
                    break;
                case glu::TYPE_FLOAT16:
                    *((deFloat16 *)compPtr) = deFloat32To16((float)rnd.getInt(-9, 9));
                    break;
...
void generateValueSrc(std::ostringstream &src, const UniformLayoutEntry &entry, const void *basePtr, int elementNdx)
{
    ...
    else
    {
        for (int scalarNdx = 0; scalarNdx < scalarSize; scalarNdx++)
        {
            ...
            {
            case glu::TYPE_FLOAT16:
                src << de::floatToString(deFloat16To32(*((const deFloat16 *)compPtr)), 1);
                break;
            case glu::TYPE_FLOAT:
                src << de::floatToString(*((const float *)compPtr), 1);
                break;
...
UniformBlockCase::UniformBlockCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                   BufferMode bufferMode, MatrixLoadFlags matrixLoadFlag, bool shuffleUniformMembers)
...
TestInstance *UniformBlockCase::createInstance(Context &context) const
{
    ...
    if (!context.getDescriptorIndexingFeatures().shaderUniformBufferArrayNonUniformIndexing &&
        usesBlockLayout(LAYOUT_DESCRIPTOR_INDEXING))
