...
protected:
    ...
    virtual void checkAdditionalRasterizationFlags(
        VkPhysicalDeviceRasterizationOrderAttachmentAccessFeaturesARM &rasterizationAccess) const
    {
        // unused parameter
...
public:
    AttachmentAccessOrderColorTestCase(tcu::TestContext &context, const std::string &name,
                                       const std::string &description, bool explicitSync, bool overlapDraws,
                                       bool overlapPrimitives, bool overlapInstances, VkSampleCountFlagBits sampleCount,
                                       uint32_t inputAttachmentNum, bool integerFormat)
        : AttachmentAccessOrderTestCase(context, name, description, explicitSync, overlapDraws, overlapPrimitives,
                                        overlapInstances, sampleCount, inputAttachmentNum, integerFormat)
    {
    }
...
public:
    AttachmentAccessOrderDepthTestCase(tcu::TestContext &context, const std::string &name,
                                       ...
                                       bool overlapPrimitives, bool overlapInstances, VkSampleCountFlagBits sampleCount)
        : AttachmentAccessOrderTestCase(context, name, description, explicitSync, overlapDraws, overlapPrimitives,
                                        overlapInstances, sampleCount, 1, false)
    {
    }
    virtual uint32_t getInputAttachmentNum() const
    {
...
public:
    AttachmentAccessOrderStencilTestCase(tcu::TestContext &context, const std::string &name,
        ...
        : AttachmentAccessOrderTestCase(context, name, description, explicitSync, overlapDraws, overlapPrimitives,
                                        overlapInstances, sampleCount, 1, true)
    {
    }
    virtual uint32_t getInputAttachmentNum() const
    {
...
AttachmentAccessOrderTestCase::AttachmentAccessOrderTestCase(tcu::TestContext &context, const std::string &name,
                                                             const std::string &description, bool explicitSync,
                                                             bool overlapDraws, bool overlapPrimitives,
                                                             bool overlapInstances, VkSampleCountFlagBits sampleCount,
                                                             uint32_t inputAttachmentNum, bool integerFormat)
                                                            ...
                                                            const std::map<std::string, std::string> &params) const
{
    ...
    fragShader
        ...
        << "    uint index;\n"
        << "    uint pre_fetch_loop = uint(gl_FragCoord.x) * uint(gl_FragCoord.y) * (drawNum * primitiveNum - drawCur "
           "* primitiveNum - primitiveCur);\n"
        << "    uint post_fetch_loop = uint(gl_FragCoord.x) + uint(gl_FragCoord.y) + (drawNum * instanceNum - drawCur "
           "* instanceNum - instanceCur);\n"
...
Move<VkRenderPass> AttachmentAccessOrderTestInstance::createRenderPass(VkFormat attFormat)
{
    ...
    if (m_testCase->m_explicitSync)
    {
        ...
        if (m_testCase->hasDepthStencil())
        {
            addDependency(dependencies, 0, 0, VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
                                                           ...
                                                           VkPipelineStageFlags dstStageMask, VkImageAspectFlags aspect)
{
    ...
    m_vk.cmdPipelineBarrier(cmdBuffer, srcStageMask, dstStageMask, (VkDependencyFlags)0, 0,
    ...
    for (uint32_t i = 0; i < numDraws; i++)
    {
             ...
             m_testCase->m_explicitSync && i != 0 && j < m_subpasses[0].getInputAttachmentNum(); j++)
        {
            addPipelineBarrier(*m_cmdBuffer, *m_subpasses[0].m_inputAtt[j], VK_IMAGE_LAYOUT_GENERAL,
                               ...
                               VK_ACCESS_INPUT_ATTACHMENT_READ_BIT, VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
                               VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                               VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT);
        }
        ...
        for (uint32_t k = 0; k < leafTestCreateParamsNum; k++)
        {
            if (depth)
            {
                subgr->addChild(new AttachmentAccessOrderDepthTestCase(
                    testCtx, leafTestCreateParams[k].name, leafTestCreateParams[k].description,
                    leafTestCreateParams[k].explicitSync, leafTestCreateParams[k].overlapDraws,
                    leafTestCreateParams[k].overlapPrimitives, leafTestCreateParams[k].overlapInstances,
                    sampleCountValues[i]));
            }
            else if (stencil)
            {
