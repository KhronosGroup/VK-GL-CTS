...
static bool verifyXfbBuffer(const tcu::Vec4 *const xfbResults, uint32_t count, VkPrimitiveTopology topology,
                            ProvokingVertexMode mode, std::string &errorMessage)
{
    ...
    for (uint32_t ndx = start; ndx < count; ndx += primitiveSize)
    {
        ...
        {
            errorMessage = "Vertex " + de::toString(ndx) + ": Expected red, got " + de::toString(xfbResults[ndx]);
            return false;
        }
    }
...
{
public:
    ProvokingVertexTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                            const Params params);
...
tcu::TestStatus ProvokingVertexTestInstance::iterate(void)
{
    ...
    // Verify result
    {
        ...
        if (m_params.transformFeedback)
        {
            ...
            if (m_params.provokingVertexMode != PROVOKING_VERTEX_PER_PIPELINE)
            {
                if (!verifyXfbBuffer(xfbResults, count, m_params.primitiveTopology, m_params.provokingVertexMode,
                                     errorMessage))
            ...
            else
            {
                ...
                if (!verifyXfbBuffer(xfbResults, halfCount, m_params.primitiveTopology, PROVOKING_VERTEX_FIRST,
                                     errorMessage))
                    return tcu::TestStatus::fail(errorMessage);

                if (!verifyXfbBuffer(&xfbResults[halfCount], halfCount, m_params.primitiveTopology,
...
{
    return createTestGroup(testCtx, "provoking_vertex", "Tests for provoking vertex", createTests);
}
