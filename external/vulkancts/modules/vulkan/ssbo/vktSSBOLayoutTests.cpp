...
{
public:
    RandomSSBOLayoutCase(tcu::TestContext &testCtx, const char *name, const char *description, BufferMode bufferMode,
...
RandomSSBOLayoutCase::RandomSSBOLayoutCase(tcu::TestContext &testCtx, const char *name, const char *description,
...
{
public:
    BlockBasicTypeCase(tcu::TestContext &testCtx, const char *name, const char *description, const VarType &type,
                       uint32_t layoutFlags, int numInstances, MatrixLoadFlags matrixLoadFlag,
                       MatrixStoreFlags matrixStoreFlag, bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, BUFFERMODE_PER_BLOCK, matrixLoadFlag, matrixStoreFlag,
                         usePhysStorageBuffer)
    {
...
{
public:
    BlockBasicUnsizedArrayCase(tcu::TestContext &testCtx, const char *name, const char *description,
                               const VarType &elementType, int arraySize, uint32_t layoutFlags,
...
static void createRandomCaseGroup(tcu::TestCaseGroup *parentGroup, tcu::TestContext &testCtx, const char *groupName,
                                  ...
                                  int numCases, uint32_t baseSeed, bool usePhysStorageBuffer)
{
...
{
public:
    BlockSingleStructCase(tcu::TestContext &testCtx, const char *name, const char *description, uint32_t layoutFlags,
...
{
public:
    BlockSingleStructArrayCase(tcu::TestContext &testCtx, const char *name, const char *description,
...
{
public:
    BlockSingleNestedStructCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                MatrixLoadFlags matrixLoadFlag, MatrixStoreFlags matrixStoreFlag,
                                bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
...
{
public:
    BlockSingleNestedStructArrayCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                     uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                     MatrixLoadFlags matrixLoadFlag, MatrixStoreFlags matrixStoreFlag,
                                     bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
...
{
public:
    BlockUnsizedStructArrayCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                MatrixLoadFlags matrixLoadFlag, MatrixStoreFlags matrixStoreFlag,
                                bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
...
{
public:
    Block2LevelUnsizedStructArrayCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                      uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                      MatrixLoadFlags matrixLoadFlag, MatrixStoreFlags matrixStoreFlag,
                                      bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
...
{
public:
    BlockUnsizedNestedStructArrayCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                      uint32_t layoutFlags, BufferMode bufferMode, int numInstances,
                                      MatrixLoadFlags matrixLoadFlag, MatrixStoreFlags matrixStoreFlag,
                                      bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
...
{
public:
    BlockMultiBasicTypesCase(tcu::TestContext &testCtx, const char *name, const char *description, uint32_t flagsA,
                             uint32_t flagsB, BufferMode bufferMode, int numInstances, MatrixLoadFlags matrixLoadFlag,
                             MatrixStoreFlags matrixStoreFlag, bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
        , m_flagsA(flagsA)
        , m_flagsB(flagsB)
...
{
public:
    BlockMultiNestedStructCase(tcu::TestContext &testCtx, const char *name, const char *description, uint32_t flagsA,
                               uint32_t flagsB, BufferMode bufferMode, int numInstances, MatrixLoadFlags matrixLoadFlag,
                               MatrixStoreFlags matrixStoreFlag, bool usePhysStorageBuffer)
        : SSBOLayoutCase(testCtx, name, description, bufferMode, matrixLoadFlag, matrixStoreFlag, usePhysStorageBuffer)
        , m_flagsA(flagsA)
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                if (!glu::dataTypeSupportsPrecisionModifier(type))
                    layoutGroup->addChild(new BlockBasicTypeCase(
                        m_testCtx, typeName, "", VarType(type, glu::PRECISION_LAST), layoutFlags[layoutFlagNdx].flags,
                        0, LOAD_FULL_MATRIX, STORE_FULL_MATRIX, m_usePhysStorageBuffer));
                else
                {
                    for (int precNdx = 0; precNdx < glu::PRECISION_LAST; precNdx++)
                    {
                        ...
                        layoutGroup->addChild(new BlockBasicTypeCase(
                            m_testCtx, caseName.c_str(), "", VarType(type, precision), layoutFlags[layoutFlagNdx].flags,
                            0, LOAD_FULL_MATRIX, STORE_FULL_MATRIX, m_usePhysStorageBuffer));
                    }
                }
                            ...
                            for (const auto &loadType : matrixLoadTypes)
                                for (const auto &storeType : matrixStoreTypes)
                                    layoutGroup->addChild(new BlockBasicTypeCase(
                                        ...
                                        loadType.second, storeType.second, m_usePhysStorageBuffer));
                        }
                    }
                }
            }
                        ...
                        for (const auto &loadType : matrixLoadTypes)
                            for (const auto &storeType : matrixStoreTypes)
                                layoutGroup->addChild(new BlockBasicUnsizedArrayCase(
                                    ...
                                    storeType.second, m_usePhysStorageBuffer));
                    }
                }
            }
        }
        ...
        for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
        {
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                layoutGroup->addChild(new BlockBasicTypeCase(m_testCtx, typeName, "", fullType,
                                                             ...
                                                             STORE_FULL_MATRIX, m_usePhysStorageBuffer));
    ...
    // ssbo.3_level_unsized_array
    {
        ...
        for (int layoutFlagNdx = 0; layoutFlagNdx < DE_LENGTH_OF_ARRAY(layoutFlags); layoutFlagNdx++)
        {
            ...
            for (int basicTypeNdx = 0; basicTypeNdx < DE_LENGTH_OF_ARRAY(basicTypes); basicTypeNdx++)
            {
                ...
                layoutGroup->addChild(new BlockBasicUnsizedArrayCase(m_testCtx, typeName, "", childType1, parentSize,
                    ...
                    for (const auto &loadType : matrixLoadTypes)
                        for (const auto &storeType : matrixStoreTypes)
                            modeGroup->addChild(new BlockMultiBasicTypesCase(
                                m_testCtx, (baseName + loadType.first + storeType.first).c_str(), "", baseFlags,
                                baseFlags, bufferModes[modeNdx].mode, isArray ? 3 : 0, loadType.second,
                                storeType.second, m_usePhysStorageBuffer));
                }
            }
    ...
    // ssbo.random
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            createRandomCaseGroup(group, m_testCtx, "vector_types", "Scalar and vector types only, per-block buffers",
                                  ...
                                  m_usePhysStorageBuffer);
            createRandomCaseGroup(group, m_testCtx, "basic_types", "All basic types, per-block buffers",
                                  SSBOLayoutCase::BUFFERMODE_PER_BLOCK,
            ...
            createRandomCaseGroup(
                group, m_testCtx, "basic_arrays", "Arrays, per-block buffers", SSBOLayoutCase::BUFFERMODE_PER_BLOCK,
                use8BitStorage | use16BitStorage | allStdLayouts | unused | allBasicTypes | matFlags | FEATURE_ARRAYS,
                25, 50, m_usePhysStorageBuffer);
            createRandomCaseGroup(group, m_testCtx, "unsized_arrays", "Unsized arrays, per-block buffers",
                                  SSBOLayoutCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allStdLayouts | unused | allBasicTypes | matFlags |
                                      unsized | FEATURE_ARRAYS,
                                  25, 50, m_usePhysStorageBuffer);
            createRandomCaseGroup(group, m_testCtx, "arrays_of_arrays", "Arrays of arrays, per-block buffers",
                                  SSBOLayoutCase::BUFFERMODE_PER_BLOCK,
                                  use8BitStorage | use16BitStorage | allStdLayouts | unused | allBasicTypes | matFlags |
            ...
            createRandomCaseGroup(
                group, m_testCtx, "nested_structs_arrays_instance_arrays",
            ...
            createRandomCaseGroup(group, m_testCtx, "all_shared_buffer", "All random features, shared buffer",
                                  SSBOLayoutCase::BUFFERMODE_SINGLE, use8BitStorage | use16BitStorage | allButRelaxed,
                                  50, 250, m_usePhysStorageBuffer);

            createRandomCaseGroup(group, m_testCtx, "relaxed", "VK_KHR_relaxed_block_layout",
                                  SSBOLayoutCase::BUFFERMODE_SINGLE, use8BitStorage | use16BitStorage | allRelaxed, 100,
                                  deInt32Hash(313), m_usePhysStorageBuffer);
            createRandomCaseGroup(group, m_testCtx, "scalar", "VK_EXT_scalar_block_layout",
                                  SSBOLayoutCase::BUFFERMODE_SINGLE, use8BitStorage | use16BitStorage | allScalar, 100,
                                  deInt32Hash(313), m_usePhysStorageBuffer);
            createRandomCaseGroup(group, m_testCtx, "descriptor_indexing", "VK_EXT_descriptor_indexing",
                                  SSBOLayoutCase::BUFFERMODE_SINGLE,
...
{
    de::MovePtr<tcu::TestCaseGroup> ssboTestGroup(
        new tcu::TestCaseGroup(testCtx, "ssbo", "Shader Storage Buffer Object Tests"));

    ssboTestGroup->addChild(new SSBOLayoutTests(testCtx, false));
    addTestGroup(ssboTestGroup.get(), "unsized_array_length", "SSBO unsized array length tests",
                 createUnsizedArrayTests);

    de::MovePtr<tcu::TestCaseGroup> physGroup(
        new tcu::TestCaseGroup(testCtx, "phys", "Physical Storage Buffer Pointer Tests"));
