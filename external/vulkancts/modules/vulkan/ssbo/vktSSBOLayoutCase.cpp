...
const char *getCompareFuncForType(glu::DataType type)
{
    ...
    case glu::TYPE_FLOAT16_VEC2:
        return "bool compare_f16vec2  (highp vec2 a, highp vec2 b)    { return compare_float(a.x, "
               "b.x)&&compare_float(a.y, b.y); }\n";
    case glu::TYPE_FLOAT16_VEC3:
        return "bool compare_f16vec3  (highp vec3 a, highp vec3 b)    { return compare_float(a.x, "
               "b.x)&&compare_float(a.y, b.y)&&compare_float(a.z, b.z); }\n";
    case glu::TYPE_FLOAT16_VEC4:
        return "bool compare_f16vec4  (highp vec4 a, highp vec4 b)    { return compare_float(a.x, "
               "b.x)&&compare_float(a.y, b.y)&&compare_float(a.z, b.z)&&compare_float(a.w, b.w); }\n";
    case glu::TYPE_INT8:
                          ...
                          const void *valuePtr, const char *resultVar, const char *typeName, const string shaderName)
{
    ...
    for (int colNdex = 0; colNdex < numCols; colNdex++)
    {
        src << "\t" << resultVar << " = " << resultVar << " && compare_" << typeName << numRows << "(" << shaderName
            << "[" << colNdex << "], " << typeName << numRows << "(";
        for (int rowNdex = 0; rowNdex < numRows; rowNdex++)
        {
            const uint8_t *compPtr =
                (const uint8_t *)valuePtr + (isRowMajor ? (rowNdex * matrixStride + colNdex * compSize) :
                        ...
                        const BufferVar &bufVar, const glu::SubTypeAccess &accessPath, MatrixLoadFlags matrixLoadFlag)
{
    ...
    if (curType.isArrayType())
    {
        ...
        for (int elemNdx = 0; elemNdx < arraySize; elemNdx++)
            generateCompareSrc(src, resultVar, bufferLayout, block, instanceNdx, blockPtr, bufVar,
                               accessPath.element(elemNdx), LOAD_FULL_MATRIX);
    }
    else if (curType.isStructType())
    {
        ...
        for (int memberNdx = 0; memberNdx < numMembers; memberNdx++)
            generateCompareSrc(src, resultVar, bufferLayout, block, instanceNdx, blockPtr, bufVar,
                               accessPath.member(memberNdx), LOAD_FULL_MATRIX);
    }
    else
    {
        ...
        DE_ASSERT(varNdx >= 0);
        {
            ...
            else
            {
                ...
                src << "\t" << resultVar << " = " << resultVar << " && compare_" << typeName << "(" << castName << "("
                    << shaderName << "), ";
        ...
        for (int instanceNdx = 0; instanceNdx < numInstances; instanceNdx++)
        {
            ...
            for (BufferBlock::const_iterator varIter = block.begin(); varIter != block.end(); varIter++)
            {
                ...
                generateCompareSrc(src, resultVar, layout, block, instanceNdx, blockPtr, bufVar,
                                   glu::SubTypeAccess(bufVar.getType()), matrixLoadFlag);
            }
        }
    }
}
...
tcu::TestStatus SSBOLayoutCaseInstance::iterate(void)
{
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk::VK_PIPELINE_STAGE_HOST_BIT,
                          (vk::VkDependencyFlags)0, 0u, DE_NULL, static_cast<uint32_t>(barriers.size()), &barriers[0],
                          0u, DE_NULL);
...
SSBOLayoutCase::SSBOLayoutCase(tcu::TestContext &testCtx, const char *name, const char *description,
                               BufferMode bufferMode, MatrixLoadFlags matrixLoadFlag, MatrixStoreFlags matrixStoreFlag,
                               bool usePhysStorageBuffer)
...
TestInstance *SSBOLayoutCase::createInstance(Context &context) const
{
    ...
    if (!context.getDescriptorIndexingFeatures().shaderStorageBufferArrayNonUniformIndexing &&
        usesDescriptorIndexing(m_interface))
