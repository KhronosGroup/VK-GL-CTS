...
void initPrograms(SourceCollections &programCollection, const CaseDef caseDef)
{
    ...
    const std::string colorExpr =
        ...
        (numColorComponents > 3 ? "(" + xMax + "-gx)^(" + yMax + "-gy)^gz" : "1") + ")" + colorScaleExpr;
...
void checkSupport(Context &context, const CaseDef caseDef)
{
    ...
    const VkResult imageFormatResult = context.getInstanceInterface().getPhysicalDeviceImageFormatProperties(
        context.getPhysicalDevice(), caseDef.format, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL,
        VK_IMAGE_USAGE_STORAGE_BIT, (VkImageCreateFlags)0, &imageFormatProperties);
...
tcu::TestStatus test(Context &context, const CaseDef caseDef)
{
    ...
    UniquePtr<Buffer> constantsBuffer(new Buffer(
    ...
    UniquePtr<Buffer> resultBuffer(
        new Buffer(vk, device, allocator, makeBufferCreateInfo(resultBufferSizeBytes, VK_BUFFER_USAGE_TRANSFER_DST_BIT),
                   MemoryRequirement::HostVisible));
...
tcu::TestCaseGroup *createImageMultisampleLoadStoreTests(tcu::TestContext &testCtx)
{
    ...
    MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "load_store_multisample", "Multisampled image store and load"));
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); ++formatNdx)
            for (int layerBindMode = 0; layerBindMode < numLayerBindModes; ++layerBindMode)
            {
                ...
                for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); ++samplesNdx)
                {
                    ...
                    addFunctionCaseWithPrograms(formatGroup.get(), samplesCaseName, "", checkSupport, initPrograms,
                                                test, caseDef);
                }
