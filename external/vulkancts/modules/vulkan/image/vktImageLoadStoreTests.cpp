...
void StoreTest::checkSupport(Context &context) const
{
    ...
    if (!m_declareImageFormatInShader &&
        !(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_2_STORAGE_WRITE_WITHOUT_FORMAT_BIT_KHR))
...
void LoadStoreTest::checkSupport(Context &context) const
{
    ...
    if (!m_bufferLoadUniform && !m_declareImageFormatInShader &&
        !(formatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT_KHR))
    ...
    if ((m_texture.type() != IMAGE_TYPE_BUFFER) &&
        !(imageFormatProperties.optimalTilingFeatures & VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT))
        TCU_THROW(NotSupportedError, "Format not supported for storage images");
...
private:
    ...
    {
        return (m_extendTestType == ExtendTestType::WRITE);
    }
