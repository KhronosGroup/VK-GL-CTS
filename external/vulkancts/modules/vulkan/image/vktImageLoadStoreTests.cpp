...
StoreTest::StoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                     const Texture &texture, const VkFormat format, const VkImageTiling tiling, const uint32_t flags)
...
public:
    ...
    LoadStoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                  const Texture &texture, const VkFormat format, const VkFormat imageFormat, const VkImageTiling tiling,
...
LoadStoreTest::LoadStoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             ...
                             const VkImageTiling tiling, const uint32_t flags, const bool imageLoadStoreLodAMD)
    : TestCase(testCtx, name, description)
    , m_texture(texture)
    , m_format(format)
...
void LoadStoreTest::checkSupport(Context &context) const
{
    ...
    if (!m_bufferLoadUniform && !m_declareImageFormatInShader &&
        !(tilingFeatures & VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT_KHR))
        TCU_THROW(NotSupportedError, "Format not supported for unformatted loads via storage images");
    if (m_texture.type() == IMAGE_TYPE_BUFFER && !m_declareImageFormatInShader &&
        !(formatProperties.bufferFeatures & VK_FORMAT_FEATURE_2_STORAGE_READ_WITHOUT_FORMAT_BIT_KHR))
...
void LoadStoreTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    if (m_bufferLoadUniform)
        ...
        src << "layout (binding = 0" << maybeFmtQualStr << ") " << maybeRestrictStr << "readonly uniform "
    ...
    m_imageSrc = de::MovePtr<Image>(
        new Image(vk, device, allocator,
                  makeImageCreateInfo(m_texture, m_imageFormat,
                                      VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, imageFlags, tiling),
                  MemoryRequirement::Any));

    m_imageDst = de::MovePtr<Image>(
        new Image(vk, device, allocator,
                  makeImageCreateInfo(m_texture, m_imageFormat,
                                      VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, imageFlags, tiling),
                  MemoryRequirement::Any));
}
    ...
    for (int textureNdx = 0; textureNdx < DE_LENGTH_OF_ARRAY(s_textures); ++textureNdx)
    {
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_formats); ++formatNdx)
        {
            for (int tilingNdx = 0; tilingNdx < DE_LENGTH_OF_ARRAY(s_tilings); tilingNdx++)
            {
                ...
                groupWithFormatByImageViewType->addChild(
                    new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + suffix, "", texture,
                                      s_formats[formatNdx], s_formats[formatNdx], s_tilings[tilingNdx]));
                groupWithoutFormatByImageViewType->addChild(
                    new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + suffix, "", texture,
                                      s_formats[formatNdx], s_formats[formatNdx], s_tilings[tilingNdx], 0));
                ...
                {
                    groupWithFormatByImageViewType->addChild(new LoadStoreTest(
                        testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign" + suffix, "", texture,
                        s_formats[formatNdx], s_formats[formatNdx], s_tilings[tilingNdx],
                        LoadStoreTest::FLAG_MINALIGN | LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER));
                    groupWithFormatByImageViewType->addChild(new LoadStoreTest(
                        testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign_uniform" + suffix, "", texture,
                        s_formats[formatNdx], s_formats[formatNdx], s_tilings[tilingNdx],
                        LoadStoreTest::FLAG_MINALIGN | LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER |
                            LoadStoreTest::FLAG_UNIFORM_TEXEL_BUFFER));
                    groupWithoutFormatByImageViewType->addChild(
                        new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign" + suffix,
                                          "", texture, s_formats[formatNdx], s_formats[formatNdx], s_tilings[tilingNdx],
                                          LoadStoreTest::FLAG_MINALIGN));
        ...
        if (texture.type() == IMAGE_TYPE_BUFFER)
        {
            for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_formatsThreeComponent); ++formatNdx)
            {
                for (int tilingNdx = 0; tilingNdx < DE_LENGTH_OF_ARRAY(s_tilings); tilingNdx++)
                {
                    ...
                    groupWithoutFormatByImageViewType->addChild(new LoadStoreTest(
                        testCtx, getFormatShortString(s_formatsThreeComponent[formatNdx]) + "_uniform" + suffix, "",
                        texture, s_formatsThreeComponent[formatNdx], s_formatsThreeComponent[formatNdx],
                        s_tilings[tilingNdx], LoadStoreTest::FLAG_UNIFORM_TEXEL_BUFFER));
                    groupWithoutFormatByImageViewType->addChild(new LoadStoreTest(
                        testCtx,
                        getFormatShortString(s_formatsThreeComponent[formatNdx]) + "_minalign_uniform" + suffix, "",
                        texture, s_formatsThreeComponent[formatNdx], s_formatsThreeComponent[formatNdx],
                        s_tilings[tilingNdx], LoadStoreTest::FLAG_MINALIGN | LoadStoreTest::FLAG_UNIFORM_TEXEL_BUFFER));
                }
            }
        }
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_formats); ++formatNdx)
        {
            ...
            groupWithFormatByImageViewType->addChild(
                new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]), "", texture,
                                  ...
                                  LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER, true));
            groupWithoutFormatByImageViewType->addChild(
                new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]), "", texture,
                                  s_formats[formatNdx], s_formats[formatNdx], VK_IMAGE_TILING_OPTIMAL, 0, true));
                                                       ...
                                                       const std::string &name)
{
    ...
    return de::MovePtr<TestCase>(
        new LoadStoreTest(testCtx, name, "", texture, format, format, VK_IMAGE_TILING_OPTIMAL,
