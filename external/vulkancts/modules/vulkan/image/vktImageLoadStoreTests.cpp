                         ...
                         const uint32_t mipmapLevel = 0u)
{
    ...
    for (int layerNdx = 0; layerNdx < numLayersOrSlices; ++layerNdx)
    {
        ...
        case tcu::TEXTURECHANNELCLASS_SIGNED_FIXED_POINT:
        {
            ...
            const tcu::Vec4 threshold(
                1.0f /
                ((tcu::UVec4(1u) << (tcu::getTextureFormatMantissaBitDepth(mapVkFormat(format)).cast<uint32_t>() -
                                     1u)) -
                 1u)
                    .cast<float>());
                                                      ...
                                                      const VkImage image, const Texture &texture)
{
    ...
    vk.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                          ...
                          (const VkBufferMemoryBarrier *)DE_NULL, 1, &shaderWriteBarrier);
}
...
public:
    ...
    StoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
              const Texture &texture, const VkFormat format,
              const uint32_t flags = FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER);
...
StoreTest::StoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                     const Texture &texture, const VkFormat format, const uint32_t flags)
...
void StoreTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    bool useClamp                      = false;
    std::string colorBaseExpr = signednessPrefix + "vec4(" + "gx^gy^gz, " + "(" + de::toString(xMax) + "-gx)^gy^gz, " +
                                "gx^(" + de::toString(yMax) + "-gy)^gz, " + "(" + de::toString(xMax) + "-gx)^(" +
                                de::toString(yMax) + "-gy)^gz)";
    ...
    , m_imageSizeBytes(getImageSizeBytes(texture.size(), format))
{
    ...
    m_imageBuffer = de::MovePtr<Buffer>(
...
void ImageStoreTestInstance::commandBeforeCompute(const VkCommandBuffer cmdBuffer)
{
    ...
    const VkImageMemoryBarrier setImageLayoutBarrier = makeImageMemoryBarrier(
        0u, 0u, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, m_image->get(), fullImageSubresourceRange);
    ...
    vk.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                          (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1, &writeConstantsBarrier, 1,
                          &setImageLayoutBarrier);
}
...
public:
    ...
    LoadStoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                  const Texture &texture, const VkFormat format, const VkFormat imageFormat,
                  const uint32_t flags = FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER, const bool imageLoadStoreLodAMD = false);
...
private:
    ...
    const bool
        m_declareImageFormatInShader; //!< Whether the shader will specify the format layout qualifier of the images
    const bool m_singleLayerBind;
...
LoadStoreTest::LoadStoreTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             const Texture &texture, const VkFormat format, const VkFormat imageFormat,
...
void LoadStoreTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    if (m_bufferLoadUniform)
        ...
        src << "layout (binding = 0, " << formatQualifierStr << ") " << maybeRestrictStr << "readonly uniform "
    ...
    if (formatHasThreeComponents(m_format))
        ...
        src << "layout (binding = 1, " << formatQualifierStr << ") " << maybeRestrictStr << "writeonly uniform "
    ...
    , m_bufferLoadUniform(bufferLoadUniform)
{
    ...
    m_imageBuffer =
        de::MovePtr<Buffer>(new Buffer(vk, device, allocator,
...
protected:
    ...
    Buffer *getResultBuffer(void) const
    {
    ...
    , m_allDstImageViews(texture.numLayers())
{
    ...
    m_imageSrc = de::MovePtr<Image>(
        new Image(vk, device, allocator,
                  makeImageCreateInfo(m_texture, m_imageFormat,
                                      VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, imageFlags),
                  MemoryRequirement::Any));

    m_imageDst = de::MovePtr<Image>(
        new Image(vk, device, allocator,
                  makeImageCreateInfo(m_texture, m_imageFormat,
                                      VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT, imageFlags),
                  MemoryRequirement::Any));
}
...
void ImageLoadStoreTestInstance::commandBeforeCompute(const VkCommandBuffer cmdBuffer)
{
        ...
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, m_texture.numLayers());
    {
        ...
        vk.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT,
                              ...
                              &barrierFlushHostWriteBeforeCopy, DE_LENGTH_OF_ARRAY(preCopyImageBarriers),
    ...
    }
    {
        ...
        vk.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                              (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                              (const VkBufferMemoryBarrier *)DE_NULL, 1, &barrierAfterCopy);
    }
}
                            ...
                            bufferLoadUniform)
{
    ...
    m_imageBufferDst = de::MovePtr<Buffer>(
        new Buffer(vk, device, allocator,
...
TestInstance *LoadStoreTest::createInstance(Context &context) const
{
    if (m_imageLoadStoreLodAMD)
        return new ImageLoadStoreLodAMDTestInstance(context, m_texture, m_format, m_imageFormat,
                                                    m_declareImageFormatInShader, m_singleLayerBind, m_minalign,
                                                    m_bufferLoadUniform);

    if (m_texture.type() == IMAGE_TYPE_BUFFER)
        return new BufferLoadStoreTestInstance(context, m_texture, m_format, m_imageFormat,
                                               m_declareImageFormatInShader, m_minalign, m_bufferLoadUniform);
    else
        return new ImageLoadStoreTestInstance(context, m_texture, m_format, m_imageFormat, m_declareImageFormatInShader,
                                              m_singleLayerBind, m_minalign, m_bufferLoadUniform);
}

class ImageExtendOperandTestInstance : public BaseTestInstance
{
public:
    ImageExtendOperandTestInstance(Context &context, const Texture &texture, const VkFormat format);
...
tcu::TestStatus ImageExtendOperandTestInstance::verifyResult(void)
{
    ...
    if (intThresholdCompare(m_context.getTestContext().getLog(), "Comparison", "Comparison", refAccess, result,
                            tcu::UVec4(0), tcu::COMPARE_LOG_RESULT))
        return tcu::TestStatus::pass("Passed");
    else
        return tcu::TestStatus::fail("Image comparison failed");
...
public:
    ImageExtendOperandTest(tcu::TestContext &testCtx, const std::string &name, const Texture texture,
                           const VkFormat format, const bool signedInt, const bool relaxedPrecision);
...
ImageExtendOperandTest::ImageExtendOperandTest(tcu::TestContext &testCtx, const std::string &name,
                                               const Texture texture, const VkFormat format, const bool operandForce,
...
void ImageExtendOperandTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    const std::map<vk::VkFormat, FormatData> formatDataMap = {// Mandatory support
...
static const VkFormat s_formats[] = {
    ...
    VK_FORMAT_R32G32_SFLOAT, VK_FORMAT_R16G16_SFLOAT, VK_FORMAT_R16_SFLOAT,

    VK_FORMAT_A2B10G10R10_UINT_PACK32, VK_FORMAT_R32G32_UINT, VK_FORMAT_R16G16_UINT, VK_FORMAT_R16_UINT,
    VK_FORMAT_R8G8_UINT, VK_FORMAT_R8_UINT,

    VK_FORMAT_R32G32_SINT, VK_FORMAT_R16G16_SINT, VK_FORMAT_R16_SINT, VK_FORMAT_R8G8_SINT, VK_FORMAT_R8_SINT,

    VK_FORMAT_A2B10G10R10_UNORM_PACK32, VK_FORMAT_R16G16B16A16_UNORM, VK_FORMAT_R16G16B16A16_SNORM,
    VK_FORMAT_R16G16_UNORM, VK_FORMAT_R16_UNORM, VK_FORMAT_R8G8_UNORM, VK_FORMAT_R8_UNORM,

    VK_FORMAT_R16G16_SNORM, VK_FORMAT_R16_SNORM, VK_FORMAT_R8G8_SNORM, VK_FORMAT_R8_SNORM};
...
tcu::TestCaseGroup *createImageStoreTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> testGroupWithFormat(
        new tcu::TestCaseGroup(testCtx, "with_format", "Declare a format layout qualifier for write images"));
    de::MovePtr<tcu::TestCaseGroup> testGroupWithoutFormat(
        new tcu::TestCaseGroup(testCtx, "without_format", "Do not declare a format layout qualifier for write images"));
    ...
    for (int textureNdx = 0; textureNdx < DE_LENGTH_OF_ARRAY(s_textures); ++textureNdx)
    {
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_formats); ++formatNdx)
        {
            groupWithFormatByImageViewType->addChild(
                new StoreTest(testCtx, getFormatShortString(s_formats[formatNdx]), "", texture, s_formats[formatNdx]));
            groupWithoutFormatByImageViewType->addChild(new StoreTest(
                testCtx, getFormatShortString(s_formats[formatNdx]), "", texture, s_formats[formatNdx], 0));

            if (isLayered)
                groupWithFormatByImageViewType->addChild(
                    new StoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + "_single_layer", "", texture,
                                  s_formats[formatNdx],
            ...
            if (texture.type() == IMAGE_TYPE_BUFFER)
            {
                groupWithFormatByImageViewType->addChild(new StoreTest(
                    testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign", "", texture,
                    s_formats[formatNdx], StoreTest::FLAG_MINALIGN | StoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER));
                groupWithoutFormatByImageViewType->addChild(
                    new StoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign", "", texture,
                                  s_formats[formatNdx], StoreTest::FLAG_MINALIGN));
            }
        }
...
tcu::TestCaseGroup *createImageLoadStoreTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "load_store", "Cases with imageLoad() followed by imageStore()"));
    de::MovePtr<tcu::TestCaseGroup> testGroupWithFormat(
        new tcu::TestCaseGroup(testCtx, "with_format", "Declare a format layout qualifier for read images"));
    de::MovePtr<tcu::TestCaseGroup> testGroupWithoutFormat(
        new tcu::TestCaseGroup(testCtx, "without_format", "Do not declare a format layout qualifier for read images"));
    ...
    for (int textureNdx = 0; textureNdx < DE_LENGTH_OF_ARRAY(s_textures); ++textureNdx)
    {
        ...
        {
            groupWithFormatByImageViewType->addChild(
                new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]), "", texture,
                                  s_formats[formatNdx], s_formats[formatNdx]));
            groupWithoutFormatByImageViewType->addChild(
                new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]), "", texture,
                                  s_formats[formatNdx], s_formats[formatNdx], 0));
            ...
            if (texture.type() == IMAGE_TYPE_BUFFER)
            {
                groupWithFormatByImageViewType->addChild(new LoadStoreTest(
                    testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign", "", texture,
                    s_formats[formatNdx], s_formats[formatNdx],
                    LoadStoreTest::FLAG_MINALIGN | LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER));
                groupWithFormatByImageViewType->addChild(new LoadStoreTest(
                    testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign_uniform", "", texture,
                    s_formats[formatNdx], s_formats[formatNdx],
                    LoadStoreTest::FLAG_MINALIGN | LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER |
                        LoadStoreTest::FLAG_UNIFORM_TEXEL_BUFFER));
                groupWithoutFormatByImageViewType->addChild(
                    new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign", "", texture,
                                      s_formats[formatNdx], s_formats[formatNdx], LoadStoreTest::FLAG_MINALIGN));
                groupWithoutFormatByImageViewType->addChild(
                    new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]) + "_minalign_uniform", "",
        ...
        if (texture.type() == IMAGE_TYPE_BUFFER)
        {
            ...
            {
                groupWithoutFormatByImageViewType->addChild(
                    new LoadStoreTest(testCtx, getFormatShortString(s_formatsThreeComponent[formatNdx]) + "_uniform",
                                      "", texture, s_formatsThreeComponent[formatNdx],
                                      s_formatsThreeComponent[formatNdx], LoadStoreTest::FLAG_UNIFORM_TEXEL_BUFFER));
                groupWithoutFormatByImageViewType->addChild(new LoadStoreTest(
                    testCtx, getFormatShortString(s_formatsThreeComponent[formatNdx]) + "_minalign_uniform", "",
                    texture, s_formatsThreeComponent[formatNdx], s_formatsThreeComponent[formatNdx],
                    LoadStoreTest::FLAG_MINALIGN | LoadStoreTest::FLAG_UNIFORM_TEXEL_BUFFER));
            }
        }
...
tcu::TestCaseGroup *createImageLoadStoreLodAMDTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "load_store_lod", "Cases with imageLoad() followed by imageStore()"));
    de::MovePtr<tcu::TestCaseGroup> testGroupWithFormat(
        new tcu::TestCaseGroup(testCtx, "with_format", "Declare a format layout qualifier for read images"));
    de::MovePtr<tcu::TestCaseGroup> testGroupWithoutFormat(
        new tcu::TestCaseGroup(testCtx, "without_format", "Do not declare a format layout qualifier for read images"));
    ...
    for (int textureNdx = 0; textureNdx < DE_LENGTH_OF_ARRAY(textures); ++textureNdx)
    {
        ...
        {
            groupWithFormatByImageViewType->addChild(new LoadStoreTest(
                testCtx, getFormatShortString(s_formats[formatNdx]), "", texture, s_formats[formatNdx],
                s_formats[formatNdx], LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER, true));
            groupWithoutFormatByImageViewType->addChild(
                new LoadStoreTest(testCtx, getFormatShortString(s_formats[formatNdx]), "", texture,
                                  s_formats[formatNdx], s_formats[formatNdx], 0, true));

            if (isLayered)
                groupWithFormatByImageViewType->addChild(new LoadStoreTest(
                    testCtx, getFormatShortString(s_formats[formatNdx]) + "_single_layer", "", texture,
                    s_formats[formatNdx], s_formats[formatNdx],
                    LoadStoreTest::FLAG_SINGLE_LAYER_BIND | LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER, true));
        }
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "format_reinterpret", "Cases with differing texture and image formats"));
        ...
        for (int imageFormatNdx = 0; imageFormatNdx < DE_LENGTH_OF_ARRAY(s_formats); ++imageFormatNdx)
            for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_formats); ++formatNdx)
            {
                ...
                if (imageFormatNdx != formatNdx &&
                    ...
                    groupByImageViewType->addChild(new LoadStoreTest(testCtx, caseName, "", texture,
                                                                     s_formats[formatNdx], s_formats[imageFormatNdx]));
            }
                                                       ...
                                                       const std::string &name)
{
    ...
    return de::MovePtr<TestCase>(
        new LoadStoreTest(testCtx, name, "", texture, format, format,
                          LoadStoreTest::FLAG_RESTRICT_IMAGES | LoadStoreTest::FLAG_DECLARE_IMAGE_FORMAT_IN_SHADER));
}
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "extend_operands_spirv1p4", "Cases with SignExtend and ZeroExtend"));
    ...
    for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_formats); ++formatNdx)
    {
        ...
        for (int prec = 0; prec < 2; prec++)
        {
            ...
            if (isIntFormat(format))
                testGroup->addChild(new ImageExtendOperandTest(testCtx, name + "_force_sign_extend", texture, format,
                                                               true, relaxedPrecision));
        }
    }
