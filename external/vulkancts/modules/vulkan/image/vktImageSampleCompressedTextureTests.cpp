...
tcu::TestStatus SampleDrawnTextureTestInstance::iterate(void)
{
    ...
    const VkImageSubresourceRange imageSubresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1, 0, layerCount);
    ...
    else
    {
        ...
        DescriptorSetUpdateBuilder()
            .writeSingle(*storageImageDescriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
    ...
    const Move<VkPipelineLayout> computePipelineLayout =
        makePipelineLayout(vk, device, 1, &(*descriptorSetLayout), 1, &pushConstantRange);
    ...
    if (m_cubemap)
    {
        ...
        for (int i = 0; i < FACES; i++)
        {
            cubeSamplerImageViews.emplace_back(makeImageView(vk, device, *storageImage, VK_IMAGE_VIEW_TYPE_2D,
                                                             m_imageFormat, cubeSubresourceRanges[i],
                                                             &imageViewUsageInfo));
            cubeSamplerDscrImageInfos.emplace_back(
                makeDescriptorImageInfo(sampler2.get(), *cubeSamplerImageViews[i], VK_IMAGE_LAYOUT_GENERAL));
            cubeSamplerDescriptorSets.emplace_back(
                makeDescriptorSet(vk, device, *descriptorPool, *graphicsDscrSetLayout));
            updateBuilder.writeSingle(*cubeSamplerDescriptorSets[i], DescriptorSetUpdateBuilder::Location::binding(0u),
                                      VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &cubeSamplerDscrImageInfos[i]);
        }
        ...
        if (m_twoSamplers)
        {
            for (int i = 0; i < FACES; i++)
            {
                cubeSampler2ImageViews.emplace_back(makeImageView(vk, device, *storageImage, VK_IMAGE_VIEW_TYPE_2D,
                                                                  m_imageViewFormat, cubeSubresourceRanges[i]));
                cubeSampler2DscrImageInfos.emplace_back(
                    makeDescriptorImageInfo(sampler.get(), *cubeSampler2ImageViews[i], VK_IMAGE_LAYOUT_GENERAL));
                updateBuilder.writeSingle(*cubeSamplerDescriptorSets[i],
                                          DescriptorSetUpdateBuilder::Location::binding(1u),
                                          VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &cubeSampler2DscrImageInfos[i]);
            }
        }
        updateBuilder.update(vk, device);
    }
    else
    {
        ...
        sampledImageView2     = makeImageView(vk, device, *storageImage, VK_IMAGE_VIEW_TYPE_2D, m_imageFormat,
                                              subresourceRange, &imageViewUsageInfo);
        ...
        {
            sampledImageView =
                makeImageView(vk, device, *storageImage, VK_IMAGE_VIEW_TYPE_2D, m_imageViewFormat, subresourceRange);
            samplerDscrImageInfo = makeDescriptorImageInfo(sampler.get(), *sampledImageView, VK_IMAGE_LAYOUT_GENERAL);
        }

        updateBuilder.writeSingle(*graphicsDescriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
                                  VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &samplerDscrImageInfo2);
        if (m_twoSamplers)
            updateBuilder.writeSingle(*graphicsDescriptorSet, DescriptorSetUpdateBuilder::Location::binding(1u),
    ...
    const Move<VkPipelineLayout> graphicsPipelineLayout =
        makePipelineLayout(vk, device, 1, &(*graphicsDscrSetLayout), 1, &pushConstantRange2);
    ...
    const auto barrier1 = makeImageMemoryBarrier(0u, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
    ...
    for (int pass = 0; pass < 2; pass++)
    {
        ...
        if (!m_twoSamplers || pass == 0)
        {
            ...
            if (m_cubemap)
            {
                for (int face = 0; face < FACES; face++)
                {
                    vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipelineLayout, 0u, 1u,
                                             &(cubeStorageDscrSets[face].get()), 0u, nullptr);
            ...
            {
                vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipelineLayout, 0u, 1u,
                                         &storageImageDescriptorSet.get(), 0u, nullptr);
                vk.cmdDispatch(*cmdBuffer, storageImageViewSize.x(), storageImageViewSize.y(), 1u);
            }
        ...
        if (m_cubemap)
        {
            for (int face = 0; face < FACES; face++)
            {
                vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *graphicsPipelineLayout, 0u, 1u,
                                         &(cubeSamplerDescriptorSets[face].get()), 0u, nullptr);

                beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                                makeRect2D(0, 0, imageSize.x(), imageSize.y()), 0u, nullptr);
                vk.cmdDraw(*cmdBuffer, vertexCount, 1u, 0u, 0u);
                endRenderPass(vk, *cmdBuffer);

                if (face < FACES - 1)
                {
                    const auto barrier4 = makeImageMemoryBarrier(
                    ...
                    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                          VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, nullptr, 0, nullptr, 1u,
                                          &barrier4);
                }
            }
        }
        ...
        {
            vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *graphicsPipelineLayout, 0u, 1u,
                                     &(graphicsDescriptorSet.get()), 0u, nullptr);

            beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, makeRect2D(0, 0, imageSize.x(), imageSize.y()),
                            0u, nullptr);
            vk.cmdDraw(*cmdBuffer, vertexCount, 1u, 0u, 0u);
            endRenderPass(vk, *cmdBuffer);
        }
        ...
        if (pass == 0)
        {
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                  VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1u, &barrier3);
    ...
    if (m_cubemap)
    {
        ...
        {
            for (uint32_t x = 0; x < renderSize.width; x++)
            {
                const uint8_t *ptr   = static_cast<const uint8_t *>(resultImage.getAccess().getPixelPtr(x, y, 0));
        ...
        m_context.getTestContext().getLog()
            << tcu::TestLog::ImageSet("Attachment ", "")
...
{
public:
    SampleDrawnTextureTest(tcu::TestContext &testCtx, const std::string &name, const VkFormat imageFormat,
                           const VkFormat imageViewFormat, const bool twoSamplers, const bool cubemap);
...
SampleDrawnTextureTest::SampleDrawnTextureTest(tcu::TestContext &testCtx, const std::string &name,
                                               const VkFormat imageFormat, const VkFormat imageViewFormat,
                                               const bool twoSamplers, const bool cubemap)
...
void SampleDrawnTextureTest::checkSupport(Context &context) const
{
    ...
    if (vki.getPhysicalDeviceImageFormatProperties(context.getPhysicalDevice(), m_imageFormat, VK_IMAGE_TYPE_2D,
                                                   VK_IMAGE_TILING_OPTIMAL, usageFlags, creationFlags,
...
void SampleDrawnTextureTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    computeSrc << "    imageStore(img, ivec2(gl_GlobalInvocationID.xy), color);\n"
               << "}\n";
...
tcu::TestCaseGroup *createImageSampleDrawnTextureTests(tcu::TestContext &testCtx)
{
    ...
    testGroup->addChild(new SampleDrawnTextureTest(testCtx, "128_bit_compressed_format_cubemap",
                                                   VK_FORMAT_BC3_UNORM_BLOCK, VK_FORMAT_R32G32B32A32_UINT, !twoSamplers,
                                                   cubemap));
    testGroup->addChild(new SampleDrawnTextureTest(testCtx, "64_bit_compressed_format_cubemap",
                                                   VK_FORMAT_BC1_RGB_UNORM_BLOCK, VK_FORMAT_R32G32_UINT, !twoSamplers,
                                                   cubemap));
    testGroup->addChild(new SampleDrawnTextureTest(testCtx, "64_bit_compressed_format_two_samplers_cubemap",
                                                   VK_FORMAT_BC1_RGB_UNORM_BLOCK, VK_FORMAT_R32G32_UINT, twoSamplers,
                                                   cubemap));
    testGroup->addChild(new SampleDrawnTextureTest(testCtx, "128_bit_compressed_format_two_samplers_cubemap",
                                                   VK_FORMAT_BC3_UNORM_BLOCK, VK_FORMAT_R32G32B32A32_UINT, twoSamplers,
                                                   cubemap));

    testGroup->addChild(new SampleDrawnTextureTest(testCtx, "64_bit_compressed_format", VK_FORMAT_BC1_RGB_UNORM_BLOCK,
                                                   VK_FORMAT_R32G32_UINT, !twoSamplers, false));
    ...
    testGroup->addChild(new SampleDrawnTextureTest(testCtx, "128_bit_compressed_format", VK_FORMAT_BC3_UNORM_BLOCK,
                                                   VK_FORMAT_R32G32B32A32_UINT, !twoSamplers, false));
