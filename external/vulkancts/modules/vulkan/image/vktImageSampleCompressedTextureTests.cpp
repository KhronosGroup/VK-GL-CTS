...
inline VkImageCreateInfo makeImageCreateInfo(const IVec3 &size, const VkFormat &format, bool storageImage, bool cubemap)
{
    ...
    const uint32_t layerCount      = cubemap ? 6u : 1u;
...
tcu::TestStatus SampleDrawnTextureTestInstance::iterate(void)
{
    ...
    const VkImageSubresourceRange imageSubresourceRange2 =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1, 0, m_cubemap ? 6 : 1);
    const auto barrier1 =
        makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                               VK_IMAGE_LAYOUT_GENERAL, storageImage.get(), imageSubresourceRange2);
...
void SampleDrawnTextureTest::checkSupport(Context &context) const
{
    ...
    if (m_cubemap &&
        vki.getPhysicalDeviceImageFormatProperties(
            context.getPhysicalDevice(), m_imageFormat, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, usageFlags,
            VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT, &imageFormatProperties) == VK_ERROR_FORMAT_NOT_SUPPORTED)
        TCU_THROW(NotSupportedError,
                  "Compressed images cannot be created with the VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT flag");
}
