...
public:
    ...
    MemoryQualifierTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                            const Qualifier qualifier, const ImageType imageType, const tcu::UVec3 &imageSize,
                            const tcu::TextureFormat &format, const glu::GLSLVersion glslVersion);
...
MemoryQualifierTestCase::MemoryQualifierTestCase(tcu::TestContext &testCtx, const std::string &name,
                                                 const std::string &description, const Qualifier qualifier,
...
void MemoryQualifierInstanceImage::prepareResources(const VkDeviceSize bufferSizeInBytes)
{
    ...
    m_buffer = de::MovePtr<Buffer>(
        new Buffer(deviceInterface, device, allocator, bufferCreateInfo, MemoryRequirement::HostVisible));
}
...
{
    de::MovePtr<tcu::TestCaseGroup> imageQualifiersTests(
        new tcu::TestCaseGroup(testCtx, "qualifiers", "Coherent, volatile and restrict"));
