...
protected:
    void generateData(uint8_t *toFill, const size_t size, const VkFormat format, const uint32_t layer,
...
TestStatus BasicComputeTestInstance::iterate(void)
{
    ...
    const VkExtent3D extentCompressed =
        makeExtent3D(getCompressedImageResolutionInBlocks(m_parameters.testedFormat, m_parameters.imageSize));
    ...
    VkImageViewASTCDecodeModeEXT decodeMode = {VK_STRUCTURE_TYPE_IMAGE_VIEW_ASTC_DECODE_MODE_EXT, DE_NULL,
    ...
    Move<VkImageView> referenceView =
        makeImageView(vk, device, referenceImage.get(), imageViewType, viewFormat, subresourceRange);
    ...
    const VkDeviceSize bufferSizeCompresed =
        getCompressedImageSizeInBytes(m_parameters.testedFormat, m_parameters.imageSize);
    const VkDeviceSize bufferSizeUncompressed =
        getImageSizeBytes(IVec3((int)extentCompressed.width, (int)extentCompressed.height, (int)extentCompressed.depth),
                          m_parameters.resultFormat);
    VkBufferCreateInfo compressedBufferCI = makeBufferCreateInfo(bufferSizeCompresed, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
    VkBufferCreateInfo uncompressedBufferCI = makeBufferCreateInfo(
        bufferSizeUncompressed, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        // copy input buffer to tested and reference images
        {
            ...
            {
                const VkImageMemoryBarrier preCopyImageBarrier =
                    makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, image->get(), subresourceRange);

                const VkBufferMemoryBarrier flushHostCopyBarrier = makeBufferMemoryBarrier(
                    VK_ACCESS_HOST_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, inBuffer.get(), 0ull, bufferSizeCompresed);
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      ...
                                      &flushHostCopyBarrier, 1u, &preCopyImageBarrier);
                ...
                vk.cmdCopyBufferToImage(*cmdBuffer, inBuffer.get(), image->get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                        1u, &copyRegion);
            }
        }
...

        {
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0u,
                                  (const VkBufferMemoryBarrier *)DE_NULL, DE_LENGTH_OF_ARRAY(preShaderImageBarriers),
...

        {
            const VkImageMemoryBarrier postShaderImageBarriers[] = {
                makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                       VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, resultImage.get(), subresourceRange)};
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  ...
                                  (const VkBufferMemoryBarrier *)DE_NULL, DE_LENGTH_OF_ARRAY(postShaderImageBarriers),
        ...
        vk.cmdCopyImageToBuffer(*cmdBuffer, resultImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, resultBuffer.get(),
                                1u, &copyRegion);

        {
            const VkBufferMemoryBarrier postCopyBufferBarrier[] = {
                makeBufferMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, resultBuffer.get(), 0ull,
                                        bufferSizeUncompressed),
            };
    ...
    uint8_t *result       = static_cast<uint8_t *>(resultAlloc.getHostPtr());
...
void BasicComputeTestInstance::generateData(uint8_t *toFill, const size_t size, const VkFormat format,
                                            const uint32_t layer, const uint32_t level)
{
    // Random data
    uint32_t *start32  = reinterpret_cast<uint32_t *>(toFill);
    size_t sizeToRnd32 = size / sizeof(uint32_t);
...
{
public:
    AstcDecodeModeCase(TestContext &testCtx, const std::string &name, const std::string &desc,
...
AstcDecodeModeCase::AstcDecodeModeCase(TestContext &testCtx, const std::string &name, const std::string &desc,
                                       const TestParameters &parameters)
...
void AstcDecodeModeCase::checkSupport(Context &context) const
{
    ...
    if (VK_ERROR_FORMAT_NOT_SUPPORTED ==
        vk.getPhysicalDeviceImageFormatProperties(physicalDevice, m_parameters.testedFormat,
    ...
    if (VK_ERROR_FORMAT_NOT_SUPPORTED ==
        vk.getPhysicalDeviceImageFormatProperties(physicalDevice, m_parameters.resultFormat,
                                                  mapImageType(m_parameters.imageType), VK_IMAGE_TILING_OPTIMAL,
    ...
    context.getInstanceInterface().getPhysicalDeviceFormatProperties(context.getPhysicalDevice(),
                                                                     m_parameters.resultFormat, &properties);
...
tcu::TestCaseGroup *createImageAstcDecodeModeTests(tcu::TestContext &testCtx)
{
    ...
    const DecodeModeData decodeModes[] = {{VK_FORMAT_R16G16B16A16_SFLOAT, "r16g16b16a16_sfloat"},
                                          {VK_FORMAT_R8G8B8A8_UNORM, "r8g8b8a8_unorm"},
                                          {VK_FORMAT_E5B9G9R9_UFLOAT_PACK32, "e5b9g9r9_ufloat_pack32"}};

    MovePtr<tcu::TestCaseGroup> astcDecodeModeTests(
        new tcu::TestCaseGroup(testCtx, "astc_decode_mode", "Intermediate decoding precision cases"));
    for (const FormatData &format : astcFormats)
    {
        for (const DecodeModeData &mode : decodeModes)
        {
            const TestParameters parameters = {IMAGE_TYPE_2D,
                                               UVec3(64u, 64u, 1u),
                                               format.format,
                                               format.isUnorm,
                                               mode.mode,
