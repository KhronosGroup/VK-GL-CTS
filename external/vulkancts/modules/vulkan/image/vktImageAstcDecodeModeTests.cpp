...
TestStatus BasicComputeTestInstance::iterate(void)
{
    ...
    const VkDeviceSize bufferSizeUncompressed = getImageSizeBytes(
        IVec3((int)extentUnCompressed.width, (int)extentUnCompressed.height, (int)extentUnCompressed.depth),
    ...
    // generate data for compressed image and copy it to in buffer
    {
        ...
        tcu::astc::generateRandomValidBlocks(generatedData.data(), blocks.x() * blocks.y() * blocks.z(),
                                             mapVkCompressedFormat(m_parameters.testedFormat),
                                             tcu::TexDecompressionParams::ASTCMODE_LDR, 1);
