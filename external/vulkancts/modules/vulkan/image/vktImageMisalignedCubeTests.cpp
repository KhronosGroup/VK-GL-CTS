...
tcu::TestStatus MisalignedCubeTestInstance::iterate(void)
{
    ...
    de::MovePtr<Buffer> resultBuffer =
        de::MovePtr<Buffer>(new Buffer(vk, device, allocator, resultBufferCreateInfo, MemoryRequirement::HostVisible));
...
{
public:
    MisalignedCubeTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       const tcu::IVec3 &size, const VkFormat format);
...
MisalignedCubeTest::MisalignedCubeTest(tcu::TestContext &testCtx, const std::string &name,
                                       const std::string &description, const tcu::IVec3 &size, const VkFormat format)
...
tcu::TestCaseGroup *createMisalignedCubeTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "misaligned_cube", "Cube image with misaligned baseArrayLayer test cases"));
