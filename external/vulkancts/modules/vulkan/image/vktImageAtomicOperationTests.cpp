...
static string getAtomicFuncArgumentShaderStr(const AtomicOperation op, const string &x, const string &y,
                                             const string &z, const IVec3 &gridSize)
{
    switch (op)
    {
...
static int32_t getOperationInitialValue(const AtomicOperation op)
{
    switch (op)
    {
...
template <typename T>
static T getAtomicFuncArgument(const AtomicOperation op, const IVec3 &invocationID, const IVec3 &gridSize)
{
...
{
public:
    BinaryAtomicEndResultCase(tcu::TestContext &testCtx, const string &name, const string &description,
                              const ImageType imageType, const tcu::UVec3 &imageSize, const tcu::TextureFormat &format,
                              const AtomicOperation operation, const glu::GLSLVersion glslVersion);
...
BinaryAtomicEndResultCase::BinaryAtomicEndResultCase(tcu::TestContext &testCtx, const string &name,
                                                     const string &description, const ImageType imageType,
...
void BinaryAtomicEndResultCase::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    else
    {
        ...
        const string compareExchangeStr =
            (m_operation == ATOMIC_OPERATION_COMPARE_EXCHANGE) ? ", 18" + string(uintFormat ? "u" : "") : "";
...
{
public:
    BinaryAtomicIntermValuesCase(tcu::TestContext &testCtx, const string &name, const string &description,
                                 const ImageType imageType, const tcu::UVec3 &imageSize,
                                 const tcu::TextureFormat &format, const AtomicOperation operation,
                                 const glu::GLSLVersion glslVersion);
...
BinaryAtomicIntermValuesCase::BinaryAtomicIntermValuesCase(TestContext &testCtx, const string &name,
                                                           const string &description, const ImageType imageType,
...
void BinaryAtomicIntermValuesCase::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    else
    {
        ...
        string source = versionDecl +
                        ...
                        invocationCoord + ", " + colorVecTypeName + "(" + atomicInvocation +
                        "));\n"
                        "}\n";
...
public:
    BinaryAtomicInstanceBase(Context &context, const string &name, const ImageType imageType,
                             const tcu::UVec3 &imageSize, const TextureFormat &format, const AtomicOperation operation);
...
tcu::TestStatus BinaryAtomicInstanceBase::iterate(void)
{
    ...
    m_resultImage =
        de::MovePtr<Image>(new Image(deviceInterface, device, allocator, imageParams, MemoryRequirement::Any));

    const VkImageSubresourceRange subresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, getNumLayers(m_imageType, m_imageSize));

    m_resultImageView = makeImageView(deviceInterface, device, m_resultImage->get(), mapImageViewType(m_imageType),
                                      mapTextureFormat(m_format), subresourceRange);

    //Prepare the buffer with the initial data for the image
    const Buffer inputBuffer(deviceInterface, device, allocator,
    ...
    tcu::PixelBufferAccess inputPixelBuffer(m_format, gridSize.x(), gridSize.y(), gridSize.z(),
                                            inputBufferAllocation.getHostPtr());
    ...
    m_outputBuffer = de::MovePtr<Buffer>(new Buffer(
        deviceInterface, device, allocator, makeBufferCreateInfo(outBuffSizeInBytes, VK_BUFFER_USAGE_TRANSFER_DST_BIT),
        MemoryRequirement::HostVisible));
    ...
    copyBufferToImage(deviceInterface, *cmdBuffer, *inputBuffer, imageSizeInBytes, bufferImageCopy,
                      VK_IMAGE_ASPECT_COLOR_BIT, 1, getNumLayers(m_imageType, m_imageSize), m_resultImage->get(),
    ...
    const VkBufferMemoryBarrier outputBufferPreHostReadBarrier = makeBufferMemoryBarrier(
        VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, m_outputBuffer->get(), 0ull, outBuffSizeInBytes);

    deviceInterface.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, false,
                                       0u, DE_NULL, 1u, &outputBufferPreHostReadBarrier, 0u, DE_NULL);
...
protected:
    template <typename T>
    bool isValueCorrect(int32_t resultValue, int32_t x, int32_t y, int32_t z, const UVec3 &gridSize,
...
void BinaryAtomicEndResultInstance::commandsAfterCompute(const VkCommandBuffer cmdBuffer) const
{
    ...
    deviceInterface.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                       false, 0u, DE_NULL, 0u, DE_NULL, 1u, &resultImagePostDispatchBarrier);

    const VkBufferImageCopy bufferImageCopyParams = makeBufferImageCopy(
        makeExtent3D(getLayerSize(m_imageType, m_imageSize)), getNumLayers(m_imageType, m_imageSize));
...
bool BinaryAtomicEndResultInstance::verifyResult(Allocation &outputBufferAllocation) const
{
    ...
    for (int32_t z = 0; z < resultBuffer.getDepth(); z++)
        for (int32_t y = 0; y < resultBuffer.getHeight(); y++)
            ...
            {
                int32_t resultValue = resultBuffer.getPixelInt(x, y, z).x();

                if (isOrderIndependentAtomicOperation(m_operation))
                {
                    if (uintFormat)
                    {
                        if (!isValueCorrect<uint32_t>(resultValue, x, y, z, gridSize, extendedGridSize))
                            return false;
                    }
                    else
                    {
                        if (!isValueCorrect<int32_t>(resultValue, x, y, z, gridSize, extendedGridSize))
                            return false;
                    }
                }
                else if (m_operation == ATOMIC_OPERATION_EXCHANGE)
                {
                    ...
                    for (int32_t i = 0; i < static_cast<int32_t>(NUM_INVOCATIONS_PER_PIXEL) && !matchFound; i++)
                    {
                        ...
                        matchFound =
                            (resultValue == getAtomicFuncArgument<int32_t>(m_operation, gid, extendedGridSize));
                    }
...
template <typename T>
bool BinaryAtomicEndResultInstance::isValueCorrect(int32_t resultValue, int32_t x, int32_t y, int32_t z,
...
protected:
    template <typename T>
    bool areValuesCorrect(tcu::ConstPixelBufferAccess &resultBuffer, int32_t x, int32_t y, int32_t z,
                          const UVec3 &gridSize, const IVec3 extendedGridSize) const;
...
void BinaryAtomicIntermValuesInstance::prepareResources(void)
{
    ...
    m_intermResultsImageView =
        makeImageView(deviceInterface, device, m_intermResultsImage->get(), mapImageViewType(m_imageType),
                      mapTextureFormat(m_format), subresourceRange);
}
...
void BinaryAtomicIntermValuesInstance::prepareDescriptors(void)
{
    ...
    DescriptorSetUpdateBuilder()
        ...
        .writeSingle(*m_descriptorSet, DescriptorSetUpdateBuilder::Location::binding(1u),
                     VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &descIntermResultsImageInfo)
        .update(deviceInterface, device);
}
...
void BinaryAtomicIntermValuesInstance::commandsAfterCompute(const VkCommandBuffer cmdBuffer) const
{
    ...
    deviceInterface.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                       false, 0u, DE_NULL, 0u, DE_NULL, 1u, &imagePostDispatchBarrier);
    ...
    deviceInterface.cmdCopyImageToBuffer(cmdBuffer, m_intermResultsImage->get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                         m_outputBuffer->get(), 1u, &bufferImageCopyParams);
}
    ...
    for (int32_t z = 0; z < resultBuffer.getDepth(); z++)
        for (int32_t y = 0; y < resultBuffer.getHeight(); y++)
            for (uint32_t x = 0; x < gridSize.x(); x++)
            {
                if (uintFormat)
                {
                    if (!areValuesCorrect<uint32_t>(resultBuffer, x, y, z, gridSize, extendedGridSize))
                        return false;
                }
                else
                {
                    if (!areValuesCorrect<int32_t>(resultBuffer, x, y, z, gridSize, extendedGridSize))
                        return false;
                }
            }
...
template <typename T>
bool BinaryAtomicIntermValuesInstance::areValuesCorrect(tcu::ConstPixelBufferAccess &resultBuffer, int32_t x, int32_t y,
                                                        int32_t z, const UVec3 &gridSize,
                                                        const IVec3 extendedGridSize) const
{
    ...
    return verifyRecursive(0, static_cast<T>(getOperationInitialValue(m_operation)), argsUsed, atomicArgs,
                           resultValues);
}
...
{
    de::MovePtr<tcu::TestCaseGroup> imageAtomicOperationsTests(
        new tcu::TestCaseGroup(testCtx, "atomic_operations", "Atomic image operations cases"));
    ...
    for (uint32_t operationI = 0; operationI < ATOMIC_OPERATION_LAST; operationI++)
    {
        ...
        for (uint32_t imageTypeNdx = 0; imageTypeNdx < DE_LENGTH_OF_ARRAY(imageParamsArray); imageTypeNdx++)
        {
            ...
            for (uint32_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
            {
                ...
                imageTypeGroup->addChild(new BinaryAtomicEndResultCase(testCtx, caseEndResult, "", imageType, imageSize,
                                                                       format, operation, glu::GLSL_VERSION_440));
                ...
                imageTypeGroup->addChild(new BinaryAtomicIntermValuesCase(
                    testCtx, caseIntermValues, "", imageType, imageSize, format, operation, glu::GLSL_VERSION_440));
            }
