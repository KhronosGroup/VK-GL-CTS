...
static string getComponentTypeStr(uint32_t componentWidth, bool intFormat, bool uintFormat, bool floatFormat)
{
...
void commonCheckSupport(Context &context, const tcu::TextureFormat &tcuFormat, ImageType imageType,
                        AtomicOperation operation, bool useTransfer, ShaderReadType readType,
                        ImageBackingType backingType)
{
    ...
    if (backingType == ImageBackingType::SPARSE)
    {
        ...
        if (!checkSparseImageFormatSupport(context.getPhysicalDevice(), context.getInstanceInterface(), format,
                                           vkImgType, VK_SAMPLE_COUNT_1_BIT, getUsageFlags(useTransfer),
...
public:
    BinaryAtomicIntermValuesCase(tcu::TestContext &testCtx, const string &name, const string &description,
                                 ...
                                 const ImageBackingType backingType, const glu::GLSLVersion glslVersion);
...
BinaryAtomicIntermValuesCase::BinaryAtomicIntermValuesCase(
    TestContext &testCtx, const string &name, const string &description, const ImageType imageType,
    const tcu::UVec3 &imageSize, const TextureFormat &format, const AtomicOperation operation, const bool useTransfer,
...
void BinaryAtomicIntermValuesCase::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    else
    {
        ...
        source += "precision highp " + shaderImageTypeStr +
                  ...
                  invocationCoord + ", " + vec4Type + "(" + atomicInvocation +
                  "));\n"
                  "}\n";
...
public:
    BinaryAtomicEndResultInstance(Context &context, const string &name, const ImageType imageType,
                                  ...
                                  const AtomicOperation operation, const bool useTransfer,
                                  const ShaderReadType shaderReadType, const ImageBackingType backingType)
        : BinaryAtomicInstanceBase(context, name, imageType, imageSize, format, operation, useTransfer, shaderReadType,
                                   backingType)
    {
    }
...
tcu::TestCaseGroup *createImageAtomicOperationTests(tcu::TestContext &testCtx)
{
    ...
    const tcu::TextureFormat formats[] = {tcu::TextureFormat(tcu::TextureFormat::R, tcu::TextureFormat::UNSIGNED_INT32),
                    ...
                    for (int backingTypeIdx = 0; backingTypeIdx < DE_LENGTH_OF_ARRAY(backingTypes); ++backingTypeIdx)
                    {
                        ...
                        for (uint32_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                        {
                            ...
                            backingTypeGroup->addChild(new BinaryAtomicEndResultCase(
                                testCtx, caseEndResult, "", imageType, imageSize, format, operation, useTransfer,
                                readType.type, backingType.type, glu::GLSL_VERSION_450));
                            ...
                            backingTypeGroup->addChild(new BinaryAtomicIntermValuesCase(
                                testCtx, caseIntermValues, "", imageType, imageSize, format, operation, useTransfer,
                                readType.type, backingType.type, glu::GLSL_VERSION_450));
                        }
