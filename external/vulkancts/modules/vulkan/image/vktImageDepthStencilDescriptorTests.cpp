...
{
public:
    DepthStencilDescriptorCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
tcu::TestStatus DepthStencilDescriptorInstance::iterate()
{
    ...
    const auto colorPostTransferBarrier =
        makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, 0u, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, colorLayout,
                               colorBuffer.get(), colorSRR);
    const auto dsPostTransferBarrier =
        makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, 0u, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, m_params.layout,
                               dsImage.get(), depthStencilSRR);
    const std::vector<VkImageMemoryBarrier> postTransferBarriers = {colorPostTransferBarrier, dsPostTransferBarrier};

    vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, 0u, 0u,
...
tcu::TestCaseGroup *createImageDepthStencilDescriptorTests(tcu::TestContext &testCtx)
{
    ...
    TestCaseGroupPtr mainGroup(
        new tcu::TestCaseGroup(testCtx, "depth_stencil_descriptor", "Tests using depth/stencil images as descriptors"));
    ...
    for (const auto &layout : kTestedLayouts)
    {
        ...
        for (const auto &format : kDepthStencilFormats)
        {
            ...
            TestCaseGroupPtr formatGroup(
                new tcu::TestCaseGroup(testCtx, formatGroupName.c_str(), formatGroupDesc.c_str()));
