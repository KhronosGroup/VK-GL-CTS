...
{
public:
    UploadDownloadExecutor(Context &context, VkDevice device, VkQueue queue, uint32_t queueFamilyIndex,
                           const CaseDef &caseSpec)
        : m_caseDef(caseSpec)
...
void UploadDownloadExecutor::run(Context &context, VkBuffer buffer)
{
    ...
    const VkImageCreateFlags imageFlags =
        VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT | (m_haveMaintenance2 ? VK_IMAGE_CREATE_EXTENDED_USAGE_BIT_KHR : 0);
...
void UploadDownloadExecutor::uploadStore(Context &context)
{
    ...
    m_uStore.computePipeline =
        makeComputePipeline(m_vk, m_device, *m_uStore.pipelineLayout, *m_uStore.computeModule, DE_NULL);
...
void UploadDownloadExecutor::downloadTexture(Context &context, VkBuffer buffer)
{
    ...
    m_dTex.computePipeline =
        makeComputePipeline(m_vk, m_device, *m_dTex.pipelineLayout, *m_dTex.computeModule, DE_NULL);
...
void UploadDownloadExecutor::downloadLoad(Context &context, VkBuffer buffer)
{
    ...
    m_dLoad.computePipeline =
        makeComputePipeline(m_vk, m_device, *m_dLoad.pipelineLayout, *m_dLoad.computeModule, DE_NULL);
...
tcu::TestStatus testMutable(Context &context, const CaseDef caseDef)
{
    ...
    // Execute the test
    UploadDownloadExecutor executor(context, device, context.getUniversalQueue(),
...
{
    de::MovePtr<TestCaseGroup> testGroup(
        new TestCaseGroup(testCtx, "swapchain_mutable", "Cases with swapchain mutable images"));
