                                                ...
                                                de::MovePtr<Image> &outputImage)
{
    ...
    const MovePtr<Buffer> vertexBuffer       = MovePtr<Buffer>(new Buffer(
    ...
    const MovePtr<Buffer> srcImageBuffer =
        MovePtr<Buffer>(new Buffer(vk, device, allocator, srcImageBufferInfo, MemoryRequirement::HostVisible));
    ...
    MovePtr<Buffer> dstImageBuffer =
        MovePtr<Buffer>(new Buffer(vk, device, allocator, dstImageBufferInfo, MemoryRequirement::HostVisible));
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, (VkDependencyFlags)0,
                          0, (const VkMemoryBarrier *)DE_NULL, 1u, &srcCopyBufferBarrierPre, 1u,
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                          ...
                          (const VkBufferMemoryBarrier *)DE_NULL, 1u, &srcCopyImageBarrierPost);
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, (VkDependencyFlags)0,
                          0, (const VkMemoryBarrier *)DE_NULL, 1, &copyBarrier, 0,
                                                                       ...
                                                                       const bool extendedImageCreateFlag)
{
    ...
    const VkImageCreateFlags imageCreateFlagsAddOn =
        extendedImageCreateFlag ? VK_IMAGE_CREATE_EXTENDED_USAGE_BIT_KHR : 0;
                                            ...
                                            de::MovePtr<Image> &outputImage)
{
    ...
    const Move<VkFramebuffer> framebuffer(makeFramebuffer(vk, device, *renderPass, 0, DE_NULL, framebufferSize.width,
    ...
    //Copy buffer to image
    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, (VkDependencyFlags)0,
                          0, (const VkMemoryBarrier *)DE_NULL, 1u, &srcCopyBufferBarrier, 1u, &srcCopyImageBarrier);
...
{
public:
    ImageTranscodingCase(TestContext &testCtx, const std::string &name, const std::string &desc,
...
ImageTranscodingCase::ImageTranscodingCase(TestContext &testCtx, const std::string &name, const std::string &desc,
                                           const TestParameters &parameters)
                                                      ...
                                                      VkImageUsageFlags formatUsageFlags) const
{
    ...
    const VkResult queryResult = vk.getPhysicalDeviceImageFormatProperties(
        ...
        VK_IMAGE_CREATE_EXTENDED_USAGE_BIT_KHR, &imageFormatProperties);
...
tcu::TestCaseGroup *createImageTranscodingSupportTests(tcu::TestContext &testCtx)
{
    ...
    MovePtr<tcu::TestCaseGroup> imageTranscodingTests(
        new tcu::TestCaseGroup(testCtx, "extended_usage_bit", "Extended usage bit test cases"));
