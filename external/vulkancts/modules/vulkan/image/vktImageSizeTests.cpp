...
public:
    ...
    SizeTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description, const Texture &texture,
             const VkFormat format, const uint32_t flags = 0);
...
SizeTest::SizeTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
std::string getCaseName(const Texture &texture, const uint32_t flags)
{
...
{
public:
    SizeTestInstance(Context &context, const Texture &texture, const VkFormat format);
...
{
public:
    ImageSizeTestInstance(Context &context, const Texture &texture, const VkFormat format);
    ...
    : SizeTestInstance(context, texture, format)
{
    ...
    m_image = de::MovePtr<Image>(
        new Image(vk, device, allocator, makeImageCreateInfo(m_texture, m_format), MemoryRequirement::Any));

    const VkImageSubresourceRange subresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, m_texture.numLayers());
    m_imageView =
        makeImageView(vk, device, m_image->get(), mapImageViewType(m_texture.type()), m_format, subresourceRange);
}
...
void ImageSizeTestInstance::commandBeforeCompute(const VkCommandBuffer cmdBuffer)
{
    ...
    const VkImageMemoryBarrier barrierSetImageLayout = makeImageMemoryBarrier(
        0u, 0u, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, m_image->get(), subresourceRange);
...
tcu::TestCaseGroup *createImageSizeTests(tcu::TestContext &testCtx)
{
    ...
    for (int imageTypeNdx = 0; imageTypeNdx < DE_LENGTH_OF_ARRAY(s_imageTypes); ++imageTypeNdx)
    {
        de::MovePtr<tcu::TestCaseGroup> imageGroup(
            new tcu::TestCaseGroup(testCtx, getImageTypeName(s_imageTypes[imageTypeNdx]).c_str(), ""));
        ...
        for (int flagNdx = 0; flagNdx < DE_LENGTH_OF_ARRAY(s_flags); ++flagNdx)
            for (int imageSizeNdx = 0; imageSizeNdx < DE_LENGTH_OF_ARRAY(s_baseImageSizes); ++imageSizeNdx)
            {
                ...
                imageGroup->addChild(new SizeTest(testCtx, getCaseName(texture, s_flags[flagNdx]), "", texture, format,
                                                  s_flags[flagNdx]));
            }
