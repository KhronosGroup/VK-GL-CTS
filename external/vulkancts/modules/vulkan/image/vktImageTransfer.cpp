...
public:
    ...
    TransferQueueCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
TransferQueueCase::TransferQueueCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const TestParams &params)
...
void TransferQueueCase::checkSupport(Context &context) const
{
    ...
    if (result != VK_SUCCESS)
    {
        if (result == VK_ERROR_FORMAT_NOT_SUPPORTED)
            TCU_THROW(NotSupportedError, "Error: format " + de::toString(m_params.imageFormat) +
...
tcu::TestCaseGroup *createTransferQueueImageTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> layoutTestGroup(
        new tcu::TestCaseGroup(testCtx, "queue_transfer", "Tests for transfering from buffer to image and back"));
