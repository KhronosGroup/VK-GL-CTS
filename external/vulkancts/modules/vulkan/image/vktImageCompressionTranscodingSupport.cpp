                ...
                for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
                    for (uint32_t layerNdx = 0u; layerNdx < getLayerCount(); ++layerNdx)
                    {
                        imageData[imageNdx].addImageView(makeImageView(
                            vk, device, imageData[imageNdx].getImage(infoNdx), mapImageViewType(m_parameters.imageType),
            ...
            for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
                for (uint32_t layerNdx = 0u; layerNdx < getLayerCount(); ++layerNdx)
                {
                    ...
                    if (!copyResultAndCompare(*cmdPool, *cmdBuffer, imageData[resultImageNdx].getImage(imageNdx),
                                              offset, size))
                        return TestStatus::fail("Uncompressed output mismatch at offset " + de::toString(offset) +
    ...
    beginCommandBuffer(vk, cmdBuffer);
    {
        ...
        for (uint32_t ndx = 0u; ndx < descriptorSets.size(); ++ndx)
        {
            ...
            vk.cmdDispatch(cmdBuffer, imageData[1].getImageInfo(ndx).extent.width,
                           imageData[1].getImageInfo(ndx).extent.height, imageData[1].getImageInfo(ndx).extent.depth);
        }
    }
    endCommandBuffer(vk, cmdBuffer);
    submitCommandsAndWait(vk, device, queue, cmdBuffer);
                                                    ...
                                                    const UVec3 &size)
{
    ...
    VkDeviceSize imageResultSize =
        getImageSizeBytes(tcu::IVec3(size.x(), size.y(), size.z()), m_parameters.formatUncompressed);
    ...
    for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
        for (uint32_t layerNdx = 0u; layerNdx < getLayerCount(); ++layerNdx)
        {
            ...
            const VkDeviceSize bufferSizeComp =
                getCompressedImageSizeInBytes(m_parameters.formatCompressed, mipMapSizes[mipNdx]);
            ...
            const Unique<VkPipeline> pipeline(makeComputePipeline(vk, device, *pipelineLayout, *shaderModule));
            const VkDeviceSize bufferSize = getImageSizeBytes(
                IVec3((int)extentCompressed.width, (int)extentCompressed.height, (int)extentCompressed.depth),
            ...
            {
                ConstPixelBufferAccess resultPixels(
                    mapVkFormat(decompressedImageInfo.format), decompressedImageInfo.extent.width,
                    decompressedImageInfo.extent.height, decompressedImageInfo.extent.depth, resultAlloc.getHostPtr());
                ConstPixelBufferAccess referencePixels(mapVkFormat(decompressedImageInfo.format),
                                                       decompressedImageInfo.extent.width,
                                                       decompressedImageInfo.extent.height,
                                                       decompressedImageInfo.extent.depth, referenceAlloc.getHostPtr());

                if (!fuzzyCompare(m_context.getTestContext().getLog(), "Image Comparison", "Image Comparison",
                                  resultPixels, referencePixels, 0.001f, tcu::COMPARE_LOG_EVERYTHING))
                    return false;
            }
...
protected:
    ...
    bool verifyDecompression(const std::vector<uint8_t> &refCompressedData,
...
TestStatus GraphicsAttachmentsTestInstance::iterate(void)
{
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
            if (isWriteToCompressedOperation())
            {
                if (!verifyDecompression(*m_srcData[levelNdx][layerNdx], m_compressedImage, levelNdx, layerNdx,
                                         m_compressedImageResVec[levelNdx]))
                    return TestStatus::fail("Images difference detected");
            }
            else
            {
                if (!verifyDecompression(*m_dstData[levelNdx][layerNdx], m_compressedImage, levelNdx, layerNdx,
                                         m_compressedImageResVec[levelNdx]))
                    return TestStatus::fail("Images difference detected");
            }
...
bool GraphicsAttachmentsTestInstance::verifyDecompression(const std::vector<uint8_t> &refCompressedData,
...
{
public:
    TexelViewCompatibleCase(TestContext &testCtx, const std::string &name, const std::string &desc,
...
TexelViewCompatibleCase::TexelViewCompatibleCase(TestContext &testCtx, const std::string &name, const std::string &desc,
                                                 const TestParameters &parameters)
...
void TexelViewCompatibleCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    case SHADER_TYPE_COMPUTE:
    {
        const std::string imageTypeStr =
            getShaderImageType(mapVkFormat(m_parameters.formatUncompressed), m_parameters.imageType);
        ...
        case OPERATION_TEXEL_FETCH:
        {
            ...
            src << "layout (binding = 0) uniform "
                << getGlslSamplerType(mapVkFormat(m_parameters.formatUncompressed),
                                      mapImageViewType(m_parameters.imageType))
                << " u_image0;\n"
                << "layout (binding = 1, " << formatQualifierStr << ") writeonly uniform " << imageTypeStr
                << " u_image1;\n\n"
        ...
        src_decompress << "layout (binding = 0) uniform "
        ...
        if (compressedReferenceImageType == IMAGE_TYPE_2D_ARRAY)
            src_decompress << "    imageStore(decompressed_reference, pos, textureLod(compressed_reference, vec3(cord, "
                              "layer), level));\n";
        else
...
tcu::TestCaseGroup *createImageCompressionTranscodingTests(tcu::TestContext &testCtx)
{
    ...
    MovePtr<tcu::TestCaseGroup> texelViewCompatibleTests(
        new tcu::TestCaseGroup(testCtx, "texel_view_compatible", "Texel view compatible cases"));
