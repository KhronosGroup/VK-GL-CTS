                                               ...
                                               vector<ImageData> &imageData, const vector<UVec3> &mipMapSizes)
{
    ...
    for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
        for (uint32_t layerNdx = 0u; layerNdx < getLayerCount(); ++layerNdx)
        {
            ...
            const uint32_t imageNdx             = layerNdx + mipNdx * getLayerCount();
            const VkExtent3D extentCompressed   = imageType == VK_IMAGE_TYPE_1D ?
                                                      makeExtent3D(mipMapSizes[mipNdx].x(), 1, mipMapSizes[mipNdx].z()) :
                                                  ...
                                                  const VkDescriptorPool &descriptorPool, vector<ImageData> &imageData)
{
    ...
    beginCommandBuffer(vk, cmdBuffer);
    {
        ...
        preShaderImageBarriers[preShaderImageBarriers.size() - 1] =
            makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                   VK_IMAGE_LAYOUT_GENERAL, imageData[0].getImage(0u), compressedRange);
