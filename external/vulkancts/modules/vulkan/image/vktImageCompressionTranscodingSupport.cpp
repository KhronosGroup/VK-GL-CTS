                                                ...
                                                const bool isCompressed)
{
    ...
    if (isCompressed)
    {
        ...
        m_context.getInstanceInterface().getPhysicalDeviceFormatProperties(m_context.getPhysicalDevice(),
                                                                           m_parameters.formatCompressed, &properties);
                                               ...
                                               vector<ImageData> &imageData, const vector<UVec3> &mipMapSizes)
{
    ...
    for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
        for (uint32_t layerNdx = 0u; layerNdx < getLayerCount(); ++layerNdx)
        {
            ...
            Move<VkImageView> compressedView = makeImageView(
                vk, device, compressed, imageViewType, m_parameters.formatCompressed,
            ...
            beginCommandBuffer(vk, cmdBuffer);
            {
...

                {
                    const VkImageMemoryBarrier preShaderImageBarriers[] = {
                        ...
                        makeImageMemoryBarrier(0, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                               layoutShaderReadOnly ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL :
                                                                      VK_IMAGE_LAYOUT_GENERAL,
...
void TexelViewCompatibleCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    case SHADER_TYPE_COMPUTE:
    {
        ...
        src_decompress << "layout (binding = 0) uniform "
                       ...
                       << "layout (binding = 2, " << formatQualifierStr << ") writeonly uniform " << imageTypeStr
                       << " decompressed_result;\n"
                       << "layout (binding = 3, " << formatQualifierStr << ") writeonly uniform " << imageTypeStr
