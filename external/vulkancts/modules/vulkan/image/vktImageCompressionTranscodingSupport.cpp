...
const uint32_t ASTC_LDR_ERROR_COLOUR = 0xFFFF00FF;
const uint32_t ASTC_HDR_ERROR_COLOUR = 0x00000000;

static BinaryCompareResult BinaryCompare(const void *reference, const void *result, VkDeviceSize sizeInBytes,
                                         VkFormat formatForVerify, BinaryCompareMode mode)
{
    ...
    else if (mode == COMPARE_MODE_ALLOW_ASTC_ERROR_COLOUR_WARNING)
    {
        bool bWarn                  = false;
        bool bFail                  = false;
        const uint32_t *pui32RefVal = (uint32_t *)reference;
        const uint32_t *pui32ResVal = (uint32_t *)result;

        DE_ASSERT(formatForVerify == VK_FORMAT_R8G8B8A8_UNORM);
        size_t numPixels = (size_t)(sizeInBytes / 4) /* bytes */;
        for (size_t i = 0; i < numPixels; i++)
        {
            const uint32_t ref = *pui32RefVal++;
            const uint32_t res = *pui32ResVal++;
        ...
        if (!bFail)
        {
            return (bWarn) ? (COMPARE_RESULT_ASTC_QUALITY_WARNING) : (COMPARE_RESULT_OK);
        }
    }
...
protected:
    void generateData(uint8_t *toFill, const size_t size, const VkFormat format, const uint32_t layer = 0u,
                      const uint32_t level = 0u);
    uint32_t getLevelCount();
    uint32_t getLayerCount();
    UVec3 getLayerDims();
    vector<UVec3> getMipLevelSizes(UVec3 baseSize);
    vector<UVec3> getCompressedMipLevelSizes(const VkFormat compressedFormat, const vector<UVec3> &uncompressedSizes);

    const TestParameters m_parameters;
    const uint32_t m_blockWidth;
    const uint32_t m_blockHeight;
    const uint32_t m_levelCount;
...
private:
    uint32_t findMipMapLevelCount();
};

uint32_t BasicTranscodingTestInstance::findMipMapLevelCount()
{
    ...
    {
        uint32_t w = m_parameters.size.x();
        uint32_t h = m_parameters.size.y();
...
uint32_t BasicTranscodingTestInstance::getLevelCount()
{
    return m_levelCount;
}

uint32_t BasicTranscodingTestInstance::getLayerCount()
{
...
vector<UVec3> BasicTranscodingTestInstance::getCompressedMipLevelSizes(const VkFormat compressedFormat,
...
void BasicTranscodingTestInstance::generateData(uint8_t *toFill, const size_t size, const VkFormat format,
                                                const uint32_t layer, const uint32_t level)
{
    ...
    // Random part
    {
        ...
        uint32_t seed      = (layer << 24) ^ (level << 16) ^ static_cast<uint32_t>(format);
        Random rnd(seed);
        ...
        else if (isFloatFormat(m_parameters.formatUncompressed))
        {
            ...
            else if (textureFormat.type == tcu::TextureFormat::FLOAT)
            {
                ...
                for (size_t i = 0; i < size; i += 4)
                {
                    ...
                    // Little endian fix
                    if (((toFill[i + 3] & 0x7F) == 0x7F && (toFill[i + 2] & 0x80) == 0x80) ||
                        ((toFill[i + 3] & 0x7F) == 0x00 && (toFill[i + 2] & 0x80) == 0x00))
                        toFill[i + 3] = 0x01;
                    // Big endian fix
                    if (((toFill[i + 0] & 0x7F) == 0x7F && (toFill[i + 1] & 0x80) == 0x80) ||
                        ((toFill[i + 0] & 0x7F) == 0x00 && (toFill[i + 1] & 0x80) == 0x00))
                        toFill[i + 0] = 0x01;
                }
            }
        }
...
protected:
    struct ImageData
    {
        uint32_t getImagesCount(void)
        {
            return static_cast<uint32_t>(images.size());
        }
        uint32_t getImageViewCount(void)
        {
            return static_cast<uint32_t>(imagesViews.size());
        }
        uint32_t getImageInfoCount(void)
        {
            return static_cast<uint32_t>(imagesInfos.size());
        }
        VkImage getImage(const uint32_t ndx)
        {
            return **images[ndx]->get();
        }
        VkImageView getImageView(const uint32_t ndx)
        {
            return **imagesViews[ndx];
        }
        VkImageCreateInfo getImageInfo(const uint32_t ndx)
        {
            return imagesInfos[ndx];
        }
        void addImage(MovePtr<Image> image)
        {
            images.push_back(makeVkSharedPtr(image));
        }
        void addImageView(Move<VkImageView> imageView)
        {
            imagesViews.push_back(makeVkSharedPtr(imageView));
        }
        void addImageInfo(const VkImageCreateInfo imageInfo)
        {
            imagesInfos.push_back(imageInfo);
        }
        void resetViews()
        {
            imagesViews.clear();
    ...
    void copyDataToImage(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer, ImageData &imageData,
                         const vector<UVec3> &mipMapSizes, const bool isCompressed);
    virtual void executeShader(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                               const VkDescriptorSetLayout &descriptorSetLayout, const VkDescriptorPool &descriptorPool,
                               vector<ImageData> &imageData);
    bool copyResultAndCompare(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                              const VkImage &uncompressed, const VkDeviceSize offset, const UVec3 &size);
    void descriptorSetUpdate(VkDescriptorSet descriptorSet, const VkDescriptorImageInfo *descriptorImageInfos);
    void createImageInfos(ImageData &imageData, const vector<UVec3> &mipMapSizes, const bool isCompressed);
    bool decompressImage(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer, vector<ImageData> &imageData,
...
TestStatus BasicComputeTestInstance::iterate(void)
{
    ...
    const vector<UVec3> mipMapSizes =
        m_parameters.useMipmaps ? getMipLevelSizes(getLayerDims()) : vector<UVec3>(1, fullSize);
    vector<ImageData> imageData(m_parameters.imagesCount);
    const uint32_t compressedNdx  = 0u;
    const uint32_t resultImageNdx = m_parameters.imagesCount - 1u;

    for (uint32_t imageNdx = 0u; imageNdx < m_parameters.imagesCount; ++imageNdx)
    {
        const bool isCompressed = compressedNdx == imageNdx ? true : false;
        createImageInfos(imageData[imageNdx], mipMapSizes, isCompressed);
        for (uint32_t infoNdx = 0u; infoNdx < imageData[imageNdx].getImageInfoCount(); ++infoNdx)
        {
            imageData[imageNdx].addImage(MovePtr<Image>(
                new Image(vk, device, allocator, imageData[imageNdx].getImageInfo(infoNdx), MemoryRequirement::Any)));
            if (isCompressed)
            {
                ...
                if (m_parameters.useMultiLayerViews())
                {
                    ...
                    imageData[imageNdx].addImageView(
                        makeImageView(vk, device, imageData[imageNdx].getImage(infoNdx), VK_IMAGE_VIEW_TYPE_2D_ARRAY,
                                      m_parameters.formatUncompressed,
                ...
                else
                {
                    for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
                        for (uint32_t layerNdx = 0u; layerNdx < getLayerCount(); ++layerNdx)
                        {
                            imageData[imageNdx].addImageView(makeImageView(
                                vk, device, imageData[imageNdx].getImage(infoNdx),
            ...
            else
            {
                if (m_parameters.useMultiLayerViews())
                {
                    ...
                    imageData[imageNdx].addImageView(makeImageView(
                        vk, device, imageData[imageNdx].getImage(infoNdx), VK_IMAGE_VIEW_TYPE_2D_ARRAY,
                        m_parameters.formatUncompressed,
                        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, getLayerCount())));
                }
                else
                {
                    imageData[imageNdx].addImageView(
                        makeImageView(vk, device, imageData[imageNdx].getImage(infoNdx),
                                      mapImageViewType(m_parameters.imageType), m_parameters.formatUncompressed,
...

    {
        ...
        for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
        {
            size += static_cast<size_t>(
                getCompressedImageSizeInBytes(m_parameters.formatCompressed, mipMapSizes[mipNdx]) * getLayerCount());
        }
        m_data.resize(size);
    ...
    std::string failString;
    {
        ...
        for (uint32_t imageNdx = 0u; imageNdx < m_parameters.imagesCount; ++imageNdx)
        {
            ...
            case OPERATION_IMAGE_LOAD:
            case OPERATION_IMAGE_STORE:
                descriptorSetLayoutBuilder.addSingleBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                                                            VK_SHADER_STAGE_COMPUTE_BIT);
                descriptorPoolBuilder.addType(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, imageData[0].getImageViewCount());
                break;
            case OPERATION_TEXEL_FETCH:
            case OPERATION_TEXTURE:
                descriptorSetLayoutBuilder.addSingleBinding((compressedNdx == imageNdx) ?
                                                                VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER :
                                                                VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                                                            VK_SHADER_STAGE_COMPUTE_BIT);
                descriptorPoolBuilder.addType((compressedNdx == imageNdx) ? VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER :
                                                                            VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                                              imageData[0].getImageViewCount());
                break;
            default:
                DE_ASSERT(false);
                break;
        ...
        descriptorPool      = descriptorPoolBuilder.build(vk, device, VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT,
                                                          imageData[0].getImageViewCount());
...

        {
            ...
            for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
                ...
                {
                    const uint32_t imageNdx = layerNdx + mipNdx * getLayerCount();
                    ...
                    if (!copyResultAndCompare(*cmdPool, *cmdBuffer, imageData[resultImageNdx].getImage(imageNdx),
                                              offset, size))
                    {
                        pass       = false;
                        failString = std::string("Uncompressed output mismatch at offset ") + de::toString(offset) +
                                     " even before executing decompression";
                    }
...
void BasicComputeTestInstance::copyDataToImage(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                                               ImageData &imageData, const vector<UVec3> &mipMapSizes,
                                               const bool isCompressed)
{
    ...
    const VkImageSubresourceRange subresourceRange = {
        ...
        imageData.getImageInfo(0u).mipLevels,  //uint32_t                levelCount
        0u,                                    //uint32_t                baseArrayLayer
        imageData.getImageInfo(0u).arrayLayers //uint32_t                layerCount
    };

    for (uint32_t imageNdx = 0u; imageNdx < imageData.getImagesCount(); ++imageNdx)
    {
        ...
        for (uint32_t mipNdx = 0u; mipNdx < imageData.getImageInfo(imageNdx).mipLevels; ++mipNdx)
        {
            const VkExtent3D imageExtent =
                isCompressed ? makeExtent3D(mipMapSizes[mipNdx]) : imageData.getImageInfo(imageNdx).extent;
            const VkBufferImageCopy copyRegion = {
                offset, //VkDeviceSize bufferOffset;
                0u,     //uint32_t bufferRowLength;
                0u,     //uint32_t bufferImageHeight;
                makeImageSubresourceLayers(
                    VK_IMAGE_ASPECT_COLOR_BIT, mipNdx, 0u,
                    imageData.getImageInfo(imageNdx).arrayLayers), //VkImageSubresourceLayers imageSubresource;
                makeOffset3D(0, 0, 0),                             //VkOffset3D imageOffset;
                imageExtent,                                       //VkExtent3D imageExtent;
            };

            vk.cmdCopyBufferToImage(cmdBuffer, imageBuffer.get(), imageData.getImage(imageNdx),
                                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1u, &copyRegion);
            offset +=
                getCompressedImageSizeInBytes(
                    m_parameters.formatCompressed,
                    UVec3(isCompressed ? imageExtent.width : imageExtent.width * m_blockWidth,
                          isCompressed ? imageExtent.height : imageExtent.height * m_blockHeight, imageExtent.depth)) *
...
void BasicComputeTestInstance::executeShader(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                                             const VkDescriptorSetLayout &descriptorSetLayout,
                                             const VkDescriptorPool &descriptorPool, vector<ImageData> &imageData)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
    const VkQueue queue       = m_context.getUniversalQueue();
    ...
    for (uint32_t viewNdx = 0u; viewNdx < descriptorSets.size(); ++viewNdx)
    {
        const uint32_t descriptorNdx = viewNdx * m_parameters.imagesCount;
        for (uint32_t imageNdx = 0; imageNdx < m_parameters.imagesCount; ++imageNdx)
        {
            descriptorImageInfos[descriptorNdx + imageNdx] =
                makeDescriptorImageInfo(*sampler, imageData[imageNdx].getImageView(viewNdx), VK_IMAGE_LAYOUT_GENERAL);
        }
    }
    ...
    beginCommandBuffer(vk, cmdBuffer);
    {
        const VkImageSubresourceRange compressedRange = {
            VK_IMAGE_ASPECT_COLOR_BIT,                //VkImageAspectFlags    aspectMask
            0u,                                       //uint32_t                baseMipLevel
            imageData[0].getImageInfo(0u).mipLevels,  //uint32_t                levelCount
            0u,                                       //uint32_t                baseArrayLayer
            imageData[0].getImageInfo(0u).arrayLayers //uint32_t                layerCount
        };
        ...
        for (uint32_t imageNdx = 0u; imageNdx < imageData[1].getImagesCount(); ++imageNdx)
        {
            preShaderImageBarriers[imageNdx] = makeImageMemoryBarrier(
                VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                VK_IMAGE_LAYOUT_GENERAL, imageData[1].getImage(imageNdx), uncompressedRange);
        }
...
bool BasicComputeTestInstance::copyResultAndCompare(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                                                    const VkImage &uncompressed, const VkDeviceSize offset,
                                                    const UVec3 &size)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkQueue queue       = m_context.getUniversalQueue();
    ...
    beginCommandBuffer(vk, cmdBuffer);
    {
        ...
        const VkImageMemoryBarrier prepareForTransferBarrier =
            makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, uncompressed, subresourceRange);
    ...
    if (deMemCmp((const void *)allocResult.getHostPtr(), (const void *)&m_data[static_cast<size_t>(offset)],
                 static_cast<size_t>(imageResultSize)) == 0ull)
        return true;
    return false;
}

void BasicComputeTestInstance::descriptorSetUpdate(VkDescriptorSet descriptorSet,
                                                   const VkDescriptorImageInfo *descriptorImageInfos)
{
    ...
    case OPERATION_IMAGE_STORE:
    {
        for (uint32_t bindingNdx = 0u; bindingNdx < m_parameters.imagesCount; ++bindingNdx)
            descriptorSetUpdateBuilder.writeSingle(descriptorSet,
                                                   DescriptorSetUpdateBuilder::Location::binding(bindingNdx),
                                                   VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &descriptorImageInfos[bindingNdx]);
    ...
    case OPERATION_TEXTURE:
    {
        for (uint32_t bindingNdx = 0u; bindingNdx < m_parameters.imagesCount; ++bindingNdx)
        {
            descriptorSetUpdateBuilder.writeSingle(
                descriptorSet, DescriptorSetUpdateBuilder::Location::binding(bindingNdx),
                bindingNdx == 0u ? VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER : VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                &descriptorImageInfos[bindingNdx]);
        }
...
void BasicComputeTestInstance::createImageInfos(ImageData &imageData, const vector<UVec3> &mipMapSizes,
                                                const bool isCompressed)
{
    ...
    if (isCompressed)
    {
        ...
        const VkImageCreateInfo compressedInfo = {
            ...
            static_cast<uint32_t>(mipMapSizes.size()), // uint32_t mipLevels;
            getLayerCount(),                           // uint32_t arrayLayers;
        ...
        if (m_context.getInstanceInterface().getPhysicalDeviceImageFormatProperties(
                m_context.getPhysicalDevice(), compressedInfo.format, compressedInfo.imageType, compressedInfo.tiling,
                compressedInfo.usage, compressedInfo.flags, &imageFormatProperties) != VK_SUCCESS)
    ...
    else
    {
        ...
        const VkExtent3D originalResolutionInBlocks =
        ...
        for (size_t mipNdx = 0ull; mipNdx < mipMapSizes.size(); ++mipNdx)
            for (size_t layerNdx = 0ull; layerNdx < layerIterations; ++layerNdx)
            {
                const VkExtent3D extentUncompressed      = m_parameters.useMipmaps ?
                                                               makeExtent3D(getCompressedImageResolutionInBlocks(
                                                              m_parameters.formatCompressed, mipMapSizes[mipNdx])) :
...
bool BasicComputeTestInstance::decompressImage(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                                               vector<ImageData> &imageData, const vector<UVec3> &mipMapSizes)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
    const VkQueue queue       = m_context.getUniversalQueue();
    Allocator &allocator      = m_context.getDefaultAllocator();
    const Unique<VkShaderModule> shaderModule(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("decompress"), 0));
    const VkImage &compressed   = imageData[0].getImage(0);
    const VkImageType imageType = mapImageType(m_parameters.imageType);

    for (uint32_t ndx = 0u; ndx < imageData.size(); ndx++)
    ...
    for (uint32_t mipNdx = 0u; mipNdx < mipMapSizes.size(); ++mipNdx)
        for (uint32_t layerNdx = 0u; layerNdx < layerIterations; ++layerNdx)
        {
            ...
            const auto bufferSizeExtent =
                (m_parameters.useMultiLayerViews() ? UVec3(mipMapSize.x(), mipMapSize.y(), getLayerCount()) :
                                                     mipMapSize);
            const VkDeviceSize bufferSizeComp =
                getCompressedImageSizeInBytes(m_parameters.formatCompressed, bufferSizeExtent);
            const auto arrayLayers = (m_parameters.useMultiLayerViews() ? getLayerCount() : 1u);

            VkFormatProperties properties;
            m_context.getInstanceInterface().getPhysicalDeviceFormatProperties(
                m_context.getPhysicalDevice(), m_parameters.formatForVerify, &properties);
            ...
            const VkImageViewType imageViewType(m_parameters.useMultiLayerViews() ?
                                                    ...
                                                    mapImageViewType(m_parameters.imageType));
            ...
            Move<VkImageView> resultView = makeImageView(
                vk, device, resultImage.get(), imageViewType, decompressedImageInfo.format,
                makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, decompressedImageInfo.extent.depth, 0u,
                                          decompressedImageInfo.arrayLayers));
            Move<VkImageView> referenceView = makeImageView(
                vk, device, referenceImage.get(), imageViewType, decompressedImageInfo.format,
                makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, decompressedImageInfo.extent.depth, 0u,
                                          decompressedImageInfo.arrayLayers));
            Move<VkImageView> uncompressedView =
                makeImageView(vk, device, uncompressedImage.get(), imageViewType, m_parameters.formatCompressed,
                              makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, compressedImageInfo.extent.depth,
                                                        0u, compressedImageInfo.arrayLayers));
            ...
            Move<VkImageView> compressedView =
                (useMultiLayer) ?
                    makeImageView(vk, device, compressed, VK_IMAGE_VIEW_TYPE_2D_ARRAY, m_parameters.formatCompressed,
                                  makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u,
                                                            static_cast<uint32_t>(mipMapSizes.size()), 0u,
                                                            m_parameters.layers),
                                  &compressedViewUsageCI) :
            ...
            const struct PushData
            {
                uint32_t layer;
                uint32_t level;
            } pushData                                  = {layerNdx, mipNdx};
            const VkPushConstantRange pushConstantRange = {VK_SHADER_STAGE_COMPUTE_BIT, 0u,
                                                           static_cast<uint32_t>(sizeof pushData)};
            const Unique<VkPipelineLayout> pipelineLayout(
                makePipelineLayout(vk, device, 1u, &descriptorSetLayout.get(), 1u, &pushConstantRange));
            const Unique<VkPipeline> pipeline(makeComputePipeline(vk, device, *pipelineLayout, *shaderModule));
            // For multilayer views, the buffer will contain the full image with all layers instead of a single layer.
            const auto bufferSizeExtentVerif =
                IVec3((int)extentCompressed.width, (int)extentCompressed.height,
            ...
            BufferWithMemory transferBuffer(vk, device, allocator,
                                            makeBufferCreateInfo(bufferSizeComp, VK_BUFFER_USAGE_TRANSFER_SRC_BIT |
            ...
            VkDescriptorImageInfo descriptorImageInfos[] = {
                makeDescriptorImageInfo(*sampler, *uncompressedView,
            ...
            DescriptorSetUpdateBuilder()
                ...
                .writeSingle(descriptorSet.get(), DescriptorSetUpdateBuilder::Location::binding(1u),
                             VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &descriptorImageInfos[1])
                .writeSingle(descriptorSet.get(), DescriptorSetUpdateBuilder::Location::binding(2u),
                             VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &descriptorImageInfos[2])
                .writeSingle(descriptorSet.get(), DescriptorSetUpdateBuilder::Location::binding(3u),
                             VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &descriptorImageInfos[3])
                .update(vk, device);
            ...
            beginCommandBuffer(vk, cmdBuffer);
            {
                ...
                const VkImageSubresourceRange subresourceRangeComp = {
                    VK_IMAGE_ASPECT_COLOR_BIT,     //VkImageAspectFlags            aspectMask
                    (useMultiLayer) ? 0u : mipNdx, //uint32_t                        baseMipLevel
                    (useMultiLayer) ? static_cast<uint32_t>(mipMapSizes.size()) :
                                      1u,                      //uint32_t                        levelCount
                    (useMultiLayer) ? 0u : layerNdx,           //uint32_t                        baseArrayLayer
                    (useMultiLayer) ? m_parameters.layers : 1u //uint32_t                        layerCount
                };
                ...
                const VkBufferImageCopy copyRegion = {
                    ...
                    0u,   // uint32_t bufferImageHeight;
                    makeImageSubresourceLayers(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 0u,
                                               arrayLayers), // VkImageSubresourceLayers imageSubresource;
                    makeOffset3D(0, 0, 0),                   // VkOffset3D imageOffset;
...

                {
                    ...
                    const VkBufferMemoryBarrier preCopyBufferBarriers = makeBufferMemoryBarrier(
                ...
                vk.cmdCopyImageToBuffer(cmdBuffer, uncompressed, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                        transferBuffer.get(), 1u, &compressedCopyRegion);

                {
                    const VkBufferMemoryBarrier postCopyBufferBarriers =
                        makeBufferMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT,
                                                transferBuffer.get(), 0ull, bufferSizeComp);

                    const VkImageMemoryBarrier preCopyImageBarriers = makeImageMemoryBarrier(
...

                {
                    const VkImageMemoryBarrier preShaderImageBarriers[] = {
                        ...
                        makeImageMemoryBarrier(0, VK_ACCESS_SHADER_READ_BIT,
                                               (useMultiLayer && !layoutShaderReadOnly && layerNdx) ?
                        ...
                        makeImageMemoryBarrier(0u, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                               VK_IMAGE_LAYOUT_GENERAL, resultImage.get(), subresourceRange),
                ...
                vk.cmdCopyImageToBuffer(cmdBuffer, resultImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                        resultBuffer.get(), 1u, &copyRegion);
                vk.cmdCopyImageToBuffer(cmdBuffer, referenceImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                                        referenceBuffer.get(), 1u, &copyRegion);
            ...
            BinaryCompareMode compareMode =
                (m_parameters.formatIsASTC()) ? (COMPARE_MODE_ALLOW_ASTC_ERROR_COLOUR_WARNING) : (COMPARE_MODE_NORMAL);

            BinaryCompareResult res = BinaryCompare(referenceAlloc.getHostPtr(), resultAlloc.getHostPtr(),
                                                    (size_t)bufferSize, m_parameters.formatForVerify, compareMode);
            ...
            if (res == COMPARE_RESULT_FAILED)
            {
                ...
                for (uint32_t i = 0u; i < innerIterations; ++i)
                {
                    ...
                    const auto layerSize =
                        extent.width * extent.height * extent.depth * static_cast<uint32_t>(pixelSize);
                    const auto imageName =
                        "Image Comparison Layer " + std::to_string(m_parameters.useMultiLayerViews() ? i : layerNdx);

                    const auto resLayerPtr = reinterpret_cast<const char *>(resultAlloc.getHostPtr()) + layerSize * i;
                    const auto refLayerPtr =
                        reinterpret_cast<const char *>(referenceAlloc.getHostPtr()) + layerSize * i;

                    ConstPixelBufferAccess resultPixels(tcuFormat, extent.width, extent.height, extent.depth,
                                                        resLayerPtr);
                    ConstPixelBufferAccess referencePixels(tcuFormat, extent.width, extent.height, extent.depth,
                                                           refLayerPtr);
...
void ImageStoreComputeTestInstance::executeShader(const VkCommandPool &cmdPool, const VkCommandBuffer &cmdBuffer,
                                                  const VkDescriptorSetLayout &descriptorSetLayout,
                                                  const VkDescriptorPool &descriptorPool, vector<ImageData> &imageData)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    ...
    beginCommandBuffer(vk, cmdBuffer);
    {
        const VkImageSubresourceRange compressedRange = {
            VK_IMAGE_ASPECT_COLOR_BIT,               //VkImageAspectFlags    aspectMask
            0u,                                      //uint32_t                baseMipLevel
            imageData[0].getImageInfo(0).mipLevels,  //uint32_t                levelCount
            0u,                                      //uint32_t                baseArrayLayer
            imageData[0].getImageInfo(0).arrayLayers //uint32_t                layerCount
        };
        ...
        for (uint32_t imageNdx = 0u; imageNdx < imageData[1].getImagesCount(); ++imageNdx)
        {
            preShaderImageBarriers[imageNdx] = makeImageMemoryBarrier(
                ...
                VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL, imageData[1].getImage(imageNdx),
                uncompressedRange);

            preShaderImageBarriers[imageNdx + imageData[1].getImagesCount()] = makeImageMemoryBarrier(
                VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                VK_IMAGE_LAYOUT_GENERAL, imageData[2].getImage(imageNdx), uncompressedRange);
        }
...
protected:
    ...
    VkImageCreateInfo makeCreateImageInfo(const VkFormat format, const ImageType type, const UVec3 &size,
                                          const VkImageUsageFlags usageFlags, const VkImageCreateFlags *createFlags,
                                          const uint32_t levels, const uint32_t layers);
    VkDeviceSize getCompressedImageData(const VkFormat format, const UVec3 &size, std::vector<uint8_t> &data,
                                        const uint32_t layer, const uint32_t level);
    ...
    bool verifyDecompression(const VkCommandPool &cmdPool, const std::vector<uint8_t> &refCompressedData,
                             const de::MovePtr<Image> &resCompressedImage, const uint32_t layer, const uint32_t level,
                             const UVec3 &mipmapDims);

    typedef std::vector<uint8_t> RawDataVector;
...
TestStatus GraphicsAttachmentsTestInstance::iterate(void)
{
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
            if (isWriteToCompressedOperation())
            {
                if (!verifyDecompression(*cmdPool, *m_srcData[levelNdx][layerNdx], m_compressedImage, levelNdx,
                                         layerNdx, m_compressedImageResVec[levelNdx]))
                    pass = false;
            }
            else
            {
                if (!verifyDecompression(*cmdPool, *m_dstData[levelNdx][layerNdx], m_compressedImage, levelNdx,
                                         layerNdx, m_compressedImageResVec[levelNdx]))
                    pass = false;
            }
...
void GraphicsAttachmentsTestInstance::prepareData()
{
    ...
    m_srcImageUsageFlags =
        isWriteToCompressedOperation() ? m_parameters.uncompressedImageUsage : m_parameters.compressedImageUsage;
    m_dstImageUsageFlags =
        isWriteToCompressedOperation() ? m_parameters.compressedImageUsage : m_parameters.uncompressedImageUsage;
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
    {
        ...
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
        {
            ...
            if (isWriteToCompressedOperation())
            {
                getUncompressedImageData(m_srcFormat, m_srcImageResolutions[levelNdx], *m_srcData[levelNdx][layerNdx],
                                         layerNdx, levelNdx);

                m_dstData[levelNdx][layerNdx]->resize(
                    (size_t)getCompressedImageSizeInBytes(m_dstFormat, m_dstImageResolutions[levelNdx]));
            }
            else
            {
                getCompressedImageData(m_srcFormat, m_srcImageResolutions[levelNdx], *m_srcData[levelNdx][layerNdx],
                                       layerNdx, levelNdx);

                m_dstData[levelNdx][layerNdx]->resize(
                    (size_t)getUncompressedImageSizeInBytes(m_dstFormat, m_dstImageResolutions[levelNdx]));
            }
...
void GraphicsAttachmentsTestInstance::prepareVertexBuffer()
{
    ...
    m_vertexCount  = static_cast<uint32_t>(vertexArray.size());
...
void GraphicsAttachmentsTestInstance::transcodeRead(const VkCommandPool &cmdPool)
{
    ...
    const VkImageCreateInfo srcImageCreateInfo =
        makeCreateImageInfo(m_srcFormat, m_parameters.imageType, m_srcImageResolutions[0], m_srcImageUsageFlags,
                            imgCreateFlagsOverride, getLevelCount(), getLayerCount());
    ...
    const Unique<VkRenderPass> renderPass(
        vkt::image::makeRenderPass(vk, device, m_parameters.formatUncompressed, m_parameters.formatUncompressed));
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertShaderModule,
                                                           *fragShaderModule, renderSizeUnused, 1u, true));
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
    {
        ...
        const UVec3 srcImageResBlocked =
            getCompressedImageResolutionBlockCeil(m_parameters.formatCompressed, srcImageResolution);

        const VkImageCreateInfo dstImageCreateInfo =
            makeCreateImageInfo(m_dstFormat, m_parameters.imageType, dstImageResolution, m_dstImageUsageFlags,
                                imgCreateFlagsOverride, SINGLE_LEVEL, SINGLE_LAYER);
        ...
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
        {
            const VkImageSubresourceRange srcSubresourceRange =
                makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, levelNdx, SINGLE_LEVEL, layerNdx, SINGLE_LAYER);
            ...
            Move<VkImageView> srcImageView(
                makeImageView(vk, device, srcImage->get(), mapImageViewType(m_parameters.imageType),
                              m_parameters.formatUncompressed, srcSubresourceRange, m_srcImageViewUsageKHR));

            de::MovePtr<Image> dstImage(new Image(vk, device, allocator, dstImageCreateInfo, MemoryRequirement::Any));
            Move<VkImageView> dstImageView(
                makeImageView(vk, device, dstImage->get(), mapImageViewType(m_parameters.imageType),
                              m_parameters.formatUncompressed, dstSubresourceRange, m_dstImageViewUsageKHR));

            const VkBufferImageCopy srcCopyRegion =
                makeBufferImageCopy(srcImageResolution.x(), srcImageResolution.y(), levelNdx, layerNdx,
                                    srcImageResBlocked.x(), srcImageResBlocked.y());
            const VkBufferMemoryBarrier srcCopyBufferBarrierPre =
                makeBufferMemoryBarrier(VK_ACCESS_HOST_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, srcImageBuffer->get(),
                                        0ull, srcImageSizeInBytes);
            const VkImageMemoryBarrier srcCopyImageBarrierPre =
                makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, srcImage->get(), srcSubresourceRange);
            const VkImageMemoryBarrier srcCopyImageBarrierPost = makeImageMemoryBarrier(
                VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                VK_IMAGE_LAYOUT_GENERAL, srcImage->get(), srcSubresourceRange);
            const VkBufferImageCopy dstCopyRegion = makeBufferImageCopy(dstImageResolution.x(), dstImageResolution.y());
            const VkImageMemoryBarrier dstInitImageBarrier =
                makeImageMemoryBarrier(0u, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                       VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, dstImage->get(), dstSubresourceRange);
            ...
            const Move<VkFramebuffer> framebuffer(
                makeFramebuffer(vk, device, *renderPass, DE_LENGTH_OF_ARRAY(attachmentBindInfos), attachmentBindInfos,
                                framebufferSize.width, framebufferSize.height, SINGLE_LAYER));
            ...
            vk.cmdDraw(*cmdBuffer, (uint32_t)m_vertexCount, 1, 0, 0);
            ...
            const VkBufferMemoryBarrier copyBarrier =
                makeBufferMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT, dstImageBuffer->get(),
...
void GraphicsAttachmentsTestInstance::transcodeWrite(const VkCommandPool &cmdPool)
{
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
    {
        ...
        const UVec3 &srcImageResolution   = m_srcImageResolutions[levelNdx];
        const UVec3 &dstImageResolution   = m_dstImageResolutions[levelNdx];
        const UVec3 dstImageResBlocked =
            getCompressedImageResolutionBlockCeil(m_parameters.formatCompressed, dstImageResolution);
        const size_t srcImageSizeInBytes = m_srcData[levelNdx][0]->size();
        const size_t dstImageSizeInBytes = m_dstData[levelNdx][0]->size();

        const VkImageCreateInfo srcImageCreateInfo =
            makeCreateImageInfo(m_srcFormat, m_parameters.imageType, srcImageResolution, m_srcImageUsageFlags,
                                imgCreateFlagsOverride, SINGLE_LEVEL, SINGLE_LAYER);
        ...
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
        {
            ...
            const VkImageMemoryBarrier srcCopyImageBarrierPre =
                makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, srcImage->get(), srcSubresourceRange);
            const VkImageMemoryBarrier srcCopyImageBarrierPost = makeImageMemoryBarrier(
                VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_INPUT_ATTACHMENT_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                VK_IMAGE_LAYOUT_GENERAL, srcImage->get(), srcSubresourceRange);
            const VkBufferImageCopy dstCopyRegion =
                makeBufferImageCopy(dstImageResolution.x(), dstImageResolution.y(), levelNdx, layerNdx,
                                    dstImageResBlocked.x(), dstImageResBlocked.y());
            const VkImageMemoryBarrier dstInitImageBarrier =
                                                                       ...
                                                                       const uint32_t levels, const uint32_t layers)
{
    ...
    if ((usageFlags & VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT) &&
        ...
        TCU_THROW(NotSupportedError,
                  "Format color/depth/stencil attachment feature not supported for input attachment usage");
...
VkDeviceSize GraphicsAttachmentsTestInstance::getCompressedImageData(const VkFormat format, const UVec3 &size,
                                                                     std::vector<uint8_t> &data, const uint32_t layer,
...
bool GraphicsAttachmentsTestInstance::verifyDecompression(const VkCommandPool &cmdPool,
                                                          const std::vector<uint8_t> &refCompressedData,
                                                          ...
                                                          const UVec3 &mipmapDims)
{
    ...
    const VkImageSubresourceRange subresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, SINGLE_LEVEL, 0u, SINGLE_LAYER);
    const VkImageSubresourceRange resSubresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, level, SINGLE_LEVEL, layer, SINGLE_LAYER);

    const VkDeviceSize dstBufferSize = getUncompressedImageSizeInBytes(m_parameters.formatForVerify, mipmapDims);
    const VkImageUsageFlags refSrcImageUsageFlags =
        VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;

    const VkBufferCreateInfo refSrcImageBufferInfo(
        makeBufferCreateInfo(refCompressedData.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT));
    const MovePtr<BufferWithMemory> refSrcImageBuffer = MovePtr<BufferWithMemory>(
        new BufferWithMemory(vk, device, allocator, refSrcImageBufferInfo, MemoryRequirement::HostVisible));

    const VkImageCreateFlags refSrcImageCreateFlags = 0;
    const VkImageCreateInfo refSrcImageCreateInfo =
        makeCreateImageInfo(m_parameters.formatCompressed, m_parameters.imageType, mipmapDimsBlocked,
                            refSrcImageUsageFlags, &refSrcImageCreateFlags, SINGLE_LEVEL, SINGLE_LAYER);
    const MovePtr<Image> refSrcImage(new Image(vk, device, allocator, refSrcImageCreateInfo, MemoryRequirement::Any));
    Move<VkImageView> refSrcImageView(makeImageView(vk, device, refSrcImage->get(),
                                                    mapImageViewType(m_parameters.imageType),
                                                    m_parameters.formatCompressed, subresourceRange));

    const VkImageUsageFlags resSrcImageUsageFlags =
        VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    const VkImageViewUsageCreateInfo resSrcImageViewUsageKHR = makeImageViewUsageCreateInfo(resSrcImageUsageFlags);
    Move<VkImageView> resSrcImageView(
        makeImageView(vk, device, resCompressedImage->get(), mapImageViewType(m_parameters.imageType),
                      m_parameters.formatCompressed, resSubresourceRange, &resSrcImageViewUsageKHR));

    const VkImageCreateFlags refDstImageCreateFlags = 0;
    const VkImageUsageFlags refDstImageUsageFlags =
        VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
    const VkImageCreateInfo refDstImageCreateInfo =
        makeCreateImageInfo(m_parameters.formatForVerify, m_parameters.imageType, mipmapDims, refDstImageUsageFlags,
                            &refDstImageCreateFlags, SINGLE_LEVEL, SINGLE_LAYER);
    const MovePtr<Image> refDstImage(new Image(vk, device, allocator, refDstImageCreateInfo, MemoryRequirement::Any));
    const Move<VkImageView> refDstImageView(makeImageView(vk, device, refDstImage->get(),
                                                          mapImageViewType(m_parameters.imageType),
                                                          m_parameters.formatForVerify, subresourceRange));
    const VkImageMemoryBarrier refDstInitImageBarrier =
    ...
    const MovePtr<BufferWithMemory> refDstBuffer = MovePtr<BufferWithMemory>(
        new BufferWithMemory(vk, device, allocator, refDstBufferInfo, MemoryRequirement::HostVisible));

    const VkImageCreateFlags resDstImageCreateFlags = 0;
    const VkImageUsageFlags resDstImageUsageFlags =
        VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
    const VkImageCreateInfo resDstImageCreateInfo =
        makeCreateImageInfo(m_parameters.formatForVerify, m_parameters.imageType, mipmapDims, resDstImageUsageFlags,
                            &resDstImageCreateFlags, SINGLE_LEVEL, SINGLE_LAYER);
    const MovePtr<Image> resDstImage(new Image(vk, device, allocator, resDstImageCreateInfo, MemoryRequirement::Any));
    const Move<VkImageView> resDstImageView(makeImageView(vk, device, resDstImage->get(),
                                                          mapImageViewType(m_parameters.imageType),
                                                          m_parameters.formatForVerify, subresourceRange));
    const VkImageMemoryBarrier resDstInitImageBarrier =
        makeImageMemoryBarrier(0u, VK_ACCESS_SHADER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
    ...
    const MovePtr<BufferWithMemory> resDstBuffer = MovePtr<BufferWithMemory>(
        new BufferWithMemory(vk, device, allocator, resDstBufferInfo, MemoryRequirement::HostVisible));

    const Unique<VkShaderModule> vertShaderModule(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert"), 0));
    const Unique<VkShaderModule> fragShaderModule(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("frag_verify"), 0));
    ...
    const VkDescriptorImageInfo descriptorRefSrcImage(makeDescriptorImageInfo(
        *refSrcSampler, *refSrcImageView,
        layoutShaderReadOnly ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_GENERAL));
    const VkDescriptorImageInfo descriptorResSrcImage(makeDescriptorImageInfo(
    ...
    const VkBufferImageCopy copyBufferToImageRegion = makeBufferImageCopy(
        copyBufferToImageDims.x(), copyBufferToImageDims.y(), 0u, 0u, mipmapDimsBlocked.x(), mipmapDimsBlocked.y());
    ...
    const VkBufferMemoryBarrier refSrcCopyBufferBarrierPre =
        makeBufferMemoryBarrier(VK_ACCESS_HOST_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, refSrcImageBuffer->get(), 0ull,
                                refCompressedData.size());
    const VkImageMemoryBarrier refSrcCopyImageBarrierPre =
        makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
                               refSrcImage->get(), subresourceRange);
    const VkImageMemoryBarrier refSrcCopyImageBarrierPost = makeImageMemoryBarrier(
        VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
        layoutShaderReadOnly ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_GENERAL, refSrcImage->get(),
        subresourceRange);
    const VkImageMemoryBarrier resCompressedImageBarrier = makeImageMemoryBarrier(
        0, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
        layoutShaderReadOnly ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_GENERAL,
    ...
    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, renderSize);
    {
        DescriptorSetUpdateBuilder()
            .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
                         VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &descriptorRefSrcImage)
            .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(1u),
                         VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &descriptorResSrcImage)
            .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(2u),
                         VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &descriptorRefDstImage)
            .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(3u),
                         VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &descriptorResDstImage)
            .update(vk, device);
    ...
    {
        const VkImageMemoryBarrier refDstImageBarrier =
            makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                   VK_IMAGE_LAYOUT_GENERAL, refDstImage->get(), subresourceRange);
    ...
    {
        const VkImageMemoryBarrier resDstImageBarrier =
            makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                                   VK_IMAGE_LAYOUT_GENERAL, resDstImage->get(), subresourceRange);
    ...
    // Compare decompressed pixel data in reference and result images
    {
        ...
        BinaryCompareResult res = BinaryCompare(refDstBufferAlloc.getHostPtr(), resDstBufferAlloc.getHostPtr(),
                                                dstBufferSize, m_parameters.formatForVerify, compareMode);
        ...
        if (res == COMPARE_RESULT_FAILED)
        {
            ...
            tcu::ConstPixelBufferAccess resPixels(mapVkFormat(m_parameters.formatForVerify), renderSize.width,
                                                  renderSize.height, 1u, resDstBufferAlloc.getHostPtr());
            tcu::ConstPixelBufferAccess refPixels(mapVkFormat(m_parameters.formatForVerify), renderSize.width,
                                                  renderSize.height, 1u, refDstBufferAlloc.getHostPtr());

            string comment = string("Image Comparison (level=") + de::toString(level) + string(", layer=") +
                             de::toString(layer) + string(")");
...
void GraphicsTextureTestInstance::transcodeRead(const VkCommandPool &cmdPool)
{
    ...
    const Unique<VkPipelineLayout> pipelineLayout(makePipelineLayout(vk, device, *descriptorSetLayout));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertShaderModule,
                                                           *fragShaderModule, renderSizeUnused, 0u, true));
    ...
    for (uint32_t levelNdx = 0; levelNdx < getLevelCount(); ++levelNdx)
    {
        ...
        for (uint32_t layerNdx = 0; layerNdx < getLayerCount(); ++layerNdx)
        {
            ...
            const VkImageMemoryBarrier srcCopyImageBarrierPost = makeImageMemoryBarrier(
                ...
                VK_IMAGE_LAYOUT_GENERAL, srcImage->get(), srcSubresourceRange);
            const VkBufferImageCopy dstCopyRegion = makeBufferImageCopy(dstImageResolution.x(), dstImageResolution.y());
            const VkImageMemoryBarrier dstInitImageBarrier = makeImageMemoryBarrier(
                0u, (VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT), VK_IMAGE_LAYOUT_UNDEFINED,
                VK_IMAGE_LAYOUT_GENERAL, dstImage->get(), dstSubresourceRange);
...
{
public:
    TexelViewCompatibleCase(TestContext &testCtx, const std::string &name, const TestParameters &parameters);
...
TexelViewCompatibleCase::TexelViewCompatibleCase(TestContext &testCtx, const std::string &name,
...
void TexelViewCompatibleCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    const unsigned int imageTypeIndex =
        (m_parameters.imageType == IMAGE_TYPE_2D) + (m_parameters.imageType == IMAGE_TYPE_3D) * 2;
    ...
    case SHADER_TYPE_COMPUTE:
    {
        ...
        const std::string imageTypeStr =
            getShaderImageType(mapVkFormat(m_parameters.formatUncompressed), m_parameters.imageType) + imageTypeSuffix;
        const std::string samplerTypeStr =
            getGlslSamplerType(mapVkFormat(m_parameters.formatUncompressed), mapImageViewType(m_parameters.imageType)) +
            imageTypeSuffix;
        const std::string formatQualifierStr =
            getShaderImageFormatQualifier(mapVkFormat(m_parameters.formatUncompressed));
        ...
        case OPERATION_IMAGE_LOAD:
        {
            ...
            src << "layout (binding = 0, " << formatQualifierStr << ") readonly uniform " << imageTypeStr
                << " u_image0;\n"
                << "layout (binding = 1, " << formatQualifierStr << ") writeonly uniform " << imageTypeStr
                << " u_image1;\n\n"
                << "void main (void)\n"
                << "{\n"
                << posDefinitions[imageTypeIndex] << "    imageStore(u_image1, pos, imageLoad(u_image0, pos));\n"
                << "}\n";
        ...
        case OPERATION_TEXTURE:
        {
            ...
            src << "layout (binding = 0) uniform " << samplerTypeStr << " u_image0;\n"
                ...
                << coordDefinitions[imageTypeIndex] << "    imageStore(u_image1, pos, texture(u_image0, coord));\n"
                << "}\n";
        ...
        case OPERATION_IMAGE_STORE:
        {
            ...
            src << "layout (binding = 0, " << formatQualifierStr << ") uniform " << imageTypeStr
                << "           u_image0;\n"
                << "layout (binding = 1, " << formatQualifierStr << ") readonly uniform " << imageTypeStr
                << "  u_image1;\n"
                << "layout (binding = 2, " << formatQualifierStr << ") writeonly uniform " << imageTypeStr
                << " u_image2;\n\n"
                << "void main (void)\n"
                << "{\n"
                << posDefinitions[imageTypeIndex] << "    imageStore(u_image0, pos, imageLoad(u_image1, pos));\n"
        ...
        const ImageType compressedReferenceImageType =
            (m_parameters.imageType == IMAGE_TYPE_2D && m_parameters.layers > 1u) ? IMAGE_TYPE_2D_ARRAY :
                                                                                    m_parameters.imageType;
        const char *coordDefinitions[3] = {
            // IMAGE_TYPE_1D
        ...
        const auto compressedResSamplerType =
            getGlslSamplerType(mapVkFormat(m_parameters.formatForVerify), mapImageViewType(m_parameters.imageType)) +
            imageTypeSuffix;
        const auto compressedRefSamplerType = getGlslSamplerType(mapVkFormat(m_parameters.formatForVerify),
                                                                 mapImageViewType(compressedReferenceImageType));
        const auto decompressedImageType =
            getShaderImageType(mapVkFormat(m_parameters.formatForVerify), m_parameters.imageType) + imageTypeSuffix;
        ...
        src_decompress << "layout (binding = 0) uniform " << compressedResSamplerType << " compressed_result;\n"
                       ...
                       << "layout (binding = 2, "
                       << getShaderImageFormatQualifier(mapVkFormat(m_parameters.formatForVerify))
                       << ") writeonly uniform " << decompressedImageType << " decompressed_result;\n"
                       << "layout (binding = 3, "
                       << getShaderImageFormatQualifier(mapVkFormat(m_parameters.formatForVerify))
                       << ") writeonly uniform " << decompressedImageType << " decompressed_reference;\n"
        ...
        if (compressedReferenceImageType == IMAGE_TYPE_2D_ARRAY && !m_parameters.useMultiLayerViews())
            src_decompress << "    imageStore(decompressed_reference, pos, textureLod(compressed_reference, "
                              "vec3(coord, layer), level));\n";
        else
    ...
    {
        ImageType imageTypeForFS =
            (m_parameters.imageType == IMAGE_TYPE_2D_ARRAY) ? IMAGE_TYPE_2D : m_parameters.imageType;
        ...
        // Fragment shader
        {
            ...
            case OPERATION_TEXTURE_WRITE:
            {
                ...
                const std::string srcSamplerTypeStr =
                    getGlslSamplerType(mapVkFormat(m_parameters.formatUncompressed), mapImageViewType(imageTypeForFS));
                const std::string dstImageTypeStr =
                    getShaderImageType(mapVkFormat(m_parameters.formatUncompressed), imageTypeForFS);
        ...
        // Verification fragment shader
        {
            ...
            const std::string samplerType =
                getGlslSamplerType(mapVkFormat(m_parameters.formatForVerify), mapImageViewType(imageTypeForFS));
            const std::string imageTypeStr =
                getShaderImageType(mapVkFormat(m_parameters.formatForVerify), imageTypeForFS);
            const std::string formatQualifierStr =
                getShaderImageFormatQualifier(mapVkFormat(m_parameters.formatForVerify));
            ...
            src << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n\n"
                ...
                << getGlslSamplerType(mapVkFormat(m_parameters.formatForVerify),
                                      mapImageViewType(m_parameters.imageType))
                << " u_imageIn0;\n"
                ...
                << getGlslSamplerType(mapVkFormat(m_parameters.formatForVerify),
                                      mapImageViewType(m_parameters.imageType))
                << " u_imageIn1;\n"
                << "layout (binding = 2, " << getShaderImageFormatQualifier(mapVkFormat(m_parameters.formatForVerify))
                << ") writeonly uniform "
                << getShaderImageType(mapVkFormat(m_parameters.formatForVerify), m_parameters.imageType)
                << " u_imageOut0;\n"
                << "layout (binding = 3, " << getShaderImageFormatQualifier(mapVkFormat(m_parameters.formatForVerify))
                << ") writeonly uniform "
                << getShaderImageType(mapVkFormat(m_parameters.formatForVerify), m_parameters.imageType)
                << " u_imageOut1;\n"
                << "\n"
...
void TexelViewCompatibleCase::checkSupport(Context &context) const
{
...

    {
        ...
        if (vk.getPhysicalDeviceImageFormatProperties(physicalDevice, m_parameters.formatUncompressed,
                                                      ...
                                                      m_parameters.uncompressedImageUsage, 0u,
        ...
        if (VK_ERROR_FORMAT_NOT_SUPPORTED ==
            vk.getPhysicalDeviceImageFormatProperties(
                physicalDevice, m_parameters.formatCompressed, mapImageType(m_parameters.imageType),
                VK_IMAGE_TILING_OPTIMAL, m_parameters.compressedImageUsage,
                VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT | VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT |
...

    {
        ...
        if (deInRange32(m_parameters.formatCompressed, VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK,
                        VK_FORMAT_EAC_R11G11_SNORM_BLOCK) &&
            !physicalDeviceFeatures.textureCompressionETC2)
            TCU_THROW(NotSupportedError, "textureCompressionETC2 not supported");

        if (m_parameters.formatIsASTC() && !physicalDeviceFeatures.textureCompressionASTC_LDR)
        ...
        {
            const VkFormatProperties p =
                getPhysicalDeviceFormatProperties(vk, physicalDevice, m_parameters.formatUncompressed);
...
static tcu::UVec3 getUnniceResolution(const VkFormat format, const uint32_t layers)
{
    const uint32_t unniceMipmapTextureSize[] = {1, 1, 1, 8, 22, 48, 117, 275, 604, 208, 611, 274, 1211};
    const uint32_t baseTextureWidth          = unniceMipmapTextureSize[getBlockWidth(format)];
    const uint32_t baseTextureHeight         = unniceMipmapTextureSize[getBlockHeight(format)];
    const uint32_t baseTextureWidthLevels    = deLog2Floor32(baseTextureWidth);
    const uint32_t baseTextureHeightLevels   = deLog2Floor32(baseTextureHeight);
    const uint32_t widthMultiplier           = (baseTextureHeightLevels > baseTextureWidthLevels) ?
                                                   1u << (baseTextureHeightLevels - baseTextureWidthLevels) :
                                                   1u;
    const uint32_t heightMultiplier          = (baseTextureWidthLevels > baseTextureHeightLevels) ?
                                                   1u << (baseTextureWidthLevels - baseTextureHeightLevels) :
                                                   1u;
    const uint32_t width                     = baseTextureWidth * widthMultiplier;
    const uint32_t height                    = baseTextureHeight * heightMultiplier;
...
tcu::TestCaseGroup *createImageCompressionTranscodingTests(tcu::TestContext &testCtx)
{
    ...
    const std::string operationName[OPERATION_LAST] = {
        "image_load",      "texel_fetch",      "texture",      "image_store",
        "attachment_read", "attachment_write", "texture_read", "texture_write",
    };
    ...
    const VkImageUsageFlags compressedImageUsageFlags[OPERATION_LAST] = {
        ...
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT), // "texel_fetch"
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT), // "texture"
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT), // "image_store"
        baseImageUsageFlagSet | static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT |
                                                                  VK_IMAGE_USAGE_SAMPLED_BIT), // "attachment_read"
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT |
                                              VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT),            // "attachment_write"
        baseImageUsageFlagSet | static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT), // "texture_read"
        baseImageUsageFlagSet | static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT |
                                                                  VK_IMAGE_USAGE_STORAGE_BIT), // "texture_write"
    };
    ...
    const VkImageUsageFlags uncompressedImageUsageFlags[OPERATION_LAST] = {
        ...
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_STORAGE_BIT), //"image_store"
        baseImageUsageFlagSet | static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT |
                                                                  VK_IMAGE_USAGE_SAMPLED_BIT), //"attachment_read"
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT), //"attachment_write"
        baseImageUsageFlagSet |
            static_cast<VkImageUsageFlagBits>(VK_IMAGE_USAGE_STORAGE_BIT | VK_IMAGE_USAGE_SAMPLED_BIT), //"texture_read"
    ...
    const VkFormat compressedFormats64bit[] = {
        ...
        VK_FORMAT_BC1_RGBA_SRGB_BLOCK,      VK_FORMAT_BC4_UNORM_BLOCK,        VK_FORMAT_BC4_SNORM_BLOCK,
        VK_FORMAT_ETC2_R8G8B8_UNORM_BLOCK,  VK_FORMAT_ETC2_R8G8B8_SRGB_BLOCK, VK_FORMAT_ETC2_R8G8B8A1_UNORM_BLOCK,
        VK_FORMAT_ETC2_R8G8B8A1_SRGB_BLOCK, VK_FORMAT_EAC_R11_UNORM_BLOCK,    VK_FORMAT_EAC_R11_SNORM_BLOCK,
    };
    ...
    const FormatsArray formatsCompressedSets[] = {
        {compressedFormats64bit, DE_LENGTH_OF_ARRAY(compressedFormats64bit)},
        {compressedFormats128bit, DE_LENGTH_OF_ARRAY(compressedFormats128bit)},
    };
    ...
    const FormatsArray formatsUncompressedSets[] = {
        {uncompressedFormats64bit, DE_LENGTH_OF_ARRAY(uncompressedFormats64bit)},
        {uncompressedFormats128bit, DE_LENGTH_OF_ARRAY(uncompressedFormats128bit)},
    };
    ...
    {
        MovePtr<tcu::TestCaseGroup> pipelineTypeGroup(
            new tcu::TestCaseGroup(testCtx, pipelineName[shaderType].c_str()));
        ...
        for (int mipmapTestNdx = 0; mipmapTestNdx < DE_LENGTH_OF_ARRAY(mipmapness); mipmapTestNdx++)
        {
            ...
            MovePtr<tcu::TestCaseGroup> mipmapTypeGroup(
                new tcu::TestCaseGroup(testCtx, mipmanpnessName[mipmapTestNdx].c_str()));

            for (int imageTypeNdx = 0; imageTypeNdx < DE_LENGTH_OF_ARRAY(imageTypes); imageTypeNdx++)
            {
                MovePtr<tcu::TestCaseGroup> imageTypeGroup(
                    new tcu::TestCaseGroup(testCtx, imageTypes[imageTypeNdx].name.c_str()));
                ...
                for (int operationNdx = OPERATION_IMAGE_LOAD; operationNdx < OPERATION_LAST; ++operationNdx)
                {
                    if (shaderType != SHADER_TYPE_FRAGMENT &&
                        deInRange32(operationNdx, OPERATION_ATTACHMENT_READ, OPERATION_TEXTURE_WRITE))
                        continue;

                    if (shaderType != SHADER_TYPE_COMPUTE &&
                        deInRange32(operationNdx, OPERATION_IMAGE_LOAD, OPERATION_IMAGE_STORE))
                        continue;
                    ...
                    uint32_t depth      = 1u + 2 * (imageType == IMAGE_TYPE_3D);
                    uint32_t imageCount = 2u + (operationNdx == OPERATION_IMAGE_STORE);

                    // Iterate through bitness groups (64 bit, 128 bit, etc)
                    for (uint32_t formatBitnessGroup = 0;
                         formatBitnessGroup < DE_LENGTH_OF_ARRAY(formatsCompressedSets); ++formatBitnessGroup)
                    {
                        for (uint32_t formatCompressedNdx = 0;
                             formatCompressedNdx < formatsCompressedSets[formatBitnessGroup].count;
                             ++formatCompressedNdx)
                        {
                            const VkFormat formatCompressed =
                                formatsCompressedSets[formatBitnessGroup].formats[formatCompressedNdx];
                            const std::string compressedFormatGroupName = getFormatShortString(formatCompressed);
                            MovePtr<tcu::TestCaseGroup> compressedFormatGroup(
                                new tcu::TestCaseGroup(testCtx, compressedFormatGroupName.c_str()));

                            for (uint32_t formatUncompressedNdx = 0;
                                 formatUncompressedNdx < formatsUncompressedSets[formatBitnessGroup].count;
                                 ++formatUncompressedNdx)
                            {
                                const VkFormat formatUncompressed =
                                    formatsUncompressedSets[formatBitnessGroup].formats[formatUncompressedNdx];
                                const std::string uncompressedFormatGroupName =
                                    getFormatShortString(formatUncompressed);
                                ...
                                const TestParameters parameters = {
                                    ...
                                    1u + 2u * mipmapTest *
                                             (imageType !=
                                              IMAGE_TYPE_3D), // 1 or 3 if mipmapTest is true but image is not 3d
                                    imageType,
                                    formatCompressed,
                                    formatUncompressed,
                                    imageCount,
                                ...
                                compressedFormatGroup->addChild(
                                    new TexelViewCompatibleCase(testCtx, uncompressedFormatGroupName, parameters));
                            }
...
#ifndef CTS_USES_VULKANSC
    {
        MovePtr<tcu::TestCaseGroup> multiLayerGroup(new tcu::TestCaseGroup(
            testCtx, "multi_layer_views", "Texel view compatible cases using multi-layer image views"));
        ...
        for (int operationNdx = OPERATION_IMAGE_LOAD; operationNdx <= OPERATION_IMAGE_STORE; ++operationNdx)
        {
            ...
            const uint32_t imageCount = 2u + (operationNdx == OPERATION_IMAGE_STORE);

            // Iterate through bitness groups (64 bit, 128 bit, etc)
            for (uint32_t formatBitnessGroup = 0; formatBitnessGroup < DE_LENGTH_OF_ARRAY(formatsCompressedSets);
                 ++formatBitnessGroup)
            {
                for (uint32_t formatCompressedNdx = 0;
                     formatCompressedNdx < formatsCompressedSets[formatBitnessGroup].count; ++formatCompressedNdx)
                {
                    const VkFormat formatCompressed =
                        formatsCompressedSets[formatBitnessGroup].formats[formatCompressedNdx];
                    const std::string compressedFormatGroupName = getFormatShortString(formatCompressed);
