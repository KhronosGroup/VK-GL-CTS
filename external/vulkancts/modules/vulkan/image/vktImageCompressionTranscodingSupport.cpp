...
protected:
    ...
    void copyDataToImage(const VkCommandBuffer &cmdBuffer, ImageData &imageData, const vector<UVec3> &mipMapSizes,
                         const bool isCompressed);
    virtual void executeShader(const VkCommandBuffer &cmdBuffer, const VkDescriptorSetLayout &descriptorSetLayout,
                               const VkDescriptorPool &descriptorPool, vector<ImageData> &imageData);
    bool copyResultAndCompare(const VkCommandBuffer &cmdBuffer, const VkImage &uncompressed, const VkDeviceSize offset,
                              const UVec3 &size);
    ...
    bool decompressImage(const VkCommandBuffer &cmdBuffer, vector<ImageData> &imageData,
...
void BasicComputeTestInstance::copyDataToImage(const VkCommandBuffer &cmdBuffer, ImageData &imageData,
                                               const vector<UVec3> &mipMapSizes, const bool isCompressed)
{
    ...
    Buffer imageBuffer(vk, device, allocator, makeBufferCreateInfo(m_data.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT),
...
bool BasicComputeTestInstance::copyResultAndCompare(const VkCommandBuffer &cmdBuffer, const VkImage &uncompressed,
                                                    const VkDeviceSize offset, const UVec3 &size)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkQueue queue       = m_context.getUniversalQueue();
    const VkDevice device     = m_context.getDevice();
                                                ...
                                                const bool isCompressed)
{
    ...
    if (isCompressed)
    {
        ...
        const VkImageCreateInfo compressedInfo = {
            ...
            DE_NULL,                             // const void* pNext;
            VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT | VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT_KHR |
...
bool BasicComputeTestInstance::decompressImage(const VkCommandBuffer &cmdBuffer, vector<ImageData> &imageData,
                                               const vector<UVec3> &mipMapSizes)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
...
void GraphicsAttachmentsTestInstance::transcodeRead()
{
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertShaderModule,
                                                           *fragShaderModule, renderSizeDummy, 1u, true));
                                                                       ...
                                                                       const uint32_t levels, const uint32_t layers)
{
    ...
    const VkImageCreateFlags imageCreateFlagsAddOn =
        isCompressedFormat(format) ?
            VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT_KHR | VK_IMAGE_CREATE_EXTENDED_USAGE_BIT_KHR :
            0;
                                                          ...
                                                          const UVec3 &mipmapDims)
{
    ...
    const MovePtr<Buffer> refSrcImageBuffer =
        MovePtr<Buffer>(new Buffer(vk, device, allocator, refSrcImageBufferInfo, MemoryRequirement::HostVisible));
    ...
    const MovePtr<Buffer> refDstBuffer =
        MovePtr<Buffer>(new Buffer(vk, device, allocator, refDstBufferInfo, MemoryRequirement::HostVisible));
    ...
    const MovePtr<Buffer> resDstBuffer =
        MovePtr<Buffer>(new Buffer(vk, device, allocator, resDstBufferInfo, MemoryRequirement::HostVisible));
...
void GraphicsTextureTestInstance::transcodeRead()
{
    ...
    const Unique<VkPipelineLayout> pipelineLayout(makePipelineLayout(vk, device, *descriptorSetLayout));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertShaderModule,
                                                           *fragShaderModule, renderSizeDummy, 0u, true));
...

    {
        ...
        if (VK_ERROR_FORMAT_NOT_SUPPORTED ==
            vk.getPhysicalDeviceImageFormatProperties(
                ...
                VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT_KHR | VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT |
