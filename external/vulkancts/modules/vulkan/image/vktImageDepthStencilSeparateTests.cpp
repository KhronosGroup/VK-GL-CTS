...
struct TestParams
{
	...
	bool                  dynamicStencilRef; // Set StencilRefEXT from the shader; used when writeAspect == VK_IMAGE_ASPECT_STENCIL_BIT and writeMechanism == TEST_*.
	...
	{
		// Some bit shuffling.
		return	((static_cast<uint32_t>(imageFormat)                 << 16) |
				 (static_cast<uint32_t>(writeAspect)                 <<  8) |
				 (static_cast<uint32_t>(writeMechanism)              <<  4) |
				 (static_cast<uint32_t>(generalLayout ? 1u : 0u)     <<  3) |
	...
	VkImageLayout getStencilImageLayout (void) const
	{
		...
		DE_ASSERT(separateLayouts);
		return (readsStencil() ? VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_STENCIL_ATTACHMENT_OPTIMAL);
	}
...
struct VertexData
{
	...
	static std::vector<VkVertexInputBindingDescription> getBindingDescriptions (void)
	{
		...
		descriptions.emplace_back(makeVertexInputBindingDescription(0u, static_cast<uint32_t>(sizeof(VertexData)), VK_VERTEX_INPUT_RATE_VERTEX));
		return descriptions;
	}
	...
	static std::vector<VkVertexInputAttributeDescription> getAttributeDescriptions (void)
	{
		...
		descriptions.emplace_back(makeVertexInputAttributeDescription(0u, 0u, VK_FORMAT_R32G32B32A32_SFLOAT, static_cast<uint32_t>(offsetof(VertexData, coords))));
		descriptions.emplace_back(makeVertexInputAttributeDescription(1u, 0u, VK_FORMAT_R32G32B32A32_SFLOAT, static_cast<uint32_t>(offsetof(VertexData, color))));
		descriptions.emplace_back(makeVertexInputAttributeDescription(2u, 0u, VK_FORMAT_R32G32B32A32_SINT,   static_cast<uint32_t>(offsetof(VertexData, extra))));
		return descriptions;
	}
};
...
VkImageCreateInfo makeImageCreateInfo (VkFormat format, VkExtent3D extent, VkSampleCountFlagBits sampleCount, VkImageUsageFlags usage)
{
...
void DepthStencilSeparateCase::checkSupport (Context& context) const
{
	...
	for (const auto& keyValue : usageCases)
	{
		...
		const auto result = ctx.vki.getPhysicalDeviceImageFormatProperties2(ctx.physicalDevice, &imgFormatInfo, &imgFormatProperties);
		...
		{
			TCU_THROW(NotSupportedError, "Format not supported or usage not supported for this format with sample count " + std::to_string(sampleCount));
		}
		VK_CHECK(result);

		if ((imgFormatProperties.imageFormatProperties.sampleCounts & dsCreateInfo.samples) != dsCreateInfo.samples)
			TCU_THROW(NotSupportedError, "Sample count " + std::to_string(sampleCount) + " not supported for this format");
	}

	ctx.vki.getPhysicalDeviceFormatProperties2(ctx.physicalDevice, m_params.imageFormat, &fp2);

	const auto requiredFeatures = ( VK_FORMAT_FEATURE_2_DEPTH_STENCIL_ATTACHMENT_BIT
...
Move<VkRenderPass> makeSeparateRenderPass (const DeviceInterface& vkd, VkDevice device, const TestParams& params)
{
	...
	attDescs.emplace_back(AttachmentDescription{
		...
		(isMultisample ? VK_ATTACHMENT_STORE_OP_DONT_CARE : VK_ATTACHMENT_STORE_OP_STORE),	//	VkAttachmentStoreOp				storeOp;
	...
	if (isMultisample || separateLayouts)
	{
		...
		const auto stencilResolveMode = (params.readsStencil() ? VK_RESOLVE_MODE_NONE : VK_RESOLVE_MODE_SAMPLE_ZERO_BIT);
		...
		const VkSubpassDescriptionDepthStencilResolve dsResolve =
		{
			...
			(isMultisample ? &references.at(3u) : nullptr),					//	const VkAttachmentReference2*	pDepthStencilResolveAttachment;
		};
#endif // CTS_USES_VULKANSC
...
float getDepthThreshold (const tcu::TextureFormat& format)
{
	...
	if (format.type == tcu::TextureFormat::UNORM_INT16)
		...
		return static_cast<float>(1.0 / 33554431.0); // D32: This could be exact, but lets simply make it a bit stricter than 24 bits.
	else
		DE_ASSERT(false);
...
tcu::TestStatus DepthStencilSeparateInstance::iterate (void)
{
	...
	const auto readLayout       = (m_params.separateLayouts ? (m_params.readsDepth() ? m_params.getDepthImageLayout() : m_params.getStencilImageLayout()) : m_params.getImageLayout());
	...
	const auto getFramebufferCoord =
		[](uint32_t c, uint32_t total) {
	...
	for (uint32_t y = 0u; y < kFramebufferDim; ++y)
		for (uint32_t x = 0u; x < kFramebufferDim; ++x)
		{
			...
			vertices.emplace_back(tcu::Vec4(xCoord, yCoord, depth, 1.0f), tcu::Vec4(r, g, b, a), tcu::IVec4(stencil, 0, 0, 0));
		}
	...
	ImageWithBuffer colorBuffer (ctx.vkd, ctx.device, ctx.allocator, vkExtent, kColorFormat, colorUsage, VK_IMAGE_TYPE_2D, colorSRR, 1u, kSingleSample);

	// Depth/stencil buffer.
	const auto dsSRR        = makeImageSubresourceRange((VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT), 0u, 1u, 0u, 1u);
	...
	if (m_params.isMultisample())
	{
		...
		colorMSBuffer.reset(new ImageWithMemory(ctx.vkd, ctx.device, ctx.allocator, colorMSCreateInfo, MemoryRequirement::Any));
		...
		dsMSBuffer.reset(new ImageWithMemory(ctx.vkd, ctx.device, ctx.allocator, dsMSCreateInfo, MemoryRequirement::Any));
	...
	const auto borderColor = (m_params.readsDepth() ? VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE : VK_BORDER_COLOR_INT_OPAQUE_WHITE);
	...
	ImageWithBuffer storageImg (ctx.vkd, ctx.device, ctx.allocator, vkExtent, storageImgFormat, storageImgUsage, VK_IMAGE_TYPE_2D, colorSRR, 1u, kSingleSample);
	...
	setUpdateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u), VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &sampledImgInfo);
	setUpdateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(1u), VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, &storageImgInfo);
	...
	const auto framebuffer = makeFramebuffer(ctx.vkd, ctx.device, *renderPass, de::sizeU32(attachments), de::dataOrNull(attachments), vkExtent.width, vkExtent.height);
	...
	const auto stencilPassOp      = (stencilTestEnabled ? VK_STENCIL_OP_REPLACE : VK_STENCIL_OP_KEEP); // VUID-vkCmdDraw-None-06887.
	const auto stencilState       = makeStencilOpState(VK_STENCIL_OP_KEEP, stencilPassOp, VK_STENCIL_OP_KEEP, VK_COMPARE_OP_ALWAYS, 0xFFu, 0xFFu, 0u);
	...
	const auto pipeline = makeGraphicsPipeline(ctx.vkd, ctx.device, *pipelineLayout,
		...
		*renderPass, viewports, scissors, topology, 0u, 0u,
	...
	BufferWithMemory preFillDepthBuffer   (ctx.vkd, ctx.device, ctx.allocator, preFillDepthBufferInfo, MemoryRequirement::HostVisible);
	BufferWithMemory prefillStencilBuffer (ctx.vkd, ctx.device, ctx.allocator, preFillStencilBufferInfo, MemoryRequirement::HostVisible);
	...
	const auto stencilVerifBufferCreateInfo = makeBufferCreateInfo(stencilCopyBufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT);

	BufferWithMemory depthVerifBuffer   (ctx.vkd, ctx.device, ctx.allocator, depthVerifBufferCreateInfo, MemoryRequirement::HostVisible);
	BufferWithMemory stencilVerifBuffer (ctx.vkd, ctx.device, ctx.allocator, stencilVerifBufferCreateInfo, MemoryRequirement::HostVisible);
	...
	const auto preCopyLayout         = (m_params.generalLayout ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
	const auto preFillPrepareBarrier = makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, dsLayout, preCopyLayout, *dsBuffer, dsSRR);
	cmdPipelineImageMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, &preFillPrepareBarrier);
	...
	if (m_params.separateLayouts)
	{
		...
		const auto stencilAccess  = (m_params.readsStencil()
									? (shaderAccesses | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT/*load_op_load*/)
									: (VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT));
		const auto stencilStages  = (m_params.readsStencil()
									? (VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT)
									: (VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT));

		const auto depthBarrier   = makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, depthAccess, dsLayout, m_params.getDepthImageLayout(), *dsBuffer, depthSRR);
		const auto stencilBarrier = makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, stencilAccess, dsLayout, m_params.getStencilImageLayout(), *dsBuffer, stencilSRR);

		const std::vector<VkImageMemoryBarrier> barriers { depthBarrier, stencilBarrier };
		cmdPipelineImageMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, (depthStages | stencilStages), de::dataOrNull(barriers), barriers.size());
		dsLayout = VK_IMAGE_LAYOUT_UNDEFINED; // Lets make it clear there's not a single dsLayout for now.
	}
	else
	{
		const auto rpAccesses       = (m_params.isMultisample()
									? (VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | shaderAccesses)
									: (VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | shaderAccesses));
		const auto rpStages         = (m_params.isMultisample()
		...
		const auto rpPrepareBarrier = makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, rpAccesses, dsLayout, readLayout, *dsBuffer, dsSRR);
	...
	if (m_params.isMultisample())
	{
		...
		const auto msAccesses       = (VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT | shaderAccesses);
		const auto msStages         = (VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
		const auto msPrepareBarrier = makeImageMemoryBarrier(0u, msAccesses, VK_IMAGE_LAYOUT_UNDEFINED, readLayout, dsMSBuffer->get(), dsSRR);
		cmdPipelineImageMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, msStages, &msPrepareBarrier);
	}

	// Transition storage image to the general layout to be used in the frag shader.
	const auto storagePrepareBarrier = makeImageMemoryBarrier(0u, shaderAccesses, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, storageImg.getImage(), colorSRR);
	cmdPipelineImageMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, &storagePrepareBarrier);
	...
	ctx.vkd.cmdBindVertexBuffers(cmdBuffer, 0u, de::sizeU32(bindingDescs), de::dataOrNull(vertexBuffers), de::dataOrNull(vertexBufferOffsets));
	if (singlePointDraws)
	{
		for (size_t i = 0u; i < vertices.size(); ++i)
		{
			ctx.vkd.cmdSetStencilReference(cmdBuffer, VK_STENCIL_FACE_FRONT_AND_BACK, static_cast<uint32_t>(vertices.at(i).extra.x()));
	...
	const auto preVerifLayout = (m_params.generalLayout ? VK_IMAGE_LAYOUT_GENERAL : VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);

	// Color and storage preparation.
	preCopyBarriers.emplace_back(makeImageMemoryBarrier(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, colorBuffer.getImage(), colorSRR));
	preCopyBarriers.emplace_back(makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL, storageImg.getImage(), colorSRR));
	...
	if (m_params.separateLayouts)
	{
		...
		preCopyBarriers.emplace_back(makeImageMemoryBarrier(depthAccess, VK_ACCESS_TRANSFER_READ_BIT, m_params.getDepthImageLayout(), preVerifLayout, *dsBuffer, depthSRR));
		preCopyBarriers.emplace_back(makeImageMemoryBarrier(stencilAccess, VK_ACCESS_TRANSFER_READ_BIT, m_params.getStencilImageLayout(), preVerifLayout, *dsBuffer, stencilSRR));
	}
	else
	{
		const auto prevAccess   = (m_params.isMultisample()
								? VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT // Depth/stencil resolve happens with this access.
								: VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT);

		preCopyBarriers.emplace_back(makeImageMemoryBarrier(prevAccess, VK_ACCESS_TRANSFER_READ_BIT, dsLayout, preVerifLayout, *dsBuffer, dsSRR));
	}

	const auto writeStages = (VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
	cmdPipelineImageMemoryBarrier(ctx.vkd, cmdBuffer, writeStages, VK_PIPELINE_STAGE_TRANSFER_BIT, de::dataOrNull(preCopyBarriers), preCopyBarriers.size());
	...
	ctx.vkd.cmdCopyImageToBuffer(cmdBuffer, storageImg.getImage(), VK_IMAGE_LAYOUT_GENERAL, storageImg.getBuffer(), 1u, &colorCopyRegion);
	...
	tcu::ConstPixelBufferAccess colorAccess   (mapVkFormat(kColorFormat), fbExtent, colorBuffer.getBufferAllocation().getHostPtr());
	tcu::ConstPixelBufferAccess	storageAccess (storageTcuFormat, fbExtent, storageImg.getBufferAllocation().getHostPtr());
	tcu::ConstPixelBufferAccess depthAccess   (depthCopyFormat, fbExtent, depthVerifBuffer.getAllocation().getHostPtr());
	tcu::ConstPixelBufferAccess stencilAccess (stencilCopyFormat, fbExtent, stencilVerifBuffer.getAllocation().getHostPtr());
	...
	for (int y = 0; y < fbExtent.y(); ++y)
		for (int x = 0; x < fbExtent.x(); ++x)
		{
			...
			const float depth = (m_params.writesDepth() && m_params.writeMechanism == WriteMechanism::RP_CLEAR ? 0.0f : vertex.coords.z());
			refDepthAccess.setPixDepth(depth, x, y);

			const int stencil = (m_params.writesStencil() && m_params.writeMechanism == WriteMechanism::RP_CLEAR ? 0 : vertex.extra.x());
	...
	bool colorOK = tcu::floatThresholdCompare(log, "ColorBuffer", "", refColorAccess, colorAccess, tcu::Vec4(kColorThreshold, kColorThreshold, kColorThreshold, 0.0f), tcu::COMPARE_LOG_EVERYTHING);

	bool storageOK	= (m_params.readsDepth()
					? tcu::floatThresholdCompare(log, "StorageBuffer", "", refStorageAccess, storageAccess, tcu::Vec4(depthThreshold, 0.0f, 0.0f, 0.0f), tcu::COMPARE_LOG_EVERYTHING)
					: tcu::intThresholdCompare(log, "StorageBuffer", "", refStorageAccess, storageAccess, tcu::UVec4(0u, 0u, 0u, 0u), tcu::COMPARE_LOG_EVERYTHING));

	bool depthOK = (m_params.readsDepth() || // In this case the depth values will be verified through the storage image.
					(m_params.writesDepth() && m_params.writeMechanism == WriteMechanism::RP_DONT_CARE) ||
					tcu::dsThresholdCompare(log, "DepthBuffer", "", refDepthAccess, depthAccess, depthThreshold, tcu::COMPARE_LOG_EVERYTHING));

	bool stencilOK = (m_params.readsStencil() || // In this case the stencil values will be verified through the storage image.
					  (m_params.writesStencil() && m_params.writeMechanism == WriteMechanism::RP_DONT_CARE) ||
					  tcu::dsThresholdCompare(log, "DepthBuffer", "", refStencilAccess, stencilAccess, stencilThreshold, tcu::COMPARE_LOG_EVERYTHING));
			...
			for (const auto& writeMechanism : writeMechanisms)
				for (const auto& generalLayout : { false, true })
					for (const auto& separateLayouts : { false, true })
						...
						{
							if (dynamicStencilRef && (writeAspect.aspect != VK_IMAGE_ASPECT_STENCIL_BIT || !writesWithTest(writeMechanism.mechanism)))
								continue;
							...
							const auto testName      = writeAspect.name + std::string("_") + writeMechanism.name + layoutSuffix + slSuffix + stencilSuffix;
