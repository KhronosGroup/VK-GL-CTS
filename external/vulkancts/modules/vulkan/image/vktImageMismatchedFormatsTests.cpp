...
{
public:
    MismatchedFormatTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         const TestType type, const VkFormat format, const std::string &spirvFormat);
...
MismatchedFormatTest::MismatchedFormatTest(tcu::TestContext &testCtx, const std::string &name,
                                           const std::string &description, const TestType type, const VkFormat format,
...
tcu::TestStatus MismatchedFormatTestInstance::iterate(void)
{
    ...
    VkImageCreateFlags imageFlag = m_type == TestType::SPARSE_READ ?
    ...
    {
        storageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vk, device, *storageImage), MemoryRequirement::Any);
        VK_CHECK(
    ...
    Move<VkImageView> storageImageView =
        makeImageView(vk, device, *storageImage, VK_IMAGE_VIEW_TYPE_2D, m_format,
...
tcu::TestCaseGroup *createImageMismatchedFormatsTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(new tcu::TestCaseGroup(
        testCtx, "mismatched_formats", "Test image load/store operations on mismatched formats"));
    de::MovePtr<tcu::TestCaseGroup> testGroupOpRead(
        new tcu::TestCaseGroup(testCtx, "image_read", "perform OpImageRead"));
    de::MovePtr<tcu::TestCaseGroup> testGroupOpWrite(
        new tcu::TestCaseGroup(testCtx, "image_write", "perform OpImageWrite"));
    de::MovePtr<tcu::TestCaseGroup> testGroupOpSparseRead(
        new tcu::TestCaseGroup(testCtx, "sparse_image_read", "perform OpSparseImageRead"));
