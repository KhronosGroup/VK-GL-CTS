...
tcu::TestCaseGroup *createImageExtendedUsageBitTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> imageExtendedUsageBitTests(
        new tcu::TestCaseGroup(testCtx, "extended_usage_bit_compatibility",
                               "VK_IMAGE_CREATE_EXTENDED_USAGE_BIT tests to check format compatibility"));
    de::MovePtr<tcu::TestCaseGroup> getPhysicalDeviceImageFormatPropertiesTests(
        new tcu::TestCaseGroup(testCtx, "image_format_properties", "vkGetPhysicalDeviceImageFormatProperties() tests"));
    de::MovePtr<tcu::TestCaseGroup> getPhysicalDeviceImageFormatProperties2Tests(new tcu::TestCaseGroup(
        testCtx, "image_format_properties2", "vkGetPhysicalDeviceImageFormatProperties2() tests"));
         ...
         imageFormat          = (VkFormat)(imageFormat + 1))
    {
        for (unsigned tilingNdx = 0; tilingNdx < DE_LENGTH_OF_ARRAY(tiling); tilingNdx++)
        {
            for (unsigned usageNdx = 0; usageNdx < DE_LENGTH_OF_ARRAY(usages); usageNdx++)
            {
                ...
                addFunctionCase(getPhysicalDeviceImageFormatPropertiesTests.get(), name.str().c_str(),
                                "Checks usage bit format compatibility among compatible image view formats",
                                checkSupport, testExtendedUsageBitCompatiblity<PhysicalDeviceImageFormatProperties>,
                                params);
                addFunctionCase(getPhysicalDeviceImageFormatProperties2Tests.get(), name.str().c_str(),
                                "Checks usage bit format compatibility among compatible image view formats",
                                checkSupport, testExtendedUsageBitCompatiblity<PhysicalDeviceImageFormatProperties2>,
                                params);
            }
        }
    }
