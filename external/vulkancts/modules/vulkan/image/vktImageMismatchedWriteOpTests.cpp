...
public:
    ...
    MismatchedWriteOpTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
{
public:
    MismatchedVectorSizesTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                              const ParamsSp params, const int sourceWidth)
...
{
public:
    MismatchedSignednessAndTypeTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                    const ParamsSp params)
        : MismatchedWriteOpTest(testCtx, name, description, params)
    {
    }
...
const char *OpCapabilityToStr(const OpCapability &cap)
{
    switch (cap)
    {
    case OpCapability::Shader:
        return "Shader";
    case OpCapability::StorageImageExtendedFormats:
        return "StorageImageExtendedFormats";
    case OpCapability::Int64ImageEXT:
...
tcu::TestCaseGroup *createImageWriteOpTests(tcu::TestContext &testCtx)
{
    ...
    auto testGroup =
        new tcu::TestCaseGroup(testCtx, "mismatched_write_op", "Test image OpImageWrite operation in various aspects.");
    auto testGroupMismatchedVectorSizes = new tcu::TestCaseGroup(
        testCtx, "mismatched_vector_sizes", "Case OpImageWrite operation on mismatched vector sizes.");
    auto testGroupMismatchedSignedness = new tcu::TestCaseGroup(
        testCtx, "mismatched_signedness_and_type", "Case OpImageWrite operation on mismatched signedness and values.");
    ...
    {
        {
            ...
            {
                if (i->capability == OpCapability::Int64ImageEXT || info.capability == OpCapability::Int64ImageEXT)
                    continue;

                const std::string testName = de::toLower(i->spirvName) + "_from_" + de::toLower(info.spirvName);
                auto params                = new MismatchedWriteOpTest::Params{
                    info.vkFormat, 12, 8 * static_cast<int>(std::distance(begin, i) + 1), i->vkFormat};
                testGroupMismatchedSignedness->addChild(new MismatchedSignednessAndTypeTest(
                    testCtx, testName, {}, MismatchedVectorSizesTest::ParamsSp(params)));
            }
        }
        ...
        for (int sourceWidth = 4; sourceWidth > 0; --sourceWidth)
        {
            ...
            {
                auto params = new MismatchedWriteOpTest::Params{info.vkFormat, 12 * sourceWidth,
                                                                8 * (4 - sourceWidth + 1), info.vkFormat};
                testGroupMismatchedVectorSizes->addChild(
                    new MismatchedVectorSizesTest(testCtx, genVectorSizesTestName(info, sourceWidth), {},
