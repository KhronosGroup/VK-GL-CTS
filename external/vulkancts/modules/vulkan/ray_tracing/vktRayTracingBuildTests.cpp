...
protected:
    ...
    de::MovePtr<TopLevelAccelerationStructure> initTopAccelerationStructure(
        VkCommandBuffer cmdBuffer, bool useGpuBuild, uint32_t workerThreadsCount,
        vector<de::SharedPtr<BottomLevelAccelerationStructure>> &bottomLevelAccelerationStructures);
    vector<de::SharedPtr<BottomLevelAccelerationStructure>> initBottomAccelerationStructures(
        VkCommandBuffer cmdBuffer, bool useGpuBuild, uint32_t workerThreadsCount);
    de::MovePtr<BottomLevelAccelerationStructure> initBottomAccelerationStructure(VkCommandBuffer cmdBuffer,
                                                                                  bool useGpuBuild,
                                                                                  uint32_t workerThreadsCount,
                                                                                  tcu::UVec2 &startPos, bool triangles);
    ...
    vector<de::SharedPtr<BottomLevelAccelerationStructure>> &bottomLevelAccelerationStructures)
{
    ...
    for (size_t instanceNdx = 0; instanceNdx < bottomLevelAccelerationStructures.size(); ++instanceNdx)
    {
        ...
        result->addInstance(bottomLevelAccelerationStructures[instanceNdx], vk::identityMatrix3x4, 0, 0xFF,
...
de::MovePtr<BottomLevelAccelerationStructure> RayTracingBuildTestInstance::initBottomAccelerationStructure(
    VkCommandBuffer cmdBuffer, bool useGpuBuild, uint32_t workerThreadsCount, tcu::UVec2 &startPos, bool triangles)
{
    const DeviceInterface &vkd                           = m_context.getDeviceInterface();
    const VkDevice device                                = m_context.getDevice();
    Allocator &allocator                                 = m_context.getDefaultAllocator();
    ...
    VkCommandBuffer cmdBuffer, bool useGpuBuild, uint32_t workerThreadsCount)
{
    ...
    for (size_t instanceNdx = 0; instanceNdx < m_data.instancesGroupCount; ++instanceNdx)
    {
        ...
        de::MovePtr<BottomLevelAccelerationStructure> bottomLevelAccelerationStructure =
            initBottomAccelerationStructure(cmdBuffer, useGpuBuild, workerThreadsCount, startPos, triangles);
...
de::MovePtr<BufferWithMemory> RayTracingBuildTestInstance::runTest(bool useGpuBuild, uint32_t workerThreadsCount)
{
    ...
    de::SharedPtr<Move<VkShaderModule>> hitShader =
        makeVkSharedPtr(createShaderModule(vkd, device, m_context.getBinaryCollection().get("ahit"), 0));
    ...
    beginCommandBuffer(vkd, *cmdBuffer, 0u);
    {
        ...
        bottomLevelAccelerationStructures =
            initBottomAccelerationStructures(*cmdBuffer, useGpuBuild, workerThreadsCount);
        topLevelAccelerationStructure = initTopAccelerationStructure(*cmdBuffer, useGpuBuild, workerThreadsCount,
                                                                     bottomLevelAccelerationStructures);
...
uint32_t RayTracingBuildTestInstance::validateBuffer(de::MovePtr<BufferWithMemory> buffer)
{
    ...
    for (uint32_t y = 0; y < m_data.height; ++y)
        for (uint32_t x = 0; x < m_data.width; ++x)
        {
            ...
            const uint32_t expectedValue = (n % 7 == 0) ? 2 : 1;
...
uint32_t RayTracingBuildTestInstance::iterateWithWorkers(void)
{
    const uint64_t singleThreadTimeStart                = deGetMicroseconds();
    de::MovePtr<BufferWithMemory> singleThreadBufferCPU = runTest(false, 0);
    const uint32_t singleThreadFailures                 = validateBuffer(singleThreadBufferCPU);
    ...
    const uint32_t multiThreadFailures                 = validateBuffer(multiThreadBufferCPU);
...
uint32_t RayTracingBuildTestInstance::iterateNoWorkers(void)
{
    de::MovePtr<BufferWithMemory> bufferGPU = runTest(true, 0);
    de::MovePtr<BufferWithMemory> bufferCPU = runTest(false, 0);
    const uint32_t failuresGPU              = validateBuffer(bufferGPU);
    const uint32_t failuresCPU              = validateBuffer(bufferCPU);
    const uint32_t failures                 = failuresGPU + failuresCPU;
...
tcu::TestCaseGroup *createBuildTests(tcu::TestContext &testCtx)
{
    ...
    for (size_t threadNdx = 0; threadNdx <= DE_LENGTH_OF_ARRAY(threads); ++threadNdx)
    {
        ...
        const uint32_t threadsCount  = threadNdx < DE_LENGTH_OF_ARRAY(threads) ? threads[threadNdx] : 0;
        const string groupName       = !defferedOperation ? "gpu_cpu" :
                                       threadsCount == 0  ? "gpu_cpuht" :
                                       threadsCount == std::numeric_limits<uint32_t>::max() ?
                                                           "cpuht_max" :
                                                           "cpuht_" + de::toString(threadsCount);
        const string groupDesc =
            !defferedOperation ?
                "Compare results of run with acceleration structures build on GPU and CPU" :
            threadsCount > 0 ?
                "Compare results of run with acceleration structures build on GPU and using host threading" :
                "Run acceleration structures build using host threading";
        de::MovePtr<tcu::TestCaseGroup> groupGpuCpuHt(
