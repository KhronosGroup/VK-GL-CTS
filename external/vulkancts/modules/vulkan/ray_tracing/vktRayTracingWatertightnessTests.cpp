...
VkImageCreateInfo makeImageCreateInfo(uint32_t width, uint32_t height, uint32_t depth, VkFormat format)
{
    ...
    const auto imageType = (depth == 1) ? VK_IMAGE_TYPE_2D : VK_IMAGE_TYPE_3D;
...
RayTracingTestCase::RayTracingTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                       const CaseDef data, const bool &useClosedFan)
    ...
    else
    {
        ...
        for (uint32_t nSharedEdge = 0; nSharedEdge < m_data.squaresGroupCount; ++nSharedEdge)
        {
            const auto newVertex =
                tcu::Vec3(deFloatSin(2.0f * float(nSharedEdge) * 3.14159265f / float(m_data.squaresGroupCount)),
                          deFloatCos(2.0f * float(nSharedEdge) * 3.14159265f / float(m_data.squaresGroupCount)), 0.0f);
...
tcu::TestStatus RayTracingWatertightnessTestInstance::iterate(void)
{
    ...
    uint32_t failures                             = 0;
    uint32_t qualityWarningIssued                 = 0;
    if (!m_useClosedFan)
    {
        uint32_t pos = 0;
...
{
    de::MovePtr<tcu::TestCaseGroup> watertightnessGroup(
        new tcu::TestCaseGroup(testCtx, "watertightness", "Ray watertightness tests"));
    ...
    // Closed fan tests
    {
        ...
        for (uint32_t nBottomASConfig = 0; nBottomASConfig < 2; ++nBottomASConfig)
        {
            ...
            for (size_t sizesNdx = 0; sizesNdx < DE_LENGTH_OF_ARRAY(sizes); ++sizesNdx)
            {
                ...
                group->addChild(
                    new RayTracingTestCase(testCtx, testName.c_str(), "", caseDef, true /* useClosedFan */));
            }
