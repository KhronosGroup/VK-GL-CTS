...
std::vector<PipelineFlagsInstance::ShaderRecordEntry> PipelineFlagsInstance::prepareShaderBindingTable() const
{
	...
	shaderRecords[0] = { VK_SHADER_STAGE_RAYGEN_BIT_KHR, {}, {}, true};
	shaderRecords[1] = { VK_SHADER_STAGE_MISS_BIT_KHR, {}, { GeometryTypes::Box, (~0u), tcu::IVec4(0, defMissRetGreenComp, 0, 0) }, true };
		...
		for (deUint32 instance = 0; instance < m_params.instCount; ++instance)
		{
			...
			for (deUint32 geometryIndex = 0; geometryIndex < m_params.geomCount; ++geometryIndex)
			{
				...
				if (usedIndexes.find(shaderGroupIndex) == usedIndexes.end())
				{
					...
					shaderRecords[shaderGroupIndex] = { flags, hitGroup, { GeometryTypes::Triangle, geometryIndex, tcu::IVec4(0, greenComp++, 0, 0) }, true };
		...
		for (deUint32 instance = 0; instance < m_params.instCount; ++instance)
		{
			...
			for (deUint32 geometryIndex = 0; geometryIndex < m_params.geomCount; ++geometryIndex)
			{
				...
				if (usedIndexes.find(shaderGroupIndex) == usedIndexes.end())
				{
					...
					shaderRecords[shaderGroupIndex] = { flags, hitGroup, { GeometryTypes::Box, geometryIndex, tcu::IVec4(0, greenComp++, 0, 0) }, true };
					usedIndexes.insert(shaderGroupIndex);
				}
			}
		}
