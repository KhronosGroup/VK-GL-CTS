...
{
public:
    RayTracingPipelineLibraryTestCase(tcu::TestContext &context, const char *name, const char *desc,
...
protected:
    std::vector<de::SharedPtr<BottomLevelAccelerationStructure>> initBottomAccelerationStructures(
        DeviceHelper &deviceHelper, VkCommandBuffer cmdBuffer);
...
RayTracingPipelineLibraryTestCase::RayTracingPipelineLibraryTestCase(tcu::TestContext &context, const char *name,
                                                                     const char *desc, const TestParams data)
...
std::vector<de::SharedPtr<BottomLevelAccelerationStructure>> RayTracingPipelineLibraryTestInstance::
    initBottomAccelerationStructures(DeviceHelper &deviceHelper, VkCommandBuffer cmdBuffer)
{
    ...
    std::vector<de::SharedPtr<BottomLevelAccelerationStructure>> &bottomLevelAccelerationStructures)
{
    ...
    uint32_t numShadersUsed       = m_data.libraryConfiguration.pipelineShaders;
    for (auto it  = begin(m_data.libraryConfiguration.pipelineLibraries),
              eit = end(m_data.libraryConfiguration.pipelineLibraries);
         it != eit; ++it)
        numShadersUsed += it->y();
...
void compileShaders(DeviceHelper &deviceHelper, Context &context,
                    ...
                    const std::vector<std::tuple<std::string, VkShaderStageFlagBits>> &shaderData)
{
    ...
    for (uint32_t i = 0; i < shaderData.size(); ++i)
    {
        ...
        pipeline->get()->addShader(
            shaderStage, createShaderModule(vkd, device, context.getBinaryCollection().get(shaderName), 0), i);
    }
}
...
de::MovePtr<BufferWithMemory> RayTracingPipelineLibraryTestInstance::runTest(DeviceHelper &deviceHelper)
{
    ...
    auto &allocator                         = *deviceHelper.allocator;
    const uint32_t pixelCount               = m_data.height * m_data.width;
    ...
    std::vector<std::tuple<int, uint32_t, uint32_t>> libraryList;
    {
    ...
    if (m_data.multithreadedCompilation)
    {
        ...
        for (uint32_t i = 0; i < pipelineLibraries.size(); ++i)
            csmds.push_back(
                CompileShadersMultithreadData{deviceHelper, m_context, pipelineLibraries[i], pipelineShaders[i]});
    ...
    std::vector<de::SharedPtr<Move<VkPipeline>>> pipelines =
        pipelineLibraries[0]->get()->createPipelineWithLibraries(vkd, device, *pipelineLayout);
    ...
    const de::MovePtr<BufferWithMemory> raygenShaderBindingTable =
        pipelineLibraries[0]->get()->createShaderBindingTable(vkd, device, pipeline, allocator, shaderGroupHandleSize,
                                                              shaderGroupBaseAlignment, 0, 1);
    const de::MovePtr<BufferWithMemory> missShaderBindingTable = pipelineLibraries[0]->get()->createShaderBindingTable(
        vkd, device, pipeline, allocator, shaderGroupHandleSize, shaderGroupBaseAlignment, 1, 1);
    const de::MovePtr<BufferWithMemory> hitShaderBindingTable = pipelineLibraries[0]->get()->createShaderBindingTable(
        vkd, device, pipeline, allocator, shaderGroupHandleSize, shaderGroupBaseAlignment, 2, numShadersUsed);
    ...
    const VkStridedDeviceAddressRegionKHR hitShaderBindingTableRegion =
        makeStridedDeviceAddressRegionKHR(getBufferDeviceAddress(vkd, device, hitShaderBindingTable->get(), 0),
                                          shaderGroupHandleSize, numShadersUsed * shaderGroupHandleSize);
    ...
    beginCommandBuffer(vkd, *cmdBuffer, 0u);
    {
        ...
        topLevelAccelerationStructure =
            initTopAccelerationStructure(deviceHelper, *cmdBuffer, bottomLevelAccelerationStructures);
    ...
    for (size_t threadNdx = 0; threadNdx < DE_LENGTH_OF_ARRAY(threadData); ++threadNdx)
    {
        ...
        for (size_t libConfigNdx = 0; libConfigNdx < DE_LENGTH_OF_ARRAY(libraryConfigurationData); ++libConfigNdx)
        {
            TestParams testParams{libraryConfigurationData[libConfigNdx].libraryConfiguration,
                                  threadData[threadNdx].multithreaded, threadData[threadNdx].pipelinesCreatedUsingDHO,
                                  RTPL_DEFAULT_SIZE, RTPL_DEFAULT_SIZE};
            threadGroup->addChild(new RayTracingPipelineLibraryTestCase(
                group->getTestContext(), libraryConfigurationData[libConfigNdx].name, "", testParams));
        }
...
tcu::TestCaseGroup *createPipelineLibraryTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "pipeline_library", "Tests verifying pipeline libraries"));

    addTestGroup(group.get(), "configurations", "Test different configurations of pipeline libraries",
                 addPipelineLibraryConfigurationsTests);
