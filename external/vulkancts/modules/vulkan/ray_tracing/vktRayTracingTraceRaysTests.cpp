...
{
public:
    RayTracingTraceRaysIndirectTestCase(tcu::TestContext &context, const char *name, const char *desc,
...
de::MovePtr<TopLevelAccelerationStructure> RayTracingTraceRaysIndirectTestInstance::initTopAccelerationStructure(
    VkCommandBuffer cmdBuffer,
    std::vector<de::SharedPtr<BottomLevelAccelerationStructure>> &bottomLevelAccelerationStructures)
{
    const DeviceInterface &vkd   = m_context.getDeviceInterface();
    const VkDevice device        = m_context.getDevice();
    Allocator &allocator         = m_context.getDefaultAllocator();
    const uint32_t instanceCount = m_imageExtent.depth * m_imageExtent.height * m_imageExtent.width / 2;
...
de::MovePtr<BufferWithMemory> RayTracingTraceRaysIndirectTestInstance::runTest()
{
    ...
    if (m_data.traceType != TraceType::DIRECT)
    {
        ...
        const VkBufferCreateInfo indirectBufferCreateInfo =
            makeBufferCreateInfo(sizeof(VkTraceRaysIndirectCommandKHR), indirectBufferUsageFlags);
...
tcu::TestCaseGroup *createTraceRaysTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "trace_rays_cmds", "Tests veryfying vkCmdTraceRays* commands"));
