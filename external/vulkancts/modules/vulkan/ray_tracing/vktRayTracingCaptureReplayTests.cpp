...
{
public:
    RayTracingCaptureReplayTestCase(tcu::TestContext &context, const char *name, const char *desc,
...
{
    const uint32_t NO_MATCH_FOUND = ~((uint32_t)0);
    ...
    uint32_t queueFamilyIndex = NO_MATCH_FOUND;
    ...
    if ((de::contains(begin(coreExtensions), end(coreExtensions), "VK_KHR_buffer_device_address") ||
        ...
        std::for_each(begin(nonCoreDeviceExtensions), end(nonCoreDeviceExtensions),
                      [&nonCoreDeviceExtensionsC](const std::string &text)
                      {
                          if (text != "VK_EXT_buffer_device_address")
                              nonCoreDeviceExtensionsC.push_back(text.c_str());
                      });
    else
        std::for_each(begin(nonCoreDeviceExtensions), end(nonCoreDeviceExtensions),
                      [&nonCoreDeviceExtensionsC](const std::string &text)
    ...
    deviceInfo.enabledExtensionCount   = uint32_t(nonCoreDeviceExtensionsC.size());
    ...
    Move<VkDevice> testDevice =
        createCustomDevice(validationEnabled, vkp, m_context.getInstance(), vki, physicalDevice, &deviceInfo);
...
void addReplayShaderBindingTablesTests(tcu::TestCaseGroup *group)
{
    ...
    for (size_t testTypeNdx = 0; testTypeNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypeNdx)
    {
        ...
        group->addChild(new RayTracingCaptureReplayTestCase(group->getTestContext(), testTypes[testTypeNdx].name,
                                                            testTypes[testTypeNdx].description, testParams));
    }
}
...
tcu::TestCaseGroup *createCaptureReplayTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "capture_replay", "Capture-replay capabilities"));

    addTestGroup(group.get(), "shader_binding_tables", "Test replaying shader binding tables",
                 addReplayShaderBindingTablesTests);
    addTestGroup(group.get(), "acceleration_structures", "Test replaying acceleration structure",
                 addReplayAccelerationStruturesTests);
