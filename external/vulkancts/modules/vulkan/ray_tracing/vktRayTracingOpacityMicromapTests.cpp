...
tcu::TestStatus OpacityMicromapInstance::iterate(void)
{
    ...
    if (!m_params.nullMicromapHandle)
    {
        ...
        micromap            = vk::createMicromapEXT(vkd, device, &maCreateInfo);
    ...
    Move<VkMicromapEXT> updatedMicromap;
    ...
    if (m_params.updateFlags != 0)
    {
        ...
        updatedMicromap                      = vk::createMicromapEXT(vkd, device, &upCreateInfo);
...
tcu::TestCaseGroup *createOpacityMicromapTests(tcu::TestContext &testCtx)
{
    ...
    for (uint32_t testFlagMask = 0; testFlagMask < TEST_FLAG_BIT_LAST; testFlagMask++)
    {
        ...
        // VK_NULL_HANDLE micromap handle
        {
            ...
            {
                TestParams testParams{
                    true, false, true, testFlagMask, 0, ~specialIndex, seed++, VK_INDEX_TYPE_UINT32, false,
                };
    ...
    // Micromap query pool size queries.
    {
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> indexTypeGroup(new tcu::TestCaseGroup(testCtx, "index_type"));
            ...
            for (const auto &itype : indexTypes)
            {
                ...
                for (const auto &mode : modes)
                {
                    ...
                    {
                        TestParams testParams{
                            false, false, false, 0u, level, mode.mode, seed++, itype.indexType, false,
                        };
                ...
                {
                    TestParams testParams{
                        true, false, false, 0u, 0u, ~specialIndex, seed++, itype.indexType, false,
                    };
                    std::ostringstream css;
                    css << specialIndex;
                ...
                indexTypeGroup->addChild(itypeGroup.release());
            }

            group->addChild(indexTypeGroup.release());
        }
        ...
        // update variant
        {
            ...
            for (const auto &kind : updateKinds)
            {
                ...
                for (const auto &mode : modes)
                {
                    ...
                    {
                        TestParams testParams{
                            false, false, false, 0u, level, mode.mode, seed++, VK_INDEX_TYPE_UINT32, kind.updateFlags,
                        };
                        std::ostringstream css;
                        css << "level_" << level;
                        modeGroup->addChild(new OpacityMicromapCase(testCtx, css.str(), testParams));
                ...
                updateGroup->addChild(umGroup.release());
            }

            group->addChild(updateGroup.release());
        }
    }
