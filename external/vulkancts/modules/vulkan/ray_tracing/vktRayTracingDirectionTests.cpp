...
{
public:
    DirectionTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
DirectionTestCase::DirectionTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const TestParams &params)
...
void DirectionTestCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    rgen << "#version 460 core\n"
         ...
         << "  traceRayEXT(topLevelAS, gl_RayFlagsNoneEXT, cullMask, 0, 0, 0, pc.origin.xyz, pc.tmin, "
            "pc.direction.xyz, pc.tmax, 0);\n"
         << "}\n";
    ...
    case VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR:
        programCollection.glslSources.add("hits")
            << glu::ClosestHitSource(updateRayTracingGLSL(hits.str())) << buildOptions;
        break;
    case VK_SHADER_STAGE_ANY_HIT_BIT_KHR:
        programCollection.glslSources.add("hits")
            << glu::AnyHitSource(updateRayTracingGLSL(hits.str())) << buildOptions;
        break;
...
tcu::TestStatus DirectionTestInstance::iterate(void)
{
...

    {
        ...
        hitSBT = rayTracingPipeline->createShaderBindingTable(
            vkd, device, pipeline.get(), alloc, shaderGroupHandleSize * hitModuleCount, shaderGroupBaseAlignment, 2, 1);
        hitSBTRegion = makeStridedDeviceAddressRegionKHR(getBufferDeviceAddress(vkd, device, hitSBT->get(), 0),
                                                         shaderGroupHandleSize, shaderGroupHandleSize * hitModuleCount);
    }
...
{
    GroupPtr directionGroup(
        new tcu::TestCaseGroup(testCtx, "direction_length", "Test direction vector length when tracing rays"));
    ...
    for (int stageIdx = 0; stageIdx < DE_LENGTH_OF_ARRAY(stages); ++stageIdx)
    {
        ...
        for (int geometryTypeIdx = 0; geometryTypeIdx < DE_LENGTH_OF_ARRAY(geometryTypes); ++geometryTypeIdx)
        {
            ...
            if (gType.geometryType == VK_GEOMETRY_TYPE_TRIANGLES_KHR &&
                stageData.hitStage == VK_SHADER_STAGE_INTERSECTION_BIT_KHR)
                continue;
...
{
    GroupPtr insideAABBsGroup(
        new tcu::TestCaseGroup(testCtx, "inside_aabbs", "Test shooting rays that start inside AABBs"));
