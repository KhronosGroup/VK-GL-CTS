...
void BarycentricCoordinatesCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    ahitTerminate << "#version 460 core\n"
                  << "#extension GL_EXT_ray_tracing : require\n"
                  << "\n"
                  << "hitAttributeEXT vec2 baryCoord;\n"
                  << "\n"
    ...
    if (m_params.testCase == TestCaseRT::CLOSEST_HIT)
        ...
        programCollection.glslSources.add("chit")
            << glu::AnyHitSource(updateRayTracingGLSL(chit.str())) << buildOptions;
    else if (m_params.testCase == TestCaseRT::CLOSEST_AND_ANY_HIT_TERMINATE)
    {
        programCollection.glslSources.add("chit")
            << glu::ClosestHitSource(updateRayTracingGLSL(chit.str())) << buildOptions;
        programCollection.glslSources.add("ahitTerminate")
            << glu::AnyHitSource(updateRayTracingGLSL(ahitTerminate.str())) << buildOptions;
    }
    else
        DE_ASSERT(false);
}
...
tcu::TestStatus BarycentricCoordinatesInstance::iterate(void)
{
    ...
    vkd.cmdTraceRaysKHR(cmdBuffer, &raygenSBTRegion, &missSBTRegion, &hitSBTRegion, &callableSBTRegion, kNumRays, 1u,
                        1u);
    ...
    for (size_t i = 0; i < outputData.size(); ++i)
    {
        ...
        if (outVal.z() != expectedVal.z() || outVal.w() != 0.0f || de::abs(outVal.x() - expectedVal.x()) > kThreshold ||
...
tcu::TestCaseGroup *createBarycentricCoordinatesTests(tcu::TestContext &testCtx)
{
    ...
    mainGroup->addChild(
        new BarycentricCoordinatesCase(testCtx, "chit", "", TestParams{TestCaseRT::CLOSEST_HIT, seed++}));
    mainGroup->addChild(new BarycentricCoordinatesCase(testCtx, "ahit", "", TestParams{TestCaseRT::ANY_HIT, seed++}));
    mainGroup->addChild(new BarycentricCoordinatesCase(testCtx, "ahitTerminate", "",
                                                       TestParams{TestCaseRT::CLOSEST_AND_ANY_HIT_TERMINATE, seed++}));
