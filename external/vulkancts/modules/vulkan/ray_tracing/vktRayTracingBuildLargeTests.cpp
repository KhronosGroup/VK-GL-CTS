...
RayTracingTestCase::RayTracingTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                       const CaseDef data)
...
tcu::TestCaseGroup *createBuildLargeShaderSetTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "large_shader_set", "Build large shader set using CPU host threading"));
    ...
    for (size_t threadsNdx = 0; threadsNdx < DE_LENGTH_OF_ARRAY(threads); ++threadsNdx)
    {
        for (size_t buildNdx = 0; buildNdx < DE_LENGTH_OF_ARRAY(buildTypes); ++buildNdx)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> buildTypeGroup(
                new tcu::TestCaseGroup(testCtx, buildTypeGroupName.c_str(), ""));
