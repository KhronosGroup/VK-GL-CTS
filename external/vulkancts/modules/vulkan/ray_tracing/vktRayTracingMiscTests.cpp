...
public:
    ...
    void initPrograms(SourceCollections &programCollection) const final
    {
        ...
        // primitiveID 0 -> T=1.3, attr=1 (closest)
        // primitiveID 1 -> T=1.5, attr=2
        // primitiveID 2 -> T=1.7, attr=3
        {
            std::stringstream css;
            ...
            css << "#version 460 core\n"
                   ...
                   "    reportIntersectionEXT(1.3 + 0.2 * float(gl_PrimitiveID), 0);\n"
                   "}\n";
    ...
    bool verifyResultBuffer(vkt::Context &context, BufferWithMemory &buffer) const final
    {
        ...
        // Expect exactly 1 closest hit invocation, from primitiveID 0 (T=1.3, the
        // smallest), carrying hitAttributeEXT value 1 (corresponding to primitiveID 0).
        const bool ok = (nCHits == 1) && (primitiveID == 0) && (hitAttributeValue == 1);

        if (!ok)
        {
            auto &log = context.getTestContext().getLog();
            log << tcu::TestLog::Message << "hitAttributeEXT closest hit correctness check failed:\n"
                << "  nCHits            = " << nCHits << " (expected 1)\n"
                << "  primitiveID       = " << primitiveID << " (expected 0)\n"
                << "  hitAttributeValue = " << hitAttributeValue << " (expected 1)" << tcu::TestLog::EndMessage;
        }
...
tcu::TestCaseGroup *createMiscTests(tcu::TestContext &testCtx)
{
...

    {
        ...
        auto newTestCasePtr =
            new RayTracingTestCase(testCtx, "hitattribute_closest_hit_correctness",
