...
public:
    ...
    virtual uint32_t getDynamicStackSize() const
    {
        DE_ASSERT(false);
...
public:
    ...
    uint32_t getDynamicStackSize() const final
    {
        uint32_t result                              = 0;
...
private:
    ...
    uint32_t m_maxPipelineRayRecursionDepth;
...
public:
    ...
    static bool usesF64(const TestType &testType)
    {
        ...
        const bool has_f64 =
            std::find(tested_var_types.begin(), tested_var_types.end(), VariableType::FLOAT) != tested_var_types.end();
        const bool has_f64vec2 =
            std::find(tested_var_types.begin(), tested_var_types.end(), VariableType::VEC2) != tested_var_types.end();
        const bool has_f64vec3 =
            std::find(tested_var_types.begin(), tested_var_types.end(), VariableType::VEC3) != tested_var_types.end();
        const bool has_f64vec4 =
            std::find(tested_var_types.begin(), tested_var_types.end(), VariableType::VEC4) != tested_var_types.end();
...
bool RayTracingMiscTestInstance::init()
{
    ...
    m_rayTracingPropsPtr = makeRayTracingProperties(instanceInterface, physicalDeviceVk);
...
de::MovePtr<BufferWithMemory> RayTracingMiscTestInstance::runTest(void)
{
    ...
    m_testPtr->init(m_context, rtPropertiesPtr.get());
...

    {
        ...
        if (resultBufferDataVec.size() > 0)
        {
            ...
            flushMappedMemoryRange(deviceInterface, deviceVk, resultBufferPtr->getAllocation().getMemory(),
                                   resultBufferPtr->getAllocation().getOffset(), resultBufferSize);
        }
    }

    beginCommandBuffer(deviceInterface, *cmdBufferPtr, 0u /* flags */);
    {
        m_testPtr->initAS(m_context, rtPropertiesPtr.get(), *cmdBufferPtr);
...

        {
            ...
            const auto raygenShaderBindingTableRegion = makeStridedDeviceAddressRegionKHR(
                ...
                m_rayTracingPropsPtr->getShaderGroupHandleSize(), m_rayTracingPropsPtr->getShaderGroupHandleSize());
            ...
            const auto callableShaderBindingTableRegion =
                (callableShaderCollectionNames.size() > 0) ?
                    makeStridedDeviceAddressRegionKHR(
                        ...
                        (m_rayTracingPropsPtr->getShaderGroupHandleSize() +
                         m_testPtr->getShaderRecordSize(ShaderGroups::FIRST_CALLABLE_GROUP)) *
                            static_cast<uint32_t>(callableShaderCollectionNames.size())) :
                    makeStridedDeviceAddressRegionKHR(DE_NULL, 0, /* stride */
                                                      0 /* size   */);

            if (m_testPtr->usesDynamicStackSize())
            {
                deviceInterface.cmdSetRayTracingPipelineStackSizeKHR(*cmdBufferPtr, m_testPtr->getDynamicStackSize());
            }
...
tcu::TestCaseGroup *createMiscTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> miscGroupPtr(
        new tcu::TestCaseGroup(testCtx, "misc", "Miscellaneous ray-tracing tests"));
            ...
            for (uint32_t nIteration = 0; nIteration < 2; ++nIteration)
            {
                ...
                auto newTestCasePtr =
                    new RayTracingTestCase(testCtx, newTestCaseName.data(),
                                           "Verifies that the maximum ray hit attribute size property reported by the "
                                           "implementation is actually supported.",
         ...
         currentGeometryType      = static_cast<GeometryType>(static_cast<uint32_t>(currentGeometryType) + 1))
    {
        for (int nUseExtraCullMaskBits = 0; nUseExtraCullMaskBits < 2 /* false, true */; ++nUseExtraCullMaskBits)
        {
            ...
            auto newTestCasePtr = new RayTracingTestCase(
                testCtx, newTestCaseName.data(), "Verifies cull mask works as specified",
         ...
         currentGeometryType      = static_cast<GeometryType>(static_cast<uint32_t>(currentGeometryType) + 1))
    {
        ...
        auto newTestCasePtr =
            new RayTracingTestCase(testCtx, newTestCaseName.data(),
                                   "Verifies top-level acceleration structures built of AABB and triangle bottom-level "
                                   "AS instances work as expected",
...

    {
        auto newTestCasePtr = new RayTracingTestCase(
            testCtx, "mixedPrimTL",
            "Verifies top-level acceleration structures built of AABB and triangle bottom-level AS instances work as "
            "expected",
         ...
         currentASLayout = static_cast<AccelerationStructureLayout>(static_cast<uint32_t>(currentASLayout) + 1))
    {
        ...
        auto newTestCasePtr = new RayTracingTestCase(testCtx, newTestCaseName.data(),
                                                     "Verifies that the maximum ray hit attribute size property "
                                                     "reported by the implementation is actually supported.",
    ...
    {
        auto newTestCasePtr = new RayTracingTestCase(testCtx, "report_intersection_result",
                                                     "Test the return value of reportIntersectionEXT",
         ...
         currentGeometryType      = static_cast<GeometryType>(static_cast<uint32_t>(currentGeometryType) + 1))
    {
        ...
        auto newTestCasePtr = new RayTracingTestCase(testCtx, newTestCaseName.data(),
                                                     "Verifies that relevant shader stages can correctly read large "
                                                     "ray payloads provided by raygen shader stage.",
...

    {
        auto newTestCaseSTD430_1Ptr =
            new RayTracingTestCase(testCtx, "shaderRecordSTD430_1",
        ...
        auto newTestCaseExplicitScalarOffset_1Ptr =
            new RayTracingTestCase(testCtx, "shaderRecordExplicitScalarOffset_1",
                                   "Tests usage of various variables inside a shader record block using scalar layout "
                                   "and explicit offset qualifiers",
         ...
         currentGeometryType      = static_cast<GeometryType>(static_cast<uint32_t>(currentGeometryType) + 1))
    {
        ...
        // 0 recursion levels.
        {
            auto newTestCasePtr =
                new RayTracingTestCase(testCtx, (newTestCaseName + "0").data(),
                                       "Verifies that relevant shader stages can correctly read large ray payloads "
                                       "provided by raygen shader stage.",
        ...
        for (uint32_t nLevels = 1; nLevels <= 15; ++nLevels)
        {
            auto newTestCasePtr = new RayTracingTestCase(
                ...
                "Verifies that relevant shader stages can correctly read large ray payloads provided by raygen shader "
                "stage.",
