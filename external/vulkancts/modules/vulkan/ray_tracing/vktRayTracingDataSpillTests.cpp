...
constexpr uint32_t kShaderRecordSize = sizeof(tcu::UVec4);
...
VkDeviceSize getElementSize(DataType dataType, VectorType vectorType)
{
    ...
    case DataType::FLOAT32:
        dataSize = sizeof(tcu::Float32);
        break;
    case DataType::FLOAT64:
        dataSize = sizeof(tcu::Float64);
        break;
    case DataType::FLOAT16:
        dataSize = sizeof(tcu::Float16);
        break;
    case DataType::STRUCT:
        dataSize = sizeof(InputStruct);
        break;
...
public:
    ...
    DataSpillTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
DataSpillTestCase::DataSpillTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const TestParams &testParams)
...
void DataSpillTestCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    spvTemplateStream
        ...
        << "                                  OpEntryPoint ${ENTRY_POINT} %main \"main\" %topLevelAS %calleeBuffer "
           "%outputBuffer %inputBuffer${MAIN_INTERFACE_EXTRAS}\n"
    ...
    else if (m_params.dataType == DataType::UINT16)
    {
        ...
        subs["CALC_ZERO_FOR_CALLABLE"] =
            "                   %zero_ushort = OpISub %ushort %input_val_before %ushort_37\n"
    ...
    else if (m_params.dataType == DataType::FLOAT64)
    {
        ...
        subs["CALC_ZERO_FOR_CALLABLE"] =
            "                   %zero_double = OpFSub %double %input_val_before %double_37\n"
    ...
    else if (m_params.dataType == DataType::STRUCT)
    {
        ...
        subs["CALC_ZERO_FOR_CALLABLE"] =
            "                 %uint_part_ptr = OpAccessChain %uint_part_ptr_type %input_val_ptr %uint_0\n"
    ...
    else if (samplersNeeded(m_params.dataType))
    {
        ...
        subs["EXTRA_BINDING_VARIABLES"] +=
            "                %sampledTexture = OpVariable %image_array_type_uniform_ptr UniformConstant\n"
        ...
        if (m_params.dataType == DataType::IMAGE || m_params.dataType == DataType::SAMPLER)
        {
            ...
            subs["CALC_ZERO_FOR_CALLABLE"] +=
                ...
                "%pixel_vec_0 = OpImageSampleExplicitLod %v4uint %sampled_image_0 %texture_coords_0 Lod|ZeroExtend "
                "%float_0\n"
        ...
        else if (m_params.dataType == DataType::SAMPLED_IMAGE)
        {
            ...
            subs["CALC_ZERO_FOR_CALLABLE"] +=
                "%sampled_image_0_ptr = OpAccessChain %sampled_image_type_uniform_ptr %combinedImageSampler %uint_0\n"
    ...
    else if (storageImageNeeded(m_params.dataType))
    {
        ...
        subs["EXTRA_BINDING_VARIABLES"] +=
            "                  %storageImage = OpVariable %image_type_uniform_ptr UniformConstant\n";

        // Load value from the image, expecting it to be 37 and swapping it with 5.
        subs["CALC_ZERO_FOR_CALLABLE"] +=
            "%coords = OpCompositeConstruct %v2int %input_val_before %input_val_before\n"
    ...
    else if (m_params.dataType == DataType::OP_NULL)
    {
        ...
        subs["CALC_ZERO_FOR_CALLABLE"] +=
            "%constant_null_copy = OpCopyObject %uint %constant_null\n"
    ...
    if (m_params.dataType == DataType::STRUCT)
    {
        ...
        subs["EXTRA_FUNCTION_VARIABLES"] =
            "         %input_val_func_before = OpVariable %input_struct_func_ptr_type Function\n"
            "          %input_val_func_after = OpVariable %input_struct_func_ptr_type Function\n";
        subs["CALC_EQUAL_STATEMENT"] =
            "                                  OpStore %input_val_func_before %input_val_before\n"
    ...
    else if (m_params.dataType == DataType::IMAGE)
    {
        ...
        subs["CALC_EQUAL_STATEMENT"] +=
            ...
            "%pixel_vec_1 = OpImageSampleExplicitLod %v4uint %sampled_image_1 %texture_coords_1 Lod|ZeroExtend "
            "%float_0\n"
    ...
    else if (m_params.dataType == DataType::SAMPLED_IMAGE)
    {
        ...
        subs["CALC_EQUAL_STATEMENT"] +=
            "%texture_coords_1 = OpCompositeConstruct %v2float %input_val_after %input_val_after\n"
            "%pixel_vec_1 = OpImageSampleExplicitLod %v4uint %sampled_image_0 %texture_coords_1 Lod|ZeroExtend "
            "%float_0\n"
            "%pixel_1 = OpCompositeExtract %uint %pixel_vec_1 0\n"
            "%equal = OpIEqual %bool %pixel_0 %pixel_1\n";
    }
    ...
    else if (m_params.dataType == DataType::PTR_SAMPLED_IMAGE)
    {
        // We attempt to use the second pointer only after the call.
        subs["CALC_EQUAL_STATEMENT"] +=
            "%sampled_image_1 = OpLoad %sampled_image_type %sampled_image_1_ptr\n"
            "%texture_coords_1 = OpCompositeConstruct %v2float %input_val_after %input_val_after\n"
            "%pixel_vec_1 = OpImageSampleExplicitLod %v4uint %sampled_image_1 %texture_coords_1 Lod|ZeroExtend "
            "%float_0\n"
            "%pixel_1 = OpCompositeExtract %uint %pixel_vec_1 0\n"
    ...
    {
        subs["CALC_EQUAL_STATEMENT"] +=
            "                         %equal = " + opEqual + " %bool %input_val_before %input_val_after\n";
    }
    ...
    if (numComponents > 1)
    {
        ...
        if (!(m_params.dataType == DataType::FLOAT32 && m_params.vectorType == VectorType::V3))
        {
            ...
            subs["EXTRA_TYPES_AND_CONSTANTS"] +=
                "%" + vectorTypeName + " = " + opType + " %" + componentTypeName + " " + componentCountStr + "\n";
        }
        subs["EXTRA_TYPES_AND_CONSTANTS"] +=
            "%v" + numComponentsStr + "bool = " + opType + " %bool " + componentCountStr + "\n";
        ...
        {
            zeroForCallable << "%component_ptr_" << i << " = OpAccessChain %comp_ptr %input_val_ptr %uint_" << i << "\n"
        ...
        for (int i = 1; i < numComponents; ++i)
        {
            ...
            const std::string resultName =
                ((i == (numComponents - 1)) ? "%total_sum" : ("%partial_" + de::toString(i)));
            zeroForCallable << resultName << " = " << opAdd << " %" << componentTypeName << " %component_" << i << " "
                            << previous << "\n";
        }

        // Recalculate the zero.
        zeroForCallable << "%zero_" << componentTypeName << " = " << opSub << " %" << componentTypeName
                        << " %total_sum %" << componentTypeName << "_37\n"
        ...
        if (isArray)
        {
            ...
            for (int i = 0; i < numComponents; ++i)
            {
                calcEqual << "%component_after_" << i << " = OpLoad %" << componentTypeName << " %component_ptr_" << i
                          << "\n"
                          << "%equal_" << i << " = " << opEqual << " %bool %component_" << i << " %component_after_"
                          << i << "\n";
                ;
                if (i > 0)
        ...
        else
        {
            ...
            subs["CALC_EQUAL_STATEMENT"] = "                  %equal_vector = " + opEqual + " %v" + numComponentsStr +
                                           "bool %input_val_before %input_val_after\n";
    ...
    if (isArray)
    {
        ...
        interfaceDecorations << "OpDecorate %v" << numComponentsStr << componentTypeName << " ArrayStride "
                             << getElementSize(m_params.dataType, VectorType::SCALAR) << "\n";
    ...
    glslBindings << inputBlockDecls.first // Additional data types needed.
                 ...
                 << "layout(set = 0, binding = 3) buffer InputBlock { " << inputBlockDecls.second
                 << " } inputBuffer;\n";

    if (samplersNeeded(m_params.dataType))
    {
        glslBindings << "layout(set = 0, binding = 4) uniform utexture2D sampledTexture[2];\n"
                     << "layout(set = 0, binding = 5) uniform sampler textureSampler[2];\n"
                     << "layout(set = 0, binding = 6) uniform usampler2D combinedImageSampler[2];\n";
    }
    else if (storageImageNeeded(m_params.dataType))
    {
        glslBindings << "layout(set = 0, binding = 4, r32ui) uniform uimage2D storageImage;\n";
    }
    ...
    if (m_params.callType == CallType::TRACE_RAY)
    {
        ...
        subs["INTERFACE_TYPES_AND_VARIABLES"] =
            "                   %payload_ptr = OpTypePointer RayPayloadKHR %v3float\n"
            "                      %hitValue = OpVariable %payload_ptr RayPayloadKHR\n";
        subs["CALL_STATEMENTS"] =
            "                      %as_value = OpLoad %as_type %topLevelAS\n"
            "                                  OpTraceRayKHR %as_value %uint_0 %uint_255 %zero_for_callable "
            "%zero_for_callable %zero_for_callable %origin_const %float_0 %direction_const %float_9 %hitValue\n";
        ...
        chit << glslHeaderStr << "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n"
             << "hitAttributeEXT vec3 attribs;\n"
             << glslBindingsStr << "void main()\n"
             << "{\n"
             << "    calleeBuffer.val = 1u;\n"
             << "}\n";
        ;
    ...
    else if (m_params.callType == CallType::EXECUTE_CALLABLE)
    {
        ...
        subs["INTERFACE_TYPES_AND_VARIABLES"] =
            "             %callable_data_ptr = OpTypePointer CallableDataKHR %float\n"
            "                  %callableData = OpVariable %callable_data_ptr CallableDataKHR\n";
        subs["CALL_STATEMENTS"] =
            "                                  OpExecuteCallableKHR %zero_for_callable %callableData\n";

        const auto rgen = spvTemplate.specialize(subs);
        programCollection.spirvAsmSources.add("rgen") << rgen << spvBuildOptions;

        std::ostringstream call;
        call << glslHeaderStr << "layout(location = 0) callableDataInEXT float callableData;\n"
             << glslBindingsStr << "void main()\n"
             << "{\n"
             << "    calleeBuffer.val = 1u;\n"
             << "}\n";
        ...
        programCollection.glslSources.add("call")
            << glu::CallableSource(updateRayTracingGLSL(call.str())) << buildOptions;
    }
    else if (m_params.callType == CallType::REPORT_INTERSECTION)
    {
        ...
        subs["INTERFACE_TYPES_AND_VARIABLES"] =
            "             %hit_attribute_ptr = OpTypePointer HitAttributeKHR %v3float\n"
            "                       %attribs = OpVariable %hit_attribute_ptr HitAttributeKHR\n";
        subs["CALL_STATEMENTS"] =
            "              %intersection_ret = OpReportIntersectionKHR %bool %float_1 %zero_for_callable\n";

        const auto rint = spvTemplate.specialize(subs);
        programCollection.spirvAsmSources.add("rint") << rint << spvBuildOptions;

        std::ostringstream rgen;
        rgen << glslHeaderStr << "layout(location = 0) rayPayloadEXT vec3 hitValue;\n"
             << glslBindingsStr << "void main()\n"
             << "{\n"
             << "  traceRayEXT(topLevelAS, 0u, 0xFFu, 0, 0, 0, vec3(0.5, 0.5, 0.0), 0.0, vec3(0.0, 0.0, -1.0), 9.0, "
                "0);\n"
             << "}\n";
        ...
        ahit << glslHeaderStr << "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n"
             << "hitAttributeEXT vec3 attribs;\n"
             << glslBindingsStr << "void main()\n"
             << "{\n"
             << "    calleeBuffer.val = 1u;\n"
...
using v2i32 = tcu::Vector<int32_t, 2>;
using v3i32 = tcu::Vector<int32_t, 3>;
using v4i32 = tcu::Vector<int32_t, 4>;
using a5i32 = std::array<int32_t, 5>;

using v2u32 = tcu::Vector<uint32_t, 2>;
using v3u32 = tcu::Vector<uint32_t, 3>;
using v4u32 = tcu::Vector<uint32_t, 4>;
using a5u32 = std::array<uint32_t, 5>;

using v2i64 = tcu::Vector<int64_t, 2>;
using v3i64 = tcu::Vector<int64_t, 3>;
using v4i64 = tcu::Vector<int64_t, 4>;
using a5i64 = std::array<int64_t, 5>;

using v2u64 = tcu::Vector<uint64_t, 2>;
using v3u64 = tcu::Vector<uint64_t, 3>;
using v4u64 = tcu::Vector<uint64_t, 4>;
using a5u64 = std::array<uint64_t, 5>;

using v2i16 = tcu::Vector<int16_t, 2>;
using v3i16 = tcu::Vector<int16_t, 3>;
using v4i16 = tcu::Vector<int16_t, 4>;
using a5i16 = std::array<int16_t, 5>;

using v2u16 = tcu::Vector<uint16_t, 2>;
using v3u16 = tcu::Vector<uint16_t, 3>;
using v4u16 = tcu::Vector<uint16_t, 4>;
using a5u16 = std::array<uint16_t, 5>;

using v2i8 = tcu::Vector<int8_t, 2>;
using v3i8 = tcu::Vector<int8_t, 3>;
using v4i8 = tcu::Vector<int8_t, 4>;
using a5i8 = std::array<int8_t, 5>;

using v2u8 = tcu::Vector<uint8_t, 2>;
using v3u8 = tcu::Vector<uint8_t, 3>;
using v4u8 = tcu::Vector<uint8_t, 4>;
using a5u8 = std::array<uint8_t, 5>;
...
void fillInputBuffer(DataType dataType, VectorType vectorType, void *bufferPtr)
{
    if (vectorType == VectorType::SCALAR)
    {
        if (dataType == DataType::INT32)
            GEN_SCALAR_FILL(int32_t);
        else if (dataType == DataType::UINT32)
            GEN_SCALAR_FILL(uint32_t);
        else if (dataType == DataType::INT64)
            GEN_SCALAR_FILL(int64_t);
        else if (dataType == DataType::UINT64)
            GEN_SCALAR_FILL(uint64_t);
        else if (dataType == DataType::INT16)
            GEN_SCALAR_FILL(int16_t);
        else if (dataType == DataType::UINT16)
            GEN_SCALAR_FILL(uint16_t);
        else if (dataType == DataType::INT8)
            GEN_SCALAR_FILL(int8_t);
        else if (dataType == DataType::UINT8)
            GEN_SCALAR_FILL(uint8_t);
        else if (dataType == DataType::FLOAT32)
            GEN_SCALAR_FILL(tcu::Float32);
        else if (dataType == DataType::FLOAT64)
            GEN_SCALAR_FILL(tcu::Float64);
        else if (dataType == DataType::FLOAT16)
            GEN_SCALAR_FILL(tcu::Float16);
        else if (dataType == DataType::STRUCT)
        {
            InputStruct data = {12u, 25.0f};
            deMemcpy(bufferPtr, &data, sizeof(data));
        }
        else if (dataType == DataType::OP_NULL)
            GEN_SCALAR_FILL(uint32_t);
        else if (dataType == DataType::OP_UNDEF)
            GEN_SCALAR_FILL(uint32_t);
        else
        {
            DE_ASSERT(false);
...
tcu::TestStatus DataSpillTestInstance::iterate(void)
{
    ...
    if (samplersNeeded(m_params.dataType) || storageImageNeeded(m_params.dataType))
    {
        ...
        {
            const auto textureDataSize       = static_cast<VkDeviceSize>(sizeof(uint32_t));
            const auto textureDataCreateInfo = makeBufferCreateInfo(textureDataSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);

            textureData = de::MovePtr<BufferWithMemory>(
                new BufferWithMemory(vkd, device, alloc, textureDataCreateInfo, MemoryRequirement::HostVisible));
        ...
        if (samplersNeeded(m_params.dataType))
        {
            ...
            for (size_t i = 0; i < kNumImages; ++i)
            {
                ...
                textureViews.emplace_back(makeImageView(vkd, device, textures.back()->get(), VK_IMAGE_VIEW_TYPE_2D,
                                                        kImageFormat, imageSubresourceRange));
            }
            ...
            vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 1u,
                                   &textureDataBarrier, 0u, nullptr, 0u, nullptr);
            ...
            for (size_t i = 0; i < kNumImages; ++i)
            {
                const auto texturePreCopyBarrier = makeImageMemoryBarrier(
                    0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                    textures[i]->get(), imageSubresourceRange);
                const auto texturePostCopyBarrier = makeImageMemoryBarrier(
                    VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, textures[i]->get(), imageSubresourceRange);

                vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u,
                                       0u, nullptr, 0u, nullptr, 1u, &texturePreCopyBarrier);
                vkd.cmdCopyBufferToImage(cmdBuffer, textureData->get(), textures[i]->get(),
                                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1u, &bufferImageCopy);
        ...
        else if (storageImageNeeded(m_params.dataType))
        {
            ...
            const auto texturePreCopyBarrier = makeImageMemoryBarrier(
                0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                textures.back()->get(), imageSubresourceRange);
            const auto texturePostCopyBarrier = makeImageMemoryBarrier(
                VK_ACCESS_TRANSFER_WRITE_BIT, (VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
                VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_GENERAL, textures.back()->get(),
            ...
            vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u,
                                   nullptr, 0u, nullptr, 1u, &texturePreCopyBarrier);
            vkd.cmdCopyBufferToImage(cmdBuffer, textureData->get(), textures.back()->get(),
                                     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1u, &bufferImageCopy);
            vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                   VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR, 0u, 0u, nullptr, 0u, nullptr, 1u,
                                   &texturePostCopyBarrier);
    ...
    // Update descriptor set.
    {
        ...
        updateBuilder.writeSingle(ds, DescriptorSetUpdateBuilder::Location::binding(0u),
                                  VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, &writeASInfo);
        updateBuilder.writeSingle(ds, DescriptorSetUpdateBuilder::Location::binding(1u),
                                  VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &calleeBufferDescriptorInfo);
        updateBuilder.writeSingle(ds, DescriptorSetUpdateBuilder::Location::binding(2u),
                                  VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &outputBufferDescriptorInfo);
        updateBuilder.writeSingle(ds, DescriptorSetUpdateBuilder::Location::binding(3u),
                                  VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &inputBufferDescriptorInfo);
        ...
        if (samplersNeeded(m_params.dataType))
        {
            ...
            for (size_t i = 0; i < kNumCombined; ++i)
                combinedSamplerInfos.push_back(makeDescriptorImageInfo(samplers[i + kNumAloneSamplers].get(),
                                                                       textureViews[i + kNumAloneImages].get(),
                                                                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL));

            updateBuilder.writeArray(ds, DescriptorSetUpdateBuilder::Location::binding(4u),
                                     VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, kNumAloneImages, textureDescInfos.data());
            updateBuilder.writeArray(ds, DescriptorSetUpdateBuilder::Location::binding(5u), VK_DESCRIPTOR_TYPE_SAMPLER,
                                     kNumAloneSamplers, textureSamplerInfos.data());
            updateBuilder.writeArray(ds, DescriptorSetUpdateBuilder::Location::binding(6u),
                                     VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, kNumCombined,
                                     combinedSamplerInfos.data());
        }
        else if (storageImageNeeded(m_params.dataType))
        {
...

    {
        ...
        rayTracingPipeline->addShader(VK_SHADER_STAGE_RAYGEN_BIT_KHR,
                                      createShaderModule(vkd, device, m_context.getBinaryCollection().get("rgen"), 0),
                                      0);
        ...
        else if (callType == CallType::EXECUTE_CALLABLE)
        {
            rayTracingPipeline->addShader(
                ...
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("call"), 0), 1);
        }
        else if (callType == CallType::REPORT_INTERSECTION)
        {
            rayTracingPipeline->addShader(
                VK_SHADER_STAGE_INTERSECTION_BIT_KHR,
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("rint"), 0), 1);
            rayTracingPipeline->addShader(
                VK_SHADER_STAGE_ANY_HIT_BIT_KHR,
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("ahit"), 0), 1);
        }
        else
        {
            DE_ASSERT(false);
        ...
        if (callType == CallType::EXECUTE_CALLABLE)
        {
            callableShaderBindingTable = rayTracingPipeline->createShaderBindingTable(
                vkd, device, pipeline.get(), alloc, shaderGroupHandleSize, shaderGroupBaseAlignment, 1, 1);
            callableShaderBindingTableRegion = makeStridedDeviceAddressRegionKHR(
                getBufferDeviceAddress(vkd, device, callableShaderBindingTable->get(), 0), shaderGroupHandleSize,
    ...
    vkd.cmdTraceRaysKHR(cmdBuffer, &raygenShaderBindingTableRegion, &missShaderBindingTableRegion,
    ...
    for (const auto &ptr : bufferPtrs)
    {
        ...
        if (outputVal != 1u)
            return tcu::TestStatus::fail("Unexpected value found in " + bufferName +
                                         " buffer: " + de::toString(outputVal));
    }
...
public:
    ...
    DataSpillPipelineInterfaceTestCase(tcu::TestContext &testCtx, const std::string &name,
...
DataSpillPipelineInterfaceTestCase::DataSpillPipelineInterfaceTestCase(tcu::TestContext &testCtx,
...
DataSpillPipelineInterfaceTestInstance::DataSpillPipelineInterfaceTestInstance(Context &context,
                                                                               const TestParams &testParams)
    : vkt::TestInstance(context)
    , m_params(testParams)
{
...
void DataSpillPipelineInterfaceTestCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    const std::string glslHeader = "#version 460 core\n"
                                   "#extension GL_EXT_ray_tracing : require\n";

    const std::string glslBindings = "layout(set = 0, binding = 0) uniform accelerationStructureEXT topLevelAS;\n"
                                     "layout(set = 0, binding = 1) buffer StorageBlock { uint val[" +
                                     std::to_string(kNumStorageValues) + "]; } storageBuffer;\n";
    ...
    if (m_params.interfaceType == InterfaceType::RAY_PAYLOAD)
    {
        ...
        std::ostringstream rgen;
        rgen << glslHeader << "layout(location = 0) rayPayloadEXT vec3 hitValue;\n"
             << glslBindings << "void main()\n"
             << "{\n"
        ...
        std::stringstream chit;
        chit << glslHeader << "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n"
             << "hitAttributeEXT vec3 attribs;\n"
             << glslBindings << "void main()\n"
             << "{\n"
    ...
    else if (m_params.interfaceType == InterfaceType::CALLABLE_DATA)
    {
        ...
        std::ostringstream rgen;
        rgen << glslHeader << "layout(location = 0) callableDataEXT float callableData;\n"
             << glslBindings << "void main()\n"
             << "{\n"
             << "  callableData = 100.0;\n"
        ...
        std::ostringstream call;
        call << glslHeader << "layout(location = 0) callableDataInEXT float callableData;\n"
             << glslBindings << "void main()\n"
             << "{\n"
    ...
    else if (m_params.interfaceType == InterfaceType::HIT_ATTRIBUTES)
    {
        ...
        rint << glslHeader << "hitAttributeEXT vec3 attribs;\n"
             << glslBindings << "void main()\n"
             << "{\n"
        ...
        programCollection.glslSources.add("rint")
            << glu::IntersectionSource(updateRayTracingGLSL(rint.str())) << buildOptions;
    ...
    else if (m_params.interfaceType == InterfaceType::SHADER_RECORD_BUFFER_RGEN)
    {
        ...
        std::ostringstream rgen;
        rgen << glslHeader << "layout(shaderRecordEXT) buffer ShaderRecordStruct {\n"
             << "  uvec4 info;\n"
             << "};\n"
        ...
        std::ostringstream call;
        call << glslHeader << "layout(location = 0) callableDataInEXT uint callableData;\n"
             << glslBindings << "void main()\n"
             << "{\n"
    ...
    else if (m_params.interfaceType == InterfaceType::SHADER_RECORD_BUFFER_CALL)
    {
        ...
        std::ostringstream rgen;
        rgen << glslHeader << "layout(location = 0) callableDataEXT uint callableData;\n"
             << glslBindings << "void main()\n"
             << "{\n"
             << "  callableData = 1u;\n"
             << "  executeCallableEXT(0, 0);\n"
        ...
        std::ostringstream call;
        call << glslHeader << "layout(shaderRecordEXT) buffer ShaderRecordStruct {\n"
             << "  uvec4 info;\n"
             << "};\n"
        ...
        subcall << glslHeader << "layout(location = 1) callableDataInEXT uint callableData;\n"
                << glslBindings << "void main()\n"
                << "{\n"
                << "  callableData = (callableData + 1u) % 4u;\n"
        ...
        programCollection.glslSources.add("subcall")
            << glu::CallableSource(updateRayTracingGLSL(subcall.str())) << buildOptions;
    }
    else if (m_params.interfaceType == InterfaceType::SHADER_RECORD_BUFFER_MISS ||
             m_params.interfaceType == InterfaceType::SHADER_RECORD_BUFFER_HIT)
    {
        ...
        std::ostringstream rgen;
        rgen << glslHeader << "layout(location = 0) rayPayloadEXT uint rayPayload;\n"
             << glslBindings << "void main()\n"
             << "{\n"
             << "  rayPayload = 2u;\n"
             << "  traceRayEXT(topLevelAS, 0u, 0xFFu, 0, 0, 0, vec3(0.5, 0.5, 0.0), 0.0, vec3(0.0, 0.0, -1.0), 9.0, "
        ...
        chitOrMiss << glslHeader << "layout(shaderRecordEXT) buffer ShaderRecordStruct {\n"
                   << "  uvec4 info;\n"
                   << "};\n"
        ...
        if (m_params.interfaceType == InterfaceType::SHADER_RECORD_BUFFER_MISS)
            programCollection.glslSources.add("miss")
                << glu::MissSource(updateRayTracingGLSL(chitOrMiss.str())) << buildOptions;
        else if (m_params.interfaceType == InterfaceType::SHADER_RECORD_BUFFER_HIT)
            programCollection.glslSources.add("chit")
                << glu::ClosestHitSource(updateRayTracingGLSL(chitOrMiss.str())) << buildOptions;
        else
            DE_ASSERT(false);
...
void createSBTWithShaderRecord(const DeviceInterface &vkd, VkDevice device, vk::Allocator &alloc, VkPipeline pipeline,
                               RayTracingPipeline *rayTracingPipeline, uint32_t shaderGroupHandleSize,
                               uint32_t shaderGroupBaseAlignment, uint32_t firstGroup, uint32_t groupCount,
                               ...
                               VkStridedDeviceAddressRegionKHR &shaderBindingTableRegion)
{
    ...
    shaderBindingTable     = rayTracingPipeline->createShaderBindingTable(
        vkd, device, pipeline, alloc, shaderGroupHandleSize, shaderGroupBaseAlignment, firstGroup, groupCount, 0u, 0u,
        MemoryRequirement::Any, 0u, 0u, kShaderRecordSize);
    shaderBindingTableRegion = makeStridedDeviceAddressRegionKHR(
        getBufferDeviceAddress(vkd, device, shaderBindingTable->get(), 0), alignedSize, groupCount * alignedSize);
    ...
    auto *dataPtr  = reinterpret_cast<uint8_t *>(sbtAlloc.getHostPtr()) + shaderGroupHandleSize;
...
tcu::TestStatus DataSpillPipelineInterfaceTestInstance::iterate(void)
{
    ...
    // Storage buffer.
    std::array<uint32_t, kNumStorageValues> storageBufferData;
    ...
    // Update descriptor set.
    {
        ...
        updateBuilder.writeSingle(ds, DescriptorSetUpdateBuilder::Location::binding(0u),
                                  VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR, &writeASInfo);
        updateBuilder.writeSingle(ds, DescriptorSetUpdateBuilder::Location::binding(1u),
                                  VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &storageBufferDescriptorInfo);
        updateBuilder.update(vkd, device);
    }
...

    {
        ...
        if (interfaceType == InterfaceType::RAY_PAYLOAD)
        {
            rayTracingPipeline->addShader(
                ...
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("chit"), 0), 1);
                 ...
                 interfaceType == InterfaceType::SHADER_RECORD_BUFFER_RGEN)
        {
            rayTracingPipeline->addShader(
                VK_SHADER_STAGE_CALLABLE_BIT_KHR,
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("call"), 0), 1);
        ...
        else if (interfaceType == InterfaceType::SHADER_RECORD_BUFFER_CALL)
        {
            ...
            rayTracingPipeline->addShader(
                VK_SHADER_STAGE_CALLABLE_BIT_KHR,
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("subcall"), 0), 2);
        }
        else if (interfaceType == InterfaceType::SHADER_RECORD_BUFFER_MISS)
        {
            rayTracingPipeline->addShader(
                VK_SHADER_STAGE_MISS_BIT_KHR,
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("miss"), 0), 1);
            rayTracingPipeline->addShader(
                VK_SHADER_STAGE_CALLABLE_BIT_KHR,
                createShaderModule(vkd, device, m_context.getBinaryCollection().get("call"), 0), 2);
        }
        ...
        {
            createSBTWithShaderRecord(vkd, device, alloc, pipeline.get(), rayTracingPipeline.get(),
                                      shaderGroupHandleSize, shaderGroupBaseAlignment, 0u, 1u, raygenShaderBindingTable,
                                      raygenShaderBindingTableRegion);
        }
        else
        {
        ...
        {
            createSBTWithShaderRecord(vkd, device, alloc, pipeline.get(), rayTracingPipeline.get(),
                                      shaderGroupHandleSize, shaderGroupBaseAlignment, 1u, 2u,
                                      callableShaderBindingTable, callableShaderBindingTableRegion);
        }
        else if (interfaceType == InterfaceType::SHADER_RECORD_BUFFER_MISS)
        {
            createSBTWithShaderRecord(vkd, device, alloc, pipeline.get(), rayTracingPipeline.get(),
                                      shaderGroupHandleSize, shaderGroupBaseAlignment, 1u, 1u, missShaderBindingTable,
                                      missShaderBindingTableRegion);

            // Callable shader table.
            callableShaderBindingTable = rayTracingPipeline->createShaderBindingTable(
                vkd, device, pipeline.get(), alloc, shaderGroupHandleSize, shaderGroupBaseAlignment, 2, 1);
            callableShaderBindingTableRegion = makeStridedDeviceAddressRegionKHR(
                getBufferDeviceAddress(vkd, device, callableShaderBindingTable->get(), 0), shaderGroupHandleSize,
                shaderGroupHandleSize);
        }
        ...
        {
            createSBTWithShaderRecord(vkd, device, alloc, pipeline.get(), rayTracingPipeline.get(),
                                      shaderGroupHandleSize, shaderGroupBaseAlignment, 1u, 1u, hitShaderBindingTable,
                                      hitShaderBindingTableRegion);
    ...
    else if (interfaceType == InterfaceType::SHADER_RECORD_BUFFER_CALL)
    {
        ...
        expectedData.push_back(450u);
             ...
             interfaceType == InterfaceType::SHADER_RECORD_BUFFER_HIT)
    {
    ...
    for (pos = 0u; pos < expectedData.size(); ++pos)
    {
        ...
        if (stored != expected)
        {
            ...
            msg << "Unexpected output value found at position " << pos << " (expected " << expected << " but got "
                << stored << ")";
...
tcu::TestCaseGroup *createDataSpillTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
        testCtx, "data_spill", "Ray tracing tests for data spilling and unspilling around shader calls"));
    ...
    struct
    {
        ...
        {VectorType::SCALAR, ""}, {VectorType::V2, "v2"}, {VectorType::V3, "v3"},
        {VectorType::V4, "v4"},   {VectorType::A5, "a5"},
    };
    ...
    for (int callTypeIdx = 0; callTypeIdx < DE_LENGTH_OF_ARRAY(callTypes); ++callTypeIdx)
    {
        ...
        for (int dataTypeIdx = 0; dataTypeIdx < DE_LENGTH_OF_ARRAY(dataTypes); ++dataTypeIdx)
        {
            ...
            for (int vectorTypeIdx = 0; vectorTypeIdx < DE_LENGTH_OF_ARRAY(vectorTypes); ++vectorTypeIdx)
            {
                ...
                if ((samplersNeeded(entryDataTypes.dataType) || storageImageNeeded(entryDataTypes.dataType) ||
                     entryDataTypes.dataType == DataType::STRUCT || entryDataTypes.dataType == DataType::OP_NULL ||
    ...
    de::MovePtr<tcu::TestCaseGroup> pipelineInterfaceGroup(new tcu::TestCaseGroup(
        testCtx, "pipeline_interface", "Test data spilling and unspilling of pipeline interface variables"));
