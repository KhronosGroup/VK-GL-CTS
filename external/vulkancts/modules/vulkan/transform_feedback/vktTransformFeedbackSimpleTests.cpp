    ...
    , m_rnd(0)
{
    ...
    if (m_parameters.streamId > 0 &&
        (m_transformFeedbackProperties.transformFeedbackRasterizationStreamSelect == VK_FALSE))
...
void createTransformFeedbackSimpleTests(tcu::TestCaseGroup *group)
{
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                for (uint32_t bufferSizesNdx = 0; bufferSizesNdx < DE_LENGTH_OF_ARRAY(bufferSizes); ++bufferSizesNdx)
                {
                    ...
                    TestParameters parameters = {
                        testType, bufferSize,         partCount, 0u,    0u,
                        0u,       STREAM_ID_0_NORMAL, false,     false, VK_PRIMITIVE_TOPOLOGY_POINT_LIST};
...

    {
        ...
        TestParameters parameters  = {
            testType, 512u, 2u, 0u, 0u, 0u, STREAM_ID_0_NORMAL, false, false, VK_PRIMITIVE_TOPOLOGY_POINT_LIST};
                ...
                for (const auto &numVertices : vertexCounts)
                {
                    for (uint32_t i = 0; i < 2; ++i)
                    {
                        ...
                        TestParameters parameters      = {testType,           bufferSize,  0u,    streamId,      0u, 0u,
                                                          STREAM_ID_0_NORMAL, query64Bits, false, topology.first};
                        ...
                        const TestParameters parametersCopy = {
                            testTypeCopy, bufferSize,         0u,          streamId, 0u,
                            0u,           STREAM_ID_0_NORMAL, query64Bits, false,    topology.first};
                        const std::string fullTestNameCopy = testNameCopy + "_" + topology.second.topologyName +
                                                             de::toString(streamId) + "_" + de::toString(vertexCount) +
                                                             widthStr;
...

    {
        ...
        for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++bufferCountsNdx)
        {
            ...
            const TestParameters parameters  = {
                ...
                false,    VK_PRIMITIVE_TOPOLOGY_POINT_LIST};
