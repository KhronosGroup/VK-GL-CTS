...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const VkShaderModule tessellationEvalModule, const VkShaderModule geometryModule,
                                      ...
                                      const bool inputVertices = false, const bool depthClipControl = false)
{
    ...
    const VkPipelineVertexInputStateCreateInfo *vertexInputStateCreateInfoPtr =
        (inputVertices) ? DE_NULL : &vertexInputStateCreateInfo;
    const VkBool32 disableRasterization = (fragmentModule == DE_NULL);
    const uint32_t rasterizationStream  = (rasterizationStreamPtr == DE_NULL) ? 0 : *rasterizationStreamPtr;
    ...
    const VkPipelineRasterizationStateCreateInfo *rasterizationStateCreateInfoPtr =
        (rasterizationStreamPtr == DE_NULL) ? DE_NULL : &rasterizationStateCreateInfo;
...
protected:
    ...
    void verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc, const uint32_t bufBytes);
...
tcu::TestStatus TransformFeedbackBasicTestInstance::iterate(void)
{
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule,
                                                           DE_NULL, DE_NULL, DE_NULL, DE_NULL, m_imageExtent2D, 0u,
                                                           &m_parameters.streamId));
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, makeRect2D(m_imageExtent2D));
        {
            ...
            for (uint32_t drawNdx = 0; drawNdx < m_parameters.partCount; ++drawNdx)
            {
                ...
                vk.cmdPushConstants(*cmdBuffer, *pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0u, sizeof(startValue),
                                    &startValue);
...
tcu::TestStatus TransformFeedbackResumeTestInstance::iterate(void)
{
    ...
    const VkBufferCreateInfo tfcBufCreateInfo =
...
tcu::TestStatus TransformFeedbackWindingOrderTestInstance::iterate(void)
{
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(
        vk, device, *pipelineLayout, *renderPass, *vertexModule, m_requiresTesselationStage ? *tescModule : DE_NULL,
        m_requiresTesselationStage ? *teseModule : DE_NULL, DE_NULL, DE_NULL, m_imageExtent2D, 0u, DE_NULL,
        m_parameters.primTopology));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(vk, device, *cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
    ...
    verifyTransformFeedbackBuffer(tfBufAllocation, static_cast<uint32_t>(bufferSize));
                                                                              ...
                                                                              const uint32_t bufBytes)
{
    ...
    {
        const uint32_t *tfDataForPrimitive             = &tfData[primitiveIndex * vertexPerPrimitive];
        std::vector<uint32_t> expectedDataForPrimitive = m_tParameters.getExpectedValuesForPrimitive(primitiveIndex);
        ...
        for (uint32_t combinationIndex = 0; combinationIndex < vertexPerPrimitive; combinationIndex++)
        {
            correctWinding = true;
            for (uint32_t vertexIndex = 0; vertexIndex < vertexPerPrimitive; vertexIndex++)
            {
                correctWinding &= (tfDataForPrimitive[vertexIndex] ==
                                   expectedDataForPrimitive[(combinationIndex + vertexIndex) % vertexPerPrimitive]);
...
protected:
    tcu::TestStatus iterate(void);
    void verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc, const VkDeviceSize offset,
                                       const uint32_t bufBytes);
};
...
void TransformFeedbackBuiltinTestInstance::verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc,
...
tcu::TestStatus TransformFeedbackBuiltinTestInstance::iterate(void)
{
    ...
    const uint32_t perVertexDataSize =
        (m_parameters.testType == TEST_TYPE_XFB_POINTSIZE)     ? static_cast<uint32_t>(sizeof(float)) :
    ...
    verifyTransformFeedbackBuffer(tfBufAllocation, tfBufBindingOffsets[m_parameters.partCount - 1],
                                  numPoints * perVertexDataSize);
...
void TransformFeedbackDepthClipControlTestInstance::verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc,
                                                                                  const VkDeviceSize offset,
                                                                                  const uint32_t bufBytes)
{
    const DeviceInterface &vk  = m_context.getDeviceInterface();
    const VkDevice device      = m_context.getDevice();
    const uint32_t numVertices = bufBytes / static_cast<uint32_t>(sizeof(float) * 4);
    const uint8_t *tfDataBytes = getInvalidatedHostPtr<uint8_t>(vk, device, *bufAlloc);
    const float *tfData        = (float *)&tfDataBytes[offset];
...
tcu::TestStatus TransformFeedbackDepthClipControlTestInstance::iterate(void)
{
    ...
    const Unique<VkPipelineLayout> pipelineLayout(TransformFeedback::makePipelineLayout(vk, device));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(
        vk, device, *pipelineLayout, *renderPass, *vertexModule, hasTessellation ? *tescModule : DE_NULL,
        hasTessellation ? *teseModule : DE_NULL, hasGeomShader ? *geomModule : DE_NULL, DE_NULL, m_imageExtent2D, 0u,
        &m_parameters.streamId, m_parameters.primTopology, false, true));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(vk, device, *cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
    ...
    const std::vector<VkDeviceSize> tfBufBindingSizes =
        std::vector<VkDeviceSize>(m_parameters.partCount, m_parameters.bufferSize);
    const std::vector<VkDeviceSize> tfBufBindingOffsets = generateOffsetsList(tfBufBindingSizes);
    const uint32_t perVertexDataSize                    = static_cast<uint32_t>(4u * sizeof(float));
    const uint32_t numVertices                          = m_parameters.bufferSize / perVertexDataSize;
...
void TransformFeedbackMultistreamTestInstance::verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc,
                                                                             const uint32_t bufBytes)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
...
tcu::TestStatus TransformFeedbackMultistreamTestInstance::iterate(void)
{
    ...
    const Unique<VkShaderModule> vertexModule(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert"), 0u));
    const Unique<VkShaderModule> geomModule(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("geom"), 0u));

    const Unique<VkFramebuffer> framebuffer(
        makeFramebuffer(vk, device, *renderPass, 0u, DE_NULL, m_imageExtent2D.width, m_imageExtent2D.height));
    const Unique<VkPipelineLayout> pipelineLayout(TransformFeedback::makePipelineLayout(vk, device));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule,
                                                           DE_NULL, DE_NULL, *geomModule, DE_NULL, m_imageExtent2D, 0u,
                                                           &m_parameters.streamId));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
...
tcu::TestStatus TransformFeedbackStreamsTestInstance::iterate(void)
{
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertModule,
                                                           DE_NULL, DE_NULL, *geomModule, *fragModule, m_imageExtent2D,
                                                           0u, &m_parameters.streamId));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(vk, device, *cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
...
tcu::TestStatus TransformFeedbackIndirectDrawTestInstance::iterate(void)
{
    ...
    const Unique<VkPipelineLayout> pipelineLayout(TransformFeedback::makePipelineLayout(vk, device));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertModule,
                                                           DE_NULL, DE_NULL, DE_NULL, *fragModule, m_imageExtent2D, 0u,
                                                           DE_NULL, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, true));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(vk, device, *cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        if (m_parameters.testType == TEST_TYPE_QUERY_COPY || m_parameters.testType == TEST_TYPE_QUERY_COPY_STRIDE_ZERO)
        {
            ...
            vk.cmdCopyQueryPoolResults(*cmdBuffer, *queryPool, queryIndex, queryCountersNumber, *queryPoolResultsBuffer,
                                       0u, copyStride, (vk::VK_QUERY_RESULT_WAIT_BIT | queryExtraFlags));
...

    {
        ...
        if (m_parameters.testType != TEST_TYPE_QUERY_COPY && m_parameters.testType != TEST_TYPE_QUERY_COPY_STRIDE_ZERO)
        {
            vk.getQueryPoolResults(device, *queryPool, queryIndex, queryCountersNumber, queryDataSize, queryData.data(),
                                   queryDataSize, (vk::VK_QUERY_RESULT_WAIT_BIT | queryExtraFlags));
        }
        else
        {
...
protected:
    std::vector<VkDeviceSize> generateSizesList(const size_t bufBytes, const size_t chunkCount);
    void verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc, const uint32_t bufBytes,
                                       const uint32_t bufOffset, const float expected);
    tcu::TestStatus iterate(void);
};
...
void TransformFeedbackMultiQueryTestInstance::verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc,
                                                                            const uint32_t bufBytes,
...
tcu::TestStatus TransformFeedbackMultiQueryTestInstance::iterate(void)
{
    ...
    const Unique<VkPipelineLayout> pipelineLayout(TransformFeedback::makePipelineLayout(vk, device));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertModule,
                                                           DE_NULL, DE_NULL, *geomModule, DE_NULL, m_imageExtent2D,
                                                           0u));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
    ...
    for (size_t counterNdx = 0; counterNdx < queryCountersNumber; ++counterNdx)
    {
        ...
        if (counterNdx == 0 && !m_parameters.omitShaderWrite)
            verifyTransformFeedbackBuffer(tfBufAllocation, bytesPerVertex * expected.written,
...
protected:
    std::vector<VkDeviceSize> generateSizesList(const size_t bufBytes, const size_t chunkCount);
    void verifyTransformFeedbackBufferLines(const MovePtr<Allocation> &bufAlloc, const uint32_t bufBytes,
                                            const std::vector<uint32_t> &primitives, const uint32_t invocationCount,
                                            const uint32_t partCount);
    void verifyTransformFeedbackBufferTriangles(const MovePtr<Allocation> &bufAlloc, const uint32_t bufBytes,
                                                const std::vector<uint32_t> &primitives, const uint32_t invocationCount,
                                                const uint32_t partCount);
    tcu::TestStatus iterate(void);
};
...
void TransformFeedbackLinesOrTrianglesTestInstance::verifyTransformFeedbackBufferLines(
    const MovePtr<Allocation> &bufAlloc, const uint32_t bufBytes, const std::vector<uint32_t> &primitives,
    const uint32_t invocationCount, const uint32_t partCount)
{
    ...
    for (uint32_t i = 0; i < numPoints; ++i)
    {
        if (tfData[i] != reference[i])
        {
            ...
            err << "Failed at item " << i << " received:" << tfData[i] << " expected:" << reference[i];
...
tcu::TestStatus TransformFeedbackLinesOrTrianglesTestInstance::iterate(void)
{
    ...
    case VK_PRIMITIVE_TOPOLOGY_LINE_STRIP:
        verifyTransformFeedbackBufferLines(tfBufAllocation, tfBufferSize, LINES_LIST, INVOCATION_COUNT,
                                           m_parameters.partCount);
        break;
    case VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP:
        verifyTransformFeedbackBufferTriangles(tfBufAllocation, tfBufferSize, TRIANGLES_LIST, INVOCATION_COUNT,
                                               m_parameters.partCount);
        break;
    default:
...
{
public:
    TransformFeedbackTestCase(tcu::TestContext &context, const char *name, const char *description,
                              const TestParameters &parameters);
...
TransformFeedbackTestCase::TransformFeedbackTestCase(tcu::TestContext &context, const char *name,
                                                     const char *description, const TestParameters &parameters)
...
void TransformFeedbackTestCase::checkSupport(Context &context) const
{
    ...
    if (m_parameters.requireRastStreamSelect &&
        (context.getTransformFeedbackPropertiesEXT().transformFeedbackRasterizationStreamSelect == VK_FALSE) &&
        (m_parameters.streamId > 0))
...
void TransformFeedbackTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    const bool requiresFullPipeline =
        m_parameters.testType == TEST_TYPE_STREAMS || m_parameters.testType == TEST_TYPE_STREAMS_POINTSIZE ||
        m_parameters.testType == TEST_TYPE_STREAMS_CULLDISTANCE ||
    ...
    {
        const std::string outputBuiltIn = (m_parameters.testType == TEST_TYPE_XFB_POINTSIZE) ?
                                              "float gl_PointSize;\n" :
        ...
        const std::string operationBuiltIn =
            ...
            (m_parameters.testType == TEST_TYPE_XFB_CLIPDISTANCE) ?
                "for (int i=0; i<8; i++) gl_ClipDistance[i] = float(8 * gl_VertexIndex + i) / 32768.0f;" :
            (m_parameters.testType == TEST_TYPE_XFB_CULLDISTANCE) ?
                "for (int i=0; i<8; i++) gl_CullDistance[i] = float(8 * gl_VertexIndex + i) / 32768.0f;" :
            (m_parameters.testType == TEST_TYPE_XFB_CLIP_AND_CULL) ?
                "for (int i=0; i<5; i++) gl_CullDistance[i] = float(6 * gl_VertexIndex + i) / 32768.0f;\n"
    ...
    if (requiresFullPipeline)
    {
        ...
        // geometry shader
        {
            ...
            const bool requirePoints              = (m_parameters.testType == TEST_TYPE_STREAMS_POINTSIZE ||
                                        m_parameters.testType == TEST_TYPE_MULTISTREAMS);
            const std::string outputPrimitiveType = requirePoints ? "points" : "triangle_strip";
            const std::string outputBuiltIn =
                (m_parameters.testType == TEST_TYPE_STREAMS_POINTSIZE)    ? "    float gl_PointSize;\n" :
...
void createTransformFeedbackSimpleTests(tcu::TestCaseGroup *group)
{
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                for (uint32_t bufferSizesNdx = 0; bufferSizesNdx < DE_LENGTH_OF_ARRAY(bufferSizes); ++bufferSizesNdx)
                {
                    ...
                    TestParameters parameters = {
                        testType,           bufferSize, partCount, 0u,   0u,    0u,
                        STREAM_ID_0_NORMAL, false,      false,     true, false, VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

                    group->addChild(new TransformFeedbackTestCase(
                        group->getTestContext(),
                        (testName + "_" + de::toString(partCount) + "_" + de::toString(bufferSize)).c_str(),
                        "Simple Transform Feedback test", parameters));
                    parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
                    group->addChild(
                        new TransformFeedbackTestCase(group->getTestContext(),
...

    {
        ...
        for (const auto &topology : topologyData)
        {
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                TestParameters parameters = {testType,           0u,    vertexCount, 0u,    0u,    0u,
                                             STREAM_ID_0_NORMAL, false, false,       false, false, topology.first};

                group->addChild(new TransformFeedbackTestCase(
                    group->getTestContext(),
                    (testName + "_" + topology.second.topologyName + de::toString(vertexCount)).c_str(),
                    "Topology winding test", parameters));
            }
        }
    }

    {
        ...
        for (uint32_t vertexStridesNdx = 0; vertexStridesNdx < DE_LENGTH_OF_ARRAY(vertexStrides); ++vertexStridesNdx)
        {
            const uint32_t vertexStrideBytes =
                static_cast<uint32_t>(sizeof(uint32_t) * vertexStrides[vertexStridesNdx]);
            TestParameters parameters = {
                testType,           0u,    0u,    0u,    0u,    vertexStrideBytes,
                STREAM_ID_0_NORMAL, false, false, false, false, VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_" + de::toString(vertexStrideBytes)).c_str(),
                                                          "Rendering tests with various strides", parameters));
            parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(),
                (testName + "_beginqueryindexed_streamid_0_" + de::toString(vertexStrideBytes)).c_str(),
                "Rendering tests with various strides", parameters));
            parameters.streamId0Mode = STREAM_ID_0_END_QUERY_INDEXED;
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(),
                (testName + "_endqueryindexed_streamid_0_" + de::toString(vertexStrideBytes)).c_str(),
                "Rendering tests with various strides", parameters));
        }
    }

    {
        ...
        TestParameters parameters  = {testType,           512u,  2u,    0u,    0u,    0u,
                                      STREAM_ID_0_NORMAL, false, false, false, false, VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

        group->addChild(new TransformFeedbackTestCase(group->getTestContext(), testName.c_str(),
                                                      "Rendering test checks backward pipeline dependency",
                                                      parameters));
        parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
        group->addChild(
            new TransformFeedbackTestCase(group->getTestContext(), (testName + "_beginqueryindexed_streamid_0").c_str(),
                                          "Rendering test checks backward pipeline dependency", parameters));
        parameters.streamId0Mode = STREAM_ID_0_END_QUERY_INDEXED;
        group->addChild(
            new TransformFeedbackTestCase(group->getTestContext(), (testName + "_endqueryindexed_streamid_0").c_str(),
                                          "Rendering test checks backward pipeline dependency", parameters));

        parameters.noOffsetArray = true;
        group->addChild(new TransformFeedbackTestCase(
            group->getTestContext(), (testName + "_no_offset_array").c_str(),
            "Rendering test checks backward pipeline dependency (using NULL for offset array)", parameters));
    }
            ...
            for (const auto &streamCounts : usedStreamId)
            {
                ...
                for (const auto &numVertices : vertexCounts)
                {
                    for (uint32_t i = 0; i < 2; ++i)
                    {
                        ...
                        TestParameters parameters      = {testType, bufferSize,         0u,          streamId, 0u,
                                                          0u,       STREAM_ID_0_NORMAL, query64Bits, false,    true,
                                                          false,    topology.first};
                        const std::string fullTestName = testName + "_" + topology.second.topologyName +
                                                         de::toString(streamId) + "_" + de::toString(vertexCount) +
                                                         widthStr;
                        group->addChild(new TransformFeedbackTestCase(group->getTestContext(), fullTestName.c_str(),
                                                                      "Written primitives query test", parameters));

                        TestParameters omitParameters  = {testType, bufferSize,         0u,          streamId, 0u,
                                                          0u,       STREAM_ID_0_NORMAL, query64Bits, false,    true,
                                                          true,     topology.first};
                        const std::string omitTestName = testName + "_omit_write_" + topology.second.topologyName +
                                                         de::toString(streamId) + "_" + de::toString(vertexCount) +
                                                         widthStr;
                        group->addChild(new TransformFeedbackTestCase(group->getTestContext(), omitTestName.c_str(),
                                                                      "Written primitives query test", omitParameters));

                        for (uint32_t testTypeCopyNdx = 0; testTypeCopyNdx < DE_LENGTH_OF_ARRAY(testTypeCopy);
                             testTypeCopyNdx++)
                        {
                            const TestParameters parametersCopy = {testTypeCopy[testTypeCopyNdx],
                                                                   bufferSize,
                                                                   0u,
                                                                   streamId,
                                                                   0u,
                            ...
                            group->addChild(
                                new TransformFeedbackTestCase(group->getTestContext(), fullTestNameCopy.c_str(),
                                                              "Written primitives query test", parametersCopy));
                        }

                        const TestParameters parametersHostQueryReset = {
                            testTypeHostQueryReset, bufferSize,  0u,    streamId, 0u,    0u,
                            STREAM_ID_0_NORMAL,     query64Bits, false, true,     false, topology.first};
                        ...
                        group->addChild(
                            new TransformFeedbackTestCase(group->getTestContext(), fullTestNameHostQueryReset.c_str(),
                                                          "Written primitives query test", parametersHostQueryReset));
    ...
    {
        TestParameters parameters = {
            TEST_TYPE_DEPTH_CLIP_CONTROL_VERTEX, 96, 1u, 0u, 0u, 0u, STREAM_ID_0_NORMAL, false, false, true, false,
            VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

        group->addChild(
            new TransformFeedbackTestCase(group->getTestContext(), "depth_clip_control_vertex", "", parameters));
    }
    {
        TestParameters parameters = {
            TEST_TYPE_DEPTH_CLIP_CONTROL_GEOMETRY, 96, 1u, 0u, 0u, 0u, STREAM_ID_0_NORMAL, false, false, true, false,
            VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

        group->addChild(
            new TransformFeedbackTestCase(group->getTestContext(), "depth_clip_control_geometry", "", parameters));
    }
    {
        TestParameters parameters = {
            TEST_TYPE_DEPTH_CLIP_CONTROL_TESE, 96, 1u, 0u, 0u, 0u, STREAM_ID_0_NORMAL, false, false, true, false,
            VK_PRIMITIVE_TOPOLOGY_PATCH_LIST};

        group->addChild(
            new TransformFeedbackTestCase(group->getTestContext(), "depth_clip_control_tese", "", parameters));
    }

    {
        ...
        for (const auto &topology : topologyData)
        {
            ...
            for (const auto &streamId : usedStreamId)
            {
                ...
                group->addChild(
                    new TransformFeedbackTestCase(group->getTestContext(), fullTestName.c_str(), "", parameters));
            }
        }
    }
}
...
void createTransformFeedbackStreamsSimpleTests(tcu::TestCaseGroup *group)
{
    ...
    for (uint32_t testTypesNdx = 0; testTypesNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypesNdx)
    {
        ...
        for (uint32_t streamCountsNdx = 0; streamCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++streamCountsNdx)
        {
            ...
            TestParameters parameters = {
                testType,           0u,    0u,    streamId, pointSize, 0u,
                STREAM_ID_0_NORMAL, false, false, true,     false,     VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_" + de::toString(streamId)).c_str(),
                                                          "Streams usage test", parameters));
        }
    }

    {
        ...
        for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++bufferCountsNdx)
        {
            ...
            const TestParameters parameters  = {testType,
            ...
            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_" + de::toString(streamId)).c_str(),
                                                          "Simultaneous multiple streams usage test", parameters));
        }
    }

    {
        ...
        for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++bufferCountsNdx)
        {
            ...
            const TestParameters parameters          = {testType,
                                                        ...
                                                        false,
                                                        VK_PRIMITIVE_TOPOLOGY_POINT_LIST};
            const TestParameters writeOmitParameters = {testType,
                                                        maxBytesPerVertex * streamsUsed,
                                                        streamsUsed,
                                                        streamId,
                                                        0u,
            ...
            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_" + de::toString(streamId)).c_str(),
                                                          "Simultaneous multiple queries usage test", parameters));
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(), (testName + "_omit_write_" + de::toString(streamId)).c_str(),
                "Simultaneous multiple queries usage test", writeOmitParameters));
        }
    }
}
...
{
    return createTestGroup(testCtx, "simple", "Transform Feedback Simple tests",
                           createTransformFeedbackAndStreamsSimpleTests);
}
