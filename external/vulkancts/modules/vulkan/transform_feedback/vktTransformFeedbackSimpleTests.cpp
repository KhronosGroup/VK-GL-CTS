...
tcu::TestStatus TransformFeedbackBackwardDependencyTestInstance::iterate(void)
{
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule,
                                                           DE_NULL, DE_NULL, DE_NULL, *fragModule,
                                                           makeExtent2D(vkExtent.width, vkExtent.height), 0u));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(vk, device, *cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
...
tcu::TestStatus TransformFeedbackDrawOutsideTestInstance::iterate(void)
{
    ...
    const Unique<VkShaderModule> vertexModule1(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert"), 0u));
    const Unique<VkShaderModule> vertexModule2(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert2"), 0u));
    const Unique<VkRenderPass> renderPass(makeRenderPass(vk, device, VK_FORMAT_UNDEFINED));
    const Unique<VkFramebuffer> framebuffer(
        makeFramebuffer(vk, device, *renderPass, 0u, DE_NULL, m_imageExtent2D.width, m_imageExtent2D.height));
    const Unique<VkPipelineLayout> pipelineLayout(TransformFeedback::makePipelineLayout(vk, device));
    const Unique<VkPipeline> pipeline1(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule1,
                                                            DE_NULL, DE_NULL, DE_NULL, DE_NULL, m_imageExtent2D, 0u,
                                                            &m_parameters.streamId));
    const Unique<VkPipeline> pipeline2(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule2,
                                                            DE_NULL, DE_NULL, DE_NULL, DE_NULL, m_imageExtent2D, 0u,
                                                            &m_parameters.streamId));
    const Unique<VkCommandPool> cmdPool(
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex));
...
void createTransformFeedbackSimpleTests(tcu::TestCaseGroup *group)
{
        ...
        for (uint32_t testTypesNdx = 0; testTypesNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypesNdx)
        {
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                for (uint32_t bufferSizesNdx = 0; bufferSizesNdx < DE_LENGTH_OF_ARRAY(bufferSizes); ++bufferSizesNdx)
                {
                    ...
                    TestParameters parameters = {testType,
                                                 bufferSize,
                                                 partCount,
                                                 0u,
                                                 0u,
        ...
        for (const auto &topology : topologyData)
        {
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                TestParameters parameters = {testType,           0u,    vertexCount, 0u,    0u,    0u,
                                             STREAM_ID_0_NORMAL, false, false,       false, false, false,
                                             topology.first,     false};

                // Topology winding test
                group->addChild(new TransformFeedbackTestCase(
                    group->getTestContext(),
                    (testName + "_" + topology.second.topologyName + de::toString(vertexCount)).c_str(), parameters));
            }
        }
    }

    {
        for (int i = 0; i < 2; ++i)
        {
                 ...
                 ++vertexStridesNdx)
            {
                ...
                group->addChild(new TransformFeedbackTestCase(
                    group->getTestContext(), (testName + "_" + de::toString(vertexStrideBytes)).c_str(), parameters));
                parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
                group->addChild(new TransformFeedbackTestCase(
                    group->getTestContext(),
                ...
                group->addChild(new TransformFeedbackTestCase(
                    group->getTestContext(),
                    (testName + "_endqueryindexed_streamid_0_" + de::toString(vertexStrideBytes)).c_str(), parameters));
            }
        }
    }

    {
        ...
        for (const auto &testCase : testCases)
        {
            ...
            parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(), (testName + "_beginqueryindexed_streamid_0").c_str(), parameters));
            parameters.streamId0Mode = STREAM_ID_0_END_QUERY_INDEXED;
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(), (testName + "_endqueryindexed_streamid_0").c_str(), parameters));
            ...
            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_no_offset_array").c_str(), parameters));
        }
    }
            ...
            for (const auto &streamCounts : usedStreamId)
            {
                ...
                for (const auto &numVertices : vertexCounts)
                {
                    for (uint32_t i = 0; i < 2; ++i)
                    {
                        ...
                        TestParameters parameters      = {testType,           bufferSize,  0u,    streamId, 0u,    0u,
                                                          STREAM_ID_0_NORMAL, query64Bits, false, true,     false, false,
                                                          topology.first,     false};
                        ...
                        TestParameters omitParameters  = {testType,           bufferSize,  0u,    streamId, 0u,   0u,
                                                          STREAM_ID_0_NORMAL, query64Bits, false, true,     true, false,
                                                          topology.first,     false};
                        ...
                        for (uint32_t testTypeCopyNdx = 0; testTypeCopyNdx < DE_LENGTH_OF_ARRAY(testTypeCopy);
                             testTypeCopyNdx++)
                        {
                            TestParameters parametersCopy      = {testTypeCopy[testTypeCopyNdx],
                                                                  bufferSize,
                                                                  0u,
                                                                  streamId,
                                                                  0u,
                            ...
                            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                                          fullTestNameCopy.c_str(), parametersCopy));
                            ...
                            group->addChild(new TransformFeedbackTestCase(
                                group->getTestContext(), fullTestNameQueryWithAvailability.c_str(), parametersCopy));
                        }
                        ...
                        group->addChild(new TransformFeedbackTestCase(
                            group->getTestContext(), fullTestNameHostQueryReset.c_str(), parametersHostQueryReset));
                        ...
                        if (streamId == 0)
                        {
                            ...
                            parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
                            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                                          testNameStream0.c_str(), parameters));
    ...
    {
        TestParameters parameters = {TEST_TYPE_DEPTH_CLIP_CONTROL_VERTEX,
                                     96,
                                     1u,
                                     0u,
                                     0u,
    ...
    {
        TestParameters parameters = {TEST_TYPE_DEPTH_CLIP_CONTROL_GEOMETRY,
                                     96,
                                     1u,
                                     0u,
                                     0u,
    ...
    {
        TestParameters parameters = {TEST_TYPE_DEPTH_CLIP_CONTROL_TESE,
                                     96,
                                     1u,
                                     0u,
                                     0u,
...
void createTransformFeedbackStreamsSimpleTests(tcu::TestCaseGroup *group)
{
    ...
    for (uint32_t testTypesNdx = 0; testTypesNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypesNdx)
    {
        ...
        for (uint32_t streamCountsNdx = 0; streamCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++streamCountsNdx)
        {
            ...
            // Streams usage test
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(), (testName + "_" + de::toString(streamId)).c_str(), parameters));
        }
    }
...

    {
        ...
        for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++bufferCountsNdx)
        {
            ...
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(), (testName + "_" + de::toString(streamId)).c_str(), parameters));
            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_omit_write_" + de::toString(streamId)).c_str(),
                                                          writeOmitParameters));
        }
    }

    {
        ...
        for (const auto &holeCase : holeCases)
            for (const auto &extraDraw : {false, true})
            {
                ...
                const TestParameters parameters{holeCase.testType,
                                                0u,
                                                partCount,
                                                0u,
                                                1u,
