...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const VkShaderModule fragmendModule, const VkExtent2D renderSize,
                                      ...
                                      const bool inputVertices           = false)
{
    ...
    return makeGraphicsPipeline(
        ...
        (tessellationEvalModule != DE_NULL) *
...
protected:
    void validateLimits();
    uint32_t getNextChunkSize(const uint32_t usedBytes, const uint32_t bufBytes);
    ...
    , m_rnd(0)
{
    ...
    if (transformFeedbackFeatures.transformFeedback == false)
                                                                  ...
                                                                  const uint32_t bufBytes)
{
    ...
    const uint32_t *tfData   = (uint32_t *)bufAlloc->getHostPtr();
    ...
    getTopologyParametersMap(void)
{
    static const TopologyParametersMap topologyParametersMap = {
        {TEST_TYPE_LINE_LIST,
         {VK_PRIMITIVE_TOPOLOGY_LINE_LIST, 2u, [](uint32_t partCount) { return partCount; },
          [](uint32_t i) {
              return std::vector<uint32_t>{2 * i, 2 * i + 1u};
          }}},
        {TEST_TYPE_LINE_STRIP,
         {VK_PRIMITIVE_TOPOLOGY_LINE_STRIP, 2u, [](uint32_t partCount) { return 2u * (partCount - 1); },
          [](uint32_t i) {
              return std::vector<uint32_t>{i, i + 1u};
          }}},
        {TEST_TYPE_TRIANGLE_LIST,
         {VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 3u, [](uint32_t partCount) { return partCount; },
          [](uint32_t i) {
              return std::vector<uint32_t>{3 * i, 3 * i + 1u, 3 * i + 2u};
          }}},
        {TEST_TYPE_TRIANGLE_STRIP,
         {VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, 3u, [](uint32_t partCount) { return 3u * (partCount - 2); },
          [](uint32_t i)
          {
        ...
        {TEST_TYPE_TRIANGLE_FAN,
         {VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN, 3u, [](uint32_t partCount) { return partCount; },
          [](uint32_t i) {
              return std::vector<uint32_t>{i + 1, i + 2, 0};
          }}},
        {TEST_TYPE_LINE_LIST_ADJACENCY,
         {VK_PRIMITIVE_TOPOLOGY_LINE_LIST_WITH_ADJACENCY, 2u,
          [](uint32_t partCount)
          {
              return partCount / 4u;
        ...
        {TEST_TYPE_LINE_STRIP_ADJACENCY,
         {VK_PRIMITIVE_TOPOLOGY_LINE_STRIP_WITH_ADJACENCY, 2u, [](uint32_t partCount) { return 2u * (partCount - 3u); },
          [](uint32_t i) {
              return std::vector<uint32_t>{i + 1u, i + 2u};
          }}},
        {TEST_TYPE_TRIANGLE_LIST_ADJACENCY,
         {VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST_WITH_ADJACENCY, 3u, [](uint32_t partCount) { return partCount / 2u; },
          [](uint32_t i) {
              return std::vector<uint32_t>{6 * i, 6 * i + 2u, 6 * i + 4u};
          }}},
        {TEST_TYPE_TRIANGLE_STRIP_ADJACENCY,
         {VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP_WITH_ADJACENCY, 3u,
          [](uint32_t partCount) { return 3u * (partCount / 2u - 2u); },
          [](uint32_t i)
          {
        ...
        {TEST_TYPE_PATCH_LIST,
         {VK_PRIMITIVE_TOPOLOGY_PATCH_LIST, 9u, [](uint32_t partCount) { return partCount * 3u; },
          [](uint32_t i)
          {
                                                                         ...
                                                                         const uint32_t bufBytes)
{
    ...
    const uint32_t numPoints   = static_cast<uint32_t>(bufBytes / sizeof(float));
    const uint8_t *tfDataBytes = (uint8_t *)bufAlloc->getHostPtr();
    ...
    : TransformFeedbackTestInstance(context, parameters)
{
    ...
    if (m_transformFeedbackProperties.transformFeedbackRasterizationStreamSelect == false)
...
tcu::TestStatus TransformFeedbackQueryTestInstance::iterate(void)
{
    ...
    const uint32_t overflowVertices    = 3u;
    ...
    const VkBufferCreateInfo tfBufCreateInfo =
        makeBufferCreateInfo(m_parameters.bufferSize, VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT);
    const Move<VkBuffer> tfBuf = createBuffer(vk, device, &tfBufCreateInfo);
    const MovePtr<Allocation> tfBufAllocation =
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        if (m_parameters.testType == TEST_TYPE_QUERY_COPY)
        {
            vk.cmdCopyQueryPoolResults(*cmdBuffer, *queryPool, queryIndex, queryCountersNumber, *queryPoolResultsBuffer,
                                       0u, queryDataSize, (vk::VK_QUERY_RESULT_WAIT_BIT | queryExtraFlags));
...
vkt::TestInstance *TransformFeedbackTestCase::createInstance(vkt::Context &context) const
{
    ...
    static const std::set<TestType> windingTests = {TEST_TYPE_LINE_LIST,
                                                    TEST_TYPE_LINE_STRIP,
                                                    TEST_TYPE_TRIANGLE_LIST,
                                                    TEST_TYPE_TRIANGLE_STRIP,
                                                    TEST_TYPE_TRIANGLE_FAN,
...
void TransformFeedbackTestCase::initPrograms(SourceCollections &programCollection) const
{
    const bool vertexShaderOnly =
        ...
        m_parameters.testType == TEST_TYPE_BACKWARD_DEPENDENCY || m_parameters.testType == TEST_TYPE_LINE_LIST ||
        m_parameters.testType == TEST_TYPE_LINE_STRIP || m_parameters.testType == TEST_TYPE_TRIANGLE_LIST ||
        m_parameters.testType == TEST_TYPE_TRIANGLE_STRIP || m_parameters.testType == TEST_TYPE_TRIANGLE_FAN ||
    ...
    const bool requiresFullPipeline =
        ...
        m_parameters.testType == TEST_TYPE_STREAMS_CULLDISTANCE ||
        m_parameters.testType == TEST_TYPE_STREAMS_CLIPDISTANCE || m_parameters.testType == TEST_TYPE_PATCH_LIST;
...
{
    {
        const uint32_t bufferCounts[]     = {1, 2, 4, 8};
        const uint32_t bufferSizes[]      = {256, 512, 128 * 1024};
        ...
        for (uint32_t testTypesNdx = 0; testTypesNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypesNdx)
        {
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                for (uint32_t bufferSizesNdx = 0; bufferSizesNdx < DE_LENGTH_OF_ARRAY(bufferSizes); ++bufferSizesNdx)
                {
                    ...
                    TestParameters parameters = {testType, bufferSize,         partCount, 0u,   0u,
                                                 0u,       STREAM_ID_0_NORMAL, false,     false};
    ...
    {
        const uint32_t bufferCounts[]     = {6, 8, 10, 12};
        const TestType testTypes[]        = {// note: no need to test POINT_LIST as is tested in many tests
                                      TEST_TYPE_LINE_LIST,
                                      TEST_TYPE_LINE_STRIP,
                                      TEST_TYPE_TRIANGLE_LIST,
                                      TEST_TYPE_TRIANGLE_STRIP,
                                      ...
                                      TEST_TYPE_TRIANGLE_LIST_ADJACENCY,
                                      TEST_TYPE_PATCH_LIST};
        const std::string testTypeNames[] = {"winding_line_list",
                                             "winding_line_strip",
        ...
        for (uint32_t testTypesNdx = 0; testTypesNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypesNdx)
        {
            ...
            for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(bufferCounts); ++bufferCountsNdx)
            {
                ...
                group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                              (testName + "_" + de::toString(vertexCount)).c_str(),
    ...
    {
        const uint32_t vertexStrides[] = {4, 61, 127, 251, 509};
        ...
        {
            const uint32_t vertexStride = static_cast<uint32_t>(sizeof(uint32_t) * vertexStrides[vertexStridesNdx]);
            TestParameters parameters   = {testType, 0u, 0u, 0u, 0u, vertexStride, STREAM_ID_0_NORMAL, false, false};

            group->addChild(new TransformFeedbackTestCase(group->getTestContext(),
                                                          (testName + "_" + de::toString(vertexStride)).c_str(),
                                                          "Rendering tests with various strides", parameters));
            parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(),
                (testName + "_beginqueryindexed_streamid_0_" + de::toString(vertexStride)).c_str(),
                "Rendering tests with various strides", parameters));
            parameters.streamId0Mode = STREAM_ID_0_END_QUERY_INDEXED;
            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(),
                (testName + "_endqueryindexed_streamid_0_" + de::toString(vertexStride)).c_str(),
                "Rendering tests with various strides", parameters));
        }
    }
    ...
    {
        const uint32_t usedStreamId[]            = {0, 1, 3, 6, 14};
        const uint32_t vertexCount[]             = {4, 61, 127, 251, 509};
        ...
        for (uint32_t streamCountsNdx = 0; streamCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++streamCountsNdx)
        {
            ...
            for (uint32_t vertexCountNdx = 0; vertexCountNdx < DE_LENGTH_OF_ARRAY(vertexCount); ++vertexCountNdx)
            {
                for (uint32_t i = 0; i < 2; ++i)
                {
                    ...
                    const uint32_t bytesPerVertex  = static_cast<uint32_t>(4 * sizeof(float));
                    const uint32_t bufferSize      = bytesPerVertex * vertexCount[vertexCountNdx];
                    TestParameters parameters      = {testType, bufferSize,         0u,          streamId, 0u,
                                                      0u,       STREAM_ID_0_NORMAL, query64Bits, false};
                    const std::string fullTestName = testName + "_" + de::toString(streamId) + "_" +
                                                     de::toString(vertexCount[vertexCountNdx]) + widthStr;
                    ...
                    const TestParameters parametersCopy = {testTypeCopy, bufferSize,         0u,          streamId, 0u,
                                                           0u,           STREAM_ID_0_NORMAL, query64Bits, false};
                    const std::string fullTestNameCopy  = testNameCopy + "_" + de::toString(streamId) + "_" +
                                                         de::toString(vertexCount[vertexCountNdx]) + widthStr;
                    group->addChild(new TransformFeedbackTestCase(group->getTestContext(), fullTestNameCopy.c_str(),
                    ...
                    const TestParameters parametersHostQueryReset = {
                        ...
                        STREAM_ID_0_NORMAL,     query64Bits, false};
                    const std::string fullTestNameHostQueryReset = testNameHostQueryReset + "_" +
                                                                   de::toString(streamId) + "_" +
                                                                   de::toString(vertexCount[vertexCountNdx]) + widthStr;
                    group->addChild(
                    ...
                    if (streamId == 0)
                    {
                        ...
                        parameters.streamId0Mode = STREAM_ID_0_BEGIN_QUERY_INDEXED;
                        group->addChild(new TransformFeedbackTestCase(group->getTestContext(), testNameStream0.c_str(),
                                                                      "Written primitives query test", parameters));
...
{
    const uint32_t usedStreamId[]     = {1, 3, 6, 14};
...

    {
        ...
        for (uint32_t bufferCountsNdx = 0; bufferCountsNdx < DE_LENGTH_OF_ARRAY(usedStreamId); ++bufferCountsNdx)
        {
            ...
            const TestParameters parameters  = {
                testType, maxBytesPerVertex * streamsUsed, streamsUsed, streamId, 0u, 0u, STREAM_ID_0_NORMAL, false,
                false};
