...
protected:
    ...
    void verifyTransformFeedbackBuffer(const MovePtr<Allocation> &bufAlloc, uint32_t bufBytes);
};

void TransformFeedbackMultistreamSameLocationTestInstance::verifyTransformFeedbackBuffer(
    const MovePtr<Allocation> &bufAlloc, const uint32_t bufBytes)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
...

    {
        for (int i = 0; i < 2; ++i)
        {
                 ...
                 ++vertexStridesNdx)
            {
                ...
                group->addChild(new TransformFeedbackTestCase(
                    group->getTestContext(), (testName + "_" + de::toString(vertexStrideBytes)).c_str(),
...
void createTransformFeedbackStreamsSimpleTests(tcu::TestCaseGroup *group)
{
...

    {
        ...
        for (const auto streamId : usedStreamId)
        {
            const uint32_t streamsUsed      = 2u;
            const TestParameters parameters = {
                testType,           32 * 4, streamsUsed, streamId, 0u,    0u,
                STREAM_ID_0_NORMAL, false,  false,       false,    false, VK_PRIMITIVE_TOPOLOGY_POINT_LIST};

            group->addChild(new TransformFeedbackTestCase(
                group->getTestContext(), (testName + "_" + de::toString(streamId)).c_str(),
                "Simultaneous multiple streams to the same location usage test", parameters));
        }
    }
