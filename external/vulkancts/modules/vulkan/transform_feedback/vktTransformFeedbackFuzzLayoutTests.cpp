                               ...
                               const VarType &type, uint32_t layoutFlags, int numInstances = 0)
{
    ...
    typeGroup->addChild(new BlockBasicTypeCase(testCtx, "vertex", "", type, layoutFlags, numInstances, LOAD_FULL_MATRIX,
                                               TEST_STAGE_VERTEX));
    typeGroup->addChild(new BlockBasicTypeCase(testCtx, "geometry", "", type, layoutFlags, numInstances,
...
public:
    BlockMultiBasicTypesCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             uint32_t flagsA, uint32_t flagsB, int numInstances, MatrixLoadFlags matrixLoadFlag,
                             TestStageFlags testStageFlags)
        : InterfaceBlockCase(testCtx, name, description, matrixLoadFlag, testStageFlags)
    {
...
{
public:
    BlockVariousBuffersCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                            uint32_t flags, uint32_t xfbBufferA, uint32_t xfbBufferB, uint32_t xfbBufferC,
...
public:
    Block2LevelStructArrayCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                               uint32_t flags, int numInstances, MatrixLoadFlags matrixLoadFlag,
                               TestStageFlags testStageFlags)
        : InterfaceBlockCase(testCtx, name, description, matrixLoadFlag, testStageFlags)
    {
        StructType &typeS = m_interface.allocStruct("S");
...
void createRandomCaseGroup(tcu::TestCaseGroup *parentGroup, tcu::TestContext &testCtx, const char *groupName,
                           const char *description, uint32_t numCases, TestStageFlags testStageFlags, uint32_t features)
{
...
void InterfaceBlockTests::init(void)
{
    ...
    // .2_level_struct_array
    {
        tcu::TestCaseGroup *structArrayArrayGroup =
            new tcu::TestCaseGroup(m_testCtx, "2_level_struct_array", "Struct array in one interface block");
        ...
        for (int isArray = 0; isArray < 2; isArray++)
        {
            ...
            structArrayArrayGroup->addChild(new Block2LevelStructArrayCase(m_testCtx, (baseName + "_vertex"), "",
    ...
    // .single_struct
    {
        tcu::TestCaseGroup *singleStructGroup =
            new tcu::TestCaseGroup(m_testCtx, "single_struct", "Single struct in interface block");
    ...
    // .single_struct_array
    {
        tcu::TestCaseGroup *singleStructArrayGroup =
            new tcu::TestCaseGroup(m_testCtx, "single_struct_array", "Struct array in one interface block");
        ...
        for (int isArray = 0; isArray < 2; isArray++)
        {
            ...
            singleStructArrayGroup->addChild(new BlockSingleStructArrayCase(m_testCtx, baseName + "_geometry", "",
    ...
    // .single_nested_struct
    {
        tcu::TestCaseGroup *singleNestedStructGroup =
            new tcu::TestCaseGroup(m_testCtx, "single_nested_struct", "Nested struct in one interface block");
        ...
        for (int isArray = 0; isArray < 2; isArray++)
        {
            ...
            singleNestedStructGroup->addChild(new BlockSingleNestedStructCase(
                m_testCtx, baseName + "_vertex", "", defaultFlags, numInstances, LOAD_FULL_MATRIX, TEST_STAGE_VERTEX));
            singleNestedStructGroup->addChild(new BlockSingleNestedStructCase(m_testCtx, baseName + "_geometry", "",
                                                                              defaultFlags, numInstances,
                                                                              LOAD_FULL_MATRIX, TEST_STAGE_GEOMETRY));
        }
    }
    ...
    // .single_nested_struct_array
    {
        tcu::TestCaseGroup *singleNestedStructArrayGroup = new tcu::TestCaseGroup(
            m_testCtx, "single_nested_struct_array", "Nested struct array in one interface block");
        ...
        for (int isArray = 0; isArray < 2; isArray++)
        {
            ...
            singleNestedStructArrayGroup->addChild(new BlockSingleNestedStructArrayCase(
                m_testCtx, baseName + "_vertex", "", defaultFlags, numInstances, LOAD_FULL_MATRIX, TEST_STAGE_VERTEX));
            singleNestedStructArrayGroup->addChild(
                new BlockSingleNestedStructArrayCase(m_testCtx, baseName + "_geometry", "", defaultFlags, numInstances,
                                                     LOAD_FULL_MATRIX, TEST_STAGE_GEOMETRY));
        }
    }
    ...
    // .multi_basic_types
    {
        ...
        for (int isArray = 0; isArray < 2; isArray++)
        {
            ...
            multiBasicTypesGroup->addChild(new BlockMultiBasicTypesCase(m_testCtx, baseName + "_vertex", "",
                                                                        defaultFlags, defaultFlags, numInstances,
                                                                        LOAD_FULL_MATRIX, TEST_STAGE_VERTEX));
            multiBasicTypesGroup->addChild(new BlockMultiBasicTypesCase(m_testCtx, baseName + "_geometry", "",
                                                                        defaultFlags, defaultFlags, numInstances,
                                                                        LOAD_FULL_MATRIX, TEST_STAGE_GEOMETRY));
        }
    }
    ...
    // .various_buffers
    {
        ...
        tcu::TestCaseGroup *multiNestedStructGroup =
            new tcu::TestCaseGroup(m_testCtx, "various_buffers", "Output data into several transform feedback buffers");
        addChild(multiNestedStructGroup);
    ...
    // .random
    {
        ...
        for (size_t stageNdx = 0; stageNdx < DE_LENGTH_OF_ARRAY(stages); ++stageNdx)
        {
            ...
            tcu::TestCaseGroup *group =
                new tcu::TestCaseGroup(m_testCtx, groupName.c_str(), "Random Interface Block cases");
            addChild(group);

            createRandomCaseGroup(group, m_testCtx, "scalar_types", "Scalar types only, per-block buffers", numCases,
                                  stage, unused);
            createRandomCaseGroup(group, m_testCtx, "vector_types", "Scalar and vector types only, per-block buffers",
                                  numCases, stage, unused | FEATURE_VECTORS);
            createRandomCaseGroup(group, m_testCtx, "basic_types", "All basic types, per-block buffers", numCases,
                                  stage, unused | allBasicTypes);
            createRandomCaseGroup(group, m_testCtx, "basic_arrays", "Arrays, per-block buffers", numCases, stage,
            ...
            createRandomCaseGroup(group, m_testCtx, "basic_instance_arrays", "Basic instance arrays, per-block buffers",
                                  numCases, stage, unused | allBasicTypes | FEATURE_INSTANCE_ARRAYS);
            createRandomCaseGroup(group, m_testCtx, "nested_structs", "Nested structs, per-block buffers", numCases,
                                  stage, unused | allBasicTypes | FEATURE_STRUCTS);
            createRandomCaseGroup(group, m_testCtx, "nested_structs_arrays",
                                  "Nested structs, arrays, per-block buffers", numCases, stage,
                                  unused | allBasicTypes | FEATURE_STRUCTS | FEATURE_ARRAYS);
            createRandomCaseGroup(group, m_testCtx, "nested_structs_instance_arrays",
                                  "Nested structs, instance arrays, per-block buffers", numCases, stage,
            ...
            createRandomCaseGroup(group, m_testCtx, "all_instance_array", "All random features, shared buffer",
                                  numCases * 2, stage, allFeatures | FEATURE_INSTANCE_ARRAYS);
            createRandomCaseGroup(group, m_testCtx, "all_unordered_and_instance_array",
                                  "All random features, out of order member offsets", numCases * 2, stage,
                                  allFeatures | FEATURE_OUT_OF_ORDER_OFFSETS | FEATURE_INSTANCE_ARRAYS);
            createRandomCaseGroup(group, m_testCtx, "all_missing", "All random features, missing interface members",
                                  numCases * 2, stage, allFeatures | FEATURE_MISSING_BLOCK_MEMBERS);
            createRandomCaseGroup(group, m_testCtx, "all_unordered_and_missing",
                                  "All random features, unordered and missing members", numCases * 2, stage,
