...
void createRandomCaseGroup (tcu::TestCaseGroup* parentGroup, tcu::TestContext& testCtx, const char* groupName, deUint32 numCases, TestStageFlags testStageFlags, deUint32 features)
{
...
void InterfaceBlockTests::init (void)
{
	...
	// .random
	{
		...
		for (size_t stageNdx = 0; stageNdx < DE_LENGTH_OF_ARRAY(stages); ++stageNdx)
		{
			...
			createRandomCaseGroup(group, m_testCtx, "basic_arrays", numCases, stage, unused|allBasicTypes|FEATURE_ARRAYS);

			// Basic instance arrays, per-block buffers
			createRandomCaseGroup(group, m_testCtx, "basic_instance_arrays", numCases, stage, unused|allBasicTypes|FEATURE_INSTANCE_ARRAYS								);
			// Nested structs, per-block buffers
			createRandomCaseGroup(group, m_testCtx, "nested_structs", numCases, stage, unused|allBasicTypes|FEATURE_STRUCTS										);
			// Nested structs, arrays, per-block buffers
			createRandomCaseGroup(group, m_testCtx, "nested_structs_arrays", numCases, stage, unused|allBasicTypes|FEATURE_STRUCTS|FEATURE_ARRAYS						);
			// Nested structs, instance arrays, per-block buffers
			createRandomCaseGroup(group, m_testCtx, "nested_structs_instance_arrays", numCases, stage, unused|allBasicTypes|FEATURE_STRUCTS|FEATURE_INSTANCE_ARRAYS				);
			// Nested structs, instance arrays, per-block buffers
			createRandomCaseGroup(group, m_testCtx, "nested_structs_arrays_instance_arrays", numCases, stage, unused|allBasicTypes|FEATURE_STRUCTS|FEATURE_ARRAYS|FEATURE_INSTANCE_ARRAYS);

			// All random features, shared buffer
			createRandomCaseGroup(group, m_testCtx, "all_instance_array", numCases*2, stage, allFeatures|FEATURE_INSTANCE_ARRAYS);
			// All random features, out of order member offsets
			createRandomCaseGroup(group, m_testCtx, "all_unordered_and_instance_array", numCases*2, stage, allFeatures|FEATURE_OUT_OF_ORDER_OFFSETS|FEATURE_INSTANCE_ARRAYS);
			// All random features, missing interface members
			createRandomCaseGroup(group, m_testCtx, "all_missing", numCases*2, stage, allFeatures|FEATURE_MISSING_BLOCK_MEMBERS);
			// All random features, unordered and missing members
			createRandomCaseGroup(group, m_testCtx, "all_unordered_and_missing", numCases*2, stage, allFeatures|FEATURE_OUT_OF_ORDER_OFFSETS|FEATURE_MISSING_BLOCK_MEMBERS);
		}
	}
}
