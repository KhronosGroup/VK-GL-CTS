...
InterfaceBlockMember::InterfaceBlockMember(const std::string &name, const VarType &type, uint32_t flags)
    : m_name(name)
    , m_type(type)
    , m_flags(flags)
{
...
{
    stream << entry.name << " { name = " << entry.name << ", buffer = " << entry.xfbBuffer
           << ", offset = " << entry.xfbOffset << ", size = " << entry.xfbSize
           << ", blockDeclarationNdx = " << entry.blockDeclarationNdx << ", instanceNdx = " << entry.instanceNdx
           << ", activeInterfaceIndices = [";

    for (std::vector<int>::const_iterator i = entry.activeInterfaceIndices.begin();
         i != entry.activeInterfaceIndices.end(); i++)
    {
...
{
    stream << entry.name << " { type = " << glu::getDataTypeName(entry.type) << ", arraySize = " << entry.arraySize
           << ", blockNdx = " << entry.blockLayoutNdx << ", offset = " << entry.offset
    ...
    for (int blockNdx = 0; blockNdx < numBlocks; blockNdx++)
    {
        ...
        for (int entryNdx = 0; entryNdx < numEntries; entryNdx++)
        {
            const InterfaceLayoutEntry &entry =
                layout.interfaces[layout.blocks[blockNdx].activeInterfaceIndices[entryNdx]];
               ...
               const void *dataMask = DE_NULL)
{
    ...
    flags = str.flags(std::ios::hex | std::ios::uppercase);
    {
        ...
        {
            if (i % 16 == 0)
                str << std::endl << std::setfill('0') << std::setw(8) << i << ":";
            else if (i % 8 == 0)
                str << "  ";
            else if (i % 4 == 0)
                str << " ";
...
struct LayoutFlagsFmt
{
    ...
    LayoutFlagsFmt(const uint32_t flags_, const uint32_t buffer_, const uint32_t stride_, const uint32_t offset_)
        : flags(flags_)
        , buffer(buffer_)
        , stride(stride_)
        , offset(offset_)
...
std::ostream &operator<<(std::ostream &str, const LayoutFlagsFmt &fmt)
{
    ...
    for (int descNdx = 0; descNdx < DE_LENGTH_OF_ARRAY(bitDesc); descNdx++)
    {
        if (remBits & bitDesc[descNdx].bit)
        {
            ...
            if (bitDesc[descNdx].bit == LAYOUT_XFBBUFFER)
                str << " = " << fmt.buffer;
            if (bitDesc[descNdx].bit == LAYOUT_XFBOFFSET)
                str << " = " << fmt.offset;
            if (bitDesc[descNdx].bit == LAYOUT_XFBSTRIDE)
                str << " = " << fmt.stride;
...
std::ostream &operator<<(std::ostream &str, const DeviceSizeVector &vec)
{
    ...
    for (size_t vecNdx = 0; vecNdx < vec.size(); vecNdx++)
        str << (uint64_t)vec[vecNdx] << (vecNdx + 1 < vec.size() ? ", " : "]");
...
int getDataTypeByteSize(glu::DataType type)
{
    ...
    {
        return glu::getDataTypeScalarSize(type) * (int)sizeof(uint64_t);
    }
    else
    {
...
void createMask(void *maskBasePtr, const InterfaceLayoutEntry &entry, const void *basePtr0, const void *basePtr)
{
    ...
    for (int elemNdx = 0; elemNdx < entry.arraySize; elemNdx++)
    {
        ...
        for (int vecNdx = 0; vecNdx < numVecs; vecNdx++)
        {
            ...
            {
                const uint8_t *compPtr = vecPtr + compSize * compNdx;
                const size_t offset    = compPtr - (uint8_t *)basePtr0;
                uint8_t *maskPtr       = (uint8_t *)maskBasePtr + offset;
...
std::vector<uint8_t> createMask(const InterfaceLayout &layout, const std::map<int, void *> &blockPointers,
                                const void *basePtr0, const size_t baseSize)
{
    std::vector<uint8_t> mask(baseSize, 0);
    const int numBlocks((int)layout.blocks.size());
...
int computeInterfaceBlockAlignment(const InterfaceBlock &interfaceBlock)
{
    ...
    for (InterfaceBlock::ConstIterator memberIter = interfaceBlock.begin(); memberIter != interfaceBlock.end();
         memberIter++)
    {
...
static inline bool isOverlaped(const int a1, const int b1, const int a2, const int b2)
{
    ...
    return deInRange32(a1, a2, b2s) || deInRange32(b1s, a2, b2s) || deInRange32(a2, a1, b1s) ||
           deInRange32(b2s, a1, b1s);
}

void computeXfbLayout(InterfaceLayout &layout, int &curOffset, int &curLocation, int curBlockNdx,
                      const std::string &curPrefix, const VarType &type, uint32_t layoutFlags)
{
    ...
    DE_ASSERT(baseAlignment == sizeof(uint32_t) || baseAlignment == sizeof(uint64_t));
    ...
    else if (type.isArrayType())
    {
        ...
        else if (elemType.isBasicType() && glu::isDataTypeMatrix(elemType.getBasicType()))
        {
            ...
            const int strideForLocation =
                getDataTypeArrayStrideForLocation(glu::getDataTypeVector(scalarType, vecSize));
        ...
        else
        {
            ...
            for (int elemNdx = 0; elemNdx < type.getArraySize(); elemNdx++)
                computeXfbLayout(layout, curOffset, curLocation, curBlockNdx,
                                 curPrefix + "[" + de::toString(elemNdx) + "]", type.getElementType(), layoutFlags);
        }
    }
    else
    {
        ...
        for (StructType::ConstIterator memberIter = type.getStruct().begin(); memberIter != type.getStruct().end();
             memberIter++)
            computeXfbLayout(layout, curOffset, curLocation, curBlockNdx, curPrefix + "." + memberIter->getName(),
                             memberIter->getType(), (memberIter->getFlags() | layoutFlags) & FIELD_OPTIONS);

        curOffset = deAlign32(curOffset, baseAlignment);
    }
}

void computeXfbLayout(InterfaceLayout &layout, ShaderInterface &shaderInterface,
                      BufferGeneralMapping &perBufferXfbOffsets, uint32_t &locationsUsed)
{
    ...
    for (BufferGeneralMapping::const_iterator xfbBuffersIter = buffersList.begin(); xfbBuffersIter != buffersList.end();
         xfbBuffersIter++)
    {
        ...
        for (int blockNdx = 0; blockNdx < numInterfaceBlocks; blockNdx++)
        {
            ...
            if (interfaceBlock.getXfbBuffer() == xfbBufferAnalyzed)
            {
                ...
                do
                {
                    ...
                    for (InterfaceBlock::ConstIterator memberIter = interfaceBlock.begin();
                         memberIter != interfaceBlock.end(); memberIter++)
                    {
                        const InterfaceBlockMember &member = *memberIter;

                        computeXfbLayout(layout, curOffset, curLocation, activeBlockNdx, blockPrefix + member.getName(),
                                         member.getType(), member.getFlags() & FIELD_OPTIONS);
                    }
                    ...
                    for (UsedRangeList::const_iterator usedRangeIt = usedRanges.begin();
                         usedRangeIt != usedRanges.end(); ++usedRangeIt)
                    {
                ...
                for (int instanceNdx = 0; instanceNdx < numInstances; instanceNdx++)
                {
                    ...
                    bufferUsedRanges[xfbBuffer].push_back(
                        UsedRange(blockEntry.xfbOffset, blockEntry.xfbOffset + blockEntry.xfbSize));
    ...
    // All XFB buffers within group must have same stride
    {
        ...
        for (BufferGeneralMapping::const_iterator xfbBuffersIter = perBufferXfbOffsets.begin();
             xfbBuffersIter != perBufferXfbOffsets.end(); xfbBuffersIter++)
        {
    ...
    locationsUsed = static_cast<uint32_t>(curLocation);
}
    ...
    for (int elemNdx = 0; elemNdx < entry.arraySize; elemNdx++)
    {
        ...
        for (int vecNdx = 0; vecNdx < numVecs; vecNdx++)
        {
            ...
            for (int compNdx = 0; compNdx < vecSize; compNdx++)
            {
                ...
                {
                case glu::TYPE_DOUBLE:
                    *((double *)compPtr) = (double)(sign * value);
                    break;
                case glu::TYPE_FLOAT:
                    *((float *)compPtr) = (float)(sign * value);
                    break;
                case glu::TYPE_INT:
                    *((int32_t *)compPtr) = (int32_t)(sign * value);
                    break;
                case glu::TYPE_UINT:
                    *((uint32_t *)compPtr) = (uint32_t)(value);
                    break;
                default:
                    DE_ASSERT(false);
                }
...
void generateValues(const InterfaceLayout &layout, const std::map<int, void *> &blockPointers, uint32_t seed)
{
    de::Random rnd(seed);
...
void generateDeclaration(std::ostringstream &src, const VarType &type, const std::string &name, int indentLevel,
                         uint32_t unusedHints, uint32_t flagsMask, uint32_t buffer, uint32_t stride, uint32_t offset);
void generateDeclaration(std::ostringstream &src, const InterfaceBlockMember &member, int indentLevel, uint32_t buffer,
...
void generateFullDeclaration(std::ostringstream &src, const StructType &structType, int indentLevel)
{
    ...
    for (StructType::ConstIterator memberIter = structType.begin(); memberIter != structType.end(); memberIter++)
    {
        ...
        generateDeclaration(src, memberIter->getType(), memberIter->getName(), indentLevel + 1,
                            memberIter->getFlags() & FIELD_OPTIONS, ~LAYOUT_MASK, 0u, 0u, 0u);
    }
...
void generateLayoutAndPrecisionDeclaration(std::ostringstream &src, uint32_t flags, uint32_t buffer, uint32_t stride,
                                           uint32_t offset)
{
...
void generateDeclaration(std::ostringstream &src, const VarType &type, const std::string &name, int indentLevel,
                         uint32_t fieldHints, uint32_t flagsMask, uint32_t buffer, uint32_t stride, uint32_t offset)
{
                         ...
                         uint32_t stride, uint32_t offset)
{
    ...
    generateDeclaration(src, member.getType(), member.getName(), indentLevel, member.getFlags() & FIELD_OPTIONS, ~0u,
                        buffer, stride, offset);
}

uint32_t getBlockMemberOffset(int blockNdx, const InterfaceBlock &block, const InterfaceBlockMember &member,
...
void generateDeclaration(std::ostringstream &src, int blockNdx, const InterfaceBlock &block,
                         const InterfaceLayout &layout, bool shuffleUniformMembers)
{
    ...
    src << " //"
        << " sizeInBytes=" << layout.blocks[ndx].xfbSize << " sizeInLocations=" << layout.blocks[ndx].locationSize;
    ...
    while (const InterfaceBlockMember *pUniform = interfaces.next())
    {
        ...
        generateDeclaration(src, *pUniform, indentOne, xfbBuffer, xfbStride,
                            xfbOffset + getBlockMemberOffset(blockNdx, block, *pUniform, layout));
    }
...
int generateValueSrc(std::ostringstream &src, const InterfaceLayoutEntry &entry, const void *basePtr, int elementNdx)
{
    ...
    if (glu::isDataTypeMatrix(entry.type))
    {
        ...
        for (int colNdx = 0; colNdx < numCols; colNdx++)
        {
            ...
            {
                const uint8_t *compPtr = elemPtr + (colNdx * entry.matrixStride + rowNdx * compSize);
    ...
    else
    {
        for (int scalarNdx = 0; scalarNdx < scalarSize; scalarNdx++)
        {
            ...
            {
            case glu::TYPE_DOUBLE:
                src << de::floatToString((float)(*((const double *)compPtr)), 1);
                break;
            case glu::TYPE_FLOAT:
                src << de::floatToString(*((const float *)compPtr), 1) << "f";
                break;
            case glu::TYPE_INT:
    ...
    return static_cast<int>(elemPtr - static_cast<const uint8_t *>(basePtr));
}

void writeMatrixTypeSrc(int columnCount, int rowCount, std::string type, std::ostringstream &src,
                        const std::string &srcName, const void *basePtr, const InterfaceLayoutEntry &entry, bool vector)
{
    if (vector) // generateTestSrcMatrixPerVec
    {
        for (int colNdx = 0; colNdx < columnCount; colNdx++)
        {
            ...
            if (glu::isDataTypeMatrix(entry.type))
            {
                const glu::DataType scalarType = glu::getDataTypeScalarType(entry.type);
                const int scalarSize           = glu::getDataTypeScalarSize(entry.type);
                const uint8_t *compPtr         = (const uint8_t *)basePtr + entry.offset;
...
void generateTestSrcMatrixPerVec(std::ostringstream &src, glu::DataType elementType, const std::string &srcName,
                                 const void *basePtr, const InterfaceLayoutEntry &entry)
{
    switch (elementType)
    {
    case glu::TYPE_FLOAT_MAT2:
        writeMatrixTypeSrc(2, 2, "vec2", src, srcName, basePtr, entry, true);
        break;
    case glu::TYPE_FLOAT_MAT2X3:
        writeMatrixTypeSrc(2, 3, "vec3", src, srcName, basePtr, entry, true);
        break;
    case glu::TYPE_FLOAT_MAT2X4:
        writeMatrixTypeSrc(2, 4, "vec4", src, srcName, basePtr, entry, true);
        break;
    case glu::TYPE_FLOAT_MAT3X4:
        writeMatrixTypeSrc(3, 4, "vec4", src, srcName, basePtr, entry, true);
        break;
    case glu::TYPE_FLOAT_MAT4:
        writeMatrixTypeSrc(4, 4, "vec4", src, srcName, basePtr, entry, true);
        break;
    case glu::TYPE_FLOAT_MAT4X2:
        writeMatrixTypeSrc(4, 2, "vec2", src, srcName, basePtr, entry, true);
        break;
    case glu::TYPE_FLOAT_MAT4X3:
        writeMatrixTypeSrc(4, 3, "vec3", src, srcName, basePtr, entry, true);
        break;
    default:
        DE_ASSERT(false && "Invalid type");
        break;
...
void generateTestSrcMatrixPerElement(std::ostringstream &src, glu::DataType elementType, const std::string &srcName,
                                     const void *basePtr, const InterfaceLayoutEntry &entry)
{
    ...
    {
    case glu::TYPE_FLOAT_MAT2:
        writeMatrixTypeSrc(2, 2, type, src, srcName, basePtr, entry, false);
        break;
    case glu::TYPE_FLOAT_MAT2X3:
        writeMatrixTypeSrc(2, 3, type, src, srcName, basePtr, entry, false);
        break;
    case glu::TYPE_FLOAT_MAT2X4:
        writeMatrixTypeSrc(2, 4, type, src, srcName, basePtr, entry, false);
        break;
    case glu::TYPE_FLOAT_MAT3X4:
        writeMatrixTypeSrc(3, 4, type, src, srcName, basePtr, entry, false);
        break;
    case glu::TYPE_FLOAT_MAT4:
        writeMatrixTypeSrc(4, 4, type, src, srcName, basePtr, entry, false);
        break;
    case glu::TYPE_FLOAT_MAT4X2:
        writeMatrixTypeSrc(4, 2, type, src, srcName, basePtr, entry, false);
        break;
    case glu::TYPE_FLOAT_MAT4X3:
        writeMatrixTypeSrc(4, 3, type, src, srcName, basePtr, entry, false);
        break;
    default:
        DE_ASSERT(false && "Invalid type");
        break;
...
void generateSingleAssignment(std::ostringstream &src, glu::DataType elementType, const std::string &srcName,
                              const void *basePtr, const InterfaceLayoutEntry &entry, MatrixLoadFlags matrixLoadFlag)
{
...
void generateAssignment(std::ostringstream &src, const InterfaceLayout &layout, const VarType &type,
                        const std::string &srcName, const std::string &apiName, int blockNdx, const void *basePtr,
                        MatrixLoadFlags matrixLoadFlag)
{
    ...
    else if (type.isArrayType())
    {
        ...
        for (int elementNdx = 0; elementNdx < type.getArraySize(); elementNdx++)
        {
            ...
            generateAssignment(src, layout, elementType, elementSrcName, elementApiName, blockNdx, basePtr,
                               LOAD_FULL_MATRIX);
        }
    }
    else
    {
             ...
             memberIter++)
        {
            ...
            if (0 == (member.getFlags() & (FIELD_UNASSIGNED | FIELD_MISSING)))
                generateAssignment(src, layout, memberIter->getType(), memberSrcName, memberApiName, blockNdx, basePtr,
                                   LOAD_FULL_MATRIX);
        }
    }
}

void generateAssignment(std::ostringstream &src, const InterfaceLayout &layout, const ShaderInterface &shaderInterface,
    ...
    for (int blockNdx = 0; blockNdx < shaderInterface.getNumInterfaceBlocks(); blockNdx++)
    {
        ...
        for (int instanceNdx = 0; instanceNdx < numInstances; instanceNdx++)
        {
            std::string instancePostfix =
                isArray ? std::string("[") + de::toString(instanceNdx) + "]" : std::string("");
            ...
            for (InterfaceBlock::ConstIterator interfaceMemberIter = block.begin(); interfaceMemberIter != block.end();
                 interfaceMemberIter++)
            {
                ...
                if ((interfaceMember.getFlags() & (FIELD_MISSING | FIELD_UNASSIGNED)) == 0)
                {
                    ...
                    generateAssignment(src, layout, interfaceMember.getType(), srcName, apiName, blockNdx, basePtr,
                                       matrixLoadFlag);
                }
            }
        }
...
std::string generateTestShader(const ShaderInterface &shaderInterface, const InterfaceLayout &layout,
                               const std::map<int, void *> &blockPointers, MatrixLoadFlags matrixLoadFlag,
                               TestStageFlags testStageFlags, bool shuffleUniformMembers)
{
    std::ostringstream src;
...
{
public:
    InterfaceBlockCaseInstance(Context &context, const InterfaceLayout &layout,
                               const std::map<int, void *> &blockPointers, const std::vector<uint8_t> &data,
                               const std::vector<VkDeviceSize> &tfBufBindingOffsets,
                               const std::vector<VkDeviceSize> &tfBufBindingSizes, const uint32_t locationsRequired,
...
private:
    Move<VkShaderModule> getGeometryShaderModule(const DeviceInterface &vk, const VkDevice device);

    bool usesFloat64(void);
    std::string validateValue(const InterfaceLayoutEntry &entry, const void *basePtr0, const void *basePtr,
                              const void *receivedBasePtr);
    ...
    const uint32_t m_locationsRequired;
    const TestStageFlags m_testStageFlags;
    const VkExtent2D m_imageExtent2D;
};

InterfaceBlockCaseInstance::InterfaceBlockCaseInstance(Context &ctx, const InterfaceLayout &layout,
                                                       const std::map<int, void *> &blockPointers,
                                                       const std::vector<uint8_t> &data,
                                                       const std::vector<VkDeviceSize> &tfBufBindingOffsets,
                                                       const std::vector<VkDeviceSize> &tfBufBindingSizes,
                                                       const uint32_t locationsRequired,
                                                       const TestStageFlags testStageFlags)
...
{
    const uint32_t componentsPerLocation = 4u;
    const uint32_t componentsRequired =
        m_locationsRequired * componentsPerLocation + 7u; // Add 7 for built-in components
    ...
    if (transformFeedbackProperties.maxTransformFeedbackBuffers < tfBufBindingSizes.size())
        TCU_THROW(NotSupportedError, "maxTransformFeedbackBuffers=" +
                                         de::toString(transformFeedbackProperties.maxTransformFeedbackBuffers) +
                                         " is less than required (" + de::toString(tfBufBindingSizes.size()) + ")");

    if (transformFeedbackProperties.maxTransformFeedbackBufferDataSize < m_data.size())
        TCU_THROW(NotSupportedError, "maxTransformFeedbackBufferDataSize=" +
                                         de::toString(transformFeedbackProperties.maxTransformFeedbackBufferDataSize) +
                                         " is less than required (" + de::toString(m_data.size()) + ")");

    if (m_testStageFlags == TEST_STAGE_VERTEX)
    {
        if (limits.maxVertexOutputComponents < componentsRequired)
            TCU_THROW(NotSupportedError, "maxVertexOutputComponents=" + de::toString(limits.maxVertexOutputComponents) +
    ...
    if (m_testStageFlags == TEST_STAGE_GEOMETRY)
    {
        ...
        if (limits.maxGeometryOutputComponents < componentsRequired)
            TCU_THROW(NotSupportedError,
                      "maxGeometryOutputComponents=" + de::toString(limits.maxGeometryOutputComponents) +
                          " is less than required (" + de::toString(componentsRequired) + ")");
    }
...
tcu::TestStatus InterfaceBlockCaseInstance::iterate(void)
{
    ...
    const VkBufferCreateInfo tfBufCreateInfo = makeBufferCreateInfo(
        m_data.size(), VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT);
    const Move<VkBuffer> tfBuf = createBuffer(vk, device, &tfBufCreateInfo);
    const de::MovePtr<Allocation> tfBufAllocation =
        allocator.allocate(getBufferMemoryRequirements(vk, device, *tfBuf), MemoryRequirement::HostVisible);
    const uint32_t tfBufBindingCount = static_cast<uint32_t>(m_tfBufBindingOffsets.size());
...
std::string InterfaceBlockCaseInstance::validateValue(const InterfaceLayoutEntry &entry, const void *basePtr0,
                                                      const void *basePtr, const void *receivedBasePtr)
{
    const glu::DataType scalarType = glu::getDataTypeScalarType(entry.type);
    const int scalarSize           = glu::getDataTypeScalarSize(entry.type);
    const bool isMatrix            = glu::isDataTypeMatrix(entry.type);
    ...
    for (int elemNdx = 0; elemNdx < entry.arraySize; elemNdx++)
    {
        ...
        for (int vecNdx = 0; vecNdx < numVecs; vecNdx++)
        {
            ...
            for (int compNdx = 0; compNdx < vecSize; compNdx++)
            {
                const uint8_t *compPtr     = vecPtr + compSize * compNdx;
                const size_t offset        = compPtr - (uint8_t *)basePtr0;
                const uint8_t *receivedPtr = (uint8_t *)receivedBasePtr + offset;
                ...
                case glu::TYPE_DOUBLE:
                {
                    ...
                    if (deAbs(received - expected) > 0.05)
                        result = "Mismatch at offset " + de::toString(offset) + " expected " + de::toString(expected) +
                                 " received " + de::toString(received);
                ...
                case glu::TYPE_INT:
                {
                    const int32_t expected = *((int32_t *)compPtr);
                    const int32_t received = *((int32_t *)receivedPtr);
                ...
                case glu::TYPE_UINT:
                {
                    const uint32_t expected = *((uint32_t *)compPtr);
                    const uint32_t received = *((uint32_t *)receivedPtr);
                ...
                if (!result.empty())
                {
                    result += " (elemNdx=" + de::toString(elemNdx) + " vecNdx=" + de::toString(vecNdx) +
                              " compNdx=" + de::toString(compNdx) + ")";
...
std::string InterfaceBlockCaseInstance::validateValues(const void *recievedDataPtr)
{
    ...
    for (int blockNdx = 0; blockNdx < numBlocks; blockNdx++)
    {
        ...
        {
            const InterfaceLayoutEntry &entry =
                m_layout.interfaces[m_layout.blocks[blockNdx].activeInterfaceIndices[entryNdx]];
            const std::string result = entry.validate ? validateValue(entry, &m_data[0], basePtr, recievedDataPtr) : "";

            if (!result.empty())
            {
                tcu::TestLog &log         = m_context.getTestContext().getLog();
                std::vector<uint8_t> mask = createMask(m_layout, m_blockPointers, &m_data[0], m_data.size());
                std::ostringstream str;

                str << "Error at entry '" << entry.name << "' block '" << m_layout.blocks[blockNdx].name << "'"
                    << std::endl;
                str << result << std::endl;
...
InterfaceBlockCase::InterfaceBlockCase(tcu::TestContext &testCtx, const std::string &name,
                                       const std::string &description, MatrixLoadFlags matrixLoadFlag,
                                       TestStageFlags testStageFlags, bool shuffleInterfaceMembers)
...
{
    return new InterfaceBlockCaseInstance(context, m_interfaceLayout, m_blockPointers, m_data, m_tfBufBindingOffsets,
...
void InterfaceBlockCase::delayedInit(void)
{
    ...
    // m_data contains all xfb buffers starting with all interfaces of first xfb_buffer, then all interfaces of next xfb_buffer
    {
        ...
        for (BufferGeneralMapping::const_iterator xfbBuffersIter = xfbBufferSize.begin();
             xfbBuffersIter != xfbBufferSize.end(); xfbBuffersIter++)
        {
        ...
        for (BufferGeneralMapping::const_iterator xfbBuffersIter = xfbBufferOffsets.begin();
             xfbBuffersIter != xfbBufferOffsets.end(); xfbBuffersIter++)
    ...
    // Overlap validation
    {
        std::vector<uint8_t> mask = createMask(m_interfaceLayout, m_blockPointers, &m_data[0], m_data.size());
    ...
    if (m_testStageFlags == TEST_STAGE_VERTEX)
    {
        m_vertShaderSource = generateTestShader(m_interface, m_interfaceLayout, m_blockPointers, m_matrixLoadFlag,
                                                m_testStageFlags, m_shuffleInterfaceMembers);
        m_geomShaderSource = "";
    }
    else if (m_testStageFlags == TEST_STAGE_GEOMETRY)
    {
        m_vertShaderSource = generatePassthroughShader();
        m_geomShaderSource = generateTestShader(m_interface, m_interfaceLayout, m_blockPointers, m_matrixLoadFlag,
                                                m_testStageFlags, m_shuffleInterfaceMembers);
    }
    else
    {
