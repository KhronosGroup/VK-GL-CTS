...
void populatePresentFenceGroup(tcu::TestCaseGroup *testGroup, Type wsiType)
{
    ...
    for (size_t presentModeNdx = 0; presentModeNdx < DE_LENGTH_OF_ARRAY(presentModes); presentModeNdx++)
    {
        ...
        addFunctionCase(&*presentModeGroup, "ordering", "Test ordering guarantee of present fence signals",
        ...
        if (canDoMultiSwapchainPresent(wsiType))
        {
            ...
            addFunctionCase(&*presentModeGroup, "multi_swapchain", "Present fence test with multiple swapchains",
                            presentFenceTest, config);

            config.verifyFenceOrdering = true;
            addFunctionCase(&*presentModeGroup, "mult_swapchain_ordering",
...
void populatePresentModesGroup(tcu::TestCaseGroup *testGroup, Type wsiType)
{
    ...
    for (size_t presentModeNdx = 0; presentModeNdx < DE_LENGTH_OF_ARRAY(presentModes); presentModeNdx++)
    {
...

        {
            ...
            addFunctionCase(&*presentModeGroup, "query", "Query compatible present modes", presentModesQueryTest,
                            config);
        }

        {
            ...
            addFunctionCase(&*presentModeGroup, "change_modes", "Switch between compatible modes", presentFenceTest,
                            config);
            ...
            if (canDoMultiSwapchainPresent(wsiType))
            {
                ...
                addFunctionCase(&*presentModeGroup, "change_modes_multi_swapchain",
                                "Switch between compatible modes with multiple swapchains", presentFenceTest, config);

                config.modes                 = std::vector<VkPresentModeKHR>(2, presentModes[presentModeNdx].mode);
                config.deferMemoryAllocation = true;

                addFunctionCase(&*presentModeGroup, "change_modes_with_deferred_alloc",
    ...
    for (size_t presentModeNdx = 0; presentModeNdx < DE_LENGTH_OF_ARRAY(presentModes); presentModeNdx++)
    {
...

        {
            ...
            de::MovePtr<tcu::TestCaseGroup> queryGroup(
                new tcu::TestCaseGroup(testGroup->getTestContext(), "query", "Query supported scaling modes"));
            addFunctionCase(&*queryGroup, "basic", "Basic test", scalingQueryTest, config);
            addFunctionCase(&*queryGroup, "verify_compatible_present_modes",
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> scalingFlagGroup(new tcu::TestCaseGroup(
                testGroup->getTestContext(), scalingFlags[scalingFlagNdx].name, scalingFlags[scalingFlagNdx].name));
            ...
            for (size_t gravityFlagXNdx = 0; gravityFlagXNdx < DE_LENGTH_OF_ARRAY(gravityFlags); gravityFlagXNdx++)
            {
                for (size_t gravityFlagYNdx = 0; gravityFlagYNdx < DE_LENGTH_OF_ARRAY(gravityFlags); gravityFlagYNdx++)
                {
                    ...
                    de::MovePtr<tcu::TestCaseGroup> gravityFlagsGroup(
                        new tcu::TestCaseGroup(scalingFlagGroup->getTestContext(), testName.c_str(), testName.c_str()));
                    ...
                    addFunctionCase(&**group, "same_size_and_aspect", "Basic test without actual scaling", scalingTest,
                                    config);

                    config.size = SwapchainWindowSize::SwapchainBigger;
                    addFunctionCase(&**group, "swapchain_bigger_same_aspect",
                                    "Swapchain is bigger than window, but has same aspect", scalingTest, config);

                    config.size = SwapchainWindowSize::SwapchainSmaller;
                    addFunctionCase(&**group, "swapchain_smaller_same_aspect",
                                    "Swapchain is smaller than window, but has same aspect", scalingTest, config);

                    config.size   = SwapchainWindowSize::Identical;
                    config.aspect = SwapchainWindowAspect::SwapchainTaller;
                    addFunctionCase(&**group, "swapchain_taller", "Swapchain has same width, but is taller than window",
                                    scalingTest, config);

                    config.size = SwapchainWindowSize::SwapchainBigger;
                    addFunctionCase(&**group, "swapchain_bigger_taller_aspect",
                                    "Swapchain is bigger than window, and is taller in aspect ratio", scalingTest,
                                    config);

                    config.size = SwapchainWindowSize::SwapchainSmaller;
                    addFunctionCase(&**group, "swapchain_smaller_taller_aspect",
                                    "Swapchain is smaller than window, but is taller in aspect ratio", scalingTest,
                                    config);

                    config.size   = SwapchainWindowSize::Identical;
                    config.aspect = SwapchainWindowAspect::SwapchainWider;
                    addFunctionCase(&**group, "swapchain_wider", "Swapchain has same height, but is wider than window",
                                    scalingTest, config);

                    config.size = SwapchainWindowSize::SwapchainBigger;
                    addFunctionCase(&**group, "swapchain_bigger_wider_aspect",
                                    "Swapchain is bigger than window, and is wider in aspect ratio", scalingTest,
                                    config);

                    config.size = SwapchainWindowSize::SwapchainSmaller;
                    addFunctionCase(&**group, "swapchain_smaller_wider_aspect",
                                    "Swapchain is smaller than window, but is wider in aspect ratio", scalingTest,
                                    config);
...
void populateScalingGroup(tcu::TestCaseGroup *testGroup, Type wsiType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> resizeWindowGroup(
        new tcu::TestCaseGroup(testGroup->getTestContext(), "resize_window",
                               "Resize the window instead of creating the swapchain with a different size"));
...
void populateDeferredAllocGroup(tcu::TestCaseGroup *testGroup, Type wsiType)
{
    ...
    for (size_t presentModeNdx = 0; presentModeNdx < DE_LENGTH_OF_ARRAY(presentModes); presentModeNdx++)
    {
        ...
        {
            addFunctionCase(&*presentModeGroup, "bind_image", "Bind image with VkBindImageMemorySwapchainInfoKHR",
                            presentFenceTest, config);
        }
        ...
        if (canDoMultiSwapchainPresent(wsiType))
        {
            ...
            addFunctionCase(&*presentModeGroup, "bind_image_multi_swapchain",
...
void populateReleaseImagesGroup(tcu::TestCaseGroup *testGroup, Type wsiType)
{
    ...
    for (size_t presentModeNdx = 0; presentModeNdx < DE_LENGTH_OF_ARRAY(presentModes); presentModeNdx++)
    {
        ...
        for (size_t scalingFlagNdx = 0; scalingFlagNdx < DE_LENGTH_OF_ARRAY(scalingFlags); scalingFlagNdx++)
        {
            ...
            addFunctionCase(&*scalingFlagGroup, "basic", "Basic release acquired images test", releaseImagesTest,
                            config);

            config.releaseBeforePresent = true;
            addFunctionCase(&*scalingFlagGroup, "release_before_present",
            ...
            addFunctionCase(&*scalingFlagGroup, "resize_window",
                            "Release acquired images after a window resize before acquire", releaseImagesTest, config);

            config.resizeWindow = ResizeWindow::BeforePresent;
            addFunctionCase(&*scalingFlagGroup, "resize_window_after_acquire",
                            "Release acquired images after a window resize after acquire", releaseImagesTest, config);

            config.releaseBeforeRetire = true;
            addFunctionCase(&*scalingFlagGroup, "resize_window_after_acquire_release_before_retire",
                            "Release acquired images after a window resize after acquire, but release the images "
                            "before retiring the swapchain",
                            releaseImagesTest, config);
