...
vk::VkRect2D getRenderFrameRect(size_t frameNdx, uint32_t imageWidth, uint32_t imageHeight)
{
    ...
    const uint32_t width =
        frameNdx == 0 ? imageWidth :
                        1 + de::min((uint32_t)(frameNdx) % de::min<uint32_t>(100, imageWidth / 3), imageWidth - x);
    const uint32_t height =
        frameNdx == 0 ? imageHeight :
                        1 + de::min((uint32_t)(frameNdx) % de::min<uint32_t>(100, imageHeight / 3), imageHeight - y);
    ...
    const vk::VkSurfaceTransformFlagsKHR transform, const vk::VkCompositeAlphaFlagsKHR alpha)
{
    ...
    const vk::VkExtent2D dummySize = {de::max(31u, properties.minImageExtent.width),
                                      de::max(31u, properties.minImageExtent.height)};
    ...
    for (size_t formatNdx = 0; formatNdx < formats.size(); formatNdx++)
    {
        ...
        const vk::VkSwapchainCreateInfoKHR dummyInfo = {vk::VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR,
                                                        DE_NULL,
                                                        0u,
                                                        surface,
                                                        properties.minImageCount,
...
void createIncrementalPresentTests(tcu::TestCaseGroup *testGroup, vk::wsi::Type wsiType)
{
    ...
    for (size_t scalingNdx = 0; scalingNdx < DE_LENGTH_OF_ARRAY(scaling); scalingNdx++)
    {
...

        {
            ...
            de::MovePtr<tcu::TestCaseGroup> scaleGroup(new tcu::TestCaseGroup(
                testGroup->getTestContext(), scaling[scalingNdx].name, scaling[scalingNdx].name));

            for (size_t presentModeNdx = 0; presentModeNdx < DE_LENGTH_OF_ARRAY(presentModes); presentModeNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> presentModeGroup(new tcu::TestCaseGroup(
                    testGroup->getTestContext(), presentModes[presentModeNdx].name, presentModes[presentModeNdx].name));

                for (size_t transformNdx = 0; transformNdx < DE_LENGTH_OF_ARRAY(transforms); transformNdx++)
                {
                    de::MovePtr<tcu::TestCaseGroup> transformGroup(new tcu::TestCaseGroup(
                        testGroup->getTestContext(), transforms[transformNdx].name, transforms[transformNdx].name));

                    for (size_t alphaNdx = 0; alphaNdx < DE_LENGTH_OF_ARRAY(alphas); alphaNdx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> alphaGroup(new tcu::TestCaseGroup(
                            testGroup->getTestContext(), alphas[alphaNdx].name, alphas[alphaNdx].name));
                        ...
                        for (size_t ref = 0; ref < 2; ref++)
                        {
                            ...
                            alphaGroup->addChild(
                                new vkt::InstanceFactory1<IncrementalPresentTestInstance, TestConfig, Programs>(
                                    testGroup->getTestContext(), tcu::NODETYPE_SELF_VALIDATE, name, name, Programs(),
                                    config));
                        }
