...
Move<VkDevice> createDeviceWithWsi(const PlatformInterface &vkp, VkInstance instance, const InstanceInterface &vki,
                                   ...
                                   const VkAllocationCallbacks *pAllocator = DE_NULL)
{
    return createDeviceWithWsi(vkp, instance, vki, physicalDevice, supportedExtensions,
                               vector<uint32_t>(1u, queueFamilyIndex), validationEnabled, pAllocator);
}
...
struct DeviceHelper
{
    ...
    DeviceHelper(Context &context, const InstanceInterface &vki, VkInstance instance,
                 const vector<VkSurfaceKHR> &surface, const VkAllocationCallbacks *pAllocator = DE_NULL)
...
public:
    NativeObjects(Context &context, const Extensions &supportedExtensions, Type wsiType, size_t windowCount = 1u,
    ...
    NativeObjects(NativeObjects &&other) : display(other.display.move()), windows()
    {
        ...
        case VK_SUCCESS:
        {
            ...
            for (int i = 0; i < numSlots; ++i)
            {
                Move<VkPrivateDataSlotEXT> s =
                    createPrivateDataSlotEXT(devHelper.vkd, *devHelper.device, &createInfo, DE_NULL);
            ...
            for (int r = 0; r < 3; ++r)
            {
                ...
                {
                    data = 1234;
                    devHelper.vkd.getPrivateDataEXT(*devHelper.device, getObjectType<VkSwapchainKHR>(),
                ...
                for (int i = 0; i < numSlots; ++i)
                    VK_CHECK(devHelper.vkd.setPrivateDataEXT(*devHelper.device, getObjectType<VkSwapchainKHR>(),
...
void populateSwapchainGroup(tcu::TestCaseGroup *testGroup, GroupParameters params)
{
    ...
    addFunctionCase(testGroup, "image_swapchain_create_info", "Test VkImageSwapchainCreateInfoKHR",
                    testImageSwapchainCreateInfo, params.wsiType);
}
...

                {
                    const VkResult acquireResult =
                        vkd.acquireNextImageKHR(device, *swapchain, std::numeric_limits<uint64_t>::max(),
                                                imageReadySemaphore, imageReadyFence, &imageNdx);
...
tcu::TestStatus acquireTooManyTest(Context &context, Type wsiType)
{
    ...
    for (uint32_t i = 0; i < numAcquirableImages; ++i)
    {
        VK_CHECK_WSI(devHelper.vkd.acquireNextImageKHR(
            *devHelper.device, *swapchain, std::numeric_limits<uint64_t>::max(), (VkSemaphore)0, **fences[i], &dummy));
    }

    const auto result = devHelper.vkd.acquireNextImageKHR(*devHelper.device, *swapchain, 0, (VkSemaphore)0,
                                                          **fences[numAcquirableImages], &dummy);
...
{
    addFunctionCaseWithPrograms(testGroup, "basic", "Basic Rendering Test", getBasicRenderPrograms,
                                basicRenderTest<AcquireNextImageWrapper>, wsiType);
    addFunctionCaseWithPrograms(testGroup, "basic2", "Basic Rendering Test using AcquireNextImage2",
                                getBasicRenderPrograms, basicRenderTest<AcquireNextImage2Wrapper>, wsiType);
    addFunctionCaseWithPrograms(testGroup, "device_group", "Basic Rendering Test using device_group",
                                getBasicRenderPrograms, deviceGroupRenderTest, wsiType);
    addFunctionCaseWithPrograms(testGroup, "device_group2",
                                "Rendering Test using device_group and VkImageSwapchainCreateInfo",
                                getBasicRenderPrograms, deviceGroupRenderTest2, wsiType);

    const MultiSwapchainParams kTwoSwapchains{wsiType, 2u};
    const MultiSwapchainParams kTenSwapchains{wsiType, 10u};

    addFunctionCaseWithPrograms(testGroup, "2swapchains", "2 Swapchains Rendering Test", getBasicRenderPrograms,
                                multiSwapchainRenderTest<AcquireNextImageWrapper>, kTwoSwapchains);
    addFunctionCaseWithPrograms(testGroup, "2swapchains2", "2 Swapchains Rendering Test using AcquireNextImage2",
                                getBasicRenderPrograms, multiSwapchainRenderTest<AcquireNextImage2Wrapper>,
                                kTwoSwapchains);
    addFunctionCaseWithPrograms(testGroup, "10swapchains", "10 Swapchains Rendering Test", getBasicRenderPrograms,
                                multiSwapchainRenderTest<AcquireNextImageWrapper>, kTenSwapchains);
    addFunctionCaseWithPrograms(testGroup, "10swapchains2", "10 Swapchains Rendering Test using AcquireNextImage2",
                                getBasicRenderPrograms, multiSwapchainRenderTest<AcquireNextImage2Wrapper>,
                                kTenSwapchains);
}
...
{
    addFunctionCase(testGroup, "null_handle", "Destroying a VK_NULL_HANDLE swapchain", destroyNullHandleSwapchainTest,
                    wsiType);
}

void populateAcquireGroup(tcu::TestCaseGroup *testGroup, Type wsiType)
{
    addFunctionCase(testGroup, "too_many", "Test acquiring too many images with 0 timeout", acquireTooManyTest,
                    wsiType);
    addFunctionCase(testGroup, "too_many_timeout", "Test acquiring too many images with timeout",
                    acquireTooManyTimeoutTest, wsiType);
}
...
void createSwapchainTests(tcu::TestCaseGroup *testGroup, vk::wsi::Type wsiType)
{
    ...
    addTestGroup(testGroup, "simulate_oom", "Simulate OOM using callbacks during swapchain construction",
                 populateSwapchainGroup, GroupParameters(wsiType, createSwapchainSimulateOOMTest));
    ...
    addTestGroup(testGroup, "private_data", "Create VkSwapchain and use VK_EXT_private_data",
                 populateSwapchainPrivateDataGroup, GroupParameters(wsiType, createSwapchainPrivateDataTest));
}
