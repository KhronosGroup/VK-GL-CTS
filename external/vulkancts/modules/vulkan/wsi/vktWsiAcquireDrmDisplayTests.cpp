...
private:
    ...
    uint32_t getConnectedConnectorId(int fd, uint32_t connectorId = 0);
    uint32_t getValidCrtcId(int fd, uint32_t connectorId);
...
tcu::TestStatus AcquireDrmDisplayTestInstance::iterate(void)
{
    ...
    switch (m_testId)
    {
    case DRM_TEST_INDEX_GET_DRM_DISPLAY:
        return testGetDrmDisplayEXT();
    case DRM_TEST_INDEX_GET_DRM_DISPLAY_INVALID_FD:
        return testGetDrmDisplayEXTInvalidFd();
    case DRM_TEST_INDEX_GET_DRM_DISPLAY_INVALID_CONNECTOR_ID:
        return testGetDrmDisplayEXTInvalidConnectorId();
    case DRM_TEST_INDEX_GET_DRM_DISPLAY_NOT_MASTER:
        return testGetDrmDisplayEXTNotMaster();
    case DRM_TEST_INDEX_GET_DRM_DISPLAY_UNOWNED_CONNECTOR_ID:
        return testGetDrmDisplayEXTUnownedConnectorId();
    case DRM_TEST_INDEX_ACQUIRE_DRM_DISPLAY:
        return testAcquireDrmDisplayEXT();
    case DRM_TEST_INDEX_ACQUIRE_DRM_DISPLAY_INVALID_FD:
        return testAcquireDrmDisplayEXTInvalidFd();
    case DRM_TEST_INDEX_ACQUIRE_DRM_DISPLAY_NOT_MASTER:
        return testAcquireDrmDisplayEXTNotMaster();
    case DRM_TEST_INDEX_ACQUIRE_DRM_DISPLAY_UNOWNED_CONNECTOR_ID:
        return testAcquireDrmDisplayEXTUnownedConnectorId();
    case DRM_TEST_INDEX_RELEASE_DISPLAY:
        return testReleaseDisplayEXT();
    default:
    {
        DE_FATAL("Impossible");
    }
...
LibDrm::FdPtr AcquireDrmDisplayTestInstance::getDrmFdPtr(void)
{
    ...
    const char *drmNode      = m_libDrm.findDeviceNode(drmDevices, numDrmDevices, deviceDrmProperties.primaryMajor,
 ...
 *//*--------------------------------------------------------------------*/
uint32_t AcquireDrmDisplayTestInstance::getConnectedConnectorId(int fd, uint32_t connectorId)
{
 ...
 *//*--------------------------------------------------------------------*/
uint32_t AcquireDrmDisplayTestInstance::getValidCrtcId(int fd, uint32_t connectorId)
{
    LibDrm::ResPtr res        = m_libDrm.getResources(fd);
...
tcu::TestStatus AcquireDrmDisplayTestInstance::testGetDrmDisplayEXT(void)
{
    ...
    uint32_t connectorId = getConnectedConnectorId(fd);
    if (!connectorId)
...
tcu::TestStatus AcquireDrmDisplayTestInstance::testGetDrmDisplayEXTUnownedConnectorId(void)
{
    ...
    uint32_t connectorId      = getConnectedConnectorId(fd);
    uint32_t otherConnectorId = getConnectedConnectorId(fd, connectorId);
    uint32_t crtcId           = getValidCrtcId(fd, connectorId);
    ...
    uint32_t objects[2]      = {connectorId, crtcId};
...
{
public:
    AcquireDrmDisplayTestsCase(tcu::TestContext &context, const char *name, const char *description,
                               const DrmTestIndex testId)
