...
tcu::TestStatus fullScreenExclusiveTest(Context &context, TestParams testParams)
{
    ...
    const Unique<VkSurfaceKHR> surface(
        createSurface(instHelper.vki, instHelper.instance, testParams.wsiType, *native.display, *native.window));
        ...
        for (uint32_t frameNdx = 0; frameNdx < numFramesToRender; ++frameNdx)
        {
            ...
            if (!fullScreenAcquired)
            {
                ...
                case VK_ERROR_INITIALIZATION_FAILED:
                {
                    if (frameNdx > 3)
                        throw tcu::TestError(
                            "vkAcquireFullScreenExclusiveModeEXT was not able to succeed in first 3 draw frames");
                    break;
                }
