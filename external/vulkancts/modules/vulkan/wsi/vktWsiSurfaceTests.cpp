...
tcu::TestStatus createSurfaceCustomAllocatorTest(Context &context, Type wsiType)
{
...

    {
        ...
        const NativeObjects native(context, instHelper.supportedExtensions, wsiType);
        const Unique<VkSurfaceKHR> surface(createSurface(instHelper.vki, instHelper.instance, wsiType,
...
tcu::TestStatus querySurfacePresentModes2TestSurfaceless(Context &context, Type wsiType)
{
    ...
    for (size_t deviceNdx = 0; deviceNdx < physicalDevices.size(); ++deviceNdx)
    {
        if (isSupportedByAnyQueue(instHelper.vki, physicalDevices[deviceNdx], *surface))
        {
            ...
            VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModes2EXT(physicalDevices[deviceNdx], &surfaceInfo,
                                                                             &numModesSurface, DE_NULL));
            VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModes2EXT(physicalDevices[deviceNdx],
                                                                             &nullSurfaceInfo, &numModesNull, DE_NULL));
            ...
            {
                VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModes2EXT(
                    physicalDevices[deviceNdx], &surfaceInfo, &numModesSurface, &modesSurface[0]));
                VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModes2EXT(
                    physicalDevices[deviceNdx], &nullSurfaceInfo, &numModesSurface, &modesNull[0]));
            }

            modesSurface.pop_back();
            modesNull.pop_back();

            for (uint32_t i = 0; i < modesSurface.size(); i++)
            {
...
tcu::TestStatus querySurfacePresentModesTestSurfaceless(Context &context, Type wsiType)
{
    ...
    for (size_t deviceNdx = 0; deviceNdx < physicalDevices.size(); ++deviceNdx)
    {
        if (isSupportedByAnyQueue(instHelper.vki, physicalDevices[deviceNdx], *surface))
        {
            ...
            VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModesKHR(physicalDevices[deviceNdx], *surface,
                                                                            &numModesSurface, DE_NULL));
            VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModesKHR(physicalDevices[deviceNdx], nullSurface,
            ...
            {
                VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModesKHR(physicalDevices[deviceNdx], *surface,
                                                                                &numModesSurface, &modesSurface[0]));
                VK_CHECK(instHelper.vki.getPhysicalDeviceSurfacePresentModesKHR(physicalDevices[deviceNdx], nullSurface,
                                                                                &numModesSurface, &modesNull[0]));
            }
...
tcu::TestStatus createSurfaceInitialSizeTest(Context &context, Type wsiType)
{
    ...
    for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(sizes); ++sizeNdx)
    {
        ...
        const Unique<VkSurfaceKHR> surface(
            createSurface(instHelper.vki, instHelper.instance, wsiType, *nativeDisplay, *nativeWindow));
...
void createSurfaceTests(tcu::TestCaseGroup *testGroup, vk::wsi::Type wsiType)
{
    ...
    addFunctionCase(testGroup, "create_custom_allocator", "Create surface with custom allocator",
                    createSurfaceCustomAllocatorTest, wsiType);
    addFunctionCase(testGroup, "create_simulate_oom", "Create surface with simulating OOM",
                    createSurfaceSimulateOOMTest, wsiType);
    addFunctionCase(testGroup, "query_support", "Query surface support", querySurfaceSupportTest, wsiType);
    addFunctionCase(testGroup, "query_presentation_support", "Query native presentation support",
                    queryPresentationSupportTest, wsiType);
    addFunctionCase(testGroup, "query_capabilities", "Query surface capabilities", querySurfaceCapabilitiesTest,
                    wsiType);
    addFunctionCase(testGroup, "query_capabilities2", "Query extended surface capabilities",
                    querySurfaceCapabilities2Test, wsiType);
    addFunctionCase(testGroup, "query_protected_capabilities", "Query protected surface capabilities",
                    querySurfaceProtectedCapabilitiesTest, wsiType);
    addFunctionCase(testGroup, "query_surface_counters", "Query and check available surface counters",
                    querySurfaceCounterTest, wsiType);
    ...
    addFunctionCase(testGroup, "query_present_modes", "Query surface present modes", querySurfacePresentModesTest,
                    wsiType);
    addFunctionCase(testGroup, "query_present_modes2", "Query extended surface present modes",
                    querySurfacePresentModes2Test, wsiType);
    addFunctionCase(testGroup, "query_devgroup_present_capabilities",
                    "Query surface present modes capabilities in device groups",
                    queryDevGroupSurfacePresentCapabilitiesTest, wsiType);
    addFunctionCase(testGroup, "query_devgroup_present_modes", "Query surface present modes for device groups",
                    queryDevGroupSurfacePresentModesTest, wsiType);
    addFunctionCase(testGroup, "destroy_null_handle", "Destroy VK_NULL_HANDLE surface", destroyNullHandleSurfaceTest,
                    wsiType);

    if ((platformProperties.features & PlatformProperties::FEATURE_INITIAL_WINDOW_SIZE) != 0)
        addFunctionCase(testGroup, "initial_size", "Create surface with initial window size set",
                        createSurfaceInitialSizeTest, wsiType);
    ...
    addFunctionCase(testGroup, "query_formats_surfaceless", "Query surface formats without surface",
                    querySurfaceFormatsTestSurfaceless, wsiType);
    addFunctionCase(testGroup, "query_present_modes_surfaceless", "Query surface present modes without surface",
                    querySurfacePresentModesTestSurfaceless, wsiType);
    addFunctionCase(testGroup, "query_present_modes2_surfaceless",
                    "Query extended surface present modes without surface", querySurfacePresentModes2TestSurfaceless,
                    wsiType);
    addFunctionCase(testGroup, "query_formats2_surfaceless", "Query extended surface formats without surface",
