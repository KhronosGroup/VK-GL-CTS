...
Move<VkDevice> createDevice(const vk::Platform &platform, const PlatformInterface &vkp, const VkInstance instance,
...
SwapchainCounterTestInstance::SwapchainCounterTestInstance(Context &context)
    ...
    , m_device(createDevice(context.getTestContext().getPlatform().getVulkanPlatform(), m_vkp, m_instance, m_vki,
                            m_physicalDevice, m_deviceExtensions, m_queueFamilyIndex,
...
tcu::TestStatus testDisplayPowerControl(Context &context)
{
    ...
    for (const auto &display : availableDisplays)
    {
        ...
        for (const auto &psd : powerStateDataVect)
        {
            ...
            if (result != VK_SUCCESS)
                tcu::TestStatus::fail(std::string("vkDisplayPowerControlEXT returned invalid result for ") +
