                                                                       ...
                                                                       const VkFormat depthStencilFormat)
{
    ...
    const VkAttachmentReference2 depthStencilAttachmentRef = {
        ...
        m_testMode == MODE_DEPTH ? VK_IMAGE_ASPECT_DEPTH_BIT :
                                   VK_IMAGE_ASPECT_STENCIL_BIT // VkImageAspectFlags aspectMask;
    };
...
{
public:
    EarlyFragmentSampleCountTestInstance(Context &context, const uint32_t sampleCount);
...
EarlyFragmentSampleCountTestInstance::EarlyFragmentSampleCountTestInstance(Context &context, const uint32_t sampleCount)
...
tcu::TestStatus EarlyFragmentSampleCountTestInstance::iterate(void)
{
    ...
    {
        uint64_t refValue  = uint64_t(uint32_t((renderSize.x() * renderSize.y()) / 4) * m_sampleCount);
...
{
public:
    EarlyFragmentSampleCountTest(tcu::TestContext &testCtx, const std::string name, const uint32_t sampleCount,
...
EarlyFragmentSampleCountTest::EarlyFragmentSampleCountTest(tcu::TestContext &testCtx, const std::string name,
                                                           const uint32_t sampleCount, const bool earlyAndLate)
...
tcu::TestCaseGroup *createEarlyFragmentTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "early_fragment", "early fragment test cases"));
    ...
    // We kill half of the samples at different points in the pipeline depending on early frag test, and then we verify the sample counting works as expected.
    {
        ...
        for (uint32_t sampleCountsNdx = 0; sampleCountsNdx < DE_LENGTH_OF_ARRAY(sampleCounts); sampleCountsNdx++)
        {
            testGroup->addChild(new EarlyFragmentSampleCountTest(
                testCtx, "sample_count_early_fragment_tests_depth_" + sampleCountsStr[sampleCountsNdx],
                sampleCounts[sampleCountsNdx], false));
#ifndef CTS_USES_VULKANSC
            testGroup->addChild(new EarlyFragmentSampleCountTest(
                testCtx, "sample_count_early_and_late_fragment_tests_depth_" + sampleCountsStr[sampleCountsNdx],
                sampleCounts[sampleCountsNdx], true));
#endif
        }
    }
