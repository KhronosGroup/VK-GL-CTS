...
static amber::EngineConfig *createEngineConfig(Context &ctx)
{
    amber::EngineConfig *vkConfig =
        GetVulkanConfig(ctx.getInstance(), ctx.getPhysicalDevice(), ctx.getDevice(), &ctx.getDeviceFeatures(),
                        &ctx.getDeviceFeatures2(), ctx.getInstanceExtensions(), ctx.getDeviceExtensions(),
                        ctx.getUniversalQueueFamilyIndex(), ctx.getUniversalQueue(), ctx.getInstanceProcAddr());
    ...
    if (ctx.isDeviceFunctionalitySupported("VK_KHR_portability_subset"))
    {
        ...
        if (ctx.getPortabilitySubsetProperties().minVertexInputBindingStrideAlignment == 4)
        {
            const std::set<std::string> casesToSkip{
                "line-strip",         "polygon-mode-lines",
                "r8g8-uint-highp",    "r8g8-uint-highp-output-uint",
                "r8g8-uint-mediump",  "r8g8-uint-mediump-output-uint",
                "inputs-outputs-mod",
            };
