...
void DeviceGroupTestInstance::init(void)
{
    ...
    m_deviceGroupQueue = getDeviceQueue(m_deviceGroup.getDriver(), *m_deviceGroup, queueFamilyIndex, queueIndex);
}

void DeviceGroupTestInstance::submitBufferAndWaitForIdle(const vk::DeviceInterface &vk, VkCommandBuffer cmdBuf,
                                                         uint32_t deviceMask)
{
...
tcu::TestStatus DeviceGroupTestInstance::iterate(void)
{
    ...
    for (uint32_t physDevID = 0; physDevID < m_physicalDeviceCount; physDevID++)
    {
        ...
        VkMemoryAllocateFlagsInfo allocDeviceMaskInfo = {
            ...
            m_useDedicated ? &dedicatedAllocInfo : nullptr, // pNext
            ...
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        {
            ...
            if ((m_testMode & TEST_MODE_SFR) &&
                vki.getPhysicalDeviceImageFormatProperties(m_deviceGroup.getPhysicalDevice(),
                                                           colorFormat,             // format
        ...
        // Create renderpass
        {
            ...
            renderPass = createRenderPass(vk, *m_deviceGroup, &renderPassInfo, nullptr);
        }
        ...
        if ((m_testMode & TEST_MODE_SFR) && (m_physicalDeviceCount > 1))
        {
            ...
            const VkPhysicalDeviceMemoryProperties deviceMemProps =
                getPhysicalDeviceMemoryProperties(vki, m_physicalDevices[secondDeviceID]);
        ...
        // Update buffers
        {
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 1, &stagingVertexBufferUpdateBarrier, 0, nullptr);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 1, &vertexBufferUpdateBarrier, 0, nullptr);

            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 1, &stagingIndexBufferUpdateBarrier, 0, nullptr);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 1, &indexBufferUpdateBarrier, 0, nullptr);

            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 1, &stagingUboBufferUpdateBarrier, 0, nullptr);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 1, &uboUpdateBarrier, 0, nullptr);
            ...
            if (m_drawTessellatedSphere)
            {
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      (VkDependencyFlags)0, 0, nullptr, 1, &stagingsboUpdateBarrier, 0, nullptr);
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      ...
                                      nullptr, 1, &sboUpdateBarrier, 0, nullptr);
            }
        ...
        // Begin renderpass
        {
            ...
            const VkDeviceGroupRenderPassBeginInfo deviceGroupRPBeginInfo = {
                VK_STRUCTURE_TYPE_DEVICE_GROUP_RENDER_PASS_BEGIN_INFO, nullptr,
                (uint32_t)((1 << m_physicalDeviceCount) - 1), m_physicalDeviceCount, &renderAreas[0]};

            const VkRenderPassBeginInfo passBeginParams = {
                VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO,                         // sType
                (m_testMode & TEST_MODE_SFR) ? &deviceGroupRPBeginInfo : nullptr, // pNext
                *renderPass,                                                      // renderPass
        ...
        // Change image layout for copy
        {
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, nullptr, 0, nullptr, 1, &renderFinishBarrier);
        }
            ...
            // Copy peer image (only needed in SFR case when peer memory as copy source is not allowed)
            {
                // Change layout on firstDeviceID
                {
                    ...
                    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                          (VkDependencyFlags)0, 0, nullptr, 0, nullptr, 1u, &preCopyBarrier);
                    endCommandBuffer(vk, *cmdBuffer);
                ...
                // Change layout back on firstDeviceID
                {
                    ...
                    vk.cmdSetDeviceMask(*cmdBuffer, 1 << firstDeviceID);
                    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                          VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, (VkDependencyFlags)0, 0, nullptr, 0,
                                          nullptr, 1u, &postCopyBarrier);
                    endCommandBuffer(vk, *cmdBuffer);
        ...
        // copy image to read buffer for checking
        {
            ...
            // Copy image to buffer
            {
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT,
                                      (VkDependencyFlags)0, 0, nullptr, 1, &copyFinishBarrier, 0, nullptr);
            }
            endCommandBuffer(vk, *cmdBuffer);
...
{
public:
    DeviceGroupTestCase(tcu::TestContext &context, const char *name, uint32_t mode)
...
void DeviceGroupTestRendering::init(void)
{
    ...
    addChild(
        new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "sfr_sys", TEST_MODE_SFR | TEST_MODE_HOSTMEMORY));
    // Test split frame rendering with dedicated memory allocations
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "sfr_dedicated",
    ...
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_dedicated_peer", TEST_MODE_SFR | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));
    ...
    // Test split frame rendering with render target in host memory
    addChild(
        new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "afr_sys", TEST_MODE_AFR | TEST_MODE_HOSTMEMORY));
    // Test split frame rendering with dedicated memory allocations
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "afr_dedicated",
                                                              TEST_MODE_AFR | TEST_MODE_DEDICATED));
    // Test split frame rendering with dedicated memory allocations and peer fetching
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "afr_dedicated_peer", TEST_MODE_AFR | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));

#ifndef CTS_USES_VULKANSC
    // Test split frame rendering with tessellated sphere
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "sfr_tessellated",
                                                              TEST_MODE_SFR | TEST_MODE_TESSELLATION |
                                                                  TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));
    // Test split frame rendering with tessellated sphere with line segments
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_tessellated_linefill",
    ...
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "afr_tessellated",
                                                              TEST_MODE_AFR | TEST_MODE_TESSELLATION |
                                                                  TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));
