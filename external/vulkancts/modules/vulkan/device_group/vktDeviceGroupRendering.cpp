...
{
public:
    DeviceGroupTestInstance(Context &context, uint32_t mode);
...
private:
    ...
    void SubmitBufferAndWaitForIdle(const DeviceDriver &vk, VkCommandBuffer cmdBuf, uint32_t deviceMask);
...
DeviceGroupTestInstance::DeviceGroupTestInstance(Context &context, const uint32_t mode)
...
uint32_t DeviceGroupTestInstance::getMemoryIndex(const uint32_t memoryTypeBits, const uint32_t memoryPropertyFlag)
{
...
bool DeviceGroupTestInstance::isPeerFetchAllowed(uint32_t memoryTypeIndex, uint32_t firstdeviceID,
                                                 uint32_t seconddeviceID)
{
    ...
    const VkPhysicalDeviceMemoryProperties deviceMemProps1 =
        getPhysicalDeviceMemoryProperties(m_context.getInstanceInterface(), m_physicalDevices[firstdeviceID]);
    const VkPhysicalDeviceMemoryProperties deviceMemProps2 =
        getPhysicalDeviceMemoryProperties(m_context.getInstanceInterface(), m_physicalDevices[seconddeviceID]);
...
void DeviceGroupTestInstance::init(void)
{
...

    {
        ...
        const vector<vk::VkPhysicalDeviceGroupProperties> properties =
            enumeratePhysicalDeviceGroups(instanceInterface, m_context.getInstance());
        ...
        const VkDeviceCreateInfo deviceCreateInfo = {
            ...
            (uint32_t)extensionPtrs.size(),                        //extensionCount;
        ...
        m_deviceGroup = createCustomDevice(m_context.getTestContext().getCommandLine().isValidationEnabled(),
                                           m_context.getPlatformInterface(), m_context.getInstance(), instanceInterface,
                                           physicalDevice, &deviceCreateInfo);
    }

    deviceDriver = de::MovePtr<vk::DeviceDriver>(
        new vk::DeviceDriver(m_context.getPlatformInterface(), m_context.getInstance(), *m_deviceGroup));
    m_deviceGroupQueue = getDeviceQueue(*deviceDriver, *m_deviceGroup, queueFamilyIndex, queueIndex);
}

void DeviceGroupTestInstance::SubmitBufferAndWaitForIdle(const DeviceDriver &vk, VkCommandBuffer cmdBuf,
                                                         uint32_t deviceMask)
{
...
tcu::TestStatus DeviceGroupTestInstance::iterate(void)
{
    ...
    const tcu::Vec4 sphereVertices[] = {
        tcu::Vec4(0.0f, 0.0f, 1.0f, 1.0f),  tcu::Vec4(0.0f, 1.0f, 0.0f, 1.0f),  tcu::Vec4(1.0f, 0.0f, 0.0f, 1.0f),
        tcu::Vec4(0.0f, 0.0f, -1.0f, 1.0f), tcu::Vec4(0.0f, -1.0f, 0.0f, 1.0f), tcu::Vec4(-1.0f, 0.0f, 0.0f, 1.0f),
    };
    const uint32_t sphereIndices[] = {0, 1, 2, 2, 1, 3, 3, 1, 5, 5, 1, 0, 0, 2, 4, 2, 3, 4, 3, 5, 4, 5, 0, 4};
    const tcu::Vec4 triVertices[]  = {tcu::Vec4(-0.5f, -0.5f, 0.0f, 1.0f), tcu::Vec4(+0.5f, -0.5f, 0.0f, 1.0f),
                                      tcu::Vec4(0.0f, +0.5f, 0.0f, 1.0f)};
    const uint32_t triIndices[]    = {0, 1, 2};
    const tcu::Vec4 *vertices      = m_drawTessellatedSphere ? &sphereVertices[0] : &triVertices[0];
    const uint32_t *indices        = m_drawTessellatedSphere ? &sphereIndices[0] : &triIndices[0];
    const uint32_t verticesSize =
        m_drawTessellatedSphere ? uint32_t(sizeof(sphereVertices)) : uint32_t(sizeof(triVertices));
    const uint32_t numIndices = m_drawTessellatedSphere ? uint32_t(sizeof(sphereIndices) / sizeof(sphereIndices[0])) :
                                                          uint32_t(sizeof(triIndices) / sizeof(triIndices[0]));
    const uint32_t indicesSize =
        m_drawTessellatedSphere ? uint32_t(sizeof(sphereIndices)) : uint32_t(sizeof(triIndices));
    ...
    for (uint32_t physDevID = 0; physDevID < m_physicalDeviceCount; physDevID++)
    {
        ...
        // create vertex buffers
        {
            ...
            stagingVertexBufferMemory = memAlloc.allocate(
                getBufferMemoryRequirements(vk, *m_deviceGroup, *stagingVertexBuffer), MemoryRequirement::HostVisible);
            VK_CHECK(vk.bindBufferMemory(*m_deviceGroup, *stagingVertexBuffer, stagingVertexBufferMemory->getMemory(),
                                         stagingVertexBufferMemory->getOffset()));
        ...
        // create index buffers
        {
            ...
            stagingIndexBufferMemory = memAlloc.allocate(
                getBufferMemoryRequirements(vk, *m_deviceGroup, *stagingIndexBuffer), MemoryRequirement::HostVisible);
            VK_CHECK(vk.bindBufferMemory(*m_deviceGroup, *stagingIndexBuffer, stagingIndexBufferMemory->getMemory(),
                                         stagingIndexBufferMemory->getOffset()));
        ...
        // create SBO buffers
        {
            ...
            stagingSboBufferMemory = memAlloc.allocate(
                getBufferMemoryRequirements(vk, *m_deviceGroup, *stagingSboBuffer), MemoryRequirement::HostVisible);
            VK_CHECK(vk.bindBufferMemory(*m_deviceGroup, *stagingSboBuffer, stagingSboBufferMemory->getMemory(),
                                         stagingSboBufferMemory->getOffset()));
            ...
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        {
            ...
            if ((m_testMode & TEST_MODE_SFR) &&
                vki.getPhysicalDeviceImageFormatProperties(m_context.getPhysicalDevice(),
                                                           colorFormat,             // format
        ...
        // Create descriptors
        {
            ...
            const VkDescriptorSetLayoutCreateInfo descriptorLayoutParams = {
                ...
                uint32_t(layoutBindings.size()),                     // uint32_t count;
            ...
            const VkDescriptorPoolCreateInfo descriptorPoolParams = {
                ...
                1u,                                                // uint32_t maxSets;
                uint32_t(descriptorTypes.size()),                  // uint32_t count;
            ...
            const VkDescriptorBufferInfo uboDescriptorInfo = {uniformBuffer.get(), 0, (VkDeviceSize)sizeof(drawColor)};
            const VkDescriptorBufferInfo sboDescriptorInfo = {sboBuffer.get(), 0, (VkDeviceSize)sizeof(tessLevel)};
            ...
            vk.updateDescriptorSets(*m_deviceGroup, uint32_t(writeDescritporSets.size()), writeDescritporSets.data(),
                                    0u, DE_NULL);
        }
        ...
        // Create Pipeline
        {
            ...
            {
                tcssShaderModule =
                    createShaderModule(vk, *m_deviceGroup, m_context.getBinaryCollection().get("tesc"), 0);
                tessShaderModule =
                    createShaderModule(vk, *m_deviceGroup, m_context.getBinaryCollection().get("tese"), 0);
            }
            ...
            const VkPrimitiveTopology topology =
                m_drawTessellatedSphere ? VK_PRIMITIVE_TOPOLOGY_PATCH_LIST : VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
            ...
            const VkImageViewCreateInfo colorAttViewParams = {
                ...
                *renderImage,                             // image
                ...
                {VK_COMPONENT_SWIZZLE_R, VK_COMPONENT_SWIZZLE_G, VK_COMPONENT_SWIZZLE_B,
                 VK_COMPONENT_SWIZZLE_A}, // components
                {
        ...
        if ((m_testMode & TEST_MODE_SFR) && (m_physicalDeviceCount > 1))
        {
            ...
            const VkPhysicalDeviceMemoryProperties deviceMemProps =
                getPhysicalDeviceMemoryProperties(m_context.getInstanceInterface(), m_physicalDevices[secondDeviceID]);
            vk.getDeviceGroupPeerMemoryFeatures(*m_deviceGroup, deviceMemProps.memoryTypes[memoryTypeNdx].heapIndex,
                                                firstDeviceID, secondDeviceID, &peerMemFeatures);
            ...
            for (uint32_t i = 0; i < m_physicalDeviceCount * m_physicalDeviceCount; i++)
                sfrRects.push_back(zeroRect);

            if (m_physicalDeviceCount == 1u)
            {
                sfrRects[0].extent.width  = (int32_t)renderSize.x();
                sfrRects[0].extent.height = (int32_t)renderSize.y();
            }
            else
            {
                // Split into 2 vertical halves
                sfrRects[firstDeviceID * m_physicalDeviceCount + firstDeviceID].extent.width =
                    (int32_t)renderSize.x() / 2;
                sfrRects[firstDeviceID * m_physicalDeviceCount + firstDeviceID].extent.height = (int32_t)renderSize.y();
                sfrRects[firstDeviceID * m_physicalDeviceCount + secondDeviceID] =
                    sfrRects[firstDeviceID * m_physicalDeviceCount + firstDeviceID];
                sfrRects[firstDeviceID * m_physicalDeviceCount + secondDeviceID].offset.x = (int32_t)renderSize.x() / 2;
        ...
        // Update buffers
        {
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  ...
                                  &stagingVertexBufferUpdateBarrier, 0, (const VkImageMemoryBarrier *)DE_NULL);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1,
                                  &vertexBufferUpdateBarrier, 0, (const VkImageMemoryBarrier *)DE_NULL);

            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1,
                                  &stagingIndexBufferUpdateBarrier, 0, (const VkImageMemoryBarrier *)DE_NULL);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1,
                                  &indexBufferUpdateBarrier, 0, (const VkImageMemoryBarrier *)DE_NULL);

            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1,
                                  &stagingUboBufferUpdateBarrier, 0, (const VkImageMemoryBarrier *)DE_NULL);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1, &uboUpdateBarrier, 0,
            ...
            if (m_drawTessellatedSphere)
            {
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1,
                                      &stagingsboUpdateBarrier, 0, (const VkImageMemoryBarrier *)DE_NULL);
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      VK_PIPELINE_STAGE_TESSELLATION_CONTROL_SHADER_BIT, (VkDependencyFlags)0, 0,
                                      (const VkMemoryBarrier *)DE_NULL, 1, &sboUpdateBarrier, 0,
                                      (const VkImageMemoryBarrier *)DE_NULL);
            }
        ...
        // Begin renderpass
        {
            const VkClearValue clearValue =
                makeClearValueColorF32(clearColor[0], clearColor[1], clearColor[2], clearColor[3]);
            ...
            if (m_physicalDeviceCount == 1u)
            {
                renderAreas[0].extent.width  = (int32_t)renderSize.x();
                renderAreas[0].extent.height = (int32_t)renderSize.y();
            }
            else
            {
                // Split into 2 vertical halves
                renderAreas[firstDeviceID].extent.width  = (int32_t)renderSize.x() / 2;
                renderAreas[firstDeviceID].extent.height = (int32_t)renderSize.y();
                renderAreas[secondDeviceID]              = renderAreas[firstDeviceID];
                renderAreas[secondDeviceID].offset.x     = (int32_t)renderSize.x() / 2;
            }

            const VkDeviceGroupRenderPassBeginInfo deviceGroupRPBeginInfo = {
                VK_STRUCTURE_TYPE_DEVICE_GROUP_RENDER_PASS_BEGIN_INFO, DE_NULL,
                (uint32_t)((1 << m_physicalDeviceCount) - 1), m_physicalDeviceCount, &renderAreas[0]};
        ...
        // Change image layout for copy
        {
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  ...
                                  (const VkBufferMemoryBarrier *)DE_NULL, 1, &renderFinishBarrier);
        }
            ...
            // Copy peer image (only needed in SFR case when peer memory as copy source is not allowed)
            {
                // Change layout on firstDeviceID
                {
                    ...
                    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                          (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                                          (const VkBufferMemoryBarrier *)DE_NULL, 1u, &preCopyBarrier);
                    endCommandBuffer(vk, *cmdBuffer);

                    const uint32_t deviceMask = 1 << firstDeviceID;
                    SubmitBufferAndWaitForIdle(vk, cmdBuffer.get(), deviceMask);
                }
                ...
                // Copy Image from secondDeviceID to firstDeviceID
                {
                    ...
                    int32_t imageOffsetX = (m_testMode & TEST_MODE_AFR) ? 0 : renderSize.x() / 2;
                    uint32_t imageExtentX =
                        (m_testMode & TEST_MODE_AFR) ? (uint32_t)renderSize.x() : (uint32_t)renderSize.x() / 2;
                    ...
                    const VkImageCopy imageCopy = {{
                                                   ...
                                                   {imageOffsetX, 0, 0},
                                                   {imageExtentX, (uint32_t)renderSize.y(), 1u}};

                    beginCommandBuffer(vk, *cmdBuffer);
                    vk.cmdSetDeviceMask(*cmdBuffer, 1 << secondDeviceID);
                    vk.cmdCopyImage(*cmdBuffer, *renderImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *peerImage,
                                    VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageCopy);
                    endCommandBuffer(vk, *cmdBuffer);

                    const uint32_t deviceMask = 1 << secondDeviceID;
                    SubmitBufferAndWaitForIdle(vk, cmdBuffer.get(), deviceMask);
                }
        ...
        // copy image to read buffer for checking
        {
            ...
            const UniquePtr<Allocation> readImageBufferMemory(memAlloc.allocate(
            ...
            // Copy image to buffer
            {
                ...
                vk.cmdCopyImageToBuffer(*cmdBuffer, isPeerMemAsCopySrcAllowed ? *renderImage : *readImage,
                                        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *readImageBuffer, 1u, &copyParams);
                ...
                vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT,
                                      (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 1, &copyFinishBarrier,
            ...
            if (m_drawTessellatedSphere)
            {
                ...
                string refImage = m_fillModeNonSolid ? "vulkan/data/device_group/sphere.png" :
                                                       "vulkan/data/device_group/spherefilled.png";
                tcu::ImageIO::loadPNG(referenceImage, m_context.getTestContext().getArchive(), refImage.c_str());
                iterateResultSuccess =
            ...
            else
            {
                ...
                {
                    tcu::TextureLevel refImage(tcuFormat, (int32_t)renderSize.x(), (int32_t)renderSize.y());
...
{
public:
    DeviceGroupTestCase(tcu::TestContext &context, const char *name, const char *description, uint32_t mode)
...
{
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "sfr", "Test split frame rendering",
                                                              TEST_MODE_SFR));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_sys", "Test split frame rendering with render target in host memory",
        TEST_MODE_SFR | TEST_MODE_HOSTMEMORY));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_dedicated", "Test split frame rendering with dedicated memory allocations",
        TEST_MODE_SFR | TEST_MODE_DEDICATED));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_dedicated_peer",
        "Test split frame rendering with dedicated memory allocations and peer fetching",
        TEST_MODE_SFR | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));

    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(m_testCtx, "afr", "Test alternate frame rendering",
                                                              TEST_MODE_AFR));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "afr_sys", "Test split frame rendering with render target in host memory",
        TEST_MODE_AFR | TEST_MODE_HOSTMEMORY));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "afr_dedicated", "Test split frame rendering with dedicated memory allocations",
        TEST_MODE_AFR | TEST_MODE_DEDICATED));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "afr_dedicated_peer",
        "Test split frame rendering with dedicated memory allocations and peer fetching",
        TEST_MODE_AFR | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));

    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_tessellated", "Test split frame rendering with tessellated sphere",
        TEST_MODE_SFR | TEST_MODE_TESSELLATION | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "sfr_tessellated_linefill", "Test split frame rendering with tessellated sphere with line segments",
        TEST_MODE_SFR | TEST_MODE_TESSELLATION | TEST_MODE_LINEFILL | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
        m_testCtx, "afr_tessellated", "Test alternate frame rendering with tesselated sphere",
        TEST_MODE_AFR | TEST_MODE_TESSELLATION | TEST_MODE_DEDICATED | TEST_MODE_PEER_FETCH));
    addChild(new DeviceGroupTestCase<DeviceGroupTestInstance>(
