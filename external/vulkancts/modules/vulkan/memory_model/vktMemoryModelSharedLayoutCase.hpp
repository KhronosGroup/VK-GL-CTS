...
{
    SharedStructVarEntry(glu::DataType type_, int arraySize_) : type(type_), arraySize(arraySize_)
    {
    }
...
public:
    ...
    SharedStruct(const std::string name, const std::string instanceName) : m_name(name), m_instanceName(instanceName)
    {
    }
    ...
    void addMember(SharedStructVar var)
    {
        m_members.push_back(var);
    }
    int getNumMembers(void)
    {
...
public:
    ...
    std::vector<NamedStructSP> &getStructs(void)
    {
        return m_structs;
    }
    int getNumStructs(void)
    {
        return static_cast<int>(m_structs.size());
    }

    int getNumSharedObjects(void) const
    {
        return static_cast<int>(m_sharedMemoryObjects.size());
    }
    std::vector<SharedStruct> &getSharedObjects(void)
    {
        return m_sharedMemoryObjects;
    }
    const std::vector<SharedStruct> &getSharedObjects(void) const
    {
        return m_sharedMemoryObjects;
    }

    void enable8BitTypes(bool enabled)
    {
        m_8BitTypesEnabled = enabled;
    }
    void enable16BitTypes(bool enabled)
    {
        m_16BitTypesEnabled = enabled;
    }
    bool is8BitTypesEnabled(void) const
    {
        return m_8BitTypesEnabled;
    }
    bool is16BitTypesEnabled(void) const
    {
        return m_16BitTypesEnabled;
    }
...
{
public:
    SharedLayoutCaseInstance(Context &context) : TestInstance(context)
    {
    }
...
{
public:
    SharedLayoutCase(tcu::TestContext &testCtx, const char *name) : TestCase(testCtx, name)
    {
    }
...
{
public:
    RandomSharedLayoutCase(tcu::TestContext &testCtx, const char *name, uint32_t features, uint32_t seed);
