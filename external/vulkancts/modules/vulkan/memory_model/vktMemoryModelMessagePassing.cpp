...
MemoryModelTestCase::MemoryModelTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                         const CaseDef data)
...
void MemoryModelTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    if (m_data.syncType != ST_CONTROL_AND_MEMORY_BARRIER && m_data.syncType != ST_CONTROL_BARRIER)
    {
        ...
        case SC_BUFFER:
            ...
            break;
        case SC_IMAGE:
            css << "layout(set=0, binding=1, r32ui) uniform uimage2D guard;\n";
            break;
        case SC_WORKGROUP:
            css << "shared S guard;\n";
            break;
    ...
    else
    {
        ...
        if (m_data.atomicRMW)
        {
            ...
            case SC_BUFFER:
                ...
                break;
            case SC_IMAGE:
                css << "   imageAtomicExchange(guard, imageCoord, (1u), " << scopeStr << atomicReleaseSemantics.str()
                    << ");\n";
                break;
            case SC_WORKGROUP:
        ...
        else
        {
            ...
            case SC_BUFFER:
                ...
                break;
            case SC_IMAGE:
                css << "   imageAtomicStore(guard, imageCoord, (1u), " << scopeStr << atomicReleaseSemantics.str()
                    << ");\n";
                break;
            case SC_WORKGROUP:
        ...
        if (m_data.atomicRMW)
        {
            ...
            case SC_PHYSBUFFER: // fall through
            case SC_BUFFER:
                css << "   skip = atomicExchange(guard.x[partnerBufferCoord], 2u, " << scopeStr
                    << atomicAcquireSemantics.str() << ") == 0;\n";
                break;
            case SC_IMAGE:
                css << "   skip = imageAtomicExchange(guard, partnerImageCoord, 2u, " << scopeStr
                    << atomicAcquireSemantics.str() << ") == 0;\n";
                break;
            case SC_WORKGROUP:
                css << "   skip = atomicExchange(guard.x[partnerSharedCoord], 2u, " << scopeStr
                    << atomicAcquireSemantics.str() << ") == 0;\n";
                break;
            }
        }
    ...
    if (m_data.testType == TT_MP)
    {
        ...
        css << "   if (!skip && r != partnerBufferCoord) { fail.x[bufferCoord] = 1; }\n"
               "}\n";
    }
    else
    {
        ...
        case SC_PHYSBUFFER: // fall through
        case SC_BUFFER:
            css << "   payload.x[bufferCoord] = bufferCoord;\n";
            break;
        case SC_IMAGE:
            css << "   imageStore(payload, imageCoord, uvec4(bufferCoord, 0, 0, 0));\n";
            break;
        case SC_WORKGROUP:
            css << "   payload.x[sharedCoord] = bufferCoord;\n";
            break;
        }
        css << "   }\n"
...
void MemoryModelTestCase::initProgramsTransitive(SourceCollections &programCollection) const
{
    ...
    case SC_BUFFER:
        ...
        break;
    case SC_IMAGE:
        css << "   imageStore(payload, imageCoord, uvec4(bufferCoord + (imageLoad(payload, partnerImageCoord).x>>31), "
               "0, 0, 0));\n";
        break;
    }
    ...
    if (m_data.syncType == ST_ATOMIC_ATOMIC || m_data.syncType == ST_ATOMIC_FENCE)
    {
        ...
        case SC_BUFFER:
            ...
            break;
        case SC_IMAGE:
            css << "       imageAtomicStore(guard, imageCoord, (1u), gl_ScopeDevice, " << storageSemanticsPayload.str()
    ...
    else
    {
        ...
        case SC_BUFFER:
            ...
            break;
        case SC_IMAGE:
            css << "       imageAtomicStore(guard, imageCoord, (1u), gl_ScopeDevice, 0, 0);\n";
            break;
        }
    }
...
tcu::TestStatus MemoryModelTestInstance::iterate(void)
{
    ...
    {
        size_t elementSize = m_data.dataType == DATA_TYPE_UINT64 ? sizeof(uint64_t) : sizeof(uint32_t);
...
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(testCtx, "memory_model", "Memory model tests"));
    ...
    for (int ttNdx = 0; ttNdx < DE_LENGTH_OF_ARRAY(ttCases); ttNdx++)
    {
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> core11Group(
                new tcu::TestCaseGroup(testCtx, core11Cases[core11Ndx].name, core11Cases[core11Ndx].description));
            for (int dtNdx = 0; dtNdx < DE_LENGTH_OF_ARRAY(dtCases); dtNdx++)
            {
                ...
                {
                    de::MovePtr<tcu::TestCaseGroup> cohGroup(
                        new tcu::TestCaseGroup(testCtx, cohCases[cohNdx].name, cohCases[cohNdx].description));
                    for (int stNdx = 0; stNdx < DE_LENGTH_OF_ARRAY(stCases); stNdx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> stGroup(
                            new tcu::TestCaseGroup(testCtx, stCases[stNdx].name, stCases[stNdx].description));
                        for (int rmwNdx = 0; rmwNdx < DE_LENGTH_OF_ARRAY(rmwCases); rmwNdx++)
                        {
                            de::MovePtr<tcu::TestCaseGroup> rmwGroup(
                                new tcu::TestCaseGroup(testCtx, rmwCases[rmwNdx].name, rmwCases[rmwNdx].description));
                            for (int scopeNdx = 0; scopeNdx < DE_LENGTH_OF_ARRAY(scopeCases); scopeNdx++)
                            {
                                de::MovePtr<tcu::TestCaseGroup> scopeGroup(new tcu::TestCaseGroup(
                                    testCtx, scopeCases[scopeNdx].name, scopeCases[scopeNdx].description));
                                for (int plNdx = 0; plNdx < DE_LENGTH_OF_ARRAY(plCases); plNdx++)
                                {
                                    de::MovePtr<tcu::TestCaseGroup> plGroup(new tcu::TestCaseGroup(
                                        testCtx, plCases[plNdx].name, plCases[plNdx].description));
                                    for (int pscNdx = 0; pscNdx < DE_LENGTH_OF_ARRAY(pscCases); pscNdx++)
                                    {
                                        de::MovePtr<tcu::TestCaseGroup> pscGroup(new tcu::TestCaseGroup(
                                            testCtx, pscCases[pscNdx].name, pscCases[pscNdx].description));
                                        for (int glNdx = 0; glNdx < DE_LENGTH_OF_ARRAY(glCases); glNdx++)
                                        {
                                            de::MovePtr<tcu::TestCaseGroup> glGroup(new tcu::TestCaseGroup(
                                                testCtx, glCases[glNdx].name, glCases[glNdx].description));
                                            for (int gscNdx = 0; gscNdx < DE_LENGTH_OF_ARRAY(gscCases); gscNdx++)
                                            {
                                                de::MovePtr<tcu::TestCaseGroup> gscGroup(new tcu::TestCaseGroup(
                                                    testCtx, gscCases[gscNdx].name, gscCases[gscNdx].description));
                                                     ...
                                                     stageNdx++)
                                                {
                                                    ...
                                                    if (c.dataType == DATA_TYPE_UINT64 &&
                                                        c.syncType != ST_ATOMIC_ATOMIC)
                                                    {
                                                        continue;
                                                    }
                                                    ...
                                                    gscGroup->addChild(
                                                        new MemoryModelTestCase(testCtx, stageCases[stageNdx].name,
                                                                                stageCases[stageNdx].description, c));
                                                }
        ...
        for (int stNdx = 0; stNdx < DE_LENGTH_OF_ARRAY(stCases); stNdx++)
        {
            ...
            for (int plNdx = 0; plNdx < DE_LENGTH_OF_ARRAY(plCases); plNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> plGroup(
                    new tcu::TestCaseGroup(testCtx, plCases[plNdx].name, plCases[plNdx].description));
                for (int pscNdx = 0; pscNdx < DE_LENGTH_OF_ARRAY(pscCases); pscNdx++)
                {
                    de::MovePtr<tcu::TestCaseGroup> pscGroup(
                        new tcu::TestCaseGroup(testCtx, pscCases[pscNdx].name, pscCases[pscNdx].description));
                    for (int glNdx = 0; glNdx < DE_LENGTH_OF_ARRAY(glCases); glNdx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> glGroup(
                            new tcu::TestCaseGroup(testCtx, glCases[glNdx].name, glCases[glNdx].description));
                        for (int gscNdx = 0; gscNdx < DE_LENGTH_OF_ARRAY(gscCases); gscNdx++)
                        {
                            de::MovePtr<tcu::TestCaseGroup> gscGroup(
                                new tcu::TestCaseGroup(testCtx, gscCases[gscNdx].name, gscCases[gscNdx].description));
                            for (int visNdx = 0; visNdx < DE_LENGTH_OF_ARRAY(transVisCases); visNdx++)
                            {
                                ...
                                gscGroup->addChild(new MemoryModelTestCase(testCtx, transVisCases[visNdx].name,
                                                                           transVisCases[visNdx].description, c));
                            }
                            glGroup->addChild(gscGroup.release());
                        }
                        pscGroup->addChild(glGroup.release());
