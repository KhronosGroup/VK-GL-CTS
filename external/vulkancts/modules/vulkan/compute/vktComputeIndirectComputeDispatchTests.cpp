...
{
    DispatchCaseDesc(const char *name, const char *description, const uintptr_t bufferSize,
                     const tcu::UVec3 workGroupSize, const DispatchCommandsVec &dispatchCommands)
        : m_name(name)
...
{
public:
    IndirectDispatchInstanceBufferUpload(Context &context, const std::string &name, const uintptr_t bufferSize,
                                         const tcu::UVec3 &workGroupSize, const DispatchCommandsVec &dispatchCommands);
...
protected:
    virtual void fillIndirectBufferData(const vk::VkCommandBuffer commandBuffer, const Buffer &indirectBuffer);

    bool verifyResultBuffer(const Buffer &resultBuffer, const vk::VkDeviceSize resultBlockSize) const;
...
IndirectDispatchInstanceBufferUpload::IndirectDispatchInstanceBufferUpload(Context &context, const std::string &name,
                                                                  ...
                                                                  const Buffer &indirectBuffer)
{
    ...
    for (DispatchCommandsVec::const_iterator cmdIter = m_dispatchCommands.begin(); cmdIter != m_dispatchCommands.end();
         ++cmdIter)
    {
        DE_ASSERT(cmdIter->m_offset >= 0);
        DE_ASSERT(cmdIter->m_offset % sizeof(uint32_t) == 0);
        DE_ASSERT(cmdIter->m_offset + INDIRECT_COMMAND_OFFSET <= (intptr_t)m_bufferSize);

        uint32_t *const dstPtr = (uint32_t *)&indirectDataPtr[cmdIter->m_offset];
...
tcu::TestStatus IndirectDispatchInstanceBufferUpload::iterate(void)
{
    ...
    testCtx.getLog() << tcu::TestLog::Message << "GL_DISPATCH_INDIRECT_BUFFER size = " << m_bufferSize
                     << tcu::TestLog::EndMessage;
    {
        tcu::ScopedLogSection section(testCtx.getLog(), "Commands",
                                      "Indirect Dispatch Commands (" + de::toString(m_dispatchCommands.size()) +
                                          " in total)");

        for (uint32_t cmdNdx = 0; cmdNdx < m_dispatchCommands.size(); ++cmdNdx)
        {
            testCtx.getLog() << tcu::TestLog::Message << cmdNdx << ": "
                             << "offset = " << m_dispatchCommands[cmdNdx].m_offset
                             << ", numWorkGroups = " << m_dispatchCommands[cmdNdx].m_numWorkGroups
                             << tcu::TestLog::EndMessage;
        }
    }

    // Create result buffer
    const vk::VkDeviceSize resultBlockSize = getResultBlockAlignedSize(
        m_context.getInstanceInterface(), m_context.getPhysicalDevice(), RESULT_BLOCK_BASE_SIZE);
    const vk::VkDeviceSize resultBufferSize = resultBlockSize * (uint32_t)m_dispatchCommands.size();

    Buffer resultBuffer(m_device_interface, m_device, m_allocator,
...

    {
        ...
        for (uint32_t cmdNdx = 0; cmdNdx < m_dispatchCommands.size(); ++cmdNdx)
        {
            ...
            *(uint32_t *)(dstPtr + 0 * sizeof(uint32_t))           = m_dispatchCommands[cmdNdx].m_numWorkGroups[0];
            *(uint32_t *)(dstPtr + 1 * sizeof(uint32_t))           = m_dispatchCommands[cmdNdx].m_numWorkGroups[1];
            *(uint32_t *)(dstPtr + 2 * sizeof(uint32_t))           = m_dispatchCommands[cmdNdx].m_numWorkGroups[2];
    ...
    const vk::Unique<vk::VkShaderModule> verifyShader(
        createShaderModule(m_device_interface, m_device,
                           m_context.getBinaryCollection().get("indirect_dispatch_" + m_name + "_verify"), 0u));
    ...
    const vk::Unique<vk::VkPipelineLayout> pipelineLayout(
        makePipelineLayout(m_device_interface, m_device, *descriptorSetLayout));
    const vk::Unique<vk::VkPipeline> computePipeline(
        makeComputePipeline(m_device_interface, m_device, *pipelineLayout, *verifyShader));

    // Create descriptor pool
    const vk::Unique<vk::VkDescriptorPool> descriptorPool(
        vk::DescriptorPoolBuilder()
            .addType(vk::VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, (uint32_t)m_dispatchCommands.size())
            .build(m_device_interface, m_device, vk::VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT,
                   static_cast<uint32_t>(m_dispatchCommands.size())));
    ...
    const vk::Unique<vk::VkCommandBuffer> cmdBuffer(
        allocateCommandBuffer(m_device_interface, m_device, *cmdPool, vk::VK_COMMAND_BUFFER_LEVEL_PRIMARY));

    // Begin recording commands
    beginCommandBuffer(m_device_interface, *cmdBuffer);

    // Create indirect buffer
    Buffer indirectBuffer(m_device_interface, m_device, m_allocator,
                          vk::makeBufferCreateInfo(m_bufferSize, vk::VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT |
    ...
    for (uint32_t cmdNdx = 0; cmdNdx < m_dispatchCommands.size(); ++cmdNdx)
    {
        ...
        // Bind descriptor set
        m_device_interface.cmdBindDescriptorSets(*cmdBuffer, vk::VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0u,
                                                 1u, &(**descriptorSets[cmdNdx]), 0u, DE_NULL);
    ...
    m_device_interface.cmdPipelineBarrier(
        *cmdBuffer, vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk::VK_PIPELINE_STAGE_HOST_BIT, (vk::VkDependencyFlags)0,
        0, (const vk::VkMemoryBarrier *)DE_NULL, 1, &ssboPostBarrier, 0, (const vk::VkImageMemoryBarrier *)DE_NULL);
...
bool IndirectDispatchInstanceBufferUpload::verifyResultBuffer(const Buffer &resultBuffer,
                                                              const vk::VkDeviceSize resultBlockSize) const
{
    ...
    for (uint32_t cmdNdx = 0; cmdNdx < m_dispatchCommands.size(); cmdNdx++)
    {
        ...
        const uint32_t numInvocationsPerGroup = m_workGroupSize[0] * m_workGroupSize[1] * m_workGroupSize[2];
...
{
public:
    IndirectDispatchCaseBufferUpload(tcu::TestContext &testCtx, const DispatchCaseDesc &caseDesc,
...
{
public:
    IndirectDispatchInstanceBufferGenerate(Context &context, const std::string &name, const uintptr_t bufferSize,
                                           const tcu::UVec3 &workGroupSize, const DispatchCommandsVec &dispatchCommands)
                                                                    ...
                                                                    const Buffer &indirectBuffer)
{
    ...
    const vk::Unique<vk::VkShaderModule> genIndirectBufferDataShader(
        createShaderModule(m_device_interface, m_device,
                           m_context.getBinaryCollection().get("indirect_dispatch_" + m_name + "_generate"), 0u));
    ...
    m_computePipeline =
        makeComputePipeline(m_device_interface, m_device, *m_pipelineLayout, *genIndirectBufferDataShader);
    ...
    const vk::VkDescriptorBufferInfo indirectDescriptorInfo =
        makeDescriptorBufferInfo(*indirectBuffer, 0ull, m_bufferSize);
    ...
    // Bind descriptor set
    m_device_interface.cmdBindDescriptorSets(commandBuffer, vk::VK_PIPELINE_BIND_POINT_COMPUTE, *m_pipelineLayout, 0u,
                                             1u, &m_descriptorSet.get(), 0u, DE_NULL);

    // Dispatch compute command
    m_device_interface.cmdDispatch(commandBuffer, 1u, 1u, 1u);

    // Insert memory barrier
    m_device_interface.cmdPipelineBarrier(commandBuffer, vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT,
...
{
public:
    IndirectDispatchCaseBufferGenerate(tcu::TestContext &testCtx, const DispatchCaseDesc &caseDesc,
                                       const glu::GLSLVersion glslVersion)
...
void IndirectDispatchCaseBufferGenerate::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    for (DispatchCommandsVec::const_iterator cmdIter = m_dispatchCommands.begin(); cmdIter != m_dispatchCommands.end();
         ++cmdIter)
    {
        const uint32_t offs = (uint32_t)(cmdIter->m_offset / sizeof(uint32_t));
        DE_ASSERT((size_t)offs * sizeof(uint32_t) == (size_t)cmdIter->m_offset);

        computeBuffer << "\twriteCmd(" << offs << "u, uvec3(" << cmdIter->m_numWorkGroups.x() << "u, "
                      << cmdIter->m_numWorkGroups.y() << "u, " << cmdIter->m_numWorkGroups.z() << "u));\n";
    }
...
{
    return new IndirectDispatchInstanceBufferGenerate(context, m_name, m_bufferSize, m_workGroupSize,
                                                      m_dispatchCommands);
}
...
DispatchCommandsVec commandsVec(const DispatchCommand &cmd0, const DispatchCommand &cmd1, const DispatchCommand &cmd2,
                                const DispatchCommand &cmd3, const DispatchCommand &cmd4)
{
    DispatchCommandsVec vec;
    vec.push_back(cmd0);
    vec.push_back(cmd1);
...
DispatchCommandsVec commandsVec(const DispatchCommand &cmd0, const DispatchCommand &cmd1, const DispatchCommand &cmd2,
                                const DispatchCommand &cmd3, const DispatchCommand &cmd4, const DispatchCommand &cmd5,
...
tcu::TestCaseGroup *createIndirectComputeDispatchTests(tcu::TestContext &testCtx)
{
    static const DispatchCaseDesc s_dispatchCases[] = {
        DispatchCaseDesc("single_invocation", "Single invocation only from offset 0", INDIRECT_COMMAND_OFFSET,
                         tcu::UVec3(1, 1, 1), commandsVec(DispatchCommand(0, tcu::UVec3(1, 1, 1)))),
        DispatchCaseDesc("multiple_groups", "Multiple groups dispatched from offset 0", INDIRECT_COMMAND_OFFSET,
                         tcu::UVec3(1, 1, 1), commandsVec(DispatchCommand(0, tcu::UVec3(2, 3, 5)))),
        DispatchCaseDesc("multiple_groups_multiple_invocations", "Multiple groups of size 2x3x1 from offset 0",
                         INDIRECT_COMMAND_OFFSET, tcu::UVec3(2, 3, 1),
        ...
        DispatchCaseDesc("large_offset_multiple_invocations", "Large offset, multiple invocations", (2 << 20),
                         tcu::UVec3(2, 3, 1), commandsVec(DispatchCommand((1 << 20) + 12, tcu::UVec3(1, 2, 3)))),
        ...
        DispatchCaseDesc(
            "multi_dispatch", "Dispatch multiple compute commands from single buffer", 1 << 10, tcu::UVec3(3, 1, 2),
        ...
        DispatchCaseDesc("multi_dispatch_reuse_command", "Dispatch multiple compute commands from single buffer",
                         1 << 10, tcu::UVec3(3, 1, 2),
                         commandsVec(DispatchCommand(0, tcu::UVec3(1, 1, 1)), DispatchCommand(0, tcu::UVec3(1, 1, 1)),
    ...
    de::MovePtr<tcu::TestCaseGroup> indirectComputeDispatchTests(
        new tcu::TestCaseGroup(testCtx, "indirect_dispatch", "Indirect dispatch tests"));
    ...
    for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(s_dispatchCases); ndx++)
    {
        groupBufferUpload->addChild(
            new IndirectDispatchCaseBufferUpload(testCtx, s_dispatchCases[ndx], glu::GLSL_VERSION_310_ES));
    }
    ...
    for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(s_dispatchCases); ndx++)
    {
        groupBufferGenerate->addChild(
            new IndirectDispatchCaseBufferGenerate(testCtx, s_dispatchCases[ndx], glu::GLSL_VERSION_310_ES));
    }
