...
tcu::TestStatus IndirectDispatchInstanceBufferUpload::iterate(void)
{
    ...
    if (m_computeQueueOnly)
    {
        ...
        m_deviceDriver = de::MovePtr<vk::DeviceDriver>(
            new vk::DeviceDriver(m_context.getPlatformInterface(), m_customInstance, m_device));
#else
    ...
    const vk::Unique<vk::VkPipeline> computePipeline(
        makeComputePipeline(vkdi, m_device, *pipelineLayout, *verifyShader));
    ...
    vk::BufferWithMemory indirectBuffer(
        ...
        vk::makeBufferCreateInfo(m_bufferSize,
    ...
    for (uint32_t cmdNdx = 0; cmdNdx < m_dispatchCommands.size(); ++cmdNdx)
    {
        ...
        // Bind descriptor set
        vkdi.cmdBindDescriptorSets(*cmdBuffer, vk::VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0u, 1u,
                                   &(**descriptorSets[cmdNdx]), 0u, DE_NULL);
...
TestInstance *IndirectDispatchCaseBufferGenerate::createInstance(Context &context) const
{
    return new IndirectDispatchInstanceBufferGenerate(context, m_name, m_bufferSize, m_workGroupSize,
                                                      m_dispatchCommands, m_computeOnlyQueue);
}
...
tcu::TestCaseGroup *createIndirectComputeDispatchTests(tcu::TestContext &testCtx)
{
    ...
    static const DispatchCaseDesc s_dispatchCases[] = {
        DispatchCaseDesc("single_invocation", "Single invocation only from offset 0", INDIRECT_COMMAND_OFFSET,
                         tcu::UVec3(1, 1, 1), commandsVec(DispatchCommand(0, tcu::UVec3(1, 1, 1))), false),
        DispatchCaseDesc("multiple_groups", "Multiple groups dispatched from offset 0", INDIRECT_COMMAND_OFFSET,
                         tcu::UVec3(1, 1, 1), commandsVec(DispatchCommand(0, tcu::UVec3(2, 3, 5))), false),
        ...
        DispatchCaseDesc("large_offset_multiple_invocations", "Large offset, multiple invocations", (2 << 20),
                         tcu::UVec3(2, 3, 1), commandsVec(DispatchCommand((1 << 20) + 12, tcu::UVec3(1, 2, 3))), false),
    ...
    for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(s_dispatchCases); ndx++)
    {
        ...
        DispatchCaseDesc computeOnlyDesc = DispatchCaseDesc(computeName.c_str(), desc.m_description, desc.m_bufferSize,
                                                            desc.m_workGroupSize, desc.m_dispatchCommands, true);
        ...
        groupBufferUpload->addChild(
            new IndirectDispatchCaseBufferUpload(testCtx, computeOnlyDesc, glu::GLSL_VERSION_310_ES));
    }
    ...
    for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(s_dispatchCases); ndx++)
    {
        ...
        groupBufferGenerate->addChild(
            new IndirectDispatchCaseBufferGenerate(testCtx, computeOnlyDesc, glu::GLSL_VERSION_310_ES));
    }
