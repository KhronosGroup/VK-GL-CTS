...
CooperativeMatrixTestCase::CooperativeMatrixTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                                     const CaseDef data)
...
void CooperativeMatrixTestCase::checkSupport(Context &context) const
{
    ...
    context.getInstanceInterface().getPhysicalDeviceCooperativeMatrixPropertiesNV(context.getPhysicalDevice(),
    ...
    {
        VkCooperativeMatrixPropertiesNV *p = &pProperties[i];
        if (m_data.testType == TT_MATRIXMULADD || m_data.testType == TT_MATRIXMULADD_ARRAY)
        {
            if (p->AType == m_data.inputType && p->BType == m_data.inputType && p->CType == m_data.outputType &&
                p->DType == m_data.outputType && p->scope == VK_SCOPE_SUBGROUP_NV)
            {
        ...
        else
        {
            ...
            for (uint32_t j = 0; j < 2; ++j)
            {
                if (p->scope == VK_SCOPE_SUBGROUP_NV &&
                    (p->AType == types[j] || p->BType == types[j] || p->CType == types[j] || p->DType == types[j]))
                {
                    supported[j] = true;
                }
            }
...
struct
{
    ...
    {"float16_t", "fcoopmatNV", 16}, {"float32_t", "fcoopmatNV", 32}, {"float64_t", "fcoopmatNV", 64},
    {"int8_t", "icoopmatNV", 8},     {"int16_t", "icoopmatNV", 16},   {"int32_t", "icoopmatNV", 32},
    {"int64_t", "icoopmatNV", 64},   {"uint8_t", "ucoopmatNV", 8},    {"uint16_t", "ucoopmatNV", 16},
    {"uint32_t", "ucoopmatNV", 32},  {"uint64_t", "ucoopmatNV", 64},
};
...
void CooperativeMatrixTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    matAType << componentTypeInfo[m_data.inputType].coopmatTypeName << "<" << componentTypeInfo[m_data.inputType].bits
             << ", gl_ScopeSubgroup, " << dims[0].rows << ", " << dims[0].cols << ">";
    matBType << componentTypeInfo[m_data.inputType].coopmatTypeName << "<" << componentTypeInfo[m_data.inputType].bits
             << ", gl_ScopeSubgroup, " << dims[1].rows << ", " << dims[1].cols << ">";
    matCType << componentTypeInfo[m_data.outputType].coopmatTypeName << "<" << componentTypeInfo[m_data.outputType].bits
             << ", gl_ScopeSubgroup, " << dims[2].rows << ", " << dims[2].cols << ">";
    ...
    if (m_data.storageClass == SC_WORKGROUP || m_data.storageClass == SC_WORKGROUP_VARIABLE_POINTERS)
    {
        ...
        for (uint32_t m = 0; m < 4; ++m)
        {
            ...
            css << "       elementS" << m << " = " << sharedStride << " * "
                ...
                << (m_data.colMajor ? dims[m].rows : dims[m].cols) << " * subgroupXY.x;\n";
        }
    ...
    if (m_data.storageClass == SC_WORKGROUP || m_data.storageClass == SC_WORKGROUP_VARIABLE_POINTERS)
    {
        css << "   coopMatLoadNV(matA, sharedA, elementS0, " << strides[0] << ", " << colMajor
            << ");\n"
               "   coopMatLoadNV(matB, sharedB, elementS1, "
            << strides[1] << ", " << colMajor
            << ");\n"
               "   coopMatLoadNV(matC, sharedC, elementS2, "
            << strides[2] << ", " << colMajor << ");\n";
    }
    else
    {
        css << "   coopMatLoadNV(matA, inputA.x, element0, " << strides[0] << ", " << colMajor
            << ");\n"
               "   coopMatLoadNV(matB, inputB.x, element1, "
            << strides[1] << ", " << colMajor
            << ");\n"
               "   coopMatLoadNV(matC, inputC.x, element2, "
            << strides[2] << ", " << colMajor << ");\n";
    }
...
static void setDataFloat(void *base, VkComponentTypeNV dt, uint32_t i, float value)
{
...
static float getDataFloat(void *base, VkComponentTypeNV dt, uint32_t i)
{
    if (dt == VK_COMPONENT_TYPE_FLOAT32_NV)
    {
        return ((float *)base)[i];
...
static void setDataInt(void *base, VkComponentTypeNV dt, uint32_t i, uint32_t value)
{
...
static uint32_t getDataInt(void *base, VkComponentTypeNV dt, uint32_t i)
{
    DE_ASSERT(componentTypeInfo[dt].bits <= 32);
    switch (dt)
    {
...
tcu::TestStatus CooperativeMatrixTestInstance::iterate(void)
{
    ...
    uint32_t propertyCount = 0;
    VkCooperativeMatrixPropertiesNV *pProperties;
    m_context.getInstanceInterface().getPhysicalDeviceCooperativeMatrixPropertiesNV(m_context.getPhysicalDevice(),
                                                                                    &propertyCount, DE_NULL);
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < propertyCount; ++i)
        {
            ...
            for (uint32_t j = 0; j < 2; ++j)
            {
                ...
                if (p->CType == types[j] || p->DType == types[j])
            ...
            if (m_data.testType != TT_MATRIXMULADD && m_data.testType != TT_MATRIXMULADD_ARRAY)
            {
                for (uint32_t i = 0; i < totalElements[3]; ++i)
                {
                    ...
                    case TT_LENGTH:
                        ...
                        if (output != (float)(N * M / subgroupProperties.subgroupSize) && res == QP_TEST_RESULT_PASS)
            ...
            if (m_data.testType != TT_MATRIXMULADD && m_data.testType != TT_MATRIXMULADD_ARRAY)
            {
                for (uint32_t i = 0; i < totalElements[3]; ++i)
                {
                    ...
                    case TT_LENGTH:
                        ...
                        // We expect the matrix to be spread evenly across invocations, it is
                        // surprising (but not necessarily illegal) if not
                        if (output != N * M / subgroupProperties.subgroupSize && res == QP_TEST_RESULT_PASS)
                            res = QP_TEST_RESULT_QUALITY_WARNING;
                        break;
                    case TT_CONSTANT:
                        if (output != 1)
...
tcu::TestCaseGroup *createCooperativeMatrixTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "cooperative_matrix", "GL_NV_cooperative_matrix tests"));
        ...
        for (int dtNdx = 0; dtNdx < DE_LENGTH_OF_ARRAY(dtCases); dtNdx++)
        {
            ...
            for (int scNdx = 0; scNdx < DE_LENGTH_OF_ARRAY(scCases); scNdx++)
            {
                ...
                for (int colNdx = 0; colNdx < DE_LENGTH_OF_ARRAY(colCases); colNdx++)
                {
                    ...
                    scGroup->addChild(
                        new CooperativeMatrixTestCase(testCtx, colCases[colNdx].name, colCases[colNdx].description, c));
                }
