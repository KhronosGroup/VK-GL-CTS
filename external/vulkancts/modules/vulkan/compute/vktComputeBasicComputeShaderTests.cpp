...
tcu::TestStatus CopySSBOToImageTestInstance::iterate(void)
{
    ...
    {
        const Unique<VkShaderModule> shaderModule(
            createShaderModule(vkd, data.device, m_context.getBinaryCollection().get("comp"), 0u));
        ...
        vkd.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0u, 1u,
                                  &descriptorSet.get(), 0u, DE_NULL);
...
tcu::TestStatus SecondaryCommandBufferComputeOnlyTestInstance::iterate()
{
...
#ifndef CTS_USES_VULKANSC
    deviceDriver =
        de::MovePtr<DeviceDriver>(new DeviceDriver(m_context.getPlatformInterface(), m_context.getInstance(), device));
#else
    ...
    auto allocator = de::MovePtr<Allocator>(
        new SimpleAllocator(vkdi, device, getPhysicalDeviceMemoryProperties(vki, m_context.getPhysicalDevice())));
...
tcu::TestCaseGroup *createBasicComputeShaderTests(tcu::TestContext &testCtx)
{
    ...
    basicComputeTests->addChild(new ComputeOnlyQueueTests::SecondaryCommandBufferComputeOnlyTest(
        testCtx, "secondary_compute_only_queue", "Test secondary command buffers in compute only queues"));
