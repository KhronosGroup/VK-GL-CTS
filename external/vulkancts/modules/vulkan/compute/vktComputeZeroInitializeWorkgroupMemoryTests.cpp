...
{
public:
    MaxWorkgroupMemoryTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
void AddMaxWorkgroupMemoryTests(tcu::TestCaseGroup *group)
{
    ...
    for (uint32_t i = 0; i < workgroups.size(); ++i)
    {
        ...
        group->addChild(new MaxWorkgroupMemoryTest(group->getTestContext(), de::toString(numWG),
                                                   de::toString(numWG) + " workgroups", numWG));
    }
}
...
{
public:
    TypeTestInstance(Context &context, const TypeCaseDef &caseDef) : TestInstance(context), m_caseDef(caseDef)
    {
    }
    tcu::TestStatus iterate(void);
...
{
public:
    TypeTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
{
public:
    CompositeTestInstance(Context &context, const CompositeCaseDef &caseDef) : TestInstance(context), m_caseDef(caseDef)
    {
    }
    tcu::TestStatus iterate(void);
...
{
public:
    CompositeTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
void AddCompositeTests(tcu::TestCaseGroup *group)
{
    ...
    for (uint32_t i = 0; i < cases.size(); ++i)
    {
        group->addChild(new CompositeTest(group->getTestContext(), de::toString(i), de::toString(i), cases[i]));
    }
}
...
{
public:
    MaxWorkgroupsInstance(Context &context, Dim dim) : TestInstance(context), m_dim(dim)
    {
    }
    tcu::TestStatus iterate(void);
...
{
public:
    MaxWorkgroupsTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description, Dim dim)
        : TestCase(testCtx, name, description)
        , m_dim(dim)
    {
    }
...
{
public:
    SpecializeWorkgroupTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
        ...
        for (uint32_t y = 1; y <= 8; ++y)
        {
            for (uint32_t x = 1; x <= 8; ++x)
            {
                group->addChild(new SpecializeWorkgroupTest(
                    group->getTestContext(), de::toString(x) + "_" + de::toString(y) + "_" + de::toString(z),
                    de::toString(x) + "_" + de::toString(y) + "_" + de::toString(z), x, y, z));
            }
        }
    }
}
...
{
public:
    RepeatedPipelineTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         uint32_t xSize, uint32_t repeat, uint32_t odd)
        : TestCase(testCtx, name, description)
        , m_xSize(xSize)
        , m_repeat(repeat)
        , m_odd(odd)
    ...
    for (uint32_t i = 0; i < xSizes.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < odds.size(); ++j)
        {
            ...
            for (uint32_t k = 0; k < repeats.size(); ++k)
            {
                ...
                group->addChild(
                    new RepeatedPipelineTest(group->getTestContext(),
                                             std::string("x_") + de::toString(x) + (odd == 1 ? "_odd" : "_even") +
                                                 "_repeat_" + de::toString(repeat),
                                             std::string("x_") + de::toString(x) + (odd == 1 ? "_odd" : "_even") +
                                                 "_repeat_" + de::toString(repeat),
                                             x, odd, repeat));
            }
...
tcu::TestCaseGroup *createZeroInitializeWorkgroupMemoryTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> tests(new tcu::TestCaseGroup(testCtx, "zero_initialize_workgroup_memory",
                                                                 "VK_KHR_zero_intialize_workgroup_memory tests"));
    ...
    tcu::TestCaseGroup *specializeWorkgroupGroup =
        new tcu::TestCaseGroup(testCtx, "specialize_workgroup", "specialize workgroup size");
    AddSpecializeWorkgroupTests(specializeWorkgroupGroup);
    tests->addChild(specializeWorkgroupGroup);

    tcu::TestCaseGroup *repeatPipelineGroup =
        new tcu::TestCaseGroup(testCtx, "repeat_pipeline", "repeated pipeline run");
    AddRepeatedPipelineTests(repeatPipelineGroup);
    tests->addChild(repeatPipelineGroup);

#ifndef CTS_USES_VULKANSC
    tcu::TestCaseGroup *subgroupInvocationGroup =
        new tcu::TestCaseGroup(testCtx, "shared_memory_blocks", "shared memory tests");
