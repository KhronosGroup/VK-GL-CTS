...
ComputeBuiltinVarInstance::ComputeBuiltinVarInstance(Context &context, const vector<SubCase> &subCases,
...
tcu::TestStatus ComputeBuiltinVarInstance::iterate(void)
{
    ...
    Buffer uniformBuffer(m_vki, m_device, m_context.getDefaultAllocator(),
                         makeBufferCreateInfo(sizeOfUniformBuffer, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT),
                         MemoryRequirement::HostVisible);
    Buffer resultBuffer(m_vki, m_device, m_context.getDefaultAllocator(),
    ...
    const Unique<VkShaderModule> shaderModule(
        createShaderModule(m_vki, m_device, m_context.getBinaryCollection().get(program_name.str()), 0u));
    ...
    m_vki.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0u, 1u,
