...
{
public:
    SamplerUniformSetup(bool useSampler) : m_useSampler(useSampler)
    {
    }
...
{
public:
    ShaderDiscardCaseInstance(Context &context, bool isVertexCase, const ShaderEvaluator &evaluator,
                              const UniformSetup &uniformSetup, bool usesTexture, bool fuzzyCompare);
                               ...
                               static_cast<uint32_t>(GRID_SIZE_DEFAULTS), fuzzyCompare)
{
    if (usesTexture)
    {
        de::SharedPtr<TextureBinding> brickTexture(new TextureBinding(
            m_context.getTestContext().getArchive(), "vulkan/data/brick.png", TextureBinding::TYPE_2D,
            tcu::Sampler(tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE,
                         tcu::Sampler::LINEAR, tcu::Sampler::LINEAR, 0.0f, true, tcu::Sampler::COMPAREMODE_NONE, 0,
...
{
public:
    ShaderDiscardCase(tcu::TestContext &testCtx, const char *name, const char *description, const char *shaderSource,
...
ShaderDiscardCase::ShaderDiscardCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                     const char *shaderSource, const ShaderEvalFunc evalFunc, bool usesTexture,
                                     bool fuzzyCompare, bool demote)
...
#define GLSL_SHADER_TEMPLATE_HEADER                                 \
    ...
    case DISCARDTEMPLATE_MAIN_BASIC:
        return GLSL_SHADER_TEMPLATE_HEADER "void main (void)\n"
                                           "{\n"
                                           "    o_color = v_color;\n"
                                           "    ${DISCARD};\n"
                                           "}\n";
    ...
    case DISCARDTEMPLATE_FUNCTION_BASIC:
        return GLSL_SHADER_TEMPLATE_HEADER "void myfunc (void)\n"
                                           "{\n"
                                           "    ${DISCARD};\n"
                                           "}\n\n"
                                           "void main (void)\n"
    ...
    case DISCARDTEMPLATE_MAIN_DYNAMIC_LOOP:
        return GLSL_SHADER_TEMPLATE_HEADER "layout(set = 0, binding = 1) uniform block1 { mediump int  ui_two; };\n\n"
                                           "void main (void)\n"
                                           "{\n"
                                           "    o_color = v_color;\n"
...
static const char *getModeDesc(DiscardMode mode)
{
    ...
    case DISCARDMODE_DERIV:
        return "Discard based on derivatives after an earlier discard";
    default:
        DE_ASSERT(false);
        return DE_NULL;
    }
                                               ...
                                               const std::string &discardStr)
{
    ...
    {
    case DISCARDMODE_ALWAYS:
        params["DISCARD"] = discardStr;
        break;
    case DISCARDMODE_NEVER:
        params["DISCARD"] = "if (false) " + discardStr;
        break;
    case DISCARDMODE_UNIFORM:
        params["DISCARD"] = "if (ui_one > 0) " + discardStr;
        break;
    case DISCARDMODE_DYNAMIC:
        params["DISCARD"] = "if (v_coords.x+v_coords.y > 0.0) " + discardStr;
        break;
    case DISCARDMODE_TEXTURE:
        params["DISCARD"] = "if (texture(ut_brick, v_coords.xy*0.25+0.5).x < 0.7) " + discardStr;
        break;
    case DISCARDMODE_DERIV:
        params["DISCARD"] =
            ...
            "  bool valid = abs(dx.x-1.0) < 0.01 && dx.y == 0.0 && dy.x == 0.0 && abs(dy.y-1.0) < 0.01 && abs(dh-1.0) "
            "< 0.1 && !isHelper;\n"
            "  if (valid) demote;\n"
    ...
    return de::MovePtr<ShaderDiscardCase>(new ShaderDiscardCase(
        testCtx, name.c_str(), description.c_str(), shaderTemplate.specialize(params).c_str(), getEvalFunc(mode),
