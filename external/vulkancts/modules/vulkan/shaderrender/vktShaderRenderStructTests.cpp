...
{
public:
    ShaderStructCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                     bool isVertexCase, ShaderEvalFunc evalFunc, UniformSetupFunc setupUniformsFunc,
...
ShaderStructCase::ShaderStructCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                   bool isVertexCase, ShaderEvalFunc evalFunc, UniformSetupFunc setupUniformsFunc,
                                   const std::string &vertShaderSource, const std::string &fragShaderSource)
...
static de::MovePtr<ShaderStructCase> createStructCase(tcu::TestContext &testCtx, const std::string &name,
                                                      const std::string &description, bool isVertexCase,
                                                      ShaderEvalFunc evalFunc, UniformSetupFunc uniformFunc,
                                                      const LineStream &shaderSrc)
{
    static std::string defaultVertSrc = "#version 310 es\n"
                                        ...
                                        "}\n";
    static std::string defaultFragSrc = "#version 310 es\n"
    ...
    if (isVertexCase)
    {
        spParams["HEADER"]     = "#version 310 es\n"
                                 "layout(location = 0) in highp vec4 a_position;\n"
                                 "layout(location = 1) in highp vec4 a_coords;\n"
    ...
    return de::MovePtr<ShaderStructCase>(
        new ShaderStructCase(testCtx, name, description, isVertexCase, evalFunc, uniformFunc, vertSrc, fragSrc));
}

class LocalStructTests : public tcu::TestCaseGroup
{
public:
    LocalStructTests(tcu::TestContext &testCtx) : TestCaseGroup(testCtx, "local", "Local structs")
    {
    }
...
#define LOCAL_STRUCT_CASE(NAME, DESCRIPTION, SHADER_SRC, SET_UNIFORMS_BODY, EVAL_FUNC_BODY)                  \
    ...
    {                                                                                                        \
        ...
        {                                                                                                    \
            static void setUniforms(ShaderRenderCaseInstance &instance, const tcu::Vec4 &) SET_UNIFORMS_BODY \
        }; /* NOLINT(SET_UNIFORMS_BODY) */                                                                   \
        struct Eval_##NAME                                                                                   \
        {                                                                                                    \
            static void eval(ShaderEvalContext &c) EVAL_FUNC_BODY                                            \
        }; /* NOLINT(EVAL_FUNC_BODY) */                                                                      \
        addChild(createStructCase(m_testCtx, #NAME "_vertex", DESCRIPTION, true, &Eval_##NAME::eval,         \
                                  &SetUniforms_##NAME::setUniforms, SHADER_SRC)                              \
                     .release());                                                                            \
        addChild(createStructCase(m_testCtx, #NAME "_fragment", DESCRIPTION, false, &Eval_##NAME::eval,      \
                                  &SetUniforms_##NAME::setUniforms, SHADER_SRC)                              \
                     .release());                                                                            \
    } while (false)

    LOCAL_STRUCT_CASE(
        basic, "Basic struct usage",
        LineStream() << "${HEADER}"
                     ...
                     << "    ${ASSIGN_POS}"
                     << "}",
        { instance.useUniform(0u, UI_ONE); }, { c.color.xyz() = c.coords.swizzle(0, 1, 2); });
    ...
    LOCAL_STRUCT_CASE(
        ...
        LineStream() << "${HEADER}"
                     ...
                     << "    ${ASSIGN_POS}"
                     << "}",
        { instance.useUniform(0u, UI_ONE); }, { c.color.xyz() = c.coords.swizzle(3, 2, 1); });

    LOCAL_STRUCT_CASE(
        array_member_dynamic_index, "Struct with array member, dynamic indexing",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
    ...
    LOCAL_STRUCT_CASE(
        parameter, "Struct as a function parameter",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_one; };"
                     << ""
                     << "struct S {"
    ...
    LOCAL_STRUCT_CASE(
        parameter_nested, "Nested struct as a function parameter",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << ""
    ...
    LOCAL_STRUCT_CASE(
        return, "Struct as a return value",
              LineStream() << "${HEADER}"
                           << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_one; };"
                           << ""
                           << "struct S {"
    ...
    LOCAL_STRUCT_CASE(
        conditional_assignment, "Conditional struct assignment",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
    ...
    LOCAL_STRUCT_CASE(
        loop_assignment, "Struct assignment in loop",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << ""
    ...
    LOCAL_STRUCT_CASE(
        dynamic_loop_assignment, "Struct assignment in loop",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
    ...
    LOCAL_STRUCT_CASE(
        nested_conditional_assignment, "Conditional assignment of nested struct",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << "layout (std140, set = 0, binding = 2) uniform buffer2 { mediump float uf_one; };"
    ...
    LOCAL_STRUCT_CASE(
        nested_loop_assignment, "Nested struct assignment in loop",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << "layout (std140, set = 0, binding = 2) uniform buffer2 { mediump float uf_one; };"
    ...
    LOCAL_STRUCT_CASE(
        nested_dynamic_loop_assignment, "Nested struct assignment in dynamic loop",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << "layout (std140, set = 0, binding = 2) uniform buffer2 { int ui_three; };"
    ...
    LOCAL_STRUCT_CASE(
        loop_struct_array, "Struct array usage in loop",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << "layout (std140, set = 0, binding = 2) uniform buffer2 { int ui_two; };"
    ...
    LOCAL_STRUCT_CASE(
        loop_nested_struct_array, "Nested struct array usage in loop",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_zero; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_one; };"
                     << "layout (std140, set = 0, binding = 2) uniform buffer2 { int ui_two; };"
    ...
    LOCAL_STRUCT_CASE(
        basic_equal, "Basic struct equality",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_one; };"
    ...
    LOCAL_STRUCT_CASE(
        basic_not_equal, "Basic struct equality",
        LineStream() << "${HEADER}"
                     << "layout (std140, set = 0, binding = 0) uniform buffer0 { int ui_one; };"
                     << "layout (std140, set = 0, binding = 1) uniform buffer1 { int ui_two; };"
                     << ""
...
{
public:
    UniformStructTests(tcu::TestContext &testCtx) : TestCaseGroup(testCtx, "uniform", "Uniform structs")
    {
    }
...
#define UNIFORM_STRUCT_CASE(NAME, DESCRIPTION, SHADER_SRC, SET_UNIFORMS_BODY, EVAL_FUNC_BODY)                       \
    ...
    {                                                                                                               \
        struct SetUniforms_##NAME                                                                                   \
        {                                                                                                           \
            static void setUniforms(ShaderRenderCaseInstance &instance,                                             \
...
ShaderStructTests::ShaderStructTests(tcu::TestContext &testCtx) : TestCaseGroup(testCtx, "struct", "Struct Tests")
{
}
