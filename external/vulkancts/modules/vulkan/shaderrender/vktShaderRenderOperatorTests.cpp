...
{
public:
    ShaderOperatorCase(tcu::TestContext &testCtx, const char *caseName, const char *description,
                       const bool isVertexCase, const ShaderEvalFunc evalFunc, const std::string &shaderOp,
...
ShaderOperatorCase::ShaderOperatorCase(tcu::TestContext &testCtx, const char *caseName, const char *description,
                                       const bool isVertexCase, const ShaderEvalFunc evalFunc,
                                       const std::string &shaderOp, const ShaderDataSpec &spec)
    : ShaderRenderCase(testCtx, caseName, description, isVertexCase,
...
#define BOOL_GENTYPE_FUNCS(FUNC_NAME) \
                ...
                for (int precision = PRECISION_MEDIUMP; precision < PRECISION_LAST; precision++)
                {
                         ...
                         precision == PRECISION_MEDIUMP)) // use mediump interpolators for booleans
                    {
                        ...
                        for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                        {
                            ...
                            innerGroup->addChild(new ShaderOperatorCase(m_testCtx, name.c_str(), desc.c_str(),
    ...
    TestCaseGroup *sequenceNoSideEffGroup =
        new TestCaseGroup(m_testCtx, "no_side_effects", "Sequence tests without side-effects");
    TestCaseGroup *sequenceSideEffGroup =
        new TestCaseGroup(m_testCtx, "side_effects", "Sequence tests with side-effects");
