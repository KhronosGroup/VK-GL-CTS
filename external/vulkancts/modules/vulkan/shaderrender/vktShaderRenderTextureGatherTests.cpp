...
struct GatherCaseBaseParams
{
    ...
    GatherCaseBaseParams(const TextureType textureType_, const GatherType gatherType_, const OffsetSize offsetSize_,
                         ...
                         const tcu::Sampler::FilterMode magFilter_, const int baseLevel_, const uint32_t flags_,
...
string genGatherFuncCall(GatherType gatherType, const tcu::TextureFormat &textureFormat, const GatherArgs &gatherArgs,
                         const string &refZExpr, const IVec2 &offsetRange, int indentationDepth, OffsetSize offsetSize,
                         const ImageBackingMode sparseCase)
{
    string result;
                       ...
                       const vector<GatherArgs> &iterations)
{
    ...
    for (int iterNdx = 0; iterNdx < numIterations; iterNdx++)
    {
        ...
        const string funcCall = genGatherFuncCall(baseParams.gatherType, baseParams.textureFormat, gatherArgs, refZExpr,
                                                  offsetRange, 1, baseParams.offsetSize, baseParams.sparseCase);
        glu::FragmentSource frag = genFragmentShaderSource(
            ...
            useNormalizedCoord, usePixCoord, baseParams.offsetSize, baseParams.sparseCase);
...
{
public:
    TextureGather2DCase(tcu::TestContext &testCtx, const string &name, const string &description,
                        const GatherType gatherType, const OffsetSize offsetSize,
                        ...
                        const MaybeTextureSwizzle &textureSwizzle, const tcu::Sampler::FilterMode minFilter,
                        const tcu::Sampler::FilterMode magFilter, const int baseLevel, const uint32_t flags,
                        const IVec2 &textureSize, const ImageBackingMode sparseCase);
...
TextureGather2DCase::TextureGather2DCase(
    ...
    const tcu::Sampler::FilterMode minFilter, const tcu::Sampler::FilterMode magFilter, const int baseLevel,
    const uint32_t flags, const IVec2 &textureSize, const ImageBackingMode sparseCase)
    : TestCase(testCtx, name, description)
    , m_baseParams(TEXTURETYPE_2D, gatherType, offsetSize, textureFormat, shadowCompareMode, wrapS, wrapT,
                   textureSwizzle, minFilter, magFilter, baseLevel, flags, sparseCase)
    , m_textureSize(textureSize)
{
}
...
void TextureGather2DCase::initPrograms(vk::SourceCollections &dst) const
{
    const vector<GatherArgs> iterations = generateBasic2DCaseIterations(
        m_baseParams.gatherType, m_baseParams.textureFormat,
...
{
    const vector<GatherArgs> iterations =
        generateBasic2DCaseIterations(m_baseParams.gatherType, m_baseParams.textureFormat,
...
vector<Gather2DArrayArgs> generate2DArrayCaseIterations(GatherType gatherType, const tcu::TextureFormat &textureFormat,
...
{
    vector<float> res;
    TextureTestUtil::computeQuadTexCoord2DArray(res, m_iterations[iterationNdx].layerNdx, Vec2(-0.3f, -0.4f),
                                                Vec2(1.5f, 1.6f));
    return res;
}
...
{
public:
    TextureGather2DArrayCase(tcu::TestContext &testCtx, const string &name, const string &description,
                             const GatherType gatherType, const OffsetSize offsetSize,
                             const tcu::TextureFormat textureFormat, const tcu::Sampler::CompareMode shadowCompareMode,
                             const tcu::Sampler::WrapMode wrapS, const tcu::Sampler::WrapMode wrapT,
                             const MaybeTextureSwizzle &textureSwizzle, const tcu::Sampler::FilterMode minFilter,
...
TextureGather2DArrayCase::TextureGather2DArrayCase(
    ...
    const tcu::Sampler::WrapMode wrapT, const MaybeTextureSwizzle &textureSwizzle,
    const tcu::Sampler::FilterMode minFilter, const tcu::Sampler::FilterMode magFilter, const int baseLevel,
    const uint32_t flags, const IVec3 &textureSize, const ImageBackingMode sparseCase)
    : TestCase(testCtx, name, description)
...
vector<GatherCubeArgs> generateCubeCaseIterations(GatherType gatherType, const tcu::TextureFormat &textureFormat,
                                                  const IVec2 &offsetRange)
{
    const vector<GatherArgs> basicIterations = generateBasic2DCaseIterations(gatherType, textureFormat, offsetRange);
...
{
public:
    TextureGatherCubeCase(tcu::TestContext &testCtx, const string &name, const string &description,
                          const tcu::TextureFormat textureFormat, const tcu::Sampler::CompareMode shadowCompareMode,
                          const tcu::Sampler::WrapMode wrapS, const tcu::Sampler::WrapMode wrapT,
                          const MaybeTextureSwizzle &textureSwizzle, const tcu::Sampler::FilterMode minFilter,
                          const tcu::Sampler::FilterMode magFilter, const int baseLevel, const uint32_t flags,
                          const int textureSize, const ImageBackingMode sparseCase);
...
TextureGatherCubeCase::TextureGatherCubeCase(
    tcu::TestContext &testCtx, const string &name, const string &description, const tcu::TextureFormat textureFormat,
    const tcu::Sampler::CompareMode shadowCompareMode, const tcu::Sampler::WrapMode wrapS,
    const tcu::Sampler::WrapMode wrapT, const MaybeTextureSwizzle &textureSwizzle,
    const tcu::Sampler::FilterMode minFilter, const tcu::Sampler::FilterMode magFilter, const int baseLevel,
    const uint32_t flags, const int textureSize, const ImageBackingMode sparseCase)
    : TestCase(testCtx, name, description)
...
{
    const vector<GatherCubeArgs> iterations =
        generateCubeCaseIterations(m_baseParams.gatherType, m_baseParams.textureFormat,
                                   getOffsetRange(m_baseParams.offsetSize, context.getDeviceProperties().limits));
...
static inline TestCase *makeTextureGatherCase(
    TextureType textureType, tcu::TestContext &testCtx, const string &name, const string &description,
    GatherType gatherType, OffsetSize offsetSize, tcu::TextureFormat textureFormat,
    tcu::Sampler::CompareMode shadowCompareMode, tcu::Sampler::WrapMode wrapS, tcu::Sampler::WrapMode wrapT,
    const MaybeTextureSwizzle &texSwizzle, tcu::Sampler::FilterMode minFilter, tcu::Sampler::FilterMode magFilter,
    int baseLevel, const IVec3 &textureSize, uint32_t flags = 0,
    const ImageBackingMode sparseCase = ShaderRenderCaseInstance::IMAGE_BACKING_MODE_REGULAR)
{
    switch (textureType)
    {
    case TEXTURETYPE_2D:
        return new TextureGather2DCase(testCtx, name, description, gatherType, offsetSize, textureFormat,
                                       shadowCompareMode, wrapS, wrapT, texSwizzle, minFilter, magFilter, baseLevel,
                                       flags, textureSize.swizzle(0, 1), sparseCase);

    case TEXTURETYPE_2D_ARRAY:
        return new TextureGather2DArrayCase(testCtx, name, description, gatherType, offsetSize, textureFormat,
    ...
    case TEXTURETYPE_CUBE:
        ...
        return new TextureGatherCubeCase(testCtx, name, description, textureFormat, shadowCompareMode, wrapS, wrapT,
...
void TextureGatherTests::init(void)
{
    ...
    for (int gatherTypeI = 0; gatherTypeI < GATHERTYPE_LAST; gatherTypeI++)
    {
        ...
        TestCaseGroup *const gatherTypeGroup =
            new TestCaseGroup(m_testCtx, gatherTypeName(gatherType), gatherTypeDescription(gatherType));
                ...
                offsetSize == OFFSETSIZE_NONE ?
                    ...
                    new TestCaseGroup(m_testCtx,
                                      ...
                                      offsetSize == OFFSETSIZE_MINIMUM_REQUIRED ?
                                          "Use offsets within Vulkan minimum required range" :
                                      offsetSize == OFFSETSIZE_IMPLEMENTATION_MAXIMUM ?
                            ...
                            for (int compareModeI = 0; compareModeI < tcu::Sampler::COMPAREMODE_LAST; compareModeI++)
                            {
                                ...
                                for (int wrapCaseNdx = 0; wrapCaseNdx < DE_LENGTH_OF_ARRAY(wrapModes); wrapCaseNdx++)
                                {
                                    ...
                                    compareModeGroup->addChild(makeTextureGatherCase(
                                        ...
                                        tcu::Sampler::NEAREST, 0, textureSize,
                                        noCorners ? GATHERCASE_DONT_SAMPLE_CUBE_CORNERS : 0,
                            ...
                            GATHERTYPE_OFFSETS) // Don't test all features for both offset size types, as they should be rather orthogonal.
                    {
                        if (!isDepthFormat(format))
                        {
                                 ...
                                 swizzleCaseNdx++)
                            {
                                ...
                                swizzleGroup->addChild(makeTextureGatherCase(
                                    ...
                                    tcu::Sampler::COMPAREMODE_NONE, tcu::Sampler::REPEAT_GL, tcu::Sampler::REPEAT_GL,
                                    swizzle, tcu::Sampler::NEAREST, tcu::Sampler::NEAREST, 0, IVec3(64, 64, 3), 0,
                                    ShaderRenderCaseInstance::IMAGE_BACKING_MODE_SPARSE));
                            }
                        }

                        {
                            TestCaseGroup *const filterModeGroup =
                                new TestCaseGroup(m_testCtx, "filter_mode", "Test that filter modes have no effect");
...

                        {
                            ...
                            for (int baseLevel = 1; baseLevel <= 2; baseLevel++)
                            {
                                ...
                                baseLevelGroup->addChild(makeTextureGatherCase(
                                    ...
                                    compareMode, tcu::Sampler::REPEAT_GL, tcu::Sampler::REPEAT_GL,
                                    MaybeTextureSwizzle::createNoneTextureSwizzle(), tcu::Sampler::NEAREST,
                                    tcu::Sampler::NEAREST, baseLevel, IVec3(64, 64, 3), 0,
                                    ShaderRenderCaseInstance::IMAGE_BACKING_MODE_SPARSE));
                            }
                        }
                    }
