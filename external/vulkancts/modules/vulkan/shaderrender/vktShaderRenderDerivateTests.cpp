    ...
    if (m_definitions.inNonUniformControlFlow)
    {
        ...
        if ((subgroupProperties.supportedOperations & VK_SUBGROUP_FEATURE_BALLOT_BIT) == 0)
            throw tcu::NotSupportedError(
                "Derivative dynamic control flow tests require VK_SUBGROUP_FEATURE_BALLOT_BIT");

        if ((subgroupProperties.supportedStages & VK_SHADER_STAGE_FRAGMENT_BIT) == 0)
            throw tcu::NotSupportedError("Derivative dynamic control flow tests require subgroup supported stage "
                                         "including VK_SHADER_STAGE_FRAGMENT_BIT");
    }
...
{
public:
    TriangleDerivateCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
TriangleDerivateCase::TriangleDerivateCase(tcu::TestContext &testCtx, const std::string &name,
                                           const std::string &description, const UniformSetup *uniformSetup)
...
{
public:
    ConstantDerivateCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
ConstantDerivateCase::ConstantDerivateCase(tcu::TestContext &testCtx, const std::string &name,
                                           const std::string &description, DerivateFunc func, glu::DataType type)
...
{
public:
    LinearDerivateCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       DerivateFunc func, glu::DataType type, glu::Precision precision, bool inNonUniformControlFlow,
...
LinearDerivateCase::LinearDerivateCase(tcu::TestContext &testCtx, const std::string &name,
                                       const std::string &description, DerivateFunc func, glu::DataType type,
                                       glu::Precision precision, bool inNonUniformControlFlow, SurfaceType surfaceType,
                                       int numSamples, const std::string &fragmentSrcTmpl,
                                       BaseUniformType usedDefaultUniform)
...
void LinearDerivateCase::initPrograms(vk::SourceCollections &programCollection) const
{
    const SpirvVersion spirvVersion =
        m_definitions.inNonUniformControlFlow ? vk::SPIRV_VERSION_1_3 : vk::SPIRV_VERSION_1_0;
    ...
    , m_textureValues(textureValues)
{
    ...
    de::SharedPtr<TextureBinding> testTexture(new TextureBinding(
        texture.release(), tcu::Sampler(tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE,
...
{
public:
    TextureDerivateCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                        DerivateFunc func, glu::DataType type, glu::Precision precision, SurfaceType surfaceType,
                        int numSamples);
...
TextureDerivateCase::TextureDerivateCase(tcu::TestContext &testCtx, const std::string &name,
                                         const std::string &description, DerivateFunc func, glu::DataType type,
...
void ShaderDerivateTests::init(void)
{
    ...
    for (int funcNdx = 0; funcNdx < DERIVATE_LAST; funcNdx++)
    {
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> constantGroup(
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> linearCaseGroup(new tcu::TestCaseGroup(
            ...
            for (int vecSize = 1; vecSize <= 4; vecSize++)
            {
                for (int precNdx = 0; precNdx < glu::PRECISION_LAST; precNdx++)
                {
                    ...
                    linearCaseGroup->addChild(
                        new LinearDerivateCase(m_testCtx, caseName.str(), "", function, dataType, precision,
                                               s_linearDerivateCases[caseNdx].inNonUniformControlFlow, surfaceType,
                                               numSamples, source, s_linearDerivateCases[caseNdx].usedDefaultUniform));
                }
            }
        ...
        for (int caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(s_fboConfigs); caseNdx++)
        {
            ...
            for (int vecSize = 1; vecSize <= 4; vecSize++)
            {
                for (int precNdx = 0; precNdx < glu::PRECISION_LAST; precNdx++)
                {
                    ...
                    fboGroup->addChild(new LinearDerivateCase(m_testCtx, caseName.str(), "", function, dataType,
                                                              precision, false, surfaceType, numSamples, source,
                                                              U_LAST));
                }
            }
        ...
        // .texture
        {
            de::MovePtr<tcu::TestCaseGroup> textureGroup(
