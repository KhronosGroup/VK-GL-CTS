...
{
public:
    ShaderLoopCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                                         ...
                                                         glu::DataType loopCountDataType)
{
    ...
    return de::MovePtr<ShaderLoopCase>(new ShaderLoopCase(testCtx, caseName, description, isVertexCase, evalFunc,
                                                          uniformSetup, vertexShaderSource, fragmentShaderSource));
}
...
ShaderLoopTests::ShaderLoopTests(tcu::TestContext &testCtx) : TestCaseGroup(testCtx, "loops", "Loop Tests")
{
}
        ...
        for (int loopCountType = 0; loopCountType < LOOPCOUNT_LAST; loopCountType++)
        {
                ...
                for (int dataTypeNdx = 0; dataTypeNdx < DE_LENGTH_OF_ARRAY(s_countDataType); dataTypeNdx++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        std::string testDesc = std::string(loopTypeName) + " loop with " + precisionName +
                                               dataTypeName + " " + loopCountName + " iteration count in " +
                                               shaderTypeName + " shader.";
                        de::MovePtr<ShaderLoopCase> testCase(createGenericLoopCase(
                            m_testCtx, testName.c_str(), testDesc.c_str(), isVertexCase, (LoopType)loopType,
            ...
            for (int loopCase = 0; loopCase < LOOPCASE_LAST; loopCase++)
            {
                ...
                for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                {
                    ...
                    std::string desc = std::string(loopCaseName) + " loop with " + loopTypeName +
                                       " iteration count in " + shaderTypeName + " shader.";
                    de::MovePtr<ShaderLoopCase> testCase(
                        createSpecialLoopCase(m_testCtx, name.c_str(), desc.c_str(), isVertexCase, (LoopCase)loopCase,
                                              (LoopType)loopType, (LoopCountType)loopCountType));
