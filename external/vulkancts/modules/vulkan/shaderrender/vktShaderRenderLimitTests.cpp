...
TestStatus FragmentInputComponentCaseInstance::iterate(void)
{
    ...
    const uint16_t indices[12] = {0, 4, 1, 0, 5, 4, 1, 2, 3, 1, 3, 4};
    ...
    compareOk = pixelThresholdCompare(m_context.getTestContext().getLog(), "Result", "Image comparison result",
                                      refImage, resImage, threshold, COMPARE_LOG_RESULT);
...
{
    const float vertices[] = {-1.0f, -1.0f, 0.0f, 1.0f, 0.0f, -1.0f, 0.0f, 1.0f, 1.0f,  -1.0f, 0.0f, 1.0f,
                              1.0f,  1.0f,  0.0f, 1.0f, 0.0f, 1.0f,  0.0f, 1.0f, -1.0f, 1.0f,  0.0f, 1.0f};

    addAttribute(0u, VK_FORMAT_R32G32B32A32_SFLOAT, uint16_t(sizeof(float) * 4), 6, vertices);
}

class FragmentInputComponentCase : public TestCase
{
public:
    FragmentInputComponentCase(TestContext &testCtx, const string &name, const string &description,
                               const uint16_t inputComponents);
...
private:
    ...
    const uint16_t m_inputComponents;
};

FragmentInputComponentCase::FragmentInputComponentCase(TestContext &testCtx, const string &name,
                                                       const string &description, const uint16_t inputComponents)
...
{
    const tcu::StringTemplate vertexCodeTemplate("#version 450\n"
    ...
    uint16_t maxLocations = (uint16_t)deCeilToInt32((float)(m_inputComponents - 4) / 4u);
    string varyingType;
    map<string, string> vertexParams;
    map<string, string> fragmentParams;

    for (uint16_t loc = 0; loc < maxLocations; loc++)
    {
        ...
        vertexParams["VARYING_OUT"] += "layout(location = " + de::toString(loc) + ") out highp " + varyingType +
                                       " o_color" + de::toString(loc) + ";\n";
        vertexParams["VARYING_DECL"] +=
            "    o_color" + de::toString(loc) + " = " + varyingType + "(" + de::toString(loc) + ".0);\n";
        fragmentParams["VARYING_IN"] += "layout(location = " + de::toString(loc) + ") in highp " + varyingType +
                                        " i_color" + de::toString(loc) + ";\n";
        fragmentParams["VERIFY"] += "    errorCount += (i_color" + de::toString(loc) + " == " + varyingType + "(" +
                                    de::toString(loc) + ".0)) ? 0 : 1;\n";
    }
...
TestInstance *FragmentInputComponentCase::createInstance(Context &context) const
{
    ...
    const uint16_t maxFragmentInputComponents = (uint16_t)limits.maxFragmentInputComponents;
    const uint16_t maxVertexOutputComponents  = (uint16_t)limits.maxVertexOutputComponents;
...
TestCaseGroup *createLimitTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> inputComponentsGroup(
        new TestCaseGroup(testCtx, "fragment_input", "Fragment input component variations"));

    // Fragment input component case
    uint16_t fragmentComponentMaxLimits[] = {64u, 128u, 256u};

    for (uint16_t limitNdx = 0; limitNdx < DE_LENGTH_OF_ARRAY(fragmentComponentMaxLimits); limitNdx++)
    {
        for (int16_t cases = 5; cases > 0; cases--)
            inputComponentsGroup->addChild(new FragmentInputComponentCase(
                testCtx, "components_" + de::toString(fragmentComponentMaxLimits[limitNdx] - cases),
                "Input component count", (uint16_t)(fragmentComponentMaxLimits[limitNdx] - cases)));
    }
