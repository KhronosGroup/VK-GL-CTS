...
{
public:
    ShaderMatrixInstance(Context &context, bool isVertex, const ShaderEvaluator &evaluator, const ShaderInput in0,
                         const ShaderInput in1, const MatrixOp m_op);
...
private:
    void addMatrixUniform(uint32_t bindingLocation, DataType dataType, const float *dataPtr);
    ...
    , m_op(op)
{
    ...
    for (int attribNdx = 0; attribNdx < 4; attribNdx++)
    {
        ...
        m_userAttribTransforms[attribNdx](0, 3) =
            (op == OP_INVERSE ? -0.5f : 0.2f); // prevent matrix*vec from going into zero (assuming vec.w != 0).
        m_userAttribTransforms[attribNdx](1, 3) =
            (op == OP_INVERSE ? -1.3f : 0.1f); // Modified input for OP_INVERSE case, as determinant of final input
        m_userAttribTransforms[attribNdx](2, 3) =
            0.4f +
            0.15f *
                float(attribNdx); // matrix is spanning both sides of 0, so 0 (and division by 0) may happen on mediump.
        m_userAttribTransforms[attribNdx](3, 3) =
            (op == OP_INVERSE ? -3.0f :
                                0.7f); // Modified OP_INVERSE final input matrix is same signed in whole input range.
...
void ShaderMatrixInstance::addMatrixUniform(uint32_t bindingLocation, DataType dataType, const float *dataPtr)
{
    Mat4 result;
    ...
    addUniform(bindingLocation, vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, matrixSize,
               result.getColumnMajorData().getPtr());
}
...
void ShaderMatrixInstance::setupUniforms(const tcu::Vec4 &)
{
    ...
    for (int inNdx = 0; inNdx < numInputs; inNdx++)
    {
        ...
        if (in.inputType == INPUTTYPE_UNIFORM)
        {
            ...
            {
            case TYPE_FLOAT:
                addUniform(uniformBinding, vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, sizeof(float),
                           &s_constInFloat[inNdx]);
                break;
            case TYPE_FLOAT_VEC2:
                addUniform(uniformBinding, vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, s_constInVec2[inNdx]);
                break;
            case TYPE_FLOAT_VEC3:
                addUniform(uniformBinding, vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, s_constInVec3[inNdx]);
                break;
            case TYPE_FLOAT_VEC4:
                addUniform(uniformBinding, vk::VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, s_constInVec4[inNdx]);
                break;
            // \note GLES3 supports transpose in matrix upload.
            case TYPE_FLOAT_MAT2:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat2x2[inNdx]);
                break;
            case TYPE_FLOAT_MAT2X3:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat2x3[inNdx]);
                break;
            case TYPE_FLOAT_MAT2X4:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat2x4[inNdx]);
                break;
            case TYPE_FLOAT_MAT3X2:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat3x2[inNdx]);
                break;
            case TYPE_FLOAT_MAT3:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat3x3[inNdx]);
                break;
            case TYPE_FLOAT_MAT3X4:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat3x4[inNdx]);
                break;
            case TYPE_FLOAT_MAT4X2:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat4x2[inNdx]);
                break;
            case TYPE_FLOAT_MAT4X3:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat4x3[inNdx]);
                break;
            case TYPE_FLOAT_MAT4:
                addMatrixUniform(uniformBinding, in.dataType, s_constInMat4x4[inNdx]);
                break;
            default:
                DE_ASSERT(false);
            }
...
{
public:
    ShaderMatrixCase(tcu::TestContext &testCtx, const std::string &name, const ShaderInput &in0, const ShaderInput &in1,
                     const MatrixOp op, bool isVertexCase);
...
ShaderMatrixCase::ShaderMatrixCase(tcu::TestContext &testCtx, const std::string &name, const ShaderInput &in0,
...
void ShaderMatrixCase::setupShader(void)
{
    ...
    static const std::string header = "#version 310 es\n";
    ...
    for (int inNdx = 0; inNdx < numInputs; inNdx++)
    {
        ...
        if (in.inputType == INPUTTYPE_DYNAMIC)
        {
            if (isDataTypeMatrix(in.dataType))
            {
                ...
                if (!m_isVertexCase)
                {
                    vtx << "layout(location = " << 1 + inNdx + padding << ") out " << precName << " " << typeName
                        << " v_" << typeName << ";\n";
                    frag << "layout(location = " << 1 + inNdx + padding << ") in " << precName << " " << typeName
                         << " v_" << typeName << ";\n";
            ...
            else
            {
                ...
                if (!m_isVertexCase)
                {
                    vtx << "layout(location = " << 1 + padding << ") out " << precName << " " << typeName
                        << " v_coords;\n";
                    frag << "layout(location = " << 1 + padding << ") in " << precName << " " << typeName
                         << " v_coords;\n";
        ...
        {
            op << "layout(std140, set = 0, binding = " << uniformBinding++ << ") uniform buffer" << inNdx << " { "
               << precName << " " << typeName << " u_in" << inNdx << "; };\n";
...
ShaderMatrixTests::ShaderMatrixTests(tcu::TestContext &testCtx) : TestCaseGroup(testCtx, "matrix")
{
}
...
void ShaderMatrixTests::init(void)
{
    ...
    static const InputTypeSpec extendedInputTypes[] = {// Constant matrix input
                                                       {"const", INPUTTYPE_CONST},
                                                       // Uniform matrix input
                                                       {"uniform", INPUTTYPE_UNIFORM},
                                                       // Dynamic matrix input
                                                       {"dynamic", INPUTTYPE_DYNAMIC}};
    static const InputTypeSpec reducedInputTypes[]  = {// Dynamic matrix input
                                                      {"dynamic", INPUTTYPE_DYNAMIC}};
                ...
                for (int precNdx = 0; precNdx < DE_LENGTH_OF_ARRAY(precisions); precNdx++)
                {
                    ...
                    if (isOperationMatrixScalar(op))
                    {
                        ...
                        ShaderInput scalarIn(op == OP_DIV ? INPUTTYPE_UNIFORM : INPUTTYPE_DYNAMIC, TYPE_FLOAT,
                                             precision);
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (baseName + "float_vertex").c_str(), matIn,
                                                               scalarIn, op, true));
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (baseName + "float_fragment").c_str(), matIn,
                                                               scalarIn, op, false));
                    }
                    ...
                    if (isOperationMatrixVector(op))
                    {
                        ...
                        inGroup->addChild(new ShaderMatrixCase(
                            m_testCtx, (baseName + getDataTypeName(colVecType) + "_vertex").c_str(), matIn, colVecIn,
                            op, true));
                        inGroup->addChild(new ShaderMatrixCase(
                            m_testCtx, (baseName + getDataTypeName(colVecType) + "_fragment").c_str(), matIn, colVecIn,
                            op, false));
                        ...
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (vecMatName + "_vertex").c_str(), rowVecIn,
                                                               matIn, op, true));
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (vecMatName + "_fragment").c_str(), rowVecIn,
                                                               matIn, op, false));
                    }
                    ...
                    if (isOperationArithmeticMatrixMatrix(op))
                    {
                        ...
                        for (int otherCols = 2; otherCols <= 4; otherCols++)
                        {
                            ...
                            inGroup->addChild(new ShaderMatrixCase(
                                m_testCtx, (baseName + getDataTypeName(otherMatIn.dataType) + "_vertex").c_str(), matIn,
                                otherMatIn, op, true));
                            inGroup->addChild(new ShaderMatrixCase(
                                m_testCtx, (baseName + getDataTypeName(otherMatIn.dataType) + "_fragment").c_str(),
                                matIn, otherMatIn, op, false));
                        }
                    }
                    else if (isOperationComponentwiseMatrixMatrix(op))
                    {
                        ...
                        ShaderInput otherMatIn(inputType == INPUTTYPE_DYNAMIC ? INPUTTYPE_UNIFORM : inputType, matType,
                                               precision);
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (baseName + matTypeName + "_vertex").c_str(),
                                                               matIn, otherMatIn, op, true));
                        inGroup->addChild(new ShaderMatrixCase(
                            m_testCtx, (baseName + matTypeName + "_fragment").c_str(), matIn, otherMatIn, op, false));
                    }
                    ...
                    if (isOperationVectorVector(op))
                    {
                        ...
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (baseName + "float_vertex").c_str(), vec1In,
                                                               vec2In, op, true));
                        inGroup->addChild(new ShaderMatrixCase(m_testCtx, (baseName + "float_fragment").c_str(), vec1In,
                                                               vec2In, op, false));
                    }
