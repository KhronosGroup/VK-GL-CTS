...
{
public:
    ShaderSwitchCase(tcu::TestContext &testCtx, const string &name, bool isVertexCase, const string &vtxSource,
                     const string &fragSource, ShaderEvalFunc evalFunc, UniformSetupFunc setupUniformsFunc);
...
static de::MovePtr<ShaderSwitchCase> makeSwitchCase(tcu::TestContext &testCtx, const string &name, SwitchType type,
...
ShaderSwitchTests::ShaderSwitchTests(tcu::TestContext &testCtx) : tcu::TestCaseGroup(testCtx, "switch")
{
}
...
void ShaderSwitchTests::makeSwitchCases(const string &name, const LineStream &switchBody, const bool skipDynamicType)
{
    ...
    for (int type = 0; type < SWITCHTYPE_LAST; type++)
    {
        ...
        addChild(makeSwitchCase(m_testCtx, (name + "_" + switchTypeNames[type] + "_vertex"), (SwitchType)type, true,
                                switchBody)
                     .release());
        addChild(makeSwitchCase(m_testCtx, (name + "_" + switchTypeNames[type] + "_fragment"), (SwitchType)type, false,
                                switchBody)
                     .release());
    }
}
...
void ShaderSwitchTests::init(void)
{
    ...
    makeSwitchCases("basic", LineStream(1) << "switch (${CONDITION})"
                                           << "{"
    ...
    makeSwitchCases("const_expr_in_label", LineStream(1) << "const int t = 2;"
                                                         << "switch (${CONDITION})"
                                                         << "{"
    ...
    // Default label usage
    makeSwitchCases("default_label", LineStream(1) << "switch (${CONDITION})"
                                                   << "{"
                                                   << "    case 0: res = coords.xyz;    break;"
                                                   << "    case 1: res = coords.wzy;    break;"
                                                   << "    case 3: res = coords.zyx;    break;"
    ...
    // Default label usage
    makeSwitchCases("default_not_last", LineStream(1) << "switch (${CONDITION})"
                                                      << "{"
                                                      << "    case 0: res = coords.xyz;    break;"
    ...
    makeSwitchCases("no_default_label", LineStream(1) << "res = coords.yzw;\n"
                                                      << "switch (${CONDITION})"
                                                      << "{"
                                                      << "    case 0: res = coords.xyz;    break;"
                                                      << "    case 1: res = coords.wzy;    break;"
    ...
    // Fall-through
    makeSwitchCases("fall_through", LineStream(1) << "switch (${CONDITION})"
                                                  << "{"
                                                  << "    case 0: res = coords.xyz;    break;"
                                                  << "    case 1: res = coords.wzy;    break;"
    ...
    // Fall-through
    makeSwitchCases("fall_through_default", LineStream(1) << "switch (${CONDITION})"
                                                          << "{"
                                                          << "    case 0: res = coords.xyz;    break;"
                                                          << "    case 1: res = coords.wzy;    break;"
                                                          << "    case 3: res = coords.zyx;    break;"
    ...
    // Fall-through
    makeSwitchCases("conditional_fall_through", LineStream(1) << "highp vec4 tmp = coords;"
                                                              << "switch (${CONDITION})"
                                                              << "{"
                                                              << "    case 0: res = coords.xyz;    break;"
                                                              << "    case 1: res = coords.wzy;    break;"
    ...
    // Fall-through
    makeSwitchCases("conditional_fall_through_2", LineStream(1) << "highp vec4 tmp = coords;"
    ...
    // Basic switch statement usage
    makeSwitchCases("scope", LineStream(1) << "switch (${CONDITION})"
                                           << "{"
                                           << "    case 0: res = coords.xyz;    break;"
                                           << "    case 1: res = coords.wzy;    break;"
                                           << "    case 2:"
    ...
    // Switch in for loop
    makeSwitchCases("switch_in_if", LineStream(1) << "if (${CONDITION} >= 0)"
                                                  << "{"
                                                  << "    switch (${CONDITION})"
                                                  << "    {"
                                                  << "        case 0: res = coords.xyz;    break;"
    ...
    // Switch in for loop
    makeSwitchCases("switch_in_for_loop", LineStream(1) << "for (int i = 0; i <= ${CONDITION}; i++)"
                                                        << "{"
                                                        << "    switch (i)"
                                                        << "    {"
                                                        << "        case 0: res = coords.xyz;    break;"
    ...
    makeSwitchCases("switch_in_while_loop", LineStream(1) << "int i = 0;"
                                                          << "while (i <= ${CONDITION})"
                                                          << "{"
                                                          << "    switch (i)"
                                                          << "    {"
    ...
    makeSwitchCases("switch_in_do_while_loop", LineStream(1) << "int i = 0;"
                                                             << "do"
                                                             << "{"
                                                             << "    switch (i)"
                                                             << "    {"
    ...
    // Basic switch statement usage
    makeSwitchCases("if_in_switch", LineStream(1) << "switch (${CONDITION})"
                                                  << "{"
                                                  << "    case 0: res = coords.xyz;    break;"
                                                  << "    case 1: res = coords.wzy;    break;"
                                                  << "    default:"
    ...
    // Basic switch statement usage
    makeSwitchCases("for_loop_in_switch", LineStream(1) << "switch (${CONDITION})"
                                                        << "{"
                                                        << "    case 0: res = coords.xyz;    break;"
                                                        << "    case 1:"
                                                        << "    case 2:"
    ...
    // Basic switch statement usage
    makeSwitchCases("while_loop_in_switch", LineStream(1) << "switch (${CONDITION})"
                                                          << "{"
                                                          << "    case 0: res = coords.xyz;    break;"
                                                          << "    case 1:"
                                                          << "    case 2:"
    ...
    // Basic switch statement usage
    makeSwitchCases("do_while_loop_in_switch", LineStream(1) << "switch (${CONDITION})"
                                                             << "{"
                                                             << "    case 0: res = coords.xyz;    break;"
                                                             << "    case 1:"
                                                             << "    case 2:"
    ...
    // Basic switch statement usage
    makeSwitchCases("switch_in_switch", LineStream(1) << "switch (${CONDITION})"
                                                      << "{"
                                                      << "    case 0: res = coords.xyz;    break;"
                                                      << "    case 1:"
                                                      << "    case 2:"
