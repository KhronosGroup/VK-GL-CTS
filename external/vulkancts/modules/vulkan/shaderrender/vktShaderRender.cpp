                                          ...
                                          ShaderRenderCaseInstance::ImageBackingMode backingMode)
{
    ...
    VkImageCreateFlags imageCreateFlags =
        (isCube ? static_cast<VkImageCreateFlags>(VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT) : 0u);
...
static const uint32_t MAX_RENDER_WIDTH     = 128;
static const uint32_t MAX_RENDER_HEIGHT    = 128;
...
{
public:
    QuadGrid(int gridSize, int screenWidth, int screenHeight, const tcu::Vec4 &constCoords,
             const std::vector<tcu::Mat4> &userAttribTransforms, const std::vector<TextureBindingSp> &textures);
    ~QuadGrid(void);
    ...
    const tcu::Vec4 &getConstCoords(void) const
    {
        return m_constCoords;
    }
    const std::vector<tcu::Mat4> getUserAttribTransforms(void) const
    {
        return m_userAttribTransforms;
    }
    const std::vector<TextureBindingSp> &getTextures(void) const
    {
        return m_textures;
    }

    const tcu::Vec4 *getPositions(void) const
    {
    ...
    const tcu::Vec4 *getCoords(void) const
    {
        return &m_coords[0];
    }
    const tcu::Vec4 *getUnitCoords(void) const
    {
        return &m_unitCoords[0];
    }

    const tcu::Vec4 *getUserAttrib(int attribNdx) const
    {
...
QuadGrid::QuadGrid(int gridSize, int width, int height, const tcu::Vec4 &constCoords,
                   const std::vector<tcu::Mat4> &userAttribTransforms, const std::vector<TextureBindingSp> &textures)
    : m_gridSize(gridSize)
...
TextureBinding::TextureBinding(const tcu::Archive &archive, const char *filename, const Type type,
    ...
    , m_sampler(sampler)
{
    ...
    {
    case TYPE_2D:
        m_binding.tex2D = loadTexture2D(archive, filename).release();
        break;
    default:
...
TextureBinding::~TextureBinding(void)
{
    ...
    {
    case TYPE_1D:
        delete m_binding.tex1D;
        break;
    case TYPE_2D:
        delete m_binding.tex2D;
        break;
    case TYPE_3D:
        delete m_binding.tex3D;
        break;
    case TYPE_CUBE_MAP:
        delete m_binding.texCube;
        break;
    case TYPE_1D_ARRAY:
        delete m_binding.tex1DArray;
        break;
    case TYPE_2D_ARRAY:
        delete m_binding.tex2DArray;
        break;
    case TYPE_CUBE_ARRAY:
        delete m_binding.texCubeArray;
        break;
    default:
        break;
    }
...
de::MovePtr<tcu::Texture2D> TextureBinding::loadTexture2D(const tcu::Archive &archive, const char *filename)
{
    ...
    de::MovePtr<tcu::Texture2D> texture(
        new tcu::Texture2D(tcu::TextureFormat(tcu::TextureFormat::RGBA, tcu::TextureFormat::UNORM_INT8),
    ...
    , m_quadGrid(quadGrid)
{
    ...
    for (int ndx = 0; ndx < (int)bindings.size(); ndx++)
    {
        ...
        {
        case TextureBinding::TYPE_1D:
            textures[ndx].tex1D = &binding.get1D();
            break;
        case TextureBinding::TYPE_2D:
            textures[ndx].tex2D = &binding.get2D();
            break;
        case TextureBinding::TYPE_3D:
            textures[ndx].tex3D = &binding.get3D();
            break;
        case TextureBinding::TYPE_CUBE_MAP:
            textures[ndx].texCube = &binding.getCube();
            break;
        case TextureBinding::TYPE_1D_ARRAY:
            textures[ndx].tex1DArray = &binding.get1DArray();
            break;
        case TextureBinding::TYPE_2D_ARRAY:
            textures[ndx].tex2DArray = &binding.get2DArray();
            break;
        case TextureBinding::TYPE_CUBE_ARRAY:
            textures[ndx].texCubeArray = &binding.getCubeArray();
            break;
        default:
...
UniformSetup::UniformSetup(UniformSetupFunc setupFunc) : m_setupFunc(setupFunc)
{
}
...
ShaderRenderCase::ShaderRenderCase(tcu::TestContext &testCtx, const std::string &name, const bool isVertexCase,
...
ShaderRenderCaseInstance::ShaderRenderCaseInstance(Context &context, const bool isVertexCase,
                                                   const ShaderEvaluator &evaluator, const UniformSetup &uniformSetup,
                                                   const AttributeSetupFunc attribFunc,
                                                   const ImageBackingMode imageBackingMode, const uint32_t gridSize,
                                                   const bool fuzzyCompare, const bool useCompute)
    : vkt::TestInstance(context)
    , m_imageBackingMode(imageBackingMode)
    , m_quadGridSize(gridSize == static_cast<uint32_t>(GRID_SIZE_DEFAULTS) ?
                         (isVertexCase ? static_cast<uint32_t>(GRID_SIZE_DEFAULT_VERTEX) :
                                         static_cast<uint32_t>(GRID_SIZE_DEFAULT_FRAGMENT)) :
                         gridSize)
    , m_memAlloc(getAllocator())
    , m_clearColor(DEFAULT_CLEAR_COLOR)
    , m_isVertexCase(isVertexCase)
...
uint32_t ShaderRenderCaseInstance::getUniversalQueueFamilyIndex(void) const
{
    return m_context.getUniversalQueueFamilyIndex();
}

uint32_t ShaderRenderCaseInstance::getComputeQueueFamilyIndex(void) const
{
    return m_context.getComputeQueueFamilyIndex();
}

uint32_t ShaderRenderCaseInstance::getSparseQueueFamilyIndex(void) const
{
...
vk::VkQueue ShaderRenderCaseInstance::getComputeQueue(void) const
{
...
tcu::TestStatus ShaderRenderCaseInstance::iterate(void)
{
    ...
    m_quadGrid = de::MovePtr<QuadGrid>(
        new QuadGrid(m_quadGridSize, width, height, getDefaultConstCoords(), m_userAttribTransforms, m_textures));

    // Render result.
    tcu::Surface resImage(width, height);

    render(m_quadGrid->getNumVertices(), m_quadGrid->getNumTriangles(), m_quadGrid->getIndices(),
           m_quadGrid->getConstCoords());
...
void ShaderRenderCaseInstance::setupUniformData(uint32_t bindingLocation, size_t size, const void *dataPtr)
{
    ...
    de::MovePtr<Allocation> alloc =
        m_memAlloc.allocate(getBufferMemoryRequirements(vk, vkDevice, *buffer), MemoryRequirement::HostVisible);
...
void ShaderRenderCaseInstance::addUniform(uint32_t bindingLocation, vk::VkDescriptorType descriptorType,
...
void ShaderRenderCaseInstance::addAttribute(uint32_t bindingLocation, vk::VkFormat format, uint32_t sizePerElement,
                                            uint32_t count, const void *dataPtr)
{
    ...
    const uint32_t binding                                   = (uint32_t)m_vertexBindingDescription.size();
    const VkVertexInputBindingDescription bindingDescription = {
        binding,                    // uint32_t binding;
        sizePerElement,             // uint32_t stride;
    ...
    const VkVertexInputAttributeDescription attributeDescription = {
        bindingLocation, // uint32_t location;
        binding,         // uint32_t binding;
...
void ShaderRenderCaseInstance::useAttribute(uint32_t bindingLocation, BaseAttributeType type)
{
...
void ShaderRenderCaseInstance::useUniform(uint32_t bindingLocation, BaseUniformType type)
{
#define UNIFORM_CASE(type, value)                                              \
    case type:                                                                 \
        addUniform(bindingLocation, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, value); \
        break
    ...
    default:
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Unknown Uniform type: " << type << tcu::TestLog::EndMessage;
        break;
    }
...
{
    return tcu::UVec2(de::min(m_renderSize.x(), MAX_RENDER_WIDTH), de::min(m_renderSize.y(), MAX_RENDER_HEIGHT));
}
                                           ...
                                           VkImage destImage, bool storageImage)
{
    ...
    // Get copy regions and write buffer data
    {
        ...
        uint8_t *destPtr         = (uint8_t *)bufferAlloc->getHostPtr();
            ...
            for (size_t layerNdx = 0; layerNdx < layerData.size(); layerNdx++)
            {
                ...
                const VkBufferImageCopy layerRegion = {
                    ...
                    (uint32_t)access.getHeight(), // uint32_t bufferImageHeight;
                    {
                        ...
                        (uint32_t)levelNdx, // uint32_t mipLevel;
                        (uint32_t)layerNdx, // uint32_t baseArrayLayer;
                                          ...
                                          VkImage destImage, bool storageImage)
{
    ...
    {
        vk.cmdClearDepthStencilImage(*cmdBuffer, destImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
bool isImageSizeSupported(const VkImageType imageType, const tcu::UVec3 &imageSize,
                          const vk::VkPhysicalDeviceLimits &limits)
{
    switch (imageType)
    {
    case VK_IMAGE_TYPE_1D:
        return (imageSize.x() <= limits.maxImageDimension1D && imageSize.y() == 1 && imageSize.z() == 1);
    case VK_IMAGE_TYPE_2D:
        return (imageSize.x() <= limits.maxImageDimension2D && imageSize.y() <= limits.maxImageDimension2D &&
                imageSize.z() == 1);
    case VK_IMAGE_TYPE_3D:
        return (imageSize.x() <= limits.maxImageDimension3D && imageSize.y() <= limits.maxImageDimension3D &&
...
#ifndef CTS_USES_VULKANSC
void ShaderRenderCaseInstance::uploadSparseImage(const tcu::TextureFormat &texFormat, const TextureData &textureData,
                                                 const tcu::Sampler &refSampler, const uint32_t mipLevels,
                                                 const uint32_t arrayLayers, const VkImage sparseImage,
                                                 const VkImageCreateInfo &imageCreateInfo, const tcu::UVec3 texSize)
{
    const VkDevice vkDevice                           = getDevice();
    const DeviceInterface &vk                         = getDeviceInterface();
    const VkPhysicalDevice physicalDevice             = getPhysicalDevice();
    ...
    allocateAndBindSparseImage(vk, vkDevice, physicalDevice, instance, imageCreateInfo, *imageMemoryBindSemaphore,
                               sparseQueue, m_memAlloc, m_allocations, texFormat, sparseImage);
    ...
    copyBufferToImage(vk, vkDevice, queue, queueFamilyIndex, *buffer, bufferSize, copyRegions,
                      &(*imageMemoryBindSemaphore), aspectMask, mipLevels, arrayLayers, sparseImage);
}
#endif // CTS_USES_VULKANSC
...
void ShaderRenderCaseInstance::useSampler(uint32_t bindingLocation, uint32_t textureId, bool storageImg)
{
    ...
    if (textureType == TextureBinding::TYPE_2D)
    {
        ...
        mipLevels   = isMSTexture ? 1u : (uint32_t)texture.getNumLevels();
        arrayLayers = 1u;
    ...
    else if (textureType == TextureBinding::TYPE_CUBE_MAP)
    {
        ...
        static const tcu::CubeFace cubeFaceMapping[tcu::CUBEFACE_LAST] = {
            tcu::CUBEFACE_POSITIVE_X, tcu::CUBEFACE_NEGATIVE_X, tcu::CUBEFACE_POSITIVE_Y,
            tcu::CUBEFACE_NEGATIVE_Y, tcu::CUBEFACE_POSITIVE_Z, tcu::CUBEFACE_NEGATIVE_Z};
    ...
    else if (textureType == TextureBinding::TYPE_2D_ARRAY)
    {
        ...
        texSize     = tcu::UVec3(texture.getWidth(), texture.getHeight(), 1u);
        mipLevels   = isMSTexture ? 1u : (uint32_t)texture.getNumLevels();
        arrayLayers = (uint32_t)texture.getNumLayers();
        ...
        for (uint32_t level = 0; level < mipLevels; ++level)
        {
            ...
            for (uint32_t layer = 0; layer < arrayLayers; ++layer)
            {
                ...
                tcu::ConstPixelBufferAccess layerData(levelLayers.getFormat(), levelLayers.getWidth(),
                                                      levelLayers.getHeight(), 1,
    ...
    else if (textureType == TextureBinding::TYPE_1D_ARRAY)
    {
        ...
        for (uint32_t level = 0; level < mipLevels; ++level)
        {
            ...
            {
                const uint32_t layerOffset = layerSize * layer;
                tcu::ConstPixelBufferAccess layerData(levelLayers.getFormat(), levelLayers.getWidth(), 1, 1,
                                                      (uint8_t *)levelLayers.getDataPtr() + layerOffset);
                textureData[level].push_back(layerData);
            }
        }
...
void ShaderRenderCaseInstance::setPushConstantRanges(const uint32_t rangeCount,
                                                     const vk::VkPushConstantRange *const pcRanges)
{
    m_pushConstantRanges.clear();
...
void ShaderRenderCaseInstance::createSamplerUniform(uint32_t bindingLocation, TextureBinding::Type textureType,
                                                    ...
                                                    TextureBinding::Parameters textureParams, bool useStorageImg)
{
    ...
    const DeviceInterface &vk       = getDeviceInterface();
    const uint32_t queueFamilyIndex = getUniversalQueueFamilyIndex();
    const uint32_t sparseFamilyIndex =
        (m_imageBackingMode == IMAGE_BACKING_MODE_SPARSE) ? getSparseQueueFamilyIndex() : queueFamilyIndex;
...
#ifndef CTS_USES_VULKANSC
    if (isShadowSampler && m_context.isDeviceFunctionalitySupported("VK_KHR_portability_subset") &&
    ...
    const uint32_t queueIndices[]  = {queueFamilyIndex, sparseFamilyIndex};
    ...
    const VkImageCreateInfo imageParams = {
        ...
        queueIndexCount,          // uint32_t queueFamilyIndexCount;
        queueIndices,             // const uint32_t* pQueueFamilyIndices;
    ...
    case TextureBinding::INIT_UPLOAD_DATA:
    {
        ...
        {
#ifndef CTS_USES_VULKANSC
            uploadSparseImage(texFormat, textureData, refSampler, mipLevels, arrayLayers, *vkTexture, imageParams,
                              texSize);
#endif // CTS_USES_VULKANSC
        }
        else
    ...
    const VkImageViewCreateInfo viewParams  = {
        ...
        *vkTexture,                               // VkImage image;
        ...
        components,                               // VkChannelMapping channels;
        {
            ...
            mipLevels - baseMipLevel, // uint32_t mipLevels;
...
void ShaderRenderCaseInstance::setupDefaultInputs(void)
{
    ...
    */
    ...
    addAttribute(0u, VK_FORMAT_R32G32B32A32_SFLOAT, sizeof(tcu::Vec4), quadGrid.getNumVertices(),
                 quadGrid.getPositions());
    addAttribute(1u, VK_FORMAT_R32G32B32A32_SFLOAT, sizeof(tcu::Vec4), quadGrid.getNumVertices(), quadGrid.getCoords());
    addAttribute(2u, VK_FORMAT_R32G32B32A32_SFLOAT, sizeof(tcu::Vec4), quadGrid.getNumVertices(),
                 quadGrid.getUnitCoords());
    ...
    static const struct
    {
        ...
        int userNdx;
    } userAttributes[] = {{A_IN0, 0}, {A_IN1, 1}, {A_IN2, 2}, {A_IN3, 3}};
    ...
    static const struct
    {
        ...
        int numCols;
        int numRows;
    } matrices[] = {{MAT2, 2, 2},   {MAT2x3, 2, 3}, {MAT2x4, 2, 4}, {MAT3x2, 3, 2}, {MAT3, 3, 3},
                    {MAT3x4, 3, 4}, {MAT4x2, 4, 2}, {MAT4x3, 4, 3}, {MAT4, 4, 4}};
    ...
    for (size_t attrNdx = 0; attrNdx < m_enabledBaseAttributes.size(); attrNdx++)
    {
        for (int userNdx = 0; userNdx < DE_LENGTH_OF_ARRAY(userAttributes); userNdx++)
        {
            ...
            addAttribute(m_enabledBaseAttributes[attrNdx].location, VK_FORMAT_R32G32B32A32_SFLOAT, sizeof(tcu::Vec4),
                         quadGrid.getNumVertices(), quadGrid.getUserAttrib(userNdx));
        }
        ...
        for (int matNdx = 0; matNdx < DE_LENGTH_OF_ARRAY(matrices); matNdx++)
        {
            ...
            {
                addAttribute(m_enabledBaseAttributes[attrNdx].location + colNdx, VK_FORMAT_R32G32B32A32_SFLOAT,
                             (uint32_t)(4 * sizeof(float)), quadGrid.getNumVertices(), quadGrid.getUserAttrib(colNdx));
            }
        }
    }
}

void ShaderRenderCaseInstance::render(uint32_t numVertices, uint32_t numTriangles, const uint16_t *indices,
                                      const tcu::Vec4 &constCoords)
{
    render(numVertices, numTriangles * 3, indices, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, constCoords);
}

void ShaderRenderCaseInstance::render(uint32_t numVertices, uint32_t numIndices, const uint16_t *indices,
                                      VkPrimitiveTopology topology, const tcu::Vec4 &constCoords)
{
    const VkDevice vkDevice         = getDevice();
    const DeviceInterface &vk       = getDeviceInterface();
    const VkQueue queue             = m_useCompute ? getComputeQueue() : getUniversalQueue();
    const uint32_t queueFamilyIndex = m_useCompute ? getComputeQueueFamilyIndex() : getUniversalQueueFamilyIndex();
    ...
    // Create color image
    {
        ...
        imageUsage |= m_useCompute ? VK_IMAGE_USAGE_STORAGE_BIT : VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;

        VkImageFormatProperties properties;

        if ((getInstanceInterface().getPhysicalDeviceImageFormatProperties(
                 getPhysicalDevice(), m_colorFormat, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, imageUsage, 0u,
                 &properties) == VK_ERROR_FORMAT_NOT_SUPPORTED))
        {
            TCU_THROW(NotSupportedError, "Format not supported");
        }
        ...
        const VkImageCreateInfo colorImageParams = {
            ...
            m_sampleCount,                            // uint32_t samples;
            VK_IMAGE_TILING_OPTIMAL,                  // VkImageTiling tiling;
        ...
        colorImageAlloc =
            m_memAlloc.allocate(getImageMemoryRequirements(vk, vkDevice, *colorImage), MemoryRequirement::Any);
    ...
    {
        if (m_useCompute)
            TCU_THROW(NotSupportedError, "Multisampling is not supported with comput-only");
        ...
        // Resolved Image
        {
            ...
            if ((getInstanceInterface().getPhysicalDeviceImageFormatProperties(
                     getPhysicalDevice(), m_colorFormat, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, imageUsage, 0,
                     &properties) == VK_ERROR_FORMAT_NOT_SUPPORTED))
            {
                TCU_THROW(NotSupportedError, "Format not supported");
            }
    ...
    if (!m_useCompute)
    {
        ...
        // Create framebuffer
        {
            const VkImageView attachments[] = {*colorImageView, *resolvedImageView};
        ...
        // Create descriptors
        {
            ...
            for (uint32_t i = 0; i < m_uniformInfos.size(); i++)
            {
                const UniformInfo *uniformInfo = m_uniformInfos[i].get()->get();
                uint32_t location              = uniformInfo->location;
                ...
                if (uniformInfo->type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER)
                {
                    ...
                    m_descriptorSetUpdateBuilder->writeSingle(*descriptorSet,
                         ...
                         uniformInfo->type == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
                {
                    ...
                    m_descriptorSetUpdateBuilder->writeSingle(*descriptorSet,
                                                              DescriptorSetUpdateBuilder::Location::binding(location),
                                                              uniformInfo->type, &samplerInfo->descriptor);
                }
                else
                    DE_FATAL("Impossible");
            }
        ...
        {
            const VkPushConstantRange *const pcRanges =
                m_pushConstantRanges.empty() ? nullptr : &m_pushConstantRanges[0];
        ...
        {
            vertexShaderModule =
                createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get(m_vertexShaderName), 0);
            fragmentShaderModule =
                createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get(m_fragmentShaderName), 0);
        }
        ...
        // Create pipeline
        {
            ...
            const VkPipelineVertexInputStateCreateInfo vertexInputStateParams = {
                ...
                (uint32_t)m_vertexAttributeDescription.size(), // uint32_t attributeCount;
        ...
        if (numIndices != 0)
        {
            ...
            VK_CHECK(vk.bindBufferMemory(vkDevice, *indexBuffer, indexBufferAlloc->getMemory(),
    ...
    else // Compute pipeline primitives
    {
        // Create descriptors
        {
            setupUniforms(constCoords);

            // Add storage image binding for compute output
            m_descriptorSetLayoutBuilder->addSingleBinding(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                                                           VK_SHADER_STAGE_COMPUTE_BIT);
            m_descriptorPoolBuilder->addType(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);

            descriptorSetLayout = m_descriptorSetLayoutBuilder->build(vk, vkDevice);

            // For compute we always have at least the storage image, so pool is always required
            descriptorPool =
                m_descriptorPoolBuilder->build(vk, vkDevice, VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT, 1u);
            const VkDescriptorSetAllocateInfo allocInfo = {
                VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO,
            ...
            // Write storage image descriptor (output image)
            ...
            m_descriptorSetUpdateBuilder->writeSingle(
                *descriptorSet, DescriptorSetUpdateBuilder::Location::binding((uint32_t)m_uniformInfos.size()),
        ...
        {
            computeShaderModule =
                createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get(m_computeShaderName), 0);
        }
        ...
        // Create pipeline
        {
            ...
            computePipeline = createComputePipeline(vk, vkDevice, VK_NULL_HANDLE, &pipelineCreateInfo);
        }
    }
    ...
    // Create command buffer
    {
...

        {
            ...
            if (!m_useCompute && isMultiSampling())
            {
        ...
        else // Dispatch
        {
            ...
            // Dispatch compute groups. We use local_size_x = 8, local_size_y = 8 in the shader.
            const uint32_t dispatchX = (m_renderSize.x() + 7) / 8;
            const uint32_t dispatchY = (m_renderSize.y() + 7) / 8;

            vk.cmdDispatch(*cmdBuffer, dispatchX, dispatchY, 1);
        }
        endCommandBuffer(vk, *cmdBuffer);
    }
    ...
    // Read back the result
    {
        ...
        const de::UniquePtr<Allocation> readImageBufferMemory(m_memAlloc.allocate(
            getBufferMemoryRequirements(vk, vkDevice, *readImageBuffer), MemoryRequirement::HostVisible));

        VK_CHECK(vk.bindBufferMemory(vkDevice, *readImageBuffer, readImageBufferMemory->getMemory(),
                                     readImageBufferMemory->getOffset()));

        // Copy image to buffer
        const Move<VkCommandBuffer> resultCmdBuffer =
            allocateCommandBuffer(vk, vkDevice, activeCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);

        beginCommandBuffer(vk, *resultCmdBuffer);

        copyImageToBuffer(vk, *resultCmdBuffer, isMultiSampling() ? *resolvedImage : *colorImage, *readImageBuffer,
                          tcu::IVec2(m_renderSize.x(), m_renderSize.y()), access, layout);
        ...
        const tcu::ConstPixelBufferAccess resultAccess(resultFormat, m_renderSize.x(), m_renderSize.y(), 1,
...
bool ShaderRenderCaseInstance::compareImages(const tcu::Surface &resImage, const tcu::Surface &refImage,
