...
{
    static const char *s_names[INDEXACCESS_LAST] = {"static", "dynamic", "static_loop", "dynamic_loop", "const"};
...
{
public:
    IndexingTestUniformSetup(const DataType varType, bool usesArray) : m_varType(varType), m_usesArray(usesArray)
    {
    }
...
{
public:
    ShaderIndexingCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       bool isVertexCase, const ShaderEvalFunc evalFunc, const std::string &vertShaderSource,
                       const std::string &fragShaderSource, const DataType varType, const bool usesArray);
...
ShaderIndexingCase::ShaderIndexingCase(tcu::TestContext &testCtx, const std::string &name,
                                       const std::string &description, const bool isVertexCase,
    ...
    : ShaderRenderCase(testCtx, name, description, isVertexCase, evalFunc,
                       new IndexingTestUniformSetup(varType, usesArray), DE_NULL)
{
...
static de::MovePtr<ShaderIndexingCase> createVaryingArrayCase(tcu::TestContext &context, const std::string &caseName,
                                                              const std::string &description, DataType varType,
                                                              IndexAccessType vertAccess, IndexAccessType fragAccess)
{
    ...
    return de::MovePtr<ShaderIndexingCase>(new ShaderIndexingCase(
        context, caseName, description, true, evalFunc, vertexShaderSource, fragmentShaderSource, varType, false));
}

static de::MovePtr<ShaderIndexingCase> createUniformArrayCase(tcu::TestContext &context, const std::string &caseName,
                                                              const std::string &description, bool isVertexCase,
                                                              DataType varType, IndexAccessType readAccess)
{
    std::ostringstream vtx;
    std::ostringstream frag;
    ...
    return de::MovePtr<ShaderIndexingCase>(new ShaderIndexingCase(context, caseName, description, isVertexCase,
                                                                  evalFunc, vertexShaderSource, fragmentShaderSource,
                                                                  varType, true));
}

static de::MovePtr<ShaderIndexingCase> createTmpArrayCase(tcu::TestContext &context, const std::string &caseName,
                                                          const std::string &description, bool isVertexCase,
                                                          DataType varType, IndexAccessType writeAccess,
...
static de::MovePtr<ShaderIndexingCase> createVectorSubscriptCase(tcu::TestContext &context, const std::string &caseName,
                                                                 const std::string &description, bool isVertexCase,
                                                                 DataType varType, VectorAccessType writeAccess,
                                                                 VectorAccessType readAccess)
{
    ...
    if (writeAccess == SUBSCRIPT_DYNAMIC_LOOP || readAccess == SUBSCRIPT_DYNAMIC_LOOP)
        op << "layout(std140, binding = " << vecLen << ") uniform something" << vecLen << " { mediump int "
           << vecLenName << "; };\n";
...
static de::MovePtr<ShaderIndexingCase> createMatrixSubscriptCase(tcu::TestContext &context, const std::string &caseName,
                                                                 const std::string &description, bool isVertexCase,
                                                                 DataType varType, IndexAccessType writeAccess,
                                                                 IndexAccessType readAccess)
{
    ...
    if (writeAccess == INDEXACCESS_DYNAMIC_LOOP || readAccess == INDEXACCESS_DYNAMIC_LOOP)
        op << "layout(std140, binding = " << numCols << ") uniform something" << numCols << " { mediump int "
           << matSizeName << "; };\n";
...
void ShaderIndexingTests::init(void)
{
    ...
    {
        de::MovePtr<TestCaseGroup> varyingGroup(
            new TestCaseGroup(m_testCtx, "varying_array", "Varying array access tests."));
        ...
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(s_floatAndVecTypes); typeNdx++)
        {
            ...
            for (int vertAccess = 0; vertAccess < INDEXACCESS_CONST; vertAccess++)
            {
                for (int fragAccess = 0; fragAccess < INDEXACCESS_CONST; fragAccess++)
                {
                    ...
                    de::MovePtr<ShaderIndexingCase> testCase(createVaryingArrayCase(
                        m_testCtx, name, desc, varType, (IndexAccessType)vertAccess, (IndexAccessType)fragAccess));
    ...
    {
        de::MovePtr<TestCaseGroup> uniformGroup(
            new TestCaseGroup(m_testCtx, "uniform_array", "Uniform array access tests."));
        ...
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(s_floatAndVecTypes); typeNdx++)
        {
            ...
            for (int readAccess = 0; readAccess < INDEXACCESS_CONST; readAccess++)
            {
                ...
                for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                {
                    ...
                    de::MovePtr<ShaderIndexingCase> testCase(createUniformArrayCase(
                        m_testCtx, name, desc, isVertexCase, varType, (IndexAccessType)readAccess));
            ...
            for (int writeAccess = 0; writeAccess < INDEXACCESS_LAST; writeAccess++)
            {
                for (int readAccess = 0; readAccess < INDEXACCESS_CONST; readAccess++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        bool isVertexCase = ((ShaderType)shaderType == SHADERTYPE_VERTEX);
                        de::MovePtr<ShaderIndexingCase> testCase(
                            createTmpArrayCase(m_testCtx, name, desc, isVertexCase, varType,
    ...
    {
        de::MovePtr<TestCaseGroup> vecGroup(
            new TestCaseGroup(m_testCtx, "vector_subscript", "Vector subscript indexing."));
            ...
            for (int writeAccess = 0; writeAccess < VECTORACCESS_LAST; writeAccess++)
            {
                for (int readAccess = 0; readAccess < VECTORACCESS_LAST; readAccess++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        bool isVertexCase = ((ShaderType)shaderType == SHADERTYPE_VERTEX);
                        de::MovePtr<ShaderIndexingCase> testCase(
                            createVectorSubscriptCase(m_testCtx, name.c_str(), desc.c_str(), isVertexCase, varType,
    ...
    {
        de::MovePtr<TestCaseGroup> matGroup(
            new TestCaseGroup(m_testCtx, "matrix_subscript", "Matrix subscript indexing."));
            ...
            for (int writeAccess = 0; writeAccess < INDEXACCESS_CONST; writeAccess++)
            {
                for (int readAccess = 0; readAccess < INDEXACCESS_CONST; readAccess++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        bool isVertexCase = ((ShaderType)shaderType == SHADERTYPE_VERTEX);
                        de::MovePtr<ShaderIndexingCase> testCase(
                            createMatrixSubscriptCase(m_testCtx, name.c_str(), desc.c_str(), isVertexCase, varType,
                                                      (IndexAccessType)writeAccess, (IndexAccessType)readAccess));
