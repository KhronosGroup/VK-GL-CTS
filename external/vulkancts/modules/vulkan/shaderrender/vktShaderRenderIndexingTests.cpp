...
{
public:
    ShaderIndexingCase(tcu::TestContext &testCtx, const std::string &name, bool isVertexCase,
...
ShaderIndexingCase::ShaderIndexingCase(tcu::TestContext &testCtx, const std::string &name, const bool isVertexCase,
                                       const ShaderEvalFunc evalFunc, const std::string &vertShaderSource,
                                                              ...
                                                              IndexAccessType fragAccess)
{
    ...
    return de::MovePtr<ShaderIndexingCase>(new ShaderIndexingCase(context, caseName, true, evalFunc, vertexShaderSource,
                                                                  fragmentShaderSource, varType, false));
}

static de::MovePtr<ShaderIndexingCase> createUniformArrayCase(tcu::TestContext &context, const std::string &caseName,
                                                              bool isVertexCase, DataType varType,
                                                              IndexAccessType readAccess)
{
    ...
    return de::MovePtr<ShaderIndexingCase>(new ShaderIndexingCase(
        context, caseName, isVertexCase, evalFunc, vertexShaderSource, fragmentShaderSource, varType, true));
}
                                                          ...
                                                          IndexAccessType writeAccess, IndexAccessType readAccess)
{
    ...
    return de::MovePtr<ShaderIndexingCase>(new ShaderIndexingCase(
        context, caseName, isVertexCase, evalFunc, vertexShaderSource, fragmentShaderSource, varType, false));
}
...
ShaderIndexingTests::ShaderIndexingTests(tcu::TestContext &context) : TestCaseGroup(context, "indexing")
{
}
            ...
            for (int vertAccess = 0; vertAccess < INDEXACCESS_CONST; vertAccess++)
            {
                for (int fragAccess = 0; fragAccess < INDEXACCESS_CONST; fragAccess++)
                {
                    ...
                    de::MovePtr<ShaderIndexingCase> testCase(createVaryingArrayCase(
                        m_testCtx, name, varType, (IndexAccessType)vertAccess, (IndexAccessType)fragAccess));
            ...
            for (int readAccess = 0; readAccess < INDEXACCESS_CONST; readAccess++)
            {
                ...
                for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                {
                    ...
                    de::MovePtr<ShaderIndexingCase> testCase(
                        createUniformArrayCase(m_testCtx, name, isVertexCase, varType, (IndexAccessType)readAccess));
            ...
            for (int writeAccess = 0; writeAccess < INDEXACCESS_LAST; writeAccess++)
            {
                for (int readAccess = 0; readAccess < INDEXACCESS_CONST; readAccess++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        bool isVertexCase = ((ShaderType)shaderType == SHADERTYPE_VERTEX);
                        de::MovePtr<ShaderIndexingCase> testCase(
                            createTmpArrayCase(m_testCtx, name, isVertexCase, varType, (IndexAccessType)writeAccess,
                                               (IndexAccessType)readAccess));
            ...
            for (int writeAccess = 0; writeAccess < VECTORACCESS_LAST; writeAccess++)
            {
                for (int readAccess = 0; readAccess < VECTORACCESS_LAST; readAccess++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        bool isVertexCase = ((ShaderType)shaderType == SHADERTYPE_VERTEX);
                        de::MovePtr<ShaderIndexingCase> testCase(
                            createVectorSubscriptCase(m_testCtx, name.c_str(), isVertexCase, varType,
            ...
            for (int writeAccess = 0; writeAccess < INDEXACCESS_CONST; writeAccess++)
            {
                for (int readAccess = 0; readAccess < INDEXACCESS_CONST; readAccess++)
                {
                    ...
                    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(s_shaderTypes); shaderTypeNdx++)
                    {
                        ...
                        bool isVertexCase = ((ShaderType)shaderType == SHADERTYPE_VERTEX);
                        de::MovePtr<ShaderIndexingCase> testCase(
                            createMatrixSubscriptCase(m_testCtx, name.c_str(), isVertexCase, varType,
                                                      (IndexAccessType)writeAccess, (IndexAccessType)readAccess));
