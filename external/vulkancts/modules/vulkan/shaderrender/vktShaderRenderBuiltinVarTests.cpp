...
{
public:
    BuiltinFragCoordMsaaTestCase(TestContext &testCtx, const char *name, VkSampleCountFlagBits sampleCount,
                                 bool sampleShading, std::vector<uint32_t> sampleMaskArray, bool useCentroid,
                                 bool useEnable);
...
BuiltinFragCoordMsaaTestCase::BuiltinFragCoordMsaaTestCase(TestContext &testCtx, const char *name,
                                                           ...
                                                           std::vector<uint32_t> sampleMaskArray, bool useCentroid,
                                                           bool useEnable)
    : TestCase(testCtx, name)
    , m_sampleCount(sampleCount)
    , m_sampleShading(sampleShading)
...
{
public:
    BuiltinFragDepthCase(TestContext &testCtx, const char *name, VkPrimitiveTopology topology, VkFormat format,
                         bool largeDepthEnable, bool depthClampEnable, const VkSampleCountFlagBits samples);
...
BuiltinFragDepthCase::BuiltinFragDepthCase(TestContext &testCtx, const char *name, VkPrimitiveTopology topology,
...
BuiltinGlFragCoordXYZCase::BuiltinGlFragCoordXYZCase(TestContext &testCtx, const string &name) : TestCase(testCtx, name)
{
}
...
BuiltinGlFragCoordWCase::BuiltinGlFragCoordWCase(TestContext &testCtx, const string &name) : TestCase(testCtx, name)
{
}
...
BuiltinInputVariationsCase::BuiltinInputVariationsCase(TestContext &testCtx, const string &name,
                                                       ShaderInputTypes shaderInputTypes)
    : TestCase(testCtx, name)
    , m_shaderInputTypes(shaderInputTypes)
{
}
...
TestCaseGroup *createBuiltinVarTests(TestContext &testCtx)
{
    ...
    simpleGroup->addChild(
        new BuiltinGlPointCoordCase(testCtx, "pointcoord_uniform_frag", POINTCOORD_VARIANT_UNIFORM_FRAGMENT));
    // PointCoord test with vertex uniform
    simpleGroup->addChild(
        new BuiltinGlPointCoordCase(testCtx, "pointcoord_uniform_vert", POINTCOORD_VARIANT_UNIFORM_VERTEX));
    ...
    // FragCoord_msaa
    {
        static const struct FragCoordMsaaCaseList
        {
            ...
            {"1_bit", VK_SAMPLE_COUNT_1_BIT},   {"2_bit", VK_SAMPLE_COUNT_2_BIT},   {"4_bit", VK_SAMPLE_COUNT_4_BIT},
            {"8_bit", VK_SAMPLE_COUNT_8_BIT},   {"16_bit", VK_SAMPLE_COUNT_16_BIT}, {"32_bit", VK_SAMPLE_COUNT_32_BIT},
        ...
        {
            fragCoordMsaaGroup->addChild(new BuiltinFragCoordMsaaTestCase(testCtx, fragCoordMsaaCaseList[caseNdx].name,
                                                                          fragCoordMsaaCaseList[caseNdx].sampleCount,
                                                                          true, sampleMaskArray, false, true));
            fragCoordMsaaInputGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                testCtx, fragCoordMsaaCaseList[caseNdx].name, fragCoordMsaaCaseList[caseNdx].sampleCount, true,
        ...
        for (uint32_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(fragCoordMsaaCaseList); caseNdx++)
        {
            fragCoordMsaaInputGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                ...
                fragCoordMsaaCaseList[caseNdx].sampleCount, false, sampleMaskArray, false, false));
        }
        ...
        for (uint32_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(fragCoordMsaaCaseList); caseNdx++)
        {
            fragCoordMsaaInputGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                ...
                (std::string(fragCoordMsaaCaseList[caseNdx].name) + "_no_sample_shading_centroid_interpolation")
                    .c_str(),
                fragCoordMsaaCaseList[caseNdx].sampleCount, false, sampleMaskArray, true, false));
        }
    }
        ...
        for (uint32_t primNdx = 0; primNdx < DE_LENGTH_OF_ARRAY(primitiveTopologyTable); primNdx++)
        {
            for (uint32_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(testCaseTable); caseNdx++)
                fragDepthGroup->addChild(new BuiltinFragDepthCase(
                    ...
                    primitiveTopologyTable[primNdx].prim, testCaseTable[caseNdx].format,
                    testCaseTable[caseNdx].largeDepthEnable, testCaseTable[caseNdx].depthClampEnable,
                    testCaseTable[caseNdx].samples));
        }
    }
    ...
    {
        inputVariationsGroup->addChild(
            new BuiltinInputVariationsCase(testCtx, shaderInputTypeToString(shaderType), shaderType));
    }
