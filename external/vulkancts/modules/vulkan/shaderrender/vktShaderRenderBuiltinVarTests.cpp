...
{
public:
    FrontFacingVertexShader(void) : rr::VertexShader(1, 0)
    {
...
TestStatus BuiltinGlFrontFacingCaseInstance::iterate(void)
{
    ...
    log << TestLog::Image("reference", "reference",
                          tcu::ConstPixelBufferAccess(tcu::TextureFormat(refDrawContext.getColorPixels().getFormat()),
                                                      refDrawContext.getColorPixels().getWidth(),
                                                      refDrawContext.getColorPixels().getHeight(), 1,
                                                      refDrawContext.getColorPixels().getDataPtr()));

    log << TestLog::Image("result", "result",
                          tcu::ConstPixelBufferAccess(tcu::TextureFormat(dc.getColorPixels().getFormat()),
                                                      dc.getColorPixels().getWidth(), dc.getColorPixels().getHeight(),
                                                      1, dc.getColorPixels().getDataPtr()));

    if (tcu::intThresholdPositionDeviationCompare(m_context.getTestContext().getLog(), "ComparisonResult",
                                                  "Image comparison result", refDrawContext.getColorPixels(),
                                                  dc.getColorPixels(), UVec4(0u), IVec3(1, 1, 0), false,
...
public:
    ...
    BuiltinFragDepthCaseInstance(Context &context, VkPrimitiveTopology topology, VkFormat format, bool largeDepthEnable,
                                 float defaultDepth, bool depthClampEnable, const VkSampleCountFlagBits samples);
    TestStatus iterate(void);

    bool validateDepthBuffer(const tcu::ConstPixelBufferAccess &validationBuffer,
                             const tcu::ConstPixelBufferAccess &markerBuffer, const float tolerance) const;
...
BuiltinFragDepthCaseInstance::BuiltinFragDepthCaseInstance(Context &context, VkPrimitiveTopology topology,
                                                           VkFormat format, bool largeDepthEnable, float defaultDepth,
                                                           bool depthClampEnable, const VkSampleCountFlagBits samples)
    ...
    , m_largeDepthBase(20.0f)
{
    ...
    try
    {
        ...
        imageFormatProperties = getPhysicalDeviceImageFormatProperties(
            vki, physicalDevice, m_format, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL,
            VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, (VkImageCreateFlags)0);
        ...
        if (m_largeDepthEnable && !de::contains(context.getDeviceExtensions().begin(),
                                                context.getDeviceExtensions().end(), "VK_EXT_depth_range_unrestricted"))
            throw tcu::NotSupportedError(
                "large_depth test variants require the VK_EXT_depth_range_unrestricted extension");
...
TestStatus BuiltinFragDepthCaseInstance::iterate(void)
{
    ...
    TestLog &log                    = m_context.getTestContext().getLog();
    const uint32_t scale            = 4; // To account for std140 stride
    ...
    // Create Buffer/Image for validation
    {
        ...
        validationAlloc  = allocator.allocate(getBufferMemoryRequirements(vk, device, *validationBuffer),
                                              MemoryRequirement::HostVisible);
        VK_CHECK(
            vk.bindBufferMemory(device, *validationBuffer, validationAlloc->getMemory(), validationAlloc->getOffset()));
        ...
        VK_CHECK(vk.bindImageMemory(device, *depthResolveImage, depthResolveAllocation->getMemory(),
                                    depthResolveAllocation->getOffset()));
    ...
    // Marker Buffer
    {
        const VkDeviceSize size =
            m_samples * m_renderSize.x() * m_renderSize.y() * getPixelSize(mapVkFormat(VK_FORMAT_R8G8B8A8_UINT));
        ...
        VK_CHECK(vk.bindBufferMemory(device, *markerBuffer, markerBufferAllocation->getMemory(),
                                     markerBufferAllocation->getOffset()));
        ...
        VK_CHECK(vk.bindImageMemory(device, *markerImage, markerImageAllocation->getMemory(),
                                    markerImageAllocation->getOffset()));
    ...
    // Depth Buffer
    {
        ...
        VK_CHECK(vk.bindImageMemory(device, *depthImage, depthImageAllocation->getMemory(),
                                    depthImageAllocation->getOffset()));
    ...
    // Initialize Marker Buffer
    {
        ...
        vk.cmdClearColorImage(*transferCmdBuffer, *markerImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    // Perform Draw
    {
        ...
        shaders.push_back(
            VulkanShader(VK_SHADER_STAGE_VERTEX_BIT, m_context.getBinaryCollection().get("FragDepthVert")));
        shaders.push_back(
            VulkanShader(VK_SHADER_STAGE_FRAGMENT_BIT, m_context.getBinaryCollection().get("FragDepthFrag")));
        ...
        log << TestLog::Image(
            "resultColor", "Result Color Buffer",
            tcu::ConstPixelBufferAccess(tcu::TextureFormat(vulkanDrawContext.getColorPixels().getFormat()),
    ...
    // Resolve Depth Buffer
    {
        ...
        shaders.push_back(
            VulkanShader(VK_SHADER_STAGE_VERTEX_BIT, m_context.getBinaryCollection().get("FragDepthVertPass2")));
        shaders.push_back(
            VulkanShader(VK_SHADER_STAGE_FRAGMENT_BIT, m_context.getBinaryCollection().get("FragDepthFragPass2")));
    ...
    {
        beginCommandBuffer(vk, *transferCmdBuffer);
        copyImageToBuffer(vk, *transferCmdBuffer, *markerImage, *markerBuffer,
                          tcu::IVec2(m_renderSize.x() * m_samples, m_renderSize.y()), VK_ACCESS_SHADER_WRITE_BIT,
                          VK_IMAGE_LAYOUT_GENERAL);
        endCommandBuffer(vk, *transferCmdBuffer);
    ...
    // Verify depth buffer
    {
        ...
        copyImageToBuffer(vk, *transferCmdBuffer, *depthResolveImage, *validationBuffer,
                          tcu::IVec2(m_renderSize.x() * m_samples, m_renderSize.y()), VK_ACCESS_SHADER_WRITE_BIT,
                          VK_IMAGE_LAYOUT_GENERAL);
        endCommandBuffer(vk, *transferCmdBuffer);
        ...
        invalidateMappedMemoryRange(vk, device, validationAlloc->getMemory(), validationAlloc->getOffset(),
                                    VK_WHOLE_SIZE);
        invalidateMappedMemoryRange(vk, device, markerBufferAllocation->getMemory(),
                                    markerBufferAllocation->getOffset(), VK_WHOLE_SIZE);

        tcu::ConstPixelBufferAccess resultPixelBuffer(mapVkFormat(VK_FORMAT_R32_SFLOAT), m_renderSize.x() * m_samples,
                                                      m_renderSize.y(), 1u, validationAlloc->getHostPtr());
        tcu::ConstPixelBufferAccess markerPixelBuffer(mapVkFormat(VK_FORMAT_R8G8B8A8_UINT),
                                                      m_renderSize.x() * m_samples, m_renderSize.y(), 1u,
                                                      markerBufferAllocation->getHostPtr());
...
bool BuiltinFragDepthCaseInstance::validateDepthBuffer(const tcu::ConstPixelBufferAccess &validationBuffer,
                                                       const tcu::ConstPixelBufferAccess &markerBuffer,
                                                       const float tolerance) const
{
    TestLog &log = m_context.getTestContext().getLog();

    for (uint32_t rowNdx = 0; rowNdx < m_renderSize.y(); rowNdx++)
    {
        for (uint32_t colNdx = 0; colNdx < m_renderSize.x(); colNdx++)
        {
            ...
            for (uint32_t sampleNdx = 0; sampleNdx < (uint32_t)m_samples; sampleNdx++)
            {
                ...
                if (markerValue != 0)
                {
                    ...
                    {
                        log << TestLog::Message << "Mismatch at pixel (" << colNdx << "," << rowNdx << "," << sampleNdx
                            << "): expected " << expectedValue << " but got " << actualValue << TestLog::EndMessage;
                        return false;
                    }
                }
                else
...
public:
    ...
    BuiltinFragCoordMsaaCaseInstance(Context &context, VkSampleCountFlagBits sampleCount, bool sampleShading,
                                     std::vector<uint32_t> sampleMaskArray, bool useEnable);
    TestStatus iterate(void);
    ...
    , m_useEnable(useEnable)
{
    ...
    try
    {
        ...
        imageFormatProperties = getPhysicalDeviceImageFormatProperties(
            vki, physicalDevice, VK_FORMAT_R32G32B32A32_SFLOAT, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL,
            VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, (VkImageCreateFlags)0);
...
TestStatus BuiltinFragCoordMsaaCaseInstance::iterate(void)
{
    ...
    // Validation buffer
    {
        ...
        vk.bindBufferMemory(device, *sampleLocationBuffer, sampleLocationBufferAllocation->getMemory(),
                            sampleLocationBufferAllocation->getOffset());
    }
    ...
    // Perform draw
    {
        ...
        shaders.push_back(
            VulkanShader(VK_SHADER_STAGE_VERTEX_BIT, m_context.getBinaryCollection().get("FragCoordMsaaVert")));
        shaders.push_back(
            VulkanShader(VK_SHADER_STAGE_FRAGMENT_BIT, m_context.getBinaryCollection().get("FragCoordMsaaFrag")));
        ...
        pipelineState.sampleShadingEnable =
            m_useEnable; // When m_useEnable is false, we rely on the gl_SampleID input to enable sample shading
    ...
    {
        beginCommandBuffer(vk, *transferCmdBuffer);
        copyImageToBuffer(vk, *transferCmdBuffer, *outputImage, *sampleLocationBuffer,
                          tcu::IVec2(m_renderSize.x() * m_sampleCount, m_renderSize.y()), VK_ACCESS_SHADER_WRITE_BIT,
                          VK_IMAGE_LAYOUT_GENERAL);
        endCommandBuffer(vk, *transferCmdBuffer);
    ...
    // Validate result
    {
        ...
        ConstPixelBufferAccess sampleLocationPixelBuffer(mapVkFormat(VK_FORMAT_R32G32B32A32_SFLOAT),
                                                         m_sampleCount * m_renderSize.x(), m_renderSize.y(), 1u,
                                                         sampleLocationBufferAllocation->getHostPtr());
...
bool BuiltinFragCoordMsaaCaseInstance::validateSampleLocations(const ConstPixelBufferAccess &sampleLocationBuffer) const
{
    ...
    TestLog &log                          = m_context.getTestContext().getLog();
    const VkPhysicalDevice physicalDevice = m_context.getPhysicalDevice();
    uint32_t logSampleCount               = deLog2Floor32(m_sampleCount);
    VkPhysicalDeviceProperties physicalDeviceProperties;

    static const Vec2 sampleCount1Bit[] = {Vec2(0.5f, 0.5f)};

    static const Vec2 sampleCount2Bit[] = {Vec2(0.25f, 0.25f), Vec2(0.75f, 0.75f)};

    static const Vec2 sampleCount4Bit[] = {Vec2(0.375f, 0.125f), Vec2(0.875f, 0.375f), Vec2(0.125f, 0.625f),
                                           Vec2(0.625f, 0.875f)};

    static const Vec2 sampleCount8Bit[] = {Vec2(0.5625f, 0.3125f), Vec2(0.4375f, 0.6875f), Vec2(0.8125f, 0.5625f),
                                           Vec2(0.3125f, 0.1875f), Vec2(0.1875f, 0.8125f), Vec2(0.0625f, 0.4375f),
                                           Vec2(0.6875f, 0.9375f), Vec2(0.9375f, 0.0625f)};
    ...
    static const Vec2 *standardSampleLocationTable[] = {sampleCount1Bit, sampleCount2Bit, sampleCount4Bit,
                                                        sampleCount8Bit, sampleCount16Bit};

    vki.getPhysicalDeviceProperties(physicalDevice, &physicalDeviceProperties);

    for (int32_t rowNdx = 0; rowNdx < (int32_t)m_renderSize.y(); rowNdx++)
    {
        for (int32_t colNdx = 0; colNdx < (int32_t)m_renderSize.x(); colNdx++)
        {
            ...
            if (m_sampleShading == true)
            {
                ...
                for (uint32_t sampleNdx = 0; sampleNdx < (uint32_t)m_sampleCount; sampleNdx++)
                {
                    ...
                    if (pixelData.z() != 0.0f)
                    {
                        log << TestLog::Message << "Pixel (" << colNdx << "," << rowNdx
                            << "): has unexpected .z component, expected: 0.0, got: " << pixelData.z()
                            << TestLog::EndMessage;
                        return false;
                    }
                    ...
                    {
                        log << TestLog::Message << "Pixel (" << colNdx << "," << rowNdx
                            << "): has unexpected .w component, expected: 1.0, got: " << pixelData.w()
                            << TestLog::EndMessage;
                        return false;
                    }
                ...
                for (std::vector<Vec2>::const_iterator sampleIt = locations.begin(); sampleIt != locations.end();
                     sampleIt++)
                {
                    ...
                    if ((sampleFloor.x() < colNdx) || (sampleCeil.x() > colNdx + 1) || (sampleFloor.y() < rowNdx) ||
                        (sampleCeil.y() > rowNdx + 1))
                    {
                        log << TestLog::Message << "Pixel (" << colNdx << "," << rowNdx << "): " << *sampleIt
                            << TestLog::EndMessage;
                        return false;
                    }
                }
                ...
                {
                    log << TestLog::Message << "Fail: Sample locations contains non-unique entry"
                        << TestLog::EndMessage;
                    return false;
                }
                ...
                if (logSampleCount < DE_LENGTH_OF_ARRAY(standardSampleLocationTable))
                {
                    if (physicalDeviceProperties.limits.standardSampleLocations)
                    {
                        for (uint32_t sampleNdx = 0; sampleNdx < (uint32_t)m_sampleCount; sampleNdx++)
                        {
                            if (!de::contains(locations.begin(), locations.end(),
                                              standardSampleLocationTable[logSampleCount][sampleNdx] +
                                                  Vec2(float(colNdx), float(rowNdx))))
                            {
                                log << TestLog::Message << "Didn't match sample locations "
                                    << standardSampleLocationTable[logSampleCount][sampleNdx] << TestLog::EndMessage;
                                return false;
                            }
                        }
                    }
            ...
            else
            {
                ...
                {
                    log << TestLog::Message << "Didn't match sample locations (" << pixelData.x() << ", "
                        << pixelData.y() << "): " << Vec2(float(colNdx) + 0.5f, float(rowNdx) + 0.5f)
                        << TestLog::EndMessage;
                    return false;
                }
            }
...
{
public:
    BuiltinFragCoordMsaaTestCase(TestContext &testCtx, const char *name, const char *description,
                                 VkSampleCountFlagBits sampleCount, bool sampleShading,
                                 std::vector<uint32_t> sampleMaskArray, bool useCentroid, bool useEnable);
...
BuiltinFragCoordMsaaTestCase::BuiltinFragCoordMsaaTestCase(TestContext &testCtx, const char *name,
                                                           const char *description, VkSampleCountFlagBits sampleCount,
                                                           bool sampleShading, std::vector<uint32_t> sampleMaskArray,
                                                           bool useCentroid, bool useEnable)
    : TestCase(testCtx, name, description)
    , m_sampleCount(sampleCount)
    , m_sampleShading(sampleShading)
    , m_sampleMaskArray(sampleMaskArray)
...
void BuiltinFragCoordMsaaTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    if (m_sampleShading == true)
    {
        ...
        fragmentSource << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                       ...
                       << "    ivec2 imageCoord = ivec2(sampleNdx + int(gl_FragCoord.x) * " << m_sampleCount
                       << ", int(gl_FragCoord.y));\n"
...
{
    return new BuiltinFragCoordMsaaCaseInstance(context, m_sampleCount, m_sampleShading, m_sampleMaskArray,
                                                m_useEnable);
}
...
{
public:
    BuiltinFragDepthCase(TestContext &testCtx, const char *name, const char *description, VkPrimitiveTopology topology,
                         VkFormat format, bool largeDepthEnable, bool depthClampEnable,
...
BuiltinFragDepthCase::BuiltinFragDepthCase(TestContext &testCtx, const char *name, const char *description,
                                           VkPrimitiveTopology topology, VkFormat format, bool largeDepthEnable,
                                           bool depthClampEnable, const VkSampleCountFlagBits samples)
    : TestCase(testCtx, name, description)
    , m_topology(topology)
    , m_format(format)
...
// Fragment
{
    // Pass 1
    {
        ...
        fragmentSource << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                       ...
                       << "        discard;\n"
                       << "    highp int index =int(gl_FragCoord.y) * " << BuiltinFragDepthCaseInstance::RENDERHEIGHT
                       << " + int(gl_FragCoord.x);\n"
                       << "    controlDepthValue = control_buffer.data[index];\n"
                       << "    gl_FragDepth = controlDepthValue;\n"
                       << "    const int sampleNdx = int(gl_SampleID);\n"
                       << "    ivec2 imageCoord = ivec2(sampleNdx + int(gl_FragCoord.x) * " << m_samples
                       << ", int(gl_FragCoord.y));\n"
    ...
    // Pass 2
    {
        ...
        fragmentSource
            ...
            << "    const int numSamples = " << m_samples << ";\n"
            << "    const int sampleNdx = int(gl_SampleID);\n"
            << "    ivec2 renderSize = ivec2(" << BuiltinFragDepthCaseInstance::RENDERWIDTH << ","
            << BuiltinFragDepthCaseInstance::RENDERHEIGHT << ");\n"
            ...
            << "    imageStore(u_outImage, ivec2(sampleNdx + int(texCoord.x * renderSize.x) * numSamples, "
               "int(texCoord.y * renderSize.y)), depthVal);\n"
            << "    color = vec4(1.0, 0.0, 0.0, 1.0);\n"
            << "}\n";
...
{
    return new BuiltinFragDepthCaseInstance(context, m_topology, m_format, m_largeDepthEnable, m_defaultDepth,
                                            m_depthClampEnable, m_samples);
}
...
TestStatus BuiltinGlFragCoordXYZCaseInstance::iterate(void)
{
    ...
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            if (de::abs(color.x() - resultColor.x()) > precision || de::abs(color.y() - resultColor.y()) > precision ||
...
void BuiltinGlFragCoordXYZCaseInstance::setupDefaultInputs(void)
{
    const float vertices[] = {
        -1.0f, 1.0f, 0.0f, 1.0f, -1.0f, -1.0f, 0.5f, 1.0f, 1.0f, 1.0f, 0.5f, 1.0f, 1.0f, -1.0f, 1.0f, 1.0f,
    };

    addAttribute(0u, VK_FORMAT_R32G32B32A32_SFLOAT, uint32_t(sizeof(float) * 4), 4, vertices);
}
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            const float oow =
                ((xf + yf) < 1.0f) ?
                    ...
                    projectedTriInterpolate(Vec3(m_w[3], m_w[2], m_w[1]), Vec3(m_w[3], m_w[2], m_w[1]), 1.0f - xf,
                                            1.0f - yf);
...
{
    const float vertices[] = {-m_w[0], m_w[0], 0.0f, m_w[0], -m_w[1], -m_w[1], 0.0f, m_w[1],
                              m_w[2],  m_w[2], 0.0f, m_w[2], m_w[3],  -m_w[3], 0.0f, m_w[3]};
...
TestStatus BuiltinGlPointCoordCaseInstance::iterate(void)
{
    ...
    // Compute coordinates.
    {
        ...
        const int pointSizeDeltaMultiples =
            de::max(1, deCeilFloatToInt32((maxPointSize - minPointSize) / limits.pointSizeGranularity));
        ...
        for (vector<Vec3>::iterator coord = coords.begin(); coord != coords.end(); ++coord)
        {
            ...
            coord->z() = de::min(maxPointSize, minPointSize + float(rnd.getInt(0, pointSizeDeltaMultiples)) *
    ...
    compareOk = fuzzyCompare(m_context.getTestContext().getLog(), "Result", "Image comparison result", refImage,
                             resImage, threshold, COMPARE_LOG_RESULT);
...
typedef uint16_t ShaderInputTypes;
...
BuiltinInputVariationsCaseInstance::BuiltinInputVariationsCaseInstance(Context &context,
                                                                       const ShaderInputTypes shaderInputTypes)
    : ShaderRenderCaseInstance(context)
...
void BuiltinInputVariationsCaseInstance::setupDefaultInputs(void)
{
    ...
    addAttribute(0u, VK_FORMAT_R32G32B32A32_SFLOAT, uint32_t(sizeof(float) * 4), 6, vertices);

    if (m_shaderInputTypes & SHADER_INPUT_VARYING_BIT)
    {
        const float colors[] = {0.6f, 0.0f, 0.0f, 1.0f, 0.3f, 0.3f, 0.0f, 1.0f, 0.0f, 0.6f, 0.0f, 1.0f,
                                0.0f, 0.6f, 0.0f, 1.0f, 0.3f, 0.3f, 0.0f, 1.0f, 0.6f, 0.0f, 0.0f, 1.0f};
        addAttribute(1u, VK_FORMAT_R32G32B32A32_SFLOAT, uint32_t(sizeof(float) * 4), 6, colors);
    }
}

void BuiltinInputVariationsCaseInstance::updatePushConstants(vk::VkCommandBuffer commandBuffer,
                                                             vk::VkPipelineLayout pipelineLayout)
{
    if (m_shaderInputTypes & SHADER_INPUT_CONSTANT_BIT)
    {
        const DeviceInterface &vk = m_context.getDeviceInterface();
        vk.cmdPushConstants(commandBuffer, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(Vec4),
                            &m_constantColor);
    }
}
...
{
public:
    BuiltinInputVariationsCase(TestContext &testCtx, const string &name, const string &description,
                               const ShaderInputTypes shaderInputTypes);
...
BuiltinInputVariationsCase::BuiltinInputVariationsCase(TestContext &testCtx, const string &name,
                                                       const string &description, ShaderInputTypes shaderInputTypes)
    : TestCase(testCtx, name, description)
    , m_shaderInputTypes(shaderInputTypes)
{
}
...
void BuiltinInputVariationsCase::initPrograms(SourceCollections &dst) const
{
    ...
    vertexParams["VARYING_DECL"] = m_shaderInputTypes & SHADER_INPUT_VARYING_BIT ?
    ...
    fragmentParams["CONSTANT_DECL"] = m_shaderInputTypes & SHADER_INPUT_CONSTANT_BIT ?
    ...
    fragmentParams["VARYING_USAGE"] = m_shaderInputTypes & SHADER_INPUT_VARYING_BIT ?
...
{
    de::MovePtr<TestCaseGroup> builtinGroup(
        new TestCaseGroup(testCtx, "builtin_var", "Shader builtin variable tests."));
    de::MovePtr<TestCaseGroup> simpleGroup(new TestCaseGroup(testCtx, "simple", "Simple cases."));
    de::MovePtr<TestCaseGroup> inputVariationsGroup(
        new TestCaseGroup(testCtx, "input_variations", "Input type variation tests."));
    de::MovePtr<TestCaseGroup> frontFacingGroup(
        new TestCaseGroup(testCtx, "frontfacing", "Test gl_Frontfacing keyword."));
    de::MovePtr<TestCaseGroup> fragDepthGroup(new TestCaseGroup(testCtx, "fragdepth", "Test gl_FragDepth keyword."));
    de::MovePtr<TestCaseGroup> fragCoordMsaaGroup(
        new TestCaseGroup(testCtx, "fragcoord_msaa", "Test interation between gl_FragCoord and msaa"));
    de::MovePtr<TestCaseGroup> fragCoordMsaaInputGroup(
        new TestCaseGroup(testCtx, "fragcoord_msaa_input", "Test interation between gl_FragCoord and msaa"));
    ...
    simpleGroup->addChild(
        new BuiltinGlPointCoordCase(testCtx, "pointcoord", "PointCoord test", POINTCOORD_VARIANT_DEFAULT));
    simpleGroup->addChild(new BuiltinGlPointCoordCase(testCtx, "pointcoord_uniform_frag",
                                                      "PointCoord test with fragment uniform",
                                                      POINTCOORD_VARIANT_UNIFORM_FRAGMENT));
    simpleGroup->addChild(new BuiltinGlPointCoordCase(
        testCtx, "pointcoord_uniform_vert", "PointCoord test with vertex uniform", POINTCOORD_VARIANT_UNIFORM_VERTEX));
    ...
    // FragCoord_msaa
    {
        ...
        for (uint32_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(fragCoordMsaaCaseList); caseNdx++)
        {
            fragCoordMsaaGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                testCtx, fragCoordMsaaCaseList[caseNdx].name, fragCoordMsaaCaseList[caseNdx].description,
                fragCoordMsaaCaseList[caseNdx].sampleCount, true, sampleMaskArray, false, true));
            fragCoordMsaaInputGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                testCtx, fragCoordMsaaCaseList[caseNdx].name, fragCoordMsaaCaseList[caseNdx].description,
                fragCoordMsaaCaseList[caseNdx].sampleCount, true, sampleMaskArray, false, false));
        }
        ...
        {
            fragCoordMsaaInputGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                testCtx, (std::string(fragCoordMsaaCaseList[caseNdx].name) + "_no_sample_shading").c_str(),
                fragCoordMsaaCaseList[caseNdx].description, fragCoordMsaaCaseList[caseNdx].sampleCount, false,
                sampleMaskArray, false, false));
        }
        ...
        for (uint32_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(fragCoordMsaaCaseList); caseNdx++)
        {
            fragCoordMsaaInputGroup->addChild(new BuiltinFragCoordMsaaTestCase(
                testCtx,
                (std::string(fragCoordMsaaCaseList[caseNdx].name) + "_no_sample_shading_centroid_interpolation")
                    .c_str(),
                fragCoordMsaaCaseList[caseNdx].description, fragCoordMsaaCaseList[caseNdx].sampleCount, false,
                sampleMaskArray, true, false));
        }
    }
    ...
    // gl_FrontFacing tests
    {
        static const struct PrimitiveTable
        {
            ...
            {"triangle_list", "Test that triangles can be frontfacing or backfacing",
             VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST},
            {"triangle_strip", "Test that traiangle strips can be front or back facing",
        ...
        for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(frontfacingCases); ndx++)
            frontFacingGroup->addChild(new BuiltinGlFrontFacingCase(
                testCtx, frontfacingCases[ndx].primitive, frontfacingCases[ndx].name, frontfacingCases[ndx].desc));
    }
    ...
    // gl_FragDepth
    {
        ...
        for (uint32_t primNdx = 0; primNdx < DE_LENGTH_OF_ARRAY(primitiveTopologyTable); primNdx++)
        {
            for (uint32_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(testCaseTable); caseNdx++)
                fragDepthGroup->addChild(new BuiltinFragDepthCase(
                    testCtx, (primitiveTopologyTable[primNdx].name + "_" + testCaseTable[caseNdx].name).c_str(),
                    primitiveTopologyTable[primNdx].desc.c_str(), primitiveTopologyTable[primNdx].prim,
                    testCaseTable[caseNdx].format, testCaseTable[caseNdx].largeDepthEnable,
                    testCaseTable[caseNdx].depthClampEnable, testCaseTable[caseNdx].samples));
        }
    }
    ...
    for (uint16_t shaderType = 0;
         shaderType <= (SHADER_INPUT_BUILTIN_BIT | SHADER_INPUT_VARYING_BIT | SHADER_INPUT_CONSTANT_BIT); ++shaderType)
    {
        inputVariationsGroup->addChild(new BuiltinInputVariationsCase(testCtx, shaderInputTypeToString(shaderType),
                                                                      "Input variation test", shaderType));
    }
