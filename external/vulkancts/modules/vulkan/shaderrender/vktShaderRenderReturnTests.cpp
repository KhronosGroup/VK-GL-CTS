...
{
public:
    ShaderReturnCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                     bool isVertexCase, const std::string &shaderSource, const ShaderEvalFunc evalFunc,
                     const UniformSetup *uniformFunc);
    virtual ~ShaderReturnCase(void);
};

ShaderReturnCase::ShaderReturnCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                   bool isVertexCase, const std::string &shaderSource, const ShaderEvalFunc evalFunc,
                                   const UniformSetup *uniformFunc)
...
de::MovePtr<ShaderReturnCase> makeConditionalReturnInFuncCase(tcu::TestContext &context, const std::string &name,
                                                              const std::string &description, ReturnMode returnMode,
                                                              bool isVertex)
{
    ...
    return de::MovePtr<ShaderReturnCase>(new ShaderReturnCase(
        context, name, description, isVertex, tmpl.specialize(params), getEvalFunc(returnMode), DE_NULL));
}

de::MovePtr<ShaderReturnCase> makeOutputWriteReturnCase(tcu::TestContext &context, const std::string &name,
                                                        const std::string &description, bool inFunction,
...
de::MovePtr<ShaderReturnCase> makeReturnInLoopCase(tcu::TestContext &context, const std::string &name,
                                                   const std::string &description, bool isDynamicLoop,
                                                   ReturnMode returnMode, bool isVertex)
{
    tcu::StringTemplate tmpl("#version 310 es\n"
                             "layout(location = ${COORDLOC}) in ${COORDPREC} vec4 ${COORDS};\n"
    ...
    return de::MovePtr<ShaderReturnCase>(new ShaderReturnCase(context, name, description, isVertex,
                                                              tmpl.specialize(params), getEvalFunc(returnMode),
                                                              new ReturnTestUniformSetup(UI_ONE)));
}
...
void ShaderReturnTests::init(void)
{
    ...
    for (int returnMode = 0; returnMode < RETURNMODE_LAST; returnMode++)
    {
        for (int isFragment = 0; isFragment < 2; isFragment++)
        {
            ...
            de::MovePtr<ShaderReturnCase> testCase(
                makeConditionalReturnInFuncCase(m_testCtx, name, description, (ReturnMode)returnMode, isFragment == 0));
    ...
    addChild(new ShaderReturnCase(m_testCtx, "last_statement_in_main_vertex", "Return as a final statement in main()",
                                  true,
                                  "#version 310 es\n"
                                  "layout(location = 0) in highp vec4 a_position;\n"
                                  "layout(location = 1) in highp vec4 a_coords;\n"
                                  ...
                                  "}\n",
                                  evalReturnAlways, DE_NULL));
    addChild(new ShaderReturnCase(m_testCtx, "last_statement_in_main_fragment", "Return as a final statement in main()",
                                  false,
                                  "#version 310 es\n"
                                  "layout(location = 0) in mediump vec4 v_coords;\n"
                                  "layout(location = 0) out mediump vec4 o_color;\n\n"
    ...
    for (int inFunc = 0; inFunc < 2; inFunc++)
    {
        for (int returnMode = 0; returnMode < RETURNMODE_LAST; returnMode++)
        {
            for (int isFragment = 0; isFragment < 2; isFragment++)
            {
                ...
                std::string desc = std::string(getReturnModeDesc((ReturnMode)returnMode)) +
                                   (inFunc ? " in user-defined function" : " in main()") + " between output writes";
                de::MovePtr<ShaderReturnCase> testCase = (makeOutputWriteReturnCase(
                    m_testCtx, name, desc, inFunc != 0, (ReturnMode)returnMode, isFragment == 0));
                addChild(testCase.release());
            }
        }
    }
    ...
    for (int isDynamicLoop = 0; isDynamicLoop < 2; isDynamicLoop++)
    {
        for (int returnMode = 0; returnMode < RETURNMODE_LAST; returnMode++)
        {
            for (int isFragment = 0; isFragment < 2; isFragment++)
            {
                ...
                de::MovePtr<ShaderReturnCase> testCase(makeReturnInLoopCase(
                    m_testCtx, name, description, isDynamicLoop != 0, (ReturnMode)returnMode, isFragment == 0));
                addChild(testCase.release());
            }
        }
    }
