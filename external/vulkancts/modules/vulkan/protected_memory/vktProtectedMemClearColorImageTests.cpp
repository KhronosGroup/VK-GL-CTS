...
{
public:
    ClearColorImageTestInstance(Context &ctx, const vk::VkClearColorValue &clearColorValue,
...
{
public:
    ClearColorImageTestCase(tcu::TestContext &testCtx, const std::string &name, vk::VkClearColorValue clearColorValue,
...
ClearColorImageTestInstance::ClearColorImageTestInstance(Context &ctx, const vk::VkClearColorValue &clearColorValue,
                                                         const ValidationData &refData, const ImageValidator &validator,
                                                         const CmdBufferType cmdBufferType)
...
tcu::TestStatus ClearColorImageTestInstance::iterate()
{
    ...
    // Create output image
    de::MovePtr<vk::ImageWithMemory> colorImage = createImage2D(
    ...
    // Image clear
    vk.cmdClearColorImage(targetCmdBuffer, **colorImage, vk::VK_IMAGE_LAYOUT_GENERAL, &m_clearColorValue, 1,
                          &subresourceRange);
...
tcu::TestCaseGroup *createClearColorImageTests(tcu::TestContext &testCtx, CmdBufferType cmdBufferType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> clearStaticTests(
        new tcu::TestCaseGroup(testCtx, "static", "Clear Color Image Tests with static input"));

    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testData); ++ndx)
    {
        const std::string name = "clear_" + de::toString(ndx + 1);
        clearStaticTests->addChild(new ClearColorImageTestCase(testCtx, name.c_str(), testData[ndx].clearColorValue,
                                                               testData[ndx].data, cmdBufferType));
    }

    /* Add a few randomized tests */
    de::MovePtr<tcu::TestCaseGroup> clearRandomTests(
        new tcu::TestCaseGroup(testCtx, "random", "Clear Color Image Tests with random input"));
    ...
    for (int ndx = 0; ndx < testCount; ++ndx)
    {
        ...
        clearRandomTests->addChild(
            new ClearColorImageTestCase(testCtx, name.c_str(), clearValue.color, data, cmdBufferType));
    }
...
{
    de::MovePtr<tcu::TestCaseGroup> clearTests(
        new tcu::TestCaseGroup(testCtx, "clear_color", "Clear Color Image Tests"));
