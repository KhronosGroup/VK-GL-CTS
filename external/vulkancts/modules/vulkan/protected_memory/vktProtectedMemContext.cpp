...
ProtectedContext::ProtectedContext	(Context&						ctx,
	...
	, m_deviceDriver		(m_context.getPlatformInterface(), m_instance, *m_device, m_context.getUsedApiVersion(), m_context.getTestContext().getCommandLine())
...
#endif // CTS_USES_VULKANSC
		ctx.getTestContext().getCommandLine()))
	, m_deviceDriver		(m_interface, m_instance, *m_device, m_context.getUsedApiVersion(), m_context.getTestContext().getCommandLine())
	, m_allocator(createAllocator())
	, m_queue				(getProtectedQueue(m_deviceDriver, *m_device, m_queueFamilyIndex, 0))
{
}
