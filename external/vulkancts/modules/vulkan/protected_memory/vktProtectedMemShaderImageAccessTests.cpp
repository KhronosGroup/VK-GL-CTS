...
{
public:
    ImageAccessTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                        const Params &params)
...
tcu::TestStatus ImageAccessTestInstance::executeComputeTest(void)
{
    ...
    // Create src and dst images
    {
        ...
        imageSrc = createImage2D(ctx, PROTECTION_ENABLED, queueFamilyIndex, IMAGE_WIDTH, IMAGE_HEIGHT,
                                 m_params.imageFormat, imageUsageFlags);

        if (m_params.accessType != ACCESS_TYPE_IMAGE_ATOMICS)
        {
            imageDst = createImage2D(ctx, PROTECTION_ENABLED, queueFamilyIndex, IMAGE_WIDTH, IMAGE_HEIGHT,
                                     m_params.imageFormat, imageUsageFlags);
        }
    }
...
tcu::TestStatus ImageAccessTestInstance::executeFragmentTest(void)
{
    ...
    de::MovePtr<vk::ImageWithMemory> colorImage(
        createImage2D(ctx, PROTECTION_ENABLED, queueFamilyIndex, RENDER_WIDTH, RENDER_HEIGHT, m_params.imageFormat,
    ...
    // Upload source image
    {
        ...
        // Copy unprotected image to protected image
        copyToProtectedImage(m_protectedContext, **unprotectedImage, **imageSrc, imageLayout, IMAGE_WIDTH,
                             IMAGE_HEIGHT);
    }
...
tcu::TestCaseGroup *createShaderImageAccessTests(tcu::TestContext &testCtx)
{
    ...
    for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(shaderTypes); ++shaderTypeNdx)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> shaderGroup(
            new tcu::TestCaseGroup(testCtx, shaderTypes[shaderTypeNdx].name, shaderTypes[shaderTypeNdx].desc));
        ...
        for (int accessNdx = 0; accessNdx < DE_LENGTH_OF_ARRAY(accessTypes); ++accessNdx)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> accessTypeGroup(
                new tcu::TestCaseGroup(testCtx, accessTypes[accessNdx].name, accessTypes[accessNdx].desc));
                ...
                for (uint32_t atomicOpI = 0; atomicOpI < ATOMIC_OPERATION_LAST; ++atomicOpI)
                {
                    ...
                    for (uint32_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                    {
                        ...
                        operationGroup->addChild(new ImageAccessTestCase(
                            testCtx, formats[formatNdx].name, "", Params(shaderType, accessType, format, atomicOp)));
                    }
            ...
            else
            {
                for (uint32_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                {
                    ...
                    accessTypeGroup->addChild(new ImageAccessTestCase(testCtx, formats[formatNdx].name, "",
                                                                      Params(shaderType, accessType, format)));
                }
            }
