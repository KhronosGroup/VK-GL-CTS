...
{
public:
    BlitImageTestInstance(Context &ctx, const vk::VkClearColorValue &clearColorValue, const ValidationData &refData,
...
{
public:
    BlitImageTestCase(tcu::TestContext &testCtx, const std::string &name, vk::VkClearColorValue clearColorValue,
...
BlitImageTestInstance::BlitImageTestInstance(Context &ctx, const vk::VkClearColorValue &clearColorValue,
...
tcu::TestStatus BlitImageTestInstance::iterate()
{
    ...
    vk.cmdBlitImage(targetCmdBuffer, **colorImageSrc, vk::VK_IMAGE_LAYOUT_GENERAL, **colorImage,
                    vk::VK_IMAGE_LAYOUT_GENERAL, 1u, &imageBlit, vk::VK_FILTER_NEAREST);
...
tcu::TestCaseGroup *createBlitImageTests(tcu::TestContext &testCtx, CmdBufferType cmdBufferType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> blitStaticTests(
        new tcu::TestCaseGroup(testCtx, "static", "Blit Image Tests with static input"));

    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testData); ++ndx)
    {
        const std::string name = "blit_" + de::toString(ndx + 1);
        blitStaticTests->addChild(new BlitImageTestCase(testCtx, name.c_str(), testData[ndx].clearColorValue,
                                                        testData[ndx].data, cmdBufferType));
    }

    /* Add a few randomized tests */
    de::MovePtr<tcu::TestCaseGroup> blitRandomTests(
        new tcu::TestCaseGroup(testCtx, "random", "Blit Image Tests with random input"));
