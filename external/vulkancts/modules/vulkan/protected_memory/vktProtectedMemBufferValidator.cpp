...
const char *generateShaderBufferString(TestType testType, BufferType bufferType)
{
    ...
    {
    case TYPE_UINT:
        return "uniform usamplerBuffer";
    case TYPE_INT:
        return "uniform isamplerBuffer";
    case TYPE_FLOAT:
        return "uniform samplerBuffer";

    default:
        DE_FATAL("Incorrect sampler buffer format");
        return "";
    }
}
...
void initBufferValidatorPrograms(vk::SourceCollections &programCollection, TestType testType, BufferType bufferType)
{
    ...
    const char *validatorShaderTemplateSamplerBuffer =
        "#version 450\n"
    ...
    const char *validatorShaderTemplateStorageBuffer =
        "#version 450\n"
        "layout(local_size_x = 1) in;\n"
        "\n"
        "layout(set=0, binding=0) ${BUFFER_TYPE} u_protectedBuffer\n"
    ...
    tcu::StringTemplate validatorShaderTemplate(bufferType == SAMPLER_BUFFER ? validatorShaderTemplateSamplerBuffer :
...
vk::VkDescriptorType getDescriptorType(BufferType bufferType)
{
    ...
    case SAMPLER_BUFFER:
        return vk::VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER;
    default:
        DE_FATAL("Incorrect buffer type specified");
        return (vk::VkDescriptorType)0;
    }
}
