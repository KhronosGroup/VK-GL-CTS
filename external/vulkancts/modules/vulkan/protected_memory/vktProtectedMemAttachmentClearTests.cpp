...
{
public:
    AttachmentClearTestInstance(Context &ctx, const vk::VkClearValue &clearValue, const ValidationData &refData,
...
{
public:
    AttachmentClearTestCase(tcu::TestContext &testCtx, const std::string &name, vk::VkClearValue clearValue,
...
AttachmentClearTestInstance::AttachmentClearTestInstance(Context &ctx, const vk::VkClearValue &clearValue,
...
tcu::TestStatus AttachmentClearTestInstance::iterate()
{
    ...
    const tcu::Vec4 clearValue(
        m_clearValue.color.float32[0] < 0.5f ? 1.0f : 0.0f, m_clearValue.color.float32[1] < 0.5f ? 1.0f : 0.0f,
        m_clearValue.color.float32[2] < 0.5f ? 1.0f : 0.0f, m_clearValue.color.float32[3] < 0.5f ? 1.0f : 0.0f);
    ...
    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, vk::makeRect2D(RENDER_WIDTH, RENDER_HEIGHT), clearValue,
                    subpassContents);
...
tcu::TestCaseGroup *createAttachmentClearTests(tcu::TestContext &testCtx, CmdBufferType cmdBufferType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> clearStaticTests(
        new tcu::TestCaseGroup(testCtx, "static", "Attachment Clear Op Tests with static input"));

    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testData); ++ndx)
    {
        const std::string name = "clear_" + de::toString(ndx + 1);
        clearStaticTests->addChild(new AttachmentClearTestCase(testCtx, name.c_str(), testData[ndx].clearValue,
                                                               testData[ndx].data, cmdBufferType));
    }

    /* Add a few randomized tests */
    de::MovePtr<tcu::TestCaseGroup> clearRandomTests(
        new tcu::TestCaseGroup(testCtx, "random", "Attachment Clear Op Tests with random input"));
...
tcu::TestCaseGroup *createAttachmentClearTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> clearTests(
        new tcu::TestCaseGroup(testCtx, "clear_op", "Attachment Clear Op Tests"));
