...
std::vector<vk::VkSwapchainCreateInfoKHR> generateSwapchainParameterCases(
    vk::wsi::Type wsiType, TestDimension dimension, const ProtectedContext &context,
    const vk::VkSurfaceCapabilitiesKHR &capabilities, const std::vector<vk::VkSurfaceFormatKHR> &formats,
    const std::vector<vk::VkPresentModeKHR> &presentModes)
{
    ...
    case TEST_DIMENSION_MIN_IMAGE_COUNT:
    {
        ...
        const uint32_t maxImageCount =
            de::min((capabilities.maxImageCount > 0) ? capabilities.maxImageCount : capabilities.minImageCount + 16u,
                    uint32_t(maxMemoryUsage / memoryRequirements.size));
    ...
    case TEST_DIMENSION_IMAGE_EXTENT:
    {
            ...
            platformProperties.swapchainExtent == vk::wsi::PlatformProperties::SWAPCHAIN_EXTENT_SCALED_TO_WINDOW_SIZE)
        {
            for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(s_testSizes); ++ndx)
            {
                ...
                {
                    cases.push_back(baseParameters);
                    cases.back().imageExtent.width = de::clamp(
                        s_testSizes[ndx].width, capabilities.minImageExtent.width, capabilities.maxImageExtent.width);
                    cases.back().imageExtent.height =
...
tcu::TestStatus createSwapchainTest(Context &baseCtx, TestParameters params)
{
    ...
    const std::vector<vk::VkSwapchainCreateInfoKHR> cases(
        generateSwapchainParameterCases(params.wsiType, params.dimension, context, surface));
    ...
    for (size_t caseNdx = 0; caseNdx < cases.size(); ++caseNdx)
    {
        ...
        context.getTestContext().getLog() << tcu::TestLog::Message << "Sub-case " << (caseNdx + 1) << " / "
                                          << cases.size() << ": " << curParams << tcu::TestLog::EndMessage;

        {
            const vk::Unique<vk::VkSwapchainKHR> swapchain(
                createSwapchainKHR(context.getDeviceDriver(), context.getDevice(), &curParams));
        }
    }
...
{
    addFunctionCaseWithPrograms(testGroup, "basic", "Basic Rendering Test", checkSupport, getBasicRenderPrograms,
                                basicRenderTest, wsiType);
}

void createSwapchainTests(tcu::TestCaseGroup *testGroup, vk::wsi::Type wsiType)
{
    addTestGroup(testGroup, "create", "Create VkSwapchain with various parameters", populateSwapchainGroup,
                 GroupParameters(wsiType, createSwapchainTest));
