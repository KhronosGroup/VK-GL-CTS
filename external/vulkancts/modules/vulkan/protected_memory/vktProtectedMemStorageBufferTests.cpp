...
const char *getSSBOTestDescription(SSBOTestType type)
{
    ...
    {
    case SSBO_READ:
        return "Test for read storage buffer on protected memory.";
    case SSBO_WRITE:
        return "Test for write storage buffer on protected memory.";
    case SSBO_ATOMIC:
        return "Test for atomic storage buffer on protected memory.";
    default:
...
void static addBufferCopyCmd(const vk::DeviceInterface &vk, vk::VkCommandBuffer cmdBuffer, uint32_t queueFamilyIndex,
                             vk::VkBuffer srcBuffer, vk::VkBuffer dstBuffer, uint32_t copySize)
{
...
public:
    StorageBufferTestCase(tcu::TestContext &testctx, const SSBOTestType testType, const glu::ShaderType shaderType,
                          ...
                          vk::VkFormat format, const std::string &extraShader = "")
...
tcu::TestStatus StorageBufferTestInstance<T>::executeFragmentTest(void)
{
    ...
    de::MovePtr<vk::BufferWithMemory> testBuffer(
        makeBuffer(ctx, PROTECTION_ENABLED, queueFamilyIndex, testBufferSize,
    ...
    vk::Unique<vk::VkPipeline> graphicsPipeline(makeGraphicsPipeline(
        ...
        tcu::UVec2(RENDER_WIDTH, RENDER_HEIGHT), vk::VK_PRIMITIVE_TOPOLOGY_POINT_LIST));
                                                      ...
                                                      size_t testCount)
{
    ...
    for (size_t ndx = 0; ndx < testCount; ++ndx)
    {
        ...
        testGroup->addChild(new StorageBufferTestCase<tcu::UVec4>(testCtx, testType, shaderType, name.c_str(),
                                                                  testData[ndx].values, testData[ndx],
...
tcu::TestCaseGroup *createRWStorageBufferTests(tcu::TestContext &testCtx, const std::string groupName,
                                               const std::string groupDescription, SSBOTestType testType,
                                               const ValidationDataStorage<tcu::UVec4> testData[], size_t testCount)
{
    de::MovePtr<tcu::TestCaseGroup> ssboRWTestGroup(
        new tcu::TestCaseGroup(testCtx, groupName.c_str(), groupDescription.c_str()));
    ...
    for (int shaderNdx = 0; shaderNdx < DE_LENGTH_OF_ARRAY(shaderTypes); ++shaderNdx)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> testShaderGroup(
            new tcu::TestCaseGroup(testCtx, shaderName.c_str(), shaderGroupDesc.c_str()));

        testShaderGroup->addChild(
            createSpecifiedStorageBufferTests(testCtx, "static", testType, shaderType, testData, testCount));
...
tcu::TestCaseGroup *createReadStorageBufferTests(tcu::TestContext &testCtx)
{
    ...
    return createRWStorageBufferTests(testCtx, "ssbo_read", "Storage Buffer Read Tests", SSBO_READ, testData,
                                      DE_LENGTH_OF_ARRAY(testData));
}
...
tcu::TestCaseGroup *createWriteStorageBufferTests(tcu::TestContext &testCtx)
{
    ...
    return createRWStorageBufferTests(testCtx, "ssbo_write", "Storage Buffer Write Tests", SSBO_WRITE, testData,
                                      DE_LENGTH_OF_ARRAY(testData));
}
...
tcu::TestCaseGroup *createAtomicStorageBufferTests(tcu::TestContext &testctx)
{
    ...
    de::Random rnd(testctx.getCommandLine().getBaseSeed());
    de::MovePtr<tcu::TestCaseGroup> ssboAtomicTests(
        new tcu::TestCaseGroup(testctx, "ssbo_atomic", "Storage Buffer Atomic Tests"));
    ...
    for (int shaderNdx = 0; shaderNdx < DE_LENGTH_OF_ARRAY(shaderTypes); ++shaderNdx)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> atomicShaderGroup(
            new tcu::TestCaseGroup(testctx, shaderName.c_str(), shaderDesc.c_str()));
        ...
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(testTypes); ++typeNdx)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> staticTests(
                new tcu::TestCaseGroup(testctx, "static", (atomicDesc + " with static input").c_str()));
            for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testData); ++ndx)
            {
                ...
                staticTests->addChild(
                    new StorageBufferTestCase<tcu::UVec4>(testctx, SSBO_ATOMIC, shaderType, name.c_str(), inputValue,
                                                          validationData, vk::VK_FORMAT_R32G32B32A32_UINT, atomicCall));
            }

            de::MovePtr<tcu::TestCaseGroup> randomTests(
                new tcu::TestCaseGroup(testctx, "random", (atomicDesc + " with random input").c_str()));
            for (int ndx = 0; ndx < RANDOM_TEST_COUNT; ndx++)
            {
                ...
                ValidationDataStorage<tcu::UVec4> validationData = {refValue};
                randomTests->addChild(
                    new StorageBufferTestCase<tcu::UVec4>(testctx, SSBO_ATOMIC, shaderType, name.c_str(), inputValue,
                                                          validationData, vk::VK_FORMAT_R32G32B32A32_UINT, atomicCall));
            }

            de::MovePtr<tcu::TestCaseGroup> atomicTests(
                new tcu::TestCaseGroup(testctx, atomicTypeStr.c_str(), atomicDesc.c_str()));
