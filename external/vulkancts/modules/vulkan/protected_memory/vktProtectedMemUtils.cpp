                                              ...
                                              const std::vector<std::string> &extraExtensions, bool validationEnabled)
{
    ...
    for (uint32_t ndx = 0; ndx < extensions.size(); ++ndx)
    {
        ...
        if (notInCore && !isExtensionSupported(supportedExtensions.begin(), supportedExtensions.end(),
    ...
    vk::VkPhysicalDeviceSamplerYcbcrConversionFeatures ycbcrFeature = {
        vk::VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLER_YCBCR_CONVERSION_FEATURES, DE_NULL, VK_FALSE};
