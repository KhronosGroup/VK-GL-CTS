...
{
public:
    CopyImageToBufferTestInstance(Context &ctx, const vk::VkClearColorValue fillValue,
                                  const BufferValidator<T> &validator, const CmdBufferType cmdBufferType,
...
{
public:
    CopyImageToBufferTestCase(tcu::TestContext &testCtx, const std::string &name, vk::VkClearColorValue fillValue,
                              ValidationData<T> data, CmdBufferType cmdBufferType, vk::VkFormat format,
...
template <typename T>
CopyImageToBufferTestInstance<T>::CopyImageToBufferTestInstance(Context &ctx, const vk::VkClearColorValue fillValue,
...
tcu::TestStatus CopyImageToBufferTestInstance<T>::iterate()
{
    ...
    vk.cmdClearColorImage(targetCmdBuffer, **colorImage, vk::VK_IMAGE_LAYOUT_GENERAL, &m_fillValue, 1,
                          &subresourceRange);
    ...
    vk.cmdCopyImageToBuffer(targetCmdBuffer, **colorImage, vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, **dstBuffer, 1u,
                            &copyRegion);
...
tcu::TestCaseGroup *createCopyImageToFloatBufferTests(tcu::TestContext &testCtx, CmdBufferType cmdBufferType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> copyStaticTests(
        new tcu::TestCaseGroup(testCtx, "static", "Copy Image to Buffer Tests with static input"));
    ...
    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(testData); ++ndx)
    {
        for (int ppa = 0; ppa < DE_LENGTH_OF_ARRAY(pipelineProtectedAccess); ++ppa)
        {
            ...
            const std::string name =
                "copy_" + de::toString(ndx + 1) + (pipelineProtectedAccess[ppa] ? "_protected_access" : "");
            ;
            copyStaticTests->addChild(new CopyImageToBufferTestCase<tcu::Vec4>(
                testCtx, name.c_str(), testData[ndx].fillValue, testData[ndx].data, cmdBufferType,
                vk::VK_FORMAT_R32G32B32A32_SFLOAT, pipelineProtectedAccess[ppa]));
        }
    }

    /* Add a few randomized tests */
    de::MovePtr<tcu::TestCaseGroup> copyRandomTests(
        new tcu::TestCaseGroup(testCtx, "random", "Copy Image to Buffer Tests with random input"));
    ...
    for (int ndx = 0; ndx < testCount; ++ndx)
    {
        for (int ppa = 0; ppa < DE_LENGTH_OF_ARRAY(pipelineProtectedAccess); ++ppa)
        {
            ...
            const tcu::Vec4 refValue(clearValue.color.float32[0], clearValue.color.float32[1],
            ...
            copyRandomTests->addChild(new CopyImageToBufferTestCase<tcu::Vec4>(
                testCtx, name.c_str(), clearValue.color, data, cmdBufferType, vk::VK_FORMAT_R32G32B32A32_SFLOAT,
                pipelineProtectedAccess[ppa]));
        }
    }
...
{
    de::MovePtr<tcu::TestCaseGroup> copyTests(
        new tcu::TestCaseGroup(testCtx, "copy_image_to_float_buffer", "Copy Image to Buffer Tests"));
