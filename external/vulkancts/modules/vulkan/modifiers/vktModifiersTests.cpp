...
VkImageFormatProperties2 verifyHandleTypeForFormatModifier(const InstanceInterface &vki,
                                                           VkPhysicalDevice physicalDevice, const VkFormat format,
...
void checkExportImportExtensions(Context &context, VkFormat format)
{
    ...
    std::vector<VkDrmFormatModifierPropertiesEXT> drmFormatModifiers =
        getDrmFormatModifiers(vki, context.getPhysicalDevice(), format);
    ...
    {
        const VkFormatFeatureFlags testFeatures =
...
tcu::TestStatus createImageListModifiersCase(Context &context, const VkFormat format)
{
    ...
    for (uint32_t modifierNdx = 0; modifierNdx < drmFormatModifiers.size(); modifierNdx++)
    {
        ...
        for (uint32_t m = 0; m <= modifierNdx; m++)
        {
        ...
        {
            Move<VkImage> image(createImageWithDrmFormatModifiers(
        ...
        for (uint32_t m = 0; m < modifiers.size(); m++)
        {
...
bool exportImportMemoryExplicitModifiersCase(Context &context, const VkFormat format,
                                             const VkDrmFormatModifierPropertiesEXT &modifier)
{
    ...
    Move<VkDeviceMemory> importedMemory(vkt::ExternalMemoryUtil::importMemory(
    ...
    copyImageToBuffer(vkd, *cmdBuffer2, *outImage, outputBuffer->get(), tcu::IVec2(imageSize.x(), imageSize.y()), 0,
...
tcu::TestCaseGroup *createTests(tcu::TestContext &testCtx)
{
...

    {
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
            addFunctionCase(group.get(), getFormatCaseName(formats[formatNdx]),
                            ...
                            listModifiersCase, formats[formatNdx]);
...

    {
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
            addFunctionCase(group.get(), getFormatCaseName(formats[formatNdx]),
                            ...
                            createImageListModifiersCase, formats[formatNdx]);
...

    {
        ...
        for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
            addFunctionCase(group.get(), getFormatCaseName(formats[formatNdx]),
                            "Test exporting/importing images with modifiers", checkExportImportExtensions,
                            exportImportMemoryExplicitModifiersCase, formats[formatNdx]);
