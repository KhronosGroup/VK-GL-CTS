...
CustomInstance::CustomInstance(Context &context, Move<VkInstance> instance, bool enableDebugReportRecorder,
    ...
    , m_recorder(enableDebugReportRecorder ? (new DebugReportRecorder(*m_driver, *m_instance, printValidationErrors)) :
                                             nullptr)
{
}

CustomInstance::CustomInstance() : m_context(nullptr), m_instance(), m_driver(nullptr), m_recorder(nullptr)
{
}
...
UncheckedInstance::UncheckedInstance(Context &context, vk::VkInstance instance,
                                     const vk::VkAllocationCallbacks *pAllocator, bool enableDebugReportRecorder,
                                     bool printValidationErrors)
    : m_context(&context)
    , m_allocator(pAllocator)
    , m_instance(instance)
    , m_driver((m_instance != DE_NULL) ? new InstanceDriver(context.getPlatformInterface(), m_instance) : nullptr)
    , m_recorder((enableDebugReportRecorder && m_instance != DE_NULL) ?
                     (new DebugReportRecorder(*m_driver, m_instance, printValidationErrors)) :
                     nullptr)
{
}
                                                  ...
                                                  const vk::VkAllocationCallbacks *pAllocator, bool allowLayers)
{
    ...
    const std::vector<vk::VkExtensionProperties> availableExtensions =
