...
template <uint32_t BindingNumber>
struct Binding
{
    static const uint32_t binding = BindingNumber;
};
...
struct TestCaseParams
{
    ...
    bool fuzzyComparison;            // if true then a test will use fuzzy comparison, otherwise float threshold
...
struct TestParams
{
    ...
    uint32_t additionalDescriptorBinding;
    ...
    TestParams(VkShaderStageFlags stageFlags_, VkDescriptorType descriptorType_, uint32_t descriptorBinding_,
               VkDescriptorType additionalDescriptorType_, uint32_t additionalDescriptorBinding_,
               bool copyBuffersToImages_, bool allowVertexStoring_, const TestCaseParams &caseParams)
...
public:
    ...
    static std::string substBinding(uint32_t binding, const char *str, uint32_t count = 0, const char *name = DE_NULL);
                                                                                ...
                                                                                uint32_t &descriptorCount) const
{
    ...
    bool optional = (m_testParams.additionalDescriptorBinding != BINDING_Undefined) &&
                    (m_testParams.additionalDescriptorType != VK_DESCRIPTOR_TYPE_UNDEFINED);
    ...
    const VkDescriptorSetLayoutBindingFlagsCreateInfo bindingCreateInfo = {
        VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_BINDING_FLAGS_CREATE_INFO, DE_NULL,
...
Move<VkDescriptorPool> CommonDescriptorInstance::createDescriptorPool(uint32_t descriptorCount) const
{
    ...
    if (m_testParams.additionalDescriptorType != VK_DESCRIPTOR_TYPE_UNDEFINED &&
        m_testParams.additionalDescriptorBinding != BINDING_Undefined)
    {
...
std::string CommonDescriptorInstance::substBinding(uint32_t binding, const char *str, uint32_t count, const char *name)
{
...
int CommonDescriptorInstance::constructShaderModules(void)
{
    ...
    {
        ++result;
        const std::string name = ut::buildShaderName(VK_SHADER_STAGE_COMPUTE_BIT, m_testParams.descriptorType,
                                                     m_testParams.updateAfterBind, m_testParams.calculateInLoop, false);
    ...
    if (m_testParams.stageFlags & VK_SHADER_STAGE_FRAGMENT_BIT)
    {
        ...
        const std::string name =
            ut::buildShaderName(VK_SHADER_STAGE_FRAGMENT_BIT, m_testParams.descriptorType, m_testParams.updateAfterBind,
                                m_testParams.calculateInLoop, m_testParams.allowVertexStoring);
    ...
    if (m_testParams.stageFlags & VK_SHADER_STAGE_VERTEX_BIT)
    {
        ++result;
        const std::string name =
            ut::buildShaderName(VK_SHADER_STAGE_VERTEX_BIT, m_testParams.descriptorType, m_testParams.updateAfterBind,
                                m_testParams.calculateInLoop, m_testParams.allowVertexStoring);
...
tcu::TestStatus CommonDescriptorInstance::iterate(void)
{
    ...
    return (iterateVerifyResults(v, programResult, referenceResult) ? tcu::TestStatus::pass :
                                                                      tcu::TestStatus::fail)("");
}
                                                         ...
                                                         VkDescriptorSet descriptorSet, uint32_t descriptorSetIndex)
{
    ...
    m_vki.cmdBindDescriptorSets(commandBuffer, pipelineBindingPoint, pipelineLayout, descriptorSetIndex, 1u,
                                &descriptorSet, 0u, static_cast<uint32_t *>(DE_NULL));
}
                                                      ...
                                                      const ut::FrameBufferSp &frameBuffer)
{
    ...
    m_vki.cmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT,
                             ...
                             DE_NULL, 1u, &bufferBarrier, 1u, &barrierAfter);
                                                     ...
                                                     bool usesMipMaps)
{
    ...
    case VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER:
        ...
        break;
    case VK_DESCRIPTOR_TYPE_SAMPLER:
        text = usesMipMaps ? "textureLod(nonuniformEXT(sampler2D(tex[0], data[${INDEX}])), normalpos, 1)" :
                             "texture(   nonuniformEXT(sampler2D(tex[0], data[${INDEX}])), normalpos   )";
        break;
    case VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
        text = usesMipMaps ? "textureLod( nonuniformEXT(sampler2D(data[${INDEX}], samp[0])), vec2(0,0), "
                             "textureQueryLevels(nonuniformEXT(sampler2D(data[${INDEX}], samp[0])))-1)" :
...
bool CommonDescriptorInstance::performWritesInVertex(VkDescriptorType descriptorType, const Context &context)
{
    ...
    if (feats.vertexPipelineStoresAndAtomics != false)
    {
                                                      ...
                                                      const TestCaseParams &testCaseParams, bool allowVertexStoring)
{
    ...
    case VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC:
        ...
        break;
    case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
        s << substBinding(BINDING_UniformBuffer, "layout(set=0,binding=${?}) uniform Data { vec4 c; } data[]; \n");
        break;
    case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC:
    ...
    case VK_SHADER_STAGE_FRAGMENT_BIT:
    {
        ...
        {
            if (testCaseParams.calculateInLoop)
                s << getFragmentLoopSource(getColorAccess(testCaseParams.descriptorType, "rIndex", false),
...
StorageBufferInstance::StorageBufferInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, BINDING_StorageBuffer,
                              VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, false,
...
UniformBufferInstance::UniformBufferInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, BINDING_UniformBuffer,
                              VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, false,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
{
}
...
StorageTexelInstance::StorageTexelInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER,
                              BINDING_StorageTexelBuffer, VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, false,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
{
}
...
UniformTexelInstance::UniformTexelInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER,
                              BINDING_UniformTexelBuffer, VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, false,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
{
}
...
DynamicStorageBufferInstance::DynamicStorageBufferInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC,
                              BINDING_StorageBufferDynamic, VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, false,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
...
DynamicUniformBufferInstance::DynamicUniformBufferInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC,
                              BINDING_UniformBufferDynamic, VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, false,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
    , DynamicBuffersInstance(context, m_testParams)
...
InputAttachmentInstance::InputAttachmentInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT,
                              BINDING_InputAttachment, VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined, true,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
{
}
...
SamplerInstance::SamplerInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_SAMPLER, BINDING_Sampler,
                              VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, BINDING_SampledImage, true,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
{
}
...
SampledImageInstance::SampledImageInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(context, TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE,
                                                   BINDING_SampledImage, VK_DESCRIPTOR_TYPE_SAMPLER, BINDING_Sampler,
                                                   true, performWritesInVertex(testCaseParams.descriptorType, context),
                                                   testCaseParams))
{
}
...
CombinedImageInstance::CombinedImageInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(context,
                               TestParams(VK_SHADER_STAGE_ALL_GRAPHICS, testCaseParams.descriptorType,
                                          BINDING_CombinedImageSampler, VK_DESCRIPTOR_TYPE_UNDEFINED, BINDING_Undefined,
...
StorageImageInstance::StorageImageInstance(Context &context, const TestCaseParams &testCaseParams)
    : CommonDescriptorInstance(
          context, TestParams(VK_SHADER_STAGE_COMPUTE_BIT, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, BINDING_StorageImage,
                              VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, (BINDING_StorageImage + 1), true,
                              performWritesInVertex(testCaseParams.descriptorType, context), testCaseParams))
    , m_buffer()
    , m_fillColor(10)
{
...
tcu::TestStatus StorageImageInstance::iterate(void)
{
    ...
    m_vki.cmdDispatch(*v.commandBuffer, m_testParams.calculateInLoop ? 1 : v.renderArea.extent.width,
...
public:
    DescriptorIndexingTestCase(tcu::TestContext &context, const char *name, const char *description,
        ...
        if (VK_SHADER_STAGE_VERTEX_BIT & m_testCaseParams.stageFlags)
        {
            programCollection.glslSources.add(
                ut::buildShaderName(VK_SHADER_STAGE_VERTEX_BIT, m_testCaseParams.descriptorType,
                                    m_testCaseParams.updateAfterBind, m_testCaseParams.calculateInLoop, false))
            ...
            if (CommonDescriptorInstance::performWritesInVertex(m_testCaseParams.descriptorType))
            {
                programCollection.glslSources.add(
                    ut::buildShaderName(VK_SHADER_STAGE_VERTEX_BIT, m_testCaseParams.descriptorType,
                                        m_testCaseParams.updateAfterBind, m_testCaseParams.calculateInLoop, true))
...
void descriptorIndexingDescriptorSetsCreateTests(tcu::TestCaseGroup *group)
{
    ...
    TestCaseInfo casesAfterBindAndLoop[] = {
        {"storage_buffer",
         "Regular Storage Buffer Descriptors",
         {VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT), RESOLUTION,
        ...
        {"storage_texel_buffer",
         ...
         {VK_DESCRIPTOR_TYPE_STORAGE_TEXEL_BUFFER, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT),
          RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // useMipMaps
        ...
        {"uniform_texel_buffer",
         ...
         {VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT),
          RESOLUTION,
          false, // updateAfterBind,
          false, // calculateInLoop
          false, // usesMipMaps
        ...
        {"storage_image",
         ...
         {VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT, RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // useMipMaps
          FUZZY_COMPARE, CMP_THRESHOLD}},
    ...
    for (int updateAfterBind = 0; updateAfterBind < 2; ++updateAfterBind)
    {
        for (int calculateInLoop = 0; calculateInLoop < 2; ++calculateInLoop)
        {
            for (uint32_t caseIdx = 0; caseIdx < DE_LENGTH_OF_ARRAY(casesAfterBindAndLoop); ++caseIdx)
            {
                ...
                group->addChild(
                    new DescriptorIndexingTestCase(context, caseName.c_str(), caseDescription.c_str(), params));
            }
        }
    }
    ...
    TestCaseInfo casesAfterBindAndLoopAndLOD[] = {
        {"sampler",
         "Sampler Descriptors",
         {VK_DESCRIPTOR_TYPE_SAMPLER, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT), RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // usesMipMaps
          FUZZY_COMPARE, CMP_THRESHOLD}},
        {"sampled_image",
         ...
         {VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT), RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // usesMipMaps
          FUZZY_COMPARE, CMP_THRESHOLD}},
        {"combined_image_sampler",
         ...
         {VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT),
          RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // usesMipMaps
    ...
    for (int updateAfterBind = 0; updateAfterBind < 2; ++updateAfterBind)
    {
        for (int calculateInLoop = 0; calculateInLoop < 2; ++calculateInLoop)
        {
            ...
            {
                for (uint32_t caseIdx = 0; caseIdx < DE_LENGTH_OF_ARRAY(casesAfterBindAndLoopAndLOD); ++caseIdx)
                {
    ...
    TestCaseInfo casesNonAfterBindAndLoop[] = {
        {"uniform_buffer",
         "Regular Uniform Buffer Descriptors",
         {VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT), RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // usesMipMaps
          FUZZY_COMPARE, CMP_THRESHOLD}},
        {"storage_buffer_dynamic",
         "Dynamic Storage Buffer Descriptors",
        ...
        {"input_attachment",
         ...
         {VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, (VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT), RESOLUTION,
          false, // updateAfterBind
          false, // calculateInLoop
          false, // useMipMaps
          FUZZY_COMPARE, CMP_THRESHOLD}},
    ...
    for (int calculateInLoop = 0; calculateInLoop < 2; ++calculateInLoop)
    {
        for (uint32_t caseIdx = 0; caseIdx < DE_LENGTH_OF_ARRAY(casesNonAfterBindAndLoop); ++caseIdx)
        {
