...
vk::Move<vk::VkDeviceMemory> importMemory(const vk::DeviceInterface &vkd, vk::VkDevice device,
                                          ...
                                          NativeHandle &handle)
{
    return importMemory(vkd, device, (vk::VkBuffer)0, (vk::VkImage)0, requirements, externalType, memoryTypeIndex,
                        handle);
}
...
public:
    ...
    virtual void describe(const vk::pt::AndroidHardwareBufferPtr buffer, uint32_t *width, uint32_t *height,
                          uint32_t *layers, uint32_t *format, uint64_t *usage, uint32_t *stride);
    virtual uint64_t vkUsageToAhbUsage(vk::VkImageUsageFlagBits vkFlag);
    virtual uint64_t vkCreateToAhbUsage(vk::VkImageCreateFlagBits vkFlag);
    virtual uint32_t vkFormatToAhbFormat(vk::VkFormat vkFormat);
    virtual uint64_t mustSupportAhbUsageFlags();
    virtual bool ahbFormatIsBlob(uint32_t ahbFormat)
    {
