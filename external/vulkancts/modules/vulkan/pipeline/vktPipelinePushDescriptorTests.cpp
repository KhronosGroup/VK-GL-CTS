...
void PushDescriptorBufferGraphicsTestInstance::init(void)
{
    ...
    // Create command buffer
    {
        ...
        m_renderPass.begin(m_vkd, *m_cmdBuffer, makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()),
...
PushDescriptorTexelBufferComputeTest::PushDescriptorTexelBufferComputeTest(tcu::TestContext &testContext,
                                                                           const string &name, const TestParams &params)
    : vkt::TestCase(testContext, name)
    , m_params(params)
{
}
...
void PushDescriptorInputAttachmentGraphicsTestInstance::init(void)
{
    ...
    for (uint32_t renderPassIdx = 0; renderPassIdx < 2; renderPassIdx++)
    {
        ...
        m_renderPasses.push_back(VkRenderPassSp(
            new RenderPassWrapper(m_params.pipelineConstructionType, m_vkd, *m_device, &renderPassInfo)));
    ...
    for (uint32_t pipelineIdx = 0; pipelineIdx < 2; pipelineIdx++)
    {
        ...
        m_graphicsPipelines.emplace_back(m_vki, m_vkd, m_physicalDevice, *m_device, m_deviceEnabledExtensions,
                                         m_params.pipelineConstructionType);
    ...
    // Create command buffer
    {
        ...
        for (uint32_t quadNdx = 0; quadNdx < m_params.numCalls; quadNdx++)
        {
            (*m_renderPasses[quadNdx])
                .begin(m_vkd, *m_cmdBuffer, makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), attachmentClearValue);
...
tcu::TestCaseGroup *createPushDescriptorTests(tcu::TestContext &testCtx, PipelineConstructionType pipelineType)
{
    ...
    if (pipelineType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
    {
        ...
        graphicsTests->addChild(
            new PushDescriptorTexelBufferGraphicsTest(testCtx, "maintenance5_storage_texel_buffer", testParams));
