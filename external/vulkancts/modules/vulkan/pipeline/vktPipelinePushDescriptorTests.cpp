...
vector<Vertex4Tex4> createTexQuads(uint32_t numQuads, float size)
{
    ...
    {
        const float xOffset                = -0.5f + (float)quadNdx;
        const Vertex4Tex4 lowerLeftVertex  = {tcu::Vec4(-size + xOffset, -size, 0.0f, 1.0f),
                                              tcu::Vec4(-0.2f, -0.2f, 0.0f, 0.0f)};
        const Vertex4Tex4 lowerRightVertex = {tcu::Vec4(size + xOffset, -size, 0.0f, 1.0f),
                                              tcu::Vec4(1.2f, -0.2f, 0.0f, 0.0f)};
        const Vertex4Tex4 UpperLeftVertex  = {tcu::Vec4(-size + xOffset, size, 0.0f, 1.0f),
                                              tcu::Vec4(-0.2f, 1.2f, 0.0f, 0.0f)};
        const Vertex4Tex4 UpperRightVertex = {tcu::Vec4(size + xOffset, size, 0.0f, 1.0f),
                                              tcu::Vec4(1.2f, 1.2f, 0.0f, 0.0f)};
...
static const tcu::Vec4 defaultTestColors[] =
    ...
    {tcu::Vec4(1.0f, 0.0f, 0.0f, 1.0f), tcu::Vec4(0.0f, 1.0f, 0.0f, 1.0f)};
...
PushDescriptorBufferGraphicsTestInstance::PushDescriptorBufferGraphicsTestInstance(Context &context,
                                                                                   const TestParams &params)
...
void PushDescriptorBufferGraphicsTestInstance::init(void)
{
    ...
    // Create color image
    {
        ...
        VK_CHECK(m_vkd.bindImageMemory(*m_device, *m_colorImage, m_colorImageAlloc->getMemory(),
    ...
    {
        for (uint32_t bufIdx = 0; bufIdx < DE_LENGTH_OF_ARRAY(defaultTestColors); bufIdx++)
        {
            const VkBufferUsageFlags usageFlags = m_params.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER ?
            ...
            m_bufferAllocs.push_back(
                AllocationSp(m_allocator
                                 .allocate(getBufferMemoryRequirements(m_vkd, *m_device, **m_buffers[bufIdx]),
                                           MemoryRequirement::HostVisible)
                                 .release()));
            VK_CHECK(m_vkd.bindBufferMemory(*m_device, **m_buffers[bufIdx], m_bufferAllocs[bufIdx]->getMemory(),
                                            m_bufferAllocs[bufIdx]->getOffset()));

            deMemcpy(m_bufferAllocs[bufIdx]->getHostPtr(), &defaultTestColors[bufIdx],
                     static_cast<size_t>(kSizeofVec4));
    ...
    // Create pipeline
    {
        ...
        m_graphicsPipeline.setDefaultRasterizationState()
            ...
            .setupPreRasterizationShaderState(viewports, scissors, *m_preRasterizationStatePipelineLayout,
                                              *m_renderPass, 0u, *m_vertexShaderModule)
    ...
    // Create vertex buffer
    {
        ...
        m_vertexBufferAlloc = m_allocator.allocate(getBufferMemoryRequirements(m_vkd, *m_device, *m_vertexBuffer),
                                                   MemoryRequirement::HostVisible);

        VK_CHECK(m_vkd.bindBufferMemory(*m_device, *m_vertexBuffer, m_vertexBufferAlloc->getMemory(),
    ...
    // Create command buffer
    {
        ...
        beginRenderPass(m_vkd, *m_cmdBuffer, *m_renderPass, *m_framebuffer,
        ...
        for (uint32_t quadNdx = 0; quadNdx < m_params.numCalls; quadNdx++)
        {
...
tcu::TestStatus PushDescriptorBufferGraphicsTestInstance::verifyImage(void)
{
    ...
    {
        for (uint32_t quadIdx = 0; quadIdx < m_params.numCalls; quadIdx++)
    ...
    // Compare result with reference image
    {
        de::MovePtr<tcu::TextureLevel> result = readColorAttachment(
            m_vkd, *m_device, m_queue, m_queueFamilyIndex, m_allocator, *m_colorImage, m_colorFormat, m_renderSize);
...
{
public:
    PushDescriptorBufferGraphicsTest(tcu::TestContext &testContext, const string &name, const string &description,
                                     const TestParams &params);
...
PushDescriptorBufferGraphicsTest::PushDescriptorBufferGraphicsTest(tcu::TestContext &testContext, const string &name,
                                                                   const string &description, const TestParams &params)
...
void PushDescriptorBufferGraphicsTest::initPrograms(SourceCollections &sourceCollections) const
{
    const string bufferType =
        m_params.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER ? "uniform" : "readonly buffer";
    const string vertexSrc = "#version 450\n"
                             "layout(location = 0) in highp vec4 position;\n"
                             "layout(location = 1) in highp vec4 color;\n"
                             "layout(location = 0) out highp vec4 vtxColor;\n"
                             "layout(set = 0, binding = " +
                             de::toString(m_params.binding) + ") " + bufferType +
                             " Block\n"
                             "{\n"
                             "    vec4 color;\n"
                             "} inputData;\n"
...
PushDescriptorBufferComputeTestInstance::PushDescriptorBufferComputeTestInstance(Context &context,
                                                                                 const TestParams &params)
    : vkt::TestInstance(context)
    , m_params(params)
    , m_vkp(context.getPlatformInterface())
...
void PushDescriptorBufferComputeTestInstance::init(void)
{
    ...
    for (uint32_t colorIdx = 0; colorIdx < m_params.numCalls; colorIdx++)
    {
        ...
        else
        {
            ...
            m_testColors[colorIdx] = defaultTestColors[0] * mix +
                                     defaultTestColors[DE_LENGTH_OF_ARRAY(defaultTestColors) - 1] * (1.0f - mix);
        }
    }

    // Create buffers. One color value in each buffer.
    {
        for (uint32_t bufIdx = 0; bufIdx < m_params.numCalls; bufIdx++)
        {
            const VkBufferUsageFlags usageFlags = m_params.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER ?
                                                      VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT :
                                                      VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
    ...
    // Create output buffer
    {
        ...
        m_outputBufferAlloc = m_allocator.allocate(getBufferMemoryRequirements(m_vkd, *m_device, *m_outputBuffer),
                                                   MemoryRequirement::HostVisible);
        VK_CHECK(m_vkd.bindBufferMemory(*m_device, *m_outputBuffer, m_outputBufferAlloc->getMemory(),
    ...
    // Create shader
    {
        m_computeShaderModule =
            createShaderModule(m_vkd, *m_device, m_context.getBinaryCollection().get("compute"), 0u);
    }
    ...
    // Create command buffer
    {
        ...
        for (uint32_t dispatchNdx = 0; dispatchNdx < m_params.numCalls; dispatchNdx++)
        {
...
tcu::TestStatus PushDescriptorBufferComputeTestInstance::verifyOutput(void)
{
    ...
    for (uint32_t i = 0; i < m_params.numCalls; ++i)
    {
...
{
public:
    PushDescriptorBufferComputeTest(tcu::TestContext &testContext, const string &name, const string &description,
                                    const TestParams &params);
...
PushDescriptorBufferComputeTest::PushDescriptorBufferComputeTest(tcu::TestContext &testContext, const string &name,
                                                                 const string &description, const TestParams &params)
    : vkt::TestCase(testContext, name, description)
    , m_params(params)
{
...
void PushDescriptorBufferComputeTest::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    const string computeSrc = "#version 450\n"
                              ...
                              "\n"
                              "layout(set = 0, binding = " +
                              de::toString(m_params.binding + 1) +
...
PushDescriptorImageGraphicsTestInstance::PushDescriptorImageGraphicsTestInstance(Context &context,
                                                                                 const TestParams &params)
    : vkt::TestInstance(context)
    , m_params(params)
    , m_vkp(context.getPlatformInterface())
...
void PushDescriptorImageGraphicsTestInstance::init(void)
{
    ...
    for (uint32_t texIdx = 0; texIdx < 2; texIdx++)
    {
        ...
        if (m_params.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLER ||
            ...
            m_params.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE)
        ...
        m_textureImageAllocs.push_back(
            AllocationSp(m_allocator
                             .allocate(getImageMemoryRequirements(m_vkd, *m_device, **m_textureImages.back()),
                                       MemoryRequirement::Any)
                             .release()));
        VK_CHECK(m_vkd.bindImageMemory(*m_device, **m_textureImages.back(), m_textureImageAllocs.back()->getMemory(),
                                       m_textureImageAllocs.back()->getOffset()));
    }
    ...
    for (uint32_t texIdx = 0; texIdx < 2; texIdx++)
    {
        ...
        m_textureViews.push_back(
            VkImageViewSp(new Unique<VkImageView>(createImageView(m_vkd, *m_device, &textureViewParams))));
    }
...
tcu::TestStatus PushDescriptorImageGraphicsTestInstance::verifyImage(void)
{
    ...
    // Render reference image
    {
        ...
        for (uint32_t quadIdx = 0; quadIdx < m_params.numCalls; quadIdx++)
            for (uint32_t vertexIdx = 0; vertexIdx < 6; vertexIdx++)
            {
                const uint32_t idx              = quadIdx * 6 + vertexIdx;
...
{
public:
    PushDescriptorImageGraphicsTest(tcu::TestContext &testContext, const string &name, const string &description,
                                    const TestParams &params);
...
PushDescriptorImageGraphicsTest::PushDescriptorImageGraphicsTest(tcu::TestContext &testContext, const string &name,
                                                                 const string &description, const TestParams &params)
    : vkt::TestCase(testContext, name, description)
    , m_params(params)
{
...
void PushDescriptorImageGraphicsTest::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    if (m_params.descriptorType == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
    {
        const string fragmentSrc = "#version 450\n"
                                   ...
                                   "layout(location = 0) out highp vec4 fragColor;\n"
                                   "layout(set = 0, binding = " +
                                   de::toString(m_params.binding) +
                                   ") uniform sampler2D combinedSampler;\n"
                                   "\n"
                                   "void main (void)\n"
                                   "{\n"
    ...
    else if (m_params.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
    {
        const string fragmentSrc = "#version 450\n"
                                   ...
                                   "layout(set = 0, binding = " +
                                   de::toString(m_params.binding) +
                                   ", rgba8) uniform readonly image2D storageImage;\n"
                                   "\n"
                                   "void main (void)\n"
                                   "{\n"
...
private:
    ...
    Move<VkCommandPool> m_cmdPool;
    Move<VkCommandBuffer> m_cmdBuffer;
    uint32_t m_outputBufferBinding;
};

PushDescriptorImageComputeTestInstance::PushDescriptorImageComputeTestInstance(Context &context,
                                                                               const TestParams &params)
    : vkt::TestInstance(context)
    , m_params(params)
    , m_vkp(context.getPlatformInterface())
...
tcu::TestStatus PushDescriptorImageComputeTestInstance::verifyOutput(void)
{
    ...
    for (uint32_t dispatchNdx = 0u; dispatchNdx < m_params.numCalls; ++dispatchNdx)
    {
        const auto refIdx = floatsPerDispatch * dispatchNdx;
        const auto bufferOffset =
            m_itemSize * dispatchNdx; // Each dispatch uses m_itemSize bytes in the buffer to meet alignment reqs.
        ...
        if (deMemCmp(&ref[refIdx], bufferDataPtr + bufferOffset, blockSize) != 0)
        {
            ...
            for (uint32_t i = 0; i < floatsPerDispatch; ++i)
                msg << ((i == 0) ? "[" : ", ") << refValues[i];
            msg << "]; Buffer ";
            for (uint32_t i = 0; i < floatsPerDispatch; ++i)
...
{
public:
    PushDescriptorImageComputeTest(tcu::TestContext &testContext, const string &name, const string &description,
                                   const TestParams &params);
...
PushDescriptorImageComputeTest::PushDescriptorImageComputeTest(tcu::TestContext &testContext, const string &name,
                                                               const string &description, const TestParams &params)
    : vkt::TestCase(testContext, name, description)
    , m_params(params)
{
...
void PushDescriptorImageComputeTest::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    else if (m_params.descriptorType == VK_DESCRIPTOR_TYPE_SAMPLER)
    {
        const string computeSrc = "#version 450\n"
                                  ...
                                  ") uniform texture2D texImage;\n"
                                  "layout(set = 0, binding = " +
                                  de::toString(m_params.binding + 2) +
                                  ") writeonly buffer Output\n"
                                  "{\n"
                                  "    vec4 innerColor;\n"
                                  "    vec4 outerColor;\n"
...
PushDescriptorTexelBufferGraphicsTestInstance::PushDescriptorTexelBufferGraphicsTestInstance(Context &context,
                                                                                             const TestParams &params)
    : vkt::TestInstance(context)
    , m_params(params)
    , m_vkp(context.getPlatformInterface())
...
void PushDescriptorTexelBufferGraphicsTestInstance::init(void)
{
    ...
    for (uint32_t bufIdx = 0; bufIdx < DE_LENGTH_OF_ARRAY(defaultTestColors); bufIdx++)
    {
        const VkBufferUsageFlags usageFlags = m_params.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER ?
    ...
    for (uint32_t bufIdx = 0; bufIdx < DE_LENGTH_OF_ARRAY(defaultTestColors); bufIdx++)
    {
        ...
        m_bufferViews.push_back(
            VkBufferViewSp(new Unique<VkBufferView>(createBufferView(m_vkd, *m_device, &bufferViewParams))));
    }
...
{
public:
    PushDescriptorTexelBufferGraphicsTest(tcu::TestContext &testContext, const string &name, const string &description,
                                          const TestParams &params);
...
void PushDescriptorTexelBufferGraphicsTest::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    if (m_params.descriptorType == VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER)
    {
        const string fragmentSrc = "#version 450\n"
                                   ...
                                   "layout(set = 0, binding = " +
                                   de::toString(m_params.binding) +
                                   ") uniform textureBuffer texelBuffer;\n"
                                   "\n"
                                   "void main (void)\n"
                                   "{\n"
    ...
    else
    {
        ...
        const string fragmentSrc = "#version 450\n"
                                   ...
                                   "layout(set = 0, binding = " +
                                   de::toString(m_params.binding) +
                                   ", rgba32f) uniform readonly imageBuffer texelBuffer;\n"
                                   "\n"
                                   "void main (void)\n"
                                   "{\n"
...
PushDescriptorTexelBufferComputeTestInstance::PushDescriptorTexelBufferComputeTestInstance(Context &context,
                                                                                           const TestParams &params)
    : vkt::TestInstance(context)
    , m_params(params)
    , m_vkp(context.getPlatformInterface())
...
{
public:
    PushDescriptorTexelBufferComputeTest(tcu::TestContext &testContext, const string &name, const string &description,
                                         const TestParams &params);
...
PushDescriptorInputAttachmentGraphicsTestInstance::PushDescriptorInputAttachmentGraphicsTestInstance(
    Context &context, const TestParams &params)
    : vkt::TestInstance(context)
    , m_params(params)
    , m_vkp(context.getPlatformInterface())
    , m_instanceExtensions(enumerateInstanceExtensionProperties(m_vkp, DE_NULL))
...
void PushDescriptorInputAttachmentGraphicsTestInstance::init(void)
{
    ...
    // Create input images
    for (uint32_t imageIdx = 0; imageIdx < 2; imageIdx++)
    {
        ...
        m_inputImageAllocs.push_back(AllocationSp(
            m_allocator
                .allocate(getImageMemoryRequirements(m_vkd, *m_device, **m_inputImages.back()), MemoryRequirement::Any)
                .release()));
        VK_CHECK(m_vkd.bindImageMemory(*m_device, **m_inputImages.back(), m_inputImageAllocs.back()->getMemory(),
                                       m_inputImageAllocs.back()->getOffset()));
    }
    ...
    for (uint32_t renderPassIdx = 0; renderPassIdx < 2; renderPassIdx++)
    {
        ...
        m_renderPasses.push_back(
            VkRenderPassSp(new Unique<VkRenderPass>(createRenderPass(m_vkd, *m_device, &renderPassInfo))));
    }

    // Create framebuffers
    for (uint32_t framebufferIdx = 0; framebufferIdx < 2; framebufferIdx++)
    {
        ...
        m_framebuffers.push_back(
            VkFramebufferSp(new Unique<VkFramebuffer>(createFramebuffer(m_vkd, *m_device, &framebufferParams))));
    }
    ...
    // Create pipelines
    for (uint32_t pipelineIdx = 0; pipelineIdx < 2; pipelineIdx++)
    {
        const VkVertexInputBindingDescription vertexInputBindingDescription = {
            0u,                         // uint32_t binding;
            sizeof(Vertex4Tex4),        // uint32_t strideInBytes;
        ...
        m_graphicsPipelines.back()
            ...
            .setupVertexInputState(&vertexInputStateParams)
            .setupPreRasterizationShaderState(viewports, scissors, *m_preRasterizationStatePipelineLayout,
                                              **m_renderPasses[pipelineIdx], 0u, *m_vertexShaderModule)
            .setupFragmentShaderState(*m_fragmentStatePipelineLayout, **m_renderPasses[pipelineIdx], 0u,
    ...
    // Create command buffer
    {
        ...
        for (uint32_t quadNdx = 0; quadNdx < m_params.numCalls; quadNdx++)
        {
            beginRenderPass(m_vkd, *m_cmdBuffer, **m_renderPasses[quadNdx], **m_framebuffers[quadNdx],
                            makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), attachmentClearValue);
            m_vkd.cmdBindPipeline(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                  m_graphicsPipelines[quadNdx].getPipeline());
            m_vkd.cmdBindVertexBuffers(*m_cmdBuffer, 0, 1, &m_vertexBuffer.get(), &vertexBufferOffset);
...
void PushDescriptorInputAttachmentGraphicsTest::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    const string fragmentSrc = "#version 450\n"
                               "layout(location = 0) in highp vec2 texcoordFrag;\n"
                               "layout(location = 0) out highp vec4 fragColor;\n"
                               "layout(input_attachment_index = 0, set = 0, binding = " +
                               de::toString(m_params.binding) +
                               ") uniform subpassInput inputColor;\n"
                               "\n"
                               "void main (void)\n"
                               "{\n"
...
{
    const TestParams params[] = {{pipelineType, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0u, 1u},
    ...
    de::MovePtr<tcu::TestCaseGroup> pushDescriptorTests(
        new tcu::TestCaseGroup(testCtx, "push_descriptor", "Push descriptor tests"));
    ...
    for (uint32_t testIdx = 0; testIdx < DE_LENGTH_OF_ARRAY(params); testIdx++)
    {
        string testName;
        testName +=
            "binding" + de::toString(params[testIdx].binding) + "_numcalls" + de::toString(params[testIdx].numCalls);
        ...
        case VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER:
            ...
            if (params[testIdx].numCalls <= 2)
                graphicsTests->addChild(
                    new PushDescriptorBufferGraphicsTest(testCtx, testName.c_str(), "", params[testIdx]));
            if (pipelineType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
                computeTests->addChild(
                    new PushDescriptorBufferComputeTest(testCtx, testName.c_str(), "", params[testIdx]));
            break;
        ...
        case VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER:
            ...
            graphicsTests->addChild(
                new PushDescriptorImageGraphicsTest(testCtx, testName.c_str(), "", params[testIdx]));
            if (pipelineType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
                computeTests->addChild(
                    new PushDescriptorImageComputeTest(testCtx, testName.c_str(), "", params[testIdx]));
            break;
        ...
        case VK_DESCRIPTOR_TYPE_UNIFORM_TEXEL_BUFFER:
            ...
            graphicsTests->addChild(
                new PushDescriptorTexelBufferGraphicsTest(testCtx, testName.c_str(), "", params[testIdx]));
            if (pipelineType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
                computeTests->addChild(
                    new PushDescriptorTexelBufferComputeTest(testCtx, testName.c_str(), "", params[testIdx]));
            break;
        ...
        case VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT:
            ...
            graphicsTests->addChild(
                new PushDescriptorInputAttachmentGraphicsTest(testCtx, testName.c_str(), "", params[testIdx]));
            break;
