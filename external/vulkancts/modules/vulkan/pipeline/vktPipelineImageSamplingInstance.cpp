    ...
    while (pNext != DE_NULL)
    {
        ...
        case VK_STRUCTURE_TYPE_SAMPLER_REDUCTION_MODE_CREATE_INFO_EXT:
        {
            VkPhysicalDeviceSamplerFilterMinmaxProperties physicalDeviceSamplerMinMaxProperties = {
                VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SAMPLER_FILTER_MINMAX_PROPERTIES_EXT, DE_NULL, false, false};
...
tcu::TestStatus ImageSamplingInstance::verifyImage(void)
{
    ...
    // Verify results
    {
        ...
        const bool allowSnorm8Bug =
            m_texture->getTextureFormat().type == tcu::TextureFormat::SNORM_INT8 &&
        ...
        for (int imgNdx = 0; imgNdx < m_imageCount; ++imgNdx)
        {
            ...
            if (!compareOk && allowSnorm8Bug)
            {
                ...
                compareOk = validateResultImage(*texture, m_imageViewType, subresource, sampler, m_componentMapping,
                                                coordAccess, lodBounds, relaxedPrecision, lookupScale, lookupBias,
