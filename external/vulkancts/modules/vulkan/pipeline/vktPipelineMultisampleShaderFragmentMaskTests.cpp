...
{
    SingletonDevice(Context &context, PipelineConstructionType pipelineType) : m_context(context), m_logicalDevice()
    {
        ...
        auto extensionProperties = enumerateDeviceExtensionProperties(m_context.getInstanceInterface(),
                                                                      m_context.getPhysicalDevice(), nullptr);
        ...
        DE_ASSERT(!de::contains(std::begin(enabledDeviceExtensions), std::end(enabledDeviceExtensions),
        ...
        m_logicalDevice =
            createCustomDevice(m_context.getTestContext().getCommandLine().isValidationEnabled(),
                               ...
                               m_context.getInstanceInterface(), m_context.getPhysicalDevice(), &createInfo, nullptr);
...
public:
    ...
    {
        return getDeviceQueue(getDeviceInterface(context, pipelineType), getDevice(context, pipelineType),
                              context.getUniversalQueueFamilyIndex(), 0);
    }
...
void drawAndSampleInputAttachment(Context &context, const TestParams &params, WorkingData &wd)
{
    ...
    GraphicsPipelineWrapper pipelineSample(vk, device, params.pipelineConstructionType);
    {
        ...
        pipelineSample.setDefaultRasterizationState()
            .setDefaultDepthStencilState()
            .setupVertexInputState(&vertexInputStateInfo)
            .setupPreRasterizationShaderState(viewports, scissors, *pipelineLayout, *renderPass, 1u,
                                              *vertexModuleSample)
    ...
    submitCommandsAndWait(vk, device, SingletonDevice::getUniversalQueue(context, params.pipelineConstructionType),
                          *cmdBuffer);
                                                             ...
                                                             PipelineConstructionType pipelineConstructionType)
{
    return createTestGroup(testCtx, "shader_fragment_mask", "Access raw texel values in a compressed MSAA surface",
                           createShaderFragmentMaskTestsInGroup, pipelineConstructionType,
                           [](tcu::TestCaseGroup *, PipelineConstructionType) { SingletonDevice::destroy(); });
}
