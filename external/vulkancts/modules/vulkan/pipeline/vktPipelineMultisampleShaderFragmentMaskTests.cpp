...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const bool useVertexInput, const VkFormat vertexAttribColorFormat,
                                      const bool useColorAttachment, const uint32_t subpassNdx, const UVec2 &renderSize,
                                      const VkSampleCountFlagBits numSamples)
{
    ...
    if (useVertexInput)
    {
        ...
        vertexInputAttributeDescriptions.push_back(
            makeVertexInputAttributeDescription(1u, 0u, vertexAttribColorFormat, sizeof(Vec4)));
    }
    ...
    const VkPipelineColorBlendStateCreateInfo pipelineColorBlendStateInfo = {
        ...
        (useColorAttachment ? 1u : 0u),                           // uint32_t attachmentCount;
...
struct TestParams
{
    ...
    TestParams(void) : numLayers(), numColorSamples(), colorFormat()
    {
    }
};
...
void initPrograms(SourceCollections &programCollection, const TestParams params)
{
    ...
    if (params.sampleSource == SAMPLE_SOURCE_SUBPASS_INPUT)
    {
        ...
        src << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
            ...
            << "} sb_out;\n"
            << "layout(input_attachment_index = " << params.numLayers << ", set = 0, binding = 2) uniform "
            << samplerPrefix << "subpassInputMS"
            << " input_attach;\n"
            << "\n"
            << "void main(void)\n"
            << "{\n"
...
void drawAndSampleInputAttachment(Context &context, const TestParams &params, WorkingData &wd)
{
    ...
    const Unique<VkShaderModule> vertexModuleDraw(
        createShaderModule(vk, device, context.getBinaryCollection().get("vert"), 0u));
    const Unique<VkShaderModule> fragmentModuleDraw(
        createShaderModule(vk, device, context.getBinaryCollection().get("frag"), 0u));

    // Create pipelines for MS draw
    const Unique<VkPipelineLayout> pipelineLayout(makePipelineLayout(vk, device, *descriptorSetLayout));
    const Unique<VkPipeline> pipelineDraw(
        makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModuleDraw, *fragmentModuleDraw,
                             true /*use vertex attribs*/, getVertexInputColorFormat(params.colorFormat),
                             true /*use color attach*/, 0u /*subpass*/, params.renderSize, params.numColorSamples));

    // Sampling pass is single-sampled, output to storage buffer
    const Unique<VkShaderModule> vertexModuleSample(
        createShaderModule(vk, device, context.getBinaryCollection().get("vert_full"), 0u));
    const Unique<VkShaderModule> fragmentModuleSample(
        createShaderModule(vk, device, context.getBinaryCollection().get("frag_fmask_fetch"), 0u));

    // Sampling pipeline
    const Unique<VkPipeline> pipelineSample(
        makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModuleSample, *fragmentModuleSample,
                             false /*use vertex attribs*/, VK_FORMAT_UNDEFINED, false /*no color output*/,
                             1u /*subpass*/, params.renderSize, VK_SAMPLE_COUNT_1_BIT));
...
void draw(Context &context, const TestParams &params, WorkingData &wd)
{
    ...
    for (uint32_t layerNdx = 0u; layerNdx < params.numLayers; ++layerNdx)
    {
        pipelines.push_back(PipelineSp(new Unique<VkPipeline>(makeGraphicsPipeline(
            vk, device, *pipelineLayout, *renderPass, *vertexModuleDraw, *fragmentModuleDraw,
            true /*use vertex attribs*/, getVertexInputColorFormat(params.colorFormat), true /*use color attachment*/,
            layerNdx /*subpass*/, params.renderSize, params.numColorSamples))));
    }
...
{
    return createTestGroup(testCtx, "shader_fragment_mask", "Access raw texel values in a compressed MSAA surface",
