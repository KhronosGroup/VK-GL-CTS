...
{
public:
    PushConstantGraphicsTestInstance(Context &context, const uint32_t rangeCount,
    ...
    void createShaderModule(const DeviceInterface &vk, VkDevice device, const BinaryCollection &programCollection,
                            const char *name, Move<VkShaderModule> *module);
...
PushConstantGraphicsTestInstance::PushConstantGraphicsTestInstance(
    Context &context, const uint32_t rangeCount, const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
    bool multipleUpdate, IndexType indexType)
...
void PushConstantGraphicsTestInstance::init(void)
{
    ...
    // Create pipeline
    {
        ...
        m_graphicsPipelines = makeGraphicsPipeline(
            ...
            *m_vertexShaderModule, // const VkShaderModule                          vertexShaderModule
            useTessellation ? *m_tessControlShaderModule :
                              DE_NULL, // const VkShaderModule                          tessellationControlShaderModule
            useTessellation ? *m_tessEvaluationShaderModule :
                              DE_NULL, // const VkShaderModule                          tessellationEvalShaderModule
            useGeometry ? *m_geometryShaderModule :
    ...
    // Create command buffer
    {
        ...
        for (int triangleNdx = 0; triangleNdx < TRIANGLE_COUNT; triangleNdx++)
        {
            ...
            if (m_multipleUpdate)
                vk.cmdPushConstants(*m_cmdBuffer, *m_pipelineLayout, m_pushConstantRange[0].range.shaderStage,
                                    m_pushConstantRange[0].range.offset, m_pushConstantRange[0].range.size,
                                    &triangleNdx);

            vk.cmdBindPipeline(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_graphicsPipelines);
            vk.cmdBindVertexBuffers(*m_cmdBuffer, 0, 1, &m_vertexBuffer.get(), &vertexBufferOffset);
            vk.cmdBindDescriptorSets(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayout, 0, 1,
...
{
public:
    PushConstantGraphicsDisjointInstance(Context &context, const uint32_t rangeCount,
                                         const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
                                         const bool multipleUpdate, const IndexType indexType);
                                                               ...
                                                               VkPipelineLayout pipelineLayout)
{
    ...
    for (size_t rangeNdx = 0; rangeNdx < m_rangeCount; rangeNdx++)
    {
        ...
        vk.cmdPushConstants(cmdBuffer, pipelineLayout, m_pushConstantRange[rangeNdx].range.shaderStage,
                            m_pushConstantRange[rangeNdx].range.offset, m_pushConstantRange[rangeNdx].range.size,
                            value);
        ...
        if (m_pushConstantRange[rangeNdx].update.size < m_pushConstantRange[rangeNdx].range.size)
        {
            ...
            vk.cmdPushConstants(cmdBuffer, pipelineLayout, m_pushConstantRange[rangeNdx].range.shaderStage,
                                m_pushConstantRange[rangeNdx].update.offset, m_pushConstantRange[rangeNdx].update.size,
                                value);
        }
    }
}
...
{
public:
    PushConstantGraphicsOverlapTestInstance(Context &context, const uint32_t rangeCount,
                                            const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
                                            const bool multipleUpdate, const IndexType indexType);
                                                                  ...
                                                                  VkPipelineLayout pipelineLayout)
{
    ...
    for (uint32_t pushNdx = 0u; pushNdx < pushConstantUpdates.size(); pushNdx++)
    {
        ...
        vk.cmdPushConstants(cmdBuffer, pipelineLayout, pushConstantUpdates[pushNdx].stageFlags,
                            pushConstantUpdates[pushNdx].offset, pushConstantUpdates[pushNdx].size,
                            &m_colorData[pushConstantUpdates[pushNdx].offset / 2u]);
...
{
public:
    PushConstantGraphicsTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                             const uint32_t rangeCount, const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
                             const bool multipleUpdate, const IndexType indexType);
...
PushConstantGraphicsTest::PushConstantGraphicsTest(tcu::TestContext &testContext, const std::string &name,
                                                   const std::string &description, const uint32_t rangeCount,
                                                   const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
                                                   const bool multipleUpdate, const IndexType indexType)
...
PushConstantGraphicsDisjointTest::PushConstantGraphicsDisjointTest(
    tcu::TestContext &testContext, const std::string &name, const std::string &description, const uint32_t rangeCount,
...
{
    return new PushConstantGraphicsDisjointInstance(context, m_rangeCount, m_pushConstantRange, m_multipleUpdate,
                                                    m_indexType);
}
...
PushConstantGraphicsOverlapTest::PushConstantGraphicsOverlapTest(
    tcu::TestContext &testContext, const std::string &name, const std::string &description, const uint32_t rangeCount,
    const PushConstantData pushConstantRange[MAX_RANGE_COUNT])
    : PushConstantGraphicsTest(testContext, name, description, rangeCount, pushConstantRange, false,
    ...
    for (size_t rangeNdx = 0; rangeNdx < m_rangeCount; rangeNdx++)
    {
        if (m_pushConstantRange[rangeNdx].range.shaderStage & VK_SHADER_STAGE_VERTEX_BIT)
        {
            const std::string source = "#version 450\n"
                                       ...
                                       "    vec4 color = inColor;\n"
                                       "    color" +
                                       getColorReadStr(m_pushConstantRange[rangeNdx].range.size) +
                                       "    vtxColor = color;\n"
                                       "}\n";
...
{
    return new PushConstantGraphicsOverlapTestInstance(context, m_rangeCount, m_pushConstantRange, false,
                                                       INDEX_TYPE_CONST_LITERAL);
}
...
{
public:
    PushConstantComputeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                            const ComputeTestType testType, const PushConstantData pushConstantRange);
...
PushConstantComputeTest::PushConstantComputeTest(tcu::TestContext &testContext, const std::string &name,
                                                 const std::string &description, const ComputeTestType testType,
                                                 const PushConstantData pushConstantRange)
    : vkt::TestCase(testContext, name, description)
    , m_testType(testType)
...
{
public:
    PushConstantLifetimeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                             const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
...
{
public:
    PushConstantLifetimeTestInstance(Context &context, const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
                                     const std::vector<CommandData> &cmdList);
...
PushConstantLifetimeTestInstance::PushConstantLifetimeTestInstance(
    Context &context, const PushConstantData pushConstantRange[MAX_RANGE_COUNT],
    const std::vector<CommandData> &cmdList)
    : vkt::TestInstance(context)
    , m_cmdList(cmdList)
    , m_renderSize(32, 32)
...
void PushConstantLifetimeTestInstance::init(void)
{
    ...
    // Create graphics pipelines
    {
        ...
        m_graphicsPipeline[0] = makeGraphicsPipeline(
            vk,                       // const DeviceInterface&                        vk
            vkDevice,                 // const VkDevice                                device
        ...
        m_graphicsPipeline[1] = makeGraphicsPipeline(
            vk,                       // const DeviceInterface&                        vk
            vkDevice,                 // const VkDevice                                device
        ...
        for (size_t ndx = 0; ndx < m_cmdList.size(); ndx++)
        {
            ...
            case CMD_DRAW:
            {
                ...
                vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                                      VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0, 0, DE_NULL, 0, DE_NULL, 1,
                                      &prePassBarrier);
                ...
                vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      0, 0, DE_NULL, 0, DE_NULL, 1, &postPassBarrier);
            ...
            case CMD_DISPATCH:
            {
                ...
                vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0,
                                      0, DE_NULL, 1, &outputBarrier, 0, DE_NULL);
...
{
public:
    OverwriteTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
OverwriteTestCase::OverwriteTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const OverwriteTestParams &params)
...
tcu::TestStatus OverwriteTestInstance::iterate(void)
{
    ...
    else
    {
        ...
        framebuffer =
            makeFramebuffer(vkd, device, renderPass.get(), 0u, nullptr, imageExtent.width, imageExtent.height);
        pipeline = makeGraphicsPipeline(vkd, device, pipelineLayout.get(), vertModule.get(), DE_NULL, DE_NULL, DE_NULL,
                                        fragModule.get(), renderPass.get(), viewports, scissors,
                                        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, 0u, 0u, &inputState);
    }
...
tcu::TestCaseGroup *createPushConstantTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> pushConstantTests(
        new tcu::TestCaseGroup(testCtx, "push_constant", "PushConstant tests"));

    de::MovePtr<tcu::TestCaseGroup> graphicsTests(
        new tcu::TestCaseGroup(testCtx, "graphics_pipeline", "graphics pipeline"));
    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(graphicsParams); ndx++)
    {
        graphicsTests->addChild(new PushConstantGraphicsDisjointTest(
            testCtx, graphicsParams[ndx].name, graphicsParams[ndx].description, graphicsParams[ndx].count,
            graphicsParams[ndx].range, graphicsParams[ndx].hasMultipleUpdates, graphicsParams[ndx].indexType));
    }

    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(overlapGraphicsParams); ndx++)
    {
        graphicsTests->addChild(new PushConstantGraphicsOverlapTest(
            testCtx, overlapGraphicsParams[ndx].name, overlapGraphicsParams[ndx].description,
            overlapGraphicsParams[ndx].count, overlapGraphicsParams[ndx].range));
    }
    ...
    de::MovePtr<tcu::TestCaseGroup> computeTests(
        new tcu::TestCaseGroup(testCtx, "compute_pipeline", "compute pipeline"));
    for (const auto &params : computeParams)
    {
        computeTests->addChild(
            new PushConstantComputeTest(testCtx, params.name, params.description, params.type, params.range));
    }
    ...
    for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(lifetimeParams); ndx++)
    {
        lifetimeTests->addChild(new PushConstantLifetimeTest(testCtx, lifetimeParams[ndx].name,
                                                             lifetimeParams[ndx].description, lifetimeParams[ndx].range,
