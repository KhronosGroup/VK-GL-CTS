...
MovePtr<GraphicsPipelineWrapper> makeGraphicsPipeline(
    const DeviceInterface &vk, const VkDevice device, const PipelineConstructionType pipelineConstructionType,
    const VkPipelineLayout pipelineLayout, const VkRenderPass renderPass,
    VkPipelineRenderingCreateInfoKHR *pipelineRenderingCreateInfo, const VkShaderModule vertexModule,
    const VkShaderModule fragmentModule, const bool enableBlend, const bool enableDepthStencilWrite,
    const bool enableDepthTest, const uint32_t intWriteMask, const uint32_t subpassNdx,
    const int32_t integerAttachmentLocation, const UVec4 &viewportIn, const UVec4 &scissorIn,
    const VkSampleCountFlagBits numSamples, const bool garbageAttachment)
{
    ...
    MovePtr<GraphicsPipelineWrapper> graphicsPipeline =
        MovePtr<GraphicsPipelineWrapper>(new GraphicsPipelineWrapper(vk, device, pipelineConstructionType, 0u));
    graphicsPipeline.get()
        ->setMonolithicPipelineLayout(pipelineLayout)
        .setupVertexInputState(&vertexInputStateInfo, &pipelineInputAssemblyStateInfo)
        .setupPreRasterizationShaderState(viewports, scissors, pipelineLayout, renderPass, subpassNdx, vertexModule,
                                          &pipelineRasterizationStateInfo, DE_NULL, DE_NULL, DE_NULL, DE_NULL, nullptr,
                                          garbageAttachment ? &pipelineRenderingCreateInfoWithGarbage :
...
void initializeAttachments(const TestParams &params, WorkingData &wd, std::vector<VkImageView> &attachments,
                           const size_t passNdx, int32_t attachmentNdxes[8])
{
                                    ...
                                    const std::vector<VkSubpassDependency2> &subpassDependencies)
{
    ...
    testObjects.framebuffers.emplace_back(
        makeFramebuffer(vk, device, *testObjects.renderPasses.back(), static_cast<uint32_t>(attachments.size()),
                        dataOrNullPtr(attachments), wd.framebufferSize.x(), wd.framebufferSize.y()));
}
...
void clearImagesBeforeDraw(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
    ...
    vk.cmdClearColorImage(*testObjects.cmdBuffer, *wd.floatColor1.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
void drawBasic(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
...

    {
        ...
        const Unique<VkPipelineLayout> pipelineLayout(
            makePipelineLayout(vk, device, 0, DE_NULL, 1, &pushConstantRange));

        testObjects.graphicsPipelines.push_back(pipeline::makeGraphicsPipeline(
            vk, device, params.pipelineConstructionType, *pipelineLayout,
            params.dynamicRendering ? DE_NULL : *testObjects.renderPasses.back(),
            params.dynamicRendering ? &pipelineRenderingCreateInfo : DE_NULL, *vertexModule, *fragmentModule, false,
            true, false, 0, 0, params.perPass[0].intColorLocation, wd.renderArea, wd.renderArea,
        ...
        vk.cmdPushConstants(*testObjects.cmdBuffer, *pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(UVec4),
                            &wd.renderArea);
        vk.cmdBindPipeline(*testObjects.cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                           (*testObjects.graphicsPipelines.back()).getPipeline());
...
void drawClearAttachments(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
...

    {
        ...
        testObjects.graphicsPipelines.push_back(pipeline::makeGraphicsPipeline(
            ...
            params.dynamicRendering ? DE_NULL : *testObjects.renderPasses.back(),
            params.dynamicRendering ? &pipelineRenderingCreateInfo : DE_NULL, *vertexModule, *fragmentModule, false,
            true, false, 0, 0, params.perPass[0].intColorLocation, regions[0], regions[0], params.perPass[0].numSamples,
        ...
        testObjects.graphicsPipelines.push_back(
            makeGraphicsPipeline(vk, device, params.pipelineConstructionType, *pipelineLayout,
                                 params.dynamicRendering ? DE_NULL : *testObjects.renderPasses.back(),
                                 params.dynamicRendering ? &pipelineRenderingCreateInfo : DE_NULL, *vertexModule,
                                 *fragmentModule, false, true, false, 0, 0, params.perPass[0].intColorLocation,
                                 regions[1], regions[1], params.perPass[0].numSamples, params.useGarbageAttachment));
...
void drawOnePass(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects,
                 const Unique<VkShaderModule> &vertexModule, const Unique<VkPipelineLayout> &pipelineLayout,
                 const uint32_t passNdx, const uint32_t subpassNdx, UVec4 regions[RegionCount],
                 VkPipelineRenderingCreateInfo *pipelineRenderingCreateInfo)
{
    const DeviceInterface &vk = context.getDeviceInterface();
    const VkDevice device     = context.getDevice();
    ...
    const Unique<VkShaderModule> fragmentModule(
        createShaderModule(vk, device, context.getBinaryCollection().get(fragName.str().c_str()), 0u));
    ...
    // Create a render pass and a framebuffer
    {
        ...
        for (uint32_t passNdx = 0; passNdx < numSubpasses; ++passNdx)
        {
            addSubpassDescription(params, passNdx, attachmentReferences[passNdx], resolveAttachmentReferences[passNdx],
                                  depthStencilResolve, &preserveAttachments[passNdx], msrtss[passNdx], subpasses, {},
                                  attachmentNdxes);
...
void drawInputAttachments(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
    ...
    // Create a render pass and a framebuffer
    {
        ...
        for (uint32_t passNdx = 0; passNdx < numSubpasses; ++passNdx)
        {
            ...
            addSubpassDescription(params, passNdx, attachmentReferences[passNdx], resolveAttachmentReferences[passNdx],
                                  depthStencilResolve, &preserveAttachments[passNdx], msrtss[passNdx], subpasses,
                                  passNdx == 0 ? noInputAttachments : inputAttachmentReferences, attachmentNdxes);
        }
    ...
    const Unique<VkShaderModule> vertexModule(
        createShaderModule(vk, device, context.getBinaryCollection().get("vert"), 0u));
    const Unique<VkShaderModule> fragmentModule0(
        createShaderModule(vk, device, context.getBinaryCollection().get("frag_0"), 0u));
    const Unique<VkShaderModule> fragmentModule1(
        createShaderModule(vk, device, context.getBinaryCollection().get("frag_1"), 0u));
    const Unique<VkPipelineLayout> pipelineLayout(makePipelineLayout(vk, device, 0, DE_NULL, 1, &pushConstantRange));
    ...
    const Unique<VkShaderModule> fragmentModuleIn(
        createShaderModule(vk, device, context.getBinaryCollection().get("frag_in"), 0u));
    const Unique<VkPipelineLayout> inputPipelineLayout(
        makePipelineLayout(vk, device, 1, &*descriptorSetLayout, 1, &inputPushConstantRange));
...

    {
        ...
        {
            testObjects.graphicsPipelines.push_back(pipeline::makeGraphicsPipeline(
                vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(), DE_NULL,
                *vertexModule, *fragmentModule0, false, true, false, 0, 0, params.perPass[0].intColorLocation,
                regions[regionNdx], regions[regionNdx], params.perPass[0].numSamples, params.useGarbageAttachment));
        ...
        {
            testObjects.graphicsPipelines.push_back(pipeline::makeGraphicsPipeline(
                vk, device, params.pipelineConstructionType, *inputPipelineLayout, *testObjects.renderPasses.back(),
                DE_NULL, *vertexModule, *fragmentModuleIn, false, false, false, 0, 1,
        ...
        for (uint32_t regionNdx = 0; regionNdx < RegionCount; ++regionNdx)
        {
            testObjects.graphicsPipelines.push_back(pipeline::makeGraphicsPipeline(
                vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(), DE_NULL,
                *vertexModule, *fragmentModule1, true, false, false, 0xC, 1, params.perPass[1].intColorLocation,
                regions[regionNdx], regions[regionNdx], params.perPass[1].numSamples, params.useGarbageAttachment));
                                    ...
                                    PipelineConstructionType pipelineConstructionType, const bool dynamicRendering)
{
    ...
    {
        MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
            rootGroup->getTestContext(), "basic", "Tests that NxMSAA rendering actually renders to N samples"));
            ...
            for (const VkFormat color2Format : color2FormatRange)
                for (const VkFormat color3Format : color3FormatRange)
                    for (const VkFormat depthStencilFormat : depthStencilFormatRange)
                    {
                        ...
                        {
                            MovePtr<tcu::TestCaseGroup> sampleGroup(
                                new tcu::TestCaseGroup(rootGroup->getTestContext(),
                                                       getSampleCountCaseName(sampleCount).c_str(), "Sample count"));
    ...
    // Test 2: Test that vkCmdClearAttachments works.
    {
        MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(rootGroup->getTestContext(), "clear_attachments",
                                                                 "Tests that vkCmdClearAttachments works"));
    ...
    if (!dynamicRendering)
    {
        ...
        for (uint32_t iteration = 0; iteration < (isMultisampledRenderToSingleSampled ? 1000 : 250); ++iteration)
        {
            TestParams testParams;
            deMemset(&testParams, 0, sizeof(testParams));
    ...
    // Test 4: Tests with a multiple render passes, a single subpass each.
    {
        MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(rootGroup->getTestContext(), "multi_renderpass",
                                                                 "Multiple render passes with a single subpass each"));
        MovePtr<tcu::TestCaseGroup> formatGroup[color1FormatCount][color2FormatCount][color3FormatCount]
                                               [depthStencilFormatCount];
    ...
    if (!dynamicRendering)
    {
        MovePtr<tcu::TestCaseGroup> group(
            new tcu::TestCaseGroup(rootGroup->getTestContext(), "input_attachments",
        ...
        !dynamicRendering)
    {
        MovePtr<tcu::TestCaseGroup> group(
            new tcu::TestCaseGroup(rootGroup->getTestContext(), "subpass_resolve_efficiency_query",
                                   "Tests that subpass resolve efficiency performance query works"));
    ...
    {
        MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
            rootGroup->getTestContext(), "garbage_color_attachment", "Tests that work with garbage color attachments"));
                ...
                for (const VkFormat color3Format : color3FormatRange)
                    for (const VkFormat depthStencilFormat : depthStencilFormatRange)
                    {
                        ...
                        addFunctionCaseWithPrograms(
                            group.get(),
                            getFormatCaseName(color1Format, color2Format, color3Format, depthStencilFormat).c_str(),
                            "Combination of framebuffer attachment formats", checkRequirements, initBasicPrograms,
                            testBasic, testParams);
                    }
                                                         ...
                                                         PipelineConstructionType pipelineConstructionType)
{
    ...
    MovePtr<tcu::TestCaseGroup> dynamicRenderingGroup(
        new tcu::TestCaseGroup(rootGroup->getTestContext(), "dynamic_rendering",
                               "Multisampled rendering to single-sampled tests with dynamic rendering"));
                                        ...
                                        PipelineConstructionType pipelineConstructionType)
{
    ...
    MovePtr<tcu::TestCaseGroup> dynamicRenderingGroup(
        new tcu::TestCaseGroup(rootGroup->getTestContext(), "dynamic_rendering",
                               "Miscellaneous multisampled rendering tests with dynamic rendering"));
...
tcu::TestCaseGroup *createMultisampledMiscTests(tcu::TestContext &testCtx,
                                                vk::PipelineConstructionType pipelineConstructionType)
{
    return createTestGroup(testCtx, "misc", "Miscellaneous multisampled rendering tests",
                           createMultisampledMiscTestsInGroup, pipelineConstructionType);
}
