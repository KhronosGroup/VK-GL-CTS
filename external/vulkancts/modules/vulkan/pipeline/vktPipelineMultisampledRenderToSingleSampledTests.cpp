...
MovePtr<GraphicsPipelineWrapper> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                                      const PipelineConstructionType pipelineConstructionType,
                                                      const VkPipelineLayout pipelineLayout,
                                                      const VkRenderPass renderPass, const VkShaderModule vertexModule,
                                                      const VkShaderModule fragmentModule, const bool enableBlend,
                                                      const bool enableDepthStencilWrite, const bool enableDepthTest,
                                                      const uint32_t intWriteMask, const uint32_t subpassNdx,
                                                      const int32_t integerAttachmentLocation, const UVec4 &viewportIn,
                                                      const UVec4 &scissorIn, const VkSampleCountFlagBits numSamples)
{
    ...
    graphicsPipeline.get()
        ...
        .setupVertexInputStete(&vertexInputStateInfo, &pipelineInputAssemblyStateInfo)
...
void drawBasic(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
...

    {
        ...
        testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
            vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(),
            *vertexModule, *fragmentModule, false, true, false, 0, 0, params.perPass[0].intColorLocation, wd.renderArea,
            wd.renderArea, params.perPass[0].numSamples));
...
void drawClearAttachments(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
...

    {
        ...
        testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
            vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(),
            *vertexModule, *fragmentModule, false, true, false, 0, 0, params.perPass[0].intColorLocation, regions[0],
            regions[0], params.perPass[0].numSamples));
        ...
        testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
            vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(),
            *vertexModule, *fragmentModule, false, true, false, 0, 0, params.perPass[0].intColorLocation, regions[1],
            regions[1], params.perPass[0].numSamples));
                 ...
                 const uint32_t passNdx, const uint32_t subpassNdx, UVec4 regions[RegionCount])
{
    ...
    for (uint32_t regionNdx = 0; regionNdx < RegionCount; ++regionNdx)
    {
        testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
            vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(),
            *vertexModule, *fragmentModule, true, true, false, 1 << passNdx, subpassNdx, perPass.intColorLocation,
            regions[regionNdx], regions[regionNdx], perPass.numSamples));
        ...
        if (perPass.hasDepthStencil)
        {
            testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
                vk, device, params.pipelineConstructionType, *pipelineLayout, *testObjects.renderPasses.back(),
                *vertexModule, *fragmentModule, true, false, true, 1 << passNdx, subpassNdx, perPass.intColorLocation,
                regions[regionNdx], regions[regionNdx], perPass.numSamples));
...
void drawSingleRenderPass(Context &context, const TestParams &params, WorkingData &wd, TestObjects &testObjects)
{
    ...
    for (uint32_t passNdx = 0; passNdx < numSubpasses; ++passNdx)
    {
        ...
        drawOnePass(context, params, wd, testObjects, vertexModule, pipelineLayout, passNdx, passNdx, regions);
    }
...

    {
        ...
        for (uint32_t regionNdx = 0; regionNdx < RegionCount; ++regionNdx)
        {
            testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
                ...
                *vertexModule, *fragmentModule0, false, true, false, 0, 0, params.perPass[0].intColorLocation,
                regions[regionNdx], regions[regionNdx], params.perPass[0].numSamples));
        ...
        for (uint32_t regionNdx = 0; regionNdx < RegionCount; ++regionNdx)
        {
            testObjects.graphicsPipelines.push_back(makeGraphicsPipeline(
                ...
                *vertexModule, *fragmentModuleIn, false, false, false, 0, 1, params.perPass[1].intColorLocation,
                regions[regionNdx], regions[regionNdx], params.perPass[1].numSamples));
