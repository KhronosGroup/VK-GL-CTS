...
{
public:
    NoPositionCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
NoPositionCase::NoPositionCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                               const TestParams &params)
...
void NoPositionCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    if (m_params.useSSBO)
    {
        ...
        for (size_t i = 0; i < writeStrings.size(); ++i)
            *writeStrings[i] = "    atomicAdd(ssbo.counters[" + de::toString(i) +
                               (multiview ? (" + uint(gl_ViewIndex) * " + stageCountStr) : "") + "], 1u);\n";
    }
...
tcu::TestStatus NoPositionInstance::iterate(void)
{
    ...
    const auto colorImageView =
        makeImageView(vkd, device, colorImage.get(), VK_IMAGE_VIEW_TYPE_2D, format, subresourceRange);
    ...
    const auto framebuffer =
        makeFramebuffer(vkd, device, renderPass.get(), colorImageView.get(), extent.width, extent.height);
    ...
    const uint32_t patchControlPoints = (tess ? 3u : 0u);
    const auto primitiveTopology      = (tess ? VK_PRIMITIVE_TOPOLOGY_PATCH_LIST : VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);

    const auto pipeline = makeGraphicsPipeline(vkd, device, pipelineLayout.get(), vert.get(), tesc.get(), tese.get(),
                                               geom.get(), frag.get(), renderPass.get(), viewports, scissors,
                                               primitiveTopology, 0u /* Subpass */, patchControlPoints);
    ...
    if (m_params.useSSBO)
    {
        ...
        std::vector<uint32_t> ssboCounters;
        ...
        for (uint32_t viewIdx = 0u; viewIdx < m_params.numViews; ++viewIdx)
            for (uint32_t stageIdx = 0u; stageIdx < kStageCount; ++stageIdx)
            {
                ...
                const uint32_t storedVal = ssboCounters[stageIdx + viewIdx * kStageCount];
...
tcu::TestCaseGroup *createNoPositionTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
        testCtx, "no_position", "Tests with shaders that do not write to the Position built-in"));
    ...
    for (int aux = 0; aux < 2; ++aux)
    {
        ...
        for (int aux2 = 0; aux2 < 2; ++aux2)
        {
            ...
            for (uint32_t viewCount = 1u; viewCount <= 2u; ++viewCount)
            {
