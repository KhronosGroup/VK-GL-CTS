...
protected:
    void prepareRenderPass(RenderPassWrapper &renderPass, GraphicsPipelineWrapper &pipeline);
...
protected:
    ...
    ShaderWrapper m_vertModule;
    ShaderWrapper m_fragModule;
};
...
void DepthRangeUnrestrictedTestInstance::prepareRenderPass(RenderPassWrapper &renderPass,
                                                           GraphicsPipelineWrapper &pipeline)
{
    ...
    renderPass.begin(vk, *m_cmdBuffer, makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), 2u, attachmentClearValues);
    ...
    if (m_param.viewportDepthBoundsMode & TEST_MODE_VIEWPORT_DYNAMIC)
    {
        ...
        if (vk::isConstructionTypeShaderObject(m_param.pipelineConstructionType))
        {
            ...
            vk.cmdSetViewportWithCount(*m_cmdBuffer, 1u, &viewport);
#else
            vk.cmdSetViewportWithCountEXT(*m_cmdBuffer, 1u, &viewport);
#endif
        }
        else
        {
                 ...
                 m_context.getDevice(), m_context.getDeviceExtensions(), param.pipelineConstructionType)
{
    ...
    // Create render pass
    m_renderPass = makeRenderPass(vk, vkDevice, m_param.pipelineConstructionType, m_colorFormat, m_param.depthFormat,
    ...
    m_vertModule = ShaderWrapper(vk, vkDevice, context.getBinaryCollection().get("vert"), 0);
    m_fragModule = ShaderWrapper(vk, vkDevice, context.getBinaryCollection().get("frag"), 0);
...
protected:
    RenderPassWrapper m_renderPassSecondDraw;
...
DepthBoundsRangeUnrestrictedTestInstance::DepthBoundsRangeUnrestrictedTestInstance(
    ...
    , m_pipelineSecondDraw(m_context.getInstanceInterface(), m_context.getDeviceInterface(),
                           ...
                           param.pipelineConstructionType)
{
    ...
    m_renderPassSecondDraw =
        makeRenderPass(vk, vkDevice, param.pipelineConstructionType, m_colorFormat, m_param.depthFormat,
    ...
    // Create framebuffer for second draw.
    {
        ...
        m_renderPassSecondDraw.createFramebuffer(vk, vkDevice, &framebufferParams, images);
    }
...
{
public:
    DepthRangeUnrestrictedTest(tcu::TestContext &testContext, const std::string &name,
                               const DepthRangeUnrestrictedParam param)
