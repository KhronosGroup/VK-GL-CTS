...
tcu::TestStatus NonSequentialInstance::iterate(void)
{
    ...
    const vk::Move<vk::VkDevice> device = createDynamicVertexStateDevice(
        m_context, queueFamilyIndex, m_pipelineConstructionType != vk::PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC);
    ...
    // Create shaders
    const std::array<vk::Move<vk::VkShaderModule>, 2> vertexShaderModules = {
        createShaderModule(vk, *device, m_context.getBinaryCollection().get("vert_0")),
        createShaderModule(vk, *device, m_context.getBinaryCollection().get("vert_1"))};

    const vk::Move<vk::VkShaderModule> fragmentShaderModule =
    ...
    vk::Move<vk::VkRenderPass> renderPass = createRenderPass(vk, *device, &renderPassInfo);
    ...
    vk::Move<vk::VkFramebuffer> framebuffer = createFramebuffer(vk, *device, &framebufferCreateInfo);
    ...
    vk::Move<vk::VkPipelineLayout> pipelineLayout = createPipelineLayout(vk, *device, &pipelineLayoutInfo);

    // Create graphics pipeline
    vk::GraphicsPipelineWrapper graphicsPipelines[2]{{vk, *device, m_pipelineConstructionType},
                                                     {vk, *device, m_pipelineConstructionType}};
    ...
    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, vk::makeRect2D(0, 0, 32u, 32u), clearColorValue);

    vk.cmdBindPipeline(*cmdBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[0].getPipeline());
    ...
    vk.cmdBindPipeline(*cmdBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, graphicsPipelines[1].getPipeline());
...
{
public:
    NonSequentialCase(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                                                      ...
                                                      vk::PipelineConstructionType pipelineConstructionType)
{
    de::MovePtr<tcu::TestCaseGroup> nonSequentialTestsGroup(
        new tcu::TestCaseGroup(testCtx, "dynamic_vertex_attribute", "Dynamic vertex attribute group."));

    nonSequentialTestsGroup->addChild(
        new NonSequentialCase(testCtx, "nonsequential", "Sequential tests.", pipelineConstructionType, 16u, {1u, 7u}));
