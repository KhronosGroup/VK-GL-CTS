...
tcu::TestStatus testEarlyDestroy(Context &context, bool usePipelineCache, bool destroyLayout)
{
    ...
    for (int i = 0; i < numTests; ++i)
    {
        ...
        else
        {
            ...
            const Unique<VkFramebuffer> framebuffer(vk::makeFramebuffer(vk, vkDevice, *renderPass, *attachmentImageView,
                                                                        framebufferWidth, framebufferHeight, 1u));
...
void addEarlyDestroyTestCasesWithFunctions(tcu::TestCaseGroup *group)
{
    ...
    addFunctionCaseWithPrograms(group, "no_cache_destroy_layout", "", initPrograms, testEarlyDestroyDestroyLayout,
                                false);
}
...
{
    return createTestGroup(testCtx, "early_destroy", "Tests where pipeline is destroyed early",
                           addEarlyDestroyTestCasesWithFunctions);
}
