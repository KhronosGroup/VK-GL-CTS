...
tcu::TestStatus BindBuffers2Instance::iterate(void)
{
    ...
    const vk::Move<vk::VkFramebuffer> framebuffer(
        makeFramebuffer(vk, device, *renderPass, 1u, &*colorImageView, extent.width, extent.height));
    const vk::Move<vk::VkShaderModule> vertShaderModule =
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert"));
    const vk::Move<vk::VkShaderModule> fragShaderModule =
    ...
    graphicsPipelineWrapper.setDefaultDepthStencilState()
        ...
        .setupFragmentShaderState(pipelineLayout.get(), renderPass.get(), 0u, fragShaderModule.get())
    ...
    vk::beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, vk::makeRect2D(0, 0, extent.width, extent.height),
                        1u, &clearColorValue);
...
{
public:
    BindBuffers2Case(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
void BindBuffers2Case::checkSupport(Context &context) const
{
    ...
    vk::checkPipelineLibraryRequirements(context.getInstanceInterface(), context.getPhysicalDevice(),
