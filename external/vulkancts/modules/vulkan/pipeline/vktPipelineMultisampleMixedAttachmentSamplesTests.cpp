...
Move<VkPipeline> makeGraphicsPipeline(
    ...
    const bool useVertexInput, const uint32_t subpassNdx, const UVec2 &renderSize,
    const VkImageAspectFlags depthStencilAspect, //!< Used to determine which D/S tests to turn on
    const VkSampleCountFlagBits numSamples, const bool sampleShadingEnable,
    const VkSampleLocationsInfoEXT *pSampleLocationsInfo = DE_NULL)
{
    ...
    const VkViewport viewport = {
        ...
        static_cast<float>(renderSize.y()), // widht, height
        0.0f,
...
struct TestParams
{
    ...
    TestParams(void) : colorFormat(), depthStencilFormat(), useProgrammableSampleLocations()
    {
    }
};
...
void draw(Context &context, const TestParams &params, WorkingData &wd)
{
    ...
    // Create a render pass and a framebuffer
    {
        ...
        framebuffer = makeFramebuffer(vk, device, *renderPass, static_cast<uint32_t>(attachments.size()),
    ...
    for (uint32_t subpassNdx = 0; subpassNdx < numSubpasses; ++subpassNdx)
    {
        ...
        pipelines.push_back(PipelineSp(new Unique<VkPipeline>(makeGraphicsPipeline(
            vk, device, *pipelineLayout, *renderPass, *vertexModule, *fragmentModule, /*use vertex input*/ true,
...
void drawResolve(Context &context, const TestParams &params, WorkingData &wd)
{
    ...
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(
        vk, device, *pipelineLayout, *renderPass, *vertexModule, *fragmentModule, useVertexInput, subpassNdx,
...
void createMixedAttachmentSamplesTestsInGroup(tcu::TestCaseGroup *rootGroup)
{
    ...
    // Test 2: Shader built-ins check
    {
        ...
        for (uint32_t caseNdx = 0u; caseNdx < DE_LENGTH_OF_ARRAY(singlePassCases); ++caseNdx)
        {
            ...
            MovePtr<tcu::TestCaseGroup> sampleCaseGroup(
                new tcu::TestCaseGroup(rootGroup->getTestContext(),
                                       getSampleCountGroupName(params.numCoverageSamples, params.numColorSamples,
            ...
            for (const VkFormat *pDepthStencilFormat = depthStencilReducedFormatRange;
                     ...
                     ++pColorFormat)
                {
                    ...
                    addFunctionCaseWithPrograms(
                        ...
                        "", ShaderBuiltins::checkRequirements, ShaderBuiltins::initPrograms, ShaderBuiltins::test,
                        params);
                }
...
{
    return createTestGroup(testCtx, "mixed_attachment_samples",
                           "Test a graphics pipeline with varying sample count per color and depth/stencil attachments",
                           createMixedAttachmentSamplesTestsInGroup);
}
