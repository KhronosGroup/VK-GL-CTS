...
public:
    CacheTestParam(const PipelineConstructionType pipelineConstructionType, const VkShaderStageFlags shaders,
                   bool noCache, bool delayedDestroy);
      ...
      }
{
    ...
    Move<VkShaderModule> vertShaderModule1 =
        createShaderModule(vk, vkDevice, context.getBinaryCollection().get("color_vert_1"), 0);
    Move<VkShaderModule> vertShaderModule2 =
        createShaderModule(vk, vkDevice, context.getBinaryCollection().get("color_vert_2"), 0);
    ...
    for (uint32_t ndx = 0; ndx < PIPELINE_CACHE_NDX_COUNT; ndx++)
    {
        VkShaderModule vertShaderModule =
            (ndx == PIPELINE_CACHE_NDX_DERIVATIVE) ? *vertShaderModule2 : *vertShaderModule1;
        ...
        preparePipelineWrapper(
            m_pipeline[ndx], vertShaderModule, *tescShaderModule, *teseShaderModule, *geomShaderModule,
...
void GraphicsCacheTestInstance::preparePipelineWrapper(GraphicsPipelineWrapper &gpw, VkShaderModule vertShaderModule,
                                                       VkShaderModule tescShaderModule, VkShaderModule teseShaderModule,
                                                       VkShaderModule geomShaderModule, VkShaderModule fragShaderModule,
                                                       VkPipelineCreationFeedbackEXT *pipelineCreationFeedback,
                                                       VkPipelineCreationFeedbackEXT *pipelineStageCreationFeedbacks,
                                                       VkPipeline basePipelineHandle)
{
    ...
    uint32_t geometryStages =
        1u + (geomShaderModule != DE_NULL) + (tescShaderModule != DE_NULL) + (teseShaderModule != DE_NULL);
    ...
    gpw.setDefaultTopology((tescShaderModule == DE_NULL) ? VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST :
        ...
        .setupPreRasterizationShaderState(viewport, scissor, *m_pipelineLayout, *m_renderPass, 0u, vertShaderModule,
                                          ...
                                          PipelineRenderingCreateInfoWrapper(), *m_cache,
        ...
        .setupFragmentShaderState(*m_pipelineLayout, *m_renderPass, 0u, fragShaderModule, &depthStencilStateParams,
                                  DE_NULL, DE_NULL, DE_NULL, *m_cache, pipelineCreationFeedbackWrapper[2])
...
tcu::TestStatus GraphicsCacheTestInstance::verifyTestResult(void)
{
         ...
         creationFeedbackNdx += step)
    {
        ...
        message << "\t\t Base Pipeline Acceleration ? \t"
                << (creationFeedbackFlags & VK_PIPELINE_CREATION_FEEDBACK_BASE_PIPELINE_ACCELERATION_BIT_EXT ? "yes" :
                                                                                                               "no")
                << "\n";
