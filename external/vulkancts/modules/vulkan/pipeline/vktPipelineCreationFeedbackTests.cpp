...
{
public:
    CacheTestParam(const VkShaderStageFlagBits *shaders, uint32_t count, bool noCache, bool delayedDestroy);
...
CacheTestParam::CacheTestParam(const VkShaderStageFlagBits *shaders, uint32_t count, bool noCache, bool delayedDestroy)
{
...
const std::string CacheTestParam::generateTestName(void) const
{
    ...
    for (uint32_t ndx = 1; ndx < m_shaderCount; ndx++)
        result += '_' + getShaderFlagStr(m_shaders[ndx], false) + cacheString[m_noCache ? 1 : 0] +
                  delayedDestroyString[m_delayedDestroy ? 1 : 0];
...
public:
    ...
    VkPipeline buildPipeline(tcu::UVec2 renderSize, VkRenderPass renderPass, VkPipelineCache cache,
...
VkPipeline SimpleGraphicsPipelineBuilder::buildPipeline(tcu::UVec2 renderSize, VkRenderPass renderPass,
                                                        ...
                                                        VkPipeline basePipelineHandle)
{
    ...
    const VkPipelineCreationFeedbackCreateInfoEXT pipelineCreationFeedbackCreateInfo = {
        ...
        m_shaderStageCount,            // uint32_t pipelineStageCreationFeedbackCount;
        pipelineStageCreationFeedbacks // VkPipelineCreationFeedbackEXT* pPipelineStageCreationFeedbacks;
    };

    uint32_t flagsCreateInfo = VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT;
    ...
    const VkGraphicsPipelineCreateInfo graphicsPipelineParams = {
        ...
        basePipelineHandle != DE_NULL ? -1 : 0,            // int32_t basePipelineIndex;
    };
    VkPipeline pipeline;
    ...
    , m_missPipelineBuilder(context)
{
    ...
    for (uint32_t ndx = 0; ndx < PIPELINE_CACHE_NDX_COUNT; ndx++)
    {
        ...
        m_pipeline[ndx] = m_pipelineBuilder.buildPipeline(
            m_renderSize, *m_renderPass, *m_cache, *m_pipelineLayout, &m_pipelineCreationFeedback[ndx],
...
tcu::TestStatus GraphicsCacheTestInstance::verifyTestResult(void)
{
    ...
    for (uint32_t ndx = 0; ndx < PIPELINE_CACHE_NDX_COUNT; ndx++)
    {
        ...
        if (ndx != PIPELINE_CACHE_NDX_DERIVATIVE &&
            m_pipelineCreationFeedback[ndx].flags & VK_PIPELINE_CREATION_FEEDBACK_BASE_PIPELINE_ACCELERATION_BIT_EXT)
        {
        ...
        for (uint32_t shader = 0; shader < m_param->getShaderCount(); shader++)
        {
            const uint32_t index = VK_MAX_SHADER_STAGES * ndx + shader;
            ...
            if (!(m_pipelineStageCreationFeedbacks[index].flags & VK_PIPELINE_CREATION_FEEDBACK_VALID_BIT_EXT))
            {
                if (m_pipelineStageCreationFeedbacks[index].flags)
                {
                    ...
                    errorMsg << getCaseStr(ndx)
                             << ": Creation feedback is not valid for " +
                                    getShaderFlagStr(m_param->getShaderFlag(shader), true) +
                                    " but there are other flags written";
                    return tcu::TestStatus::fail(errorMsg.str());
                }
                message << "\t\t Pipeline Creation Feedback data is not valid\n";
                continue;
            }
            if (m_param->isCacheDisabled() && m_pipelineStageCreationFeedbacks[index].flags &
                                                  VK_PIPELINE_CREATION_FEEDBACK_APPLICATION_PIPELINE_CACHE_HIT_BIT_EXT)
            {
                std::ostringstream errorMsg;
                errorMsg << getCaseStr(ndx)
                         << ": feedback indicates pipeline " + getShaderFlagStr(m_param->getShaderFlag(shader), true) +
                                " stage hit cache when it shouldn't";
                return tcu::TestStatus::fail(errorMsg.str());
            }
...
void ComputeCacheTestInstance::buildPipeline(const CacheTestParam *param, uint32_t ndx)
{
    ...
    const VkPipelineCreationFeedbackCreateInfoEXT pipelineCreationFeedbackCreateInfo = {
        ...
        1u,                                   // uint32_t pipelineStageCreationFeedbackCount;
...
{
    de::MovePtr<tcu::TestCaseGroup> cacheTests(
    ...
    // Graphics Pipeline Tests
    {
        de::MovePtr<tcu::TestCaseGroup> graphicsTests(new tcu::TestCaseGroup(
            testCtx, "graphics_tests", "Test pipeline creation feedback with graphics pipeline."));
        ...
        const CacheTestParam testParams[] = {
            CacheTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), false, false),
            CacheTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), false, false),
            CacheTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), false, false),
            CacheTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), true, false),
            CacheTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), true, false),
            CacheTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), true, false),
            CacheTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), false, true),
            CacheTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), false, true),
            CacheTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), false, true),
        };
    ...
    // Compute Pipeline Tests
    {
        de::MovePtr<tcu::TestCaseGroup> computeTests(
            new tcu::TestCaseGroup(testCtx, "compute_tests", "Test pipeline creation feedback with compute pipeline."));
