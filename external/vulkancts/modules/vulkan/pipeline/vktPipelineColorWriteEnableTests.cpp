...
constexpr uint32_t kNumColorAttachments = 3u;

const vk::VkFormat kDepthStencilFormats[] = {vk::VK_FORMAT_D32_SFLOAT_S8_UINT, vk::VK_FORMAT_D24_UNORM_S8_UINT};

constexpr auto kCoordsSize = static_cast<uint32_t>(2 * sizeof(float));
...
struct MeshParams
{
    ...
    MeshParams(const tcu::Vec4 &color_ = kDefaultTriangleColor, float depth_ = 0.0f, float scaleX_ = 1.0f,
               float scaleY_ = 1.0f, float offsetX_ = 0.0f, float offsetY_ = 0.0f)
        : color(color_)
        , depth(depth_)
        , scaleX(scaleX_)
        , scaleY(scaleY_)
...
{
public:
    ColorWriteEnableTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
ColorWriteEnableTest::ColorWriteEnableTest(tcu::TestContext &testCtx, const std::string &name,
...
void ColorWriteEnableTest::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    vertSource << "#version 450\n"
               << pushSource.str() << "layout(location=0) in vec2 position;\n"
               << "out gl_PerVertex\n"
               << "{\n"
               << "    vec4 gl_Position;\n"
               << "};\n"
    ...
    for (uint32_t i = 0u; i < kNumColorAttachments; ++i)
    {
    ...
    fragSource << "#version 450\n"
               << pushSource.str() << fragOutputs.str() << "void main() {\n"
               << colorWrite.str() << "}\n";
...
void setDynamicStates(const TestConfig &testConfig, const vk::DeviceInterface &vkd, vk::VkCommandBuffer cmdBuffer)
{
    if (testConfig.colorWriteEnableConfig.dynamicValue)
    {
        ...
        vkd.cmdSetColorWriteEnableEXT(cmdBuffer, static_cast<uint32_t>(colorWriteEnables.size()),
...
tcu::TestStatus ColorWriteEnableInstance::iterate(void)
{
    ...
    for (uint32_t i = 0u; i < kNumIterations * kNumColorAttachments; ++i)
    ...
    for (const auto &img : dsImages)
        dsImageViews.emplace_back(
            vk::makeImageView(vkd, device, img->get(), vk::VK_IMAGE_VIEW_TYPE_2D, dsFormat, dsSubresourceRange));
    ...
    std::vector<float> vertices = {
        0.0f, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, 0.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f,
    };

    const auto vertDataSize = vertices.size() * sizeof(float);
    const auto vertBufferInfo =
        vk::makeBufferCreateInfo(static_cast<vk::VkDeviceSize>(vertDataSize), vk::VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    ...
    const vk::VkAttachmentReference dsAttachmentReference = {
        kNumColorAttachments,                                 // uint32_t attachment;
    ...
    const vk::VkSubpassDescription subpassDescription = {
        ...
        kNumColorAttachments,                // uint32_t colorAttachmentCount;
    ...
    const vk::VkPipelineColorWriteCreateInfoEXT colorWriteCreateInfo = {
        ...
        nullptr,                                                    // const void* pNext;
        kNumColorAttachments,                                       // uint32_t attachmentCount;
    ...
    if (useStaticPipeline)
    {
        staticPipeline.setupVertexInputStete(&vertexInputStateCreateInfo, &inputAssemblyStateCreateInfo)
            .setupPreRasterizationShaderState(viewport, scissor, *pipelineLayout, *renderPass, 0u, *vertModule,
    ...
    for (uint32_t attachmentIndex = 0u; attachmentIndex < kNumColorAttachments;
         ++attachmentIndex, ++nextAttachmentImage)
    {
        const auto colorBuffer = readColorAttachment(vkd, device, queue, queueIndex, allocator,
                                                     (*nextAttachmentImage)->get(), kColorFormat, renderSize);
        ...
        for (int y = 0; y < kHeight; ++y)
            for (int x = 0; x < kWidth; ++x)
            {
                ...
                bool match = tcu::boolAll(tcu::lessThan(
                    tcu::absDiff(colorPixel, m_testConfig.expectedColor[attachmentIndex]), kColorThreshold));
    ...
    const auto depthBuffer =
        readDepthAttachment(vkd, device, queue, queueIndex, allocator, dsImages.back()->get(), dsFormat, renderSize);
...
VectorType MaskVector(const VectorType &valueIfMaskIsFalse, const VectorType &valueIfMaskIsTrue,
                      const std::vector<bool> &mask, bool inverse = false)
{
...
void ApplyChannelMask(std::vector<tcu::Vec4> &meshColors, const tcu::BVec4 &channelMask, const tcu::Vec4 &clearColor)
{
    for (auto &&attachmentColor : meshColors)
        attachmentColor = tcu::Vec4(channelMask.x() ? attachmentColor.x() : clearColor.x(),
...
void AddSingleTestCaseStatic(const std::string &name, const std::string &description,
                             vk::PipelineConstructionType pipelineConstructionType, const std::vector<bool> mask,
                             const tcu::BVec4 channelMask, bool inverse, tcu::TestCaseGroup *orderingGroup,
                             tcu::TestContext &testCtx)
{
    ...
    for (uint32_t i = 1u; i < kNumColorAttachments; ++i)
...
void AddSingleTestCaseDynamic(const std::string &name, const std::string &description,
                              vk::PipelineConstructionType pipelineConstructionType, const std::vector<bool> mask,
                              const tcu::BVec4 channelMask, bool inverse, tcu::TestCaseGroup *orderingGroup,
                              tcu::TestContext &testCtx, SequenceOrdering ordering)
{
...
{
public:
    ColorWriteEnable2Test(TestContext &testCtx, const std::string &name, const std::string &description,
...
public:
    ...
    {
        GraphicsPipelineWrapperEx(const DeviceInterface &vkd, const VkDevice dev, const PipelineConstructionType pct)
            : GraphicsPipelineWrapper(vkd, dev, pct)
            , m_isDynamicColorWriteEnable(false)
        {
        }
        bool isDynamicColorWriteEnable() const
        {
            return m_isDynamicColorWriteEnable;
        }
    ...
    ColorWriteEnable2Instance(Context &context, const TestParams &testParams);
    virtual ~ColorWriteEnable2Instance() = default;

    de::MovePtr<BufferWithMemory> createVerrtexBuffer() const;
    Move<VkRenderPass> createRenderPass(uint32_t colorAttachmentCount) const;
    Framebuffer createFramebuffer(VkRenderPass renderPass, uint32_t colorAttachmentCount) const;
    void setupAndBuildPipeline(GraphicsPipelineWrapperEx &owner, VkPipelineLayout pipelineLayout,
                               VkRenderPass renderPass, uint32_t colorAttachmentCount,
                               const ColorWriteEnables &colorWriteEnables, float blendComp, bool dynamic) const;
    virtual TestStatus iterate() override;
    bool verifyAttachment(const uint32_t attachmentIndex, const uint32_t attachmentCount,
                          const ConstPixelBufferAccess &attachmentContent, const ColorWriteEnables &colorWriteEnables,
                          const Vec4 &background, const float blendComp) const;
...
bool TestParams::selectOptimalBlendableFormat(const InstanceInterface &vk, VkPhysicalDevice dev)
{
    auto doesFormatMatch = [](const VkFormat fmt) -> bool
    {
        ...
        return tcuFmt.order == TextureFormat::ChannelOrder::RGBA || tcuFmt.order == TextureFormat::ChannelOrder::sRGBA;
    };
...
{
    const char nl     = '\n';
    const uint32_t ac = m_params.attachmentCount;
    std::ostringstream vs;
    std::ostringstream fs;

    vs << "#version 450" << nl << "layout(location = 0) in vec4 position;" << nl
       << "layout(location = 0) out flat int instance;" << nl << "void main() {" << nl
       << "    gl_Position = vec4(position.xy, 0.0, 1.0);" << nl << "    instance = gl_InstanceIndex;" << nl << "}"
       << nl;
    programCollection.glslSources.add("vert") << glu::VertexSource(vs.str());

    fs << "#version 450" << nl << "layout(location = 0) in flat int attachments;" << nl
       << "layout(location = 0) out vec4 colors[" << ac << "];" << nl << "void main() {" << nl
       << "    for (int a = 0; a < attachments; ++a) {" << nl << "        float c = float(attachments - a);" << nl
       << "        colors[a] = vec4(pow(0.5, c));" << nl << "}}" << nl;
...
Move<VkRenderPass> ColorWriteEnable2Instance::createRenderPass(uint32_t colorAttachmentCount) const
{
...
{
    const std::vector<float> quad{-1.0f, -1.0f, 0.0f, 0.0f, +1.0f, -1.0f, 0.0f, 0.0f, -1.0f, +1.0f, 0.0f, 0.0f,
                                  -1.0f, +1.0f, 0.0f, 0.0f, +1.0f, -1.0f, 0.0f, 0.0f, +1.0f, +1.0f, 0.0f, 0.0f};

    const auto vertDataSize = quad.size() * sizeof(float);
    const auto vertBufferInfo =
        makeBufferCreateInfo(static_cast<VkDeviceSize>(vertDataSize), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
    de::MovePtr<BufferWithMemory> vertBuffer(
        new BufferWithMemory(m_vkd, m_device, m_allocator, vertBufferInfo, vk::MemoryRequirement::HostVisible));
    auto &alloc = vertBuffer->getAllocation();
...
ColorWriteEnable2Instance::Framebuffer ColorWriteEnable2Instance::createFramebuffer(VkRenderPass renderPass,
                                                                                    uint32_t colorAttachmentCount) const
{
    ...
    for (uint32_t i = 0; i < colorAttachmentCount; ++i)
    {
        ...
        attachment.image = de::MovePtr<ImageWithMemory>(
            new ImageWithMemory(m_vkd, m_device, allocator, imageCreateInfo, MemoryRequirement::Any));

        attachment.view = makeImageView(m_vkd, m_device, **attachment.image, VK_IMAGE_VIEW_TYPE_2D, m_params.format,
                                        imageSubresource);
    ...
    const VkFramebufferCreateInfo framebufferCreateInfo{
        ...
        m_params.width,                            // uint32_t width;
        m_params.height,                           // uint32_t height;
        1u,                                        // uint32_t layers;
    };
...
void ColorWriteEnable2Instance::setupAndBuildPipeline(GraphicsPipelineWrapperEx &owner, VkPipelineLayout pipelineLayout,
                                                      VkRenderPass renderPass, uint32_t colorAttachmentCount,
                                                      const ColorWriteEnables &colorWriteEnables, float blendComp,
                                                      bool dynamic) const
{
    ...
    const auto vertexBinding =
        makeVertexInputBindingDescription(0u, static_cast<uint32_t>(4 * sizeof(float)), VK_VERTEX_INPUT_RATE_VERTEX);
    ...
    owner.setDefaultRasterizationState()
...
bool ColorWriteEnable2Instance::verifyAttachment(const uint32_t attachmentIndex, const uint32_t attachmentCount,
                                                 const ConstPixelBufferAccess &attachmentContent,
                                                 const ColorWriteEnables &colorWriteEnables, const Vec4 &background,
                                                 const float blendComp) const
{
    ...
    for (uint32_t y = 0; y < m_params.height; ++y)
    {
        for (uint32_t x = 0; x < m_params.width; ++x)
        {
            ...
            const float er    = expected.x();
            const float rr    = result.x();
            const float eg    = expected.y();
            const float rg    = result.y();
            const float eb    = expected.z();
            const float rb    = result.z();
            const float ea    = expected.w();
            const float ra    = result.w();
...
TestStatus ColorWriteEnable2Instance::iterate(void)
{
    ...
    const uint32_t attachmentCount = m_params.attachmentCount;
    ...
    for (uint32_t i = 0; i < attachmentCount; ++i)
        writeEnables[i] = (i % 2) ? VK_TRUE : VK_FALSE;
    ...
    for (uint32_t i = 0; i < attachmentCount; ++i)
    {
        ...
        setupAndBuildPipeline(pipelines.back(), *pipelineLayout, *renderPasses[i], (i + 1), writeEnables, blendComp,
                              dynamicColorWriteEnable);
    }

    const VkImageSubresourceRange attachmentResource =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, 1u);
    VkImageMemoryBarrier attachmentReady = makeImageMemoryBarrier(
        VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VkImage(0), attachmentResource);
    ...
    for (uint32_t a = 0; a < attachmentCount; ++a)
    {
        if (m_params.setCweBeforePlBind)
        {
            if (pipelines[a].isDynamicColorWriteEnable())
                m_vkd.cmdSetColorWriteEnableEXT(*cmdBuff, static_cast<uint32_t>(writeEnables.size()),
                                                writeEnables.data());
        ...
        beginRenderPass(m_vkd, *cmdBuff, *renderPasses[a], *framebuffers[a].framebuffer, renderArea, attachmentCount,
                        clearValues.data());
    ...
    for (uint32_t i = 0; i < attachmentCount; ++i)
        for (uint32_t a = 0; a < (i + 1); ++a)
        {
            ...
            failureCount +=
                verifyAttachment(a, (i + 1), colorBuffer->getAccess(), writeEnables, background, blendComp) ? 0u : 1u;
        }
...
{
    de::MovePtr<tcu::TestCaseGroup> colorWriteEnableGroup(
        new tcu::TestCaseGroup(testCtx, "color_write_enable", "Tests for VK_EXT_color_write_enable"));

    DE_ASSERT(kNumColorAttachments >= 2);

    std::vector<bool> mask_all(kNumColorAttachments, true);
    std::vector<bool> mask_first(kNumColorAttachments, false);
    mask_first[0] = true;
    std::vector<bool> mask_second(kNumColorAttachments, false);
    mask_second[1] = true;
    std::vector<bool> mask_last(kNumColorAttachments, false);
    mask_last.back() = true;
    std::vector<bool> mask_first_and_second(kNumColorAttachments, false);
    mask_first_and_second[0] = mask_first_and_second[1] = true;
    std::vector<bool> mask_second_and_last(kNumColorAttachments, false);
    mask_second_and_last[1] = mask_second_and_last.back() = true;
    ...
    for (int channelCaseIdx = 0; channelCaseIdx < DE_LENGTH_OF_ARRAY(kChannelCases); ++channelCaseIdx)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> channelGroup(
            new tcu::TestCaseGroup(testCtx, kChannelCase.name.c_str(), kChannelCase.desc.c_str()));
        ...
        for (int orderingIdx = 0; orderingIdx < DE_LENGTH_OF_ARRAY(kOrderingCases); ++orderingIdx)
        {
            ...
            AddSingleTestCaseDynamic("enable_all", "Dynamically enable writes to all color attachments", pct, mask_all,
                                     kChannelCase.enabledChannels, false, orderingGroup.get(), testCtx, kOrdering);
            AddSingleTestCaseDynamic("enable_first", "Dynamically enable writes to the first color attachment", pct,
                                     mask_first, kChannelCase.enabledChannels, false, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic("enable_second", "Dynamically enable writes to the second color attachment", pct,
                                     mask_second, kChannelCase.enabledChannels, false, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic("enable_last", "Dynamically enable writes to the last color attachment", pct,
                                     mask_last, kChannelCase.enabledChannels, false, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic(
                "enable_first_and_second", "Dynamically enable writes to the first two color attachments", pct,
                mask_first_and_second, kChannelCase.enabledChannels, false, orderingGroup.get(), testCtx, kOrdering);
            AddSingleTestCaseDynamic(
                "enable_second_and_last", "Dynamically enable writes to the second and last color attachments", pct,
                mask_second_and_last, kChannelCase.enabledChannels, false, orderingGroup.get(), testCtx, kOrdering);

            AddSingleTestCaseDynamic("disable_all", "Dynamically disable writes to all color attachments", pct,
                                     mask_all, kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic("disable_first", "Dynamically disable writes to the first color attachment", pct,
                                     mask_first, kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic("disable_second", "Dynamically disable writes to the second color attachment", pct,
                                     mask_second, kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic("disable_last", "Dynamically disable writes to the last color attachment", pct,
                                     mask_last, kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx,
                                     kOrdering);
            AddSingleTestCaseDynamic(
                "disable_first_and_second", "Dynamically disable writes to the first two color attachments", pct,
                mask_first_and_second, kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx, kOrdering);
            AddSingleTestCaseDynamic(
                "disable_second_and_last", "Dynamically disable writes to the second and last color attachments", pct,
                mask_second_and_last, kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx, kOrdering);
        ...
        de::MovePtr<tcu::TestCaseGroup> staticOrderingGroup(
            new tcu::TestCaseGroup(testCtx, "static", "Static state set"));

        AddSingleTestCaseStatic("enable_all", "Statically enable writes to all color attachments", pct, mask_all,
                                kChannelCase.enabledChannels, false, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic("enable_first", "Statically enable writes to the first color attachment", pct,
                                mask_first, kChannelCase.enabledChannels, false, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic("enable_second", "Statically enable writes to the second color attachment", pct,
                                mask_second, kChannelCase.enabledChannels, false, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic("enable_last", "Statically enable writes to the last color attachment", pct, mask_last,
                                kChannelCase.enabledChannels, false, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic(
            "enable_first_and_second", "Statically enable writes to the first two color attachments", pct,
            mask_first_and_second, kChannelCase.enabledChannels, false, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic(
            "enable_second_and_last", "Statically enable writes to the second and last color attachments", pct,
            mask_second_and_last, kChannelCase.enabledChannels, false, staticOrderingGroup.get(), testCtx);

        AddSingleTestCaseStatic("disable_all", "Statically disable writes to all color attachments", pct, mask_all,
                                kChannelCase.enabledChannels, true, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic("disable_first", "Statically disable writes to the first color attachment", pct,
                                mask_first, kChannelCase.enabledChannels, true, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic("disable_second", "Statically disable writes to the second color attachment", pct,
                                mask_second, kChannelCase.enabledChannels, true, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic("disable_last", "Statically disable writes to the last color attachment", pct,
                                mask_last, kChannelCase.enabledChannels, true, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic(
            "disable_first_and_second", "Statically disable writes to the first two color attachments", pct,
            mask_first_and_second, kChannelCase.enabledChannels, true, staticOrderingGroup.get(), testCtx);
        AddSingleTestCaseStatic(
            "disable_second_and_last", "Statically disable writes to the second and last color attachments", pct,
            mask_second_and_last, kChannelCase.enabledChannels, true, staticOrderingGroup.get(), testCtx);
...
{
    const uint32_t attachmentCounts[]{3, 4, 5};
    const uint32_t attachentMores[]{0, 1, 2, 3};

    std::pair<bool, const char *> static const setCweMoments[]{{true, "cwe_before_bind"}, {false, "cwe_after_bind"}};

    tcu::TestCaseGroup *rootGroup =
        new tcu::TestCaseGroup(testCtx, "color_write_enable_maxa", "Tests for VK_EXT_color_write_enable");

    for (const auto &setCweMoment : setCweMoments)
    {
        tcu::TestCaseGroup *setCweGroup =
            new tcu::TestCaseGroup(testCtx, setCweMoment.second, "A moment when cmdSetColorWriteEnableEXT() is called");
        ...
        for (auto attachmentCount : attachmentCounts)
        {
            for (auto attachentMore : attachentMores)
            {
                const std::string title =
                    "attachments" + std::to_string(attachmentCount) + "_more" + std::to_string(attachentMore);
