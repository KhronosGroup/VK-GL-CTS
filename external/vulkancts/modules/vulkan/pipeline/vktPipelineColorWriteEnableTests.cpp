...
tcu::TestStatus ColorWriteEnableInstance::iterate(void)
{
    ...
    for (size_t imgIdx = 0; imgIdx < dsImageViews.size(); ++imgIdx)
    {
        ...
        framebuffers.emplace_back(
            vk::RenderPassWrapper(m_testConfig.pipelineConstructionType, vkd, device, &renderPassCreateInfo));
    ...
    vk::GraphicsPipelineWrapper staticPipeline(vki, vkd, physicalDevice, device, m_context.getDeviceExtensions(),
    ...
    if (useStaticPipeline)
    {
        staticPipeline.setupVertexInputState(&vertexInputStateCreateInfo, &inputAssemblyStateCreateInfo)
            .setupPreRasterizationShaderState(viewport, scissor, pipelineLayout, *framebuffers[0], 0u, vertModule,
                                              &rasterizationStateCreateInfo)
            .setupFragmentShaderState(pipelineLayout, *framebuffers[0], 0u, fragModule, &depthStencilStateCreateInfo,
    ...
    vk::GraphicsPipelineWrapper graphicsPipeline(vki, vkd, physicalDevice, device, m_context.getDeviceExtensions(),
                                                 m_testConfig.pipelineConstructionType);
    ;
    ...
    for (uint32_t iteration = 0u; iteration < kNumIterations; ++iteration)
    {
        ...
        // Begin render pass.
        framebuffers[iteration].begin(vkd, cmdBuffer, vk::makeRect2D(kFramebufferWidth, kFramebufferHeight),
...
void AddSingleTestCaseStatic(const std::string &name, vk::PipelineConstructionType pipelineConstructionType,
                             const std::vector<bool> mask, const tcu::BVec4 channelMask, bool inverse,
                             tcu::TestCaseGroup *orderingGroup, tcu::TestContext &testCtx)
{
...
void AddSingleTestCaseDynamic(const std::string &name, vk::PipelineConstructionType pipelineConstructionType,
                              const std::vector<bool> mask, const tcu::BVec4 channelMask, bool inverse,
                              tcu::TestCaseGroup *orderingGroup, tcu::TestContext &testCtx, SequenceOrdering ordering)
{
...
{
public:
    ColorWriteEnable2Test(TestContext &testCtx, const std::string &name, const TestParams &testParams)
        : vkt::TestCase(testCtx, name)
        , m_params(testParams)
    {
    }
...
public:
    ...
    {
        GraphicsPipelineWrapperEx(const InstanceInterface &vki, const DeviceInterface &vkd,
                                  const VkPhysicalDevice physDev, const VkDevice dev,
                                  const std::vector<std::string> &exts, const PipelineConstructionType pct)
    ...
    RenderPassWrapper createRenderPass(uint32_t colorAttachmentCount) const;
    Framebuffer createFramebuffer(uint32_t colorAttachmentCount) const;
    void setupAndBuildPipeline(GraphicsPipelineWrapperEx &owner, PipelineLayoutWrapper &pipelineLayout,
...
RenderPassWrapper ColorWriteEnable2Instance::createRenderPass(uint32_t colorAttachmentCount) const
{
...
ColorWriteEnable2Instance::Framebuffer ColorWriteEnable2Instance::createFramebuffer(uint32_t colorAttachmentCount) const
{
...
void ColorWriteEnable2Instance::setupAndBuildPipeline(GraphicsPipelineWrapperEx &owner,
                                                      PipelineLayoutWrapper &pipelineLayout, VkRenderPass renderPass,
...
TestStatus ColorWriteEnable2Instance::iterate(void)
{
    ...
    for (uint32_t i = 0; i < attachmentCount; ++i)
    {
        ...
        setupAndBuildPipeline(pipelines.back(), pipelineLayout, framebuffers[i].framebuffer.get(), (i + 1),
                              writeEnables, blendComp, dynamicColorWriteEnable);
    }
...
tcu::TestCaseGroup *createColorWriteEnableTests(tcu::TestContext &testCtx, vk::PipelineConstructionType pct)
{
    ...
    for (int channelCaseIdx = 0; channelCaseIdx < DE_LENGTH_OF_ARRAY(kChannelCases); ++channelCaseIdx)
    {
        ...
        for (int orderingIdx = 0; orderingIdx < DE_LENGTH_OF_ARRAY(kOrderingCases); ++orderingIdx)
        {
            ...
            if (vk::isConstructionTypeShaderObject(pct) &&
            ...
            AddSingleTestCaseDynamic("enable_all", pct, mask_all, kChannelCase.enabledChannels, false,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically enable writes to the first color attachment
            AddSingleTestCaseDynamic("enable_first", pct, mask_first, kChannelCase.enabledChannels, false,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically enable writes to the second color attachment
            AddSingleTestCaseDynamic("enable_second", pct, mask_second, kChannelCase.enabledChannels, false,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically enable writes to the last color attachment
            AddSingleTestCaseDynamic("enable_last", pct, mask_last, kChannelCase.enabledChannels, false,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically enable writes to the first two color attachments
            AddSingleTestCaseDynamic("enable_first_and_second", pct, mask_first_and_second,
            ...
            AddSingleTestCaseDynamic("enable_second_and_last", pct, mask_second_and_last, kChannelCase.enabledChannels,
                                     false, orderingGroup.get(), testCtx, kOrdering);

            // Dynamically disable writes to all color attachments
            AddSingleTestCaseDynamic("disable_all", pct, mask_all, kChannelCase.enabledChannels, true,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically disable writes to the first color attachment
            AddSingleTestCaseDynamic("disable_first", pct, mask_first, kChannelCase.enabledChannels, true,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically disable writes to the second color attachment
            AddSingleTestCaseDynamic("disable_second", pct, mask_second, kChannelCase.enabledChannels, true,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically disable writes to the last color attachment
            AddSingleTestCaseDynamic("disable_last", pct, mask_last, kChannelCase.enabledChannels, true,
                                     orderingGroup.get(), testCtx, kOrdering);
            // Dynamically disable writes to the first two color attachments
            AddSingleTestCaseDynamic("disable_first_and_second", pct, mask_first_and_second,
                                     kChannelCase.enabledChannels, true, orderingGroup.get(), testCtx, kOrdering);
            // Dynamically disable writes to the second and last color attachments
            AddSingleTestCaseDynamic("disable_second_and_last", pct, mask_second_and_last, kChannelCase.enabledChannels,
                                     true, orderingGroup.get(), testCtx, kOrdering);
        ...
        AddSingleTestCaseStatic("enable_all", pct, mask_all, kChannelCase.enabledChannels, false,
                                staticOrderingGroup.get(), testCtx);
        // Statically enable writes to the first color attachment
        AddSingleTestCaseStatic("enable_first", pct, mask_first, kChannelCase.enabledChannels, false,
                                staticOrderingGroup.get(), testCtx);
        // Statically enable writes to the second color attachment
        AddSingleTestCaseStatic("enable_second", pct, mask_second, kChannelCase.enabledChannels, false,
                                staticOrderingGroup.get(), testCtx);
        // Statically enable writes to the last color attachment
        AddSingleTestCaseStatic("enable_last", pct, mask_last, kChannelCase.enabledChannels, false,
                                staticOrderingGroup.get(), testCtx);
        // Statically enable writes to the first two color attachments
        AddSingleTestCaseStatic("enable_first_and_second", pct, mask_first_and_second, kChannelCase.enabledChannels,
                                false, staticOrderingGroup.get(), testCtx);
        // Statically enable writes to the second and last color attachments
        AddSingleTestCaseStatic("enable_second_and_last", pct, mask_second_and_last, kChannelCase.enabledChannels,
                                false, staticOrderingGroup.get(), testCtx);

        // Statically disable writes to all color attachments
        AddSingleTestCaseStatic("disable_all", pct, mask_all, kChannelCase.enabledChannels, true,
                                staticOrderingGroup.get(), testCtx);
        // Statically disable writes to the first color attachment
        AddSingleTestCaseStatic("disable_first", pct, mask_first, kChannelCase.enabledChannels, true,
                                staticOrderingGroup.get(), testCtx);
        // Statically disable writes to the second color attachment
        AddSingleTestCaseStatic("disable_second", pct, mask_second, kChannelCase.enabledChannels, true,
                                staticOrderingGroup.get(), testCtx);
        // Statically disable writes to the last color attachment
        AddSingleTestCaseStatic("disable_last", pct, mask_last, kChannelCase.enabledChannels, true,
                                staticOrderingGroup.get(), testCtx);
        // Statically disable writes to the first two color attachments
        AddSingleTestCaseStatic("disable_first_and_second", pct, mask_first_and_second, kChannelCase.enabledChannels,
                                true, staticOrderingGroup.get(), testCtx);
        // Statically disable writes to the second and last color attachments
        AddSingleTestCaseStatic("disable_second_and_last", pct, mask_second_and_last, kChannelCase.enabledChannels,
                                true, staticOrderingGroup.get(), testCtx);
