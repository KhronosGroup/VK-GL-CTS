...
{
public:
    SourcesAndSupportFromParamsBase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                    BaseParamsPtr &&params)
...
public:
    ...
    ConstantModuleIdentifiersCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                  ParamsPtr &&params)
        : SourcesAndSupportFromParamsBase(testCtx, name, description,
                                          BaseParamsPtr(static_cast<BaseParams *>(params.release())))
    {
    }
...
public:
    ...
    struct Params : public BaseParams
    {
        ...
        Params(PipelineType pipelineType_, GraphicsShaderVec graphicsShaders_, RTShaderVec rtShaders_,
               uint8_t pipelineCount_, const tcu::Maybe<uint8_t> &pipelineToRun_, bool useSCs_, bool useCache_,
               PipelineConstructionType constructionType_, bool useRTLibraries_, UseModuleCase moduleUseCase_,
...
{
public:
    CreateAndUseIdsCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                        BaseParamsPtr &&params)
...
public:
    PipelineStageInfo() : m_module(DE_NULL), m_moduleId(), m_moduleIdCreateInfo(), m_specInfo()
    {
    }

    void setModule(const DeviceInterface &vkd, const VkDevice device, const VkShaderModule module,
...
tcu::TestStatus CreateAndUseIdsInstance::iterate(void)
{
    ...
    if (isGraphics)
    {
        ...
        framebuffer =
            makeFramebuffer(vkd, device, renderPass.get(), colorAttView.get(), fbExtent.width, fbExtent.height);
    ...
    if (isGraphics)
    {
        ...
        for (uint32_t i = 0; i < pipelineCount32; ++i)
        {
            ...
            pipelineWrappers.emplace_back(
                new GraphicsPipelineWrapper(vkd, device, m_params->constructionType, captureFlags));
            ...
            wrapper.setDefaultPatchControlPoints(patchCPs)
                ...
                .setupPreRasterizationShaderState2(
                    viewports, scissors, pipelineLayout.get(), renderPass.get(), 0u, vertModule, &rasterizationState,
                    tescModule, teseModule, geomModule, vertSpecInfo.get(), tescSpecInfo.get(), teseSpecInfo.get(),
                    geomSpecInfo.get(), PipelineRenderingCreateInfoWrapper(), pipelineCache.get())
                .setupFragmentShaderState(pipelineLayout.get(), renderPass.get(), 0u, fragModule, &depthStencilState,
                                          &multisampleState, nullptr, fragSpecInfo.get(), pipelineCache.get())
            ...
            try
            {
                wrapper.setDefaultPatchControlPoints(patchCPs)
                    ...
                    .setupPreRasterizationShaderState3(
                        viewports, scissors, pipelineLayout.get(), renderPass.get(), 0u,
                        ...
                        geomToRun.getSpecInfo(), PipelineRenderingCreateInfoWrapper(), pipelineCache.get())
                    .setupFragmentShaderState2(pipelineLayout.get(), renderPass.get(), 0u,
                                               fragToRun.getUsedModule(m_params->moduleUseCase),
                                               fragToRun.getModuleIdCreateInfo(), &depthStencilState, &multisampleState,
                                               nullptr, fragToRun.getSpecInfo(), pipelineCache.get())
                    .setupFragmentOutputState(*renderPass, 0u, &colorBlendState, &multisampleState, pipelineCache.get())
                    .setMonolithicPipelineLayout(pipelineLayout.get())
                    .buildPipeline(pipelineCache.get());
    ...
    else if (isCompute)
    {
        ...
        for (uint32_t i = 0; i < pipelineCount32; ++i)
        {
            ...
            pipelinePtrs.push_back(makeComputePipeline(vkd, device, pipelineLayout.get(), captureFlags,
                                                       compModules.back().get(), 0u, scInfo.get(),
                                                       pipelineCache.get()));
    ...
    else if (isRT)
    {
        ...
        for (uint32_t i = 0; i < pipelineCount32; ++i)
        {
...

            {
                ...
                if (chitModule != DE_NULL)
                    rayTracingPipeline->addShader(VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR, chitModule, xhitGroup.get(),
                                                  chitSpecInfo.get());

                if (ahitModule != DE_NULL)
                    rayTracingPipeline->addShader(VK_SHADER_STAGE_ANY_HIT_BIT_KHR, ahitModule, xhitGroup.get(),
                                                  ahitSpecInfo.get());

                if (isecModule != DE_NULL)
                    rayTracingPipeline->addShader(VK_SHADER_STAGE_INTERSECTION_BIT_KHR, isecModule, xhitGroup.get(),
                                                  isecSpecInfo.get());

                if (missModule != DE_NULL)
                    rayTracingPipeline->addShader(VK_SHADER_STAGE_MISS_BIT_KHR, missModule, missGroup.get(),
                                                  missSpecInfo.get());

                if (callModule != DE_NULL)
                    rayTracingPipeline->addShader(VK_SHADER_STAGE_CALLABLE_BIT_KHR, callModule, callGroup.get(),
                                                  callSpecInfo.get());
        ...
        if (runOnePipeline)
        {
            ...
            shaderOwningPipeline->addShader(VK_SHADER_STAGE_RAYGEN_BIT_KHR,
                                            rgenToRun.getUsedModule(m_params->moduleUseCase), rgenGroup,
                                            rgenToRun.getSpecInfo(), 0, rgenToRun.getModuleIdCreateInfo());

            if (chitModule != DE_NULL)
            {
                shaderOwningPipeline->addShader(VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR,
                                                chitToRun.getUsedModule(m_params->moduleUseCase), xhitGroup.get(),
                                                chitToRun.getSpecInfo(), 0, chitToRun.getModuleIdCreateInfo());
            }

            if (ahitModule != DE_NULL)
            {
                shaderOwningPipeline->addShader(VK_SHADER_STAGE_ANY_HIT_BIT_KHR,
                                                ahitToRun.getUsedModule(m_params->moduleUseCase), xhitGroup.get(),
                                                ahitToRun.getSpecInfo(), 0, ahitToRun.getModuleIdCreateInfo());
            }

            if (isecModule != DE_NULL)
            {
                shaderOwningPipeline->addShader(VK_SHADER_STAGE_INTERSECTION_BIT_KHR,
                                                isecToRun.getUsedModule(m_params->moduleUseCase), xhitGroup.get(),
                                                isecToRun.getSpecInfo(), 0, isecToRun.getModuleIdCreateInfo());
            }

            if (missModule != DE_NULL)
            {
                shaderOwningPipeline->addShader(VK_SHADER_STAGE_MISS_BIT_KHR,
                                                missToRun.getUsedModule(m_params->moduleUseCase), missGroup.get(),
                                                missToRun.getSpecInfo(), 0, missToRun.getModuleIdCreateInfo());
            }

            if (callModule != DE_NULL)
            {
                shaderOwningPipeline->addShader(VK_SHADER_STAGE_CALLABLE_BIT_KHR,
                                                callToRun.getUsedModule(m_params->moduleUseCase), callGroup.get(),
                                                callToRun.getSpecInfo(), 0, callToRun.getModuleIdCreateInfo());
            }
...
{
public:
    HLSLTessellationCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         PipelineConstructionType constructionType)
        : vkt::TestCase(testCtx, name, description)
        , m_constructionType(constructionType)
    {
    ...
    // Create temporary pipelines with them to prime the cache.
    {
        for (const auto &tescModule : tescModules)
        {
            ...
            try
            {
                wrapper->setDefaultPatchControlPoints(patchCPs)
                    .setupVertexInputStete(&vertexInputState, &inputAssemblyState, pipelineCache.get())
                    .setupPreRasterizationShaderState2(rpViewports, rpScissors, pipelineLayout.get(), renderPass.get(),
                                                       0u, vertModule.get(), &rasterizationState, tescModule.get(),
                                                       teseModule.get(), DE_NULL, nullptr, nullptr, nullptr, nullptr,
                                                       PipelineRenderingCreateInfoWrapper(), pipelineCache.get())
                    .setupFragmentShaderState(pipelineLayout.get(), renderPass.get(), 0u, fragModule.get(),
                                              &depthStencilState, &multisampleState, nullptr, nullptr,
                                              pipelineCache.get())
                    .setupFragmentOutputState(*renderPass, 0u, &colorBlendState, &multisampleState, pipelineCache.get())
                    .setMonolithicPipelineLayout(pipelineLayout.get())
                    .buildPipeline(pipelineCache.get());
        ...
        try
        {
            wrapper->setDefaultPatchControlPoints(patchCPs)
                ...
                .setupPreRasterizationShaderState3(
                    ...
                    pipelineLayout.get(), renderPass.get(), 0u, DE_NULL,
                    PipelineShaderStageModuleIdentifierCreateInfoWrapper(vertIdInfo.get()), &rasterizationState,
                    DE_NULL, PipelineShaderStageModuleIdentifierCreateInfoWrapper(tescIdInfos.at(tescIdx).get()),
                    DE_NULL, PipelineShaderStageModuleIdentifierCreateInfoWrapper(teseIdInfo.get()), DE_NULL,
                    PipelineShaderStageModuleIdentifierCreateInfoWrapper(), nullptr, nullptr, nullptr, nullptr,
                    PipelineRenderingCreateInfoWrapper(), pipelineCache.get())
                .setupFragmentShaderState2(pipelineLayout.get(), renderPass.get(), 0u, DE_NULL,
                                           PipelineShaderStageModuleIdentifierCreateInfoWrapper(fragIdInfo.get()),
                                           &depthStencilState, &multisampleState, nullptr, nullptr, pipelineCache.get())
                .setupFragmentOutputState(*renderPass, 0u, &colorBlendState, &multisampleState, pipelineCache.get())
                .setMonolithicPipelineLayout(pipelineLayout.get())
                .buildPipeline(pipelineCache.get());
        }
                                                      ...
                                                      vk::PipelineConstructionType constructionType)
{
    ...
    GroupPtr mainGroup(
        new tcu::TestCaseGroup(testCtx, "shader_module_identifier", "Tests for VK_EXT_shader_module_identifier"));

    if (constructionType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
    {
        // Property tests.
        GroupPtr propertiesGroup(
            new tcu::TestCaseGroup(testCtx, "properties", "Test shader module identifier extension properties"));
            ...
            for (const auto &pipelineCountCase : pipelineCountCases)
            {
                ...
                for (const auto &useSCCase : useSCCases)
                {
                    ...
                    for (const auto &apiCallCase : apiCallCases)
                    {
                        ...
                        {
                            GroupPtr differentDeviceGroup(
                                new tcu::TestCaseGroup(testCtx, differentDeviceCase.name, ""));
    ...
    // Tests creating pipelines using the module id extension structures.
    {
        ...
        GroupPtr pipelineFromIdsGroup(new tcu::TestCaseGroup(
            testCtx, "pipeline_from_id", "Test creating and using pipelines from shader module identifiers"));
                        ...
                        for (const auto &moduleUsageCase : moduleUsageCases)
                        {
                            ...
                            for (const auto &capturingCase : capturingCases)
                            {
                                ...
                                CreateAndUseIdsInstance::Params baseParams(
                                    ...
                                    useSCCase.useSCs, pipelineCacheCase.useVkPipelineCache, constructionType,
                                ...
                                else // Compute
                                {
                                    ...
                                    captureGroup->addChild(
                                        new CreateAndUseIdsCase(testCtx, "comp", "", std::move(params)));
                                }
    ...
    {
        GroupPtr hlslTessGroup(new tcu::TestCaseGroup(
            testCtx, "hlsl_tessellation", "Tests checking HLSL tessellation shaders with module identifiers"));
