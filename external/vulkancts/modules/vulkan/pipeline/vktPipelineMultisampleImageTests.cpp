...
Move<VkRenderPass> makeMultisampleRenderPass(const DeviceInterface &vk, const VkDevice device,
...
Move<VkRenderPass> makeSimpleRenderPass(const DeviceInterface &vk, const VkDevice device, const VkFormat colorFormat)
{
    const VkAttachmentDescription colorAttachmentDescription = {
        (VkAttachmentDescriptionFlags)0,          // VkAttachmentDescriptionFlags flags;
        colorFormat,                              // VkFormat format;
...
void renderMultisampledImage(Context &context, const CaseDef &caseDef, const VkImage colorImage)
{
...

    {
        ...
        const Unique<VkRenderPass> renderPass(
            makeMultisampleRenderPass(vk, device, caseDef.colorFormat, caseDef.numSamples, caseDef.numLayers));
        const Unique<VkFramebuffer> framebuffer(makeFramebuffer(
            vk, device, *renderPass, caseDef.numLayers, &attachmentHandles[0],
            static_cast<uint32_t>(caseDef.renderSize.x()), static_cast<uint32_t>(caseDef.renderSize.y())));
        const Unique<VkPipelineLayout> pipelineLayout(makePipelineLayout(vk, device));
        const std::vector<PipelineSp> pipelines(makeGraphicsPipelines(
        ...
        beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                        makeRect2D(0, 0, caseDef.renderSize.x(), caseDef.renderSize.y()), (uint32_t)clearValues.size(),
                        &clearValues[0]);
        {
...
tcu::TestStatus test(Context &context, const CaseDef caseDef)
{
    ...
    // Step 2: Sample texture
    {
        ...
        const Unique<VkShaderModule> vertexModule(
            createShaderModule(vk, device, context.getBinaryCollection().get("sample_vert"), 0u));
        const Unique<VkShaderModule> fragmentModule(
            createShaderModule(vk, device, context.getBinaryCollection().get("sample_frag"), 0u));
        ...
        const std::vector<PipelineSp> pipelines(
        ...
        beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                        makeRect2D(0, 0, caseDef.renderSize.x(), caseDef.renderSize.y()), tcu::UVec4(0u));
        ...
        for (int layerNdx = 0; layerNdx < DE_LENGTH_OF_ARRAY(numLayers); ++layerNdx)
        {
            ...
            for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(format); ++formatNdx)
            {
                ...
                for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); ++samplesNdx)
                {
                    ...
                    addFunctionCaseWithPrograms(formatGroup.get(), caseName.str(), "", checkSupport, initPrograms,
                                                testFunc, caseDef);
                }
...
{
    return createTestGroup(testCtx, "sampled_image", "Multisampled image direct sample access",
                           createSampledImageTestsInGroup);
}
...
{
    return createTestGroup(testCtx, "storage_image", "Multisampled image draw and read/write in compute shader",
                           createStorageImageTestsInGroup);
}
...
{
    return createTestGroup(testCtx, "standardsampleposition", "Multisampled image standard sample position tests",
                           createStandardSamplePositionTestsInGroup);
}
