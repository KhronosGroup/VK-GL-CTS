...
uint32_t getImageSizeInBytes(const vk::VkExtent3D &baseExtents, const uint32_t layersCount,
                             ...
                             const uint32_t numSamples = 1u);
uint32_t getImageMaxMipLevels(const vk::VkImageFormatProperties &imageFormatProperties, const vk::VkExtent3D &extent);
...
void requireFeatures(const vk::InstanceInterface &instanceInterface, const vk::VkPhysicalDevice physicalDevice,
