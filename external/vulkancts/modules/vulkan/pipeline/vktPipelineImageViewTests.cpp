...
{
public:
    ImageViewTest(tcu::TestContext &testContext, const char *name, const char *description,
...
ImageViewTest::ImageViewTest(tcu::TestContext &testContext, const char *name, const char *description,
                             VkImageViewType imageViewType, VkFormat imageFormat, float samplerLod,
                             const VkComponentMapping &componentMapping,
    ...
    const VkImageSubresourceRange &subresourceRange) const
{
    ...
    return ImageSamplingInstanceParams(renderSize, imageViewType, imageFormat, imageSize, arraySize, componentMapping,
                                       subresourceRange, samplerParams, samplerLod, vertices);
}
...
static de::MovePtr<tcu::TestCaseGroup> createSubresourceRangeTests(tcu::TestContext &testCtx, VkImageViewType viewType,
...
#define ADD_SUBRESOURCE_RANGE_TESTS(TEST_CASES)                                                                     \
    ...
    {                                                                                                               \
        ...
        {                                                                                                           \
            ...
            rangeTests->addChild(new ImageViewTest(testCtx, config.name, desc.str().c_str(), viewType, imageFormat, \
                                                   config.samplerLod, componentMapping, config.subresourceRange));  \
        }                                                                                                           \
    } while (false)
...
static de::MovePtr<tcu::TestCaseGroup> createComponentSwizzleTests(tcu::TestContext &testCtx, VkImageViewType viewType,
                                                                   VkFormat imageFormat)
{
    ...
    {
        swizzleTests->addChild(
            new ImageViewTest(testCtx, getComponentMappingCaseName(componentMappings[mappingNdx]).c_str(), "", viewType,
...
tcu::TestCaseGroup *createImageViewTests(tcu::TestContext &testCtx)
{
    ...
    for (int viewTypeNdx = 0; viewTypeNdx < DE_LENGTH_OF_ARRAY(imageViewTypes); viewTypeNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> formatTests(
            new tcu::TestCaseGroup(testCtx, "format", "Uses samplable formats"));
        ...
        for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> subresourceRangeTests =
                createSubresourceRangeTests(testCtx, viewType, format);
            de::MovePtr<tcu::TestCaseGroup> componentSwizzleTests =
                createComponentSwizzleTests(testCtx, viewType, format);
