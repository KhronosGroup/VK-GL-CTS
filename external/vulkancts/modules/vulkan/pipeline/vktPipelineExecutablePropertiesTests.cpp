...
std::string getShaderFlagStr(const VkShaderStageFlagBits shader, bool isDescription)
{
...
{
public:
    ExecutablePropertiesTestParam(const VkShaderStageFlagBits *shaders, uint32_t count, bool testStatistics,
    ...
    VkShaderStageFlagBits getShaderFlag(uint32_t ndx) const
    {
        return m_shaders[ndx];
    }
    uint32_t getShaderCount(void) const
    {
        return (uint32_t)m_shaderCount;
    }
...
ExecutablePropertiesTestParam::ExecutablePropertiesTestParam(const VkShaderStageFlagBits *shaders, uint32_t count,
...
const std::string ExecutablePropertiesTestParam::generateTestName(void) const
{
    ...
    for (uint32_t ndx = 1; ndx < m_shaderCount; ndx++)
    {
...
public:
    ...
    void enableTessellationStage(uint32_t patchControlPoints);
    Move<VkPipeline> buildPipeline(tcu::UVec2 renderSize, VkRenderPass renderPass, VkPipelineCache cache,
...
SimpleGraphicsPipelineBuilder::SimpleGraphicsPipelineBuilder(Context &context) : m_context(context)
{
...
void SimpleGraphicsPipelineBuilder::bindShaderStage(VkShaderStageFlagBits stage, const char *sourceName,
                                                              ...
                                                              VkPipelineCreateFlags flags)
{
    ...
    const VkPipelineTessellationStateCreateInfo *pTessCreateInfo =
        (m_patchControlPoints > 0) ? &tessStateCreateInfo : DE_NULL;
...
tcu::TestStatus ExecutablePropertiesTestInstance::verifyTestResult(void)
{
    ...
    for (uint32_t ndx = 0; ndx < PIPELINE_CACHE_NDX_COUNT; ndx++)
    {
        ...
        for (uint32_t execNdx = 0; execNdx < executableCount; execNdx++)
        {
            ...
            for (uint32_t stageNdx = 0; stageNdx < m_param->getShaderCount(); stageNdx++)
            {
...
{
    for (uint32_t shaderNdx = 0; shaderNdx < m_param.getShaderCount(); shaderNdx++)
    {
    ...
    , m_missPipelineBuilder(context)
{
    ...
    for (uint32_t ndx = 0; ndx < PIPELINE_CACHE_NDX_COUNT; ndx++)
    {
        ...
        m_pipeline[ndx] =
            m_pipelineBuilder.buildPipeline(m_renderSize, *m_renderPass, *m_cache, *m_pipelineLayout, flags);
...
tcu::TestCaseGroup *createExecutablePropertiesTests(tcu::TestContext &testCtx)
{
    ...
    // Graphics Pipeline Tests
    {
        ...
        const ExecutablePropertiesTestParam testParams[] = {
            ExecutablePropertiesTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), false, false),
            ExecutablePropertiesTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), false, false),
            ExecutablePropertiesTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), false, false),
            ExecutablePropertiesTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), true, false),
            ExecutablePropertiesTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), true, false),
            ExecutablePropertiesTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), true, false),
            ExecutablePropertiesTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), false, true),
            ExecutablePropertiesTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), false, true),
            ExecutablePropertiesTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), false, true),
            ExecutablePropertiesTestParam(testParamShaders0, DE_LENGTH_OF_ARRAY(testParamShaders0), true, true),
            ExecutablePropertiesTestParam(testParamShaders1, DE_LENGTH_OF_ARRAY(testParamShaders1), true, true),
            ExecutablePropertiesTestParam(testParamShaders2, DE_LENGTH_OF_ARRAY(testParamShaders2), true, true),
        };
