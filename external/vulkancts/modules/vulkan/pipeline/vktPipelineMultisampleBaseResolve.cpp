...
tcu::TestStatus MSInstanceBaseResolve::iterate(void)
{
    ...
    const de::UniquePtr<Image> imageMS(
        new Image(deviceInterface, device, allocator, imageMSInfo, MemoryRequirement::Any));
    ...
    const de::UniquePtr<Image> imageRS(
        new Image(deviceInterface, device, allocator, imageRSInfo, MemoryRequirement::Any));
    ...
    de::SharedPtr<Buffer> vertexBuffer = de::SharedPtr<Buffer>(
        new Buffer(deviceInterface, device, allocator,
    ...
    const Unique<VkShaderModule> vsModule(createShaderModule(
        deviceInterface, device, m_context.getBinaryCollection().get("vertex_shader"), (VkShaderModuleCreateFlags)0));
    const Unique<VkShaderModule> fsModule(createShaderModule(
    ...
    const Unique<VkPipeline> graphicsPipeline(makeGraphicsPipeline(
        deviceInterface,                  // const DeviceInterface&                        vk
...

    {
        ...
        beginRenderPass(deviceInterface, *commandBuffer, *renderPass, *framebuffer,
    ...
    const de::UniquePtr<Buffer> bufferRS(
        new Buffer(deviceInterface, device, allocator, bufferRSInfo, MemoryRequirement::HostVisible));
