...
{
public:
    BindPointTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
BindPointTest::BindPointTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             const TestParams &params)
...
void BindPointTest::checkSupport(Context &context) const
{
    if (m_params.graphicsSetUpdateType != SetUpdateType::WRITE || m_params.computeSetUpdateType != SetUpdateType::WRITE)
    {
        ...
        if (m_params.graphicsSetUpdateType == SetUpdateType::PUSH_WITH_TEMPLATE ||
            m_params.computeSetUpdateType == SetUpdateType::PUSH_WITH_TEMPLATE)
...
tcu::TestStatus BindPointInstance::iterate(void)
{
    ...
    if (hasGraphics)
        graphicsPipelineLayout = makePipelineLayout(vkd, device, graphicsSetLayout.get());
    if (hasCompute)
        computePipelineLayout = makePipelineLayout(vkd, device, computeSetLayout.get());
    if (hasRayTracing)
        rayTracingPipelineLayout = makePipelineLayout(vkd, device, rayTracingSetLayout.get());
    ...
    if (hasGraphics)
        vertShader = createShaderModule(vkd, device, m_context.getBinaryCollection().get("vert"), 0u);
    if (hasGraphics)
        fragShader = createShaderModule(vkd, device, m_context.getBinaryCollection().get("frag"), 0u);
    if (hasCompute)
        compShader = createShaderModule(vkd, device, m_context.getBinaryCollection().get("comp"), 0u);
    if (hasRayTracing)
        rgenShader = createShaderModule(vkd, device, m_context.getBinaryCollection().get("rgen"), 0u);
    ...
    if (hasGraphics)
    {
        ...
        graphicsPipeline = makeGraphicsPipeline(vkd, device, graphicsPipelineLayout.get(), vertShader.get(), DE_NULL,
                                                DE_NULL, DE_NULL, fragShader.get(), // Shaders.
                                                renderPass.get(), viewports, scissors,
                                                VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN, 0u, 0u, &vertexInputState);
    }
...
tcu::TestCaseGroup *createBindPointTests(tcu::TestContext &testCtx)
{
    ...
    GroupPtr bindPointGroup(
        new tcu::TestCaseGroup(testCtx, "bind_point", "Tests checking bind points are independent and used properly"));
