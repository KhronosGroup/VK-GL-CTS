...
void updatePreRasterization(Context &context, GraphicsPipelineCreateInfo &graphicsPipelineCreateInfo,
                            bool delayedShaderCreate, VkPolygonMode polygonMode = VK_POLYGON_MODE_FILL)
{
    ...
    const VkShaderModule shaderModule = delayedShaderCreate ? DE_NULL : *graphicsPipelineCreateInfo.m_vertModule;
                                          ...
                                          const bool optimize, const bool delayedShaderCreate)
{
    ...
    const Move<VkPipelineLayout> pipelineLayoutVert =
        makePipelineLayout(vk, device, 2, vecLayoutVert, pipelineLayoutCreateFlag);
    const Move<VkPipelineLayout> pipelineLayoutFrag =
        makePipelineLayout(vk, device, 2, vecLayoutFrag, pipelineLayoutCreateFlag);
...
PipelineLibraryTestCase::PipelineLibraryTestCase(tcu::TestContext &context, const char *name, const char *desc,
...
tcu::TestStatus PipelineLibraryMiscTestInstance::runNullDescriptorSet(void)
{
    ...
    vk::beginCommandBuffer(vk, *m_cmdBuffer, 0u);
    {
        ...
        vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT,
...
tcu::TestCaseGroup *createPipelineLibraryTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "graphics_library", "Tests verifying graphics pipeline libraries"));

    addTestGroup(group.get(), "fast", "Tests graphics pipeline libraries linkage without optimization",
                 addPipelineLibraryConfigurationsTests, false);
    addTestGroup(group.get(), "optimize", "Tests graphics pipeline libraries linkage with optimization",
                 addPipelineLibraryConfigurationsTests, true);

    de::MovePtr<tcu::TestCaseGroup> miscTests(
        new tcu::TestCaseGroup(testCtx, "misc", "Miscellaneous graphics pipeline library tests"));
