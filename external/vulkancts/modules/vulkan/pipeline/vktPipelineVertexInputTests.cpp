...
public:
    ...
    VertexInputTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
public:
    ...
    VertexInputInstance(Context &context, const AttributeDescriptionList &attributeDescriptions,
...
VertexInputTest::VertexInputTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                                 const std::vector<AttributeInfo> &attributeInfos, BindingMapping bindingMapping,
...
TestInstance *VertexInputTest::createInstance(Context &context) const
{
        ...
        for (int descNdx = 0; descNdx < glslTypeDescription.vertexInputCount; ++descNdx)
        {
            ...
            if (m_attributeLayout == ATTRIBUTE_LAYOUT_INTERLEAVED)
            {
                const uint32_t offsetToComponentAlignment = getNextMultipleOffset(
                    getVertexFormatSize(attributeInfo.vkType),
                    (uint32_t)bindingOffsets[attributeBinding] + attributeOffsets[attributeBinding]);
                ...
                attributeMaxSizes[attributeBinding] =
                    de::max(attributeMaxSizes[attributeBinding], getVertexFormatSize(attributeInfo.vkType));
            }
    ...
    for (size_t bindingNdx = 0; bindingNdx < bindingDescriptions.size(); ++bindingNdx)
    {
        if (attributeMaxSizes[bindingNdx] > 0)
            bindingDescriptions[bindingNdx].stride +=
                getNextMultipleOffset(attributeMaxSizes[bindingNdx], bindingDescriptions[bindingNdx].stride);
    }
...
void VertexInputTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    programCollection.glslSources.add("attribute_test_frag")
        << glu::FragmentSource("#version 440\n"
...
std::string VertexInputTest::getGlslInputDeclarations(void) const
{
    ...
    else
    {
        for (size_t attributeNdx = 0; attributeNdx < m_attributeInfos.size(); attributeNdx++)
        {
            ...
            glslInputs << "layout(location = " << m_locations[attributeNdx] << ") in " << glslTypeDesc.name << " attr"
                       << attributeNdx << ";\n";
        }
    }
...
std::string VertexInputTest::getGlslVertexCheck(void) const
{
    ...
    if (m_queryMaxAttributes)
    {
        ...
        glslCode << "    if (okCount == " << totalInputComponentCount
                 ...
                 << totalInputComponentCount << " * numAttributes), 0.0f, 0.0f, 1.0);\n"
                 << "    }\n\n"
                    "    if (gl_InstanceIndex == 0)\n"
                    "    {\n"
    ...
    else
    {
        for (size_t attributeNdx = 0; attributeNdx < m_attributeInfos.size(); attributeNdx++)
        {
            ...
            totalInputComponentCount +=
                vertexInputCount * VertexInputTest::s_glslTypeDescriptions[m_attributeInfos[attributeNdx].glslType]
        ...
        glslCode << "    if (okCount == " << totalInputComponentCount
                 << ")\n"
                    ...
                    "    {\n"
                    "        vtxColor = vec4(okCount / float("
                 << totalInputComponentCount << "), 0.0f, 0.0f, 1.0);\n"
                 << "    }\n\n"
                    "    if (gl_InstanceIndex == 0)\n"
                    "    {\n"
                    "        if (gl_VertexIndex == 0) gl_Position = vec4(-1.0, -1.0, 0.0, 1.0);\n"
        ...
        for (int rowNdx = 0; rowNdx < componentCount; rowNdx++)
        {
            ...
            if (isVertexFormatSint(attributeInfo.vkType))
            {
                if (isVertexFormatPacked(attributeInfo.vkType))
                {
                    ...
                    glslCode << indentStr << "if (" << accessStr << " == -(" << totalComponentCount << " * " << indexId
                             << " + " << componentIndex << "))\n";
            }
            ...
            {
                if (VertexInputTest::s_glslTypeDescriptions[attributeInfo.glslType].basicType ==
                    VertexInputTest::GLSL_BASIC_TYPE_DOUBLE)
                {
                ...
                else
                {
                    glslCode << indentStr << "if (abs(" << accessStr << " + (0.01 * (" << totalComponentCount
                             << ".0 * float(" << indexId << ") + " << componentIndex << ".0))) < " << threshold[rowNdx]
                             << ")\n";
                }
            }
            else if (isVertexFormatSscaled(attributeInfo.vkType))
            {
                if (isVertexFormatPacked(attributeInfo.vkType))
                {
                    ...
                    glslCode << indentStr << "if (abs(" << accessStr << " + (" << totalComponentCount << ".0 * float("
            ...
            else if (isVertexFormatUnorm(attributeInfo.vkType) || isVertexFormatSRGB(attributeInfo.vkType))
            {
                ...
                if (isVertexFormatPacked(attributeInfo.vkType))
                    ...
                    glslCode << indentStr << "if (abs(" << accessStr << " - "
                             << "(" << representableDiff << " * (" << totalComponentCount << ".0 * float(" << indexId
                             << ") + " << componentIndex << ".0))) < " << threshold[rowNdx] << ")\n";
            }
...
VertexInputInstance::VertexInputInstance(Context &context, const AttributeDescriptionList &attributeDescriptions,
                                         const std::vector<VkVertexInputBindingDescription> &bindingDescriptions,
                                         const std::vector<VkDeviceSize> &bindingOffsets)
    ...
    , m_colorFormat(VK_FORMAT_R8G8B8A8_UNORM)
{
    ...
    m_vertexShaderModule =
        createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("attribute_test_vert"), 0);
    m_fragmentShaderModule =
        createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("attribute_test_frag"), 0);
                                 ...
                                 uint32_t componentNdx, float value)
{
    ...
    uint32_t valueUFloat = f16 >> (16 - componentWidth - 1);
...
std::string getAttributeInfoDescription(const VertexInputTest::AttributeInfo &attributeInfo)
{
    ...
    caseDesc << std::string(VertexInputTest::s_glslTypeDescriptions[attributeInfo.glslType].name) << " from type "
             << getFormatName(attributeInfo.vkType) << " with ";
...
void createSingleAttributeCases(tcu::TestCaseGroup *singleAttributeTests, VertexInputTest::GlslType glslType)
{
    const VkFormat vertexFormats[] = {
        ...
        VK_FORMAT_R16G16_SFLOAT, VK_FORMAT_R16G16B16A16_UNORM, VK_FORMAT_R16G16B16A16_SNORM,
        VK_FORMAT_R16G16B16A16_UINT, VK_FORMAT_R16G16B16A16_SINT, VK_FORMAT_R16G16B16A16_SFLOAT, VK_FORMAT_R32_UINT,
        VK_FORMAT_R32_SINT, VK_FORMAT_R32_SFLOAT, VK_FORMAT_R32G32_UINT, VK_FORMAT_R32G32_SINT, VK_FORMAT_R32G32_SFLOAT,
        VK_FORMAT_R32G32B32_UINT, VK_FORMAT_R32G32B32_SINT, VK_FORMAT_R32G32B32_SFLOAT, VK_FORMAT_R32G32B32A32_UINT,
...
void createSingleAttributeTests(tcu::TestCaseGroup *singleAttributeTests)
{
    for (int glslTypeNdx = 0; glslTypeNdx < VertexInputTest::GLSL_TYPE_COUNT; glslTypeNdx++)
    {
        ...
        addTestGroup(singleAttributeTests, VertexInputTest::s_glslTypeDescriptions[glslType].name, "",
                     createSingleAttributeCases, glslType);
    }
}

// Create all unique GlslType combinations recursively
void createMultipleAttributeCases(
    uint32_t depth, uint32_t firstNdx, CompatibleFormats *compatibleFormats, de::Random &randomFunc,
    tcu::TestCaseGroup &testGroup, VertexInputTest::BindingMapping bindingMapping,
    VertexInputTest::AttributeLayout attributeLayout, VertexInputTest::LayoutSkip layoutSkip,
    VertexInputTest::LayoutOrder layoutOrder,
    const std::vector<VertexInputTest::AttributeInfo> &attributeInfos = std::vector<VertexInputTest::AttributeInfo>(0))
{
    ...
    for (uint32_t currentNdx = firstNdx; currentNdx < VertexInputTest::GLSL_TYPE_DOUBLE - depth; currentNdx++)
    {
        ...
        if (depth == 0)
        {
            ...
            testGroup.addChild(new VertexInputTest(testCtx, caseName, caseDesc, newAttributeInfos, bindingMapping,
                                                   attributeLayout, layoutSkip, layoutOrder));
        }
        // Add test group
        else
        {
            ...
            createMultipleAttributeCases(depth - 1u, currentNdx + 1u, compatibleFormats, randomFunc, *newTestGroup,
                                         bindingMapping, attributeLayout, layoutSkip, layoutOrder, newAttributeInfos);
...
void createMultipleAttributeTests(tcu::TestCaseGroup *multipleAttributeTests)
{
    ...
    const VkFormat vertexFormats[] = {VK_FORMAT_R8_UNORM,
                                      VK_FORMAT_R8_SNORM,
                                      VK_FORMAT_R8_UINT,
                                      VK_FORMAT_R8_SINT,
    ...
    for (uint32_t layoutSkipNdx = 0; layoutSkipNdx < DE_LENGTH_OF_ARRAY(layoutSkips); layoutSkipNdx++)
        for (uint32_t layoutOrderNdx = 0; layoutOrderNdx < DE_LENGTH_OF_ARRAY(layoutOrders); layoutOrderNdx++)
        {
            ...
            createMultipleAttributeCases(2u, 0u, compatibleFormats, randomFunc, *oneToOneAttributeTests,
                                         VertexInputTest::BINDING_MAPPING_ONE_TO_ONE,
                                         VertexInputTest::ATTRIBUTE_LAYOUT_INTERLEAVED, layoutSkip, layoutOrder);
            createMultipleAttributeCases(2u, 0u, compatibleFormats, randomFunc, *oneToManyAttributeTests,
                                         VertexInputTest::BINDING_MAPPING_ONE_TO_MANY,
                                         VertexInputTest::ATTRIBUTE_LAYOUT_INTERLEAVED, layoutSkip, layoutOrder);
            createMultipleAttributeCases(2u, 0u, compatibleFormats, randomFunc, *oneToManySequentialAttributeTests,
                                         VertexInputTest::BINDING_MAPPING_ONE_TO_MANY,
                                         VertexInputTest::ATTRIBUTE_LAYOUT_SEQUENTIAL, layoutSkip, layoutOrder);

            if (layoutSkip == VertexInputTest::LAYOUT_SKIP_ENABLED)
            {
                de::MovePtr<tcu::TestCaseGroup> layoutSkipTests(
                    new tcu::TestCaseGroup(testCtx, "layout_skip", "Skip one layout after each attribute"));

                de::MovePtr<tcu::TestCaseGroup> bindingOneToOneTests(
                    new tcu::TestCaseGroup(testCtx, "binding_one_to_one", "Each attribute uses a unique binding"));
                ...
                de::MovePtr<tcu::TestCaseGroup> bindingOneToManyTests(
                    new tcu::TestCaseGroup(testCtx, "binding_one_to_many", "Attributes share the same binding"));
            ...
            {
                de::MovePtr<tcu::TestCaseGroup> layoutOutOfOrderTests(
                    new tcu::TestCaseGroup(testCtx, "out_of_order", "Layout slots out of order"));
...
void createMaxAttributeTests(tcu::TestCaseGroup *maxAttributeTests)
{
    ...
    for (uint32_t attributeCountNdx = 0; attributeCountNdx < DE_LENGTH_OF_ARRAY(attributeCount); attributeCountNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> numAttributeTests(
            new tcu::TestCaseGroup(testCtx, groupName.c_str(), groupDesc.c_str()));
        de::MovePtr<tcu::TestCaseGroup> bindingOneToOneTests(
            new tcu::TestCaseGroup(testCtx, "binding_one_to_one", "Each attribute uses a unique binding"));
        de::MovePtr<tcu::TestCaseGroup> bindingOneToManyTests(
            new tcu::TestCaseGroup(testCtx, "binding_one_to_many", "Attributes share the same binding"));
        ...
        bindingOneToOneTests->addChild(new VertexInputTest(testCtx, "interleaved", "Interleaved attribute layout",
        ...
        bindingOneToManyTests->addChild(new VertexInputTest(
            testCtx, "interleaved", "Interleaved attribute layout", attributeInfos,
            VertexInputTest::BINDING_MAPPING_ONE_TO_MANY, VertexInputTest::ATTRIBUTE_LAYOUT_INTERLEAVED));
        bindingOneToManyTests->addChild(new VertexInputTest(
            testCtx, "sequential", "Sequential attribute layout", attributeInfos,
...
void createVertexInputTests(tcu::TestCaseGroup *vertexInputTests)
{
    ...
    addTestGroup(vertexInputTests, "max_attributes",
                 "Implementations can use as many vertex input attributes as they advertise", createMaxAttributeTests);
}
