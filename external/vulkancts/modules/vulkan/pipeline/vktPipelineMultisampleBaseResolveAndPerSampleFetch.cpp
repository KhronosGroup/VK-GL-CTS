...
tcu::TestStatus MSInstanceBaseResolveAndPerSampleFetch::iterate(void)
{
    ...
    for (uint32_t sampleNdx = 0u; sampleNdx < numSamples; ++sampleNdx)
    {
        imagesPerSampleVec[sampleNdx] =
            de::SharedPtr<Image>(new Image(deviceInterface, device, allocator, imageRSInfo, MemoryRequirement::Any));
    }
    ...
    for (uint32_t sampleNdx = 0u; sampleNdx < (numSamples - 1); ++sampleNdx)
    {
        const VkSubpassDependency subpassDependency = {
    ...
    const Unique<VkPipelineLayout> pipelineLayoutMSPass(
        createPipelineLayout(deviceInterface, device, &pipelineLayoutMSPassParams));
    ...
    const Unique<VkShaderModule> vsMSPassModule(createShaderModule(
        deviceInterface, device, m_context.getBinaryCollection().get("vertex_shader"), (VkShaderModuleCreateFlags)0u));
    const Unique<VkShaderModule> fsMSPassModule(
        createShaderModule(deviceInterface, device, m_context.getBinaryCollection().get("fragment_shader"),
                           (VkShaderModuleCreateFlags)0u));

    const Unique<VkPipeline> graphicsPipelineMSPass(makeGraphicsPipeline(
    ...
    const Unique<VkPipelineLayout> pipelineLayoutPerSampleFetchPass(
        makePipelineLayout(deviceInterface, device, *descriptorSetLayout));

    const uint32_t bufferPerSampleFetchPassSize = 4u * (uint32_t)sizeof(tcu::Vec4);

    de::SharedPtr<Buffer> vertexBufferPerSampleFetchPass = de::SharedPtr<Buffer>(
        new Buffer(deviceInterface, device, allocator,
    ...
    {
        const Unique<VkShaderModule> vsPerSampleFetchPassModule(
            createShaderModule(deviceInterface, device, m_context.getBinaryCollection().get("per_sample_fetch_vs"),
                               (VkShaderModuleCreateFlags)0u));
        const Unique<VkShaderModule> fsPerSampleFetchPassModule(
            createShaderModule(deviceInterface, device, m_context.getBinaryCollection().get("per_sample_fetch_fs"),
                               (VkShaderModuleCreateFlags)0u));
        ...
        for (uint32_t sampleNdx = 0u; sampleNdx < numSamples; ++sampleNdx)
        {
            graphicsPipelinesPerSampleFetch[sampleNdx] = makeVkSharedPtr((makeGraphicsPipeline(
                deviceInterface,                   // const DeviceInterface&                        vk
                device,                            // const VkDevice                                device
                ...
                *renderPass,                 // const VkRenderPass                            renderPass
                ...
                1u + sampleNdx)));                    // const uint32_t                                subpass
        }
    }
    ...
    const de::UniquePtr<Buffer> bufferSampleID(
        new Buffer(deviceInterface, device, allocator, bufferSampleIDInfo, MemoryRequirement::HostVisible));
...

    {
        ...
        for (uint32_t sampleNdx = 0u; sampleNdx < numSamples; ++sampleNdx)
        {
            ...
            deviceInterface.cmdBindPipeline(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                            **graphicsPipelinesPerSampleFetch[sampleNdx]);
    ...
    for (uint32_t sampleNdx = 0u; sampleNdx < numSamples; ++sampleNdx)
    {
        buffersPerSample[sampleNdx] = de::SharedPtr<Buffer>(
            new Buffer(deviceInterface, device, allocator, bufferRSInfo, MemoryRequirement::HostVisible));
