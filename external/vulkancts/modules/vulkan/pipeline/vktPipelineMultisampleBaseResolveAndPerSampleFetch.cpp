...
tcu::TestStatus MSInstanceBaseResolveAndPerSampleFetch::iterate(void)
{
    ...
    graphicsPipelineMSPass.setDefaultColorBlendState()
        ...
        .setupFragmentShaderState(*pipelineLayoutMSPass, *renderPass, 0u, *fsMSPassModule, DE_NULL,
    ...
    // Create graphics pipelines for per sample texel fetch passes
    {
        ...
        for (uint32_t sampleNdx = 0u; sampleNdx < numSamples; ++sampleNdx)
        {
            ...
            graphicsPipelinesPerSampleFetch.emplace_back(deviceInterface, device,
...

    {
        ...
        deviceInterface.cmdBindPipeline(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                        graphicsPipelineMSPass.getPipeline());
        ...
        for (uint32_t sampleNdx = 0u; sampleNdx < numSamples; ++sampleNdx)
        {
            ...
            // Bind graphics pipeline
            deviceInterface.cmdBindPipeline(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                            graphicsPipelinesPerSampleFetch[sampleNdx].getPipeline());
