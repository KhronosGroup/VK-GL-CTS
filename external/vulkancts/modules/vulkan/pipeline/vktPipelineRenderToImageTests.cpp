...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const uint32_t subpass, const bool useDepth, const bool useStencil)
{
    ...
    const VkPipelineCreateFlags flags =
        (basePipeline == DE_NULL ? VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT : VK_PIPELINE_CREATE_DERIVATIVE_BIT);
                                  ...
                                  MemoryRequirement requirement)
{
    ...
    const uint32_t candidates      = allowedMemTypeBits & compatibleTypes;
...
uint32_t getMemoryTypeNdx(Context &context, const CaseDef &caseDef)
{
    ...
    colorImage = makeImage(vk, device, getImageCreateFlags(caseDef.viewType), getImageType(caseDef.viewType),
                           caseDef.colorFormat, imageSize.swizzle(0, 1, 2), 1u, imageSize.w(), imageUsage);
...
VkDeviceSize getMaxDeviceHeapSize(Context &context, const CaseDef &caseDef)
{
    ...
    const VkPhysicalDevice physDevice                       = context.getPhysicalDevice();
    const VkPhysicalDeviceMemoryProperties memoryProperties = getPhysicalDeviceMemoryProperties(vki, physDevice);
    const uint32_t memoryTypeNdx                            = getMemoryTypeNdx(context, caseDef);
...
tcu::TestStatus testWithSizeReduction(Context &context, const CaseDef &caseDef)
{
    ...
    VkDeviceSize depthStencilSize =
        (useDepthStencil ? product(imageSize) * tcu::getPixelSize(mapVkFormat(caseDef.depthStencilFormat)) : 0ull);
    ...
    VkDeviceSize neededMemory =
        static_cast<VkDeviceSize>(static_cast<float>(colorSize + depthStencilSize) * additionalMemory) +
        reserveForChecking;
    ...
    while (!allocationPossible)
    {
        ...
        const VkMemoryAllocateInfo allocateInfo = {
            ...
            getMemoryTypeNdx(context, caseDef)      //uint32_t memoryTypeIndex;
        };
        ...
        if (VK_ERROR_OUT_OF_DEVICE_MEMORY == result || VK_ERROR_OUT_OF_HOST_MEMORY == result)
        {
            ...
            colorSize = product(imageSize) * tcu::getPixelSize(mapVkFormat(caseDef.colorFormat));
            depthStencilSize =
                (useDepthStencil ? product(imageSize) * tcu::getPixelSize(mapVkFormat(caseDef.depthStencilFormat)) :
                                   0ull);
            neededMemory =
                static_cast<VkDeviceSize>(static_cast<double>(colorSize + depthStencilSize) * additionalMemory) +
                reserveForChecking;
        }
    ...
    const Unique<VkRenderPass> renderPass(
        makeRenderPass(vk, device, caseDef.colorFormat, caseDef.depthStencilFormat, static_cast<uint32_t>(numSlices),
    ...
    // For each image layer or slice (3D), create an attachment and a pipeline
    {
        ...
        for (int subpassNdx = 0; subpassNdx < numSlices; ++subpassNdx)
        {
            ...
            pipelines.push_back(makeSharedPtr(
                makeGraphicsPipeline(vk, device, basePipeline, *pipelineLayout, *renderPass, *vertexModule,
    ...
    framebuffer =
        makeFramebuffer(vk, device, *renderPass, static_cast<uint32_t>(attachmentHandles.size()), &attachmentHandles[0],
...
void drawToMipLevel(const Context &context, const CaseDef &caseDef, const int mipLevel, const IVec4 &mipSize,
                    ...
                    const VkBuffer vertexBuffer, const VkPipelineLayout pipelineLayout,
                    const VkShaderModule vertexModule, const VkShaderModule fragmentModule)
{
    const DeviceInterface &vk                   = context.getDeviceInterface();
    const VkDevice device                       = context.getDevice();
    ...
    const Unique<VkFramebuffer> framebuffer(
        makeFramebuffer(vk, device, *renderPass, static_cast<uint32_t>(attachmentHandles.size()), &attachmentHandles[0],
                        static_cast<uint32_t>(mipSize.x()), static_cast<uint32_t>(mipSize.y())));

    {
        ...
        beginCommandBuffer(vk, *cmdBuffer);
        {
            ...
            beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, makeRect2D(0, 0, mipSize.x(), mipSize.y()),
                            (uint32_t)clearValues.size(), &clearValues[0]);
...
void addTestCasesWithFunctions(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
    ...
    for (int caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(testCase); ++caseNdx)
    {
        ...
        {
            const vector<IVec4> sizes = genSizeCombinations(testCase[caseNdx].baselineSize, testCase[caseNdx].sizeMask,
...
{
    de::MovePtr<tcu::TestCaseGroup> renderToImageTests(
        new tcu::TestCaseGroup(testCtx, "render_to_image", "Render to image tests"));

    renderToImageTests->addChild(
        createTestGroup(testCtx, "core", "Core render to image tests", addCoreRenderToImageTests));
    renderToImageTests->addChild(createTestGroup(testCtx, "dedicated_allocation",
                                                 ...
                                                 addDedicatedAllocationRenderToImageTests));
