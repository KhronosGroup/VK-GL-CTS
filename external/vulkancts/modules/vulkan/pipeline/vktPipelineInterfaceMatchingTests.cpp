...
tcu::TestStatus InterfaceMatchingTestInstance::iterate(void)
{
    ...
    m_graphicsPipeline = makeGraphicsPipeline(
        ...
        *m_renderPass,              // VkRenderPass                        renderPass
        ...
        useTess ? VK_PRIMITIVE_TOPOLOGY_PATCH_LIST :
                  VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, // VkPrimitiveTopology                topology
        0u,                                             // const uint32_t                    subpass
        useTess ? 1u : 0u);                             // const uint32_t                    patchControlPoints
    ...
    beginRenderPass(vk, *m_cmdBuffer, *m_renderPass, *m_framebuffer, makeRect2D(renderSize),
...
tcu::TestCaseGroup *createInterfaceMatchingTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> vectorMatching(
        new tcu::TestCaseGroup(testCtx, "vector_length", "Tests vector matching"));
            ...
            for (uint32_t vecDataFormat = 0; vecDataFormat < 3; ++vecDataFormat)
            {
                ...
                for (uint32_t outVecSizeIndex = 0; outVecSizeIndex < 3; ++outVecSizeIndex)
                {
                    ...
                    for (uint32_t inVecSizeIndex = 0; inVecSizeIndex < 3; ++inVecSizeIndex)
                    {
                        ...
                        auto testParams =
                            new TestParams{TestType::VECTOR_LENGTH, outVecType,   inVecType, DecorationType::NONE,
                                           DecorationType::NONE,    pipelineType, defType};
    ...
    de::MovePtr<tcu::TestCaseGroup> decorationMismatching(
        new tcu::TestCaseGroup(testCtx, "decoration_mismatch", "Decoration mismatch tests"));
    for (PipelineType stageType : pipelineTypeList)
        for (DefinitionType defType : definitionsTypeList)
            for (const auto &decoration : decorationPairs)
            {
                ...
                auto testParams = new TestParams{TestType::DECORATION_MISMATCH,
                                                 VecType::VEC4,
                                                 VecType::VEC4,
                                                 decoration.first,
                                                 decoration.second,
