...
Specialization::Specialization(const std::vector<SpecConstant> &specConstants)
{
    ...
    for (std::vector<SpecConstant>::const_iterator it = specConstants.begin(); it != specConstants.end(); ++it)
        if (it->size != 0)
        {
            ...
            offset += (uint32_t)sizeof(GenericValue);
        }
...
SpecConstantTest::SpecConstantTest(tcu::TestContext &testCtx, const VkShaderStageFlagBits stage,
                                   const CaseDefinition &caseDef)
...
{
public:
    ComputeTestInstance(Context &context, const VkDeviceSize ssboSize, const std::vector<SpecConstant> &specConstants,
...
ComputeTestInstance::ComputeTestInstance(Context &context, const VkDeviceSize ssboSize,
...
{
public:
    GraphicsTestInstance(Context &context, const VkDeviceSize ssboSize, const std::vector<SpecConstant> &specConstants,
...
GraphicsTestInstance::GraphicsTestInstance(Context &context, const VkDeviceSize ssboSize,
                                           const std::vector<SpecConstant> &specConstants,
                                           const std::vector<OffsetValue> &expectedValues,
...
tcu::TestStatus GraphicsTestInstance::iterate(void)
{
    ...
    const Image colorImage(vk, device, allocator,
    ...
    const Unique<VkFramebuffer> framebuffer(makeFramebuffer(vk, device, *renderPass, colorImageView.get(),
    ...
    pipelineBuilder.setRenderSize(renderSize)
    ...
    if ((m_stage == VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT) ||
        ...
        pipelineBuilder
            .setShader(vk, device, VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT,
                       m_context.getBinaryCollection().get("tesc"), pSpecInfo)
            .setShader(vk, device, VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT,
                       m_context.getBinaryCollection().get("tese"), pSpecInfo);

    if (m_stage == VK_SHADER_STAGE_GEOMETRY_BIT)
        pipelineBuilder.setShader(vk, device, VK_SHADER_STAGE_GEOMETRY_BIT, m_context.getBinaryCollection().get("geom"),
                                  pSpecInfo);
...
TestInstance *SpecConstantTest::createInstance(Context &context) const
{
    if (m_stage & VK_SHADER_STAGE_COMPUTE_BIT)
        ...
        return new GraphicsTestInstance(context, m_caseDef.ssboSize, m_caseDef.specConstants, m_caseDef.expectedValues,
                                        m_stage);
}
...
tcu::TestCaseGroup *createDefaultValueTests(tcu::TestContext &testCtx, const VkShaderStageFlagBits shaderStage)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "default_value", "use default constant value"));
...
tcu::TestCaseGroup *createBasicSpecializationTests(tcu::TestContext &testCtx, const VkShaderStageFlagBits shaderStage)
{
    ...
    const CaseDefinition defs[] = {
        ...
        },
        {
            "double",
            makeVector(
                SpecConstant(1u, "layout(constant_id = ${ID}) const double sc0 = 2.75LF;", 8, makeValueFloat64(22.5)),
            ...
            "    sb_out.r0 = sc0;\n"
            "    sb_out.r1 = sc1;\n",
            makeVector(OffsetValue(8, 0, makeValueFloat64(22.5)), OffsetValue(8, 8, makeValueFloat64(9.25))),
            FEATURE_SHADER_FLOAT_64,
        },
    };
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "local_size", "work group size specialization"));
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "expression", "specialization constants usage in expressions"));
...
{
    de::MovePtr<tcu::TestCaseGroup> compositeTests(
        new tcu::TestCaseGroup(testCtx, "composite", "specialization constants usage in composite types"));
    ...
    // Vectors
    {
        ...
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(types); ++typeNdx)
            group->addChild(
                new SpecConstantTest(testCtx, shaderStage, makeMatrixVectorCompositeCaseDefinition(types[typeNdx])));
    ...
    // Array cases
    {
        ...
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(allTypes); ++typeNdx)
            group->addChild(
                new SpecConstantTest(testCtx, shaderStage, makeArrayCompositeCaseDefinition(allTypes[typeNdx], 3)));

        // Array of array of T
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(allTypes); ++typeNdx)
            group->addChild(
                new SpecConstantTest(testCtx, shaderStage, makeArrayCompositeCaseDefinition(allTypes[typeNdx], 3, 2)));
    ...
    // Struct cases
    {
        ...
        for (int typeNdx = 0; typeNdx < DE_LENGTH_OF_ARRAY(allTypes); ++typeNdx)
            group->addChild(
                new SpecConstantTest(testCtx, shaderStage, makeStructCompositeCaseDefinition(allTypes[typeNdx])));
...
{
    de::MovePtr<tcu::TestCaseGroup> allTests(
        new tcu::TestCaseGroup(testCtx, "spec_constant", "Specialization constants tests"));
