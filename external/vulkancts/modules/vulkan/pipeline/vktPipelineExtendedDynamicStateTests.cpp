...
public:
    ...
    virtual std::vector<std::string> getDescriptorCoordCalc() const override
    {
        ...
        statements.push_back("const bool provokingLast = " + std::string(m_lastVertex ? "true" : "false") + ";");
        statements.push_back("colorMultiplier[gl_LocalInvocationIndex] = (((!provokingLast && gl_LocalInvocationIndex "
                             "== 0) || (provokingLast && gl_LocalInvocationIndex == 2)) ? 1 : 0);");
        return statements;
    }
...
struct TestConfig
{
    ...
    {
        return ((primRestartEnableConfig.dynamicValue && !m_swappedValues) ?
    ...
    {
        return ((logicOpEnableConfig.dynamicValue && !m_swappedValues) ? logicOpEnableConfig.dynamicValue.get() :
                                                                         logicOpEnableConfig.staticValue);
    }
...
void ExtendedDynamicStateTest::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    if (m_testConfig.useMeshShaders)
    {
        ...
        meshSource
            ...
            << descCalcs.str() << "    SetMeshOutputsEXT(3u, 1u);\n"
...
tcu::TestStatus ExtendedDynamicStateInstance::iterate(void)
{
    ...
    const auto teseModule =
        (m_testConfig.needsTessellation() ? vk::createShaderModule(vkd, device, binaries.get("tese")) :
                                            vk::Move<vk::VkShaderModule>());
    const auto meshModule = (m_testConfig.useMeshShaders ? vk::createShaderModule(vkd, device, binaries.get("mesh")) :
                                                           vk::Move<vk::VkShaderModule>());
    ...
    {
        pColorBlendAdvanced =
            ColorBlendAdvancedPtr(new vk::VkPipelineColorBlendAdvancedStateCreateInfoEXT(vk::initVulkanStructure()));
    ...
    vk::GraphicsPipelineWrapper graphicsPipeline(vkd, device, m_testConfig.pipelineConstructionType);
    {
        ...
        if (m_testConfig.useMeshShaders)
        {
            graphicsPipeline.setupPreRasterizationMeshShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u,
                                                                  DE_NULL, *meshModule, &rasterizationStateCreateInfo);
        }
        else
#endif // CTS_USES_VULKANSC
        {
    ...
    for (const auto &kMeshShadingCase : kMeshShadingCases)
        for (const auto &kOrderingCase : kOrderingCases)
        {
            ...
            // Color blend equation.
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "color_blend_equation_new_color",
                                                 "Dynamically set a color equation that picks the mesh color", config));

                config.colorBlendEquationConfig.swapValues();
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));

                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "color_blend_equation_old_color",
                    "Dynamically set a color equation that picks the clear color", config));
            }
            ...
            // Color blend advanced.
            {
                ...
                // The dynamic value picks the new color.
                config.colorBlendEquationConfig.dynamicValue =
                    ColorBlendEq(vk::VK_BLEND_FACTOR_ONE, vk::VK_BLEND_FACTOR_ZERO, vk::VK_BLEND_OP_LIGHTEN_EXT,
                                 vk::VK_BLEND_FACTOR_ONE, vk::VK_BLEND_FACTOR_ZERO, vk::VK_BLEND_OP_LIGHTEN_EXT);

                config.colorBlendEnableConfig.staticValue = true;

                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "color_blend_equation_advanced_new_color",
                    "Dynamically set an advanced color equation that picks the mesh color", config));

                config.colorBlendEquationConfig.swapValues();
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));

                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "color_blend_equation_advanced_old_color",
                    "Dynamically set an advanced color equation that picks the clear color", config));
            }
