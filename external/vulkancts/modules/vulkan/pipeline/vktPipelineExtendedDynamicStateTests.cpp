...
public:
    ...
    // Vertex bindings for VkPipelineVertexInputStateCreateInfo.
    virtual std::vector<vk::VkVertexInputBindingDescription> getBindingDescriptions(
    ...
    virtual std::vector<std::vector<uint8_t>> createVertexData(const std::vector<tcu::Vec2> &coords,
                                                               ...
                                                               vk::VkDeviceSize trailingPadding,
                                                               const void *paddingPattern,
                                                               size_t patternSize) const override
    {
...
{
public:
    ExtendedDynamicStateTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
ExtendedDynamicStateTest::ExtendedDynamicStateTest(tcu::TestContext &testCtx, const std::string &name,
                                                   const std::string &description, const TestConfig &testConfig)
...
void ExtendedDynamicStateTest::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    fragSourceTemplateStream
        ...
        << (m_testConfig.representativeFragmentTest ?
                "layout(set=" + fragSetIndex +
    ...
    fragSourceTemplateStream << "${FRAG_CALCULATIONS}"
                             << (m_testConfig.representativeFragmentTest ?
                                     "    atomicAdd(counterBuffer.fragCounter, 1u);\n" :
                                     "")
                             << "}\n";
...
public:
    ShadingRateImageDeviceHelper(Context &context)
    {
        ...
        vk::VkPhysicalDeviceShadingRateImageFeaturesNV shadingRateImageFeatures =
            vk::initVulkanStructure(&eds3Features);
...
tcu::TestStatus ExtendedDynamicStateInstance::iterate(void)
{
    ...
    const auto dynamicFragModule =
        vk::createShaderModule(vkd, device, m_context.getBinaryCollection().get("dynamicFrag"), 0u);
    const auto staticFragModule =
        vk::createShaderModule(vkd, device, m_context.getBinaryCollection().get("staticFrag"), 0u);
    const auto geomModule =
        (m_testConfig.needsGeometryShader() ? vk::createShaderModule(vkd, device, binaries.get("geom")) :
                                              vk::Move<vk::VkShaderModule>());
    const auto tescModule =
        (m_testConfig.needsTessellation() ? vk::createShaderModule(vkd, device, binaries.get("tesc")) :
                                            vk::Move<vk::VkShaderModule>());
    const auto teseModule =
        (m_testConfig.needsTessellation() ? vk::createShaderModule(vkd, device, binaries.get("tese")) :
                                            vk::Move<vk::VkShaderModule>());
    const auto dynamicMeshModule =
        (m_testConfig.useMeshShaders ? vk::createShaderModule(vkd, device, binaries.get("dynamicMesh")) :
                                       vk::Move<vk::VkShaderModule>());
    const auto staticMeshModule =
        (m_testConfig.useMeshShaders ? vk::createShaderModule(vkd, device, binaries.get("staticMesh")) :
                                       vk::Move<vk::VkShaderModule>());
    const auto meshNoOutModule =
        (m_testConfig.bindUnusedMeshShadingPipeline ? vk::createShaderModule(vkd, device, binaries.get("meshNoOut")) :
                                                      vk::Move<vk::VkShaderModule>());
    ...
    const vk::VkPipelineColorBlendStateCreateInfo colorBlendStateCreateInfo = {
        ...
        static_cast<uint32_t>(
            colorBlendAttachmentStateVec.size()), // uint32_t                                      attachmentCount
    ...
    if (m_testConfig.useExtraDynPCPPipeline)
    {
        ...
        extraDynPCPPipeline = vk::makeGraphicsPipeline(
            vkd, device, *extraPipelineLayout, vertDPCPModule.get(), DE_NULL, DE_NULL, DE_NULL, fragDPCPModule.get(),
    ...
    if (useStaticPipeline)
    {
        ...
        if (m_testConfig.useMeshShaders)
        {
            staticPipeline.setupPreRasterizationMeshShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u,
                                                                VK_NULL_HANDLE, *staticMeshModule,
...
#endif // CTS_USES_VULKANSC
            .setupFragmentShaderState(*pipelineLayout, *renderPass, 0u, *staticFragModule, &depthStencilStateCreateInfo,
    ...
    vk::GraphicsPipelineWrapper graphicsPipeline(vkd, device, m_testConfig.pipelineConstructionType);
    {
        ...
        if (m_testConfig.useMeshShaders)
        {
            graphicsPipeline.setupPreRasterizationMeshShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u,
                                                                  DE_NULL, *dynamicMeshModule,
                                                                  &rasterizationStateCreateInfo);
        }
        else
#endif // CTS_USES_VULKANSC
        {
            graphicsPipeline.setupVertexInputState(&vertexInputStateCreateInfo, &inputAssemblyStateCreateInfo)
                .setupPreRasterizationShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u,
                                                  *dynamicVertModule, &rasterizationStateCreateInfo, *tescModule,
                                                  *teseModule, *geomModule);
        }
        ...
        graphicsPipeline
            ...
            .setRepresentativeFragmentTestState(pReprFragment.get())
#endif // CTS_USES_VULKANSC
            .setupFragmentShaderState(*pipelineLayout, *renderPass, 0u, *dynamicFragModule,
    ...
    if (m_testConfig.bindUnusedMeshShadingPipeline)
    {
        ...
        meshNoOutPipeline.setDynamicState(&meshNoOutDynamicStateInfo)
            .setDefaultPatchControlPoints(m_testConfig.patchControlPointsConfig.staticValue)
            .setupPreRasterizationMeshShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u, VK_NULL_HANDLE,
                                                  *meshNoOutModule, &rasterizationStateCreateInfo)
            .setupFragmentShaderState(*pipelineLayout, *renderPass, 0u, VK_NULL_HANDLE, &depthStencilStateCreateInfo,
                                      &multisampleStateCreateInfo)
            .setupFragmentOutputState(*renderPass, 0u, &colorBlendStateCreateInfo, &multisampleStateCreateInfo)
            .setMonolithicPipelineLayout(*pipelineLayout)
            .buildPipeline();
    ...
    for (uint32_t iteration = 0u; iteration < kNumIterations; ++iteration)
    {
        ...
        // Begin render pass.
        vk::beginRenderPass(vkd, cmdBuffer, renderPass.get(), framebuffers[iteration].get(),
...
tcu::TestCaseGroup *createExtendedDynamicStateTests(tcu::TestContext &testCtx,
                                                    vk::PipelineConstructionType pipelineConstructionType)
{
    GroupPtr extendedDynamicStateGroup(
        new TestGroupWithClean(testCtx, "extended_dynamic_state", "Tests for VK_EXT_extended_dynamic_state"));
    GroupPtr meshShaderGroup(
        new tcu::TestCaseGroup(testCtx, "mesh_shader", "Extended dynamic state with mesh shading pipelines"));
    ...
    static const struct
    {
        ...
        {SequenceOrdering::BETWEEN_PIPELINES, "between_pipelines",
         "Dynamic after a pipeline with static states has been bound and before a pipeline with dynamic states has "
         "been bound"},
        ...
        {SequenceOrdering::BEFORE_GOOD_STATIC, "before_good_static",
         "Dynamic state set after a dynamic pipeline has been bound and before a second static-state pipeline with the "
         "right values has been bound"},
    ...
    for (const auto &kMeshShadingCase : kMeshShadingCases)
        for (const auto &kOrderingCase : kOrderingCases)
        {
            ...
            GroupPtr orderingGroup(
                new tcu::TestCaseGroup(testCtx, kOrderingCase.name.c_str(), kOrderingCase.desc.c_str()));
            ...
            // Cull modes.
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "cull_none", "Dynamically set cull mode to none", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "cull_back", "Dynamically set cull mode to back", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "cull_front", "Dynamically set cull mode to front", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "cull_front_and_back", "Dynamically set cull mode to front and back", config));
            }
            ...
            // Front face.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "front_face_cw", "Dynamically set front face to clockwise", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "front_face_ccw", "Dynamically set front face to counter-clockwise", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                config.cullModeConfig.staticValue   = vk::VK_CULL_MODE_BACK_BIT;
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "front_face_cw_reversed",
                    "Dynamically set front face to clockwise with a counter-clockwise mesh", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                // Pass triangles in clockwise order.
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "front_face_ccw_reversed",
                    "Dynamically set front face to counter-clockwise with a clockwise mesh", config));
            }
            ...
            // Rasterizer discard
            {
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "disable_raster", "Dynamically disable rasterizer", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "enable_raster", "Dynamically enable rasterizer", config));
            }
            ...
            // Logic op
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "logic_op_or", "Dynamically change logic op to VK_LOGIC_OP_OR", config));
            }
            ...
            // Logic op enable.
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "logic_op_enable", "Dynamically enable logic OP", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "logic_op_disable", "Dynamically disable logic OP", config));
            }
            ...
            // Color blend enable.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "color_blend_enable",
                                                                     "Dynamically enable color blending", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "color_blend_disable",
                                                                     "Dynamically disable color blending", config));
            }
                ...
                for (const auto &cbSubCase : cbSubCases)
                {
                    ...
                    for (int j = 0; j < 2; ++j)
                    {
                        ...
                        const std::string descSuffix =
                            (onlyEq ? "" :
                                      (allCBDynamic ? " and dynamically enable color blending" :
                                                      " and dynamically enable color blending except for logic op"));

                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "color_blend_equation_new_color" + nameSuffix,
                            "Dynamically set a color equation that picks the mesh color" + descSuffix, config));
                        ...
                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "color_blend_equation_old_color" + nameSuffix,
                            "Dynamically set a color equation that picks the clear color" + descSuffix, config));
                    }
                }
            }
            ...
            // Color blend advanced.
            {
                for (const auto &cbSubCase : cbSubCases)
                {
                    ...
                    for (int j = 0; j < 2; ++j)
                    {
                        ...
                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "color_blend_equation_advanced_new_color" + nameSuffix,
                            "Dynamically set an advanced color equation that picks the mesh color" + descSuffix,
                            config));
                        ...
                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "color_blend_equation_advanced_old_color" + nameSuffix,
                            "Dynamically set an advanced color equation that picks the clear color" + descSuffix,
                            config));
                    }
                }
            }
            ...
            // All color blend as dynamic, including both blend equations.
            {
                for (int i = 0; i < 2; ++i)
                {
                    for (int j = 0; j < 2; ++j)
                    {
                        ...
                        const auto testDesc = std::string(
                            std::string("Set all color blend to dynamic and dynamically set color blend to ") +
                            colorBlendStateName + " and pick the " + colorBlendResultName + " color");
            ...
            // Null color blend pipeline pAttachments pointer with all structure contents as dynamic states.
            {
                ...
                const char *testDesc = "Set all VkPipelineColorBlendAttachmentState substates as dynamic and pass a "
                                       "null pointer in VkPipelineColorBlendStateCreateInfo::pAttachments";
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, testName, testDesc, config));
            }
            ...
            // Dynamic color blend equation with dual blending.
            {
                ...
                for (size_t dynamicPick = 0u; dynamicPick < de::arrayLength(dualSrcCases); ++dynamicPick)
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "color_blend_dual_index_" + indexStr,
                        "Dynamically change dual source blending equation to pick color index " + indexStr, config));
                }
            }
            ...
            if (!kUseMeshShaders)
            {
                for (const auto &bindUnusedCase : kBindUnusedCases)
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, std::string("prim_restart_enable") + bindUnusedCase.nameSuffix,
                        "Dynamically enable primitiveRestart" + bindUnusedCase.descSuffix, config));
                }
            }
            ...
            if (!kUseMeshShaders)
            {
                for (const auto &bindUnusedCase : kBindUnusedCases)
                {
                    ...
                    config.bindUnusedMeshShadingPipeline         = bindUnusedCase.bindUnusedMeshShadingPipeline;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "patch_control_points" + bindUnusedCase.nameSuffix,
                        "Dynamically change patch control points" + bindUnusedCase.descSuffix, config));
                }

                {
                    ...
                    const auto testDesc = "Dynamically change patch control points and draw first with a pipeline "
                                          "using the state and no tessellation shaders";
            ...
            {
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "tess_domain_origin_lower_left",
                                                     "Dynamically set the right domain origin to lower left", config));
                }
                {
                    TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                    config.topologyConfig.staticValue           = vk::VK_PRIMITIVE_TOPOLOGY_PATCH_LIST;
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "tess_domain_origin_upper_left",
                                                     "Dynamically set the right domain origin to upper left", config));
                }
            }
                    ...
                    for (const auto &kTopologyCase : kTopologyCases)
                    {
                        ...
                        for (const auto &bindUnusedCase : kBindUnusedCases)
                        {
                            ...
                            const std::string desc =
                                "Dynamically switch primitive topologies from the " + className + " class" +
                                (forceGeometryShader ? " and use a geometry shader" : "") + bindUnusedCase.descSuffix;
            ...
            // Line stipple enable.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "line_stipple_disable",
                                                                     "Dynamically disable line stipple", config));

                config.lineStippleEnableConfig.swapValues();
                config.referenceColor.reset(
                    new VerticalStripesGenerator(kDefaultTriangleColor, kDefaultClearColor, 1u));

                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "line_stipple_enable",
                                                                     "Dynamycally enable line stipple", config));
            }
            ...
            // Line stipple params.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "line_stipple_params", "Dynamically change the line stipple parameters", config));
            }
            ...
            // Line rasterization mode.
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "line_raster_mode_bresenham",
                                                 "Dynamically set line rasterization mode to bresenham", config));

                config.lineRasterModeConfig.swapValues();
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "line_raster_mode_rectangular",
                                                 "Dynamically set line rasterization mode to rectangular", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "line_raster_mode_smooth", "Dynamically set line rasterization mode to smooth", config));
            }
            ...
            // Viewport.
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "2_viewports", "Dynamically set 2 viewports", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "1_full_viewport", "Dynamically set viewport to cover full framebuffer", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "2_viewports_switch", "Dynamically switch the order with 2 viewports", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "2_viewports_switch_clean",
                    "Dynamically switch the order with 2 viewports resulting in clean image", config));
            }
            ...
            // Scissor.
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "2_scissors", "Dynamically set 2 scissors", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "1_full_scissor", "Dynamically set scissor to cover full framebuffer", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "2_scissors_switch", "Dynamically switch the order with 2 scissors", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "2_scissors_switch_clean", "Dynamically switch the order with 2 scissors to avoid drawing",
                    config));
            }
            ...
            if (!kUseMeshShaders)
            {
                ...
                for (const auto &strideCase : strideCases)
                {
                    ...
                    for (const auto &bindUnusedCase : kBindUnusedCases)
                    {
...

                        {
                            ...
                            config.bindUnusedMeshShadingPipeline = bindUnusedCase.bindUnusedMeshShadingPipeline;
                            orderingGroup->addChild(new ExtendedDynamicStateTest(
                                testCtx, prefix + bindUnusedCase.nameSuffix,
                                "Dynamically set stride" + bindUnusedCase.descSuffix, config));
                        }
                        {
                            TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders, factory);
                            config.strideConfig.staticValue      = halfStrides;
                            ...
                            config.bindUnusedMeshShadingPipeline = bindUnusedCase.bindUnusedMeshShadingPipeline;
                            orderingGroup->addChild(new ExtendedDynamicStateTest(
                                testCtx, prefix + "_with_offset" + bindUnusedCase.nameSuffix,
                                "Dynamically set stride using a nonzero vertex data offset" + bindUnusedCase.descSuffix,
                                config));
                        }
                        {
                            TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders, factory);
                            ...
                            orderingGroup->addChild(new ExtendedDynamicStateTest(
                                testCtx, prefix + "_with_offset_and_padding" + bindUnusedCase.nameSuffix,
                                "Dynamically set stride using a nonzero vertex data offset and extra bytes" +
                                    bindUnusedCase.descSuffix,
                                config));
                        }
                    }
                }
                ...
                if (kOrdering != SequenceOrdering::TWO_DRAWS_STATIC && kOrdering != SequenceOrdering::TWO_DRAWS_DYNAMIC)
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "zero_stride_with_offset",
                        "Dynamically set zero stride using a nonzero vertex data offset", config));
                }
            }
            ...
            // Depth test enable.
            {
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "depth_test_enable",
                                                                     "Dynamically enable depth test", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                config.depthTestEnableConfig.staticValue  = true;
                config.depthTestEnableConfig.dynamicValue = tcu::just(false);
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "depth_test_disable",
                                                                     "Dynamically disable depth test", config));
            }
            ...
            // Depth write enable.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "depth_write_enable", "Dynamically enable writes to the depth buffer", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "depth_write_disable", "Dynamically disable writes to the depth buffer", config));
            }
            ...
            // Depth clamp enable.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "depth_clamp_enable",
                                                                     "Dynamically enable depth clamp", config));
            }
            {
                // Reverse situation.
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "depth_clamp_disable",
                                                                     "Dynamically disable depth clamp", config));
            }
            ...
            // Polygon mode.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "polygon_mode_point", "Dynamically set polygon draw mode to points", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "polygon_mode_fill", "Dynamically set polygon draw mode to fill", config));
            }
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                // Sample count.
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, namePrefix + "rasterization_samples",
                        "Dynamically set the rasterization sample count" + descSuffix, config));
                }
                ...
                // Sample mask
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, namePrefix + "sample_mask_enable",
                        "Dynamically set a sample mask that allows drawing" + descSuffix, config));
                }
                {
                    TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                    config.rasterizationSamplesConfig    = activeSampleCount;
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, namePrefix + "sample_mask_disable",
                        "Dynamically set a sample mask that prevents drawing" + descSuffix, config));
                }
                ...
                // Alpha to coverage.
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, namePrefix + "alpha_to_coverage_enable",
                                                     "Dynamically enable alpha to coverage" + descSuffix, config));
                }
                {
                    TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, namePrefix + "alpha_to_coverage_disable",
                                                     "Dynamically disable alpha to coverage" + descSuffix, config));
                }
                ...
                // Alpha to one.
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, namePrefix + "alpha_to_one_enable",
                                                                         "Dynamically enable alpha to one" + descSuffix,
                                                                         config));
                }
                {
                    TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, namePrefix + "alpha_to_one_disable",
                                                     "Dynamically disable alpha to one" + descSuffix, config));
                }
            }
            ...
            // Special sample mask case: make sure the dynamic sample mask count does not overwrite the actual sample mask.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "sample_mask_count", "Dynamically set sample mask with slightly different sample count",
                    config));
            }
            ...
            // Color write mask.
            {
                ...
                for (const auto &colorCompCase : colorComponentCases)
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, testName, "Dynamically set color write mask to " + dynamicCode, config));
                }
            }
            ...
            if (!kUseMeshShaders)
            {
                ...
                for (const auto &rastStreamCase : rastStreamCases)
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, testName, "Dynamically switch rasterization streams", config));
                }
            }
            ...
            // Provoking vertex mode.
            {
                ...
                for (const auto &provokingVtxCase : provokingVtxCases)
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, provokingVtxCase.name, provokingVtxCase.desc, config));
                }
            }
            ...
            // Depth clip negative one to one.
            {
                ...
                for (const auto &negOneToOneCase : negativeOneToOneCases)
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, negOneToOneCase.name, negOneToOneCase.desc, config));
                }
            }
            ...
            // Depth clip enable.
            {
                ...
                for (const auto &depthClipEnableCase : depthClipEnableCases)
                {
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, depthClipEnableCase.name,
                                                                         depthClipEnableCase.desc, config));
                }
            }
            ...
            // Sample locations enablement.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "sample_locations_enable",
                                                                     "Dynamically enable sample locations", config));
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "sample_locations_disable",
                                                                     "Dynamically disable sample locations", config));
            }
            ...
            // Coverage to color enable.
            {
                for (int i = 0; i < 2; ++i)
                {
                    ...
                    for (int j = 0; j < 2; ++j)
                    {
                        ...
                        const auto testDesc = "Dynamically " + finalState + " coverage to color in " +
                                              (multisample ? "multisample" : "single sample") + " images";
            ...
            // Coverage to color location.
            {
                for (int i = 0; i < 2; ++i)
                {
                    ...
                    for (int j = 0; j < 2; ++j)
                    {
                        ...
                        const auto testDesc = "Dynamically enable coverage to color in location " + locName +
                                              " using " + (multisample ? "multisample" : "single sample") + " images";
            ...
            // Coverage modulation mode.
            {
                ...
                for (const auto &modulationModeCase : modulationModeCases)
                {
                    ...
                    const auto testDesc =
                        std::string("Dynamically set coverage modulation mode to ") + modulationModeCase.name;
            ...
            // Coverage modulation table enable.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "coverage_modulation_table_enable",
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "coverage_modulation_table_disable",
            ...
            // Coverage modulation table.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "coverage_modulation_table_change",
            ...
            // Coverage reduction mode.
            {
                ...
                config.referenceColor.reset(new TopLeftBorderGenerator(kDefaultTriangleColor, kDefaultClearColor,
                                                                       kDefaultClearColor, kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "coverage_reduction_truncate",
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "coverage_reduction_merge", "Dynamically set coverage reduction merge mode", config));
            }
            ...
            // Viewport swizzle.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "viewport_swizzle_yx", "Dynamically set a viewport swizzle with X and Y switched around",
                    config));

                config.viewportSwizzleConfig.swapValues();
                config.frontFaceConfig.staticValue = vk::VK_FRONT_FACE_COUNTER_CLOCKWISE;
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "viewport_swizzle_xy", "Dynamically set the viewport identity swizzle", config));
            }
            ...
            // Shading rate image enable.
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "shading_rate_image_" + enableStr,
                                                     "Dynamically " + enableStr + " a shading rate image", config));
                }
            }
            ...
            // Viewport W Scaling enable.
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "viewport_w_scaling_" + enableStr,
                                                     "Dynamically " + enableStr + " viewport W scaling", config));
                }
            }
            ...
            // Representative fragment test state.
            {
                ...
                for (int i = 0; i < 2; ++i)
                {
                    ...
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "repr_frag_test_" + enableStr,
                                                     "Dynamically " + enableStr + " representative frag test", config));
                }
            }
#endif // CTS_USES_VULKANSC
            ...
            // Conservative rasterization mode.
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "conservative_rasterization_mode_overestimate",
                    "Dynamically set conservative rasterization mode to overestimation", config));

                config.conservativeRasterModeConfig.swapValues();
                config.referenceColor.reset(new TopLeftBorderGenerator(kDefaultTriangleColor, kDefaultClearColor,
                                                                       kDefaultClearColor, kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "conservative_rasterization_mode_disabled",
                    "Dynamically set conservative rasterization mode to disabled", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                config.offCenterTriangle = true;
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "conservative_rasterization_mode_underestimate",
                    "Dynamically set conservative rasterization mode to underestimation", config));
            }
            ...
            //    * extraPrimitiveOverestimationSizeGranularity is typically 0.0 or 0.25 (no other values).
            {
                ...
                orderingGroup->addChild(new ExtendedDynamicStateTest(
                    testCtx, "extra_overestimation_size_large",
                    "Dynamically set the extra overestimation size to a large value", config));
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "extra_overestimation_size_none",
                                                 "Dynamically set the extra overestimation size to zero", config));
            }
            ...
            // Depth compare op.
            {
...

                {
                    ...
                    config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_never", "Dynamically set the depth compare operator to NEVER", config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    config.expectedDepth                     = 0.25f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_less", "Dynamically set the depth compare operator to LESS", config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    config.expectedDepth                     = 0.75f;
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "depth_compare_greater",
                                                     "Dynamically set the depth compare operator to GREATER", config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_equal", "Dynamically set the depth compare operator to EQUAL", config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    config.expectedDepth                     = 0.25f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_less_equal_less",
                ...
                }
                {
                    ...
                    config.expectedDepth                     = 0.5f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_less_equal_equal",
                ...
                }
                {
                    ...
                    config.referenceColor.reset(new SingleColorGenerator(kAlternativeColor));
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "depth_compare_less_equal_less_then_equal",
                                                     "Dynamically set the depth compare operator to LESS_OR_EQUAL and "
                                                     "draw two meshes with less and equal depth",
                                                     config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    config.expectedDepth                     = 0.75f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_greater_equal_greater",
                ...
                }
                {
                    ...
                    config.expectedDepth                     = 0.5f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_greater_equal_equal",
                ...
                }
                {
                    ...
                    config.referenceColor.reset(new SingleColorGenerator(kAlternativeColor));
                    orderingGroup->addChild(
                        new ExtendedDynamicStateTest(testCtx, "depth_compare_greater_equal_greater_then_equal",
                                                     "Dynamically set the depth compare operator to GREATER_OR_EQUAL "
                                                     "and draw two meshes with greater and equal depth",
                                                     config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    config.expectedDepth = 0.5f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_not_equal", "Dynamically set the depth compare operator to NOT_EQUAL",
                        config));
                }
                {
                    TestConfig config                        = baseConfig;
                    ...
                    config.expectedDepth       = 0.5f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_always_equal",
                        "Dynamically set the depth compare operator to ALWAYS and draw with equal depth", config));

                    config.meshParams[0].depth = 0.25f;
                    config.expectedDepth       = 0.25f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_always_less",
                        "Dynamically set the depth compare operator to ALWAYS and draw with less depth", config));

                    config.meshParams[0].depth = 0.75f;
                    config.expectedDepth       = 0.75f;
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_compare_always_greater",
                        "Dynamically set the depth compare operator to ALWAYS and draw with greater depth", config));
                }
            }
            ...
            // Depth bounds test.
            {
...

                {
                    ...
                    config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_bounds_test_enable", "Dynamically enable the depth bounds test", config));
                }
                {
                    TestConfig config                               = baseConfig;
                    ...
                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                        testCtx, "depth_bounds_test_disable", "Dynamically disable the depth bounds test", config));
                }
            }
            ...
            // Stencil test enable.
            {
                ...
                config.referenceColor.reset(new SingleColorGenerator(kDefaultClearColor));
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "stencil_test_enable",
                                                                     "Dynamically enable the stencil test", config));
            }
            {
                TestConfig config(pipelineConstructionType, kOrdering, kUseMeshShaders);
                ...
                config.stencilOpConfig.staticValue.front().compareOp = vk::VK_COMPARE_OP_NEVER;
                orderingGroup->addChild(new ExtendedDynamicStateTest(testCtx, "stencil_test_disable",
                                                                     "Dynamically disable the stencil test", config));
            }
            ...
            // Stencil operation. Many combinations are possible.
            {
                ...
                static const struct
                {
                    ...
                    {vk::VK_COMPARE_OP_NEVER, "xf"},   {vk::VK_COMPARE_OP_LESS, "lt"},
                    {vk::VK_COMPARE_OP_EQUAL, "eq"},   {vk::VK_COMPARE_OP_LESS_OR_EQUAL, "le"},
                    {vk::VK_COMPARE_OP_GREATER, "gt"}, {vk::VK_COMPARE_OP_GREATER_OR_EQUAL, "ge"},
                ...
                static const auto kMidVal  = static_cast<uint8_t>(kMaxVal * 2u / 5u);
                ...
                static const struct
                {
                    ...
                    {vk::VK_STENCIL_OP_DECREMENT_AND_CLAMP, "dec_clamp", u8vec{kMinVal + 1, kMinVal},
                     vk::VK_STENCIL_OP_INCREMENT_AND_CLAMP},
                    {vk::VK_STENCIL_OP_INVERT, "invert", u8vec{kMidVal}, vk::VK_STENCIL_OP_ZERO},
                    {vk::VK_STENCIL_OP_INCREMENT_AND_WRAP, "inc_wrap", u8vec{kMaxVal - 1, kMaxVal},
                                    ...
                                    for (int subCaseIdx = 0; subCaseIdx < subCases; ++subCaseIdx)
                                    {
                                        ...
                                        *inactiveOps[1] = op.incompatibleOp;
                                        ...
                                        const std::string testName =
                                            std::string("stencil_state") + "_" + face.name + "_" + compare.name + "_" +
                                            op.name + "_clear_" + de::toString(static_cast<int>(clearVal)) + "_ref_" +
                                            de::toString(refVal) + "_" +
                                            (wouldPass ? (depthFail ? "depthfail" : "pass") : "fail");

                                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                                            testCtx, testName,
                                            "Dynamically configure stencil test, variant " + testName, config));
                                    }
                                }
                            }
                        }
            ...
            if (!kUseMeshShaders)
            {
                for (const auto &bindUnusedCase : kBindUnusedCases)
                {
                    ...
                    {
                        {
                            ...
                            config.bindUnusedMeshShadingPipeline = bindUnusedCase.bindUnusedMeshShadingPipeline;
                            orderingGroup->addChild(new ExtendedDynamicStateTest(
                                testCtx, "vertex_input" + bindUnusedCase.nameSuffix,
                                "Dynamically set vertex input" + bindUnusedCase.descSuffix, config));
                        }
                        {
                            ...
                            orderingGroup->addChild(new ExtendedDynamicStateTest(
                                testCtx, "instance_rate_stride" + bindUnusedCase.nameSuffix,
                                "Dynamically set instance rate stride" + bindUnusedCase.descSuffix, config));
                        }
                    }

                    {
                        ...
                        config.bindUnusedMeshShadingPipeline = bindUnusedCase.bindUnusedMeshShadingPipeline;
                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "vertex_input_no_dyn_stride" + bindUnusedCase.nameSuffix,
                            "Dynamically set vertex input without using dynamic strides" + bindUnusedCase.descSuffix,
                            config));
                    }

                    {
                        ...
                        config.bindUnusedMeshShadingPipeline = bindUnusedCase.bindUnusedMeshShadingPipeline;
                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "vertex_input_multiple_bindings" + bindUnusedCase.nameSuffix,
                            "Dynamically set vertex input with multiple bindings" + bindUnusedCase.descSuffix, config));
                    }

                    {
                        ...
                        config.bindUnusedMeshShadingPipeline = bindUnusedCase.bindUnusedMeshShadingPipeline;
                        orderingGroup->addChild(new ExtendedDynamicStateTest(
                            testCtx, "vertex_input_float16" + bindUnusedCase.nameSuffix,
                            "Dynamically set vertex input with float16 inputs" + bindUnusedCase.descSuffix, config));
                    }
                }
            }
