...
const tcu::Vec4 kLogicOpTriangleColor(
    0.0f, 0.0f, 255.f, 255.f); // Opaque blue. Note: tcu::Vec4 and will be cast to the appropriate type in the shader.
...
struct TestConfig
{
    ...
    // Sane defaults.
    TestConfig(vk::PipelineConstructionType pipelineType, SequenceOrdering ordering,
    ...
    std::vector<std::string> getRequiredExtensions() const
    {
        ...
        if (cullModeConfig.dynamicValue || frontFaceConfig.dynamicValue || topologyConfig.dynamicValue ||
        ...
        if (patchControlPointsConfig.dynamicValue || rastDiscardEnableConfig.dynamicValue ||
            depthBiasEnableConfig.dynamicValue || logicOpConfig.dynamicValue || primRestartEnableConfig.dynamicValue ||
...
void ExtendedDynamicStateTest::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    fragSource << "#version 450\n"
               << pushConstants << "layout(location=0) out " << vecType << " color;\n"
               << "void main() {\n"
                          ...
                          const std::vector<tcu::Vec2> &vertices, uint32_t dataOffset, uint32_t trailingSize)
{
    ...
    for (const auto &bufferBytes : vertexData)
    {
        ...
        bufferInfo.buffer = de::MovePtr<vk::BufferWithMemory>(
...
tcu::TestStatus ExtendedDynamicStateInstance::iterate(void)
{
    ...
    for (const auto &kDepthStencilFormat : kDepthStencilFormats)
    {
        const auto dsProperties =
            vk::getPhysicalDeviceFormatProperties(vki, physicalDevice, kDepthStencilFormat.imageFormat);
    ...
    std::vector<uint32_t> indices{0, 1, 2, 3, 0xFFFFFFFF, 2, 3, 4, 5};
    ...
    // Shader modules.
    const auto dynamicVertModule =
        vk::createShaderModule(vkd, device, m_context.getBinaryCollection().get("dynamicVert"), 0u);
    const auto staticVertModule =
        vk::createShaderModule(vkd, device, m_context.getBinaryCollection().get("staticVert"), 0u);
    ...
    if (useStaticPipeline)
    {
        ...
        staticPipeline.setDefaultPatchControlPoints(m_testConfig.patchControlPointsConfig.staticValue)
            ...
            .setupPreRasterizationShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u, *staticVertModule,
    ...
    vk::GraphicsPipelineWrapper graphicsPipeline(vkd, device, m_testConfig.pipelineConstructionType);
    {
        ...
        graphicsPipeline.setDynamicState(&dynamicStateCreateInfo)
            ...
            .setupVertexInputStete(&vertexInputStateCreateInfo, &inputAssemblyStateCreateInfo)
            .setupPreRasterizationShaderState(viewports, scissors, *pipelineLayout, *renderPass, 0u, *dynamicVertModule,
                                              &rasterizationStateCreateInfo, *tescModule, *teseModule, *geomModule)
            .setupFragmentShaderState(*pipelineLayout, *renderPass, 0u, *fragModule, &depthStencilStateCreateInfo,
                                      &multisampleStateCreateInfo)
            .setupFragmentOutputState(*renderPass, 0u, &colorBlendStateCreateInfo, &multisampleStateCreateInfo)
    ...
    for (const auto &kOrderingCase : kOrderingCases)
    {
        ...
        // Dynamically change the number of primitive control points
        {
            for (int i = 0; i < 2; ++i)
            {
                ...
                const auto testDesc =
                    std::string("Dynamically change patch control points") +
        ...
        // Dynamic topology.
        {
            TestConfig baseConfig(pipelineConstructionType, kOrdering);
        ...
        // Stride.
        {
            ...
            for (const auto &strideCase : strideCases)
            {
                ...
                {
                    TestConfig config(pipelineConstructionType, kOrdering, factory);
            ...
            {
                TestConfig config(pipelineConstructionType, kOrdering, getVertexWithExtraAttributesGenerator());
            ...
            }
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "depth_compare_less_equal_less_then_equal",
                                                 "Dynamically set the depth compare operator to LESS_OR_EQUAL and draw "
                                                 "two meshes with less and equal depth",
                                                 config));
            }
            {
                TestConfig config                        = baseConfig;
            ...
            }
            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "depth_compare_greater_equal_greater_then_equal",
                                                 "Dynamically set the depth compare operator to GREATER_OR_EQUAL and "
                                                 "draw two meshes with greater and equal depth",
                                                 config));
            }
            {
                TestConfig config                        = baseConfig;
                ...
                config.expectedDepth  = 0.5f;
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "depth_compare_not_equal",
                                                 "Dynamically set the depth compare operator to NOT_EQUAL", config));
            }
            {
                TestConfig config                        = baseConfig;
                            ...
                            for (int delta = -1; delta <= 1; ++delta)
                            {
                                ...
                                for (int subCaseIdx = 0; subCaseIdx < subCases; ++subCaseIdx)
                                {
                                    ...
                                    *inactiveOps[1] = op.incompatibleOp;
                                    ...
                                    orderingGroup->addChild(new ExtendedDynamicStateTest(
                                        testCtx, testName, "Dynamically configure stencil test, variant " + testName,
                                        config));
                                }
                            }
                        }
        ...
        // Vertex input.
        {
            ...
            if (kOrdering != SequenceOrdering::TWO_DRAWS_STATIC)
            {
                ...
                TestConfig config(pipelineConstructionType, kOrdering, staticGen, dynamicGen);
...

            {
                ...
                TestConfig config(pipelineConstructionType, kOrdering, getVertexWithPaddingGenerator(),
                                  getVertexWithExtraAttributesGenerator());
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "vertex_input_no_dyn_stride",
                                                 "Dynamically set vertex input without using dynamic strides", config));
            }

            {
                ...
                orderingGroup->addChild(
                    new ExtendedDynamicStateTest(testCtx, "vertex_input_multiple_bindings",
                                                 "Dynamically set vertex input with multiple bindings", config));
            }
        }
