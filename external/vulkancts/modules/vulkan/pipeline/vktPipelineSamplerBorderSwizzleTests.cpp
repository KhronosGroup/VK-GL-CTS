...
{
public:
    BorderSwizzleCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
BorderSwizzleCase::BorderSwizzleCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const TestParams &params)
...
bool comparePixelToColorClearValue(const TestParams &params, const tcu::ConstPixelBufferAccess &access,
                                   const tcu::TextureFormat &textureFormat, const VkClearColorValue &ref,
...
tcu::TestStatus BorderSwizzleInstance::iterate(void)
{
    ...
    graphicsPipeline.setDefaultTopology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP)
        ...
        .setupFragmentShaderState(*pipelineLayout, *renderPass, 0u, *fragShader, DE_NULL, DE_NULL, DE_NULL,
                                  &specializationInfo)
    ...
    const auto framebuffer =
        makeFramebuffer(vkd, device, renderPass.get(), colorAttachmentView.get(), extent.width, extent.height);
    ...
    const auto preClearBarrier =
        makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                               VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, texture.get(), colorSubresourceRange);
    ...
    const auto postClearBarrier = makeImageMemoryBarrier(
        ...
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, texture.get(), colorSubresourceRange);
    ...
    vkd.cmdClearColorImage(cmdBuffer, texture.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &m_params.textureColor, 1u,
                                                    ...
                                                    PipelineConstructionType pipelineConstructionType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> mainGroup(
        new tcu::TestCaseGroup(testCtx, "border_swizzle", "Border color swizzle tests"));
