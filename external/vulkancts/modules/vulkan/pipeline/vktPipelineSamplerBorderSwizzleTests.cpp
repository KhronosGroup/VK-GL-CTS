...
tcu::TestStatus BorderSwizzleInstance::iterate(void)
{
    ...
    if (isDSFormat)
        vkd.cmdClearDepthStencilImage(cmdBuffer, texture.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                      &m_params.textureDepthStencilValue, 1u, &imageSubresourceRange);
    else
        vkd.cmdClearColorImage(cmdBuffer, texture.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &m_params.textureColor,
                               1u, &imageSubresourceRange);
    ...
    for (const auto &format : textureFormats)
    {
        ...
        for (const auto sampleStencil : sampleStencilFlag)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> formatGroup(
                new tcu::TestCaseGroup(testCtx, formatGroupName.str().c_str(), ""));
