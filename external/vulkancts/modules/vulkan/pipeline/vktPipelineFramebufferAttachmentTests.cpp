...
Move<VkRenderPass> makeRenderPassMultiAttachments(const DeviceInterface &vk, const VkDevice device,
...
tcu::TestStatus testMultiAttachments(Context &context, const CaseDef caseDef)
{
    ...
    GraphicsPipelineWrapper pipeline(vk, device, caseDef.pipelineConstructionType);
    const Unique<VkRenderPass> renderPass(
        makeRenderPassMultiAttachments(vk, device, COLOR_FORMAT, numRenderTargets, caseDef.multisample));
    Move<VkFramebuffer> framebuffer;
