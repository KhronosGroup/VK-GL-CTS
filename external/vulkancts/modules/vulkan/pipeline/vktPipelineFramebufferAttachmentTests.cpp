...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const IVec3 renderSize, const VkPrimitiveTopology topology,
                                      const uint32_t subpass, const uint32_t numAttachments, const bool multisample)
{
    ...
    const VkPipelineMultisampleStateCreateInfo pipelineMultisampleStateInfo = {
        ...
        !multisample ? VK_SAMPLE_COUNT_1_BIT :
                       VK_SAMPLE_COUNT_4_BIT, // VkSampleCountFlagBits                    rasterizationSamples;
...
tcu::TestStatus test(Context &context, const CaseDef caseDef)
{
    ...
    const Unique<VkRenderPass> renderPass(
        makeRenderPass(vk, device, COLOR_FORMAT, caseDef.numLayers, caseDef.multisample));
    ...
    for (uint32_t layerNdx = 0; layerNdx < caseDef.numLayers; ++layerNdx)
    {
        ...
        pipeline.push_back(makeSharedPtr(makeGraphicsPipeline(
            vk, device, *pipelineLayout, *renderPass, *vertexModule, *fragmentModule, caseDef.renderSize,
            VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, layerNdx, 1u, caseDef.multisample)));
    }

    // create framebuffer
    framebuffer =
        makeFramebuffer(vk, device, *renderPass, caseDef.numLayers, &attachmentHandles[0],
                        static_cast<uint32_t>(caseDef.renderSize.x()), static_cast<uint32_t>(caseDef.renderSize.y()));
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        // Render pass: this should render only to the area defined by renderSize (smaller than the size of the attachment)
        {
            ...
            beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                            makeRect2D(0, 0, caseDef.renderSize.x(), caseDef.renderSize.y()));
            {
...
tcu::TestStatus testNoAtt(Context &context, const bool multisample)
{
    ...
    const uint8_t numSamples           = multisample ? 4 : 1;
    ...
    pipeline.push_back(
        makeSharedPtr(makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule, *fragmentModule,
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        // Render pass
        {
            ...
            beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                            makeRect2D(0, 0, renderSize.x(), renderSize.y()));
...
Move<VkRenderPass> makeRenderPassDifferentAttachmentSizes(const DeviceInterface &vk, const VkDevice device,
...
tcu::TestStatus testDifferentAttachmentSizes(Context &context, const CaseDef caseDef)
{
    ...
    const Unique<VkRenderPass> renderPass(
        makeRenderPassDifferentAttachmentSizes(vk, device, COLOR_FORMAT, numRenderTargets, caseDef.multisample));
    Move<VkFramebuffer> framebuffer;
    ...
    {
        // Host memory buffer where we will copy the rendered image for verification
        const uint32_t att_size_x = attachmentSizes[renderTargetIdx].x();
        const uint32_t att_size_y = attachmentSizes[renderTargetIdx].y();
        const uint32_t att_size_z = attachmentSizes[renderTargetIdx].z();
        const VkDeviceSize colorBufferSize =
    ...
    for (uint32_t renderTargetIdx = 0; renderTargetIdx < numRenderTargets; renderTargetIdx++)
    {
        ...
        if (caseDef.multisample)
        {
            ...
            msColorImages[renderTargetIdx] =
                makeImage(vk, device, VkImageViewCreateFlags(0), getImageType(caseDef.imageType), COLOR_FORMAT,
                          attachmentSizes[renderTargetIdx], 1, msImageUsage, true);
    ...
    pipeline = makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule, *fragmentModule,
                                    caseDef.renderSize, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, 0, numRenderTargets,
                                    caseDef.multisample);

    // create framebuffer
    framebuffer =
        makeFramebuffer(vk, device, *renderPass, numRenderTargets, &attachmentHandles[0],
                        static_cast<uint32_t>(caseDef.renderSize.x()), static_cast<uint32_t>(caseDef.renderSize.y()));
...
Move<VkRenderPass> makeRenderPassInputResolveSameAttachment(const DeviceInterface &vk, const VkDevice device,
...
tcu::TestStatus testInputResolveSameAttachment(Context &context, const CaseDef caseDef)
{
    ...
    pipeline = makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, *vertexModule, *fragmentModule,
                                    caseDef.renderSize, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP, 0, 1u, true);

    // create framebuffer
    framebuffer =
        makeFramebuffer(vk, device, *renderPass, 2u, &attachmentHandles[0],
                        static_cast<uint32_t>(caseDef.renderSize.x()), static_cast<uint32_t>(caseDef.renderSize.y()));
...
void addAttachmentTestCasesWithFunctions(tcu::TestCaseGroup *group)
{
    ...
    for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(caseDef); ++sizeNdx)
        addFunctionCaseWithPrograms(group, getTestCaseString(caseDef[sizeNdx]).c_str(), "", initColorPrograms, test,
                                    caseDef[sizeNdx]);
    ...
    for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(differentAttachmentSizesCaseDef); ++sizeNdx)
        addFunctionCaseWithPrograms(
            ...
            "", initDifferentAttachmentSizesPrograms, testDifferentAttachmentSizes,
            differentAttachmentSizesCaseDef[sizeNdx]);

    const CaseDef resolveInputSameAttachmentCaseDef = {VK_IMAGE_VIEW_TYPE_2D, IVec3(64, 64, 1), IVec3(64, 64, 1), 1,
                                                       true};
    addFunctionCaseWithPrograms(group, "resolve_input_same_attachment", "", initInputResolveSameAttachmentPrograms,
                                testInputResolveSameAttachment, resolveInputSameAttachmentCaseDef);
}
...
{
    return createTestGroup(testCtx, "framebuffer_attachment", "Framebuffer attachment tests",
                           addAttachmentTestCasesWithFunctions);
}
