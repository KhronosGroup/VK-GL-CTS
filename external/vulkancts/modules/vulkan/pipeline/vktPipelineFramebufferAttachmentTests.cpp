...
void preparePipelineWrapper(GraphicsPipelineWrapper &gpw, const VkPipelineLayout pipelineLayout,
                            ...
                            const VkShaderModule fragmentModule, const IVec3 renderSize,
...
void addAttachmentTestCasesWithFunctions(tcu::TestCaseGroup *group, PipelineConstructionType pipelineConstructionType)
{
    ...
    for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(caseDef); ++sizeNdx)
        addFunctionCaseWithPrograms(group, getTestCaseString(caseDef[sizeNdx]).c_str(), "", checkSupport,
                                    initColorPrograms, test, caseDef[sizeNdx]);
    ...
    addFunctionCaseWithPrograms(group, "no_attachments", "", checkSupportNoAtt, initImagePrograms, testNoAtt,
                                noAttCaseDef);
    ...
    for (int sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(differentAttachmentSizesCaseDef); ++sizeNdx)
        addFunctionCaseWithPrograms(
            ...
            "", checkSupport, initDifferentAttachmentSizesPrograms, testDifferentAttachmentSizes,
                                                     ...
                                                     PipelineConstructionType pipelineConstructionType)
{
    return createTestGroup(testCtx, "framebuffer_attachment", "Framebuffer attachment tests",
                           addAttachmentTestCasesWithFunctions, pipelineConstructionType);
}
