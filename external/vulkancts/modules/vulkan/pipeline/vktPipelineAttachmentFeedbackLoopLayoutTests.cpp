...
{
public:
    AttachmentFeedbackLoopLayoutDepthStencilImageSamplingInstance(Context &context, ImageSamplingInstanceParams params,
                                                                  bool useImageAsColorOrDSAttachment_,
                                                                  bool useDifferentAreasSampleWrite_,
                                                                  bool interleaveReadWriteComponents_);
    ...
    // Create command buffer
    {
        ...
        if (!m_useImageAsColorOrDSAttachment)
        {
            ...
            beginRenderPass(vk, *m_cmdBuffer, *m_renderPass, *m_framebuffer,
                            makeRect2D(0, 0, renderSize.x(), renderSize.y()), (uint32_t)attachmentClearValues.size(),
        ...
        else
        {
            ...
            beginRenderPass(vk, *m_cmdBuffer, *m_renderPass, *m_framebuffer,
                            makeRect2D(0, 0, renderSize.x(), renderSize.y()), 0u, DE_NULL);
        }
...
tcu::TestStatus AttachmentFeedbackLoopLayoutImageSamplingInstance::verifyImage(void)
{
    ...
    {
        // Read back result image
        de::MovePtr<tcu::TextureLevel> resultTexture(
            readColorAttachment(m_context.getDeviceInterface(), m_context.getDevice(), m_context.getUniversalQueue(),
                                m_context.getUniversalQueueFamilyIndex(), m_context.getDefaultAllocator(),
                                **m_colorImages[imgNdx], m_colorFormat, renderSize));
        const tcu::ConstPixelBufferAccess result = resultTexture->getAccess();
...
public:
    AttachmentFeedbackLoopLayoutSamplerTest(tcu::TestContext &testContext,
                                            ...
                                            VkFormat imageFormat, int imageSize, VkDescriptorType imageDescriptorType,
                                            float samplerLod, TestMode testMode,
...
AttachmentFeedbackLoopLayoutSamplerTest::AttachmentFeedbackLoopLayoutSamplerTest(
    ...
    const char *description, SamplerViewType imageViewType, VkFormat imageFormat, int imageSize,
...
void AttachmentFeedbackLoopLayoutSamplerTest::checkSupport(Context &context) const
{
    ...
    vk::VkPhysicalDeviceAttachmentFeedbackLoopLayoutFeaturesEXT attachmentFeedbackLoopLayoutFeatures = {
        ...
        DE_NULL,                                                                            // void* pNext;
        false, // VkBool32 attachmentFeedbackLoopLayout;
    };
    ...
    if (attachmentFeedbackLoopLayoutFeatures.attachmentFeedbackLoopLayout == false)
    {
    ...
    ImageSamplingInstanceParams params = getImageSamplingInstanceParams(m_imageViewType, m_imageFormat, m_imageSize,
                                                                        m_imageDescriptorType, m_samplerLod);
    ...
    if (useImageAsColorOrDSAttachment)
    {
        ...
        instanceInterface.getPhysicalDeviceFormatProperties(context.getPhysicalDevice(), params.imageFormat,
                                                            &formatProps);
        bool error = (formatProps.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_BIT) == 0u ||
    ...
    tcu::TestContext &testCtx, vk::PipelineConstructionType pipelineConstructionType)
{
         ...
         imageDescriptorTypeNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> imageDescriptorTypeGroup(new tcu::TestCaseGroup(
            testCtx, imageDescriptorTypes[imageDescriptorTypeNdx].name,
            (std::string("Uses a ") + imageDescriptorTypes[imageDescriptorTypeNdx].name + " sampler").c_str()));
                         ...
                         restrictColorNdx < DE_LENGTH_OF_ARRAY(interleaveReadWriteComponentsModes); restrictColorNdx++)
                    {
                        ...
                        if (!isCompressed && isDepthStencil)
                        {
                            ...
                            std::string stencilTestName =
                                ...
                                testModes[testModeNdx].name + interleaveReadWriteComponentsModes[restrictColorNdx].name;
                            formatTests->addChild(new AttachmentFeedbackLoopLayoutSamplerTest(
...
{
    de::MovePtr<tcu::TestCaseGroup> attachmentFeedbackLoopLayoutTests(new tcu::TestCaseGroup(
        testCtx, "attachment_feedback_loop_layout", "VK_EXT_attachment_feedback_loop_layout tests"));
    {
