...
void addInstanceTestCaseWithPrograms(tcu::TestCaseGroup *group, const std::string &name, const std::string &desc,
                                     ...
                                     typename FunctionProgramsSimple1<Arg0>::Function initPrograms, Arg0 arg0)
{
    group->addChild(
        new InstanceFactory1WithSupport<Instance, Arg0, FunctionSupport1<Arg0>, FunctionProgramsSimple1<Arg0>>(
            group->getTestContext(), tcu::NODETYPE_SELF_VALIDATE, name, desc,
            FunctionProgramsSimple1<Arg0>(initPrograms), arg0,
                                      ...
                                      const bool useFragmentShadingRate)
{
    ...
    const VkGraphicsPipelineCreateInfo pipelineCreateInfo{
        ...
        useFragmentShadingRate ? &shadingRateStateCreateInfo :
                                 DE_NULL,           // const void*                                        pNext
...
inline Move<VkPipeline> makeGraphicsPipelineSinglePassColor(
    const DeviceInterface &vk, const VkDevice device, const std::vector<VkDynamicState> &dynamicState,
    const VkPipelineLayout pipelineLayout, const VkRenderPass renderPass, const VkShaderModule vertexModule,
    const VkShaderModule fragmentModule, const VkViewport &viewport, const VkRect2D scissor,
    const VkSampleCountFlagBits numSamples, const bool useSampleLocations,
...
public:
    ...
    uint32_t addAttachment(const VkImageView imageView, const VkAttachmentDescriptionFlags flags, const VkFormat format,
                           const VkSampleCountFlagBits numSamples, const VkAttachmentLoadOp loadOp,
                           ...
                           const VkImageLayout finalLayout, const VkClearValue clearValue,
    ...
    void bake(const DeviceInterface &vk, const VkDevice device, const UVec2 &framebufferSize)
    {
        ...
        m_framebuffer = makeFramebuffer(vk, device, *m_renderPass, static_cast<uint32_t>(m_attachments.size()),
                               ...
                               const VkSubpassContents subpassContents) const
    {
        ...
        const VkRenderPassBeginInfo renderPassBeginInfo = {
            ...
            *m_framebuffer,                              // VkFramebuffer           framebuffer;
            ...
            static_cast<uint32_t>(m_clearValues.size()), // uint32_t                clearValueCount;
...
protected:
    ...
    void drawSinglePass(const VertexInputConfig vertexInputConfig)
    {
        ...
        rt.addAttachment(*m_colorImageView,                        // VkImageView                    imageView,
        ...
        rt.addAttachment(*m_resolveImageView,                  // VkImageView                    imageView,
                         (VkAttachmentDescriptionFlags)0,      // VkAttachmentDescriptionFlags    flags,
                         m_colorFormat,                        // VkFormat                        format,
        ...
        if (useDynamicStateSampleLocations)
        {
            ...
            pipeline = makeGraphicsPipelineSinglePassColor(
                vk, device, dynamicState, *pipelineLayout, rt.getRenderPass(), *vertexModule, *fragmentModule, viewport,
        ...
        {
            pipeline = makeGraphicsPipelineSinglePassColor(
                vk, device, std::vector<VkDynamicState>(), *pipelineLayout, rt.getRenderPass(), *vertexModule,
...
void addCases(tcu::TestCaseGroup *group, const VkSampleCountFlagBits numSamples, bool useFragmentShadingRate,
              const ProgramsFunc initPrograms)
{
    ...
    addInstanceTestCaseWithPrograms<Test>(group, getString(numSamples).c_str(), "", checkSupportVerifyTests,
                                          initPrograms, params);

    params.options |= (TestOptionFlags)TEST_OPTION_DYNAMIC_STATE_BIT;
    addInstanceTestCaseWithPrograms<Test>(group, (getString(numSamples) + "_dynamic").c_str(), "",
                                          checkSupportVerifyTests, initPrograms, params);

    params.options |= (TestOptionFlags)TEST_OPTION_CLOSELY_PACKED_BIT;
    addInstanceTestCaseWithPrograms<Test>(group, (getString(numSamples) + "_packed").c_str(), "",
                                          checkSupportVerifyTests, initPrograms, params);
}
...
protected:
    ...
    void drawRenderPasses(void)
    {
        ...
        {
            rt[0].addAttachment(*m_colorImageView,                // VkImageView                    imageView,
                                (VkAttachmentDescriptionFlags)0,  // VkAttachmentDescriptionFlags    flags,
                                m_colorFormat,                    // VkFormat                        format,
                                m_params.numSamples,              // VkSampleCountFlagBits        numSamples,
                                VK_ATTACHMENT_LOAD_OP_CLEAR,      // VkAttachmentLoadOp            loadOp,
        ...
        // Second render pass
        {
            ...
            rt[1].addAttachment(*m_colorImageView,                // VkImageView                    imageView,
                                (VkAttachmentDescriptionFlags)0,  // VkAttachmentDescriptionFlags    flags,
                                m_colorFormat,                    // VkFormat                        format,
                                m_params.numSamples,              // VkSampleCountFlagBits        numSamples,
                                loadOp,                           // VkAttachmentLoadOp            loadOp,
            ...
            rt[1].addAttachment(*m_resolveImageView,                  // VkImageView                    imageView,
                                (VkAttachmentDescriptionFlags)0,      // VkAttachmentDescriptionFlags    flags,
                                m_colorFormat,                        // VkFormat                        format,
                                VK_SAMPLE_COUNT_1_BIT,                // VkSampleCountFlagBits        numSamples,
                                VK_ATTACHMENT_LOAD_OP_DONT_CARE,      // VkAttachmentLoadOp            loadOp,
            ...
            if (useDepth() || useStencil())
            {
                rt[1].addAttachment(*m_depthStencilImageView,        // VkImageView                    imageView,
                                    (VkAttachmentDescriptionFlags)0, // VkAttachmentDescriptionFlags    flags,
                                    m_depthStencilFormat,            // VkFormat                        format,
                                    m_params.numSamples,             // VkSampleCountFlagBits        numSamples,
                                    loadOp,                          // VkAttachmentLoadOp            loadOp,
        ...
        if (useDynamicState())
        {
            ...
            for (uint32_t passNdx = 0; passNdx < NUM_PASSES; ++passNdx)
            {
                pipeline[passNdx] = makeGraphicsPipeline(
                    vk, device, dynamicState, *pipelineLayout, rt[passNdx].getRenderPass(), *vertexModule,
                    *fragmentModule,
                    /*subpass index*/ 0u, viewport, scissor, m_params.numSamples, /*use sample locations*/ true,
            ...
            {
                pipeline[passNdx] = makeGraphicsPipeline(
                    vk, device, std::vector<VkDynamicState>(), *pipelineLayout, rt[passNdx].getRenderPass(),
                    *vertexModule, *fragmentModule,
                    /*subpass index*/ 0u, viewport, scissor, m_params.numSamples, /*use sample locations*/ true,
                    sampleLocationsInfo[passNdx], useDepth(), useStencil(), VERTEX_INPUT_VEC4_VEC4,
                    VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, stencilOpStateDrawOnce(), useFragmentShadingRate());
            }
        ...
        if (useSecondaryCmdBuffer())
        {
            ...
            beginSecondaryCommandBuffer(vk, *secondaryCmdBuffer[0], rt[0].getRenderPass(), /*subpass*/ 0u,
                                        rt[0].getFramebuffer());
            ...
            beginSecondaryCommandBuffer(vk, *secondaryCmdBuffer[1], rt[1].getRenderPass(), /*subpass*/ 0u,
                                        rt[1].getFramebuffer());
            recordSecondPassContents(*secondaryCmdBuffer[1], *pipeline[1], sampleLocationsInfo[1], clearColor1,
        ...
        else
        {
            ...
            recordSecondPassContents(currentCmdBuffer, *pipeline[1], sampleLocationsInfo[1], clearColor1,
                                     clearDepthStencil0, scissor);
            endRenderPass(vk, currentCmdBuffer);
        }
    ...
    void recordFirstPassContents(const VkCommandBuffer cmdBuffer, const VkPipeline pipeline,
    ...
    void recordSecondPassContents(const VkCommandBuffer cmdBuffer, const VkPipeline pipeline,
    ...
    void drawSubpasses(void)
    {
        ...
        if (useSecondaryCmdBuffer())
        {
            ...
            beginSecondaryCommandBuffer(vk, *secondaryCmdBuffer[0], rt.getRenderPass(), /*subpass*/ 0u,
                                        rt.getFramebuffer());
            recordFirstPassContents(*secondaryCmdBuffer[0], *pipeline[0], sampleLocationsInfo[0]);
            endCommandBuffer(vk, *secondaryCmdBuffer[0]);

            // Second subpass contents
            beginSecondaryCommandBuffer(vk, *secondaryCmdBuffer[1], rt.getRenderPass(), /*subpass*/ 1u,
                                        rt.getFramebuffer());
            recordSecondPassContents(*secondaryCmdBuffer[1], *pipeline[1], sampleLocationsInfo[1], clearColor1,
                                     clearDepthStencil0, scissor);
            endCommandBuffer(vk, *secondaryCmdBuffer[1]);
        ...
        else
        {
            ...
            recordSecondPassContents(*cmdBuffer, *pipeline[1], sampleLocationsInfo[1], clearColor1, clearDepthStencil0,
                                     scissor);
        }
    ...
    void drawSameSubpass(void)
    {
        ...
        if (useSecondaryCmdBuffer())
        {
            ...
            beginSecondaryCommandBuffer(vk, *secondaryCmdBuffer, rt.getRenderPass(), /*subpass*/ 0u,
                                        rt.getFramebuffer());
            recordFirstPassContents(*secondaryCmdBuffer, *pipeline[0], sampleLocationsInfo[0]);
            recordSecondPassContents(*secondaryCmdBuffer, *pipeline[1], sampleLocationsInfo[1], clearColor1,
                                     clearDepthStencil0, scissor);
...
void createTestsInGroup(tcu::TestCaseGroup *rootGroup, bool useFragmentShadingRate)
{
    ...
    // Verify custom sample locations and interpolation
    {
        ...
        for (const VkSampleCountFlagBits *pLoopNumSamples = sampleCountRange;
             pLoopNumSamples < DE_ARRAY_END(sampleCountRange); ++pLoopNumSamples)
        {
            addCases<VerifyLocationTest>(groupLocation.get(), *pLoopNumSamples, useFragmentShadingRate,
                                         addProgramsVerifyLocationGeometry);
            addCases<VerifyInterpolationTest>(groupInterpolation.get(), *pLoopNumSamples, useFragmentShadingRate,
                                              addProgramsVerifyInterpolation);
        }
...
{
    return createTestGroup(testCtx, "sample_locations_ext",
                           "Test a graphics pipeline with user-defined sample locations", createTestsInGroup,
