    ...
    void drawSinglePass(const VertexInputConfig vertexInputConfig)
    {
        ...
        if (useDynamicStateSampleLocations)
        {
            ...
            pipeline = makeGraphicsPipelineSinglePassColor(
                ...
                vertexInputConfig, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
        }
        else
        {
            pipeline = makeGraphicsPipelineSinglePassColor(
                ...
                *fragmentModule, viewport, scissor, m_params.numSamples, /*use sample locations*/ true,
                sampleLocationsInfo, vertexInputConfig, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
        }
...
protected:
    ...
    void drawRenderPasses(void)
    {
        ...
        if (useDynamicState())
        {
            ...
            {
                pipeline[passNdx] = makeGraphicsPipeline(vk, device, dynamicState, *pipelineLayout,
                                                         rt[passNdx].getRenderPass(), *vertexModule, *fragmentModule,
                                                         /*subpass index*/ 0u, viewport, scissor, m_params.numSamples,
                                                         /*use sample locations*/ true, makeEmptySampleLocationsInfo(),
                                                         useDepth(), useStencil(), VERTEX_INPUT_VEC4_VEC4,
                                                         VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, stencilOpStateDrawOnce());
            }
        }
        else
            for (uint32_t passNdx = 0; passNdx < NUM_PASSES; ++passNdx)
            {
                pipeline[passNdx] = makeGraphicsPipeline(vk, device, std::vector<VkDynamicState>(), *pipelineLayout,
                                                         rt[passNdx].getRenderPass(), *vertexModule, *fragmentModule,
                                                         /*subpass index*/ 0u, viewport, scissor, m_params.numSamples,
                                                         /*use sample locations*/ true, sampleLocationsInfo[passNdx],
                                                         useDepth(), useStencil(), VERTEX_INPUT_VEC4_VEC4,
                                                         VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, stencilOpStateDrawOnce());
            }
...
void createTestsInGroup(tcu::TestCaseGroup *rootGroup)
{
    ...
    // Verify custom sample locations and interpolation
    {
             ...
             pLoopNumSamples < DE_ARRAY_END(sampleCountRange); ++pLoopNumSamples)
        {
            ...
            addCases<VerifyInterpolationTest>(groupInterpolation.get(), *pLoopNumSamples,
