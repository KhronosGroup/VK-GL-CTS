...
{
public:
    LogicOpTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
LogicOpTest::LogicOpTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         const TestParams &testParams)
...
void LogicOpTest::checkSupport(Context &ctx) const
{
    ...
    checkPipelineLibraryRequirements(ctx.getInstanceInterface(), ctx.getPhysicalDevice(),
    ...
    , m_graphicsPipeline(m_context.getDeviceInterface(), m_context.getDevice(), testParams.pipelineConstructionType)
{
    ...
    m_framebuffer =
        makeFramebuffer(vk, vkDevice, *m_renderPass, m_colorAttachmentView.get(), m_renderSize.x(), m_renderSize.y());
...
{
    de::MovePtr<tcu::TestCaseGroup> logicOpTests(
        new tcu::TestCaseGroup(testCtx, "logic_op", "Logical Operations tests"));
    ...
    for (int formatIdx = 0; formatIdx < DE_LENGTH_OF_ARRAY(formatList); ++formatIdx)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> formatGroup(
            new tcu::TestCaseGroup(testCtx, formatName.c_str(), formatDesc.c_str()));

        for (auto &params : logicOpTestParams)
        {
            params.format = format;
            formatGroup->addChild(
                new LogicOpTest(testCtx, params.name, "Tests the " + params.name + " logical operator", params));
        }
