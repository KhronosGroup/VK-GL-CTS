...
static constexpr TestParams DUPLICATE_SINGLE_RECREATE_EXPLICIT_CACHING = {
    "duplicate_single_recreate_explicit_caching", "Duplicate single pipeline recreation with explicit caching",
...
static constexpr TestParams DUPLICATE_SINGLE_RECREATE_NO_CACHING = {
    "duplicate_single_recreate_no_caching", "Duplicate single pipeline recreation with no explicit cache",
    TestParams::NO_CACHE,
    TestParams::IterationArray{
        TestParams::Iteration{// Iteration [0]: Force compilation of pipeline
                              TestParams::Iteration::SINGLE_NORMAL,
...
static constexpr TestParams DUPLICATE_SINGLE_RECREATE_DERIVATIVE = {
    "duplicate_single_recreate_derivative", "Duplicate single pipeline recreation using derivative pipelines",
...
static constexpr TestParams SINGLE_PIPELINE_NO_COMPILE = {
    "single_pipeline_no_compile", "Single creation of never before seen pipeline without compile", TestParams::NO_CACHE,
...
static constexpr TestParams DUPLICATE_BATCH_PIPELINES_EXPLICIT_CACHE = {
    "duplicate_batch_pipelines_explicit_cache", "Batch creation of duplicate pipelines with explicit caching",
    TestParams::EXPLICIT_CACHE,
    TestParams::IterationArray{TestParams::Iteration{
...
static constexpr TestParams DUPLICATE_BATCH_PIPELINES_NO_CACHE = {
    "duplicate_batch_pipelines_no_cache", "Batch creation of duplicate pipelines with no caching", TestParams::NO_CACHE,
    TestParams::IterationArray{TestParams::Iteration{
        TestParams::Iteration::BATCH_NOCOMPILE_COMPILE_NOCOMPILE,
        ValidatorArray{// Fail if pipeline[1] is not valid
                       checkPipelineMustBeValid<1>,
...
static constexpr TestParams DUPLICATE_BATCH_PIPELINES_DERIVATIVE_INDEX = {
    ...
    "Batch creation of duplicate pipelines with derivative pipeline index", TestParams::DERIVATIVE_INDEX,
    TestParams::IterationArray{TestParams::Iteration{
        TestParams::Iteration::BATCH_NOCOMPILE_COMPILE_NOCOMPILE,
        ValidatorArray{// Fail if pipeline[1] is not valid
                       checkPipelineMustBeValid<1>,
...
static constexpr TestParams BATCH_PIPELINES_EARLY_RETURN = {
    "batch_pipelines_early_return", "Batch creation of pipelines with early return", TestParams::NO_CACHE,
    TestParams::IterationArray{TestParams::Iteration{
 ...
 *//*--------------------------------------------------------------------*/
static constexpr TestParams TEST_CASES[] = {SINGLE_PIPELINE_NO_COMPILE,
...
void addGraphicsPipelineTests(TestCaseGroup &group)
{
    ...
    auto tests = newMovePtr<TestCaseGroup>(group.getTestContext(), "graphics_pipelines",
                                           "Test pipeline creation cache control with graphics pipelines");

    for (const auto &params : TEST_CASES)
    {
        addFunctionCaseWithPrograms<const TestParams &>(tests.get(), params.name, params.description, checkSupport,
                                                        initPrograms, testInstance, params);
    }
...
void addComputePipelineTests(TestCaseGroup &group)
{
    ...
    auto tests = newMovePtr<TestCaseGroup>(group.getTestContext(), "compute_pipelines",
                                           "Test pipeline creation cache control with compute pipelines");
