...
struct BindingParams
{
	...
	BindingParams (VkFormat format_, ShaderFormat shaderFormat_, uint32_t bindingStride_, uint32_t attributeOffset_, uint32_t memoryOffset_)
		: format(format_)
...
BytesVector getOutputData (const BytesVector& inputData, const BindingParams& params, uint32_t numPoints)
{
	...
	tcu::ConstPixelBufferAccess memoryAccess (tcuFormat, size, pitch, de::dataOrNull(inputData) + params.attributeOffset);
...
BytesVector genInputData (const BindingParams& params, uint32_t numPoints, de::Random& rnd)
{
	...
	const auto tcuFormat      = mapVkFormat(params.format);
	const auto channelClass   = tcu::getTextureChannelClass(tcuFormat.type);
	const bool floatsUsed     = (channelClass == tcu::TEXTURECHANNELCLASS_FLOATING_POINT || params.shaderFormat == ShaderFormat::FLOAT);
...
{
public:
					LegacyVertexAttributesCase	(tcu::TestContext& testCtx, const std::string& name, const LegacyVertexAttributesParams& params)
	...
	TestInstance*	createInstance				(Context& context) const override { return new LegacyVertexAttributesInstance(context, m_params); }
...
void LegacyVertexAttributesCase::initPrograms (SourceCollections& dst) const
{
	...
	for (size_t i = 0; i < m_params.bindings.size(); ++i)
	{
		...
		inOutVert
			<< "layout (location=" << (i+1) << ") in " << shaderType << " inData" << i << ";\n"
			<< "layout (location=" << i << ") out flat " << shaderType << " outData" << i << ";\n"
			;
		copyVert << "    outData" << i << " = inData" << i << ";\n";
		inSetFrag
			<< "layout (location=" << i << ") in flat " << shaderType << " inData" << i << ";\n"
...
tcu::TestStatus LegacyVertexAttributesInstance::iterate (void)
{
	...
	// Positions.
	{
		...
		vertexBuffers.emplace_back(new BufferWithMemory(ctx.vkd, ctx.device, ctx.allocator, vbInfo, MemoryRequirement::HostVisible));
	...
	for (size_t i = 0; i < m_params.bindings.size(); ++i)
	{
		...
		vertexBuffers.emplace_back(new BufferWithMemory(ctx.vkd, ctx.device, offsetAllocator, vbInfo, MemoryRequirement::HostVisible));
	...
	for (size_t i = 0; i < byteInputs.size(); ++i)
	{
		...
		verifBuffers.emplace_back(new BufferWithMemory(ctx.vkd, ctx.device, ctx.allocator, createInfo, MemoryRequirement::HostVisible));
	...
	for (size_t i = 0; i < verifBuffers.size(); ++i)
	{
		...
		updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(static_cast<uint32_t>(i)), descType, &dbDescInfo);
	}
	...
	GraphicsPipelineWrapper pipeline (ctx.vki, ctx.vkd, ctx.physicalDevice, ctx.device, m_context.getDeviceExtensions(), m_params.constructionType);
	...
	ctx.vkd.cmdBindVertexBuffers(cmdBuffer, 0u, de::sizeU32(rawVertexBuffers), de::dataOrNull(rawVertexBuffers), de::dataOrNull(rawVertexBufferOffsets));
	ctx.vkd.cmdBindDescriptorSets(cmdBuffer, bindPoint, *pipelineLayout, 0u, 1u, &descriptorSet.get(), 0u, nullptr);
	ctx.vkd.cmdSetVertexInputEXT(cmdBuffer, de::sizeU32(bindingDescriptions), de::dataOrNull(bindingDescriptions), de::sizeU32(attributeDescriptions), de::dataOrNull(attributeDescriptions));
		...
		VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_ASPECT_COLOR_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
	{
		...
		cmdPipelineMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, &barrier);
	}
	...
	if (!tcu::floatThresholdCompare(log, "Result", "", referenceAccess, resultAccess, colorThres, tcu::COMPARE_LOG_ON_ERROR))
			...
			for (uint32_t chIdx = 0u; chIdx < channelCountU32; ++chIdx)
			{
					...
					channelClass == tcu::TEXTURECHANNELCLASS_FLOATING_POINT)
				{
					...
					if (de::abs(res - ref) > thr)
					{
						...
						log << tcu::TestLog::Message
							<< "Unexpected result in point " << pointIdx << " channel " << chIdx << ": found " << res << " but expected " << ref << " (threshold " << thr << ")"
				...
				else if (channelClass == tcu::TEXTURECHANNELCLASS_SIGNED_INTEGER)
				{
					...
					if (res != ref)
					{
						dataOK = false;
						log << tcu::TestLog::Message
							<< "Unexpected result in point " << pointIdx << " channel " << chIdx << ": found " << res << " but expected " << ref
							<< tcu::TestLog::EndMessage;
					}
				}
...
void createLegacyVertexAttributesTests (tcu::TestCaseGroup* group, PipelineConstructionType constructionType)
{
	...
	for (const auto& format : formatsToTest)
	{
		...
		for (const uint32_t stride : strides)
			...
			{
				const bool isFloatFormat   = (fmtClass == tcu::TEXTURECHANNELCLASS_FLOATING_POINT || fmtClass == tcu::TEXTURECHANNELCLASS_UNSIGNED_FIXED_POINT || fmtClass == tcu::TEXTURECHANNELCLASS_SIGNED_FIXED_POINT);
				...
				for (const auto attributeOffset : { 0u, 1u })
					for (const auto memoryOffset : { 0u, 1u })
					{
						if (attributeOffset != 0u || memoryOffset != 0u)
						{
							...
							bool aligned = !checkAny(vertexBitWidth, channelCount, [](int width) { return width > 8/*bits*/; });
							if (aligned)
								continue;
						}

						const auto shortName  = getFormatShortName(format);
						const auto aoSuffix   = ((attributeOffset > 0u) ? std::string("_attribute_offset_") + std::to_string(attributeOffset) : "");
						const auto moSuffix   = ((memoryOffset > 0u)    ? std::string("_memory_offset_") + std::to_string(memoryOffset)       : "");
						const auto testName   = shortName + "_" + shaderFormat.desc + "_stride_" + std::to_string(stride) + aoSuffix + moSuffix;
					...
					for (const auto memoryOffset : { 0u, 1u })
					{
						...
						for (const auto format : tuple)
						{
							...
							else
							{
								...
								const ShaderFormat integerAlternative = (signedClass ? ShaderFormat::UNSIGNED_INT : ShaderFormat::SIGNED_INT);
