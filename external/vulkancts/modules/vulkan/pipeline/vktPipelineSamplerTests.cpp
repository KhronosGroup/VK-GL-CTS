...
{
public:
    SamplerTest(tcu::TestContext &testContext, const char *name, const char *description, SamplerViewType imageViewType,
...
{
public:
    SamplerMagFilterTest(tcu::TestContext &testContext, const char *name, const char *description,
...
{
public:
    SamplerMinFilterTest(tcu::TestContext &testContext, const char *name, const char *description,
...
{
public:
    SamplerMagReduceFilterTest(tcu::TestContext &testContext, const char *name, const char *description,
                               SamplerViewType imageViewType, VkFormat imageFormat, VkComponentMapping componentMapping,
...
{
public:
    SamplerMinReduceFilterTest(tcu::TestContext &testContext, const char *name, const char *description,
                               SamplerViewType imageViewType, VkFormat imageFormat, VkComponentMapping componentMapping,
                               VkSamplerReductionMode reductionMode, bool separateStencilUsage);
...
{
public:
    SamplerLodTest(tcu::TestContext &testContext, const char *name, const char *description,
...
{
public:
    SamplerAddressModesTest(tcu::TestContext &testContext, const char *name, const char *description,
                            SamplerViewType imageViewType, VkFormat imageFormat, VkSamplerAddressMode addressU,
                            VkSamplerAddressMode addressV, VkSamplerAddressMode addressW, VkBorderColor borderColor,
...
SamplerTest::SamplerTest(tcu::TestContext &testContext, const char *name, const char *description,
                                                                        ...
                                                                        bool separateStencilUsage) const
{
    ...
    const VkImageAspectFlags imageAspect =
        (!isCompressedFormat(imageFormat) && hasDepthComponent(mapVkFormat(imageFormat).order)) ?
    ...
    return ImageSamplingInstanceParams(renderSize, imageViewType, imageFormat, getImageSize(imageViewType, imageSize),
                                       getArraySize(imageViewType), componentMapping, subresourceRange, samplerParams,
                                       samplerLod, vertices, separateStencilUsage);
}

void SamplerTest::checkSupport(Context &context) const
{
    checkSupportImageSamplingInstance(context,
                                      getImageSamplingInstanceParams(m_imageViewType, m_imageFormat, m_imageSize,
                                                                     m_samplerLod, m_separateStencilUsage));
}
...
SamplerMagFilterTest::SamplerMagFilterTest(tcu::TestContext &testContext, const char *name, const char *description,
                                           SamplerViewType imageViewType, VkFormat imageFormat, VkFilter magFilter,
                                           bool separateStencilUsage)
...
SamplerMinFilterTest::SamplerMinFilterTest(tcu::TestContext &testContext, const char *name, const char *description,
                                           SamplerViewType imageViewType, VkFormat imageFormat, VkFilter minFilter,
                                           bool separateStencilUsage)
...
SamplerMagReduceFilterTest::SamplerMagReduceFilterTest(tcu::TestContext &testContext, const char *name,
                                                       const char *description, SamplerViewType imageViewType,
...
SamplerLodTest::SamplerLodTest(tcu::TestContext &testContext, const char *name, const char *description,
                               SamplerViewType imageViewType, VkFormat imageFormat, VkSamplerMipmapMode mipmapMode,
                               float minLod, float maxLod, float mipLodBias, float samplerLod,
                               bool separateStencilUsage)
...
SamplerAddressModesTest::SamplerAddressModesTest(tcu::TestContext &testContext, const char *name,
                                                 const char *description, SamplerViewType imageViewType,
                                                 VkFormat imageFormat, VkSamplerAddressMode addressU,
                                                 VkSamplerAddressMode addressV, VkSamplerAddressMode addressW,
                                                 VkBorderColor borderColor, rr::GenericVec4 customBorderColorValue,
                                                 bool customBorderColorFormatless, bool separateStencilUsage)
    : SamplerTest(testContext, name, description, imageViewType, imageFormat, 8, 0.0f, separateStencilUsage)
    , m_addressU(addressU)
    , m_addressV(addressV)
    , m_addressW(addressW)
...
MovePtr<tcu::TestCaseGroup> createSamplerMagFilterTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                        VkFormat imageFormat, bool separateStencilUsage)
{
    MovePtr<tcu::TestCaseGroup> samplerMagFilterTests(
        new tcu::TestCaseGroup(testCtx, "mag_filter", "Tests for magnification filter"));

    if (imageViewType.isNormalized() &&
        (isCompressedFormat(imageFormat) || (!isIntFormat(imageFormat) && !isUintFormat(imageFormat))))
        samplerMagFilterTests->addChild(
            new SamplerMagFilterTest(testCtx, "linear", "Magnifies image using VK_FILTER_LINEAR", imageViewType,
                                     imageFormat, VK_FILTER_LINEAR, separateStencilUsage));
    samplerMagFilterTests->addChild(new SamplerMagFilterTest(testCtx, "nearest",
                                                             "Magnifies image using VK_FILTER_NEAREST", imageViewType,
                                                             imageFormat, VK_FILTER_NEAREST, separateStencilUsage));

    return samplerMagFilterTests;
}

MovePtr<tcu::TestCaseGroup> createSamplerMinFilterTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                        VkFormat imageFormat, bool separateStencilUsage)
{
    MovePtr<tcu::TestCaseGroup> samplerMinFilterTests(
        new tcu::TestCaseGroup(testCtx, "min_filter", "Tests for minification filter"));

    if (imageViewType.isNormalized() &&
        (isCompressedFormat(imageFormat) || (!isIntFormat(imageFormat) && !isUintFormat(imageFormat))))
        samplerMinFilterTests->addChild(new SamplerMinFilterTest(testCtx, "linear",
                                                                 "Minifies image using VK_FILTER_LINEAR", imageViewType,
                                                                 imageFormat, VK_FILTER_LINEAR, separateStencilUsage));
    samplerMinFilterTests->addChild(new SamplerMinFilterTest(testCtx, "nearest",
                                                             "Minifies image using VK_FILTER_NEAREST", imageViewType,
                                                             imageFormat, VK_FILTER_NEAREST, separateStencilUsage));
...
MovePtr<tcu::TestCaseGroup> createSamplerMagReduceFilterTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                              VkFormat imageFormat, bool separateStencilUsage)
{
    MovePtr<tcu::TestCaseGroup> samplerMagReduceFilterTests(
        new tcu::TestCaseGroup(testCtx, "mag_reduce", "Tests for magnification reduce filter"));
    ...
    for (size_t i = 0; i < DE_LENGTH_OF_ARRAY(reduceFilterComponentMappings); ++i)
    {
        ...
        MovePtr<tcu::TestCaseGroup> componentGroup(new tcu::TestCaseGroup(
            testCtx, getComponentMappingGroupName(mapping).c_str(), "Group for given view component mapping"));
        ...
        if (isCompressedFormat(imageFormat) || (!isIntFormat(imageFormat) && !isUintFormat(imageFormat)))
        {
            componentGroup->addChild(new SamplerMagReduceFilterTest(
                testCtx, "average", "Magnifies image using VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE_EXT",
                imageViewType, imageFormat, mapping, VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE_EXT,
                separateStencilUsage));
        }
        componentGroup->addChild(new SamplerMagReduceFilterTest(
            testCtx, "min", "Magnifies and reduces image using VK_SAMPLER_REDUCTION_MODE_MIN_EXT", imageViewType,
            imageFormat, mapping, VK_SAMPLER_REDUCTION_MODE_MIN_EXT, separateStencilUsage));
        componentGroup->addChild(new SamplerMagReduceFilterTest(
            testCtx, "max", "Magnifies and reduces image using VK_SAMPLER_REDUCTION_MODE_MAX_EXT", imageViewType,
            imageFormat, mapping, VK_SAMPLER_REDUCTION_MODE_MAX_EXT, separateStencilUsage));
        samplerMagReduceFilterTests->addChild(componentGroup.release());
    }
    return samplerMagReduceFilterTests;
}

MovePtr<tcu::TestCaseGroup> createSamplerMinReduceFilterTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                              VkFormat imageFormat, bool separateStencilUsage)
{
    MovePtr<tcu::TestCaseGroup> samplerMinReduceFilterTests(
        new tcu::TestCaseGroup(testCtx, "min_reduce", "Tests for minification reduce filter"));
    ...
    for (size_t i = 0; i < DE_LENGTH_OF_ARRAY(reduceFilterComponentMappings); ++i)
    {
        ...
        if (isCompressedFormat(imageFormat) || (!isIntFormat(imageFormat) && !isUintFormat(imageFormat)))
        {
            componentGroup->addChild(new SamplerMinReduceFilterTest(
                testCtx, "average", "Minifies image using VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE_EXT",
                imageViewType, imageFormat, mapping, VK_SAMPLER_REDUCTION_MODE_WEIGHTED_AVERAGE_EXT,
                separateStencilUsage));
        }
        componentGroup->addChild(new SamplerMinReduceFilterTest(
            testCtx, "min", "Minifies and reduces image using VK_SAMPLER_REDUCTION_MODE_MIN_EXT", imageViewType,
            imageFormat, mapping, VK_SAMPLER_REDUCTION_MODE_MIN_EXT, separateStencilUsage));
        componentGroup->addChild(new SamplerMinReduceFilterTest(
            testCtx, "max", "Minifies and reduces image using VK_SAMPLER_REDUCTION_MODE_MAX_EXT", imageViewType,
            imageFormat, mapping, VK_SAMPLER_REDUCTION_MODE_MAX_EXT, separateStencilUsage));
...
MovePtr<tcu::TestCaseGroup> createSamplerLodTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                  VkFormat imageFormat, VkSamplerMipmapMode mipmapMode,
                                                  bool separateStencilUsage)
{
    ...
    for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testCaseConfigs); configNdx++)
    {
        ...
        samplerLodTests->addChild(new SamplerLodTest(testCtx, config.name, config.description, imageViewType,
...
MovePtr<tcu::TestCaseGroup> createSamplerMipmapTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                     VkFormat imageFormat, bool separateStencilUsage)
{
    MovePtr<tcu::TestCaseGroup> samplerMipmapTests(new tcu::TestCaseGroup(testCtx, "mipmap", "Tests for mipmap modes"));

    // Mipmap mode: nearest
    MovePtr<tcu::TestCaseGroup> mipmapNearestTests(
        new tcu::TestCaseGroup(testCtx, "nearest", "Uses VK_TEX_MIPMAP_MODE_NEAREST"));
    mipmapNearestTests->addChild(
        createSamplerLodTests(testCtx, imageViewType, imageFormat, VK_SAMPLER_MIPMAP_MODE_NEAREST, separateStencilUsage)
            .release());
    ...
    if (isCompressedFormat(imageFormat) || (!isIntFormat(imageFormat) && !isUintFormat(imageFormat)))
    {
        MovePtr<tcu::TestCaseGroup> mipmapLinearTests(
            new tcu::TestCaseGroup(testCtx, "linear", "Uses VK_TEX_MIPMAP_MODE_LINEAR"));
        mipmapLinearTests->addChild(createSamplerLodTests(testCtx, imageViewType, imageFormat,
...
std::string getAddressModesCaseName(VkSamplerAddressMode u, VkSamplerAddressMode v, VkSamplerAddressMode w,
                                    BorderColor border, tcu::IVec4 customIntValue, bool formatless)
{
...
MovePtr<tcu::TestCaseGroup> createSamplerAddressModesTests(tcu::TestContext &testCtx, SamplerViewType imageViewType,
                                                           VkFormat imageFormat, bool separateStencilUsage)
{
    struct TestCaseConfig
    {
    ...
    MovePtr<tcu::TestCaseGroup> samplerAddressModesTests(
        new tcu::TestCaseGroup(testCtx, "address_modes", "Tests for address modes"));
    ...
    for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testCaseConfigs); configNdx++)
    {
        ...
        samplerAddressModesTests->addChild(new SamplerAddressModesTest(
            ...
            getAddressModesCaseName(config.u, config.v, config.w, config.border, config.customColorValueInt,
                ...
                .c_str(),
            "", imageViewType, imageFormat, config.u, config.v, config.w,
            ...
            config.customColorFormatless, separateStencilUsage));
    }
...
public:
    ...
    ExactSamplingCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                      const Params &params);
...
ExactSamplingCase::ExactSamplingCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const Params &params)
...
tcu::TestStatus ExactSamplingInstance::iterate(void)
{
    ...
    const float maxAnisotropy =
        (anisotropyEnable ? vk::getPhysicalDeviceProperties(vki, physDevice).limits.maxSamplerAnisotropy : 1.0f);
    ...
    const auto framebuffer = vk::makeFramebuffer(vkd, device, renderPass.get(), 1u, &fbView.get(), texExtent.width,
    ...
    const auto pipeline = vk::makeGraphicsPipeline(
        vkd, device, pipelineLayout.get(), vertexModule.get(), DE_NULL, DE_NULL, DE_NULL, fragModule.get(),
        renderPass.get(), viewports, scissors, vk::VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, 0u, 0u, &vertexInputInfo);
    ...
    for (int viewTypeNdx = 0; viewTypeNdx < DE_LENGTH_OF_ARRAY(imageViewTypes); viewTypeNdx++)
    {
        ...
        for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
        {
            ...
            if (!isCompressed && viewType.isNormalized())
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> minFilterTests =
                    createSamplerMinFilterTests(testCtx, viewType, format, separateStencilUsage);
                de::MovePtr<tcu::TestCaseGroup> minReduceFilterTests =
                    createSamplerMinReduceFilterTests(testCtx, viewType, format, separateStencilUsage);
            ...
            de::MovePtr<tcu::TestCaseGroup> magFilterTests =
                createSamplerMagFilterTests(testCtx, viewType, format, separateStencilUsage);
            formatGroup->addChild(magFilterTests.release());

            if (viewType.isNormalized())
            {
                de::MovePtr<tcu::TestCaseGroup> magReduceFilterTests =
                    createSamplerMagReduceFilterTests(testCtx, viewType, format, separateStencilUsage);
                de::MovePtr<tcu::TestCaseGroup> mipmapTests =
                    createSamplerMipmapTests(testCtx, viewType, format, separateStencilUsage);
            ...
            {
                de::MovePtr<tcu::TestCaseGroup> addressModesTests =
                    createSamplerAddressModesTests(testCtx, viewType, format, separateStencilUsage);
...
{
    de::MovePtr<tcu::TestCaseGroup> exactSamplingTests(
        new tcu::TestCaseGroup(testCtx, "exact_sampling", "Exact sampling tests"));
    ...
    for (const auto format : formats)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> formatGroup(
            new tcu::TestCaseGroup(testCtx, formatName.c_str(), description.c_str()));
        ...
        for (const auto &solid : solidColor)
        {
            ...
            for (int unIdx = 0; unIdx < DE_LENGTH_OF_ARRAY(unnormalizedCoordinates); ++unIdx)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> coordGroup(
                    new tcu::TestCaseGroup(testCtx, unnorm.name.c_str(), unnorm.desc.c_str()));
...
tcu::TestCaseGroup *createSamplerTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> separateStencilUsageSamplerTests(
        new tcu::TestCaseGroup(testCtx, "separate_stencil_usage", "testing VK_EXT_separate_stencil_uasge"));
    {
