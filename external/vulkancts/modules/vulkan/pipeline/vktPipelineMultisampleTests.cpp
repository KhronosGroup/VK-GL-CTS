...
public:
    RasterizationSamplesTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                             ...
                             ImageBackingMode backingMode, TestModeFlags modeFlags = 0u);
...
public:
    MinSampleShadingTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                         ...
                         float pointSize, ImageBackingMode backingMode, const bool minSampleShadingEnabled = true);
...
public:
    ...
    MultisampleRenderer(Context &context, const VkFormat colorFormat, const VkFormat depthStencilFormat,
                        ...
                        const VkPipelineMultisampleStateCreateInfo &multisampleStateParams,
                        const VkPipelineColorBlendAttachmentState &blendState, const RenderType renderType,
                        const ImageBackingMode backingMode, const float depthClearValue = 1.0f);
...
MultisampleTest::MultisampleTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                                 const VkPipelineMultisampleStateCreateInfo &multisampleStateParams,
                                 const VkPipelineColorBlendAttachmentState &blendState, GeometryType geometryType,
                                 float pointSize, ImageBackingMode backingMode)
    ...
    const VkPipelineColorBlendAttachmentState &colorBlendState) const
{
    return new RasterizationSamplesInstance(context, topology, pointSize, vertices, multisampleStateParams,
                                            colorBlendState, m_modeFlags, m_backingMode);
}
...
AlphaToOneTest::AlphaToOneTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
    ...
    : MultisampleTest(testContext, name, description, getAlphaToOneStateParams(rasterizationSamples),
                      getAlphaToOneBlendState(), GEOMETRY_TYPE_GRADIENT_QUAD, 1.0f, backingMode)
    , m_backingMode(backingMode)
{
}
...
AlphaToCoverageTest::AlphaToCoverageTest(tcu::TestContext &testContext, const std::string &name,
    ...
    : MultisampleTest(testContext, name, description, getAlphaToCoverageStateParams(rasterizationSamples),
                      getDefaultColorBlendAttachmentState(), geometryType, 1.0f, backingMode)
    , m_geometryType(geometryType)
    , m_backingMode(backingMode)
{
}
    ...
    , m_modeFlags(modeFlags)
{
    if (m_modeFlags != 0)
    {
        ...
        m_multisampleRenderer = de::MovePtr<MultisampleRenderer>(new MultisampleRenderer(
            ...
            multisampleStateParams, blendState, RENDER_TYPE_RESOLVE, backingMode));
    }
    else
    {
        m_multisampleRenderer = de::MovePtr<MultisampleRenderer>(
            new MultisampleRenderer(context, m_colorFormat, m_renderSize, topology, vertices, multisampleStateParams,
                                    blendState, RENDER_TYPE_RESOLVE, backingMode));
    }
}
...
tcu::TestStatus AlphaToCoverageNoColorAttachmentInstance::iterate(void)
{
    ...
    MultisampleRenderer renderer(m_context, m_colorFormat, m_depthStencilFormat, m_renderSize, true, false, 1u,
                                 ...
                                 RENDER_TYPE_DEPTHSTENCIL_ONLY, m_backingMode, 1.0f);
        ...
        for (int y = 0; y < height; ++y)
        {
            ...
            else if (x + y == 1)
            {
                ...
                const float minCoverage =
                    (m_enablePostDepthCoverage ? (float)m_refCoverageAfterDepthTest[m_rasterizationSamples].min /
                                                     (float)m_rasterizationSamples :
                                                 1.0f) *
                    ((float)m_refCoverageAfterDepthTest[m_rasterizationSamples].min / (float)m_rasterizationSamples);
                const float maxCoverage =
                    (m_enablePostDepthCoverage ? (float)m_refCoverageAfterDepthTest[m_rasterizationSamples].max /
                                                     (float)m_rasterizationSamples :
                                                 1.0f) *
...
MultisampleRenderer::MultisampleRenderer(
    ...
    const VkPipelineColorBlendAttachmentState &blendState, const RenderType renderType,
    const ImageBackingMode backingMode, const float depthClearValue)
    : m_context(context)
    , m_bindSemaphore(createSemaphore(context.getDeviceInterface(), context.getDevice()))
    , m_colorFormat(colorFormat)
            ...
            for (uint32_t i = 0u; i < numTopologies; ++i)
            {
                m_graphicsPipelines.push_back(VkPipelineSp(new Unique<VkPipeline>(makeGraphicsPipeline(
                    ...
                    *m_renderPass,           // const VkRenderPass                            renderPass
                    ...
                    subpassIdx,              // const uint32_t                                subpass
...
tcu::TestCaseGroup *createMultisampleTests(tcu::TestContext &testCtx)
{
    ...
    // Rasterization samples tests
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "primitive_triangle", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_OPAQUE_TRIANGLE, 1.0f,
                                                                IMAGE_BACKING_MODE_REGULAR));
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "primitive_line", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_OPAQUE_LINE, 1.0f,
                                                                IMAGE_BACKING_MODE_REGULAR));
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "primitive_point_1px", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_OPAQUE_POINT, 1.0f,
                                                                IMAGE_BACKING_MODE_REGULAR));
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "primitive_point", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_OPAQUE_POINT, 3.0f,
                                                                IMAGE_BACKING_MODE_REGULAR));

            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "depth", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_INVISIBLE_TRIANGLE, 1.0f,
                                                                IMAGE_BACKING_MODE_REGULAR, TEST_MODE_DEPTH_BIT));
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "stencil", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_INVISIBLE_TRIANGLE, 1.0f,
                                                                IMAGE_BACKING_MODE_REGULAR, TEST_MODE_STENCIL_BIT));
            samplesTests->addChild(new RasterizationSamplesTest(
                ...
                IMAGE_BACKING_MODE_REGULAR, TEST_MODE_DEPTH_BIT | TEST_MODE_STENCIL_BIT));

            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "primitive_triangle_sparse", "",
                                                                samples[samplesNdx], GEOMETRY_TYPE_OPAQUE_TRIANGLE,
                                                                1.0f, IMAGE_BACKING_MODE_SPARSE));
            ...
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "depth_sparse", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_INVISIBLE_TRIANGLE, 1.0f,
                                                                IMAGE_BACKING_MODE_SPARSE, TEST_MODE_DEPTH_BIT));
            samplesTests->addChild(new RasterizationSamplesTest(testCtx, "stencil_sparse", "", samples[samplesNdx],
                                                                GEOMETRY_TYPE_INVISIBLE_TRIANGLE, 1.0f,
                                                                IMAGE_BACKING_MODE_SPARSE, TEST_MODE_STENCIL_BIT));
            samplesTests->addChild(new RasterizationSamplesTest(
                testCtx, "depth_stencil_sparse", "", samples[samplesNdx], GEOMETRY_TYPE_INVISIBLE_TRIANGLE, 1.0f,
                IMAGE_BACKING_MODE_SPARSE, TEST_MODE_DEPTH_BIT | TEST_MODE_STENCIL_BIT));
            ...
            for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testConfigs); configNdx++)
            {
                ...
                for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
                {
                    ...
                    samplesTests->addChild(new MinSampleShadingTest(
                        ...
                        GEOMETRY_TYPE_OPAQUE_TRIANGLE, 1.0f, IMAGE_BACKING_MODE_REGULAR));
                    samplesTests->addChild(new MinSampleShadingTest(
            ...
            for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testConfigs); configNdx++)
            {
                ...
                for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
                {
                    ...
                    samplesTests->addChild(
                        new MinSampleShadingTest(testCtx, "quad", "", samples[samplesNdx], testConfig.minSampleShading,
                                                 GEOMETRY_TYPE_OPAQUE_QUAD, 1.0f, IMAGE_BACKING_MODE_REGULAR, true));
...

        {
            ...
            for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testConfigs); configNdx++)
            {
                ...
                for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
                {
                    ...
                    samplesTests->addChild(
                        new MinSampleShadingTest(testCtx, "quad", "", samples[samplesNdx], testConfig.minSampleShading,
                                                 GEOMETRY_TYPE_OPAQUE_QUAD, 1.0f, IMAGE_BACKING_MODE_REGULAR, false));
    ...
    // AlphaToOne tests
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            alphaToOneTests->addChild(
                new AlphaToOneTest(testCtx, caseName.str(), "", samples[samplesNdx], IMAGE_BACKING_MODE_REGULAR));

            caseName << "_sparse";
            alphaToOneTests->addChild(
                new AlphaToOneTest(testCtx, caseName.str(), "", samples[samplesNdx], IMAGE_BACKING_MODE_SPARSE));
        }
    ...
    // AlphaToCoverageEnable without color buffer tests
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            samplesTests->addChild(
                new AlphaToCoverageNoColorAttachmentTest(testCtx, "alpha_opaque", "", samples[samplesNdx],
                                                         GEOMETRY_TYPE_OPAQUE_QUAD, IMAGE_BACKING_MODE_REGULAR));
            samplesTests->addChild(
                new AlphaToCoverageNoColorAttachmentTest(testCtx, "alpha_opaque_sparse", "", samples[samplesNdx],
                                                         GEOMETRY_TYPE_OPAQUE_QUAD, IMAGE_BACKING_MODE_SPARSE));
    ...
    // Set color output at location 0 as unused, but use the alpha write to control coverage for rendering to color buffer at location 1.
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            samplesTests->addChild(
                new AlphaToCoverageColorUnusedAttachmentTest(testCtx, "alpha_opaque", "", samples[samplesNdx],
                                                             GEOMETRY_TYPE_OPAQUE_QUAD, IMAGE_BACKING_MODE_REGULAR));
            samplesTests->addChild(
                new AlphaToCoverageColorUnusedAttachmentTest(testCtx, "alpha_opaque_sparse", "", samples[samplesNdx],
                                                             GEOMETRY_TYPE_OPAQUE_QUAD, IMAGE_BACKING_MODE_SPARSE));
            samplesTests->addChild(
                new AlphaToCoverageColorUnusedAttachmentTest(testCtx, "alpha_invisible", "", samples[samplesNdx],
                                                             GEOMETRY_TYPE_INVISIBLE_QUAD, IMAGE_BACKING_MODE_REGULAR));
            samplesTests->addChild(
                new AlphaToCoverageColorUnusedAttachmentTest(testCtx, "alpha_invisible_sparse", "", samples[samplesNdx],
                                                             GEOMETRY_TYPE_INVISIBLE_QUAD, IMAGE_BACKING_MODE_SPARSE));
    ...
    // Sample mask with and without vk_ext_post_depth_coverage
    {
        ...
        for (int ndx = 0; ndx < DE_LENGTH_OF_ARRAY(standardSamplesSet); ++ndx)
        {
            ...
            sampleMaskWithDepthTestGroup->addChild(
                new SampleMaskWithDepthTestTest(testCtx, caseName.str(), "", standardSamplesSet[ndx]));

            caseName << "_post_depth_coverage";
            sampleMaskWithDepthTestGroup->addChild(
                new SampleMaskWithDepthTestTest(testCtx, caseName.str(), "", standardSamplesSet[ndx], true));
        }
