...
RasterizationSamplesTest::RasterizationSamplesTest(tcu::TestContext &testContext, const std::string &name,
                                                   ...
                                                   const bool useFragmentShadingRate)
    : MultisampleTest(testContext, name, description, pipelineConstructionType,
...
AlphaToOneTest::AlphaToOneTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                               ...
                               const bool useFragmentShadingRate)
    : MultisampleTest(testContext, name, description, pipelineConstructionType,
                      getAlphaToOneStateParams(rasterizationSamples), getAlphaToOneBlendState(),
                      GEOMETRY_TYPE_GRADIENT_QUAD, 1.0f, backingMode, useFragmentShadingRate)
    , m_backingMode(backingMode)
{
}
...
AlphaToCoverageTest::AlphaToCoverageTest(tcu::TestContext &testContext, const std::string &name,
                                         ...
                                         ImageBackingMode backingMode, const bool useFragmentShadingRate)
    : MultisampleTest(testContext, name, description, pipelineConstructionType,
                      getAlphaToCoverageStateParams(rasterizationSamples), getDefaultColorBlendAttachmentState(),
                      geometryType, 1.0f, backingMode, useFragmentShadingRate)
    , m_geometryType(geometryType)
    , m_backingMode(backingMode)
{
}
...
AlphaToCoverageNoColorAttachmentTest::AlphaToCoverageNoColorAttachmentTest(
    ...
    const PipelineConstructionType pipelineConstructionType, VkSampleCountFlagBits rasterizationSamples,
    GeometryType geometryType, ImageBackingMode backingMode, const bool useFragmentShadingRate)
    : MultisampleTest(testContext, name, description, pipelineConstructionType, getStateParams(rasterizationSamples),
...
SampleMaskWithConservativeTest::SampleMaskWithConservativeTest(
    tcu::TestContext &testContext, const std::string &name, const std::string &description,
    const PipelineConstructionType pipelineConstructionType, const VkSampleCountFlagBits rasterizationSamples,
        ...
        for (uint32_t subpassIdx = 0; subpassIdx < numSubpasses; subpassIdx++)
            for (uint32_t i = 0u; i < numTopologies; ++i)
            {
                ...
                m_graphicsPipelines.back()
                    ...
                    .setupPreRasterizationShaderState(viewports, scissors, *m_pipelineLayout, *m_renderPass, subpassIdx,
                                                      *m_vertexShaderModule, &rasterizationStateCreateInfo)
        ...
        else if (m_renderType == RENDER_TYPE_COPY_SAMPLES)
        {
            ...
            for (int32_t sampleId = 0; sampleId < (int32_t)m_perSampleImages.size(); ++sampleId)
            {
                ...
                vk.cmdBindPipeline(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                   m_copySamplePipelines[sampleId].getPipeline());
    ...
    // Rasterization samples tests
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            samplesTests->addChild(new RasterizationSamplesTest(
                ...
                GEOMETRY_TYPE_OPAQUE_POINT, 3.0f, IMAGE_BACKING_MODE_REGULAR, 0u, useFragmentShadingRate));

            samplesTests->addChild(new RasterizationSamplesTest(
                testCtx, "depth", "", pipelineConstructionType, samples[samplesNdx], GEOMETRY_TYPE_INVISIBLE_TRIANGLE,
                1.0f, IMAGE_BACKING_MODE_REGULAR, TEST_MODE_DEPTH_BIT, useFragmentShadingRate));
            samplesTests->addChild(new RasterizationSamplesTest(
            ...
            samplesTests->addChild(new RasterizationSamplesTest(
                testCtx, "depth_stencil", "", pipelineConstructionType, samples[samplesNdx],
            ...
            for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testConfigs); configNdx++)
            {
                ...
                for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
                {
                    ...
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_triangle", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_TRIANGLE, 1.0f, IMAGE_BACKING_MODE_REGULAR,
                        true, useFragmentShadingRate));
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_line", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_LINE, 1.0f, IMAGE_BACKING_MODE_REGULAR, true,
                        useFragmentShadingRate));
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_point_1px", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_POINT, 1.0f, IMAGE_BACKING_MODE_REGULAR, true,
                        useFragmentShadingRate));
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_point", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_POINT, 3.0f, IMAGE_BACKING_MODE_REGULAR, true,
                        useFragmentShadingRate));
#ifndef CTS_USES_VULKANSC
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_triangle_sparse", "", pipelineConstructionType, samples[samplesNdx],
                    ...
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_line_sparse", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_LINE, 1.0f, IMAGE_BACKING_MODE_SPARSE, true,
                        useFragmentShadingRate));
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_point_1px_sparse", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_POINT, 1.0f, IMAGE_BACKING_MODE_SPARSE, true,
                        useFragmentShadingRate));
                    samplesTests->addChild(new MinSampleShadingTest(
                        testCtx, "primitive_point_sparse", "", pipelineConstructionType, samples[samplesNdx],
                        testConfig.minSampleShading, GEOMETRY_TYPE_OPAQUE_POINT, 3.0f, IMAGE_BACKING_MODE_SPARSE, true,
                        useFragmentShadingRate));
#endif // CTS_USES_VULKANSC
    ...
    // SampleMask tests
    {
        ...
        for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testConfigs); configNdx++)
        {
            ...
            for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
            {
                ...
                samplesTests->addChild(new SampleMaskTest(
                    testCtx, "primitive_line_sparse", "", pipelineConstructionType, samples[samplesNdx], mask,
                    GEOMETRY_TYPE_OPAQUE_LINE, 1.0f, IMAGE_BACKING_MODE_SPARSE, useFragmentShadingRate));
                samplesTests->addChild(new SampleMaskTest(
                ...
                samplesTests->addChild(new SampleMaskTest(
                    testCtx, "primitive_point_sparse", "", pipelineConstructionType, samples[samplesNdx], mask,
                    GEOMETRY_TYPE_OPAQUE_POINT, 3.0f, IMAGE_BACKING_MODE_SPARSE, useFragmentShadingRate));
#endif // CTS_USES_VULKANSC
    ...
    // AlphaToOne tests
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            alphaToOneTests->addChild(new AlphaToOneTest(testCtx, caseName.str(), "", pipelineConstructionType,
                                                         samples[samplesNdx], IMAGE_BACKING_MODE_REGULAR,
                                                         useFragmentShadingRate));
#ifndef CTS_USES_VULKANSC
            caseName << "_sparse";
            alphaToOneTests->addChild(new AlphaToOneTest(testCtx, caseName.str(), "", pipelineConstructionType,
                                                         samples[samplesNdx], IMAGE_BACKING_MODE_SPARSE,
                                                         useFragmentShadingRate));
#endif // CTS_USES_VULKANSC
        }
    ...
    // AlphaToCoverageEnable tests
    {
        ...
        for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(samples); samplesNdx++)
        {
            ...
            samplesTests->addChild(new AlphaToCoverageTest(
                ...
                GEOMETRY_TYPE_TRANSLUCENT_QUAD, IMAGE_BACKING_MODE_SPARSE, useFragmentShadingRate));
            samplesTests->addChild(new AlphaToCoverageTest(
...
#endif // CTS_USES_VULKANSC
            samplesTests->addChild(new AlphaToCoverageColorUnusedAttachmentTest(
                testCtx, "alpha_invisible", "", pipelineConstructionType, samples[samplesNdx],
            ...
            for (int samplesNdx = 0; samplesNdx < DE_LENGTH_OF_ARRAY(standardSamplesSet); ++samplesNdx)
            {
                ...
                for (int configNdx = 0; configNdx < DE_LENGTH_OF_ARRAY(testConfigs); configNdx++)
                {
                    ...
                    modesGroup->addChild(new SampleMaskWithConservativeTest(
                        testCtx, caseName + testConfig.name, testConfig.description, pipelineConstructionType,
                        standardSamplesSet[samplesNdx], rasterizationMode[modeNdx], testConfig.enableMinSampleShading,
                        testConfig.minSampleShading, testConfig.enableSampleMask, testConfig.sampleMask,
