...
tcu::TestStatus VariableRateTestInstance::iterate(void)
{
    ...
    for (const auto samples : m_params.subpassCounts)
    {
        ...
        outputPipelines.back()
            ...
            .setupFragmentOutputState(*renderPassSingleSubpass, 0u, &colorBlendStateCreateInfo,
                                      &multisampleStateCreateInfo)
    ...
    for (size_t i = 0; i < m_params.subpassCounts.size(); ++i)
    {
        ...
        referencePipelines.back()
            ...
            .setupFragmentShaderState(*pipelineLayout, *renderPassMultiplePasses, subpass, *fragModule, DE_NULL,
                                      &multisampleStateCreateInfo)
            .setupFragmentOutputState(*renderPassMultiplePasses, subpass, &colorBlendStateCreateInfo,
                                      &multisampleStateCreateInfo)
            .setMonolithicPipelineLayout(*pipelineLayout)
            .buildPipeline();
    }
