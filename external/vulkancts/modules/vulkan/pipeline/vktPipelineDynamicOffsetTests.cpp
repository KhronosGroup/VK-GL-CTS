...
void DynamicOffsetGraphicsTestInstance::init(void)
{
    ...
    for (uint32_t renderPassIdx = 0; renderPassIdx < m_params.numCmdBuffers; renderPassIdx++)
    {
        ...
        m_renderPasses.push_back(
            VkRenderPassSp(new Unique<VkRenderPass>(createRenderPass(vk, vkDevice, &renderPassInfo))));
    }

    // Create framebuffers
    for (uint32_t framebufferIdx = 0; framebufferIdx < m_params.numCmdBuffers; framebufferIdx++)
    {
        ...
        m_framebuffers.push_back(
            VkFramebufferSp(new Unique<VkFramebuffer>(createFramebuffer(vk, vkDevice, &framebufferParams))));
    }
    ...
    for (uint32_t cmdBufferIdx = 0; cmdBufferIdx < m_params.numCmdBuffers; cmdBufferIdx++)
    {
        ...
        beginRenderPass(vk, **m_cmdBuffers[idx], **m_renderPasses[idx], **m_framebuffers[idx],
                        makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), attachmentClearValue);
        vk.cmdBindPipeline(**m_cmdBuffers[idx], VK_PIPELINE_BIND_POINT_GRAPHICS,
                           m_graphicsPipelines[idx].getPipeline());
...
{
public:
    DynamicOffsetGraphicsTest(tcu::TestContext &testContext, const string &name, const string &description,
...
{
public:
    DynamicOffsetComputeTest(tcu::TestContext &testContext, const string &name, const string &description,
                             const TestParams &params);
...
{
public:
    DynamicOffsetMixedTestInstance(Context &context, const tcu::IVec2 renderSize, const uint32_t numInstances,
                                   const bool testAllOffsets, const bool reverseOrder, const bool runComputeFirst,
                                   const uint32_t vertexOffset, const uint32_t sharedUboOffset,
                                   const uint32_t fragUboOffset, const uint32_t ssboReadOffset,
                                   const uint32_t ssboWriteOffset)
        : vkt::TestInstance(context)
...
tcu::TestStatus DynamicOffsetMixedTestInstance::iterate(void)
{
    ...
    Move<VkPipeline> computePipeline =
        createComputePipeline(vk, device, (vk::VkPipelineCache)0u, &compPipelinecreateInfo);
    ...
    uint32_t dynamicAlignmentVec4          = bufferElementSizeVec4;
    uint32_t dynamicAlignmentMat4          = bufferElementSizeMat4;

    if (minUboAlignment > 0)
    {
        dynamicAlignmentVec4 =
            (dynamicAlignmentVec4 + (uint32_t)minUboAlignment - 1) & ~((uint32_t)minUboAlignment - 1);
        dynamicAlignmentMat4 =
            (dynamicAlignmentMat4 + (uint32_t)minUboAlignment - 1) & ~((uint32_t)minUboAlignment - 1);
    }

    const uint32_t bufferSizeVec4 = m_numInstances * dynamicAlignmentVec4;
    const uint32_t bufferSizeMat4 = m_numInstances * dynamicAlignmentMat4;
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if (runGraphics)
        {
            beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                            makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), clearColorValue);
        ...
        if (m_testAllOffsets)
        {
            ...
            {
                uint32_t offsetVec4 = dynamicAlignmentVec4 * instance;
                uint32_t offsetMat4 = dynamicAlignmentMat4 * instance;
        ...
        else
        {
            ...
            offsets.push_back(m_reverseOrder ? dynamicAlignmentMat4 * m_ssboReadOffset :
                                               dynamicAlignmentVec4 * m_vertexOffset);
            offsets.push_back(m_reverseOrder ? dynamicAlignmentMat4 * m_fragUboOffset :
                                               dynamicAlignmentVec4 * m_sharedUboOffset);
            offsets.push_back(dynamicAlignmentVec4 * m_ssboWriteOffset);
            offsets.push_back(m_reverseOrder ? dynamicAlignmentVec4 * m_sharedUboOffset :
                                               dynamicAlignmentMat4 * m_fragUboOffset);
            offsets.push_back(m_reverseOrder ? dynamicAlignmentVec4 * m_vertexOffset :
                                               dynamicAlignmentMat4 * m_ssboReadOffset);
    ...
    // Check result buffer values
    {
        ...
        for (uint32_t i = 0; i < m_numInstances; i++)
        {
            ...
            outColors.push_back(
                *(tcu::Vec4 *)((uint8_t *)ssboBufferAllocWrite->getHostPtr() + dynamicAlignmentVec4 * i));
...
{
public:
    DynamicOffsetMixedTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                           const tcu::IVec2 renderSize, const uint32_t numInstances, const bool testAllOffsets,
                           const bool reverseOrder, const bool runComputeFirst = false,
...
{
    return new DynamicOffsetMixedTestInstance(context, m_renderSize, m_numInstances, m_testAllOffsets, m_reverseOrder,
                                              m_runComputeFirst, m_vertexOffset, m_sharedUboOffset, m_fragUboOffset,
                                              m_ssboReadOffset, m_ssboWriteOffset);
}
                                             ...
                                             PipelineConstructionType pipelineConstructionType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> dynamicOffsetTests(
        new tcu::TestCaseGroup(testCtx, "dynamic_offset", "Dynamic offset tests"));
                     ...
                     numCmdBuffersIdx++)
                {
                         ...
                         reverseOrderIdx++)
                    {
                             ...
                             numDescriptorSetBindingsIdx++)
                        {
                            ...
                            {
                                de::MovePtr<tcu::TestCaseGroup> numDynamicBindingsGroup(new tcu::TestCaseGroup(
                                    testCtx, numDynamicBindings[numDynamicBindingsIdx].name, ""));
                ...
                for (const auto &offsets : numOffsets)
                {
                    for (const auto &pipeline : pipelineOrders)
                    {
                        allOffsetsGroup->addChild(new DynamicOffsetMixedTest(
                            testCtx, std::string(order.name) + "_" + std::string(offsets.name) + "_" + pipeline.name,
