...
template <>
void MSCase<MSCaseInterpolateAtSampleDistinctValues>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
template <>
void MSCase<MSCaseInterpolateAtSampleSingleSample>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
template <>
void MSCase<MSCaseInterpolateAtSampleIgnoresCentroid>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
template <>
void MSCase<MSCaseInterpolateAtSampleConsistency>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
void MSCase<MSCaseInterpolateAtCentroidConsistency>::init(void)
{
    ...
    m_testCtx.getLog()
        << tcu::TestLog::Message
        << "Verifying that interpolateAtCentroid does not return different values than a corresponding centroid "
           "qualified varying.\n"
        << (componentMsg.empty() ? std::string() : componentMsg + "\n")
    ...
    if (m_imageMSParams.componentData.source == multisample::ComponentSource::NONE)
    {
        fs << "    const vec2 pos_interpolated_at_centroid = interpolateAtCentroid(fs_in_pos_screen_sample);\n"
           << "    const bool valuesEqual                  = all(lessThan(abs(pos_interpolated_at_centroid - "
              "fs_in_pos_screen_centroid), vec2(threshold)));\n";
    }
    else if (m_imageMSParams.componentData.source == multisample::ComponentSource::CONSTANT)
    {
        const auto &index = m_imageMSParams.componentData.index;
        fs << "    const float pos_interpolated_at_centroid = interpolateAtCentroid(fs_in_pos_screen_sample[" << index
           ...
           << "    const bool  valuesEqual                  = (abs(pos_interpolated_at_centroid - "
              "fs_in_pos_screen_centroid["
           << index << "]) < threshold);\n";
    }
    else // multisample::ComponentSource::PUSH_CONSTANT
    {
        fs << "    const float pos_interpolated_at_centroid = "
              "interpolateAtCentroid(fs_in_pos_screen_sample[component]);\n"
           << "    const bool  valuesEqual                  = (abs(pos_interpolated_at_centroid - "
              "fs_in_pos_screen_centroid[component]) < threshold);\n";
    }
...
template <>
void MSCase<MSCaseInterpolateAtCentroidConsistency>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
template <>
void MSCase<MSCaseInterpolateAtOffsetPixelCenter>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
void MSCase<MSCaseInterpolateAtOffsetSamplePosition>::init(void)
{
    ...
    m_testCtx.getLog()
        << tcu::TestLog::Message
        << "Verifying that interpolateAtOffset of screen position with the offset of current sample position returns "
           "value "
...
template <>
void MSCase<MSCaseInterpolateAtOffsetSamplePosition>::checkSupport(Context &context) const
{
    context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING);
}
...
tcu::TestCaseGroup *createMultisampleInterpolationTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "multisample_interpolation", "Multisample Interpolation"));
    ...
    de::MovePtr<tcu::TestCaseGroup> caseGroup(
        new tcu::TestCaseGroup(testCtx, "sample_interpolate_at_single_sample", ""));
    ...
    for (uint32_t imageSizeNdx = 0u; imageSizeNdx < sizesElemCount; ++imageSizeNdx)
    {
        ...
        sizeGroup->addChild(multisample::MSCase<multisample::MSCaseInterpolateAtSampleSingleSample>::createCase(
            testCtx, "samples_" + de::toString(1), multisample::ImageMSParams(vk::VK_SAMPLE_COUNT_1_BIT, imageSize)));
    ...
    testGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleDistinctValues>>(
        testCtx, "sample_interpolate_at_distinct_values", imageSizes, sizesElemCount, imageSamples, samplesElemCount));
    testGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleIgnoresCentroid>>(
        testCtx, "sample_interpolate_at_ignores_centroid", imageSizes, sizesElemCount, imageSamples, samplesElemCount));
    ...
    sampleGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleConsistency>>(
        testCtx, "all_components", imageSizes, sizesElemCount, imageSamples, samplesElemCount));
    sampleGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleConsistency>>(
        testCtx, "component_0", imageSizes, sizesElemCount, imageSamples, samplesElemCount,
        multisample::ComponentData{multisample::ComponentSource::CONSTANT, 0u}));
    sampleGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleConsistency>>(
        testCtx, "component_1", imageSizes, sizesElemCount, imageSamples, samplesElemCount,
        multisample::ComponentData{multisample::ComponentSource::CONSTANT, 1u}));
    sampleGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleConsistency>>(
        testCtx, "pushc_component_0", imageSizes, sizesElemCount, imageSamples, samplesElemCount,
        multisample::ComponentData{multisample::ComponentSource::PUSH_CONSTANT, 0u}));
    sampleGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtSampleConsistency>>(
        testCtx, "pushc_component_1", imageSizes, sizesElemCount, imageSamples, samplesElemCount,
        multisample::ComponentData{multisample::ComponentSource::PUSH_CONSTANT, 1u}));
    ...
    testGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleQualifierDistinctValues>>(
        testCtx, "sample_qualifier_distinct_values", imageSizes, sizesElemCount, imageSamples, samplesElemCount));

    de::MovePtr<tcu::TestCaseGroup> centroidGroup(new tcu::TestCaseGroup(
        testCtx, "centroid_interpolation_consistency", "Test consistency in centroid interpolation function"));
    ...
    testGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseCentroidQualifierInsidePrimitive>>(
        testCtx, "centroid_qualifier_inside_primitive", imageSizes, sizesElemCount, imageSamples, samplesElemCount));
    testGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseInterpolateAtOffsetPixelCenter>>(
        testCtx, "offset_interpolate_at_pixel_center", imageSizes, sizesElemCount, imageSamples, samplesElemCount));

    de::MovePtr<tcu::TestCaseGroup> offsetGroup(
        new tcu::TestCaseGroup(testCtx, "offset_interpolation_at_sample_position",
                               "Test interpolation at offset function works for sample positions"));
