...
struct SelectedShaders
{
    ...
    SelectedShaders(VkShaderStageFlagBits shaderStage, std::string name) : stage(shaderStage), shaderName(name)
    {
    }
};
...
tcu::TestStatus test(Context &context, const MaxVaryingsParam param)
{
    ...
    const Unique<VkFramebuffer> framebuffer(makeFramebuffer(vk, device, *renderPass, 1u, &colorImageView.get(),
    ...
    {
        pipelineBuilder.setShader(vk, device, shaders[i].stage,
                                  context.getBinaryCollection().get(shaders[i].shaderName.c_str()), &pSpecInfo);
    }
...
tcu::TestCaseGroup *createMaxVaryingsTests(tcu::TestContext &testCtx)
{
    ...
    tests.push_back(MaxVaryingsParam(VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_FRAGMENT_BIT,
                                     VK_SHADER_STAGE_VERTEX_BIT)); // Test max vertex outputs: VS-FS
    tests.push_back(MaxVaryingsParam(VK_SHADER_STAGE_VERTEX_BIT, VK_SHADER_STAGE_FRAGMENT_BIT,
                                     VK_SHADER_STAGE_FRAGMENT_BIT)); // Test max FS inputs: VS-FS
    tests.push_back(MaxVaryingsParam(
        VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, VK_SHADER_STAGE_FRAGMENT_BIT,
        VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)); // Test max tess evaluation outputs: VS-TCS-TES-FS
    tests.push_back(MaxVaryingsParam(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, VK_SHADER_STAGE_FRAGMENT_BIT,
                                     VK_SHADER_STAGE_FRAGMENT_BIT)); // Test fragment inputs: VS-TCS-TES-FS
    tests.push_back(MaxVaryingsParam(VK_SHADER_STAGE_GEOMETRY_BIT, VK_SHADER_STAGE_FRAGMENT_BIT,
    ...
    for (uint32_t testIndex = 0; testIndex < (uint32_t)tests.size(); ++testIndex)
    {
