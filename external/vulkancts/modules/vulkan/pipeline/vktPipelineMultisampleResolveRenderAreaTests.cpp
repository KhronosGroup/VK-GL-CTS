...
private:
    ...
    void preparePipelineWrapper(GraphicsPipelineWrapper &gpw, const VkPipelineLayout pipelineLayout,
                                const VkRenderPass renderPass, const VkShaderModule vertexModule,
                                const VkShaderModule fragmentModule, const tcu::IVec2 &framebufferSize);
...
Move<VkRenderPass> MultisampleRenderAreaTestInstance::makeRenderPass(const DeviceInterface &vk, const VkDevice device,
...
void MultisampleRenderAreaTestInstance::preparePipelineWrapper(
    GraphicsPipelineWrapper &gpw, const VkPipelineLayout pipelineLayout, const VkRenderPass renderPass,
    const VkShaderModule vertexModule, const VkShaderModule fragmentModule, const tcu::IVec2 &framebufferSize)
{
...
tcu::TestStatus MultisampleRenderAreaTestInstance::iterate(void)
{
    ...
    const Unique<VkFramebuffer> framebuffer(makeFramebuffer(vk, device, *renderPassOne, numUsedAttachmentImages,
                                                            attachmentImages, m_framebufferSize.x(),
                                                            m_framebufferSize.y()));
    ...
    beginRenderPass(vk, *commandBuffer, *renderPassOne, *framebuffer, fullRenderArea,
                    static_cast<uint32_t>(clearValuesFullArea.size()), clearValuesFullArea.data());
    endRenderPass(vk, *commandBuffer);

    // Draw shape when render area size is halved.
    beginRenderPass(vk, *commandBuffer, *renderPassTwo, *framebuffer, testRenderArea,
                    static_cast<uint32_t>(clearValuesTestArea.size()), clearValuesTestArea.data());
...
{
    de::MovePtr<tcu::TestCaseGroup> testGroupResolve(
        new tcu::TestCaseGroup(testCtx, "resolve", "resolving multisample image tests"));

    de::MovePtr<tcu::TestCaseGroup> testGroupRenderArea(
        new tcu::TestCaseGroup(testCtx, "renderpass_renderarea", "renderpass render area tests"));
