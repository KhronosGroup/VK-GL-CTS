...
constexpr VkImageUsageFlags kSSUsage   = (VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT);
...
void dynamicSampleShadingSupport (Context& context, DSS::Params params)
{
	...
	const auto ctx              = context.getContextCommonData();
	const auto formatProperties = getPhysicalDeviceImageFormatProperties(ctx.vki, ctx.physicalDevice, DSS::kFormat,
	                                                                     DSS::kImageType, DSS::kTiling, DSS::kMSUsage, 0u);
...
tcu::TestStatus dynamicSampleShadingTest (Context& context, DSS::Params params)
{
	...
	const auto msColorBuffer     = ImageWithMemory(ctx.vkd, ctx.device, ctx.allocator, msColorBufferCreateInfo, MemoryRequirement::Any);
	const auto msColorBufferView = makeImageView(ctx.vkd, ctx.device, *msColorBuffer, DSS::kViewType, DSS::kFormat, colorSRR);
	...
	ImageWithBuffer colorBuffer (
		...
		ctx.device,
		ctx.allocator,
		msColorBufferCreateInfo.extent,
		msColorBufferCreateInfo.format,
		DSS::kSSUsage,
		msColorBufferCreateInfo.imageType,
		colorSRR,
		msColorBufferCreateInfo.arrayLayers,
	...
	const auto textureView = makeImageView(ctx.vkd, ctx.device, *texture, DSS::kViewType, textureCreateInfo.format, colorSRR);
	...
	updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u), VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &texDescInfo);
	updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(1u), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &dbDescInfo);
	updateBuilder.update(ctx.vkd, ctx.device);
	...
	const auto subpass = makeSubpassDescription(
		0u,
		bindPoint,
		0u,
		nullptr,
		1u,
		&attachmentReferences.at(0u),
		&attachmentReferences.at(1u),
		nullptr,
		0u,
		nullptr);
	...
	renderPass.createFramebuffer(ctx.vkd, ctx.device, de::sizeU32(fbImageViews),
	                             de::dataOrNull(fbImages), de::dataOrNull(fbImageViews), vkExtent.width, vkExtent.height);
	...
	{
		makeVertexInputAttributeDescription(0u, 0u, vk::VK_FORMAT_R32G32B32A32_SFLOAT, static_cast<uint32_t>(offsetof(VertexData, position))),
		makeVertexInputAttributeDescription(1u, 0u, vk::VK_FORMAT_R32G32_SFLOAT, static_cast<uint32_t>(offsetof(VertexData, texCoords))),
	};
	...
	{
		const auto preClearBarrier = makeImageMemoryBarrier(0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, *texture, colorSRR);
		...
		ctx.vkd.cmdClearColorImage(cmdBuffer, *texture, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &texClearColor.color, 1u, &colorSRR);
		const auto postClearBarrier = makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, *texture, colorSRR);
		cmdPipelineImageMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, &postClearBarrier);
	}
		...
		VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_ASPECT_COLOR_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT);
	{
		...
		cmdPipelineMemoryBarrier(ctx.vkd, cmdBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, &atomicDataBarrier);
	}
	endCommandBuffer(ctx.vkd, cmdBuffer);
	submitCommandsAndWait(ctx.vkd, ctx.device, ctx.queue, cmdBuffer);
	...
	{
		std::ostringstream msg;
		msg << "Unexpected fragment shader count: expected at least " << minExpectedCount << " but found " << resultCount;
		TCU_FAIL(msg.str());
	}
	...
	if (!isConstructionTypeShaderObject(pipelineConstructionType))
	{
		...
		for (size_t i = 0; i < sampleCounts.size(); ++i)
			for (size_t j = i + 1; j < sampleCounts.size(); ++j)
			{
				...
				const auto dynamicCount = sampleCounts.at(j); // The actual dynamic count is always greater than the static value.
				...
				const auto testName = "dynamic_sample_shading_static_" + std::to_string(staticCount) + "_dynamic_" + std::to_string(dynamicCount);
				addFunctionCaseWithPrograms(miscGroup.get(), testName, dynamicSampleShadingSupport, dynamicSampleShadingPrograms, dynamicSampleShadingTest, params);
			}
	}
#endif // CTS_USES_VULKANSC
