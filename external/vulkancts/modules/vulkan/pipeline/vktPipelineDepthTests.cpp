...
public:
    ...
    DepthTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
              const VkFormat depthFormat, const VkCompareOp depthCompareOps[QUAD_COUNT],
              const bool separateDepthStencilLayouts, const bool depthBoundsTestEnable = false,
              const float depthBoundsMin = 0.0f, const float depthBoundsMax = 1.0f, const bool depthTestEnable = true,
...
{
public:
    DepthTestInstance(Context &context, const VkFormat depthFormat,
...
{
    return new DepthTestInstance(context, m_depthFormat, m_depthCompareOps, m_separateDepthStencilLayouts,
                                 m_depthBoundsTestEnable, m_depthBoundsMin, m_depthBoundsMax, m_depthTestEnable,
                                 m_stencilTestEnable, m_colorAttachmentEnable);
}
...
DepthTestInstance::DepthTestInstance(Context &context, const VkFormat depthFormat,
    ...
    , m_colorAttachmentEnable(colorAttachmentEnable)
{
    ...
    // Create pipeline
    {
        ...
        for (int quadNdx = 0; quadNdx < DepthTest::QUAD_COUNT; quadNdx++)
        {
            ...
            m_graphicsPipelines[quadNdx]           = makeGraphicsPipeline(
                     ...
                     ++separateDepthStencilLayouts)
                {
                    ...
                    for (size_t opsNdx = 0; opsNdx < DE_LENGTH_OF_ARRAY(depthOps); opsNdx++)
                    {
                        compareOpsTests->addChild(new DepthTest(
                            testCtx, getCompareOpsName(depthOps[opsNdx]), getCompareOpsDescription(depthOps[opsNdx]),
                        ...
                        compareOpsTests->addChild(new DepthTest(
                            ...
                            depthFormats[formatNdx], depthOps[opsNdx], useSeparateDepthStencilLayouts, true, 0.1f,
                            0.25f, true, false, colorEnabled));
                    }
                    // Special VkPipelineDepthStencilStateCreateInfo known to have issues
                    {
                        ...
                        compareOpsTests->addChild(new DepthTest(
                            testCtx, "never_zerodepthbounds_depthdisabled_stencilenabled",
                            "special VkPipelineDepthStencilStateCreateInfo", depthFormats[formatNdx], depthOpsSpecial,
                            useSeparateDepthStencilLayouts, true, 0.0f, 0.0f, false, true, colorEnabled));
                    }
                        ...
                        new tcu::TestCaseGroup(testCtx, "depth_test_disabled", "Test for disabled depth test"));
                    {
                        ...
                        depthTestDisabled->addChild(new DepthTest(
                            ...
                            depthFormats[formatNdx], depthOpsDepthTestDisabled, useSeparateDepthStencilLayouts,
