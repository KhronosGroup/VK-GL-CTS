                     ...
                     ++separateDepthStencilLayouts)
                {
                    ...
                    for (size_t topologyNdx = 0; topologyNdx < DE_LENGTH_OF_ARRAY(primitiveTopologies); topologyNdx++)
                    {
                        ...
                        for (size_t opsNdx = 0; opsNdx < DE_LENGTH_OF_ARRAY(depthOps); opsNdx++)
                        {
                            compareOpsTests->addChild(
                                new DepthTest(testCtx, topologyName + getCompareOpsName(depthOps[opsNdx]),
                                              getCompareOpsDescription(depthOps[opsNdx]), pipelineConstructionType,
                        ...
                        new tcu::TestCaseGroup(testCtx, "depth_test_disabled", "Test for disabled depth test"));
                    {
                        ...
                        depthTestDisabled->addChild(new DepthTest(
                            ...
                            useSeparateDepthStencilLayouts, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
                    ...
                    de::MovePtr<tcu::TestCaseGroup> hostVisibleTests(
                        new tcu::TestCaseGroup(testCtx, "host_visible", "Test for disabled depth test"));
                    {
                        const VkCompareOp hostVisibleOps[DepthTest::QUAD_COUNT] = {
                            VK_COMPARE_OP_NEVER, VK_COMPARE_OP_LESS, VK_COMPARE_OP_GREATER, VK_COMPARE_OP_ALWAYS};

                        hostVisibleTests->addChild(
                            new DepthTest(testCtx, "local_memory_depth_buffer", "Depth buffer placed in local memory",
                                          pipelineConstructionType, depthFormats[formatNdx], hostVisibleOps,
                                          useSeparateDepthStencilLayouts, VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
                                          false,        /* depthBoundsTestEnable */
                                          0.0f,         /* depthBoundMin*/
                                          1.0f,         /* depthBoundMax*/
        ...
        for (const auto &viewportCase : kViewportCases)
            for (const auto &format : depthFormats)
                for (const auto &compareOp : compareOps)
                {
                    ...
                    depthClipControlTests->addChild(
                        new DepthTest(testCtx, testName, "", pipelineConstructionType, format, ops, false,
                                      VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, false, 0.0f, 1.0f, true, false, true, false,
