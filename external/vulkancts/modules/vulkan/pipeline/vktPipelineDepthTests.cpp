...
enum class DepthClipControlCase
{
    ...
    BEFORE_TWO_DYNAMICS =
        4, // Set dynamic viewport state, bind dynamic pipeline with [0,1] view volume, then bind dynamic pipeline with [-1,1] view volume.
...
public:
    ...
    DepthTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
public:
    DepthTestInstance(Context &context, const PipelineConstructionType pipelineConstructionType,
                      ...
                      const bool separateDepthStencilLayouts, const bool depthBoundsTestEnable,
...
DepthTest::DepthTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                     const PipelineConstructionType pipelineConstructionType, const VkFormat depthFormat,
                     const VkCompareOp depthCompareOps[QUAD_COUNT], const bool separateDepthStencilLayouts,
...
TestInstance *DepthTest::createInstance(Context &context) const
{
    return new DepthTestInstance(context, m_pipelineConstructionType, m_depthFormat, m_depthCompareOps,
                                 m_separateDepthStencilLayouts, m_depthBoundsTestEnable, m_depthBoundsMin,
                                 m_depthBoundsMax, m_depthTestEnable, m_stencilTestEnable, m_colorAttachmentEnable,
                                 m_depthClipControl);
}
...
DepthTestInstance::DepthTestInstance(Context &context, const PipelineConstructionType pipelineConstructionType,
    ...
    , m_colorAttachmentEnable(colorAttachmentEnable)
    , m_depthClipControl(depthClipControl)
    , m_graphicsPipelines{{context.getDeviceInterface(), context.getDevice(), pipelineConstructionType},
                          {context.getDeviceInterface(), context.getDevice(), pipelineConstructionType},
                          {context.getDeviceInterface(), context.getDevice(), pipelineConstructionType},
                          {context.getDeviceInterface(), context.getDevice(), pipelineConstructionType}}
    , m_altGraphicsPipelines{{context.getDeviceInterface(), context.getDevice(), pipelineConstructionType},
                             {context.getDeviceInterface(), context.getDevice(), pipelineConstructionType},
                             {context.getDeviceInterface(), context.getDevice(), pipelineConstructionType},
                             {context.getDeviceInterface(), context.getDevice(), pipelineConstructionType}}
{
    ...
    // Create command buffer
    {
        ...
        beginRenderPass(vk, *m_cmdBuffer, *m_renderPass, *m_framebuffer,
                        makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), (uint32_t)attachmentClearValues.size(),
        ...
        for (int quadNdx = 0; quadNdx < DepthTest::QUAD_COUNT; quadNdx++)
        {
            ...
            if (useAltGraphicsPipelines)
                vk.cmdBindPipeline(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                   m_altGraphicsPipelines[quadNdx].getPipeline());
            vk.cmdBindPipeline(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
...
tcu::TestCaseGroup *createDepthTests(tcu::TestContext &testCtx, PipelineConstructionType pipelineConstructionType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> noColorAttachmentTests(
        new tcu::TestCaseGroup(testCtx, "nocolor", "Depth tests with no color attachment"));

    // Tests for format features
    if (pipelineConstructionType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
    {
        de::MovePtr<tcu::TestCaseGroup> formatFeaturesTests(
            new tcu::TestCaseGroup(testCtx, "format_features", "Checks depth format features"));

        // Formats that must be supported in all implementations
        addFunctionCase(formatFeaturesTests.get(), "support_d16_unorm",
                        "Tests if VK_FORMAT_D16_UNORM is supported as depth/stencil attachment format",
                        testSupportsDepthStencilFormat, VK_FORMAT_D16_UNORM);
        ...
        addFunctionCase(
            formatFeaturesTests.get(), "support_d24_unorm_or_d32_sfloat",
            "Tests if any of VK_FORMAT_D24_UNORM_X8 or VK_FORMAT_D32_SFLOAT are supported as depth/stencil attachment "
            "format",
        ...
        addFunctionCase(
            ...
            "Tests if any of VK_FORMAT_D24_UNORM_S8_UINT or VK_FORMAT_D32_SFLOAT_S8_UINT are supported as "
         ...
         colorAttachmentEnabledIdx++)
    {
        ...
        // Tests for format and compare operators
        {
            de::MovePtr<tcu::TestCaseGroup> formatTests(
                new tcu::TestCaseGroup(testCtx, "format", "Uses different depth formats"));
            ...
            for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(depthFormats); formatNdx++)
            {
                     ...
                     ++separateDepthStencilLayouts)
                {
                    ...
                    de::MovePtr<tcu::TestCaseGroup> formatTest(new tcu::TestCaseGroup(
                        testCtx,
                        (getFormatCaseName(depthFormats[formatNdx]) +
                         ((useSeparateDepthStencilLayouts) ? "_separate_layouts" : ""))
                            .c_str(),
                        (std::string("Uses format ") + getFormatName(depthFormats[formatNdx]) +
                         ((useSeparateDepthStencilLayouts) ? " with separate depth/stencil layouts" : ""))
                            .c_str()));
                    de::MovePtr<tcu::TestCaseGroup> compareOpsTests(
                        new tcu::TestCaseGroup(testCtx, "compare_ops", "Combines depth compare operators"));

                    for (size_t opsNdx = 0; opsNdx < DE_LENGTH_OF_ARRAY(depthOps); opsNdx++)
                    {
                        compareOpsTests->addChild(new DepthTest(testCtx, getCompareOpsName(depthOps[opsNdx]),
                                                                getCompareOpsDescription(depthOps[opsNdx]),
                                                                pipelineConstructionType, depthFormats[formatNdx],
                                                                depthOps[opsNdx], useSeparateDepthStencilLayouts));

                        compareOpsTests->addChild(new DepthTest(
                            testCtx, getCompareOpsName(depthOps[opsNdx]) + "_depth_bounds_test",
                            ...
                            useSeparateDepthStencilLayouts, true, 0.1f, 0.25f, true, false, colorEnabled));
                    }
                    // Special VkPipelineDepthStencilStateCreateInfo known to have issues
                    {
                        ...
                        compareOpsTests->addChild(
                            new DepthTest(testCtx, "never_zerodepthbounds_depthdisabled_stencilenabled",
                                          "special VkPipelineDepthStencilStateCreateInfo", pipelineConstructionType,
                                          depthFormats[formatNdx], depthOpsSpecial, useSeparateDepthStencilLayouts,
                                          true, 0.0f, 0.0f, false, true, colorEnabled));
                    }
                    ...
                    de::MovePtr<tcu::TestCaseGroup> depthTestDisabled(
                        new tcu::TestCaseGroup(testCtx, "depth_test_disabled", "Test for disabled depth test"));
                    {
                        ...
                        depthTestDisabled->addChild(new DepthTest(
                            testCtx, "depth_write_enabled", "Depth writes should not occur if depth test is disabled",
                            pipelineConstructionType, depthFormats[formatNdx], depthOpsDepthTestDisabled,
                            useSeparateDepthStencilLayouts, false, /* depthBoundsTestEnable */
                            0.0f,                                  /* depthBoundMin*/
                            1.0f,                                  /* depthBoundMax*/
...
#ifndef CTS_USES_VULKANSC
    de::MovePtr<tcu::TestCaseGroup> depthClipControlTests(
        new tcu::TestCaseGroup(testCtx, "depth_clip_control", "Depth tests with depth clip control enabled"));
    {
            ...
            for (const auto &format : depthFormats)
                for (const auto &compareOp : compareOps)
                {
                    ...
                    depthClipControlTests->addChild(new DepthTest(testCtx, testName, "", pipelineConstructionType,
                                                                  format, ops, false, false, 0.0f, 1.0f, true, false,
                                                                  true, viewportCase.viewportCase));
                }
    }
