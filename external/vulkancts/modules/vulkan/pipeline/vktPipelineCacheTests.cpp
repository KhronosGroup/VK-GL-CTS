...
{
public:
    CacheTestParam(const VkShaderStageFlagBits *shaders, uint32_t count, bool compileCacheMissShaders);
    ...
    {
        return m_compileCacheMissShaders;
    }
...
CacheTestParam::CacheTestParam(const VkShaderStageFlagBits *shaders, uint32_t count, bool compileCacheMissShaders)
...
{
public:
    CacheTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
{
public:
    GraphicsCacheTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                      const CacheTestParam *param)
...
void GraphicsCacheTest::initPrograms(SourceCollections &programCollection) const
{
    ...
    for (uint32_t shaderOpNdx = 0u; shaderOpNdx < SHADERS_CACHE_OP_LAST; shaderOpNdx++)
    {
        ...
        for (uint32_t shaderNdx = 0; shaderNdx < m_param.getShaderCount(); shaderNdx++)
        {
            ...
            case VK_SHADER_STAGE_GEOMETRY_BIT:
                programCollection.glslSources.add("dummy_geo" + missSuffix)
    ...
    , m_missPipelineBuilder(context)
{
    ...
    for (uint32_t shaderNdx = 0; shaderNdx < m_param->getShaderCount(); shaderNdx++)
    {
        ...
        case VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT:
            ...
            if (m_param->getCompileMissShaders())
            {
                m_missPipelineBuilder.bindShaderStage(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, "basic_tcs_miss",
                                                      "main");
        ...
        case VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT:
            ...
            if (m_param->getCompileMissShaders())
            {
                m_missPipelineBuilder.bindShaderStage(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, "basic_tes_miss",
                                                      "main");
                m_missPipelineBuilder.enableTessellationStage(3);
            }
            break;
    ...
    m_pipeline[PIPELINE_CACHE_NDX_NO_CACHE] =
        m_pipelineBuilder.buildPipeline(m_renderSize, *m_renderPass, *m_cache, *m_pipelineLayout);
...
tcu::TestStatus GraphicsCacheTestInstance::verifyTestResult(void)
{
    ...
    de::MovePtr<tcu::TextureLevel> resultNoCache =
        readColorAttachment(vk, vkDevice, queue, queueFamilyIndex, m_context.getDefaultAllocator(),
                            *m_colorImage[PIPELINE_CACHE_NDX_NO_CACHE], m_colorFormat, m_renderSize);
    de::MovePtr<tcu::TextureLevel> resultCache =
        readColorAttachment(vk, vkDevice, queue, queueFamilyIndex, m_context.getDefaultAllocator(),
                            *m_colorImage[PIPELINE_CACHE_NDX_CACHED], m_colorFormat, m_renderSize);
...
{
public:
    ComputeCacheTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                     const CacheTestParam *param)
        : CacheTest(testContext, name, description, param)
    {
    }
...
void ComputeCacheTestInstance::buildPipeline(uint32_t ndx)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice vkDevice   = m_context.getDevice();
...
tcu::TestStatus ComputeCacheTestInstance::verifyTestResult(void)
{
    ...
    // Compare the content
    uint8_t *bufNoCache = reinterpret_cast<uint8_t *>(m_outputBufferAlloc[PIPELINE_CACHE_NDX_NO_CACHE]->getHostPtr());
    uint8_t *bufCached  = reinterpret_cast<uint8_t *>(m_outputBufferAlloc[PIPELINE_CACHE_NDX_CACHED]->getHostPtr());
...
{
public:
    PipelineFromCacheTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                          const CacheTestParam *param);
...
PipelineFromCacheTest::PipelineFromCacheTest(tcu::TestContext &testContext, const std::string &name,
                                             const std::string &description, const CacheTestParam *param)
    ...
    , m_data(DE_NULL)
{
    ...
    m_pipeline[PIPELINE_CACHE_NDX_CACHED] =
        m_pipelineBuilder.buildPipeline(m_renderSize, *m_renderPass, *m_newCache, *m_pipelineLayout);
}
...
{
public:
    PipelineFromIncompleteCacheTest(tcu::TestContext &testContext, const std::string &name,
                                    const std::string &description, const CacheTestParam *param);
...
PipelineFromIncompleteCacheTest::PipelineFromIncompleteCacheTest(tcu::TestContext &testContext, const std::string &name,
...
{
public:
    MergeCacheTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                   const CacheTestParam *param, const MergeCacheTestParam *mergeCacheParam)
        : GraphicsCacheTest(testContext, name, description, param)
...
{
public:
    MergeCacheTestInstance(Context &context, const CacheTestParam *param, const MergeCacheTestParam *mergeCacheParam);
    ...
    : GraphicsCacheTestInstance(context, param)
{
    ...
    m_pipeline[PIPELINE_CACHE_NDX_CACHED] =
        m_pipelineBuilder.buildPipeline(m_renderSize, *m_renderPass, *m_cacheMerged, *m_pipelineLayout);
}

Move<VkPipelineCache> MergeCacheTestInstance::createPipelineCache(const DeviceInterface &vk, VkDevice device,
                                                                  MergeCacheType type)
{
...
{
public:
    CacheHeaderTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                    const CacheTestParam *param)
        : GraphicsCacheTest(testContext, name, description, param)
    {
    }
...
{
public:
    InvalidSizeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                    const CacheTestParam *param);
...
InvalidSizeTest::InvalidSizeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                                 const CacheTestParam *param)
    : GraphicsCacheTest(testContext, name, description, param)
{
}
...
{
public:
    ZeroSizeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                 const CacheTestParam *param);
...
ZeroSizeTest::ZeroSizeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                           const CacheTestParam *param)
    : GraphicsCacheTest(testContext, name, description, param)
{
}
...
{
public:
    InvalidBlobTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                    const CacheTestParam *param);
...
InvalidBlobTest::InvalidBlobTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                                 const CacheTestParam *param)
    : GraphicsCacheTest(testContext, name, description, param)
{
}
...
tcu::TestCaseGroup *createCacheTests(tcu::TestContext &testCtx)
{
    ...
    // Graphics Pipeline Tests
    {
        de::MovePtr<tcu::TestCaseGroup> graphicsTests(
            new tcu::TestCaseGroup(testCtx, "graphics_tests", "Test pipeline cache with graphics pipeline."));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> graphicsTests(
            new tcu::TestCaseGroup(testCtx, "pipeline_from_get_data", "Test pipeline cache with graphics pipeline."));
    ...
    // Graphics Pipeline Tests
    {
        de::MovePtr<tcu::TestCaseGroup> graphicsTests(new tcu::TestCaseGroup(
            testCtx, "pipeline_from_incomplete_get_data", "Test pipeline cache with graphics pipeline."));
    ...
    // Compute Pipeline Tests
    {
        de::MovePtr<tcu::TestCaseGroup> computeTests(
            new tcu::TestCaseGroup(testCtx, "compute_tests", "Test pipeline cache with compute pipeline."));
    ...
    // Merge cache Tests
    {
        ...
        for (uint32_t i = 0; i < DE_LENGTH_OF_ARRAY(testParams); i++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> mergeStagesTests(new tcu::TestCaseGroup(
                testCtx, testParams[i].generateTestName().c_str(), testParams[i].generateTestDescription().c_str()));

            for (uint32_t destTypeIdx = 0u; destTypeIdx <= MERGE_CACHE_TYPE_LAST; destTypeIdx++)
                for (uint32_t srcType1Idx = 0u; srcType1Idx <= MERGE_CACHE_TYPE_LAST; srcType1Idx++)
                {
                    ...
                    {
                        std::string testName = "src_" + getMergeCacheTypesStr(cacheTestParam.srcCacheTypes) + "_dst_" +
                                               getMergeCacheTypeStr(cacheTestParam.destCacheType);
                        mergeStagesTests->addChild(new MergeCacheTest(
                            testCtx, testName.c_str(), "Merge the caches test.", &testParams[i], &cacheTestParam));
                    }

                    // merge with two caches
                    for (uint32_t srcType2Idx = 0u; srcType2Idx <= MERGE_CACHE_TYPE_LAST; srcType2Idx++)
                    {
                        ...
                        std::string testName = "src_" + getMergeCacheTypesStr(cacheTestParamTwoCaches.srcCacheTypes) +
                                               "_dst_" + getMergeCacheTypeStr(cacheTestParamTwoCaches.destCacheType);
                        mergeStagesTests->addChild(new MergeCacheTest(testCtx, testName.c_str(),
                                                                      "Merge the caches test.", &testParams[i],
    ...
    // Misc Tests
    {
        ...
        miscTests->addChild(new CacheHeaderTest(testCtx, "cache_header_test", "Cache header test.", &testParam));

        miscTests->addChild(new InvalidSizeTest(testCtx, "invalid_size_test", "Invalid size test.", &testParam));

        miscTests->addChild(new ZeroSizeTest(testCtx, "zero_size_test", "Zero size test.", &testParam));

        miscTests->addChild(new InvalidBlobTest(testCtx, "invalid_blob_test", "Invalid cache blob test.", &testParam));
