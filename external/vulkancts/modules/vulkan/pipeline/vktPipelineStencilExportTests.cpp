...
void preparePipelineWrapper(GraphicsPipelineWrapper &gpw, const VkPipelineLayout pipelineLayout,
                            const VkRenderPass renderPass, const uint32_t subpass, const VkShaderModule vertexModule,
    ...
    // The second pass will use the stencil with a compare operation EQUAL with reference value 0.
    {
        ...
        for (int stencilModeNdx = 0; stencilModeNdx < stencilModeCount; stencilModeNdx++)
        {
            ...
            Move<VkShaderModule> fragmentStencilModule =
                createShaderModule(vk, device, context.getBinaryCollection().get(shaderName.str()), 0);
            GraphicsPipelineWrapper stencilPipeline(vk, device, params.pipelineConstructionType);

            preparePipelineWrapper(stencilPipeline, *pipelineLayout, *renderPass, 0, *vertexModule,
                                   *fragmentStencilModule, renderSize, false);
            ...
            if (params.early_and_late)
            {
                ...
                case MODE_STENCIL_REF_GREATER_BACK_AMD:
                    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                                    makeRect2D(0, 0, renderSize.x(), renderSize.y()), clearColor, 0.0, 1u); //0
                    break;
                case MODE_STENCIL_REF_LESS_FRONT_AMD:
                case MODE_STENCIL_REF_LESS_BACK_AMD:
                    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                                    makeRect2D(0, 0, renderSize.x(), renderSize.y()), clearColor, 0.0, 1u); //10
                    break;
                default:
                    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                                    makeRect2D(0, 0, renderSize.x(), renderSize.y()), clearColor, 0.0, 1u);
                    break;
                }
            }
            else
            {
                beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                                makeRect2D(0, 0, renderSize.x(), renderSize.y()), clearColor, 0.0, 0u);
            }
