...
Move<VkRenderPass> makeTestRenderPass(const DeviceInterface &vk, const VkDevice device, const VkFormat colorFormat,
...
Move<VkPipeline> makeGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                      ...
                                      const uint32_t subpass, const VkShaderModule vertexModule,
                                      const VkShaderModule fragmentModule, const UVec2 renderSize, const bool useColor)
{
    ...
    const VkStencilOpState stencilOpState =
        makeStencilOpState(useColor ? VK_STENCIL_OP_KEEP : VK_STENCIL_OP_REPLACE, // stencil fail
...
tcu::TestStatus testStencilExportReplace(Context &context, vk::VkFormat stencilFormat)
{
    ...
    // The second pass will use the stencil with a compare operation EQUAL with reference value 0.
    {
        ...
        Move<VkImage> stencilImage = makeImage(
            vk, device, makeImageCreateInfo(stencilFormat, renderSize, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT));
        MovePtr<Allocation> stencilImageAlloc = bindImage(vk, device, allocator, *stencilImage, MemoryRequirement::Any);
        Move<VkImageView> stencilAttachment =
        ...
        Move<VkShaderModule> vertexModule =
            createShaderModule(vk, device, context.getBinaryCollection().get("vert"), 0);
        Move<VkShaderModule> fragmentStencilModule =
            createShaderModule(vk, device, context.getBinaryCollection().get("frag-stencil"), 0);
        Move<VkShaderModule> fragmentColorModule =
            createShaderModule(vk, device, context.getBinaryCollection().get("frag-color"), 0);
        ...
        Move<VkPipeline> stencilPipeline      = makeGraphicsPipeline(
            vk, device, *pipelineLayout, *renderPass, 0, *vertexModule, *fragmentStencilModule, renderSize, false);
        Move<VkPipeline> colorPipeline = makeGraphicsPipeline(vk, device, *pipelineLayout, *renderPass, 1,
                                                              *vertexModule, *fragmentColorModule, renderSize, true);
        ...
        Move<VkFramebuffer> framebuffer =
            makeFramebuffer(vk, device, *renderPass, 2u, &attachments[0], renderSize.x(), renderSize.y());
...
tcu::TestCaseGroup *createStencilExportTests(tcu::TestContext &testCtx)
{
    ...
    for (int fmtIdx = 0; fmtIdx < DE_LENGTH_OF_ARRAY(kFormats); ++fmtIdx)
    {
        ...
        addFunctionCaseWithPrograms<vk::VkFormat>(formatGroup.get(), "op_replace", "", checkSupport, initPrograms,
                                                  testStencilExportReplace, kFormats[fmtIdx].format);
