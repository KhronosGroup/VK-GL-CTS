                                                     ...
                                                     VkImage image, VkBuffer outputBuffer)
{
    ...
    const Unique<VkShaderModule> vertShader(
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert"), 0u));
    const Unique<VkShaderModule> fragShader(
    ...
    const Move<VkFramebuffer> framebuffer =
        makeFramebuffer(vk, device, *renderPass, 0u, DE_NULL, testMipLevelSize.x(), testMipLevelSize.y());
...
tcu::TestStatus Image2DView3DImageInstance::iterate(void)
{
    ...
    else
    {
        ...
        clearColorImage(vk, device, queue, m_context.getUniversalQueueFamilyIndex(), testImage.get(),
                        tcu::Vec4(0, 0, 0, 1), VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, pipelineStage, 0u,
                        1u, 0u, mipLevelCount);
    }
    ...
    if (m_testParameters.testType == Compute)
        runComputePipeline(*descriptorSet, *descriptorSetLayout, testMipLevelSize, *cmdBuffer,
                           useSampler ? **resultImage : *testImage, *outputBuffer);
    else
...
{
public:
    ComputeImage2DView3DImageTest(tcu::TestContext &testContext, const char *name, const char *description,
...
{
public:
    FragmentImage2DView3DImageTest(tcu::TestContext &testContext, const char *name, const char *description,
                                   const TestParameters &testParameters)
        : vkt::TestCase(testContext, name, description)
        , m_testParameters(testParameters)
    {
...
tcu::TestCaseGroup *createImage2DViewOf3DTests(tcu::TestContext &testCtx,
                                               PipelineConstructionType pipelineConstructionType)
{
    de::MovePtr<tcu::TestCaseGroup> imageTests(
        new tcu::TestCaseGroup(testCtx, "image_2d_view_3d_image", "2D view 3D image tests"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(new tcu::TestCaseGroup(testCtx, "compute", "Compute shader tests."));
    de::MovePtr<tcu::TestCaseGroup> fragmentGroup(
        new tcu::TestCaseGroup(testCtx, "fragment", "Fragment shader tests."));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> computeSubGroup(
            new tcu::TestCaseGroup(testCtx, imageAccessType.name.c_str(), "Fragment shader tests."));
        de::MovePtr<tcu::TestCaseGroup> fragmentSubGroup(
            new tcu::TestCaseGroup(testCtx, imageAccessType.name.c_str(), "Fragment shader tests."));
        for (uint32_t mipLevel = 0; mipLevel < 3; mipLevel += 2)
        {
            ...
            for (const auto &layer : layers)
            {
                ...
                fragmentSubGroup->addChild(
                    new FragmentImage2DView3DImageTest(testCtx, testName.c_str(), "description", testParameters));

                if (pipelineConstructionType == PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
                {
                    testParameters.testType = Compute;
                    computeSubGroup->addChild(
                        new ComputeImage2DView3DImageTest(testCtx, testName.c_str(), "description", testParameters));
                }
            }
        }
