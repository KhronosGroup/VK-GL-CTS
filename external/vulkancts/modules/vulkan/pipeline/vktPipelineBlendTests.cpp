...
public:
    ...
    BlendTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
public:
    ...
    DualSourceBlendTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                        const VkFormat colorFormat, const VkPipelineColorBlendAttachmentState blendStates[QUAD_COUNT]);
...
{
public:
    BlendTestInstance(Context &context, const VkFormat colorFormat,
...
{
public:
    DualSourceBlendTestInstance(Context &context, const VkFormat colorFormat,
...
BlendTest::BlendTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                     const VkFormat colorFormat, const VkPipelineColorBlendAttachmentState blendStates[QUAD_COUNT])
...
DualSourceBlendTest::DualSourceBlendTest(tcu::TestContext &testContext, const std::string &name,
                                         const std::string &description, const VkFormat colorFormat,
...
BlendTestInstance::BlendTestInstance(Context &context, const VkFormat colorFormat,
                                     const VkPipelineColorBlendAttachmentState blendStates[BlendTest::QUAD_COUNT])
...
DualSourceBlendTestInstance::DualSourceBlendTestInstance(
    Context &context, const VkFormat colorFormat,
    const VkPipelineColorBlendAttachmentState blendStates[DualSourceBlendTest::QUAD_COUNT])
    : vkt::TestInstance(context)
    , m_renderSize(32, 32)
    , m_colorFormat(colorFormat)
...
{
public:
    ClampTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
ClampTest::ClampTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                     const ClampTestParams &testParams)
    : vkt::TestCase(testContext, name, description)
    , m_params(testParams)
{
...
tcu::TestCaseGroup *createBlendTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> formatTests(
        new tcu::TestCaseGroup(testCtx, "format", "Uses different blend formats"));
    de::MovePtr<tcu::TestCaseGroup> clampTests(
        new tcu::TestCaseGroup(testCtx, "clamp", "Verifies clamping for normalized formats"));
    de::MovePtr<tcu::TestCaseGroup> dualSourceBlendTests(
        new tcu::TestCaseGroup(testCtx, "dual_source", "Blend tests taking into account dual-source blend factors"));
    de::MovePtr<tcu::TestCaseGroup> dualSourceFormatTests(
        new tcu::TestCaseGroup(testCtx, "format", "Uses different blend formats"));
    ...
    for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(blendFormats); formatNdx++)
    {
        ...
        // Blend tests
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> blendStateTests;
            {
                ...
                blendStateTests = de::MovePtr<tcu::TestCaseGroup>(
                    new tcu::TestCaseGroup(testCtx, "states", blendStateDescription.str().c_str()));
            }
            ...
            while (blendStateItr.hasNext())
            {
                ...
                blendStateTests->addChild(new BlendTest(testCtx, getBlendStateSetName(quadBlendConfigs),
                                                        getBlendStateSetDescription(quadBlendConfigs), format,
                                                        quadBlendConfigs));
            }
    ...
    for (int formatIdx = 0; formatIdx < DE_LENGTH_OF_ARRAY(clampFormats); ++formatIdx)
    {
        ...
        clampTests->addChild(new ClampTest(testCtx, getFormatCaseName(format),
                                           std::string("Using format ") + getFormatName(format), testParams));
    }
