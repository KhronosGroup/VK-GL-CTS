...
tcu::TestStatus WriteSampleMaskTestInstance::iterate(void)
{
    ...
    const auto firstSubpassPipeline = vk::makeGraphicsPipeline(
        vkd, device, emptyPipelineLayout.get(), vertModule.get(), DE_NULL, DE_NULL, DE_NULL, writeModule.get(),
    ...
    const auto secondSubpassPipeline = vk::makeGraphicsPipeline(
        vkd, device, checkPipelineLayout.get(), vertModule.get(), DE_NULL, DE_NULL, DE_NULL, checkModule.get(),
        renderPass.get(), 1u, &vertexInputInfo, &inputAssemblyInfo, nullptr, &viewportInfo, &rasterizationInfo,
        &multisampleInfo, &depthStencilInfo, &colorBlendInfo, &dynamicStateInfo);
