...
{
public:
    WriteSampleMaskTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
WriteSampleMaskTestCase::WriteSampleMaskTestCase(tcu::TestContext &testCtx, const std::string &name,
                                                 const std::string &description, const WriteSampleMaskParams &params)
...
tcu::TestStatus WriteSampleMaskTestInstance::iterate(void)
{
    ...
    const auto framebuffer =
        vk::makeFramebuffer(vkd, device, renderPass.get(), static_cast<uint32_t>(imageViews.size()), imageViews.data(),
                            kImageExtent.width, kImageExtent.height);
    ...
    vk::beginRenderPass(vkd, cmdBuffer, renderPass.get(), framebuffer.get(), vk::makeRect2D(kImageExtent),
                        WriteSampleMaskTestCase::kClearColor);
                                                        ...
                                                        vk::PipelineConstructionType pipelineConstructionType)
{
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> writeSampleMaskGroup(
            new tcu::TestCaseGroup(testCtx, "write_sample_mask", "Test writes to SampleMask variable"));
        ...
        for (auto count : multisample::kValidSquareSampleCounts)
        {
            multisample::WriteSampleMaskParams params{pipelineConstructionType,
                                                      static_cast<vk::VkSampleCountFlagBits>(count)};
            const auto countStr = de::toString(count);
            writeSampleMaskGroup->addChild(new multisample::WriteSampleMaskTestCase(
                testCtx, countStr + "_samples", "Test image with " + countStr + " samples", params));
        }
