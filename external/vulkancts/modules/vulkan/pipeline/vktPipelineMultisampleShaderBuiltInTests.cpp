                                                                        ...
                                                                        const VkDescriptorSetLayout *descSetLayout)
{
    ...
    m_buffer = de::MovePtr<BufferWithMemory>(
        new BufferWithMemory(deviceInterface, device, allocator, bufferSampleMaskInfo, MemoryRequirement::HostVisible));
...
void MSCase<MSCaseSampleMaskBitCount>::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    const int minInvocations =
        static_cast<int>(std::ceil(static_cast<float>(m_imageMSParams.numSamples) * m_imageMSParams.shadingRate));
...
const set<uint32_t> kValidSquareSampleCounts = {
...
void assertSquareSampleCount(uint32_t sampleCount)
{
...
uint32_t bitsPerCoord(uint32_t numSamples)
{
...
vk::VkExtent2D imageSize(uint32_t sampleCount)
{
...
vk::VkExtent3D getExtent3D(uint32_t sampleCount)
{
...
public:
    ...
    static uint32_t getBufferElems(uint32_t sampleCount);
...
WriteSampleMaskTestCase::WriteSampleMaskTestCase(tcu::TestContext &testCtx, const std::string &name,
                                                 const WriteSampleMaskParams &params)
    : vkt::TestCase(testCtx, name)
    , m_params(params)
{
}
...
void WriteSampleMaskTestCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    fragShaderWrite << fragHeaderStr << "void main()\n"
                    << "{\n"
    ...
    fragShaderCheck << fragHeaderStr << "layout(set=0, binding=0, input_attachment_index=0) uniform subpassInput"
                    << (isMultiSample ? "MS" : "") << " inputAttachment;\n"
                    ...
                    << "    uvec2 ucoords          = uvec2(gl_FragCoord);\n"
                    << "    ivec2 icoords          = ivec2(ucoords);\n"
                    << "    uint  bufferp          = ((ucoords.y * " << size.width << " + ucoords.x) * "
                    << m_params.sampleCount << ") + uint(gl_SampleID);\n"
                    << "    vec4  storedc          = subpassLoad(inputAttachment"
                    << (isMultiSample ? ", gl_SampleID" : "") << ");\n"
...
uint32_t WriteSampleMaskTestCase::getBufferElems(uint32_t sampleCount)
{
...
tcu::TestStatus WriteSampleMaskTestInstance::iterate(void)
{
    ...
    const auto kBufferSize             = static_cast<vk::VkDeviceSize>(kBufferElems * sizeof(int32_t));
    ...
    const vk::ImageWithMemory auxiliarImage(vkd, device, alloc, imageCreateInfo,
                                            vk::MemoryRequirement::Any); // For the second subpass.
    ...
    const auto colorImageView =
        vk::makeImageView(vkd, device, colorImage.get(), vk::VK_IMAGE_VIEW_TYPE_2D, kImageFormat, subresourceRange);
    const auto auxiliarImageView =
        vk::makeImageView(vkd, device, auxiliarImage.get(), vk::VK_IMAGE_VIEW_TYPE_2D, kImageFormat, subresourceRange);

    // Create storage buffer used to verify results.
    const vk::BufferWithMemory storageBuffer(
        vkd, device, alloc, vk::makeBufferCreateInfo(kBufferSize, vk::VK_BUFFER_USAGE_STORAGE_BUFFER_BIT),
    ...
    const auto vertexBufferSize =
        static_cast<vk::VkDeviceSize>(quadVertices.size() * sizeof(decltype(quadVertices)::value_type));
    const vk::BufferWithMemory vertexBuffer(
        vkd, device, alloc, vk::makeBufferCreateInfo(vertexBufferSize, vk::VK_BUFFER_USAGE_VERTEX_BUFFER_BIT),
        vk::MemoryRequirement::HostVisible);
    ...
    renderPass.createFramebuffer(vkd, device, static_cast<uint32_t>(imageViews.size()), images.data(),
                                 imageViews.data(), kImageExtent.width, kImageExtent.height);
    ...
    const PipelineLayoutWrapper checkPipelineLayout(m_params.pipelineConstructionType, vkd, device,
    ...
    const std::vector<vk::VkVertexInputBindingDescription> vertexBindings = {
        {
            0u,                                                                // uint32_t binding;
            static_cast<uint32_t>(sizeof(decltype(quadVertices)::value_type)), // uint32_t stride;
    ...
    const auto stencilState =
        vk::makeStencilOpState(vk::VK_STENCIL_OP_KEEP, vk::VK_STENCIL_OP_KEEP, vk::VK_STENCIL_OP_KEEP,
    ...
    vk::GraphicsPipelineWrapper firstSubpassPipeline(vki, vkd, physicalDevice, device, m_context.getDeviceExtensions(),
                                                     m_params.pipelineConstructionType);
    firstSubpassPipeline.setDynamicState(&dynamicStateInfo)
        ...
        .setupFragmentShaderState(emptyPipelineLayout, *renderPass, 0u, writeModule, &depthStencilInfo,
                                  &multisampleInfo)
    ...
    vk::GraphicsPipelineWrapper secondSubpassPipeline(vki, vkd, physicalDevice, device, m_context.getDeviceExtensions(),
                                                      m_params.pipelineConstructionType);
    secondSubpassPipeline.setDynamicState(&dynamicStateInfo)
        ...
        .setupFragmentShaderState(checkPipelineLayout, *renderPass, 1u, checkModule, &depthStencilInfo,
                                  &multisampleInfo)
    ...
    const auto bufferBarrier = vk::makeBufferMemoryBarrier(vk::VK_ACCESS_SHADER_WRITE_BIT, vk::VK_ACCESS_HOST_READ_BIT,
                                                           storageBuffer.get(), 0ull, VK_WHOLE_SIZE);
    ...
    vkd.cmdDraw(cmdBuffer, static_cast<uint32_t>(quadVertices.size()), 1u, 0u, 0u);

    renderPass.nextSubpass(vkd, cmdBuffer, vk::VK_SUBPASS_CONTENTS_INLINE);
    secondSubpassPipeline.bind(cmdBuffer);
    vkd.cmdBindDescriptorSets(cmdBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, checkPipelineLayout.get(), 0u, 1u,
                              &descriptorSet.get(), 0u, nullptr);
    vkd.cmdDraw(cmdBuffer, static_cast<uint32_t>(quadVertices.size()), 1u, 0u, 0u);

    renderPass.end(vkd, cmdBuffer);
    vkd.cmdPipelineBarrier(cmdBuffer, vk::VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, vk::VK_PIPELINE_STAGE_HOST_BIT, 0u, 0u,
                           nullptr, 1u, &bufferBarrier, 0u, nullptr);
    vk::endCommandBuffer(vkd, cmdBuffer);
    ...
    std::vector<int32_t> bufferContents(kBufferElems, 0);

    vk::invalidateAlloc(vkd, device, bufferAlloc);
    deMemcpy(bufferContents.data(), bufferPtr, static_cast<size_t>(kBufferSize));

    const auto sampleCount = static_cast<uint32_t>(m_params.sampleCount);
    const auto bpc         = bitsPerCoord(sampleCount);

    for (uint32_t x = 0; x < kImageExtent.width; ++x)
        for (uint32_t y = 0; y < kImageExtent.height; ++y)
        {
            // Samples on which we expect writes.
            const uint32_t sampleMask = ((x << bpc) | y);

            // Starting location for the pixel sample values in the buffer.
            const uint32_t pixelOffset = (y * kImageExtent.width + x) * sampleCount;

            for (uint32_t s = 0; s < sampleCount; ++s)
            {
                const uint32_t sampleIndex = pixelOffset + s;
                const int32_t &value       = bufferContents[sampleIndex];

                if (value != 0 && value != 1)
                {
                    // Garbage!
                    std::ostringstream msg;
                    msg << "Found garbage value " << value << " in buffer position " << sampleIndex << " (x=" << x
                        << ", y=" << y << ", sample=" << s << ")";
                    return tcu::TestStatus::fail(msg.str());
                }

                const int32_t expected = (((sampleMask & (1u << s)) != 0u) ? 1 : 0);
                if (value != expected)
                {
                    std::ostringstream msg;
                    msg << "Read " << value << " while expecting " << expected << " in buffer position " << sampleIndex
                        << " (x=" << x << ", y=" << y << ", sample=" << s << ")";
                    return tcu::TestStatus::fail(msg.str());
                }
            }
                                                        ...
                                                        vk::PipelineConstructionType pipelineConstructionType)
{
    ...
    testGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleID>>(
        testCtx, "sample_id", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetFull,
        samplesSetFullCount));
    ...
    samplePositionGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSamplePosDistribution>>(
        testCtx, "distribution", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetFull,
        samplesSetFullCount));
    samplePositionGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSamplePosCorrectness>>(
        testCtx, "correctness", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetFull,
        samplesSetFullCount));
    ...
    sampleMaskGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleMaskPattern>>(
        testCtx, "pattern", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetReduced,
        samplesSetReducedCount));
    sampleMaskGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleMaskBitCount>>(
        testCtx, "bit_count", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetReduced,
        samplesSetReducedCount));
    sampleMaskGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleMaskBitCount>>(
        testCtx, "bit_count_0_5", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetReduced,
        samplesSetReducedCount, multisample::ComponentData{}, 0.5f));
    sampleMaskGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleMaskCorrectBit>>(
        testCtx, "correct_bit", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetReduced,
        samplesSetReducedCount));
    sampleMaskGroup->addChild(makeMSGroup<multisample::MSCase<multisample::MSCaseSampleMaskWrite>>(
        testCtx, "write", pipelineConstructionType, imageSizes, sizesElemCount, samplesSetReduced,
        samplesSetReducedCount));
    ...
    if (pipelineConstructionType == vk::PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC)
    {
        ...
        for (auto count : multisample::kValidSquareSampleCounts)
        {
            ...
            multisample::WriteSampleParams params{pipelineConstructionType,
                                                  static_cast<vk::VkSampleCountFlagBits>(count)};
    ...
    // Write to gl_SampleMask from the fragment shader.
    {
        ...
        for (auto count : multisample::kValidSquareSampleCounts)
        {
            multisample::WriteSampleMaskParams params{pipelineConstructionType,
                                                      static_cast<vk::VkSampleCountFlagBits>(count)};
            const auto countStr = de::toString(count);
            writeSampleMaskGroup->addChild(
                new multisample::WriteSampleMaskTestCase(testCtx, countStr + "_samples", params));
        }
