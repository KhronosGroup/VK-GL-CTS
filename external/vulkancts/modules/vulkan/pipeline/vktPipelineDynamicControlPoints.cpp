...
{
public:
    DynamicControlPointsTestCase(tcu::TestContext &context, const std::string &name, const std::string &description,
                                 TestConfig config);
...
DynamicControlPointsTestCase::DynamicControlPointsTestCase(tcu::TestContext &context, const std::string &name,
                                                           const std::string &description, TestConfig config)
...
tcu::TestStatus DynamicControlPointsTestInstance::iterate(void)
{
    ...
    auto framebuffer = makeFramebuffer(vkd, device, renderPass.get(), colorAttachmentView.get(), imageExtent.width,
                                       imageExtent.height);
    ...
    const std::vector<vk::VkRect2D> scissors_left{
        vk::makeRect2D(0.0f, 0.0f, imageExtent.width / 2, imageExtent.height)};
    const std::vector<vk::VkRect2D> scissors_right{
        vk::makeRect2D(imageExtent.width / 2, 0.0, imageExtent.width / 2, imageExtent.height)};
                                                   ...
                                                   vk::PipelineConstructionType pipelineConstructionType)
{
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
        testCtx, "dynamic_control_points", "Tests checking dynamic bind points and switching pipelines"));

    group->addChild(new DynamicControlPointsTestCase(
        testCtx, "change_output",
    ...
    group->addChild(new DynamicControlPointsTestCase(
        testCtx, "change_winding", "test switching pipelines with dynamic control points while switching winding",
