...
{
public:
    ImageTest(tcu::TestContext &testContext, const char *name, const char *description, AllocationKind allocationKind,
              ...
              int arraySize, bool pipelineProtectedAccess, bool pipelineProtectedFlag);
...
ImageTest::ImageTest(tcu::TestContext &testContext, const char *name, const char *description,
                     AllocationKind allocationKind, PipelineConstructionType pipelineConstructionType,
                     VkDescriptorType samplingType, VkImageViewType imageViewType, VkFormat imageFormat,
                     const tcu::IVec3 &imageSize, int imageCount, int arraySize, bool pipelineProtectedAccess,
...
std::string getSizeName(VkImageViewType viewType, const tcu::IVec3 &size, int arraySize, bool pipelineProtectedAccess,
                        bool pipelineProtectedFlag)
{
                                                     ...
                                                     VkFormat imageFormat, int imageCount)
{
    ...
    for (size_t protectedNdx = 0; protectedNdx < DE_LENGTH_OF_ARRAY(pipelineProtectedAccess); ++protectedNdx)
    {
        ...
        {
            if (!pipelineProtectedAccess[protectedNdx] && pipelineProtectedFlag[flagNdx])
                continue;
            ...
            for (size_t sizeNdx = 0; sizeNdx < imageSizes.size(); sizeNdx++)
            {
                ...
                {
                    imageSizeTests->addChild(
                        new ImageTest(testCtx,
                                      getSizeName(imageViewType, imageSizes[sizeNdx], arraySizes[arraySizeNdx],
                                                  pipelineProtectedAccess[protectedNdx], pipelineProtectedFlag[flagNdx])
                                          .c_str(),
                                      "", allocationKind, pipelineConstructionType, samplingType, imageViewType,
                                      imageFormat, imageSizes[sizeNdx], imageCount, arraySizes[arraySizeNdx],
                                      pipelineProtectedAccess[protectedNdx], pipelineProtectedFlag[flagNdx]));
                }
            }
        }
