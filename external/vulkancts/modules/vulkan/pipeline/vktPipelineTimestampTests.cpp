...
TwoCmdBuffersTestParam::TwoCmdBuffersTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount,
                                               ...
                                               const VkQueryResultFlags queryPoolResultFlags)
    : TimestampTestParam(stages, stageCount, inRenderPass, hostQueryReset, queryPoolResultFlags)
...
tcu::TestStatus TimestampTestInstance::iterate(void)
{
    ...
    const uint32_t queryDataSize = sizeof(uint64_t) * (availabilityBit ? 2u : 1u);
    ...
    vk.getQueryPoolResults(vkDevice, *m_queryPool, 0u, stageSize, queryDataSize * stageSize, (void *)m_timestampValues,
                           queryDataSize, m_queryResultFlags);
...
tcu::TestCaseGroup *createTimestampTests(tcu::TestContext &testCtx)
{
    ...
    // Misc Tests
    {
        ...
        for (uint32_t flagsIdx = 0u; flagsIdx < DE_LENGTH_OF_ARRAY(queryResultFlagsMiscTests); flagsIdx++)
        {
            ...
            miscTests->addChild(new TimestampTest(testCtx, "timestamp_only" + queryResultsFlagsMiscTestsStr[flagsIdx],
                                                  "Only write timestamp command in the commmand buffer", &param));

            TwoCmdBuffersTestParam twoCmdBuffersParamPrimary(
                miscStages, 1u, false, false, VK_COMMAND_BUFFER_LEVEL_PRIMARY, queryResultFlagsMiscTests[flagsIdx]);
            miscTests->addChild(
            ...
            TwoCmdBuffersTestParam twoCmdBuffersParamSecondary(
                miscStages, 1u, false, false, VK_COMMAND_BUFFER_LEVEL_SECONDARY, queryResultFlagsMiscTests[flagsIdx]);
            miscTests->addChild(new TwoCmdBuffersTest(
                testCtx, "two_cmd_buffers_secondary" + queryResultsFlagsMiscTestsStr[flagsIdx],
            ...
            param.toggleHostQueryReset();
            miscTests->addChild(
                new TimestampTest(testCtx, "timestamp_only_host_query_reset" + queryResultsFlagsMiscTestsStr[flagsIdx],
                                  "Only write timestamp command in the commmand buffer", &param));
            TwoCmdBuffersTestParam twoCmdBuffersParamPrimaryHostQueryReset(
                miscStages, 1u, false, true, VK_COMMAND_BUFFER_LEVEL_PRIMARY, queryResultFlagsMiscTests[flagsIdx]);
            miscTests->addChild(new TwoCmdBuffersTest(
            ...
            TwoCmdBuffersTestParam twoCmdBuffersParamSecondaryHostQueryReset(
                miscStages, 1u, false, true, VK_COMMAND_BUFFER_LEVEL_SECONDARY, queryResultFlagsMiscTests[flagsIdx]);
            miscTests->addChild(new TwoCmdBuffersTest(
