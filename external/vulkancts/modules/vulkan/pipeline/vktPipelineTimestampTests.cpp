...
uint64_t checkTimestampsSupported(Context &context)
{
...
{
public:
    TimestampTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount, const bool inRenderPass,
...
TimestampTestParam::TimestampTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount,
...
{
public:
    TransferTimestampTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount,
                               const bool inRenderPass, const bool hostQueryReset, const uint32_t methodNdx);
...
TransferTimestampTestParam::TransferTimestampTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount,
                                                       const bool inRenderPass, const bool hostQueryReset,
                                                       const uint32_t methodNdx)
...
{
public:
    TwoCmdBuffersTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount, const bool inRenderPass,
                           const bool hostQueryReset, const VkCommandBufferLevel cmdBufferLevel);
...
TwoCmdBuffersTestParam::TwoCmdBuffersTestParam(const VkPipelineStageFlagBits *stages, const uint32_t stageCount,
                                               const bool inRenderPass, const bool hostQueryReset,
...
public:
    ...
    void bindShaderStage(VkShaderStageFlagBits stage, const char *source_name);

    void enableTessellationStage(uint32_t patchControlPoints);

    Move<VkPipeline> buildPipeline(tcu::UVec2 renderSize, VkRenderPass renderPass);
...
void SimpleGraphicsPipelineBuilder::bindShaderStage(VkShaderStageFlagBits stage, const char *source_name)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice vkDevice   = m_context.getDevice();

    // Create shader module
    uint32_t *pCode   = (uint32_t *)m_context.getBinaryCollection().get(source_name).getBinary();
    uint32_t codeSize = (uint32_t)m_context.getBinaryCollection().get(source_name).getSize();
...
Move<VkPipeline> SimpleGraphicsPipelineBuilder::buildPipeline(tcu::UVec2 renderSize, VkRenderPass renderPass)
{
    ...
    for (uint32_t i = 0; i < m_shaderStageCount; i++)
    {
    ...
    VkPrimitiveTopology primitiveTopology =
        (m_patchControlPoints > 0) ? VK_PRIMITIVE_TOPOLOGY_PATCH_LIST : VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
    ...
    return makeGraphicsPipeline(
        ...
        *m_pipelineLayout,         // const VkPipelineLayout                        pipelineLayout
        ...
        m_patchControlPoints,      // const uint32_t                                patchControlPoints
...
public:
    ...
    TimestampTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
    ...
    , m_hostQueryReset(hostQueryReset)
{
    ...
    m_timestampValues = new uint64_t[m_stages.size()];
...
tcu::TestStatus TimestampTestInstance::iterate(void)
{
    ...
    vk.getQueryPoolResults(vkDevice, *m_queryPool, 0u, stageSize, sizeof(uint64_t) * stageSize,
                           (void *)m_timestampValues, sizeof(uint64_t),
    ...
    if (m_hostQueryReset)
    {
        ...
        // Get timestamp value from query pool
        vk::VkResult res =
            vk.getQueryPoolResults(vkDevice, *m_queryPool, 0u, stageSize, sizeof(uint64_t) * stageSize * 2,
...
{
    for (uint32_t first = 0; first < m_stages.size(); first++)
    {
        for (uint32_t second = 0; second < first; second++)
        {
...
{
public:
    CalibratedTimestampTest(tcu::TestContext &testContext, const std::string &name, const std::string &description)
        : vkt::TestCase{testContext, name, description}
    {
    }
...
protected:
    ...
    std::vector<VkTimeDomainEXT> getDomainSubset(const std::vector<VkTimeDomainEXT> &available,
                                                 const std::vector<VkTimeDomainEXT> &interesting) const;
    std::string domainName(VkTimeDomainEXT domain) const;
    uint64_t getHostNativeTimestamp(VkTimeDomainEXT hostDomain) const;
...
std::vector<VkTimeDomainEXT> CalibratedTimestampTestInstance::getDomainSubset(
    const std::vector<VkTimeDomainEXT> &available, const std::vector<VkTimeDomainEXT> &interesting) const
{
...
uint64_t CalibratedTimestampTestInstance::getHostNativeTimestamp(VkTimeDomainEXT hostDomain) const
{
#if (DE_OS == DE_OS_WIN32)
...
#else
    DE_ASSERT(hostDomain == VK_TIME_DOMAIN_CLOCK_MONOTONIC_EXT || hostDomain == VK_TIME_DOMAIN_CLOCK_MONOTONIC_RAW_EXT);
...
std::vector<CalibratedTimestampTestInstance::CalibratedTimestamp> CalibratedTimestampTestInstance::
    getCalibratedTimestamps(const std::vector<VkTimeDomainEXT> &domains)
{
    ...
    VK_CHECK(vk.getCalibratedTimestampsEXT(vkDevice, static_cast<uint32_t>(domains.size()), infos.data(),
    ...
    for (const auto devDomain : m_devDomains)
    {
        ...
        if (outOfRange(before.timestamp, written, after.timestamp))
        {
            return tcu::TestStatus::fail(domainName(devDomain) +
                                         ": vkCmdWriteTimestamp() inconsistent with vkGetCalibratedTimestampsEXT()");
        }
    }
    ...
    for (const auto hostDomain : m_hostDomains)
    {
        ...
        if (outOfRange(before, vkTS.timestamp, after))
        {
            return tcu::TestStatus::fail(domainName(hostDomain) +
                                         ": vkGetCalibratedTimestampsEXT() inconsistent with native host API");
        }
    }
...
{
public:
    BasicGraphicsTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                      const TimestampTestParam *param)
...
public:
    ...
    BasicGraphicsTestInstance(Context &context, const StageFlagVector stages, const bool inRenderPass,
                              const bool hostQueryReset);
...
BasicGraphicsTestInstance::BasicGraphicsTestInstance(Context &context, const StageFlagVector stages,
                                                     const bool inRenderPass, const bool hostQueryReset)
...
void BasicGraphicsTestInstance::configCommandBuffer(void)
{
    ...
    beginRenderPass(vk, *m_cmdBuffer, *m_renderPass, *m_framebuffer,
                    makeRect2D(0, 0, m_renderSize.x(), m_renderSize.y()), 2u, attachmentClearValues);
...
{
public:
    AdvGraphicsTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                    const TimestampTestParam *param)
...
{
public:
    AdvGraphicsTestInstance(Context &context, const StageFlagVector stages, const bool inRenderPass,
                            const bool hostQueryReset);
...
AdvGraphicsTestInstance::AdvGraphicsTestInstance(Context &context, const StageFlagVector stages,
                                                 const bool inRenderPass, const bool hostQueryReset)
...
{
public:
    BasicComputeTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                     const TimestampTestParam *param)
        : TimestampTest(testContext, name, description, param)
    {
    }
...
{
public:
    TransferTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                 const TimestampTestParam *param);
...
public:
    TransferTestInstance(Context &context, const StageFlagVector stages, const bool inRenderPass,
                         const bool hostQueryReset, const TransferMethod method);
...
TransferTest::TransferTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
                           const TimestampTestParam *param)
    : TimestampTest(testContext, name, description, param)
{
...
void TransferTestInstance::configCommandBuffer(void)
{
    ...
    {
        vk.cmdWriteTimestamp(*m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, *m_queryPool, 0u);
        vk.cmdCopyQueryPoolResults(*m_cmdBuffer, *m_queryPool, 0u, 1u, *m_dstBuffer, 0u, 8u,
                                   VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WAIT_BIT);
                                                  ...
                                                  VkImageSubresourceRange subRange, VkImageLayout layout)
{
    ...
    vk.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, 0, 0, DE_NULL, 0,
...
{
public:
    TwoCmdBuffersTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
public:
    TwoCmdBuffersTestInstance(Context &context, const StageFlagVector stages, const bool inRenderPass,
                              const bool hostQueryReset, VkCommandBufferLevel cmdBufferLevel);
...
void TwoCmdBuffersTestInstance::configCommandBuffer(void)
{
    ...
    if (m_cmdBufferLevel == VK_COMMAND_BUFFER_LEVEL_PRIMARY)
    {
        ...
        vk.cmdCopyQueryPoolResults(*m_secondCmdBuffer, *m_queryPool, 0u, 1u, *m_dstBuffer, 0u, 0u,
...
{
public:
    ConsistentQueryResultsTest(tcu::TestContext &testContext, const std::string &name, const std::string &description)
        : vkt::TestCase(testContext, name, description)
    {
    }
...
tcu::TestCaseGroup *createTimestampTests(tcu::TestContext &testCtx)
{
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> basicGraphicsTests(new tcu::TestCaseGroup(
            testCtx, "basic_graphics_tests", "Record timestamp in different pipeline stages of basic graphics tests"));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> advGraphicsTests(
            new tcu::TestCaseGroup(testCtx, "advanced_graphics_tests",
    ...
    // Basic Compute Tests
    {
        de::MovePtr<tcu::TestCaseGroup> basicComputeTests(
            new tcu::TestCaseGroup(testCtx, "basic_compute_tests", "Record timestamp for compute stages"));
    ...
    // Transfer Tests
    {
        de::MovePtr<tcu::TestCaseGroup> transferTests(
            new tcu::TestCaseGroup(testCtx, "transfer_tests", "Record timestamp for transfer stages"));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> calibratedTimestampTests(
            new tcu::TestCaseGroup(testCtx, "calibrated", "VK_EXT_calibrated_timestamps tests"));

        calibratedTimestampTests->addChild(new CalibratedTimestampTest<CalibratedTimestampDevDomainTestInstance>(
            testCtx, "dev_domain_test", "Test device domain"));
        calibratedTimestampTests->addChild(new CalibratedTimestampTest<CalibratedTimestampHostDomainTestInstance>(
            testCtx, "host_domain_test", "Test host domain"));
        calibratedTimestampTests->addChild(new CalibratedTimestampTest<CalibratedTimestampCalibrationTestInstance>(
            testCtx, "calibration_test", "Test calibration using device and host domains"));
    ...
    // Misc Tests
    {
        de::MovePtr<tcu::TestCaseGroup> miscTests(
            new tcu::TestCaseGroup(testCtx, "misc_tests", "Misc tests that can not be categorized to other group."));
        ...
        miscTests->addChild(new TimestampTest(testCtx, "timestamp_only",
                                              "Only write timestamp command in the commmand buffer", &param));

        TwoCmdBuffersTestParam twoCmdBuffersParamPrimary(miscStages, 1u, false, false, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
        miscTests->addChild(
            new TwoCmdBuffersTest(testCtx, "two_cmd_buffers_primary",
                                  "Issue query in a command buffer and copy it on another primary command buffer",
                                  &twoCmdBuffersParamPrimary));

        TwoCmdBuffersTestParam twoCmdBuffersParamSecondary(miscStages, 1u, false, false,
                                                           VK_COMMAND_BUFFER_LEVEL_SECONDARY);
        miscTests->addChild(
            new TwoCmdBuffersTest(testCtx, "two_cmd_buffers_secondary",
        ...
        param.toggleHostQueryReset();
        miscTests->addChild(new TimestampTest(testCtx, "timestamp_only_host_query_reset",
                                              "Only write timestamp command in the commmand buffer", &param));
        TwoCmdBuffersTestParam twoCmdBuffersParamPrimaryHostQueryReset(miscStages, 1u, false, true,
                                                                       VK_COMMAND_BUFFER_LEVEL_PRIMARY);
        miscTests->addChild(
            new TwoCmdBuffersTest(testCtx, "two_cmd_buffers_primary_host_query_reset",
                                  "Issue query in a command buffer and copy it on another primary command buffer",
                                  &twoCmdBuffersParamPrimaryHostQueryReset));

        TwoCmdBuffersTestParam twoCmdBuffersParamSecondaryHostQueryReset(miscStages, 1u, false, true,
                                                                         VK_COMMAND_BUFFER_LEVEL_SECONDARY);
        miscTests->addChild(
            new TwoCmdBuffersTest(testCtx, "two_cmd_buffers_secondary_host_query_reset",
                                  "Issue query in a secondary command buffer and copy it on a primary command buffer",
                                  &twoCmdBuffersParamSecondaryHostQueryReset));

        // Reset timestamp query before copying results.
        miscTests->addChild(new ResetTimestampQueryBeforeCopyTest(
            testCtx, "reset_query_before_copy", "Issue a timestamp query and reset it before copying results"));
