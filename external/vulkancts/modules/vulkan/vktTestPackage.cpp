...
vk::ProgramBinary *buildProgram(const std::string &casePath, IteratorType iter,
                                const vk::BinaryRegistryReader &prebuiltBinRegistry, tcu::TestLog &log,
                                vk::BinaryCollection *progCollection, const tcu::CommandLine &commandLine)
{
...
MovePtr<vk::DebugReportRecorder> createDebugReportRecorder(const vk::PlatformInterface &vkp,
                                                           const vk::InstanceInterface &vki, vk::VkInstance instance)
{
...
TestCaseExecutor::TestCaseExecutor(tcu::TestContext &testCtx)
    ...
    , m_debugReportRecorder(testCtx.getCommandLine().isValidationEnabled() ?
                                createDebugReportRecorder(m_library->getPlatformInterface(),
                                                          m_context.getInstanceInterface(), m_context.getInstance()) :
...
void TestCaseExecutor::init(tcu::TestCase *testCase, const std::string &casePath)
{
         ...
         progIter != sourceProgs.glslSources.end(); ++progIter)
    {
        ...
        const vk::ProgramBinary *const binProg =
            buildProgram<glu::ShaderProgramInfo, vk::GlslSourceCollection::Iterator>(
                casePath, progIter, m_prebuiltBinRegistry, log, &m_progCollection, commandLine);
         ...
         progIter != sourceProgs.hlslSources.end(); ++progIter)
    {
        ...
        const vk::ProgramBinary *const binProg =
            buildProgram<glu::ShaderProgramInfo, vk::HlslSourceCollection::Iterator>(
                casePath, progIter, m_prebuiltBinRegistry, log, &m_progCollection, commandLine);
         ...
         asmIterator != sourceProgs.spirvAsmSources.end(); ++asmIterator)
    {
        ...
        buildProgram<vk::SpirVProgramInfo, vk::SpirVAsmCollection::Iterator>(
            casePath, asmIterator, m_prebuiltBinRegistry, log, &m_progCollection, commandLine);
    }
...
TestPackage::TestPackage(tcu::TestContext &testCtx) : tcu::TestPackage(testCtx, "dEQP-VK", "dEQP Vulkan Tests")
{
}
