...
static void generateRandomInputData(de::Random &rnd, glu::DataType dataType, glu::Precision precision, uint32_t *dst,
                                    int numValues)
{
    const int scalarSize         = glu::getDataTypeScalarSize(dataType);
    const uint32_t integerLength = (uint32_t)getShaderUintBitCount(precision);
