...
static int findMSB(int32_t value)
{
    if (value > 0)
        return 31 - deClz32((uint32_t)value);
    else if (value < 0)
        return 31 - deClz32(~(uint32_t)value);
    else
        return -1;
...
static int findMSB(uint32_t value)
{
    if (value > 0)
...
static void addFunctionCases(tcu::TestCaseGroup *parent, const char *functionName, bool intTypes, bool uintTypes,
...
{
public:
    IntegerFunctionCase(tcu::TestContext &testCtx, const char *name, const char *description,
...
IntegerFunctionCase::IntegerFunctionCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                         glu::ShaderType shaderType)
...
{
public:
    UaddCarryCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                          const char *name)
...
{
public:
    UaddCarryCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                  glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "uaddCarry",
                              shaderType)
    {
...
{
public:
    UsubBorrowCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                           const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
{
public:
    UsubBorrowCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                   glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "usubBorrow",
                              shaderType)
    {
        m_spec.inputs.push_back(Symbol("x", glu::VarType(baseType, precision)));
        m_spec.inputs.push_back(Symbol("y", glu::VarType(baseType, precision)));
...
{
public:
    UmulExtendedCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                             const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
{
public:
    UmulExtendedCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                     glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "umulExtended",
                              shaderType)
    {
        m_spec.inputs.push_back(Symbol("x", glu::VarType(baseType, precision)));
        m_spec.inputs.push_back(Symbol("y", glu::VarType(baseType, precision)));
...
{
public:
    ImulExtendedCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                             const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
{
public:
    ImulExtendedCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                     glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "imulExtended",
                              shaderType)
    {
        m_spec.inputs.push_back(Symbol("x", glu::VarType(baseType, precision)));
        m_spec.inputs.push_back(Symbol("y", glu::VarType(baseType, precision)));
...
{
public:
    BitfieldExtractCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                                const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
    ...
    bool compare(const void *const *inputs, const void *const *outputs)
    {
        ...
        for (int compNdx = 0; compNdx < scalarSize; compNdx++)
        {
            ...
            const uint32_t ref     = baseVal | ((isSigned && (baseVal & (1 << (bits - 1)))) ? ~valMask : 0u);
...
{
public:
    BitfieldExtractCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                        glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(),
...
{
public:
    BitfieldInsertCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                               const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
{
public:
    BitfieldInsertCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                       glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(),
...
{
public:
    BitfieldReverseCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                                const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
{
public:
    BitfieldReverseCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                        glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(),
...
{
public:
    BitCountCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                         const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
{
public:
    BitCountCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision,
                 glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "bitCount",
                              shaderType)
    {
...
{
public:
    FindLSBCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                        const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
...
public:
    FindLSBCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision, glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "findLSB",
                              shaderType)
    {
        const int vecSize           = glu::getDataTypeScalarSize(baseType);
        const glu::DataType intType = vecSize == 1 ? glu::TYPE_INT : glu::getDataTypeIntVec(vecSize);
...
{
public:
    findMSBCaseInstance(Context &context, glu::ShaderType shaderType, const ShaderSpec &spec, int numValues,
                        const char *name)
        : IntegerFunctionTestInstance(context, shaderType, spec, numValues, name)
    {
    }
    ...
    bool compare(const void *const *inputs, const void *const *outputs)
    {
        ...
        for (int compNdx = 0; compNdx < scalarSize; compNdx++)
        {
            ...
            const int minRef     = isSigned ? findMSB(toPrecision(int32_t(value), integerLength)) :
                                              findMSB(toPrecision(value, integerLength));
            const int maxRef     = isSigned ? findMSB(int32_t(value)) : findMSB(value);
...
public:
    findMSBCase(tcu::TestContext &testCtx, glu::DataType baseType, glu::Precision precision, glu::ShaderType shaderType)
        : IntegerFunctionCase(testCtx, getIntegerFuncCaseName(baseType, precision, shaderType).c_str(), "findMSB",
                              shaderType)
    {
        const int vecSize           = glu::getDataTypeScalarSize(baseType);
        const glu::DataType intType = vecSize == 1 ? glu::TYPE_INT : glu::getDataTypeIntVec(vecSize);
