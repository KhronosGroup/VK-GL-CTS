...
using namespace vk;
constexpr deUint32 kNumElements			  = 32;
...
public:
	...
	virtual tcu::TestStatus iterate(void)
	{
		...
		const deUint32* outputData = reinterpret_cast<deUint32*>(m_outputAlloc->getHostPtr());
		return validateOutput(outputData);
	}

private:
	tcu::TestStatus validateOutput(const deUint32* outputData)
	{
	...
	void generateGraphicsPipeline()
	{
		...
		if (m_testParam.dataClass == DataClass::StorageBuffer)
		{
			...
			const VkDescriptorSetLayoutCreateInfo setLayoutCreateInfo = {
				...
				static_cast<deUint32>(bindings.size()),					 // bindingCount
		...
		if (m_testParam.dataClass == DataClass::StorageBuffer)
		{
			...
			for (deUint32 i = 0; i < static_cast<deUint32>(bindings.size()); ++i)
			{
			...
			VkWriteDescriptorSet w = {
				...
				*m_descriptorSet,						   // dstSet
				...
				0,										   // dstArrayEllement
				static_cast<deUint32>(bufferInfos.size()), // descriptorCount
				VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,		   // descriptorType
				nullptr,								   // pImageInfo
	...
	void render()
	{
									...
									vk::VkPipelineStageFlags dstStageMask)
		{
			...
			vk.cmdPipelineBarrier(cmdBuffer, srcStageMask, dstStageMask, (vk::VkDependencyFlags)0, 0,
								  (const vk::VkMemoryBarrier*)nullptr, 0, (const vk::VkBufferMemoryBarrier*)nullptr, 1,
								  &barrier);
		};
		...
		m_vk.cmdPipelineBarrier(
			*m_cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
...
public:
	ShaderExpectAssumeCase(tcu::TestContext& testCtx, TestParam testParam)
		: TestCase(testCtx, testParam.testName, testParam.testName), m_testParam(testParam)
	{
	}
...
{
	return createTestGroup(testCtx, "shader_expect_assume", "Shader Expect Assume Tests", addShaderExpectAssumeTests);
}
