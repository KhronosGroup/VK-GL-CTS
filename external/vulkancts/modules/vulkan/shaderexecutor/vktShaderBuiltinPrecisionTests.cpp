...
typedef uint32_t PrecisionTestFeatures;

void areFeaturesSupported(const Context &context, uint32_t toCheck)
{
    if (toCheck == PRECISION_TEST_FEATURES_NONE)
        return;

    const vk::VkPhysicalDevice16BitStorageFeatures &extensionFeatures = context.get16BitStorageFeatures();

    if ((toCheck & PRECISION_TEST_FEATURES_16BIT_BUFFER_ACCESS) != 0 &&
        extensionFeatures.storageBuffer16BitAccess == VK_FALSE)
        TCU_THROW(NotSupportedError, "Requested 16bit storage features not supported");

    if ((toCheck & PRECISION_TEST_FEATURES_16BIT_UNIFORM_AND_STORAGE_BUFFER_ACCESS) != 0 &&
        extensionFeatures.uniformAndStorageBuffer16BitAccess == VK_FALSE)
        TCU_THROW(NotSupportedError, "Requested 16bit storage features not supported");

    if ((toCheck & PRECISION_TEST_FEATURES_16BIT_PUSH_CONSTANT) != 0 &&
        extensionFeatures.storagePushConstant16 == VK_FALSE)
        TCU_THROW(NotSupportedError, "Requested 16bit storage features not supported");

    if ((toCheck & PRECISION_TEST_FEATURES_16BIT_INPUT_OUTPUT) != 0 &&
        extensionFeatures.storageInputOutput16 == VK_FALSE)
        TCU_THROW(NotSupportedError, "Requested 16bit storage features not supported");

    if ((toCheck & PRECISION_TEST_FEATURES_16BIT_SHADER_FLOAT) != 0 &&
        context.getShaderFloat16Int8Features().shaderFloat16 == VK_FALSE)
        TCU_THROW(NotSupportedError, "Requested 16-bit floats (halfs) are not supported in shader code");

    if ((toCheck & PRECISION_TEST_FEATURES_64BIT_SHADER_FLOAT) != 0 &&
        context.getDeviceFeatures().shaderFloat64 == VK_FALSE)
...
template <>
bool isInteger<int>(void)
{
    return true;
}
template <>
bool isInteger<tcu::IVec2>(void)
{
    return true;
}
template <>
bool isInteger<tcu::IVec3>(void)
{
    return true;
}
template <>
bool isInteger<tcu::IVec4>(void)
{
    return true;
}
...
bool contains(const typename Traits<T>::IVal &ival, const T &value, bool is16Bit = false,
              const tcu::Maybe<U> &modularDivisor = tcu::Nothing)
{
...
const std::string getComparisonOperation(const int ndx)
{
    ...
    const std::string operations[operationCount] = {
        "OpFOrdEqual\t\t\t",        "OpFOrdGreaterThan\t",  "OpFOrdLessThan\t\t",    "OpFOrdGreaterThanEqual",
        "OpFOrdLessThanEqual\t",    "OpFUnordEqual\t\t",    "OpFUnordGreaterThan\t", "OpFUnordLessThan\t",
        "OpFUnordGreaterThanEqual", "OpFUnordLessThanEqual"};
...
string comparisonMessage(const tcu::IVec3 &val)
{
    ...
    {
        oss << getComparisonOperation(ndx) << "\t:\t" << ((flags.x() & 1) == 1 ? "TRUE" : "FALSE") << "\t"
            << ((flags.y() & 1) == 1 ? "TRUE" : "FALSE") << "\t" << ((flags.z() & 1) == 1 ? "TRUE" : "FALSE") << "\n";
        flags.x() = flags.x() >> 1;
        flags.y() = flags.y() >> 1;
        flags.z() = flags.z() >> 1;
    }
...
struct Traits<float> : ScalarTraits<float>
{
    ...
    static void doPrintValue16(const FloatFormat &fmt, const float &value, ostream &os)
    {
        const uint32_t iRep = reinterpret_cast<const uint32_t &>(value);
    ...
    static void doPrintValue32(const FloatFormat &fmt, const float &value, ostream &os)
    {
        os << fmt.floatToHex(value);
    }

    static void doPrintValue64(const FloatFormat &fmt, const float &value, ostream &os)
    {
        os << fmt.floatToHex(value);
    }
...
struct Traits<double> : ScalarTraits<double>
{
    ...
    static void doPrintValue16(const FloatFormat &fmt, const double &value, ostream &os)
    {
        const uint64_t iRep = reinterpret_cast<const uint64_t &>(value);
        ...
        os << fmt.floatToHex(byte0) << " " << fmt.floatToHex(byte1) << " " << fmt.floatToHex(byte2) << " "
           << fmt.floatToHex(byte3);
    }

    static void doPrintValue32(const FloatFormat &fmt, const double &value, ostream &os)
    {
        const uint64_t iRep = reinterpret_cast<const uint64_t &>(value);
    ...
    static void doPrintValue64(const FloatFormat &fmt, const double &value, ostream &os)
    {
        os << fmt.floatToHex(value);
    }
...
struct Traits<deFloat16> : ScalarTraits<deFloat16>
{
    ...
    static void doPrintValue16(const FloatFormat &fmt, const deFloat16 &value, ostream &os)
    {
        const float res0 = deFloat16To32(value);
        os << fmt.floatToHex(static_cast<double>(res0));
    }
    static void doPrintValue32(const FloatFormat &fmt, const deFloat16 &value, ostream &os)
    {
        const float res0 = deFloat16To32(value);
        os << fmt.floatToHex(static_cast<double>(res0));
    }

    static void doPrintValue64(const FloatFormat &fmt, const deFloat16 &value, ostream &os)
    {
    ...
    static bool doContains(const Interval &a, const deFloat16 &value, bool is16Bit,
                           const tcu::Maybe<deFloat16> &modularDivisor)
    {
        DE_UNREF(is16Bit);
        float res0 = deFloat16To32(value);
        const tcu::Maybe<float> convertedDivisor =
            (modularDivisor ? tcu::just(deFloat16To32(modularDivisor.get())) : tcu::Nothing);
...
{
    static void doPrintValue16(const FloatFormat &, const float &value, ostream &os)
    {
        os << (value != 0.0f ? "true" : "false");
    }

    static void doPrintValue32(const FloatFormat &, const float &value, ostream &os)
    {
        os << (value != 0.0f ? "true" : "false");
    }

    static void doPrintValue64(const FloatFormat &, const float &value, ostream &os)
    {
        os << (value != 0.0f ? "true" : "false");
    }
...
{
    static void doPrintValue16(const FloatFormat &, const int &value, ostream &os)
    {
    ...
    static void doPrintValue32(const FloatFormat &, const int &value, ostream &os)
    {
        os << value;
    }

    static void doPrintValue64(const FloatFormat &, const int &value, ostream &os)
    {
        os << value;
    }
...
struct ContainerTraits
{
    ...
    static bool doContains(const IVal &ival, const T &value, bool is16Bit, const tcu::Maybe<T> &modularDivisor)
    {
        ...
        for (int ndx = 0; ndx < T::SIZE; ++ndx)
        {
            const tcu::Maybe<DivisorElement> divisorElement =
                (modularDivisor ? tcu::just((*modularDivisor)[ndx]) : tcu::Nothing);
...
struct Traits<Void>
{
    ...
    template <typename U>
    static bool doContains(const Void &, const Void &value, bool is16Bit, const tcu::Maybe<U> &modularDivisor)
    {
        DE_UNREF(value);
        DE_UNREF(is16Bit);
        DE_UNREF(modularDivisor);
        return true;
    }
...
{
    tcu::IVec4 result(static_cast<int>(val[0].lo()), static_cast<int>(val[1].lo()), static_cast<int>(val[2].lo()),
                      static_cast<int>(val[3].lo()));
    return comparisonMessage(result);
}
...
struct EvalContext
{
    EvalContext(const FloatFormat &format_, Precision floatPrecision_, Environment &env_, int callDepth_)
        : format(format_)
...
public:
    ...
    void failed(EvalContext &ctx) const
    {
        this->doFail(ctx);
    }
...
template <typename T, int Size>
ExprP<Vector<T, Size>> operator-(const ExprP<Vector<T, Size>> &arg0, const ExprP<Vector<T, Size>> &arg1);
template <int Size, typename T>
ExprP<Vector<T, Size>> operator*(const ExprP<Vector<T, Size>> &arg0, const ExprP<T> &arg1);
template <typename T, int Size>
ExprP<Vector<T, Size>> operator*(const ExprP<Vector<T, Size>> &arg0, const ExprP<Vector<T, Size>> &arg1);
template <int Rows, int Cols, typename T>
ExprP<Vector<T, Rows>> operator*(const ExprP<Vector<T, Cols>> &left, const ExprP<Matrix<T, Rows, Cols>> &right);
template <int Rows, int Cols, typename T>
ExprP<Vector<T, Cols>> operator*(const ExprP<Matrix<T, Rows, Cols>> &left, const ExprP<Vector<T, Rows>> &right);
template <int Rows, int Cols, typename T>
ExprP<Matrix<T, Rows, Cols>> operator*(const ExprP<Matrix<T, Rows, Cols>> &left, const ExprP<T> &right);
...
public:
    ...
    virtual Interval getInputRange(const bool is16bit) const
    {
        DE_UNREF(is16bit);
        return Interval(true, -TCU_INFINITY, TCU_INFINITY);
    }
...
protected:
    Interval doApply(const EvalContext &ctx,
                     const typename Signature<typename T::Ret, typename T::Arg0>::IArgs &iargs) const
    {
...
{
public:
    CFloatFunc1(const string &name, tcu::DoubleFunc1 &func) : m_name(name), m_func(func)
    {
    }
...
{
public:
    CFloatFunc2(const string &name, tcu::DoubleFunc2 &func) : m_name(name), m_func(func)
    {
    }
...
protected:
    Interval doApply(const EvalContext &ctx,
                     const typename Signature<typename T::Ret, typename T::Arg0, typename T::Arg1,
                                              typename T::Arg2>::IArgs &iargs) const
    {
...
public:
    ...
    SpirVCaseT getSpirvCase() const
    {
        return SPIRV_CASETYPE_COMPARE;
    }

    Interval doApply(const EvalContext &ctx, const typename Comparison<T>::IArgs &iargs) const
    {
        DE_UNREF(ctx);
...
public:
                     ...
                     const typename Signature<typename T::Ret, typename T::Arg0, typename T::Arg1>::IArgs &iargs) const
    {
        ...
        if (a.isOrdinary(ctx.format.getMaxValue()) && b.isOrdinary(ctx.format.getMaxValue()))
        {
            ...
            if (a.lo() >= 0 && b.lo() >= 0)
            {
                TCU_SET_INTERVAL_BOUNDS(ret, prod, prod = a.lo() * b.lo(), prod = a.hi() * b.hi());
                return ctx.format.convert(ctx.format.roundOut(ret, true));
            }
            if (a.lo() >= 0 && b.hi() <= 0)
            {
                TCU_SET_INTERVAL_BOUNDS(ret, prod, prod = a.hi() * b.lo(), prod = a.lo() * b.hi());
                return ctx.format.convert(ctx.format.roundOut(ret, true));
            }
        }
        return this->applyMonotone(ctx, iargs.a, iargs.b);
...
{
public:
    ExpFunc(const string &name, DoubleFunc1 &func) : CFloatFunc1<T>(name, func)
    {
    }
...
template <class T>
class Exp2 : public ExpFunc<T>
{
public:
    Exp2(void) : ExpFunc<T>("exp2", deExp2)
    {
    }
};
template <class T>
class Exp : public ExpFunc<T>
{
public:
    Exp(void) : ExpFunc<T>("exp", deExp)
    {
    }
};

template <typename T>
ExprP<T> exp2(const ExprP<T> &x)
{
    return app<Exp2<Signature<T, T>>>(x);
}
template <typename T>
ExprP<T> exp(const ExprP<T> &x)
{
    return app<Exp<Signature<T, T>>>(x);
}
...
{
public:
    LogFunc(const string &name, DoubleFunc1 &func) : CFloatFunc1<T>(name, func)
    {
    }
...
template <class T>
class Log2 : public LogFunc<T>
{
public:
    Log2(void) : LogFunc<T>("log2", deLog2)
    {
    }
};
template <class T>
class Log : public LogFunc<T>
{
public:
    Log(void) : LogFunc<T>("log", deLog)
    {
    }
};
...
{
    return app<Log2<Signature<float, float>>>(x);
}
ExprP<float> log(const ExprP<float> &x)
{
    return app<Log<Signature<float, float>>>(x);
}

ExprP<deFloat16> log2(const ExprP<deFloat16> &x)
{
    return app<Log2<Signature<deFloat16, deFloat16>>>(x);
}
ExprP<deFloat16> log(const ExprP<deFloat16> &x)
{
    return app<Log<Signature<deFloat16, deFloat16>>>(x);
}

ExprP<double> log2(const ExprP<double> &x)
{
    return app<Log2<Signature<double, double>>>(x);
}
ExprP<double> log(const ExprP<double> &x)
{
    return app<Log<Signature<double, double>>>(x);
}
...
DEFINE_DERIVED_FLOAT1_16BIT(Radians16, radians, d,
                            (constant((deFloat16)DE_PI_16BIT) / constant((deFloat16)FLOAT16_180_0)) * d)
...
DEFINE_DERIVED_FLOAT1_16BIT(Degrees16, degrees, r,
                            (constant((deFloat16)FLOAT16_180_0) / constant((deFloat16)DE_PI_16BIT)) * r)
...
{
    return app<Sin<Signature<float, float>>>(x);
}
ExprP<deFloat16> sin(const ExprP<deFloat16> &x)
{
    return app<Sin<Signature<deFloat16, deFloat16>>>(x);
}
ExprP<double> sin(const ExprP<double> &x)
{
    return app<Sin<Signature<double, double>>>(x);
}
...
{
    return app<Cos<Signature<float, float>>>(x);
}
ExprP<deFloat16> cos(const ExprP<deFloat16> &x)
{
    return app<Cos<Signature<deFloat16, deFloat16>>>(x);
}
ExprP<double> cos(const ExprP<double> &x)
{
    return app<Cos<Signature<double, double>>>(x);
}

DEFINE_DERIVED_FLOAT1_INPUTRANGE(Tan, tan, x, sin(x) * (constant(1.0f) / cos(x)),
                                 Interval(false, -DE_PI_DOUBLE, DE_PI_DOUBLE))
DEFINE_DERIVED_FLOAT1_INPUTRANGE_16BIT(Tan16Bit, tan, x, sin(x) * (constant((deFloat16)FLOAT16_1_0) / cos(x)),
                                       Interval(false, -DE_PI_DOUBLE, DE_PI_DOUBLE))
DEFINE_DERIVED_DOUBLE1_INPUTRANGE(Tan64Bit, tan, x, sin(x) * (constant(1.0) / cos(x)),
                                  Interval(false, -DE_PI_DOUBLE, DE_PI_DOUBLE))
...
DEFINE_DERIVED_FLOAT1_16BIT(ACosh16Bit, acosh, x,
                            log(x + sqrt(alternatives((x + constant((deFloat16)FLOAT16_1_0)) *
                                                          (x - constant((deFloat16)FLOAT16_1_0)),
                                                      (x * x - constant((deFloat16)FLOAT16_1_0))))))
DEFINE_DERIVED_FLOAT1_16BIT(ATanh16Bit, atanh, x,
                            constant((deFloat16)FLOAT16_0_5) *
                                log((constant((deFloat16)FLOAT16_1_0) + x) / (constant((deFloat16)FLOAT16_1_0) - x)))
...
DEFINE_DERIVED_DOUBLE1(ACosh64Bit, acosh, x,
                       log(x + sqrt(alternatives((x + constant(1.0)) * (x - constant(1.0)), (x * x - constant(1.0))))))
DEFINE_DERIVED_DOUBLE1(ATanh64Bit, atanh, x, constant(0.5) * log((constant(1.0) + x) / (constant(1.0) - x)))
...
template <>
string vecNamePrefix<deFloat16>(void)
{
    return "";
}
template <>
string vecNamePrefix<double>(void)
{
    return "d";
}
template <>
...
template <typename T, int Rows, int Cols>
class MatNeg : public PrimitiveFunc<Signature<Matrix<T, Rows, Cols>, Matrix<T, Rows, Cols>>>
{
public:
...
class CompMatFuncBase
    : public CompWiseFunc<T, Signature<Matrix<T, Rows, Cols>, Matrix<T, Rows, Cols>, Matrix<T, Rows, Cols>>>
{
public:
...
template <int Size, class T>
class Length : public DerivedFunc<Signature<T, typename ContainerOf<T, Size>::Container>>
{
public:
...
class Distance : public DerivedFunc<
                     Signature<T, typename ContainerOf<T, Size>::Container, typename ContainerOf<T, Size>::Container>>
{
public:
...
template <int Size, class T>
class Normalize
    : public DerivedFunc<Signature<typename ContainerOf<T, Size>::Container, typename ContainerOf<T, Size>::Container>>
{
public:
...
class FaceForward
    : public DerivedFunc<Signature<typename ContainerOf<T, Size>::Container, typename ContainerOf<T, Size>::Container,
                                   typename ContainerOf<T, Size>::Container, typename ContainerOf<T, Size>::Container>>
{
public:
...
protected:
    ExprP<Ret> doExpand(ExpandContext &ctx, const ArgExprs &args) const
    {
        ...
        return i - alternatives(
                       (n * dotNI) * getConstTwo<T>(),
                       alternatives(n * (dotNI * getConstTwo<T>()),
                                    alternatives(n * dot(i * getConstTwo<T>(), n), n * dot(i, n * getConstTwo<T>()))));
    }
};

template <class T>
class Reflect<1, T> : public DerivedFunc<Signature<T, T, T>>
{
public:
    typedef typename Reflect::Ret Ret;
    typedef typename Reflect::Arg0 Arg0;
...
class Refract
    : public DerivedFunc<Signature<typename ContainerOf<T, Size>::Container, typename ContainerOf<T, Size>::Container,
                                   typename ContainerOf<T, Size>::Container, T>>
{
public:
...
protected:
    ExprP<Ret> doExpand(ExpandContext &ctx, const ArgExprs &args) const
    {
        ...
        const ExprP<T> dotNI   = bindExpression("dotNI", ctx, dot(n, i));
        const ExprP<T> k = bindExpression("k", ctx, getConstOne<T>() - eta * eta * (getConstOne<T>() - dotNI * dotNI));
        return cond(k < getConstZero<T>(), genXType<T, Size>(getConstZero<T>()), i * eta - n * (eta * dotNI + sqrt(k)));
    }
};
...
{
public:
    Min(void) : PreciseFunc2<T>("min", deMin)
    {
    }
};
template <class T>
class Max : public PreciseFunc2<T>
{
public:
    Max(void) : PreciseFunc2<T>("max", deMax)
    {
    }
};
...
DEFINE_DERIVED_FLOAT3_16BIT(Mix16Bit, mix, x, y, a,
                            alternatives((x * (constant((deFloat16)FLOAT16_1_0) - a)) + y * a, x + (y - x) * a))

DEFINE_DERIVED_DOUBLE3(Mix64Bit, mix, x, y, a, alternatives((x * (constant(1.0) - a)) + y * a, x + (y - x) * a))
...
template <class T>
class Step : public PreciseFunc2<T>
{
public:
    Step(void) : PreciseFunc2<T>("step", step)
    {
    }
};
...
template <>
ExprP<SmoothStep<Signature<float, float, float, float>>::Ret> SmoothStep<Signature<float, float, float, float>>::
    doExpand(ExpandContext &ctx, const SmoothStep<Signature<float, float, float, float>>::ArgExprs &args) const
{
    const ExprP<float> &edge0 = args.a;
    const ExprP<float> &edge1 = args.b;
    const ExprP<float> &x     = args.c;
    const ExprP<float> tExpr =
...
template <>
ExprP<SmoothStep<Signature<deFloat16, deFloat16, deFloat16, deFloat16>>::TRet> SmoothStep<
    Signature<deFloat16, deFloat16, deFloat16, deFloat16>>::doExpand(ExpandContext &ctx, const TArgExprs &args) const
{
    const ExprP<deFloat16> &edge0 = args.a;
    const ExprP<deFloat16> &edge1 = args.b;
    const ExprP<deFloat16> &x     = args.c;
    const ExprP<deFloat16> tExpr =
        clamp((x - edge0) / (edge1 - edge0), constant((deFloat16)FLOAT16_0_0), constant((deFloat16)FLOAT16_1_0)) +
        nanIfGreaterOrEqual(edge0, edge1); // force NaN (and non-analyzable result) for cases edge0 >= edge1
...
template <>
ExprP<SmoothStep<Signature<double, double, double, double>>::Ret> SmoothStep<
    Signature<double, double, double, double>>::
    doExpand(ExpandContext &ctx, const SmoothStep<Signature<double, double, double, double>>::ArgExprs &args) const
{
    const ExprP<double> &edge0 = args.a;
    const ExprP<double> &edge1 = args.b;
    const ExprP<double> &x     = args.c;
    const ExprP<double> tExpr =
        clamp((x - edge0) / (edge1 - edge0), constant(0.0), constant(1.0)) +
        nanIfGreaterOrEqual(edge0, edge1); // force NaN (and non-analyzable result) for cases edge0 >= edge1
...
protected:
    Interval doApply(const EvalContext &ctx, const IArgs &iargs) const
    {
        ...
        if (iargs.b.lo() < minExp)
            ret |= 0.0;
        if (!ret.isFinite(ctx.format.getMaxValue()))
            ret |= TCU_NAN;
        return ctx.format.convert(ret);
    }
};
...
class Transpose : public PrimitiveFunc<Signature<Matrix<T, Rows, Columns>, Matrix<T, Columns, Rows>>>
{
public:
...
class MatMul : public MulFunc<Matrix<T, LeftRows, RightCols>, Matrix<T, LeftRows, Middle>, Matrix<T, Middle, RightCols>>
{
protected:
        ...
        for (int row = 0; row < LeftRows; ++row)
        {
            for (int col = 0; col < RightCols; ++col)
            {
                ...
                for (int ndx = 0; ndx < Middle; ++ndx)
                    element = call<Add<Signature<T, T, T>>>(
                        ctx, element, call<Mul<Signature<T, T, T>>>(ctx, left[ndx][row], right[col][ndx]));
...
template <typename T, int Rows, int Cols>
class VecMatMul : public MulFunc<Vector<T, Cols>, Vector<T, Rows>, Matrix<T, Rows, Cols>>
{
public:
        ...
        for (int col = 0; col < Cols; ++col)
        {
            ...
            for (int row = 0; row < Rows; ++row)
                element = call<Add<Signature<T, T, T>>>(ctx, element,
                                                        call<Mul<Signature<T, T, T>>>(ctx, left[row], right[col][row]));
...
template <int Rows, int Cols, class T>
class MatVecMul : public MulFunc<Vector<T, Rows>, Matrix<T, Rows, Cols>, Vector<T, Cols>>
{
public:
...
protected:
    IRet doApply(const EvalContext &ctx, const IArgs &iargs) const
    {
        ...
        return call<VecMatMul<T, Cols, Rows>>(ctx, right, call<Transpose<Rows, Cols, T>>(ctx, left));
    }
};

template <int Rows, int Cols, class T>
class OuterProduct : public PrimitiveFunc<Signature<Matrix<T, Rows, Cols>, Vector<T, Rows>, Vector<T, Cols>>>
{
public:
...
template <int Rows, int Cols, class T>
ExprP<Matrix<T, Rows, Cols>> outerProduct(const ExprP<Vector<T, Rows>> &left, const ExprP<Vector<T, Cols>> &right)
{
...
class Determinant;
template <int Size>
class Determinant16bit;
template <int Size>
class Determinant64bit;
...
protected:
    ExprP<Ret> doExpand(ExpandContext &ctx, const ArgExprs &args) const
    {
        ...
        ExprP<Mat3_16b::Scalar> schur =
            bindExpression("schur", ctx, constant((deFloat16)FLOAT16_1_0) / (matD - dot(matC * invA, matB)));
        ...
        ExprP<Vec2_16Bit> blockB = bindExpression("blockB", ctx, (invA * matB) * -schur);
        ExprP<Vec2_16Bit> blockC = bindExpression("blockC", ctx, (matC * invA) * -schur);
...
protected:
    ExprP<Ret> doExpand(ExpandContext &ctx, const ArgExprs &args) const
    {
        ...
        ExprP<Mat2_16b> invA =
            bindExpression("invA", ctx, inverse(mat2(vec2(mat[0][0], mat[0][1]), vec2(mat[1][0], mat[1][1]))));
        ExprP<Mat2_16b> matB =
            bindExpression("matB", ctx, mat2(vec2(mat[2][0], mat[2][1]), vec2(mat[3][0], mat[3][1])));
        ExprP<Mat2_16b> matC =
            bindExpression("matC", ctx, mat2(vec2(mat[0][2], mat[0][3]), vec2(mat[1][2], mat[1][3])));
        ExprP<Mat2_16b> matD =
            bindExpression("matD", ctx, mat2(vec2(mat[2][2], mat[2][3]), vec2(mat[3][2], mat[3][3])));
        ExprP<Mat2_16b> schur  = bindExpression("schur", ctx, inverse(matD + -(matC * invA * matB)));
        ExprP<Mat2_16b> blockA = bindExpression("blockA", ctx, invA + (invA * matB * schur * matC * invA));
        ExprP<Mat2_16b> blockB = bindExpression("blockB", ctx, (-invA) * matB * schur);
        ExprP<Mat2_16b> blockC = bindExpression("blockC", ctx, (-schur) * matC * invA);
...
protected:
    ExprP<Ret> doExpand(ExpandContext &ctx, const ArgExprs &args) const
    {
        ...
        ExprP<Matrix3d::Scalar> schur = bindExpression("schur", ctx, constant(1.0) / (matD - dot(matC * invA, matB)));
        ...
        ExprP<Vec2_64Bit> blockB = bindExpression("blockB", ctx, (invA * matB) * -schur);
        ExprP<Vec2_64Bit> blockC = bindExpression("blockC", ctx, (matC * invA) * -schur);
...
protected:
    ExprP<Ret> doExpand(ExpandContext &ctx, const ArgExprs &args) const
    {
        ...
        ExprP<Matrix2d> matD =
            bindExpression("matD", ctx, mat2(vec2(mat[2][2], mat[2][3]), vec2(mat[3][2], mat[3][3])));
        ExprP<Matrix2d> schur  = bindExpression("schur", ctx, inverse(matD + -(matC * invA * matB)));
        ExprP<Matrix2d> blockA = bindExpression("blockA", ctx, invA + (invA * matB * schur * matC * invA));
        ExprP<Matrix2d> blockB = bindExpression("blockB", ctx, (-invA) * matB * schur);
        ExprP<Matrix2d> blockC = bindExpression("blockC", ctx, (-schur) * matC * invA);
...
ExprP<Matrix<T, LeftRows, RightCols>> operator*(const ExprP<Matrix<T, LeftRows, Middle>> &left,
...
{
public:
    void genFixeds(const FloatFormat &, const Precision, vector<Void> &dst, const Interval &) const
    {
        dst.push_back(Void());
    }
};
...
public:
    ...
    void genFixeds(const FloatFormat &format, const Precision prec, vector<float> &dst,
...
public:
    ...
    void genFixeds(const FloatFormat &format, const Precision prec, vector<double> &dst,
                   const Interval &inputRange) const;
...
double randomDouble(const FloatFormat &format, Random &rnd, const Interval &inputRange)
{
    ...
    {
    case 0:
        return inputRange.contains(0) ? 0 : midpoint;
    case 1:
        return inputRange.contains(TCU_INFINITY) ? TCU_INFINITY : midpoint;
    case 2:
        return inputRange.contains(-TCU_INFINITY) ? -TCU_INFINITY : midpoint;
    case 3:
        return inputRange.contains(TCU_NAN) ? TCU_NAN : midpoint;
    default:
        break;
    }
...
float DefaultSampling<float>::genRandom(const FloatFormat &format, Precision prec, Random &rnd,
                                        const Interval &inputRange) const
{
    DE_UNREF(prec);
...
void DefaultSampling<float>::genFixeds(const FloatFormat &format, const Precision prec, vector<float> &dst,
                                       const Interval &inputRange) const
{
    const int minExp          = format.getMinExp();
    const int maxExp          = format.getMaxExp();
...
void DefaultSampling<float>::removeNotInRange(vector<float> &dst, const Interval &inputRange,
                                              const Precision prec) const
{
    for (vector<float>::iterator it = dst.begin(); it < dst.end();)
    {
        ...
        if (!inputRange.contains(static_cast<double>(*it)) ||
            (prec == glu::PRECISION_LAST && isDenorm16(deFloat32To16Round(*it, DE_ROUNDINGMODE_TO_ZERO))))
            it = dst.erase(it);
        else
            ++it;
    }
...
//! Generate a random double from a reasonable general-purpose distribution.
double DefaultSampling<double>::genRandom(const FloatFormat &format, Precision prec, Random &rnd,
                                          const Interval &inputRange) const
{
    DE_UNREF(prec);
...
//! Generate a standard set of floats that should always be tested.
void DefaultSampling<double>::genFixeds(const FloatFormat &format, const Precision prec, vector<double> &dst,
                                        const Interval &inputRange) const
{
    const int minExp           = format.getMinExp();
    const int maxExp           = format.getMaxExp();
...
public:
    ...
    void genFixeds(const FloatFormat &format, const Precision prec, vector<deFloat16> &dst,
                   const Interval &inputRange) const;
...
//! Generate a random float from a reasonable general-purpose distribution.
deFloat16 DefaultSampling<deFloat16>::genRandom(const FloatFormat &format, const Precision prec, Random &rnd,
                                                const Interval &inputRange) const
{
    DE_UNREF(prec);
                                           ...
                                           const Interval &inputRange) const
{
    ...
    for (float sign = -1.0; sign <= 1.0f; sign += 2.0f)
    {
        ...
        // Largest number
        dst.push_back(
            deFloat32To16Round(sign * (deFloatLdExp(1.0f, maxExp) + (deFloatLdExp(1.0f, maxExp) - maxQuantum)),
...
void DefaultSampling<deFloat16>::removeNotInRange(vector<deFloat16> &dst, const Interval &inputRange,
...
struct CaseContext
{
    CaseContext(const string &name_, TestContext &testContext_, const FloatFormat &floatFormat_,
                const FloatFormat &highpFormat_, const Precision precision_, const ShaderType shaderType_,
                const size_t numRandoms_,
                const PrecisionTestFeatures precisionTestFeatures_ = PRECISION_TEST_FEATURES_NONE,
                const bool isPackFloat16b_ = false, const bool isFloat64b_ = false)
        : name(name_)
...
tcu::TestStatus BuiltinPrecisionCaseTestInstance<In, Out>::iterate(void)
{
    ...
    for (size_t valueNdx = 0; valueNdx < numValues; valueNdx++)
    {
...

        {
            ...
            case 2:
                ...
                if (!status.check(contains(reference1, outputs.out1[valueNdx], m_caseCtx.isPackFloat16b),
            ...
            case 1:
            {
                ...
                if (!status.check(
                        contains(reference0, outputs.out0[valueNdx], m_caseCtx.isPackFloat16b, modularDivisor),
        ...
        if ((!result && numErrors <= maxMsgs) || GLS_LOG_ALL_RESULTS)
        {
            ...
            if (inCount > 0)
            {
                builder << "\t" << m_variables.in0->getName() << " = "
                        << (isInput64Bit ? value64ToString(highpFmt, inputs.in0[valueNdx]) :
                                           (isInput16Bit ? value16ToString(highpFmt, inputs.in0[valueNdx]) :
                                                           value32ToString(highpFmt, inputs.in0[valueNdx])))
                        << "\n";
            }
            ...
            if (inCount > 1)
            {
                builder << "\t" << m_variables.in1->getName() << " = "
                        << (isInput64Bit ? value64ToString(highpFmt, inputs.in1[valueNdx]) :
                                           (isInput16Bit ? value16ToString(highpFmt, inputs.in1[valueNdx]) :
                                                           value32ToString(highpFmt, inputs.in1[valueNdx])))
                        << "\n";
            }
            ...
            if (inCount > 2)
            {
                builder << "\t" << m_variables.in2->getName() << " = "
                        << (isInput64Bit ? value64ToString(highpFmt, inputs.in2[valueNdx]) :
                                           (isInput16Bit ? value16ToString(highpFmt, inputs.in2[valueNdx]) :
                                                           value32ToString(highpFmt, inputs.in2[valueNdx])))
                        << "\n";
            }
            ...
            if (inCount > 3)
            {
                builder << "\t" << m_variables.in3->getName() << " = "
                        << (isInput64Bit ? value64ToString(highpFmt, inputs.in3[valueNdx]) :
                                           (isInput16Bit ? value16ToString(highpFmt, inputs.in3[valueNdx]) :
                                                           value32ToString(highpFmt, inputs.in3[valueNdx])))
                        << "\n";
            }
            ...
            if (outCount > 0)
            {
                ...
                else
                {
                    builder << "\t" << m_variables.out0->getName() << " = "
                            << (m_executor->isOutput64Bit(0u) ?
                                    value64ToString(highpFmt, outputs.out0[valueNdx]) :
                                    (m_executor->isOutput16Bit(0u) || m_caseCtx.isPackFloat16b ?
            ...
            if (outCount > 1)
            {
                builder << "\t" << m_variables.out1->getName() << " = "
                        << (m_executor->isOutput64Bit(1u) ? value64ToString(highpFmt, outputs.out1[valueNdx]) :
                                                            (m_executor->isOutput16Bit(1u) || m_caseCtx.isPackFloat16b ?
    ...
    {
        testLog << TestLog::Message << "(Skipped " << (numErrors - maxMsgs) << " messages.)" << TestLog::EndMessage;
    }

    if (numErrors == 0)
    {
        testLog << TestLog::Message << "All " << numValues << " inputs passed." << TestLog::EndMessage;
    }
    else
    {
        testLog << TestLog::Message << numErrors << "/" << numValues << " inputs failed." << TestLog::EndMessage;
    }
...
{
protected:
    PrecisionCase(const CaseContext &context, const string &name, const Interval &inputRange,
...
protected:
    FuncCaseBase(const CaseContext &context, const string &name, const FuncBase &func)
        : PrecisionCase(context, name,
                        func.getInputRange(!context.isFloat64b &&
                                           (context.precision == glu::PRECISION_LAST || context.isPackFloat16b)),
                        func.getRequiredExtension())
    {
    }
...
public:
    MovePtr<TestNode> createCase(const CaseContext &ctx) const
    {
        ...
        if (ctx.name != "mediump")
        {
            ...
            group->addChild(cts_amber::createAmberTestCase(ctx.testContext, "mat3", "Square matrix 3x3 precision tests",
                                                           dataDir, fileName + "_mat_3x3.amber", requirements));
            group->addChild(cts_amber::createAmberTestCase(ctx.testContext, "mat4", "Square matrix 4x4 precision tests",
                                                           dataDir, fileName + "_mat_4x4.amber", requirements));
        }
...
TestCaseGroup *createFuncGroupDouble(TestContext &ctx, const CaseFactory &factory, int numRandoms)
{
    ...
    const CaseContext caseCtx("compute", ctx, highp, highp, precision, glu::SHADERTYPE_COMPUTE, numRandoms,
                              precisionTestFeatures, false, true);
    group->addChild(factory.createCase(caseCtx).release());
...
TestCaseGroup *createFuncGroup16Bit(TestContext &ctx, const CaseFactory &factory, int numRandoms, bool storage32)
{
    ...
    const CaseContext caseCtx("compute", ctx, float16, float16, precision, glu::SHADERTYPE_COMPUTE, numRandoms,
                              precisionTestFeatures, storage32);
    group->addChild(factory.createCase(caseCtx).release());
...
void addBuiltinPrecisionTests(TestContext &ctx, TestCaseGroup &dstGroup, const bool test16Bit = false,
                              const bool storage32Bit = false)
{
    ...
    MovePtr<const CaseFactories> cases =
        (test16Bit && !storage32Bit) ? createBuiltinCases16Bit() : createBuiltinCases();
...
void addBuiltinPrecisionDoubleTests(TestContext &ctx, TestCaseGroup &dstGroup)
{
    const int userRandoms = ctx.getCommandLine().getTestIterationCount();
    const int numRandoms  = userRandoms > 0 ? userRandoms : defRandoms;
