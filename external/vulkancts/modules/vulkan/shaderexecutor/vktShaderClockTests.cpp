...
static inline void *getPtrOfVar(uint64_t &var)
{
    return &var;
}
...
{
public:
    ShaderClockTestInstance(Context &context, bool realtimeTest, const ShaderSpec &shaderSpec,
    ...
    {
        const uint64_t initValue = 0xcdcdcdcd;

        std::vector<uint64_t> outputs(NUM_ELEMENTS, initValue);
...
private:
    ...
    bool validateOutput(std::vector<uint64_t> &outputs)
    {
        // The shader will write a 1 in the output if the clock did not increase
        return (outputs.size() == uint64_t(std::count(std::begin(outputs), std::end(outputs), 0)));
    }
...
private:
    void initShaderSpec()
    {
        ...
        std::map<std::string, std::string> specializations = {{"EXTENSIONS", extensions.str()},
                                                              {"SOURCE", source.str()}};
...
{
    static glu::ShaderType stages[] = {glu::SHADERTYPE_VERTEX, glu::SHADERTYPE_FRAGMENT, glu::SHADERTYPE_COMPUTE};

    static testType operations[] = {{SUBGROUP, BIT_64, "clockARB"},
    ...
    for (size_t i = 0; i != DE_LENGTH_OF_ARRAY(stages); ++i)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> stageGroupTest(
            new tcu::TestCaseGroup(testCtx, setName.c_str(), "Shader Clock Tests"));
