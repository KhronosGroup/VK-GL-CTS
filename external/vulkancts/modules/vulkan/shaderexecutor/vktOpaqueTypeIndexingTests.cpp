...
{
public:
    OpaqueTypeIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description,
...
OpaqueTypeIndexingCase::OpaqueTypeIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                               const glu::ShaderType shaderType, const IndexExprType indexExprType)
    ...
    const tcu::Sampler refSampler =
        isShadowSampler(m_samplerType) ?
            tcu::Sampler(tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE,
                         filterMode, filterMode, 0.0f, false /* non-normalized */, tcu::Sampler::COMPAREMODE_LESS) :
            tcu::Sampler(tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE, tcu::Sampler::CLAMP_TO_EDGE,
...
{
public:
    SamplerIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description,
                        const glu::ShaderType shaderType, glu::DataType samplerType, IndexExprType indexExprType);
...
SamplerIndexingCase::SamplerIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                         const glu::ShaderType shaderType, glu::DataType samplerType,
...
{
public:
    BlockArrayIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description, BlockType blockType,
...
BlockArrayIndexingCase::BlockArrayIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                               BlockType blockType, IndexExprType indexExprType,
                                               const glu::ShaderType shaderType, uint32_t flags)
    : OpaqueTypeIndexingCase(testCtx, name, description, shaderType, indexExprType)
    , m_blockType(blockType)
    , m_flags(flags)
...
{
public:
    AtomicCounterIndexingCase(tcu::TestContext &testCtx, const char *name, const char *description,
...
AtomicCounterIndexingCase::AtomicCounterIndexingCase(tcu::TestContext &testCtx, const char *name,
                                                     const char *description, IndexExprType indexExprType,
        ...
        for (int indexTypeNdx = 0; indexTypeNdx < DE_LENGTH_OF_ARRAY(indexingTypes); indexTypeNdx++)
        {
            ...
            for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(shaderTypes); shaderTypeNdx++)
            {
                ...
                for (int samplerTypeNdx = 0; samplerTypeNdx < DE_LENGTH_OF_ARRAY(samplerTypes); samplerTypeNdx++)
                {
                    ...
                    shaderGroup->addChild(new SamplerIndexingCase(m_testCtx, caseName.c_str(), "", shaderType,
    ...
    // .ubo / .ssbo / .atomic_counter
    {
        ...
        tcu::TestCaseGroup *const ssboStorageBufGroup =
            new tcu::TestCaseGroup(m_testCtx, "ssbo_storage_buffer_decoration",
                                   "Buffer Block (new StorageBuffer decoration) Instance Array Indexing Tests");
        ...
        for (int indexTypeNdx = 0; indexTypeNdx < DE_LENGTH_OF_ARRAY(indexingTypes); indexTypeNdx++)
        {
            ...
            for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(shaderTypes); shaderTypeNdx++)
            {
                ...
                uboGroup->addChild(new BlockArrayIndexingCase(m_testCtx, name.c_str(), indexExprDesc, BLOCKTYPE_UNIFORM,
                                                              indexExprType, shaderType));
                acGroup->addChild(
                    new AtomicCounterIndexingCase(m_testCtx, name.c_str(), indexExprDesc, indexExprType, shaderType));
                ssboGroup->addChild(new BlockArrayIndexingCase(m_testCtx, name.c_str(), indexExprDesc, BLOCKTYPE_BUFFER,
                                                               indexExprType, shaderType));
                ssboStorageBufGroup->addChild(new BlockArrayIndexingCase(
                    m_testCtx, name.c_str(), indexExprDesc, BLOCKTYPE_BUFFER, indexExprType, shaderType,
