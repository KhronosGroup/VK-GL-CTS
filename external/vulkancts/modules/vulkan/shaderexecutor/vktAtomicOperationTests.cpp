...
std::string dataType2Str(DataType type)
{
    static const char *const s_names[] = {
        "int", "uint", "float", "int64_t", "uint64_t", "double",
    };
    ...
    for (int elementNdx = 0; elementNdx < NUM_ELEMENTS / 2; elementNdx++)
    {
        ...
        if (!exp[0].compare(resIo, resOutput0, resOutput1) && !exp[1].compare(resIo, resOutput0, resOutput1))
        {
            ...
            errorMessage << "ERROR: Result value check failed at index " << elementNdx
                         ...
                         << ", Output0 = " << exp[0].m_output[0] << ", Output1 = " << exp[0].m_output[1]
                         << ", or InOut = " << exp[1].m_inout << ", Output0 = " << exp[1].m_output[0]
                         << ", Output1 = " << exp[1].m_output[1] << ". Got: InOut = " << resIo
                         << ", Output0 = " << resOutput0 << ", Output1 = " << resOutput1
...
tcu::TestStatus AtomicOperationCaseInstance::iterate(void)
{
    ...
    const int numWorkGroups =
        ((m_shaderType.getMemoryType() == AtomicMemoryType::SHARED) ? 1 : static_cast<int>(NUM_ELEMENTS));
...
{
public:
    AtomicOperationCase(tcu::TestContext &testCtx, const char *name, const char *description, AtomicShaderType type,
                        DataType dataType, AtomicOperation atomicOp);
...
AtomicOperationCase::AtomicOperationCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                         AtomicShaderType shaderType, DataType dataType, AtomicOperation atomicOp)
...
void AtomicOperationCase::createShaderSpec(void)
{
    ...
    {
        nonVertexShaderTemplateStream << "int idx = atomicAdd(buf.data.index, 1);\n"
                                      << "buf.data.outputValues[idx] = ${ATOMICOP}(buf.data.inoutValues[idx % "
                                         "(${N}/2)], ${COMPARE_ARG}buf.data.inputValues[idx]);\n";
    }
    ...
    {
        extensions << "#extension GL_EXT_shader_atomic_float : enable\n"
            ...
            for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(shaderTypes); shaderTypeNdx++)
            {
                ...
                for (int memoryTypeNdx = 0; memoryTypeNdx < DE_LENGTH_OF_ARRAY(kMemoryTypes); ++memoryTypeNdx)
                {
                    ...
                    atomicOperationTestsGroup->addChild(new AtomicOperationCase(
                        testCtx, name.c_str(), description.c_str(),
