...
{
public:
    AtomicOperationCaseInstance(Context &context, const ShaderSpec &shaderSpec, glu::ShaderType shaderType,
...
AtomicOperationCaseInstance::AtomicOperationCaseInstance(Context &context, const ShaderSpec &shaderSpec,
                                                         glu::ShaderType shaderType, DataType dataType,
    ...
    if ((m_dataType == DATA_TYPE_INT64) || (m_dataType == DATA_TYPE_UINT64))
    {
        ...
        if (shaderAtomicInt64Features.shaderBufferInt64Atomics == VK_FALSE)
        {
            TCU_THROW(NotSupportedError,
                      "VkShaderAtomicInt64: 64-bit unsigned and signed integer atomic operations not supported");
        }
    }
}
...
tcu::TestStatus AtomicOperationCaseInstance::iterate(void)
{
    ...
    const VkDescriptorPoolSize poolSizes[]    = {{VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1u}};
...
{
public:
    AtomicOperationCase(tcu::TestContext &testCtx, const char *name, const char *description, glu::ShaderType type,
                        DataType dataType, AtomicOperation atomicOp);
...
AtomicOperationCase::AtomicOperationCase(tcu::TestContext &testCtx, const char *name, const char *description,
                                         glu::ShaderType shaderType, DataType dataType, AtomicOperation atomicOp)
...
{
    const tcu::StringTemplate shaderTemplateGlobal("${EXTENSIONS}\n"
    ...
    const tcu::StringTemplate vertexShaderTemplateSrc("int idx = gl_VertexIndex;\n"
                                                      ...
                                                      "{\n"
                                                      "    buf.outputValues[idx] = ${ATOMICOP}(buf.inoutValues[idx % "
                                                      "(${N}/2)], ${COMPARE_ARG}buf.inputValues[idx]);\n"
                                                      "}\n");
...
void addAtomicOperationTests(tcu::TestCaseGroup *atomicOperationTestsGroup)
{
    ...
    } dataSign[] = {{DATA_TYPE_INT32, "signed", "Tests using signed data (int)"},
        ...
        for (int signNdx = 0; signNdx < DE_LENGTH_OF_ARRAY(dataSign); signNdx++)
        {
            for (int shaderTypeNdx = 0; shaderTypeNdx < DE_LENGTH_OF_ARRAY(shaderTypes); shaderTypeNdx++)
            {
                const std::string description =
                    std::string("Tests atomic operation ") + atomicOp2Str(atomicOp[opNdx].value) + std::string(".");
                std::string name = std::string(atomicOp[opNdx].name) + "_" + std::string(dataSign[signNdx].name) + "_" +
                                   std::string(shaderTypes[shaderTypeNdx].name);
                atomicOperationTestsGroup->addChild(
                    new AtomicOperationCase(testCtx, name.c_str(), description.c_str(), shaderTypes[shaderTypeNdx].type,
