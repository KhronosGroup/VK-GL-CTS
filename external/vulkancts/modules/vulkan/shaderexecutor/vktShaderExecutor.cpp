    ...
    for (vector<Symbol>::const_iterator symIter = outputs.begin(); symIter != outputs.end(); ++symIter)
    {
        if (glu::isDataTypeFloatType(symIter->varType.getBasicType()))
        {
            if (glu::isDataTypeVector(symIter->varType.getBasicType()))
            {
                ...
                {
                    src << "\tpacked_" << symIter->name << "[" << i << "] = uintBitsToFloat(packFloat2x16(f16vec2("
                        << symIter->name << "[" << i << "], -1.0)));\n";
                }
            }
            else if (glu::isDataTypeMatrix(symIter->varType.getBasicType()))
            {
                ...
                for (int i = 0; i < maxRow; i++)
                    ...
                    {
                        src << "\tpacked_" << symIter->name << "[" << i << "][" << j
                            << "] = uintBitsToFloat(packFloat2x16(f16vec2(" << symIter->name << "[" << i << "][" << j
                            << "], -1.0)));\n";
                    }
            }
            else
            {
                src << "\tpacked_" << symIter->name << " = uintBitsToFloat(packFloat2x16(f16vec2(" << symIter->name
                    << ", -1.0)));\n";
            }
        }
    }
...
static std::string generatePassthroughVertexShader(const ShaderSpec &shaderSpec, const char *inputPrefix,
                                                   const char *outputPrefix)
{
    ...
    for (vector<Symbol>::const_iterator input = shaderSpec.inputs.begin(); input != shaderSpec.inputs.end(); ++input)
    {
        ...
        src << "layout(location = " << location << ") in " << glu::declare(input->varType, inputPrefix + input->name)
            << ";\n"
            << "layout(location = " << location - 1 << ") flat out "
            << glu::declare(input->varType, outputPrefix + input->name) << ";\n";
    }
                                        ...
                                        const std::string &outputPrefix)
{
    ...
    for (vector<Symbol>::const_iterator input = shaderSpec.inputs.begin(); input != shaderSpec.inputs.end();
         ++input, ++locationNumber)
    {
        src << "layout(location = " << locationNumber << ") in "
            << glu::declare(input->varType, inputPrefix + input->name) << ";\n";
    }

    locationNumber = 0;
    for (vector<Symbol>::const_iterator output = shaderSpec.outputs.begin(); output != shaderSpec.outputs.end();
         ++output, ++locationNumber)
    {
        ...
        if (glu::isDataTypeBoolOrBVec(output->varType.getBasicType()))
        {
            ...
            src << "layout(location = " << locationNumber << ") flat out "
                << glu::declare(intType, outputPrefix + output->name) << ";\n";
        }
        else
            src << "layout(location = " << locationNumber << ") flat out "
                << glu::declare(output->varType, outputPrefix + output->name) << ";\n";
    }
                                        ...
                                        const bool isInput16Bit = false)
{
    ...
    for (vector<Symbol>::const_iterator output = shaderSpec.outputs.begin(); output != shaderSpec.outputs.end();
         ++output)
    {
        const std::string packPrefix =
            (isInput16Bit && glu::isDataTypeFloatType(output->varType.getBasicType())) ? "packed_" : "";
        ...
        else if (glu::isDataTypeMatrix(output->varType.getBasicType()))
        {
            ...
            for (int vecNdx = 0; vecNdx < numVecs; ++vecNdx)
                if (useIntOutputs)
                    ...
                    src << "\t" << outputPrefix << output->name << "_" << vecNdx << " = " << packPrefix << valuePrefix
                        << output->name << "[" << vecNdx << "];\n";
        }
        else if (glu::isDataTypeBoolOrBVec(output->varType.getBasicType()))
        {
                                                     ...
                                                     const std::string &inputPrefix, const std::string &outputPrefix)
{
         ...
         ++output, ++locationNumber)
    {
        if (glu::isDataTypeBoolOrBVec(output->varType.getBasicType()))
        {
            ...
            src << "layout(location = " << locationNumber << ") flat in "
                << glu::declare(intType, inputPrefix + output->name) << ";\n";
        }
        else
            src << "layout(location = " << locationNumber << ") flat in "
                << glu::declare(output->varType, inputPrefix + output->name) << ";\n";
    }
...
static std::string generateGeometryShader(const ShaderSpec &shaderSpec, const std::string &inputPrefix,
                                          const std::string &outputPrefix, const bool pointSizeSupported)
{
    DE_ASSERT(!inputPrefix.empty() && !outputPrefix.empty());
...
{
public:
    FragmentOutExecutor(Context &context, glu::ShaderType shaderType, const ShaderSpec &shaderSpec,
...
private:
    void bindAttributes(int numValues, const void *const *inputs);

    void addAttribute(uint32_t bindingLocation, VkFormat format, uint32_t sizePerElement, uint32_t count,
                      const void *dataPtr);
...
FragmentOutExecutor::FragmentOutExecutor(Context &context, glu::ShaderType shaderType, const ShaderSpec &shaderSpec,
                                         VkDescriptorSetLayout extraResourcesLayout)
...
static tcu::TextureFormat getRenderbufferFormatForOutput(const glu::VarType &outputType, bool useIntOutputs)
{
    ...
    case glu::TYPE_FLOAT16:
        channelType = useIntOutputs ? tcu::TextureFormat::UNSIGNED_INT32 : tcu::TextureFormat::HALF_FLOAT;
        break;
    default:
...
void FragmentOutExecutor::addAttribute(uint32_t bindingLocation, VkFormat format, uint32_t sizePerElement,
                                       uint32_t count, const void *dataPtr)
{
    ...
    const uint32_t binding                                   = (uint32_t)m_vertexBindingDescriptions.size();
    const VkVertexInputBindingDescription bindingDescription = {binding, sizePerElement, VK_VERTEX_INPUT_RATE_VERTEX};
    ...
    de::MovePtr<Allocation> alloc = m_context.getDefaultAllocator().allocate(
        getBufferMemoryRequirements(vk, vkDevice, *buffer), MemoryRequirement::HostVisible);
...
void FragmentOutExecutor::bindAttributes(int numValues, const void *const *inputs)
{
    ...
    for (int inputNdx = 0; inputNdx < (int)m_shaderSpec.inputs.size(); inputNdx++)
    {
        ...
        {
            addAttribute((uint32_t)m_vertexBindingDescriptions.size(), format, elementSize * vecSize, numValues, ptr);
        }
    }
}
...
static Move<VkDescriptorSet> allocateSingleDescriptorSet(const DeviceInterface &vkd, VkDevice device,
                                  ...
                                  VkDescriptorSet extraResources)
{
    ...
    const uint32_t renderSizeX = de::min(static_cast<uint32_t>(128), (uint32_t)numValues);
    const uint32_t renderSizeY =
        ((uint32_t)numValues / renderSizeX) + (((uint32_t)numValues % renderSizeX != 0) ? 1u : 0u);
    ...
    // Bind attributes
    addAttribute(0u, VK_FORMAT_R32G32_SFLOAT, sizeof(tcu::Vec2), (uint32_t)positions.size(), &positions[0]);
    ...
    // Create color images
    {
        ...
        for (int outNdx = 0; outNdx < (int)m_outputLayout.locationSymbols.size(); ++outNdx)
        {
            ...
            const VkFormat colorFormat =
                (isDouble ? VK_FORMAT_R64G64B64A64_SFLOAT :
                            (isFloat16b ? VK_FORMAT_R16G16B16A16_SFLOAT :
                                          (isFloat ? VK_FORMAT_R32G32B32A32_SFLOAT :
                                                     (isSigned || isBool ? VK_FORMAT_R32G32B32A32_SINT :
                                                                           VK_FORMAT_R32G32B32A32_UINT))));

            {
                const VkFormatProperties formatProperties = getPhysicalDeviceFormatProperties(
                    m_context.getInstanceInterface(), m_context.getPhysicalDevice(), colorFormat);
            ...
            {
                de::MovePtr<Allocation> colorImageAlloc = memAlloc.allocate(
                    getImageMemoryRequirements(vk, vkDevice, *((const VkImage *)colorImages.back().get())),
                    MemoryRequirement::Any);
                VK_CHECK(vk.bindImageMemory(vkDevice, colorImages.back().get()->get(), colorImageAlloc->getMemory(),
                                            colorImageAlloc->getOffset()));
                ...
                const VkAttachmentReference colorAttachmentReference = {
                    (uint32_t)(colorImages.size() - 1),      // uint32_t attachment;
        ...
        if (useGeometryShader)
        {
            if (m_context.getDeviceFeatures().shaderTessellationAndGeometryPointSize)
                geometryShaderModule =
                    createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("geom_point_size"), 0);
            else
    ...
    // Create pipeline
    {
        const VkPipelineVertexInputStateCreateInfo vertexInputStateParams = {
            ...
            (uint32_t)m_vertexAttributeDescriptions.size(), // uint32_t attributeCount;
    ...
    // Create command buffer
    {
        ...
        vk.cmdDraw(*cmdBuffer, (uint32_t)positions.size(), 1u, 0u, 0u);
    ...
    // Read back result and output
    {
        ...
        Move<VkCommandPool> copyCmdPool =
        ...
        for (int outNdx = 0; outNdx < (int)m_shaderSpec.outputs.size(); ++outNdx)
        {
            ...
            for (int locNdx = 0; locNdx < outNumLocs; ++locNdx)
            {
                ...
                const de::UniquePtr<Allocation> readImageBufferMemory(memAlloc.allocate(
                ...
                // Copy image to buffer
                {
                    ...
                    Move<VkCommandBuffer> copyCmdBuffer =
                        allocateCommandBuffer(vk, vkDevice, *copyCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);

                    beginCommandBuffer(vk, *copyCmdBuffer);
                    vk.cmdCopyImageToBuffer(*copyCmdBuffer, colorImages[outLocation + locNdx].get()->get(),
                ...
                const tcu::ConstPixelBufferAccess resultAccess(resultFormat, renderSize.x(), renderSize.y(), 1,
                ...
                {
                    uint16_t *dstPtrBase = static_cast<uint16_t *>(outputs[outNdx]);
                    if (outSize == 4 && outNumLocs == 1)
                        deMemcpy(dstPtrBase, tmpBuf.getAccess().getDataPtr(),
                                 numValues * outVecSize * sizeof(uint16_t));
                    else
                    {
                        for (int valNdx = 0; valNdx < numValues; valNdx++)
                        {
                            const uint16_t *srcPtr = (const uint16_t *)tmpBuf.getAccess().getDataPtr() + valNdx * 4;
                            uint16_t *dstPtr       = &dstPtrBase[outSize * valNdx + outVecSize * locNdx];
                            deMemcpy(dstPtr, srcPtr, outVecSize * sizeof(uint16_t));
                        }
                    }
                }
                else
                {
                    ...
                    if (outSize == 4 && outNumLocs == 1)
                        deMemcpy(dstPtrBase, tmpBuf.getAccess().getDataPtr(),
                                 numValues * outVecSize * sizeof(uint32_t));
                    else
                    {
                        for (int valNdx = 0; valNdx < numValues; valNdx++)
                        {
...
VertexShaderExecutor::VertexShaderExecutor(Context &context, const ShaderSpec &shaderSpec,
                                           VkDescriptorSetLayout extraResourcesLayout)
...
void VertexShaderExecutor::generateSources(const ShaderSpec &shaderSpec, SourceCollections &programCollection)
{
    ...
    programCollection.glslSources.add("vert")
        << glu::VertexSource(generateVertexShader(shaderSpec, "a_", "vtx_out_")) << shaderSpec.buildOptions;
    ...
    programCollection.glslSources.add("frag") << glu::FragmentSource(generatePassthroughFragmentShader(
                                                     shaderSpec, false, outputLayout.locationMap, "vtx_out_", "o_"))
                                              << shaderSpec.buildOptions;
}
...
GeometryShaderExecutor::GeometryShaderExecutor(Context &context, const ShaderSpec &shaderSpec,
                                               VkDescriptorSetLayout extraResourcesLayout)
...
void GeometryShaderExecutor::generateSources(const ShaderSpec &shaderSpec, SourceCollections &programCollection)
{
    ...
    programCollection.glslSources.add("vert")
        << glu::VertexSource(generatePassthroughVertexShader(shaderSpec, "a_", "vtx_out_")) << shaderSpec.buildOptions;

    programCollection.glslSources.add("geom")
        << glu::GeometrySource(generateGeometryShader(shaderSpec, "vtx_out_", "geom_out_", false))
        << shaderSpec.buildOptions;
    programCollection.glslSources.add("geom_point_size")
        << glu::GeometrySource(generateGeometryShader(shaderSpec, "vtx_out_", "geom_out_", true))
        << shaderSpec.buildOptions;

    /* \todo [2015-09-18 rsipka] set useIntOutputs parameter if needed. */
    programCollection.glslSources.add("frag") << glu::FragmentSource(generatePassthroughFragmentShader(
                                                     shaderSpec, false, outputLayout.locationMap, "geom_out_", "o_"))
                                              << shaderSpec.buildOptions;
}
...
FragmentShaderExecutor::FragmentShaderExecutor(Context &context, const ShaderSpec &shaderSpec,
                                               VkDescriptorSetLayout extraResourcesLayout)
...
void FragmentShaderExecutor::generateSources(const ShaderSpec &shaderSpec, SourceCollections &programCollection)
{
    ...
    programCollection.glslSources.add("frag")
        << glu::FragmentSource(generateFragmentShader(shaderSpec, false, outputLayout.locationMap, "vtx_out_", "o_"))
        << shaderSpec.buildOptions;
}
...
protected:
    ...
    VkBuffer getInputBuffer(void) const
    {
        return *m_inputBuffer;
    }
    VkBuffer getOutputBuffer(void) const
    {
...
private:
    ...
    static void copyToBuffer(const glu::VarType &varType, const VarLayout &layout, int numValues,
                             const void *srcBasePtr, void *dstBasePtr, bool packFloat16Bit);
...
void BufferIoExecutor::computeVarLayout(const std::vector<Symbol> &symbols, std::vector<VarLayout> *layout)
{
    ...
    for (size_t varNdx = 0; varNdx < symbols.size(); varNdx++)
    {
        ...
        if (glu::isDataTypeScalarOrVector(basicType))
        {
            ...
            const uint32_t size =
                (uint32_t)glu::getDataTypeScalarSize(basicType) *
                (isDataTypeDoubleType(basicType) ?
                     (int)(sizeof(uint64_t)) :
                     (isDataTypeFloat16OrVec(basicType) ? (int)sizeof(uint16_t) : (int)sizeof(uint32_t)));
        ...
        else if (glu::isDataTypeMatrix(basicType))
        {
            ...
            const glu::DataType vecType =
                glu::getDataTypeVector(glu::getDataTypeScalarType(basicType), glu::getDataTypeMatrixNumRows(basicType));
...
void BufferIoExecutor::generateExecBufferIo(std::ostream &src, const ShaderSpec &spec, const char *invocationNdxName)
{
    ...
    for (vector<Symbol>::const_iterator symIter = spec.inputs.begin(); symIter != spec.inputs.end(); ++symIter)
    {
        ...
        src << "\t" << tname << " " << symIter->name << " = " << tname << "(inputs[" << invocationNdxName << "]."
            << symIter->name << ");\n";
    }
                                    ...
                                    const void *srcBasePtr, void *dstBasePtr, bool packFloat16Bit)
{
    if (varType.isBasicType())
    {
        ...
        const int size                = (glu::isDataTypeDoubleType(basicType) ?
                                             (int)sizeof(uint64_t) :
                                             (glu::isDataTypeFloat16OrVec(basicType) ? (int)sizeof(uint16_t) : (int)sizeof(uint32_t)));
                                      ...
                                      const void *srcBasePtr, void *dstBasePtr)
{
    if (varType.isBasicType())
    {
        ...
        for (int elemNdx = 0; elemNdx < numValues; elemNdx++)
        {
            ...
            {
                const int size =
                    (glu::isDataTypeDoubleType(basicType) ?
                         (int)sizeof(uint64_t) :
                         (glu::isDataTypeFloat16OrVec(basicType) ? (int)sizeof(uint16_t) : (int)sizeof(uint32_t)));
                const int srcOffset =
                    layout.offset + layout.stride * elemNdx + (isMatrix ? layout.matrixStride * vecNdx : 0);
...
void BufferIoExecutor::initBuffers(int numValues)
{
    ...
    m_inputAlloc =
        memAlloc.allocate(getBufferMemoryRequirements(vk, vkDevice, *m_inputBuffer), MemoryRequirement::HostVisible);
    ...
    m_outputAlloc =
        memAlloc.allocate(getBufferMemoryRequirements(vk, vkDevice, *m_outputBuffer), MemoryRequirement::HostVisible);
...
std::string scalarComparison(const std::string operation, const int operationNdx, const glu::DataType type,
                             const std::string &outputType, const int scalarSize)
{
    std::ostringstream src;
    std::string boolType;
    ...
    case glu::TYPE_DOUBLE:
        src << "\n"
            ...
            << "OpBranchConditional %operation_result_" << operationNdx << " %label_IF_" << operationNdx << " %IF_"
            << operationNdx << "\n"
            << "%label_IF_" << operationNdx << " = OpLabel\n"
            << "%operation_val_" << operationNdx << " = OpLoad %i32 %operation\n"
            << "%out_val_" << operationNdx << " = OpLoad %i32 %out0\n"
            << "%add_if_" << operationNdx << " = OpIAdd %i32 %out_val_" << operationNdx << " %operation_val_"
            << operationNdx << "\n"
    ...
    src << "\n"
        ...
        << "%ivec_result_" << operationNdx << " = OpSelect " << outputType << " %operation_result_" << operationNdx
        << " %c_" << &outputType[1] << "_1 %c_" << &outputType[1] << "_0\n"
        << "%operation_val_" << operationNdx << " = OpLoad %i32 %operation\n";
    ...
    src << "%toAdd" << operationNdx << " = OpIMul " << outputType << " %ivec_result_" << operationNdx
        << " %operation_vec_" << operationNdx << "\n"
        << "%out_val_" << operationNdx << " = OpLoad " << outputType << " %out0\n"

        << "%add_if_" << operationNdx << " = OpIAdd " << outputType << " %out_val_" << operationNdx << " %toAdd"
        << operationNdx << "\n"
        << "OpStore %out0 %add_if_" << operationNdx << "\n";
...
{
    static const std::string COMPARE_OPERATIONS[] = {"OpFOrdEqual",
                                                     "OpFOrdGreaterThan",
                                                     "OpFOrdLessThan",
    ...
    const std::string packType =
        spec.packFloat16Bit ? getTypeSpirv(getDataTypeFloat16Scalars(spec.inputs[0].varType.getBasicType())) : "";
    ...
    if (isMediump)
    {
        ...
        {
            src << "OpMemberDecorate %SSB0_IN " << i
                << " RelaxedPrecision\n"
                   "OpDecorate %in"
                << i
                << " RelaxedPrecision\n"
                   "OpDecorate %src_val_0_"
                << i
                << " RelaxedPrecision\n"
                   "OpDecorate %in"
                << i << "_val RelaxedPrecision\n";
        }
    ...
    if (useF32Types)
        src << "%c_f32_0 = OpConstant %f32 0\n"
               "%c_f32_1 = OpConstant %f32 1\n";

    if (useF16Types)
        src << "%c_f16_0 = OpConstant %f16 0\n"
               "%c_f16_1 = OpConstant %f16 1\n"
               "%c_f16_minus1 = OpConstant %f16 -0x1p+0";

    if (useF64Types)
        src << "%c_f64_0 = OpConstant %f64 0\n"
    ...
    if (useF32Types)
        src << "%c_v2f32_0 = OpConstantComposite %v2f32 %c_f32_0 %c_f32_0\n"
    ...
    if (useF16Types)
        src << "%c_v2f16_0 = OpConstantComposite %v2f16 %c_f16_0 %c_f16_0\n"
    ...
    if (useF64Types)
        src << "%c_v2f64_0 = OpConstantComposite %v2f64 %c_f64_0 %c_f64_0\n"
    ...
    src << "%up_SSB0_IN = OpTypeRuntimeArray %SSB0_IN\n"
    ...
    src << "%up_SSB0_OUT = OpTypeRuntimeArray %SSB0_OUT\n"
    ...
    for (size_t i = 0; i < outputTypes.size(); ++i)
        src << "%out" << i << " = OpVariable "
            << (packFloatRes[i] ? std::string("%fp_f16") : std::string("%fp_out_") + de::toString(i)) << " Function\n";
    ...
    {
        src << "\n"
            << "%src_ptr_0_" << inputNdx << " = OpAccessChain %up_float %ssbo_src %c_i32_0 %invocationNdx_val %c_i32_"
            << inputNdx << "\n"
        ...
        if (spec.packFloat16Bit)
        {
            if (spec.inputs[inputNdx].varType.getScalarSize() > 1)
            {
                ...
                {
                    src << "%src_val_0_" << inputNdx << "_" << i << " = OpCompositeExtract %u32 %src_val_0_" << inputNdx
                        << " " << i
                        << "\n"
                           "%val_v2f16_0_"
                        << inputNdx << "_" << i << " = OpBitcast %v2f16 %src_val_0_" << inputNdx << "_" << i
                        << "\n"
                           "%val_f16_0_"
                        << inputNdx << "_" << i << " = OpCompositeExtract %f16 %val_v2f16_0_" << inputNdx << "_" << i
                        << " 0\n";
                }
            ...
            else
            {
                src << "%val_v2f16_0_" << inputNdx << " = OpBitcast %v2f16 %src_val_0_" << inputNdx
                    << "\n"
                       "%val_f16_0_"
                    << inputNdx << " = OpCompositeExtract %f16 %val_v2f16_0_" << inputNdx << " 0\n";
        ...
        src << "%in" << inputNdx << "_val = OpLoad " << (spec.packFloat16Bit ? packType : inputTypes[inputNdx])
            << " %in" << inputNdx << "\n";
    }
    ...
    case SPIRV_CASETYPE_COMPARE:
        for (int operationNdx = 0; operationNdx < DE_LENGTH_OF_ARRAY(COMPARE_OPERATIONS); ++operationNdx)
        {
            src << scalarComparison(COMPARE_OPERATIONS[operationNdx], operationNdx,
                                    spec.inputs[0].varType.getBasicType(), outputTypes[0],
    ...
    for (size_t outputNdx = 0; outputNdx < outputTypes.size(); ++outputNdx)
    {
        src << "\n"
               "%out_val_final_"
            << outputNdx << " = OpLoad " << (packFloatRes[outputNdx] ? packType : outputTypes[outputNdx]) << " %out"
            << outputNdx
            << "\n"
               "%ssbo_dst_ptr_"
            << outputNdx << " = OpAccessChain %up_out_" << outputNdx << " %ssbo_dst %c_i32_0 %invocationNdx_val %c_i32_"
            << outputNdx << "\n";
        ...
        if (packFloatRes[outputNdx])
        {
            if (spec.outputs[outputNdx].varType.getScalarSize() > 1)
            {
                ...
                {
                    src << "%out_val_final_" << outputNdx << "_" << i << " = OpCompositeExtract %f16 %out_val_final_"
                        << outputNdx << " " << i << "\n";
                    src << "%out_composite_" << outputNdx << "_" << i
                        << " = OpCompositeConstruct %v2f16 %out_val_final_" << outputNdx << "_" << i
                        << " %c_f16_minus1\n";
                    src << "%u32_val_" << outputNdx << "_" << i << " = OpBitcast %u32 %out_composite_" << outputNdx
                        << "_" << i << "\n";
                }
            ...
            {
                src << "%out_composite_" << outputNdx << " = OpCompositeConstruct %v2f16 %out_val_final_" << outputNdx
                    << " %c_f16_minus1\n"
                       "%out_result_"
                    << outputNdx << " = OpBitcast " << outputTypes[outputNdx] << " %out_composite_" << outputNdx
                    << "\n"
                       "OpStore %ssbo_dst_ptr_"
                    << outputNdx << " %out_result_" << outputNdx << "\n";
            }
        }
        else
        {
...
void ComputeShaderExecutor::generateSources(const ShaderSpec &shaderSpec, SourceCollections &programCollection)
{
    if (shaderSpec.spirvCase != SPIRV_CASETYPE_NONE)
        programCollection.spirvAsmSources.add("compute")
            ...
            << generateComputeShader(shaderSpec);
    else
        programCollection.glslSources.add("compute")
            << glu::ComputeSource(generateComputeShader(shaderSpec)) << shaderSpec.buildOptions;
}
                                    ...
                                    VkDescriptorSet extraResources)
{
    ...
    uploadInputBuffer(inputs, numValues,
                      m_shaderSpec.packFloat16Bit && (m_shaderSpec.spirvCase != SPIRV_CASETYPE_NONE));
    ...
    while (curOffset < numValues)
    {
        ...
        // Update descriptors
        {
            ...
            descriptorSetUpdateBuilder.writeSingle(
                *descriptorSet, vk::DescriptorSetUpdateBuilder::Location::binding((uint32_t)OUTPUT_BUFFER_BINDING),
            ...
            if (inputStride)
            {
                ...
                descriptorSetUpdateBuilder.writeSingle(
                    *descriptorSet, vk::DescriptorSetUpdateBuilder::Location::binding((uint32_t)INPUT_BUFFER_BINDING),
                    VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &inputDescriptorBufferInfo);
            }
...
void MeshTaskShaderExecutor::generateSources(const ShaderSpec &shaderSpec, SourceCollections &programCollection,
                                             bool useTask)
{
    DE_ASSERT(shaderSpec.spirvCase == SPIRV_CASETYPE_NONE);
    programCollection.glslSources.add("mesh")
        << glu::MeshSource(generateMeshShader(shaderSpec, useTask)) << shaderSpec.buildOptions;
    if (useTask)
        programCollection.glslSources.add("task")
            << glu::TaskSource(generateTaskShader(shaderSpec)) << shaderSpec.buildOptions;
}

void MeshTaskShaderExecutor::execute(int numValues, const void *const *inputs, void *const *outputs,
                                     VkDescriptorSet extraResources)
{
    ...
    const auto pipelineLayout =
        makePipelineLayout(vk, vkDevice, static_cast<uint32_t>(setLayouts.size()), de::dataOrNull(setLayouts));

    // Create shaders
    const auto meshShaderModule = createShaderModule(vk, vkDevice, binaries.get("mesh"));
    const auto taskShaderModule =
        (useTask ? createShaderModule(vk, vkDevice, binaries.get("task")) : Move<VkShaderModule>());

    // Render pass and framebuffer.
    const auto fbExtent   = makeExtent2D(1u, 1u);
    const auto renderPass = makeRenderPass(vk, vkDevice);
    const auto framebuffer =
        makeFramebuffer(vk, vkDevice, renderPass.get(), 0u, nullptr, fbExtent.width, fbExtent.height);
    ...
    const uint32_t inputStride       = getInputStride();
    const uint32_t outputStride      = getOutputStride();
    const auto outputBufferBinding =
        DescriptorSetUpdateBuilder::Location::binding(static_cast<uint32_t>(OUTPUT_BUFFER_BINDING));
    const auto inputBufferBinding =
        DescriptorSetUpdateBuilder::Location::binding(static_cast<uint32_t>(INPUT_BUFFER_BINDING));
    int curOffset = 0;
    ...
    while (curOffset < numValues)
    {
        ...
        // Update descriptors
        {
            ...
            const auto outputDescriptorBufferInfo =
                makeDescriptorBufferInfo(m_outputBuffer.get(), curOffset * outputStride, numToExec * outputStride);
            descriptorSetUpdateBuilder.writeSingle(descriptorSet.get(), outputBufferBinding,
                                                   VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &outputDescriptorBufferInfo);

            if (inputStride)
            {
                const auto inputDescriptorBufferInfo =
                    makeDescriptorBufferInfo(m_inputBuffer.get(), curOffset * inputStride, numToExec * inputStride);
                descriptorSetUpdateBuilder.writeSingle(descriptorSet.get(), inputBufferBinding,
                                                       VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &inputDescriptorBufferInfo);
            }
        ...
        const auto bufferBarrier = makeBufferMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT,
                                                           m_outputBuffer.get(), 0ull, VK_WHOLE_SIZE);
...
public:
    ...
    void renderTess(uint32_t numValues, uint32_t vertexCount, uint32_t patchControlPoints,
                    VkDescriptorSet extraResources);
...
void TessellationExecutor::renderTess(uint32_t numValues, uint32_t vertexCount, uint32_t patchControlPoints,
                                      VkDescriptorSet extraResources)
{
    ...
    // Create color image
    {
        ...
        // Allocate and bind color image memory
        colorImageAlloc =
            memAlloc.allocate(getImageMemoryRequirements(vk, vkDevice, *colorImage), MemoryRequirement::Any);
    ...
    // Create render pass
    {
        ...
        const VkAttachmentDescription attachments[1] = {colorAttachmentDescription};
    ...
    // Create shader modules
    {
        ...
        tessControlShaderModule =
            createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("tess_control"), 0);
...
TessControlExecutor::TessControlExecutor(Context &context, const ShaderSpec &shaderSpec,
                                         VkDescriptorSetLayout extraResourcesLayout)
...
{
    programCollection.glslSources.add("vert")
        << glu::VertexSource(generateVertexShaderForTess()) << shaderSpec.buildOptions;
    programCollection.glslSources.add("tess_control")
        << glu::TessellationControlSource(generateTessControlShader(shaderSpec)) << shaderSpec.buildOptions;
    programCollection.glslSources.add("tess_eval")
        << glu::TessellationEvaluationSource(generateEmptyTessEvalShader()) << shaderSpec.buildOptions;
    programCollection.glslSources.add("frag")
        << glu::FragmentSource(generateEmptyFragmentSource()) << shaderSpec.buildOptions;
}

void TessControlExecutor::execute(int numValues, const void *const *inputs, void *const *outputs,
                                  VkDescriptorSet extraResources)
{
    const uint32_t patchSize = 3;
...
TessEvaluationExecutor::TessEvaluationExecutor(Context &context, const ShaderSpec &shaderSpec,
                                               VkDescriptorSetLayout extraResourcesLayout)
    : TessellationExecutor(context, shaderSpec, extraResourcesLayout)
{
}
...
void TessEvaluationExecutor::generateSources(const ShaderSpec &shaderSpec, SourceCollections &programCollection)
{
    programCollection.glslSources.add("vert")
        << glu::VertexSource(generateVertexShaderForTess()) << shaderSpec.buildOptions;
    programCollection.glslSources.add("tess_control")
        << glu::TessellationControlSource(generatePassthroughTessControlShader()) << shaderSpec.buildOptions;
    programCollection.glslSources.add("tess_eval")
        << glu::TessellationEvaluationSource(generateTessEvalShader(shaderSpec)) << shaderSpec.buildOptions;
    programCollection.glslSources.add("frag")
        << glu::FragmentSource(generateEmptyFragmentSource()) << shaderSpec.buildOptions;
}
                                     ...
                                     VkDescriptorSet extraResources)
{
    ...
    renderTess((uint32_t)alignedValues, (uint32_t)alignedValues, (uint32_t)patchSize, extraResources);
...
{
    for (vector<Symbol>::const_iterator symIter = m_shaderSpec.inputs.begin(); symIter != m_shaderSpec.inputs.end();
         ++symIter)
    {
        if (glu::isDataTypeFloat16OrVec(symIter->varType.getBasicType()))
            return true;
...
{
    for (vector<Symbol>::const_iterator symIter = m_shaderSpec.outputs.begin(); symIter != m_shaderSpec.outputs.end();
         ++symIter)
    {
        if (glu::isDataTypeFloat16OrVec(symIter->varType.getBasicType()))
            return true;
...
void generateSources(glu::ShaderType shaderType, const ShaderSpec &shaderSpec, vk::SourceCollections &dst)
{
    ...
    case glu::SHADERTYPE_VERTEX:
        VertexShaderExecutor::generateSources(shaderSpec, dst);
        break;
    case glu::SHADERTYPE_TESSELLATION_CONTROL:
        TessControlExecutor::generateSources(shaderSpec, dst);
        break;
    case glu::SHADERTYPE_TESSELLATION_EVALUATION:
        TessEvaluationExecutor::generateSources(shaderSpec, dst);
        break;
    case glu::SHADERTYPE_GEOMETRY:
        GeometryShaderExecutor::generateSources(shaderSpec, dst);
        break;
    case glu::SHADERTYPE_FRAGMENT:
        FragmentShaderExecutor::generateSources(shaderSpec, dst);
        break;
    case glu::SHADERTYPE_COMPUTE:
        ComputeShaderExecutor::generateSources(shaderSpec, dst);
        break;
#ifndef CTS_USES_VULKANSC
    case glu::SHADERTYPE_MESH:
        MeshTaskShaderExecutor::generateSources(shaderSpec, dst, false /*useTask*/);
        break;
    case glu::SHADERTYPE_TASK:
        MeshTaskShaderExecutor::generateSources(shaderSpec, dst, true /*useTask*/);
        break;
#endif // CTS_USES_VULKANSC
    default:
                               ...
                               VkDescriptorSetLayout extraResourcesLayout)
{
    ...
    {
    case glu::SHADERTYPE_VERTEX:
        return new VertexShaderExecutor(context, shaderSpec, extraResourcesLayout);
    case glu::SHADERTYPE_TESSELLATION_CONTROL:
        return new TessControlExecutor(context, shaderSpec, extraResourcesLayout);
    case glu::SHADERTYPE_TESSELLATION_EVALUATION:
        return new TessEvaluationExecutor(context, shaderSpec, extraResourcesLayout);
    case glu::SHADERTYPE_GEOMETRY:
        return new GeometryShaderExecutor(context, shaderSpec, extraResourcesLayout);
    case glu::SHADERTYPE_FRAGMENT:
