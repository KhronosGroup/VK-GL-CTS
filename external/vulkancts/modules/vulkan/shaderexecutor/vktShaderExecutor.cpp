...
std::string generateSpirv(const ShaderSpec &spec, const bool are16Bit, const bool are64Bit, const bool isMediump)
{
    ...
    //input offset
    {
        ...
        for (vector<Symbol>::const_iterator symIter = spec.inputs.begin(); symIter != spec.inputs.end(); ++symIter)
        {
            ...
            offset += (scalarSize + ((scalarSize == 3) ? 1 : 0)) *
                      (isDataTypeDoubleType(symIter->varType.getBasicType()) ?
                           (int)sizeof(uint64_t) :
                           (isDataTypeFloat16OrVec(symIter->varType.getBasicType()) ? (int)sizeof(uint16_t) :
                                                                                      (int)sizeof(uint32_t)));
        }
    ...
    if (isMediump)
    {
        ...
        if (floatResult)
        {
            src << "OpDecorate %out RelaxedPrecision\n"
        ...
        if (spec.inputs[0].varType.getScalarSize() > 1)
        {
            ...
            for (int i = 0; i < spec.inputs[0].varType.getScalarSize(); ++i)
            {
                src << "%src_val_0_0_" << i << " = OpCompositeExtract %u32 %src_val_0_0 " << i
                    << "\n"
                       "%val_v2f16_0_0_"
                    << i << " = OpBitcast %v2f16 %src_val_0_0_" << i
                    << "\n"
                       "%val_f16_0_0_"
                    << i << " = OpCompositeExtract %f16 %val_v2f16_0_0_" << i << " 0\n";
            }
        ...
        if (spec.inputs[0].varType.getScalarSize() > 1)
        {
            ...
            for (int i = 0; i < spec.inputs[0].varType.getScalarSize(); ++i)
            {
                src << "%src_val_0_1_" << i << " = OpCompositeExtract %u32 %src_val_0_1 " << i
                    << "\n"
                       "%val_v2f16_0_1_"
                    << i << " = OpBitcast %v2f16 %src_val_0_1_" << i
                    << "\n"
                       "%val_f16_0_1_"
                    << i << " = OpCompositeExtract %f16 %val_v2f16_0_1_" << i << " 0\n";
            }
    ...
    src << "\n"
           ...
           "OpStore %out %c_"
        << (packFloatRes ? &packType[1] : &outputType[1])
        << "_0\n"
           "\n";

    if (spec.packFloat16Bit)
        src << "%in0_val = OpLoad " << packType
            << " %in0\n"
               "%in1_val = OpLoad "
            << packType << " %in1\n";
    else
        src << "%in0_val = OpLoad " << inputType1
            << " %in0\n"
               "%in1_val = OpLoad "
            << inputType2 << " %in1\n";
    ...
    case SPIRV_CASETYPE_COMPARE:
        for (int operationNdx = 0; operationNdx < DE_LENGTH_OF_ARRAY(COMPARE_OPERATIONS); ++operationNdx)
        {
            src << scalarComparison(COMPARE_OPERATIONS[operationNdx], operationNdx,
                                    spec.inputs[0].varType.getBasicType(), outputType,
    ...
    src << "\n"
           "%out_val_final = OpLoad "
        << (packFloatRes ? packType : outputType)
        << " %out\n"
    ...
    if (packFloatRes)
    {
        if (spec.inputs[0].varType.getScalarSize() > 1)
        {
            for (int i = 0; i < spec.inputs[0].varType.getScalarSize(); ++i)
            {
                ...
                src << "%out_composite_" << i << " = OpCompositeConstruct %v2f16 %out_val_final_" << i
                    << " %c_f16_minus1\n";
        ...
        {
            src << "%out_composite = OpCompositeConstruct %v2f16 %out_val_final %c_f16_minus1\n"
                   "%out_result = OpBitcast "
                << outputType
                << " %out_composite\n"
