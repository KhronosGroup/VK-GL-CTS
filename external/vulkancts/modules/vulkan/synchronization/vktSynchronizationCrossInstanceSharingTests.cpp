...
struct TestConfig
{
    TestConfig(const ResourceDescription &resource_, vk::VkSemaphoreType semaphoreType_, OperationName writeOp_,
               OperationName readOp_, vk::VkExternalMemoryHandleTypeFlagBits memoryHandleType_,
        ...
        : m_context(context)
    {
        ...
        if (config.resource.type == RESOURCE_TYPE_IMAGE)
        {
            ...
            if ((externalProperties.externalMemoryProperties.externalMemoryFeatures &
                 vk::VK_EXTERNAL_MEMORY_FEATURE_EXPORTABLE_BIT_KHR) == 0)
                TCU_THROW(NotSupportedError, "Exporting image resource not supported");

            if ((externalProperties.externalMemoryProperties.externalMemoryFeatures &
                 vk::VK_EXTERNAL_MEMORY_FEATURE_IMPORTABLE_BIT_KHR) == 0)
            ...
            if (!config.dedicated && (externalProperties.externalMemoryProperties.externalMemoryFeatures &
                                      vk::VK_EXTERNAL_MEMORY_FEATURE_DEDICATED_ONLY_BIT_KHR) != 0)
            {
...
void recordWriteBarrier(const vk::DeviceInterface &vkd, vk::VkCommandBuffer commandBuffer, const Resource &resource,
                        const SyncInfo &writeSync, uint32_t writeQueueFamilyIndex, const SyncInfo &readSync)
{
...
void recordReadBarrier(const vk::DeviceInterface &vkd, vk::VkCommandBuffer commandBuffer, const Resource &resource,
                       const SyncInfo &writeSync, const SyncInfo &readSync, uint32_t readQueueFamilyIndex)
{
...
tcu::TestStatus SharingTestInstance::iterate(void)
{
    ...
    try
    {
        ...
        if (resourceDesc.type == RESOURCE_TYPE_IMAGE)
        {
            ...
            resourceA =
                de::MovePtr<Resource>(new Resource(image, allocation, extent, resourceDesc.imageType,
                                                   resourceDesc.imageFormat, subresourceRange, subresourceLayers));
        }
        else
        {
        ...
        OperationContext operationContextA(m_context, m_vkiA, m_vkdA, m_physicalDeviceA, *m_deviceA, allocatorA,
        ...
        OperationContext operationContextB(m_context, m_vkiB, m_vkdB, m_physicalDeviceB, *m_deviceB, allocatorB,
                                           m_context.getBinaryCollection(), m_pipelineCacheData);
        ...
        {
            de::Random rng(1234);
            const uint64_t timelineValue                                = rng.getInt(1, deIntMaxValue32(32));
            ...
            const vk::VkTimelineSemaphoreSubmitInfo semaphoreSubmitInfo = {
                ...
                0u,                                                   // uint32_t            waitSemaphoreValueCount
                DE_NULL,                                              // const uint64_t*    pWaitSemaphoreValues
                1u,                                                   // uint32_t            signalSemaphoreValueCount
                &timelineValue,                                       // const uint64_t*    pSignalSemaphoreValues
            };
        ...
        }
        {
            ...
            const vk::VkTimelineSemaphoreSubmitInfo semaphoreSubmitInfo = {
                ...
                &timelineValue,                                       // const uint64_t*    pWaitSemaphoreValues
                0u,                                                   // uint32_t            signalSemaphoreValueCount
                DE_NULL,                                              // const uint64_t*    pSignalSemaphoreValues
            };
            const vk::VkSubmitInfo submitInfo = {
                vk::VK_STRUCTURE_TYPE_SUBMIT_INFO,
                m_config.semaphoreType == vk::VK_SEMAPHORE_TYPE_TIMELINE_KHR ? &semaphoreSubmitInfo : DE_NULL,
                ...
                for (size_t resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    for (size_t caseNdx = 0; caseNdx < DE_LENGTH_OF_ARRAY(cases); caseNdx++)
                    {
                        for (int semaphoreType = 0; semaphoreType < vk::VK_SEMAPHORE_TYPE_LAST; semaphoreType++)
                        {
                            ...
                            {
                                const TestConfig config(resource, (vk::VkSemaphoreType)semaphoreType, writeOp, readOp,
                                                        cases[caseNdx].memoryType, cases[caseNdx].semaphoreType,
                                                        dedicated);
                                ...
                                opGroup->addChild(new InstanceFactory1<SharingTestInstance, TestConfig, Progs>(
                                    testCtx, tcu::NODETYPE_SELF_VALIDATE, name, "", Progs(), config));
                                empty = false;
                            }
                        }
                    }
