...
Move<VkDevice> createTestDevice(const PlatformInterface &vkp, VkInstance instance, const InstanceInterface &vki,
                                VkPhysicalDevice physicalDevice, bool validationEnabled, uint32_t *outQueueFamilyIndex)
{
...
static void initSubmitInfo(VkSubmitInfo *submitInfo, uint32_t submitInfoCount)
{
    for (uint32_t ndx = 0; ndx < submitInfoCount; ndx++)
    {
...
static void initTimelineSemaphoreSubmitInfo(VkTimelineSemaphoreSubmitInfo *submitInfo, uint32_t submitInfoCount)
{
    for (uint32_t ndx = 0; ndx < submitInfoCount; ndx++)
    {
...
tcu::TestStatus testSemaphores(Context &context, VkSemaphoreType semaphoreType)
{
    ...
    vk::Move<VkDevice> device =
        createTestDevice(platformInterface, context.getInstance(), instanceInterface, physicalDevice,
                         context.getTestContext().getCommandLine().isValidationEnabled(), &queueFamilyIdx);
    const DeviceDriver deviceInterface(platformInterface, context.getInstance(), *device);
    SimpleAllocator allocator(deviceInterface, *device,
                              getPhysicalDeviceMemoryProperties(instanceInterface, physicalDevice));
    const VkQueue queue[2] = {getDeviceQueue(deviceInterface, *device, queueFamilyIdx, 0),
    ...
    createCommandBuffer(deviceInterface, device.get(), queueFamilyIdx, &testContext1.cmdBuffer,
                        &testContext1.commandPool);
    generateWork(testContext1);

    createCommandBuffer(deviceInterface, device.get(), queueFamilyIdx, &testContext2.cmdBuffer,
                        &testContext2.commandPool);
    ...
    testStatus = deviceInterface.waitForFences(device.get(), 1, &testContext1.fences[0], true,
    ...
    testStatus = deviceInterface.waitForFences(device.get(), 1, &testContext2.fences[0], true,
                                               std::numeric_limits<uint64_t>::max());
    if (testStatus != VK_SUCCESS)
    {
        log << TestLog::Message << "testSynchPrimitives failed to wait for a set fence" << TestLog::EndMessage;
