...
Move<VkDevice> createTestDevice(Context &context, SemaphoreTestConfig &config, uint32_t *outQueueFamilyIndex)
{
...
tcu::TestStatus testSemaphores(Context &context, SemaphoreTestConfig config)
{
...

    {
        ...
        SynchronizationWrapperPtr synchronizationWrapper =
            getSynchronizationWrapper(config.synchronizationType, deviceInterface, isTimelineSemaphore);
