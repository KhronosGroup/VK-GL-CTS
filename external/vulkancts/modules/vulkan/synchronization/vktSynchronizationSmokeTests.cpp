...
static CustomDevice createTestDevice(Context &context, SemaphoreTestConfig &config, const InstanceWrapper &instance,
...
tcu::TestStatus testSemaphores(Context &context, SemaphoreTestConfig config)
{
    ...
    const DeviceWrapper device(createTestDevice(context, config, instance, &queueFamilyIdx));
    ...
    VkResult testStatus;
    TestContext testContext1(deviceDriver, device, queueFamilyIdx, context.getBinaryCollection(), allocator,
                             context.getResourceInterface());
    TestContext testContext2(deviceDriver, device, queueFamilyIdx, context.getBinaryCollection(), allocator,
                             context.getResourceInterface());
    ...
    createCommandBuffer(deviceDriver, device, queueFamilyIdx, &testContext1.cmdBuffer, &testContext1.commandPool);
    generateWork(testContext1);

    createCommandBuffer(deviceDriver, device, queueFamilyIdx, &testContext2.cmdBuffer, &testContext2.commandPool);
    ...
    testStatus =
        deviceDriver.waitForFences(device, 1, &testContext1.fences[0], true, std::numeric_limits<uint64_t>::max());
    ...
    invalidateAlloc(deviceDriver, device, *testContext1.renderReadBuffer);
    ...
    testStatus =
        deviceDriver.waitForFences(device, 1, &testContext2.fences[0], true, std::numeric_limits<uint64_t>::max());
    if (testStatus != VK_SUCCESS)
    {
        log << TestLog::Message << "testSynchPrimitives failed to wait for a set fence" << TestLog::EndMessage;
        return tcu::TestStatus::fail("failed to wait for a set fence");
    ...
    invalidateAlloc(deviceDriver, device, *testContext2.renderReadBuffer);
