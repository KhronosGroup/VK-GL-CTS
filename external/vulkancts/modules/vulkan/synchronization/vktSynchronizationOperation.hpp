...
{
public:
    OperationContext(Context &context, SynchronizationType syncType, PipelineCacheData &pipelineCacheData);

    OperationContext(Context &context, SynchronizationType syncType, const vk::DeviceInterface &vkd,
                     const vk::VkDevice device, vk::Allocator &allocator, PipelineCacheData &pipelineCacheData);

    OperationContext(Context &context, SynchronizationType syncType, const vk::InstanceInterface &vki,
                     const vk::DeviceInterface &vkd, vk::VkPhysicalDevice physicalDevice, vk::VkDevice device,
                     vk::Allocator &allocator, vk::BinaryCollection &programCollection,
                     PipelineCacheData &pipelineCacheData);

    SynchronizationType getSynchronizationType(void) const
    {
        return m_syncType;
    }
    ...
    PipelineCacheData &getPipelineCacheData(void) const
    {
        return m_pipelineCacheData;
    }
...
{
public:
    Resource(OperationContext &context, const ResourceDescription &desc, const uint32_t usage,
             const vk::VkSharingMode sharingMode           = vk::VK_SHARING_MODE_EXCLUSIVE,
             const std::vector<uint32_t> &queueFamilyIndex = std::vector<uint32_t>());

    Resource(ResourceType type, vk::Move<vk::VkBuffer> buffer, de::MovePtr<vk::Allocation> allocation,
             vk::VkDeviceSize offset, vk::VkDeviceSize size);

    Resource(vk::Move<vk::VkImage> image, de::MovePtr<vk::Allocation> allocation, const vk::VkExtent3D &extent,
    ...
    const BufferResource &getBuffer(void) const
    {
        DE_ASSERT(m_bufferData.get());
        return *m_bufferData;
    }
    const ImageResource &getImage(void) const
    {
        DE_ASSERT(m_imageData.get());
        return *m_imageData;
    }
...
{
public:
    Operation(void) : m_specializedAccess(false)
    {
    }
...
{
public:
    OperationSupport(void) : m_specializedAccess(false)
    {
    }
    ...
    virtual de::MovePtr<Operation> build(OperationContext &context, Resource &inResource,
                                         Resource &outResource) const                         = 0;

    virtual vk::VkShaderStageFlagBits getShaderStage(void)
    {
        return vk::VK_SHADER_STAGE_FLAG_BITS_MAX_ENUM;
    }
