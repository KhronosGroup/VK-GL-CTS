...
public:
    ...
    const uint32_t &getQueueFamilyIndex(const int index)
    {
    ...
    bool getFreeQueue(uint32_t &returnQueueFamilyIndex, VkQueue &returnQueues, int &returnQueueIndex)
    {
    ...
    void releaseQueue(const uint32_t &queueFamilyIndex, const int &queueIndex)
    {
        m_mutex.lock();
...
{
public:
    PipelineCacheComputeTest(TestContext &testCtx, const string &name, const string &description)
...
{
public:
    PipelineCacheGraphicTest(TestContext &testCtx, const string &name, const string &description)
        : TestCase(testCtx, name, description)
    {
    }
...
tcu::TestCaseGroup *createInternallySynchronizedObjects(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> tests(
        new tcu::TestCaseGroup(testCtx, "internally_synchronized_objects", "Internally synchronized objects"));
    tests->addChild(
        new PipelineCacheComputeTest(testCtx, "pipeline_cache_compute",
                                     "Internally synchronized object VkPipelineCache for compute pipeline is tested"));
    tests->addChild(
        new PipelineCacheGraphicTest(testCtx, "pipeline_cache_graphics",
