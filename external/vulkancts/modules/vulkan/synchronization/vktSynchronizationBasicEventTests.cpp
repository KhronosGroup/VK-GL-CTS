...
tcu::TestStatus singleSubmissionCase(Context &context)
{
    ...
    vk.cmdWaitEvents(cmdBuffers[WAIT], 1u, &event.get(), VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
...
tcu::TestStatus secondaryCommandBufferCase(Context &context)
{
    ...
    vk.cmdWaitEvents(secondaryCmdBuffers[WAIT], 1u, &event.get(), VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
                     VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0u, DE_NULL, 0u, DE_NULL, 0u, DE_NULL);
...
tcu::TestCaseGroup *createBasicEventTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCase(basicTests.get(), "host_set_reset", "Basic event tests set and reset on host", checkEventSupport,
                    hostResetSetEventCase);
    addFunctionCase(basicTests.get(), "device_set_reset", "Basic event tests set and reset on device",
                    checkEventSupport, deviceResetSetEventCase);
    addFunctionCase(basicTests.get(), "single_submit_multi_command_buffer",
                    "Wait and set event single submission on device", checkEventSupport, singleSubmissionCase);
    addFunctionCase(basicTests.get(), "multi_submit_multi_command_buffer",
                    "Wait and set event mutli submission on device", checkEventSupport, multiSubmissionCase);
    addFunctionCase(basicTests.get(), "multi_secondary_command_buffer", "Event used on secondary command buffer ",
                    checkEventSupport, secondaryCommandBufferCase);
