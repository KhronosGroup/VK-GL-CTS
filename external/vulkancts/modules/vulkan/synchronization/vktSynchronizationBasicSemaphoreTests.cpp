    ...
    const VkSubmitInfo submitInfo[2] = {
        {
            ...
            config.semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE_KHR ? &timelineSignalInfo :
                                                                     DE_NULL, // const void* pNext;
...
tcu::TestStatus basicChainTimelineCase(Context &context)
{
    ...
    const VkQueue queue            = context.getUniversalQueue();
    VkPipelineStageFlags flags     = VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT;
    VkSemaphoreTypeCreateInfo scti = {VK_STRUCTURE_TYPE_SEMAPHORE_TYPE_CREATE_INFO_KHR, DE_NULL,
    ...
    for (int i = 0; err == VK_SUCCESS && i < basicChainLength; i++)
    {
        uint64_t waitValue                = i;
        uint64_t signalValue              = i + 1;
        VkTimelineSemaphoreSubmitInfo tsi = {
            VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO, // VkStructureType sType;
            DE_NULL,                                          // const void* pNext;
            i == 0 ? 0u : 1u,                                 // uint32_t            waitSemaphoreValueCount
            &waitValue,                                       // const uint64_t*    pWaitSemaphoreValues
            1u,                                               // uint32_t            signalSemaphoreValueCount
            &signalValue,                                     // const uint64_t*    pSignalSemaphoreValues
        };
        VkSubmitInfo si = {
            VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
            &tsi,                          // const void* pNext;
            i == 0 ? 0u : 1u,              // uint32_t waitSemaphoreCount;
    ...
    uint64_t waitValue                = basicChainLength;
    VkTimelineSemaphoreSubmitInfo tsi = {
        VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO, // VkStructureType sType;
        DE_NULL,                                          // const void* pNext;
        1u,                                               // uint32_t            waitSemaphoreValueCount
...
tcu::TestStatus basicMultiQueueCase(Context &context, TestConfig config)
{
    ...
    logicalDevice = createCustomDevice(context.getTestContext().getCommandLine().isValidationEnabled(),
                                       context.getPlatformInterface(), context.getInstance(), instance, physicalDevice,
                                       &deviceInfo);
    ...
    submitInfo[FIRST].pNext =
        config.semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE_KHR ? &timelineSubmitInfo[FIRST] : DE_NULL;
    ...
    submitInfo[SECOND].pNext =
        config.semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE_KHR ? &timelineSubmitInfo[SECOND] : DE_NULL;
...
tcu::TestCaseGroup *createBasicTimelineSemaphoreTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCase(basicTests.get(), "one_queue", "Basic timeline semaphore tests with one queue", basicOneQueueCase,
                    config);
