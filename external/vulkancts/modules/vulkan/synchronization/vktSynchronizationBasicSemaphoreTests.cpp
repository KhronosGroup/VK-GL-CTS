                                                    ...
                                                    VideoCodecOperationFlags videoCodecOperationFlags)
{
    ...
    if (type == SynchronizationType::SYNCHRONIZATION2)
        addFunctionCase(basicTests.get(), "none_wait_submit", "Test waiting on the none pipeline stage",
                        checkCommandBufferSimultaneousUseSupport, noneWaitSubmitTest, config);
                                                      ...
                                                      VideoCodecOperationFlags videoCodecOperationFlags)
{
    ...
    if (type == SynchronizationType::LEGACY)
    {
        ...
        addFunctionCase(basicTests.get(), "wait_for_any_current_value",
                        "Wait for the currently signalled timeline semaphore value (wait for any)", checkSupport,
                        basicWaitForAnyCurrentTimelineValueCase, config);
        addFunctionCase(basicTests.get(), "wait_for_any_lesser_value",
                        "Wait for a value less than the currently signalled timeline semaphore value (wait for any)",
                        checkSupport, basicWaitForAnyLesserTimelineValueCase, config);
        addFunctionCase(basicTests.get(), "wait_for_all_current_value",
                        "Wait for the currently signalled timeline semaphore value (wait for all)", checkSupport,
                        basicWaitForAllCurrentTimelineValueCase, config);
        addFunctionCase(basicTests.get(), "wait_for_all_lesser_value",
                        "Wait for a value less than the currently signalled timeline semaphore value (wait for all)",
                        checkSupport, basicWaitForAllLesserTimelineValueCase, config);
    }
