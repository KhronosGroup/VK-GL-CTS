...
public:
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        if (m_resource->getType() == RESOURCE_TYPE_IMAGE)
        {
            ...
            VkDependencyInfoKHR dependencyInfo = makeCommonDependencyInfo(DE_NULL, DE_NULL, &imageMemoryBarrier2, true);
        ...
        else
        {
            ...
            VkDependencyInfoKHR dependencyInfo =
                makeCommonDependencyInfo(DE_NULL, &bufferMemoryBarrier2, DE_NULL, true);
            synchronizationWrapper->cmdSetEvent(*cmdBuffer, *event, &dependencyInfo);
            synchronizationWrapper->cmdWaitEvents(*cmdBuffer, 1u, &event.get(), &dependencyInfo);
        }
...
public:
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        synchronizationWrapper->addSubmitInfo(0u, DE_NULL, 1u, &commandBufferSubmitInfo[WRITE], 1u,
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        for (uint32_t opNdx = 0; opNdx < m_ops.size(); opNdx++)
        {
            ...
            synchronizationWrapper->addSubmitInfo(
                opNdx == 0 ? 0u : 1u, opNdx == 0 ? DE_NULL : &waitSemaphoreSubmitInfos[opNdx - 1], 1u,
...
public:
    SyncTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                 ...
                 const OperationName writeOp, const OperationName readOp, PipelineCacheData &pipelineCacheData)
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    if (isResourceSupported(writeOp, resource) && isResourceSupported(readOp, resource))
                    {
                        opGroup->addChild(new SyncTestCase(testCtx, name, "", data.type, groups[groupNdx].syncPrimitive,
                                                           resource, writeOp, readOp, *data.pipelineCacheData));
                        empty = false;
                    }
                }
                if (!empty)
