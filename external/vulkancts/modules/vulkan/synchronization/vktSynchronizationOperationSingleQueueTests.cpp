...
{
public:
    EventTestInstance(Context &context, const ResourceDescription &resourceDesc, const OperationSupport &writeOp,
                      const OperationSupport &readOp, PipelineCacheData &pipelineCacheData)
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        if (m_resource->getType() == RESOURCE_TYPE_BUFFER || isIndirectBuffer(m_resource->getType()))
        {
            const VkBufferMemoryBarrier barrier =
                makeBufferMemoryBarrier(writeSync.accessMask, readSync.accessMask, m_resource->getBuffer().handle,
                                        m_resource->getBuffer().offset, m_resource->getBuffer().size);
            vk.cmdWaitEvents(*cmdBuffer, 1u, &event.get(), writeSync.stageMask, readSync.stageMask, 0u, DE_NULL, 1u,
                             &barrier, 0u, DE_NULL);
        }
        else if (m_resource->getType() == RESOURCE_TYPE_IMAGE)
        {
            ...
            vk.cmdWaitEvents(*cmdBuffer, 1u, &event.get(), writeSync.stageMask, readSync.stageMask, 0u, DE_NULL, 0u,
                             DE_NULL, 1u, &barrier);
        }
...
{
public:
    BarrierTestInstance(Context &context, const ResourceDescription &resourceDesc, const OperationSupport &writeOp,
                        const OperationSupport &readOp, PipelineCacheData &pipelineCacheData)
        : BaseTestInstance(context, resourceDesc, writeOp, readOp, pipelineCacheData)
    {
    }
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        if (m_resource->getType() == RESOURCE_TYPE_IMAGE)
        {
            const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
                writeSync.accessMask, readSync.accessMask, writeSync.imageLayout, readSync.imageLayout,
                m_resource->getImage().handle, m_resource->getImage().subresourceRange);
            vk.cmdPipelineBarrier(cmdBuffers[WRITE], writeSync.stageMask, readSync.stageMask, (VkDependencyFlags)0, 0u,
                                  (const VkMemoryBarrier *)DE_NULL, 0u, (const VkBufferMemoryBarrier *)DE_NULL, 1u,
                                  &barrier);
        }
        else
        {
            const VkBufferMemoryBarrier barrier = makeBufferMemoryBarrier(
                writeSync.accessMask, readSync.accessMask, m_resource->getBuffer().handle, 0, VK_WHOLE_SIZE);
            vk.cmdPipelineBarrier(cmdBuffers[WRITE], writeSync.stageMask, readSync.stageMask, (VkDependencyFlags)0, 0u,
                                  (const VkMemoryBarrier *)DE_NULL, 1u, &barrier, 0u,
                                  (const VkImageMemoryBarrier *)DE_NULL);
        }
        ...
        for (uint32_t copyOpNdx = 0; copyOpNdx < DE_LENGTH_OF_ARRAY(s_copyOps); copyOpNdx++)
        {
            if (isResourceSupported(s_copyOps[copyOpNdx], resourceDesc))
                m_opSupports.push_back(de::SharedPtr<OperationSupport>(
                    makeOperationSupport(s_copyOps[copyOpNdx], resourceDesc).release()));
        }
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        for (uint32_t opNdx = 0; opNdx < m_ops.size(); opNdx++)
        {
            const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
                ...
                DE_NULL,                                           // const void* pNext;
                opNdx == 0 ? 0u : 1u,                              // uint32_t            waitSemaphoreValueCount
                opNdx == 0 ? DE_NULL : &timelineValues[opNdx - 1], // const uint64_t*    pWaitSemaphoreValues
                1u,                                                // uint32_t            signalSemaphoreValueCount
                &timelineValues[opNdx],                            // const uint64_t*    pSignalSemaphoreValues
            };
            timelineSubmitInfos[opNdx]    = timelineSubmitInfo;
            const VkSubmitInfo submitInfo = {
                VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
                &timelineSubmitInfos[opNdx],   // const void* pNext;
                opNdx == 0 ? 0u : 1u,          // uint32_t waitSemaphoreCount;
                &semaphore.get(),              // const VkSemaphore* pWaitSemaphores;
                stageBits,
                1u,                 // uint32_t commandBufferCount;
            ...
            if (opNdx > 0)
            {
                ...
                if (resource.getType() == RESOURCE_TYPE_IMAGE)
                {
                    ...
                    const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
                        lastSync.accessMask, currentSync.accessMask, lastSync.imageLayout, currentSync.imageLayout,
                        resource.getImage().handle, resource.getImage().subresourceRange);
                    vk.cmdPipelineBarrier(cmdBuffers[opNdx], lastSync.stageMask, currentSync.stageMask,
                ...
                {
                    const VkBufferMemoryBarrier barrier = makeBufferMemoryBarrier(
                        lastSync.accessMask, currentSync.accessMask, resource.getBuffer().handle, 0, VK_WHOLE_SIZE);
                    vk.cmdPipelineBarrier(cmdBuffers[opNdx], lastSync.stageMask, currentSync.stageMask,
...
{
public:
    SyncTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
        ...
        for (int writeOpNdx = 0; writeOpNdx < DE_LENGTH_OF_ARRAY(s_writeOps); ++writeOpNdx)
            for (int readOpNdx = 0; readOpNdx < DE_LENGTH_OF_ARRAY(s_readOps); ++readOpNdx)
            {
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    {
                        opGroup->addChild(new SyncTestCase(testCtx, name, "", groups[groupNdx].syncPrimitive, resource,
                                                           writeOp, readOp, *pipelineCacheData));
                        empty = false;
                    }
                }
                if (!empty)
