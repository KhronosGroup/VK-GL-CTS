...
{
    CheckerboardBuilder(uint32_t width, uint32_t height) : m_width(width), m_height(height)
    {
    }
...
public:
    ...
    bool verify(const BufferAccess &result, uint32_t blackColor, uint32_t whiteColor) const;
...
Move<VkPipeline> GPQInstanceBase::createGraphicsPipeline(VkPipelineLayout pipelineLayout, VkRenderPass renderPass)
{
    ...
    return makeGraphicsPipeline(vkd, dev, pipelineLayout, *m_vertex, VkShaderModule(0), VkShaderModule(0),
                                VkShaderModule(0), *m_fragment, renderPass, viewports, scissors,
...
{
public:
    GPQCase(tcu::TestContext &ctx, const std::string &name, const TestConfig &cfg, const std::string &desc = {});
...
GPQCase::GPQCase(tcu::TestContext &ctx, const std::string &name, const TestConfig &cfg, const std::string &desc)
...
void GPQCase::checkSupport(Context &context) const
{
    ...
    const uint32_t queueFromIndex =
        findQueueFamilyIndex(vki, dev, flagFrom, SpecialDevice::getColissionFlags(m_config.transitionFrom),
                             priorityQueryEnabled, QueueGlobalPriorities({m_config.priorityFrom}));
    assertUnavailableQueue(queueFromIndex, m_config.transitionFrom, m_config.priorityFrom);

    const uint32_t queueToIndex =
        findQueueFamilyIndex(vki, dev, flagTo, SpecialDevice::getColissionFlags(m_config.transitionTo),
                             priorityQueryEnabled, QueueGlobalPriorities({m_config.priorityTo}));
...
void GPQCase::initPrograms(SourceCollections &programs) const
{
    ...
    const std::map<std::string, std::string> abbreviations{
        {std::string("TEST_VALUE"), std::to_string(testValue)}, {std::string("IMAGE_FORMAT"), std::string(imageFormat)},
        {std::string("IMAGE_TYPE"), std::string(imageType)},    {std::string("IMAGE_BUFFER"), std::string(imageBuffer)},
        {std::string("COLOR_TYPE"), std::string(colorType)},
    };

    programs.glslSources.add("comp") << glu::ComputeSource(
        m_config.transitionFrom == VK_QUEUE_COMPUTE_BIT ? producerComp : consumerComp.specialize(abbreviations));
...
tcu::TestStatus GPQInstance<VK_QUEUE_COMPUTE_BIT, VK_QUEUE_GRAPHICS_BIT>::iterate(void)
{
    ...
    DE_UNREF(producerQueue);
    const VkQueue consumerQueue = m_device.queueTo;
    DE_UNREF(consumerQueue);
    ...
    const MemoryRequirement memReqs =
        (m_config.enableProtected ? MemoryRequirement::Protected : MemoryRequirement::Any);

    VkBufferCreateFlags buffsCreateFlags = 0;
    if (m_config.enableProtected)
        buffsCreateFlags |= VK_BUFFER_CREATE_PROTECTED_BIT;
    if (m_config.enableSparseBinding)
        buffsCreateFlags |= VK_BUFFER_CREATE_SPARSE_BINDING_BIT;
    ...
    const VkBufferCreateInfo vertBuffInfo =
        makeBufferCreateInfo(vertBuffSize, vertBuffUsage, {producerIndex}, buffsCreateFlags);
    const VkBufferCreateInfo indexBuffInfo =
        makeBufferCreateInfo(indexBuffSize, indexBuffUsage, {producerIndex}, buffsCreateFlags);
    ...
    DescriptorSetUpdateBuilder()
        .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0),
                     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &dsVertInfo)
        .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(1),
                     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &dsIndexInfo)
        .update(vkd, device);
    ...
    const VkImageMemoryBarrier colorReadyBarrier = makeImageMemoryBarrier(
        VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, **image, colorResourceRange);
    const VkBufferImageCopy colorCopyRegion = makeBufferImageCopy(
        makeExtent3D(width, height, 1), makeImageSubresourceLayers(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 0u, 1u));
    ...
    vkd.cmdBindDescriptorSets(*producerCmd, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0, 1, &(*descriptorSet), 0,
                              nullptr);
    vkd.cmdBindPipeline(*producerCmd, VK_PIPELINE_BIND_POINT_COMPUTE, *producerPipeline);
    vkd.cmdPushConstants(*producerCmd, *pipelineLayout, VK_SHADER_STAGE_ALL, 0,
                         static_cast<uint32_t>(sizeof(PushConstant)), &pc);
    ...
    vkd.cmdBindVertexBuffers(*consumerCmd, 0, 1, &static_cast<const VkBuffer &>(*vertexBuffer),
                             &static_cast<const VkDeviceSize &>(0));
    ...
    vkd.cmdPipelineBarrier(*consumerCmd, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                           0u, 0u, nullptr, 0u, nullptr, 1u, &colorReadyBarrier);
    vkd.cmdCopyImageToBuffer(*consumerCmd, **image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *resultBuffer, 1u,
                             &colorCopyRegion);
    vkd.cmdPipelineBarrier(*consumerCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 1u,
    ...
    const tcu::ConstPixelBufferAccess resultBufferAccess(mapVkFormat(m_config.format), width, height, 1,
                                                         resultBuffer.getHostPtr());

    return verify(resultBufferAccess, GPQCase::testValue, clearComp) ? tcu::TestStatus::pass("") :
                                                                       tcu::TestStatus::fail("");
}
...
tcu::TestStatus GPQInstance<VK_QUEUE_GRAPHICS_BIT, VK_QUEUE_COMPUTE_BIT>::iterate(void)
{
    ...
    const VkDeviceSize vertBuffSize        = vertexCount * mapVkFormat(VK_FORMAT_R32G32_SFLOAT).getPixelSize();
    const VkBufferCreateInfo vertBuffInfo =
        makeBufferCreateInfo(vertBuffSize, vertBuffUsage, {producerIndex}, graphCreateFlags);
    ...
    const VkDeviceSize indexBuffSize        = indexCount * sizeof(uint32_t);
    const VkBufferCreateInfo indexBuffInfo =
        makeBufferCreateInfo(indexBuffSize, indexBuffUsage, {producerIndex}, graphCreateFlags);
    ...
    const VkImageUsageFlags producerUsage =
    ...
    const VkDescriptorImageInfo producerImageInfo =
        makeDescriptorImageInfo(VkSampler(0), *producerView, VK_IMAGE_LAYOUT_GENERAL);
    ...
    const VkBufferCreateInfo consumerBuffInfo =
        makeBufferCreateInfo(consumerBuffSize, consumerUsage, {consumerIndex}, consumerCreateFlags);
    BufferWithMemory consumerBuffer(vki, vkd, phys, device, allocator, consumerBuffInfo, consumerMemReqs,
                                    consumerQueue);
    ...
    const VkBufferImageCopy tmpCopyRegion = makeBufferImageCopy(
        makeExtent3D(width, height, 1), makeImageSubresourceLayers(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 0u, 1u));
    ...
    DescriptorSetUpdateBuilder()
        .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0), VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
                     &producerImageInfo)
        .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(1),
                     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &consumerInfo)
        .update(vkd, device);

    Move<VkRenderPass> renderPass   = makeRenderPass(vkd, device, m_config.format);
    Move<VkFramebuffer> framebuffer = makeFramebuffer(vkd, device, *renderPass, *producerView, width, height);

    const VkImageMemoryBarrier producerReadyBarrier =
        makeImageMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_NONE, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                               VK_IMAGE_LAYOUT_GENERAL, **producerImage, producerResRange);
    ...
    //   consumerIndex, consumerIndex);
    const VkBufferMemoryBarrier consumerBarriers[]{
        makeBufferMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_NONE, *consumerBuffer, 0, consumerBuffSize,
                                consumerIndex, consumerIndex),
        makeBufferMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_NONE, *tmpBuffer, 0, tmpBuffSize, consumerIndex,
                                consumerIndex)};
    struct PushConstant
    {
        uint32_t width, height;
    ...
    vkd.cmdBindVertexBuffers(*producerCmd, 0, 1, &static_cast<const VkBuffer &>(*vertexBuffer),
                             &static_cast<const VkDeviceSize &>(0));
    ...
    vkd.cmdPipelineBarrier(*producerCmd, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
                           VkDependencyFlags(0), 0u, nullptr, 0u, nullptr, 1u, &producerReadyBarrier);
    endCommandBuffer(vkd, *producerCmd);

    beginCommandBuffer(vkd, *consumerCmd);
    vkd.cmdBindPipeline(*consumerCmd, VK_PIPELINE_BIND_POINT_COMPUTE, *consumerPipeline);
    vkd.cmdBindDescriptorSets(*consumerCmd, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0, 1, &descriptorSet.get(),
                              0, nullptr);
    vkd.cmdCopyImageToBuffer(*consumerCmd, **producerImage, VK_IMAGE_LAYOUT_GENERAL, *tmpBuffer, 1u, &tmpCopyRegion);
    vkd.cmdPushConstants(*consumerCmd, *pipelineLayout, VK_SHADER_STAGE_ALL, 0,
                         static_cast<uint32_t>(sizeof(PushConstant)), &pc);
    vkd.cmdDispatch(*consumerCmd, width, height, 1);
    vkd.cmdPipelineBarrier(*consumerCmd, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                           VkDependencyFlags(0), 0, nullptr, DE_LENGTH_OF_ARRAY(consumerBarriers), consumerBarriers, 0,
                           nullptr);
    endCommandBuffer(vkd, *consumerCmd);
    ...
    const tcu::ConstPixelBufferAccess tmpBufferAccess(mapVkFormat(m_config.format), width, height, 1,
                                                      tmpBuffer.getHostPtr());
    const bool tmpRes = verify(tmpBufferAccess, GPQCase::testValue, clearComp);

    consumerBuffer.invalidateAlloc(vkd, device);
    const tcu::ConstPixelBufferAccess resultBufferAccess(mapVkFormat(m_config.format), width, height, 1,
                                                         consumerBuffer.getHostPtr());

    return (tmpRes && verify(resultBufferAccess, (GPQCase::testValue + 1), (clearComp + 1))) ?
               tcu::TestStatus::pass("") :
               tcu::TestStatus::fail("");
}

bool GPQInstanceBase::verify(const BufferAccess &result, uint32_t blackColor, uint32_t whiteColor) const
{
    bool ok = true;

    for (uint32_t y = 0; ok && y < m_config.height; ++y)
    {
        for (uint32_t x = 0; ok && x < m_config.width; ++x)
        {
            const uint32_t color = result.getPixelT<uint32_t>(x, y).x();
            if (((x + y) % 2) == 0)
            {
                ok = color == blackColor;
            }
