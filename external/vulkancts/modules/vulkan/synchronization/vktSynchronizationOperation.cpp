...
public:
    ...
    SyncInfo getInSyncInfo(void) const
    {
        const VkAccessFlags2KHR accessFlags =
            (m_mode == ACCESS_MODE_READ ? (m_bufferType == BUFFER_TYPE_UNIFORM ? VK_ACCESS_2_UNIFORM_READ_BIT_KHR :
    ...
    : m_type(desc.type)
{
    ...
    else if (m_type == RESOURCE_TYPE_IMAGE)
    {
        ...
        VkImageCreateInfo imageInfo = makeImageCreateInfo(m_imageData.imageType, m_imageData.extent, m_imageData.format,
                                                          usage, desc.imageSamples);
