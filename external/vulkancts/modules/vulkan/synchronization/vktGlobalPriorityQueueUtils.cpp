                              ...
                              VkQueueFlags includeFlags, VkQueueFlags excludeFlags, uint32_t excludeIndex)
{
    ...
    for (uint32_t familyIdx = 0; familyIdx < queueFamilyPropertyCount; ++familyIdx)
    {
    ...
    uint32_t familyFound = INVALID_UINT32;

    for (uint32_t familyIdx = 0; familyIdx < queueFamilyPropertyCount; ++familyIdx)
    {
        if (familyIdx == excludeIndex)
            continue;

        bool priorityMatches = false;
        for (uint32_t priorityIdx = 0;
             !priorityMatches && priorityIdx < familyPriorityProperties[familyIdx].priorityCount; ++priorityIdx)
        {
    ...
    , m_createFileLine(INVALID_UINT32)
{
    ...
    m_queueFamilyIndexFrom =
        findQueueFamilyIndex(vki, phys, priorityFrom, flagFrom, getColissionFlags(transitionFrom), INVALID_UINT32);
    m_queueFamilyIndexTo =
        findQueueFamilyIndex(vki, phys, priorityTo, flagTo, getColissionFlags(transitionTo), m_queueFamilyIndexFrom);
