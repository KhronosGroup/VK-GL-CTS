...
class MultiQueues
{
    ...
    MultiQueues(const Context &context, bool timelineSemaphore) : m_queueCount(0)
    {
...

        {
            ...
            const VkDeviceCreateInfo deviceInfo = {
                ...
                timelineSemaphore ? 1u : 0u,              //uint32_t enabledExtensionCount;
            ...
            m_logicalDevice = createCustomDevice(context.getTestContext().getCommandLine().isValidationEnabled(),
                                                 context.getPlatformInterface(), context.getInstance(), instance,
                                                 physicalDevice, &deviceInfo);
            m_deviceDriver  = MovePtr<DeviceDriver>(
            ...
            m_allocator = MovePtr<Allocator>(new SimpleAllocator(
                *m_deviceDriver, *m_logicalDevice, getPhysicalDeviceMemoryProperties(instance, physicalDevice)));
...
void createBarrierMultiQueue(const DeviceInterface &vk, const VkCommandBuffer &cmdBuffer, const SyncInfo &writeSync,
                             const SyncInfo &readSync, const Resource &resource, const uint32_t writeFamily,
                             const uint32_t readFamily, const VkSharingMode sharingMode, const bool secondQueue = false)
{
    if (resource.getType() == RESOURCE_TYPE_IMAGE)
    {
        VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
            secondQueue ? 0u : writeSync.accessMask, !secondQueue ? 0u : readSync.accessMask, writeSync.imageLayout,
            readSync.imageLayout, resource.getImage().handle, resource.getImage().subresourceRange);
        ...
        if (writeFamily != readFamily && VK_SHARING_MODE_EXCLUSIVE == sharingMode)
        {
            ...
            vk.cmdPipelineBarrier(
                cmdBuffer, secondQueue ? VkPipelineStageFlags(VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT) : writeSync.stageMask,
    ...
    else if (resource.getType() == RESOURCE_TYPE_BUFFER || isIndirectBuffer(resource.getType()))
    {
        VkBufferMemoryBarrier barrier = makeBufferMemoryBarrier(
            secondQueue ? 0u : writeSync.accessMask, !secondQueue ? 0u : readSync.accessMask,
        ...
        vk.cmdPipelineBarrier(
            cmdBuffer, secondQueue ? VkPipelineStageFlags(VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT) : writeSync.stageMask,
            !secondQueue ? VkPipelineStageFlags(VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT) : readSync.stageMask,
            (VkDependencyFlags)0, 0u, (const VkMemoryBarrier *)DE_NULL, 1u, (const VkBufferMemoryBarrier *)&barrier, 0u,
...
{
public:
    BaseTestInstance(Context &context, const ResourceDescription &resourceDesc, const OperationSupport &writeOp,
                     const OperationSupport &readOp, PipelineCacheData &pipelineCacheData, bool timelineSemaphore)
        : TestInstance(context)
        , m_queues(MultiQueues::getInstance(context, timelineSemaphore))
        , m_opContext(new OperationContext(context, pipelineCacheData, m_queues->getDeviceInterface(),
                                           m_queues->getDevice(), m_queues->getAllocator()))
...
{
public:
    BinarySemaphoreTestInstance(Context &context, const ResourceDescription &resourceDesc,
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        const std::vector<QueuePair> queuePairs =
            m_queues->getQueuesPairs(m_writeOp.getQueueFlags(*m_opContext), m_readOp.getQueueFlags(*m_opContext));
        ...
        for (uint32_t pairNdx = 0; pairNdx < static_cast<uint32_t>(queuePairs.size()); ++pairNdx)
        {
            ...
            const VkCommandBuffer cmdBuffers[] = {*ptrCmdBuffer[QUEUETYPE_WRITE], *ptrCmdBuffer[QUEUETYPE_READ]};
            ...
            createBarrierMultiQueue(vk, cmdBuffers[QUEUETYPE_WRITE], writeSync, readSync, *resource,
                                    queuePairs[pairNdx].familyIndexWrite, queuePairs[pairNdx].familyIndexRead,
                                    m_sharingMode);
            endCommandBuffer(vk, cmdBuffers[QUEUETYPE_WRITE]);

            beginCommandBuffer(vk, cmdBuffers[QUEUETYPE_READ]);
            createBarrierMultiQueue(vk, cmdBuffers[QUEUETYPE_READ], writeSync, readSync, *resource,
                                    queuePairs[pairNdx].familyIndexWrite, queuePairs[pairNdx].familyIndexRead,
                                    m_sharingMode, true);
...
{
public:
    TimelineSemaphoreTestInstance(Context &context, const ResourceDescription &resourceDesc,
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        for (uint32_t opIdx = 0; opIdx < m_ops.size(); opIdx++)
        {
            ...
            const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
                ...
                DE_NULL,                                           // const void* pNext;
                opIdx == 0 ? 0u : 1u,                              // uint32_t            waitSemaphoreValueCount
                opIdx == 0 ? DE_NULL : &timelineValues[opIdx - 1], // const uint64_t*    pWaitSemaphoreValues
                1u,                                                // uint32_t            signalSemaphoreValueCount
                &timelineValues[opIdx],                            // const uint64_t*    pSignalSemaphoreValues
            };
            const VkSubmitInfo submitInfo = {
                VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
                &timelineSubmitInfo,           // const void* pNext;
                opIdx == 0 ? 0u : 1u,          // uint32_t waitSemaphoreCount;
                &semaphore.get(),              // const VkSemaphore* pWaitSemaphores;
                stageBits,
                1u,                 // uint32_t commandBufferCount;
            ...
            if (opIdx > 0)
            {
                ...
                createBarrierMultiQueue(vk, cmdBuffers[opIdx], writeSync, readSync, resource,
                                        m_opQueues[opIdx - 1].family, m_opQueues[opIdx].family, m_sharingMode, true);
            }
            ...
            if (opIdx < (m_ops.size() - 1))
            {
                ...
                createBarrierMultiQueue(vk, cmdBuffers[opIdx], writeSync, readSync, resource, m_opQueues[opIdx].family,
                                        m_opQueues[opIdx + 1].family, m_sharingMode);
            }
...
{
public:
    FenceTestInstance(Context &context, const ResourceDescription &resourceDesc, const OperationSupport &writeOp,
                      const OperationSupport &readOp, PipelineCacheData &pipelineCacheData,
                ...
                for (int optionNdx = 0; optionNdx <= groups[groupNdx].numOptions; ++optionNdx)
                    for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                    {
                        ...
                        if (isResourceSupported(writeOp, resource) && isResourceSupported(readOp, resource))
                        {
                            ...
                            opGroup->addChild(new BaseTestCase(testCtx, name, "", groups[groupNdx].syncPrimitive,
                                                               resource, writeOp, readOp, sharingMode,
                                                               *pipelineCacheData));
                            empty = false;
                        }
                    }
                                                               ...
                                                               PipelineCacheData &pipelineCacheData)
{
    return createTestGroup(testCtx, "multi_queue", "Synchronization of a memory-modifying operation", createTests,
                           &pipelineCacheData, cleanupGroup);
}
