        ...
        , m_queueCount(0)
    {
...

        {
            ...
            m_logicalDevice = createCustomDevice(context.getTestContext().getCommandLine().isValidationEnabled(),
                                                 context.getPlatformInterface(), m_instance, instanceDriver,
                                                 physicalDevice, &deviceInfo);
#ifndef CTS_USES_VULKANSC
            m_deviceDriver = de::MovePtr<DeviceDriver>(
                new DeviceDriver(context.getPlatformInterface(), m_instance, *m_logicalDevice));
#else
            m_deviceDriver = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
                new DeviceDriverSC(context.getPlatformInterface(), m_instance, *m_logicalDevice,
...
public:
    ...
    {
        uint32_t index = (uint32_t)type << 1 | timelineSemaphore;
