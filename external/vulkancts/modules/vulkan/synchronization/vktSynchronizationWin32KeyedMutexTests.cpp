...
typedef HRESULT(WINAPI *LPD3DX11COMPILEFROMMEMORY)(LPCSTR, SIZE_T, LPCSTR, CONST D3D10_SHADER_MACRO *, LPD3D10INCLUDE,
                                                   LPCSTR, LPCSTR, UINT, UINT, void *, /* ID3DX11ThreadPump */
...
static const ResourceDescription s_resourcesWin32KeyedMutex[] = {
    {RESOURCE_TYPE_BUFFER, tcu::IVec4(0x4000, 0, 0, 0), vk::VK_IMAGE_TYPE_LAST, vk::VK_FORMAT_UNDEFINED,
     (vk::VkImageAspectFlags)0}, // 16 KiB (min max UBO range)
    {RESOURCE_TYPE_BUFFER, tcu::IVec4(0x40000, 0, 0, 0), vk::VK_IMAGE_TYPE_LAST, vk::VK_FORMAT_UNDEFINED,
     (vk::VkImageAspectFlags)0}, // 256 KiB

    {RESOURCE_TYPE_IMAGE, tcu::IVec4(128, 128, 0, 0), vk::VK_IMAGE_TYPE_2D, vk::VK_FORMAT_R8_UNORM,
     vk::VK_IMAGE_ASPECT_COLOR_BIT},
    {RESOURCE_TYPE_IMAGE, tcu::IVec4(128, 128, 0, 0), vk::VK_IMAGE_TYPE_2D, vk::VK_FORMAT_R16_UINT,
     vk::VK_IMAGE_ASPECT_COLOR_BIT},
    {RESOURCE_TYPE_IMAGE, tcu::IVec4(128, 128, 0, 0), vk::VK_IMAGE_TYPE_2D, vk::VK_FORMAT_R8G8B8A8_UNORM,
     vk::VK_IMAGE_ASPECT_COLOR_BIT},
    {RESOURCE_TYPE_IMAGE, tcu::IVec4(128, 128, 0, 0), vk::VK_IMAGE_TYPE_2D, vk::VK_FORMAT_R16G16B16A16_UINT,
     vk::VK_IMAGE_ASPECT_COLOR_BIT},
...
struct TestConfig
{
    TestConfig(const ResourceDescription &resource_, OperationName writeOp_, OperationName readOp_,
...
vk::Move<vk::VkDeviceMemory> importMemory(const vk::DeviceInterface &vkd, vk::VkDevice device,
                                          ...
                                          vk::VkExternalMemoryHandleTypeFlagBits externalType, NativeHandle &handle,
                                          bool requiresDedicated, vk::VkBuffer buffer, vk::VkImage image)
{
    ...
    const vk::VkMemoryAllocateInfo info = {
        vk::VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO, &importInfo, requirements.size,
...
public:
...
#if (DE_OS == DE_OS_WIN32)
    DX11Operation(const ResourceDescription &resourceDesc, vk::VkExternalMemoryHandleTypeFlagBits memoryHandleType,
                  ID3D11Device *pDevice, ID3D11DeviceContext *pContext,
                  LPD3DX11COMPILEFROMMEMORY fnD3DX11CompileFromMemory, pD3DCompile fnD3DCompile)
        ...
        else
        {
            ...
            for (UINT i = 0; i < BUFFER_COUNT; ++i)
            {
                ...
                if (m_isMemNtHandle)
                {
                    ...
                    hr = tempResource1->CreateSharedHandle(getSecurityAttributes(),
                                                           DXGI_SHARED_RESOURCE_READ | DXGI_SHARED_RESOURCE_WRITE,
                                                           /*lpName*/ NULL, &m_sharedMemHandle[i]);
            ...
            LPCSTR shader = "Texture2D txDiffuse : register(t0);\n"
            ...
            createShaders(shader, "VS", "vs_4_0", ARRAYSIZE(layout), layout, &m_pVertexShader, "PS", "ps_4_0",
                          &m_pPixelShader);
    ...
    {
#if (DE_OS == DE_OS_WIN32)
        return NativeHandle((m_isMemNtHandle) ? NativeHandle::WIN32HANDLETYPE_NT : NativeHandle::WIN32HANDLETYPE_KMT,
                            vk::pt::Win32Handle(m_sharedMemHandle[buffer]));
#else
        DE_UNREF(buffer);
        return NativeHandle();
#endif
    ...
    void copyMemory()
    {
        ...
        if (m_resourceDesc.type == RESOURCE_TYPE_BUFFER)
        {
            m_pContext->CopySubresourceRegion(m_pBuffer[BUFFER_VK_READ], 0, 0, 0, 0, m_pBuffer[BUFFER_VK_WRITE], 0,
                                              NULL);
        }
        else
        {
...
#if (DE_OS == DE_OS_WIN32)
    void d3dx11CompileShader(const char *shaderCode, const char *entryPoint, const char *shaderModel,
                             ID3D10Blob **ppBlobOut)
    {
        HRESULT hr;

        ID3D10Blob *pErrorBlob;
        hr = m_fnD3DX11CompileFromMemory(shaderCode, strlen(shaderCode), "Memory", NULL, NULL, entryPoint, shaderModel,
    ...
    void d3dCompileShader(const char *shaderCode, const char *entryPoint, const char *shaderModel, ID3DBlob **ppBlobOut)
    {
        ...
        ID3DBlob *pErrorBlob;
        hr = m_fnD3DCompile(shaderCode, strlen(shaderCode), NULL, NULL, NULL, entryPoint, shaderModel, 0, 0, ppBlobOut,
                            &pErrorBlob);
        if (pErrorBlob)
            pErrorBlob->Release();
    ...
    void createShaders(const char *shaderSrc, const char *vsEntryPoint, const char *vsShaderModel, UINT numLayoutDesc,
                       D3D11_INPUT_ELEMENT_DESC *pLayoutDesc, ID3D11VertexShader **pVertexShader,
                       const char *psEntryPoint, const char *psShaderModel, ID3D11PixelShader **pPixelShader)
    {
        HRESULT hr;
        ...
        if (m_fnD3DX11CompileFromMemory)
        {
            ...
            hr = m_pDevice->CreateVertexShader(pVSBlob->GetBufferPointer(), pVSBlob->GetBufferSize(), NULL,
                                               pVertexShader);
            if (FAILED(hr))
                TCU_FAIL("Failed to create DX11 vertex shader");

            ID3D11InputLayout *pVertexLayout;
            hr = m_pDevice->CreateInputLayout(pLayoutDesc, numLayoutDesc, pVSBlob->GetBufferPointer(),
            ...
            hr =
                m_pDevice->CreatePixelShader(pPSBlob->GetBufferPointer(), pPSBlob->GetBufferSize(), NULL, pPixelShader);
            if (FAILED(hr))
...
{
public:
    DX11OperationSupport(const vk::InstanceInterface &vki, vk::VkPhysicalDevice physicalDevice)
#if (DE_OS == DE_OS_WIN32)
        : m_hD3D11Lib(0)
        , m_hD3DX11Lib(0)
        , m_hD3DCompilerLib(0)
...
#endif
    {
        ...
        if (m_hD3DX11Lib)
            m_fnD3DX11CompileFromMemory =
                (LPD3DX11COMPILEFROMMEMORY)GetProcAddress(m_hD3DX11Lib, "D3DX11CompileFromMemory");
        else
        {
        ...
        LPCREATEDXGIFACTORY1 CreateDXGIFactory1 =
            (LPCREATEDXGIFACTORY1)GetProcAddress(m_hDxgiLib, "CreateDXGIFactory1");
        ...
        hr = m_fnD3D11CreateDevice(pAdapter, pAdapter ? D3D_DRIVER_TYPE_UNKNOWN : D3D_DRIVER_TYPE_HARDWARE, NULL,
                                   devflags, fLevel, DE_LENGTH_OF_ARRAY(fLevel), D3D11_SDK_VERSION, &m_pDevice, NULL,
                                   &m_pContext);
    ...
    virtual de::MovePtr<DX11Operation> build(const ResourceDescription &resourceDesc,
                                             vk::VkExternalMemoryHandleTypeFlagBits memoryHandleType) const
    {
#if (DE_OS == DE_OS_WIN32)
        return de::MovePtr<DX11Operation>(new DX11Operation(resourceDesc, memoryHandleType, m_pDevice, m_pContext,
                                                            m_fnD3DX11CompileFromMemory, m_fnD3DCompile));
#else
...
private:
#if (DE_OS == DE_OS_WIN32)
    typedef HRESULT(WINAPI *LPD3D11CREATEDEVICE)(IDXGIAdapter *, D3D_DRIVER_TYPE, HMODULE, UINT,
...
{
public:
    Win32KeyedMutexTestInstance(Context &context, TestConfig config);
...
tcu::TestStatus Win32KeyedMutexTestInstance::iterate(void)
{
    ...
    try
    {
        const uint32_t queueFamily = (uint32_t)m_queueNdx;

        const tcu::ScopedLogSection queuePairSection(log, "Queue-" + de::toString(queueFamily),
                                                     "Queue-" + de::toString(queueFamily));
        ...
        const vk::Unique<vk::VkCommandBuffer> commandBufferWrite(
        ...
        OperationContext operationContext(m_context, m_vki, m_vkd, m_physicalDevice, *m_device, allocator,
        ...
        const de::UniquePtr<DX11Operation> dx11Op(
            InstanceAndDevice::getSupportDX11()->build(m_config.resource, m_memoryHandleType));

        NativeHandle nativeHandleWrite = dx11Op->getNativeHandle(DX11Operation::BUFFER_VK_WRITE);
        const de::UniquePtr<Resource> resourceWrite(
            importResource(m_vkd, *m_device, m_config.resource, m_queueFamilyIndices, *m_supportReadOp,
                           *m_supportWriteOp, nativeHandleWrite, m_memoryHandleType));

        NativeHandle nativeHandleRead = dx11Op->getNativeHandle(DX11Operation::BUFFER_VK_READ);
        const de::UniquePtr<Resource> resourceRead(
            importResource(m_vkd, *m_device, m_config.resource, m_queueFamilyIndices, *m_supportReadOp,
                           *m_supportWriteOp, nativeHandleRead, m_memoryHandleType));
...
static void createTests(tcu::TestCaseGroup *group)
{
    ...
    const struct
    {
        ...
        {(vk::VkExternalMemoryHandleTypeFlagBits)0u, // DX11 doesn't support buffers with an NT handle
         vk::VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_BIT, "_nt"},
        {vk::VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_KMT_BIT,
         vk::VK_EXTERNAL_MEMORY_HANDLE_TYPE_D3D11_TEXTURE_KMT_BIT, "_kmt"},
    };
