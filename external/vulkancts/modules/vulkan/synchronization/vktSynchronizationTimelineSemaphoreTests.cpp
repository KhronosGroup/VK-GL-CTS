...
public:
    DeviceHostSyncTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                           SynchronizationType type, const ResourceDescription resourceDesc,
                           const OperationName writeOp, const OperationName readOp,
                           PipelineCacheData &pipelineCacheData)
        ...
        for (int writeOpNdx = 0; writeOpNdx < DE_LENGTH_OF_ARRAY(writeOps); ++writeOpNdx)
            for (int readOpNdx = 0; readOpNdx < DE_LENGTH_OF_ARRAY(readOps); ++readOpNdx)
            {
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    {
                        opGroup->addChild(new DeviceHostSyncTestCase(m_testCtx, name, "", m_type, resource, writeOp,
...
public:
    ...
    void init(void)
    {
        ...
        addFunctionCase(miscGroup.get(), "max_difference_value", "Timeline semaphore properties test", checkSupport,
                        maxDifferenceValueCase, m_type);
        addFunctionCase(miscGroup.get(), "initial_value", "Timeline semaphore initial value test", checkSupport,
                        initialValueCase, m_type);
...
public:
    WaitBeforeSignalTestInstance(Context &context, SynchronizationType type, const ResourceDescription &resourceDesc,
        ...
        , m_deviceDriver(de::MovePtr<DeviceDriver>(
              new DeviceDriver(context.getPlatformInterface(), SingletonDevice::getInstance(), *m_device)))
#else
        , m_deviceDriver(de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
              new DeviceDriverSC(context.getPlatformInterface(), SingletonDevice::getInstance(), *m_device,
        ...
        , m_allocator(new SimpleAllocator(*m_deviceDriver, *m_device,
                                          getPhysicalDeviceMemoryProperties(
                                              SingletonDevice::getDriver(),
                                              chooseDevice(SingletonDevice::getDriver(), SingletonDevice::getInstance(),
        ...
        , m_opContext(context, type, *m_deviceDriver, *m_device, *m_allocator, pipelineCacheData)
    {
        ...
        const VkPhysicalDevice physicalDevice = chooseDevice(
            SingletonDevice::getDriver(), SingletonDevice::getInstance(), context.getTestContext().getCommandLine());
        const std::vector<VkQueueFamilyProperties> queueFamilyProperties =
            getPhysicalDeviceQueueFamilyProperties(SingletonDevice::getDriver(), physicalDevice);
...
public:
    OneToNTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                   SynchronizationType type, const ResourceDescription resourceDesc, const OperationName writeOp,
