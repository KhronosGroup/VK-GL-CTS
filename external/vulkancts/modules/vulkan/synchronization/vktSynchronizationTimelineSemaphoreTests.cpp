...
void deviceSignal(const DeviceInterface &vk, const VkDevice device, const VkQueue queue, const VkFence fence,
                  const VkSemaphore semaphore, const uint64_t timelineValue)
{
...
{
public:
    HostWaitBeforeSignalTestInstance(Context &context) : TestInstance(context)
    {
    }
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        for (uint32_t i = 0; i < 12; i++)
        {
            ...
            const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
                ...
                &timelineValues.back(),                           // const uint64_t*    pWaitSemaphoreValues
                1u,                                               // uint32_t            signalSemaphoreValueCount
                &newTimelineValue,                                // const uint64_t*    pSignalSemaphoreValues
            };
            ...
            VK_CHECK(vk.queueSubmit(queue, (uint32_t)1u, &submitInfo, DE_NULL));
        ...
        for (uint32_t semIdx = 0; semIdx < semaphores.size(); semIdx++)
        {
            if (m_signalFromDevice)
            {
                deviceSignal(vk, device, queue, semIdx == (semaphores.size() - 1) ? *fence : DE_NULL,
                             semaphores[semIdx], timelineValues[semIdx]);
            }
            else
                hostSignal(vk, device, semaphores[semIdx], timelineValues[semIdx]);
        }
...
tcu::TestStatus initialValueCase(Context &context)
{
    ...
    const Unique<VkSemaphore> semaphoreInitialValue(
        createSemaphoreType(vk, device, VK_SEMAPHORE_TYPE_TIMELINE_KHR, 0, nonZeroValue));
    ...
    if (maxTimelineValueDifference != std::numeric_limits<uint64_t>::max())
    {
        ...
        const Unique<VkSemaphore> semaphoreMaxValue(
            createSemaphoreType(vk, device, VK_SEMAPHORE_TYPE_TIMELINE_KHR, 0, nonZeroMaxValue));
...
public:
    ...
    void init(void)
    {
        ...
        for (uint32_t caseIdx = 0; caseIdx < DE_LENGTH_OF_ARRAY(waitCases); caseIdx++)
            addChild(new WaitTestCase(m_testCtx, waitCases[caseIdx].name, waitCases[caseIdx].waitAll,
...
{
public:
    DeviceHostTestInstance(Context &context, const ResourceDescription &resourceDesc,
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        for (uint32_t iterIdx = 0; iterIdx < m_iterations.size(); iterIdx++)
        {
            // Write operation
            {
                const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
                    VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO, // VkStructureType sType;
                    DE_NULL,                                          // const void* pNext;
                    iterIdx == 0 ? 0u : 1u,                           // uint32_t            waitSemaphoreValueCount
                    iterIdx == 0 ? DE_NULL :
                                   &m_iterations[iterIdx - 1]->cpuValue, // const uint64_t*    pWaitSemaphoreValues
                    1u,                                 // uint32_t            signalSemaphoreValueCount
                    &m_iterations[iterIdx]->writeValue, // const uint64_t*    pSignalSemaphoreValues
                };
                const VkSubmitInfo submitInfo = {
                    VK_STRUCTURE_TYPE_SUBMIT_INFO,     // VkStructureType sType;
                    &timelineSubmitInfos[2 * iterIdx], // const void* pNext;
                    iterIdx == 0 ? 0u : 1u,            // uint32_t waitSemaphoreCount;
                    &semaphore.get(),                  // const VkSemaphore* pWaitSemaphores;
                    stageBits,
...

                {
                    ...
                    if (resource.getType() == RESOURCE_TYPE_IMAGE)
                    {
                        ...
                        const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
                            writeSync.accessMask, readSync.accessMask, writeSync.imageLayout, readSync.imageLayout,
                            resource.getImage().handle, resource.getImage().subresourceRange);
                        vk.cmdPipelineBarrier(cmdBuffers[2 * iterIdx], writeSync.stageMask, readSync.stageMask,
                                              (VkDependencyFlags)0, 0u, (const VkMemoryBarrier *)DE_NULL, 0u,
                                              (const VkBufferMemoryBarrier *)DE_NULL, 1u, &barrier);
                    }
                    else
                    {
                        const VkBufferMemoryBarrier barrier = makeBufferMemoryBarrier(
                            writeSync.accessMask, readSync.accessMask, resource.getBuffer().handle, 0, VK_WHOLE_SIZE);
                        vk.cmdPipelineBarrier(cmdBuffers[2 * iterIdx], writeSync.stageMask, readSync.stageMask,
                                              (VkDependencyFlags)0, 0u, (const VkMemoryBarrier *)DE_NULL, 1u, &barrier,
            ...
            // Read operation
            {
                const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
                    ...
                    DE_NULL,                                          // const void* pNext;
                    1u,                                               // uint32_t            waitSemaphoreValueCount
                    &m_iterations[iterIdx]->writeValue,               // const uint64_t*    pWaitSemaphoreValues
                    1u,                                               // uint32_t            signalSemaphoreValueCount
                    &m_iterations[iterIdx]->readValue,                // const uint64_t*    pSignalSemaphoreValues
                };
                const VkSubmitInfo submitInfo = {
                    VK_STRUCTURE_TYPE_SUBMIT_INFO,         // VkStructureType sType;
        ...
        VK_CHECK(vk.queueSubmit(queue, (uint32_t)submitInfos.size(), &submitInfos[0], DE_NULL));
...
{
public:
    DeviceHostSyncTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
public:
    ...
    void init(void)
    {
        ...
        for (int writeOpNdx = 0; writeOpNdx < DE_LENGTH_OF_ARRAY(writeOps); ++writeOpNdx)
            for (int readOpNdx = 0; readOpNdx < DE_LENGTH_OF_ARRAY(readOps); ++readOpNdx)
            {
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    {
                        opGroup->addChild(new DeviceHostSyncTestCase(m_testCtx, name, "", resource, writeOp, readOp,
                                                                     m_pipelineCacheData));
                        empty = false;
                    }
                }
...

        {
            ...
            addFunctionCase(miscGroup.get(), "max_difference_value", "Timeline semaphore properties test",
                            checkTimelineSupport, maxDifferenceValueCase);
            addFunctionCase(miscGroup.get(), "initial_value", "Timeline semaphore initial value test",
                            checkTimelineSupport, initialValueCase);
...
Move<VkDevice> createDevice(const Context &context)
{
    ...
    const VkDeviceCreateInfo deviceInfo                   = {
        ...
        1u,                          //uint32_t enabledExtensionCount;
    ...
    return createDevice(context.getPlatformInterface(), context.getInstance(), context.getInstanceInterface(),
...
{
    SingletonDevice(const Context &context) : m_logicalDevice(createDevice(context))
    {
    }
...
{
public:
    WaitBeforeSignalTestInstance(Context &context, const ResourceDescription &resourceDesc,
                                 const SharedPtr<OperationSupport> &writeOp, const SharedPtr<OperationSupport> &readOp,
                                 PipelineCacheData &pipelineCacheData)
        : TestInstance(context)
        , m_resourceDesc(resourceDesc)
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        for (uint32_t _iterIdx = 0; _iterIdx < (m_iterations.size() - 1); _iterIdx++)
        {
            ...
            const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
                ...
                DE_NULL,                                          // const void* pNext;
                1u,                                               // uint32_t            waitSemaphoreValueCount
                iterIdx == 0 ? &m_hostTimelineValue :
                               &m_iterations[iterIdx - 1]->timelineValue, // const uint64_t*    pWaitSemaphoreValues
                1u,                                    // uint32_t            signalSemaphoreValueCount
                &m_iterations[iterIdx]->timelineValue, // const uint64_t*    pSignalSemaphoreValues
            };
            const VkSubmitInfo submitInfo = {
                VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
                &timelineSubmitInfo,           // const void* pNext;
...

            {
                ...
                if (resource.getType() == RESOURCE_TYPE_IMAGE)
                {
                    ...
                    const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
                        writeSync.accessMask, readSync.accessMask, writeSync.imageLayout, readSync.imageLayout,
                        resource.getImage().handle, resource.getImage().subresourceRange,
                        m_iterations[iterIdx]->queueFamilyIdx, m_iterations[iterIdx + 1]->queueFamilyIdx);
                    vk.cmdPipelineBarrier(cmdBuffers[iterIdx], writeSync.stageMask, readSync.stageMask,
                                          (VkDependencyFlags)0, 0u, (const VkMemoryBarrier *)DE_NULL, 0u,
                                          (const VkBufferMemoryBarrier *)DE_NULL, 1u, &barrier);
                }
                else
                {
                    const VkBufferMemoryBarrier barrier = makeBufferMemoryBarrier(
                        ...
                        m_iterations[iterIdx]->queueFamilyIdx, m_iterations[iterIdx + 1]->queueFamilyIdx);
                    vk.cmdPipelineBarrier(cmdBuffers[iterIdx], writeSync.stageMask, readSync.stageMask,
                                          (VkDependencyFlags)0, 0u, (const VkMemoryBarrier *)DE_NULL, 1u, &barrier, 0u,
...
{
public:
    WaitBeforeSignalTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             const ResourceDescription resourceDesc, const OperationName writeOp,
                             const OperationName readOp, PipelineCacheData &pipelineCacheData)
        : TestCase(testCtx, name, description)
        , m_resourceDesc(resourceDesc)
        ...
        for (int writeOpNdx = 0; writeOpNdx < DE_LENGTH_OF_ARRAY(writeOps); ++writeOpNdx)
            for (int readOpNdx = 0; readOpNdx < DE_LENGTH_OF_ARRAY(readOps); ++readOpNdx)
            {
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    if (isResourceSupported(writeOp, resource) && isResourceSupported(readOp, resource))
                    {
                        opGroup->addChild(new WaitBeforeSignalTestCase(m_testCtx, name, "", resource, writeOp, readOp,
                                                                       m_pipelineCacheData));
                        empty = false;
                    }
                }
...
{
public:
    OneToNTestInstance(Context &context, const ResourceDescription &resourceDesc,
                       const SharedPtr<OperationSupport> &writeOp, const SharedPtr<OperationSupport> &readOp,
                       PipelineCacheData &pipelineCacheData)
        : TestInstance(context)
        , m_resourceDesc(resourceDesc)
                       ...
                       const QueueTimelineIteration &outIter, const Resource &resource)
    {
        ...
        if (resource.getType() == RESOURCE_TYPE_IMAGE)
        {
            ...
            const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
                writeSync.accessMask, readSync.accessMask, writeSync.imageLayout, readSync.imageLayout,
                resource.getImage().handle, resource.getImage().subresourceRange, inIter.queueFamilyIdx,
                outIter.queueFamilyIdx);
            vk.cmdPipelineBarrier(cmdBuffer, writeSync.stageMask, readSync.stageMask, (VkDependencyFlags)0, 0u,
        ...
        else
        {
            const VkBufferMemoryBarrier barrier =
                makeBufferMemoryBarrier(writeSync.accessMask, readSync.accessMask, resource.getBuffer().handle, 0,
                                        VK_WHOLE_SIZE, inIter.queueFamilyIdx, outIter.queueFamilyIdx);
            vk.cmdPipelineBarrier(cmdBuffer, writeSync.stageMask, readSync.stageMask, (VkDependencyFlags)0, 0u,
                                  (const VkMemoryBarrier *)DE_NULL, 1u, &barrier, 0u,
                                  (const VkImageMemoryBarrier *)DE_NULL);
        }
    }
                ...
                VkSemaphore semaphore, const uint64_t *waitValues, const uint32_t waitValuesCount)
    {
        ...
        const VkTimelineSemaphoreSubmitInfo timelineSubmitInfo = {
            VK_STRUCTURE_TYPE_TIMELINE_SEMAPHORE_SUBMIT_INFO, // VkStructureType sType;
            DE_NULL,                                          // const void* pNext;
            waitValuesCount,                                  // uint32_t            waitSemaphoreValueCount
            waitValues,                                       // const uint64_t*    pWaitSemaphoreValues
            1u,                                               // uint32_t            signalSemaphoreValueCount
            &iter.timelineValue,                              // const uint64_t*    pSignalSemaphoreValues
        };
        ...
        const VkSubmitInfo submitInfo = {
            VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
            &timelineSubmitInfo,           // const void* pNext;
            waitValuesCount,               // uint32_t waitSemaphoreCount;
        ...
        // Record command buffers
        {
            ...
            for (uint32_t readOpIdx = 0; readOpIdx < m_readIterations.size(); readOpIdx++)
            {
                ...
                recordBarrier(vk, **readPtrCmdBuffers[readOpIdx], *m_copyIterations[readOpIdx],
                              *m_readIterations[readOpIdx], *m_copyResources[readOpIdx]);
...
{
public:
    OneToNTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
        ...
        for (int writeOpNdx = 0; writeOpNdx < DE_LENGTH_OF_ARRAY(writeOps); ++writeOpNdx)
            for (int readOpNdx = 0; readOpNdx < DE_LENGTH_OF_ARRAY(readOps); ++readOpNdx)
            {
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    {
                        opGroup->addChild(
                            new OneToNTestCase(m_testCtx, name, "", resource, writeOp, readOp, m_pipelineCacheData));
                        empty = false;
                    }
                }
                if (!empty)
...
{
public:
    SparseBindCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                   const SparseBindParams &params);
...
SparseBindCase::SparseBindCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                               const SparseBindParams &params)
...
public:
    ...
    virtual void init(void)
    {
        ...
        for (int i = 0; i < DE_LENGTH_OF_ARRAY(kSparseBindCases); ++i)
            addChild(new SparseBindCase(m_testCtx, kSparseBindCases[i].name, kSparseBindCases[i].desc,
...
tcu::TestCaseGroup *createTimelineSemaphoreTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> basicTests(
        new tcu::TestCaseGroup(testCtx, "timeline_semaphore", "Timeline semaphore tests"));
