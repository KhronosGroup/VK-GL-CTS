...
{
public:
    QueueSubmitSignalOrderSharedTestInstance(Context &context, const SharedPtr<OperationSupport> writeOpSupport,
    ...
    tcu::TestStatus iterate(void)
    {
        ...
        UniquePtr<OperationContext> operationContextA(
            new OperationContext(m_context, m_pipelineCacheData, vkA, deviceA, *allocatorA));
        UniquePtr<OperationContext> operationContextB(
            new OperationContext(m_context, m_pipelineCacheData, vkB, *deviceB, *allocatorB));
        ...
        for (uint32_t iterIdx = 0; iterIdx < iterations.size(); iterIdx++)
        {
            ...
            if (m_resourceDesc.type == RESOURCE_TYPE_IMAGE)
            {
                ...
                iter.resourceA =
                    makeSharedPtr(new Resource(image, allocation, extent, m_resourceDesc.imageType,
                                               m_resourceDesc.imageFormat, subresourceRange, subresourceLayers));
            }
            else
            {
                const VkDeviceSize offset = 0u;
        ...
        for (uint32_t iterIdx = 0; iterIdx < iterations.size(); iterIdx++)
        {
...

            {
                ...
                if (resource.getType() == RESOURCE_TYPE_IMAGE)
                {
                    ...
                    vkA.cmdPipelineBarrier(cmdBuffersA.back(), writeSync.stageMask, readSync.stageMask,
        ...
        // operation done below.
        {
            ...
            for (uint32_t iterIdx = 0; iterIdx < iterations.size(); iterIdx++)
            {
                const VkTimelineSemaphoreSubmitInfoKHR timelineSubmitInfo = {
                    ...
                    &timelineValuesA[iterIdx], // const uint64_t*    pSignalSemaphoreValues
                };
                const VkSubmitInfo submitInfo = {
                    VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
                    m_semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE_KHR ? &timelineSubmitInfos[iterIdx] :
                                                                        DE_NULL, // const void* pNext;
                    m_semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE_KHR ? 1u : 0u, // uint32_t waitSemaphoreCount;
            ...
            VK_CHECK(vkA.queueSubmit(queueA, (uint32_t)submitInfos.size(), &submitInfos[0], *fenceA));
        }
        ...
        // when read operations kick in all writes have completed.
        {
            const VkTimelineSemaphoreSubmitInfoKHR timelineSubmitInfo = {
                ...
                &timelineValuesA.back(),                              // const uint64_t*    pWaitSemaphoreValues
                1u,                                                   // uint32_t            signalSemaphoreValueCount
                &timelineValuesB.back(),                              // const uint64_t*    pSignalSemaphoreValues
            };
            const VkSubmitInfo submitInfo = {
                VK_STRUCTURE_TYPE_SUBMIT_INFO, // VkStructureType sType;
                m_semaphoreType == VK_SEMAPHORE_TYPE_TIMELINE_KHR ? &timelineSubmitInfo : DE_NULL, // const void* pNext;
...
{
public:
    QueueSubmitSignalOrderSharedTestCase(tcu::TestContext &testCtx, const std::string &name, OperationName writeOp,
                                         OperationName readOp, const ResourceDescription &resourceDesc,
                                         VkExternalMemoryHandleTypeFlagBits memoryHandleType,
                                         VkSemaphoreType semaphoreType,
                                         VkExternalSemaphoreHandleTypeFlagBits semaphoreHandleType,
                                         PipelineCacheData &pipelineCacheData)
    ...
    {
        return new QueueSubmitSignalOrderSharedTestInstance(context, m_writeOpSupport, m_readOpSupport, m_resourceDesc,
                                                            m_memoryHandleType, m_semaphoreType, m_semaphoreHandleType,
                                                            m_pipelineCacheData);
    }
                    ...
                    if (isResourceSupported(writeOp, resource) && isResourceSupported(readOp, resource))
                    {
                        for (uint32_t exportIdx = 0; exportIdx < DE_LENGTH_OF_ARRAY(exportCases); exportIdx++)
                        {
                            ...
                            opGroup->addChild(new QueueSubmitSignalOrderSharedTestCase(
                                m_testCtx, caseName, writeOp, readOp, resource, exportCases[exportIdx].memoryType,
                                m_semaphoreType, exportCases[exportIdx].semaphoreType, m_pipelineCacheData));
                            empty = false;
                        }
                    }
                }
...
{
public:
    QueueSubmitSignalOrderTestInstance(Context &context, const SharedPtr<OperationSupport> writeOpSupport,
                                       const SharedPtr<OperationSupport> readOpSupport,
        ...
        , m_operationContext(
              new OperationContext(context, pipelineCacheData, m_deviceInterface, *m_device, *m_allocator))
        , m_queueA(DE_NULL)
        , m_queueB(DE_NULL)
        , m_rng(1234)

    {
        ...
        {
            if ((queueFamilyProperties[familyIdx].queueFlags & m_writeOpSupport->getQueueFlags(*m_operationContext)) ==
        ...
        for (uint32_t familyIdx = 0; familyIdx < queueFamilyProperties.size(); familyIdx++)
        {
            if ((queueFamilyProperties[familyIdx].queueFlags & m_readOpSupport->getQueueFlags(*m_operationContext)) ==
    ...
    tcu::TestStatus iterate(void)
    {
...

            {
                ...
                if (resource.getType() == RESOURCE_TYPE_IMAGE)
                {
                    ...
                    const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
                        writeSync.accessMask, readSync.accessMask, writeSync.imageLayout, readSync.imageLayout,
                        resource.getImage().handle, resource.getImage().subresourceRange);
                    vk.cmdPipelineBarrier(cmdBuffersA.back(), writeSync.stageMask, readSync.stageMask,
                                          (VkDependencyFlags)0, 0u, (const VkMemoryBarrier *)DE_NULL, 0u,
                                          (const VkBufferMemoryBarrier *)DE_NULL, 1u, &barrier);
                }
                else
        ...
        // operation done below.
        {
            ...
            VK_CHECK(vk.queueSubmit(m_queueA, (uint32_t)submitInfos.size(), &submitInfos[0], DE_NULL));
        }
...
{
public:
    QueueSubmitSignalOrderTestCase(tcu::TestContext &testCtx, const std::string &name, OperationName writeOp,
                                   OperationName readOp, const ResourceDescription &resourceDesc,
    ...
    TestInstance *createInstance(Context &context) const
    {
        return new QueueSubmitSignalOrderTestInstance(context, m_writeOpSupport, m_readOpSupport, m_resourceDesc,
        ...
        for (uint32_t writeOpIdx = 0; writeOpIdx < DE_LENGTH_OF_ARRAY(writeOps); writeOpIdx++)
            for (uint32_t readOpIdx = 0; readOpIdx < DE_LENGTH_OF_ARRAY(readOps); readOpIdx++)
            {
                ...
                for (int resourceNdx = 0; resourceNdx < DE_LENGTH_OF_ARRAY(s_resources); ++resourceNdx)
                {
                    ...
                    if (isResourceSupported(writeOp, resource) && isResourceSupported(readOp, resource))
                    {
                        opGroup->addChild(new QueueSubmitSignalOrderTestCase(m_testCtx, getResourceName(resource),
                                                                             writeOp, readOp, resource, m_semaphoreType,
                                                                             m_pipelineCacheData));
                        empty = false;
                    }
                }
...
{
    de::MovePtr<tcu::TestCaseGroup> orderingTests(
        new tcu::TestCaseGroup(testCtx, "signal_order", "Signal ordering tests"));
    ...
    orderingTests->addChild(
        new QueueSubmitSignalOrderTests(testCtx, VK_SEMAPHORE_TYPE_TIMELINE_KHR, "timeline_semaphore"));
    orderingTests->addChild(
        new QueueSubmitSignalOrderSharedTests(testCtx, VK_SEMAPHORE_TYPE_BINARY_KHR, "shared_binary_semaphore"));
    orderingTests->addChild(
        new QueueSubmitSignalOrderSharedTests(testCtx, VK_SEMAPHORE_TYPE_TIMELINE_KHR, "shared_timeline_semaphore"));
