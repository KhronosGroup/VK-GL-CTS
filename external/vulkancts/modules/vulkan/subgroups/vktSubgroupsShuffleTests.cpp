...
string getExtHeader(const CaseDefinition &caseDef)
{
    ...
    return eSource + "#extension GL_KHR_shader_subgroup_ballot: enable\n" +
...
const string getTestSource(const CaseDefinition &caseDef)
{
    const string id         = caseDef.opType == OPTYPE_SHUFFLE      ? "id_in" :
                              ...
                              caseDef.opType == OPTYPE_SHUFFLE_DOWN ? "gl_SubgroupInvocationID + id_in" :
                                                                      "";
    const string idInSource = caseDef.argType == ArgType::DYNAMIC ?
...
TestCaseGroup *createSubgroupsShuffleTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> graphicGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup shuffle category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup shuffle category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup shuffle category tests: framebuffer"));
#ifndef CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup shuffle category tests: ray tracing"));
    de::MovePtr<TestCaseGroup> meshGroup(
        new TestCaseGroup(testCtx, "mesh", "Subgroup shuffle category tests: mesh shading"));
#endif // CTS_USES_VULKANSC
            ...
            for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
            {
                for (const auto &argCase : argCases)
                {
                    ...
                    for (size_t groupSizeNdx = 0; groupSizeNdx < DE_LENGTH_OF_ARRAY(boolValues); ++groupSizeNdx)
                    {
                        ...
                        addFunctionCaseWithPrograms(computeGroup.get(), testName, "", supportedCheck, initPrograms,
                                                    test, caseDef);
                    }
                    ...
                    for (size_t groupSizeNdx = 0; groupSizeNdx < DE_LENGTH_OF_ARRAY(boolValues); ++groupSizeNdx)
                    {
                        for (const auto &stage : meshStages)
                        {
                            ...
                            addFunctionCaseWithPrograms(meshGroup.get(), testName, "", supportedCheck, initPrograms,
                                                        test, caseDef);
                        }
                    }
#endif // CTS_USES_VULKANSC
