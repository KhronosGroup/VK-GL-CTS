...
{
    return "layout(set = 0, binding = " + to_string(baseBinding) +
           ...
           ", std430) readonly buffer Buffer3\n"
           "{\n"
           "  uint data2[];\n"
           "};\n";
}
...
void initPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        src << extSource << "layout (local_size_x_id = 0, local_size_y_id = 1, local_size_z_id = 2) in;\n"
    ...
    else
    {
        ...
        {
            const string geometry = extSource +
                                    "layout(${TOPOLOGY}) in;\n"
                                    "layout(points, max_vertices = 1) out;\n"
        ...
        {
            const string fragment = extSource + "layout(location = 0) out uint result;\n" + declSource +
                                    "void main (void)\n"
                                    "{\n" +
                                    testSource +
                                    "  result = temp_res;\n"
    ...
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
             ...
             size <= subgroupSizeControlProperties.maxSubgroupSize; size *= 2)
        {
            tcu::TestStatus result =
                subgroups::makeComputeTest(context, VK_FORMAT_R32_UINT, inputData, 2, DE_NULL, checkCompute, size,
...
tcu::TestCaseGroup *createSubgroupsShuffleTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup shuffle category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup shuffle category tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup shuffle category tests: framebuffer"));
    ...
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "shuffle", "Subgroup shuffle category tests"));
