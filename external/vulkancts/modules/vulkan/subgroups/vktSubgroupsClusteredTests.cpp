...
std::string getTestSrc(CaseDefinition caseDef)
{
    ...
    for (uint32_t i = 1; i <= subgroups::maxSupportedSubgroupSize(); i *= 2)
    {
        bdy << "  {\n"
            ...
            << " op = " << getOpTypeName(caseDef.op) + "(data[gl_SubgroupInvocationID], clusterSize);\n"
...
tcu::TestCaseGroup *createSubgroupsClusteredTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup clustered category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup clustered category tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup clustered category tests: framebuffer"));
    ...
    for (size_t formatIndex = 0; formatIndex < formats.size(); ++formatIndex)
    {
        ...
        for (int opTypeIndex = 0; opTypeIndex < OPTYPE_CLUSTERED_LAST; ++opTypeIndex)
        {
            ...
            const std::string name = de::toLower(getOpTypeName(op)) + "_" + subgroups::getFormatNameForGLSL(format);

            {
                CaseDefinition caseDef = {op, VK_SHADER_STAGE_COMPUTE_BIT, format, de::SharedPtr<bool>(new bool),
                                          false};
                ...
                addFunctionCaseWithPrograms(computeGroup.get(), name + "_requiredsubgroupsize", "", supportedCheck,
                                            initPrograms, test, caseDef);
            }

            {
                const CaseDefinition caseDef = {op, VK_SHADER_STAGE_ALL_GRAPHICS, format, de::SharedPtr<bool>(new bool),
                                                false};
            ...
            {
                const CaseDefinition caseDef = {op, stages[stageIndex], format, de::SharedPtr<bool>(new bool), false};
                addFunctionCaseWithPrograms(framebufferGroup.get(),
                                            name + "_" + getShaderStageName(caseDef.shaderStage), "", supportedCheck,
                                            initFrameBufferPrograms, noSSBOtest, caseDef);
            }
        }
    }
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "clustered", "Subgroup clustered category tests"));
