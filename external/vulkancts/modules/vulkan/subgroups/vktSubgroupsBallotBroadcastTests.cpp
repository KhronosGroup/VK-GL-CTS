        ...
        if (isCompute)
        {
            if (caseDef.subgroupSizeControl)
                return subgroups::makeComputeTest(context, VK_FORMAT_R32_UINT, &inputData, 1, DE_NULL,
                                                  checkComputeOrMesh, caseDef.requiredSubgroupSize);
            else
                return subgroups::makeComputeTest(context, VK_FORMAT_R32_UINT, &inputData, 1, DE_NULL,
                                                  checkComputeOrMesh);
        }
...
TestCaseGroup *createSubgroupsBallotBroadcastTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> meshGroup(
        new TestCaseGroup(testCtx, "mesh", "Subgroup ballot broadcast category tests: mesh"));
    de::MovePtr<TestCaseGroup> meshGroupARB(
        new TestCaseGroup(testCtx, "mesh", "Subgroup ballot broadcast category tests: mesh"));
#endif // CTS_USES_VULKANSC
                ...
                for (size_t extNdx = 0; extNdx < DE_LENGTH_OF_ARRAY(boolValues); ++extNdx)
                {
                    ...
                    for (const auto &stage : meshStages)
                    {
                        ...
                        TestCaseGroup *testGroup = extShaderSubGroupBallotTests ? meshGroupARB.get() : meshGroup.get();
                        {
                            ...
                            addFunctionCaseWithPrograms(testGroup, name + stageName, supportedCheck, initPrograms, test,
                                                        caseDef);
                        }
                             ...
                             subgroupSize *= 2)
                        {
                            ...
                            const string testName =
                                name + "_requiredsubgroupsize" + de::toString(subgroupSize) + stageName;
