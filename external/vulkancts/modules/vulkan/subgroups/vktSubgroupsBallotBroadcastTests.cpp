...
void initFrameBufferPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    const vk::SpirvVersion spirvVersion =
        (caseDef.opType == OPTYPE_BROADCAST_NONCONST) ? vk::SPIRV_VERSION_1_5 : vk::SPIRV_VERSION_1_3;
...
tcu::TestCaseGroup *createSubgroupsBallotBroadcastTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup ballot broadcast category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup ballot broadcast category tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup ballot broadcast category tests: framebuffer"));

    de::MovePtr<tcu::TestCaseGroup> graphicGroupARB(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup ballot broadcast category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroupARB(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup ballot broadcast category tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroupARB(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup ballot broadcast category tests: framebuffer"));
        ...
        for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
        {
...

            {
                CaseDefinition caseDef = {
                    opTypeIndex, VK_SHADER_STAGE_COMPUTE_BIT, format, de::SharedPtr<bool>(new bool), false, false, 0};
                addFunctionCaseWithPrograms(computeGroup.get(), name, "", supportedCheck, initPrograms, test, caseDef);
                caseDef.extShaderSubGroupBallotTests = true;
                if (formatTypeIsSupportedARB)
                    addFunctionCaseWithPrograms(computeGroupARB.get(), name, "", supportedCheck, initPrograms, test,
                                                caseDef);

                for (int subgroupSize = 1; subgroupSize <= (int)subgroups::maxSupportedSubgroupSize();
                     subgroupSize *= 2)
                {
                    std::string testName                 = name + "_requiredsubgroupsize" + de::toString(subgroupSize);
                    caseDef.extShaderSubGroupBallotTests = false;
                    caseDef.subgroupSizeControl          = true;
                    ...
                    caseDef.extShaderSubGroupBallotTests = true;
                    if (formatTypeIsSupportedARB)
                        addFunctionCaseWithPrograms(computeGroupARB.get(), testName, "", supportedCheck, initPrograms,
                                                    test, caseDef);
                }
            }
            ...
            {
                CaseDefinition caseDef = {
                    opTypeIndex, VK_SHADER_STAGE_ALL_GRAPHICS, format, de::SharedPtr<bool>(new bool), false, false, 0};
                addFunctionCaseWithPrograms(graphicGroup.get(), name, "", supportedCheck, initPrograms, test, caseDef);
                caseDef.extShaderSubGroupBallotTests = true;
                if (formatTypeIsSupportedARB)
                    addFunctionCaseWithPrograms(graphicGroupARB.get(), name, "", supportedCheck, initPrograms, test,
                                                caseDef);
            }
            ...
            {
                CaseDefinition caseDef = {
                    opTypeIndex, stages[stageIndex], format, de::SharedPtr<bool>(new bool), false, false, 0};
                ...
                if (formatTypeIsSupportedARB)
                    addFunctionCaseWithPrograms(framebufferGroupARB.get(),
                                                name + getShaderStageName(caseDef.shaderStage), "", supportedCheck,
                                                initFrameBufferPrograms, noSSBOtest, caseDef);
            }
        }
    }

    de::MovePtr<tcu::TestCaseGroup> groupARB(
        new tcu::TestCaseGroup(testCtx, "ext_shader_subgroup_ballot", "VK_EXT_shader_subgroup_ballot category tests"));
    ...
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "ballot_broadcast", "Subgroup ballot broadcast category tests"));
