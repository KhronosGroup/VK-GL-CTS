...
tcu::TestStatus MultipleDispatchesUniformSubgroupSizeInstance::iterate(void)
{
    ...
    const VkDeviceSize bufferSize = maxLocalSize * 2 * sizeof(uint32_t);
    ...
    const Unique<VkDescriptorPool> descriptorPool(
        DescriptorPoolBuilder()
            .addType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC)
    ...
    const VkDescriptorBufferInfo resultInfo =
        makeDescriptorBufferInfo(*resultBuffer, 0u, (VkDeviceSize)bufferSize - maxLocalSize * sizeof(uint32_t));
    ...
    builder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
                        VK_DESCRIPTOR_TYPE_STORAGE_BUFFER_DYNAMIC, &resultInfo);
    ...
    for (uint32_t localSize1 = 8; localSize1 < maxLocalSize + 1; localSize1 *= 2)
    {
        for (uint32_t localSize2 = 8; localSize2 < maxLocalSize + 1; localSize2 *= 2)
        {
            ...
            Move<VkPipeline> computePipeline =
                createComputePipeline(vk, device, (VkPipelineCache)0u, &pipelineCreateInfo);
            Move<VkPipeline> computePipeline2 =
                createComputePipeline(vk, device, (VkPipelineCache)0u, &pipelineCreateInfo2);
            ...
            vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipelineLayout, 0u, 1u,
                                     &descriptorSet.get(), 1, &zero);
            vk.cmdBindPipeline(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipeline);
            vk.cmdDispatch(*cmdBuffer, 1, 1, 1);

            const auto barrier = makeBufferMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_SHADER_WRITE_BIT,
            ...
            const uint32_t offset = static_cast<uint32_t>(maxLocalSize * sizeof(uint32_t));
            vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipelineLayout, 0u, 1u,
                                     &descriptorSet.get(), 1u, &offset);
            ...
            const uint32_t *res = static_cast<const uint32_t *>(resultBufferMemory->getHostPtr());
            uint32_t size       = 0;
            ...
            for (uint32_t i = 0; i < maxLocalSize; i++)
            {
                if (res[i] != 0)
                {
                    size = res[i];
            ...
            for (uint32_t i = 0; i < maxLocalSize * 2; i++)
            {
                if (size != res[i] && res[i] != 0)
                    return tcu::TestStatus::fail("Subgroup size not uniform in command scope. " +
                                                 std::to_string(res[i]) + " != " + std::to_string(size));
            }
        }
    }
...
tcu::TestCaseGroup *createMultipleDispatchesUniformSubgroupSizeTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "multiple_dispatches", "Multiple dispatches uniform subgroup size tests"));
