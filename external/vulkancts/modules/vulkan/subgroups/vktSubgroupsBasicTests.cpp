...
static bool checkComputeSubgroupElect(const void *internalData, vector<const void *> datas,
...
static bool checkComputeSubgroupBarriers(const void *internalData, vector<const void *> datas,
                                         const uint32_t numWorkgroups[3], const uint32_t localSize[3], uint32_t)
{
    DE_UNREF(internalData);
...
void initComputePrograms(SourceCollections &programCollection, CaseDefinition &caseDef, const string &extensions,
...
TestStatus test(Context &context, const CaseDefinition caseDef)
{
    if (isAllComputeStages(caseDef.shaderStage))
    {
        ...
        if (OPTYPE_ELECT == caseDef.opType)
        {
            ...
            {
                TestStatus result = subgroups::makeComputeTest(
...
TestCaseGroup *createSubgroupsBasicTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> graphicGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup basic category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup basic category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup basic category tests: framebuffer"));
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup basic category tests: ray tracing"));
