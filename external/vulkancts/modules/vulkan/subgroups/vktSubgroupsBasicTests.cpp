...
static bool _checkFragmentSubgroupBarriersNoSSBO(std::vector<const void *> datas, uint32_t width, uint32_t height,
                                                 bool withImage)
{
...
static bool checkVertexPipelineStagesSubgroupElect(const void *internalData, std::vector<const void *> datas,
                                                   uint32_t width, uint32_t, bool multipleCallsPossible)
{
    DE_UNREF(internalData);
...
static bool _checkVertexPipelineStagesSubgroupBarriersNoSSBO(std::vector<const void *> datas, uint32_t width,
                                                             bool withImage)
{
    const float *const resultData = reinterpret_cast<const float *>(datas[0]);
...
static bool checkVertexPipelineStagesSubgroupBarriersWithImageNoSSBO(const void *internalData,
                                                                     std::vector<const void *> datas, uint32_t width,
                                                                     uint32_t)
{
    DE_UNREF(internalData);
...
static bool _checkTessellationEvaluationSubgroupBarriersNoSSBO(std::vector<const void *> datas, uint32_t width,
                                                               uint32_t, bool withImage)
{
    const float *const resultData = reinterpret_cast<const float *>(datas[0]);
...
void initFrameBufferPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    else
    {
        ...
        string color = (VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT == caseDef.shaderStage) ?
    ...
    if (OPTYPE_ELECT == caseDef.opType)
    {
        if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
        {
            ...
            programCollection.glslSources.add("comp")
                ...
                << vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u);
        }
        else
        {
...

            {
                ...
                vertex << "#version 450\n"
                       ...
                       << "void main (void)\n"
                       << "{\n"
                       << testSrc.str() << "  result[gl_VertexIndex] = tempRes;\n"
...

            {
                ...
                tesc << "#version 450\n"
                     ...
                     << "void main (void)\n"
                     << "{\n"
                     << testSrc.str() << "  result[gl_PrimitiveID] = tempRes;\n"
                     << "  if (gl_InvocationID == 0)\n"
                     << "  {\n"
                     << "    gl_TessLevelOuter[0] = 1.0f;\n"
                     << "    gl_TessLevelOuter[1] = 1.0f;\n"
                     ...
                     << "  gl_out[gl_InvocationID].gl_Position = gl_in[gl_InvocationID].gl_Position;\n"
                     << (*caseDef.geometryPointSizeSupported ?
                             "  gl_out[gl_InvocationID].gl_PointSize = gl_in[0].gl_PointSize;\n" :
                             "")
                     << "}\n";
                programCollection.glslSources.add("tesc")
                    << glu::TessellationControlSource(tesc.str())
                    << vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u);
            }

            {
                ...
                tese << "#version 450\n"
                     ...
                     << "void main (void)\n"
                     << "{\n"
                     << testSrc.str() << "  result[gl_PrimitiveID * 2 + uint(gl_TessCoord.x + 0.5)] = tempRes;\n"
                     << "  float pixelSize = 2.0f/1024.0f;\n"
...

            {
                ...
                geometry << "#version 450\n"
                         ...
                         << "void main (void)\n"
                         << "{\n"
                         << testSrc.str() << "  result[gl_PrimitiveIDIn] = tempRes;\n"
                         << "  gl_Position = gl_in[0].gl_Position;\n"
                         << (*caseDef.geometryPointSizeSupported ? "  gl_PointSize = gl_in[0].gl_PointSize;\n" : "")
                         << "  EmitVertex();\n"
                         << "  EndPrimitive();\n"
...

            {
                ...
                fragment << "#version 450\n"
                         ...
                         << "{\n"
                         << "  if (gl_HelperInvocation) return;\n"
                         << testSrc.str() << "  data = tempRes;\n"
                         << "}\n";
                programCollection.glslSources.add("fragment")
                    << glu::FragmentSource(fragment.str())
                    << vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u);
            }
        ...
        else
        {
...

            {
                const string tesc =
                    ...
                    "}\n";
                programCollection.glslSources.add("tesc")
                    << glu::TessellationControlSource(tesc)
                    << vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u);
            }

            {
                const string tese =
                    ...
                    (*caseDef.geometryPointSizeSupported ? "  gl_PointSize = gl_in[0].gl_PointSize;\n" : "") + "}\n";
                programCollection.glslSources.add("tese")
                    << glu::TessellationEvaluationSource(tese)
                    << vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u);
            }

            {
                const string geometry =
                    ...
                    "}\n";
                subgroups::addGeometryShadersFromTemplate(
                    geometry, vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u),
                    programCollection.glslSources);
            }
...
tcu::TestStatus test(Context &context, const CaseDefinition caseDef)
{
    ...
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        if (OPTYPE_ELECT == caseDef.opType)
        {
            ...
            log << tcu::TestLog::Message << "Testing required subgroup size range ["
            ...
            {
                tcu::TestStatus result = subgroups::makeComputeTest(
                    context, VK_FORMAT_R32_UINT, DE_NULL, 0u, DE_NULL, checkComputeSubgroupElect, size,
        ...
        else
        {
            ...
            {
                tcu::TestStatus result = subgroups::makeComputeTest(
                    context, VK_FORMAT_R32_UINT, inputDatas, inputDatasCount, DE_NULL, checkComputeSubgroupBarriers,
    ...
    else
    {
        ...
        VkShaderStageFlagBits stages =
            (VkShaderStageFlagBits)(caseDef.shaderStage & subgroupProperties.supportedStages);
...
{
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup basic category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup basic category tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup basic category tests: framebuffer"));
    ...
    for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
    {
        ...
        {
            CaseDefinition caseDef = {opTypeIndex, VK_SHADER_STAGE_COMPUTE_BIT, de::SharedPtr<bool>(new bool), false};
            addFunctionCaseWithPrograms(computeGroup.get(), op, "", supportedCheck, initPrograms, test, caseDef);
            caseDef.requiredSubgroupSize = true;
            addFunctionCaseWithPrograms(computeGroup.get(), op + "_requiredsubgroupsize", "", supportedCheck,
        ...
        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
            ...
            const CaseDefinition caseDefFrag = {opTypeIndex, stages[stageIndex], de::SharedPtr<bool>(new bool), false};
            addFunctionCaseWithPrograms(framebufferGroup.get(), op + "_" + getShaderStageName(caseDefFrag.shaderStage),
                                        "", supportedCheck, initFrameBufferPrograms, noSSBOtest, caseDefFrag);
        }
    }

    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(testCtx, "basic", "Subgroup basic category tests"));
