    ...
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
             ...
             size <= subgroupSizeControlProperties.maxSubgroupSize; size *= 2)
        {
            tcu::TestStatus result =
                subgroups::makeComputeTest(context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL, checkCompute, size,
...
tcu::TestCaseGroup *createSubgroupsShapeTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup shape category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup shape category tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup shape category tests: framebuffer"));
    ...
    for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
    {
        ...
        {
            const CaseDefinition caseDef = {opTypeIndex, stages[stageIndex], de::SharedPtr<bool>(new bool), false};
            addFunctionCaseWithPrograms(framebufferGroup.get(), op + "_" + getShaderStageName(caseDef.shaderStage), "",
                                        supportedCheck, initFrameBufferPrograms, noSSBOtest, caseDef);
        }
    }

    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(testCtx, "shape", "Subgroup shape category tests"));
