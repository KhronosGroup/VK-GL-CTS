...
const char *TestTypeSpirvOps[] = {
    "OpIEqual", "OpUGreaterThanEqual", "OpUGreaterThan", "OpULessThanEqual", "OpULessThan",
};
...
{
    return TestTypeSpirvBuiltins[static_cast<uint32_t>(testType)];
}
...
{
    return TestTypeMathOps[static_cast<uint32_t>(testType)];
}

static inline string getTestSpirvOp(TestType testType)
{
    return TestTypeSpirvOps[static_cast<uint32_t>(testType)];
}
...
static bool checkComputeOrMeshStage(const void *internalData, vector<const void *> datas,
    ...
    else if (VK_SHADER_STAGE_GEOMETRY_BIT == caseDef.shaderStage)
    {
        ...
        */
        ...
        geometry << "; SPIR-V\n"
                 ...
                 << mask << "OpDecorate %32 RelaxedPrecision\n"
                 << "OpDecorate %32 BuiltIn SubgroupSize\n"
                 << "OpDecorate %33 RelaxedPrecision\n"
                 << "OpDecorate %36 RelaxedPrecision\n"
                 << "OpDecorate %36 BuiltIn SubgroupLocalInvocationId\n"
                 ...
                 << (*caseDef.geometryPointSizeSupported ? "%125 = OpTypePointer Input %105\n"
                 ...
                 << "OpStore %124 %122\n"
                 << (*caseDef.geometryPointSizeSupported ? "%127 = OpAccessChain %125 %119 %115 %47\n"
                                                           "%128 = OpLoad %105 %127\n"
...
TestStatus noSSBOtest(Context &context, const CaseDefinition caseDef)
{
    ...
    case VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT:
        return makeTessellationEvaluationFrameBufferTest(context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL,
...
TestCaseGroup *createSubgroupsBuiltinMaskVarTests(TestContext &testCtx)
{
    de::MovePtr<TestCaseGroup> group(
        new TestCaseGroup(testCtx, "builtin_mask_var", "Subgroup builtin mask variable tests"));
    de::MovePtr<TestCaseGroup> graphicGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup builtin mask category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup builtin mask category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup builtin mask category tests: framebuffer"));
#ifndef CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup builtin mask category tests: ray tracing"));
    de::MovePtr<TestCaseGroup> meshGroup(
        new TestCaseGroup(testCtx, "mesh", "Subgroup builtin mask category tests: mesh shading"));
#endif // CTS_USES_VULKANSC
    const TestType allStagesBuiltinVars[] = {
        TEST_TYPE_SUBGROUP_EQ_MASK, TEST_TYPE_SUBGROUP_GE_MASK, TEST_TYPE_SUBGROUP_GT_MASK,
        TEST_TYPE_SUBGROUP_LE_MASK, TEST_TYPE_SUBGROUP_LT_MASK,
    };
        ...
        for (size_t groupSizeNdx = 0; groupSizeNdx < DE_LENGTH_OF_ARRAY(boolValues); ++groupSizeNdx)
        {
            for (const auto &stage : meshStages)
            {
                ...
                const string testName =
                    name + (requiredSubgroupSize ? "_requiredsubgroupsize" : "") + "_" + getShaderStageName(stage);
                const CaseDefinition caseDef = {
                    testType,                      //  TestType testType;
