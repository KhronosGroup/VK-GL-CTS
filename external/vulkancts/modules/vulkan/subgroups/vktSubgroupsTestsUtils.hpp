...
void initStdPrograms(vk::SourceCollections &programCollection, const vk::ShaderBuildOptions &buildOptions,
                     vk::VkShaderStageFlags shaderStage, vk::VkFormat format, bool gsPointSize, std::string extHeader,
                     std::string testSrc, std::string helperStr);
...
tcu::TestStatus allStages(Context &context, vk::VkFormat format, SSBOData *extraData, uint32_t extraDataCount,
                          const void *internalData, const VerificationFunctor &checkResult,
                          const vk::VkShaderStageFlags shaderStage);

tcu::TestStatus makeVertexFrameBufferTest(Context &context, vk::VkFormat format, SSBOData *extraData,
                                          uint32_t extraDataCount, const void *internalData,
                                          bool (*checkResult)(const void *internalData, std::vector<const void *> datas,
                                                              uint32_t width, uint32_t subgroupSize));

tcu::TestStatus makeFragmentFrameBufferTest(Context &context, vk::VkFormat format, SSBOData *extraData,
                                            uint32_t extraDataCount, const void *internalData,
                                            bool (*checkResult)(const void *internalData,
                                                                std::vector<const void *> datas, uint32_t width,
                                                                uint32_t height, uint32_t subgroupSize));

tcu::TestStatus makeComputeTest(Context &context, vk::VkFormat format, SSBOData *inputs, uint32_t inputsCount,
...
tcu::TestStatus makeGeometryFrameBufferTestRequiredSubgroupSize(
    Context &context, vk::VkFormat format, SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width,
                        uint32_t subgroupSize),
    const uint32_t geometryShaderStageCreateFlags = 0u, const uint32_t requiredSubgroupSize = 0u);
...
tcu::TestStatus makeVertexFrameBufferTestRequiredSubgroupSize(
    Context &context, vk::VkFormat format, SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width,
                        uint32_t subgroupSize),
    const uint32_t vertexShaderStageCreateFlags = 0u, const uint32_t requiredSubgroupSize = 0u);

tcu::TestStatus makeFragmentFrameBufferTestRequiredSubgroupSize(
    Context &context, vk::VkFormat format, SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width, uint32_t height,
                        uint32_t subgroupSize),
    const uint32_t fragmentShaderStageCreateFlags = 0u, const uint32_t requiredSubgroupSize = 0u);

tcu::TestStatus makeComputeTestRequiredSubgroupSize(
    Context &context, vk::VkFormat format, SSBOData *inputs, uint32_t inputsCount, const void *internalData,
