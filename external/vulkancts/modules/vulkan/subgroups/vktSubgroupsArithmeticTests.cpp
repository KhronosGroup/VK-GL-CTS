...
std::string getTestSrc(CaseDefinition caseDef)
{
    ...
    return "  uvec4 mask = subgroupBallot(true);\n" + indexVars + "  " +
           subgroups::getFormatNameForGLSL(caseDef.format) + " ref = " + getIdentity(caseDef.op, caseDef.format) +
           ";\n"
           "  tempRes = 0;\n"
...
tcu::TestCaseGroup *createSubgroupsArithmeticTests(tcu::TestContext &testCtx)
{
    ...
    for (size_t formatIndex = 0; formatIndex < formats.size(); ++formatIndex)
    {
        ...
        for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
        {
            ...
            {
                CaseDefinition caseDef = {op,   st, VK_SHADER_STAGE_COMPUTE_BIT, format, de::SharedPtr<bool>(new bool),
                                          false};
...

            {
                const CaseDefinition caseDef = {
                    op, st, VK_SHADER_STAGE_ALL_GRAPHICS, format, de::SharedPtr<bool>(new bool), false};
                addFunctionCaseWithPrograms(graphicGroup.get(), name, "", supportedCheck, initPrograms, test, caseDef);
            }

            for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
            {
                const CaseDefinition caseDef = {op,   st, stages[stageIndex], format, de::SharedPtr<bool>(new bool),
                                                false};
    ...
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "arithmetic", "Subgroup arithmetic category tests"));
