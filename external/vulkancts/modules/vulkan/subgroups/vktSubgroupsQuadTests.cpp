...
void initFrameBufferPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    const SpirvVersion spirvVersion =
        (caseDef.opType == OPTYPE_QUAD_BROADCAST_NONCONST) ? SPIRV_VERSION_1_5 : SPIRV_VERSION_1_3;
...
void initPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    const SpirvVersion spirvVersion = spirv15required ? SPIRV_VERSION_1_5 :
                                      spirv14required ? SPIRV_VERSION_1_4 :
                                                        SPIRV_VERSION_1_3;
    const ShaderBuildOptions buildOptions(programCollection.usedVulkanVersion, spirvVersion, 0u,
                                          (spirv14required && !spirv15required));
...
TestCaseGroup *createSubgroupsQuadTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> graphicGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup arithmetic category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup arithmetic category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup arithmetic category tests: framebuffer"));
#ifndef CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup arithmetic category tests: ray tracing"));
    de::MovePtr<TestCaseGroup> meshGroup(
        new TestCaseGroup(testCtx, "mesh", "Subgroup arithmetic category tests: mesh shading"));
#endif // CTS_USES_VULKANSC
...
#ifndef CTS_USES_VULKANSC
    {
        ...
        for (size_t formatIndex = 0; formatIndex < formats.size(); ++formatIndex)
        {
            ...
            for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
            {
                ...
                addFunctionCaseWithPrograms(raytracingGroup.get(), testName, supportedCheck, initPrograms, test,
                                            caseDef);
            }
        }
    }
