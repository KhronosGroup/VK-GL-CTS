                    ...
                    uint32_t numWorkGroupInvocations)
{
    ...
    const uint32_t localSizeX = gcd(numWorkGroupInvocations, maxWorkGroupSize[0]);
    const uint32_t localSizeY = gcd(deMax32(numWorkGroupInvocations / localSizeX, 1u), maxWorkGroupSize[1]);
...
TestStatus test(Context &context, const CaseDefinition caseDef)
{
    ...
    else if (isAllMeshShadingStages(caseDef.shaderStage))
    {
        ...
        return subgroups::makeMeshTestRequiredSubgroupSize(context, VK_FORMAT_R32_UINT, DE_NULL, 0, &internalData,
                                                           checkCompute, caseDef.pipelineShaderStageCreateFlags,
                                                           numWorkgroups, false, subgroupSize, localSizesToTest,
                                                           localSizesToTestCount);
    }
...
TestStatus testRequireSubgroupSize(Context &context, const CaseDefinition caseDef)
{
    if (isAllComputeStages(caseDef.shaderStage))
    {
        ...
        return subgroups::makeComputeTestRequiredSubgroupSize(
            context, VK_FORMAT_R32G32B32A32_UINT, DE_NULL, 0, &internalData, checkResult,
...
TestCaseGroup *createSubgroupsSizeControlTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> meshGroup(
        new TestCaseGroup(testCtx, "mesh", "Subgroup size control category tests: mesh shading"));
#endif // CTS_USES_VULKANSC
    ...
    for (const auto &params : testParams)
    {
        ...
        for (const auto &stage : meshStages)
        {
            ...
            addFunctionCaseWithPrograms(meshGroup.get(), name, "", supportedCheckFeatures, initPrograms, test,
                                        caseDefMesh);
        }
#endif // CTS_USES_VULKANSC
