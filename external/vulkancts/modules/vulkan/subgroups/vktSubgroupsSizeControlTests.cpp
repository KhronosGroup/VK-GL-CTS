...
uint32_t gcd(uint32_t a, uint32_t b)
{
    ...
    {
        uint32_t greater = max(a, b);
        uint32_t lesser  = min(a, b);

        return gcd(lesser, greater % lesser);
    }
}

UVec3 getLocalSizes(const VkPhysicalDeviceProperties &physicalDeviceProperties, uint32_t numWorkGroupInvocations)
{
    DE_ASSERT(numWorkGroupInvocations <= physicalDeviceProperties.limits.maxComputeWorkGroupInvocations);
    const uint32_t localSizeX =
        gcd(numWorkGroupInvocations, physicalDeviceProperties.limits.maxComputeWorkGroupSize[0]);
    const uint32_t localSizeY = gcd(deMax32(numWorkGroupInvocations / localSizeX, 1u),
                                    physicalDeviceProperties.limits.maxComputeWorkGroupSize[1]);
    const uint32_t localSizeZ = deMax32(numWorkGroupInvocations / (localSizeX * localSizeY), 1u);
...
TestCaseGroup *createSubgroupsSizeControlTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup size control category tests: framebuffer"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup size control category tests: compute"));
    de::MovePtr<TestCaseGroup> graphicsGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup size control category tests: graphics"));
#ifndef CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup size control category tests: ray tracing"));
#endif // CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> genericGroup(
        new TestCaseGroup(testCtx, "generic", "Subgroup size control category tests: generic"));
...
#ifndef CTS_USES_VULKANSC
        const CaseDefinition caseDefAllRaytracing = {
            VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT, SHADER_STAGE_ALL_RAY_TRACING, false,
            REQUIRED_SUBGROUP_SIZE_NONE, de::SharedPtr<bool>(new bool)};
        addFunctionCaseWithPrograms(raytracingGroup.get(), "allow_varying_subgroup_size", "",
    ...
    // Tests to check setting a required subgroup size value.
    {
        const CaseDefinition caseDefAllGraphicsMax = {0u, VK_SHADER_STAGE_ALL_GRAPHICS, false,
                                                      REQUIRED_SUBGROUP_SIZE_MAX, de::SharedPtr<bool>(new bool)};
...
#ifndef CTS_USES_VULKANSC
        const CaseDefinition caseDefAllRaytracingMax = {0u, SHADER_STAGE_ALL_RAY_TRACING, false,
                                                        REQUIRED_SUBGROUP_SIZE_MAX, de::SharedPtr<bool>(new bool)};
        addFunctionCaseWithPrograms(raytracingGroup.get(), "required_subgroup_size_max", "",
                                    supportedCheckFeaturesShader, initPrograms, testRequireSubgroupSize,
                                    caseDefAllRaytracingMax);
#endif // CTS_USES_VULKANSC

        const CaseDefinition caseDefAllGraphicsMin = {0u, VK_SHADER_STAGE_ALL_GRAPHICS, false,
                                                      REQUIRED_SUBGROUP_SIZE_MIN, de::SharedPtr<bool>(new bool)};
...
#ifndef CTS_USES_VULKANSC
        const CaseDefinition caseDefAllRaytracingMin = {0u, SHADER_STAGE_ALL_RAY_TRACING, false,
                                                        REQUIRED_SUBGROUP_SIZE_MIN, de::SharedPtr<bool>(new bool)};
        addFunctionCaseWithPrograms(raytracingGroup.get(), "required_subgroup_size_min", "",
                                    supportedCheckFeaturesShader, initPrograms, testRequireSubgroupSize,
                                    caseDefAllRaytracingMin);
#endif // CTS_USES_VULKANSC
        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
