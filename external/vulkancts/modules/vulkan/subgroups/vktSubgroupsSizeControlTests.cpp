...
uint32_t gcd(uint32_t a, uint32_t b)
{
    ...
    {
        uint32_t greater = std::max(a, b);
        uint32_t lesser  = std::min(a, b);

        return gcd(lesser, greater % lesser);
    }
}

void getLocalSizes(VkPhysicalDeviceProperties physicalDeviceProperties, uint32_t numWorkGroupInvocations,
                   uint32_t &localSizeX, uint32_t &localSizeY, uint32_t &localSizeZ)
{
    ...
    localSizeY = gcd(deMax32(numWorkGroupInvocations / localSizeX, 1u),
...
uint32_t getRequiredSubgroupSizeFromMode(Context &context, const CaseDefinition caseDef,
                                         VkPhysicalDeviceSubgroupSizeControlPropertiesEXT subgroupSizeControlProperties)
{
    ...
    case REQUIRED_SUBGROUP_SIZE_NONE:
        return vkt::subgroups::getSubgroupSize(context);
    default:
                                      ...
                                      uint32_t)
{
    ...
    for (uint32_t i = 0; i < width; i++)
    {
        ...
        {
            log << tcu::TestLog::Message << "gl_SubgroupSize (" << data[i] << ") value is outside limits ("
        ...
        {
            log << tcu::TestLog::Message << "gl_SubgroupSize (" << data[i]
...
static bool checkFragmentPipelineStages(const void *internalData, std::vector<const void *> datas, uint32_t width,
                                        uint32_t height, uint32_t)
{
    ...
    for (uint32_t x = 0u; x < width; ++x)
    {
        for (uint32_t y = 0u; y < height; ++y)
        {
            ...
            {
                log << tcu::TestLog::Message << "gl_SubgroupSize (" << data[ndx] << ") value is outside limits ("
                    << subgroupSizeControlProperties.minSubgroupSize << ", "
                    << subgroupSizeControlProperties.maxSubgroupSize << ")" << tcu::TestLog::EndMessage;
                return false;
            }
            ...
            {
                log << tcu::TestLog::Message << "gl_SubgroupSize (" << data[ndx]
                    << ") is not equal to the required subgroup size value (" << checkInternalData->requiredSubgroupSize
                    << ")" << tcu::TestLog::EndMessage;
                return false;
            }
    ...
    for (uint32_t i = 0; i < width; i++)
    {
            ...
            data[i] != checkInternalData->requiredSubgroupSize)
        {
            log << tcu::TestLog::Message << "[" << localSize[0] << ", " << localSize[1] << ", " << localSize[2] << "] "
                ...
                << checkInternalData->requiredSubgroupSize << ")" << tcu::TestLog::EndMessage;
            return false;
        }
    }
                                    ...
                                    const uint32_t numWorkgroups[3], const uint32_t localSize[3], uint32_t)
{
    ...
    uint32_t numSubgroups = (localSize[0] * localSize[1] * localSize[2]) / checkInternalData->requiredSubgroupSize;
    ...
    for (uint32_t i = 0; i < width; i++)
    {
        ...
        if (data[i].x() != data[i].y())
        {
            log << tcu::TestLog::Message << "[" << localSize[0] << ", " << localSize[1] << ", " << localSize[2] << "] "
                ...
                << data[i].y() << ")" << tcu::TestLog::EndMessage;
            return false;
        }

        if (checkInternalData->caseDef.pipelineShaderStageCreateFlags ==
                VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT &&
            data[i].x() != checkInternalData->requiredSubgroupSize)
        {
            log << tcu::TestLog::Message << "[" << localSize[0] << ", " << localSize[1] << ", " << localSize[2] << "] "
                << "expected subgroupSize (" << checkInternalData->requiredSubgroupSize
                << ") doesn't match gl_SubgroupSize ( " << data[i].x() << ")" << tcu::TestLog::EndMessage;
            return false;
        }
...
tcu::TestStatus noSSBOtest(Context &context, const CaseDefinition caseDef)
{
    ...
    if (VK_SHADER_STAGE_VERTEX_BIT == caseDef.shaderStage)
        return subgroups::makeVertexFrameBufferTestRequiredSubgroupSize(context, VK_FORMAT_R32_UINT, DE_NULL, 0,
                                                                        &internalData, checkVertexPipelineStages,
                                                                        caseDef.pipelineShaderStageCreateFlags, 0u);
    else if (VK_SHADER_STAGE_GEOMETRY_BIT == caseDef.shaderStage)
        return subgroups::makeGeometryFrameBufferTestRequiredSubgroupSize(context, VK_FORMAT_R32_UINT, DE_NULL, 0,
                                                                          &internalData, checkVertexPipelineStages,
                                                                          caseDef.pipelineShaderStageCreateFlags, 0u);
        ...
        return subgroups::makeTessellationEvaluationFrameBufferTestRequiredSubgroupSize(
            context, VK_FORMAT_R32_UINT, DE_NULL, 0, &internalData, checkVertexPipelineStages, caseDef.shaderStage,
            caseDef.pipelineShaderStageCreateFlags, 0u);
    else if (VK_SHADER_STAGE_FRAGMENT_BIT == caseDef.shaderStage)
        return subgroups::makeFragmentFrameBufferTestRequiredSubgroupSize(context, VK_FORMAT_R32_UINT, DE_NULL, 0,
                                                                          &internalData, checkFragmentPipelineStages,
                                                                          caseDef.pipelineShaderStageCreateFlags, 0u);
...
tcu::TestStatus test(Context &context, const CaseDefinition caseDef)
{
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        uint32_t subgroupSize           = vkt::subgroups::getSubgroupSize(context);

        VkPhysicalDeviceProperties physicalDeviceProperties;
        context.getInstanceInterface().getPhysicalDeviceProperties(context.getPhysicalDevice(),
        ...
        getLocalSizes(physicalDeviceProperties, physicalDeviceProperties.limits.maxComputeWorkGroupInvocations,
                      localSizeX, localSizeY, localSizeZ);

        const uint32_t localSizesToTestCount                = 16;
        uint32_t localSizesToTest[localSizesToTestCount][3] = {
            {1, 1, 1},  {32, 4, 1},  {32, 1, 4},           {1, 32, 4},           {1, 4, 32},
            {4, 1, 32}, {4, 32, 1},  {subgroupSize, 1, 1}, {1, subgroupSize, 1}, {1, 1, subgroupSize},
            {3, 5, 7},  {128, 1, 1}, {1, 128, 1},          {1, 1, 64},           {localSizeX, localSizeY, localSizeZ},
        ...
        return subgroups::makeComputeTestRequiredSubgroupSize(context, VK_FORMAT_R32_UINT, DE_NULL, 0, &internalData,
                                                              checkCompute, caseDef.pipelineShaderStageCreateFlags,
                                                              numWorkgroups, false, subgroupSize, localSizesToTest,
    ...
    else
    {
        ...
        return subgroups::allStagesRequiredSubgroupSize(
            ...
            caseDef.pipelineShaderStageCreateFlags, caseDef.pipelineShaderStageCreateFlags,
            caseDef.pipelineShaderStageCreateFlags, DE_NULL);
    }
...
tcu::TestStatus testRequireFullSubgroups(Context &context, const CaseDefinition caseDef)
{
    ...
    uint32_t localSizeX, localSizeY, localSizeZ;
    // Calculate the local workgroup sizes to exercise the maximum supported by the driver
    getLocalSizes(physicalDeviceProperties, physicalDeviceProperties.limits.maxComputeWorkGroupInvocations, localSizeX,
                  localSizeY, localSizeZ);

    const uint32_t subgroupSize = vkt::subgroups::getSubgroupSize(context);
    ...
    return subgroups::makeComputeTestRequiredSubgroupSize(context, VK_FORMAT_R32G32B32A32_UINT, DE_NULL, 0,
                                                          &internalData, checkComputeRequireFull,
                                                          caseDef.pipelineShaderStageCreateFlags, numWorkgroups, false,
...
tcu::TestStatus testRequireSubgroupSize(Context &context, const CaseDefinition caseDef)
{
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        const uint32_t maxTotalLocalSize =
            deMin32(requiredSubgroupSize * subgroupSizeControlProperties.maxComputeWorkgroupSubgroups,
        ...
        const uint32_t localSizesToTestCount                = 5;
        uint32_t localSizesToTest[localSizesToTestCount][3] = {
        ...
        return subgroups::makeComputeTestRequiredSubgroupSize(
            context, VK_FORMAT_R32G32B32A32_UINT, DE_NULL, 0, &internalData,
            caseDef.pipelineShaderStageCreateFlags == VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT ?
                checkComputeRequireFull :
                checkCompute,
            caseDef.pipelineShaderStageCreateFlags, numWorkgroups, true, requiredSubgroupSize, localSizesToTest,
            localSizesToTestCount);
    }
    else
    {
...
tcu::TestStatus noSSBOtestRequireSubgroupSize(Context &context, const CaseDefinition caseDef)
{
    ...
    uint32_t requiredSubgroupSize = getRequiredSubgroupSizeFromMode(context, caseDef, subgroupSizeControlProperties);
    struct internalDataStruct internalData = {
        &context,
        caseDef,
        requiredSubgroupSize,
    ...
    if (VK_SHADER_STAGE_VERTEX_BIT == caseDef.shaderStage)
        return subgroups::makeVertexFrameBufferTestRequiredSubgroupSize(
            context, VK_FORMAT_R32_UINT, DE_NULL, 0, &internalData, checkVertexPipelineStages,
            caseDef.pipelineShaderStageCreateFlags, requiredSubgroupSize);
    else if (VK_SHADER_STAGE_GEOMETRY_BIT == caseDef.shaderStage)
        return subgroups::makeGeometryFrameBufferTestRequiredSubgroupSize(
            context, VK_FORMAT_R32_UINT, DE_NULL, 0, &internalData, checkVertexPipelineStages,
            caseDef.pipelineShaderStageCreateFlags, requiredSubgroupSize);
    else if ((VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT | VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) &
             caseDef.shaderStage)
        ...
        return subgroups::makeFragmentFrameBufferTestRequiredSubgroupSize(
            context, VK_FORMAT_R32_UINT, DE_NULL, 0, &internalData, checkFragmentPipelineStages,
            caseDef.pipelineShaderStageCreateFlags, requiredSubgroupSize);
    else
        TCU_THROW(InternalError, "Unhandled shader stage");
}
...
tcu::TestCaseGroup *createSubgroupsSizeControlTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "size_control", "VK_EXT_subgroup_size_control tests"));

    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup size control category tests: framebuffer"));

    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup size control category tests: compute"));

    de::MovePtr<tcu::TestCaseGroup> graphicsGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup size control category tests: graphics"));

    de::MovePtr<tcu::TestCaseGroup> genericGroup(
        new tcu::TestCaseGroup(testCtx, "generic", "Subgroup size control category tests: generic"));
    ...
    {
        const CaseDefinition caseDefCompute = {VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT,
                                               VK_SHADER_STAGE_COMPUTE_BIT, false, REQUIRED_SUBGROUP_SIZE_NONE};
        addFunctionCaseWithPrograms(computeGroup.get(), "allow_varying_subgroup_size", "", supportedCheckFeatures,
                                    initPrograms, test, caseDefCompute);
        const CaseDefinition caseDefAllGraphics = {VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT,
                                                   VK_SHADER_STAGE_ALL_GRAPHICS, false, REQUIRED_SUBGROUP_SIZE_NONE};
        addFunctionCaseWithPrograms(graphicsGroup.get(), "allow_varying_subgroup_size", "", supportedCheckFeatures,
                                    initPrograms, test, caseDefAllGraphics);

        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
            const CaseDefinition caseDefStage = {VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT,
                                                 stages[stageIndex], false, REQUIRED_SUBGROUP_SIZE_NONE};
            addFunctionCaseWithPrograms(framebufferGroup.get(),
                                        getShaderStageName(caseDefStage.shaderStage) + "_allow_varying_subgroup_size",
                                        "", supportedCheckFeatures, initFrameBufferPrograms, noSSBOtest, caseDefStage);
        }
    }

    // Require full subgroups case (only compute shaders).
    {
        const CaseDefinition caseDef = {VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT,
                                        VK_SHADER_STAGE_COMPUTE_BIT, true, REQUIRED_SUBGROUP_SIZE_NONE};
        addFunctionCaseWithPrograms(computeGroup.get(), "require_full_subgroups", "", supportedCheckFeatures,
    ...
    {
        uint32_t flags = VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT |
                         VK_PIPELINE_SHADER_STAGE_CREATE_ALLOW_VARYING_SUBGROUP_SIZE_BIT_EXT;
        const CaseDefinition caseDef = {flags, VK_SHADER_STAGE_COMPUTE_BIT, true, REQUIRED_SUBGROUP_SIZE_NONE};
        addFunctionCaseWithPrograms(computeGroup.get(), "require_full_subgroups_allow_varying_subgroup_size", "",
    ...
    {
        const CaseDefinition caseDefAllGraphicsMax = {0u, VK_SHADER_STAGE_ALL_GRAPHICS, false,
                                                      REQUIRED_SUBGROUP_SIZE_MAX};
        addFunctionCaseWithPrograms(graphicsGroup.get(), "required_subgroup_size_max", "", supportedCheckFeatures,
                                    initPrograms, testRequireSubgroupSize, caseDefAllGraphicsMax);
        const CaseDefinition caseDefComputeMax = {0u, VK_SHADER_STAGE_COMPUTE_BIT, false, REQUIRED_SUBGROUP_SIZE_MAX};
        ...
        const CaseDefinition caseDefAllGraphicsMin = {0u, VK_SHADER_STAGE_ALL_GRAPHICS, false,
                                                      REQUIRED_SUBGROUP_SIZE_MIN};
        addFunctionCaseWithPrograms(graphicsGroup.get(), "required_subgroup_size_min", "", supportedCheckFeatures,
                                    initPrograms, testRequireSubgroupSize, caseDefAllGraphicsMin);
        const CaseDefinition caseDefComputeMin = {0u, VK_SHADER_STAGE_COMPUTE_BIT, false, REQUIRED_SUBGROUP_SIZE_MIN};
        ...
        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
            const CaseDefinition caseDefStageMax = {0u, stages[stageIndex], false, REQUIRED_SUBGROUP_SIZE_MAX};
            addFunctionCaseWithPrograms(
                ...
                "", supportedCheckFeatures, initFrameBufferPrograms, noSSBOtestRequireSubgroupSize, caseDefStageMax);
            const CaseDefinition caseDefStageMin = {0u, stages[stageIndex], false, REQUIRED_SUBGROUP_SIZE_MIN};
            addFunctionCaseWithPrograms(
                framebufferGroup.get(), getShaderStageName(caseDefStageMin.shaderStage) + "_required_subgroup_size_min",
                "", supportedCheckFeatures, initFrameBufferPrograms, noSSBOtestRequireSubgroupSize, caseDefStageMin);
        }
    }
    ...
    {
        uint32_t flags                  = VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT;
        const CaseDefinition caseDefMax = {flags, VK_SHADER_STAGE_COMPUTE_BIT, true, REQUIRED_SUBGROUP_SIZE_MAX};
        addFunctionCaseWithPrograms(computeGroup.get(), "required_subgroup_size_max_require_full_subgroups", "",
                                    supportedCheckFeatures, initProgramsRequireFull, testRequireSubgroupSize,
                                    caseDefMax);
        const CaseDefinition caseDefMin = {flags, VK_SHADER_STAGE_COMPUTE_BIT, true, REQUIRED_SUBGROUP_SIZE_MIN};
        addFunctionCaseWithPrograms(computeGroup.get(), "required_subgroup_size_min_require_full_subgroups", "",
                                    supportedCheckFeatures, initProgramsRequireFull, testRequireSubgroupSize,
                                    caseDefMin);
    }
