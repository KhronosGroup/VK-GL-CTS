...
tcu::TestStatus testRequireSubgroupSize(Context &context, const CaseDefinition caseDef)
{
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        const uint32_t maxTotalLocalSize = (uint32_t)std::min<uint64_t>(
...
tcu::TestCaseGroup *createSubgroupsSizeControlTests(tcu::TestContext &testCtx)
{
    ...
    // Allow varying subgroup case.
    {
        ...
        addFunctionCaseWithPrograms(graphicsGroup.get(), "allow_varying_subgroup_size", "",
                                    supportedCheckFeaturesShader, initPrograms, test, caseDefAllGraphics);
        ...
        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
            ...
            addFunctionCaseWithPrograms(
                framebufferGroup.get(), getShaderStageName(caseDefStage.shaderStage) + "_allow_varying_subgroup_size",
                "", supportedCheckFeaturesShader, initFrameBufferPrograms, noSSBOtest, caseDefStage);
        }
    }
    ...
    // Tests to check setting a required subgroup size value.
    {
        ...
        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
            ...
            addFunctionCaseWithPrograms(framebufferGroup.get(),
                                        getShaderStageName(caseDefStageMax.shaderStage) + "_required_subgroup_size_max",
                                        "", supportedCheckFeaturesShader, initFrameBufferPrograms,
                                        noSSBOtestRequireSubgroupSize, caseDefStageMax);
            const CaseDefinition caseDefStageMin = {0u, stages[stageIndex], false, REQUIRED_SUBGROUP_SIZE_MIN};
            addFunctionCaseWithPrograms(framebufferGroup.get(),
                                        getShaderStageName(caseDefStageMin.shaderStage) + "_required_subgroup_size_min",
                                        "", supportedCheckFeaturesShader, initFrameBufferPrograms,
                                        noSSBOtestRequireSubgroupSize, caseDefStageMin);
        }
    }
