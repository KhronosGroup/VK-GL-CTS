...
TestCaseGroup *createSubgroupsBallotTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> graphicGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup ballot category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup ballot category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup ballot category tests: framebuffer"));
#ifndef CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup ballot category tests: ray tracing"));
    de::MovePtr<TestCaseGroup> meshGroup(
        new TestCaseGroup(testCtx, "mesh", "Subgroup ballot category tests: mesh shading"));
    de::MovePtr<TestCaseGroup> meshGroupEXT(
        new TestCaseGroup(testCtx, "mesh", "VK_EXT_shader_subgroups_ballot category tests: mesh shading"));
#endif // CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> groupEXT(
        new TestCaseGroup(testCtx, "ext_shader_subgroup_ballot", "VK_EXT_shader_subgroups_ballot category tests"));
    de::MovePtr<TestCaseGroup> graphicGroupEXT(
        new TestCaseGroup(testCtx, "graphics", "VK_EXT_shader_subgroups_ballot category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroupEXT(
        new TestCaseGroup(testCtx, "compute", "VK_EXT_shader_subgroups_ballot category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroupEXT(
        new TestCaseGroup(testCtx, "framebuffer", "VK_EXT_shader_subgroups_ballot category tests: framebuffer"));
        ...
        for (size_t extNdx = 0; extNdx < DE_LENGTH_OF_ARRAY(boolValues); ++extNdx)
        {
            ...
            for (const auto &stage : meshStages)
            {
                ...
                const string testName =
                    getShaderStageName(caseDef.shaderStage) + testNameSuffix + "_" + getShaderStageName(stage);
    ...
    for (size_t extNdx = 0; extNdx < DE_LENGTH_OF_ARRAY(boolValues); ++extNdx)
    {
        ...
        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(fbStages); ++stageIndex)
        {
            ...
            addFunctionCaseWithPrograms(testGroup, getShaderStageName(caseDef.shaderStage), supportedCheck,
