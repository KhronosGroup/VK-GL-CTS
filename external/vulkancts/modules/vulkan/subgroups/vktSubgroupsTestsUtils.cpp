...
Move<VkRenderPass> makeRenderPass(Context &context, VkFormat format)
{
    ...
    const VkSubpassDescription subpassDescription = {
        0u, VK_PIPELINE_BIND_POINT_GRAPHICS, 0, DE_NULL, 1, &colorReference, DE_NULL, DE_NULL, 0, DE_NULL};

    const VkSubpassDependency subpassDependencies[2] = {
        {VK_SUBPASS_EXTERNAL, 0u, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
         VK_ACCESS_MEMORY_READ_BIT, VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
         VK_DEPENDENCY_BY_REGION_BIT},
        {0u, VK_SUBPASS_EXTERNAL, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
         VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_ACCESS_MEMORY_READ_BIT,
         VK_DEPENDENCY_BY_REGION_BIT},
    };

    VkAttachmentDescription attachmentDescription = {0u,
                                                     format,
    ...
    const uint32_t fragmentShaderStageCreateFlags = 0u, const uint32_t requiredSubgroupSize[5] = DE_NULL)
{
    ...
    const VkPipelineColorBlendAttachmentState colorBlendAttachmentState = {
        ...
        VK_BLEND_OP_ADD, colorComponent};
...
struct Image : public BufferOrImage
{
        ...
        : BufferOrImage(true)
    {
        ...
        const VkImageViewCreateInfo imageViewCreateInfo = {VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
                                                           DE_NULL,
                                                           0,
                                                           *m_image,
...
void vkt::subgroups::initStdFrameBufferPrograms(SourceCollections &programCollection,
                                                ...
                                                std::string extHeader, std::string testSrc, std::string helperStr)
{
    ...
    if (shaderStage == VK_SHADER_STAGE_VERTEX_BIT)
    {
        ...
        vertex << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
               << extHeader.c_str() << "layout(location = 0) in highp vec4 in_position;\n"
               ...
               << "};\n"
               << "\n"
               << helperStr.c_str() << "void main (void)\n"
               << "{\n"
               << "  uint tempRes;\n"
    ...
    else if (shaderStage == VK_SHADER_STAGE_GEOMETRY_BIT)
    {
        ...
        geometry << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                 << extHeader.c_str() << "layout(points) in;\n"
    ...
    else if (shaderStage == VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT)
    {
        ...
        controlSource << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                      << extHeader.c_str() << "layout(vertices = 2) out;\n"
                      << "layout(location = 0) out float out_color[];\n"
                      << "layout(set = 0, binding = 0) uniform Buffer1\n"
                      << "{\n"
    ...
    else if (shaderStage == VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)
    {
        ...
        evaluationSource << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                         << extHeader.c_str() << "layout(isolines, equal_spacing, ccw ) in;\n"
                         << "layout(location = 0) out float out_color;\n"
                         << "layout(set = 0, binding = 0) uniform Buffer1\n"
                         << "{\n"
                         << "  " << subgroups::getFormatNameForGLSL(format) << " data["
...
void vkt::subgroups::initStdPrograms(vk::SourceCollections &programCollection,
                                     ...
                                     vk::VkFormat format, bool gsPointSize, std::string extHeader, std::string testSrc,
...
bool vkt::subgroups::areSubgroupOperationsRequiredForStage(VkShaderStageFlags stage)
{
    switch (stage)
    {
    default:
...
bool vkt::subgroups::isFormatSupportedForDevice(Context &context, vk::VkFormat format)
{
    ...
    case VK_FORMAT_R16G16B16_SFLOAT:
    case VK_FORMAT_R16G16B16A16_SFLOAT:
        return subgroupExtendedTypesFeatures.shaderSubgroupExtendedTypes & float16Int8Features.shaderFloat16 &
                       storage16bit.storageBuffer16BitAccess ?
                   true :
                   false;
    case VK_FORMAT_R64_SFLOAT:
    case VK_FORMAT_R64G64_SFLOAT:
    ...
    case VK_FORMAT_R8G8B8_UINT:
    case VK_FORMAT_R8G8B8A8_UINT:
        return subgroupExtendedTypesFeatures.shaderSubgroupExtendedTypes & float16Int8Features.shaderInt8 &
                       storage8bit.storageBuffer8BitAccess ?
                   true :
                   false;
    case VK_FORMAT_R16_SINT:
    case VK_FORMAT_R16G16_SINT:
    ...
    case VK_FORMAT_R16G16B16_UINT:
    case VK_FORMAT_R16G16B16A16_UINT:
        return subgroupExtendedTypesFeatures.shaderSubgroupExtendedTypes & features2.features.shaderInt16 &
                       storage16bit.storageBuffer16BitAccess ?
                   true :
                   false;
    case VK_FORMAT_R64_SINT:
    ...
    case VK_FORMAT_R64G64B64_UINT:
    case VK_FORMAT_R64G64B64A64_UINT:
        return subgroupExtendedTypesFeatures.shaderSubgroupExtendedTypes & features2.features.shaderInt64 ? true :
                                                                                                            false;
    }
}
...
tcu::TestStatus vkt::subgroups::makeTessellationEvaluationFrameBufferTest(
    Context &context, VkFormat format, SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width,
...
tcu::TestStatus vkt::subgroups::makeTessellationEvaluationFrameBufferTestRequiredSubgroupSize(
    ...
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width,
                        uint32_t subgroupSize),
    const VkShaderStageFlags shaderStage, const uint32_t tessShaderStageCreateFlags,
...
tcu::TestStatus vkt::subgroups::makeGeometryFrameBufferTest(Context &context, VkFormat format, SSBOData *extraData,
                                                            uint32_t extraDataCount, const void *internalData,
                                                            bool (*checkResult)(const void *internalData,
                                                                                std::vector<const void *> datas,
...
tcu::TestStatus vkt::subgroups::makeGeometryFrameBufferTestRequiredSubgroupSize(
    ...
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width,
                        uint32_t subgroupSize),
    const uint32_t geometryShaderStageCreateFlags, const uint32_t requiredSubgroupSize)
{
    const DeviceInterface &vk = context.getDeviceInterface();
    const VkDevice device     = context.getDevice();
    const uint32_t maxWidth   = getMaxWidth();
...
tcu::TestStatus vkt::subgroups::allStages(Context &context, VkFormat format, SSBOData *extraData,
                                          uint32_t extraDataCount, const void *internalData,
...
tcu::TestStatus vkt::subgroups::allStagesRequiredSubgroupSize(
    Context &context, VkFormat format, SSBOData *extraDatas, uint32_t extraDatasCount, const void *internalData,
    const VerificationFunctor &checkResult, const VkShaderStageFlags shaderStageTested,
...
tcu::TestStatus vkt::subgroups::makeVertexFrameBufferTest(Context &context, vk::VkFormat format, SSBOData *extraData,
                                                          uint32_t extraDataCount, const void *internalData,
                                                          bool (*checkResult)(const void *internalData,
                                                                              std::vector<const void *> datas,
                                                                              uint32_t width, uint32_t subgroupSize))
...
tcu::TestStatus vkt::subgroups::makeVertexFrameBufferTestRequiredSubgroupSize(
    Context &context, vk::VkFormat format, SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width,
                        uint32_t subgroupSize),
    const uint32_t vertexShaderStageCreateFlags, const uint32_t requiredSubgroupSize)
{
    const DeviceInterface &vk       = context.getDeviceInterface();
    const VkDevice device           = context.getDevice();
    const VkQueue queue             = context.getUniversalQueue();
...
tcu::TestStatus vkt::subgroups::makeFragmentFrameBufferTest(
    Context &context, VkFormat format, SSBOData *extraDatas, uint32_t extraDatasCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width, uint32_t height,
                        uint32_t subgroupSize))
{
...
tcu::TestStatus vkt::subgroups::makeFragmentFrameBufferTestRequiredSubgroupSize(
    ...
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, uint32_t width, uint32_t height,
                        uint32_t subgroupSize),
    const uint32_t fragmentShaderStageCreateFlags, const uint32_t requiredSubgroupSize)
{
    const DeviceInterface &vk       = context.getDeviceInterface();
    const VkDevice device           = context.getDevice();
    const VkQueue queue             = context.getUniversalQueue();
    ...
    for (uint32_t i = 0; i < extraDatasCount; i++)
    {
        ...
        {
            VkDescriptorBufferInfo info = makeDescriptorBufferInfo(inputBuffers[i]->getAsBuffer()->getBuffer(), 0ull,
                                                                   inputBuffers[i]->getAsBuffer()->getSize());
...
tcu::TestStatus vkt::subgroups::makeComputeTestRequiredSubgroupSize(
    Context &context, VkFormat format, SSBOData *inputs, uint32_t inputsCount, const void *internalData,
    bool (*checkResult)(const void *internalData, std::vector<const void *> datas, const uint32_t numWorkgroups[3],
    ...
    const uint32_t subgroupSize, const uint32_t localSizesToTest[][3], const uint32_t localSizesToTestCount)
{
    ...
    if (isRequiredSubgroupSize)
    {
        ...
        maxSubgroupSize =
            deMax32(subgroupSizeControlProperties.maxSubgroupSize, static_cast<uint32_t>(maxSubgroupSize));
    }
    ...
    pipelines[0] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(makeComputePipeline(
        ...
        VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT, (VkPipeline)DE_NULL, localSizesToTest[0][0], localSizesToTest[0][1],
    ...
    for (uint32_t index = 1; index < (localSizesToTestCount - 1); index++)
    {
        ...
        context.getTestContext().touchWatchdog();
        pipelines[index] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(makeComputePipeline(
            context, *pipelineLayout, *shaderModule, pipelineShaderStageCreateFlags, VK_PIPELINE_CREATE_DERIVATIVE_BIT,
