...
{
    explicit Buffer(Context &context, VkDeviceSize sizeInBytes,
...
{
    const uint32_t stageCount = isAllRayTracingStages(shaderStages) ? 6 :
...
bool vkt::subgroups::isSubgroupBroadcastDynamicIdSupported(Context &context)
{
    return context.contextSupports(vk::ApiVersion(1, 2, 0)) &&
           context.getDeviceVulkan12Features().subgroupBroadcastDynamicId;
}
...
bool vkt::subgroups::checkCompute(std::vector<const void *> datas, const uint32_t numWorkgroups[3],
    ...
    const uint32_t localSizesToTestCount)
{
    ...
    const VkDeviceSize maxSubgroupSize =
        isRequiredSubgroupSize ? deMax32(subgroupSizeControlProperties.maxSubgroupSize, maxSupportedSubgroupSize()) :
    ...
    DescriptorSetLayoutBuilder layoutBuilder;
    layoutBuilder.addBinding(resultBuffer.getType(), 1, VK_SHADER_STAGE_COMPUTE_BIT, DE_NULL);

    for (uint32_t i = 0; i < inputsCount; i++)
    {
        layoutBuilder.addBinding(inputBuffers[i]->getType(), 1, VK_SHADER_STAGE_COMPUTE_BIT, DE_NULL);
    }
    ...
    context.getTestContext().touchWatchdog();
    {
        pipelines[0] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(makeComputePipeline(
            context, *pipelineLayout, *shaderModule, pipelineShaderStageCreateFlags,
            VK_PIPELINE_CREATE_ALLOW_DERIVATIVES_BIT, (VkPipeline)DE_NULL, localSizesToTest[0][0],
            localSizesToTest[0][1], localSizesToTest[0][2], isRequiredSubgroupSize ? subgroupSize : 0u)));
    }
    context.getTestContext().touchWatchdog();

    for (uint32_t index = 1; index < (localSizesToTestCount - 1); index++)
    {
        const uint32_t nextX = localSizesToTest[index][0];
        const uint32_t nextY = localSizesToTest[index][1];
        const uint32_t nextZ = localSizesToTest[index][2];

        context.getTestContext().touchWatchdog();
        {
            pipelines[index] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(
                makeComputePipeline(context, *pipelineLayout, *shaderModule, pipelineShaderStageCreateFlags,
                                    VK_PIPELINE_CREATE_DERIVATIVE_BIT, **pipelines[0], nextX, nextY, nextZ,
                                                ...
                                                const uint32_t pipelineShaderStageCreateFlags)
{
    ...
    uint32_t subgroupSize           = requiredSubgroupSize;
    ...
    return makeComputeTestRequiredSubgroupSize(
        context, format, inputs, inputsCount, internalData, checkResult, pipelineShaderStageCreateFlags, numWorkgroups,
        requiredSubgroupSize != 0u, subgroupSize, localSizesToTest, localSizesToTestCount);
}
...
static inline void checkShaderStageSetValidity(const VkShaderStageFlags shaderStages)
{
    ...
    const uint32_t exclusivePipelinesCount = (isAllComputeStages(shaderStages) ? 1 : 0) +
                                                      ...
                                                      const vector<VkShaderStageFlagBits> &stagesVector)
{
    ...
    for (size_t stageNdx = stagesCount; stageNdx < stagesCount + extraDatasCount; ++stageNdx)
    {
        ...
        {
            const VkDeviceSize size = getElementSizeInBytes(extraDatas[datasNdx].format, extraDatas[datasNdx].layout) *
                                      extraDatas[datasNdx].numElements;
