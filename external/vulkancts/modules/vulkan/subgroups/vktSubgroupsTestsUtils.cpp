...
{
    const uint32_t bytes = getFormatSizeInBytes(format);
...
Move<VkPipeline> makeGraphicsPipeline(
    ...
    const VkPipelineDynamicStateCreateInfo *dynamicStateCreateInfo, const uint32_t vertexShaderStageCreateFlags,
    const uint32_t tessellationControlShaderStageCreateFlags, const uint32_t tessellationEvalShaderStageCreateFlags,
...
Move<VkPipeline> makeGraphicsPipeline(
    Context &context, const VkPipelineLayout pipelineLayout, const VkShaderStageFlags stages,
    ...
    const uint32_t fragmentShaderStageCreateFlags = 0u, const uint32_t requiredSubgroupSize[5] = DE_NULL)
{
    ...
    const uint32_t patchControlPoints = (VK_SHADER_STAGE_FRAGMENT_BIT & stages && frameBufferTests) ? 2u : 1u;
    ...
    return makeGraphicsPipeline(
        ...
        geometryShaderStageCreateFlags, // const uint32_t                                 geometryShaderStageCreateFlags
        fragmentShaderStageCreateFlags, // const uint32_t                                 fragmentShaderStageCreateFlags
        requiredSubgroupSize);          // const uint32_t                                 requiredSubgroupSize[5]
}
...
Move<VkCommandBuffer> makeCommandBuffer(Context &context, const VkCommandPool commandPool)
{
    ...
    return allocateCommandBuffer(context.getDeviceInterface(), context.getDevice(), &bufferAllocateParams);
}
...
{
    explicit Image(Context &context, uint32_t width, uint32_t height, VkFormat format,
        ...
        : BufferOrImage(true)
    {
        ...
        // Transition input image layouts
        {
            ...
            const VkImageMemoryBarrier imageBarrier =
                makeImageMemoryBarrier((VkAccessFlags)0u, VK_ACCESS_TRANSFER_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
...
uint32_t vkt::subgroups::getStagesCount(const VkShaderStageFlags shaderStages)
{
    const uint32_t stageCount = isAllGraphicsStages(shaderStages) ? 4 :
...
uint32_t vkt::subgroups::getSubgroupSize(Context &context)
{
    return context.getSubgroupProperties().subgroupSize;
}

uint32_t vkt::subgroups::maxSupportedSubgroupSize()
{
    return 128u;
}
...
#endif // CTS_USES_VULKANSC
    default:
        TCU_THROW(InternalError, "Unhandled stage");
    }
}
...
std::string vkt::subgroups::getSubgroupFeatureName(vk::VkSubgroupFeatureFlagBits bit)
{
    ...
    default:
        TCU_THROW(InternalError, "Unknown subgroup feature category");
    }
}
                                                    ...
                                                    const std::vector<std::string> &declarations, const uint32_t stage)
{
    if (declarations.empty())
    {
        ...
        const std::string result = "layout(location = 0) out float " + name + suffix +
                                   ";\n"
                                                ...
                                                const std::vector<std::string> &declarations)
{
    ...
    else if (shaderStage == VK_SHADER_STAGE_GEOMETRY_BIT)
    {
        ...
        geometry << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                 ...
                 << "{\n"
                 << "  uint tempRes;\n"
                 << testSrc << "  out_color = float(tempRes);\n"
    ...
    else if (shaderStage == VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT)
    {
        ...
        controlSource << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                      ...
                      << "  }\n"
                      << "  uint tempRes;\n"
                      << testSrc << "  out_color[gl_InvocationID] = float(tempRes);\n"
                      << "  gl_out[gl_InvocationID].gl_Position = gl_in[gl_InvocationID].gl_Position;\n"
                      << (gsPointSize ?
...
static std::string getBufferDeclarations(vk::VkShaderStageFlags shaderStage, const std::string &formatName,
                                         const std::vector<std::string> &declarations, const uint32_t stage)
{
    if (declarations.empty())
    {
        const uint32_t stageCount = vkt::subgroups::getStagesCount(shaderStage);
        const uint32_t binding0   = stage;
        const uint32_t binding1   = stageCount;
        const bool fragment       = (shaderStage & VK_SHADER_STAGE_FRAGMENT_BIT) && (stage == stageCount);
        const string buffer1      = fragment ? "layout(location = 0) out uint result;\n" :
        ...
        const string buffer2 = "layout(set = 0, binding = " + de::toString(binding1) + ", std430)" +
                               (stageCount == 1 ? "" : " readonly") + " buffer Buffer" + (fragment ? "1" : "2") +
                               "\n"
                               "{\n"
                               "  " +
                               formatName +
                                     ...
                                     const std::string &tempRes)
{
    ...
    if (isAllComputeStages(shaderStage))
    {
        ...
        src << "#version 450\n"
            ...
            << tempRes << testSrc << "  result[offset] = tempRes;\n"
            << "}\n";
    ...
    else if (isAllMeshShadingStages(shaderStage))
    {
        ...
        if (testMesh)
        {
            ...
            mesh << "#version 450\n"
                 ...
                 << extHeader << "layout (local_size_x_id = 0, local_size_y_id = 1, local_size_z_id = 2) in;\n"
                 << "layout (points) out;\n"
    ...
    else if (isAllRayTracingStages(shaderStage))
    {
        const std::string rgenShader =
            ...
            "}\n";
        const std::string ahitShader = "#version 460 core\n"
                                       "#extension GL_EXT_ray_tracing: require\n" +
                                       extHeader +
                                       ...
                                       "  result[gl_LaunchIDEXT.x] = tempRes;\n"
                                       "}\n";
        const std::string chitShader = "#version 460 core\n"
                                       "#extension GL_EXT_ray_tracing: require\n" +
                                       extHeader +
                                       "hitAttributeEXT vec3 attribs;\n"
                                       "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n" +
        ...
        const std::string missShader = "#version 460 core\n"
                                       "#extension GL_EXT_ray_tracing: require\n" +
                                       extHeader + "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n" +
                                       getBufferDeclarations(shaderStage, formatName, declarations, 3) + "\n" +
                                       helperStr +
                                       "void main()\n"
                                       "{\n" +
                                       ...
                                       "  result[gl_LaunchIDEXT.x] = tempRes;\n"
                                       "}\n";
        const std::string sectShader = "#version 460 core\n"
                                       "#extension GL_EXT_ray_tracing: require\n" +
                                       extHeader + "hitAttributeEXT vec3 hitAttribute;\n" +
                                       ...
                                       "  result[gl_LaunchIDEXT.x] = tempRes;\n"
                                       "}\n";
        const std::string callShader = "#version 460 core\n"
                                       "#extension GL_EXT_ray_tracing: require\n" +
                                       extHeader + "layout(location = 0) callableDataInEXT float callData;\n" +
...
bool vkt::subgroups::isSubgroupBroadcastDynamicIdSupported(Context &context)
{
    return context.contextSupports(vk::ApiVersion(0, 1, 2, 0)) &&
           vk::getPhysicalDeviceVulkan12Features(context.getInstanceInterface(), context.getPhysicalDevice())
...
void vkt::subgroups::setVertexShaderFrameBuffer(SourceCollections &programCollection)
{
    /*
        ...
        "}\n";
    */
    programCollection.spirvAsmSources.add("vert") << "; SPIR-V\n"
                                                     "; Version: 1.3\n"
...
void vkt::subgroups::setFragmentShaderFrameBuffer(vk::SourceCollections &programCollection)
{
    /*
        ...
        "}\n";
    */
    programCollection.spirvAsmSources.add("fragment") << "; SPIR-V\n"
                                                         "; Version: 1.3\n"
                                                         "; Generator: Khronos Glslang Reference Front End; 2\n"
                                                         "; Bound: 14\n"
                                                         "; Schema: 0\n"
...
void vkt::subgroups::setTesCtrlShaderFrameBuffer(vk::SourceCollections &programCollection)
{
    /*
        ...
        "}\n";
    */
    programCollection.spirvAsmSources.add("tesc") << "; SPIR-V\n"
                                                     "; Version: 1.3\n"
                                                     "; Generator: Khronos Glslang Reference Front End; 2\n"
                                                     "; Bound: 46\n"
                                                     "; Schema: 0\n"
...
void vkt::subgroups::addGeometryShadersFromTemplate(const std::string &glslTemplate,
                                                    const vk::ShaderBuildOptions &options,
...
void vkt::subgroups::addGeometryShadersFromTemplate(const std::string &spirvTemplate,
                                                    const vk::SpirVAsmBuildOptions &options,
...
void initializeMemory(Context &context, const Allocation &alloc, const subgroups::SSBOData &data)
{
    ...
    if (subgroups::SSBOData::InitializeNonZero == data.initializeType)
    {
        ...
        case VK_FORMAT_R8G8B8A8_UINT:
        {
            ...
            for (vk::VkDeviceSize k = 0; k < (size / sizeof(uint8_t)); k++)
            {
                ptr[k] = rnd.getUint8();
            }
        }
        ...
        case VK_FORMAT_R16G16B16A16_UINT:
        {
            ...
            for (vk::VkDeviceSize k = 0; k < (size / sizeof(uint16_t)); k++)
            {
                ptr[k] = rnd.getUint16();
            }
        }
        ...
        case VK_FORMAT_R8G8B8A8_USCALED:
        {
            ...
            for (vk::VkDeviceSize k = 0; k < (size / sizeof(uint32_t)); k++)
            {
        ...
        case VK_FORMAT_R64G64B64A64_UINT:
        {
            ...
            for (vk::VkDeviceSize k = 0; k < (size / sizeof(uint64_t)); k++)
            {
                ptr[k] = rnd.getUint64();
            }
        }
...
uint32_t getResultBinding(const VkShaderStageFlagBits shaderStage)
{
    switch (shaderStage)
    {
    case VK_SHADER_STAGE_VERTEX_BIT:
...
tcu::TestStatus vkt::subgroups::makeTessellationEvaluationFrameBufferTest(
    Context &context, VkFormat format, const SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    subgroups::CheckResult checkResult, const VkShaderStageFlags shaderStage)
{
    return makeTessellationEvaluationFrameBufferTestRequiredSubgroupSize(
        context, format, extraData, extraDataCount, internalData, checkResult, shaderStage, 0u, 0u);
}

tcu::TestStatus vkt::subgroups::makeTessellationEvaluationFrameBufferTestRequiredSubgroupSize(
    Context &context, VkFormat format, const SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    subgroups::CheckResult checkResult, const VkShaderStageFlags shaderStage, const uint32_t tessShaderStageCreateFlags,
    const uint32_t requiredSubgroupSize)
{
    ...
    const Unique<VkShaderModule> teCtrlShaderModule(
        createShaderModule(vk, device, context.getBinaryCollection().get("tesc"), 0u));
    const Unique<VkShaderModule> teEvalShaderModule(
    ...
    const uint32_t requiredSubgroupSizes[5] = {
        0u, ((shaderStage & VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT) ? requiredSubgroupSize : 0u),
        ((shaderStage & VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) ? requiredSubgroupSize : 0u), 0u, 0u};
    ...
    for (uint32_t buffersNdx = 0u; buffersNdx < inputBuffers.size(); buffersNdx++)
    {
        if (inputBuffers[buffersNdx]->isImage())
        {
            VkDescriptorImageInfo info = makeDescriptorImageInfo(inputBuffers[buffersNdx]->getAsImage()->getSampler(),
                                                                 inputBuffers[buffersNdx]->getAsImage()->getImageView(),
                                                                 VK_IMAGE_LAYOUT_GENERAL);

            updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(buffersNdx),
                                      inputBuffers[buffersNdx]->getType(), &info);
        }
        else
        {
            VkDescriptorBufferInfo info =
                makeDescriptorBufferInfo(inputBuffers[buffersNdx]->getAsBuffer()->getBuffer(), 0ull,
                                         inputBuffers[buffersNdx]->getAsBuffer()->getSize());
    ...
    const Unique<VkFramebuffer> framebuffer(
        makeFramebuffer(vk, device, *renderPass, discardableImage.getImageView(), maxWidth, 1u));
    ...
    for (uint32_t width = 1u; width < maxWidth; width = getNextWidth(width))
    {
        ...
        beginCommandBuffer(vk, *cmdBuffer);
        {
            ...
            copyImageToBuffer(vk, *cmdBuffer, discardableImage.getImage(), imageBufferResult.getBuffer(),
                              tcu::IVec2(maxWidth, 1), VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
    ...
    if (0 < failedIterations)
    {
        ...
        context.getTestContext().getLog()
            << TestLog::Message << valuesPassed << " / " << totalIterations << " values passed" << TestLog::EndMessage;
...
bool vkt::subgroups::check(std::vector<const void *> datas, uint32_t width, uint32_t ref)
{
...
bool vkt::subgroups::checkComputeOrMesh(std::vector<const void *> datas, const uint32_t numWorkgroups[3],
...
tcu::TestStatus vkt::subgroups::makeGeometryFrameBufferTest(Context &context, VkFormat format,
                                                            const SSBOData *extraData, uint32_t extraDataCount,
                                                            const void *internalData,
                                                            subgroups::CheckResult checkResult)
{
    return makeGeometryFrameBufferTestRequiredSubgroupSize(context, format, extraData, extraDataCount, internalData,
                                                           checkResult, 0u, 0u);
}

tcu::TestStatus vkt::subgroups::makeGeometryFrameBufferTestRequiredSubgroupSize(
    Context &context, VkFormat format, const SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    subgroups::CheckResult checkResult, const uint32_t geometryShaderStageCreateFlags,
    const uint32_t requiredSubgroupSize)
{
    const DeviceInterface &vk = context.getDeviceInterface();
    const VkDevice device     = context.getDevice();
    ...
    const Unique<VkShaderModule> geometryShaderModule(
        createShaderModule(vk, device, context.getBinaryCollection().get("geometry"), 0u));
    const Unique<VkShaderModule> fragmentShaderModule(
        createShaderModule(vk, device, context.getBinaryCollection().get("fragment"), 0u));
    const Unique<VkRenderPass> renderPass(makeRenderPass(context, format));
    const VkVertexInputBindingDescription vertexInputBinding = {
    ...
    const uint32_t requiredSubgroupSizes[5] = {0u, 0u, 0u, requiredSubgroupSize, 0u};
...
vk::VkShaderStageFlags vkt::subgroups::getPossibleGraphicsSubgroupStages(Context &context,
                                                                         const vk::VkShaderStageFlags testedStages)
{
...
tcu::TestStatus vkt::subgroups::allStages(Context &context, vk::VkFormat format, const SSBOData *extraData,
    ...
    const uint32_t fragmentShaderStageCreateFlags, const uint32_t requiredSubgroupSize[5])
{
    ...
    if (shaderStageTested & VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT)
    {
        ...
        shaderStageRequired |= (shaderStageTested & (VkShaderStageFlags)VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) ?
                                   (VkShaderStageFlags)0u :
                                   (VkShaderStageFlags)VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT;
        shaderStageRequired |= (shaderStageTested & (VkShaderStageFlags)VK_SHADER_STAGE_VERTEX_BIT) ?
                                   (VkShaderStageFlags)0u :
    ...
    if (shaderStageTested & VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)
    {
        ...
        shaderStageRequired |= (shaderStageTested & (VkShaderStageFlags)VK_SHADER_STAGE_VERTEX_BIT) ?
                                   (VkShaderStageFlags)0u :
                                   (VkShaderStageFlags)VK_SHADER_STAGE_VERTEX_BIT;
        shaderStageRequired |= (shaderStageTested & (VkShaderStageFlags)VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT) ?
                                   (VkShaderStageFlags)0u :
    ...
    const string tese =
        (shaderStageRequired & VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) ? "tese_noSubgroup" : "tese";
    ...
    if (shaderStageRequired & VK_SHADER_STAGE_GEOMETRY_BIT)
    {
        if (shaderStageRequired & VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)
        {
            ...
            geometryShaderModule =
                createShaderModule(vk, device, context.getBinaryCollection().get("geometry_lines"), 0u);
        }
        else
        {
            // otherwise points are processed by geometry shader
            geometryShaderModule =
                createShaderModule(vk, device, context.getBinaryCollection().get("geometry_points"), 0u);
        }
    }
    ...
    {
        const VkDeviceSize shaderSize =
            (stagesVector[ndx] == VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) ? maxWidth * 2 : maxWidth;
    ...
    const Unique<VkRenderPass> renderPass(makeRenderPass(context, format));
    const Unique<VkPipeline> pipeline(makeGraphicsPipeline(
        context, *pipelineLayout, shaderStageRequired, *vertexShaderModule, *fragmentShaderModule,
    ...
    if (inputBuffers.size() > 0)
    {
        ...
        for (uint32_t ndx = 0u; ndx < static_cast<uint32_t>(inputBuffers.size()); ndx++)
        {
            poolBuilder.addType(inputBuffers[ndx]->getType());
        }
        ...
        for (uint32_t ndx = 0u; ndx < stagesCount + extraDatasCount; ndx++)
        {
            uint32_t binding;
            if (ndx < stagesCount)
                binding = getResultBinding(stagesVector[ndx]);
            else
                binding = extraDatas[ndx - stagesCount].binding;
            ...
            if (inputBuffers[ndx]->isImage())
            {
                ...
                updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(binding),
                                          inputBuffers[ndx]->getType(), &info);
            }
            else
            {
                VkDescriptorBufferInfo info = makeDescriptorBufferInfo(
                    inputBuffers[ndx]->getAsBuffer()->getBuffer(), 0ull, inputBuffers[ndx]->getAsBuffer()->getSize());
...

    {
        ...
        Image resultImage(context, maxWidth, 1, format,
                          VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);
        const Unique<VkFramebuffer> framebuffer(
            makeFramebuffer(vk, device, *renderPass, resultImage.getImageView(), maxWidth, 1u));
        const VkViewport viewport              = makeViewport(maxWidth, 1u);
        const VkRect2D scissor                 = makeRect2D(maxWidth, 1u);
        const vk::VkDeviceSize imageResultSize = tcu::getPixelSize(vk::mapVkFormat(format)) * maxWidth;
        Buffer imageBufferResult(context, imageResultSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT);
        ...
        const VkImageMemoryBarrier colorAttachmentBarrier =
            makeImageMemoryBarrier((VkAccessFlags)0u, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                   VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, resultImage.getImage(), subresourceRange);
        ...
        for (uint32_t width = 1u; width < maxWidth; width = getNextWidth(width))
        {
            ...
            copyImageToBuffer(vk, *cmdBuffer, resultImage.getImage(), imageBufferResult.getBuffer(),
                              tcu::IVec2(width, 1), VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                              VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
            ...
            for (uint32_t ndx = 0u; ndx < stagesCount; ++ndx)
            {
                ...
                const uint32_t usedWidth =
                    ((stagesVector[ndx] == VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) ? width * 2 : width);
        ...
        if (0 < failedIterations)
        {
            ...
            context.getTestContext().getLog() << TestLog::Message << valuesPassed << " / " << totalIterations
...
tcu::TestStatus vkt::subgroups::makeVertexFrameBufferTest(Context &context, vk::VkFormat format,
                                                          const SSBOData *extraData, uint32_t extraDataCount,
                                                          const void *internalData, subgroups::CheckResult checkResult)
{
    return makeVertexFrameBufferTestRequiredSubgroupSize(context, format, extraData, extraDataCount, internalData,
                                                         checkResult, 0u, 0u);
}

tcu::TestStatus vkt::subgroups::makeVertexFrameBufferTestRequiredSubgroupSize(
    Context &context, vk::VkFormat format, const SSBOData *extraData, uint32_t extraDataCount, const void *internalData,
    subgroups::CheckResult checkResult, const uint32_t vertexShaderStageCreateFlags,
    const uint32_t requiredSubgroupSize)
{
    const DeviceInterface &vk       = context.getDeviceInterface();
    const VkDevice device           = context.getDevice();
    ...
    const VkVertexInputBindingDescription vertexInputBinding = {
        0u,                                       // binding;
        static_cast<uint32_t>(sizeof(tcu::Vec4)), // stride;
...
tcu::TestStatus vkt::subgroups::makeFragmentFrameBufferTest(Context &context, VkFormat format,
                                                            const SSBOData *extraDatas, uint32_t extraDatasCount,
                                                            const void *internalData, CheckResultFragment checkResult)
{
    return makeFragmentFrameBufferTestRequiredSubgroupSize(context, format, extraDatas, extraDatasCount, internalData,
                                                           checkResult, 0u, 0u);
}

tcu::TestStatus vkt::subgroups::makeFragmentFrameBufferTestRequiredSubgroupSize(
    Context &context, VkFormat format, const SSBOData *extraDatas, uint32_t extraDatasCount, const void *internalData,
    CheckResultFragment checkResult, const uint32_t fragmentShaderStageCreateFlags, const uint32_t requiredSubgroupSize)
{
    const DeviceInterface &vk       = context.getDeviceInterface();
    const VkDevice device           = context.getDevice();
    const VkQueue queue             = context.getUniversalQueue();
    ...
    for (uint32_t i = 0; i < extraDatasCount; i++)
    {
        if (inputBuffers[i]->isImage())
        {
            ...
            updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(i),
                                      inputBuffers[i]->getType(), &info);
        }
        else
        {
            const VkDescriptorBufferInfo info = makeDescriptorBufferInfo(
                inputBuffers[i]->getAsBuffer()->getBuffer(), 0ull, inputBuffers[i]->getAsBuffer()->getSize());
    ...
    for (uint32_t width = 8; width <= subgroupSize; width *= 2)
    {
        for (uint32_t height = 8; height <= subgroupSize; height *= 2)
        {
            ...
            const Unique<VkFramebuffer> framebuffer(
                makeFramebuffer(vk, device, *renderPass, resultImage.getImageView(), width, height));
            ...
            beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer, makeRect2D(0, 0, width, height),
                            tcu::Vec4(0.0f));
            ...
            copyImageToBuffer(vk, *cmdBuffer, resultImage.getImage(), resultBuffer.getBuffer(),
                              tcu::IVec2(width, height), VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                              VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
...
Move<VkPipeline> makeComputePipeline(Context &context, const VkPipelineLayout pipelineLayout,
                                     const VkShaderModule shaderModule, const uint32_t pipelineShaderStageFlags,
                                     const uint32_t pipelineCreateFlags, VkPipeline basePipelineHandle,
                                     uint32_t localSizeX, uint32_t localSizeY, uint32_t localSizeZ,
                                     uint32_t requiredSubgroupSize)
{
    const uint32_t localSize[3]                   = {localSizeX, localSizeY, localSizeZ};
    const vk::VkSpecializationMapEntry entries[3] = {
        {0, sizeof(uint32_t) * 0, sizeof(uint32_t)},
        {1, sizeof(uint32_t) * 1, sizeof(uint32_t)},
        {2, static_cast<uint32_t>(sizeof(uint32_t) * 2), sizeof(uint32_t)},
    };
    const vk::VkSpecializationInfo info                                                     = {/* mapEntryCount = */ 3,
                                           /* pMapEntries   = */ entries,
...
#ifndef CTS_USES_VULKANSC
Move<VkPipeline> makeMeshPipeline(Context &context, const VkPipelineLayout pipelineLayout,
                                  const VkShaderModule taskModule, const VkShaderModule meshModule,
                                  const uint32_t pipelineShaderStageFlags, const uint32_t pipelineCreateFlags,
                                  VkPipeline basePipelineHandle, uint32_t localSizeX, uint32_t localSizeY,
                                  uint32_t localSizeZ, uint32_t requiredSubgroupSize, const VkRenderPass renderPass)
{
    const uint32_t localSize[3]                   = {localSizeX, localSizeY, localSizeZ};
    const vk::VkSpecializationMapEntry entries[3] = {
        {0, sizeof(uint32_t) * 0, sizeof(uint32_t)},
    ...
    const VkPipelineShaderStageRequiredSubgroupSizeCreateInfoEXT *pSubgroupSizeCreateInfo =
        ((requiredSubgroupSize != 0u) ? &subgroupSizeCreateInfo : nullptr);
    ...
    return makeGraphicsPipeline(context.getDeviceInterface(), context.getDevice(), basePipelineHandle, pipelineLayout,
                                pipelineCreateFlags, shaderStageParams, renderPass, viewports, scissors);
}
#endif // CTS_USES_VULKANSC

tcu::TestStatus makeComputeOrMeshTestRequiredSubgroupSize(
    ComputeLike testType, Context &context, VkFormat format, const vkt::subgroups::SSBOData *inputs,
    uint32_t inputsCount, const void *internalData, vkt::subgroups::CheckResultCompute checkResult,
    ...
    const uint32_t subgroupSize, const uint32_t localSizesToTest[][3], const uint32_t localSizesToTestCount)
{
    ...
    const auto shaderStageFlags =
        ((testType == ComputeLike::COMPUTE) ? VK_SHADER_STAGE_COMPUTE_BIT
#ifndef CTS_USES_VULKANSC
                                              :
                                              ...
                                              0);
#endif // CTS_USES_VULKANSC
    const auto pipelineBindPoint =
        ((testType == ComputeLike::COMPUTE) ? VK_PIPELINE_BIND_POINT_COMPUTE : VK_PIPELINE_BIND_POINT_GRAPHICS);
    ...
    for (uint32_t i = 0; i < localSizesToTestCount; ++i)
    {
    ...
    const VkDescriptorBufferInfo resultDescriptorInfo =
        makeDescriptorBufferInfo(resultBuffer.getBuffer(), 0ull, resultBufferSizeInBytes);
    DescriptorSetUpdateBuilder updateBuilder;
    ...
    for (uint32_t i = 0; i < inputsCount; i++)
    {
        if (inputBuffers[i]->isImage())
        {
            ...
            updateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(i + 1),
                                      inputBuffers[i]->getType(), &info);
        }
        else
        {
    ...
    if (testType == ComputeLike::MESH)
    {
        ...
        framebuffer = makeFramebuffer(vk, device, renderPass.get(), 0u, nullptr, renderArea.extent.width,
    ...
    for (uint32_t index = 1; index < (localSizesToTestCount - 1); index++)
    {
        const uint32_t nextX = usedLocalSizes[index][0];
        const uint32_t nextY = usedLocalSizes[index][1];
        const uint32_t nextZ = usedLocalSizes[index][2];

        context.getTestContext().touchWatchdog();
        {
            if (testType == ComputeLike::COMPUTE)
            {
                pipelines[index] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(
                    makeComputePipeline(context, *pipelineLayout, *compShader, pipelineShaderStageCreateFlags,
#ifndef CTS_USES_VULKANSC
                                        ...
                                        0u,
#endif // CTS_USES_VULKANSC
                                        **pipelines[0], nextX, nextY, nextZ, reqSubgroupSize)));
            }
#ifndef CTS_USES_VULKANSC
            else if (testType == ComputeLike::MESH)
            {
                pipelines[index] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(makeMeshPipeline(
                    context, pipelineLayout.get(), taskShader.get(), meshShader.get(), pipelineShaderStageCreateFlags,
                    VK_PIPELINE_CREATE_DERIVATIVE_BIT, pipelines[0].get()->get(), nextX, nextY, nextZ, reqSubgroupSize,
                    renderPass.get())));
            }
#endif // CTS_USES_VULKANSC
            else
    ...
    for (uint32_t index = 0; index < (localSizesToTestCount - 1); index++)
    {
        ...
        vk.cmdPipelineBarrier(*cmdBuffer, pipelineStage, VK_PIPELINE_STAGE_HOST_BIT, 0u, 1u, &postShaderBarrier, 0u,
                              nullptr, 0u, nullptr);
...
tcu::TestStatus vkt::subgroups::makeComputeTestRequiredSubgroupSize(
    Context &context, VkFormat format, const SSBOData *inputs, uint32_t inputsCount, const void *internalData,
    CheckResultCompute checkResult, const uint32_t pipelineShaderStageCreateFlags, const uint32_t numWorkgroups[3],
    const bool isRequiredSubgroupSize, const uint32_t subgroupSize, const uint32_t localSizesToTest[][3],
    const uint32_t localSizesToTestCount)
{
    return makeComputeOrMeshTestRequiredSubgroupSize(ComputeLike::COMPUTE, context, format, inputs, inputsCount,
                                                     internalData, checkResult, pipelineShaderStageCreateFlags,
                                                     numWorkgroups, isRequiredSubgroupSize, subgroupSize,
                                                     localSizesToTest, localSizesToTestCount);
}

tcu::TestStatus vkt::subgroups::makeMeshTestRequiredSubgroupSize(
    Context &context, VkFormat format, const SSBOData *inputs, uint32_t inputsCount, const void *internalData,
    CheckResultCompute checkResult, const uint32_t pipelineShaderStageCreateFlags, const uint32_t numWorkgroups[3],
    const bool isRequiredSubgroupSize, const uint32_t subgroupSize, const uint32_t localSizesToTest[][3],
    const uint32_t localSizesToTestCount)
{
    return makeComputeOrMeshTestRequiredSubgroupSize(ComputeLike::MESH, context, format, inputs, inputsCount,
                                                     internalData, checkResult, pipelineShaderStageCreateFlags,
                                                     numWorkgroups, isRequiredSubgroupSize, subgroupSize,
                                                     localSizesToTest, localSizesToTestCount);
}

tcu::TestStatus makeComputeOrMeshTest(ComputeLike testType, Context &context, VkFormat format,
                                      const vkt::subgroups::SSBOData *inputs, uint32_t inputsCount,
                                      const void *internalData, vkt::subgroups::CheckResultCompute checkResult,
                                      uint32_t requiredSubgroupSize, const uint32_t pipelineShaderStageCreateFlags)
{
    const uint32_t numWorkgroups[3]   = {4, 2, 2};
    const bool isRequiredSubgroupSize = (requiredSubgroupSize != 0u);
    const uint32_t subgroupSize =
        (isRequiredSubgroupSize ? requiredSubgroupSize : vkt::subgroups::getSubgroupSize(context));

    const uint32_t localSizesToTestCount                = 8;
    uint32_t localSizesToTest[localSizesToTestCount][3] = {
        {1, 1, 1}, {subgroupSize, 1, 1}, {1, subgroupSize, 1}, {1, 1, subgroupSize}, {32, 4, 1}, {1, 4, 32}, {3, 5, 7},
        {1, 1, 1} // Isn't used, just here to make double buffering checks easier
    };

    if (testType == ComputeLike::COMPUTE)
        return makeComputeTestRequiredSubgroupSize(
            context, format, inputs, inputsCount, internalData, checkResult, pipelineShaderStageCreateFlags,
            numWorkgroups, isRequiredSubgroupSize, subgroupSize, localSizesToTest, localSizesToTestCount);
    else
        return makeMeshTestRequiredSubgroupSize(context, format, inputs, inputsCount, internalData, checkResult,
                                                pipelineShaderStageCreateFlags, numWorkgroups, isRequiredSubgroupSize,
                                                subgroupSize, localSizesToTest, localSizesToTestCount);
}

tcu::TestStatus vkt::subgroups::makeComputeTest(Context &context, VkFormat format, const SSBOData *inputs,
                                                uint32_t inputsCount, const void *internalData,
                                                CheckResultCompute checkResult, uint32_t requiredSubgroupSize,
                                                const uint32_t pipelineShaderStageCreateFlags)
{
    return makeComputeOrMeshTest(ComputeLike::COMPUTE, context, format, inputs, inputsCount, internalData, checkResult,
                                 requiredSubgroupSize, pipelineShaderStageCreateFlags);
}

tcu::TestStatus vkt::subgroups::makeMeshTest(Context &context, VkFormat format, const SSBOData *inputs,
                                             uint32_t inputsCount, const void *internalData,
                                             CheckResultCompute checkResult, uint32_t requiredSubgroupSize,
                                             const uint32_t pipelineShaderStageCreateFlags)
{
    return makeComputeOrMeshTest(ComputeLike::MESH, context, format, inputs, inputsCount, internalData, checkResult,
                                 requiredSubgroupSize, pipelineShaderStageCreateFlags);
}
...
static inline void checkShaderStageSetValidity(const VkShaderStageFlags shaderStages)
{
    ...
    const uint32_t exclusivePipelinesCount =
        (isAllComputeStages(shaderStages) ? 1 : 0) + (isAllGraphicsStages(shaderStages) ? 1 : 0)
#ifndef CTS_USES_VULKANSC
        + (isAllRayTracingStages(shaderStages) ? 1 : 0) + (isAllMeshShadingStages(shaderStages) ? 1 : 0)
#endif // CTS_USES_VULKANSC
        ;
...
void addRayTracingNoSubgroupShader(SourceCollections &programCollection)
{
    ...
    const std::string rgenShaderNoSubgroups =
        ...
        "  executeCallableEXT(0, 0);"
        "}\n";
    const std::string hitShaderNoSubgroups  = "#version 460 core\n"
                                              "#extension GL_EXT_ray_tracing: require\n"
                                              "hitAttributeEXT vec3 attribs;\n"
                                              "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n"
                                              "\n"
                                              ...
                                              "{\n"
                                              "}\n";
    const std::string missShaderNoSubgroups = "#version 460 core\n"
                                              "#extension GL_EXT_ray_tracing: require\n"
                                              "layout(location = 0) rayPayloadInEXT vec3 hitValue;\n"
                                              "\n"
                                              "void main()\n"
                                              "{\n"
                                              "}\n";
    const std::string sectShaderNoSubgroups = "#version 460 core\n"
                                              "#extension GL_EXT_ray_tracing: require\n"
    ...
    programCollection.glslSources.add("sect_noSubgroup")
        << glu::IntersectionSource(sectShaderNoSubgroups) << buildOptions;
...
static uint32_t getRayTracingResultBinding(const VkShaderStageFlagBits shaderStage)
{
    ...
    for (uint32_t shaderStageNdx = 0; shaderStageNdx < DE_LENGTH_OF_ARRAY(shaderStageFlags); ++shaderStageNdx)
    {
...
static vectorBufferOrImage makeRayTracingInputBuffers(Context &context, VkFormat format, const SSBOData *extraDatas,
                                                      ...
                                                      const vector<VkShaderStageFlagBits> &stagesVector)
{
    ...
    for (size_t stageNdx = 0u; stageNdx < stagesCount; ++stageNdx)
        inputBuffers[stageNdx] =
            de::SharedPtr<BufferOrImage>(new Buffer(context, inputBufferSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT));
...
static Move<VkDescriptorSetLayout> makeRayTracingDescriptorSetLayout(Context &context, const SSBOData *extraDatas,
                                                                     uint32_t extraDatasCount,
                                                                     const vector<VkShaderStageFlagBits> &stagesVector,
...
static Move<VkDescriptorPool> makeRayTracingDescriptorPool(Context &context, const vectorBufferOrImage &inputBuffers)
{
    const DeviceInterface &vkd       = context.getDeviceInterface();
    const VkDevice device            = context.getDevice();
    const uint32_t maxDescriptorSets = 2u;
    DescriptorPoolBuilder poolBuilder;
...
static Move<VkDescriptorSet> makeRayTracingDescriptorSet(Context &context, VkDescriptorPool descriptorPool,
                                                         ...
                                                         const vectorBufferOrImage &inputBuffers)
{
    ...
    if (inputBuffers.size() > 0)
    {
        ...
        {
            const uint32_t binding = (ndx < stagesCount) ? getRayTracingResultBinding(stagesVector[ndx]) :
...
static Move<VkDescriptorSet> makeRayTracingDescriptorSetAS(
    Context &context, VkDescriptorPool descriptorPool, VkDescriptorSetLayout descriptorSetLayout,
                                                           ...
                                                           const VkDescriptorSetLayout descriptorSetLayout1)
{
    ...
    const uint32_t descriptorSetLayoutsSize = static_cast<uint32_t>(descriptorSetLayouts.size());

    return makePipelineLayout(vkd, device, descriptorSetLayoutsSize, descriptorSetLayouts.data());
}

static de::MovePtr<TopLevelAccelerationStructure> createTopAccelerationStructure(
    Context &context, de::SharedPtr<BottomLevelAccelerationStructure> bottomLevelAccelerationStructure)
{
    const DeviceInterface &vkd                        = context.getDeviceInterface();
    const VkDevice device                             = context.getDevice();
...
static de::MovePtr<RayTracingPipeline> makeRayTracingPipeline(
    Context &context, const VkShaderStageFlags shaderStageTested, const VkPipelineLayout pipelineLayout,
    const uint32_t shaderStageCreateFlags[6], const uint32_t requiredSubgroupSize[6], Move<VkPipeline> &pipelineOut)
{
    const DeviceInterface &vkd   = context.getDeviceInterface();
    const VkDevice device        = context.getDevice();
    BinaryCollection &collection = context.getBinaryCollection();
    const char *shaderRgenName =
        (0 != (shaderStageTested & VK_SHADER_STAGE_RAYGEN_BIT_KHR)) ? "rgen" : "rgen_noSubgroup";
    const char *shaderAhitName =
        (0 != (shaderStageTested & VK_SHADER_STAGE_ANY_HIT_BIT_KHR)) ? "ahit" : "ahit_noSubgroup";
    const char *shaderChitName =
        (0 != (shaderStageTested & VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR)) ? "chit" : "chit_noSubgroup";
    const char *shaderMissName = (0 != (shaderStageTested & VK_SHADER_STAGE_MISS_BIT_KHR)) ? "miss" : "miss_noSubgroup";
    const char *shaderSectName =
        (0 != (shaderStageTested & VK_SHADER_STAGE_INTERSECTION_BIT_KHR)) ? "sect" : "sect_noSubgroup";
    const char *shaderCallName =
        (0 != (shaderStageTested & VK_SHADER_STAGE_CALLABLE_BIT_KHR)) ? "call" : "call_noSubgroup";
    const VkShaderModuleCreateFlags noShaderModuleCreateFlags = static_cast<VkShaderModuleCreateFlags>(0);
    Move<VkShaderModule> rgenShaderModule =
        createShaderModule(vkd, device, collection.get(shaderRgenName), noShaderModuleCreateFlags);
    Move<VkShaderModule> ahitShaderModule =
        createShaderModule(vkd, device, collection.get(shaderAhitName), noShaderModuleCreateFlags);
    Move<VkShaderModule> chitShaderModule =
        createShaderModule(vkd, device, collection.get(shaderChitName), noShaderModuleCreateFlags);
    Move<VkShaderModule> missShaderModule =
        createShaderModule(vkd, device, collection.get(shaderMissName), noShaderModuleCreateFlags);
    Move<VkShaderModule> sectShaderModule =
        createShaderModule(vkd, device, collection.get(shaderSectName), noShaderModuleCreateFlags);
    Move<VkShaderModule> callShaderModule =
        createShaderModule(vkd, device, collection.get(shaderCallName), noShaderModuleCreateFlags);
...
tcu::TestStatus allRayTracingStages(Context &context, VkFormat format, const SSBOData *extraDatas,
                                    uint32_t extraDataCount, const void *internalData,
...
tcu::TestStatus allRayTracingStagesRequiredSubgroupSize(Context &context, VkFormat format, const SSBOData *extraDatas,
                                                        uint32_t extraDatasCount, const void *internalData,
                                                        const VerificationFunctor &checkResult,
                                                        const VkShaderStageFlags shaderStageTested,
                                                        const uint32_t shaderStageCreateFlags[6],
                                                        const uint32_t requiredSubgroupSize[6])
{
    const DeviceInterface &vkd                       = context.getDeviceInterface();
    const VkDevice device                            = context.getDevice();
    const VkQueue queue                              = context.getUniversalQueue();
    ...
    de::SharedPtr<BottomLevelAccelerationStructure> bottomLevelAccelerationStructure =
        createBottomAccelerationStructure(context);
    de::MovePtr<TopLevelAccelerationStructure> topLevelAccelerationStructure =
        createTopAccelerationStructure(context, bottomLevelAccelerationStructure);
    vectorBufferOrImage inputBuffers =
        makeRayTracingInputBuffers(context, format, extraDatas, extraDatasCount, stagesVector);
    const Move<VkDescriptorSetLayout> descriptorSetLayout =
        makeRayTracingDescriptorSetLayout(context, extraDatas, extraDatasCount, stagesVector, inputBuffers);
    ...
    const Move<VkPipelineLayout> pipelineLayout =
        makeRayTracingPipelineLayout(context, *descriptorSetLayout, *descriptorSetLayoutAS);
    Move<VkPipeline> pipeline                                = Move<VkPipeline>();
    const de::MovePtr<RayTracingPipeline> rayTracingPipeline = makeRayTracingPipeline(
        context, shaderStageTested, *pipelineLayout, shaderStageCreateFlags, requiredSubgroupSize, pipeline);
    const uint32_t shaderGroupHandleSize    = context.getRayTracingPipelineProperties().shaderGroupHandleSize;
    const uint32_t shaderGroupBaseAlignment = context.getRayTracingPipelineProperties().shaderGroupBaseAlignment;
    de::MovePtr<BufferWithMemory> rgenShaderBindingTable = rayTracingPipeline->createShaderBindingTable(
    ...
    de::MovePtr<BufferWithMemory> hitsShaderBindingTable = rayTracingPipeline->createShaderBindingTable(
        vkd, device, *pipeline, allocator, shaderGroupHandleSize, shaderGroupBaseAlignment, HIT_GROUP, 1u);
    de::MovePtr<BufferWithMemory> callShaderBindingTable = rayTracingPipeline->createShaderBindingTable(
        vkd, device, *pipeline, allocator, shaderGroupHandleSize, shaderGroupBaseAlignment, CALL_GROUP, 1u);
    const VkStridedDeviceAddressRegionKHR rgenShaderBindingTableRegion =
        makeStridedDeviceAddressRegionKHR(getBufferDeviceAddress(vkd, device, rgenShaderBindingTable->get(), 0),
    ...
    const VkStridedDeviceAddressRegionKHR missShaderBindingTableRegion =
        makeStridedDeviceAddressRegionKHR(getBufferDeviceAddress(vkd, device, missShaderBindingTable->get(), 0),
                                          shaderGroupHandleSize, shaderGroupHandleSize);
    const VkStridedDeviceAddressRegionKHR hitsShaderBindingTableRegion =
        makeStridedDeviceAddressRegionKHR(getBufferDeviceAddress(vkd, device, hitsShaderBindingTable->get(), 0),
                                          shaderGroupHandleSize, shaderGroupHandleSize);
    const VkStridedDeviceAddressRegionKHR callShaderBindingTableRegion =
        makeStridedDeviceAddressRegionKHR(getBufferDeviceAddress(vkd, device, callShaderBindingTable->get(), 0),
                                          shaderGroupHandleSize, shaderGroupHandleSize);
    const Move<VkDescriptorPool> descriptorPool = makeRayTracingDescriptorPool(context, inputBuffers);
    const Move<VkDescriptorSet> descriptorSet   = makeRayTracingDescriptorSet(
        context, *descriptorPool, *descriptorSetLayout, extraDatas, extraDatasCount, stagesVector, inputBuffers);
    const Move<VkDescriptorSet> descriptorSetAS =
        makeRayTracingDescriptorSetAS(context, *descriptorPool, *descriptorSetLayoutAS, topLevelAccelerationStructure);
    ...
    uint32_t passIterations               = 0u;
    uint32_t failIterations               = 0u;
    ...
    for (uint32_t width = 1u; width < maxWidth; width = getNextWidth(width))
    {
        ...
        beginCommandBuffer(vkd, *cmdBuffer);
        {
            ...
            const VkMemoryBarrier postTraceMemoryBarrier =
                makeMemoryBarrier(VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_HOST_READ_BIT);
            cmdPipelineMemoryBarrier(vkd, *cmdBuffer, VK_PIPELINE_STAGE_RAY_TRACING_SHADER_BIT_KHR,
                                     VK_PIPELINE_STAGE_HOST_BIT, &postTraceMemoryBarrier);
        }
    ...
    if (failIterations > 0 || passIterations == 0)
        return tcu::TestStatus::fail("Failed " + de::toString(failIterations) + " out of " +
                                     de::toString(failIterations + passIterations) + " iterations.");
    else
        return tcu::TestStatus::pass("OK");
}
#endif // CTS_USES_VULKANSC
