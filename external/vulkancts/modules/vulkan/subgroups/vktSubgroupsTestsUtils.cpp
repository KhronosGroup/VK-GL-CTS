    ...
    context.getTestContext().touchWatchdog();
    {
        pipelines[0] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(
            makeComputePipeline(context, *pipelineLayout, *shaderModule, pipelineShaderStageCreateFlags,
                                ...
                                0u,
#endif // CTS_USES_VULKANSC
                                (VkPipeline)DE_NULL, localSizesToTest[0][0], localSizesToTest[0][1],
                                localSizesToTest[0][2], isRequiredSubgroupSize ? subgroupSize : 0u)));
    }
    context.getTestContext().touchWatchdog();
        ...
        context.getTestContext().touchWatchdog();
        {
            pipelines[index] = de::SharedPtr<Move<VkPipeline>>(new Move<VkPipeline>(
                makeComputePipeline(context, *pipelineLayout, *shaderModule, pipelineShaderStageCreateFlags,
                                    ...
                                    0u,
#endif // CTS_USES_VULKANSC
                                    **pipelines[0], nextX, nextY, nextZ, isRequiredSubgroupSize ? subgroupSize : 0u)));
        }
        context.getTestContext().touchWatchdog();
    }
