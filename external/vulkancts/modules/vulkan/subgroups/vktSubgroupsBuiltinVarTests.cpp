...
bool checkVertexPipelineStagesSubgroupSize(const void *internalData, std::vector<const void *> datas, uint32_t width,
...
static bool checkComputeNumSubgroups(const void *internalData, std::vector<const void *> datas,
...
static bool checkComputeSubgroupID(const void *internalData, std::vector<const void *> datas,
                                   const uint32_t numWorkgroups[3], const uint32_t localSize[3], uint32_t)
{
    DE_UNREF(internalData);
    const uint32_t *data = reinterpret_cast<const uint32_t *>(datas[0]);
    ...
    {
        {
            ...
            */
            ...
            programCollection.spirvAsmSources.add("vert")
                << vertex << SpirVAsmBuildOptions(programCollection.usedVulkanVersion, SPIRV_VERSION_1_3);
        }

        {
            ...
            */
            ...
            programCollection.spirvAsmSources.add("tesc")
                << tesc << SpirVAsmBuildOptions(programCollection.usedVulkanVersion, SPIRV_VERSION_1_3);
        }

        {
            ...
            */
            ...
            programCollection.spirvAsmSources.add("tese")
                << tese << SpirVAsmBuildOptions(programCollection.usedVulkanVersion, SPIRV_VERSION_1_3);
        }

        {
            ...
            */
            ...
            addGeometryShadersFromTemplate(geometry,
...

        {
            ...
            */
            ...
            programCollection.spirvAsmSources.add("fragment")
                << fragment << SpirVAsmBuildOptions(programCollection.usedVulkanVersion, SPIRV_VERSION_1_3);
        }
...
tcu::TestStatus test(Context &context, const CaseDefinition caseDef)
{
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
        ...
        if ("gl_SubgroupSize" == caseDef.varName)
        {
                 ...
                 size <= subgroupSizeControlProperties.maxSubgroupSize; size *= 2)
            {
                tcu::TestStatus result = subgroups::makeComputeTest(
                    context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL, checkComputeSubgroupSize, size,
        ...
        else if ("gl_SubgroupInvocationID" == caseDef.varName)
        {
            ...
            {
                tcu::TestStatus result = subgroups::makeComputeTest(
                    context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL, checkComputeSubgroupInvocationID, size,
                    VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT);
                if (result.getCode() != QP_TEST_RESULT_PASS)
                {
                    log << tcu::TestLog::Message << "subgroupSize " << size << " failed" << tcu::TestLog::EndMessage;
        ...
        else if ("gl_NumSubgroups" == caseDef.varName)
        {
            ...
            {
                tcu::TestStatus result = subgroups::makeComputeTest(
                    context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL, checkComputeNumSubgroups, size,
                    VK_PIPELINE_SHADER_STAGE_CREATE_REQUIRE_FULL_SUBGROUPS_BIT_EXT);
                if (result.getCode() != QP_TEST_RESULT_PASS)
                {
                    log << tcu::TestLog::Message << "subgroupSize " << size << " failed" << tcu::TestLog::EndMessage;
        ...
        else if ("gl_SubgroupID" == caseDef.varName)
        {
                 ...
                 size <= subgroupSizeControlProperties.maxSubgroupSize; size *= 2)
            {
                tcu::TestStatus result =
                    subgroups::makeComputeTest(context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL, checkComputeSubgroupID,
...
tcu::TestCaseGroup *createSubgroupsBuiltinVarTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "Subgroup builtin variable tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "Subgroup builtin variable tests: compute"));
    de::MovePtr<tcu::TestCaseGroup> framebufferGroup(
        new tcu::TestCaseGroup(testCtx, "framebuffer", "Subgroup builtin variable tests: framebuffer"));
    ...
    for (int a = 0; a < DE_LENGTH_OF_ARRAY(all_stages_vars); ++a)
    {
        ...
        {
            CaseDefinition caseDef = {"gl_" + var, VK_SHADER_STAGE_COMPUTE_BIT, de::SharedPtr<bool>(new bool), false};
            addFunctionCaseWithPrograms(computeGroup.get(), varLower + "_" + getShaderStageName(caseDef.shaderStage),
                                        "", supportedCheck, initPrograms, test, caseDef);
            caseDef.requiredSubgroupSize = true;
            addFunctionCaseWithPrograms(
                computeGroup.get(), varLower + "_" + getShaderStageName(caseDef.shaderStage) + "_requiredsubgroupsize",
                "", supportedCheck, initPrograms, test, caseDef);
        }

        for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
        {
            const CaseDefinition caseDef = {"gl_" + var, stages[stageIndex], de::SharedPtr<bool>(new bool), false};
            addFunctionCaseWithPrograms(framebufferGroup.get(),
                                        varLower + "_" + getShaderStageName(caseDef.shaderStage), "", supportedCheck,
    ...
    for (int a = 0; a < DE_LENGTH_OF_ARRAY(compute_only_vars); ++a)
    {
        ...
        addFunctionCaseWithPrograms(computeGroup.get(), de::toLower(var), "", supportedCheck, initPrograms, test,
                                    caseDef);
        caseDef.requiredSubgroupSize = true;
        addFunctionCaseWithPrograms(computeGroup.get(), de::toLower(var) + "_requiredsubgroupsize", "", supportedCheck,
                                    initPrograms, test, caseDef);
    }

    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "builtin_var", "Subgroup builtin variable tests"));
