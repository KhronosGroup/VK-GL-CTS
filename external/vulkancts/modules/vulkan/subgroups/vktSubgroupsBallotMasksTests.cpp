...
static bool checkVertexPipelineStages(const void *internalData, std::vector<const void *> datas, uint32_t width,
                                      uint32_t)
{
...
static bool checkCompute(const void *internalData, std::vector<const void *> datas, const uint32_t numWorkgroups[3],
...
tcu::TestStatus noSSBOtest(Context &context, const CaseDefinition caseDef)
{
    ...
    if (VK_SHADER_STAGE_VERTEX_BIT == caseDef.shaderStage)
        ...
        return subgroups::makeGeometryFrameBufferTest(context, VK_FORMAT_R32_UINT, DE_NULL, 0, DE_NULL,
                                                      checkVertexPipelineStages);
    else if ((VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT | VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT) &
             caseDef.shaderStage)
    ...
    if (VK_SHADER_STAGE_COMPUTE_BIT == caseDef.shaderStage)
    {
             ...
             size <= subgroupSizeControlProperties.maxSubgroupSize; size *= 2)
        {
            tcu::TestStatus result =
                subgroups::makeComputeTest(context, VK_FORMAT_R32_UINT, DE_NULL, 0u, DE_NULL, checkCompute, size,
...
tcu::TestCaseGroup *createSubgroupsBallotMasksTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> graphicGroup(
        new tcu::TestCaseGroup(testCtx, "graphics", "VK_EXT_shader_subgroup_ballot masks category tests: graphics"));
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(testCtx, "compute", "VK_EXT_shader_subgroup_ballot masks category tests: compute"));
    ...
    for (int maskTypeIndex = 0; maskTypeIndex < MASKTYPE_LAST; ++maskTypeIndex)
    {
        ...
        {
            CaseDefinition caseDef = {maskTypeIndex, VK_SHADER_STAGE_COMPUTE_BIT, de::SharedPtr<bool>(new bool), false};
            ...
            addFunctionCaseWithPrograms(computeGroup.get(), mask + "_requiredsubgroupsize", "", supportedCheck,
        ...
        {
            const CaseDefinition caseDef = {maskTypeIndex, stages[stageIndex], de::SharedPtr<bool>(new bool), false};
            addFunctionCaseWithPrograms(framebufferGroup.get(), mask + "_" + getShaderStageName(caseDef.shaderStage),
    ...
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "ballot_mask", "VK_EXT_shader_subgroup_ballot mask category tests"));
