...
TestCaseGroup *createSubgroupsVoteTests(TestContext &testCtx)
{
    ...
    de::MovePtr<TestCaseGroup> graphicGroup(
        new TestCaseGroup(testCtx, "graphics", "Subgroup vote category tests: graphics"));
    de::MovePtr<TestCaseGroup> computeGroup(
        new TestCaseGroup(testCtx, "compute", "Subgroup vote category tests: compute"));
    de::MovePtr<TestCaseGroup> framebufferGroup(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup vote category tests: framebuffer"));
    de::MovePtr<TestCaseGroup> fragHelperGroup(
        new TestCaseGroup(testCtx, "frag_helper", "Subgroup vote category tests: fragment helper invocation"));
#ifndef CTS_USES_VULKANSC
    de::MovePtr<TestCaseGroup> raytracingGroup(
        new TestCaseGroup(testCtx, "ray_tracing", "Subgroup vote category tests: raytracing"));
#endif // CTS_USES_VULKANSC

    de::MovePtr<TestCaseGroup> groupARB(
        new TestCaseGroup(testCtx, "ext_shader_subgroup_vote", "VK_EXT_shader_subgroup_vote category tests"));
    ...
    de::MovePtr<TestCaseGroup> framebufferGroupARB(
        new TestCaseGroup(testCtx, "framebuffer", "Subgroup vote category tests: framebuffer"));
    de::MovePtr<TestCaseGroup> fragHelperGroupARB(
        new TestCaseGroup(testCtx, "frag_helper", "Subgroup vote category tests: fragment helper invocation"));
...

    {
        ...
        for (size_t formatIndex = 0; formatIndex < formats.size(); ++formatIndex)
        {
            ...
            for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
            {
                ...
                TestCaseGroup *computeGroupPtr =
                    (opType < OPTYPE_LAST_NON_ARB) ? computeGroup.get() : computeGroupARB.get();
                TestCaseGroup *graphicGroupPtr =
                    (opType < OPTYPE_LAST_NON_ARB) ? graphicGroup.get() : graphicGroupARB.get();
                TestCaseGroup *framebufferGroupPtr =
                    (opType < OPTYPE_LAST_NON_ARB) ? framebufferGroup.get() : framebufferGroupARB.get();
                TestCaseGroup *fragHelperGroupPtr =
                    (opType < OPTYPE_LAST_NON_ARB) ? fragHelperGroup.get() : fragHelperGroupARB.get();
                ...
                for (size_t groupSizeNdx = 0; groupSizeNdx < DE_LENGTH_OF_ARRAY(boolValues); ++groupSizeNdx)
                {
                    ...
                    addFunctionCaseWithPrograms(computeGroupPtr, testName, "", supportedCheck, initPrograms, test,
                                                caseDef);
                }
                ...
                for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
                {
                    ...
                    addFunctionCaseWithPrograms(framebufferGroupPtr, testName, "", supportedCheck,
