...
void initFrameBufferPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
    ...
    if (VK_SHADER_STAGE_VERTEX_BIT == caseDef.shaderStage)
    {
        ...
        vertexSrc << glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_450) << "\n"
                  << extensionHeader.c_str() << subgroups::getAdditionalExtensionForFormat(caseDef.format)
                  ...
                  << "{\n"
                  << "  uint result;\n"
                  << source << "  out_color.r = float(result);\n"
    ...
    else if (VK_SHADER_STAGE_FRAGMENT_BIT == caseDef.shaderStage)
    {
        ...
        programCollection.glslSources.add("fragment")
            << glu::FragmentSource(fragmentSource.str())
            << vk::ShaderBuildOptions(programCollection.usedVulkanVersion, vk::SPIRV_VERSION_1_3, 0u);
    }
    else
    {
...
void initPrograms(SourceCollections &programCollection, CaseDefinition caseDef)
{
        ...
        const string source =
            (OPTYPE_ALL == caseDef.opType || OPTYPE_ALL_ARB == caseDef.opType) ?
                "  result = " + op +
                    ...
                    "(false) ? 0 : 0x1A;\n"
                    "  result |= " +
                    op + "(data[gl_SubgroupInvocationID] > 0) ? 0x4 : 0;\n" :
            (OPTYPE_ANY == caseDef.opType || OPTYPE_ANY_ARB == caseDef.opType) ?
                "  result = " + op +
                    "(true) ? 0x1 : 0;\n"
                    "  result |= " +
                    ...
                    "(false) ? 0 : 0x1A;\n"
                    "  result |= " +
                    op + "(data[gl_SubgroupInvocationID] == data[0]) ? 0x4 : 0;\n" :
            (OPTYPE_ALLEQUAL == caseDef.opType || OPTYPE_ALLEQUAL_ARB == caseDef.opType) ?
                "  " + subgroups::getFormatNameForGLSL(caseDef.format) +
                    " valueEqual = " + subgroups::getFormatNameForGLSL(caseDef.format) +
                    "(1.25 * float(data[gl_SubgroupInvocationID]) + 5.0);\n"
    ...
    else
    {
        ...
        {
            const string vertex =
                "#version 450\n" + extensionHeader + subgroups::getAdditionalExtensionForFormat(caseDef.format) +
                "layout(set = 0, binding = 0, std430) buffer Buffer1\n"
                "{\n"
                "  uint res[];\n"
                "};\n"
...
tcu::TestStatus noSSBOtest(Context &context, const CaseDefinition caseDef)
{
    ...
    inputData.initializeType = (OPTYPE_ALLEQUAL == caseDef.opType || OPTYPE_ALLEQUAL_ARB == caseDef.opType) ?
...
tcu::TestCaseGroup *createSubgroupsVoteTests(tcu::TestContext &testCtx)
{
    ...
    for (size_t formatIndex = 0; formatIndex < formats.size(); ++formatIndex)
    {
        ...
        const bool formatIsNotVector = VK_FORMAT_R8_USCALED == format || VK_FORMAT_R32_UINT == format ||
                                       VK_FORMAT_R32_SINT == format || VK_FORMAT_R32_SFLOAT == format ||
                                       VK_FORMAT_R64_SFLOAT == format;
        ...
        for (int opTypeIndex = 0; opTypeIndex < OPTYPE_LAST; ++opTypeIndex)
        {
            ...
            if ((VK_FORMAT_R32_UINT != format) && (OPTYPE_ALLEQUAL != opTypeIndex) &&
                (OPTYPE_ALLEQUAL_ARB != opTypeIndex))
            {
                continue;
            }
...

            {
                CaseDefinition caseDef = {
                    opTypeIndex, VK_SHADER_STAGE_COMPUTE_BIT, format, de::SharedPtr<bool>(new bool), false, bool(false),
                    bool(false)};
                if (opTypeIndex < OPTYPE_LAST_NON_ARB)
                {
                    addFunctionCaseWithPrograms(computeGroup.get(), op + "_" + subgroups::getFormatNameForGLSL(format),
                    ...
                    addFunctionCaseWithPrograms(computeGroup.get(),
                                                op + "_" + subgroups::getFormatNameForGLSL(format) +
                                                    "_requiredsubgroupsize",
                                                "", supportedCheck, initPrograms, test, caseDef);
                }
                else
                {
                    addFunctionCaseWithPrograms(computeGroupARB.get(),
                                                op + "_" + subgroups::getFormatNameForGLSL(format), "", supportedCheck,
...

            {
                const CaseDefinition caseDef = {opTypeIndex, VK_SHADER_STAGE_ALL_GRAPHICS,
                                                format,      de::SharedPtr<bool>(new bool),
                                                false,       bool(false),
                                                bool(false)};
                if (opTypeIndex < OPTYPE_LAST_NON_ARB)
                {
                    addFunctionCaseWithPrograms(graphicGroup.get(), op + "_" + subgroups::getFormatNameForGLSL(format),
                                                "", supportedCheck, initPrograms, test, caseDef);
                }
                else
                {
            ...
            for (int stageIndex = 0; stageIndex < DE_LENGTH_OF_ARRAY(stages); ++stageIndex)
            {
                const CaseDefinition caseDef = {
                    opTypeIndex, stages[stageIndex], format,     de::SharedPtr<bool>(new bool),
                    false,       bool(false),        bool(false)};
                if (opTypeIndex < OPTYPE_LAST_NON_ARB)
                {
                    addFunctionCaseWithPrograms(framebufferGroup.get(),
                                                op + "_" + subgroups::getFormatNameForGLSL(format) + "_" +
            ...
            const CaseDefinition caseDef = {opTypeIndex,
                                            VK_SHADER_STAGE_FRAGMENT_BIT,
                                            format,
                                            de::SharedPtr<bool>(new bool),
                                            false,
                                            bool(needs8BitUBOStorage),
                                            bool(needs16BitUBOStorage)};
            if (opTypeIndex < OPTYPE_LAST_NON_ARB)
            {
    ...
    de::MovePtr<tcu::TestCaseGroup> groupARB(
        new tcu::TestCaseGroup(testCtx, "ext_shader_subgroup_vote", "VK_EXT_shader_subgroup_vote category tests"));
    ...
    de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(testCtx, "vote", "Subgroup vote category tests"));
