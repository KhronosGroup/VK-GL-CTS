...
#define MAKE_FEATURE_IMPL(clazz, enume, versione) \
    template <>                                   \
    FeatureDesc makeFeatureDesc<clazz>()          \
    {                                             \
        return FeatureDesc{enume, "", versione};  \
    }
...
MAKE_FEATURE_IMPL(VkPhysicalDeviceFeatures, VK_STRUCTURE_TYPE_MAX_ENUM, VK_API_VERSION_1_0)
INIT_FEATURE_IMPL(VkPhysicalDeviceFeatures, VK_STRUCTURE_TYPE_MAX_ENUM)
MAKE_FEATURE_IMPL(VkPhysicalDeviceFeatures2, VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2, VK_API_VERSION_1_1)
...
#ifdef CTS_USES_VULKANSC
MAKE_FEATURE_IMPL(VkFaultCallbackInfo, VK_STRUCTURE_TYPE_FAULT_CALLBACK_INFO, VKSC_API_VERSION_1_0)
...
de::SharedPtr<ContextManager> ContextManager::getCustomManagerById(const std::string &managerId) const
{
    for (auto mgr : m_customManagers)
    {
        if (mgr->id == managerId)
