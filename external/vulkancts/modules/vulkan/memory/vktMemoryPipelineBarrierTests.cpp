...
HostMemoryAccess::HostMemoryAccess(bool read, bool write, uint32_t seed) : m_read(read), m_write(write), m_seed(seed)
{
}
...
CreateImage::CreateImage(vk::VkImageUsageFlags usage, vk::VkSharingMode sharing) : m_usage(usage), m_sharing(sharing)
{
}
...
SubmitCommandBuffer::SubmitCommandBuffer(const vector<CmdCommand *> &commands) : m_commands(commands)
{
}
...
{
public:
    FillBuffer(uint32_t value) : m_value(value)
    {
    }
    ~FillBuffer(void)
    {
...
{
public:
    UpdateBuffer(uint32_t seed) : m_seed(seed)
    {
    }
    ~UpdateBuffer(void)
    {
...
void BufferCopyToBuffer::submit(SubmitContext &context)
{
    ...
    const vk::VkBufferCopy range            = {0, 0, // Offsets
                                               m_bufferSize};
...
{
public:
    BufferCopyFromBuffer(uint32_t seed) : m_seed(seed)
    {
    }
...
void BufferCopyToImage::submit(SubmitContext &context)
{
    const vk::DeviceInterface &vkd          = context.getContext().getDeviceInterface();
    const vk::VkCommandBuffer commandBuffer = context.getCommandBuffer();
    const vk::VkBufferImageCopy region      = {0,
                                               0,
                                               0,
                                               {
                                              vk::VK_IMAGE_ASPECT_COLOR_BIT,
...

    {
        ...
        const vk::VkBufferImageCopy region              = {0,
                                                  ...
                                                  1  // layerCount
                                                           ...
                                                           {0, 0, 0},
                                                           {(uint32_t)m_srcImageWidth, (uint32_t)m_srcImageHeight, 1u}};
...
void ImageBlitFromImage::submit(SubmitContext &context)
{
    const vk::DeviceInterface &vkd          = context.getContext().getDeviceInterface();
    const vk::VkCommandBuffer commandBuffer = context.getCommandBuffer();
    const vk::VkImageBlit region            = {// Src
                                    {
                                        vk::VK_IMAGE_ASPECT_COLOR_BIT,
                                        0, // mipLevel
                                        0, // arrayLayer
                                    ...
                                    {{0, 0, 0}, {m_imageWidth, m_imageHeight, 1u}}};
...
void ImageBlitToImage::submit(SubmitContext &context)
{
    ...
    const vk::VkImageBlit region            = {// Src
                                    ...
                                    {{0, 0, 0}, {m_dstImageWidth, m_dstImageHeight, 1u}}};
...

    {
        const vk::VkImageViewCreateInfo createInfo = {vk::VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
                                                      ...
                                                      *m_colorTarget,
                                                      ...
                                                      vk::VK_FORMAT_R8G8B8A8_UNORM,
                                                      {vk::VK_COMPONENT_SWIZZLE_R, vk::VK_COMPONENT_SWIZZLE_G,
...
{
public:
    RenderVertexBuffer(uint32_t stride) : m_stride(stride), m_name("RenderVertexBuffer" + de::toString(stride))
    {
    }
...
void RenderVertexBuffer::prepare(PrepareRenderPassContext &context)
{
    ...
    {
        const vk::VkVertexInputBindingDescription vertexBindingDescription = {0, m_stride,
                                                                              vk::VK_VERTEX_INPUT_RATE_VERTEX};

        vertexBindingDescriptions.push_back(vertexBindingDescription);
    }
    {
        const vk::VkVertexInputAttributeDescription vertexAttributeDescription = {0, 0, vk::VK_FORMAT_R8G8_UNORM, 0};
    ...
    for (size_t descriptorSetNdx = 0; descriptorSetNdx < m_descriptorSets.size(); descriptorSetNdx++)
    {
...

        {
            const vk::VkDescriptorBufferInfo bufferInfo = {
                context.getBuffer(), (vk::VkDeviceSize)(descriptorSetNdx * (size_t)MAX_UNIFORM_BUFFER_SIZE),
...
void RenderVertexStorageBuffer::prepare(PrepareRenderPassContext &context)
{
    ...
    for (size_t descriptorSetNdx = 0; descriptorSetNdx < m_descriptorSets.size(); descriptorSetNdx++)
    {
        ...
        {
            const vk::VkDescriptorBufferInfo bufferInfo = {
                context.getBuffer(), descriptorSetNdx * MAX_STORAGE_BUFFER_SIZE,
...
void RenderFragmentUniformBuffer::prepare(PrepareRenderPassContext &context)
{
    ...
    const vk::VkPushConstantRange pushConstantRange = {vk::VK_SHADER_STAGE_FRAGMENT_BIT, 0u, 8u};
    ...
    for (size_t descriptorSetNdx = 0; descriptorSetNdx < m_descriptorSets.size(); descriptorSetNdx++)
    {
        const struct
        {
            ...
            const uint32_t valuesPerPixel;
                        ...
                        (uint32_t)divRoundUp<size_t>(m_descriptorSets.size() * (MAX_UNIFORM_BUFFER_SIZE / 4),
...
void RenderFragmentUniformBuffer::verify(VerifyRenderPassContext &context, size_t)
{
    const uint32_t valuesPerPixel = (uint32_t)divRoundUp<size_t>(
        m_descriptorSets.size() * (MAX_UNIFORM_BUFFER_SIZE / 4), m_targetWidth * m_targetHeight);
    ...
    for (int y = 0; y < context.getReferenceTarget().getSize().y(); y++)
        for (int x = 0; x < context.getReferenceTarget().getSize().x(); x++)
        {
            const size_t firstDescriptorSetNdx =
                de::min<size_t>((y * 256u + x) / (arrayIntSize / valuesPerPixel), m_descriptorSets.size() - 1);
                 ...
                 descriptorSetNdx++)
            {
                ...
                const uint32_t id =
                    callId * ((uint32_t)arrayIntSize / valuesPerPixel) + (uint32_t)y * 256u + (uint32_t)x;
                ...
                else
                {
                    ...
                    {
                        value = ((uint32_t)context.getReference().get(
                                    offset + (value % (MAX_UNIFORM_BUFFER_SIZE / sizeof(uint32_t))) * 4 + 0)) |
                                (((uint32_t)context.getReference().get(
                                     offset + (value % (MAX_UNIFORM_BUFFER_SIZE / sizeof(uint32_t))) * 4 + 1))
                                 << 8u) |
                                (((uint32_t)context.getReference().get(
                                     offset + (value % (MAX_UNIFORM_BUFFER_SIZE / sizeof(uint32_t))) * 4 + 2))
                                 << 16u) |
                                (((uint32_t)context.getReference().get(
                                     offset + (value % (MAX_UNIFORM_BUFFER_SIZE / sizeof(uint32_t))) * 4 + 3))
                                 << 24u);
                    }
...
struct State
{
    State(Usage usage, uint32_t seed)
        : stage(STAGE_HOST)
...
void getAvailableOps(const State &state, bool supportsBuffers, bool supportsImages, Usage usage, vector<Op> &ops)
{
    if (state.stage == STAGE_HOST)
    {
        ...
        if (state.hostFlushed &&
            ...
            && (usageToStageFlags(usage) & (~vk::VK_PIPELINE_STAGE_HOST_BIT)) !=
    ...
    else if (state.stage == STAGE_COMMAND_BUFFER)
    {
        ...
        if ((state.memoryDefined && state.hasBoundBufferMemory &&
...
de::MovePtr<CmdCommand> createSecondaryCmdCommands(const Memory &memory, de::Random &nextOpRng, State &state,
                                                   Usage usage, size_t &opNdx, size_t opCount)
{
                                       ...
                                       const TestConfig &testConfig, size_t &opNdx, size_t opCount)
{
    ...
    try
    {
        ...
        commands.push_back(new PipelineBarrier(state.cache.getAllowedStages(), state.cache.getAllowedAcceses(),
                                               ...
                                               PipelineBarrier::TYPE_GLOBAL, tcu::nothing<vk::VkImageLayout>()));
...

            {
                ...
                else
                {
                    ...
                    else if (op == OP_SECONDARY_COMMAND_BUFFER_BEGIN)
                    {
                        applyOp(state, memory, op, testConfig.usage);
                        commands.push_back(
                            createSecondaryCmdCommands(memory, nextOpRng, state, testConfig.usage, opNdx, opCount)
                                .release());
                    }
                    else
                    {
...

            {
                ...
                fragmentShader << "#version 310 es\n"
                               ...
                               << "];\n"
                                  ...
                                  "} pushC;\n"
                                  "void main (void) {\n"
                                  "\thighp uint id = pushC.callId * ("
                               << arrayIntSize
                               << "u / pushC.valuesPerPixel) + uint(gl_FragCoord.y) * 256u + uint(gl_FragCoord.x);\n"
                               ...
                               << "u / pushC.valuesPerPixel))\n"
                                  ...
                                  "\t{\n"
                                  "\t\thighp uvec4 vecVal = block.values[(value / 4u) % "
                               << arraySize
                               << "u];\n"
                                  "\t\tif ((value % 4u) == 0u)\n"
                                  "\t\t\tvalue = vecVal.x;\n"
...
tcu::TestCaseGroup *createPipelineBarrierTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "pipeline_barrier", "Pipeline barrier tests."));
    ...
    for (size_t writeUsageNdx = 0; writeUsageNdx < DE_LENGTH_OF_ARRAY(writeUsages); writeUsageNdx++)
    {
        ...
        for (size_t readUsageNdx = 0; readUsageNdx < DE_LENGTH_OF_ARRAY(readUsages); readUsageNdx++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> usageGroup(
                new tcu::TestCaseGroup(testCtx, usageGroupName.c_str(), usageGroupName.c_str()));
                ...
                if (readUsage == USAGE_VERTEX_BUFFER)
                {
                    for (size_t strideNdx = 0; strideNdx < DE_LENGTH_OF_ARRAY(vertexStrides); ++strideNdx)
                    {
                        ...
                        usageGroup->addChild(new InstanceFactory1<MemoryTestInstance, TestConfig, AddPrograms>(
                            testCtx, tcu::NODETYPE_SELF_VALIDATE, finalTestName, finalTestName, AddPrograms(), config));
                    }
                }
                else
                {
                    usageGroup->addChild(new InstanceFactory1<MemoryTestInstance, TestConfig, AddPrograms>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, testName, testName, AddPrograms(), config));
                }
            }
...

        {
            ...
            for (size_t sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(sizes); sizeNdx++)
            {
                ...
                for (size_t strideNdx = 0; strideNdx < DE_LENGTH_OF_ARRAY(vertexStrides); ++strideNdx)
                {
                    ...
                    const TestConfig config = {all, stride, size, vk::VK_SHARING_MODE_EXCLUSIVE};
