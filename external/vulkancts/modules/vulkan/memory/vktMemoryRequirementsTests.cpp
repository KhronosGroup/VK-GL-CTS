...
Move<VkBuffer> makeBuffer(const DeviceInterface &vk, const VkDevice device, const VkDeviceSize size,
...
VkMemoryRequirements getBufferMemoryRequirements(const DeviceInterface &vk, const VkDevice device,
...
std::vector<uint32_t> bitsToIndices(uint32_t bits)
{
    std::vector<uint32_t> indices;
    for (uint32_t i = 0u; bits != 0u; ++i, bits >>= 1)
    {
        if (bits & 1u)
            indices.push_back(i);
    }
...
T nextEnum(T value)
{
    return static_cast<T>(static_cast<uint32_t>(value) + 1);
}
...
T nextFlagExcluding(T value, T excludedFlags)
{
    ...
    while ((tmp = nextFlag(tmp)) & static_cast<uint32_t>(excludedFlags))
        ;
...
protected:
    virtual void addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name, const std::string &desc,
                                     VkBufferCreateFlags arg0) = 0;

    virtual tcu::TestStatus execTest(Context &context, const VkBufferCreateFlags bufferFlags) = 0;

    virtual void updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device, const VkDeviceSize size,
                                          const VkBufferCreateFlags flags, const VkBufferUsageFlags usage,
                                          const bool all) = 0;

    virtual void verifyMemoryRequirements(tcu::ResultCollector &result,
                                          const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties,
                                          const VkPhysicalDeviceLimits &limits, const VkBufferCreateFlags bufferFlags,
                                          const VkBufferUsageFlags usage) = 0;
};

class BufferMemoryRequirementsOriginal : public IBufferMemoryRequirements
{
    static tcu::TestStatus testEntryPoint(Context &context, const VkBufferCreateFlags bufferFlags);
...
protected:
    ...
    virtual tcu::TestStatus execTest(Context &context, const VkBufferCreateFlags bufferFlags);
...
tcu::TestStatus BufferMemoryRequirementsOriginal::testEntryPoint(Context &context,
...
void BufferMemoryRequirementsOriginal::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
                                                           const std::string &desc, VkBufferCreateFlags arg0)
{
...
tcu::TestStatus BufferMemoryRequirementsOriginal::execTest(Context &context, const VkBufferCreateFlags bufferFlags)
{
    ...
    const VkBufferUsageFlags allUsageFlags =
        static_cast<VkBufferUsageFlags>((VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT << 1) - 1);
    ...
    for (VkBufferUsageFlags usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT; usage <= VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT;
         usage                    = nextFlag(usage))
    {
        uint32_t previousMemoryTypeBits = 0u;
        VkDeviceSize previousAlignment  = 0u;

        log << tcu::TestLog::Message
            << "Verify a buffer with usage flags: " << de::toString(getBufferUsageFlagsStr(usage))
...
void BufferMemoryRequirementsOriginal::updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device,
                                                                const VkDeviceSize size,
                                                                const VkBufferCreateFlags flags,
                                                                const VkBufferUsageFlags usage, const bool all)
{
    if (all)
    {
...
void BufferMemoryRequirementsOriginal::verifyMemoryRequirements(
    tcu::ResultCollector &result, const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties,
    const VkPhysicalDeviceLimits &limits, const VkBufferCreateFlags bufferFlags, const VkBufferUsageFlags usage)
{
    if (result.check(m_currentTestRequirements.memoryTypeBits != 0,
                     "VkMemoryRequirements memoryTypeBits has no bits set"))
    {
        typedef std::vector<uint32_t>::const_iterator IndexIterator;
        const std::vector<uint32_t> usedMemoryTypeIndices = bitsToIndices(m_currentTestRequirements.memoryTypeBits);
        bool deviceLocalMemoryFound                       = false;
        bool hostVisibleCoherentMemoryFound               = false;

        for (IndexIterator memoryTypeNdx = usedMemoryTypeIndices.begin(); memoryTypeNdx != usedMemoryTypeIndices.end();
             ++memoryTypeNdx)
        {
            ...
            const VkMemoryPropertyFlags memoryPropertyFlags =
                deviceMemoryProperties.memoryTypes[*memoryTypeNdx].propertyFlags;
        ...
        result.check(deIsPowerOfTwo64(static_cast<uint64_t>(m_currentTestRequirements.alignment)) == true,
        ...
        result.check((bufferFlags & VK_BUFFER_CREATE_SPARSE_BINDING_BIT) || hostVisibleCoherentMemoryFound,
                     "Required memory type doesn't include VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT and "
                     "VK_MEMORY_PROPERTY_HOST_COHERENT_BIT");

        result.check((m_currentTestRequirements.memoryTypeBits & m_allUsageFlagsRequirements.memoryTypeBits) ==
                         m_allUsageFlagsRequirements.memoryTypeBits,
...
tcu::TestStatus BufferMemoryRequirementsExtended::testEntryPoint(Context &context,
                                                                 const VkBufferCreateFlags bufferFlags)
{
...
void BufferMemoryRequirementsExtended::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
                                                           const std::string &desc, VkBufferCreateFlags arg0)
{
    addFunctionCase(group, name, desc, checkSupportBufferMemoryRequirementsExtended, testEntryPoint, arg0);
}

void BufferMemoryRequirementsExtended::updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device,
                                                                const VkDeviceSize size,
                                                                const VkBufferCreateFlags flags,
                                                                const VkBufferUsageFlags usage, const bool all)
{
...
tcu::TestStatus BufferMemoryRequirementsDedicatedAllocation::testEntryPoint(Context &context,
                                                                            const VkBufferCreateFlags bufferFlags)
{
...
void BufferMemoryRequirementsDedicatedAllocation::updateMemoryRequirements(
    const DeviceInterface &vk, const VkDevice device, const VkDeviceSize size, const VkBufferCreateFlags flags,
    const VkBufferUsageFlags usage, const bool all)
{
    const uint32_t invalidVkBool32 = static_cast<uint32_t>(~0);
    ...
    if (all)
    {
        m_allUsageFlagsRequirements =
            getBufferMemoryRequirements2(vk, device, size, flags, usage, &dedicatedRequirements);
...
void BufferMemoryRequirementsDedicatedAllocation::verifyMemoryRequirements(
    tcu::ResultCollector &result, const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties,
    const VkPhysicalDeviceLimits &limits, const VkBufferCreateFlags bufferFlags, const VkBufferUsageFlags usage)
{
    BufferMemoryRequirementsExtended::verifyMemoryRequirements(result, deviceMemoryProperties, limits, bufferFlags,
                                                               usage);
...
tcu::TestStatus BufferMemoryRequirementsCreateInfo::testEntryPoint(Context &context,
                                                                   const VkBufferCreateFlags bufferFlags)
{
...
void BufferMemoryRequirementsCreateInfo::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
                                                             const std::string &desc, VkBufferCreateFlags arg0)
{
    addFunctionCase(group, name, desc, checkSupportBufferMemoryRequirementsCreateInfo, testEntryPoint, arg0);
}

void BufferMemoryRequirementsCreateInfo::updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device,
                                                                  const VkDeviceSize size,
                                                                  const VkBufferCreateFlags flags,
                                                                  const VkBufferUsageFlags usage, const bool all)
{
...
void BufferMemoryRequirementsCreateInfo::verifyMemoryRequirements(
    tcu::ResultCollector &result, const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties,
    const VkPhysicalDeviceLimits &limits, const VkBufferCreateFlags bufferFlags, const VkBufferUsageFlags usage)
{
    BufferMemoryRequirementsOriginal::verifyMemoryRequirements(result, deviceMemoryProperties, limits, bufferFlags,
                                                               usage);

    result.check(m_currentTestRequirements.alignment == m_currentTestOriginalRequirements.alignment,
                 "VkMemoryRequirements alignment queried from vkBufferCreateInfo does not match alignment from "
                 "equivalent object");

    result.check(m_currentTestRequirements.memoryTypeBits == m_currentTestOriginalRequirements.memoryTypeBits,
                 "VkMemoryRequirements memoryTypeBits queried from vkBufferCreateInfo does not match memoryTypeBits "
                 "from equivalent object");

    result.check(
        m_currentTestRequirements.size == m_currentTestOriginalRequirements.size,
...
{
    ImageTestParams(VkImageCreateFlags flags_, VkImageTiling tiling_, bool transient_, bool useMaint4_ = false)
        : flags(flags_)
        , tiling(tiling_)
...
protected:
    ...
    virtual tcu::TestStatus execTest(Context &context, const ImageTestParams bufferFlags) = 0;

    virtual void updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device) = 0;
...
{
    static tcu::TestStatus testEntryPoint(Context &context, const ImageTestParams params);
...
protected:
    ...
    virtual tcu::TestStatus execTest(Context &context, const ImageTestParams params);

    virtual void updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device);

    virtual void verifyMemoryRequirements(tcu::ResultCollector &result,
                                          const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties);

private:
    virtual bool isImageSupported(const Context &context, const InstanceInterface &vki,
                                  const VkPhysicalDevice physDevice, const VkImageCreateInfo &info);

    virtual bool isFormatMatchingAspect(const VkFormat format, const VkImageAspectFlags aspect);
...
void ImageMemoryRequirementsOriginal::populateTestGroup(tcu::TestCaseGroup *group)
{
    const struct
    {
        ...
        {VK_IMAGE_CREATE_SPARSE_BINDING_BIT | VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT | VK_IMAGE_CREATE_SPARSE_ALIASED_BIT,
         false, "sparse_residency_aliased"},
#endif // CTS_USES_VULKANSC
    };
...
void checkSupportImageMemoryRequirementsOriginal(Context &context, ImageTestParams params)
{
    ...
    if ((params.flags & VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT) &&
        !(features.sparseResidencyImage2D || features.sparseResidencyImage3D))
        TCU_THROW(NotSupportedError, "Feature not supported: sparseResidencyImage (2D and 3D)");
}

void ImageMemoryRequirementsOriginal::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
                                                          const std::string &desc, const ImageTestParams arg0)
{
    addFunctionCase(group, name, desc, checkSupportImageMemoryRequirementsOriginal, testEntryPoint, arg0);
}

void ImageMemoryRequirementsOriginal::updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device)
{
...
void ImageMemoryRequirementsOriginal::verifyMemoryRequirements(
    tcu::ResultCollector &result, const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties)
{
    if (result.check(m_currentTestRequirements.memoryTypeBits != 0,
                     "VkMemoryRequirements memoryTypeBits has no bits set"))
             ...
             ++memoryTypeNdx)
        {
            ...
            if (memoryPropertyFlags & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT)
            {
                result.check((m_currentTestImageInfo.usage & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT) != 0u,
                             "Memory type includes VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT for a non-transient "
                             "attachment image");
            }
        }
...
bool ImageMemoryRequirementsOriginal::isImageSupported(const Context &context, const InstanceInterface &vki,
                                                       const VkPhysicalDevice physDevice, const VkImageCreateInfo &info)
{
    DE_ASSERT(info.extent.width >= 1u && info.extent.height >= 1u && info.extent.depth >= 1u);

    if (isYCbCrFormat(info.format) && (info.imageType != VK_IMAGE_TYPE_2D || info.mipLevels != 1 ||
    ...
    if ((info.samples != VK_SAMPLE_COUNT_1_BIT) &&
        (info.imageType != VK_IMAGE_TYPE_2D || (info.flags & VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT) ||
         info.tiling != VK_IMAGE_TILING_OPTIMAL || info.mipLevels > 1u))
        return false;
    ...
    if (info.samples != VK_SAMPLE_COUNT_1_BIT && (info.usage & VK_IMAGE_USAGE_STORAGE_BIT) &&
    ...
    const VkFormatFeatureFlags formatFeatures =
        (info.tiling == VK_IMAGE_TILING_LINEAR ? formatProperties.linearTilingFeatures :
...
{
    DE_ASSERT(aspect == VK_IMAGE_ASPECT_COLOR_BIT ||
...
std::string getImageInfoString(const VkImageCreateInfo &imageInfo)
{
    ...
    str << "samples:" << static_cast<uint32_t>(imageInfo.samples) << " ";
    str << "flags:" << static_cast<uint32_t>(imageInfo.flags) << " ";
    str << "usage:" << static_cast<uint32_t>(imageInfo.usage) << " ";
...
tcu::TestStatus ImageMemoryRequirementsOriginal::execTest(Context &context, const ImageTestParams params)
{
    ...
    tcu::TestLog &log                                       = context.getTestContext().getLog();
    bool allPass                                            = true;
    uint32_t numCheckedImages                               = 0u;

    log << tcu::TestLog::Message
        << "Verify memory requirements for the following parameter combinations:" << tcu::TestLog::EndMessage;

    for (uint32_t loopAspectNdx = 0u; loopAspectNdx < DE_LENGTH_OF_ARRAY(allAspects); ++loopAspectNdx)
    {
        const VkImageAspectFlags aspect = allAspects[loopAspectNdx];
        uint32_t previousMemoryTypeBits = notInitializedBits;
        ...
        for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
        {
            ...
            if (isFormatMatchingAspect(format, aspect))
            {
                ...
                for (VkImageType loopImageType = VK_IMAGE_TYPE_1D; loopImageType != VK_IMAGE_TYPE_LAST;
                     loopImageType             = nextEnum(loopImageType))
                    for (VkImageCreateFlags loopCreateFlags = (VkImageCreateFlags)0;
                         loopCreateFlags <= VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT;
                         loopCreateFlags = nextFlagExcluding(loopCreateFlags, sparseFlags))
                        for (VkImageUsageFlags loopUsageFlags = VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
                            ...
                            for (VkSampleCountFlagBits loopSampleCount                      = VK_SAMPLE_COUNT_1_BIT;
                                 loopSampleCount <= VK_SAMPLE_COUNT_16_BIT; loopSampleCount = nextFlag(loopSampleCount))
                            {
                                const VkImageCreateFlags actualCreateFlags = loopCreateFlags | params.flags;
                                const VkImageUsageFlags actualUsageFlags =
                                    loopUsageFlags | (params.transient ? VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT :
                                ...
                                log << tcu::TestLog::Message << "- " << getImageInfoString(m_currentTestImageInfo)
                                    << tcu::TestLog::EndMessage;
                                ++numCheckedImages;
                                ...
                                result.check((previousMemoryTypeBits == notInitializedBits) ||
                                                             (m_currentTestRequirements.memoryTypeBits == previousMemoryTypeBits),
    ...
    if (numCheckedImages == 0u)
        log << tcu::TestLog::Message << "NOTE: No supported image configurations -- nothing to check"
            << tcu::TestLog::EndMessage;
...
void ImageMemoryRequirementsExtended::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
                                                          const std::string &desc, const ImageTestParams arg0)
{
    addFunctionCase(group, name, desc, checkSupportImageMemoryRequirementsExtended, testEntryPoint, arg0);
}

void ImageMemoryRequirementsExtended::updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device)
{
...
tcu::TestStatus ImageMemoryRequirementsDedicatedAllocation::testEntryPoint(Context &context,
...
void ImageMemoryRequirementsDedicatedAllocation::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
...
void ImageMemoryRequirementsDedicatedAllocation::verifyMemoryRequirements(
    tcu::ResultCollector &result, const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties)
{
...
void ImageMemoryRequirementsCreateInfo::addFunctionTestCase(tcu::TestCaseGroup *group, const std::string &name,
                                                            const std::string &desc, const ImageTestParams arg0)
{
    addFunctionCase(group, name, desc, checkSupportImageMemoryRequirementsCreateInfo, testEntryPoint, arg0);
}

void ImageMemoryRequirementsCreateInfo::updateMemoryRequirements(const DeviceInterface &vk, const VkDevice device)
{
    ...
    if (m_currentTestImageInfo.flags & VK_IMAGE_CREATE_SPARSE_BINDING_BIT)
    {
        m_currentTestSparseRequirements =
            getImageCreateInfoSparseMemoryRequirements(vk, device, m_currentTestImageInfo);
        m_currentTestOriginalSparseRequirements = getImageSparseMemoryRequirements(vk, device, *image);
    }
}

void ImageMemoryRequirementsCreateInfo::verifyMemoryRequirements(
    tcu::ResultCollector &result, const VkPhysicalDeviceMemoryProperties &deviceMemoryProperties)
{
    ...
    result.check(
        m_currentTestRequirements.memoryTypeBits == m_currentTestOriginalRequirements.memoryTypeBits,
    ...
    if (m_currentTestImageInfo.flags & VK_IMAGE_CREATE_SPARSE_BINDING_BIT)
    {
        result.check(m_currentTestSparseRequirements.size() == m_currentTestOriginalSparseRequirements.size(),
                     "VkSparseImageMemoryRequirements count queried from vkImageCreateInfo differs from equivalent "
                     "object query");

        for (uint32_t ndx = 0; ndx < m_currentTestSparseRequirements.size(); ++ndx)
        {
            const VkSparseImageMemoryRequirements &sparseRequirementsFromCreateInfo =
                m_currentTestSparseRequirements[ndx];
            const VkSparseImageMemoryRequirements &sparseRequirementsFromObject =
                m_currentTestOriginalSparseRequirements[ndx];

            result.check(sparseRequirementsFromCreateInfo.formatProperties.aspectMask ==
                             sparseRequirementsFromObject.formatProperties.aspectMask,
                         "VkSparseImageMemoryRequirements aspectMask queried from vkImageCreateInfo differs from "
                         "equivalent object query");

            result.check(sparseRequirementsFromCreateInfo.formatProperties.flags ==
                             sparseRequirementsFromObject.formatProperties.flags,
                         "VkSparseImageMemoryRequirements flags queried from vkImageCreateInfo differs from equivalent "
                         "object query");

            result.check(sparseRequirementsFromCreateInfo.formatProperties.imageGranularity.width ==
                             sparseRequirementsFromObject.formatProperties.imageGranularity.width,
                         "VkSparseImageMemoryRequirements imageGranularity queried from vkImageCreateInfo differs from "
                         "equivalent object query");
            result.check(sparseRequirementsFromCreateInfo.formatProperties.imageGranularity.height ==
                             sparseRequirementsFromObject.formatProperties.imageGranularity.height,
                         "VkSparseImageMemoryRequirements imageGranularity queried from vkImageCreateInfo differs from "
                         "equivalent object query");
            result.check(sparseRequirementsFromCreateInfo.formatProperties.imageGranularity.depth ==
                             sparseRequirementsFromObject.formatProperties.imageGranularity.depth,
                         "VkSparseImageMemoryRequirements imageGranularity queried from vkImageCreateInfo differs from "
                         "equivalent object query");

            result.check(sparseRequirementsFromCreateInfo.imageMipTailFirstLod ==
                             sparseRequirementsFromObject.imageMipTailFirstLod,
                         "VkSparseImageMemoryRequirements imageMipTailFirstLod queried from vkImageCreateInfo differs "
                         "from equivalent object query");

            result.check(sparseRequirementsFromCreateInfo.imageMipTailSize ==
                             sparseRequirementsFromObject.imageMipTailSize,
                         "VkSparseImageMemoryRequirements imageMipTailSize queried from vkImageCreateInfo differs from "
                         "equivalent object query");

            result.check(sparseRequirementsFromCreateInfo.imageMipTailOffset ==
                             sparseRequirementsFromObject.imageMipTailOffset,
                         "VkSparseImageMemoryRequirements imageMipTailOffset queried from vkImageCreateInfo differs "
                         "from equivalent object query");

            result.check(sparseRequirementsFromCreateInfo.imageMipTailStride ==
                             sparseRequirementsFromObject.imageMipTailStride,
                         "VkSparseImageMemoryRequirements imageMipTailStride queried from vkImageCreateInfo differs "
                         "from equivalent object query");
        }
    }
}
...
bool isMultiplaneImageSupported(const InstanceInterface &vki, const VkPhysicalDevice physicalDevice,
...
tcu::TestStatus testMultiplaneImages(Context &context, ImageTestParams params)
{
    ...
    nonSparseCreateFlags.insert(nonSparseCreateFlags.end(), nonSparseCreateFlagsNoSC.begin(),
                                nonSparseCreateFlagsNoSC.end());
#endif // CTS_USES_VULKANSC
    ...
    for (const VkFormat format : multiplaneFormats)
    {
        for (const VkImageCreateFlags loopCreateFlags : (isSparseTest ? sparseCreateFlags : nonSparseCreateFlags))
        {
            const VkImageType imageType =
                ((loopCreateFlags & VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT)
#ifndef CTS_USES_VULKANSC
                 || (loopCreateFlags & VK_IMAGE_CREATE_2D_VIEW_COMPATIBLE_BIT_EXT)
#endif // CTS_USES_VULKANSC
                     ) ?
                    isYCbCrConversionFormat(format) ? VK_IMAGE_TYPE_MAX_ENUM : VK_IMAGE_TYPE_3D :
                    VK_IMAGE_TYPE_2D;
            if (imageType == VK_IMAGE_TYPE_MAX_ENUM)
                continue;

            if ((loopCreateFlags & VK_IMAGE_CREATE_SAMPLE_LOCATIONS_COMPATIBLE_DEPTH_BIT_EXT) &&
                 ...
                 loopUsageFlags = nextFlagExcluding(loopUsageFlags, transientFlags))
            {
                ...
                const VkImageUsageFlags actualUsageFlags =
                    loopUsageFlags |
                    (params.transient ? VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT : (VkImageUsageFlagBits)0);
                const uint32_t arrayLayers =
                    ((imageType == VK_IMAGE_TYPE_2D) && (loopCreateFlags & VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT)) ? 6u :
                                                                                                                   1u;
                const VkImageCreateInfo imageInfo = {
                    VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO, // VkStructureType          sType;
                    DE_NULL,                             // const void*              pNext;
                ...
                {
                    const Unique<VkImage> image((params.useMaint4) ? Move<VkImage>() :
                                                                     createImage(vk, device, &imageInfo));
                    ...
                    for (uint32_t planeNdx = 0; planeNdx < (uint32_t)getPlaneCount(format); planeNdx++)
                    {
                        ...
                        log << TestLog::Message << "Aspect: " << getImageAspectFlagsStr(aspect)
                            << ", Requirements: " << requirements << TestLog::EndMessage;

                        result.check(deIsPowerOfTwo64(static_cast<uint64_t>(requirements.memoryRequirements.alignment)),
                                     "VkMemoryRequirements alignment isn't power of two");
                            ...
                            for (const VkSparseImageMemoryRequirements &sr : sparseRequirements)
                            {
                                if (sr.formatProperties.aspectMask == aspect)
                                {
                                    result.check((sr.imageMipTailSize % requirements.memoryRequirements.alignment) == 0,
                                                 "VkSparseImageMemoryRequirements::imageMipTailSize is not aligned "
                                                 "with sparse block size");
                                    break;
                                }
                            }
                        }
                        ...
                        if (result.check(requirements.memoryRequirements.memoryTypeBits != 0,
                                         "No supported memory types"))
                        {
                            typedef std::vector<uint32_t>::const_iterator IndexIterator;
                            const std::vector<uint32_t> usedMemoryTypeIndices =
                                bitsToIndices(requirements.memoryRequirements.memoryTypeBits);
                            bool hasHostVisibleType = false;

                            for (IndexIterator memoryTypeNdx = usedMemoryTypeIndices.begin();
                                 memoryTypeNdx != usedMemoryTypeIndices.end(); ++memoryTypeNdx)
                            {
                                if (result.check(*memoryTypeNdx < memoryProperties.memoryTypeCount,
                                                 "Unknown memory type bits set in memory requirements"))
                                {
                                    const VkMemoryPropertyFlags propertyFlags(
                                        memoryProperties.memoryTypes[*memoryTypeNdx].propertyFlags);
                                    ...
                                    if (propertyFlags & VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT)
                                    {
                                        result.check((imageInfo.usage & VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT) != 0u,
                                                     "Memory type includes VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT for "
                                                     "a non-transient attachment image");
                                    }
                                }
                                else
                                    break;
                            ...
                            result.check(params.tiling != VK_IMAGE_TILING_LINEAR ||
                                             (hasHostVisibleType || (loopCreateFlags & VK_IMAGE_CREATE_PROTECTED_BIT)),
...
tcu::TestStatus testVkMemoryPropertyFlags(Context &context)
{
    ...
    for (uint32_t ndx = 0; ndx < memoryProperties.memoryTypeCount; ndx++)
    {
        ...
        {
            if (memoryProperties.memoryTypes[ndx].propertyFlags == flag)
                matchingTypes++;
        }
    }
    ...
    result.check(matchingTypes == memoryProperties.memoryTypeCount,
                 "Unknown memory type bits set in memory requirements: " + diffStr + " mismatch");
...
void populateCreateInfoTestGroup(tcu::TestCaseGroup *group)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> multiplaneGroup(
        new tcu::TestCaseGroup(group->getTestContext(), "multiplane_image", ""));
...
{
    de::MovePtr<tcu::TestCaseGroup> requirementsGroup(
        new tcu::TestCaseGroup(testCtx, "requirements", "Buffer and image memory requirements"));

    requirementsGroup->addChild(
        createTestGroup(testCtx, "core", "Memory requirements tests with core functionality", populateCoreTestGroup));
    requirementsGroup->addChild(
        createTestGroup(testCtx, "extended", "Memory requirements tests with extension VK_KHR_get_memory_requirements2",
                        populateExtendedTestGroup));
    requirementsGroup->addChild(createTestGroup(testCtx, "dedicated_allocation",
                                                "Memory requirements tests with extension VK_KHR_dedicated_allocation",
                                                populateDedicatedAllocationTestGroup));
    requirementsGroup->addChild(createTestGroup(testCtx, "multiplane_image",
                                                "Memory requirements tests with vkGetImagePlaneMemoryRequirements",
                                                populateMultiplaneTestGroup));
    requirementsGroup->addChild(createTestGroup(testCtx, "memory_property_flags",
                                                "Memory requirements tests with vkGetPhysicalDeviceMemoryProperties",
                                                populateMemoryPropertyFlagsTestGroup));
#ifndef CTS_USES_VULKANSC
    requirementsGroup->addChild(createTestGroup(testCtx, "create_info",
                                                "Memory requirements tests with extension VK_KHR_maintenance4",
                                                populateCreateInfoTestGroup));
#endif // CTS_USES_VULKANSC
