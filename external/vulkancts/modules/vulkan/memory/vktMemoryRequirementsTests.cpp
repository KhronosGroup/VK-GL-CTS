    ...
    const VkPhysicalDeviceLimits &limits, const VkBufferCreateFlags bufferFlags, const VkBufferUsageFlags usage)
{
    ...
    result.check(m_currentTestPrefersDedicatedAllocation == VK_FALSE ||
                     m_currentTestPrefersDedicatedAllocation == VK_FALSE,
                 "Preferred and required flags for dedicated memory cannot be set to true at the same time");
}

struct ImageTestParams
{
    ImageTestParams(VkImageCreateFlags flags_, VkImageTiling tiling_, bool transient_)
        : flags(flags_)
        , tiling(tiling_)
...
{
    const VkFormat formats[]          = {VK_FORMAT_R4G4_UNORM_PACK8,
