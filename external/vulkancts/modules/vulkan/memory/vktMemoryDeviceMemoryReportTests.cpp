...
tcu::TestStatus vkDeviceMemoryAllocationFailedTest(Context &context)
{
...

    {
        ...
        VkResult result =
            vkd.allocateMemory(*device, &memoryAllocateInfo, (const VkAllocationCallbacks *)DE_NULL, &memory1);
        if (result != VK_SUCCESS)
        ...
        if (result == VK_SUCCESS)
        {
            ...
            return tcu::TestStatus::fail(std::string("Should not be able to allocate ") + std::to_string(testSize) +
                                         " bytes of memory");
        }
    ...
    bool allocationFailedEvent = false;
...
{
    MovePtr<tcu::TestCaseGroup> deviceMemoryReportTests(
        new tcu::TestCaseGroup(testCtx, "device_memory_report", "Device Memory Report tests"));
    ...
    deviceMemoryReportTests->addChild(createObjectTestsGroup(testCtx, "create_and_destroy_object",
                                                             "Check emitted callbacks are properly paired",
                                                             s_createDestroyObjectGroup));
    deviceMemoryReportTests->addChild(createVkDeviceMemoryTestsGroup(
        testCtx, "vk_device_memory", "Check callbacks are emitted properly for VkDeviceMemory"));
    deviceMemoryReportTests->addChild(createExternalMemoryTestsGroup(
        testCtx, "external_memory", "Check callbacks are emitted properly for external memory"));
