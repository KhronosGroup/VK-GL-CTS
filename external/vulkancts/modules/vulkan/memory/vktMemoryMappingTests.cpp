...
Move<VkImage> makeImage(const DeviceInterface &vk, VkDevice device, VkDeviceSize size, uint32_t queueFamilyIndex)
{
    ...
    const uint32_t powerOfTwoSize   = deSmallestGreaterOrEquallPowerOfTwoU32(sqrtSize);
...
bool compareAndLogBuffer(TestLog &log, size_t size, const uint8_t *result, const uint8_t *reference)
{
        ...
        try
        {
            ...
            else
            {
                ...
                const Unique<VkDeviceMemory> memory(
                    allocMemory(vkd, device, allocationSize, memoryTypeIndex, image, buffer));
                de::Random rng(config.seed);
                vector<uint8_t> reference((size_t)(allocationSize));
                ...
                if (!compareAndLogBuffer(log, static_cast<size_t>(mappingSize), mapping,
...
public:
    MemoryHeap(const VkMemoryHeap &heap, const vector<MemoryType> &memoryTypes,
               const PlatformMemoryLimits &memoryLimits, const VkDeviceSize nonCoherentAtomSize,
        ...
        for (size_t memoryTypeNdx = 0; memoryTypeNdx < m_memoryTypes.size(); memoryTypeNdx++)
        {
            ...
            if (isUMA)
            {
                ...
                const VkDeviceSize maxAllocationSize =
                    roundDownToMultiple(((atomSize * availableBits) / (17 * atomSize + 1)), allocationSizeGranularity);
            ...
            else
            {
                ...
                const VkDeviceSize maxAllocationSize =
                    roundDownToMultiple(de::min(totalMemClass - usedMemClass,
...
{
public:
    RandomMemoryMappingInstance(Context &context, uint32_t seed)
...
tcu::TestCaseGroup *createMappingTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> dedicated(
        new tcu::TestCaseGroup(testCtx, "dedicated_alloc", "Dedicated memory mapping tests."));
    de::MovePtr<tcu::TestCaseGroup> sets[] = {de::MovePtr<tcu::TestCaseGroup>(new tcu::TestCaseGroup(
                                                  testCtx, "suballocation", "Suballocated memory mapping tests.")),
                                              de::MovePtr<tcu::TestCaseGroup>(new tcu::TestCaseGroup(
                                                  testCtx, "buffer", "Buffer dedicated memory mapping tests.")),
                                              de::MovePtr<tcu::TestCaseGroup>(new tcu::TestCaseGroup(
                                                  testCtx, "image", "Image dedicated memory mapping tests."))};
    ...
    for (size_t allocationKindNdx = 0; allocationKindNdx < ALLOCATION_KIND_LAST; allocationKindNdx++)
    {
        ...
        for (size_t allocationSizeNdx = 0; allocationSizeNdx < DE_LENGTH_OF_ARRAY(allocationSizes); allocationSizeNdx++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> allocationSizeGroup(
                new tcu::TestCaseGroup(testCtx, de::toString(allocationSize).c_str(), ""));
            ...
            for (size_t opNdx = 0; opNdx < DE_LENGTH_OF_ARRAY(ops); opNdx++)
            {
                ...
                const TestConfig config =
                    fullMappedConfig(allocationSize, op, seed, static_cast<AllocationKind>(allocationKindNdx));
                ...
                for (size_t sizeNdx = 0; sizeNdx < DE_LENGTH_OF_ARRAY(sizes); sizeNdx++)
                {
                    ...
                    for (size_t opNdx = 0; opNdx < DE_LENGTH_OF_ARRAY(ops); opNdx++)
                    {
                        ...
                        const TestConfig config = subMappedConfig(allocationSize, MemoryRange(offset, size), op, seed,
                                                                  static_cast<AllocationKind>(allocationKindNdx));
    ...
    // .random
    {
        de::MovePtr<tcu::TestCaseGroup> randomGroup(
            new tcu::TestCaseGroup(testCtx, "random", "Random memory mapping tests."));
        ...
        for (size_t ndx = 0; ndx < 100; ndx++)
        {
            ...
            randomGroup->addChild(new InstanceFactory1<RandomMemoryMappingInstance, uint32_t>(
                testCtx, tcu::NODETYPE_SELF_VALIDATE, de::toString(ndx), "Random case", seed));
        }
