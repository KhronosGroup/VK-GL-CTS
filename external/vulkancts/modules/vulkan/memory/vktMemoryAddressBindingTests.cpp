...
public:
    ...
    {
        const VkDeviceAddressBindingCallbackDataEXT *bindingCallbackData =
            static_cast<const VkDeviceAddressBindingCallbackDataEXT *>(pCallbackData->pNext);

        const BindingData bindingData = {bindingCallbackData->baseAddress, bindingCallbackData->size,
                                         bindingCallbackData->bindingType, pCallbackData->pObjects[0].objectHandle};
...
struct Environment
{
    ...
    Environment(const PlatformInterface &vkp_, const InstanceInterface &vki_, VkInstance instance_,
                ...
                uint32_t queueFamilyIndex_, const BinaryCollection &programBinaries_,
...
tcu::TestCaseGroup *createObjectTestsGroup(tcu::TestContext &testCtx, const char *name, const char *desc,
...
static bool validateCallbackRecords(Context &context, const BindingCallbackRecorder &recorder)
{
    ...
    for (auto bindRecord = recorder.getRecordsBegin(); bindRecord != recorder.getRecordsEnd(); bindRecord++)
    {
        if (bindRecord->bindingType == VK_DEVICE_ADDRESS_BINDING_TYPE_BIND_EXT)
        {
            ...
            for (auto pairRecord = bindRecord; pairRecord != recorder.getRecordsEnd(); pairRecord++)
            {
                if (pairRecord->bindingType == VK_DEVICE_ADDRESS_BINDING_TYPE_UNBIND_EXT)
                {
                    ...
                    {
                        log << tcu::TestLog::Message << "Bind/Unbind base adress:" << bindRecord->bindingAddress
                            << tcu::TestLog::EndMessage;
                        log << tcu::TestLog::Message << "Bind/Unbind size:" << bindRecord->size
                            << tcu::TestLog::EndMessage;
                        log << tcu::TestLog::Message << "Bind/Unbind object handle:" << bindRecord->objectHandle
                            << tcu::TestLog::EndMessage;
            ...
            {
                log << tcu::TestLog::Message << "Lonely bind base adress:" << bindRecord->bindingAddress
                    << tcu::TestLog::EndMessage;
                log << tcu::TestLog::Message << "Lonely bind size:" << bindRecord->size << tcu::TestLog::EndMessage;
                log << tcu::TestLog::Message << "Lonely bind object handle:" << bindRecord->objectHandle
                    << tcu::TestLog::EndMessage;
        ...
        else if (bindRecord->bindingType == VK_DEVICE_ADDRESS_BINDING_TYPE_UNBIND_EXT)
        {
            ...
            if (matchedBoundUnbound == false)
            {
                log << tcu::TestLog::Message << "Lonely unbind base adress:" << bindRecord->bindingAddress
                    << tcu::TestLog::EndMessage;
                log << tcu::TestLog::Message << "Lonely unbind size:" << bindRecord->size << tcu::TestLog::EndMessage;
                log << tcu::TestLog::Message << "Lonely unbind object handle:" << bindRecord->objectHandle
                    << tcu::TestLog::EndMessage;
...
tcu::TestStatus createDestroyObjectTest(Context &context, typename Object::Parameters params)
{
    ...
    customInstance.getDriver().createDebugUtilsMessengerEXT(customInstance, &debugUtilsMessengerCreateInfo, nullptr,
                                                            &messenger);

    {
        ...
        de::MovePtr<DeviceDriver> deviceInterface =
            de::MovePtr<DeviceDriver>(new DeviceDriver(context.getPlatformInterface(), customInstance, device.get()));
        ...
        const Environment env(context.getPlatformInterface(), customInstance.getDriver(), customInstance,
                              ...
                              context.getBinaryCollection(), context.getTestContext().getCommandLine(), &recorder);
    ...
    customInstance.getDriver().destroyDebugUtilsMessengerEXT(customInstance, messenger, nullptr);
...
{
    MovePtr<tcu::TestCaseGroup> addressBindingReportTests(
        new tcu::TestCaseGroup(testCtx, "address_binding_report", "Address Binding Report tests"));
    ...
    addressBindingReportTests->addChild(createObjectTestsGroup(testCtx, "create_and_destroy_object",
                                                               "Check emitted callbacks are properly paired",
                                                               s_createDestroyObjectGroup));
