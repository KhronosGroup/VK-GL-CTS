...
void addCasesWithProgs(const MovePtr<tcu::TestCaseGroup> &group, const CaseDescription<Object> &cases)
{
    for (const NamedParameters<Object> *cur = cases.paramsBegin; cur != cases.paramsEnd; cur++)
    {
        addFunctionCaseWithPrograms(group.get(), cur->name, "", checkSupport<Object>, Object::initPrograms,
...
static bool validateCallbackRecords(Context &context, const BindingCallbackRecorder &recorder)
{
    ...
    {
        const VkDeviceAddressBindingCallbackDataEXT &bindingData =
            reinterpret_cast<const VkDeviceAddressBindingCallbackDataEXT &>(record->pNext);
    ...
    for (auto record = recorder.getRecordsBegin(); record != recorder.getRecordsEnd(); record++)
    {
        ...
        if (bindingData.bindingType == VK_DEVICE_ADDRESS_BINDING_TYPE_UNBIND_EXT)
        {
            ...
            {
                log << tcu::TestLog::Message << "Bind base adress:" << baseAddress
                    << " differ from unbound one: " << bindingData.baseAddress << tcu::TestLog::EndMessage;
                matchedBoundUnbound = false;
                return false;
            }
            if (bindingData.size != memorySize)
            {
                log << tcu::TestLog::Message << "Bind memory size:" << memorySize
                    << " differ from unbound one: " << bindingData.size << tcu::TestLog::EndMessage;
                matchedBoundUnbound = false;
            }
        }
    }
...
struct EnvClone
{
    ...
    EnvClone(const Environment &parent)
        ...
        , env(parent.vkp, parent.vki, parent.instance, parent.physicalDevice, vkd, *device, parent.queueFamilyIndex,
              parent.programBinaries, parent.commandLine, nullptr)
    {
    }
};
...
tcu::TestStatus createDestroyObjectTest(Context &context, typename Object::Parameters params)
{
    ...
    const Environment env(context.getPlatformInterface(), context.getInstanceInterface(), customInstance,
