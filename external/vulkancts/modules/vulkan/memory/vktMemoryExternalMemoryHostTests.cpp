...
tcu::TestCaseGroup *createMemoryExternalMemoryHostTests(tcu::TestContext &testCtx)
{
    ...
    simpleAllocation->addChild(
        new InstanceFactory1WithSupport<ExternalMemoryHostBaseTestInstance, VkDeviceSize, FunctionSupport0>(
            testCtx, tcu::NODETYPE_SELF_VALIDATE, "minImportedHostPointerAlignment_x1", 1, checkSupport));
    simpleAllocation->addChild(
        new InstanceFactory1WithSupport<ExternalMemoryHostBaseTestInstance, VkDeviceSize, FunctionSupport0>(
            testCtx, tcu::NODETYPE_SELF_VALIDATE, "minImportedHostPointerAlignment_x3", 3, checkSupport));
    ...
    for (const auto &formatName : testFormats)
    {
        with_zero_offset->addChild(
            new InstanceFactory1WithSupport<ExternalMemoryHostRenderImageTestInstance, TestParams, FunctionSupport0,
                                            AddPrograms>(testCtx, tcu::NODETYPE_SELF_VALIDATE, formatName.name,
                                                         AddPrograms(), TestParams(formatName.format), checkSupport));
    }
    ...
    for (const auto &formatName : testFormats)
    {
        with_non_zero_offset->addChild(new InstanceFactory1WithSupport<ExternalMemoryHostRenderImageTestInstance,
            ...
            testCtx, tcu::NODETYPE_SELF_VALIDATE, formatName.name, AddPrograms(), TestParams(formatName.format, true),
            checkSupport));
    }
    ...
    synchronization->addChild(new InstanceFactory1WithSupport<ExternalMemoryHostSynchronizationTestInstance, TestParams,
        ...
        testCtx, tcu::NODETYPE_SELF_VALIDATE, "synchronization", AddPrograms(), TestParams(testFormats[0].format, true),
        checkTimelineSemaphore));
