...
protected:
    ...
    void createBuffer(Move<VkBuffer> &buffer, Move<VkDeviceMemory> &memory);

    void pushData(VkDeviceMemory memory, uint32_t dataSeed);

    bool checkData(VkDeviceMemory memory, uint32_t dataSeed);
...
void BaseTestInstance::createBuffer(Move<VkBuffer> &buffer, Move<VkDeviceMemory> &memory)
{
    const DeviceInterface &vk       = getDeviceInterface();
    const VkDevice vkDevice         = getDevice();
    VkBufferCreateInfo bufferParams = makeBufferCreateInfo(m_context, m_params);
...
void BaseTestInstance::pushData(VkDeviceMemory memory, uint32_t dataSeed)
{
    const DeviceInterface &vk = getDeviceInterface();
    const VkDevice vkDevice   = getDevice();
...
bool BaseTestInstance::checkData(VkDeviceMemory memory, uint32_t dataSeed)
{
    const DeviceInterface &vk = getDeviceInterface();
    const VkDevice vkDevice   = getDevice();
    MemoryMappingRAII hostMemory(vk, vkDevice, memory, 0u, m_params.bufferSize, 0u);
        ...
        for (uint32_t sizeNdx = 0u; sizeNdx < DE_LENGTH_OF_ARRAY(allocationSizes); ++sizeNdx)
        {
            ...
            const BindingCaseParameters params =
                makeBindingCaseParameters(10, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                          VK_SHARING_MODE_EXCLUSIVE, bufferSize, 0u, priorityMode);
            const BindingCaseParameters aliasparams = makeBindingCaseParameters(
                ...
                bufferSize, VK_IMAGE_CREATE_ALIAS_BIT, priorityMode);
        ...
        for (uint32_t widthNdx = 0u; widthNdx < DE_LENGTH_OF_ARRAY(imageSizes); ++widthNdx)
            for (uint32_t heightNdx = 0u; heightNdx < DE_LENGTH_OF_ARRAY(imageSizes); ++heightNdx)
            {
                ...
                const BindingCaseParameters regularparams =
                    makeBindingCaseParameters(10, width, height, 0u, priorityMode);
                const BindingCaseParameters aliasparams =
                    makeBindingCaseParameters(10, width, height, VK_IMAGE_CREATE_ALIAS_BIT, priorityMode);
                std::ostringstream testName;
