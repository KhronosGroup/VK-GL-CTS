...
BindingCaseParameters makeBindingCaseParameters(uint32_t targetsCount, uint32_t width, uint32_t height,
                                                VkImageCreateFlags imageCreateFlags, bool usePriority)
{
...
const VkBindBufferMemoryInfo makeBufferMemoryBindingInfo(VkBuffer buffer, VkDeviceMemory memory)
{
...
const VkBindImageMemoryInfo makeImageMemoryBindingInfo(VkImage image, VkDeviceMemory memory)
{
...
void createBindingTargets(std::vector<de::SharedPtr<Move<TTarget>>> &targets, Context &ctx,
                          BindingCaseParameters params);

template <>
void createBindingTargets<VkBuffer>(BuffersList &targets, Context &ctx, BindingCaseParameters params)
{
    const uint32_t count      = params.targetsCount;
...
template <>
void createBindingTargets<VkImage>(ImagesList &targets, Context &ctx, BindingCaseParameters params)
{
    const uint32_t count      = params.targetsCount;
    const VkDevice vkDevice   = ctx.getDevice();
    const DeviceInterface &vk = ctx.getDeviceInterface();
...
void createMemory(std::vector<de::SharedPtr<Move<TTarget>>> &targets, MemoryRegionsList &memory, Context &ctx,
                  BindingCaseParameters params);

template <>
void createMemory<VkBuffer, false>(BuffersList &targets, MemoryRegionsList &memory, Context &ctx,
                                   BindingCaseParameters params)
{
    DE_UNREF(params);
...
template <>
void createMemory<VkImage, false>(ImagesList &targets, MemoryRegionsList &memory, Context &ctx,
                                  BindingCaseParameters params)
{
    DE_UNREF(params);
    const uint32_t count      = static_cast<uint32_t>(targets.size());
...
template <>
void createMemory<VkBuffer, true>(BuffersList &targets, MemoryRegionsList &memory, Context &ctx,
                                  BindingCaseParameters params)
{
    DE_UNREF(params);
    const uint32_t count      = static_cast<uint32_t>(targets.size());
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        const VkMemoryAllocateInfo memAlloc =
            makeMemoryAllocateInfo(memReqs, params.usePriority ? &priority : (const void *)&dedicatedAllocationInfo);
        VkDeviceMemory rawMemory = DE_NULL;
...
template <>
void createMemory<VkImage, true>(ImagesList &targets, MemoryRegionsList &memory, Context &ctx,
                                 BindingCaseParameters params)
{
    DE_UNREF(params);
    const uint32_t count      = static_cast<uint32_t>(targets.size());
...
template <typename TTarget>
void makeBinding(std::vector<de::SharedPtr<Move<TTarget>>> &targets, MemoryRegionsList &memory, Context &ctx,
                 BindingCaseParameters params);

template <>
void makeBinding<VkBuffer>(BuffersList &targets, MemoryRegionsList &memory, Context &ctx, BindingCaseParameters params)
{
    DE_UNREF(params);
    const uint32_t count      = static_cast<uint32_t>(targets.size());
    const VkDevice vkDevice   = ctx.getDevice();
...
template <>
void makeBinding<VkImage>(ImagesList &targets, MemoryRegionsList &memory, Context &ctx, BindingCaseParameters params)
{
    DE_UNREF(params);
    const uint32_t count      = static_cast<uint32_t>(targets.size());
    const VkDevice vkDevice   = ctx.getDevice();
...
template <typename TTarget>
void fillUpResource(Move<VkBuffer> &source, Move<TTarget> &target, Context &ctx, BindingCaseParameters params);

template <>
void fillUpResource<VkBuffer>(Move<VkBuffer> &source, Move<VkBuffer> &target, Context &ctx,
                              BindingCaseParameters params)
{
    const DeviceInterface &vk = ctx.getDeviceInterface();
    const VkDevice vkDevice   = ctx.getDevice();
    ...
    const VkBufferMemoryBarrier srcBufferBarrier =
        makeMemoryBarrierInfo(*source, params.bufferSize, TransferFromResource);
    const VkBufferMemoryBarrier dstBufferBarrier =
        makeMemoryBarrierInfo(*target, params.bufferSize, TransferToResource);
...
template <>
void fillUpResource<VkImage>(Move<VkBuffer> &source, Move<VkImage> &target, Context &ctx, BindingCaseParameters params)
{
    const DeviceInterface &vk = ctx.getDeviceInterface();
    const VkDevice vkDevice   = ctx.getDevice();
    const VkQueue queue       = ctx.getUniversalQueue();
    ...
    const VkBufferImageCopy copyRegion = {
        ...
        params.imageSize.width,  // uint32_t bufferRowLength;
        params.imageSize.height, // uint32_t bufferImageHeight;
        {
...
template <typename TTarget>
void readUpResource(Move<TTarget> &source, Move<VkBuffer> &target, Context &ctx, BindingCaseParameters params);

template <>
void readUpResource(Move<VkBuffer> &source, Move<VkBuffer> &target, Context &ctx, BindingCaseParameters params)
{
    fillUpResource(source, target, ctx, params);
}

template <>
void readUpResource(Move<VkImage> &source, Move<VkBuffer> &target, Context &ctx, BindingCaseParameters params)
{
    const DeviceInterface &vk = ctx.getDeviceInterface();
    const VkDevice vkDevice   = ctx.getDevice();
    const VkQueue queue       = ctx.getUniversalQueue();
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    copyImageToBuffer(vk, *cmdBuffer, *source, *target, tcu::IVec2(params.imageSize.width, params.imageSize.height),
...
template <typename TTarget>
void layoutTransitionResource(Move<TTarget> &target, Context &ctx);

template <>
void layoutTransitionResource(Move<VkBuffer> &target, Context &ctx)
{
    DE_UNREF(target);
    DE_UNREF(ctx);
}
...
template <>
void layoutTransitionResource<VkImage>(Move<VkImage> &target, Context &ctx)
{
    const DeviceInterface &vk = ctx.getDeviceInterface();
    const VkDevice vkDevice   = ctx.getDevice();
    const VkQueue queue       = ctx.getUniversalQueue();
...
void createBuffer(Move<VkBuffer> &buffer, Move<VkDeviceMemory> &memory, Context &ctx, BindingCaseParameters params)
{
    const DeviceInterface &vk       = ctx.getDeviceInterface();
    const VkDevice vkDevice         = ctx.getDevice();
    VkBufferCreateInfo bufferParams = makeBufferCreateInfo(ctx, params);
...
void pushData(VkDeviceMemory memory, uint32_t dataSeed, Context &ctx, BindingCaseParameters params)
{
    const DeviceInterface &vk = ctx.getDeviceInterface();
    const VkDevice vkDevice   = ctx.getDevice();
    ...
    for (uint32_t i = 0u; i < params.bufferSize; ++i)
    {
...
bool checkData(VkDeviceMemory memory, uint32_t dataSeed, Context &ctx, BindingCaseParameters params)
{
    const DeviceInterface &vk = ctx.getDeviceInterface();
    const VkDevice vkDevice   = ctx.getDevice();
    MemoryMappingRAII hostMemory(vk, vkDevice, memory, 0u, params.bufferSize, 0u);
...
{
public:
    MemoryBindingInstance(Context &ctx, BindingCaseParameters params) : TestInstance(ctx), m_params(params)
    {
    }
    ...
    virtual tcu::TestStatus iterate(void)
    {
        ...
        bool passed = true;
        for (uint32_t i = 0; passed && i < m_params.targetsCount; ++i)
        {
...
{
public:
    AliasedMemoryBindingInstance(Context &ctx, BindingCaseParameters params) : TestInstance(ctx), m_params(params)
    {
    }
...
{
public:
    MemoryBindingTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                      BindingCaseParameters params)
...
tcu::TestCaseGroup *createMemoryBindingTests(tcu::TestContext &testCtx)
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> regular(
            new tcu::TestCaseGroup(testCtx, "regular", "Basic memory binding tests."));
        de::MovePtr<tcu::TestCaseGroup> aliasing(
            new tcu::TestCaseGroup(testCtx, "aliasing", "Memory binding tests with aliasing of two resources."));

        de::MovePtr<tcu::TestCaseGroup> regular_suballocated(
            new tcu::TestCaseGroup(testCtx, "suballocated", "Basic memory binding tests with suballocated memory."));
        de::MovePtr<tcu::TestCaseGroup> regular_dedicated(new tcu::TestCaseGroup(
            testCtx, "dedicated", "Basic memory binding tests with deditatedly allocated memory."));

        de::MovePtr<tcu::TestCaseGroup> aliasing_suballocated(new tcu::TestCaseGroup(
            testCtx, "suballocated", "Memory binding tests with aliasing of two resources with suballocated mamory."));
        ...
        for (uint32_t sizeNdx = 0u; sizeNdx < DE_LENGTH_OF_ARRAY(allocationSizes); ++sizeNdx)
        {
            ...
            const BindingCaseParameters params =
                makeBindingCaseParameters(10, VK_BUFFER_USAGE_TRANSFER_SRC_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                          VK_SHARING_MODE_EXCLUSIVE, bufferSize, 0u, usePriority);
            const BindingCaseParameters aliasparams = makeBindingCaseParameters(
                ...
                bufferSize, VK_IMAGE_CREATE_ALIAS_BIT, usePriority);
        ...
        for (uint32_t widthNdx = 0u; widthNdx < DE_LENGTH_OF_ARRAY(imageSizes); ++widthNdx)
            for (uint32_t heightNdx = 0u; heightNdx < DE_LENGTH_OF_ARRAY(imageSizes); ++heightNdx)
            {
                ...
                const BindingCaseParameters regularparams =
                    makeBindingCaseParameters(10, width, height, 0u, usePriority);
                const BindingCaseParameters aliasparams =
                    makeBindingCaseParameters(10, width, height, VK_IMAGE_CREATE_ALIAS_BIT, usePriority);
                std::ostringstream testName;
        ...
        if (usePriority)
        {
            de::MovePtr<tcu::TestCaseGroup> priority(
                new tcu::TestCaseGroup(testCtx, "priority", "Using VK_EXT_memory_priority."));
