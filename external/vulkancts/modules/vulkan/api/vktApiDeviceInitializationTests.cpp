...
tcu::TestStatus createInstanceTest(Context &context)
{
    ...
    const uint32_t manjorNum = unpackVersion(apiVersion).majorNum;
    ...
    for (int patchVersion = 0; patchVersion < DE_LENGTH_OF_ARRAY(patchNumbers); patchVersion++)
    {
        const VkApplicationInfo appInfo = {
            ...
            "engineName",                                                     // const char* pEngineName;
            0u,                                                               // uint32_t engineVersion;
            VK_MAKE_VERSION(manjorNum, minorNum, patchNumbers[patchVersion]), // uint32_t apiVersion;
        };
...
tcu::TestStatus createInstanceWithInvalidApiVersionTest(Context &context)
{
    ...
    const uint32_t invalidMajorVersion = (1 << 10) - 1;
...

        {
            ...
            else if (apiVersion.majorNum == 1 && apiVersion.minorNum >= 1)
            {
                if (result == VK_SUCCESS)
                {
                    TCU_CHECK(static_cast<bool>(instance));
                    log << TestLog::Message
                        << "Pass, instance creation with nonstandard apiVersion succeeds for Vulkan 1.1"
                        << TestLog::EndMessage;
                }
                else if (result == VK_ERROR_INCOMPATIBLE_DRIVER)
                {
                    resultCollector.fail(
                        "Fail, In Vulkan 1.1 instance creation must not return VK_ERROR_INCOMPATIBLE_DRIVER.");
                }
                else
                {
...
#define FEATURE_ITEM(MEMBER)                                \
    {                                                       \
        #MEMBER, offsetof(VkPhysicalDeviceFeatures, MEMBER) \
    }
...
tcu::TestStatus createDeviceWithUnsupportedFeaturesTest(Context &context)
{
    ...
    static const Feature features[] = {FEATURE_ITEM(robustBufferAccess),
    ...
    for (int featureNdx = 0; featureNdx < numFeatures; featureNdx++)
    {
        ...
        if (*(((VkBool32 *)((uint8_t *)(&physicalDeviceFeatures) + features[featureNdx].offset))))
            continue;

        VkPhysicalDeviceFeatures enabledFeatures;

        for (int i = 0; i < numFeatures; i++)
            *((VkBool32 *)((uint8_t *)(&enabledFeatures) + features[i].offset)) =
                (i == featureNdx ? VK_TRUE : VK_FALSE);
    ...
    if (numErrors > 1)
        return tcu::TestStatus(resultCollector.getResult(),
                               "Enabling " + de::toString(numErrors) +
                                   " unsupported features didn't return VK_ERROR_FEATURE_NOT_PRESENT.");
    else
        return tcu::TestStatus(resultCollector.getResult(), resultCollector.getMessage());
}
...
tcu::TestStatus createDeviceQueue2UnmatchedFlagsTest(Context &context)
{
...

    {
        ...
        if (queue2 != DE_NULL)
            return tcu::TestStatus::fail("Fail, getDeviceQueue2 should return VK_NULL_HANDLE when flags in "
                                         "VkDeviceQueueCreateInfo and VkDeviceQueueInfo2 are different.");
...
{
    de::MovePtr<tcu::TestCaseGroup> deviceInitializationTests(
        new tcu::TestCaseGroup(testCtx, "device_init", "Device Initialization Tests"));

    addFunctionCase(deviceInitializationTests.get(), "create_instance_name_version", "", createInstanceTest);
    addFunctionCase(deviceInitializationTests.get(), "create_instance_invalid_api_version", "",
                    createInstanceWithInvalidApiVersionTest);
    addFunctionCase(deviceInitializationTests.get(), "create_instance_null_appinfo", "",
                    createInstanceWithNullApplicationInfoTest);
    addFunctionCase(deviceInitializationTests.get(), "create_instance_unsupported_extensions", "",
                    createInstanceWithUnsupportedExtensionsTest);
    addFunctionCase(deviceInitializationTests.get(), "create_instance_extension_name_abuse", "",
                    createInstanceWithExtensionNameAbuseTest);
    addFunctionCase(deviceInitializationTests.get(), "create_instance_layer_name_abuse", "",
    ...
    addFunctionCase(deviceInitializationTests.get(), "create_device_unsupported_extensions", "",
                    createDeviceWithUnsupportedExtensionsTest);
    addFunctionCase(deviceInitializationTests.get(), "create_device_various_queue_counts", "",
                    createDeviceWithVariousQueueCountsTest);
    addFunctionCase(deviceInitializationTests.get(), "create_device_global_priority", "", checkGlobalPrioritySupport,
                    createDeviceWithGlobalPriorityTest);
    addFunctionCase(deviceInitializationTests.get(), "create_device_global_priority_query", "",
                    checkGlobalPriorityQuerySupport, createDeviceWithQueriedGlobalPriorityTest);
    addFunctionCase(deviceInitializationTests.get(), "create_device_features2", "", createDeviceFeatures2Test);
    addFunctionCase(deviceInitializationTests.get(), "create_device_unsupported_features", "",
                    createDeviceWithUnsupportedFeaturesTest);
    addFunctionCase(deviceInitializationTests.get(), "create_device_queue2", "", createDeviceQueue2Test);
    addFunctionCase(deviceInitializationTests.get(), "create_device_queue2_unmatched_flags", "",
                    createDeviceQueue2UnmatchedFlagsTest);
    addFunctionCase(deviceInitializationTests.get(), "create_instance_device_intentional_alloc_fail", "",
