...
tcu::TestStatus validateGetter(Context &context)
{
    ...
    if (toolCount > 0)
    {
        ...
        result = context.getInstanceInterface().getPhysicalDeviceToolProperties(
...
tcu::TestStatus validateToolsProperties(Context &context)
{
    ...
    if (toolCount > 0)
    {
        ...
        VK_CHECK(context.getInstanceInterface().getPhysicalDeviceToolProperties(context.getPhysicalDevice(), &toolCount,
                                                                                &deviceToolPropertiesEXTArray[0]));
        ...
        for (uint32_t i = 0; i < toolCount; ++i)
        {
            ...
            result = result &&
                     (deviceToolPropertiesEXTArray[i].sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_TOOL_PROPERTIES_EXT);
            ...
            if (result == false)
            {
                ...
                testLog << tcu::TestLog::Message
                        << "Purposes: " << getToolPurposeFlagsStr(deviceToolPropertiesEXTArray[i].purposes)
                        << tcu::TestLog::EndMessage;
                if (layerSize > 0)
                {
