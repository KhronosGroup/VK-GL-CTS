...
{
    const VkToolPurposeFlagsEXT validPurposes =
        VK_TOOL_PURPOSE_VALIDATION_BIT_EXT | VK_TOOL_PURPOSE_PROFILING_BIT_EXT | VK_TOOL_PURPOSE_TRACING_BIT_EXT |
        VK_TOOL_PURPOSE_ADDITIONAL_FEATURES_BIT_EXT | VK_TOOL_PURPOSE_MODIFYING_FEATURES_BIT_EXT |
        VK_TOOL_PURPOSE_DEBUG_REPORTING_BIT_EXT | VK_TOOL_PURPOSE_DEBUG_MARKERS_BIT_EXT;
...
CustomInstance createCustomInstance(Context &context, bool allowLayers)
{
    ...
    Move<VkInstance> instance =
        vk::createDefaultInstance(vkp, apiVersion, enabledLayersStr, enabledExtensions, DE_NULL);
    return CustomInstance(context, instance, !enabledLayers.empty(),
                          context.getTestContext().getCommandLine().printValidationErrors());
}

bool checkToolsProperties(Context &context,
                          const std::vector<VkPhysicalDeviceToolPropertiesEXT> &deviceToolPropertiesEXTArray)
{
    ...
    for (size_t i = 0; i < deviceToolPropertiesEXTArray.size(); ++i)
    {
        ...
        result =
            result && (deviceToolPropertiesEXTArray[i].sType == VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_TOOL_PROPERTIES_EXT);
        ...
        if (result == false)
        {
            ...
            testLog << tcu::TestLog::Message << "Tool name: " << deviceToolPropertiesEXTArray[i].name
                    << tcu::TestLog::EndMessage;
            testLog << tcu::TestLog::Message << "Version: " << deviceToolPropertiesEXTArray[i].version
                    << tcu::TestLog::EndMessage;
            testLog << tcu::TestLog::Message << "Description: " << deviceToolPropertiesEXTArray[i].description
                    << tcu::TestLog::EndMessage;
            testLog << tcu::TestLog::Message
                    << "Purposes: " << getToolPurposeFlagsEXTStr(deviceToolPropertiesEXTArray[i].purposes)
                    << tcu::TestLog::EndMessage;
            if (layerSize > 0)
            {
                testLog << tcu::TestLog::Message << "Corresponding Layer: " << deviceToolPropertiesEXTArray[i].layer
                        << tcu::TestLog::EndMessage;
            }
...
tcu::TestStatus validateGetter(Context &context)
{
    ...
    result = context.getInstanceInterface().getPhysicalDeviceToolPropertiesEXT(context.getPhysicalDevice(), &toolCount,
                                                                               DE_NULL);

    if (result != VK_SUCCESS)
    {
        testLog << tcu::TestLog::Message << "getPhysicalDeviceToolPropertiesEXT wrong result code"
                << tcu::TestLog::EndMessage;
    ...
    if (toolCount > 0)
    {
        uint32_t toolCountSecondCall = toolCount;

        std::vector<VkPhysicalDeviceToolPropertiesEXT> deviceToolPropertiesEXTArray(toolCountSecondCall);

        result = context.getInstanceInterface().getPhysicalDeviceToolPropertiesEXT(
            context.getPhysicalDevice(), &toolCountSecondCall, &deviceToolPropertiesEXTArray[0]);
        ...
        {
            testLog << tcu::TestLog::Message << "Got different tools count on the second call"
                    << tcu::TestLog::EndMessage;
            return tcu::TestStatus::fail("Fail");
        }
    ...
    if (toolCount > 1)
    {
        uint32_t toolCountSecondCall = toolCount / 2;
...
tcu::TestStatus validateToolsProperties(Context &context)
{
    ...
    VK_CHECK(context.getInstanceInterface().getPhysicalDeviceToolPropertiesEXT(context.getPhysicalDevice(), &toolCount,
                                                                               DE_NULL));
    ...
    if (toolCount > 0)
    {
        ...
        VK_CHECK(context.getInstanceInterface().getPhysicalDeviceToolPropertiesEXT(
            context.getPhysicalDevice(), &toolCount, &deviceToolPropertiesEXTArray[0]));
...
tcu::TestStatus validateInstanceLayers(Context &context)
{
...

    {
        ...
        if (toolCount > 0)
        {
            ...
            VK_CHECK(context.getInstanceInterface().getPhysicalDeviceToolPropertiesEXT(
                physicalDevice, &toolCount, &deviceToolPropertiesEXTArray[0]));
            ...
            for (size_t layerNdx = 0; layerNdx < layers.size(); ++layerNdx)
            {
                ...
                for (uint32_t toolNdx = 0; toolNdx < toolCount; ++toolNdx)
                {
...
{
    addFunctionCase(group, "validate_getter", "Validate getPhysicalDeviceToolPropertiesEXT", checkSupport,
                    validateGetter);
    addFunctionCase(group, "validate_tools_properties", "Validate tools properties", checkSupport,
                    validateToolsProperties);
    addFunctionCase(group, "validate_instance_layers", "Validate instance layers", checkSupport,
