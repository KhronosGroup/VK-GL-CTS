    ...
    , m_params(testParams)
{
    ...
    testParams.bufferAllocator->createTestBuffer(m_params.dstSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, context, memAlloc,
                                                 m_destination, MemoryRequirement::HostVisible,
...
tcu::TestStatus FillWholeBufferTestInstance::iterate(void)
{
    ...
    vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 0, DE_NULL, 1,
                          &gpuToHostBarrier, 0, DE_NULL);
...
{
public:
    FillWholeBufferTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
{
public:
    FillBufferTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
{
public:
    UpdateBufferTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
tcu::TestCaseGroup *createFillAndUpdateBufferTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> fillAndUpdateBufferTests(
        new tcu::TestCaseGroup(testCtx, "fill_and_update_buffer", "Fill and Update Buffer Tests"));
    tcu::TestCaseGroup *bufferViewAllocationGroupTests[] = {
        ...
        new tcu::TestCaseGroup(testCtx, "dedicated_alloc",
    ...
    for (uint32_t buffersAllocationNdx = 0u; buffersAllocationNdx < DE_LENGTH_OF_ARRAY(bufferAllocators);
         ++buffersAllocationNdx)
    {
        ...
        const uint32_t testCaseGroupNdx       = buffersAllocationNdx;
...

        {
            ...
            currentTestsGroup->addChild(
                new FillBufferTestCase(testCtx, "fill_" + testName, "Fill " + description, params));
            currentTestsGroup->addChild(
                new UpdateBufferTestCase(testCtx, "update_" + testName, "Update " + description, params));
        }
        ...
        // VK_WHOLE_SIZE tests.
        {
            for (VkDeviceSize i = 0; i < sizeof(uint32_t); ++i)
            {
                for (VkDeviceSize j = 0; j < sizeof(uint32_t); ++j)
                {
                    ...
                    const std::string description = "vkCmdFillBuffer with VK_WHOLE_SIZE, " + de::toString(extraBytes) +
                                                    " extra bytes and offset " + de::toString(params.dstOffset);
