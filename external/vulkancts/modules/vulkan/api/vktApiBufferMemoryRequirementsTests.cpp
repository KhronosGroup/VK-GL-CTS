...
void MemoryRequirementsTest::checkSupport(Context &context) const
{
    ...
    const std::vector<VkExtensionProperties> supportedExtensions =
        enumerateDeviceExtensionProperties(intf, physDevice, nullptr);
...
template <>
void *BufferMemoryRequirementsInstance::chainVkStructure<VkVideoProfilesKHR>(void *pNext) const
{
...
tcu::TestCaseGroup *createBufferMemoryRequirementsTests(tcu::TestContext &testCtx)
{
    ...
    auto groupRoot =
        new TestCaseGroup(testCtx, "buffer_memory_requirements", "vkGetBufferMemoryRequirements*(...) routines tests.");
