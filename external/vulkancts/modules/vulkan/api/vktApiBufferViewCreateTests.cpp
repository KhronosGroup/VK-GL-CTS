                                                      ...
                                                      Move<VkBuffer> &testBuffer, Move<VkDeviceMemory> &memory) const
{
    ...
    if (size > memReqs.size)
    {
        ...
        errorMsg << "Requied memory size (" << memReqs.size << " bytes) smaller than the buffer's size (" << size
                 << " bytes)!";
...
tcu::TestCaseGroup *createBufferViewCreateTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> bufferViewTests(
        new tcu::TestCaseGroup(testCtx, "create", "BufferView Construction Tests"));
    ...
    de::MovePtr<tcu::TestCaseGroup> bufferViewAllocationGroupTests[ALLOCATION_KIND_LAST] = {
        de::MovePtr<tcu::TestCaseGroup>(
            new tcu::TestCaseGroup(testCtx, "suballocation", "BufferView Construction Tests for Suballocated Buffer")),
        de::MovePtr<tcu::TestCaseGroup>(new tcu::TestCaseGroup(
            testCtx, "dedicated_alloc", "BufferView Construction Tests for Dedicatedly Allocated Buffer"))};
        ...
        for (uint32_t usageNdx = 0; usageNdx < DE_LENGTH_OF_ARRAY(usage); ++usageNdx)
        {
            ...
            for (uint32_t format = vk::VK_FORMAT_UNDEFINED + 1; format < VK_CORE_FORMAT_LAST; format++)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> formatGroup(new tcu::TestCaseGroup(
                    testCtx, "suballocation", "BufferView Construction Tests for Suballocated Buffer"));
...

                {
                    ...
                    usageGroup->addChild(
                        new BufferViewTestCase(testCtx, testName.c_str(), testDescription.c_str(), testParams));
                }
            }
