...
tcu::TestStatus renderTriangleUnusedExtStructTest(Context &context)
{
    ...
    const struct VkShaderModuleCreateInfo vertModuleInfo = {
        VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, &unusedExtStruct, 0, (uintptr_t)vertBin.getSize(),
        (const uint32_t *)vertBin.getBinary(),
    };

    const struct VkShaderModuleCreateInfo fragModuleInfo = {
        VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, &unusedExtStruct, 0, (uintptr_t)fragBin.getSize(),
        (const uint32_t *)fragBin.getBinary(),
    };
    ...
    vk.cmdPipelineBarrier(*cmdBuf, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, (VkDependencyFlags)0,
                          1, &vertFlushBarrier, 0, (const VkBufferMemoryBarrier *)DE_NULL, 1, &colorAttBarrier);
    ...
    vk.cmdPipelineBarrier(*cmdBuf, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, DE_NULL,
    ...
    vk.cmdPipelineBarrier(*cmdBuf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 0u, DE_NULL, 1u,
                          &bufferBarrier, 0u, DE_NULL);

    endCommandBuffer(vk, *cmdBuf);

    // Upload vertex data
    const VkMappedMemoryRange flushRange = {VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, &unusedExtStruct,
                                            vertexBufferMemory->getMemory(), vertexBufferMemory->getOffset(),
                                            VK_WHOLE_SIZE};
    deMemcpy(vertexBufferMemory->getHostPtr(), &vertices[0], sizeof(vertices));
    VK_CHECK(vk.flushMappedMemoryRanges(vkDevice, 1u, &flushRange));

    // Submit & wait for completion
    const VkFenceCreateInfo createInfo = {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, &unusedExtStruct, 0u};
...
tcu::TestCaseGroup *createSmokeTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(smokeTests.get(), "triangle_ext_structs", "", createTriangleProgs,
                                renderTriangleUnusedExtStructTest);
    addFunctionCaseWithPrograms(smokeTests.get(), "asm_triangle", "", createTriangleAsmProgs, renderTriangleTest);
    addFunctionCaseWithPrograms(smokeTests.get(), "asm_triangle_no_opname", "", createProgsNoOpName,
                                renderTriangleTest);
