...
tcu::TestStatus renderTriangleDummyExtStructTest(Context &context)
{
    ...
    const struct VkShaderModuleCreateInfo vertModuleInfo = {
        VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, &dummyExtStruct, 0, (uintptr_t)vertBin.getSize(),
        (const uint32_t *)vertBin.getBinary(),
    };

    const struct VkShaderModuleCreateInfo fragModuleInfo = {
        VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, &dummyExtStruct, 0, (uintptr_t)fragBin.getSize(),
    ...
    // Upload vertex data
    const VkMappedMemoryRange flushRange = {VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE, &dummyExtStruct,
    ...
    // Submit & wait for completion
    const VkFenceCreateInfo createInfo = {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, &dummyExtStruct, 0u};
...
tcu::TestCaseGroup *createSmokeTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(smokeTests.get(), "triangle_ext_structs", "", createTriangleProgs,
                                renderTriangleDummyExtStructTest);
