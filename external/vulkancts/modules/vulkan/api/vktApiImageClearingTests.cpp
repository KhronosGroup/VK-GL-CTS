...
inline bool isInInitialClearRange(bool isAttachmentformat, uint32_t mipLevel, uint32_t arrayLayer,
...
bool comparePixelToDepthClearValue(const ConstPixelBufferAccess &access, int x, int y, float ref,
...
// This method is copied from the vktRenderPassTests.cpp and extended with the stringResult parameter.
bool comparePixelToStencilClearValue(const ConstPixelBufferAccess &access, int x, int y, uint32_t ref,
                                     std::string &stringResult)
{
    const uint32_t stencil = access.getPixStencil(x, y);
...
bool comparePixelToColorClearValue(const ConstPixelBufferAccess &access, int x, int y, int z,
                                   const VkClearColorValue &ref, std::string &stringResult)
{
    ...
    case TEXTURECHANNELCLASS_SIGNED_FIXED_POINT:
    {
        ...
        const bool result =
            !(anyNotEqual(logicalAnd(lessThanEqual(absDiff(resColor, refColor), threshold), channelMask), channelMask));
    ...
    case TEXTURECHANNELCLASS_UNSIGNED_INTEGER:
    {
        ...
        const UVec4 refColor(ref.uint32[0], ref.uint32[1], ref.uint32[2], ref.uint32[3]);
    ...
    case TEXTURECHANNELCLASS_SIGNED_INTEGER:
    {
        ...
        const IVec4 refColor(ref.int32[0], ref.int32[1], ref.int32[2], ref.int32[3]);
    ...
    case TEXTURECHANNELCLASS_FLOATING_POINT:
    {
        ...
        for (int ndx = 0; ndx < 4; ndx++)
        {
            ...
            if (!result)
            {
                float floatThreshold = Float32((uint32_t)(threshold)[0]).asFloat();
...
public:
    ...
    Move<VkImage> createImage(VkImageType imageType, VkFormat format, VkImageTiling tiling, VkExtent3D extent,
                              uint32_t arrayLayerCount, VkImageUsageFlags usage) const;
    ...
    Move<VkFramebuffer> createFrameBuffer(VkImageView imageView, VkRenderPass renderPass, uint32_t imageWidth,
                                          uint32_t imageHeight, uint32_t imageLayersCount) const;
...
ImageClearingTestInstance::ImageClearingTestInstance(Context &context, const TestParams &params)
    ...
    , m_image(createImage(params.imageType, params.imageFormat, params.imageTiling, params.imageExtent,
                          params.imageLayerCount, m_imageUsageFlags))
...

{
    ...
    if (m_isAttachmentFormat)
    {
        ...
        m_frameBuffer = createFrameBuffer(*m_imageView, *m_renderPass, params.imageExtent.width,
                                          params.imageExtent.height, params.imageViewLayerRange.layerCount);
    }
}
...
Move<VkRenderPass> ImageClearingTestInstance::createRenderPass(VkFormat format) const
{
    if (m_params.separateDepthStencilLayoutMode == SEPARATE_DEPTH_STENCIL_LAYOUT_MODE_NONE)
    {
        ...
        const VkAttachmentDescription attachments[1] = {attachmentDesc};
...
Move<VkFramebuffer> ImageClearingTestInstance::createFrameBuffer(VkImageView imageView, VkRenderPass renderPass,
                                                                 ...
                                                                 uint32_t imageLayersCount) const
{
    const VkImageView attachmentViews[1] = {imageView};
                                                             ...
                                                             const UVec4 &clearCoords) const
{
        ...
        m_params.separateDepthStencilLayoutMode != SEPARATE_DEPTH_STENCIL_LAYOUT_MODE_STENCIL)
    {
        ...
        for (uint32_t arrayLayer = 0; arrayLayer < m_params.imageLayerCount; ++arrayLayer)
        {
            ...
            for (uint32_t y = 0; y < m_params.imageExtent.height; ++y)
                for (uint32_t x = 0; x < m_params.imageExtent.width; ++x)
                {
                    ...
                    else if (isInInitialClearRange(m_isAttachmentFormat, 0u /* mipLevel */, arrayLayer,
                                                   m_params.imageViewLayerRange))
                    {
                ...
                for (uint32_t z = 0; z < extent.depth; ++z)
                    for (uint32_t y = 0; y < extent.height; ++y)
                        for (uint32_t x = 0; x < extent.width; ++x)
                        {
                            ...
                            {
                                if (isInInitialClearRange(m_isAttachmentFormat, mipLevel, arrayLayer,
                                                          m_params.imageViewLayerRange))
                                {
                            ...
                            if (!comparePixelToColorClearValue(image->getLevel(mipLevel), x, y, z, *pColorValue,
                                                               message))
...
TestStatus ClearColorImageMultipleSubresourceRangeTestInstance::iterate(void)
{
    ...
    pipelineImageBarrier(VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,  // VkPipelineStageFlags        srcStageMask
                         ...
                         (m_isAttachmentFormat ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL :
...
{
    VkImageLayout layout          = (m_isAttachmentFormat ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL :
...
TestCaseGroup *createImageClearingTestsCommon(TestContext &testCtx, tcu::TestCaseGroup *imageClearingTests,
                                              AllocationKind allocationKind)
{
    de::MovePtr<TestCaseGroup> colorImageClearTests(
        new TestCaseGroup(testCtx, "clear_color_image", "Color Image Clear Tests"));
    de::MovePtr<TestCaseGroup> depthStencilImageClearTests(
        new TestCaseGroup(testCtx, "clear_depth_stencil_image", "Color Depth/Stencil Image Tests"));
    de::MovePtr<TestCaseGroup> colorAttachmentClearTests(
        new TestCaseGroup(testCtx, "clear_color_attachment", "Color Color Attachment Tests"));
    de::MovePtr<TestCaseGroup> depthStencilAttachmentClearTests(
        new TestCaseGroup(testCtx, "clear_depth_stencil_attachment", "Color Depth/Stencil Attachment Tests"));
    de::MovePtr<TestCaseGroup> partialColorAttachmentClearTests(
        new TestCaseGroup(testCtx, "partial_clear_color_attachment", "Clear Partial Color Attachment Tests"));
    de::MovePtr<TestCaseGroup> partialDepthStencilAttachmentClearTests(new TestCaseGroup(
        testCtx, "partial_clear_depth_stencil_attachment", "Clear Partial Depth/Stencil Attachment Tests"));
    ...
    const VkFormat colorImageFormatsToTest[] = {
        ...
        VK_FORMAT_R8_USCALED, VK_FORMAT_R8_SSCALED, VK_FORMAT_R8_UINT, VK_FORMAT_R8_SINT, VK_FORMAT_R8_SRGB,
        ...
        VK_FORMAT_R8G8B8A8_UINT, VK_FORMAT_R8G8B8A8_SINT, VK_FORMAT_R8G8B8A8_SRGB, VK_FORMAT_B8G8R8A8_UNORM,
        VK_FORMAT_B8G8R8A8_SNORM, VK_FORMAT_B8G8R8A8_USCALED, VK_FORMAT_B8G8R8A8_SSCALED, VK_FORMAT_B8G8R8A8_UINT,
        ...
        VK_FORMAT_R16_UNORM, VK_FORMAT_R16_SNORM, VK_FORMAT_R16_USCALED, VK_FORMAT_R16_SSCALED, VK_FORMAT_R16_UINT,
        VK_FORMAT_R16_SINT, VK_FORMAT_R16_SFLOAT, VK_FORMAT_R16G16_UNORM, VK_FORMAT_R16G16_SNORM,
        VK_FORMAT_R16G16_USCALED, VK_FORMAT_R16G16_SSCALED, VK_FORMAT_R16G16_UINT, VK_FORMAT_R16G16_SINT,
        VK_FORMAT_R16G16_SFLOAT, VK_FORMAT_R16G16B16_UNORM, VK_FORMAT_R16G16B16_SNORM, VK_FORMAT_R16G16B16_USCALED,
        VK_FORMAT_R16G16B16_SSCALED, VK_FORMAT_R16G16B16_UINT, VK_FORMAT_R16G16B16_SINT, VK_FORMAT_R16G16B16_SFLOAT,
                             ...
                             ++imageFormatIndex)
                        {
                            ...
                            if (!imageLayerParamsToTest[imageLayerParamsIndex].twoStep)
                            {
                                imageLayersGroup->addChild(
                                    new InstanceFactory1<ClearColorImageTestInstance, TestParams>(
                                        testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Clear Color Image",
                                        testParams));
                                ...
                                if (imageTilingsToTest[imageTilingIndex] == VK_IMAGE_TILING_OPTIMAL)
                                {
                                    ...
                                    imageLayersGroup->addChild(
                                        new InstanceFactory1<ClearColorImageMultipleSubresourceRangeTestInstance,
                                                             TestParams>(testCtx, NODETYPE_SELF_VALIDATE, testCaseName,
                            ...
                            {
                                imageLayersGroup->addChild(
                                    new InstanceFactory1<TwoStepClearColorImageTestInstance, TestParams>(
                                        testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Clear Color Image",
                                        testParams));
                            }
                        }
                 ...
                 ++imageDimensionsIndex)
            {
                     ...
                     ++imageFormatIndex)
                {
                    ...
                    for (int separateDepthStencilLayoutMode = 0;
                         separateDepthStencilLayoutMode < separateLayoutsLoopCount; ++separateDepthStencilLayoutMode)
                    {
                        ...
                        TestParams testParams = {
                            ...
                            imageLayerParamsToTest[imageLayerParamsIndex].imageLayerCount, // uint32_t imageLayerCount;
                            {0u, imageLayerParamsToTest[imageLayerParamsIndex]
                                     .imageLayerCount},             // LayerRange imageViewLayerRange;
                        ...
                        {
                            imageLayersGroup->addChild(
                                new InstanceFactory1<ClearDepthStencilImageTestInstance, TestParams>(
                                    testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Clear Depth/Stencil Image",
                                    testParams));

                            if (separateDepthStencilLayoutMode == SEPARATE_DEPTH_STENCIL_LAYOUT_MODE_NONE && hasDepth &&
                                hasStencil)
                            {
                                const std::string testCaseNameRanges =
                                    getFormatCaseName(format) + dimensionsString + "_multiple_subresourcerange";
                                imageLayersGroup->addChild(
                                    new InstanceFactory1<ClearDepthStencilImageMultipleSubresourceRangeTestInstance,
                                                         TestParams>(
                                        testCtx, NODETYPE_SELF_VALIDATE, testCaseNameRanges,
                                        "Clear Depth/Stencil Image with ranges", testParams));
                            }
                        }
                        else
                        {
                            imageLayersGroup->addChild(
                                new InstanceFactory1<TwoStepClearDepthStencilImageTestInstance, TestParams>(
                                    testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Clear Depth/Stencil Image",
                                    testParams));
                        }
                    }
                     ...
                     ++imageDimensionsIndex)
                {
                         ...
                         ++imageFormatIndex)
                    {
                        ...
                        colorAttachmentClearLayersGroup->addChild(
                            new InstanceFactory1<ClearAttachmentTestInstance, TestParams>(
                                testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Clear Color Attachment", testParams));
                        if (dimensions.width > 1)
                            partialColorAttachmentClearLayersGroup->addChild(
                                new InstanceFactory1<PartialClearAttachmentTestInstance, TestParams>(
                                    testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Partial Clear Color Attachment",
                                    testParams));
                    }
                }
                     ...
                     ++imageDimensionsIndex)
                {
                         ...
                         ++imageFormatIndex)
                    {
                             ...
                             ++separateDepthStencilLayoutMode)
                        {
                            ...
                            depthStencilLayersGroup->addChild(
                                new InstanceFactory1<ClearAttachmentTestInstance, TestParams>(
                                    testCtx, NODETYPE_SELF_VALIDATE, testCaseName, "Clear Depth/Stencil Attachment",
                                    testParams));
                            if (dimensions.width > 1)
                                partialDepthStencilLayersGroup->addChild(
                                    new InstanceFactory1<PartialClearAttachmentTestInstance, TestParams>(
                                        testCtx, NODETYPE_SELF_VALIDATE, testCaseName,
                                        "Partial Clear Depth/Stencil Attachment", testParams));
                        }
                    }
                }
...
TestCaseGroup *createImageClearingTests(TestContext &testCtx)
{
    ...
    imageClearingTests->addChild(
        createTestGroup(testCtx, "core", "Core Image Clearing Tests", createCoreImageClearingTests));
    imageClearingTests->addChild(createTestGroup(testCtx, "dedicated_allocation",
                                                 "Image Clearing Tests For Dedicated Memory Allocation",
                                                 createDedicatedAllocationImageClearingTests));
