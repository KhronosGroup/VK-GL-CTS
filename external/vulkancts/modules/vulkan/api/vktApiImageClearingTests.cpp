                                   ...
                                   const BVec4 &channelMask, const TextureChannelClass &channelClass)
{
    ...
    case TEXTURECHANNELCLASS_UNSIGNED_INTEGER:
    {
        ...
        const bool result = !(anyNotEqual(
            logicalAnd(lessThanEqual(absDiff(resColor, refColor), threshold.uvec4), channelMask), channelMask));

        if (!result)
        {
            std::stringstream s;
            s << "Ref:" << refColor << " Mask:" << channelMask << " Threshold:" << threshold.uvec4
              << " Color:" << resColor;
            stringResult = s.str();
        }
    ...
    case TEXTURECHANNELCLASS_SIGNED_INTEGER:
    {
        ...
        const bool result = !(anyNotEqual(
            logicalAnd(lessThanEqual(absDiff(resColor, refColor), threshold.ivec4), channelMask), channelMask));

        if (!result)
        {
            std::stringstream s;
            s << "Ref:" << refColor << " Mask:" << channelMask << " Threshold:" << threshold.ivec4
              << " Color:" << resColor;
            stringResult = s.str();
        }
