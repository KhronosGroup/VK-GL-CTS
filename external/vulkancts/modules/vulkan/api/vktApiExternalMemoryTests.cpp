...
void submitEmptySignalAndGetSemaphoreNative(const vk::DeviceInterface &vk, vk::VkDevice device, vk::VkQueue queue,
...
void submitEmptySignalAndGetFenceNative(const vk::DeviceInterface &vk, vk::VkDevice device, vk::VkQueue queue,
...
tcu::TestStatus testSemaphoreImportTwice(Context &context, const SemaphoreTestConfig config)
{
...

    {
        ...
        if (transference == TRANSFERENCE_COPY)
            submitEmptySignalAndGetSemaphoreNative(vkd, *device, queue, queueFamilyIndex, *semaphore,
...
tcu::TestStatus testSemaphoreImportReimport(Context &context, const SemaphoreTestConfig config)
{
...

    {
        ...
        if (transference == TRANSFERENCE_COPY)
            submitEmptySignalAndGetSemaphoreNative(vkd, *device, queue, queueFamilyIndex, *semaphoreA,
                                                   config.externalType, handleA);
        else
...
tcu::TestStatus testSemaphoreExportImportSignalWait(Context &context, const SemaphoreTestConfig config)
{
...

    {
        ...
        submitEmptySignalAndGetSemaphoreNative(vkd, *device, queue, queueFamilyIndex, *semaphoreA, config.externalType,
                                               handle);
...

    {
...

        {
            ...
            if (transference == TRANSFERENCE_COPY)
            {
                submitEmptySignalAndGetSemaphoreNative(vkd, *device, queue, queueFamilyIndex, *semaphoreA,
                                                       config.externalType, fd);
                submitEmptySignalAndGetSemaphoreNative(vkd, *device, queue, queueFamilyIndex, *semaphoreB,
...
tcu::TestStatus testFenceImportTwice(Context &context, const FenceTestConfig config)
{
...

    {
        ...
        if (transference == TRANSFERENCE_COPY)
            submitEmptySignalAndGetFenceNative(vkd, *device, queue, queueFamilyIndex, *fence, config.externalType,
                                               handleA);
        else
            getFenceNative(vkd, *device, *fence, config.externalType, handleA);
...
tcu::TestStatus testFenceImportReimport(Context &context, const FenceTestConfig config)
{
...

    {
        ...
        if (transference == TRANSFERENCE_COPY)
            submitEmptySignalAndGetFenceNative(vkd, *device, queue, queueFamilyIndex, *fenceA, config.externalType,
                                               handleA);
        else
...

    {
...

        {
            ...
            if (transference == TRANSFERENCE_COPY)
            {
                submitEmptySignalAndGetFenceNative(vkd, *device, queue, queueFamilyIndex, *fenceA, config.externalType,
                                                   fd);
                submitEmptySignalAndGetFenceNative(vkd, *device, queue, queueFamilyIndex, *fenceB, config.externalType,
                                                   secondFd);
            }
            else
            {
