...
public:
    ...
    virtual tcu::TestStatus iterate(void)
    {
        ...
        if (instanceVersion.majorNum > maxVulkanVersion.majorNum ||
            instanceVersion.minorNum > maxVulkanVersion.minorNum)
            return tcu::TestStatus::fail(
                de::toString("This version of CTS does not support a Vulkan instance with version ") +
                instanceVersionString);
        else if (deviceVersion.majorNum > maxVulkanVersion.majorNum ||
                 deviceVersion.minorNum > maxVulkanVersion.minorNum)
    ...
    virtual tcu::TestStatus iterate(void)
    {
        ...
        // Tests with default instance and device without extensions
        {
            ...
            // Check special cases
            {
                ...
                log << tcu::TestLog::Message
                    << "Special check - tries to get some dummy functions from various vkGet*ProcAddr."
                    << tcu::TestLog::EndMessage;
                const char *const result =
                    specialCasesCheck(ctx, log, failsQuantity, dummyFunctions) ? "Passed" : "Failed";
                log << tcu::TestLog::Message << result << tcu::TestLog::EndMessage;
            }
        }
...
private:
                      ...
                      const vector<pair<const char *, FunctionOrigin>> &testsArr)
    {
        ...
        {
            if (deStringEqual(testsArr[ndx].first, "vkGetInstanceProcAddr") &&
                m_context.getUsedApiVersion() < VK_API_VERSION_1_2)
                continue;

            const uint32_t functionType = testsArr[ndx].second;
            if (functionType == FUNCTIONORIGIN_PLATFORM)
                checkPlatformFunction(ctx, log, testsArr[ndx].first, true, failsQuantity);
            else if (functionType == FUNCTIONORIGIN_INSTANCE)
            {
                checkInstanceFunction(ctx, log, testsArr[ndx].first, true, failsQuantity);
                checkDeviceFunction(ctx, log, testsArr[ndx].first, false, failsQuantity);
            }
            else if (functionType == FUNCTIONORIGIN_DEVICE)
            {
                checkInstanceFunction(ctx, log, testsArr[ndx].first, true, failsQuantity);
                checkDeviceFunction(ctx, log, testsArr[ndx].first, true, failsQuantity);
            }
        }
        return startingQuantity == failsQuantity;
    }
