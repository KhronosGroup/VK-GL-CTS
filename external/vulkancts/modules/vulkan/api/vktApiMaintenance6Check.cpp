...
{
public:
    Maintenance6MaxCombinedImageSamplerDescriptorCountTestInstance(Context &ctx) : TestInstance(ctx)
    {
    }
    virtual tcu::TestStatus iterate(void)
    {
        ...
        for (int rangeNdx = 0; rangeNdx < DE_LENGTH_OF_ARRAY(s_formatRanges); ++rangeNdx)
        {
            ...
            for (VkFormat format = rangeBegin; format != rangeEnd; format = (VkFormat)(format + 1))
            {
                ...
                if (conversionImageFormatProps.combinedImageSamplerDescriptorCount >
                    maintProp6.maxCombinedImageSamplerDescriptorCount)
                {
                    return tcu::TestStatus::fail(
                        "Fail: format " + std::string(getFormatName(format)) +
                        " requires a larger combinedImageSamplerDescriptorCount=" +
                        std::to_string(conversionImageFormatProps.combinedImageSamplerDescriptorCount) +
                        " than maxCombinedImageSamplerDescriptorCount=" +
                        std::to_string(maintProp6.maxCombinedImageSamplerDescriptorCount));
                }
            }
        }
...
{
    de::MovePtr<tcu::TestCaseGroup> main6Tests(
        new tcu::TestCaseGroup(testCtx, "maintenance6_check", "Maintenance6 Tests"));
