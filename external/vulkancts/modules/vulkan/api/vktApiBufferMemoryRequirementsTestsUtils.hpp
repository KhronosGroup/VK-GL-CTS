...
std::vector<Flag> mergeFlags(const std::vector<Flag> &flags1,
...
template <class Flag, class Bits, class... Ignored>
void mergeFlags(std::vector<BitsSet<Flag, Bits, Ignored...>> &inout,
...
template <class Flag, class Bit, class... Ignored>
void combine(std::vector<BitsSet<Flag, Bit, Ignored...>> &result, const BitsSet<Flag, Bit, Ignored...> &bits,
             std::vector<Flag> &hints)
{
    const Flag flag = bits();
    if (bits.empty() || hints.end() != std::find(hints.begin(), hints.end(), flag))
        return;
