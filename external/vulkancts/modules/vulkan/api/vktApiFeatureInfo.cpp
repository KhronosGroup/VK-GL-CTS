...
tcu::TestStatus formatProperties(Context &context, VkFormat format)
{
    ...
    for (int fieldNdx = 0; fieldNdx < DE_LENGTH_OF_ARRAY(fields); fieldNdx++)
    {
        ...
        results.check((supported & ~allowed) == 0, de::toString(fieldName) + ": has: " +
                                                       de::toString(getFormatFeatureFlagsStr(supported & ~allowed)));
              ...
              VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_CHROMA_RECONSTRUCTION_EXPLICIT_FORCEABLE_BIT) == 0))
        {
            results.addResult(
                ...
                de::toString(fieldName) +
                    " supports VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_CHROMA_RECONSTRUCTION_EXPLICIT_BIT but "
                    "not "
                    "VK_FORMAT_FEATURE_SAMPLED_IMAGE_YCBCR_CONVERSION_CHROMA_RECONSTRUCTION_EXPLICIT_FORCEABLE_BIT");
        }
        ...
        if (!isYCbCrFormat(format) && !isCompressedFormat(format))
        {
            ...
            if (tcu::getNumUsedChannels(tcuFormat.order) != 1 &&
                (supported &
                 (VK_FORMAT_FEATURE_STORAGE_TEXEL_BUFFER_ATOMIC_BIT | VK_FORMAT_FEATURE_STORAGE_IMAGE_ATOMIC_BIT)) != 0)
            {
...
tcu::TestCaseGroup *createFeatureInfoTests(tcu::TestContext &testCtx)
{
...

    {
        ...
        addFunctionCaseInNewSubgroup(testCtx, extendedPropertiesTests.get(), "properties", "Extended Device Properties",
                                     deviceProperties2);
        addFunctionCaseInNewSubgroup(testCtx, extendedPropertiesTests.get(), "format_properties",
                                     "Extended Device Format Properties", deviceFormatProperties2);
        ...
        addFunctionCaseInNewSubgroup(testCtx, extendedPropertiesTests.get(), "memory_properties",
                                     "Extended Device Memory Properties", deviceMemoryProperties2);
...

    {
        ...
        addFunctionCase(extendedPropertiesTests.get(), "feature_bits_influence",
                        "Validate feature bits influence on feature activation", checkApiVersionSupport<1, 2>,
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> extendedPropertiesTests(
            new tcu::TestCaseGroup(testCtx, "vulkan1p3", "Vulkan 1.3 related tests"));

        addFunctionCase(extendedPropertiesTests.get(), "features", "Extended Vulkan 1.3 Device Features",
                        deviceFeaturesVulkan13);
        addFunctionCase(extendedPropertiesTests.get(), "properties", "Extended Vulkan 1.3 Device Properties",
                        devicePropertiesVulkan13);
        addFunctionCase(extendedPropertiesTests.get(), "feature_extensions_consistency",
                        "Vulkan 1.3 consistency between Features and Extensions",
                        deviceFeatureExtensionsConsistencyVulkan13);
        addFunctionCase(extendedPropertiesTests.get(), "property_extensions_consistency",
                        "Vulkan 1.3 consistency between Properties and Extensions",
                        devicePropertyExtensionsConsistencyVulkan13);
        addFunctionCase(extendedPropertiesTests.get(), "feature_bits_influence",
                        "Validate feature bits influence on feature activation", checkApiVersionSupport<1, 3>,
...

    {
        ...
        addFunctionCase(limitsValidationTests.get(), "general", "Vulkan 1.2 Limit validation",
                        checkApiVersionSupport<1, 2>, validateLimits12);
#ifndef CTS_USES_VULKANSC
        ...
        addFunctionCase(limitsValidationTests.get(), "robustness2", "VK_EXT_robustness2 limit validation",
                        checkSupportRobustness2, validateLimitsRobustness2);
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> limitsValidationTests(new tcu::TestCaseGroup(
            testCtx, "vulkan1p3_limits_validation", "Vulkan 1.3 and core extensions limits validation"));

#ifndef CTS_USES_VULKANSC
        addFunctionCase(limitsValidationTests.get(), "khr_maintenance4", "VK_KHR_maintenance4",
                        checkSupportKhrMaintenance4, validateLimitsKhrMaintenance4);
        addFunctionCase(limitsValidationTests.get(), "max_inline_uniform_total_size",
                        "maxInlineUniformTotalSize limit validation", checkApiVersionSupport<1, 3>,
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> profilesValidationTests(
            new tcu::TestCaseGroup(testCtx, "profiles", "Profiles limits and features validation"));

        addFunctionCase(profilesValidationTests.get(), "roadmap_2022", "Limits and features check for roadmap 2022",
                        checkApiVersionSupport<1, 3>, validateRoadmap2022);
...
void createFeatureInfoInstanceTests(tcu::TestCaseGroup *testGroup)
{
    ...
    addFunctionCase(testGroup, "instance_extension_device_functions", "Validates device-level entry-points",
