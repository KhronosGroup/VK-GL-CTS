...
bool validateFeatureLimits(VkPhysicalDeviceProperties *properties, VkPhysicalDeviceFeatures *features, TestLog &log)
{
    ...
    for (uint32_t ndx = 0; ndx < DE_LENGTH_OF_ARRAY(featureLimitTable); ndx++)
    {
        ...
        case LIMIT_FORMAT_UNSIGNED_INT:
        {
            ...
            {
                if (!deIntIsPow2(*((uint32_t *)((uint8_t *)limits + featureLimitTable[ndx].offset))))
                {
...
tcu::TestStatus enumerateDeviceExtensions(Context &context)
{
...

    {
        ...
        for (const auto &version : releasedApiVersions)
        {
            ...
            if (context.contextSupports(vk::ApiVersion(versionMajor, versionMinor, 0)))
            {
                checkDeviceExtensionDependencies(results, DE_LENGTH_OF_ARRAY(deviceExtensionDependencies),
                                                 deviceExtensionDependencies, versionMajor, versionMinor,
...
tcu::TestStatus extensionCoreVersions(Context &context)
{
    ...
    for (const auto &majorMinorName : extensionRequiredCoreVersion)
    {
        ...
        if ((isExtensionSupported(instanceExtensionProperties, reqExt) ||
             isExtensionSupported(deviceExtensionProperties, reqExt)) &&
...
tcu::TestCaseGroup *createFeatureInfoTests(tcu::TestContext &testCtx)
{
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> extCoreVersionGrp(new tcu::TestCaseGroup(
            testCtx, "extension_core_versions", "Tests checking extension required core versions"));
