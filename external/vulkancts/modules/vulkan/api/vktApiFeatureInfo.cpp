...
void checkInstanceExtensionDependencies(
    tcu::ResultCollector &results, int dependencyLength,
    const std::tuple<uint32_t, uint32_t, uint32_t, const char *, const char *> *dependencies, uint32_t apiVariant,
    uint32_t versionMajor, uint32_t versionMinor, const vector<VkExtensionProperties> &extensionProperties)
{
    for (int ndx = 0; ndx < dependencyLength; ndx++)
    {
        uint32_t currentApiVariant, currentVersionMajor, currentVersionMinor;
        const char *extensionFirst;
        const char *extensionSecond;
        std::tie(currentApiVariant, currentVersionMajor, currentVersionMinor, extensionFirst, extensionSecond) =
            dependencies[ndx];
        if (currentApiVariant != apiVariant || currentVersionMajor != versionMajor ||
...
void checkDeviceExtensionDependencies(
    tcu::ResultCollector &results, int dependencyLength,
    const std::tuple<uint32_t, uint32_t, uint32_t, const char *, const char *> *dependencies, uint32_t apiVariant,
    uint32_t versionMajor, uint32_t versionMinor, const vector<VkExtensionProperties> &instanceExtensionProperties,
        ...
        for (const auto &version : releasedApiVersions)
        {
            ...
            if (context.contextSupports(vk::ApiVersion(apiVariant, versionMajor, versionMinor, 0)))
            {
                checkInstanceExtensionDependencies(results, DE_LENGTH_OF_ARRAY(instanceExtensionDependencies),
                                                   instanceExtensionDependencies, apiVariant, versionMajor,
        ...
        for (const auto &version : releasedApiVersions)
        {
            ...
            if (context.contextSupports(vk::ApiVersion(apiVariant, versionMajor, versionMinor, 0)))
            {
                checkDeviceExtensionDependencies(results, DE_LENGTH_OF_ARRAY(deviceExtensionDependencies),
                                                 deviceExtensionDependencies, apiVariant, versionMajor, versionMinor,
...
VkFormatFeatureFlags getAllowedOptimalTilingFeatures(Context &context, VkFormat format)
{
    ...
    const VkFormatFeatureFlags ycbcrAllows =
        ...
        VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_MINMAX_BIT | VK_FORMAT_FEATURE_DISJOINT_BIT;
