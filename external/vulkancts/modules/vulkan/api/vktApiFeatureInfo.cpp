...
bool validateFeatureLimits(VkPhysicalDeviceProperties *properties, VkPhysicalDeviceFeatures *features, TestLog &log)
{
    ...
    struct FeatureLimitTable
    {
    ...
    } featureLimitTable[] = //!< Based on 1.0.28 Vulkan spec
        {
            ...
            {LIMIT(maxPerStageDescriptorInputAttachments), 4, 0, 0, 0.0f, LIMIT_FORMAT_UNSIGNED_INT, LIMIT_TYPE_MIN,
             -1},
            ...
            {LIMIT(maxDescriptorSetUniformBuffers), shaderStages * 12, 0, 0, 0.0f, LIMIT_FORMAT_UNSIGNED_INT,
             LIMIT_TYPE_MIN, -1},
            {LIMIT(maxDescriptorSetUniformBuffersDynamic), 8, 0, 0, 0.0f, LIMIT_FORMAT_UNSIGNED_INT, LIMIT_TYPE_MIN,
             -1},
            {LIMIT(maxDescriptorSetStorageBuffers), shaderStages * 4, 0, 0, 0.0f, LIMIT_FORMAT_UNSIGNED_INT,
             LIMIT_TYPE_MIN, -1},
            {LIMIT(maxDescriptorSetStorageBuffersDynamic), 4, 0, 0, 0.0f, LIMIT_FORMAT_UNSIGNED_INT, LIMIT_TYPE_MIN,
             -1},
            ...
            {LIMIT(framebufferColorSampleCounts), VK_SAMPLE_COUNT_1_BIT | VK_SAMPLE_COUNT_4_BIT, 0, 0, 0.0f,
             LIMIT_FORMAT_BITMASK, LIMIT_TYPE_MIN, -1},
...
tcu::TestStatus validateLimitsExtDescriptorIndexing(Context &context)
{
    ...
    const VkPhysicalDeviceProperties2 &properties2 = context.getDeviceProperties2();
    const VkPhysicalDeviceLimits &limits           = properties2.properties.limits;
    const VkPhysicalDeviceDescriptorIndexingPropertiesEXT &descriptorIndexingPropertiesEXT =
    ...
    FeatureLimitTableItem featureLimitTable[] = {
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxUpdateAfterBindDescriptorsInAllPools),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindSamplers),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindUniformBuffers),
         LIM_MIN_UINT32(12)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindStorageBuffers),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindSampledImages),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindStorageImages),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindInputAttachments),
         LIM_MIN_UINT32(4)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageUpdateAfterBindResources),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindSamplers),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindUniformBuffers),
         LIM_MIN_UINT32(shaderStages * 12)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindUniformBuffersDynamic),
         LIM_MIN_UINT32(8)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindStorageBuffers),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindStorageBuffersDynamic),
         LIM_MIN_UINT32(4)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindSampledImages),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindStorageImages),
         LIM_MIN_UINT32(500000)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxDescriptorSetUpdateAfterBindInputAttachments),
         LIM_MIN_UINT32(4)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindSamplers),
         LIM_MIN_UINT32(limits.maxPerStageDescriptorSamplers)},
        {PN(checkAlways), PN(descriptorIndexingPropertiesEXT.maxPerStageDescriptorUpdateAfterBindUniformBuffers),
...
{
    const VkBool32 checkAlways = VK_TRUE;
    const VkPhysicalDeviceInlineUniformBlockPropertiesEXT &inlineUniformBlockPropertiesEXT =
        context.getInlineUniformBlockPropertiesEXT();
    TestLog &log  = context.getTestContext().getLog();
    bool limitsOk = true;
...
{
    const VkBool32 checkAlways = VK_TRUE;
    const VkPhysicalDeviceVertexAttributeDivisorPropertiesEXT &vertexAttributeDivisorPropertiesEXT =
...
{
    const VkBool32 checkAlways = VK_TRUE;
    const VkPhysicalDeviceTimelineSemaphorePropertiesKHR &timelineSemaphorePropertiesKHR =
        context.getTimelineSemaphoreProperties();
    bool limitsOk = true;
    TestLog &log  = context.getTestContext().getLog();

    FeatureLimitTableItem featureLimitTable[] = {
        {PN(checkAlways), PN(timelineSemaphorePropertiesKHR.maxTimelineSemaphoreValueDifference),
         LIM_MIN_DEVSIZE((1ull << 31) - 1)},
    };
...
void createDevice(Context &context, void *pNext, const char *const *ppEnabledExtensionNames,
                  uint32_t enabledExtensionCount)
{
    ...
    const Unique<VkDevice> device(
        createDevice(platformInterface, *instance, instanceDriver, physicalDevice, &deviceCreateInfo));
...
#define DEPENDENCY_SINGLE_ITEM(CORE, FIELD, PARENT) \
    ...
    struct FeatureTable
    {
        ...
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, shaderInputAttachmentArrayDynamicIndexing,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, shaderUniformTexelBufferArrayDynamicIndexing,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, shaderStorageTexelBufferArrayDynamicIndexing,
                           "VK_EXT_descriptor_indexing"),
        ...
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, shaderStorageImageArrayNonUniformIndexing,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, shaderInputAttachmentArrayNonUniformIndexing,
                           "VK_EXT_descriptor_indexing"),
        ...
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, descriptorBindingUniformBufferUpdateAfterBind,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, descriptorBindingSampledImageUpdateAfterBind,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, descriptorBindingStorageImageUpdateAfterBind,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, descriptorBindingStorageBufferUpdateAfterBind,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures,
        ...
        FEATURE_TABLE_ITEM(vulkan12Features, descriptorIndexingFeatures, runtimeDescriptorArray,
                           "VK_EXT_descriptor_indexing"),
        FEATURE_TABLE_ITEM(vulkan12Features, dummyExtensionFeatures, samplerFilterMinmax,
        ...
        FEATURE_TABLE_ITEM(vulkan12Features, vulkanMemoryModelFeatures, vulkanMemoryModelDeviceScope,
                           "VK_KHR_vulkan_memory_model"),
        FEATURE_TABLE_ITEM(vulkan12Features, vulkanMemoryModelFeatures, vulkanMemoryModelAvailabilityVisibilityChains,
                           "VK_KHR_vulkan_memory_model"),
    };
    struct FeatureDependencyTable
    {
        ...
        DEPENDENCY_SINGLE_ITEM(vulkan12Features, descriptorBindingUniformTexelBufferUpdateAfterBind,
                               descriptorIndexing),
        DEPENDENCY_SINGLE_ITEM(vulkan12Features, descriptorBindingStorageTexelBufferUpdateAfterBind,
                               descriptorIndexing),
        ...
        DEPENDENCY_DUAL_ITEM(vulkan12Features, vulkanMemoryModelFeatures, vulkanMemoryModelDeviceScope,
                             vulkanMemoryModel),
        DEPENDENCY_DUAL_ITEM(vulkan12Features, vulkanMemoryModelFeatures, vulkanMemoryModelAvailabilityVisibilityChains,
                             vulkanMemoryModel),
    };
    ...
    for (size_t featureTableNdx = 0; featureTableNdx < DE_LENGTH_OF_ARRAY(featureTable); ++featureTableNdx)
    {
        ...
        // Core test
        {
            ...
            if (coreFeatureState)
            {
                ...
                for (size_t featureDependencyTableNdx = 0;
                     featureDependencyTableNdx < DE_LENGTH_OF_ARRAY(featureDependencyTable);
                     ++featureDependencyTableNdx)
                    if (featureDependencyTable[featureDependencyTableNdx].featurePtr == featurePtr)
                        featureDependencyTable[featureDependencyTableNdx].dependOnPtr[0] = true;
        ...
        // ext test
        {
            ...
            if (extStringPtr == DE_NULL ||
                isExtensionSupported(deviceExtensionProperties, RequiredExtension(extStringPtr)))
            {
                vki.getPhysicalDeviceFeatures2(physicalDevice, &features2);
...
void checkInstanceExtensionDependencies(tcu::ResultCollector &results, int dependencyLength,
                                        const std::tuple<uint32_t, uint32_t, const char *, const char *> *dependencies,
                                        ...
                                        const vector<VkExtensionProperties> &extensionProperties)
{
    ...
    {
        uint32_t currentVersionMajor, currentVersionMinor;
...
void checkDeviceExtensionDependencies(tcu::ResultCollector &results, int dependencyLength,
                                      const std::tuple<uint32_t, uint32_t, const char *, const char *> *dependencies,
                                      uint32_t versionMajor, uint32_t versionMinor,
...
tcu::TestStatus enumerateInstanceExtensions(Context &context)
{
...

    {
        ...
        for (const auto &version : releasedApiVersions)
        {
            ...
            {
                checkInstanceExtensionDependencies(results, DE_LENGTH_OF_ARRAY(instanceExtensionDependencies),
                                                   instanceExtensionDependencies, versionMajor, versionMinor,
                                                   properties);
                break;
            }
        }
...
tcu::TestStatus enumerateDeviceExtensions(Context &context)
{
...

    {
        ...
        for (const auto &version : releasedApiVersions)
        {
            ...
            if (context.contextSupports(vk::ApiVersion(versionMajor, versionMinor, 0)))
            {
                checkDeviceExtensionDependencies(results, DE_LENGTH_OF_ARRAY(instanceExtensionDependencies),
                                                 instanceExtensionDependencies, versionMajor, versionMinor,
...
tcu::TestStatus formatProperties(Context &context, VkFormat format)
{
    ...
    const VkFormatFeatureFlags extOptimalFeatures =
        getRequiredOptimalExtendedTilingFeatures(context, format, properties.optimalTilingFeatures);
    ...
    } fields[] = {{&VkFormatProperties::linearTilingFeatures, "linearTilingFeatures", (VkFormatFeatureFlags)0},
                  {&VkFormatProperties::optimalTilingFeatures, "optimalTilingFeatures",
                   getRequiredOptimalTilingFeatures(format) | extOptimalFeatures},
    ...
    for (int fieldNdx = 0; fieldNdx < DE_LENGTH_OF_ARRAY(fields); fieldNdx++)
    {
        ...
        if ((supported & required) != required)
        {
            log << TestLog::Message << "ERROR in " << fieldName << ":\n"
                ...
                << "  missing: " << getFormatFeatureFlagsStr(~supported & required) << TestLog::EndMessage;
            allOk = false;
        }
        ...
        {
            const VkFormatFeatureFlags required2 = VK_FORMAT_FEATURE_TRANSFER_SRC_BIT |
            ...
            if ((supported & required2) != required2)
            {
                log << TestLog::Message << "ERROR in " << fieldName << ":\n"
                    ...
                    << "  missing: " << getFormatFeatureFlagsStr(~supported & required2) << TestLog::EndMessage;
                allOk = false;
            }
        }
        ...
        {
            log << TestLog::Message << "ERROR in " << fieldName << ":\n"
                << "  has: " << getFormatFeatureFlagsStr(supported & notAllowedFeatures) << TestLog::EndMessage;
            allOk = false;
        }
    }
...
tcu::TestStatus ycbcrFormatProperties(Context &context, VkFormat format)
{
    ...
    } fields[] = {{&VkFormatProperties::linearTilingFeatures, "linearTilingFeatures", false, allowedImageFeatures},
    ...
    static const VkFormat s_requiredBaseFormats[] = {VK_FORMAT_G8_B8R8_2PLANE_420_UNORM,
                                                     VK_FORMAT_G8_B8_R8_3PLANE_420_UNORM};
    const bool isRequiredBaseFormat =
        isYcbcrConversionSupported(context) &&
    ...
    for (int fieldNdx = 0; fieldNdx < DE_LENGTH_OF_ARRAY(fields); fieldNdx++)
    {
        ...
        if (isRequiredBaseFormat && fields[fieldNdx].requiredFeatures)
        {
            ...
            {
                log << TestLog::Message << "ERROR in " << fieldName << ":\n"
                    << "  Either VK_FORMAT_FEATURE_MIDPOINT_CHROMA_SAMPLES_BIT or "
                       "VK_FORMAT_FEATURE_COSITED_CHROMA_SAMPLES_BIT required"
                    << TestLog::EndMessage;
                allOk = false;
            }
        }
        ...
        {
            log << TestLog::Message << "ERROR in " << fieldName << ":\n"
                << "  has: " << getFormatFeatureFlagsStr(supported & ~allowed) << TestLog::EndMessage;
            allOk = false;
        }
                                      ...
                                      const VkImageTiling tiling)
{
    ...
    if (isYcbcrConversionSupported(context) &&
        (format == VK_FORMAT_G8_B8_R8_3PLANE_420_UNORM_KHR || format == VK_FORMAT_G8_B8R8_2PLANE_420_UNORM_KHR))
    {
        VkFormatFeatureFlags requiredFeatures =
    ...
    for (VkImageUsageFlags curUsageFlags = 0; curUsageFlags <= usageFlagSet; curUsageFlags++)
    {
        ...
        for (VkImageCreateFlags curCreateFlags = 0; curCreateFlags <= createFlagSet; curCreateFlags++)
        {
            ...
            {
                const uint32_t fullMipPyramidSize = de::max(de::max(deLog2Ceil32(properties.maxExtent.width),
...
tcu::TestStatus deviceProperties2(Context &context)
{
    ...
    const bool khr_external_fence_capabilities = checkExtension(properties, "VK_KHR_external_fence_capabilities") ||
                                                 context.contextSupports(vk::ApiVersion(1, 1, 0));
    ...
    const bool khr_multiview =
        checkExtension(properties, "VK_KHR_multiview") || context.contextSupports(vk::ApiVersion(1, 1, 0));
    ...
    const bool khr_maintenance2 =
        checkExtension(properties, "VK_KHR_maintenance2") || context.contextSupports(vk::ApiVersion(1, 1, 0));
    const bool khr_maintenance3 =
        checkExtension(properties, "VK_KHR_maintenance3") || context.contextSupports(vk::ApiVersion(1, 1, 0));
    const bool khr_depth_stencil_resolve =
        checkExtension(properties, "VK_KHR_depth_stencil_resolve") || context.contextSupports(vk::ApiVersion(1, 2, 0));
    const bool khr_driver_properties =
        checkExtension(properties, "VK_KHR_driver_properties") || context.contextSupports(vk::ApiVersion(1, 2, 0));
    const bool khr_shader_float_controls =
        checkExtension(properties, "VK_KHR_shader_float_controls") || context.contextSupports(vk::ApiVersion(1, 2, 0));
    const bool khr_descriptor_indexing =
        checkExtension(properties, "VK_EXT_descriptor_indexing") || context.contextSupports(vk::ApiVersion(1, 2, 0));
    const bool khr_sampler_filter_minmax =
        checkExtension(properties, "VK_EXT_sampler_filter_minmax") || context.contextSupports(vk::ApiVersion(1, 2, 0));
    ...
    if (isExtensionSupported(properties, RequiredExtension("VK_KHR_performance_query")))
    {
        ...
        if (performanceQueryProperties[0].allowCommandBufferQueryCopies !=
            performanceQueryProperties[0].allowCommandBufferQueryCopies)
        {
...
tcu::TestStatus deviceMemoryProperties2(Context &context)
{
    ...
    if (deMemCmp(&coreProperties, &extProperties.memoryProperties, sizeof(VkPhysicalDeviceMemoryProperties)) != 0)
        TCU_FAIL("Mismatch between properties reported by vkGetPhysicalDeviceMemoryProperties and "
                                       ...
                                       const VkImageTiling tiling)
{
    ...
    const VkImageCreateFlags ycbcrFlags =
        isYCbCrFormat(format) ? (VkImageCreateFlags)VK_IMAGE_CREATE_DISJOINT_BIT_KHR : (VkImageCreateFlags)0u;
...
void createImageFormatTests(tcu::TestCaseGroup *testGroup, ImageFormatPropertyCase::Function testFunction)
{
    testGroup->addChild(createTestGroup(
        ...
        ImageFormatPropertyCase(testFunction, VK_FORMAT_UNDEFINED, VK_IMAGE_TYPE_1D, VK_IMAGE_TILING_MAX_ENUM)));
    testGroup->addChild(createTestGroup(
        testGroup->getTestContext(), "2d", "", createImageFormatTypeTests,
        ImageFormatPropertyCase(testFunction, VK_FORMAT_UNDEFINED, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_MAX_ENUM)));
    testGroup->addChild(createTestGroup(
        testGroup->getTestContext(), "3d", "", createImageFormatTypeTests,
        ImageFormatPropertyCase(testFunction, VK_FORMAT_UNDEFINED, VK_IMAGE_TYPE_3D, VK_IMAGE_TILING_MAX_ENUM)));
}
...
tcu::TestCaseGroup *createFeatureInfoTests(tcu::TestContext &testCtx)
{
    ...
    infoTests->addChild(createTestGroup(testCtx, "format_properties", "VkGetPhysicalDeviceFormatProperties() Tests",
                                        createFormatTests));
    infoTests->addChild(createTestGroup(testCtx, "image_format_properties",
                                        "VkGetPhysicalDeviceImageFormatProperties() Tests", createImageFormatTests,
                                        imageFormatProperties));

    {
        de::MovePtr<tcu::TestCaseGroup> extendedPropertiesTests(new tcu::TestCaseGroup(
        ...
        addFunctionCase(extendedPropertiesTests.get(), "queue_family_properties",
                        "Extended Device Queue Family Properties", deviceQueueFamilyProperties2);
        addFunctionCase(extendedPropertiesTests.get(), "memory_properties", "Extended Device Memory Properties",
...

    {
        de::MovePtr<tcu::TestCaseGroup> extendedPropertiesTests(
            new tcu::TestCaseGroup(testCtx, "vulkan1p2", "Vulkan 1.2 related tests"));

        addFunctionCase(extendedPropertiesTests.get(), "features", "Extended Vulkan 1.2 Device Features",
                        deviceFeaturesVulkan12);
        addFunctionCase(extendedPropertiesTests.get(), "properties", "Extended Vulkan 1.2 Device Properties",
                        devicePropertiesVulkan12);
        addFunctionCase(extendedPropertiesTests.get(), "feature_extensions_consistency",
                        "Vulkan 1.2 consistency between Features and Extensions",
                        deviceFeatureExtensionsConsistencyVulkan12);
        addFunctionCase(extendedPropertiesTests.get(), "property_extensions_consistency",
                        "Vulkan 1.2 consistency between Properties and Extensions",
                        devicePropertyExtensionsConsistencyVulkan12);
        addFunctionCase(extendedPropertiesTests.get(), "feature_bits_influence",
                        "Validate feature bits influence on feature activation", checkSupportFeatureBitInfluence,
                        featureBitInfluenceOnDeviceCreate);

        infoTests->addChild(extendedPropertiesTests.release());
    }

    {
        de::MovePtr<tcu::TestCaseGroup> limitsValidationTests(new tcu::TestCaseGroup(
            testCtx, "vulkan1p2_limits_validation", "Vulkan 1.2 and core extensions limits validation"));

        addFunctionCase(limitsValidationTests.get(), "general", "Vulkan 1.2 Limit validation",
                        validateLimitsCheckSupport, validateLimits12);
        addFunctionCase(limitsValidationTests.get(), "khr_push_descriptor", "VK_KHR_push_descriptor limit validation",
                        checkSupportKhrPushDescriptor, validateLimitsKhrPushDescriptor);
        addFunctionCase(limitsValidationTests.get(), "khr_multiview", "VK_KHR_multiview limit validation",
                        checkSupportKhrMultiview, validateLimitsKhrMultiview);
        addFunctionCase(limitsValidationTests.get(), "ext_discard_rectangles",
                        "VK_EXT_discard_rectangles limit validation", checkSupportExtDiscardRectangles,
                        validateLimitsExtDiscardRectangles);
        addFunctionCase(limitsValidationTests.get(), "ext_sample_locations", "VK_EXT_sample_locations limit validation",
                        checkSupportExtSampleLocations, validateLimitsExtSampleLocations);
        addFunctionCase(limitsValidationTests.get(), "ext_external_memory_host",
                        "VK_EXT_external_memory_host limit validation", checkSupportExtExternalMemoryHost,
                        validateLimitsExtExternalMemoryHost);
        addFunctionCase(limitsValidationTests.get(), "ext_blend_operation_advanced",
                        "VK_EXT_blend_operation_advanced limit validation", checkSupportExtBlendOperationAdvanced,
                        validateLimitsExtBlendOperationAdvanced);
        addFunctionCase(limitsValidationTests.get(), "khr_maintenance_3", "VK_KHR_maintenance3 limit validation",
                        checkSupportKhrMaintenance3, validateLimitsKhrMaintenance3);
        addFunctionCase(limitsValidationTests.get(), "ext_conservative_rasterization",
                        "VK_EXT_conservative_rasterization limit validation", checkSupportExtConservativeRasterization,
                        validateLimitsExtConservativeRasterization);
        addFunctionCase(limitsValidationTests.get(), "ext_descriptor_indexing",
                        "VK_EXT_descriptor_indexing limit validation", checkSupportExtDescriptorIndexing,
                        validateLimitsExtDescriptorIndexing);
        addFunctionCase(limitsValidationTests.get(), "ext_inline_uniform_block",
                        "VK_EXT_inline_uniform_block limit validation", checkSupportExtInlineUniformBlock,
                        validateLimitsExtInlineUniformBlock);
        addFunctionCase(limitsValidationTests.get(), "ext_vertex_attribute_divisor",
                        "VK_EXT_vertex_attribute_divisor limit validation", checkSupportExtVertexAttributeDivisor,
                        validateLimitsExtVertexAttributeDivisor);
        addFunctionCase(limitsValidationTests.get(), "nv_mesh_shader", "VK_NV_mesh_shader limit validation",
                        checkSupportNvMeshShader, validateLimitsNvMeshShader);
        addFunctionCase(limitsValidationTests.get(), "ext_transform_feedback",
                        "VK_EXT_transform_feedback limit validation", checkSupportExtTransformFeedback,
                        validateLimitsExtTransformFeedback);
        addFunctionCase(limitsValidationTests.get(), "fragment_density_map",
                        "VK_EXT_fragment_density_map limit validation", checkSupportExtFragmentDensityMap,
                        validateLimitsExtFragmentDensityMap);
        addFunctionCase(limitsValidationTests.get(), "nv_ray_tracing", "VK_NV_ray_tracing limit validation",
                        checkSupportNvRayTracing, validateLimitsNvRayTracing);
        addFunctionCase(limitsValidationTests.get(), "timeline_semaphore", "VK_KHR_timeline_semaphore limit validation",
                        checkSupportKhrTimelineSemaphore, validateLimitsKhrTimelineSemaphore);
        addFunctionCase(limitsValidationTests.get(), "ext_line_rasterization",
                        "VK_EXT_line_rasterization limit validation", checkSupportExtLineRasterization,
                        validateLimitsExtLineRasterization);

        infoTests->addChild(limitsValidationTests.release());
    }

    infoTests->addChild(createTestGroup(testCtx, "image_format_properties2",
                                        "VkGetPhysicalDeviceImageFormatProperties2() Tests", createImageFormatTests,
                                        imageFormatProperties2));
    infoTests->addChild(createTestGroup(testCtx, "sparse_image_format_properties2",
                                        "VkGetPhysicalDeviceSparseImageFormatProperties2() Tests",
                                        createImageFormatTests, sparseImageFormatProperties2));

    {
        de::MovePtr<tcu::TestCaseGroup> androidTests(new tcu::TestCaseGroup(testCtx, "android", "Android CTS Tests"));

        addFunctionCase(androidTests.get(), "mandatory_extensions", "Test that all mandatory extensions are supported",
...
void createFeatureInfoDeviceTests(tcu::TestCaseGroup *testGroup)
{
    ...
    addFunctionCase(testGroup, "device_queue_family_properties", "Queue family properties",
...
{
    addFunctionCase(testGroup, "device_group_peer_memory_features", "Device Group peer memory features",
