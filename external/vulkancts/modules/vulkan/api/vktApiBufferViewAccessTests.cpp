    ...
    , m_pixelDataSize(m_renderSize.x() * m_renderSize.y() * mapVkFormat(m_colorFormat).getPixelSize())
{
    ...
    {
        BufferDedicatedAllocation().createTestBuffer(m_pixelDataSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, m_context,
                                                     memAlloc, m_resultBuffer, MemoryRequirement::HostVisible,
                                                     m_resultBufferAlloc);
    }
    else
    {
        BufferSuballocation().createTestBuffer(m_pixelDataSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT, m_context, memAlloc,
    ...
    // Create descriptors
    {
        ...
        BufferSuballocation().createTestBuffer(uniformSize, testCase.usage, m_context, memAlloc, m_uniformBuffer,
    ...
    // Create vertex buffer
    {
        ...
        BufferSuballocation().createTestBuffer(vertexDataSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, m_context, memAlloc,
                                               m_vertexBuffer, MemoryRequirement::HostVisible, m_vertexBufferAlloc);
...
{
public:
    BufferViewTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
private:
    void checkTexelBufferSupport(Context &context, VkFormat format, VkFormatFeatureFlags feature);
    ...
    , m_bufferFormat(testCase.format)
{
    ...
    BufferSuballocation().createTestBuffer(sizeof(tcu::Vec4[4]), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, m_context,
                                           memAlloc, m_resultBuffer, MemoryRequirement::HostVisible,
                                           m_resultBufferAlloc);
    ...
    // Create descriptors
    {
        ...
        BufferSuballocation().createTestBuffer(sourceBuffer.size(), testCase.usage, m_context, memAlloc,
                                               m_uniformBuffer, MemoryRequirement::HostVisible, m_uniformBufferAlloc);
...
{
public:
    BufferViewAllFormatsTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                 BufferViewCaseParams bufferViewTestInfo)
        : vkt::TestCase(testCtx, name, description)
        , m_bufferViewTestInfo(bufferViewTestInfo)
    {
...
void BufferViewAllFormatsTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    buf << "#version 440\n"
        ...
        << "layout(set = 0, binding = 1" << fmtLayout << ") uniform highp " << extraOption << outFormat << storageType
        << " texelBuffer;\n"
...
tcu::TestCaseGroup *createBufferViewAccessTests(tcu::TestContext &testCtx)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> bufferViewTests(
        new tcu::TestCaseGroup(testCtx, "access", "BufferView Access Tests"));
    de::MovePtr<tcu::TestCaseGroup> bufferViewAllocationGroupTests[] = {
        de::MovePtr<tcu::TestCaseGroup>(
            new tcu::TestCaseGroup(testCtx, "suballocation", "BufferView Access Tests for Suballocated Objects")),
        de::MovePtr<tcu::TestCaseGroup>(new tcu::TestCaseGroup(
            testCtx, "dedicated_alloc", "BufferView Access Tests for Dedicatedly Allocated Objects"))};
    ...
    for (uint32_t buffersAllocationNdx = 0u; buffersAllocationNdx < ALLOCATION_KIND_LAST; ++buffersAllocationNdx)
        for (uint32_t imageAllocationNdx = 0u; imageAllocationNdx < ALLOCATION_KIND_LAST; ++imageAllocationNdx)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> &currentTestsGroup = bufferViewAllocationGroupTests[testCaseGroupNdx];
            {
                ...
                description << "bufferSize: " << info.bufferSize << " bufferViewSize: " << info.bufferViewSize
                            << " bufferView element offset: " << info.elementOffset;
    ...
    for (uint32_t usageNdx = 0; usageNdx < DE_LENGTH_OF_ARRAY(usage); ++usageNdx)
    {
        ...
        for (uint32_t formatIdx = 0; formatIdx < DE_LENGTH_OF_ARRAY(testFormats); formatIdx++)
        {
            ...
            if (usage[usageNdx] == VK_BUFFER_USAGE_STORAGE_TEXEL_BUFFER_BIT &&
            ...
            std::ostringstream description;
            description << "bufferFormat: " << getFormatName(testFormats[formatIdx])
                        << " bufferSize: " << info.bufferSize << " bufferViewSize: " << info.bufferViewSize
                        << " bufferView element offset: " << info.elementOffset;
