    ...
    // Create the test buffer and a memory allocation for it
    {
        ...
        while (*memory == DE_NULL)
        {
            ...
            if (size > memReqs.size)
            {
                ...
                errorMsg << "Requied memory size (" << memReqs.size << " bytes) smaller than the buffer's size ("
                         << size << " bytes)!";
...
tcu::TestStatus testOverlyLargeBuffer(Context &context, uint64_t bufferSize)
{
...
tcu::TestCaseGroup *createBufferTests(tcu::TestContext &testCtx)
{
...

    {
        ...
        addFunctionCase(
            ...
            testOverlyLargeBuffer, std::numeric_limits<uint64_t>::max());
