...
de::MovePtr<tcu::TextureLevel> CommandBufferRenderPassTestEnvironment::readColorAttachment()
{
    ...
    copyImageToBuffer(m_vkd, m_primaryCommandBuffers[0], *m_colorImage, *buffer,
...
tcu::TestStatus simultaneousUsePrimaryBufferTest(Context &context)
{
    ...
    {
        // wait for event
        vk.cmdWaitEvents(*primCmdBuf, 1u, &eventOne.get(), VK_PIPELINE_STAGE_HOST_BIT,
                         VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0u, DE_NULL, 0u, DE_NULL, 0u, DE_NULL);
...
tcu::TestStatus executeSecondaryBufferTwiceTest(Context &context)
{
    ...
    for (uint32_t ndx = 0; ndx < BUFFER_COUNT; ++ndx)
    {
        ...
        VK_CHECK(vk.beginCommandBuffer(cmdBuffers[ndx], &secCmdBufBeginInfo));
        {
            ...
            // wait for event
            vk.cmdWaitEvents(cmdBuffers[ndx], 1, &eventOne.get(), stageMask, stageMask, 0, DE_NULL, 0u, DE_NULL, 0u,
                             DE_NULL);
        }
        // end recording of secondary buffers
        endCommandBuffer(vk, cmdBuffers[ndx]);
...
{
public:
    ManyDrawsCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                  const ManyDrawsParams &params);
...
ManyDrawsCase::ManyDrawsCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             const ManyDrawsParams &params)
...
{
    de::MovePtr<tcu::TestCaseGroup> commandBuffersTests(
        new tcu::TestCaseGroup(testCtx, "command_buffers", "Command Buffers Tests"));
    ...
    addFunctionCase(commandBuffersTests.get(), "execute_small_primary", "", checkEventSupport,
    ...
    addFunctionCase(commandBuffersTests.get(), "trim_command_pool_secondary", "", checkEventSupport,
    ...
    addFunctionCase(commandBuffersTests.get(), "record_many_primary", "", checkEventSupport,
                    recordLargePrimaryBufferTest);
    addFunctionCase(commandBuffersTests.get(), "record_single_secondary", "", checkEventSupport,
                    recordSingleSecondaryBufferTest);
    addFunctionCase(commandBuffersTests.get(), "record_many_secondary", "", checkEventSupport,
                    recordLargeSecondaryBufferTest);
    {
    ...
    addFunctionCase(commandBuffersTests.get(), "submit_twice_primary", "", checkEventSupport,
                    submitPrimaryBufferTwiceTest);
    addFunctionCase(commandBuffersTests.get(), "submit_twice_secondary", "", checkEventSupport,
    ...
    addFunctionCase(commandBuffersTests.get(), "record_one_time_submit_secondary", "", checkEventSupport,
    ...
    addFunctionCase(commandBuffersTests.get(), "record_simul_use_primary", "", checkEventSupport,
                    simultaneousUsePrimaryBufferTest);
    addFunctionCase(commandBuffersTests.get(), "record_simul_use_secondary", "", checkEventSupport,
                    simultaneousUseSecondaryBufferTest);
    ...
    /* 19.4. Command Buffer Submission (5.4 in VK 1.0 Spec) */
    addFunctionCase(commandBuffersTests.get(), "submit_count_non_zero", "", checkEventSupport,
                    submitBufferCountNonZero);
    ...
    addFunctionCase(commandBuffersTests.get(), "secondary_execute_twice", "", checkEventSupport,
