...
void createDeviceWithExtension(Context &context, WorkingDevice &wd, VkQueueFlagBits testQueue, bool hostPtr)
{
    ...
    wd.allocator      = MovePtr<Allocator>(new SimpleAllocator(
        *wd.deviceDriver, *wd.logicalDevice, getPhysicalDeviceMemoryProperties(instanceDriver, physicalDevice)));
                       ...
                       const std::vector<uint32_t> &expected)
{
    ...
    const uint32_t *data = reinterpret_cast<const uint32_t *>(static_cast<const char *>(memory->getHostPtr()) + offset);
...
{
public:
    ExternalHostAllocation(Move<VkDeviceMemory> mem, void *hostPtr)
...
void createMarkerBufferMemory(const InstanceInterface &vki, const DeviceInterface &vkd, VkPhysicalDevice physicalDevice,
                              VkDevice device, VkBuffer buffer, MovePtr<Allocator> &allocator,
                              ...
                              MovePtr<ExternalHostMemory> &hostMemory, MovePtr<Allocation> &deviceMemory)
{
    ...
    else
    {
        ...
        hostMemory = MovePtr<ExternalHostMemory>(
            new ExternalHostMemory(memReqs.size, hostProps.minImportedHostPointerAlignment));
        ...
        deviceMemory =
            MovePtr<Allocation>(new ExternalHostAllocation(allocateMemory(vkd, device, &info), hostMemory->data));
    }
...
tcu::TestStatus bufferMarkerSequential(Context &context, BaseTestParams params)
{
    ...
    createMarkerBufferMemory(context.getInstanceInterface(), vk, context.getPhysicalDevice(), device, *markerBuffer,
                             wd.allocator, MemoryRequirement::HostVisible, params.useHostPtr, hostMemory, markerMemory);
    ...
    vk.cmdPipelineBarrier(*cmdBuffer, params.stage, VK_PIPELINE_STAGE_HOST_BIT, 0, 1, &memoryDep, 0, DE_NULL, 0,
                          DE_NULL);
...
void computeMemoryDepBarrier(MemoryDepMethod method, MemoryDepOwner owner, VkAccessFlags *memoryDepAccess,
...
tcu::TestStatus bufferMarkerMemoryDep(Context &context, MemoryDepParams params)
{
    ...
    Move<VkBuffer> markerBuffer(makeBuffer(vk, device, params.base.size * sizeof(uint32_t), usageFlags));
    MovePtr<ExternalHostMemory> hostMemory;
    MovePtr<Allocation> markerMemory;

    createMarkerBufferMemory(context.getInstanceInterface(), vk, context.getPhysicalDevice(), device, *markerBuffer,
                             wd.allocator, MemoryRequirement::HostVisible, params.base.useHostPtr, hostMemory,
                             markerMemory);
...
tcu::TestCaseGroup *createBufferMarkerTestsInGroup(tcu::TestContext &testCtx)
{
    ...
    {
        tcu::TestCaseGroup *queueGroup =
            (new tcu::TestCaseGroup(testCtx, queueNames[queueNdx], "Buffer marker tests for a specific queue family"));
        ...
        {
            tcu::TestCaseGroup *memoryGroup = (new tcu::TestCaseGroup(
                testCtx, memoryNames[memNdx], "Buffer marker tests for different kinds of backing memory"));
            ...
            {
                tcu::TestCaseGroup *stageGroup = (new tcu::TestCaseGroup(
                    testCtx, stageNames[stageNdx], "Buffer marker tests for a specific pipeline stage"));

                base.stage = stages[stageNdx];

                {
                    tcu::TestCaseGroup *sequentialGroup =
                        (new tcu::TestCaseGroup(testCtx, "sequential", "Buffer marker tests for sequentially writing"));

                    base.size = 4;

                    addFunctionCase(sequentialGroup, "4", "Writes 4 sequential marker values into a buffer",
                                    checkBufferMarkerSupport, bufferMarkerSequential, base);

                    base.size = 64;

                    addFunctionCase(sequentialGroup, "64", "Writes 64 sequential marker values into a buffer",
                                    checkBufferMarkerSupport, bufferMarkerSequential, base);

                    base.size = 65536;

                    addFunctionCase(sequentialGroup, "65536", "Writes 65536 sequential marker values into a buffer",
                                    checkBufferMarkerSupport, bufferMarkerSequential, base);
                ...
                {
                    tcu::TestCaseGroup *overwriteGroup = (new tcu::TestCaseGroup(
                        testCtx, "overwrite",
                        "Buffer marker tests for overwriting values with implicit synchronization"));

                    base.size = 1;

                    addFunctionCase(overwriteGroup, "1", "Randomly overwrites marker values to a 1-size buffer",
                                    checkBufferMarkerSupport, bufferMarkerOverwrite, base);

                    base.size = 4;

                    addFunctionCase(overwriteGroup, "4", "Randomly overwrites marker values to a 4-size buffer",
                                    checkBufferMarkerSupport, bufferMarkerOverwrite, base);

                    base.size = 64;

                    addFunctionCase(overwriteGroup, "64", "Randomly overwrites markers values to a 64-size buffer",
                                    checkBufferMarkerSupport, bufferMarkerOverwrite, base);
                ...
                {
                    tcu::TestCaseGroup *memoryDepGroup = (new tcu::TestCaseGroup(
                        testCtx, "memory_dep",
                        "Buffer marker tests for memory dependencies between marker writes and other operations"));
                    ...
                    if (params.base.testQueue == VK_QUEUE_GRAPHICS_BIT)
                    {
                        ...
                        addFunctionCaseWithPrograms(
                            memoryDepGroup, "draw", "Test memory dependencies between marker writes and draws",
                    ...
                    addFunctionCaseWithPrograms(memoryDepGroup, "buffer_copy",
                                                "Test memory dependencies between marker writes and buffer copies",
                                                checkBufferMarkerSupport, initMemoryDepPrograms, bufferMarkerMemoryDep,
                                                params);
