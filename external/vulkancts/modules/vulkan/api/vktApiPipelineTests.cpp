...
tcu::TestStatus drawTriangleTest(Context &context, DrawTriangleMode mode)
{
    ...
    const VkImageTiling imageTiling =
        (formatProperties.linearTilingFeatures & VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT)  ? VK_IMAGE_TILING_LINEAR :
    ...
    de::MovePtr<Allocation> attachmentImageMemory = context.getDefaultAllocator().allocate(
        getImageMemoryRequirements(vk, device, *attachmentImage), MemoryRequirement::Any);

    VK_CHECK(vk.bindImageMemory(device, *attachmentImage, attachmentImageMemory->getMemory(),
                                attachmentImageMemory->getOffset()));
    ...
    const de::UniquePtr<Allocation> readImageBufferMemory(context.getDefaultAllocator().allocate(
        getBufferMemoryRequirements(vk, device, *readImageBuffer), MemoryRequirement::HostVisible));
    ...
    const Unique<VkShaderModule> fragmentShaderModule(
        createShaderModule(vk, device, context.getBinaryCollection().get("fragment"), 0));
    ...
    const Unique<VkPipeline> graphicsPipeline(makeGraphicsPipeline(
    ...
    beginRenderPass(vk, *commandBuffer, renderPassB.get(), frameBuffer.get(), makeRect2D(0, 0, 256u, 256u),
                    tcu::Vec4(0.25f, 0.25f, 0.25f, 0.0f));
...
void changeColorAttachmentImageLayout(const DeviceInterface &vk, VkCommandBuffer commandBuffer, VkImage image)
{
    ...
    vk.cmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                          (VkDependencyFlagBits)0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &imageMemoryBarrier);
}

Move<VkRenderPass> createSimpleRenderPass(const DeviceInterface &vk, const VkDevice &device, VkFormat format,
                                          VkAttachmentLoadOp loadOp, VkAttachmentLoadOp stencilLoadOp,
                                          VkAttachmentStoreOp stencilStoreOp, VkImageLayout layout)
{
    const VkAttachmentDescription attachmentDescription = {
        (VkAttachmentDescriptionFlags)0u, // VkAttachmentDescriptionFlags    flags;
        format,                           // VkFormat                        format;
...
tcu::TestStatus framebufferCompatibleRenderPassTest(Context &context)
{
    ...
    Unique<VkRenderPass> renderPassA(
        createSimpleRenderPass(vk, device, format, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_LOAD_OP_DONT_CARE,
    ...
    const Unique<VkRenderPass> renderPassB(
        createSimpleRenderPass(vk, device, format, VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_LOAD_OP_LOAD,
...
Move<VkDescriptorSetLayout> getDescriptorSetLayout(const DeviceInterface &vk, const VkDevice &device,
...
VkDescriptorSet getDescriptorSet(const DeviceInterface &vk, const VkDevice &device, VkDescriptorPool descriptorPool,
...
Move<VkPipelineLayout> getPipelineLayout(const DeviceInterface &vk, const VkDevice &device, uint32_t setLayoutCount,
...
Move<VkPipeline> createSimpleGraphicsPipeline(const DeviceInterface &vk, const VkDevice &device,
                                              ...
                                              de::SharedPtr<vk::ResourceInterface> resourceInterface)
{
    ...
    const VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo = {
        ...
        numShaderStages,                                 // uint32_t                                         stageCount;
...
tcu::TestStatus pipelineLayoutLifetimeTest(Context &context, VkPipelineBindPoint bindPoint)
{
    ...
    if (isGraphics)
    {
        ...
        attachmentImageMemory = context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vk, device, *attachmentImage), MemoryRequirement::Any);
    ...
    const VkDescriptorPoolCreateInfo descriptorPoolCreateInfo = {
        ...
        (isGraphics ? 3u : 5u),                        // uint32_t                       maxSets;
        DE_LENGTH_OF_ARRAY(descriptorPoolSizes),       // uint32_t                       poolSizeCount;
    ...
    const VkDescriptorSetLayout setLayoutHandlesAC[] = {descriptorSetLayouts[0].get(), descriptorSetLayouts[2].get()};

    const VkDescriptorSetLayout setLayoutHandlesB[] = {descriptorSetLayouts[1].get()};

    const VkDescriptorSetLayout setLayoutHandlesBC[] = {descriptorSetLayouts[1].get(), descriptorSetLayouts[2].get()};
    ...
    const VkDescriptorSet setHandlesAC[] = {descriptorSets[0], descriptorSets[2]};

    const VkDescriptorSet setHandlesC[] = {descriptorSets[2]};

    const Unique<VkPipelineLayout> pipelineLayoutAC(
        getPipelineLayout(vk, device, DE_LENGTH_OF_ARRAY(setLayoutHandlesAC), setLayoutHandlesAC));
    const Unique<VkPipelineLayout> pipelineLayoutBC(
        getPipelineLayout(vk, device, DE_LENGTH_OF_ARRAY(setLayoutHandlesBC), setLayoutHandlesBC));
    ...
    VkPipelineLayout pipelineLayoutB;
    {
        const VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = {
            ...
            DE_NULL,                                       // const void*                     pNext;
            (VkPipelineLayoutCreateFlags)0u,               // VkPipelineLayoutCreateFlags     flags;
            DE_LENGTH_OF_ARRAY(setLayoutHandlesB),         // uint32_t                        setLayoutCount;
    ...
    if (isGraphics)
    {
        ...
        pipeline =
            createSimpleGraphicsPipeline(vk, device, DE_LENGTH_OF_ARRAY(shaderStageCreateInfos), shaderStageCreateInfos,
                                         pipelineLayoutB, renderPass.get(), context.getResourceInterface());
    }
    else
    {
    ...
    if (isGraphics)
    {
        beginRenderPass(vk, commandBuffer.get(), renderPass.get(), frameBuffer.get(), makeRect2D(0, 0, 256u, 256u),
                        tcu::Vec4(0.25f, 0.25f, 0.25f, 0.0f));
    }
    ...
    vk.cmdBindDescriptorSets(commandBuffer.get(), bindPoint, pipelineLayoutAC.get(), 0u,
                             DE_LENGTH_OF_ARRAY(setHandlesAC), setHandlesAC, 0u, DE_NULL);
    vk.cmdBindDescriptorSets(commandBuffer.get(), bindPoint, pipelineLayoutBC.get(), 1u,
                             DE_LENGTH_OF_ARRAY(setHandlesC), setHandlesC, 0u, DE_NULL);
...
{
    dst.glslSources.add("vertex") << glu::VertexSource("#version 450\n"
                                                       "\n"
                                                       "void main (void)\n"
                                                       "{\n"
                                                       "   gl_Position = vec4(1);\n"
...
tcu::TestStatus destroyEarlyTest(Context &context, DestroyPipelineLayoutMode mode)
{
    ...
    const uint32_t kBufferElements = 100u;
    const uint32_t kBufferSize     = kBufferElements * sizeof(uint32_t);
    const auto kBufferSizeDS       = static_cast<VkDeviceSize>(kBufferSize);
    const uint32_t kInitialValue   = 50u;
    const uint32_t kFlagValue      = 1u;
    const uint32_t kBaseValue      = 75u;
    ...
    {
        const std::vector<uint32_t> bufferValues(kBufferElements, kInitialValue);
    ...
    const auto pushConstantRange =
        vk::makePushConstantRange(vk::VK_SHADER_STAGE_COMPUTE_BIT, 0u, static_cast<uint32_t>(sizeof(kBaseValue)));
    ...
    const auto specConstantSize = static_cast<uintptr_t>(sizeof(kFlagValue));

    const vk::VkSpecializationMapEntry mapEntry = {
        0u,               // uint32_t constantID;
        0u,               // uint32_t offset;
        specConstantSize, // uintptr_t size;
    };
    ...
    const vk::VkSpecializationInfo specializationInfo = {
        ...
        specConstantSize, // uintptr_t dataSize;
    ...
    vkd.cmdPushConstants(cmdBuffer, pipelineLayout.get(), vk::VK_SHADER_STAGE_COMPUTE_BIT, 0u,
                         static_cast<uint32_t>(sizeof(kBaseValue)), &kBaseValue);
    vkd.cmdDispatch(cmdBuffer, kBufferElements, 1u, 1u);
    vkd.cmdPipelineBarrier(cmdBuffer, vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk::VK_PIPELINE_STAGE_HOST_BIT, 0u, 1u,
    ...
    std::vector<uint32_t> outputData(kBufferElements);
    ...
    for (uint32_t i = 0; i < kBufferElements; ++i)
    {
        ...
        if (outputData[i] != expectedValue)
        {
            ...
            msg << "Unexpected value at buffer position " << i << ": expected " << expectedValue << " but found "
                << outputData[i];
...
Move<VkPipeline> createSimpleGraphicsPipelineInvalidPointers(
    const DeviceInterface &vk, const VkDevice &device, uint32_t numShaderStages,
    const VkPipelineShaderStageCreateInfo *shaderStageCreateInfos, VkPipelineLayout pipelineLayout,
    VkRenderPass renderPass, de::SharedPtr<vk::ResourceInterface> resourceInterface)
{
#ifndef CTS_USES_VULKANSC
    DE_UNREF(resourceInterface);
#endif // CTS_USES_VULKANSC
    ...
    const VkPipelineVertexInputStateCreateInfo vertexInputStateCreateInfo = {
        ...
        (const VkVertexInputAttributeDescription *)
            invalidPointer // const VkVertexInputAttributeDescription*    pVertexAttributeDescriptions;
    };
    ...
    const VkGraphicsPipelineCreateInfo graphicsPipelineCreateInfo = {
        ...
        (const VkPipelineViewportStateCreateInfo *)
            invalidPointer,            // const VkPipelineViewportStateCreateInfo*         pViewportState;
        &rasterizationStateCreateInfo, // const VkPipelineRasterizationStateCreateInfo*    pRasterizationState;
        (const VkPipelineMultisampleStateCreateInfo *)
            invalidPointer, // const VkPipelineMultisampleStateCreateInfo*      pMultisampleState;
        (const VkPipelineDepthStencilStateCreateInfo *)
            invalidPointer, // const VkPipelineDepthStencilStateCreateInfo*     pDepthStencilState;
        (const VkPipelineColorBlendStateCreateInfo *)
            invalidPointer, // const VkPipelineColorBlendStateCreateInfo*       pColorBlendState;
...
tcu::TestStatus pipelineInvalidPointersUnusedStructsTest(Context &context, VkPipelineBindPoint bindPoint)
{
...

    {
        const VkSubpassDescription subpassDescription = {
            ...
            DE_NULL,                                       // const VkAttachmentReference*    pDepthStencilAttachment
            0u,                                            // uint32_t                        preserveAttachmentCount
            (const uint32_t *)invalidPointer               // const uint32_t*                 pPreserveAttachments
        };
    ...
    if (isGraphics)
    {
        ...
        pipeline = createSimpleGraphicsPipelineInvalidPointers(vk, device, DE_LENGTH_OF_ARRAY(shaderStageCreateInfos),
                                                               shaderStageCreateInfos, *pipelineLayout,
                                                               renderPass.get(), context.getResourceInterface());
    }
    else
    {
        shaderModules.push_back(createShaderModule(vk, device, context.getBinaryCollection().get("compute"), 0));
...
tcu::TestCaseGroup *createrenderpassTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(renderPassTests.get(), "destroy_pipeline_renderpass",
                                "Draw after destroying the renderpass used to create a pipeline", checkSupport,
                                createDrawTriangleSource, renderpassLifetimeTest);
    addFunctionCase(renderPassTests.get(), "framebuffer_compatible_renderpass",
                    "Use a render pass with a framebuffer that was created using another compatible render pass",
                    checkSupport, framebufferCompatibleRenderPassTest);
...
{
    de::MovePtr<tcu::TestCaseGroup> pipelineLayoutLifetimeTests(
        new tcu::TestCaseGroup(testCtx, "lifetime", "Pipeline layout lifetime tests"));

    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "graphics",
                                "Test pipeline layout lifetime in graphics pipeline", checkSupport,
                                createPipelineLayoutLifetimeGraphicsSource, pipelineLayoutLifetimeGraphicsTest);
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "compute",
                                "Test pipeline layout lifetime in compute pipeline", checkSupport,
                                createPipelineLayoutLifetimeComputeSource, pipelineLayoutLifetimeComputeTest);
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "destroy_after_end",
                                "Test destroying the pipeline layout after vkEndCommandBuffer",
                                destroyEarlyComputeSource, destroyAfterEndCommndBufferTest);
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "destroy_after_compute_pipeline_construction",
                                "Test destroying the pipeline layout after compute pipeline creation",
                                checkMaintenance4Support, destroyEarlyComputeSource,
                                destroyAfterCreateComputePipelineTest);
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "destroy_after_graphics_pipeline_construction",
                                "Test destroying the pipeline layout after graphics pipeline creation",
                                checkMaintenance4Support, createDrawTriangleSource,
                                destroyAfterCreateGraphicsPipelineTest);
...
{
    de::MovePtr<tcu::TestCaseGroup> pipelineLayoutTests(
        new tcu::TestCaseGroup(testCtx, "pipeline_layout", "Pipeline layout tests"));
...
{
    de::MovePtr<tcu::TestCaseGroup> pipelineInvalidPointersUnusedStructsTests(
        new tcu::TestCaseGroup(testCtx, "pipeline_invalid_pointers_unused_structs",
                               "Create pipelines with invalid pointers for unused structs"));

    addFunctionCaseWithPrograms(pipelineInvalidPointersUnusedStructsTests.get(), "graphics",
                                "Test structs when creating a graphics pipeline", checkSupport,
                                createPipelineInvalidPointersUnusedStructsGraphicsSource,
                                pipelineInvalidPointersUnusedStructsGraphicsTest);
    addFunctionCaseWithPrograms(pipelineInvalidPointersUnusedStructsTests.get(), "compute",
                                "Test structs when creating a compute pipeline", checkSupport,
                                createPipelineInvalidPointersUnusedStructsComputeSource,
                                pipelineInvalidPointersUnusedStructsComputeTest);
