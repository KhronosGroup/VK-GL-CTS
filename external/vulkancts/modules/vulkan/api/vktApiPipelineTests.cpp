...
tcu::TestStatus pipelineLayoutLifetimeTest(Context &context, VkPipelineBindPoint bindPoint)
{
    ...
    if (isGraphics)
    {
        ...
        pipeline = createSimpleGraphicsPipeline(vk, device, DE_LENGTH_OF_ARRAY(shaderStageCreateInfos),
                                                shaderStageCreateInfos, pipelineLayoutB, renderPass.get());
    }
    else
    {
...
tcu::TestStatus pipelineInvalidPointersUnusedStructsTest(Context &context, VkPipelineBindPoint bindPoint)
{
    ...
    if (isGraphics)
    {
        ...
        pipeline =
            createSimpleGraphicsPipelineInvalidPointers(vk, device, DE_LENGTH_OF_ARRAY(shaderStageCreateInfos),
                                                        shaderStageCreateInfos, *pipelineLayout, renderPass.get());
    }
    else
    {
        shaderModules.push_back(createShaderModule(vk, device, context.getBinaryCollection().get("compute"), 0));
