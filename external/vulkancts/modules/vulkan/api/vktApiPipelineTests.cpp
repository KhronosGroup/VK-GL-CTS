...
tcu::TestStatus drawTriangleTest(Context &context, DrawTriangleMode mode)
{
    ...
    const Unique<VkCommandPool> commandPool(createCommandPool(vk, device, &commandPoolParams, nullptr));
    ...
    if (DTM_DESTROY_RENDER_PASS_AFTER_CREATING_PIPELINE == mode)
        VK_CHECK(vk.createRenderPass(device, &renderPassCreateInfo, nullptr, &renderPassA));
    ...
    if (DTM_DESTROY_RENDER_PASS_AFTER_CREATING_PIPELINE == mode)
        vk.destroyRenderPass(device, renderPassA, nullptr);
...
void changeColorAttachmentImageLayout(const DeviceInterface &vk, VkCommandBuffer commandBuffer, VkImage image)
{
    ...
    vk.cmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                          (VkDependencyFlagBits)0u, 0u, nullptr, 0u, nullptr, 1u, &imageMemoryBarrier);
}
...
tcu::TestStatus pipelineLayoutLifetimeTest(Context &context, VkPipelineBindPoint bindPoint)
{
    ...
    Move<VkDescriptorPool> descriptorPool = createDescriptorPool(vk, device, &descriptorPoolCreateInfo, nullptr);
    ...
    VkPipelineLayout pipelineLayoutB;
    {
        ...
        VK_CHECK(vk.createPipelineLayout(device, &pipelineLayoutCreateInfo, nullptr, &pipelineLayoutB));
    }
    ...
    vk.destroyPipelineLayout(device, pipelineLayoutB, nullptr);

    vk.cmdBindDescriptorSets(commandBuffer.get(), bindPoint, pipelineLayoutAC.get(), 0u,
                             DE_LENGTH_OF_ARRAY(setHandlesAC), setHandlesAC, 0u, nullptr);
    vk.cmdBindDescriptorSets(commandBuffer.get(), bindPoint, pipelineLayoutBC.get(), 1u,
                             DE_LENGTH_OF_ARRAY(setHandlesC), setHandlesC, 0u, nullptr);
...
tcu::TestCaseGroup *createrenderpassTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(renderPassTests.get(), "destroy_pipeline_renderpass", checkSupport,
    ...
    addFunctionCase(renderPassTests.get(), "framebuffer_compatible_renderpass", checkSupport,
                    framebufferCompatibleRenderPassTest);
...
tcu::TestCaseGroup *createPipelineLayoutLifetimeTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "graphics", checkSupport,
                                createPipelineLayoutLifetimeGraphicsSource, pipelineLayoutLifetimeGraphicsTest);
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "compute", checkSupport,
                                createPipelineLayoutLifetimeComputeSource, pipelineLayoutLifetimeComputeTest);
    addFunctionCaseWithPrograms(pipelineLayoutLifetimeTests.get(), "destroy_after_end", destroyEarlyComputeSource,
                                destroyAfterEndCommndBufferTest);
...
tcu::TestCaseGroup *createPipelineInvalidPointersUnusedStructsTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(pipelineInvalidPointersUnusedStructsTests.get(), "graphics", checkSupport,
                                createPipelineInvalidPointersUnusedStructsGraphicsSource,
                                pipelineInvalidPointersUnusedStructsGraphicsTest);
    addFunctionCaseWithPrograms(pipelineInvalidPointersUnusedStructsTests.get(), "compute", checkSupport,
