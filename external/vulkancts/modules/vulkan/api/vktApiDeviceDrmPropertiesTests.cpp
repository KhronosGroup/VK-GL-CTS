...
void testFilesExist(const VkPhysicalDeviceDrmPropertiesEXT &deviceDrmProperties)
{
    ...
    if (libDrm.findDeviceNode(drmDevices, numDrmDevices, deviceDrmProperties.primaryMajor,
                              deviceDrmProperties.primaryMinor))
        primaryFound = true;
    if (libDrm.findDeviceNode(drmDevices, numDrmDevices, deviceDrmProperties.renderMajor,
...
static tcu::TestStatus testDeviceDrmProperties(Context &context, const TestType testType)
{
    ...
    case TEST_FILES_EXIST:
        testFilesExist(deviceDrmProperties);
        break;
    default:
...
{
    addFunctionCase(group, "drm_files_exist", "Verify device files for major/minor nodes exist", checkSupport,
                    testDeviceDrmProperties, TEST_FILES_EXIST);
}
