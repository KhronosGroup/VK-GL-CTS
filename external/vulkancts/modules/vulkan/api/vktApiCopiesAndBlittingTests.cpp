...
tcu::TestStatus CopyImageToImage::checkTestResult(tcu::ConstPixelBufferAccess result)
{
    ...
    else
    {
        ...
        else if (isSnormFormat(mapTextureFormat(result.getFormat())))
        {
            ...
            tcu::TextureLevel expectedSnorm(
                m_expectedTextureLevel[0]->getFormat(), m_expectedTextureLevel[0]->getWidth(),
                m_expectedTextureLevel[0]->getHeight(), m_expectedTextureLevel[0]->getDepth());
...
tcu::TestStatus BlittingImages::iterate(void)
{
    ...
    m_destinationTextureLevel = de::MovePtr<tcu::TextureLevel>(
        new tcu::TextureLevel(dstTcuFormat, (int)m_params.dst.image.extent.width, (int)m_params.dst.image.extent.height,
                              (int)m_params.dst.image.extent.depth));
    generateBuffer(m_destinationTextureLevel->getAccess(), m_params.dst.image.extent.width,
                   m_params.dst.image.extent.height, m_params.dst.image.extent.depth, FILL_MODE_WHITE);
    generateExpectedResult();
...
void addImageToImageSimpleTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        group->addChild(new CopyImageToImageTestCase(
            testCtx, "partial_image_npot_diff_format_clear",
            "Partial image with npot dimensions, different format, and clearing of the destination image", params));
    }
