...
void CopiesAndBlittingTestInstance::uploadBuffer(tcu::ConstPixelBufferAccess bufferAccess,
...
{
public:
    CopiesAndBlittingTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description)
                                                    ...
                                                    const ImageParms &imageParms, const uint32_t mipLevel)
{
    ...
    const VkBufferImageCopy copyRegion = {
        0u,                 // VkDeviceSize bufferOffset;
        imageExtent.width,  // uint32_t bufferRowLength;
        imageExtent.height, // uint32_t bufferImageHeight;
        {
                                                                        ...
                                                                        const uint32_t mipLevel)
{
    ...
    if (tcu::isCombinedDepthStencilType(imageFormat.type))
    {
        ...
        {
            tcu::TextureLevel depthTexture(mapCombinedToDepthTransferFormat(imageFormat), parms.extent.width,
        ...
        {
            tcu::TextureLevel stencilTexture(
                tcu::getEffectiveDepthStencilTextureFormat(imageFormat, tcu::Sampler::MODE_STENCIL), parms.extent.width,
                parms.extent.height, parms.extent.depth);
...
{
public:
    ResolveImageToImage(Context &context, TestParams params, const ResolveImageToImageOptions options);
    virtual tcu::TestStatus iterate(void);
    ...
    , m_options(options)
{
    ...
    // Create color image.
    {
        ...
        m_multisampledImageAlloc = allocateImage(vki, vk, vkPhysDevice, vkDevice, *m_multisampledImage,
                                                 MemoryRequirement::Any, memAlloc, m_params.allocationKind);
        VK_CHECK(vk.bindImageMemory(vkDevice, *m_multisampledImage, m_multisampledImageAlloc->getMemory(),
                                    m_multisampledImageAlloc->getOffset()));
    ...
    // Barriers for copying image to buffer
    VkImageMemoryBarrier srcImageBarrier = {VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,   // VkStructureType sType;
                                            DE_NULL,                                  // const void* pNext;
                                            0u,                                       // VkAccessFlags srcAccessMask;
...
tcu::TestStatus ResolveImageToImage::iterate(void)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdResolveImage(*m_cmdBuffer, sourceImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_destination.get(),
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, (uint32_t)m_params.regions.size(),
                           imageResolves.data());
    }
    else
...
tcu::TestStatus ResolveImageToImage::checkTestResult(tcu::ConstPixelBufferAccess result)
{
    ...
    for (int arrayLayerNdx = 0; arrayLayerNdx < (int)getArraySize(m_params.dst.image); ++arrayLayerNdx)
    {
        const tcu::ConstPixelBufferAccess expectedSub =
            getSubregion(expected, 0, 0, arrayLayerNdx, expected.getWidth(), expected.getHeight(), 1u);
        const tcu::ConstPixelBufferAccess resultSub =
            getSubregion(result, 0, 0, arrayLayerNdx, result.getWidth(), result.getHeight(), 1u);
...
void ResolveImageToImage::copyMSImageToMSImage(uint32_t copyArraySize)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdCopyImage(*m_cmdBuffer, m_multisampledImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                        m_multisampledCopyImage.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                          (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                          (const VkBufferMemoryBarrier *)DE_NULL, 1u, &postImageBarriers);
    endCommandBuffer(vk, *m_cmdBuffer);
...
const int32_t defaultFourthSize    = defaultSize / 4;
                                         ...
                                         ExtensionUse extensionUse)
{
    ...
    // 2D tests.
    {
        ...
        for (int32_t i = 0; i < defaultSize; i += defaultFourthSize)
        {
            const VkImageCopy testCopy = {
                defaultSourceLayer,                          // VkImageSubresourceLayers srcSubresource;
                {0, 0, 0},                                   // VkOffset3D srcOffset;
    ...
    // 1D tests.
    {
        ...
        for (int32_t i = defaultFourthSize; i < defaultSize; i += defaultSize / 2)
        {
            const VkImageCopy testCopy = {
                defaultSourceLayer,        // VkImageSubresourceLayers srcSubresource;
                {0, 0, 0},                 // VkOffset3D srcOffset;
    ...
    // 3D tests. Note we use smaller dimensions here for performance reasons.
    {
        ...
        for (int32_t i = 0; i < defaultFourthSize; i += defaultSixteenthSize)
        {
            const VkImageCopy testCopy = {
                defaultSourceLayer,                                   // VkImageSubresourceLayers srcSubresource;
                {0, 0, 0},                                            // VkOffset3D srcOffset;
...
void addImageToBufferTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        {
            const uint32_t bufferRowLength   = defaultFourthSize;
            const uint32_t bufferImageHeight = defaultFourthSize;
...
void addBufferToImageTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        CopyRegion region;
        uint32_t divisor = 1;
        for (int offset = 0; (offset + defaultFourthSize / divisor < defaultSize) && (defaultFourthSize > divisor);
             offset += defaultFourthSize / divisor++)
        {
            const VkBufferImageCopy bufferImageCopy = {
                0u,                           // VkDeviceSize bufferOffset;
                0u,                           // uint32_t bufferRowLength;
...
void addBlittingImageSimpleScalingAndOffsetTests(tcu::TestCaseGroup *group, TestParams params)
{
    DE_ASSERT(params.src.image.imageType == params.dst.image.imageType);
    const int32_t imageDepth      = params.src.image.imageType == VK_IMAGE_TYPE_3D ? defaultSize : 1;
    const int32_t srcDepthOffset  = params.src.image.imageType == VK_IMAGE_TYPE_3D ? defaultFourthSize : 0;
    const int32_t srcDepthSize    = params.src.image.imageType == VK_IMAGE_TYPE_3D ? defaultFourthSize * 3 : 1;
    params.src.image.extent       = defaultExtent;
    params.dst.image.extent       = defaultExtent;
    params.src.image.extent.depth = imageDepth;
    params.dst.image.extent.depth = imageDepth;

    {
        const VkImageBlit imageBlit = {
            ...
            {{defaultFourthSize, defaultFourthSize, srcDepthOffset},
             {defaultFourthSize * 3, defaultFourthSize * 3, srcDepthSize}}, // VkOffset3D srcOffsets[2];
...
void addBlittingImageSimpleWithoutScalingPartialTests(tcu::TestCaseGroup *group, TestParams params)
{
...

    {
        ...
        for (int i = 0; i < defaultSize; i += defaultFourthSize)
        {
            const VkImageBlit imageBlit = {
                defaultSourceLayer, // VkImageSubresourceLayers srcSubresource;
                {{defaultSize - defaultFourthSize - i, defaultSize - defaultFourthSize - i,
                  is3dBlit ? defaultSize - defaultFourthSize - i : 0},
                 {defaultSize - i, defaultSize - i, is3dBlit ? defaultSize - i : 1}}, // VkOffset3D srcOffsets[2];

                defaultSourceLayer, // VkImageSubresourceLayers dstSubresource;
                {{i, i, is3dBlit ? i : 0},
                 {i + defaultFourthSize, i + defaultFourthSize,
                  is3dBlit ? i + defaultFourthSize : 1}} // VkOffset3D dstOffset[2];
            };
            region.imageBlit = imageBlit;
            params.regions.push_back(region);
        }
...
const VkFormat compatibleFormatsFloats[] = {
    ...
    VK_FORMAT_B5G5R5A1_UNORM_PACK16, VK_FORMAT_A1R5G5B5_UNORM_PACK16, VK_FORMAT_R8_UNORM, VK_FORMAT_R8_SNORM,
    VK_FORMAT_R8_USCALED, VK_FORMAT_R8_SSCALED, VK_FORMAT_R8G8_UNORM, VK_FORMAT_R8G8_SNORM, VK_FORMAT_R8G8_USCALED,
    VK_FORMAT_R8G8_SSCALED, VK_FORMAT_R8G8B8_UNORM, VK_FORMAT_R8G8B8_SNORM, VK_FORMAT_R8G8B8_USCALED,
    VK_FORMAT_R8G8B8_SSCALED, VK_FORMAT_B8G8R8_UNORM, VK_FORMAT_B8G8R8_SNORM, VK_FORMAT_B8G8R8_USCALED,
    VK_FORMAT_B8G8R8_SSCALED, VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_R8G8B8A8_SNORM, VK_FORMAT_R8G8B8A8_USCALED,
    VK_FORMAT_R8G8B8A8_SSCALED, VK_FORMAT_B8G8R8A8_UNORM, VK_FORMAT_B8G8R8A8_SNORM, VK_FORMAT_B8G8R8A8_USCALED,
    VK_FORMAT_B8G8R8A8_SSCALED, VK_FORMAT_A8B8G8R8_UNORM_PACK32, VK_FORMAT_A8B8G8R8_SNORM_PACK32,
    VK_FORMAT_A8B8G8R8_USCALED_PACK32, VK_FORMAT_A8B8G8R8_SSCALED_PACK32, VK_FORMAT_A2R10G10B10_UNORM_PACK32,
    VK_FORMAT_A2R10G10B10_SNORM_PACK32, VK_FORMAT_A2R10G10B10_USCALED_PACK32, VK_FORMAT_A2R10G10B10_SSCALED_PACK32,
    VK_FORMAT_A2B10G10R10_UNORM_PACK32, VK_FORMAT_A2B10G10R10_SNORM_PACK32, VK_FORMAT_A2B10G10R10_USCALED_PACK32,
    VK_FORMAT_A2B10G10R10_SSCALED_PACK32, VK_FORMAT_R16_UNORM, VK_FORMAT_R16_SNORM, VK_FORMAT_R16_USCALED,
    VK_FORMAT_R16_SSCALED, VK_FORMAT_R16_SFLOAT, VK_FORMAT_R16G16_UNORM, VK_FORMAT_R16G16_SNORM,
    VK_FORMAT_R16G16_USCALED, VK_FORMAT_R16G16_SSCALED, VK_FORMAT_R16G16_SFLOAT, VK_FORMAT_R16G16B16_UNORM,
    VK_FORMAT_R16G16B16_SNORM, VK_FORMAT_R16G16B16_USCALED, VK_FORMAT_R16G16B16_SSCALED, VK_FORMAT_R16G16B16_SFLOAT,
    VK_FORMAT_R16G16B16A16_UNORM, VK_FORMAT_R16G16B16A16_SNORM, VK_FORMAT_R16G16B16A16_USCALED,
    VK_FORMAT_R16G16B16A16_SSCALED, VK_FORMAT_R16G16B16A16_SFLOAT, VK_FORMAT_R32_SFLOAT, VK_FORMAT_R32G32_SFLOAT,
    VK_FORMAT_R32G32B32_SFLOAT, VK_FORMAT_R32G32B32A32_SFLOAT, VK_FORMAT_R64_SFLOAT, VK_FORMAT_R64G64_SFLOAT,
    VK_FORMAT_R64G64B64_SFLOAT, VK_FORMAT_R64G64B64A64_SFLOAT, VK_FORMAT_B10G11R11_UFLOAT_PACK32,
                                          ...
                                          ExtensionUse extensionUse)
{
    ...
    // 2D tests.
    {
        ...
        CopyRegion region;
        for (int i = 0, j = 1; (i + defaultFourthSize / j < defaultSize) && (defaultFourthSize > j);
             i += defaultFourthSize / j++)
        {
            const VkImageBlit imageBlit = {
                defaultSourceLayer,                         // VkImageSubresourceLayers srcSubresource;
                ...
                defaultSourceLayer, // VkImageSubresourceLayers dstSubresource;
                {{i, 0, 0}, {i + defaultFourthSize / j, defaultFourthSize / j, 1}} // VkOffset3D dstOffset[2];
            };
            region.imageBlit = imageBlit;
            params.regions.push_back(region);
        }
        for (int i = 0; i < defaultSize; i += defaultFourthSize)
        {
            const VkImageBlit imageBlit = {
                defaultSourceLayer, // VkImageSubresourceLayers srcSubresource;
                {{i, i, 0}, {i + defaultFourthSize, i + defaultFourthSize, 1}}, // VkOffset3D srcOffsets[2];

                defaultSourceLayer, // VkImageSubresourceLayers dstSubresource;
                {{i, defaultSize / 2, 0},
                 {i + defaultFourthSize, defaultSize / 2 + defaultFourthSize, 1}} // VkOffset3D dstOffset[2];
            };
            region.imageBlit = imageBlit;
            params.regions.push_back(region);
        }
    ...
    // 1D tests.
    {
        ...
        for (int i = 0; i < defaultSize; i += defaultSize / 2)
        {
            const VkImageBlit imageBlit = {
                ...
                defaultSourceLayer,                        // VkImageSubresourceLayers dstSubresource;
                {{i, 0, 0}, {i + defaultFourthSize, 1, 1}} // VkOffset3D dstOffset[2];
            };
            region.imageBlit = imageBlit;
            params.regions.push_back(region);
        }

        {
            const VkImageBlit imageBlit = {
                defaultSourceLayer,                     // VkImageSubresourceLayers srcSubresource;
                {{0, 0, 0}, {defaultFourthSize, 1, 1}}, // VkOffset3D srcOffsets[2];

                defaultSourceLayer,                                        // VkImageSubresourceLayers dstSubresource;
                {{defaultFourthSize, 0, 0}, {2 * defaultFourthSize, 1, 1}} // VkOffset3D dstOffset[2];
            };
            region.imageBlit = imageBlit;
            params.regions.push_back(region);
        }
    ...
    // 3D tests. Note we use smaller dimensions here for performance reasons.
    {
        ...
        CopyRegion region;
        for (int i = 0, j = 1; (i + defaultSixteenthSize / j < defaultFourthSize) && (defaultSixteenthSize > j);
             i += defaultSixteenthSize / j++)
        {
            const VkImageBlit imageBlit = {
                defaultSourceLayer, // VkImageSubresourceLayers srcSubresource;
                {{0, 0, 0}, {defaultFourthSize, defaultFourthSize, defaultFourthSize}}, // VkOffset3D srcOffsets[2];
        ...
        for (int i = 0; i < defaultFourthSize; i += defaultSixteenthSize)
        {
            const VkImageBlit imageBlit = {
                ...
                {{i, defaultFourthSize / 2, i},
                 {i + defaultSixteenthSize, defaultFourthSize / 2 + defaultSixteenthSize,
    ...
    // 3D tests. Note we use smaller dimensions here for performance reasons.
    {
             ...
             ++compatibleFormatsIndex)
        {
            ...
            for (int i = 0; i < defaultFourthSize; i += defaultSixteenthSize)
            {
                ...
                const VkOffset3D dstOffset1 = {i + defaultSixteenthSize, defaultFourthSize / 2 + defaultSixteenthSize,
                                               i + defaultSixteenthSize};
                ...
                if (hasDepth && hasStencil)
                {
                    ...
                    const VkOffset3D dstDSOffset1 = {i + defaultSixteenthSize, defaultFourthSize,
                                                     i + defaultSixteenthSize};
                    const VkImageBlit imageBlit   = {
                        defaultDSSourceLayer,        // VkImageSubresourceLayers srcSubresource;
                        {srcOffset0, srcOffset1},    // VkOffset3D srcOffsets[2];
