...
tcu::TestStatus CopyImageToImage::iterate(void)
{
    ...
    generateBuffer(m_destinationTextureLevel->getAccess(), m_params.dst.image.extent.width,
                   m_params.dst.image.extent.height, m_params.dst.image.extent.depth, FILL_MODE_GRADIENT);
...
tcu::TestStatus BlittingImages::checkTestResult(tcu::ConstPixelBufferAccess result)
{
    ...
    if (m_params.filter == VK_FILTER_LINEAR)
    {
        ...
        else
        {
            ...
            if (!checkLinearFilteredResult(result, m_expectedTextureLevel[0]->getAccess(),
...
void scaleFromWholeSrcBuffer(const tcu::PixelBufferAccess &dst, const tcu::ConstPixelBufferAccess &src,
                             ...
                             tcu::Sampler::FilterMode filter, const MirrorMode mirrorMode = MIRROR_MODE_NONE)
{
    ...
    for (int y = 0; y < dst.getHeight(); y++)
        ...
        {
            float srcX = (mirrorMode == MIRROR_MODE_X || mirrorMode == MIRROR_MODE_XY) ?
                             (float)regionExtent.x + (float)regionOffset.x - ((float)x + 0.5f) * sX :
                             (float)regionOffset.x + ((float)x + 0.5f) * sX;
            float srcY = (mirrorMode == MIRROR_MODE_Y || mirrorMode == MIRROR_MODE_XY) ?
        ...
        for (int y = 0; y < dst.getHeight(); ++y)
            for (int x = 0; x < dst.getWidth(); ++x)
            {
                ...
                dst.setPixel(linearToSRGBIfNeeded(dst.getFormat(), src.sample2D(sampler, filter, ((float)x + 0.5f) * sX,
                                                                                ((float)y + 0.5f) * sY, 0)),
                             (int)round(xy[0]) + xOffset, (int)round(xy[1]) + yOffset);
            }
    }
    else
    {
            ...
            for (int y = 0; y < dst.getHeight(); ++y)
                for (int x = 0; x < dst.getWidth(); ++x)
                {
                    ...
                    dst.setPixel(linearToSRGBIfNeeded(dst.getFormat(),
                                 ...
                                 (int)round(xy[0]) + xOffset, (int)round(xy[1]) + yOffset, z);
                }
    }
}
...
MirrorMode getMirrorMode(const VkOffset3D s1, const VkOffset3D s2, const VkOffset3D d1, const VkOffset3D d2)
{
    ...
    else if ((source == MIRROR_MODE_XY && destination == MIRROR_MODE_X) ||
             (destination == MIRROR_MODE_XY && source == MIRROR_MODE_X) ||
             (source == MIRROR_MODE_Y && destination == MIRROR_MODE_NONE) ||
             (destination == MIRROR_MODE_Y && source == MIRROR_MODE_NONE))
    {
        return MIRROR_MODE_Y;
    }
    else if ((source == MIRROR_MODE_XY && destination == MIRROR_MODE_Y) ||
             (destination == MIRROR_MODE_XY && source == MIRROR_MODE_Y) ||
             (source == MIRROR_MODE_X && destination == MIRROR_MODE_NONE) ||
             (destination == MIRROR_MODE_X && source == MIRROR_MODE_NONE))
    {
        return MIRROR_MODE_X;
    }
    else if ((source == MIRROR_MODE_XY && destination == MIRROR_MODE_NONE) ||
             (destination == MIRROR_MODE_XY && source == MIRROR_MODE_NONE))
    {
        return MIRROR_MODE_XY;
    }
    return MIRROR_MODE_LAST;
                                              ...
                                              CopyRegion region, uint32_t mipLevel)
{
    ...
    const tcu::Sampler::FilterMode filter =
        (m_params.filter == VK_FILTER_LINEAR) ? tcu::Sampler::LINEAR : tcu::Sampler::NEAREST;
    ...
    {
        const tcu::ConstPixelBufferAccess srcSubRegion =
            tcu::getSubregion(src, srcOffset.x, srcOffset.y, srcExtent.x, srcExtent.y);
        const tcu::PixelBufferAccess dstSubRegion =
            tcu::getSubregion(dst, dstOffset.x, dstOffset.y, dstExtent.x, dstExtent.y);
        blit(dstSubRegion, srcSubRegion, filter, mirrorMode);

        if (filter == tcu::Sampler::LINEAR)
        {
            const tcu::PixelBufferAccess unclampedSubRegion = tcu::getSubregion(
                m_unclampedExpectedTextureLevel->getAccess(), dstOffset.x, dstOffset.y, dstExtent.x, dstExtent.y);
...
tcu::TestStatus BlittingMipmaps::iterate(void)
{
    ...
    {
        {
            ...
            vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  ...
                                  (const VkBufferMemoryBarrier *)DE_NULL, 1, &dstImageBarrier);
            vk.cmdBlitImage(*m_cmdBuffer, m_source.get(), m_params.src.image.operationLayout, m_destination.get(),
                            m_params.dst.image.operationLayout, (uint32_t)regions.size(), &regions[0], m_params.filter);
        }
    }
    ...
    else
    {
        ...
        for (uint32_t regionNdx = 0u; regionNdx < (uint32_t)regions.size(); regionNdx++)
        {
            const uint32_t mipLevel = regions[regionNdx].dstSubresource.mipLevel;
            ...
            vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                                  (const VkBufferMemoryBarrier *)DE_NULL, 1, &preImageBarrier);
            vk.cmdBlitImage(*m_cmdBuffer, m_destination.get(), m_params.src.image.operationLayout, m_destination.get(),
                            m_params.dst.image.operationLayout, 1u, &regions[regionNdx], m_params.filter);
            vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                                  (const VkBufferMemoryBarrier *)DE_NULL, 1, &postImageBarrier);
        }
...
tcu::TestStatus ResolveImageToImage::iterate(void)
{
    ...
    vk.cmdResolveImage(*m_cmdBuffer, sourceImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_destination.get(),
                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, (uint32_t)m_params.regions.size(), imageResolves.data());
    vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT,
                          (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                          (const VkBufferMemoryBarrier *)DE_NULL, 1, &postImageBarrier);
    endCommandBuffer(vk, *m_cmdBuffer);
...
void ResolveImageToImage::copyMSImageToMSImage(uint32_t copyArraySize)
{
    ...
    vk.cmdCopyImage(*m_cmdBuffer, m_multisampledImage.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
                    m_multisampledCopyImage.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, (uint32_t)imageCopies.size(),
                    imageCopies.data());
...
void addImageToImageAllFormatsColorTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
         ...
         ++compatibleFormatsIndex)
    {
        ...
        for (int srcFormatIndex = 0; compatibleFormats[srcFormatIndex] != VK_FORMAT_UNDEFINED; ++srcFormatIndex)
        {
            ...
            addTestGroup(group, getFormatCaseName(params.src.image.format), description,
                         addImageToImageAllFormatsColorSrcFormatTests, testParams);
        }
    }
}
...
void addImageToImageAllFormatsDepthStencilTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
         ...
         ++compatibleFormatsIndex)
    {
        ...
        const std::string testName =
            "2d_" + getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
        const std::string description = "2D copy from " + getFormatCaseName(params.src.image.format) + " to " +
         ...
         ++compatibleFormatsIndex)
    {
        ...
        const std::string testName =
            "1d_" + getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
        const std::string description = "1D copy from " + getFormatCaseName(params.src.image.format) + " to " +
                                        getFormatCaseName(params.dst.image.format);
        addTestGroup(group, testName, description, addImageToImageAllFormatsDepthStencilFormatsTests, params);
    }
         ...
         ++compatibleFormatsIndex)
    {
        ...
        const std::string testName =
            "3d_" + getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
        const std::string description = "3D copy from " + getFormatCaseName(params.src.image.format) + " to " +
                                        getFormatCaseName(params.dst.image.format);
        addTestGroup(group, testName, description, addImageToImageAllFormatsDepthStencilFormatsTests, params);
...
void addImageToImageAllFormatsTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
    ...
    addTestGroup(group, "depth_stencil", "Copy image to image with depth/stencil formats",
                 addImageToImageAllFormatsDepthStencilTests, allocationKind);
}
...
void addImageToImageTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
    ...
    addTestGroup(group, "all_formats", "Copy from image to image with all compatible formats",
                 addImageToImageAllFormatsTests, allocationKind);
    ...
    addTestGroup(group, "dimensions", "Copying operations on different image dimensions",
                 addImageToImageDimensionsTests, allocationKind);
}
...
void addBlittingImageSimpleMirrorXYTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
...

    {
        const VkImageBlit imageBlit = {
            ...
            defaultSourceLayer,                        // VkImageSubresourceLayers dstSubresource;
            {{defaultSize, defaultSize, 0}, {0, 0, 1}} // VkOffset3D dstOffset[2];
        };
...
void addBlittingImageSimpleMirrorXTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
...

    {
        const VkImageBlit imageBlit = {
            ...
            defaultSourceLayer,                        // VkImageSubresourceLayers dstSubresource;
            {{defaultSize, 0, 0}, {0, defaultSize, 1}} // VkOffset3D dstOffset[2];
        };
...
void addBlittingImageSimpleMirrorYTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
...

    {
        const VkImageBlit imageBlit = {
            ...
            defaultSourceLayer,                        // VkImageSubresourceLayers dstSubresource;
            {{0, defaultSize, 0}, {defaultSize, 0, 1}} // VkOffset3D dstOffset[2];
        };
...
void addBlittingImageSimpleMirrorSubregionsTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
    ...
    // No mirroring.
    {
        const VkImageBlit imageBlit = {
            defaultSourceLayer,                                 // VkImageSubresourceLayers srcSubresource;
            {{0, 0, 0}, {defaultHalfSize, defaultHalfSize, 1}}, // VkOffset3D srcOffsets[2];

            defaultSourceLayer,                                // VkImageSubresourceLayers dstSubresource;
            {{0, 0, 0}, {defaultHalfSize, defaultHalfSize, 1}} // VkOffset3D dstOffset[2];
        };
    ...
    // Flipping y coordinates.
    {
        const VkImageBlit imageBlit = {
            defaultSourceLayer,                                           // VkImageSubresourceLayers srcSubresource;
            {{defaultHalfSize, 0, 0}, {defaultSize, defaultHalfSize, 1}}, // VkOffset3D srcOffsets[2];

            defaultSourceLayer,                                          // VkImageSubresourceLayers dstSubresource;
            {{defaultHalfSize, defaultHalfSize, 0}, {defaultSize, 0, 1}} // VkOffset3D dstOffset[2];
        };
        CopyRegion region;
        region.imageBlit = imageBlit;
        params.regions.push_back(region);
    ...
    // Flipping x coordinates.
    {
        const VkImageBlit imageBlit = {
            defaultSourceLayer,                                           // VkImageSubresourceLayers srcSubresource;
            {{0, defaultHalfSize, 0}, {defaultHalfSize, defaultSize, 1}}, // VkOffset3D srcOffsets[2];

            defaultSourceLayer,                                          // VkImageSubresourceLayers dstSubresource;
            {{defaultHalfSize, defaultHalfSize, 0}, {0, defaultSize, 1}} // VkOffset3D dstOffset[2];
        };
    ...
    // Flipping x and y coordinates.
    {
        const VkImageBlit imageBlit = {
            defaultSourceLayer, // VkImageSubresourceLayers srcSubresource;
            {{defaultHalfSize, defaultHalfSize, 0}, {defaultSize, defaultSize, 1}}, // VkOffset3D srcOffsets[2];

            defaultSourceLayer, // VkImageSubresourceLayers dstSubresource;
            {{defaultSize, defaultSize, 0}, {defaultHalfSize, defaultHalfSize, 1}} // VkOffset3D dstOffset[2];
        };
...

    {
        const VkImageBlit imageBlit = {
            ...
            {{defaultFourthSize, defaultFourthSize, 0},
             {defaultFourthSize * 3, defaultFourthSize * 3, 1}}, // VkOffset3D srcOffsets[2];
        ...
        for (int i = 0; i < defaultSize; i += defaultFourthSize)
        {
            const VkImageBlit imageBlit = {
                ...
                {{defaultSize - defaultFourthSize - i, defaultSize - defaultFourthSize - i, 0},
                 {defaultSize - i, defaultSize - i, 1}}, // VkOffset3D srcOffsets[2];

                defaultSourceLayer, // VkImageSubresourceLayers dstSubresource;
                {{i, i, 0}, {i + defaultFourthSize, i + defaultFourthSize, 1}} // VkOffset3D dstOffset[2];
            };
            region.imageBlit = imageBlit;
            params.regions.push_back(region);
        }
...
void addBlittingImageAllFormatsDepthStencilTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
         ...
         ++compatibleFormatsIndex)
    {
        ...
        const std::string testName =
            "2d_" + getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
        const std::string description = "2D blit from " + getFormatCaseName(params.src.image.format) + " to " +
                                        getFormatCaseName(params.dst.image.format);
         ...
         ++compatibleFormatsIndex)
    {
        ...
        const std::string testName =
            "1d_" + getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
        const std::string description = "1D blit from " + getFormatCaseName(params.src.image.format) + " to " +
                                        getFormatCaseName(params.dst.image.format);
        addTestGroup(group, testName, description, addBlittingImageAllFormatsDepthStencilFormatsTests, params);
    }
         ...
         ++compatibleFormatsIndex)
    {
        ...
        const std::string testName =
            "3d_" + getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
        const std::string description = "3D blit from " + getFormatCaseName(params.src.image.format) + " to " +
                                        getFormatCaseName(params.dst.image.format);
        addTestGroup(group, testName, description, addBlittingImageAllFormatsDepthStencilFormatsTests, params);
...
void addBlittingImageAllFormatsMipmapTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
    addTestGroup(group, "from_base_level", "Generate all mipmap levels from base level",
                 addBlittingImageAllFormatsBaseLevelMipmapTests, allocationKind);
    addTestGroup(group, "from_previous_level", "Generate next mipmap level from previous level",
                 addBlittingImageAllFormatsPreviousLevelMipmapTests, allocationKind);
}
...
void addBlittingImageAllFormatsTests(tcu::TestCaseGroup *group, AllocationKind allocationKind)
{
    ...
    addTestGroup(group, "depth_stencil", "Blitting image with depth/stencil formats",
                 addBlittingImageAllFormatsDepthStencilTests, allocationKind);
    addTestGroup(group, "generate_mipmaps", "Generating mipmaps with vkCmdBlitImage()",
                 addBlittingImageAllFormatsMipmapTests, allocationKind);
}
