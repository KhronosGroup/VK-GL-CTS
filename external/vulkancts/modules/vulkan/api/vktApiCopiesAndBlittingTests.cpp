...
tcu::TestStatus CopyImageToImage::iterate(void)
{
    ...
    generateBuffer(m_destinationTextureLevel->getAccess(), m_params.dst.image.extent.width,
                   ...
                   m_params.clearDestination ? FILL_MODE_WHITE : FILL_MODE_GRADIENT);
...
tcu::TestStatus CopyImageToImage::checkTestResult(tcu::ConstPixelBufferAccess result)
{
    ...
    else
    {
        ...
        {
            if (!tcu::floatThresholdCompare(m_context.getTestContext().getLog(), "Compare", "Result comparison",
                                            m_expectedTextureLevel[0]->getAccess(), result, fThreshold,
        ...
        {
            if (!tcu::intThresholdCompare(m_context.getTestContext().getLog(), "Compare", "Result comparison",
                                          m_expectedTextureLevel[0]->getAccess(), result, uThreshold,
                                          tcu::COMPARE_LOG_RESULT))
                return tcu::TestStatus::fail("CopiesAndBlitting test");
        }
    }
...
bool BlittingMipmaps::checkNonNearestFilteredResult(void)
{
    ...
    for (uint32_t mipLevelNdx = 0u; mipLevelNdx < m_params.mipLevels; mipLevelNdx++)
    {
        ...
        else
        {
            ...
            for (uint32_t i = 0; i < 4; ++i)
                threshold[i] =
                    1 + de::max(((1 << dstBitDepth[i]) - 1) / de::clamp((1 << srcBitDepth[i]) - 1, 1, 256), 1);
...
Move<VkDevice> createCustomDevice(Context &context, const ResolveImageToImageOptions imageCreateOptions,
                                  ...
                                  uint32_t &queueFamilyIndex)
{
    ...
    DE_ASSERT(imageCreateOptions == COPY_MS_IMAGE_TO_MS_IMAGE_COMPUTE ||
              imageCreateOptions == COPY_MS_IMAGE_TO_MS_IMAGE_TRANSFER);

    // When testing with compute or transfer queue, find a queue family that supports compute or transfer queue, but does NOT support graphics queue.
    ...
    {
        if (imageCreateOptions == COPY_MS_IMAGE_TO_MS_IMAGE_COMPUTE)
        {
        ...
        else if (imageCreateOptions == COPY_MS_IMAGE_TO_MS_IMAGE_TRANSFER)
        {
    ...
    // One should be found, because this is checked in "checkSupport" function.
...
#ifndef CTS_USES_VULKANSC
        m_deviceDriver =
            de::MovePtr<DeviceDriver>(new DeviceDriver(context.getPlatformInterface(), m_customInstance, m_device));
#else
        m_deviceDriver = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(context.getPlatformInterface(), m_customInstance, m_device,
...
void addImageToImage3dImagesTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "3d_to_2d_by_slices",
                                                     "copy 2d layers to 3d slices one by one", params3DTo2D));
    }
