...
tcu::TestStatus CopyImageToImage::iterate(void)
{
    ...
    vk.cmdPipelineBarrier(cmdbuf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, (VkDependencyFlags)0,
    ...
    if (m_params.clearDestinationWithRed)
    {
        ...
        vk.cmdClearColorImage(cmdbuf, m_destination.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearColor, 1u,
                              &range);
        imageBarriers[0].oldLayout = imageBarriers[0].newLayout;
        imageBarriers[1].oldLayout = imageBarriers[1].newLayout;
        vk.cmdPipelineBarrier(cmdbuf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                              (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
    ...
    if (!(m_params.extensionFlags & COPY_COMMANDS_2))
    {
        vk.cmdCopyImage(cmdbuf, m_source.get(), m_params.src.image.operationLayout, m_destination.get(),
...
tcu::TestStatus CopyImageToImageMipmap::iterate(void)
{
    ...
    if (!(m_params.extensionFlags & COPY_COMMANDS_2))
    {
        vk.cmdCopyImage(*m_universalCmdBuffer, m_source.get(), m_params.src.image.operationLayout, m_destination.get(),
                        m_params.dst.image.operationLayout, (uint32_t)imageCopies.size(), imageCopies.data());
    }
    else
    {
...
tcu::TestStatus CopyBufferToBuffer::iterate(void)
{
    ...
    if (!(m_params.extensionFlags & COPY_COMMANDS_2))
    {
        vk.cmdCopyBuffer(*m_universalCmdBuffer, m_source.get(), m_destination.get(), (uint32_t)m_params.regions.size(),
                         &bufferCopies[0]);
    }
    else
    {
...
tcu::TestStatus CopyImageToBuffer::iterate(void)
{
    ...
    if (!(m_params.extensionFlags & COPY_COMMANDS_2))
    {
        vk.cmdCopyImageToBuffer(*m_universalCmdBuffer, m_source.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
...
tcu::TestStatus CopyCompressedImageToBuffer::iterate(void)
{
    ...
    copyBufferToImage(vk, vkDevice, queue, m_context.getUniversalQueueFamilyIndex(), m_sourceBuffer->get(),
