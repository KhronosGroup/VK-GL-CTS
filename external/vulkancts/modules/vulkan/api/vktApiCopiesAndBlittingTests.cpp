                                                      ...
                                                      const uint32_t mipLevels)
{
    ...
    const VkImageAspectFlags formatAspect =
        (m_params.separateDepthStencilLayouts) ? getAspectFlags(imageAccess.getFormat()) : getAspectFlags(parms.format);
    const bool skipPreImageBarrier = (m_params.separateDepthStencilLayouts) ?
                                         false :
    ...
    {
        const VkExtent3D copyExtent = {imageExtent.width >> mipLevelNdx, imageExtent.height >> mipLevelNdx,
                                       imageExtent.depth};
...
CopyImageToImage::CopyImageToImage(Context &context, TestParams params) : CopiesAndBlittingTestInstance(context, params)
{
    ...
    // Create source image
    {
        ...
        m_sourceImageAlloc = allocateImage(vki, vk, vkPhysDevice, vkDevice, *m_source, MemoryRequirement::Any, memAlloc,
                                           m_params.allocationKind, 0u);
        VK_CHECK(
            vk.bindImageMemory(vkDevice, *m_source, m_sourceImageAlloc->getMemory(), m_sourceImageAlloc->getOffset()));
    }
...
tcu::TestStatus CopyImageToImage::iterate(void)
{
    ...
    if (m_params.clearDestinationWithRed)
    {
        ...
        vk.cmdClearColorImage(*m_cmdBuffer, m_destination.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &clearColor, 1u,
                              &range);
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdCopyImage(*m_cmdBuffer, m_source.get(), m_params.src.image.operationLayout, m_destination.get(),
                                                ...
                                                CopyRegion region, uint32_t mipLevel)
{
    ...
    if (m_params.src.image.imageType == vk::VK_IMAGE_TYPE_2D)
    {
        ...
        if (m_params.dst.image.imageType == vk::VK_IMAGE_TYPE_2D &&
...
{
public:
    CopyImageToImageTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
tcu::TestStatus CopyImageToImageMipmap::iterate(void)
{
    ...
    m_sourceTextureLevel = de::MovePtr<tcu::TextureLevel>(
        new tcu::TextureLevel(srcTcuFormat, (int)m_params.src.image.extent.width, (int)m_params.src.image.extent.height,
                              (int)m_params.src.image.extent.depth));
    generateBuffer(m_sourceTextureLevel->getAccess(), m_params.src.image.extent.width, m_params.src.image.extent.height,
                   m_params.src.image.extent.depth, FILL_MODE_GRADIENT);
...
{
public:
    CopyImageToImageMipmapTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                   const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
    ...
    : CopiesAndBlittingTestInstance(context, params)
{
    ...
    // Create destination buffer
    {
        ...
        m_destinationBufferAlloc = allocateBuffer(vki, vk, vkPhysDevice, vkDevice, *m_destination,
                                                  MemoryRequirement::HostVisible, memAlloc, m_params.allocationKind);
        VK_CHECK(vk.bindBufferMemory(vkDevice, *m_destination, m_destinationBufferAlloc->getMemory(),
...
tcu::TestStatus CopyBufferToBuffer::iterate(void)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdCopyBuffer(*m_cmdBuffer, m_source.get(), m_destination.get(), (uint32_t)m_params.regions.size(),
                         &bufferCopies[0]);
    }
#ifndef CTS_USES_VULKANSC
    else
...
{
public:
    BufferToBufferTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                           const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
tcu::TestStatus CopyImageToBuffer::iterate(void)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdCopyImageToBuffer(*m_cmdBuffer, m_source.get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_destination.get(),
...
{
public:
    CopyImageToBufferTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                              const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
// TODO: Potentially move this to vktTestCase.cpp when multi-queue support lands.
static uint32_t findQueueFamilyIndexWithCaps(const InstanceInterface &vkInstance, VkPhysicalDevice physicalDevice,
                                             VkQueueFlags requiredCaps, VkQueueFlags excludedCaps = 0u)
{
...
Move<VkDevice> createCustomDevice(Context &context, const QueueSelectionOptions queueOptions,
                                  uint32_t &queueFamilyIndex)
{
    ...
    case QueueSelectionOptions::Universal:
        TCU_THROW(NotSupportedError,
                  "Invalid queue selection option, only compute or transfer queues make sense for a custom device");
        break;
    case QueueSelectionOptions::ComputeOnly:
        queueFamilyIndex =
            findQueueFamilyIndexWithCaps(instanceDriver, physicalDevice, VK_QUEUE_COMPUTE_BIT, VK_QUEUE_GRAPHICS_BIT);
        break;
    ...
    return vkt::createCustomDevice(context.getTestContext().getCommandLine().isValidationEnabled(),
                                   context.getPlatformInterface(), context.getInstance(), instanceDriver,
...
{
public:
    CopyCompressedImageToBuffer(Context &context, TestParams testParams, QueueSelectionOptions options);
...
tcu::TestStatus CopyCompressedImageToBuffer::iterate(void)
{
    ...
    if (useTwoQueues)
    {
        ...
        m_deviceDriver = de::MovePtr<DeviceDriver>(
            new DeviceDriver(m_context.getPlatformInterface(), m_context.getInstance(), m_device));
#else
        m_deviceDriver = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(m_context.getPlatformInterface(), m_context.getInstance(), m_device,
    ...
    m_alternativeAllocator = de::MovePtr<Allocator>(
        new SimpleAllocator(vk, m_device, getPhysicalDeviceMemoryProperties(vki, m_context.getPhysicalDevice())));
    m_allocator = m_alternativeAllocator.get();

    m_alternativeCmdPool =
        createCommandPool(vk, m_device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, queueFamilyIndex);
    m_alternativeCmdBuffer =
        allocateCommandBuffer(vk, m_device, *m_alternativeCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
    ...
    copyBufferToImage(vk, vkDevice, queue, queueFamilyIndex, m_sourceBuffer->get(), m_texture->getCompressedSize(),
...
{
public:
    CopyCompressedImageToBufferTestCase(tcu::TestContext &testCtx, const std::string &name,
                                        const std::string &description, const TestParams params,
                                        QueueSelectionOptions options)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
        , m_options(options)
    ...
    virtual TestInstance *createInstance(Context &context) const
    {
        return new CopyCompressedImageToBuffer(context, m_params, m_options);
    }
    virtual void checkSupport(Context &context) const;
...
tcu::TestStatus CopyBufferToImage::iterate(void)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdCopyBufferToImage(*m_cmdBuffer, m_source.get(), m_destination.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
{
public:
    CopyBufferToImageTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                              const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
tcu::TestStatus CopyBufferToDepthStencil::iterate(void)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        if (m_params.singleCommand)
        {
            ...
            vk.cmdCopyBufferToImage(*m_cmdBuffer, m_source.get(), m_destination.get(),
...
{
public:
    CopyBufferToDepthStencilTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                     const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
tcu::TestStatus BlittingImages::iterate(void)
{
    ...
    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdBlitImage(*m_cmdBuffer, m_source.get(), srcImageParams.operationLayout, m_destination.get(),
...
struct CompareEachPixelInEachRegion
{
    ...
    bool forEach(const void *pUserData, const std::vector<CopyRegion> &regions, const int sourceWidth,
                 const int sourceHeight, const int sourceDepth, const tcu::PixelBufferAccess &errorMask) const
    {
        bool compareOk = true;

        for (std::vector<CopyRegion>::const_iterator regionIter = regions.begin(); regionIter != regions.end();
             ++regionIter)
        {
            ...
            const float zScale = static_cast<float>(blit.srcOffsets[1].z - blit.srcOffsets[0].z) /
                                 static_cast<float>(blit.dstOffsets[1].z - blit.dstOffsets[0].z);
            ...
            for (int z = zStart; z < zEnd; z++)
                for (int y = yStart; y < yEnd; y++)
                    ...
                    {
                        const tcu::Vec3 srcNormCoord((xScale * (static_cast<float>(x - blit.dstOffsets[0].x) + 0.5f) +
                                                     ...
                                                     (yScale * (static_cast<float>(y - blit.dstOffsets[0].y) + 0.5f) +
                                                      static_cast<float>(blit.srcOffsets[0].y)) *
                                                         srcInvH,
                                                     (zScale * (static_cast<float>(z - blit.dstOffsets[0].z) + 0.5f) +
                                                      static_cast<float>(blit.srcOffsets[0].z)) *
                                                         srcInvD);
...
bool floatNearestBlitCompare(const tcu::ConstPixelBufferAccess &source, const tcu::ConstPixelBufferAccess &result,
                             ...
                             const tcu::PixelBufferAccess &errorMask, const std::vector<CopyRegion> &regions)
{
    ...
    const struct Capture
    {
        ...
        const bool isSRGB;
    } capture = {source, result, sampler, precision, tcu::isSRGB(result.getFormat())};
...
{
public:
    BlitImageTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                      const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
...
tcu::TestStatus BlittingMipmaps::iterate(void)
{
    ...
    {
        {
            ...
            if (m_params.extensionUse == EXTENSION_USE_NONE)
            {
                vk.cmdBlitImage(*m_cmdBuffer, m_source.get(), m_params.src.image.operationLayout, m_destination.get(),
                                m_params.dst.image.operationLayout, (uint32_t)m_params.regions.size(), &regions[0],
                                m_params.filter);
            }
#ifndef CTS_USES_VULKANSC
            else
    ...
    else
    {
        ...
        for (uint32_t regionNdx = 0u; regionNdx < (uint32_t)m_params.regions.size(); regionNdx++)
        {
            ...
            if (m_params.extensionUse == EXTENSION_USE_NONE)
            {
                vk.cmdBlitImage(*m_cmdBuffer, m_destination.get(), m_params.src.image.operationLayout,
...
{
public:
    BlitMipmapTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       const TestParams params)
        : vkt::TestCase(testCtx, name, description)
        , m_params(params)
    {
    ...
    , m_options(options)
{
    ...
    if (m_options == COPY_MS_IMAGE_TO_MS_IMAGE_COMPUTE || m_options == COPY_MS_IMAGE_TO_MS_IMAGE_TRANSFER)
    {
        ...
        m_customDevice =
            createCustomDevice(context,
                               (m_options == COPY_MS_IMAGE_TO_MS_IMAGE_COMPUTE) ? QueueSelectionOptions::ComputeOnly :
...
#ifndef CTS_USES_VULKANSC
        m_deviceDriver = de::MovePtr<DeviceDriver>(
            new DeviceDriver(context.getPlatformInterface(), m_context.getInstance(), m_device));
#else
        m_deviceDriver = de::MovePtr<DeviceDriverSC, DeinitDeviceDeleter>(
            new DeviceDriverSC(context.getPlatformInterface(), m_context.getInstance(), m_device,
    ...
    m_alternativeAllocator = de::MovePtr<Allocator>(
        new SimpleAllocator(vk, m_device, getPhysicalDeviceMemoryProperties(vki, context.getPhysicalDevice())));
    m_allocator = m_alternativeAllocator.get();
    ...
    m_cmdPool   = createCommandPool(vk, m_device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT,
...

    {
        ...
        // Create command buffer
        {
            ...
            if (m_options == COPY_MS_IMAGE_LAYER_TO_MS_IMAGE || m_options == COPY_MS_IMAGE_TO_MS_IMAGE_MULTIREGION)
            {
                ...
                vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                                      (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                                      (const VkBufferMemoryBarrier *)DE_NULL, (uint32_t)srcImageBarriers.size(),
                                      srcImageBarriers.data());
                ...
                // Clear the 'm_multisampledImage'.
                {
                    ...
                    const auto clearRange = makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u,
                                                                      m_params.src.image.extent.depth);
                    vk.cmdClearColorImage(*m_cmdBuffer, m_multisampledImage.get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                          &clearValue, 1u, &clearRange);
                }
                ...
                // Clear the 'm_multisampledCopyImage' with different color.
                {
                    ...
                    const auto clearRange = makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u,
                                                                      m_params.src.image.extent.depth);
                    vk.cmdClearColorImage(*m_cmdBuffer, m_multisampledCopyImage.get(),
            ...
            beginRenderPass(vk, *m_cmdBuffer, *renderPass, *framebuffer,
            ...
            vk.cmdDraw(*m_cmdBuffer, (uint32_t)vertices.size(), 1, 0, 0);
...
tcu::TestStatus ResolveImageToImage::iterate(void)
{
    ...
    beginCommandBuffer(vk, *m_cmdBuffer);
    vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
                          (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                          (const VkBufferMemoryBarrier *)DE_NULL, DE_LENGTH_OF_ARRAY(imageBarriers), imageBarriers);

    if (m_params.extensionUse == EXTENSION_USE_NONE)
    {
        vk.cmdResolveImage(*m_cmdBuffer, sourceImage, m_params.src.image.operationLayout, m_destination.get(),
...
void ResolveImageToImage::copyMSImageToMSImage(uint32_t copyArraySize)
{
    ...
    if (useTwoQueues)
    {
        // Release ownership from graphics queue.
        {
            ...
            vk.cmdPipelineBarrier(m_cmdBuffer.get(), VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
    ...
    if (useTwoQueues)
    {
        ...
        VkImageMemoryBarrier srcImageBarrier =
            makeImageMemoryBarrier(0u, 0u, m_params.src.image.operationLayout, m_params.src.image.operationLayout,
                                   m_multisampledImage.get(), m_multisampledImageBarrier.subresourceRange,
                                   m_alternativeQueueFamilyIndex, m_context.getUniversalQueueFamilyIndex());
        ...
        // Acquire ownership to graphics queue.
        {
            ...
            beginCommandBuffer(vk, m_cmdBuffer.get());
            vk.cmdPipelineBarrier(m_cmdBuffer.get(), VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
...
{
public:
    ResolveImageToImageTestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                                const TestParams params,
...
{
public:
    DepthStencilMSAATestCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
void addImageToImageSimpleTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "whole_image_diff_fromat",
                                                     "Whole image with different format", params));
    }
...
void addImageToImageAllFormatsColorSrcFormatTests(tcu::TestCaseGroup *group, CopyColorTestParams testParams)
{
    ...
    for (int dstFormatIndex = 0; formatList[dstFormatIndex] != VK_FORMAT_UNDEFINED; ++dstFormatIndex)
    {
        ...
        addTestGroup(group, getFormatCaseName(dstFormat), description,
                     addImageToImageAllFormatsColorSrcFormatDstFormatTests, testParams.params);
    }
}
...
const VkFormat *colorImageFormatsToTest[] = {compatibleFormats8Bit,  compatibleFormats16Bit,  compatibleFormats24Bit,
                                             compatibleFormats32Bit, compatibleFormats48Bit,  compatibleFormats64Bit,
                                             compatibleFormats96Bit, compatibleFormats128Bit, compatibleFormats192Bit,
    ...
    // 2D tests.
    {
             ...
             ++compatibleFormatsIndex)
        {
            ...
            for (int srcFormatIndex = 0; compatibleFormats[srcFormatIndex] != VK_FORMAT_UNDEFINED; ++srcFormatIndex)
            {
                ...
                addTestGroup(subGroup.get(), testName, description, addImageToImageAllFormatsColorSrcFormatTests,
                             testParams);
            }
        }
...
void addImageToImageDimensionsTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    ...
    for (size_t dimensionNdx = 0; dimensionNdx < DE_LENGTH_OF_ARRAY(imageDimensions); dimensionNdx++)
    {
        ...
        tcu::TestCaseGroup *imageSizeGroup =
            new tcu::TestCaseGroup(testCtx, dimensionStr.c_str(), ("Image sizes " + dimensionStr).c_str());
             ...
             compatibleFormatsIndex++)
        {
                 ...
                 srcFormatIndex++)
            {
                ...
                const std::string srcDescription =
                    "Copy from source format " + getFormatCaseName(testParams.params.src.image.format);
                tcu::TestCaseGroup *srcFormatGroup = new tcu::TestCaseGroup(
                    testCtx, getFormatCaseName(testParams.params.src.image.format).c_str(), srcDescription.c_str());
                     ...
                     dstFormatIndex++)
                {
                    ...
                    const std::string dstDescription =
                        "Copy to destination format " + getFormatCaseName(testParams.params.dst.image.format);
                    tcu::TestCaseGroup *dstFormatGroup = new tcu::TestCaseGroup(
                        testCtx, getFormatCaseName(testParams.params.dst.image.format).c_str(), dstDescription.c_str());
                    ...
                    for (int srcLayoutNdx = 0u; srcLayoutNdx < DE_LENGTH_OF_ARRAY(copySrcLayouts); srcLayoutNdx++)
                    {
                        ...
                        for (int dstLayoutNdx = 0u; dstLayoutNdx < DE_LENGTH_OF_ARRAY(copyDstLayouts); dstLayoutNdx++)
                        {
                            ...
                            const std::string description =
                                "From layout " + getImageLayoutCaseName(testParams.params.src.image.operationLayout) +
                                " to " + getImageLayoutCaseName(testParams.params.dst.image.operationLayout);
                            const TestParams params = testParams.params;

                            dstFormatGroup->addChild(
                                new CopyImageToImageTestCase(testCtx, testName, description, params));
                        }
                    }
                                                ...
                                                ExtensionUse extensionUse)
{
    ...
    // 2D tests.
    {
             ...
             ++compatibleFormatsIndex)
        {
            ...
            params.allocationKind              = allocationKind;
            params.extensionUse                = extensionUse;
            params.separateDepthStencilLayouts = false;
            ...
            const std::string description = "Copy from " + getFormatCaseName(params.src.image.format) + " to " +
                                            getFormatCaseName(params.dst.image.format);
            addTestGroup(subGroup.get(), testName, description, addImageToImageAllFormatsDepthStencilFormatsTests,
                         params);

            if (hasDepth && hasStencil)
            {
                params.separateDepthStencilLayouts = true;
                ...
                const std::string description2 = "Copy from " + getFormatCaseName(params.src.image.format) + " to " +
                                                 getFormatCaseName(params.dst.image.format) +
                                                 " with separate depth/stencil layouts";
                addTestGroup(subGroup.get(), testName2, description2, addImageToImageAllFormatsDepthStencilFormatsTests,
                             params);
                ...
                // DS Image copy
                {
                    ...
                    const std::string description3 = "Copy both depth and stencil aspects from " +
                                                     getFormatCaseName(params.src.image.format) + " to " +
                                                     getFormatCaseName(params.dst.image.format);
                    addTestGroup(subGroup.get(), testName3, description3,
                                 addImageToImageAllFormatsDepthStencilFormatsTests, params);
                }
            }
        }
...
void addImageToImageAllFormatsTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    addTestGroup(group, "color", "Copy image to image with color formats", addImageToImageAllFormatsColorTests,
                 allocationKind, extensionUse);
    addTestGroup(group, "depth_stencil", "Copy image to image with depth/stencil formats",
                 addImageToImageAllFormatsDepthStencilTests, allocationKind, extensionUse);
}
...
void addImageToImage3dImagesTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "3d_to_2d_by_slices",
                                                     "copy 3d slices to 2d layers one by one", params3DTo2D));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "2d_to_3d_by_layers",
                                                     "copy 2d layers to 3d slices one by one", params2DTo3D));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "3d_to_2d_whole",
                                                     "copy 3d slices to 2d layers all at once", params3DTo2D));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "2d_to_3d_whole",
                                                     "copy 2d layers to 3d slices all at once", params2DTo3D));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "3d_to_2d_regions", "copy 3d slices regions to 2d layers",
                                                     params3DTo2D));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "2d_to_3d_regions", "copy 2d layers regions to 3d slices",
                                                     params2DTo3D));
    }
}
...
void addImageToImageCubeTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "cube_to_array_layers",
                                                     "copy cube compatible image to 2d layers layer by layer",
                                                     paramsCubeToArray));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(
            testCtx, "cube_to_array_whole", "copy cube compatible image to 2d layers all at once", paramsCubeToArray));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "array_to_cube_layers",
                                                     "copy 2d layers to cube compatible image layer by layer",
                                                     paramsArrayToCube));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(
            testCtx, "array_to_cube_whole", "copy 2d layers to cube compatible image all at once", paramsArrayToCube));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(
            testCtx, "cube_to_cube_layers", "copy cube compatible image to cube compatible image layer by layer",
            paramsCubeToArray));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "cube_to_cube_whole",
                                                     "copy cube compatible image to cube compatible image all at once",
                                                     paramsCubeToCube));
    }
}
...
void addImageToImageArrayTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
...

    {
        ...
        group->addChild(new CopyImageToImageTestCase(testCtx, "array_to_array_layers",
                                                     "copy 2d array image to 2d array image layer by layer",
                                                     paramsArrayToArray));
    }

    {
        ...
        group->addChild(new CopyImageToImageTestCase(
            testCtx, "array_to_array_whole", "copy 2d array image to 2d array image all at once", paramsArrayToArray));
    }

    {
        ...
        for (uint32_t imageFormatsNdx = 0; imageFormatsNdx < DE_LENGTH_OF_ARRAY(imageFormats); imageFormatsNdx++)
        {
            ...
            group->addChild(new CopyImageToImageMipmapTestCase(
                testCtx, testName.str(), "copy 2d array mipmap image to 2d array mipmap image all at once",
                paramsArrayToArray));
        }
    }
}
...
void addImageToImageTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    addTestGroup(group, "simple_tests", "Copy from image to image simple tests", addImageToImageSimpleTests,
                 allocationKind, extensionUse);
    addTestGroup(group, "all_formats", "Copy from image to image with all compatible formats",
                 addImageToImageAllFormatsTests, allocationKind, extensionUse);
    addTestGroup(group, "3d_images", "Coping operations on 3d images", addImageToImage3dImagesTests, allocationKind,
                 extensionUse);
    addTestGroup(group, "dimensions", "Copying operations on different image dimensions",
                 addImageToImageDimensionsTests, allocationKind, extensionUse);
    addTestGroup(group, "cube", "Coping operations on cube compatible images", addImageToImageCubeTests, allocationKind,
                 extensionUse);
    addTestGroup(group, "array", "Copying operations on array of images", addImageToImageArrayTests, allocationKind,
                 extensionUse);
}

void add1dImageToBufferTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    tcu::TestContext &testCtx = group->getTestContext();

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(
            testCtx, "tightly_sized_buffer",
            "Copy from image to a buffer that is just large enough to contain the data", params));
    }

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(
            testCtx, "larger_buffer", "Copy from image to a buffer that is larger than necessary", params));
    }

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(testCtx, "array_tightly_sized_buffer",
                                                      "Copy each layer from array to tightly sized buffer", params));
    }

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(
            testCtx, "array_larger_buffer", "Copy each layer from array to a buffer that is larger than necessary",
            params));
    }
}

void add2dImageToBufferTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    tcu::TestContext &testCtx = group->getTestContext();
...

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(testCtx, "buffer_offset",
                                                      "Copy from image to buffer with buffer offset", params));
    }

    {
        ...
        group->addChild(
            new CopyImageToBufferTestCase(testCtx, "buffer_offset_relaxed",
                                          "Copy from image to buffer with buffer offset not a multiple of 4", params));
    }

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(testCtx, "regions",
                                                      "Copy from image to buffer with multiple regions", params));
    }
...

    {
        ...
        group->addChild(new CopyImageToBufferTestCase(
            testCtx, "tightly_sized_buffer_offset",
            "Copy from image to a buffer that is just large enough to contain the data", params));
    }

    {
        ...
        group->addChild(
            new CopyImageToBufferTestCase(testCtx, "array", "Copy each layer from array to buffer", params));
    }
            ...
            for (const auto numLayers : arrayLayers)
            {
                ...
                for (const VkFormat *format = compressedFormatsFloats; *format != VK_FORMAT_UNDEFINED; format++)
                {
                    ...
                    group->addChild(new CopyCompressedImageToBufferTestCase(
                        testCtx, getCaseName(*format, params.src.image.extent, numLayers, "universal"),
                        "Copy from image to buffer", params, QueueSelectionOptions::Universal));
                    group->addChild(new CopyCompressedImageToBufferTestCase(
                        testCtx, getCaseName(*format, params.src.image.extent, numLayers, "compute"),
                        "Copy from image to buffer", params, QueueSelectionOptions::ComputeOnly));
                }
            }
    }
}
    ...
    for (const auto config : depthAndStencilFormats)
        for (const auto offset : useOffset)
        {
            ...
            if (hasDepth && hasStencil)
            {
                ...
                group->addChild(new CopyBufferToDepthStencilTestCase(testCtx, description + "_DS",
                                                                     "Copy from depth&stencil to image", params));

                params.singleCommand = false;

                group->addChild(new CopyBufferToDepthStencilTestCase(testCtx, description + "_D_S",
                                                                     "Copy from depth then stencil to image", params));

                params.regions.clear();
                params.regions.push_back(copyStencilRegion);
                params.regions.push_back(copyDepthRegion);

                group->addChild(new CopyBufferToDepthStencilTestCase(testCtx, description + "_S_D",
                                                                     "Copy from depth then stencil to image", params));

                params.singleCommand = true;
                group->addChild(new CopyBufferToDepthStencilTestCase(testCtx, description + "_SD",
                                                                     "Copy from depth&stencil to image", params));
            }
            ...
            if (hasStencil)
            {
                ...
                group->addChild(new CopyBufferToDepthStencilTestCase(testCtx, description + "_S",
                                                                     "Copy from stencil to image", params));
            }
            ...
            if (hasDepth)
            {
                ...
                group->addChild(new CopyBufferToDepthStencilTestCase(testCtx, description + "_D",
                                                                     "Copy from depth to image", params));
            }
        }
}

void add1dBufferToImageTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    tcu::TestContext &testCtx = group->getTestContext();

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(testCtx, "tightly_sized_buffer",
                                                      "Copy from tightly packed buffer to image", params));
    }
...

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(
            testCtx, "array_tightly_sized_buffer",
            "Copy from a different part of the tightly packed buffer to each layer", params));
    }

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(
            testCtx, "array_larger_buffer", "Copy from a different part of the buffer to each layer", params));
    }
}

void add2dBufferToImageTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    tcu::TestContext &testCtx = group->getTestContext();
...

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(testCtx, "regions",
                                                      "Copy from buffer to image with multiple regions", params));
    }

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(testCtx, "buffer_offset",
                                                      "Copy from buffer to image with buffer offset", params));
    }

    {
        ...
        group->addChild(
            new CopyBufferToImageTestCase(testCtx, "buffer_offset_relaxed",
                                          "Copy from buffer to image with buffer offset not a multiple of 4", params));
    }

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(
            testCtx, "tightly_sized_buffer",
            "Copy from buffer that is just large enough to contain the accessed elements", params));
    }

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(
            testCtx, "larger_buffer", "Copy from buffer that is larger than necessary to image", params));
    }
...

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(
            testCtx, "array", "Copy from a different part of the buffer to each layer", params));
    }

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(testCtx, "array_larger_buffer",
                                                      "Copy from different part of buffer to each layer", params));
    }

    {
        ...
        group->addChild(new CopyBufferToImageTestCase(testCtx, "array_tightly_sized_buffer",
                                                      "Copy from different part of tightly sized buffer to each layer",
                                                      params));
    }
}
...
void addBlittingImageSimpleTests(tcu::TestCaseGroup *group, TestParams &params)
{
    ...
    // Filter is VK_FILTER_NEAREST.
    {
        ...
        group->addChild(new BlitImageTestCase(testCtx, getFormatCaseName(params.dst.image.format) + "_nearest",
                                              descriptionOfRGBAToR32, params));
        ...
        group->addChild(new BlitImageTestCase(testCtx, getFormatCaseName(params.dst.image.format) + "_nearest",
                                              descriptionOfRGBAToBGRA, params));
    }
    ...
    // Filter is VK_FILTER_LINEAR.
    {
        ...
        params.dst.image.format = VK_FORMAT_R32_SFLOAT;
        const std::string descriptionOfRGBAToR32(description + " and different formats (R8G8B8A8 -> R32)");
        group->addChild(new BlitImageTestCase(testCtx, getFormatCaseName(params.dst.image.format) + "_linear",
                                              descriptionOfRGBAToR32, params));
    ...
    if (params.dst.image.imageType == VK_IMAGE_TYPE_2D)
    {
        ...
        params.dst.image.format = VK_FORMAT_R32_SFLOAT;
        const std::string descriptionOfRGBAToR32(description + " and different formats (R8G8B8A8 -> R32)");
        group->addChild(new BlitImageTestCase(testCtx, getFormatCaseName(params.dst.image.format) + "_cubic",
                                              descriptionOfRGBAToR32, params));
...
void addBlittingImageSimpleTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    ...
    addTestGroup(group, "mirror_subregions", "Mirroring subregions in image (no flip, y flip, x flip, xy flip)",
                 addBlittingImageSimpleMirrorSubregionsTests, params);
    addTestGroup(group, "scaling_whole1", "Blit with scaling (whole, src extent bigger)",
                 addBlittingImageSimpleScalingWhole1Tests, params);
    addTestGroup(group, "scaling_whole2", "Blit with scaling (whole, dst extent bigger)",
                 addBlittingImageSimpleScalingWhole2Tests, params);
    addTestGroup(group, "scaling_and_offset", "Blit with scaling and offset (whole, dst extent bigger)",
                 addBlittingImageSimpleScalingAndOffsetTests, params);
    addTestGroup(group, "without_scaling_partial", "Blit without scaling (partial)",
                 addBlittingImageSimpleWithoutScalingPartialTests, params);

    params.src.image.imageType = VK_IMAGE_TYPE_3D;
    params.dst.image.imageType = VK_IMAGE_TYPE_3D;
    addTestGroup(group, "whole_3d", "3D blit without scaling (whole)", addBlittingImageSimpleWholeTests, params);
    addTestGroup(group, "mirror_xy_3d", "Flipping x and y coordinates of a 3D image (whole)",
                 addBlittingImageSimpleMirrorXYTests, params);
    addTestGroup(group, "mirror_x_3d", "Flipping x coordinates of a 3D image (whole)",
                 addBlittingImageSimpleMirrorXTests, params);
    addTestGroup(group, "mirror_y_3d", "Flipping y coordinates of a 3D image (whole)",
                 addBlittingImageSimpleMirrorYTests, params);
    addTestGroup(group, "mirror_z_3d", "Flipping z coordinates of a 3D image (whole)",
                 addBlittingImageSimpleMirrorZTests, params);
    addTestGroup(group, "mirror_subregions_3d", "Mirroring subregions in a 3D image (no flip, y flip, x flip, xy flip)",
                 addBlittingImageSimpleMirrorSubregionsTests, params);
    addTestGroup(group, "scaling_whole1_3d", "3D blit a with scaling (whole, src extent bigger)",
                 addBlittingImageSimpleScalingWhole1Tests, params);
    addTestGroup(group, "scaling_whole2_3d", "3D blit with scaling (whole, dst extent bigger)",
                 addBlittingImageSimpleScalingWhole2Tests, params);
    addTestGroup(group, "scaling_and_offset_3d", "3D blit with scaling and offset (whole, dst extent bigger)",
                 addBlittingImageSimpleScalingAndOffsetTests, params);
    addTestGroup(group, "without_scaling_partial_3d", "3D blit without scaling (partial)",
                 addBlittingImageSimpleWithoutScalingPartialTests, params);
}
                ...
                for (int dstLayoutNdx = 0u; dstLayoutNdx < DE_LENGTH_OF_ARRAY(blitDstLayouts); ++dstLayoutNdx)
                {
                    ...
                    const std::string description =
                        ...
                        getBlitImageTilingLayoutCaseName(testParams.params.dst.image.tiling,
                                                         testParams.params.dst.image.operationLayout);
                    group->addChild(
                        new BlitImageTestCase(testCtx, testName + "_nearest", description, testParams.params));

                    if (testParams.testFilters & FILTER_MASK_LINEAR)
                    {
                        testParams.params.filter = VK_FILTER_LINEAR;
                        group->addChild(
                            new BlitImageTestCase(testCtx, testName + "_linear", description, testParams.params));
                    }
                    ...
                    if (testParams.testFilters & FILTER_MASK_CUBIC)
                    {
                        ...
                        group->addChild(
                            new BlitImageTestCase(testCtx, testName + "_cubic", description, testParams.params));
                    }
                        ...
                        !isCompressedFormat(testParams.params.src.image.format))
                    {
                        ...
                        for (int i = 0; i < DE_LENGTH_OF_ARRAY(modeList); ++i)
                        {
                            ...
                            group->addChild(new BlitImageTestCase(testCtx,
                                                                  testName + "_linear_stripes_" + modeList[i].name,
                                                                  description, otherParams.params));

                            otherParams.params.filter = VK_FILTER_NEAREST;
                            group->addChild(new BlitImageTestCase(testCtx,
                                                                  testName + "_nearest_stripes_" + modeList[i].name,
                                                                  description, otherParams.params));
                        }
                    }
                }
...
void addBlittingImageAllFormatsColorSrcFormatTests(tcu::TestCaseGroup *group, BlitColorTestParams testParams)
{
    ...
    if (testParams.compatibleFormats)
    {
             ...
             ++dstFormatIndex)
        {
            ...
            const std::string description =
                "Blit destination format " + getFormatCaseName(testParams.params.dst.image.format);
            addTestGroup(group, getFormatCaseName(testParams.params.dst.image.format), description,
    ...
    if (!testParams.compatibleFormats || isCompressedFormat(srcFormat))
    {
        ...
        addTestGroup(group, getFormatCaseName(srcFormat), description,
                     addBlittingImageAllFormatsColorSrcFormatDstFormatTests, testParams);
    }
}
...
{
    addTestGroup(group, "1d_images", "Copying operations on 1d images", add1dBufferToImageTests, allocationKind,
                 extensionUse);
    addTestGroup(group, "2d_images", "Copying operations on 2d images", add2dBufferToImageTests, allocationKind,
                 extensionUse);
}

void addImageToBufferTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    addTestGroup(group, "1d_images", "Copying operations on 1d images", add1dImageToBufferTests, allocationKind,
                 extensionUse);
    addTestGroup(group, "2d_images", "Copying operations on 2d images", add2dImageToBufferTests, allocationKind,
                 extensionUse);
}
                                          ...
                                          ExtensionUse extensionUse)
{
    ...
    // 2D tests.
    {
        de::MovePtr<tcu::TestCaseGroup> subGroup(
            new tcu::TestCaseGroup(group->getTestContext(), "2d", "2D blitting tests"));
             ...
             ++compatibleFormatsIndex)
        {
            ...
            for (int srcFormatIndex = 0; sourceFormats[srcFormatIndex] != VK_FORMAT_UNDEFINED; ++srcFormatIndex)
            {
                ...
                addTestGroup(subGroup.get(), getFormatCaseName(params.src.image.format), description,
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> subGroup(
            new tcu::TestCaseGroup(group->getTestContext(), "1d", "1D blitting tests"));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> subGroup(
            new tcu::TestCaseGroup(group->getTestContext(), "3d", "3D blitting tests"));
                                                 ...
                                                 ExtensionUse extensionUse)
{
    ...
    // 2D tests
    {
             ...
             ++compatibleFormatsIndex)
        {
            ...
            const std::string testName =
                getFormatCaseName(params.src.image.format) + "_" + getFormatCaseName(params.dst.image.format);
            const std::string description = "Blit from " + getFormatCaseName(params.src.image.format) + " to " +
                                            getFormatCaseName(params.dst.image.format);
            addTestGroup(subGroup.get(), testName, description, addBlittingImageAllFormatsDepthStencilFormatsTests,
                         params);
            ...
            if (hasDepth && hasStencil)
            {
                ...
                const std::string testName2        = getFormatCaseName(params.src.image.format) + "_" +
                                              getFormatCaseName(params.dst.image.format) + "_separate_layouts";
                const std::string description2 = "Blit from " + getFormatCaseName(params.src.image.format) + " to " +
                                                 getFormatCaseName(params.dst.image.format) +
                                                 " with separate depth/stencil layouts";
                addTestGroup(subGroup.get(), testName2, description2,
                             addBlittingImageAllFormatsDepthStencilFormatsTests, params);
            }
        }
    ...
    for (int srcLayoutNdx = 0u; srcLayoutNdx < DE_LENGTH_OF_ARRAY(blitSrcLayouts); ++srcLayoutNdx)
    {
        ...
        for (int dstLayoutNdx = 0u; dstLayoutNdx < DE_LENGTH_OF_ARRAY(blitDstLayouts); ++dstLayoutNdx)
        {
            ...
            const std::string testName = getImageLayoutCaseName(testParams.params.src.image.operationLayout) + "_" +
                                         getImageLayoutCaseName(testParams.params.dst.image.operationLayout);
            const std::string description =
                "Blit from layout " + getImageLayoutCaseName(testParams.params.src.image.operationLayout) + " to " +
                getImageLayoutCaseName(testParams.params.dst.image.operationLayout);
                                                    ...
                                                    ExtensionUse extensionUse)
{
    ...
    for (int layerCountIndex = 0; layerCountIndex < DE_LENGTH_OF_ARRAY(layerCountsToTest); layerCountIndex++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> layerCountGroup(
            new tcu::TestCaseGroup(group->getTestContext(), layerGroupName.c_str(), layerGroupDesc.c_str()));
             ...
             ++compatibleFormatsIndex)
        {
            ...
            for (int srcFormatIndex = 0; compatibleFormats[srcFormatIndex] != VK_FORMAT_UNDEFINED; ++srcFormatIndex)
            {
                ...
                addTestGroup(layerCountGroup.get(), getFormatCaseName(params.src.image.format), description,
    ...
    for (int multiLayer = 0; multiLayer < 2; multiLayer++)
    {
        ...
        for (int barrierCount = 1; barrierCount < 4; barrierCount++)
        {
            if (layerCount != 1 || barrierCount != 1)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> barrierCountGroup(new tcu::TestCaseGroup(
                    group->getTestContext(), barrierGroupName.c_str(), barrierGroupDesc.c_str()));
                ...
                for (int srcFormatIndex = 2; srcFormatIndex < 6; ++srcFormatIndex)
                {
                    ...
                    addTestGroup(barrierCountGroup.get(), getFormatCaseName(params.src.image.format), description,
                                 addBlittingImageAllFormatsMipmapFormatTests, testParams);
                }
...
void addBlittingImageAllFormatsMipmapTests(tcu::TestCaseGroup *group, AllocationKind allocationKind,
                                           ExtensionUse extensionUse)
{
    addTestGroup(group, "from_base_level", "Generate all mipmap levels from base level",
                 addBlittingImageAllFormatsBaseLevelMipmapTests, allocationKind, extensionUse);
    addTestGroup(group, "from_previous_level", "Generate next mipmap level from previous level",
                 addBlittingImageAllFormatsPreviousLevelMipmapTests, allocationKind, extensionUse);
}
                                     ...
                                     ExtensionUse extensionUse)
{
    addTestGroup(group, "color", "Blitting image with color formats", addBlittingImageAllFormatsColorTests,
                 allocationKind, extensionUse);
    addTestGroup(group, "depth_stencil", "Blitting image with depth/stencil formats",
                 addBlittingImageAllFormatsDepthStencilTests, allocationKind, extensionUse);
    addTestGroup(group, "generate_mipmaps", "Generating mipmaps with vkCmdBlitImage()",
                 addBlittingImageAllFormatsMipmapTests, allocationKind, extensionUse);
}
...
void addResolveImageWholeTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    ...
    for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
    {
        ...
        group->addChild(new ResolveImageToImageTestCase(
            group->getTestContext(), getSampleCountCaseName(samples[samplesIndex]), description, params));
        params.imageOffset = true;
        if (allocationKind != ALLOCATION_KIND_DEDICATED)
        {
            group->addChild(new ResolveImageToImageTestCase(
                group->getTestContext(), getSampleCountCaseName(samples[samplesIndex]) + "_bind_offset", description,
                params));
        }
    }
}
                                                  ...
                                                  ExtensionUse extensionUse)
{
    ...
    for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
    {
        ...
        group->addChild(new ResolveImageToImageTestCase(group->getTestContext(),
                                                        getSampleCountCaseName(samples[samplesIndex]), description,
...
void addComputeAndTransferQueueTests(tcu::TestCaseGroup *group, AllocationKind allocationKind,
                                     ExtensionUse extensionUse)
{
    de::MovePtr<tcu::TestCaseGroup> computeGroup(
        new tcu::TestCaseGroup(group->getTestContext(), "whole_copy_before_resolving_compute",
                               "Resolve from image to image using compute queue (whole copy before resolving)"));
    ...
    for (const auto &sample : samples)
    {
        ...
        computeGroup->addChild(new ResolveImageToImageTestCase(group->getTestContext(), getSampleCountCaseName(sample),
                                                               description, params, COPY_MS_IMAGE_TO_MS_IMAGE_COMPUTE));
                                                            ...
                                                            ExtensionUse extensionUse)
{
    ...
    for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
    {
        ...
        group->addChild(new ResolveImageToImageTestCase(group->getTestContext(),
                                                        getSampleCountCaseName(samples[samplesIndex]), description,
                                                        params, COPY_MS_IMAGE_TO_MS_IMAGE_NO_CAB));
        params.imageOffset = true;
        if (allocationKind != ALLOCATION_KIND_DEDICATED)
        {
            group->addChild(new ResolveImageToImageTestCase(
    ...
    for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
        for (int srcLayoutIndex = 0; srcLayoutIndex < DE_LENGTH_OF_ARRAY(imageLayouts); ++srcLayoutIndex)
            for (int dstLayoutIndex = 0; dstLayoutIndex < DE_LENGTH_OF_ARRAY(imageLayouts); ++dstLayoutIndex)
            {
                ...
                params.imageOffset = false;
                group->addChild(new ResolveImageToImageTestCase(group->getTestContext(), testName, description, params,
                                                                COPY_MS_IMAGE_TO_MS_IMAGE));
                params.imageOffset = true;
                if (allocationKind != ALLOCATION_KIND_DEDICATED)
                {
                    group->addChild(new ResolveImageToImageTestCase(group->getTestContext(), testName + "_bind_offset",
                                                                    description, params, COPY_MS_IMAGE_TO_MS_IMAGE));
                }
            }
}
                                                  ...
                                                  ExtensionUse extensionUse)
{
    ...
    for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
    {
        ...
        group->addChild(new ResolveImageToImageTestCase(group->getTestContext(),
                                                        getSampleCountCaseName(samples[samplesIndex]), description,
                                                        params, COPY_MS_IMAGE_LAYER_TO_MS_IMAGE));
        params.imageOffset = true;
        if (allocationKind != ALLOCATION_KIND_DEDICATED)
        {
            group->addChild(new ResolveImageToImageTestCase(
                                         ...
                                         ExtensionUse extensionUse)
{
    ...
    for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
    {
        ...
        group->addChild(new ResolveImageToImageTestCase(group->getTestContext(),
                                                        getSampleCountCaseName(samples[samplesIndex]), description,
                                                        params, COPY_MS_IMAGE_TO_MS_IMAGE_MULTIREGION));
        params.imageOffset = true;
        if (allocationKind != ALLOCATION_KIND_DEDICATED)
        {
            group->addChild(new ResolveImageToImageTestCase(
                                       ...
                                       ExtensionUse extensionUse)
{
    ...
    for (int dstImageExtentIndex = 0; dstImageExtentIndex < DE_LENGTH_OF_ARRAY(imageExtents); ++dstImageExtentIndex)
    {
        ...
        for (int samplesIndex = 0; samplesIndex < DE_LENGTH_OF_ARRAY(samples); ++samplesIndex)
        {
            ...
            if (allocationKind != ALLOCATION_KIND_DEDICATED)
            {
                group->addChild(new ResolveImageToImageTestCase(testCtx, testName.str() + "_bind_offset",
            ...
            for (const auto &format : depthAndStencilFormats)
            {
                ...
                if (hasDepth)
                {
                    ...
                    for (auto sample : samples)
                    {
                        ...
                        std::string description =
                            "Copy depth component with sample count: " + getSampleCountCaseName(sample);
                        testCreateParams.imageOffset = false;
                        group->addChild(new DepthStencilMSAATestCase(
                            group->getTestContext(), testNameBase + "D_" + getSampleCountCaseName(sample), description,
                            testCreateParams));
                        ...
                        if (testCreateParams.allocationKind != ALLOCATION_KIND_DEDICATED)
                        {
                            group->addChild(new DepthStencilMSAATestCase(
                                ...
                                testNameBase + "D_" + getSampleCountCaseName(sample) + "_bind_offset", description,
                                testCreateParams));
                        }
                    }
                }
                ...
                if (hasStencil)
                {
                    ...
                    for (auto sample : samples)
                    {
                        testCreateParams.samples = sample;
                        std::string description =
                            "Copy stencil component with sample count: " + getSampleCountCaseName(sample);
                        testCreateParams.imageOffset = false;
                        group->addChild(new DepthStencilMSAATestCase(
                            group->getTestContext(), testNameBase + "S_" + getSampleCountCaseName(sample), description,
                            testCreateParams));
                        testCreateParams.imageOffset = true;
                        if (testCreateParams.allocationKind != ALLOCATION_KIND_DEDICATED)
                        {
                            group->addChild(new DepthStencilMSAATestCase(
                                group->getTestContext(),
                                testNameBase + "S_" + getSampleCountCaseName(sample) + "_bind_offset", description,
                                testCreateParams));
                        }
                    }
                }
...
void addDepthStencilCopyMSAATestGroup(tcu::TestCaseGroup *group, AllocationKind allocationKind,
                                      ExtensionUse extensionUse)
{
    ...
    testParams.extensionUse                     = extensionUse;

    testParams.copyOptions = DepthStencilMSAA::COPY_WHOLE_IMAGE;
    addTestGroup(group, "whole", "Copy from depth stencil to depth stencil with multi sample tests",
                 addDepthStencilCopyMSAATest, testParams);

    testParams.copyOptions = DepthStencilMSAA::COPY_PARTIAL;
    addTestGroup(group, "partial", "Copy from depth stencil to depth stencil with multi sample tests",
                 addDepthStencilCopyMSAATest, testParams);

    testParams.copyOptions = DepthStencilMSAA::COPY_ARRAY_TO_ARRAY;
    addTestGroup(group, "array_to_array", "Copy from array layer to array layer", addDepthStencilCopyMSAATest,
                 testParams);
}

void addBufferCopyOffsetTests(tcu::TestCaseGroup *group)
{
    de::MovePtr<tcu::TestCaseGroup> subGroup(new tcu::TestCaseGroup(
        group->getTestContext(), "buffer_to_buffer_with_offset",
        "Copy from buffer to buffer using different offsets in the source and destination buffers"));
    ...
    for (uint32_t srcOffset = 0u; srcOffset < BufferOffsetParams::kMaxOffset; ++srcOffset)
        for (uint32_t dstOffset = 0u; dstOffset < BufferOffsetParams::kMaxOffset; ++dstOffset)
        {
            ...
            addFunctionCase(subGroup.get(), de::toString(srcOffset) + "_" + de::toString(dstOffset), "",
                            bufferOffsetTest, params);
        }
...
void addResolveImageTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    ...
    addTestGroup(group, "with_regions", "Resolve from image to image (with regions)", addResolveImageWithRegionsTests,
                 allocationKind, extensionUse);
    addTestGroup(group, "whole_copy_before_resolving", "Resolve from image to image (whole copy before resolving)",
                 addResolveImageWholeCopyBeforeResolvingTests, allocationKind, extensionUse);
    addTestGroup(group, "whole_copy_before_resolving_no_cab",
                 "Resolve from image to image without using USAGE_COLOR_ATTACHMENT_BIT (whole copy before resolving)",
                 addResolveImageWholeCopyWithoutCabBeforeResolvingTests, allocationKind, extensionUse);
    addComputeAndTransferQueueTests(group, allocationKind, extensionUse);
    addTestGroup(group, "diff_layout_copy_before_resolving",
                 "Resolve from image to image (whole copy before resolving with different layouts)",
                 addResolveImageWholeCopyDiffLayoutsBeforeResolvingTests, allocationKind, extensionUse);
    addTestGroup(group, "layer_copy_before_resolving", "Resolve from image to image (layer copy before resolving)",
                 addResolveImageLayerCopyBeforeResolvingTests, allocationKind, extensionUse);
    addTestGroup(group, "copy_with_regions_before_resolving",
                 "Resolve from image to image (region copy before resolving)", addResolveCopyImageWithRegionsTests,
                 allocationKind, extensionUse);
    addTestGroup(group, "whole_array_image", "Resolve from image to image (whole array image)",
                 addResolveImageWholeArrayImageTests, allocationKind, extensionUse);
    addTestGroup(group, "whole_array_image_one_region",
                 "Resolve from image to image (whole array image with single region)",
                 addResolveImageWholeArrayImageSingleRegionTests, allocationKind, extensionUse);
    addTestGroup(group, "diff_image_size", "Resolve from image to image of different size",
                 addResolveImageDiffImageSizeTests, allocationKind, extensionUse);
}
...
void addCopiesAndBlittingTests(tcu::TestCaseGroup *group, AllocationKind allocationKind, ExtensionUse extensionUse)
{
    ...
    addTestGroup(group, "buffer_to_image", "Copy from buffer to image", addBufferToImageTests, allocationKind,
                 extensionUse);
    addTestGroup(group, "buffer_to_depthstencil", "Copy from buffer to depth/Stencil", addBufferToDepthStencilTests,
                 allocationKind, extensionUse);
    ...
    addTestGroup(group, "depth_stencil_msaa_copy", "Copy depth/stencil with MSAA", addDepthStencilCopyMSAATestGroup,
                 allocationKind, extensionUse);
}
...
{
    de::MovePtr<tcu::TestCaseGroup> copiesAndBlittingTests(
        new tcu::TestCaseGroup(testCtx, "copy_and_blit", "Copies And Blitting Tests"));

    copiesAndBlittingTests->addChild(
        createTestGroup(testCtx, "core", "Core Copies And Blitting Tests", addCoreCopiesAndBlittingTests));
    copiesAndBlittingTests->addChild(createTestGroup(testCtx, "dedicated_allocation",
                                                     "Copies And Blitting Tests For Dedicated Memory Allocation",
                                                     addDedicatedAllocationCopiesAndBlittingTests));
#ifndef CTS_USES_VULKANSC
    copiesAndBlittingTests->addChild(createTestGroup(testCtx, "copy_commands2",
                                                     "Copies And Blitting Tests using KHR_copy_commands2",
                                                     addExtensionCopiesAndBlittingTests));
#endif // CTS_USES_VULKANSC
