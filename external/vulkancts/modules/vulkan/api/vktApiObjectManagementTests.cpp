...
size_t getSafeObjectCount(const PlatformMemoryLimits &memoryLimits, size_t objectSystemMemoryUsage,
...
struct Instance
{
    ...
    static Move<VkInstance> create(const Environment &env, const Resources &, const Parameters &params)
    {
        ...
        for (const auto &extName : params.instanceExtensions)
        {
            ...
            if (extNotInCore &&
                !isExtensionSupported(instanceExts.begin(), instanceExts.end(), RequiredExtension(extName)))
    ...
    for (int d = 0; d < SingletonDevice::NUM_DEVICES; ++d)
    {
        ...
        for (int r = 0; r < 3; ++r)
        {
            ...
            for (int o = 0; o < 4; ++o)
            {
                ...
                {
                    data = 1234;
                    env.vkd.getPrivateDataEXT(*device, getObjectType<typename Object::Type>(), HandleToInt(obj.get()),
                                              **slots[i], &data);
                    if (data != 0)
            ...
            for (int o = 0; o < 4; ++o)
            {
                auto &obj = *objs[o];
                for (int i = 0; i < numSlots; ++i)
                    VK_CHECK(env.vkd.setPrivateDataEXT(*device, getObjectType<typename Object::Type>(),
            ...
            for (int o = 0; o < numSlots; ++o)
            {
                ...
                {
                    data = 1234;
                    env.vkd.getPrivateDataEXT(*device, VK_OBJECT_TYPE_PRIVATE_DATA_SLOT_EXT,
            ...
            for (int o = 0; o < numSlots; ++o)
            {
                auto &obj = **slots[o];
                for (int i = 0; i < numSlots; ++i)
                    VK_CHECK(env.vkd.setPrivateDataEXT(*device, VK_OBJECT_TYPE_PRIVATE_DATA_SLOT_EXT,
            ...
            {
                data = 1234;
                env.vkd.getPrivateDataEXT(*device, VK_OBJECT_TYPE_DEVICE, (uint64_t)(uintptr_t)(*device), **slots[i],
                                          &data);
                if (data != 0)
                    return tcu::TestStatus::fail("Expected initial value of zero for device");
            }
            for (int i = 0; i < numSlots; ++i)
                VK_CHECK(env.vkd.setPrivateDataEXT(*device, VK_OBJECT_TYPE_DEVICE, (uint64_t)(uintptr_t)(*device),
                                                   **slots[i], i * i * i + r * r + 1));
            for (int i = 0; i < numSlots; ++i)
            {
                data = 1234;
...
void addCasesWithProgs(tcu::TestCaseGroup *group, const CaseDescription<Object> &cases)
{
    for (const NamedParameters<Object> *cur = cases.paramsBegin; cur != cases.paramsEnd; ++cur)
    {
        if (cases.supportFunction == DE_NULL)
            ...
            addFunctionCaseWithPrograms(group, cur->name, "", cases.supportFunction, Object::initPrograms,
                                        cases.function, cur->parameters);
    }
}
...
tcu::TestCaseGroup *createGroup(tcu::TestContext &testCtx, const char *name, const char *desc,
...
{
    MovePtr<tcu::TestCaseGroup> objectMgmtTests(
        new tcu::TestCaseGroup(testCtx, "object_management", "Object management tests"));
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "multiple_unique_resources",
                                          "Multiple objects with per-object unique resources",
                                          s_createMultipleUniqueResourcesGroup));
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "multiple_shared_resources",
                                          "Multiple objects with shared resources",
                                          s_createMultipleSharedResourcesGroup));
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "max_concurrent", "Maximum number of concurrently live objects",
                                          s_createMaxConcurrentGroup));
#endif // CTS_USES_VULKANSC
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "multithreaded_per_thread_device",
                                          "Multithreaded object construction with per-thread device ",
                                          s_multithreadedCreatePerThreadDeviceGroup));
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "multithreaded_per_thread_resources",
                                          "Multithreaded object construction with per-thread resources",
                                          s_multithreadedCreatePerThreadResourcesGroup));
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "multithreaded_shared_resources",
                                          "Multithreaded object construction with shared resources",
                                          s_multithreadedCreateSharedResourcesGroup));
    ...
    objectMgmtTests->addChild(
        createGroup(testCtx, "single_alloc_callbacks", "Create single object", s_createSingleAllocCallbacksGroup));
#endif // CTS_USES_VULKANSC
    ...
    objectMgmtTests->addChild(
        createGroup(testCtx, "alloc_callback_fail", "Allocation callback failure", s_allocCallbackFailGroup));
#endif // CTS_USES_VULKANSC
    ...
    objectMgmtTests->addChild(createGroup(testCtx, "alloc_callback_fail_multiple",
                                          "Allocation callback failure creating multiple objects with one call",
                                          s_allocCallbackFailMultipleObjectsGroup));
#endif // CTS_USES_VULKANSC
    ...
    objectMgmtTests->addChild(createTestGroup(testCtx, "private_data", "Multiple objects with private data",
                                              createTests, s_privateDataResourcesGroup, cleanupGroup));
#endif // CTS_USES_VULKANSC
