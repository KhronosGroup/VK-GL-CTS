                ...
                for (uint32_t formatIndex = 0; formatIndex < numFormats; formatIndex++)
                {
                    ...
                    for (auto spirvData : spirvDataVect)
                    {
                        ...
                        de::MovePtr<tcu::TestCaseGroup> typeGroup(
                            new tcu::TestCaseGroup(testCtx, testname.c_str(), ""));

                        de::MovePtr<tcu::TestCaseGroup> depthGroup(
                            new tcu::TestCaseGroup(testCtx, "depth_property", ""));
                        for (uint32_t propertyNdx = 0u; propertyNdx < DEPTH_PROPERTY_LAST; propertyNdx++)
                        {
                            const std::string imageSamplerTypes =
                                getImageSamplerTypeStr((DescriptorType)descNdx, (ReadOp)opNdx,
                                                       (DepthProperty)propertyNdx, (TestType)testNdx, formatIndex);
                            const string shaderSource =
                                ...
                                "              %color = " + imageReadOp +
                                "\n"
                                "                 %36 = OpAccessChain %_ptr_Uniform_v4f %OutputData %c_u32_0 "
                                "%func_ndx\n"
                                "                       OpStore %36 %color\n"
                                "                       OpReturn\n"
                                "                       OpFunctionEnd\n"
