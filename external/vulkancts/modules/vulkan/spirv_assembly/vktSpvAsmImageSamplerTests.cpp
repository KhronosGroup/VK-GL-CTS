...
static const VkFormat optypeimageFormatMismatchVkFormat[] = {
    ...
    VK_FORMAT_R8G8B8A8_SINT,       VK_FORMAT_R16G16B16A16_UINT, VK_FORMAT_R16G16B16A16_SINT,
...
static const char *optypeimageFormatMismatchSpirvFormat[] = {"Rgba8",    "Rgba8Snorm", "Rgba8ui", "Rgba8i",
                                                             "Rgba16ui", "Rgba16i",    "Rgba16f", "R32ui",
                                                             "R32i",     "Rgba32ui",   "Rgba32i", "Rgba32f"};

static const char *optypeimageFormatMismatchCase[] = {"rgba8",    "rgba8snorm", "rgba8ui", "rgba8i",
                                                      "rgba16ui", "rgba16i",    "rgba16f", "r32ui",
                                                      "r32i",     "rgba32ui",   "rgba32i", "rgba32f"};
...
std::string getImageSamplerTypeStr(DescriptorType descType, ReadOp readOp, uint32_t depthProperty, TestType testType,
                                   int formatIndex)
{
    const string imageFormat = (testType == TESTTYPE_OPTYPEIMAGE_MISMATCH) ?
                                   optypeimageFormatMismatchSpirvFormat[formatIndex] :
            ...
            for (uint32_t testNdx = 0u; testNdx < TESTTYPE_LAST; testNdx++)
            {
                ...
                for (uint32_t formatIndex = 0; formatIndex < numFormats; formatIndex++)
                {
                    ...
                    const std::string imageSamplerTypes =
                        getImageSamplerTypeStr((DescriptorType)descNdx, (ReadOp)opNdx, DEPTH_PROPERTY_NON_DEPTH,
                                                       ...
                                                       uint32_t formatIndex)
{
    ...
    const std::string imageSamplerTypes =
        getImageSamplerTypeStr(descriptorType, readOp, depthProperty, testType, formatIndex);
    ...
    source["decoration"] = "                       OpDecorate %_arr_f32_u32_64 ArrayStride 4\n"
            ...
            for (uint32_t testNdx = 0u; testNdx < TESTTYPE_LAST; testNdx++)
            {
                ...
                for (uint32_t formatIndex = 0; formatIndex < formatCount; formatIndex++)
                {
                    ...
                    const char *groupname = testNdx == TESTTYPE_OPTYPEIMAGE_MISMATCH ?
                                                optypeimageFormatMismatchCase[formatIndex] :
                    ...
                    if ((DescriptorType)descNdx == DESCRIPTOR_TYPE_SAMPLED_IMAGE)
                    {
                        vector<tcu::Vec4> dummyData;
                        resources.inputs.push_back(
                            Resource(BufferSp(new Vec4Buffer(dummyData)), VK_DESCRIPTOR_TYPE_SAMPLER));
                    }
                    ...
                    const map<string, string> fragments = generateGraphicsImageSamplerSource(
                        (ReadOp)opNdx, (DescriptorType)descNdx, (TestType)testNdx, DEPTH_PROPERTY_NON_DEPTH,
                        (uint32_t)resources.inputs.size(),
                        (uint32_t)((formatIndex + 1) % optypeimageFormatMismatchFormatCount));
                    ...
                    createTestForStage(VK_SHADER_STAGE_VERTEX_BIT, "shader_vert", defaultColors, defaultColors,
                                       ...
                                       noExtensions, vulkanFeatures, typeGroup.get());
                    ...
                    createTestForStage(VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT, "shader_tessc", defaultColors,
                                       ...
                                       noInterfaces, noExtensions, vulkanFeatures, typeGroup.get());

                    createTestForStage(VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT, "shader_tesse", defaultColors,
                                       defaultColors, fragments, noSpecConstants, noPushConstants, resources,
                                       noInterfaces, noExtensions, vulkanFeatures, typeGroup.get());

                    createTestForStage(VK_SHADER_STAGE_GEOMETRY_BIT, "shader_geom", defaultColors, defaultColors,
                                       fragments, noSpecConstants, noPushConstants, resources, noInterfaces,
                                       noExtensions, vulkanFeatures, typeGroup.get());
...
void addGraphicsDepthPropertyTest(tcu::TestCaseGroup *group)
{
    ...
    for (uint32_t propertyNdx = 0u; propertyNdx < DEPTH_PROPERTY_LAST; propertyNdx++)
    {
        ...
        {
            de::MovePtr<tcu::TestCaseGroup> readOpGroup(
                new tcu::TestCaseGroup(testCtx, getReadOpName((ReadOp)opNdx), ""));

            for (uint32_t descNdx = DESCRIPTOR_TYPE_SAMPLED_IMAGE; descNdx < DESCRIPTOR_TYPE_LAST; descNdx++)
            {
                de::MovePtr<tcu::TestCaseGroup> descGroup(
                    new tcu::TestCaseGroup(testCtx, getDescriptorName((DescriptorType)descNdx), ""));
                ...
                const map<string, string> fragments = generateGraphicsImageSamplerSource(
                    (ReadOp)opNdx, (DescriptorType)descNdx, TESTTYPE_LOCAL_VARIABLES, (DepthProperty)propertyNdx,
                    (uint32_t)resources.inputs.size(), 0);

                getDefaultColors(defaultColors);

                createTestForStage(VK_SHADER_STAGE_FRAGMENT_BIT, "shader_frag", defaultColors, defaultColors, fragments,
                                   noSpecConstants, noPushConstants, resources, noInterfaces, noExtensions,
                                   vulkanFeatures, descGroup.get());
...
tcu::TestCaseGroup *createImageSamplerComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "image_sampler", "Compute tests for combining images and samplers."));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "image_sampler", "Graphics tests for combining images and samplers."));
