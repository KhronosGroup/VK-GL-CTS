...
void addCopyingComputeGroup(tcu::TestCaseGroup *group)
{
    ...
    std::vector<BufferType> bufferTypes{{"ubo",
    ...
    std::vector<CopyingMethod> copyingMethods{{"copy_object",
...
tcu::TestCaseGroup *createEmptyStructComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "empty_struct", "Tests empty structs in UBOs and SSBOs"));

    addTestGroup(group.get(), "copying", "Test copying struct which contains an empty struct", addCopyingComputeGroup);
    addTestGroup(group.get(), "pointer_comparison", "Test pointer comparisons of empty struct members",
                 addPointerComparisionComputeGroup);
    addTestGroup(group.get(), "function", "Test empty structs as function arguments or return type",
