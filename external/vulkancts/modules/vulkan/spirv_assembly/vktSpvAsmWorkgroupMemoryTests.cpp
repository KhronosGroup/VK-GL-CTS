...
bool checkResultsFloat16(const vector<Resource> &inputs, const vector<AllocationSp> &outputAllocs,
                         const vector<Resource> &expectedOutputs, tcu::TestLog &log)
{
    ...
    for (size_t i = 0; i < expectedBytes.size() / sizeof(uint16_t); i++)
    {
...
bool checkResultsFloat32(const vector<Resource> &inputs, const vector<AllocationSp> &outputAllocs,
                         const vector<Resource> &expectedOutputs, tcu::TestLog &log)
{
    DE_UNREF(inputs);
    DE_UNREF(log);
    ...
    for (size_t i = 0; i < expectedBytes.size() / sizeof(uint32_t); i++)
    {
        if (results[i] == expected[i])
            continue;
...
bool isNanFloat64(uint64_t f)
{
    // NaN has full exponent bits and non-zero mantissa.
    const uint64_t exponentBits = 0x7ff0000000000000;
    const uint64_t mantissaBits = 0x000fffffffffffff;
    return ((f & exponentBits) == exponentBits && (f & mantissaBits) != 0);
}

bool checkResultsFloat64(const vector<Resource> &inputs, const vector<AllocationSp> &outputAllocs,
                         const vector<Resource> &expectedOutputs, tcu::TestLog &log)
{
    DE_UNREF(inputs);
    DE_UNREF(log);
    ...
    for (size_t i = 0; i < expectedBytes.size() / sizeof(uint64_t); i++)
    {
        if (results[i] == expected[i])
            continue;
...
void addComputeWorkgroupMemoryTests(tcu::TestCaseGroup *group)
{
    ...
    */
    ...
    // int64
    {
        ...
        vector<int64_t> inputData = getInt64s(rnd, numElements);
        vector<int64_t> outputData;
        ComputeShaderSpec spec;
    ...
    // int32
    {
        ...
        vector<int32_t> inputData = getInt32s(rnd, numElements);
    ...
    // int16
    {
        ...
        vector<int16_t> inputData = getInt16s(rnd, numElements);
    ...
    // int8
    {
        ...
        vector<int8_t> inputData = getInt8s(rnd, numElements);
        vector<int8_t> outputData;
        ComputeShaderSpec spec;
...
tcu::TestCaseGroup *createWorkgroupMemoryComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "workgroup_memory", "Compute tests for workgroup memory.."));
