...
namespace
{
static const uint32_t arrayStrideInBytesUniform = 16u; // from the spec
...
bool computeCheckBuffers(const std::vector<Resource> &originalInts, const vector<AllocationSp> &outputAllocs,
...
vector<int8_t> data8bit(const ShaderTemplate std, de::Random &rnd, const bool isData = true)
{
    const int size = getStructSize(std);
    if (!isData)
        return vector<int8_t>(size, 0);
    return getInt8s(rnd, size);
}

vector<int32_t> data32bit(const ShaderTemplate std, de::Random &rnd, const bool isData = true)
{
    const int size = getStructSize(std);
    if (!isData)
        return vector<int32_t>(size, 0);
...
bool compareStruct(const resultType *returned, const originType *original)
{
    ...
    {
        if (static_cast<int8_t>(originToCompare[ndx]) != static_cast<int8_t>(resultToCompare[ndx]))
            return false;
    }
    return true;
}
...
template <typename originType, typename resultType, ShaderTemplate funcOrigin, ShaderTemplate funcResult>
bool checkStruct(const std::vector<Resource> &originalFloats, const vector<AllocationSp> &outputAllocs,
                 const std::vector<Resource> & /* expectedOutputs */, tcu::TestLog & /* log */)
{
...
template <typename originType, typename resultType, uint32_t compositCount>
bool checkUniformsArray(const std::vector<Resource> &originalFloats, const vector<AllocationSp> &outputAllocs,
                        const std::vector<Resource> & /* expectedOutputs */, tcu::TestLog & /* log */)
{
    const uint32_t originTypeSize = static_cast<uint32_t>(sizeof(originType));

    DE_ASSERT((originTypeSize * compositCount) <=
              arrayStrideInBytesUniform); // one element of array can't be bigger then 16B
    ...
    for (uint32_t outputNdx = 0; outputNdx < static_cast<uint32_t>(outputAllocs.size()); ++outputNdx)
    {
        ...
        {
            for (uint32_t ndxData = 0u; ndxData < compositCount; ++ndxData)
            {
                if (static_cast<int8_t>(*original) != static_cast<int8_t>(*returned))
                    return false;
                original++;
                returned++;
            }
...
bool checkUniformsArrayConstNdx(const std::vector<Resource> &originalFloats, const vector<AllocationSp> &outputAllocs,
                                const std::vector<Resource> & /* expectedOutputs */, tcu::TestLog & /* log */)
{
    const uint32_t originTypeSize = static_cast<uint32_t>(sizeof(originType));
    ...
    for (uint32_t outputNdx = 0; outputNdx < static_cast<uint32_t>(outputAllocs.size()); ++outputNdx)
    {
        ...
        uint32_t idx = (arrayStrideInBytesUniform / originTypeSize) * ndxConts;

        for (int ndx = 0; ndx < elemntsNumber; ++ndx)
        {
            for (int ndxData = 0; ndxData < compositCount; ++ndxData)
            {
                if (static_cast<int8_t>(original[idx + ndxData]) != static_cast<int8_t>(*returned))
                    return false;
                returned++;
            }
        }
...
void addCompute8bitStorage32To8Group(tcu::TestCaseGroup *group)
{
    ...
    { // Integers
        const char sintTypes[] = "%i8       = OpTypeInt 8 1\n"
        ...
        const CompositeType cTypes[] = {
            {"scalar_sint", sintTypes, "i32", "i8", "OpSConvert",
             "OpDecorate %i32arr ArrayStride 4\nOpDecorate %i8arr ArrayStride 1\n", numElements},
            {"scalar_uint", uintTypes, "u32", "u8", "OpUConvert",
             "OpDecorate %u32arr ArrayStride 4\nOpDecorate %u8arr ArrayStride 1\n", numElements},
            {"vector_sint", sintTypes, "v2i32", "v2i8", "OpSConvert",
             "OpDecorate %v2i32arr ArrayStride 8\nOpDecorate %v2i8arr ArrayStride 2\n", numElements / 2},
            {"vector_uint", uintTypes, "v2u32", "v2u8", "OpUConvert",
             "OpDecorate %v2u32arr ArrayStride 8\nOpDecorate %v2u8arr ArrayStride 2\n", numElements / 2},
        };

        vector<int32_t> inputs = getInt32s(rnd, numElements);
        vector<int8_t> outputs(inputs.size());

        for (uint32_t numNdx = 0; numNdx < inputs.size(); ++numNdx)
            outputs[numNdx] = (static_cast<int8_t>(0xff & inputs[numNdx]));
...
void addCompute8bitUniform8To32Group(tcu::TestCaseGroup *group)
{
    ...
    { // Integers
        ...
        for (uint32_t tyIdx = 0; tyIdx < DE_LENGTH_OF_ARRAY(cTypes); ++tyIdx)
        {
            ...
            vector<int8_t> inputs = getInt8s(rnd, (arrayStrideInBytesUniform / static_cast<uint32_t>(sizeof(int8_t))) *
            ...
            spec.inputs.push_back(
                Resource(BufferSp(new Int8Buffer(inputs)), CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].dtype));
            ...
            if (cTypes[tyIdx].componentsCount == 4)
                spec.verifyIO = checkUniformsArray<int8_t, int32_t, 4>;
            else
                spec.verifyIO = checkUniformsArray<int8_t, int32_t, 1>;
...
void addCompute8bitStoragePushConstant8To32Group(tcu::TestCaseGroup *group)
{
    ...
    { // integers
        ...
        const CompositeType cTypes[] = {
            {"scalar_sint", true, sintTypes, "i32", "i8", "OpSConvert",
             "OpDecorate %i32arr ArrayStride 4\nOpDecorate %i8arr ArrayStride 1\n", numElements},
            {"scalar_uint", false, uintTypes, "u32", "u8", "OpUConvert",
             "OpDecorate %u32arr ArrayStride 4\nOpDecorate %u8arr ArrayStride 1\n", numElements},
            {"vector_sint", true, sintTypes, "v2i32", "v2i8", "OpSConvert",
             "OpDecorate %v2i32arr ArrayStride 8\nOpDecorate %v2i8arr ArrayStride 2\n", numElements / 2},
            {"vector_uint", false, uintTypes, "v2u32", "v2u8", "OpUConvert",
             "OpDecorate %v2u32arr ArrayStride 8\nOpDecorate %v2u8arr ArrayStride 2\n", numElements / 2},
        };

        vector<int8_t> inputs = getInt8s(rnd, numElements);
        vector<int32_t> sOutputs;
        vector<int32_t> uOutputs;
        const uint8_t signBitMask     = 0x80;
        ...
        for (uint32_t numNdx = 0; numNdx < inputs.size(); ++numNdx)
        {
            uOutputs.push_back(static_cast<uint8_t>(inputs[numNdx]));
...
void addCompute8bitStorage16To8Group(tcu::TestCaseGroup *group)
{
    ...
    { // Integers
        ...
        const CompositeType cTypes[] = {
            {"scalar_sint", sintTypes, "i16", "i8", "OpSConvert",
             "OpDecorate %i16arr ArrayStride 2\nOpDecorate %i8arr ArrayStride 1\n", numElements},
            {"scalar_uint", uintTypes, "u16", "u8", "OpUConvert",
             "OpDecorate %u16arr ArrayStride 2\nOpDecorate %u8arr ArrayStride 1\n", numElements},
            {"vector_sint", sintTypes, "v2i16", "v2i8", "OpSConvert",
             "OpDecorate %v2i16arr ArrayStride 4\nOpDecorate %v2i8arr ArrayStride 2\n", numElements / 2},
            {"vector_uint", uintTypes, "v2u16", "v2u8", "OpUConvert",
        ...
        for (uint32_t numNdx = 0; numNdx < inputs.size(); ++numNdx)
            outputs.push_back(static_cast<int8_t>(0xff & inputs[numNdx]));
...
void addCompute8bitUniform8To16Group(tcu::TestCaseGroup *group)
{
    ...
    { // Integers
        ...
        const CompositeType cTypes[] = {
            ...
            {"scalar_uint", uintTypes, "u16", "u8", "OpUConvert",
             "OpDecorate %u16arr ArrayStride 2\nOpDecorate %u8arr ArrayStride 16\n", 1},
            {"vector_sint", sintTypes, "v4i16", "v4i8", "OpSConvert",
             "OpDecorate %v4i16arr ArrayStride 8\nOpDecorate %v4i8arr ArrayStride 16\n", 4},
            {"vector_uint", uintTypes, "v4u16", "v4u8", "OpUConvert",
             "OpDecorate %v4u16arr ArrayStride 8\nOpDecorate %v4u8arr ArrayStride 16\n", 4},
        };

        vector<int16_t> outputs(numElements);
        ...
        for (uint32_t tyIdx = 0; tyIdx < DE_LENGTH_OF_ARRAY(cTypes); ++tyIdx)
        {
            ...
            if (cTypes[tyIdx].componentsCount == 4)
                spec.verifyIO = checkUniformsArray<int8_t, int16_t, 4>;
            else
                spec.verifyIO = checkUniformsArray<int8_t, int16_t, 1>;
...
void addCompute8bitStoragePushConstant8To16Group(tcu::TestCaseGroup *group)
{
    ...
    { // integers
        ...
        const CompositeType cTypes[] = {
            {"scalar_sint", true, sintTypes, "i16", "i8", "OpSConvert",
             "OpDecorate %i16arr ArrayStride 2\nOpDecorate %i8arr ArrayStride 1\n", numElements},
            {"scalar_uint", false, uintTypes, "u16", "u8", "OpUConvert",
             "OpDecorate %u16arr ArrayStride 2\nOpDecorate %u8arr ArrayStride 1\n", numElements},
            {"vector_sint", true, sintTypes, "v2i16", "v2i8", "OpSConvert",
             "OpDecorate %v2i16arr ArrayStride 4\nOpDecorate %v2i8arr ArrayStride 2\n", numElements / 2},
            {"vector_uint", false, uintTypes, "v2u16", "v2u8", "OpUConvert",
             "OpDecorate %v2u16arr ArrayStride 4\nOpDecorate %v2u8arr ArrayStride 2\n", numElements / 2},
        };

        vector<int8_t> inputs = getInt8s(rnd, numElements);
        vector<int16_t> sOutputs;
        vector<int16_t> uOutputs;
        const uint8_t signBitMask     = 0x80;
        const uint16_t signExtendMask = 0xff00;
        ...
        for (uint32_t numNdx = 0; numNdx < inputs.size(); ++numNdx)
        {
            uOutputs.push_back(static_cast<uint8_t>(inputs[numNdx]));
            if (inputs[numNdx] & signBitMask)
                sOutputs.push_back(static_cast<int16_t>(inputs[numNdx] | signExtendMask));
            else
                sOutputs.push_back(static_cast<int16_t>(inputs[numNdx]));
        }
...
void addCompute8bitStorageBuffer8To8Group(tcu::TestCaseGroup *group)
{
    ...
    de::Random rnd(deStringHash(group->getName()));
    const int numElements         = 128;
    const vector<int8_t> int8Data = getInt8s(rnd, numElements);
    const vector<int8_t> int8UnusedData(numElements, 0);
    ComputeShaderSpec spec;
...
void addCompute8bitStorageUniform8StructTo32StructGroup(tcu::TestCaseGroup *group)
{
    ...
    const StringTemplate shaderTemplate(
        ...
        "%i32Struct = OpTypeStruct %i32 %v2i32 %v3i32 %v4i32 %i32arr3 %struct32arr11 %v2i32arr11 %i32 %v3i32arr11 "
        "%v4i32arr3\n"
        "\n"
    ...
    { // int
        vector<int32_t> int32Data = data32bit(SHADERTEMPLATE_STRIDE32BIT_STD430, rnd, false);

        vector<int8_t> in8DData = data8bit(SHADERTEMPLATE_STRIDE8BIT_STD140, rnd);
        ComputeShaderSpec spec;
        map<string, string> specs;
        ...
        spec.verifyIO =
            checkStruct<int8_t, int32_t, SHADERTEMPLATE_STRIDE8BIT_STD140, SHADERTEMPLATE_STRIDE32BIT_STD430>;
        spec.inputs.push_back(
            Resource(BufferSp(new Int8Buffer(in8DData)), CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].dtype));
...
void addCompute8bitStorageUniform32StructTo8StructGroup(tcu::TestCaseGroup *group)
{
    ...
    { // Int
        vector<int8_t> int8Data = data8bit(SHADERTEMPLATE_STRIDE8BIT_STD430, rnd, false);
        ...
        spec.numWorkGroups = IVec3(structData.structArraySize, structData.nestedArraySize, 1);
        spec.verifyIO =
            checkStruct<int32_t, int8_t, SHADERTEMPLATE_STRIDE32BIT_STD140, SHADERTEMPLATE_STRIDE8BIT_STD430>;

        spec.inputs.push_back(
            Resource(BufferSp(new Int32Buffer(int32DData)), CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].dtype));
...
void addCompute8bitStorage8bitStructMixedTypesGroup(tcu::TestCaseGroup *group)
{
    tcu::TestContext &testCtx = group->getTestContext();
    de::Random rnd(deStringHash(group->getName()));
    vector<int8_t> outData = data8bit(SHADERTEMPLATE_STRIDEMIX_STD430, rnd, false);
    ...
    const StringTemplate shaderTemplate(
        ...
        "%structIn           = OpTypeStruct %i8 %i32 %v2i8 %v2i32 %v3i8 %v3i32 %v4i8 %v4i32 %sNestedArr11In "
        "%sb8Arr11In %sb32Arr11In\n"
        ...
        "%structOut          = OpTypeStruct %i8 %i32 %v2i8 %v2i32 %v3i8 %v3i32 %v4i8 %v4i32 %sNestedArr11Out "
        "%sb8Arr11Out %sb32Arr11Out\n"
    ...
    { // int
        ...
        vector<int8_t> inData =
            isUniform ? data8bit(SHADERTEMPLATE_STRIDEMIX_STD140, rnd) : data8bit(SHADERTEMPLATE_STRIDEMIX_STD430, rnd);
        ComputeShaderSpec spec;
        ...
        specs["capability"]   = "";
        specs["uniformPtr"]   = isUniform ? "%i8inPtr     = OpTypePointer Uniform %i8\n"
        ...
        spec.numWorkGroups = IVec3(structData.structArraySize, structData.nestedArraySize, 1);
        spec.verifyIO =
            isUniform ? checkStruct<int8_t, int8_t, SHADERTEMPLATE_STRIDEMIX_STD140, SHADERTEMPLATE_STRIDEMIX_STD430> :
                        checkStruct<int8_t, int8_t, SHADERTEMPLATE_STRIDEMIX_STD430, SHADERTEMPLATE_STRIDEMIX_STD430>;
...
void addGraphics8BitStorageUniformInt32To8Group(tcu::TestCaseGroup *testGroup)
{
    ...
    const uint32_t numDataPoints = 256u;
    RGBA defaultColors[4];
    GraphicsResources resources;
    vector<string> extensions;
    const StringTemplate capabilities("OpCapability ${cap}\n");
    vector<int8_t> outputs(numDataPoints);
    ...
    const StringTemplate scalarPreMain("${itype8} = OpTypeInt 8 ${signed}\n"
    ...
    for (uint32_t catIdx = 0; catIdx < DE_LENGTH_OF_ARRAY(categories); ++catIdx)
    {
        ...
        vector<int32_t> inputs =
            getInt32s(rnd, ((arrayStrideInBytesUniform / static_cast<uint32_t>(sizeof(int32_t))) * numDataPoints) /
                               categories[catIdx].numElements);

        if (0 != (arrayStrideInBytesUniform - static_cast<uint32_t>(sizeof(int32_t)) * categories[catIdx].numElements))
        ...
        resources.inputs.push_back(
            Resource(BufferSp(new Int32Buffer(inputs)), CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].dtype));
        ...
        for (uint32_t factIdx = 0; factIdx < DE_LENGTH_OF_ARRAY(intFacts); ++factIdx)
        {
            map<string, string> specs;
            VulkanFeatures features;
            string name = string(CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].name) + "_" + categories[catIdx].name +
                          "_" + intFacts[factIdx].name;
...
void addGraphics8BitStorageUniformInt8To32Group(tcu::TestCaseGroup *testGroup)
{
    ...
    RGBA defaultColors[4];
    vector<int32_t> outputs(numDataPoints);
    GraphicsResources resources;
    vector<string> extensions;
    const StringTemplate capabilities("OpCapability ${cap}\n");
    ...
    for (uint32_t catIdx = 0; catIdx < DE_LENGTH_OF_ARRAY(categories); ++catIdx)
    {
        resources.inputs.clear();
        vector<int8_t> inputs = getInt8s(rnd, (arrayStrideInBytesUniform / static_cast<uint32_t>(sizeof(int8_t))) *
                                                  (numDataPoints / categories[catIdx].numElements));
        resources.inputs.push_back(
            Resource(BufferSp(new Int8Buffer(inputs)), CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].dtype));
        for (uint32_t factIdx = 0; factIdx < DE_LENGTH_OF_ARRAY(intFacts); ++factIdx)
            for (uint32_t constIndexIdx = 0; constIndexIdx < DE_LENGTH_OF_ARRAY(constantIndices); ++constIndexIdx)
            {
                ...
                map<string, string> specs;
                VulkanFeatures features;
                string name = string(CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].name) + "_" +
                              categories[catIdx].name + "_" + intFacts[factIdx].name;
                ...
                resources.outputs.push_back(
                    Resource(BufferSp(new Int32Buffer(outputs)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
                if (useConstIdx)
                {
                    ...
                    case 0:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 2, 0>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 1, 0>;
                        break;
                    case 4:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 2, 4>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 1, 4>;
                        break;
                    case 5:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 2, 5>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 1, 5>;
                        break;
                    case 6:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 2, 6>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int32_t, 1, 6>;
                        break;
                    default:
                        DE_FATAL("Impossible");
                        break;
                ...
                {
                    if (categories[catIdx].numElements == 2)
                        resources.verifyIO = checkUniformsArray<int8_t, int32_t, 2>;
                    else
                        resources.verifyIO = checkUniformsArray<int8_t, int32_t, 1>;
                }
...
void addGraphics8BitStoragePushConstantInt8To32Group(tcu::TestCaseGroup *testGroup)
{
    ...
    vector<int8_t> inputs        = getInt8s(rnd, numDataPoints);
    vector<int32_t> sOutputs;
    vector<int32_t> uOutputs;
    PushConstants pcs;
    GraphicsResources resources;
    ...
    { // Vector cases
        const StringTemplate preMain("    %${base_type8} = OpTypeInt 8 ${signed}\n"
...
void addGraphics8BitStorageUniformInt16To8Group(tcu::TestCaseGroup *testGroup)
{
    ...
    fragments["extension"] = "OpCapability StorageUniform16\n"
                             "OpExtension \"SPV_KHR_storage_buffer_storage_class\"\n"
                             "OpExtension \"SPV_KHR_8bit_storage\"\n"
                             "OpExtension \"SPV_KHR_16bit_storage\"\n";
    ...
    for (uint32_t catIdx = 0; catIdx < DE_LENGTH_OF_ARRAY(categories); ++catIdx)
    {
        resources.inputs.clear();
        resources.outputs.clear();
        vector<int16_t> inputs =
            getInt16s(rnd, ((arrayStrideInBytesUniform / static_cast<uint32_t>(sizeof(int16_t))) * numDataPoints) /
                               categories[catIdx].numElements);
        vector<int8_t> outputs(numDataPoints / categories[catIdx].numElements);

        switch (categories[catIdx].numElements)
        {
        case 1:
            resources.verifyIO = checkUniformsArray<int16_t, int8_t, 1>;
            break;
        case 4:
            resources.verifyIO = checkUniformsArray<int16_t, int8_t, 4>;
            break;
        default:
            DE_FATAL("Impossible");
            break;
        ...
        resources.inputs.push_back(
            Resource(BufferSp(new Int16Buffer(inputs)), CAPABILITIES[UNIFORM_AND_STORAGEBUFFER_TEST].dtype));
        resources.outputs.push_back(Resource(BufferSp(new Int8Buffer(outputs)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
...
void addGraphics8BitStorageUniformInt8To16Group(tcu::TestCaseGroup *testGroup)
{
    ...
    map<string, string> fragments;
    const uint32_t numDataPoints = 256;
    vector<int16_t> outputs(numDataPoints);
    RGBA defaultColors[4];
    GraphicsResources resources;
    vector<string> extensions;
    const StringTemplate capabilities("OpCapability ${cap}\n");
        ...
        for (uint32_t factIdx = 0; factIdx < DE_LENGTH_OF_ARRAY(intFacts); ++factIdx)
            for (uint32_t constIndexIdx = 0; constIndexIdx < DE_LENGTH_OF_ARRAY(constantIndices); ++constIndexIdx)
            {
                ...
                if (useConstIdx)
                {
                    ...
                    case 0:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 2, 0>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 1, 0>;
                        break;
                    case 4:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 2, 4>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 1, 4>;
                        break;
                    case 5:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 2, 5>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 1, 5>;
                        break;
                    case 6:
                        if (categories[catIdx].numElements == 2)
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 2, 6>;
                        else
                            resources.verifyIO = checkUniformsArrayConstNdx<int8_t, int16_t, 1, 6>;
                        break;
                    default:
                        DE_FATAL("Impossible");
                        break;
                ...
                {
                    if (categories[catIdx].numElements == 2)
                        resources.verifyIO = checkUniformsArray<int8_t, int16_t, 2>;
                    else
                        resources.verifyIO = checkUniformsArray<int8_t, int16_t, 1>;
                }
    ...
    { // Scalar cases
        ...
        { // signed int
            ...
            for (uint32_t constIndexIdx = 0; constIndexIdx < DE_LENGTH_OF_ARRAY(constantIndices); ++constIndexIdx)
            {
                ...
                resources.outputs.clear();
                resources.outputs.push_back(Resource(BufferSp(new Int16Buffer(useConstIdx ? constIdxData : sOutputs)),
                                                     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
        ...
        { // signed int
            ...
            for (uint32_t constIndexIdx = 0; constIndexIdx < DE_LENGTH_OF_ARRAY(constantIndices); ++constIndexIdx)
            {
                ...
                resources.outputs.clear();
                resources.outputs.push_back(Resource(BufferSp(new Int16Buffer(useConstIdx ? constIdxData : uOutputs)),
                                                     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
...
void addGraphics8BitStorageUniformStruct8To32Group(tcu::TestCaseGroup *testGroup)
{
    ...
    RGBA defaultColors[4];
    const StringTemplate capabilities("OpCapability ${cap}\n");
    vector<int32_t> i32Data = data32bit(SHADERTEMPLATE_STRIDE32BIT_STD430, rnd, false);
    ...
    const StringTemplate preMain(
        ...
        "%struct32arr11 = OpTypeArray %struct32 %c_u32_11\n"
        "%i32Struct = OpTypeStruct %${32type} %v2${32type} %v3${32type} %v4${32type} %i32arr3 %struct32arr11 "
        "%v2i32arr11 %${32type} %v3i32arr11 %v4i32arr3\n"
        "\n"
        "%i8StructArr7  = OpTypeArray %i8Struct %c_u32_7\n"
        "%i32StructArr7 = OpTypeArray %i32Struct %c_u32_7\n"
        "%SSBO_IN       = OpTypeStruct %i8StructArr7\n"
    ...
    const StringTemplate testFun(
        ...
        "%Sv2i32dst_1  = OpAccessChain %v2i32ptr %ssboOUT %zero %valLoopNdx %c_i32_5 %valInsideLoopNdx %c_i32_1 "
        "%c_i32_1\n"
        "OpStore %Sv2i32dst_1 %Sv2i32_1\n"
        "\n"
        ...
        "%Sv2i32dst_2  = OpAccessChain %v2i32ptr %ssboOUT %zero %valLoopNdx %c_i32_5 %valInsideLoopNdx %c_i32_1 "
        "%c_i32_2\n"
        "OpStore %Sv2i32dst_2 %Sv2i32_2\n"
        "\n"
        //Array with 3 elements
    ...
    for (uint32_t capIdx = 0; capIdx < DE_LENGTH_OF_ARRAY(CAPABILITIES); ++capIdx)
        for (uint32_t intFactsNdx = 0; intFactsNdx < DE_LENGTH_OF_ARRAY(intFacts); ++intFactsNdx)
        {
            const bool isUniform  = (VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER == CAPABILITIES[capIdx].dtype);
            vector<int8_t> i8Data = isUniform ? data8bit(SHADERTEMPLATE_STRIDE8BIT_STD140, rnd) :
                                                data8bit(SHADERTEMPLATE_STRIDE8BIT_STD430, rnd);
            GraphicsResources resources;
            map<string, string> specs;
            VulkanFeatures features;
            ...
            specs["stridei8"]  = getStructShaderComponet(isUniform ? SHADERTEMPLATE_STRIDE8BIT_STD140 :
                                                                     SHADERTEMPLATE_STRIDE8BIT_STD430);
            specs["stridei32"] = getStructShaderComponet(SHADERTEMPLATE_STRIDE32BIT_STD430);
            specs["32Storage"] = "StorageBuffer";
            ...
            resources.outputs.push_back(
                Resource(BufferSp(new Int32Buffer(i32Data)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
            if (isUniform)
                resources.verifyIO =
                    checkStruct<int8_t, int32_t, SHADERTEMPLATE_STRIDE8BIT_STD140, SHADERTEMPLATE_STRIDE32BIT_STD430>;
            else
                resources.verifyIO =
                    checkStruct<int8_t, int32_t, SHADERTEMPLATE_STRIDE8BIT_STD430, SHADERTEMPLATE_STRIDE32BIT_STD430>;
...
void addGraphics8BitStorageUniformStruct32To8Group(tcu::TestCaseGroup *testGroup)
{
    ...
    RGBA defaultColors[4];
    const StringTemplate capabilities("OpCapability ${cap}\n");
    vector<int8_t> i8Data = data8bit(SHADERTEMPLATE_STRIDE8BIT_STD430, rnd, false);
    ...
    for (uint32_t capIdx = 0; capIdx < DE_LENGTH_OF_ARRAY(CAPABILITIES); ++capIdx)
        for (uint32_t intFactsNdx = 0; intFactsNdx < DE_LENGTH_OF_ARRAY(intFacts); ++intFactsNdx)
        {
            ...
            vector<int32_t> i32Data = isUniform ? data32bit(SHADERTEMPLATE_STRIDE32BIT_STD140, rnd) :
                                                  data32bit(SHADERTEMPLATE_STRIDE32BIT_STD430, rnd);
            GraphicsResources resources;
            VulkanFeatures features;

            specs["cap"]       = CAPABILITIES[STORAGE_BUFFER_TEST].cap;
            specs["stridei8"]  = getStructShaderComponet(SHADERTEMPLATE_STRIDE8BIT_STD430);
            specs["stridei32"] = getStructShaderComponet(isUniform ? SHADERTEMPLATE_STRIDE32BIT_STD140 :
                                                                     SHADERTEMPLATE_STRIDE32BIT_STD430);
            specs["8Storage"]  = "StorageBuffer";
            ...
            resources.outputs.push_back(
                Resource(BufferSp(new Int8Buffer(i8Data)), CAPABILITIES[STORAGE_BUFFER_TEST].dtype));
            if (isUniform)
                resources.verifyIO =
                    checkStruct<int32_t, int8_t, SHADERTEMPLATE_STRIDE32BIT_STD140, SHADERTEMPLATE_STRIDE8BIT_STD430>;
            else
                resources.verifyIO =
                    checkStruct<int32_t, int8_t, SHADERTEMPLATE_STRIDE32BIT_STD430, SHADERTEMPLATE_STRIDE8BIT_STD430>;
...
void addGraphics8bitStorage8bitStructMixedTypesGroup(tcu::TestCaseGroup *group)
{
    ...
    const StringTemplate preMain("\n" //Types
                                 ...
                                 "%sNestedIn          = OpTypeStruct %i8 %i32 %v2b8NestedArr11In %b32NestedArr11In\n"
                                 "%sNestedArr11In     = OpTypeArray %sNestedIn %c_u32_11\n"
                                 "%structIn           = OpTypeStruct %i8 %i32 %v2i8 %v2i32 %v3i8 %v3i32 %v4i8 %v4i32 "
                                 "%sNestedArr11In %sb8Arr11In %sb32Arr11In\n"
                                 "%structArr7In       = OpTypeArray %structIn %c_u32_7\n"
                                 "%v2b8NestedArr11Out = OpTypeArray %v2i8 %c_u32_11\n"
                                 "%b32NestedArr11Out  = OpTypeArray %i32 %c_u32_11\n"
                                 "%sb8Arr11Out        = OpTypeArray %i8 %c_u32_11\n"
                                 ...
                                 "%sNestedOut         = OpTypeStruct %i8 %i32 %v2b8NestedArr11Out %b32NestedArr11Out\n"
                                 "%sNestedArr11Out    = OpTypeArray %sNestedOut %c_u32_11\n"
                                 "%structOut          = OpTypeStruct %i8 %i32 %v2i8 %v2i32 %v3i8 %v3i32 %v4i8 %v4i32 "
                                 "%sNestedArr11Out %sb8Arr11Out %sb32Arr11Out\n"
                                 "%structArr7Out      = OpTypeArray %structOut %c_u32_7\n"
                                 "\n" //Pointers
                                 "${uniformPtr}"
                                 "%i8outPtr    = OpTypePointer StorageBuffer %i8\n"
    ...
    { // int
        ...
        resources.outputs.push_back(
            Resource(BufferSp(new Int8Buffer(outData)), CAPABILITIES[STORAGE_BUFFER_TEST].dtype));
...
tcu::TestCaseGroup *create8BitStorageComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "8bit_storage", "Compute tests for VK_KHR_8bit_storage extension"));

    addTestGroup(group.get(), "storagebuffer_32_to_8",
                 "32bit ints to 8bit tests under capability StorageBuffer8BitAccess", addCompute8bitStorage32To8Group);
    addTestGroup(group.get(), "uniform_8_to_32",
                 "8bit ints to 32bit tests under capability UniformAndStorageBuffer8BitAccess",
                 addCompute8bitUniform8To32Group);
    addTestGroup(group.get(), "push_constant_8_to_32", "8bit ints to 32bit tests under capability StoragePushConstant8",
                 addCompute8bitStoragePushConstant8To32Group);

    addTestGroup(group.get(), "storagebuffer_16_to_8",
                 "16bit ints to 8bit tests under capability StorageBuffer8BitAccess", addCompute8bitStorage16To8Group);
    addTestGroup(group.get(), "uniform_8_to_16",
                 "8bit ints to 16bit tests under capability UniformAndStorageBuffer8BitAccess",
                 addCompute8bitUniform8To16Group);
    addTestGroup(group.get(), "push_constant_8_to_16", "8bit ints to 16bit tests under capability StoragePushConstant8",
                 addCompute8bitStoragePushConstant8To16Group);

    addTestGroup(group.get(), "uniform_8_to_8",
                 "8bit ints to 8bit tests under capability UniformAndStorageBuffer8BitAccess",
                 addCompute8bitStorageBuffer8To8Group);

    addTestGroup(group.get(), "uniform_8struct_to_32struct",
    ...
    addTestGroup(group.get(), "storagebuffer_32struct_to_8struct",
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "8bit_storage", "Graphics tests for VK_KHR_8bit_storage extension"));

    addTestGroup(group.get(), "storagebuffer_int_32_to_8",
                 "32-bit int into 8-bit tests under capability StorageBuffer8BitAccess",
                 addGraphics8BitStorageUniformInt32To8Group);
    addTestGroup(group.get(), "uniform_int_8_to_32",
                 "8-bit int into 32-bit tests under capability UniformAndStorageBuffer8BitAccess",
                 addGraphics8BitStorageUniformInt8To32Group);
    addTestGroup(group.get(), "push_constant_int_8_to_32",
                 "8-bit int into 32-bit tests under capability StoragePushConstant8",
                 addGraphics8BitStoragePushConstantInt8To32Group);

    addTestGroup(group.get(), "storagebuffer_int_16_to_8",
                 "16-bit int into 8-bit tests under capability StorageBuffer8BitAccess",
                 addGraphics8BitStorageUniformInt16To8Group);
    addTestGroup(group.get(), "uniform_int_8_to_16",
                 "8-bit int into 16-bit tests under capability UniformAndStorageBuffer8BitAccess",
                 addGraphics8BitStorageUniformInt8To16Group);
    addTestGroup(group.get(), "push_constant_int_8_to_16",
                 "8-bit int into 16-bit tests under capability StoragePushConstant8",
                 addGraphics8BitStoragePushConstantInt8To16Group);

    addTestGroup(group.get(), "8struct_to_32struct", "8bit floats struct to 32bit tests ",
                 addGraphics8BitStorageUniformStruct8To32Group);
    addTestGroup(group.get(), "32struct_to_8struct", "32bit floats struct to 8bit tests ",
                 addGraphics8BitStorageUniformStruct32To8Group);
    addTestGroup(group.get(), "struct_mixed_types", "mixed type of 8bit and 32bit struc",
