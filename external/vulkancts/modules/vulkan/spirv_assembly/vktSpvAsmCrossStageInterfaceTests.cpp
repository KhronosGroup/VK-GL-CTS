...
{
    TestParameters(TestType q, size_t s) : testOptions(s), qualifier(q)
    {
    }
...
VkImageCreateInfo makeImageCreateInfo(const VkImageType imageType, const VkExtent3D &extent, const VkFormat format,
                                      const VkImageUsageFlags usage, uint32_t queueFamilyIndex)
{
...
private:
    ...
    void makeShaderModule(map<VkShaderStageFlagBits, ShaderModuleSP> &shaderModule,
                          const VkShaderStageFlagBits stageFlag, const int optionNdx);
    ...
    bool checkImage(VkImage image, VkCommandPool cmdPool, VkCommandBuffer cmdBuffer, const string &description,
...
tcu::TestStatus CrossStageTestInstance::iterate(void)
{
    ...
    const VkDeviceSize vertexDataSize        = static_cast<VkDeviceSize>(
        deAlignSize(static_cast<size_t>(m_data.size() * sizeof(Vec4)),
    ...
    MovePtr<Allocation> allocationVertex = m_context.getDefaultAllocator().allocate(
        getBufferMemoryRequirements(vk, vkDevice, *vertexBuffer), MemoryRequirement::HostVisible);

    const VkImageSubresourceRange imageSubresourceRange =
        makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, 1u);
    const VkImageCreateInfo colorAttachmentInfo = makeImageCreateInfo(
        VK_IMAGE_TYPE_2D, m_extent, m_colorFormat,
    ...
    MovePtr<Allocation> allocationAttachment = m_context.getDefaultAllocator().allocate(
        getImageMemoryRequirements(vk, vkDevice, *colorAttachmentImage), MemoryRequirement::Any);
    VK_CHECK(vk.bindImageMemory(vkDevice, *colorAttachmentImage, allocationAttachment->getMemory(),
                                allocationAttachment->getOffset()));
    Move<VkImageView> colorAttachmentView =
        makeImageView(vk, vkDevice, *colorAttachmentImage, VK_IMAGE_VIEW_TYPE_2D, m_colorFormat, imageSubresourceRange);

    MovePtr<tcu::Texture2DArray> referenceImage1 = MovePtr<tcu::Texture2DArray>(
        new tcu::Texture2DArray(mapVkFormat(m_colorFormat), m_extent.width, m_extent.height, m_extent.depth));
    MovePtr<tcu::Texture2DArray> referenceImage2 = MovePtr<tcu::Texture2DArray>(
        new tcu::Texture2DArray(mapVkFormat(m_colorFormat), m_extent.width, m_extent.height, m_extent.depth));

    // Init host buffer data
    VK_CHECK(
        vk.bindBufferMemory(vkDevice, *vertexBuffer, allocationVertex->getMemory(), allocationVertex->getOffset()));
    ...
    Move<VkFramebuffer> frameBuffer =
        makeFramebuffer(vk, vkDevice, *renderPass, *colorAttachmentView, m_extent.width, m_extent.height);
    ...
    if (!supportsTessellation)
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Tessellation not supported" << tcu::TestLog::EndMessage;

    if (!supportsGeometry)
        m_context.getTestContext().getLog()
            << tcu::TestLog::Message << "Geometry not supported" << tcu::TestLog::EndMessage;

    vector<uint32_t> shadersStagesFlagsBits;
    shadersStagesFlagsBits.push_back(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);

    if (supportsTessellation)
        shadersStagesFlagsBits.push_back(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT |
                                         VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);

    if (supportsGeometry)
        shadersStagesFlagsBits.push_back(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_GEOMETRY_BIT |
    ...
    for (uint32_t optionNdx = 0; optionNdx < m_parameters.testOptions.size(); optionNdx++)
        for (size_t stagesNdx = 0ull; stagesNdx < shadersStagesFlagsBits.size(); stagesNdx++)
        {
            ...
            Move<VkPipeline> graphicsPipeline = makeGraphicsPipeline(
                *renderPass, *pipelineLayout, (VkShaderStageFlagBits)shadersStagesFlagsBits[stagesNdx], shaderModule,
                ((VkShaderStageFlagBits)shadersStagesFlagsBits[stagesNdx] & VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT) ?
            ...
            imageBarrier(vk, *cmdBuffer, *colorAttachmentImage, imageSubresourceRange, VK_IMAGE_LAYOUT_UNDEFINED,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 0u, VK_ACCESS_TRANSFER_WRITE_BIT,
                         VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT);

            vk.cmdClearColorImage(*cmdBuffer, *colorAttachmentImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                                  &renderPassClearValue.color, 1, &imageSubresourceRange);
            ...
            beginRenderPass(vk, *cmdBuffer, *renderPass, *frameBuffer,
                            makeRect2D(0, 0, m_extent.width, m_extent.height), tcu::Vec4(0.0f));
...

            {
                const string geometry =
                    (VK_SHADER_STAGE_GEOMETRY_BIT & (VkShaderStageFlagBits)shadersStagesFlagsBits[stagesNdx]) ?
                        "Geometry->" :
                        "";
                const string tessellation = (VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT &
                                             (VkShaderStageFlagBits)shadersStagesFlagsBits[stagesNdx]) ?
                                                "Tessellation->" :
                                                "";
            ...
            {
                resultComparison = checkImage(*colorAttachmentImage, *cmdPool, *cmdBuffer,
                                              imageDescription + " Expected Pass", *referenceImage1);
            }
            else
            {
                if (DECORATION_IN_VERTEX == m_parameters.testOptions[optionNdx])
                    resultComparison = checkImage(*colorAttachmentImage, *cmdPool, *cmdBuffer,
                                                  imageDescription + " Expected Pass", *referenceImage1);
                else if ((VkShaderStageFlagBits)(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT) ==
                         (VkShaderStageFlagBits)shadersStagesFlagsBits[stagesNdx])
                    resultComparison = checkImage(*colorAttachmentImage, *cmdPool, *cmdBuffer,
                                                  imageDescription + " Expected Pass", *referenceImage2);
                else
                    resultComparison = !checkImage(*colorAttachmentImage, *cmdPool, *cmdBuffer,
                                                   imageDescription + " Expected Fail", *referenceImage1);
            }
                                              ...
                                              const VkShaderStageFlagBits stageFlag, const int optionNdx)
{
    ...
    if (stageFlag & VK_SHADER_STAGE_VERTEX_BIT)
        shaderModule[VK_SHADER_STAGE_VERTEX_BIT] = (ShaderModuleSP(new Unique<VkShaderModule>(
            createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get(vertex.str()), 0))));

    if (stageFlag & VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT)
        shaderModule[VK_SHADER_STAGE_TESSELLATION_CONTROL_BIT] = (ShaderModuleSP(new Unique<VkShaderModule>(
            createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("tessellation_control"), 0))));

    if (stageFlag & VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT)
        shaderModule[VK_SHADER_STAGE_TESSELLATION_EVALUATION_BIT] = (ShaderModuleSP(new Unique<VkShaderModule>(
            createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("tessellation_evaluation"), 0))));
    ...
    if (stageFlag & VK_SHADER_STAGE_GEOMETRY_BIT)
        shaderModule[VK_SHADER_STAGE_GEOMETRY_BIT] = (ShaderModuleSP(new Unique<VkShaderModule>(
            createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("geometry"), 0))));
    ...
    if (stageFlag & VK_SHADER_STAGE_FRAGMENT_BIT)
        shaderModule[VK_SHADER_STAGE_FRAGMENT_BIT] = (ShaderModuleSP(new Unique<VkShaderModule>(
            createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get(fragment.str()), 0))));
}
                                                              ...
                                                              const VkPrimitiveTopology primitiveTopology)
{
    ...
    const VkVertexInputBindingDescription vertexInputBindingDescription = {
        0u,                                       // binding;
        static_cast<uint32_t>(2u * sizeof(Vec4)), // stride;
...
bool CrossStageTestInstance::checkImage(VkImage image, VkCommandPool cmdPool, VkCommandBuffer cmdBuffer,
                                        const string &description, const tcu::Texture2DArray &referenceFrame)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice vkDevice   = m_context.getDevice();
    ...
    vector<uint8_t> pixelAccessData(m_extent.width * m_extent.height * m_extent.depth * pixelSize);
    tcu::PixelBufferAccess dst(referenceFrame.getFormat(), m_extent.width, m_extent.height, m_extent.depth,
                               pixelAccessData.data());
    ...
    // Create destination buffer
    {
        ...
        bufferAlloc = m_context.getDefaultAllocator().allocate(getBufferMemoryRequirements(vk, vkDevice, *buffer),
    ...
    beginCommandBuffer(vk, cmdBuffer);
    copyImageToBuffer(vk, cmdBuffer, image, *buffer, tcu::IVec2(m_extent.width, m_extent.height), 0u,
                      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, m_extent.depth);
    endCommandBuffer(vk, cmdBuffer);
    submitCommandsAndWait(vk, vkDevice, m_context.getUniversalQueue(), cmdBuffer);
    m_context.resetCommandPoolForVKSC(vkDevice, cmdPool);
...
{
    for (uint32_t x = 0u; x < m_extent.width; ++x)
    {
        float u = static_cast<float>(x) / static_cast<float>(m_extent.width - 1);
        const Vec4 resultColor(
            m_colorRed.x() * (1.0f - u) + m_colorGreen.x() * u, m_colorRed.y() * (1.0f - u) + m_colorGreen.y() * u,
            m_colorRed.z() * (1.0f - u) + m_colorGreen.z() * u, m_colorRed.w() * (1.0f - u) + m_colorGreen.w() * u);

        referenceFrame.getLevel(0).setPixel(resultColor, x, 0);
    }

    for (uint32_t y = 0u; y < m_extent.height; ++y)
    {
        deMemcpy(referenceFrame.getLevel(0).getPixelPtr(0, y), referenceFrame.getLevel(0).getPixelPtr(0, 0),
                 m_extent.width * m_extent.depth * referenceFrame.getFormat().getPixelSize());
    }
}
...
struct Decorations
{
    ...
    Decorations(const string &f, const string &v, const string &o) : fragment(f), vertex(v), others(o)
    {
    }
    string fragment;
    string vertex;
...
{
public:
    CrossStageBasicTestsCase(tcu::TestContext &context, const char *name, const char *description,
...
void CrossStageBasicTestsCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    for (uint32_t ndx = 0; ndx < decorations.size(); ++ndx)
    {
        ...
        */
        const string vertexShaderSource = "; SPIR-V\n"
                                          "; Version: 1.3\n"
                                          ...
                                          "OpEntryPoint Vertex %4 \"main\" %13 %17 %color_out %color_in %r_float_out "
                                          "%rg_float_out %rgb_float_out %rgba_float_out\n"
        ...
        */
        ...
        const string fragmentShaderSource = "; SPIR-V\n"
                                            "; Version: 1.3\n"
                                            "; Generator: Khronos Glslang Reference Front End; 2\n"
                                            "; Bound: 64\n"
                                            "; Schema: 0\n"
                                            ...
                                            "%1 = OpExtInstImport \"GLSL.std.450\"\n"
                                            "OpMemoryModel Logical GLSL450\n"
                                            "OpEntryPoint Fragment %4 \"main\" %color_out %color_in %r_float_in "
                                            "%rg_float_in %rgb_float_in %rgba_float_in\n"
    ...
    }
    {
        ...
        const string tessellationControlSource =
            ...
            "%1 = OpExtInstImport \"GLSL.std.450\"\n"
            "OpMemoryModel Logical GLSL450\n"
            "OpEntryPoint TessellationControl %4 \"main\" %8 %20 %29 %color_out %color_in %r_float_out %r_float_in "
            "%rg_float_out %rg_float_in %rgb_float_out %rgb_float_in %rgba_float_out %rgba_float_in %101 %106\n"
        ...
        const string tessellationEvaluationSource =
            ...
            "%1 = OpExtInstImport \"GLSL.std.450\"\n"
            "OpMemoryModel Logical GLSL450\n"
            "OpEntryPoint TessellationEvaluation %4 \"main\" %11 %color_out %color_in %r_float_out %r_float_in "
            "%rg_float_out %rg_float_in %rgb_float_out %rgb_float_in %rgba_float_out %rgba_float_in %216 %225\n"
    ...
    }
    {
        ...
        */
        const string geometrySource =
            ...
            "%1 = OpExtInstImport \"GLSL.std.450\"\n"
            "OpMemoryModel Logical GLSL450\n"
            "OpEntryPoint Geometry %4 \"main\" %color_out %color_in %r_float_out %r_float_in %rg_float_out "
            "%rg_float_in %rgb_float_out %rgb_float_in %rgba_float_out %rgba_float_in %54 %58\n"
...
{
public:
    CrossStageInterfaceTestsCase(tcu::TestContext &context, const char *name, const char *description,
                                 const TestParameters &parameters)
        : TestCase(context, name, description)
        , m_parameters(parameters)
    {
...
void CrossStageInterfaceTestsCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    }
    {
        ...
        const string geometrySource = "; SPIR-V\n"
                                      "; Version: 1.3\n"
                                      "; Generator: Khronos Glslang Reference Front End; 2\n"
                                      "; Bound: 73\n"
                                      "; Schema: 0\n"
