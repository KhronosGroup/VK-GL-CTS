...
void addComputePointerParamToParamTest(tcu::TestCaseGroup *group)
{
    ...
    for (uint32_t numIdx = 0; numIdx < numFloats; ++numIdx)
...
void addComputePointerBufferMemoryTest(tcu::TestCaseGroup *group)
{
    ...
    expectedOutput.reserve(numFloats);
    for (uint32_t numIdx = 0; numIdx < numFloats / 2; ++numIdx)
...
void addComputePointerWorkgroupMemoryVariablePointersTest(tcu::TestCaseGroup *group)
{
    ...
    expectedOutput.reserve(numFloats);
    for (uint32_t vecIdx = 0; vecIdx < numFloats / 8; ++vecIdx)
    {
        const uint32_t shuffleIdx = (vecIdx + 1) % 16;
...
void addGraphicsPointerParamToParamTest(tcu::TestCaseGroup *group)
{
    ...
    // return param;
    //    }
    fragments["pre_main"] = "            %func0_decl = OpTypeFunction %f32 %fp_f32 %fp_f32\n"
    ...
    resources.outputs.push_back(
        Resource(BufferSp(new Float32Buffer(expectedOutput)), vk::VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));

    createTestsForAllStages("global_to_param", defaultColors, defaultColors, fragments, resources, extensions, group,
                            requiredFeatures);
}
...
void addGraphicsPointerParamToGlobalTest(tcu::TestCaseGroup *group)
{
    ...
    // return param;
    //    }
    fragments["pre_main"] = "                %pp_f32 = OpTypePointer Private %f32\n"
    ...
    createTestsForAllStages("param_to_global", defaultColors, defaultColors, fragments, resources, extensions, group,
                            requiredFeatures);
}
...
void addGraphicsPointerBufferMemoryTest(tcu::TestCaseGroup *group)
{
    ...
    // return param;
    //    }
    fragments["pre_main"] = "           %arr_v4f32_2 = OpTypeArray %v4f32 %c_u32_2\n"
    ...
    createTestsForAllStages("buffer_memory", defaultColors, defaultColors, fragments, resources, extensions, group,
                            requiredFeatures);
}
...
void addGraphicsPointerBufferMemoryVariablePointersTest(tcu::TestCaseGroup *group)
{
    ...
    createTestsForAllStages("buffer_memory_variable_pointers", defaultColors, defaultColors, fragments, resources,
                            extensions, group, requiredFeatures);
}
...
tcu::TestCaseGroup *createPointerParameterComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "pointer_parameter", "Compute tests for pointer as function parameter."));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "pointer_parameter", "Graphics tests for pointer as function parameter."));
