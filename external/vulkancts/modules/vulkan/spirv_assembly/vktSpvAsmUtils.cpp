...
bool is8BitStorageFeaturesSupported(const Context &context, Extension8BitStorageFeatures toCheck)
{
    ...
    if ((toCheck & EXT8BITSTORAGEFEATURES_UNIFORM_STORAGE_BUFFER) != 0 &&
        extensionFeatures.uniformAndStorageBuffer8BitAccess == VK_FALSE)
        return false;
...
bool is16BitStorageFeaturesSupported(const Context &context, Extension16BitStorageFeatures toCheck)
{
    ...
    if ((toCheck & EXT16BITSTORAGEFEATURES_UNIFORM_BUFFER_BLOCK) != 0 &&
        extensionFeatures.storageBuffer16BitAccess == VK_FALSE)
        return false;

    if ((toCheck & EXT16BITSTORAGEFEATURES_UNIFORM) != 0 &&
...
bool isVariablePointersFeaturesSupported(const Context &context, ExtensionVariablePointersFeatures toCheck)
{
    ...
    if ((toCheck & EXTVARIABLEPOINTERSFEATURES_VARIABLE_POINTERS_STORAGEBUFFER) != 0 &&
        extensionFeatures.variablePointersStorageBuffer == VK_FALSE)
        return false;

    if ((toCheck & EXTVARIABLEPOINTERSFEATURES_VARIABLE_POINTERS) != 0 &&
        extensionFeatures.variablePointers == VK_FALSE)
        return false;
...
bool isVulkanMemoryModelFeaturesSupported(const Context &context, ExtensionVulkanMemoryModelFeatures toCheck)
{
    ...
    if ((toCheck & EXTVULKANMEMORYMODELFEATURES_DEVICESCOPE) != 0 &&
        extensionFeatures.vulkanMemoryModelDeviceScope == VK_FALSE)
        return false;

    if ((toCheck & EXTVULKANMEMORYMODELFEATURES_AVAILABILITYVISIBILITYCHAINS) != 0 &&
        extensionFeatures.vulkanMemoryModelAvailabilityVisibilityChains == VK_FALSE)
        return false;
