...
void addComputeSameLabelsTest(tcu::TestCaseGroup *group)
{
    ...
    for (int conditionIdx = 0; conditionIdx < DE_LENGTH_OF_ARRAY(conditions); ++conditionIdx)
    {
        ...
        group->addChild(new SpvAsmComputeShaderCase(testCtx, testName.c_str(),
                                                    "Tests both labels pointing to a same branch.", spec));
    }
}
...
tcu::TestCaseGroup *createConditionalBranchComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "conditional_branch", "Compute tests for OpBranchConditional."));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "conditional_branch", "Graphics tests for OpBranchConditional."));
