...
vk::Move<vk::VkShaderModule> createShaderModule(const vk::DeviceInterface &deviceInterface, vk::VkDevice device,
                                                const vk::ProgramBinary &binary)
{
    ...
    const struct vk::VkShaderModuleCreateInfo shaderModuleInfo = {
        vk::VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO,        DE_NULL, 0, static_cast<uintptr_t>(binary.getSize()),
        reinterpret_cast<const uint32_t *>(binary.getBinary()),
    };
...
tcu::TestStatus T64bitCompareTestInstance<T>::iterate(void)
{
    ...
    if (m_params.stage == vk::VK_SHADER_STAGE_COMPUTE_BIT)
    {
        ...
        vkdi.cmdPipelineBarrier(*cmdBuffer, vk::VK_PIPELINE_STAGE_HOST_BIT, vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0,
                                0u, DE_NULL, static_cast<uint32_t>(hostToDevBarriers.size()), hostToDevBarriers.data(),
                                0u, DE_NULL);
        ...
        vkdi.cmdPipelineBarrier(*cmdBuffer, vk::VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, vk::VK_PIPELINE_STAGE_HOST_BIT, 0,
                                0u, DE_NULL, static_cast<uint32_t>(devToHostBarriers.size()), devToHostBarriers.data(),
                                0u, DE_NULL);
    ...
    else if (m_params.stage == vk::VK_SHADER_STAGE_VERTEX_BIT || m_params.stage == vk::VK_SHADER_STAGE_FRAGMENT_BIT)
    {
        ...
        vk::beginCommandBuffer(vkdi, *cmdBuffer);
        vkdi.cmdPipelineBarrier(*cmdBuffer, vk::VK_PIPELINE_STAGE_HOST_BIT, pipelineStage, 0, 0u, DE_NULL,
                                static_cast<uint32_t>(hostToDevBarriers.size()), hostToDevBarriers.data(), 0u, DE_NULL);
        ...
        vkdi.cmdPipelineBarrier(*cmdBuffer, pipelineStage, vk::VK_PIPELINE_STAGE_HOST_BIT, 0, 0u, DE_NULL,
                                static_cast<uint32_t>(devToHostBarriers.size()), devToHostBarriers.data(), 0u, DE_NULL);
        vk::endCommandBuffer(vkdi, *cmdBuffer);
        vk::submitCommandsAndWait(vkdi, device, m_context.getUniversalQueue(), *cmdBuffer);
    }
...
{
public:
    T64bitCompareTest(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
T64bitCompareTest<T>::T64bitCompareTest(tcu::TestContext &testCtx, const std::string &name,
                                        const std::string &description, const TestParameters<T> &params)
...
template <>
std::string TestMgr::getGroupDesc<double>()
{
    return "64-bit floating point tests";
}
template <>
std::string TestMgr::getGroupDesc<int64_t>()
{
    return "64-bit signed integer tests";
}
template <>
std::string TestMgr::getGroupDesc<uint64_t>()
{
    return "64-bit unsigned integer tests";
}
...
tcu::TestCaseGroup *create64bitCompareGraphicsGroup(tcu::TestContext &testCtx)
{
    ...
    tcu::TestCaseGroup *newGroup =
        new tcu::TestCaseGroup(testCtx, TestMgr::getParentGroupName(), TestMgr::getParentGroupDesc());
    newGroup->addChild(createTestGroup(testCtx, TestMgr::getGroupName<double>(), TestMgr::getGroupDesc<double>(),
                                       TestMgr::getCreationFunction<double>(), &graphicStages));
    newGroup->addChild(createTestGroup(testCtx, TestMgr::getGroupName<int64_t>(), TestMgr::getGroupDesc<int64_t>(),
                                       TestMgr::getCreationFunction<int64_t>(), &graphicStages));
    newGroup->addChild(createTestGroup(testCtx, TestMgr::getGroupName<uint64_t>(), TestMgr::getGroupDesc<uint64_t>(),
...
tcu::TestCaseGroup *create64bitCompareComputeGroup(tcu::TestContext &testCtx)
{
    ...
    tcu::TestCaseGroup *newGroup =
        new tcu::TestCaseGroup(testCtx, TestMgr::getParentGroupName(), TestMgr::getParentGroupDesc());
    newGroup->addChild(createTestGroup(testCtx, TestMgr::getGroupName<double>(), TestMgr::getGroupDesc<double>(),
                                       TestMgr::getCreationFunction<double>(), &computeStages));
    newGroup->addChild(createTestGroup(testCtx, TestMgr::getGroupName<int64_t>(), TestMgr::getGroupDesc<int64_t>(),
