                       ...
                       const map<string, string> &fragments, tcu::TestCaseGroup &group, const std::string &namePrefix)
{
    ...
    group.addChild(
        new SpvAsmComputeShaderCase(group.getTestContext(), testName.c_str(), testName.c_str(), computeResources));
}
...
public:
    ...
    SpvAsmTypeTests(tcu::TestContext &testCtx, const char *name, const char *description, const char *deviceFeature,
                    const char *spirvCapability, const char *spirvType, InputType inputType, uint32_t typeSize,
                    uint32_t vectorSize);
...
template <class T>
SpvAsmTypeTests<T>::SpvAsmTypeTests(tcu::TestContext &testCtx, const char *name, const char *description,
                                    const char *deviceFeature, const char *spirvCapability, const char *spirvType,
                                    InputType inputType, uint32_t typeSize, uint32_t vectorSize)
...
SpvAsmTypeUint16Tests::SpvAsmTypeUint16Tests(tcu::TestContext &testCtx, uint32_t vectorSize)
    : SpvAsmTypeTests(testCtx, "u16", "uint16 tests", "shaderInt16", "Int16", "OpTypeInt 16 0", TYPE_U16, 16,
                      vectorSize)
{
    m_cases[0] = 0;
    m_cases[1] = 3210;
...
SpvAsmTypeUint64Tests::SpvAsmTypeUint64Tests(tcu::TestContext &testCtx, uint32_t vectorSize)
    : SpvAsmTypeTests(testCtx, "u64", "uint64 tests", "shaderInt64", "Int64", "OpTypeInt 64 0", TYPE_U64, 64,
                      vectorSize)
{
    m_cases[0] = 3210;
    m_cases[1] = 268438669;
...
tcu::TestCaseGroup *createTypeTests(tcu::TestContext &testCtx)
{
    ...
    for (uint32_t ndx = 0; ndx < 3; ++ndx)
    {
        ...
        typeVectorTests[ndx] =
            de::MovePtr<tcu::TestCaseGroup>(new tcu::TestCaseGroup(testCtx, testName.c_str(), "vector tests"));
    }
