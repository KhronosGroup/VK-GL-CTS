...
typedef uint32_t Extension8BitStorageFeatures;
...
typedef uint32_t Extension16BitStorageFeatures;
...
typedef uint32_t ExtensionVariablePointersFeatures;
...
typedef uint32_t ExtensionFloat16Int8Features;
...
typedef uint32_t ExtensionVulkanMemoryModelFeatures;
...
std::vector<double> getFloat64s(de::Random &rnd, uint32_t count);

// Generate and return 32-bit floats
//
// The first 24 number pairs are manually picked, while the rest are randomly generated.
// Expected count to be at least 24 (numPicks).
std::vector<float> getFloat32s(de::Random &rnd, uint32_t count);
...
std::vector<deFloat16> getFloat16s(de::Random &rnd, uint32_t count);
