...
void addComputeVectorCompositeInsertTests(tcu::TestCaseGroup *group)
{
    ...
    for (bool useUndef : {true, false})
        for (uint32_t elements = 2; elements <= 4; elements++)
        {
            ...
            group->addChild(
                new SpvAsmComputeShaderCase(testCtx, testName.c_str(), "Tests vector composite insert.", spec));
        }
}
    ...
    for (bool useUndef : {true, false})
        for (uint32_t rows = 2; rows <= 4; rows++)
        {
            ...
            for (uint32_t cols = 2; cols <= 4; cols++)
            {
                ...
                group->addChild(
                    new SpvAsmComputeShaderCase(testCtx, testName.c_str(), "Tests matrix composite insert.", spec));
            }
        }
}
...
void addComputeNestedStructCompositeInsertTests(tcu::TestCaseGroup *group)
{
    ...
    for (bool useUndef : {true, false})
    {
        ...
        group->addChild(
            new SpvAsmComputeShaderCase(testCtx, testName.c_str(), "Tests nested struct composite insert.", spec));
    }
}
...
tcu::TestCaseGroup *createCompositeInsertComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "composite_insert", "Compute tests for composite insert."));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "composite_insert", "Graphics tests for composite insert."));
