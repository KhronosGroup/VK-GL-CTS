...
void addComputeUboMatrixPaddingTest(tcu::TestCaseGroup *group)
{
    ...
    group->addChild(new SpvAsmComputeShaderCase(
        testCtx, "mat2x2", "Tests mat2x2 member in UBO struct without padding (treated as vec4).", spec));
}
...
tcu::TestCaseGroup *createUboMatrixPaddingComputeGroup(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "ubo_padding", "Compute tests for UBO struct member packing."));
...
{
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "ubo_padding", "Graphics tests for UBO struct member packing."));
