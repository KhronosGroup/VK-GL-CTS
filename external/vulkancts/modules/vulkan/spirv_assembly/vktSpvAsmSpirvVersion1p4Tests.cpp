...
{
    de::MovePtr<tcu::TestCaseGroup> spirv1p4Tests(
        new tcu::TestCaseGroup(testCtx, "spirv1p4", "SPIR-V 1.4 new features"));
    ...
    group.add("variable_pointers_vars_wg_diff", "pointer diff in workgroup storage, stored in private var",
              Varptr_full);
    ...
    group.add("simple_variable_pointers_ptr_not_equal", "ptr not equal between simple data primitives in SSBOs",
              Varptr_ssbo);
    group.add("variable_pointers_vars_ssbo_not_equal", "ptr not equal SSBO, store pointer in Private var", Varptr_ssbo);
    group.add("variable_pointers_vars_wg_not_equal", "ptr not equal Workgroup, store pointer in Private var",
              Varptr_full);
    ...
    spirv1p4Tests->addChild(createTestGroup(testCtx, "opcopymemory", "OpCopyMemory 2 memory access operands",
    ...
    group.add("subgroup_cfg_uniform_id",
              "subgroup uniform compare result inside control flow"); // Assumes subgroup size <= LocalSize of 8
    ...
    spirv1p4Tests->addChild(
        createTestGroup(testCtx, "uniformid", "UniformId decoration", addTestsForAmberFiles, group));
    ...
    spirv1p4Tests->addChild(
        createTestGroup(testCtx, "nonwritable", "NonWritable decoration", addTestsForAmberFiles, group));
    ...
    spirv1p4Tests->addChild(createTestGroup(testCtx, "entrypoint", "EntryPoint lists all module-scope variables",
                                            addTestsForAmberFiles, group));
    ...
    spirv1p4Tests->addChild(createTestGroup(testCtx, "hlsl_functionality1",
    ...
    spirv1p4Tests->addChild(
        createTestGroup(testCtx, "loop_control", "SPIR-V 1.4 loop controls", addTestsForAmberFiles, group));
    ...
    group.add("wg_pointers_2_select", "OpSelect Workgroup pointers to different buffers, verify SPIR-V 1.0",
              Varptr_full);
    group.add("wg_pointers_select", "OpSelect Workgroup pointers to same buffer, verify SPIR-V 1.0", Varptr_full);
    spirv1p4Tests->addChild(
        createTestGroup(testCtx, "opselect", "SPIR-V 1.4 OpSelect more cases", addTestsForAmberFiles, group));
    ...
    spirv1p4Tests->addChild(
        createTestGroup(testCtx, "uconvert", "SPIR-V 1.4 UConvert in OpSpecConstantOp", addTestsForAmberFiles, group));
    ...
    spirv1p4Tests->addChild(
        createTestGroup(testCtx, "wrap", "SPIR-V 1.4 integer wrap decorations", addTestsForAmberFiles, group));
