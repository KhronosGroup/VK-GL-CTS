...
void copyBufferToImage(const DeviceInterface &vk, const VkDevice &device, const VkQueue &queue,
                       VkCommandBuffer cmdBuffer, VkBuffer buffer, VkImage image, VkImageAspectFlags aspect)
{
...
TestStatus runAndVerifyDefaultPipeline(Context &context, InstanceContext instance)
{
    ...
    const uint32_t numResources =
        static_cast<uint32_t>(instance.resources.inputs.size() + instance.resources.outputs.size());
    ...
    // Core features
    {
        ...
        if ((localRequiredCoreFeatures.fragmentStoresAndAtomics != false) &&
            (instance.customizedStages & vk::VK_SHADER_STAGE_FRAGMENT_BIT) == 0)
        {
            localRequiredCoreFeatures.fragmentStoresAndAtomics = false;
        }

        // reset vertex pipeline stores and atomics feature requirement
        if (localRequiredCoreFeatures.vertexPipelineStoresAndAtomics != false &&
            (instance.customizedStages & vertexPipelineStoresAndAtomicsAffected) == 0)
        {
            localRequiredCoreFeatures.vertexPipelineStoresAndAtomics = false;
        }

        if (!isCoreFeaturesSupported(context, localRequiredCoreFeatures, &unsupportedFeature))
            TCU_THROW(NotSupportedError,
                      std::string("At least following requested core feature is not supported: ") + unsupportedFeature);
    }
...
if (needInterface)
{
    ...
    vkInstance.getPhysicalDeviceFormatProperties(vkPhysicalDevice, instance.interfaces.getOutputType().getVkFormat(),
                                                 &formatProperties);
...
const UniquePtr<Allocation> imageMemory(allocator.allocate(getImageMemoryRequirements(vk, device, *image),
...
if (needInterface)
{
    ...
    vertexInputMemory =
        allocator.allocate(getBufferMemoryRequirements(vk, device, *vertexInputBuffer), MemoryRequirement::HostVisible);
    ...
    fragOutputMemory =
        allocator.allocate(getBufferMemoryRequirements(vk, device, *fragOutputBuffer), MemoryRequirement::HostVisible);
    VK_CHECK(
        vk.bindBufferMemory(device, *fragOutputBuffer, fragOutputMemory->getMemory(), fragOutputMemory->getOffset()));
...
if (needInterface)
{
    const VkVertexInputBindingDescription vertexBinding1 = {
        1u,                                               // uint32_t binding;
        instance.interfaces.getInputType().getNumBytes(), // uint32_t strideInBytes;
        VK_VERTEX_INPUT_RATE_VERTEX                       // VkVertexInputStepRate stepRate;
    };
...
const VkPipelineTessellationStateCreateInfo tessellationState = {
    VK_STRUCTURE_TYPE_PIPELINE_TESSELLATION_STATE_CREATE_INFO, DE_NULL, (VkPipelineTessellationStateCreateFlags)0, 3u};
...

{
    ...
    if (needInterface)
    {
        ...
        vk.cmdPipelineBarrier(*cmdBuf, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT,
                              (VkDependencyFlags)0, 1, &vertFlushBarrier, 0, (const VkBufferMemoryBarrier *)DE_NULL, 2,
...

{
    ...
    clearValue.push_back(
        makeClearValueColorF32(defaulClearColor[0], defaulClearColor[1], defaulClearColor[2], defaulClearColor[3]));
    if (needInterface)
    {
...
vk.cmdDraw(*cmdBuf, uint32_t(vertexCount), 1u /*run pipeline once*/, 0u /*first vertex*/, 0u /*first instanceIndex*/);
...

{
    ...
    if (needInterface)
    {
        vk.cmdCopyImageToBuffer(*cmdBuf, *fragOutputImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *fragOutputBuffer, 1u,
                                &copyParams);
    }
}

{
    ...
    if (needInterface)
    {
        ...
        vk.cmdPipelineBarrier(*cmdBuf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, (VkDependencyFlags)0,
                              0, (const VkMemoryBarrier *)DE_NULL, 2, cpFinishBarriers.data(), 0,
                              (const VkImageMemoryBarrier *)DE_NULL);
    }
    else
    {
        vk.cmdPipelineBarrier(*cmdBuf, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, (VkDependencyFlags)0,
                              0, (const VkMemoryBarrier *)DE_NULL, 1, cpFinishBarriers.data(), 0,
                              (const VkImageMemoryBarrier *)DE_NULL);
    }
}
...
for (uint32_t outputNdx = 0; outputNdx < numOutResources; ++outputNdx)
{
    ...
    {
        if (!(*instance.resources.verifyIO)(instance.resources.inputs, outResourceMemories, instance.resources.outputs,
                                            context.getTestContext().getLog()))
