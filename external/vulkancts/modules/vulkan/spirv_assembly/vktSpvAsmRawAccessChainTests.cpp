...
Move<VkDescriptorSetLayout> createDescriptorSetLayout (const DeviceInterface& vkdi, const VkDevice& device, int numDescriptors)
{
...
Move<VkDescriptorSet> createDescriptorSet (const DeviceInterface& vkdi, const VkDevice& device, VkDescriptorPool pool, VkDescriptorSetLayout layout)
{
...
Move<VkPipelineLayout> createPipelineLayout (const DeviceInterface& vkdi, const VkDevice& device, VkDescriptorSetLayout descriptorSetLayout, bool usesPhysicalBuffers)
{
										  ...
										  bool						physStorageBuffer)
{
	...
	MemoryRequirement physicalBufferRequirement	= (physStorageBuffer ? MemoryRequirement::DeviceAddress : MemoryRequirement::Any);
...
tcu::TestStatus SpvAsmRawAccessChainInstance::iterate (void)
{
	...
	Unique<VkBuffer>					inputBuffer			= Unique<VkBuffer>(createBufferAndBindMemory(vkdi, device, allocator, m_spec.inputData.size(), &inputAlloc, m_spec.usesPhysicalBuffers));
	Unique<VkBuffer>					outputBuffer		= Unique<VkBuffer>(createBufferAndBindMemory(vkdi, device, allocator, m_spec.outputData.size(), &outputAlloc, m_spec.usesPhysicalBuffers));
	...
	Unique<VkPipelineLayout>			pipelineLayout		(createPipelineLayout(vkdi, device, *descriptorSetLayout, m_spec.usesPhysicalBuffers));
	...
	const DescriptorSetUpdateBuilder::Location location		= DescriptorSetUpdateBuilder::Location::bindingArrayElement(0, descriptorIndex);

	descriptorSetBuilder.writeSingle(*descriptorSet1, location, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &inputDescriptorInfo);
	descriptorSetBuilder.writeSingle(*descriptorSet2, location, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &outputDescriptorInfo);
	...
	{
		vkdi.cmdPushConstants(*cmdBuffer, *pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(descriptorAddresses), &descriptorAddresses);
	}
	else
	{
		vkdi.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 0, 1, &descriptorSet1.get(), 0, DE_NULL);
		vkdi.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *pipelineLayout, 1, 1, &descriptorSet2.get(), 0, DE_NULL);
		vkdi.cmdPushConstants(*cmdBuffer, *pipelineLayout, VK_SHADER_STAGE_COMPUTE_BIT, 0, sizeof(descriptorIndex), &descriptorIndex);
	}
	...
	vkdi.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0, 1, &memory_barrier, 0, DE_NULL, 0, DE_NULL);
	...
	if (deMemCmp(outputAlloc->getHostPtr(), m_spec.expectedOutput.data(), m_spec.expectedOutput.size()) != 0)
	{
		...
		const deUint8*	ptrHost		= static_cast<const deUint8*>(outputAlloc->getHostPtr());
		const deUint8*	ptrExpected	= static_cast<const deUint8*>(m_spec.expectedOutput.data());
		size_t			errors		= 0u;
		size_t			ndx			= 0u;
...
SpvAsmRawAccessChainTestCase::SpvAsmRawAccessChainTestCase (tcu::TestContext& testCtx, const char* name, const Spec& spec)
...
template <typename T>
vector<deUint8> castVector(const vector<T>& input)
{
	vector<deUint8> result(input.size() * sizeof(T));
...
void addTest (tcu::TestCaseGroup* group, const Parameters& p)
{
	...
	vector<deUint8> inputData(inputComponentStride * localSize, 0xcc);
	vector<deUint8> outputData(outputComponentStride * localSize, 0xcc);
	vector<deUint8> expectedOutput = outputData;

	// Fill input with garbage first
	for (size_t i = 0; i < inputData.size(); i++)
		inputData[i] = random.getUint8();

	deUint8* inputIterator = inputData.data();
	deUint8* expectedOutputIterator = expectedOutput.data();
	for (VkDeviceSize i = 0; i < static_cast<VkDeviceSize>(localSize); i++)
	{
		const deUint64 values[4] = { random.getUint64(), random.getUint64(), random.getUint64(), random.getUint64() };
		deUint64 expectedResult = 0;
		for (int j = 0; j < p.inputComponents; j++)
		{
			deUint64 value = 0;
			memcpy(&value, reinterpret_cast<const deUint8*>(&values[0]) + j * p.inputSize, p.inputSize);
	...
	{
		gen.AddHeader("OpEntryPoint GLCompute %main \"main\" %gl_LocalInvocationID %pushConstants %inputBuffers %outputBuffers\n");
	}
	else
	{
		gen.AddHeader("OpEntryPoint GLCompute %main \"main\" %gl_LocalInvocationID %pushConstants %inputBuffer %outputBuffer\n");
	}
	...
	if (p.outputComponents != 1)
	{
		...
		if (p.outputSize != 4)
			gen.AddDeclaration('%' + outputType + " = OpTypeVector %" + scalarOutputType + ' ' + to_string(p.outputComponents) + '\n');
	}
	...
	gen.AddBody(
		...
		"%mainLabel = OpLabel\n"
		"%localInvocationPtr = OpAccessChain %_ptr_Input_uint %gl_LocalInvocationID " + gen.Uint32(0) + "\n"
	...
	else
	{
		...
		offset = "%loadOffset";
		gen.AddBody(
			"%elementOffset = OpIMul %uint %localInvocation " + inputStrideText + "\n"
			"%loadOffset = OpIAdd %uint %elementOffset " + gen.Uint32(p.inputPrePadding) + '\n'
		);
	}
	...
	{
		gen.AddBody(
			"%inputBufferPointer = OpAccessChain %_ptr_PushConstant_v2uint %pushConstants " + gen.Uint32(0) + "\n"
			"%inputBufferAddress = OpLoad %v2uint %inputBufferPointer\n"
			"%inputBuffer = OpBitcast %_ptr_Storage_type %inputBufferAddress\n"
			"%outputBufferPointer = OpAccessChain %_ptr_PushConstant_v2uint %pushConstants " + gen.Uint32(1) + "\n"
	...
	{
		gen.AddBody(
			"%descriptorIndexPointer = OpAccessChain %_ptr_PushConstant_uint %pushConstants " + gen.Uint32(0) + "\n"
	...
	gen.AddBody(
		"%pointer = OpRawAccessChainNV %_ptr_Storage_" + type + " %inputBuffer " + stride + ' ' + elementIndex + ' ' + offset + inputRobustness + '\n' +
		result + " = OpLoad %" + type + " %pointer" + inputAlignmentText + '\n'
	);
	...
	switch (p.inputComponents)
	{
		...
		case 3:
			gen.AddBody(
				"%value12 = OpIAdd %type %value1 %value2\n"
	...
	{
		gen.AddBody(
			"%storePointer = OpRawAccessChainNV %_ptr_Storage_" + outputType + " %outputBuffer " +
	...
	{
		gen.AddBody(
			"%storeElementOffset = OpIMul %uint %localInvocation " + outputStrideText + "\n"
			"%storeOffset = OpIAdd %uint %storeElementOffset " + gen.Uint32(p.outputPrePadding) + "\n"
			"%storePointer = OpRawAccessChainNV %_ptr_Storage_" + outputType + " %outputBuffer " +
					gen.Uint32(0) + ' ' + gen.Uint32(0) + " %storeOffset" + outputRobustness + '\n'
		);
	}
	...
	else
	{
		...
		gen.AddBody(
			"%storeVector = OpCompositeConstruct %" + outputType + ' ' + composites + "\n"
			"OpStore %storePointer %storeVector" + outputAlignmentText + '\n'
		);
	}
	gen.AddBody(
		"OpReturn\n"
...
void addTests (tcu::TestCaseGroup* group)
{
	...
	for (const BoundsCheck boundsCheck : {NO_BOUNDS_CHECK, BOUNDS_CHECK_PER_COMPONENT, BOUNDS_CHECK_PER_ELEMENT})
	...
	for (const int alignmentDiv : {1, 4, 2, 3})
	{
		...
		if (!stride && boundsCheck == BOUNDS_CHECK_PER_ELEMENT) continue;

		// Skip alignments that don't match the number of components.
		if (components < alignmentDiv) continue;
		if (components % alignmentDiv != 0) continue;

		// Skip physical pointer related tests.
		if (physicalBuffers)
		{
			if (variablePointers) continue;
			if (descriptorIndexing) continue;
			if (boundsCheck != NO_BOUNDS_CHECK) continue;
		}

		// Skip complex qualifiers mixed with other complex configurations.
		if (qualifiers != QUALIFIER_NONE && qualifiers != QUALIFIER_LOAD_NON_WRITABLE)
		{
			if (size != 4) continue;
			if (components != 4) continue;
			if (alignmentDiv != 1) continue;
		}
		...
		while (!deIsPowerOfTwo32(inputComponents * inputSize + inputPrePadding + inputPostPadding))
			inputPostPadding += inputSize;

		int outputPostPadding = 0;
		while (!deIsPowerOfTwo32(outputComponents * outputSize + outputPrePadding + outputPostPadding))
		...
		VkDeviceSize inputDescriptorRange = testingStore ? VK_WHOLE_SIZE : descriptorRange;
		VkDeviceSize outputDescriptorRange = testingStore ? descriptorRange : VK_WHOLE_SIZE;

		const Parameters parameters =
		{
			name.c_str(),
			inputSize, inputComponents, inputPrePadding, inputPostPadding, inputAlignment,
			outputSize, outputComponents, outputPrePadding, outputPostPadding, outputAlignment,
			strideLoad,
			strideStore,
			variablePointers,
			descriptorIndexing,
