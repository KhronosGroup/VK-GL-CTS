...
struct Case
{
    ...
    Case(const char *b, const char *d, bool v, const std::vector<std::string> &e)
        : basename(b)
        , description(d)
        , spv1p3(v)
        , requirements(e)
...
void addTestsForAmberFiles(tcu::TestCaseGroup *tests, CaseGroup group)
{
    ...
    {
        uint32_t vulkan_version        = cases[i].spv1p3 ? VK_MAKE_VERSION(1, 1, 0) : VK_MAKE_VERSION(1, 0, 0);
        ...
        cts_amber::AmberTestCase *testCase =
            cts_amber::createAmberTestCase(testCtx, cases[i].basename, cases[i].description, category.c_str(), file);
...
{
    de::MovePtr<tcu::TestCaseGroup> terminateTests(
        new tcu::TestCaseGroup(testCtx, "terminate_invocation", "VK_KHR_shader_terminate_invocation tests"));
    ...
    group.add("no_output_write_before_terminate", "no write to output despite occurring before terminate invocation",
              false);
    ...
    group.add("ssbo_store_before_terminate", "ssbo store commits when it occurs before terminate invocation", false,
              Stores);
    ...
    group.add("no_null_pointer_load", "null pointer should not be accessed by a load in a terminated invocation", false,
              VarPtr);
    group.add("no_null_pointer_store", "null pointer should not be accessed by a store in a terminated invocation",
              false, VarPtr);
    group.add("no_out_of_bounds_load",
              "out of bounds pointer should not be accessed by a load in a terminated invocation", false, VarPtr);
    group.add("no_out_of_bounds_store",
              "out of bounds pointer should not be accessed by a store in a terminated invocation", false, VarPtr);
    group.add("no_out_of_bounds_atomic",
              "out of bounds pointer should not be accessed by an atomic in a terminated invocation", false, VarPtr);
    ...
    terminateTests->addChild(
        createTestGroup(testCtx, "terminate", "Terminate Invocation", addTestsForAmberFiles, group));
