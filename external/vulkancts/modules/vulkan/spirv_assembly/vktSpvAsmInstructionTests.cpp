...
tcu::TestCaseGroup *createFloat16LogicalSet(tcu::TestContext &testCtx, const bool nanSupported)
{
    ...
    { // scalar cases
        ...
        for (uint32_t testOpsIdx = 0; testOpsIdx < DE_LENGTH_OF_ARRAY(testOps); ++testOpsIdx)
        {
            ...
            specResource.outputs.push_back(
                Resource(BufferSp(new Float16Buffer(float16OutDummy)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
    ...
    { // vector cases
        ...
        for (uint32_t testOpsIdx = 0; testOpsIdx < DE_LENGTH_OF_ARRAY(testOps); ++testOpsIdx)
        {
            ...
            specResource.outputs.push_back(
                Resource(BufferSp(new Float16Buffer(float16OutVecDummy)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
            specResource.verifyIO = nanSupported ? testOp.verifyFuncNan : testOp.verifyFuncNonNan;
...
tcu::TestCaseGroup *createFloat16FuncSet(tcu::TestContext &testCtx)
{
    ...
    for (uint32_t testTypeIdx = 0; testTypeIdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypeIdx)
    {
        ...
        specResource.outputs.push_back(
            Resource(BufferSp(new Float16Buffer(float16OutputDummy)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
...
tcu::TestCaseGroup *createFloat16CompositeInsertExtractSet(tcu::TestContext &testCtx, const char *op)
{
    ...
    for (size_t typeTestNdx = 0; typeTestNdx < DE_LENGTH_OF_ARRAY(typeTestParameters); ++typeTestNdx)
    {
        ...
        specResource.outputs.push_back(
            Resource(BufferSp(new Float16Buffer(dummyFP16Output)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
        specResource.verifyIO = compareFP16CompositeFunc;
                                     ...
                                     const Math16TestFunc &testFunc)
{
    ...
    specResource.outputs.push_back(
        Resource(BufferSp(new Float16Buffer(float16DummyOutput)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
