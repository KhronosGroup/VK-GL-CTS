...
void addComputeIndexingStructTests(tcu::TestCaseGroup *group)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> structGroup(
        new tcu::TestCaseGroup(testCtx, "struct", "Tests for indexing input struct."));
    ...
    for (uint32_t numIdx = 0; numIdx < numInputFloats; ++numIdx)
        inputData.push_back(rnd.getFloat());

    indexSelectorData.reserve(numItems);
    for (uint32_t numIdx = 0; numIdx < numItems; ++numIdx)
        indexSelectorData.push_back(tcu::randomVector<uint32_t, 4>(rnd, UVec4(0), UVec4(31, 31, 3, 3)));
        ...
        for (int idxSizeIdx = 0; idxSizeIdx < DE_LENGTH_OF_ARRAY(idxSizes); ++idxSizeIdx)
        {
            for (int sign = 0; sign < 2; ++sign)
            {
                ...
                case CHAIN_OP_ACCESS_CHAIN:
                    specs["accesschain"] =
                        "OpAccessChain %_ptr_Uniform_float %inputFirstElement %idx_0 %i0 %i1 %i2 %i3\n";
                    specs["inputdecoration"]   = "BufferBlock";
                    specs["inputstorageclass"] = "Uniform";
                    break;
                case CHAIN_OP_IN_BOUNDS_ACCESS_CHAIN:
                    specs["accesschain"] =
                        "OpInBoundsAccessChain %_ptr_Uniform_float %inputFirstElement %idx_0 %i0 %i1 %i2 %i3\n";
                    specs["inputdecoration"]   = "BufferBlock";
                    specs["inputstorageclass"] = "Uniform";
                    break;
                default:
                    ...
                    specs["accesschain"] =
                        "OpPtrAccessChain %_ptr_StorageBuffer_float %inputFirstElement %idx_1 %idx_0 %i0 %i1 %i2 %i3\n";
                ...
                for (uint32_t numIdx = 0; numIdx < numItems; ++numIdx)
                {
                    ...
                    outputData.push_back(inputData[element * sizeof(InputData) / 4 + vec.x() * (32 * 4 * 4) +
                                                   vec.y() * 4 * 4 + vec.z() * 4 + vec.w()]);
                }
    ...
    for (int chainOpIdx = 0; chainOpIdx < CHAIN_OP_LAST; ++chainOpIdx)
    {
        for (int idxSizeIdx = 0; idxSizeIdx < DE_LENGTH_OF_ARRAY(idxSizes); ++idxSizeIdx)
        {
            for (int sign = 0; sign < 2; sign++)
            {
                ...
                const StringTemplate decoration("OpDecorate %_arr_float_uint_128 ArrayStride 4\n"
                                                "OpMemberDecorate %Output 0 Offset 0\n"
                                                "OpDecorate %Output BufferBlock\n"
                                                "OpDecorate %dataOutput DescriptorSet 0\n"
                                                "OpDecorate %dataOutput Binding 2\n"
                ...
                case CHAIN_OP_ACCESS_CHAIN:
                    specs["accesschain"] =
                        "OpAccessChain %_ptr_Uniform_float %inputFirstElement %int_0 %i0 %i1 %i2 %i3\n";
                    specs["inputdecoration"]   = "BufferBlock";
                    specs["inputstorageclass"] = "Uniform";
                    break;
                case CHAIN_OP_IN_BOUNDS_ACCESS_CHAIN:
                    specs["accesschain"] =
                        "OpInBoundsAccessChain %_ptr_Uniform_float %inputFirstElement %int_0 %i0 %i1 %i2 %i3\n";
                    specs["inputdecoration"]   = "BufferBlock";
                    specs["inputstorageclass"] = "Uniform";
                    break;
                default:
                    DE_ASSERT(chainOpIdx == CHAIN_OP_PTR_ACCESS_CHAIN);
                    specs["accesschain"]     = "OpPtrAccessChain %_ptr_StorageBuffer_float %inputFirstElement %uint_1 "
                                               "%int_0 %i0 %i1 %i2 %i3\n";
                    specs["inputdecoration"] = "Block";
                    specs["inputstorageclass"] = "StorageBuffer";
                ...
                resources.outputs.push_back(
                    Resource(BufferSp(new Float32Buffer(outputData)), VK_DESCRIPTOR_TYPE_STORAGE_BUFFER));
...
void addGraphicsOutputComponentIndexingTests(tcu::TestCaseGroup *testGroup)
{
    ...
    const uint32_t numItems       = 4;
    vector<int32_t> inputData;
    vector<float> outputData;
    const int32_t pattern[] = {2, 0, 1, 3};
    ...
    fragments["pre_main"] = "             %a3u32 = OpTypeArray %u32 %c_i32_3\n"
    ...
    fragments["post_interface_op_tessc"] = "%cpntPtr0 = OpAccessChain %op_f32 %IF_output %c_i32_0 %IF_input_val0\n"
    ...
    interfaces.setInputOutput(
        std::make_pair(IFDataType(1, NUMBERTYPE_UINT32), BufferSp(new Int32Buffer(inputData))),
...
{
    tcu::TestContext &testCtx = group->getTestContext();
    de::MovePtr<tcu::TestCaseGroup> non16BaseAlignmentGroup(new tcu::TestCaseGroup(
        testCtx, "non16basealignment", "Tests for indexing array with base alignment less than 16."));
    de::Random rnd(deStringHash(group->getName()));
    ...
    string shaderStr = "                             OpCapability Shader\n"
                       "                             ${variablepointercaps:opt}\n"
                       "                             ${extensions:opt}\n"
                       "                        %1 = OpExtInstImport \"GLSL.std.450\"\n"
                       "                             OpMemoryModel Logical GLSL450\n"
    ...
    shaderStr += "                             OpDecorate %runtimearr_struct1 ArrayStride " +
                 de::toString(floatArraySize * 4) + "\n";
    shaderStr += "                             OpDecorate %_ptr_struct1_sb ArrayStride " +
                 de::toString(floatArraySize * 4) + "\n";
    shaderStr += "                             OpMemberDecorate %Output 0 Offset 0\n"
                 ...
                 "        %_ptr_Input_uint32 = OpTypePointer Input %u32\n"
                 "                    %float = OpTypeFloat 32\n";
    for (uint32_t floatIdx = 0; floatIdx < floatArraySize + 1; ++floatIdx)
        shaderStr += string("%uint_") + de::toString(floatIdx) + " = OpConstant %u32 " + de::toString(floatIdx) + "\n";
    shaderStr += "                  %uint_" + de::toString(numFloatArrays) + " = OpConstant %u32 " +
                 de::toString(numFloatArrays) + "\n";
    shaderStr += "              %input_array = OpTypeArray %float %uint_" + de::toString(floatArraySize) + "\n";
    shaderStr += "             %output_array = OpTypeArray %float %uint_" + de::toString(numFloatArrays) + "\n";
    shaderStr += "                   %Output = OpTypeStruct %output_array\n"
    ...
    for (uint32_t floatIdx = 0; floatIdx < floatArraySize; ++floatIdx)
    {
        shaderStr += string("%dataPtr") + de::toString(floatIdx) + " = ${chainop} %invid %uint_0 %uint_" +
                     de::toString(floatIdx) + "\n";
        if (floatIdx == 0)
        {
        ...
        {
            shaderStr +=
                string("%tmp") + de::toString(floatIdx) + " = OpLoad %float %dataPtr" + de::toString(floatIdx) + "\n";
            shaderStr += string("%acc") + de::toString(floatIdx) + " = OpFAdd %float %tmp" + de::toString(floatIdx) +
                         " %acc" + de::toString(floatIdx - 1) + "\n";
        }
    }
    shaderStr += "                   %outPtr = OpAccessChain %_ptr_float_sb %dataOutput %uint_0 %invid\n";
    shaderStr += "                             OpStore %outPtr %acc" + de::toString(floatArraySize - 1) + "\n";
    shaderStr += "                             OpReturn\n"
                 "                             OpFunctionEnd\n";
    ...
    for (uint32_t outputIdx = 0; outputIdx < numOutputFloats; ++outputIdx)
    {
        float f = 0.0f;
        for (uint32_t arrIdx = 0; arrIdx < floatArraySize; ++arrIdx)
    ...
    for (int chainOpIdx = 0; chainOpIdx < DE_LENGTH_OF_ARRAY(chainOps); ++chainOpIdx)
    {
        ...
        non16BaseAlignmentGroup->addChild(
            new SpvAsmComputeShaderCase(testCtx, testName.c_str(), testName.c_str(), spec));
    }
...
{
    de::MovePtr<tcu::TestCaseGroup> indexingGroup(
        new tcu::TestCaseGroup(testCtx, "indexing", "Compute tests for data indexing."));
    de::MovePtr<tcu::TestCaseGroup> inputGroup(
        new tcu::TestCaseGroup(testCtx, "input", "Tests for indexing input data."));
...
{
    de::MovePtr<tcu::TestCaseGroup> indexingGroup(
        new tcu::TestCaseGroup(testCtx, "indexing", "Graphics tests for data indexing."));
    de::MovePtr<tcu::TestCaseGroup> inputGroup(
        new tcu::TestCaseGroup(testCtx, "input", "Tests for indexing input data."));
    de::MovePtr<tcu::TestCaseGroup> outputGroup(
        new tcu::TestCaseGroup(testCtx, "output", "Tests for indexing output data."));
