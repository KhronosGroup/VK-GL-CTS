...
bool compareWithFileImage(Context &context, const tcu::ConstPixelBufferAccess &resultImage, std::string testName)
{
    ...
    if (tcu::fuzzyCompare(context.getTestContext().getLog(), "ImageComparison", "Image Comparison",
        ...
        return tcu::intThresholdPositionDeviationCompare(
            context.getTestContext().getLog(), "ImageComparison", "Image Comparison", referenceImage.getAccess(),
            resultImage, tcu::UVec4(1u, 1u, 1u, 1u), tcu::IVec3(2, 2, 2), false, tcu::COMPARE_LOG_RESULT);
    else
        return false;
}
...
void fillBuffer(const DeviceInterface &vk, const VkDevice device, const Allocation &alloc, const VkDeviceSize offset,
                const VkDeviceSize size, const VkFormat format, const float depth)
{
    const tcu::TextureFormat textureFormat = mapVkFormat(format);
    const uint32_t colorPixelSize          = static_cast<uint32_t>(tcu::getPixelSize(textureFormat));
    tcu::TextureLevel colorPixelBuffer(textureFormat, 1, 1);
