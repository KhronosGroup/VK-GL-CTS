...
tcu::TestStatus testLayeredReadBack(Context &context, const TestParams params)
{
    ...
    const Unique<VkImage> dsImage(
        makeImage(vk, device,
                  makeImageCreateInfo(imageCreateFlags, imageType, dsFormat, params.image.size, params.image.numLayers,
                                      dsImageUsage)));
    ...
    const VkImageSubresourceRange dsSubresRange =
        makeImageSubresourceRange(dsAspectFlags, 0u, 1u, 0u, params.image.numLayers);
    std::string result;
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        for (uint32_t layerNdx = 0; layerNdx < numLayers; ++layerNdx)
        {
            ...
            if (dsUsed)
            {
                ...
                const VkDeviceSize bufferOffset = layerNdx * bufferSliceSize;
                const VkImageSubresourceLayers imageSubresource =
                    makeImageSubresourceLayers(VK_IMAGE_ASPECT_DEPTH_BIT, 0u, layer, 1u);
                const VkBufferImageCopy bufferImageCopyRegion =
                    makeBufferImageCopy(bufferOffset, imageSubresource, imageOffset, imageExtent);
            ...
            if (dsUsed)
            {
                ...
                const VkDeviceSize bufferOffset = layerNdx * bufferSliceSize;
                const VkImageSubresourceLayers imageSubresource =
                    makeImageSubresourceLayers(VK_IMAGE_ASPECT_STENCIL_BIT, 0u, layer, 1u);
                const VkBufferImageCopy bufferImageCopyRegion =
                    makeBufferImageCopy(bufferOffset, imageSubresource, imageOffset, imageExtent);
    ...
    // Change images layouts
    {
        ...
        if (dsUsed)
            vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
                                  0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &dsBarrier);
    }
    ...
    beginCommandBuffer(vk, *cmdBuffer);
    {
        ...
        if (dsUsed)
        {
            ...
            const VkBufferImageCopy depthCopyRegion =
                makeBufferImageCopy(params.image.size, makeImageSubresourceLayers(VK_IMAGE_ASPECT_DEPTH_BIT, 0u, 0u,
                                                                                  params.image.numLayers));
            const VkBufferImageCopy stencilCopyRegion =
                makeBufferImageCopy(params.image.size, makeImageSubresourceLayers(VK_IMAGE_ASPECT_STENCIL_BIT, 0u, 0u,
                                                                                  params.image.numLayers));
...
tcu::TestStatus testSecondaryCmdBuffer(Context &context, const TestParams params)
{
    ...
    // Barrier between draws
    {
        ...
        vk.cmdPipelineBarrier(*secondaryCmdBuffer, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                              VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0u, 1u, &barrier, 0u, DE_NULL, 0u, DE_NULL);
    }
    vk.cmdDraw(*secondaryCmdBuffer, 1u, 1u, 0u, 0u);
...
tcu::TestCaseGroup *createLayeredRenderingTests(tcu::TestContext &testCtx)
{
    ...
    const struct
    {
        ...
        {
            TEST_TYPE_MULTIPLE_LAYERS_PER_INVOCATION,
    ...
    for (int imageParamGroupNdx = 0; imageParamGroupNdx < DE_LENGTH_OF_ARRAY(imageParamGroups); ++imageParamGroupNdx)
    {
        ...
        for (int imageParamNdx = 0; imageParamNdx < 2; imageParamNdx++)
        {
            ...
            MovePtr<tcu::TestCaseGroup> viewTypeGroup(
                new tcu::TestCaseGroup(testCtx, viewTypeGroupName.str().c_str(), ""));
            ...
            for (int testTypeNdx = 0; testTypeNdx < DE_LENGTH_OF_ARRAY(testTypes); ++testTypeNdx)
            {
                ...
                if (testTypes[testTypeNdx].test == TEST_TYPE_LAYERED_READBACK)
                    addFunctionCaseWithPrograms(viewTypeGroup.get(), testTypes[testTypeNdx].name,
                                                testTypes[testTypeNdx].description, checkSupport, initPrograms,
                                                testLayeredReadBack, params);
                else if (testTypes[testTypeNdx].test == TEST_TYPE_SECONDARY_CMD_BUFFER)
                {
                    addFunctionCaseWithPrograms(viewTypeGroup.get(), "secondary_cmd_buffer",
                                                testTypes[testTypeNdx].description, checkSupport, initPrograms,
                    ...
                    addFunctionCaseWithPrograms(viewTypeGroup.get(), testTypes[testTypeNdx].name,
                                                testTypes[testTypeNdx].description, checkSupport, initPrograms, test,
                                                params);
            }
