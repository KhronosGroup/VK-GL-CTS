...
void generateInputAttachmentParams(uint32_t activeAttachmentCount, uint32_t allAttachmentCount,
                                   std::vector<uint32_t> &attachmentIndices, std::vector<uint32_t> &descriptorBindings)
{
...
void InputAttachmentSparseFillingTest::initPrograms(SourceCollections &sourceCollections) const
{
    ...
    generateInputAttachmentParams(m_testParams.activeInputAttachmentCount, 2u * m_testParams.activeInputAttachmentCount,
                                  attachmentIndices, descriptorBindings);
...
void InputAttachmentSparseFillingTestInstance::createCommandBuffer(const DeviceInterface &vk, VkDevice vkDevice)
{
    ...
    VkImageSubresourceRange range = makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, 1u);
    {
        ...
        vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u,
                              DE_NULL, 0u, DE_NULL, 1u, &outputImageInitBarrier);
    ...
    for (auto &inputImage : m_inputImages)
    {
        ...
        vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u,
                              DE_NULL, 0u, DE_NULL, 1u, &inputImageInitBarrier);
    ...
    copyImageToBuffer(vk, *m_cmdBuffer, **m_outputImage, **m_outputBuffer,
                                                                              ...
                                                                              VkDevice vkDevice)
{
    const VkImageAspectFlags aspectMask =
        m_testParams.renderingType == RENDERING_TYPE_RENDERPASS_LEGACY ? 0 : VK_IMAGE_ASPECT_COLOR_BIT;
...
{
    de::MovePtr<tcu::TestCaseGroup> unusedAttTests(
        new tcu::TestCaseGroup(testCtx, "attachment_sparse_filling", "Unused attachment tests"));
