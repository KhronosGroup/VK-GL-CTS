...
Move<VkPipeline> createRenderPipeline(const DeviceInterface &vkd, VkDevice device, VkRenderPass renderPass,
                                      VkPipelineLayout pipelineLayout, const BinaryCollection &binaryCollection,
...
struct TestConfig
{
    TestConfig(VkFormat format_, RenderPassType renderPassType_) : format(format_), renderPassType(renderPassType_)
    {
    }
...
SparseRenderTargetTestInstance::SparseRenderTargetTestInstance(Context &context, TestConfig testConfig)
    ...
    , m_extensionSupported((testConfig.renderPassType == RENDERPASS_TYPE_RENDERPASS2) &&
    ...
    , m_renderPass(
          createRenderPass(context.getDeviceInterface(), context.getDevice(), m_format, testConfig.renderPassType))
    ...
    , m_renderPipeline(createRenderPipeline(context.getDeviceInterface(), context.getDevice(), *m_renderPass,
                                            *m_renderPipelineLayout, context.getBinaryCollection(), m_width, m_height))
...

    {
        ...
        case tcu::TEXTURECHANNELCLASS_SIGNED_FIXED_POINT:
        {
...

            {
                ...
                if (!tcu::floatThresholdCompare(m_context.getTestContext().getLog(), "", "", reference.getAccess(),
...
void initTests(tcu::TestCaseGroup *group, const RenderPassType renderPassType)
{
    ...
    for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
    {
        ...
        group->addChild(new InstanceFactory1<SparseRenderTargetTestInstance, TestConfig, Programs>(
            testCtx, tcu::NODETYPE_SELF_VALIDATE, testName.c_str(), testName.c_str(), testConfig));
    }
}
