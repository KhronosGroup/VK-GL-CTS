...
struct BeginRenderPassData
{
    ...
    uint32_t depthImageLayer; // specifies which depth image layer should be used for attachment
};
...
using TestStepVariant = std::variant<BeginRenderPassData, BindPipelineData, DrawData, ClearBlitData, BarrierData>;
struct TestStep
{
    TestStepType type;
...
{
TestStep bindPipeline(VkCompareOp depthCompareOp, FS frag = FS::BASIC, DepthTest depthTest = DepthTest::ENABLE,
                      ...
                      VkStencilOp stencilPassOp = VK_STENCIL_OP_ZERO, VkStencilOp stencilFailOp = VK_STENCIL_OP_ZERO,
                      VkStencilOp depthFailOp = VK_STENCIL_OP_ZERO, uint32_t stencilCompareMask = 0xFF,
                      uint32_t stencilWriteMask = 0xFF, uint32_t stencilReference = 0, Blend blend = Blend::DISABLE,
...
TestStep bindBlendPipeline(VkCompareOp depthCompareOp, DepthWrite depthWrite, Blend blend,
                           VkColorComponentFlags colorWriteMask = 0xF)
{
    return {TestStepType::BIND_PIPELINE,
            BindPipelineData{FS::BASIC, DepthTest::ENABLE, depthWrite, depthCompareOp, StencilTest::DISABLE,
                             ...
                             0xFF, 0, blend, colorWriteMask}};
}

TestStep beginRenderPass(VkAttachmentLoadOp depthLoadOp = VK_ATTACHMENT_LOAD_OP_CLEAR, float depthClearValue = 1.0f,
                         VkAttachmentStoreOp depthStoreOp = VK_ATTACHMENT_STORE_OP_STORE,
                         uint32_t stencilClearValue = 0, uint32_t depthImageLayer = 0)
{
    return {TestStepType::BEGIN_RENDER_PASS,
            BeginRenderPassData{depthLoadOp, depthStoreOp, depthClearValue, stencilClearValue, depthImageLayer}};
}

TestStep endRenderPass(void)
{
    return {TestStepType::END_RENDER_PASS};
}

TestStep suspendResumeRendering(void)
{
    // note: this step applies only to dynamic rendering tests
    return {TestStepType::SUSPEND_RESUME_RENDERING};
}

TestStep draw(tcu::Vec4 color, float depth, DrawMode mode = DrawMode::FULL_QUAD)
{
    return {TestStepType::DRAW, DrawData{color, depth, 1.0f, mode}};
}

TestStep clearDepthAttachment(float depth, uint32_t depthImageLayer = 0)
{
    return {TestStepType::CLEAR_ATTACHMENT, ClearBlitData{depth, depthImageLayer}};
}

TestStep clearDepthImage(float depth, uint32_t depthImageLayer = 0)
{
    return {TestStepType::CLEAR_IMAGE, ClearBlitData{depth, depthImageLayer}};
}

TestStep blitDepth(float depth, uint32_t depthImageLayer = 0)
{
    return {TestStepType::BLIT, ClearBlitData{depth, depthImageLayer}};
}

TestStep beginSecondary(void)
{
    return {TestStepType::BEGIN_SECONDARY};
}

TestStep endSecondary(void)
{
...
// number of depth image layers used in tests
...
struct SharedData
{
    ...
    std::shared_ptr<ImageWithMemory> depthStencilBuffer;
    std::shared_ptr<BufferWithMemory> computeBuffer;
    Move<VkImageView> depthStencilViews[DEPTH_LAYER_COUNT];
};
...
protected:
    ...
    void simulateDraw(FramebufferState &fb, uint32_t depthImageLayer, const BindPipelineData &pipeline,
                      const DrawData &dd) const;
    ...
    void preparePipelines(const PipelineLayoutWrapper &graphicsPipelineLayout, VkRenderPass renderPass);
    ...
    void beginRendering(const DeviceInterface &vk, const VkCommandBuffer cmdBuffer,
                        ...
                        uint32_t stencilClear, uint32_t depthIndex) const;
    void endRendering(const DeviceInterface &vk, const VkCommandBuffer cmdBuffer) const;

    VkImageCreateInfo makeImageCreateInfo(VkFormat format, uint32_t size, VkImageUsageFlags usage,
...
private:
    ...
    VkImageSubresourceRange m_dsIsr =
    ...
    , m_groupParams(*groupParams)
{
    ...
    vki.getPhysicalDeviceFormatProperties(physicalDevice, m_depthStencilFormat, &formatProperties);
    ...
    // check if singleton with shared data is already created
    if (!g_sharedData)
    {
        g_sharedData = std::make_unique<SharedData>();

        // define image for depth/stencil attachment
        // (most test use single DS attachment but there is a test that uses three DS attachments)
        auto dsCreateInfo = makeImageCreateInfo(m_depthStencilFormat, m_imageSize, imageUsage, DEPTH_LAYER_COUNT);
        g_sharedData->depthStencilBuffer =
            std::make_shared<ImageWithMemory>(vk, device, allocator, dsCreateInfo, MemoryRequirement::Any);

        // define separate image view for each layer of depth/stencil image
        for (uint32_t i = 0; i < DEPTH_LAYER_COUNT; i++)
        {
            m_dsIsr.baseArrayLayer             = i;
            g_sharedData->depthStencilViews[i] = makeImageView(vk, device, g_sharedData->depthStencilBuffer->get(),
                                                               VK_IMAGE_VIEW_TYPE_2D, m_depthStencilFormat, m_dsIsr);
        }
        m_dsIsr.baseArrayLayer = 0;

        // define image for color attachment
        const VkExtent3D imageExtent{m_imageSize, m_imageSize, 1};
        imageUsage = VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_STORAGE_BIT;
        g_sharedData->colorBuffer = std::make_shared<ImageWithBuffer>(vk, device, allocator, imageExtent, m_colorFormat,
                                                                      imageUsage, VK_IMAGE_TYPE_2D);

        // define buffer for compute shader - one buffer for input and output
        const auto bufferUsage =
            VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
        auto computeBufferCreateInfo = makeBufferCreateInfo(m_computeBufferSize, bufferUsage);
        g_sharedData->computeBuffer = std::make_shared<BufferWithMemory>(vk, device, allocator, computeBufferCreateInfo,
    ...
    if (m_context.getShaderObjectFeaturesEXT().shaderObject && m_groupParams.useSecondaryCmdBuffer)
    {
        if (m_groupParams.secondaryCmdBufferCompletelyContainsDynamicRenderpass)
            m_groupParams.pipelineConstructionType = PIPELINE_CONSTRUCTION_TYPE_SHADER_OBJECT_UNLINKED_SPIRV;
        else
            m_groupParams.pipelineConstructionType = PIPELINE_CONSTRUCTION_TYPE_SHADER_OBJECT_LINKED_BINARY;
    }
...
tcu::TestStatus LRZTestInstance::iterate(void)
{
    ...
    ImageWithMemory &depthStencilBuffer = *g_sharedData->depthStencilBuffer;
    BufferWithMemory &computeBuffer     = *g_sharedData->computeBuffer;
    ...
    // create graphics pipeline layout with push constant for draw data; all pipelines will use the same layout
    const auto nh                            = VK_NULL_HANDLE;
    const auto vfShaderState                 = VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_VERTEX_BIT;
    VkPushConstantRange graphicsPushConstant = {vfShaderState, 0u, sizeof(DrawData)};
    const PipelineLayoutWrapper graphicsPipelineLayout(m_groupParams.pipelineConstructionType, vk, device, nh,
                                                       &graphicsPushConstant);
    preparePipelines(graphicsPipelineLayout, !useDynamicRendering ? *renderPassVect[0] : nh);
    ...
    VkImageCreateInfo imageCreateInfo      = makeImageCreateInfo(m_depthStencilFormat, 1, blitImageUsage);

    std::vector<std::shared_ptr<ImageWithMemory>> blitImages;
    for (size_t i = 0; i < m_blitDepths.size(); ++i)
        blitImages.push_back(
    ...
    Move<VkFramebuffer> framebuffer[DEPTH_LAYER_COUNT];
    VkImageView iamgeViews[]{colorBuffer.getImageView(), nh};
    for (uint32_t i = 0; i <= m_highestUsedDepthImageLayer && !useDynamicRendering; ++i)
    {
        iamgeViews[1]  = *g_sharedData->depthStencilViews[i];
        framebuffer[i] = makeFramebuffer(vk, device, *renderPassVect[0], 2, iamgeViews, m_imageSize, m_imageSize);
    }
    ...
    const auto computeBufferAlloc = computeBuffer.getAllocation();
    float *computeBufferData      = static_cast<float *>(computeBufferAlloc.getHostPtr());

    // store expected results for even and odd rows in compute buffer
    const VkDeviceSize vec4Size = sizeof(tcu::Vec4);
    deMemcpy(computeBufferData, expectedEven.getPtr(), vec4Size);
    deMemcpy(computeBufferData + 4, expectedOdd.getPtr(), vec4Size);
    ...
    static_cast<uint32_t *>(computeBufferAlloc.getHostPtr())[8] = 0u;
    flushAlloc(vk, device, computeBufferAlloc);

    // create descriptor pool for compute pipeline
    const auto descriptorPool = DescriptorPoolBuilder()
                                    .addType(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE)
                                    .addType(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER)
                                    .build(vk, device, 1u, 1u);
    ...
    const auto computeSetLayout = DescriptorSetLayoutBuilder()
                                      ...
                                      .build(vk, device);
    const auto descriptorSet = makeDescriptorSet(vk, device, *descriptorPool, *computeSetLayout);

    // update descriptor set for compute shader
    const auto diDescInfo = makeDescriptorImageInfo(VK_NULL_HANDLE, iamgeViews[0], VK_IMAGE_LAYOUT_GENERAL);
    const auto dbDescInfo = makeDescriptorBufferInfo(*computeBuffer, 0, m_computeBufferSize);
    ...
    std::vector<VkImageMemoryBarrier> layoutTransitionBarriers{initialImageBarrier};

    // transition depth image
    initialImageBarrier.subresourceRange            = m_dsIsr;
    initialImageBarrier.subresourceRange.layerCount = DEPTH_LAYER_COUNT;
    initialImageBarrier.image                       = *depthStencilBuffer;
    ...
    {
        initialImageBarrier.image = **blitImages[i];
        layoutTransitionBarriers.push_back(initialImageBarrier);
    }
    ...
    if (m_groupParams.useSecondaryCmdBuffer)
    {
        ...
        inheritanceRenderingInfo.pColorAttachmentFormats = &m_colorFormat;
        inheritanceRenderingInfo.depthAttachmentFormat   = m_depthStencilFormat;
        inheritanceRenderingInfo.stencilAttachmentFormat = m_depthStencilFormat;
        ...
        for (uint32_t i = 0; i < m_numberOfRequiredSecondaryCommandBuffers; ++i)
    ...
    m_dsIsr.baseArrayLayer = 0u;
    for (size_t i = 0; i < blitImages.size(); ++i)
    {
        const auto clearDepth = makeClearDepthStencilValue(m_blitDepths[i], 0);
        vk.cmdClearDepthStencilImage(cmdBuffer, **blitImages[i], VK_IMAGE_LAYOUT_GENERAL, &clearDepth, 1, &m_dsIsr);
    }
    ...
    vk.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, 0u,
                          !blitImages.empty(), &postBlitMemoryBarrier, 0u, nullptr, 0u, nullptr);

    uint32_t renderPassIndex = 0;
    uint32_t pipelineIndex   = 0;
    uint32_t blitImageIndex  = 0;
    uint32_t secondaryIndex  = 0;
    const auto renderArea    = makeRect2D(tcu::UVec2(m_imageSize, m_imageSize));
    ...
    for (const auto &testStep : m_processedStepVect)
    {
...
#ifndef CTS_USES_VULKANSC
            if (m_suspendResumeCountPerRendering[renderPassIndex] > 0)
                renderingFlag = VK_RENDERING_SUSPENDING_BIT;
#endif // CTS_USES_VULKANSC

            currentRenderpassData = std::get_if<BeginRenderPassData>(&testStep.data);
            beginRendering(vk, cmdBuffer, renderPassVect, *framebuffer[currentRenderpassData->depthImageLayer],
                           ...
                           currentRenderpassData->depthImageLayer);
        }
        ...
        else if (currStep == TestStepType::SUSPEND_RESUME_RENDERING)
        {
            ...
            if (m_suspendResumeCountPerRendering[renderPassIndex] > 1)
                renderingFlag |= VK_RENDERING_SUSPENDING_BIT;
            --m_suspendResumeCountPerRendering[renderPassIndex];
        ...
        else if (currStep == TestStepType::DRAW)
        {
            ...
            vk.cmdPushConstants(cmdBuffer, *graphicsPipelineLayout, vfShaderState, 0u, sizeof(DrawData), &dd);
        ...
        {
            auto cd = std::get<ClearBlitData>(testStep.data);
        ...
        else if (currStep == TestStepType::CLEAR_IMAGE)
        {
            ...
            auto cd                = std::get<ClearBlitData>(testStep.data);
            const auto clearValue  = makeClearValueDepthStencil(cd.depth, 0);
            m_dsIsr.baseArrayLayer = cd.depthImageLayer;
            vk.cmdClearDepthStencilImage(cmdBuffer, *depthStencilBuffer, VK_IMAGE_LAYOUT_GENERAL,
                                         &clearValue.depthStencil, 1, &m_dsIsr);
        }
        else if (currStep == TestStepType::BLIT)
        {
            DE_ASSERT(cmdBuffer == *primCmdBuffer);
            auto cd        = std::get<ClearBlitData>(testStep.data);
            const auto isl = makeImageSubresourceLayers(m_dsIsr.aspectMask, 0u, cd.depthImageLayer, 1u);
            const VkImageBlit blitRegion{
                isl, {{}, {1, 1, 1}}, isl, {{}, {(int32_t)m_imageSize, (int32_t)m_imageSize, 1}}};
            vk.cmdBlitImage(cmdBuffer, **blitImages[blitImageIndex++], VK_IMAGE_LAYOUT_GENERAL, *depthStencilBuffer,
    ...
    vk.cmdBindDescriptorSets(cmdBuffer, bp, *computePipelineLayout, 0u, 1u, &*descriptorSet, 0u, nullptr);
    ...
    // verify result
    const Allocation &bufferAllocation = computeBuffer.getAllocation();
    ...
    tcu::PixelBufferAccess resultAccess(mapVkFormat(m_colorFormat), m_imageSize, m_imageSize, 1,
...
void LRZTestInstance::preprocessSteps()
{
    ...
    for (const auto &testStep : m_testParams.stepVect)
    {
        ...
        if (currStep == TestStepType::BLIT)
        {
            ...
            m_blitDepths.push_back(std::get<ClearBlitData>(testStep.data).depth);
            continue;
        }
        if (currStep == TestStepType::BEGIN_RENDER_PASS)
        {
            ...
            // memorize highest depth image layer used in render pass steps to know how many framebuffers we need to create
        ...
        if (currStep == TestStepType::END_RENDER_PASS)
        {
            ...
            m_suspendResumeCountPerRendering.push_back(customSuspendResumeCount);
    ...
    if (renderingFullyInSecondaries || renderingPartiallyInSecondaries)
        m_numberOfRequiredSecondaryCommandBuffers = (uint32_t)m_suspendResumeCountPerRendering.size();
...
{
    FramebufferState fb{{0, 0, 0, 0}, {0, 0, 0}, 0};
    bool isRowOdd                         = (y % 2);
    ...
    for (const auto &testStep : m_testParams.stepVect)
    {
        ...
        if (tType == TestStepType::BEGIN_RENDER_PASS)
        {
            ...
            fb.depth[depthImageLayer] = brpd.depthClearValue;
        ...
        else if (tType == TestStepType::DRAW)
        {
            ...
            simulateDraw(fb, depthImageLayer, *boundPipeline, dd);
        }
        else if ((tType == TestStepType::CLEAR_ATTACHMENT) || (tType == TestStepType::CLEAR_IMAGE) ||
                 (tType == TestStepType::BLIT))
        {
            const auto &cbd               = std::get<ClearBlitData>(testStep.data);
            fb.depth[cbd.depthImageLayer] = cbd.depth;
        }
...
void LRZTestInstance::simulateDraw(FramebufferState &fb, uint32_t depthImageLayer, const BindPipelineData &pipeline,
                                   const DrawData &dd) const
{
    ...
    float &fbDepth  = fb.depth[depthImageLayer];
    if (pipeline.frag == FS::DEPTH_GREATER_OVERRIDE || pipeline.frag == FS::DEPTH_LESS_OVERRIDE ||
        pipeline.frag == FS::WRITE_DEPTH_OVERRIDE)
...
void LRZTestInstance::preparePipelines(const PipelineLayoutWrapper &graphicsPipelineLayout, VkRenderPass renderPass)
{
    const auto &vki     = m_context.getInstanceInterface();
    const auto &vk      = m_context.getDeviceInterface();
    auto device         = m_context.getDevice();
    ...
    const std::vector<VkRect2D> scissors{makeRect2D(m_imageSize, m_imageSize)};
    const std::vector<VkViewport> viewports{makeViewport(m_imageSize, m_imageSize)};
    ...
    renderingCreateInfo.pColorAttachmentFormats       = &m_colorFormat;
    renderingCreateInfo.depthAttachmentFormat         = m_depthStencilFormat;
    renderingCreateInfo.stencilAttachmentFormat       = m_depthStencilFormat;

    VkPipelineRenderingCreateInfo *renderingCreateInfoPtr = nullptr;
    if (renderPass == VK_NULL_HANDLE)
        renderingCreateInfoPtr = &renderingCreateInfo;
#else
    void *renderingCreateInfoPtr = nullptr;
#endif // CTS_USES_VULKANSC
    ...
    for (const auto pd : m_pipelineDataVect)
    {
        ...
        (m_pipelineVect.back())
            ...
            .setupPreRasterizationShaderState2(viewports, scissors, graphicsPipelineLayout, renderPass, 0u, vs, nullptr,
                                               {}, {}, {}, nullptr, nullptr, nullptr, nullptr, nullptr,
                                               renderingCreateInfoPtr)
            .setupFragmentShaderState(graphicsPipelineLayout, renderPass, 0u, fs, &depthStencilState)
...
Move<VkRenderPass> LRZTestInstance::makeRenderPass(const VkAttachmentLoadOp loadOperation) const
{
    ...
    const AttachmentDescription attachments[]{
        {nullptr, 0, m_colorFormat, VK_SAMPLE_COUNT_1_BIT, loadOperation, VK_ATTACHMENT_STORE_OP_STORE,
         VK_ATTACHMENT_LOAD_OP_DONT_CARE, VK_ATTACHMENT_STORE_OP_DONT_CARE, lg, lg},
        {nullptr, 0, m_depthStencilFormat, VK_SAMPLE_COUNT_1_BIT, loadOperation, VK_ATTACHMENT_STORE_OP_STORE,
    ...
    AttachmentReference colorRef(nullptr, 0u, lg, VK_IMAGE_ASPECT_COLOR_BIT);
    AttachmentReference dsRef(nullptr, 1, lg, dsAspect);

    SubpassDescription subpass(nullptr, (VkSubpassDescriptionFlags)0, VK_PIPELINE_BIND_POINT_GRAPHICS, 0u, 0u, nullptr,
                               1, &colorRef, nullptr, &dsRef, 0u, nullptr);

    RenderPassCreateInfo rpci(nullptr, (VkRenderPassCreateFlags)0, (uint32_t)std::size(attachments), attachments, 1u,
                              &subpass, 0u, nullptr, 0u, nullptr);
                                     ...
                                     uint32_t depthLayer) const
{
    ...
    if (m_groupParams.renderingType == RENDERING_TYPE_DYNAMIC_RENDERING)
    {
        ...
        dsAttachment.imageView                    = *g_sharedData->depthStencilViews[depthLayer];
...
VkImageCreateInfo LRZTestInstance::makeImageCreateInfo(VkFormat format, uint32_t size, VkImageUsageFlags usage,
...
void BaseTestCase::initPrograms(SourceCollections &programCollection) const
{
    ...
    auto commonFragPreamble = commonShaderPreamble + "layout(location = 0) out vec4 fragColor;\n";
...
static void createChildren(tcu::TestCaseGroup *lrzTests, const SharedGroupParams groupParams)
{
    ...
    const std::vector<TestParams> directionPreserveVect{
        ...
        {"no_depth_test",
         {beginRenderPass(), bindPipeline(VK_COMPARE_OP_NEVER, fsb, DepthTest::DISABLE, DepthWrite::DISABLE), //
          draw({1, 0, 0, 1}, 0.5f),                                                                           //
          draw({0, 1, 0, 1}, 0.3f),                                                                           //
          endRenderPass()}},
    };
        ...
        {"three_images_rotation",
         {
             ...
             draw({0, 1, 0, 1}, 0.5f),            //
             endRenderPass(),
             beginRenderPass(VK_ATTACHMENT_LOAD_OP_CLEAR, 1.0f, VK_ATTACHMENT_STORE_OP_STORE, 0, 2),
             bindPipeline(VK_COMPARE_OP_LESS), //
             draw({1, 1, 0, 1}, 0.7f),         //
             endRenderPass(),
             beginRenderPass(VK_ATTACHMENT_LOAD_OP_LOAD, 0.0f, VK_ATTACHMENT_STORE_OP_STORE, 0, 0),
