...
Move<VkPipelineLayout> createSubpassPipelineLayout(const DeviceInterface &vkd, VkDevice device,
...
Move<VkPipeline> createSubpassPipeline(const DeviceInterface &vkd, VkDevice device, VkRenderPass renderPass,
                                       VkPipelineLayout pipelineLayout, const vk::BinaryCollection &binaryCollection,
                                       uint32_t width, uint32_t height, uint32_t sampleCount)
{
    const Unique<VkShaderModule> vertexShaderModule(
        createShaderModule(vkd, device, binaryCollection.get("quad-vert"), 0u));
    const Unique<VkShaderModule> fragmentShaderModule(
        createShaderModule(vkd, device, binaryCollection.get("quad-subpass-frag"), 0u));
...
Move<VkDescriptorSet> createSubpassDescriptorSet(const DeviceInterface &vkd, VkDevice device, VkDescriptorPool pool,
                                                 VkDescriptorSetLayout layout, VkImageView imageView)
{
    ...
    {
        const VkDescriptorImageInfo imageInfo = {(VkSampler)0u, imageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
...
struct TestConfig
{
    TestConfig(uint32_t sampleCount_, TestMode testMode_, uint32_t selectedSample_, RenderingType renderingType_)
...
SampleReadTestInstance::SampleReadTestInstance(Context &context, TestConfig config)
    ...
    , m_extensionSupported(context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING) &&
                           ((config.renderingType != RENDERING_TYPE_RENDERPASS2) ||
    ...
    , m_renderPass(createRenderPass(context.getDeviceInterface(), context.getDevice(), VK_FORMAT_R32_UINT,
                                    VK_FORMAT_R32_UINT, m_sampleCount, config.renderingType))
    ...
    , m_subpassPipelineLayout(
          createSubpassPipelineLayout(context.getDeviceInterface(), context.getDevice(), *m_subpassDescriptorSetLayout))
    , m_subpassPipeline(createSubpassPipeline(context.getDeviceInterface(), context.getDevice(), *m_renderPass,
                                              *m_subpassPipelineLayout, context.getBinaryCollection(), m_width,
                                              m_height, m_sampleCount))
    , m_subpassDescriptorPool(createSubpassDescriptorPool(context.getDeviceInterface(), context.getDevice()))
    , m_subpassDescriptorSet(createSubpassDescriptorSet(context.getDeviceInterface(), context.getDevice(),
                                                        *m_subpassDescriptorPool, *m_subpassDescriptorSetLayout,
                                                        *m_srcInputImageView))
...
tcu::TestStatus SampleReadTestInstance::iterateInternal(void)
{
    ...
    vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_subpassPipelineLayout, 0u, 1u,
                              &*m_subpassDescriptorSet, 0u, DE_NULL);
    vkd.cmdDraw(*commandBuffer, 6u, 1u, 0u, 0u);

    RenderpassSubpass::cmdEndRenderPass(vkd, *commandBuffer, &subpassEndInfo);

    copyImageToBuffer(vkd, *commandBuffer, *m_dstSinglesampleImage, *m_dstBuffer, tcu::IVec2(m_width, m_height),
...
void initTests(tcu::TestCaseGroup *group, RenderingType renderingType)
{
    ...
    for (uint32_t sampleCountNdx = 0; sampleCountNdx < DE_LENGTH_OF_ARRAY(sampleCounts); sampleCountNdx++)
    {
        const uint32_t sampleCount(sampleCounts[sampleCountNdx]);
        {
            ...
            group->addChild(new InstanceFactory1<SampleReadTestInstance, TestConfig, Programs>(
...
{
    return createTestGroup(testCtx, "sampleread", initTests, RENDERING_TYPE_RENDERPASS_LEGACY);
}

tcu::TestCaseGroup *createRenderPass2SampleReadTests(tcu::TestContext &testCtx)
{
    return createTestGroup(testCtx, "sampleread", initTests, RENDERING_TYPE_RENDERPASS2);
}
