                                    ...
                                    uint32_t sampleCount, RenderPassType renderPassType)
{
    ...
    const VkImageAspectFlagBits aspectFlag((renderPassType == RENDERPASS_TYPE_RENDERPASS2) ?
...
struct TestConfig
{
    TestConfig(uint32_t sampleCount_, TestMode testMode_, uint32_t selectedSample_, RenderPassType renderPassType_)
...
SampleReadTestInstance::SampleReadTestInstance(Context &context, TestConfig config)
    ...
    , m_extensionSupported(context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_SAMPLE_RATE_SHADING) &&
                           ((config.renderPassType != RENDERPASS_TYPE_RENDERPASS2) ||
    ...
    , m_renderPass(createRenderPass(context.getDeviceInterface(), context.getDevice(), VK_FORMAT_R32_UINT,
                                    VK_FORMAT_R32_UINT, m_sampleCount, config.renderPassType))
