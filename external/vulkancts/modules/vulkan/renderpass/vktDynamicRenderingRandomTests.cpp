...
tcu::TestStatus DynamicRenderingTestInstance::iterate(void)
{
    ...
    uint32_t validPipelines = (1 << PIPELINE_TYPE_VERTEX_FRAGMENT) | (1 << PIPELINE_TYPE_VERTEX_FRAGMENT_MULTIVIEW) |
                              (1 << PIPELINE_TYPE_ATTACHMENT_CLEAR);
...
{
public:
    RandomTestCase(tcu::TestContext &context, const std::string &name, const std::string &description,
...
RandomTestCase::RandomTestCase(tcu::TestContext &context, const std::string &name, const std::string &description,
                               const TestParameters &parameters)
...
{
    const std::string testName = "seed" + de::toString(parameters.randomSeed) +
                                 (parameters.enableGeometry ? "_geometry" : "") +
                                 (parameters.enableTessellation ? "_tessellation" : "");
...
tcu::TestCaseGroup *createDynamicRenderingRandomTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> dynamicRenderingGroup(
        new tcu::TestCaseGroup(testCtx, "random", "Random dynamic rendering tests"));

    for (auto geometry : {true, false})
        for (auto tessellation : {true, false})
        {
            TestParameters parameters = {VK_FORMAT_R8G8B8A8_UNORM, (UVec2(256, 256)), geometry, tessellation, 0u};
