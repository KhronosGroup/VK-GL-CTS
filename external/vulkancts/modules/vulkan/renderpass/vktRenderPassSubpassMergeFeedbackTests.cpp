...
{
public:
    SubpassMergeFeedbackTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
SubpassMergeFeedbackTest::SubpassMergeFeedbackTest(tcu::TestContext &testContext, const std::string &name,
                                                                            ...
                                                                            VkDevice vkDevice)
{
    ...
    for (uint32_t i = 0; i < m_testParams.subpassCount; ++i)
    {
        ...
        resultAttachments.emplace_back(nullptr,                                  // const void*            pNext
                                       i,                                        // uint32_t                attachment
                                       VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // VkImageLayout        layout
                                       aspectMask                                // VkImageAspectFlags    aspectMask
        );

        inputAttachments.emplace_back(nullptr,                                  // const void*            pNext
                                      i,                                        // uint32_t                attachment
    ...
    for (uint32_t i = 0; i < m_testParams.subpassCount; ++i)
    {
        subpassDescriptions.emplace_back(
            ...
            (i > 0) ? 1u : 0u,                            // uint32_t                            inputAttachmentCount
    ...
    for (uint32_t i = 1; i < m_testParams.subpassCount; ++i)
    {
    ...
    if (m_testParams.disallowMergeRenderpass)
    {
        ...
        for (uint32_t i = 0; i < m_testParams.subpassCount; ++i)
        {
            if (subpassFeedbackCreateInfos[i].pSubpassFeedback->subpassMergeStatus !=
                VK_SUBPASS_MERGE_STATUS_DISALLOWED_EXT)
            {
                return tcu::TestStatus::fail("Fail");
            }

            if (i > 0 && subpassFeedbackCreateInfos[i].pSubpassFeedback->postMergeIndex ==
                             subpassFeedbackCreateInfos[i - 1].pSubpassFeedback->postMergeIndex)
            {
                return tcu::TestStatus::fail("Fail");
            }
        }
    ...
    else
    {
        ...
        if (m_testParams.subpassCount == 1 && subpassFeedbackCreateInfos[0].pSubpassFeedback->subpassMergeStatus !=
        ...
        {
            if (i == 1 && m_testParams.disallowMergeSubPass1 &&
                subpassFeedbackCreateInfos[i].pSubpassFeedback->subpassMergeStatus !=
                    VK_SUBPASS_MERGE_STATUS_DISALLOWED_EXT)
            {
                return tcu::TestStatus::fail("Fail");
            }
            ...
            if (subpassFeedbackCreateInfos[i].pSubpassFeedback->subpassMergeStatus ==
                ...
                subpassFeedbackCreateInfos[i].pSubpassFeedback->postMergeIndex !=
                    subpassFeedbackCreateInfos[i - 1].pSubpassFeedback->postMergeIndex)
            {
                return tcu::TestStatus::fail("Fail");
            }
                                                              ...
                                                              const RenderingType renderingType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> subpassMergeFeedbackTests(
        new tcu::TestCaseGroup(testCtx, "subpass_merge_feedback", "Subpass merge feedback tests"));
