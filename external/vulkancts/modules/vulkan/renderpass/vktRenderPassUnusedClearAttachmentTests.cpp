...
struct TestParams
{
    TestParams(size_t numColorAttachments, DepthStencilType depthStencilType_, bool depthStencilUsed_,
               VkFormat depthStencilFormat_, RenderPassType renderPassType_)
...
{
public:
    UnusedClearAttachmentTest(tcu::TestContext &testContext, const std::string &name, const std::string &description,
...
void UnusedClearAttachmentTestInstance::createCommandBuffer(const DeviceInterface &vk, VkDevice vkDevice)
{
    ...
    vk.cmdClearAttachments(*m_cmdBuffer, static_cast<uint32_t>(clearAttachments.size()),
                           (clearAttachments.empty() ? DE_NULL : clearAttachments.data()), 1u, &clearRect);
                                                               ...
                                                               const RenderPassType renderPassType)
{
    de::MovePtr<tcu::TestCaseGroup> testGroup(
        new tcu::TestCaseGroup(testCtx, "unused_clear_attachments", "Unused attachments with vkCmdClearAttachments"));
        ...
        for (const auto dsFormat : dsFormats)
        {
            for (size_t i = 0; i < DE_LENGTH_OF_ARRAY(DE_BOOL_VALUES); ++i)
            {
                ...
                if (hasDepthStencil(dsType))
                    testTypes.emplace_back(0, dsType, depthStencilUse, dsFormat,
                                           renderPassType); // No color attachments.
                testTypes.emplace_back(1, dsType, depthStencilUse, dsFormat,
                                       renderPassType); // Single color attachment.
                testTypes.emplace_back(COLOR_ATTACHMENTS_NUMBER, dsType, depthStencilUse, dsFormat,
                                       renderPassType); // Multiple color attachments.
