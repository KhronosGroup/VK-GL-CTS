...
{
    TestConfigExternal(AllocationKind allocationKind_, RenderPassType renderPassType_)
...
void beginCommandBuffer(const DeviceInterface &vk, VkCommandBuffer cmdBuffer,
                        VkCommandBufferUsageFlags pBeginInfo_flags, VkRenderPass pInheritanceInfo_renderPass,
...
std::string clearColorToString(VkFormat vkFormat, VkClearColorValue value, bool useFormatCompCount)
{
    ...
    const uint32_t componentCount = (useFormatCompCount ? (uint32_t)tcu::getNumUsedChannels(format.order) : 4);
...
public:
    ...
    {
        return m_depthStencilAttachment ? tcu::just(m_depthStencilAttachment->getImageLayout()) :
                                          tcu::nothing<VkImageLayout>();
    }
    Maybe<uint32_t> getDepthStencilAttachmentIndex(void) const
    {
        return m_depthStencilAttachment ? tcu::just(m_depthStencilAttachment->getAttachment()) :
                                          tcu::nothing<uint32_t>();
    };
...
Move<VkPipeline> createSubpassPipeline(const DeviceInterface &vk, VkDevice device, VkRenderPass renderPass,
                                       VkShaderModule vertexShaderModule, VkShaderModule fragmentShaderModule,
                                       VkPipelineLayout pipelineLayout, const SubpassRenderInfo &renderInfo)
{
    Maybe<VkSampleCountFlagBits> rasterSamples;
    vector<VkPipelineColorBlendAttachmentState> attachmentBlendStates;

    for (uint32_t attachmentNdx = 0; attachmentNdx < renderInfo.getColorAttachmentCount(); attachmentNdx++)
    {
...

        {
            const VkPipelineColorBlendAttachmentState attachmentBlendState = {
                ...
                (attachmentNdx < renderInfo.getDrawStartNdx() ?
                     (uint32_t)0 :
                     VK_COLOR_COMPONENT_R_BIT | VK_COLOR_COMPONENT_G_BIT | VK_COLOR_COMPONENT_B_BIT |
                         VK_COLOR_COMPONENT_A_BIT) // channelWriteMask
            };
    ...
    const VkViewport viewport = {(float)renderInfo.getViewportOffset().x(),
                                 (float)renderInfo.getViewportOffset().y(),
                                 (float)renderInfo.getViewportSize().x(),
                                 (float)renderInfo.getViewportSize().y(),
                                 0.0f,
                                 1.0f};

    const VkRect2D scissor = {
        {(int32_t)renderInfo.getViewportOffset().x(), (int32_t)renderInfo.getViewportOffset().y()},
    ...
    const VkBool32 writeDepth =
        renderInfo.getDepthStencilAttachmentLayout() &&
    ...
    const VkPipelineColorBlendStateCreateInfo blendState = {
        ...
        (uint32_t)attachmentBlendStates.size(),                              // attachmentCount
    ...
    return makeGraphicsPipeline(
        ...
        renderInfo.getSubpassIndex(), // const uint32_t                                subpass
...
public:
    SubpassRenderer(Context &context, const DeviceInterface &vk, VkDevice device, Allocator &allocator,
                    VkRenderPass renderPass, VkFramebuffer framebuffer, VkCommandPool commandBufferPool,
                    uint32_t queueFamilyIndex, const vector<VkImage> &attachmentImages,
                    const vector<pair<VkImageView, VkImageView>> &attachmentViews, const SubpassRenderInfo &renderInfo,
                    const vector<Attachment> &attachmentInfos, const AllocationKind allocationKind)
        : m_renderInfo(renderInfo)
    {
        ...
        if (renderInfo.getRenderQuad())
        {
            ...
            m_vertexShaderModule = createShaderModule(
                vk, device, context.getBinaryCollection().get(de::toString(subpassIndex) + "-vert"), 0u);
            m_fragmentShaderModule = createShaderModule(
                vk, device, context.getBinaryCollection().get(de::toString(subpassIndex) + "-frag"), 0u);
            m_pipelineLayout = createPipelineLayout(vk, device, &pipelineLayoutParams);
            m_pipeline = createSubpassPipeline(vk, device, renderPass, *m_vertexShaderModule, *m_fragmentShaderModule,
                                               *m_pipelineLayout, m_renderInfo);
            ...
            uploadBufferData(vk, device, *m_vertexBufferMemory, static_cast<size_t>(vertexBufferSize),
                ...
                }
                {
                         ...
                         inputAttachmentNdx++)
                    {
                        ...
                        const VkImageLayout inputAttachmentLayout =
                            renderInfo.getInputAttachmentLayout(inputAttachmentNdx);
        ...
        if (renderInfo.isSecondary())
        {
            ...
            beginCommandBuffer(vk, *m_commandBuffer, vk::VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT, renderPass,
                               subpassIndex, framebuffer, VK_FALSE, (VkQueryControlFlags)0,
    ...
    void pushRenderCommands(const DeviceInterface &vk, VkCommandBuffer commandBuffer)
    {
...
{
    {
        ...
        if (!initializeLayouts.empty())
            vk.cmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                                  (const VkBufferMemoryBarrier *)DE_NULL, (uint32_t)initializeLayouts.size(),
                                  &initializeLayouts[0]);
    }
...

    {
        ...
        if (!renderPassLayouts.empty())
            vk.cmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                                  (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                                  (const VkBufferMemoryBarrier *)DE_NULL, (uint32_t)renderPassLayouts.size(),
                                  &renderPassLayouts[0]);
    }
}
...
void pushRenderPassCommands(const DeviceInterface &vk, VkCommandBuffer commandBuffer, VkRenderPass renderPass,
                            ...
                            const UVec2 &renderPos, const UVec2 &renderSize,
                            const vector<Maybe<VkClearValue>> &renderPassClearValues, TestConfig::RenderTypes render)
{
    const float clearNan = tcu::Float32::nan().asFloat();
                            ...
                            RenderPassType renderPassType)
{
    ...
    case RENDERPASS_TYPE_LEGACY:
        return pushRenderPassCommands<RenderpassSubpass1>(vk, commandBuffer, renderPass, framebuffer, subpassRenderers,
                                                          renderPos, renderSize, renderPassClearValues, render);
    case RENDERPASS_TYPE_RENDERPASS2:
...
{
public:
    PixelValue(const Maybe<bool> &x = nothing<bool>(), const Maybe<bool> &y = nothing<bool>(),
               const Maybe<bool> &z = nothing<bool>(), const Maybe<bool> &w = nothing<bool>());
    ...
    for (int y = 0; y < result.getHeight(); y++)
        for (int x = 0; x < result.getWidth(); x++)
        {
            ...
            const uint32_t componentCount =
                useFormatCompCount ? (uint32_t)tcu::getNumUsedChannels(result.getFormat().order) : 4;
        ...
        if (!attachmentIsLazy[attachmentNdx])
        {
            ...
            if (tcu::hasDepthComponent(format.order) && tcu::hasStencilComponent(format.order))
            {
...

                {
                    ...
                    {
                        log << TestLog::ImageSet(
                            "TestImages", "Output depth and stencil attachments, reference images and error masks");
                        log << TestLog::Image("Attachment" + de::toString(attachmentNdx) + "Depth",
                                              "Attachment " + de::toString(attachmentNdx) + " Depth", depthAccess);
                        log << TestLog::Image("Attachment" + de::toString(attachmentNdx) + "Stencil",
                                              "Attachment " + de::toString(attachmentNdx) + " Stencil", stencilAccess);
                        ...
                        if (!depthOK)
                            log << TestLog::Image("DepthAttachmentError" + de::toString(attachmentNdx),
                                                  "Depth Attachment Error " + de::toString(attachmentNdx),
                                                  depthErrorImage.getAccess());

                        if (!stencilOK)
                            log << TestLog::Image("StencilAttachmentError" + de::toString(attachmentNdx),
                                                  "Stencil Attachment Error " + de::toString(attachmentNdx),
                                                  stencilErrorImage.getAccess());
            ...
            if (subpass.getInputAttachments().empty())
            {
                     ...
                     attachmentNdx++)
                {
                    ...
                    const size_t componentCount =
                        config.useFormatCompCount ? (size_t)tcu::getNumUsedChannels(format.order) : 4;
...
tcu::TestStatus renderPassTest(Context &context, TestConfig config)
{
    ...
    checkTextureFormatSupport(log, context.getInstanceInterface(), context.getPhysicalDevice(),
                              config.renderPass.getAttachments());
...

    {
        ...
        beginCommandBuffer(vk, *initializeImagesCommandBuffer, (VkCommandBufferUsageFlags)0, DE_NULL, 0, DE_NULL,
                           VK_FALSE, (VkQueryControlFlags)0, (VkQueryPipelineStatisticFlags)0);
...

        {
            const Unique<VkFramebuffer> framebuffer(
                createFramebuffer(vk, device, *renderPass, targetSize, attachmentViews));

            for (size_t subpassNdx = 0; subpassNdx < renderPassInfo.getSubpasses().size(); subpassNdx++)
                subpassRenderers.push_back(de::SharedPtr<SubpassRenderer>(new SubpassRenderer(
                    context, vk, device, allocator, *renderPass, *framebuffer, *commandBufferPool, queueIndex,
                    attachmentImages, inputAttachmentViews, subpassRenderInfo[subpassNdx],
                    config.renderPass.getAttachments(), config.allocationKind)));

            beginCommandBuffer(vk, *renderCommandBuffer, (VkCommandBufferUsageFlags)0, DE_NULL, 0, DE_NULL, VK_FALSE,
                               (VkQueryControlFlags)0, (VkQueryPipelineStatisticFlags)0);
            pushRenderPassCommands(vk, *renderCommandBuffer, *renderPass, *framebuffer, subpassRenderers, renderPos,
                                   renderSize, renderPassClearValues, config.renderTypes, config.renderPassType);
            endCommandBuffer(vk, *renderCommandBuffer);

            beginCommandBuffer(vk, *readImagesToBuffersCommandBuffer, (VkCommandBufferUsageFlags)0, DE_NULL, 0, DE_NULL,
                               VK_FALSE, (VkQueryControlFlags)0, (VkQueryPipelineStatisticFlags)0);
...
void addAttachmentTests(tcu::TestCaseGroup *group, const TestConfigExternal testConfigExternal)
{
    ...
    for (size_t attachmentCountNdx = 0; attachmentCountNdx < DE_LENGTH_OF_ARRAY(attachmentCounts); attachmentCountNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> attachmentCountGroup(new tcu::TestCaseGroup(
            testCtx, de::toString(attachmentCount).c_str(), de::toString(attachmentCount).c_str()));
        ...
        for (size_t testCaseNdx = 0; testCaseNdx < testCaseCount; testCaseNdx++)
        {
...

            {
                ...
                addFunctionCaseWithPrograms<TestConfig>(attachmentCountGroup.get(), testCaseName.c_str(),
                                                        testCaseName.c_str(), createTestShaders, renderPassTest,
                                                        testConfig);
            }
        }
...
void addAttachmentWriteMaskTests(tcu::TestCaseGroup *group, const TestConfigExternal testConfigExternal)
{
         ...
         attachmentCountNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> attachmentCountGroup(
            new tcu::TestCaseGroup(testCtx, groupName.c_str(), de::toString(attachmentCount).c_str()));
        ...
        for (uint32_t drawStartNdx = 0; drawStartNdx < (attachmentCount); drawStartNdx++)
        {
...

            {
                ...
                const TestConfig testConfig(renderPass, render, commandBuffer, imageMemory, targetSize, renderPos,
                                            ...
                                            testConfigExternal.allocationKind, testConfigExternal.renderPassType,
                                            requiredFeatures);

                addFunctionCaseWithPrograms<TestConfig>(attachmentCountGroup.get(), testCaseName.c_str(),
                                                        testCaseName.c_str(), checkSupport, createTestShaders,
...
void addAttachmentAllocationTests(tcu::TestCaseGroup *group, const TestConfigExternal testConfigExternal)
{
    ...
    for (size_t allocationTypeNdx = 0; allocationTypeNdx < DE_LENGTH_OF_ARRAY(allocationTypes); allocationTypeNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> allocationTypeGroup(new tcu::TestCaseGroup(
            testCtx, allocationTypeStr[allocationTypeNdx], allocationTypeStr[allocationTypeNdx]));
        ...
        for (size_t testCaseNdx = 0; testCaseNdx < testCaseCount; testCaseNdx++)
        {
            if (allocationType == ALLOCATIONTYPE_IO_GENERIC)
            {
                ...
                vector<Maybe<uint32_t>> lastUseOfAttachment(attachments.size(), nothing<uint32_t>());
                ...
                for (uint32_t subpassIndex = 0; subpassIndex < subpassCount; subpassIndex++)
                {
                    ...
                    Maybe<uint32_t> depthStencilAttachment(useDepthStencilAttachment ?
...

                    {
                             ...
                             colorAttachmentNdx++)
                        {
                            ...
                            if (lastUseOfAttachment[colorAttachmentIndex])
                            {
                                ...
                                deps.push_back(SubpassDependency(
                                    *lastUseOfAttachment[colorAttachmentIndex], subpassIndex,
                             ...
                             inputAttachmentNdx++)
                        {
                            ...
                            {
                                const bool byRegion =
                                    (*lastUseOfAttachment[inputAttachmentIndex] == subpassIndex) || rng.getBool();

                                deps.push_back(SubpassDependency(
                                    *lastUseOfAttachment[inputAttachmentIndex], subpassIndex,
                                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
                                        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT |
                                        VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
                                        VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
                        ...
                        if (depthStencilAttachment)
                        {
                            ...
                            {
                                const bool byRegion =
                                    (*lastUseOfAttachment[*depthStencilAttachment] == subpassIndex) || rng.getBool();

                                deps.push_back(SubpassDependency(
                                    *lastUseOfAttachment[*depthStencilAttachment], subpassIndex,
                                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT |
                                        VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT |
                                        VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
                                        VK_PIPELINE_STAGE_LATE_FRAGMENT_TESTS_BIT,
            ...
            else
            {
                ...
                for (size_t attachmentNdx = 0; attachmentNdx < attachmentCount; attachmentNdx++)
                {
                    ...
                    const VkImageLayout initialLayout = rng.choose<VkImageLayout>(
                        DE_ARRAY_BEGIN(initialAndFinalColorLayouts), DE_ARRAY_END(initialAndFinalColorLayouts));
                    const VkImageLayout finalizeLayout = rng.choose<VkImageLayout>(
                        DE_ARRAY_BEGIN(initialAndFinalColorLayouts), DE_ARRAY_END(initialAndFinalColorLayouts));
                ...
                else if (allocationType == ALLOCATIONTYPE_IO_CHAIN)
                {
                    ...
                    for (size_t subpassNdx = 1; subpassNdx < attachmentCount; subpassNdx++)
                    {
                        const VkImageAspectFlags inputAttachmentAspectMask =
                            (testConfigExternal.renderPassType == RENDERPASS_TYPE_RENDERPASS2) ?
                                VK_IMAGE_ASPECT_COLOR_BIT :
...
void addSimpleTests(tcu::TestCaseGroup *group, const TestConfigExternal testConfigExternal)
{
    ...
    // color
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "color", "Single color attachment case.", createTestShaders,
                                                renderPassTest, testConfig);
    }
    ...
    // depth
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "depth", "Single depth attachment case.", createTestShaders,
                                                renderPassTest, testConfig);
    }
    ...
    // stencil
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "stencil", "Single stencil attachment case.", createTestShaders,
                                                renderPassTest, testConfig);
    }
    ...
    // depth_stencil
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "depth_stencil", "Single depth stencil attachment case.",
    ...
    // color_depth
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "color_depth", "Color and depth attachment case.",
                                                createTestShaders, renderPassTest, testConfig);
    }
    ...
    // color_stencil
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "color_stencil", "Color and stencil attachment case.",
                                                createTestShaders, renderPassTest, testConfig);
    }
    ...
    // color_depth_stencil
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "color_depth_stencil",
                                                "Color, depth and stencil attachment case.", createTestShaders,
                                                renderPassTest, testConfig);
    }
    ...
    // no attachments
    {
        ...
        addFunctionCaseWithPrograms<TestConfig>(group, "no_attachments", "No attachments case.", createTestShaders,
                                                renderPassTest, testConfig);
    }
    ...
    // color_unused_omit_blend_state
    {
        ...
        const TestConfig testConfig(renderPass, TestConfig::RENDERTYPES_DRAW, TestConfig::COMMANDBUFFERTYPES_INLINE,
                                    TestConfig::IMAGEMEMORY_STRICT, targetSize, renderPos, renderSize, false, 90239, 0,
                                    testConfigExternal.allocationKind, testConfigExternal.renderPassType);
        addFunctionCaseWithPrograms<TestConfig>(group, "color_unused_omit_blend_state",
...
void addFormatTests(tcu::TestCaseGroup *group, const TestConfigExternal testConfigExternal)
{
    ...
    for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_coreColorFormats); formatNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> formatGroup(
            new tcu::TestCaseGroup(testCtx, formatToName(format).c_str(), de::toString(format).c_str()));
        ...
        for (size_t loadOpNdx = 0; loadOpNdx < DE_LENGTH_OF_ARRAY(loadOps); loadOpNdx++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> loadOpGroup(
                new tcu::TestCaseGroup(testCtx, loadOps[loadOpNdx].str, loadOps[loadOpNdx].str));
            ...
            for (size_t renderTypeNdx = 0; renderTypeNdx < DE_LENGTH_OF_ARRAY(renderTypes); renderTypeNdx++)
            {
                ...
                addFunctionCaseWithPrograms<TestConfig>(loadOpGroup.get(), renderTypes[renderTypeNdx].str,
                                                        renderTypes[renderTypeNdx].str, createTestShaders,
                                                        renderPassTest, testConfig);
            }
...

        {
            de::MovePtr<tcu::TestCaseGroup> inputGroup(
                new tcu::TestCaseGroup(testCtx, "input", "Test attachment format as input"));
            ...
            for (size_t loadOpNdx = 0; loadOpNdx < DE_LENGTH_OF_ARRAY(loadOps); loadOpNdx++)
            {
                ...
                for (size_t storeOpNdx = 0; storeOpNdx < DE_LENGTH_OF_ARRAY(storeOps); storeOpNdx++)
                {
                    ...
                    de::MovePtr<tcu::TestCaseGroup> storeOpGroup(
                        new tcu::TestCaseGroup(testCtx, storeOps[storeOpNdx].str, storeOps[storeOpNdx].str));
                        ...
                        {
                            {
...

                                {
                                    ...
                                    addFunctionCaseWithPrograms<TestConfig>(
                                        storeOpGroup.get(), testName, renderTypes[renderTypeNdx].str, createTestShaders,
                                        renderPassTest, testConfig);
                                }
                            }
                            {
...

                                {
                                    ...
                                    addFunctionCaseWithPrograms<TestConfig>(
                                        storeOpGroup.get(), testName,
                                        string("self_dep_") + renderTypes[renderTypeNdx].str, createTestShaders,
                                        renderPassTest, testConfig);
                                }
                            }
                        }
    ...
    for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(s_coreDepthStencilFormats); formatNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> formatGroup(
            new tcu::TestCaseGroup(testCtx, formatToName(vkFormat).c_str(), de::toString(vkFormat).c_str()));
                ...
                {
                    {
                        ...
                        addFunctionCaseWithPrograms<TestConfig>(loadOpGroup.get(), testName,
                                                                renderTypes[renderTypeNdx].str, createTestShaders,
                                                                renderPassTest, testConfig);
                    }
                            ...
                            if (isStencilAttachment && isDepthAttachment)
                            {
                                // Depth read only
                                {
...

                                    {
                                        ...
                                        addFunctionCaseWithPrograms<TestConfig>(
                                            storeOpGroup.get(), testName, renderTypes[renderTypeNdx].str,
                                            createTestShaders, renderPassTest, testConfig);
                                    }
                                }
                                {
                        ...
                        const RenderPassType renderPassType)
{
    ...
    addTestGroup(group, "attachment", "Attachment format and count tests with load and store ops and image layouts",
                 addAttachmentTests, testConfigExternal);
    addTestGroup(group, "attachment_allocation", "Attachment allocation tests", addAttachmentAllocationTests,
                 testConfigExternal);
    addTestGroup(group, "attachment_write_mask", "Attachment write mask tests", addAttachmentWriteMaskTests,
                 testConfigExternal);
}
...
de::MovePtr<tcu::TestCaseGroup> createSuballocationTests(tcu::TestContext &testCtx, RenderPassType renderPassType)
{
    de::MovePtr<tcu::TestCaseGroup> suballocationTestsGroup(
        new tcu::TestCaseGroup(testCtx, "suballocation", "Suballocation RenderPass Tests"));
...
de::MovePtr<tcu::TestCaseGroup> createDedicatedAllocationTests(tcu::TestContext &testCtx, RenderPassType renderPassType)
{
    de::MovePtr<tcu::TestCaseGroup> dedicatedAllocationTestsGroup(
        new tcu::TestCaseGroup(testCtx, "dedicated_allocation", "RenderPass Tests For Dedicated Allocation"));
...
tcu::TestCaseGroup *createRenderPassTestsInternal(tcu::TestContext &testCtx, RenderPassType renderPassType)
{
    ...
    const char *renderpassTestsGroupDescription = (renderPassType == RENDERPASS_TYPE_LEGACY) ? "RenderPass Tests" :
                                                  (renderPassType == RENDERPASS_TYPE_RENDERPASS2) ?
                                                                                               "RenderPass2 Tests" :
                                                                                               "";
    de::MovePtr<tcu::TestCaseGroup> renderpassTests(
        new tcu::TestCaseGroup(testCtx, renderpassTestsGroupName, renderpassTestsGroupDescription));
    de::MovePtr<tcu::TestCaseGroup> suballocationTestGroup = createSuballocationTests(testCtx, renderPassType);
    de::MovePtr<tcu::TestCaseGroup> dedicatedAllocationTestGroup =
        createDedicatedAllocationTests(testCtx, renderPassType);

    suballocationTestGroup->addChild((renderPassType == RENDERPASS_TYPE_LEGACY) ?
                                         createRenderPassMultisampleTests(testCtx) :
                                         createRenderPass2MultisampleTests(testCtx));
    suballocationTestGroup->addChild((renderPassType == RENDERPASS_TYPE_LEGACY) ?
                                         createRenderPassMultisampleResolveTests(testCtx) :
                                         createRenderPass2MultisampleResolveTests(testCtx));
    suballocationTestGroup->addChild((renderPassType == RENDERPASS_TYPE_LEGACY) ?
                                         createRenderPassSubpassDependencyTests(testCtx) :
                                         createRenderPass2SubpassDependencyTests(testCtx));
    suballocationTestGroup->addChild((renderPassType == RENDERPASS_TYPE_LEGACY) ?
                                         createRenderPassSampleReadTests(testCtx) :
                                         createRenderPass2SampleReadTests(testCtx));
    suballocationTestGroup->addChild((renderPassType == RENDERPASS_TYPE_LEGACY) ?
                                         createRenderPassSparseRenderTargetTests(testCtx) :
                                         createRenderPass2SparseRenderTargetTests(testCtx));
