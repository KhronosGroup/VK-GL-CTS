...
{
    TestConfigExternal(AllocationKind allocationKind_, RenderingType renderingType_)
...
struct TestConfig
{
    ...
    TestConfig(const RenderPass &renderPass_, RenderTypes renderTypes_, CommandBufferTypes commandBufferTypes_,
               ...
               RenderingType renderingType_, vector<DeviceCoreFeature> requiredFeatures_ = vector<DeviceCoreFeature>())
...
void beginCommandBuffer(const DeviceInterface &vk, VkCommandBuffer cmdBuffer,
                        ...
                        const SubpassRenderInfo *pRenderInfo = 0, bool dynamicRenderPass = false)
{
    ...
    if (pRenderInfo)
    {
        for (uint32_t i = 0; i < pRenderInfo->getColorAttachmentCount(); ++i)
                                       ...
                                       VkPipelineLayout pipelineLayout, const SubpassRenderInfo &renderInfo)
{
    ...
    std::vector<vk::VkFormat> colorAttachmentFormats;
    for (uint32_t i = 0; i < renderInfo.getColorAttachmentCount(); ++i)
    ...
    return makeGraphicsPipeline(
        ...
        (renderPass == DE_NULL) ? &renderingCreateInfo :
                                  DE_NULL); // const void*                                   pNext)
}
        ...
        : m_renderInfo(renderInfo)
    {
        ...
        if (renderInfo.isSecondary())
        {
            ...
            beginCommandBuffer(vk, *m_commandBuffer, vk::VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT, renderPass,
                               ...
                               (VkQueryPipelineStatisticFlags)0, &renderInfo, dynamicRendering);
...
void pushDynamicRenderingCommands(const DeviceInterface &vk, VkCommandBuffer commandBuffer,
                                  ...
                                  const UVec2 &renderPos, const UVec2 &renderSize,
                                  const vector<Maybe<VkClearValue>> &renderPassClearValues, uint32_t queueIndex,
                                  TestConfig::RenderTypes render)
{
    ...
    const VkRect2D renderArea{{(int32_t)renderPos.x(), (int32_t)renderPos.y()}, {renderSize.x(), renderSize.y()}};
    ...
    for (uint32_t i = 0; i < colorAttachmentsInfo.size(); ++i)
    {
        const AttachmentReference &colorAttachmentReference = colorAttachmentsInfo[i];
        const uint32_t colorAttachmentIndex                 = colorAttachmentReference.getAttachment();
        ...
        if (!resolveAttachmentsInfo.empty())
        {
            ...
            const uint32_t resolveAttachmentIndex                 = resolveAttachmentReference.getAttachment();
        ...
        colorAttachmentVect.push_back({
            ...
            (renderPassClearValues[colorAttachmentIndex] ? *renderPassClearValues[colorAttachmentIndex] :
    ...
    const uint32_t dsAttachmentIndex                           = depthStencilAttachmentReference.getAttachment();
    ...
    if (!imageBarriersBeforeRendering.empty())
        vk.cmdPipelineBarrier(commandBuffer, getAllPipelineStageFlags(), getAllPipelineStageFlags(),
                              (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                              (const VkBufferMemoryBarrier *)DE_NULL, (uint32_t)imageBarriersBeforeRendering.size(),
    ...
    vk::VkRenderingInfoKHR renderingInfo{
        ...
        colorAttachmentVect.empty() ? DE_NULL :
                                      &colorAttachmentVect[0], // const VkRenderingAttachmentInfoKHR* pColorAttachments;
    ...
    if (!imageBarriersAfterRendering.empty())
        vk.cmdPipelineBarrier(commandBuffer, getAllPipelineStageFlags(), getAllPipelineStageFlags(),
                              (VkDependencyFlags)0, 0, (const VkMemoryBarrier *)DE_NULL, 0,
                              (const VkBufferMemoryBarrier *)DE_NULL, (uint32_t)imageBarriersAfterRendering.size(),
...
void pushRenderPassCommands(const DeviceInterface &vk, VkCommandBuffer commandBuffer, VkRenderPass renderPass,
                            ...
                            const UVec2 &renderPos, const UVec2 &renderSize,
                            const vector<Maybe<VkClearValue>> &renderPassClearValues, uint32_t queueIndex,
                            TestConfig::RenderTypes render, RenderingType renderingType)
{
    ...
    case RENDERING_TYPE_DYNAMIC_RENDERING:
        return pushDynamicRenderingCommands(vk, commandBuffer, renderPassInfo, attachmentResources, subpassRenderers,
                                            renderPos, renderSize, renderPassClearValues, queueIndex, render);
    default:
        TCU_THROW(InternalError, "Impossible");
    }
}
...

        {
            ...
            for (size_t subpassNdx = 0; subpassNdx < renderPassInfo.getSubpasses().size(); subpassNdx++)
                subpassRenderers.push_back(de::SharedPtr<SubpassRenderer>(new SubpassRenderer(
                    ...
                    config.renderingType == RENDERING_TYPE_DYNAMIC_RENDERING)));
            ...
            pushRenderPassCommands(vk, *renderCommandBuffer, *renderPass, renderPassInfo, attachmentResources,
                                   *framebuffer, subpassRenderers, renderPos, renderSize, renderPassClearValues,
                                   queueIndex, config.renderTypes, config.renderingType);
        ...
        for (uint32_t drawStartNdx = 0; drawStartNdx < (attachmentCount); drawStartNdx++)
        {
            ...
            for (uint32_t attachmentNdx = 0; attachmentNdx < attachmentCount; attachmentNdx++)
            {
                ...
                const VkImageLayout finalizeLayout =
                    (testConfigExternal.renderingType == RENDERING_TYPE_DYNAMIC_RENDERING) ?
                        initialLayout :
                        VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
...

            {
                ...
                const TestConfig testConfig(renderPass, render, commandBuffer, imageMemory, targetSize, renderPos,
                                            ...
                                            testConfigExternal.allocationKind, testConfigExternal.renderingType,
                                            requiredFeatures);
                                ...
                                for (SubpassDependency &dependency : deps)
                                {
                                    if (dependency.getSrcPass() == srcPass && dependency.getDstPass() == dstPass)
                                    {
                                        const VkAccessFlags newSrcFlags = dependency.getSrcAccessMask() |
                                                                          VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT;
                                        const VkAccessFlags newDstFlags =
                                            dependency.getDstAccessMask() | VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT;
                                        dependency.setDstAccessMask(newSrcFlags);
                                        dependency.setDstAccessMask(newDstFlags);
                                        foundDuplicate = true;
                                        break;
                ...
                else if (allocationType == ALLOCATIONTYPE_IO_CHAIN)
                {
                    ...
                    for (size_t subpassNdx = 1; subpassNdx < attachmentCount; subpassNdx++)
                    {
                        const VkImageAspectFlags inputAttachmentAspectMask =
                            (testConfigExternal.renderingType == RENDERING_TYPE_RENDERPASS2) ?
                                VK_IMAGE_ASPECT_COLOR_BIT :
...
tcu::TestCaseGroup *createRenderPassTestsInternal(tcu::TestContext &testCtx, RenderingType renderingType)
{
    const char *renderingTestsGroupName = (renderingType == RENDERING_TYPE_RENDERPASS_LEGACY) ? "renderpass" :
                                          ...
                                          (renderingType == RENDERING_TYPE_DYNAMIC_RENDERING) ? "dynamic_rendering" :
                                                                                                "";
    const char *renderingTestsGroupDescription =
        (renderingType == RENDERING_TYPE_RENDERPASS_LEGACY) ? "RenderPass Tests" :
    ...
    de::MovePtr<tcu::TestCaseGroup> renderingTests(
        new tcu::TestCaseGroup(testCtx, renderingTestsGroupName, renderingTestsGroupDescription));
    ...
    de::MovePtr<tcu::TestCaseGroup> dedicatedAllocationTestGroup =
        createDedicatedAllocationTests(testCtx, renderingType);
