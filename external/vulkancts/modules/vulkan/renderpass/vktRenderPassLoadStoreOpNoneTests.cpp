...
Move<VkRenderPass> createRenderPass(const DeviceInterface &vk, VkDevice vkDevice, const TestParams testParams)
{
    ...
    const VkImageAspectFlags depthStencilAspectMask = testParams.renderingType == RENDERING_TYPE_RENDERPASS_LEGACY ?
                                                          0 :
                                                      ...
                                                      std::vector<Move<VkPipeline>> &pipelines)
{
    ...
    for (size_t i = 0; i < m_testParams.subpasses.size(); i++)
    {
        if (i != 0)
        {
            ...
            DE_ASSERT(
                m_testParams.renderingType !=
                RENDERING_TYPE_DYNAMIC_RENDERING); // multi subpass tests should not be executed for dynamic rendering
        }
...
tcu::TestStatus LoadStoreOpNoneTestInstance::iterate(void)
{
    ...
    for (const auto &att : m_testParams.attachments)
    {
        ...
        if (att.init & ATTACHMENT_INIT_PRE)
        {
            ...
            {
                VkClearColorValue clearColor = att.usage & ATTACHMENT_USAGE_INTEGER ?
                                                   makeClearValueColorU32(0u, 255u, 0u, 255u).color :
    ...
    Unique<VkShaderModule> fragmentShaderModuleUint(
        createShaderModule(vk, vkDevice, m_context.getBinaryCollection().get("color_frag_uint"), 0));
    ...
    for (size_t i = 0; i < m_testParams.attachments.size(); i++)
    {
        if (m_testParams.attachments[i].verifyInner || m_testParams.attachments[i].verifyOuter)
        {
            ...
            {
                VkImageLayout layout = (m_testParams.renderingType == RENDERING_TYPE_DYNAMIC_RENDERING) ?
            ...
            else
            {
                textureLevelResult = pipeline::readColorAttachment(vk, vkDevice, queue, queueFamilyIndex, allocator,
                                                                   *attachmentImages[i], format, m_imageSize,
                                                                   VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL);
            }
...
tcu::TestCaseGroup *createRenderPassLoadStoreOpNoneTests(tcu::TestContext &testCtx, const RenderingType renderingType)
{
    ...
    // doesn't store the results.
    {
        ...
        params.attachments.push_back({ATTACHMENT_USAGE_COLOR | ATTACHMENT_USAGE_MULTISAMPLE | ATTACHMENT_USAGE_INTEGER,
                                      VK_ATTACHMENT_LOAD_OP_NONE_EXT, VK_ATTACHMENT_STORE_OP_NONE_EXT,
                                      ATTACHMENT_INIT_PRE, false, green, false, green});
        params.attachments.push_back(
            {ATTACHMENT_USAGE_COLOR | ATTACHMENT_USAGE_RESOLVE_TARGET | ATTACHMENT_USAGE_INTEGER,
             VK_ATTACHMENT_LOAD_OP_LOAD, VK_ATTACHMENT_STORE_OP_STORE, ATTACHMENT_INIT_PRE, true, redUint, true,
             greenUint});
