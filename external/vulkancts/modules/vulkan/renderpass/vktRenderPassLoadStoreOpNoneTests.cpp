    ...
    for (const auto &subpass : testParams.subpasses)
    {
        ...
        for (const auto &ref : subpass.attachmentRefs)
        {
            ...
            else if (ref.usage & ATTACHMENT_USAGE_DEPTH_STENCIL)
            {
                ...
                const auto depthStencilAspectMask = testParams.renderingType == RENDERING_TYPE_RENDERPASS_LEGACY ?
                                                        0 :
    ...
    for (const auto &att : m_testParams.attachments)
    {
        ...
        if (att.init & ATTACHMENT_INIT_PRE)
        {
            ...
            else
            {
                ...
                const auto layout = ((firstUsage & ATTACHMENT_USAGE_COLOR) ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL :
...
tcu::TestCaseGroup *createRenderPassLoadStoreOpNoneTests(tcu::TestContext &testCtx, const RenderingType renderingType)
{
    ...
    for (uint32_t f = 0; f < formats.size(); ++f)
    {
        ...
        if (hasStencil)
        {
            ...
            params.subpasses.push_back(
                {{{0u, ATTACHMENT_USAGE_COLOR},
                  {1u, ATTACHMENT_USAGE_STENCIL | ATTACHMENT_USAGE_STENCIL_TEST_OFF | ATTACHMENT_USAGE_DEPTH_TEST_OFF}},
                 1u});
        ...
        if (hasDepth && hasStencil)
        {
            ...
            params.subpasses.push_back({{{0u, ATTACHMENT_USAGE_COLOR},
        ...
        if (hasDepth && hasStencil)
        {
            ...
            params.attachments.push_back({ATTACHMENT_USAGE_DEPTH_STENCIL,
                                          ...
                                          {{VK_IMAGE_ASPECT_DEPTH_BIT, true, depthInit, true, depthInit},
                                           {VK_IMAGE_ASPECT_STENCIL_BIT, true, stencilFull, true, stencilInit}}});
            params.subpasses.push_back(
                {{{0u, ATTACHMENT_USAGE_COLOR}, {1u, ATTACHMENT_USAGE_DEPTH_STENCIL | ATTACHMENT_USAGE_DEPTH_TEST_OFF}},
                 1u});
