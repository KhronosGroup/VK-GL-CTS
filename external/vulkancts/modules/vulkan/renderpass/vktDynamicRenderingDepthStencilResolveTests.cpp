...
protected:
	...
	void						beginSecondaryCommandBuffer(VkCommandBuffer cmdBuffer, VerifyBuffer attachmentType, VkRenderingFlagsKHR renderingFlags = 0) const;
...
DepthStencilResolveTest::DepthStencilResolveTest(Context& context, TestConfig config)
	...
	, m_singlesampleImage(createImage(m_config.format, 1, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT))
...
VkImageViewSp DepthStencilResolveTest::createImageView(VkImageSp image, VkFormat vkformat, deUint32 baseArrayLayer)
{
	...
	const VkImageViewCreateInfo	pCreateInfo	=
	{
		...
		** image,																			// VkImage                    image
		(m_config.viewLayers > 1) ? VK_IMAGE_VIEW_TYPE_2D_ARRAY : VK_IMAGE_VIEW_TYPE_2D,	// VkImageViewType            viewType
		vkformat,																			// VkFormat                   format
...
#ifndef CTS_USES_VULKANSC
void DepthStencilResolveTest::beginSecondaryCommandBuffer(VkCommandBuffer cmdBuffer, VerifyBuffer attachmentType, VkRenderingFlagsKHR renderingFlags) const
{
...
void DepthStencilResolveTest::submit(void)
{
	...
	{
		const Unique<VkCommandBuffer>	clearCmdBuffer(allocateCommandBuffer(vkd, device, *m_commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));

		beginCommandBuffer(vkd, *clearCmdBuffer);
		vkd.cmdPipelineBarrier(*clearCmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &preClearBarrier);

		vkd.cmdClearDepthStencilImage(*clearCmdBuffer, **m_singlesampleImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, &m_config.clearValue, 1u, &imageRange);
	...
	if (testingDepth)
	{
		...
		if (m_config.groupParams->useSecondaryCmdBuffer)
		{
			...
			if (m_config.groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
			{
				beginSecondaryCommandBuffer(*secCmdBuffer, VB_DEPTH, VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT);
			...
			vkd.cmdPipelineBarrier(*cmdBuffer, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, vk::VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT |
				vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &preRenderBarrier);
	...
	else
	{
		...
		if (m_config.groupParams->useSecondaryCmdBuffer)
		{
			...
			for (deUint32 i = 0; i < m_config.sampleCount; i++)
			{
				if (i == 0)
				{
					...
					vkd.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, 0, 0, 0, 0, 0, 0, 0);

					if (m_config.groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
						beginSecondaryCommandBuffer(*secCmdBuffer, VB_STENCIL, VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT);
					else
				...
				const deUint32 stencilReference = 1 + 254 * (i >= halfOfSamples);

				vkd.cmdBindPipeline(*secCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_renderPipeline);
				vkd.cmdPushConstants(*secCmdBuffer, *m_renderPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0u, sizeof(i), &i);
		...
		else
		{
			for (deUint32 i = 0; i < m_config.sampleCount; i++)
			{
				...
				vkd.cmdBindPipeline(*cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_renderPipeline);
				vkd.cmdPushConstants(*cmdBuffer, *m_renderPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0u, sizeof(i), &i);
	...
	// Memory barriers between rendering and copying
	{
		const VkImageMemoryBarrier barrier =
		{
			...
			**m_singlesampleImage,
			{
				(m_config.separateDepthStencilLayouts) ? VkImageAspectFlags(testingDepth ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_STENCIL_BIT) : aspectFlagsForFormat(m_config.format),
				0u,
				1u,
				0u,
				m_config.viewLayers
		...
		vkd.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, 1u, &barrier);
	}
	...
	vkd.cmdCopyImageToBuffer(*cmdBuffer, **m_singlesampleImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, **m_buffer, 1u, &region);
	...
	// Memory barriers between copies and host access
	{
		...
		vkd.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 0u, DE_NULL, 1u, &barrier, 0u, DE_NULL);
	}
