...
tcu::TextureLevel generateColroImage(const tcu::TextureFormat format, const UVec2 &renderSize, const int attachmentNdx)
{
...
tcu::TextureLevel generateDepthImage(const tcu::TextureFormat format, const UVec2 &renderSize)
{
    tcu::TextureLevel image(format, renderSize.x(), renderSize.y());
                             ...
                             const ImagesFormat &imagesFormat)
{
    const VkFormat depthStencilFormat =
        (imagesFormat.depth != VK_FORMAT_UNDEFINED) ? imagesFormat.depth : imagesFormat.stencil;
...
protected:
    ...
    void preBarier(const uint32_t colorAtchCount, ImagesLayout &imagesLayout, const ImagesFormat &imagesFormat);
    void beginRenderingKHR(VkCommandBuffer cmdBuffer, const std::vector<VkImageView> &attachmentBindInfos,
...
tcu::TestStatus DynamicRenderingTestInstance::iterate(void)
{
    ...
    for (int attachmentTest = 0; attachmentTest < TEST_ATTACHMENT_LAST; ++attachmentTest)
    {
        ...
        Move<VkPipeline> pipeline = makeGraphicsPipeline(
            vk, device, *m_pipelineLayout, *m_vertexModule, *m_fragmentModule, m_parameters.renderSize, colorAtchCount,
            imagesFormat.colors,
...
void DynamicRenderingTestInstance::initialize(void)
{
    ...
    //Image stencil and depth attachment.
    {
        ...
        const VkDeviceSize imageBufferDepthlSize = m_parameters.renderSize.x() * m_parameters.renderSize.y() *
                                                   tcu::getPixelSize(getDepthTextureFormat(m_formatStencilDepthImage));

        const VkImageUsageFlags imageStenciDepthlUsage =
            VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
        const VkImageCreateInfo imageInfo =
            makeImageCreateInfo(m_formatStencilDepthImage, m_parameters.renderSize, imageStenciDepthlUsage);
        const VkBufferCreateInfo bufferStencilInfo =
            makeBufferCreateInfo(imageBufferStencilSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT);
        const VkBufferCreateInfo bufferDepthlInfo =
            makeBufferCreateInfo(imageBufferDepthlSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT);
        ...
        m_imageDepthBuffer =
            Buffer::createAndAlloc(vk, device, bufferDepthlInfo, allocator, MemoryRequirement::HostVisible);
    ...
    for (uint32_t ndx = 0; ndx < COLOR_ATTACHMENTS_NUMBER; ++ndx)
    {
        m_referenceImages.push_back(
            generateColroImage(mapVkFormat(m_parameters.imageFormat), m_parameters.renderSize, ndx));
    }

    m_referenceImages.push_back(
        generateDepthImage(getDepthTextureFormat(m_formatStencilDepthImage), m_parameters.renderSize));

    m_referenceImages.push_back(generateStencilImage(mapVkFormat(VK_FORMAT_S8_UINT), m_parameters.renderSize));
}
                                             ...
                                             const ImagesFormat &imagesFormat)
{
    ...
    for (uint32_t attachmentLoadOp = 0; attachmentLoadOp < TEST_ATTACHMENT_LOAD_OP_LAST; ++attachmentLoadOp)
        for (uint32_t attachmentStoreOp = 0; attachmentStoreOp < TEST_ATTACHMENT_STORE_OP_LAST; ++attachmentStoreOp)
        {
            ...
            beginRenderingKHR(*m_cmdBuffer, attachmentBindInfos, 0, colorAtchCount, imagesFormat,
...
void DynamicRenderingTestInstance::preBarier(const uint32_t colorAtchCount, ImagesLayout &imagesLayout,
                                             const ImagesFormat &imagesFormat)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    ...
    if (imagesFormat.depth != VK_FORMAT_UNDEFINED)
    {
        ...
        const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
            VK_ACCESS_NONE_KHR, VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT, imagesLayout.oldDepth,
    ...
    if (imagesFormat.stencil != VK_FORMAT_UNDEFINED)
    {
        ...
        const VkImageMemoryBarrier barrier = makeImageMemoryBarrier(
            VK_ACCESS_NONE_KHR, VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT, imagesLayout.oldStencil,
            VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, *m_imageStencilDepth, subresource);
...
void DynamicRenderingTestInstance::verifyResults(const uint32_t colorAtchCount, const ImagesFormat &imagesFormat)
{
    ...
    if (imagesFormat.depth != VK_FORMAT_UNDEFINED)
    {
        ...
        if (m_formatStencilDepthImage == VK_FORMAT_D24_UNORM_S8_UINT)
        {
            ...
            tcu::copy(tcu::getEffectiveDepthStencilAccess(result->getAccess(), tcu::Sampler::MODE_DEPTH),
                      resultDepthImage);

            const tcu::ConstPixelBufferAccess depthResult =
                tcu::getEffectiveDepthStencilAccess(result->getAccess(), tcu::Sampler::MODE_DEPTH);
            const tcu::ConstPixelBufferAccess expectedResult = tcu::getEffectiveDepthStencilAccess(
                m_referenceImages[COLOR_ATTACHMENTS_NUMBER].getAccess(), tcu::Sampler::MODE_DEPTH);
                                        ...
                                        const ImagesFormat &imagesFormat)
{
    ...
    for (uint32_t attachmentLoadOp = 0; attachmentLoadOp < TEST_ATTACHMENT_LOAD_OP_LAST; ++attachmentLoadOp)
        for (uint32_t attachmentStoreOp = 0; attachmentStoreOp < TEST_ATTACHMENT_STORE_OP_LAST; ++attachmentStoreOp)
        {
            ...
            beginRenderingKHR(*m_cmdBuffer, attachmentBindInfos, VK_RENDERING_SUSPENDING_BIT_KHR, colorAtchCount,
            ...
            beginRenderingKHR(*m_cmdBuffer, attachmentBindInfos, VK_RENDERING_RESUMING_BIT_KHR, colorAtchCount,
                              imagesFormat, static_cast<VkAttachmentLoadOp>(attachmentLoadOp),
                              static_cast<VkAttachmentStoreOp>(attachmentStoreOp));
                                            ...
                                            const ImagesFormat &imagesFormat)
{
    ...
    for (uint32_t attachmentLoadOp = 0; attachmentLoadOp < TEST_ATTACHMENT_LOAD_OP_LAST; ++attachmentLoadOp)
        for (uint32_t attachmentStoreOp = 0; attachmentStoreOp < TEST_ATTACHMENT_STORE_OP_LAST; ++attachmentStoreOp)
        {
            ...
            beginRenderingKHR(*m_cmdBuffer2, attachmentBindInfos, VK_RENDERING_RESUMING_BIT_KHR, colorAtchCount,
                              imagesFormat, static_cast<VkAttachmentLoadOp>(attachmentLoadOp),
                              static_cast<VkAttachmentStoreOp>(attachmentStoreOp));
                                              ...
                                              const ImagesFormat &imagesFormat)
{
    ...
    for (uint32_t attachmentLoadOp = 0; attachmentLoadOp < TEST_ATTACHMENT_LOAD_OP_LAST; ++attachmentLoadOp)
        for (uint32_t attachmentStoreOp = 0; attachmentStoreOp < TEST_ATTACHMENT_STORE_OP_LAST; ++attachmentStoreOp)
        {
            ...
            beginRenderingKHR(secCmdBuffers[0], attachmentBindInfos, VK_RENDERING_SUSPENDING_BIT_KHR, colorAtchCount,
                              imagesFormat, static_cast<VkAttachmentLoadOp>(attachmentLoadOp),
                              static_cast<VkAttachmentStoreOp>(attachmentStoreOp));
            ...
            beginRenderingKHR(secCmdBuffers[1], attachmentBindInfos, VK_RENDERING_RESUMING_BIT_KHR, colorAtchCount,
                              imagesFormat, static_cast<VkAttachmentLoadOp>(attachmentLoadOp),
                              static_cast<VkAttachmentStoreOp>(attachmentStoreOp));
                ...
                (static_cast<VkAttachmentStoreOp>(attachmentStoreOp) == VK_ATTACHMENT_STORE_OP_STORE))
            {
                ...
                beginRenderingKHR(secCmdBuffers[0], attachmentBindInfos, VK_RENDERING_SUSPENDING_BIT_KHR,
                                           ...
                                           const ImagesFormat &imagesFormat)
{
    ...
    for (uint32_t attachmentLoadOp = 0; attachmentLoadOp < TEST_ATTACHMENT_LOAD_OP_LAST; ++attachmentLoadOp)
        for (uint32_t attachmentStoreOp = 0; attachmentStoreOp < TEST_ATTACHMENT_STORE_OP_LAST; ++attachmentStoreOp)
        {
            ...
            beginRenderingKHR(*m_cmdBuffer, attachmentBindInfos,
                                                                ...
                                                                const ImagesFormat &imagesFormat)
{
    ...
    for (uint32_t attachmentLoadOp = 0; attachmentLoadOp < TEST_ATTACHMENT_LOAD_OP_LAST; ++attachmentLoadOp)
        for (uint32_t attachmentStoreOp = 0; attachmentStoreOp < TEST_ATTACHMENT_STORE_OP_LAST; ++attachmentStoreOp)
        {
            ...
            beginRenderingKHR(*m_cmdBuffer2, attachmentBindInfos,
                              VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT_KHR | VK_RENDERING_RESUMING_BIT_KHR,
                              colorAtchCount, imagesFormat, static_cast<VkAttachmentLoadOp>(attachmentLoadOp),
                              static_cast<VkAttachmentStoreOp>(attachmentStoreOp));
...
tcu::TestCaseGroup *createDynamicRenderingBasicTests(tcu::TestContext &testCtx)
{
    de::MovePtr<tcu::TestCaseGroup> dynamicRenderingGroup(
        new tcu::TestCaseGroup(testCtx, "basic", "Basic dynamic rendering tests"));
