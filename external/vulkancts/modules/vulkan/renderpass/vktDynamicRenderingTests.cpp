...
void DynamicRenderingTestInstance::verifyDepth(const tcu::TextureLevel &depthReference)
{
    ...
    {
        if (!tcu::floatThresholdCompare(log, "Compare Depth Image", "Result comparison", depthReference.getAccess(),
                                        resultDepthImage, Vec4(0.02f), tcu::COMPARE_LOG_ON_ERROR))
        {
            TCU_FAIL("Rendered depth image is not correct");
        }
    }
                                          ...
                                          const ImagesFormat &imagesFormat, bool clearOnly, bool useSecondary)
{
    ...
    // Clear before we start rendering with a different value to ensure unbound one is not modified when rendering
    {
        ...
        cmdPipelineImageMemoryBarrier(
            vk, *m_cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
            VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT | VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, &barrier, 1u);
    ...
    // Verify depth
    {
        ...
        {
            reference = tcu::TextureLevel(getDepthTextureFormat(m_formatStencilDepthImage), m_parameters.renderSize.x(),
