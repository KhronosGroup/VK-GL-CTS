...
void DynamicRenderingTestInstance::initialize(void)
{
    ...
    m_referenceImages.push_back(
        generateDepthImage(getDepthTextureFormat(m_formatStencilDepthImage), m_parameters.renderSize, 0.2f));
                                             ...
                                             ImagesLayout &imagesLayout, const ImagesFormat &imagesFormat)
{
    ...
    cmdPipelineImageMemoryBarrier(vk, cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT,
...
{
    const std::string testName[TEST_TYPE_LAST] = {"single_cmdbuffer",
