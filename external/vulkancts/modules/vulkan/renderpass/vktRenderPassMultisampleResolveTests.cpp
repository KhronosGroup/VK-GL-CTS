...
protected:
    ...
    vector<VkImageViewSp> createImageViews(const vector<VkImageSp> &images, uint32_t mipLevel = 0) const;
                                                                      ...
                                                                      uint32_t mipLevel) const
{
    ...
    const VkImageSubresourceRange range = {VK_IMAGE_ASPECT_COLOR_BIT, mipLevel, 1u, 0u, m_layerCount};
    ...
    for (size_t layerCountNdx = 0; layerCountNdx < DE_LENGTH_OF_ARRAY(layerCounts); layerCountNdx++)
    {
        ...
        for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
        {
            ...
            for (size_t sampleCountNdx = 0; sampleCountNdx < DE_LENGTH_OF_ARRAY(sampleCounts); sampleCountNdx++)
            {
                ...
                const TestConfig testConfig = {RESOLVE, format, sampleCount, layerCount, 4u, 32u, 32u, renderPassType};
                ...
                for (uint32_t resolveLevel : resolveLevels)
                {
                    ...
                    formatGroup->addChild(
                        new InstanceFactory1WithSupport<MultisampleRenderPassResolveLevelTestInstance, TestConfig2,
                            ...
                            testCtx, tcu::NODETYPE_SELF_VALIDATE, resolveLevelTestName, resolveLevelTestName,
                            testConfig2, typename FunctionSupport1<TestConfig2>::Args(checkSupport, testConfig2)));
                }
                ...
                if (layerCount == 1)
                {
...

                    {
                        ...
                        formatGroup->addChild(
                            new InstanceFactory1<MultisampleRenderPassTestInstance, TestConfig, Programs>(
