        ...
        for (size_t dstNdx = 0; dstNdx < m_singlesampleImages.size(); dstNdx++)
        {
            const VkImageMemoryBarrier barrier = {VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER,
                                                  ...
                                                  **m_singlesampleImages[dstNdx],
                                                  {VK_IMAGE_ASPECT_COLOR_BIT, m_renderLevel, 1u, 0u, m_layerCount}};
...
#ifndef CTS_USES_VULKANSC
void MultisampleRenderPassTestInstance::beginSecondaryCmdBuffer(VkCommandBuffer cmdBuffer,
    ...
    for (size_t layerCountNdx = 0; layerCountNdx < DE_LENGTH_OF_ARRAY(layerCounts); layerCountNdx++)
    {
        ...
        for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
        {
            ...
            for (size_t sampleCountNdx = 0; sampleCountNdx < DE_LENGTH_OF_ARRAY(sampleCounts); sampleCountNdx++)
            {
                ...
                const TestConfig testConfig{RESOLVE, format, sampleCount, layerCount, 4u, 32u, 32u, groupParams};
...
tcu::TestCaseGroup *createDynamicRenderingMultisampleResolveTests(tcu::TestContext &testCtx,
                                                                  const renderpass::SharedGroupParams groupParams)
{
    return createTestGroup(testCtx, "multisample_resolve", "Multisample dynamic rendering resolve tests", initTests,
                           groupParams);
}
