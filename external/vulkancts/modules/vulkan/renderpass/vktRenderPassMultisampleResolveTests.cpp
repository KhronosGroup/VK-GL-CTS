...
{
public:
    MultisampleRenderPassTestBase(Context &context, TestConfig config, uint32_t attachmentsCount);
...
protected:
    ...
    bool featuresSupported(Context &context, TestConfig config) const;
...
MultisampleRenderPassTestBase::MultisampleRenderPassTestBase(Context &context, TestConfig config,
    ...
    , m_extensionSupported((config.renderPassType == RENDERPASS_TYPE_RENDERPASS2) &&
...
vector<VkImageViewSp> MultisampleRenderPassTestBase::createImageViews(const vector<VkImageSp> &images) const
{
    ...
    const VkImageSubresourceRange range = {VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, m_layerCount};
...
bool MultisampleRenderPassTestBase::featuresSupported(Context &context, TestConfig config) const
{
    bool result = ((config.layerCount > 1) && context.requireDeviceCoreFeature(DEVICE_CORE_FEATURE_GEOMETRY_SHADER));
...
tcu::Vec4 MultisampleRenderPassTestBase::getFormatThreshold() const
{
    ...
    const uint32_t componentCount(tcu::getNumUsedChannels(tcuFormat.order));
...
void MultisampleRenderPassTestBase::logImage(const std::string &name, const tcu::ConstPixelBufferAccess &image) const
{
    ...
    for (uint32_t layerNdx = 0; layerNdx < m_layerCount; ++layerNdx)
    {
...
MultisampleRenderPassTestInstance::MultisampleRenderPassTestInstance(Context &context, TestConfig config)
    ...
    , m_singlesampleImages(
          createImages(VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT))
...
void MultisampleRenderPassTestInstance::submit(void)
{
    ...
    // Render black samples
    {
        ...
        vkd.cmdPushConstants(*commandBuffer, *m_renderPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0u,
                             sizeof(m_sampleMask), &m_sampleMask);
    ...
    for (size_t dstNdx = 0; dstNdx < m_singlesampleImages.size(); dstNdx++)
        copyImageToBuffer(vkd, *commandBuffer, **m_singlesampleImages[dstNdx], **m_buffers[dstNdx],
                          ...
                          VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, m_layerCount);
...
void MultisampleRenderPassTestInstance::verify(void)
{
    ...
    case tcu::TEXTURECHANNELCLASS_SIGNED_FIXED_POINT:
    {
        ...
        for (uint32_t z = 0; z < m_layerCount; z++)
            for (uint32_t y = 0; y < m_height; y++)
                for (uint32_t x = 0; x < m_width; x++)
                {
                    ...
                    const Vec4 refColor(
                        m_sampleMask == 0x0u ?
                            Vec4(clearValue, componentCount > 1 ? clearValue : 0.0f,
                                 componentCount > 2 ? clearValue : 0.0f, componentCount > 3 ? clearValue : 1.0f) :
                        m_sampleMask == ((0x1u << m_sampleCount) - 1u) ?
                            Vec4(renderValue, componentCount > 1 ? renderValue : 0.0f,
                                 componentCount > 2 ? renderValue : 0.0f, componentCount > 3 ? renderValue : 1.0f) :
                            firstColor);
...
tcu::TestStatus MultisampleRenderPassTestInstance::iterate(void)
{
    ...
    if (m_sampleMask == ((0x1u << m_sampleCount) - 1u))
    {
            ...
            channelClass == tcu::TEXTURECHANNELCLASS_FLOATING_POINT)
        {
            ...
            case tcu::TEXTURECHANNELCLASS_UNSIGNED_FIXED_POINT:
            {
                expectedAverage = Vec4(0.5f, componentCount > 1 ? 0.5f : 0.0f, componentCount > 2 ? 0.5f : 0.0f,
                                       componentCount > 3 ? 0.5f : 1.0f);
                break;
            }
            ...
            for (uint32_t z = 0; z < m_layerCount; z++)
                for (uint32_t y = 0; y < m_height; y++)
                    for (uint32_t x = 0; x < m_width; x++)
                    {
                        ...
                        if (diff[0] > threshold.x() || diff[1] > threshold.y() || diff[2] > threshold.z() ||
                            diff[3] > threshold.w())
                        {
                            isOk    = false;
...
void MaxAttachmenstsRenderPassTestInstance::submit(void)
{
    ...
    // Second subpasss - merge attachments
    {
        ...
        vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayoutPass1, 0, 1u,
    ...
    // Memory barriers between rendering and copies
    {
        ...
        vkd.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                               VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, DE_NULL, 0u, DE_NULL, (uint32_t)barriers.size(),
                               &barriers[0]);
    }
    ...
    for (size_t dstNdx = 0; dstNdx < m_singlesampleImages.size(); dstNdx++)
    {
        const VkBufferImageCopy region = {0u,
                                          0u,
                                          0u,
                                          {
        ...
        vkd.cmdCopyImageToBuffer(*commandBuffer, **m_singlesampleImages[dstNdx], VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
    ...
    // Memory barriers between copies and host access
    {
        ...
        vkd.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 0u,
                               DE_NULL, (uint32_t)barriers.size(), &barriers[0], 0u, DE_NULL);
    }
...
void MaxAttachmenstsRenderPassTestInstance::submitSwitch(RenderPassType renderPassType)
{
    switch (renderPassType)
    {
    case RENDERPASS_TYPE_LEGACY:
...
Move<VkDescriptorSet> MaxAttachmenstsRenderPassTestInstance::createDescriptorSet()
{
    ...
    vkd.updateDescriptorSets(device, (uint32_t)descriptorWrites.size(), &descriptorWrites[0], 0u, DE_NULL);
...
Move<VkRenderPass> MaxAttachmenstsRenderPassTestInstance::createRenderPass(void)
{
    ...
    for (size_t attachmentNdx = 0; attachmentNdx < m_attachmentsCount; attachmentNdx++)
    {
        ...
        {
            const AttDesc multisampleAttachment(DE_NULL,                                 // pNext
                                                0u,                                      // flags
                                                m_format,                                // format
                                                m_sampleCount,                           // samples
                                                VK_ATTACHMENT_LOAD_OP_DONT_CARE,         // loadOp
            ...
            const AttRef attachmentRef(DE_NULL,
                                       (uint32_t)attachments.size(),             // attachment
                                       VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // layout
                                       0u                                        // aspectMask
            );
        ...
        {
            const AttDesc singlesampleAttachment(DE_NULL,                          // pNext
                                                 0u,                               // flags
                                                 m_format,                         // format
                                                 VK_SAMPLE_COUNT_1_BIT,            // samples
                                                 VK_ATTACHMENT_LOAD_OP_DONT_CARE,  // loadOp
            ...
            const AttRef attachmentRef(DE_NULL,                      // pNext
                                       (uint32_t)attachments.size(), // attachment
        ...
        {
            const AttRef colorAttachmentRef(DE_NULL,                          // pNext
                                            (uint32_t)attachments.size() - 1, // attachment
                                            VK_IMAGE_LAYOUT_GENERAL,          // layout
                                            0u                                // aspectMask
            );
...

    {
        ...
        const SubpassDep subpassDependency(DE_NULL,                                       // pNext
                                           0u,                                            // srcSubpass
                                           1u,                                            // dstSubpass
...
Move<VkRenderPass> MaxAttachmenstsRenderPassTestInstance::createRenderPassSwitch(const RenderPassType renderPassType)
{
    switch (renderPassType)
    {
    case RENDERPASS_TYPE_LEGACY:
...
Move<VkPipeline> MaxAttachmenstsRenderPassTestInstance::createRenderPipeline(bool secondSubpass)
{
    ...
    const Unique<VkShaderModule> vertexShaderModule(
        createShaderModule(vkd, device, binaryCollection.get("quad-vert"), 0u));
    const Unique<VkShaderModule> fragmentShaderModule(
        createShaderModule(vkd, device, binaryCollection.get(fragShaderName), 0u));
    const Move<VkShaderModule> geometryShaderModule(
        m_layerCount == 1 ? Move<VkShaderModule>() : createShaderModule(vkd, device, binaryCollection.get("geom"), 0u));
...
void initTests(tcu::TestCaseGroup *group, RenderPassType renderPassType)
{
    ...
    for (size_t layerCountNdx = 0; layerCountNdx < DE_LENGTH_OF_ARRAY(layerCounts); layerCountNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> layerGroup(
            new tcu::TestCaseGroup(testCtx, layerGroupName.c_str(), layerGroupName.c_str()));
        ...
        for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
        {
            ...
            for (size_t sampleCountNdx = 0; sampleCountNdx < DE_LENGTH_OF_ARRAY(sampleCounts); sampleCountNdx++)
            {
                ...
                TestConfig testConfig = {RESOLVE, format, sampleCount, layerCount, 4u, 32u, 32u, renderPassType};
                ...
                if (layerCount == 1)
                {
                    for (uint32_t power = 2; power < 5; ++power)
                    {
                        ...
                        formatGroup->addChild(
                            new InstanceFactory1<MaxAttachmenstsRenderPassTestInstance, TestConfig, Programs>(
                                testCtx, tcu::NODETYPE_SELF_VALIDATE, maxAttName.c_str(), maxAttName.c_str(),
                                testConfig));
                    }
                }
            }
