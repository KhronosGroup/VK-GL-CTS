...
RenderPassWrapper<RenderingTypeValue>::RenderPassWrapper(const DeviceInterface &vk, const VkDevice vkDevice,
    ...
    , m_subpassBeginInfo(DE_NULL, testParams.groupParams->useSecondaryCmdBuffer ?
...
Move<VkPipeline> buildGraphicsPipeline(const DeviceInterface &vk, const VkDevice device,
                                       ...
                                       const VkViewport &viewport, const VkRect2D &scissor, const uint32_t subpass,
    ...
    , m_testParams(testParams)
{
    ...
    // Create pipelines
    {
        ...
        const VkViewport viewportsProduceDynamicDensityMap =
        ...
        std::vector<vk::VkPipelineRenderingCreateInfoKHR> renderingCreateInfo(
            3, {vk::VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR, DE_NULL, m_viewMask, 1u,
                &m_testParams.densityMapFormat, vk::VK_FORMAT_UNDEFINED, vk::VK_FORMAT_UNDEFINED});
    ...
    if (isDynamicRendering)
        ...
        createCommandBufferForRenderpass(renderPassWrapper, colorImageSize, dynamicDensityMapRenderArea,
                                         colorImageRenderArea, outputRenderArea);
}
...
void FragmentDensityMapTestInstance::drawResampleSubsampledImage(VkCommandBuffer cmdBuffer)
{
    ...
    vk.cmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayoutOperateOnSubsampledImage, 0,
                             1, &m_descriptorSetOperateOnSubsampledImage.get(), 0, DE_NULL);
    vk.cmdBindVertexBuffers(cmdBuffer, 0, 1, &m_vertexBuffer.get(), &vertexBufferOffset);
    vk.cmdDraw(cmdBuffer, (uint32_t)m_vertices.size(), 1, 0, 0);
}
...
void FragmentDensityMapTestInstance::drawCopySubsampledImage(VkCommandBuffer cmdBuffer)
{
    ...
    vk.cmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayoutOutputSubsampledImage, 0, 1,
                             &m_descriptorSetOutputSubsampledImage.get(), 0, DE_NULL);
                                                                      ...
                                                                      const VkRect2D &outputRenderArea)
{
    ...
    if (m_testParams.groupParams->useSecondaryCmdBuffer)
    {
        ...
        if (m_testParams.makeCopy)
        {
            ...
            vk.cmdBindPipeline(*m_subsampledImageSecCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                               *m_graphicsPipelineCopySubsampledImage);
            vk.cmdBindDescriptorSets(*m_subsampledImageSecCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
            ...
            vk.cmdDraw(*m_subsampledImageSecCmdBuffer, (uint32_t)m_vertices.size(), 1, 0, 0);
        }
        ...
        m_copySubsampledImageSecCmdBuffer =
            allocateCommandBuffer(vk, vkDevice, *m_cmdPool, VK_COMMAND_BUFFER_LEVEL_SECONDARY);
                                                                            ...
                                                                            const VkDevice &vkDevice)
{
    ...
    const VkImageMemoryBarrier dynamicDensitMapBarrier =
        makeImageMemoryBarrier(VK_ACCESS_FRAGMENT_DENSITY_MAP_READ_BIT_EXT,      // VkAccessFlags srcAccessMask;
    ...
    const VkImageMemoryBarrier densityMapImageBarrier =
        makeImageMemoryBarrier(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,             // VkAccessFlags srcAccessMask;
                               VK_ACCESS_FRAGMENT_DENSITY_MAP_READ_BIT_EXT,      // VkAccessFlags dstAccessMask;
    ...
    std::vector<VkImageMemoryBarrier> cbImageBarrier(
        2, makeImageMemoryBarrier(VK_ACCESS_NONE_KHR,                       // VkAccessFlags srcAccessMask;
                                  VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,     // VkAccessFlags dstAccessMask;
                                  VK_IMAGE_LAYOUT_UNDEFINED,                // VkImageLayout oldLayout;
                                  VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // VkImageLayout newLayout;
    ...
    const VkImageMemoryBarrier subsampledImageBarrier =
        makeImageMemoryBarrier(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,     // VkAccessFlags srcAccessMask;
                               VK_ACCESS_SHADER_READ_BIT,                // VkAccessFlags dstAccessMask;
                               VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // VkImageLayout oldLayout;
    ...
    const VkImageMemoryBarrier outputImageBarrier =
        makeImageMemoryBarrier(VK_ACCESS_NONE_KHR,                       // VkAccessFlags srcAccessMask;
                               VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,     // VkAccessFlags dstAccessMask;
                               VK_IMAGE_LAYOUT_UNDEFINED,                // VkImageLayout oldLayout;
                               VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, // VkImageLayout newLayout;
    ...
    if (m_testParams.groupParams->useSecondaryCmdBuffer)
    {
        ...
        if (m_testParams.dynamicDensityMap)
        {
            ...
            vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_NONE_KHR,
                                  VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0, 0, DE_NULL, 0, DE_NULL, 1,
            ...
            vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                  VK_PIPELINE_STAGE_FRAGMENT_DENSITY_PROCESS_BIT_EXT, 0, 0, DE_NULL, 0, DE_NULL, 1,
        ...
        vk.cmdPipelineBarrier(*m_cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                              VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, DE_NULL, 0, DE_NULL, 1u,
...
static void createChildren(tcu::TestCaseGroup *fdmTests, const SharedGroupParams groupParams)
{
    ...
    if (groupParams->renderingType != RENDERING_TYPE_DYNAMIC_RENDERING)
    {
        ...
        for (const auto &cmdBuffType : commandBufferType)
        {
            ...
            params.deferredDensityMap = false;
            params.dynamicDensityMap  = false;
            propertiesGroup->addChild(new FragmentDensityMapTest(
                testCtx, namePrefix + "imageless_framebuffer_static_subsampled", "", params));
            params.deferredDensityMap = true;
            propertiesGroup->addChild(new FragmentDensityMapTest(
                testCtx, namePrefix + "imageless_framebuffer_deferred_subsampled", "", params));
            params.deferredDensityMap = false;
            params.dynamicDensityMap  = true;
            propertiesGroup->addChild(new FragmentDensityMapTest(
                testCtx, namePrefix + "imageless_framebuffer_dynamic_subsampled", "", params));
        }
    }
...
tcu::TestCaseGroup *createFragmentDensityMapTests(tcu::TestContext &testCtx, const SharedGroupParams groupParams)
{
    return createTestGroup(testCtx, "fragment_density_map",
                           ...
                           createChildren, groupParams, cleanupGroup);
}
