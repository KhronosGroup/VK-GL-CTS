...
Move<VkRenderPass> createRenderPassProduceDynamicDensityMap(const DeviceInterface &vk, VkDevice vkDevice,
                                                            uint32_t viewMask, const TestParams &testParams)
{
    ...
    std::vector<uint32_t> viewMasks(subpassDescriptions.size(), viewMask);

    if (testParams.viewCount > 1)
    {
        renderPassMultiviewCreateInfo = {VK_STRUCTURE_TYPE_RENDER_PASS_MULTIVIEW_CREATE_INFO,
                                         renderPassInfoPNext,
...
Move<VkRenderPass> createRenderPassProduceSubsampledImage(const DeviceInterface &vk, VkDevice vkDevice,
                                                          uint32_t viewMask, bool makeCopySubpass,
                                                          bool resampleSubsampled, const TestParams &testParams)
{
    typedef AttachmentDescription2 AttachmentDesc;
    typedef AttachmentReference2 AttachmentRef;
    typedef SubpassDescription2 SubpassDesc;
    ...
    AttachmentRef resolveAttachmentRef{DE_NULL, multisampleAttachmentIndex, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
    ...
    if (testParams.viewCount > 1)
    {
        renderPassMultiviewCreateInfo = {
            ...
            renderPassInfoPNext,
            static_cast<uint32_t>(viewMasks.size()),   // uint32_t                subpassCount
            viewMasks.data(),                          // const uint32_t*        pViewMasks
            makeCopySubpass ? 1u : 0u,                 // uint32_t                dependencyCount
            makeCopySubpass ? &pViewOffsets : DE_NULL, // const int32_t*        pViewOffsets
...
{
    sourceCollections.glslSources.add("vert") << glu::VertexSource("#version 450\n"
...
#if DRY_RUN_WITHOUT_FDM_EXTENSION
    sourceCollections.glslSources.add("frag_produce_subsampled")
    ...
    sourceCollections.glslSources.add("frag_update_subsampled")
        << glu::FragmentSource("#version 450\n"
                               "#extension GL_EXT_multiview : enable\n"
                               "layout(location = 0) in vec4 inUV;\n"
                               "layout(location = 1) in vec4 inColor;\n"
    ...
    , m_testParams(testParams)
{
    ...
    prepareImageAndImageView(
        vk, vkDevice, memAlloc, colorImageCreateFlags, VK_FORMAT_R8G8B8A8_UNORM, colorImageSize, colorImageLayers,
    ...
    if (isColorImageMultisampled)
    {
        prepareImageAndImageView(
            vk, vkDevice, memAlloc, colorImageCreateFlags, VK_FORMAT_R8G8B8A8_UNORM, colorImageSize, colorImageLayers,
            VK_SAMPLE_COUNT_1_BIT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, queueFamilyIndex,
            0u, colorImageViewType, componentMappingRGBA, {VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, colorImageLayers},
    ...
    prepareImageAndImageView(
        vk, vkDevice, memAlloc, 0u, VK_FORMAT_R8G8B8A8_UNORM, outputImageSize, 1u, VK_SAMPLE_COUNT_1_BIT,
    ...
    for (uint32_t mapIndex = 0; mapIndex < densitiMapCount; ++mapIndex)
    {
        ...
        prepareImageAndImageView(vk, vkDevice, memAlloc, 0u, m_testParams.densityMapFormat, densityMapImageSize,
                                 ...
                                 {VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, densityMapImageLayers}, densityMapImage,
                                 densityMapImageAlloc, densityMapImageView);
    ...
    if (testParams.dynamicDensityMap)
        m_renderPassProduceDynamicDensityMap =
            createRenderPassProduceDynamicDensityMap(vk, vkDevice, m_viewMask, testParams);
    m_renderPassProduceSubsampledImage =
        createRenderPassProduceSubsampledImage(vk, vkDevice, m_viewMask, testParams.makeCopy, false, testParams);
    if (testParams.subsampledLoads)
        m_renderPassUpdateSubsampledImage =
            createRenderPassProduceSubsampledImage(vk, vkDevice, m_viewMask, false, true, testParams);
    ...
    if (testParams.dynamicDensityMap)
    {
        m_framebufferProduceDynamicDensityMap = createFrameBuffer(vk, vkDevice, *m_renderPassProduceDynamicDensityMap,
                                                                  densityMapImageSize, {**m_densityMapImageViews[0]});
    }
    ...
    if (testParams.subsampledLoads)
    {
        m_framebufferUpdateSubsampledImage = createFrameBuffer(vk, vkDevice, *m_renderPassUpdateSubsampledImage,
                                                               colorImageSize, imageViewsUpdateSubsampledImage);
    }
    m_framebufferOutputSubsampledImage =
        createFrameBuffer(vk, vkDevice, *m_renderPassOutputSubsampledImage, outputImageSize, {*m_outputImageView});
    ...
    // Create pipelines
    {
        ...
        const std::vector<VkViewport> viewportsSubsampledImage{
        ...
        const std::vector<VkRect2D> scissorsOutputSubsampledImage{
            makeRect2D(outputImageSize.width, outputImageSize.height)};
        ...
        if (m_testParams.makeCopy)
            m_graphicsPipelineCopySubsampledImage = makeGraphicsPipeline(
                ...
                *m_renderPassProduceSubsampledImage, // const VkRenderPass                                renderPass
                ...
                &multisampleStateCreateInfo); // const VkPipelineMultisampleStateCreateInfo*        multisampleStateCreateInfo
        if (m_testParams.subsampledLoads)
            m_graphicsPipelineUpdateSubsampledImage = makeGraphicsPipeline(
                vk,                                        // const DeviceInterface&                            vk
                vkDevice,                                  // const VkDevice                                    device
                *m_pipelineLayoutOperateOnSubsampledImage, // const VkPipelineLayout                            pipelineLayout
                *m_vertexCommonShaderModule, // const VkShaderModule                                vertexShaderModule
    ...
    const uint32_t attachmentCount          = 1 + testParams.makeCopy + isColorImageMultisampled;
    ...
    if (testParams.makeCopy)
    {
        ...
        vk.cmdBindDescriptorSets(*m_cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                 *m_pipelineLayoutOperateOnSubsampledImage, 0, 1,
                                 &m_descriptorSetOperateOnSubsampledImage.get(), 0, DE_NULL);
        vk.cmdBindVertexBuffers(*m_cmdBuffer, 0, 1, &m_vertexBuffer.get(), &vertexBufferOffset);
        vk.cmdDraw(*m_cmdBuffer, (uint32_t)m_vertices.size(), 1, 0, 0);
    }
    RPS2::cmdEndRenderPass(vk, *m_cmdBuffer, &subpassEndInfo);
...
{
    return createTestGroup(testCtx, "fragment_density_map",
                           "VK_EXT_fragment_density_map and VK_EXT_fragment_density_map2 extensions tests",
                           createChildren, cleanupGroup);
}
