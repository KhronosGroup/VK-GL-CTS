                          ...
                          VkImageLayout initialLayout, TestSeparateUsage separateStencilUsage)
{
    VkImageUsageFlags depthUsage =
        (separateStencilUsage == TEST_DEPTH) ? usage : (VkImageUsageFlags)VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
    VkImageUsageFlags stencilUsage =
        (separateStencilUsage == TEST_STENCIL) ? usage : (VkImageUsageFlags)VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
...
Move<VkRenderPass> createRenderPass(const DeviceInterface &vkd, VkDevice device, VkFormat srcFormat, VkFormat dstFormat,
                                    uint32_t sampleCount, RenderPassType renderPassType,
                                    TestSeparateUsage separateStencilUsage)
{
...
Move<VkRenderPass> createRenderPass(const DeviceInterface &vkd, VkDevice device, VkFormat srcFormat, VkFormat dstFormat,
                                    uint32_t sampleCount, const RenderPassType renderPassType,
                                    const TestSeparateUsage separateStencilUsage)
{
    ...
    case RENDERPASS_TYPE_LEGACY:
        return createRenderPass<AttachmentDescription1, AttachmentReference1, SubpassDescription1, SubpassDependency1,
                                RenderPassCreateInfo1>(vkd, device, srcFormat, dstFormat, sampleCount, renderPassType,
                                                       separateStencilUsage);
    case RENDERPASS_TYPE_RENDERPASS2:
        return createRenderPass<AttachmentDescription2, AttachmentReference2, SubpassDescription2, SubpassDependency2,
                                RenderPassCreateInfo2>(vkd, device, srcFormat, dstFormat, sampleCount, renderPassType,
                                                       separateStencilUsage);
    default:
...
Move<VkPipeline> createRenderPipeline(const DeviceInterface &vkd, VkDevice device, VkFormat srcFormat,
                                      VkRenderPass renderPass, VkPipelineLayout pipelineLayout,
                                      const vk::BinaryCollection &binaryCollection, uint32_t width, uint32_t height,
...
Move<VkPipelineLayout> createSplitPipelineLayout(const DeviceInterface &vkd, VkDevice device,
...
Move<VkPipeline> createSplitPipeline(const DeviceInterface &vkd, VkDevice device, VkRenderPass renderPass,
                                     uint32_t subpassIndex, VkPipelineLayout pipelineLayout,
                                     const vk::BinaryCollection &binaryCollection, uint32_t width, uint32_t height,
                                     uint32_t sampleCount)
{
    const Unique<VkShaderModule> vertexShaderModule(
        createShaderModule(vkd, device, binaryCollection.get("quad-vert"), 0u));
    const Unique<VkShaderModule> fragmentShaderModule(
        createShaderModule(vkd, device, binaryCollection.get("quad-split-frag"), 0u));
    // Disable blending
    const VkPipelineColorBlendAttachmentState attachmentBlendState = {
        VK_FALSE,
        VK_BLEND_FACTOR_SRC_ALPHA,
    ...
    const std::vector<VkPipelineColorBlendAttachmentState> attachmentBlendStates(
        de::min((uint32_t)MAX_COLOR_ATTACHMENT_COUNT, sampleCount), attachmentBlendState);
    const VkPipelineVertexInputStateCreateInfo vertexInputState = {
        VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO,
        DE_NULL,
        (VkPipelineVertexInputStateCreateFlags)0u,
    ...
    return makeGraphicsPipeline(
        ...
        *fragmentShaderModule, // const VkShaderModule                          fragmentShaderModule
        ...
        scissors,              // const std::vector<VkRect2D>&                  scissors
        VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, // const VkPrimitiveTopology                     topology
        subpassIndex,                        // const uint32_t                                subpass
        0u,                                  // const uint32_t                                patchControlPoints
        &vertexInputState,                   // const VkPipelineVertexInputStateCreateInfo*   vertexInputStateCreateInfo
        DE_NULL,           // const VkPipelineRasterizationStateCreateInfo* rasterizationStateCreateInfo
        &multisampleState, // const VkPipelineMultisampleStateCreateInfo*   multisampleStateCreateInfo
...
vector<VkPipelineSp> createSplitPipelines(const DeviceInterface &vkd, VkDevice device, VkRenderPass renderPass,
                                          VkPipelineLayout pipelineLayout, const vk::BinaryCollection &binaryCollection,
                                          uint32_t width, uint32_t height, uint32_t sampleCount)
{
    ...
    for (size_t ndx = 0; ndx < pipelines.size(); ndx++)
        pipelines[ndx] = safeSharedPtr(
            new Unique<VkPipeline>(createSplitPipeline(vkd, device, renderPass, (uint32_t)(ndx + 1), pipelineLayout,
                                                       binaryCollection, width, height, sampleCount)));
...
Move<VkDescriptorSet> createSplitDescriptorSet(const DeviceInterface &vkd, VkDevice device, VkDescriptorPool pool,
                                               VkDescriptorSetLayout layout, VkImageView primaryImageView,
                                               VkImageView secondaryImageView)
{
...

    {
        const VkDescriptorImageInfo imageInfos[] = {
            {(VkSampler)0u, primaryImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},
...
struct TestConfig
{
    TestConfig(VkFormat format_, uint32_t sampleCount_, RenderPassType renderPassType_,
...
MultisampleRenderPassTestInstance::MultisampleRenderPassTestInstance(Context &context, TestConfig config)
    ...
    , m_renderPass(createRenderPass(context.getDeviceInterface(), context.getDevice(), m_srcFormat, m_dstFormat,
                                    m_sampleCount, config.renderPassType, m_separateStencilUsage))
    ...
    , m_renderPipeline(createRenderPipeline(context.getDeviceInterface(), context.getDevice(), m_srcFormat,
                                            *m_renderPass, *m_renderPipelineLayout, context.getBinaryCollection(),
                                            m_width, m_height, m_sampleCount))
    ...
    , m_splitPipelineLayout(
          createSplitPipelineLayout(context.getDeviceInterface(), context.getDevice(), *m_splitDescriptorSetLayout))
    , m_splitPipelines(createSplitPipelines(context.getDeviceInterface(), context.getDevice(), *m_renderPass,
                                            *m_splitPipelineLayout, context.getBinaryCollection(), m_width, m_height,
                                            m_sampleCount))
    , m_splitDescriptorPool(createSplitDescriptorPool(context.getDeviceInterface(), context.getDevice()))
    , m_splitDescriptorSet(createSplitDescriptorSet(context.getDeviceInterface(), context.getDevice(),
                                                    *m_splitDescriptorPool, *m_splitDescriptorSetLayout,
                                                    *m_srcPrimaryInputImageView, *m_srcSecondaryInputImageView))
...
tcu::TestStatus MultisampleRenderPassTestInstance::iterateInternal(void)
{
    ...
    for (uint32_t splitPipelineNdx = 0; splitPipelineNdx < m_splitPipelines.size(); splitPipelineNdx++)
    {
        ...
        vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_splitPipelineLayout, 0u, 1u,
                                  &*m_splitDescriptorSet, 0u, DE_NULL);
        vkd.cmdPushConstants(*commandBuffer, *m_splitPipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, 0u,
                             sizeof(splitPipelineNdx), &splitPipelineNdx);
        vkd.cmdDraw(*commandBuffer, 6u, 1u, 0u, 0u);
    }
    ...
    for (size_t dstNdx = 0; dstNdx < m_dstSinglesampleImages.size(); dstNdx++)
        copyImageToBuffer(vkd, *commandBuffer, **m_dstSinglesampleImages[dstNdx], **m_dstBuffers[dstNdx],
                          tcu::IVec2(m_width, m_height), VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
        ...
        for (uint32_t sampleNdx = 0; sampleNdx < m_sampleCount; sampleNdx++)
        {
            ...
            if (verifyDepth || verifyStencil)
            {
                ...
                }
                {
                    ...
                    if (!tcu::floatThresholdCompare(m_context.getTestContext().getLog(), name.c_str(), name.c_str(),
            ...
            else
            {
                ...
                case tcu::TEXTURECHANNELCLASS_UNSIGNED_INTEGER:
                {
                            ...
                            while (nextSrcBit < de::min(bitSize, 10u))
                            {
                                for (int compNdx = 0; compNdx < componentCount; compNdx++)
                                {
                                    ...
                                    color[compNdx] +=
                                        ...
                                        (((nextSrcBit % 2 == 0 ? x1 : y1) & (0x1u << (nextSrcBit / 2u))) == 0u ? 0u :
                                                                                                                 1u);
                    ...
                    if (!tcu::intThresholdCompare(m_context.getTestContext().getLog(), name.c_str(), name.c_str(),
                ...
                case tcu::TEXTURECHANNELCLASS_FLOATING_POINT:
                {
                    ...
                    if (channelClass == tcu::TEXTURECHANNELCLASS_FLOATING_POINT)
                    {
                        ...
                        const UVec4 threshold(
                            64u * (UVec4(1u)
                                   << (UVec4(23) - tcu::getTextureFormatMantissaBitDepth(format).cast<uint32_t>())));

                        if (!tcu::floatUlpThresholdCompare(m_context.getTestContext().getLog(), name.c_str(),
                                                           name.c_str(), reference.getAccess(), access, threshold,
                                                           tcu::COMPARE_LOG_ON_ERROR))
                            m_resultCollector.fail("Compare failed for sample " + de::toString(sampleNdx));
                    }
                    else
...
void initTests(tcu::TestCaseGroup *group, RenderPassType renderPassType)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> extGroup(
        new tcu::TestCaseGroup(testCtx, "separate_stencil_usage", "test VK_EXT_separate_stencil_usage"));
    ...
    for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
    {
        ...
        for (size_t sampleCountNdx = 0; sampleCountNdx < DE_LENGTH_OF_ARRAY(sampleCounts); sampleCountNdx++)
        {
            ...
            formatGroup->addChild(new InstanceFactory1<MultisampleRenderPassTestInstance, TestConfig, Programs>(
                ...
                tcu::hasStencilComponent(mapVkFormat(format).order))
            {
                de::MovePtr<tcu::TestCaseGroup> sampleGroup(
                    new tcu::TestCaseGroup(testCtx, testName.c_str(), testName.c_str()));
                {
                    const TestConfig separateUsageDepthTestConfig(format, sampleCount, renderPassType, TEST_DEPTH);
                    sampleGroup->addChild(new InstanceFactory1<MultisampleRenderPassTestInstance, TestConfig, Programs>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, "test_depth", "depth with input attachment bit",
                        separateUsageDepthTestConfig));

                    const TestConfig separateUsageStencilTestConfig(format, sampleCount, renderPassType, TEST_STENCIL);
                    sampleGroup->addChild(new InstanceFactory1<MultisampleRenderPassTestInstance, TestConfig, Programs>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, "test_stencil", "stencil with input attachment bit",
                        separateUsageStencilTestConfig));
                }
