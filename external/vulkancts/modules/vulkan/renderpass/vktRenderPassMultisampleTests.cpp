...
MultisampleRenderPassTestInstance::MultisampleRenderPassTestInstance(Context &context, TestConfig config)
    ...
    , m_extensionSupported(
          isExtensionSupported(context, config.renderingType, config.separateStencilUsage, config.format))
                ...
                tcu::hasStencilComponent(mapVkFormat(format).order))
            {
                de::MovePtr<tcu::TestCaseGroup> sampleGroup(new tcu::TestCaseGroup(testCtx, testName.c_str()));
                {
                    ...
                    sampleGroup->addChild(new InstanceFactory1<MultisampleRenderPassTestInstance, TestConfig, Programs>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, "test_depth", separateUsageDepthTestConfig));

                    const TestConfig separateUsageStencilTestConfig(format, sampleCount, renderingType, TEST_STENCIL);
                    sampleGroup->addChild(new InstanceFactory1<MultisampleRenderPassTestInstance, TestConfig, Programs>(
                        testCtx, tcu::NODETYPE_SELF_VALIDATE, "test_stencil", separateUsageStencilTestConfig));
                }
