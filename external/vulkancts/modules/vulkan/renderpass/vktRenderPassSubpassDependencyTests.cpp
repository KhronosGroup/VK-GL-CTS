...
struct ExternalTestConfig
{
    ExternalTestConfig(VkFormat format_, UVec2 imageSize_, vector<RenderPass> renderPasses_,
                       RenderingType renderingType_, SynchronizationType synchronizationType_, uint32_t blurKernel_ = 4)
        : format(format_)
...
ExternalDependencyTestInstance::ExternalDependencyTestInstance(Context &context, ExternalTestConfig testConfig)
    ...
    , m_renderPasses(createRenderPasses(context.getDeviceInterface(), context.getDevice(), testConfig.renderPasses,
                                        testConfig.renderingType, testConfig.synchronizationType))
...
{
    SubpassTestConfig(VkFormat format_, UVec2 imageSize_, RenderPass renderPass_, RenderingType renderingType_)
        : format(format_)
        , imageSize(imageSize_)
...
{
    SeparateChannelsTestConfig(VkFormat format_, RenderingType renderingType_)
        : format(format_)
        , renderingType(renderingType_)
    {
    }
...
tcu::TestStatus SeparateChannelsTestInstance::iterateInternal(void)
{
    ...
    // Create render pass.
    {
        ...
        const VkImageAspectFlags inputAttachmentAspectMask(
            (m_renderingType == RENDERING_TYPE_RENDERPASS2) ?
        ...
        renderPass =
            createRenderPass(vkd, device, RenderPass(attachments, subpasses, subpassDependency), m_renderingType);
    }
...
{
    SingleAttachmentTestConfig(VkFormat format_, RenderingType renderingType_)
        : format(format_)
        , renderingType(renderingType_)
    {
    }
...
void initTests(tcu::TestCaseGroup *group, const RenderingType renderingType)
{
    ...
    // Test implicit subpass dependencies
    {
             ...
             renderPassCountNdx++)
        {
            ...
            const uint32_t blurKernel(12u);
            const ExternalTestConfig testConfig(VK_FORMAT_R8G8B8A8_UNORM, UVec2(128, 128), renderPasses, renderingType,
    ...
    // Test subpass self dependency
    {
        ...
        for (size_t renderSizeNdx = 0; renderSizeNdx < DE_LENGTH_OF_ARRAY(renderSizes); renderSizeNdx++)
        {
            ...
            const SubpassSelfDependencyBackwardsTestConfig testConfig(VK_FORMAT_R8G8B8A8_UNORM,
                                                                      renderSizes[renderSizeNdx], renderingType);
            renderSizeGroup->addChild(
