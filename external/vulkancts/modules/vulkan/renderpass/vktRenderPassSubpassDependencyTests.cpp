...
bool verifyDepth(Context &context, const ConstPixelBufferAccess &reference, const ConstPixelBufferAccess &result,
...
bool verifyStencil(Context &context, const ConstPixelBufferAccess &reference, const ConstPixelBufferAccess &result)
{
    tcu::TextureLevel stencilErrorImage(tcu::TextureFormat(tcu::TextureFormat::RGBA, tcu::TextureFormat::UNORM_INT8),
...
void beginSecondaryCmdBuffer(const DeviceInterface &vkd, VkCommandBuffer secCmdBuffer, VkFormat colorAttachmentFormat,
                             VkFormat dsAttachmentFormat, bool secondaryCmdBufferCompletelyContainsDynamicRenderpass)
{
...
{
public:
    DepthVertShader(void) : rr::VertexShader(1, 1)
    {
...
{
public:
    DepthFragShader(void) : rr::FragmentShader(1, 1)
    {
        m_inputs[0].type  = rr::GENERICVECTYPE_FLOAT;
        m_outputs[0].type = rr::GENERICVECTYPE_FLOAT;
    }
...
{
public:
    SelfDependencyBackwardsVertShader(void) : rr::VertexShader(1, 0)
    {
        m_inputs[0].type = rr::GENERICVECTYPE_FLOAT;
    }
...
{
public:
    SelfDependencyBackwardsFragShader(void) : rr::FragmentShader(0, 1)
    {
        m_outputs[0].type = rr::GENERICVECTYPE_FLOAT;
    }
...
vector<SharedPtrVkImageView> createImageViews(const DeviceInterface &vkd, VkDevice device,
                                              vector<SharedPtrVkImage> images, VkFormat format,
                                              VkImageAspectFlags aspect)
{
    ...
    for (size_t imageViewNdx = 0; imageViewNdx < images.size(); imageViewNdx++)
    {
        ...
        imageViews.push_back(
            makeSharedPtr(createImageView(vkd, device, 0u, **images[imageViewNdx], VK_IMAGE_VIEW_TYPE_2D, format,
                                          makeComponentMappingRGBA(), range)));
    }
...
vector<SharedPtrVkDescriptorLayout> createDescriptorSetLayouts(const DeviceInterface &vkd, VkDevice device,
...
vector<SharedPtrVkDescriptorPool> createDescriptorPools(const DeviceInterface &vkd, VkDevice device,
                                                        vector<SharedPtrVkDescriptorLayout> &layouts,
...
{
    ExternalTestConfig(VkFormat format_, UVec2 imageSize_, vector<RenderPass> renderPasses_,
                       const SharedGroupParams groupParams_, SynchronizationType synchronizationType_,
                       uint32_t blurKernel_ = 4)
        : format(format_)
...
public:
    ...
    vector<SharedPtrVkImage> createAndAllocateImages(const DeviceInterface &vk, VkDevice device, Allocator &allocator,
                                                     ...
                                                     uint32_t universalQueueFamilyIndex, VkFormat format,
                                                     uint32_t width, uint32_t height, vector<RenderPass> renderPasses);

    vector<SharedPtrVkSampler> createSamplers(const DeviceInterface &vkd, const VkDevice device,
                                              vector<RenderPass> &renderPasses);

    vector<SharedPtrVkRenderPass> createRenderPasses(const DeviceInterface &vkd, VkDevice device,
                                                     vector<RenderPass> renderPassInfos,
                                                     const RenderingType renderingType,
                                                     const SynchronizationType synchronizationType);

    vector<SharedPtrVkFramebuffer> createFramebuffers(const DeviceInterface &vkd, VkDevice device,
                                                      vector<SharedPtrVkRenderPass> &renderPasses,
                                                      vector<SharedPtrVkImageView> &dstImageViews, uint32_t width,
                                                      uint32_t height);

    vector<SharedPtrVkPipelineLayout> createRenderPipelineLayouts(
        const DeviceInterface &vkd, VkDevice device, vector<SharedPtrVkRenderPass> &renderPasses,
        vector<SharedPtrVkDescriptorLayout> &descriptorSetLayouts);

    vector<SharedPtrVkPipeline> createRenderPipelines(const DeviceInterface &vkd, VkDevice device,
                                                      vector<SharedPtrVkRenderPass> &renderPasses,
                                                      vector<SharedPtrVkPipelineLayout> &pipelineLayouts,
                                                      const BinaryCollection &binaryCollection, uint32_t width,
                                                      uint32_t height);

    vector<SharedPtrVkDescriptorSet> createDescriptorSets(const DeviceInterface &vkd, VkDevice device,
...
ExternalDependencyTestInstance::ExternalDependencyTestInstance(Context &context, ExternalTestConfig testConfig)
    ...
    , m_images(createAndAllocateImages(context.getDeviceInterface(), context.getDevice(), context.getDefaultAllocator(),
                                       m_imageMemories, context.getUniversalQueueFamilyIndex(), m_format, m_width,
                                       m_height, testConfig.renderPasses))
    , m_imageViews(createImageViews(context.getDeviceInterface(), context.getDevice(), m_images, m_format,
    ...
    , m_renderPasses(createRenderPasses(context.getDeviceInterface(), context.getDevice(), testConfig.renderPasses,
                                        testConfig.groupParams->renderingType, testConfig.synchronizationType))
    , m_framebuffers(createFramebuffers(context.getDeviceInterface(), context.getDevice(), m_renderPasses, m_imageViews,
                                        m_width, m_height))
    , m_subpassDescriptorSetLayouts(
          createDescriptorSetLayouts(context.getDeviceInterface(), context.getDevice(), m_samplers))
    , m_subpassDescriptorPools(createDescriptorPools(context.getDeviceInterface(), context.getDevice(),
                                                     m_subpassDescriptorSetLayouts,
                                                     VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER))
    , m_subpassDescriptorSets(createDescriptorSets(context.getDeviceInterface(), context.getDevice(),
                                                   m_subpassDescriptorPools, m_subpassDescriptorSetLayouts,
                                                   m_imageViews, m_samplers))
    , m_renderPipelineLayouts(createRenderPipelineLayouts(context.getDeviceInterface(), context.getDevice(),
                                                          m_renderPasses, m_subpassDescriptorSetLayouts))
    , m_renderPipelines(createRenderPipelines(context.getDeviceInterface(), context.getDevice(), m_renderPasses,
                                              m_renderPipelineLayouts, context.getBinaryCollection(), m_width,
                                              m_height))
...
vector<SharedPtrVkImage> ExternalDependencyTestInstance::createAndAllocateImages(
    const DeviceInterface &vk, VkDevice device, Allocator &allocator, vector<de::SharedPtr<Allocation>> &imageMemories,
    uint32_t universalQueueFamilyIndex, VkFormat format, uint32_t width, uint32_t height,
    vector<RenderPass> renderPasses)
{
    ...
    for (size_t imageNdx = 0; imageNdx < renderPasses.size(); imageNdx++)
    {
        ...
        images.push_back(makeSharedPtr(vk::createImage(vk, device, &imageCreateInfo, nullptr)));
        imageMemories.push_back(
            (de::SharedPtr<Allocation>)allocator
                .allocate(getImageMemoryRequirements(vk, device, **images[imageNdx]), MemoryRequirement::Any)
                .release());
        VK_CHECK(vk.bindImageMemory(device, **images[imageNdx], imageMemories[imageNdx]->getMemory(),
                                    imageMemories[imageNdx]->getOffset()));
    }
...
vector<SharedPtrVkRenderPass> ExternalDependencyTestInstance::createRenderPasses(
    const DeviceInterface &vkd, VkDevice device, vector<RenderPass> renderPassInfos, const RenderingType renderingType,
    const SynchronizationType synchronizationType)
{
    vector<SharedPtrVkRenderPass> renderPasses;
    ...
    for (const auto &renderPassInfo : renderPassInfos)
        renderPasses.push_back(
            makeSharedPtr(createRenderPass(vkd, device, renderPassInfo, renderingType, synchronizationType)));

    return renderPasses;
}

vector<SharedPtrVkFramebuffer> ExternalDependencyTestInstance::createFramebuffers(
    const DeviceInterface &vkd, VkDevice device, vector<SharedPtrVkRenderPass> &renderPasses,
    vector<SharedPtrVkImageView> &dstImageViews, uint32_t width, uint32_t height)
{
...
vector<SharedPtrVkDescriptorSet> ExternalDependencyTestInstance::createDescriptorSets(
    const DeviceInterface &vkd, VkDevice device, vector<SharedPtrVkDescriptorPool> &pools,
    vector<SharedPtrVkDescriptorLayout> &layouts, vector<SharedPtrVkImageView> &imageViews,
    vector<SharedPtrVkSampler> &samplers)
{
...
vector<SharedPtrVkPipelineLayout> ExternalDependencyTestInstance::createRenderPipelineLayouts(
    const DeviceInterface &vkd, VkDevice device, vector<SharedPtrVkRenderPass> &renderPasses,
    vector<SharedPtrVkDescriptorLayout> &descriptorSetLayouts)
{
    vector<SharedPtrVkPipelineLayout> pipelineLayouts;
    ...
    for (size_t renderPassNdx = 0; renderPassNdx < renderPasses.size(); renderPassNdx++)
    {
        const VkPipelineLayoutCreateInfo createInfo = {
            ...
            renderPassNdx > 0 ? 1u : 0u,                   // uint32_t                        setLayoutCount
            renderPassNdx > 0 ? &**descriptorSetLayouts[renderPassNdx - 1] :
...
vector<SharedPtrVkPipeline> ExternalDependencyTestInstance::createRenderPipelines(
    const DeviceInterface &vkd, VkDevice device, vector<SharedPtrVkRenderPass> &renderPasses,
    vector<SharedPtrVkPipelineLayout> &pipelineLayouts, const BinaryCollection &binaryCollection, uint32_t width,
    uint32_t height)
{
    ...
    for (size_t renderPassNdx = 0; renderPassNdx < renderPasses.size(); renderPassNdx++)
    {
        const Unique<VkShaderModule> vertexShaderModule(
            createShaderModule(vkd, device, binaryCollection.get("quad-vert-" + de::toString(renderPassNdx)), 0u));
        const Unique<VkShaderModule> fragmentShaderModule(
            createShaderModule(vkd, device, binaryCollection.get("quad-frag-" + de::toString(renderPassNdx)), 0u));
...
tcu::TestStatus ExternalDependencyTestInstance::iterateInternal(void)
{
    ...
    for (size_t renderPassNdx = 0; renderPassNdx < m_renderPasses.size(); renderPassNdx++)
    {
        ...
        if (renderPassNdx > 0)
            vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                      **m_renderPipelineLayouts[renderPassNdx], 0, 1,
                                      &**m_subpassDescriptorSets[renderPassNdx - 1], 0, nullptr);
    ...
    // Memory barrier between rendering and copy
    {
        ...
        vkd.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u,
    ...
    // Copy image memory to buffer
    {
        ...
        vkd.cmdCopyImageToBuffer(*commandBuffer, **m_images[m_renderPasses.size() - 1],
                                 VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *m_dstBuffer, 1u, &region);
    }
    ...
    invalidateMappedMemoryRange(vkd, m_context.getDevice(), m_dstBufferMemory->getMemory(),
                                m_dstBufferMemory->getOffset(), VK_WHOLE_SIZE);

    {
        ...
        for (uint32_t renderPassNdx = 0; renderPassNdx < m_renderPasses.size(); renderPassNdx++)
        {
            ...
            else
            {
                for (uint32_t y = 0; y < m_height; y++)
                    for (uint32_t x = 0; x < m_width; x++)
                    {
                        ...
                        for (uint32_t sampleNdx = 0; sampleNdx < (m_blurKernel + 1); sampleNdx++)
                        {
                            if (renderPassNdx % 2 == 0)
                            {
                                ...
                                blurColor +=
                                    0.12f *
                                    textureB.getAccess().getPixel(
                                        deClamp32((int32_t)x - (m_blurKernel / 2) + sampleNdx, 0u, m_width - 1u), y);
                            }
                            else
                            {
                                // Do a vertical blur
                                blurColor += 0.12f * textureA.getAccess().getPixel(
                                                         x, deClamp32((int32_t)y - (m_blurKernel / 2) + sampleNdx, 0u,
                                                                      m_height - 1u));
                            }
                        }

                        renderPassNdx % 2 == 0 ? textureA.getAccess().setPixel(blurColor, x, y) :
                                                 textureB.getAccess().setPixel(blurColor, x, y);
                    }
            }
        }
...
{
    SubpassTestConfig(VkFormat format_, UVec2 imageSize_, RenderPass renderPass_, SharedGroupParams groupParams_)
        : format(format_)
        , imageSize(imageSize_)
...
{
public:
    SubpassDependencyTestInstance(Context &context, SubpassTestConfig testConfig);

    ~SubpassDependencyTestInstance(void) = default;

    vector<SharedPtrVkImage> createAndAllocateImages(const DeviceInterface &vk, VkDevice device, Allocator &allocator,
                                                     vector<de::SharedPtr<Allocation>> &imageMemories,
                                                     uint32_t universalQueueFamilyIndex, RenderPass renderPassInfo,
                                                     VkFormat format, uint32_t width, uint32_t height);

    vector<SharedPtrVkPipelineLayout> createRenderPipelineLayouts(
        const DeviceInterface &vkd, VkDevice device, RenderPass renderPassInfo,
        vector<SharedPtrVkDescriptorLayout> descriptorSetLayouts);

    vector<SharedPtrVkPipeline> createRenderPipelines(const DeviceInterface &vkd, VkDevice device,
                                                      RenderPass renderPassInfo, VkRenderPass renderPass,
                                                      vector<SharedPtrVkPipelineLayout> &pipelineLayouts,
                                                      const BinaryCollection &binaryCollection, VkFormat format,
                                                      uint32_t width, uint32_t height);

    Move<VkFramebuffer> createFramebuffer(const DeviceInterface &vkd, VkDevice device, RenderPass renderPassInfo,
                                          VkRenderPass renderPass, vector<SharedPtrVkImageView> &dstImageViews,
                                          uint32_t width, uint32_t height);

    vector<SharedPtrVkDescriptorLayout> createDescriptorSetLayouts(const DeviceInterface &vkd, VkDevice device,
                                                                   RenderPass renderPassInfo);

    vector<SharedPtrVkDescriptorSet> createDescriptorSets(const DeviceInterface &vkd, VkDevice device, VkFormat format,
                                                          vector<SharedPtrVkDescriptorPool> &pools,
                                                          vector<SharedPtrVkDescriptorLayout> &layouts,
                                                          vector<SharedPtrVkImageView> &imageViews);
...
SubpassDependencyTestInstance::SubpassDependencyTestInstance(Context &context, SubpassTestConfig testConfig)
    ...
    , m_format(testConfig.format)
    , m_images(createAndAllocateImages(context.getDeviceInterface(), context.getDevice(), context.getDefaultAllocator(),
                                       m_imageMemories, context.getUniversalQueueFamilyIndex(), m_renderPassInfo,
                                       m_format, m_width, m_height))
    , m_imageViews(
          createImageViews(context.getDeviceInterface(), context.getDevice(), m_images, m_format,
                           isDepthStencilFormat(m_format) ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT))
    ...
    , m_primaryBufferMemory(createBufferMemory(context.getDeviceInterface(), context.getDevice(),
                                               context.getDefaultAllocator(), *m_primaryBuffer))
    , m_secondaryBufferMemory(createBufferMemory(context.getDeviceInterface(), context.getDevice(),
                                                 context.getDefaultAllocator(), *m_secondaryBuffer))
    , m_renderPass(createRenderPass(context.getDeviceInterface(), context.getDevice(), m_renderPassInfo,
                                    m_groupParams->renderingType))
    , m_framebuffer(createFramebuffer(context.getDeviceInterface(), context.getDevice(), m_renderPassInfo,
                                      *m_renderPass, m_imageViews, m_width, m_height))
    , m_subpassDescriptorSetLayouts(
          createDescriptorSetLayouts(context.getDeviceInterface(), context.getDevice(), m_renderPassInfo))
    , m_subpassDescriptorPools(createDescriptorPools(context.getDeviceInterface(), context.getDevice(),
                                                     m_subpassDescriptorSetLayouts,
                                                     VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT))
    , m_subpassDescriptorSets(createDescriptorSets(context.getDeviceInterface(), context.getDevice(), m_format,
                                                   m_subpassDescriptorPools, m_subpassDescriptorSetLayouts,
                                                   m_imageViews))
    , m_renderPipelineLayouts(createRenderPipelineLayouts(context.getDeviceInterface(), context.getDevice(),
                                                          m_renderPassInfo, m_subpassDescriptorSetLayouts))
    , m_renderPipelines(createRenderPipelines(context.getDeviceInterface(), context.getDevice(), m_renderPassInfo,
                                              *m_renderPass, m_renderPipelineLayouts, context.getBinaryCollection(),
                                              m_format, m_width, m_height))
    , m_commandPool(createCommandPool(context.getDeviceInterface(), context.getDevice(),
                                      VK_COMMAND_POOL_CREATE_TRANSIENT_BIT, context.getUniversalQueueFamilyIndex()))
{
...
vector<SharedPtrVkImage> SubpassDependencyTestInstance::createAndAllocateImages(
    const DeviceInterface &vk, VkDevice device, Allocator &allocator, vector<de::SharedPtr<Allocation>> &imageMemories,
    uint32_t universalQueueFamilyIndex, RenderPass renderPassInfo, VkFormat format, uint32_t width, uint32_t height)
{
    ...
    for (size_t imageNdx = 0; imageNdx < renderPassInfo.getAttachments().size(); imageNdx++)
    {
        ...
        VkImageUsageFlags usage = ((isDepthStencilFormat(format) ? VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT :
...
vector<SharedPtrVkPipelineLayout> SubpassDependencyTestInstance::createRenderPipelineLayouts(
    const DeviceInterface &vkd, VkDevice device, RenderPass renderPassInfo,
    vector<SharedPtrVkDescriptorLayout> descriptorSetLayouts)
{
    vector<SharedPtrVkPipelineLayout> pipelineLayouts;
    ...
    for (size_t subpassNdx = 0; subpassNdx < renderPassInfo.getSubpasses().size(); subpassNdx++)
    {
        const VkPipelineLayoutCreateInfo createInfo = {
            ...
            nullptr,                                       // const void*                    pNext
            (vk::VkPipelineLayoutCreateFlags)0,            // VkPipelineLayoutCreateFlags    flags
            (uint32_t)descriptorSetLayoutCount,            // uint32_t                        setLayoutCount
...
vector<SharedPtrVkPipeline> SubpassDependencyTestInstance::createRenderPipelines(
    const DeviceInterface &vkd, VkDevice device, RenderPass renderPassInfo, VkRenderPass renderPass,
    vector<SharedPtrVkPipelineLayout> &pipelineLayouts, const BinaryCollection &binaryCollection, VkFormat format,
    uint32_t width, uint32_t height)
{
    vector<SharedPtrVkPipeline> pipelines;
    ...
    {
        const Unique<VkShaderModule> vertexShaderModule(
            createShaderModule(vkd, device, binaryCollection.get("subpass-vert-" + de::toString(subpassNdx)), 0u));
        const Unique<VkShaderModule> fragmentShaderModule(
            createShaderModule(vkd, device, binaryCollection.get("subpass-frag-" + de::toString(subpassNdx)), 0u));
        ...
        const VkPipelineDepthStencilStateCreateInfo depthStencilCreateInfo(
            isDepthStencilFormat(format) ? depthStencilStateCreateInfo : VkPipelineDepthStencilStateCreateInfo());
        ...
        pipelines.push_back(makeSharedPtr(makeGraphicsPipeline(
            ...
            *fragmentShaderModule, // const VkShaderModule                                fragmentShaderModule
            ...
            scissors,              // const std::vector<VkRect2D>&                        scissors
            VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST, // const VkPrimitiveTopology                        topology
            (uint32_t)subpassNdx,                // const uint32_t                                    subpass
            0u,                                  // const uint32_t                                    patchControlPoints
            &vertexInputState,       // const VkPipelineVertexInputStateCreateInfo*        vertexInputStateCreateInfo
...
Move<VkFramebuffer> SubpassDependencyTestInstance::createFramebuffer(const DeviceInterface &vkd, VkDevice device,
                                                                     RenderPass renderPassInfo, VkRenderPass renderPass,
...
vector<SharedPtrVkDescriptorLayout> SubpassDependencyTestInstance::createDescriptorSetLayouts(
    const DeviceInterface &vkd, VkDevice device, RenderPass renderPassInfo)
{
    vector<SharedPtrVkDescriptorLayout> layouts;
...
vector<SharedPtrVkDescriptorSet> SubpassDependencyTestInstance::createDescriptorSets(
    const DeviceInterface &vkd, VkDevice device, VkFormat format, vector<SharedPtrVkDescriptorPool> &pools,
    vector<SharedPtrVkDescriptorLayout> &layouts, vector<SharedPtrVkImageView> &imageViews)
{
    vector<SharedPtrVkDescriptorSet> descriptorSets;
    ...
    for (size_t setNdx = 0; setNdx < layouts.size(); setNdx++)
    {
        ...
        {
            VkImageLayout imageLayout = isDepthStencilFormat(format) ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL :
...
tcu::TestStatus SubpassDependencyTestInstance::iterateInternal(void)
{
    ...
    if (isDepthStencilFormat(m_format))
    {
        ...
        for (uint32_t attachmentNdx = 0; attachmentNdx < attachmentCount; ++attachmentNdx)
        {
            ...
            vkd.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    // Begin render pass
    {
        ...
        for (size_t attachmentNdx = 0; attachmentNdx < attachmentCount; attachmentNdx++)
            clearValues.push_back(isDepthStencilFormat(m_format) ? makeClearValueDepthStencil(1.0f, 255u) :
                                                                   makeClearValueColor(Vec4(1.0f, 0.0f, 0.0f, 1.0f)));
    ...
    const de::UniquePtr<Allocation> vertexBufferMemory(m_context.getDefaultAllocator().allocate(
        getBufferMemoryRequirements(vkd, m_context.getDevice(), *vertexBuffer), MemoryRequirement::HostVisible));

    VK_CHECK(vkd.bindBufferMemory(m_context.getDevice(), *vertexBuffer, vertexBufferMemory->getMemory(),
    ...
    for (size_t subpassNdx = 0; subpassNdx < subpassCount; subpassNdx++)
    {
        if (subpassNdx > 0)
        {
            ...
            vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                                      **m_renderPipelineLayouts[subpassNdx], 0, 1,
                                      &**m_subpassDescriptorSets[subpassNdx - 1], 0, nullptr);
        }
        ...
        if (subpassNdx == 0)
        {
            ...
            vkd.cmdDraw(*commandBuffer, (uint32_t)vertexData.size(), 1u, 0u, 0u);
        }
        else
            vkd.cmdDraw(*commandBuffer, 6u, 1u, 0u, 0u);
    }
    ...
    // Memory barrier between rendering and copy
    {
        ...
        vkd.cmdPipelineBarrier(*commandBuffer, srcStageMask, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 0u, nullptr, 0u,
                               nullptr, 1u, &barrier);
    }
    ...
    // Copy image memory to buffer
    {
        if (isDepthStencilFormat(m_format))
        {
            ...
            vkd.cmdCopyImageToBuffer(*commandBuffer, **m_images[attachmentCount - 1],
                                     VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *m_primaryBuffer, 1u, &regionDepth);
            ...
            vkd.cmdCopyImageToBuffer(*commandBuffer, **m_images[attachmentCount - 1],
                                     VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *m_secondaryBuffer, 1u, &regionStencil);
        }
        else
        {
            // Copy color
            ...
            vkd.cmdCopyImageToBuffer(*commandBuffer, **m_images[attachmentCount - 1],
                                     VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, *m_primaryBuffer, 1u, &region);
        }
    }
    ...
    // Memory barrier between copy and host access
    {
        ...
        if (isDepthStencilFormat(m_format))
        {
            ...
            vkd.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 0u,
                                   nullptr, 1u, &stencilBarrier, 0u, nullptr);
        }
    }

    endCommandBuffer(vkd, *commandBuffer);
    submitCommandsAndWait(vkd, m_context.getDevice(), m_context.getUniversalQueue(), *commandBuffer);
    invalidateMappedMemoryRange(vkd, m_context.getDevice(), m_primaryBufferMemory->getMemory(),
                                m_primaryBufferMemory->getOffset(), VK_WHOLE_SIZE);
    invalidateMappedMemoryRange(vkd, m_context.getDevice(), m_secondaryBufferMemory->getMemory(),
                                m_secondaryBufferMemory->getOffset(), VK_WHOLE_SIZE);
    ...
    // Verify result
    {
        ...
        if (isDepthStencilFormat(m_format))
        {
            ...
            const tcu::ConstPixelBufferAccess resultDepthAccess(getDepthCopyFormat(m_format), m_width, m_height, 1,
                                                                ptrDepth);
            const tcu::ConstPixelBufferAccess resultStencilAccess(getStencilCopyFormat(m_format), m_width, m_height, 1,
                                                                  ptrStencil);
            const PixelBufferAccess referenceDepthAccess(
                tcu::getEffectiveDepthStencilAccess(reference.getAccess(), tcu::Sampler::MODE_DEPTH));
            const PixelBufferAccess referenceStencilAccess(
                tcu::getEffectiveDepthStencilAccess(reference.getAccess(), tcu::Sampler::MODE_STENCIL));
            ...
            // Setup and run reference renderer
            {
                ...
                const rr::MultisamplePixelBufferAccess depthBuffer(
                    rr::MultisamplePixelBufferAccess::fromSinglesampleAccess(referenceDepthAccess));
                const rr::MultisamplePixelBufferAccess colorBufferAccess(
                    rr::MultisamplePixelBufferAccess::fromSinglesampleAccess(colorBuffer.getAccess()));
                const rr::RenderTarget renderTarget(rr::MultisamplePixelBufferAccess(colorBufferAccess), depthBuffer,
                                                    rr::MultisamplePixelBufferAccess());
                const rr::PrimitiveType primitiveType(rr::PRIMITIVETYPE_TRIANGLES);
                const rr::PrimitiveList primitiveList(rr::PrimitiveList(primitiveType, (uint32_t)vertexData.size(), 0));
                rr::RenderState renderState((rr::ViewportState(depthBuffer)),
                                            m_context.getDeviceProperties().limits.subPixelPrecisionBits);

                const rr::VertexAttrib vertices =
                    rr::VertexAttrib(rr::VERTEXATTRIBTYPE_FLOAT, 4, sizeof(tcu::Vec4), 0, &vertexData[0]);
            ...
            const float depthThreshold(
                (float)subpassCount *
                (1.0f /
                 ((UVec4(1u) << tcu::getTextureFormatMantissaBitDepth(resultDepthAccess.getFormat()).cast<uint32_t>()) -
                  1u)
                     .cast<float>()
                     .x()));
...
{
    SubpassSelfDependencyBackwardsTestConfig(VkFormat format_, UVec2 imageSize_, RenderingType renderingType_)
        : format(format_)
        , imageSize(imageSize_)
...
tcu::TestStatus SubpassSelfDependencyBackwardsTestInstance::iterateInternal(void)
{
    ...
    const Unique<VkCommandPool> commandPool(
        createCommandPool(vkd, m_context.getDevice(), VK_COMMAND_POOL_CREATE_TRANSIENT_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> commandBuffer(
        allocateCommandBuffer(vkd, m_context.getDevice(), *commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY));
    const typename RenderpassSubpass::SubpassBeginInfo subpassBeginInfo(nullptr, VK_SUBPASS_CONTENTS_INLINE);
    const typename RenderpassSubpass::SubpassEndInfo subpassEndInfo(nullptr);
    vector<Vec4> vertexData;
    ...
    const VkDeviceSize indirectBufferSize = 4 * sizeof(uint32_t);
    ...
    // Create output image.
    {
        ...
        outputImage           = createImage(vkd, device, &imageCreateInfo, nullptr);
        outputImageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vkd, device, *outputImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(device, *outputImage, outputImageAllocation->getMemory(),
                                     outputImageAllocation->getOffset()));
    }
    ...
    // Update descriptor set information.
    {
        ...
        DescriptorSetUpdateBuilder()
            .writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
                         VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &descIndirectBuffer)
            .update(vkd, device);
    }
    ...
    // Create render pass.
    {
        ...
        attachments.push_back(Attachment(m_format, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_CLEAR,
        ...
        deps.push_back(SubpassDependency(0u,                                    // uint32_t                srcPass
                                         0u,                                    // uint32_t                dstPass
    ...
    // Create render pipeline.
    {
        ...
        const Unique<VkShaderModule> geometryShaderModule(
            createShaderModule(vkd, device, m_context.getBinaryCollection().get("geom"), 0u));
        const Unique<VkShaderModule> fragmentShaderModule(
            createShaderModule(vkd, device, m_context.getBinaryCollection().get("frag"), 0u));
        ...
        renderPipeline = makeGraphicsPipeline(
            vkd,                   // const DeviceInterface&                        vk
            device,                // const VkDevice                                device
    ...
    for (int primitiveNdx = 0; primitiveNdx < 128; primitiveNdx++)
    {
        vertexData.push_back(Vec4((float)((rand.getUint32() % m_width) * 2) / (float)m_width - 1.0f,
                                  (float)((rand.getUint32() % m_height) * 2) / (float)m_height - 1.0f, 1.0f, 1.0f));
    }
    ...
    vkd.cmdBindDescriptorSets(*commandBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, *pipelineLayout, 0u, 1u,
    ...
    // Barrier for indirect buffer.
    {
        ...
        vkd.cmdPipelineBarrier(*commandBuffer, VK_PIPELINE_STAGE_GEOMETRY_SHADER_BIT,
                               VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT, 0u, 1u, &barrier, 0u, nullptr, 0u, nullptr);
    }
    ...
    endCommandBuffer(vkd, *commandBuffer);
    submitCommandsAndWait(vkd, m_context.getDevice(), m_context.getUniversalQueue(), *commandBuffer);
    invalidateMappedMemoryRange(vkd, m_context.getDevice(), resultBufferMemory->getMemory(),
                                resultBufferMemory->getOffset(), VK_WHOLE_SIZE);
    ...
    // Verify result.
    {
        ...
        // Setup and run reference renderer.
        {
            ...
            const rr::Renderer renderer;
            const rr::Program program(&vertShader, &fragShader);
            const rr::MultisamplePixelBufferAccess msAccess(
                rr::MultisamplePixelBufferAccess::fromSinglesampleAccess(referenceAccess));
            const rr::RenderTarget renderTarget(msAccess);
            const rr::PrimitiveType primitiveType(rr::PRIMITIVETYPE_TRIANGLES);
            const rr::PrimitiveList primitiveList(rr::PrimitiveList(primitiveType, (uint32_t)triangles.size(), 0));
            const rr::ViewportState viewportState(msAccess);
            const rr::RenderState renderState(viewportState,
                                              m_context.getDeviceProperties().limits.subPixelPrecisionBits);
            const rr::VertexAttrib vertices =
                rr::VertexAttrib(rr::VERTEXATTRIBTYPE_FLOAT, 4, sizeof(tcu::Vec4), 0, &triangles[0]);
...
{
    SeparateChannelsTestConfig(VkFormat format_, SharedGroupParams groupParams_)
        : format(format_)
        , groupParams(groupParams_)
    {
    }
...
{
public:
    SeparateChannelsTestInstance(Context &context, SeparateChannelsTestConfig testConfig);
...
protected:
    void setup(void);
#ifndef CTS_USES_VULKANSC
    void startDynamicRendering(VkCommandBuffer cmdBuffer, VkRenderingFlagsKHR renderingFlags = 0u);
#endif // CTS_USES_VULKANSC
...
void SeparateChannelsTestInstance::setup(void)
{
    ...
    // Create image used for both input and output in case of color test, and as a color output in depth/stencil test.
    {
        ...
        m_colorImage           = createImage(vkd, device, &imageCreateInfo, nullptr);
        m_colorImageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vkd, device, *m_colorImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(device, *m_colorImage, m_colorImageAllocation->getMemory(),
                                     m_colorImageAllocation->getOffset()));
    }
    ...
    if (isDSFormat)
    {
        ...
        m_dsImage           = createImage(vkd, device, &imageCreateInfo, nullptr);
        m_dsImageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vkd, device, *m_dsImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(device, *m_dsImage, m_dsImageAllocation->getMemory(),
                                     m_dsImageAllocation->getOffset()));

        // Initialize depth / stencil image
        initDepthStencilImageChessboardPattern(
            vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(), *m_dsImage, m_format,
            m_depthInitValues[0], m_depthInitValues[1], m_stencilInitValues[0], m_stencilInitValues[1], m_width,
            m_height, m_tileSize, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
            VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    }

    // Initialize color image
    initColorImageChessboardPattern(vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(),
                                    *m_colorImage, colorFormat, m_colorInitValues[0], m_colorInitValues[1], m_width,
                                    m_height, m_tileSize, VK_IMAGE_LAYOUT_UNDEFINED, colorImageLayout,
                                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    ...
    if (!isDSFormat)
    {
        ...
        DescriptorSetUpdateBuilder()
            .writeSingle(*m_descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
                         VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, &descInputAttachment)
            .update(vkd, device);
    }

    // Create render pipeline layout.
    m_pipelineLayout =
        PipelineLayoutWrapper(m_groupParams->pipelineConstructionType, vkd, device, *m_descriptorSetLayout);
    ...
    if (m_groupParams->renderingType != RENDERING_TYPE_DYNAMIC_RENDERING)
    {
        ...
        AttachmentReference dsAttachmentReference(1u, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                                                  VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT);

        const VkImageAspectFlags inputAttachmentAspectMask(
            (m_groupParams->renderingType == RENDERING_TYPE_RENDERPASS2) ?
                static_cast<VkImageAspectFlags>(VK_IMAGE_ASPECT_COLOR_BIT) :
                static_cast<VkImageAspectFlags>(0));

        attachments.push_back(Attachment(colorFormat, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_LOAD,
                                         VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_DONT_CARE,
                                         VK_ATTACHMENT_STORE_OP_DONT_CARE, colorImageLayout, colorImageLayout));
        colorAttachmentReferences.push_back(AttachmentReference(0u, colorImageLayout));

        if (isDSFormat)
        {
            attachments.push_back(Attachment(
                m_format, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_LOAD, VK_ATTACHMENT_STORE_OP_STORE,
        ...
        else
        {
            inputAttachmentReferences.push_back(
                AttachmentReference(0u, VK_IMAGE_LAYOUT_GENERAL, inputAttachmentAspectMask));
        }

        const vector<Subpass> subpasses(
            1, Subpass(VK_PIPELINE_BIND_POINT_GRAPHICS, 0u, inputAttachmentReferences, colorAttachmentReferences,
                       vector<AttachmentReference>(),
                       isDSFormat ? dsAttachmentReference :
                                    AttachmentReference(VK_ATTACHMENT_UNUSED, VK_IMAGE_LAYOUT_GENERAL),
                       vector<uint32_t>()));
        ...
        if (!isDSFormat)
        {
            ...
            subpassDependency.push_back(
                SubpassDependency(0u, 0u, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
        ...
        m_renderPass = createRenderPass(vkd, device, RenderPass(attachments, subpasses, subpassDependency),
                                        m_groupParams->renderingType);

        // Create framebuffer.
        const VkImageView dsAttachments[]{*m_imageView, *m_dsImageView};
    ...
    // Create render pipeline.
    {
        ...
        const VkStencilOpState stencilOpState{
            ...
            m_stencilRefValue      // uint32_t        reference
        };
...
#ifndef CTS_USES_VULKANSC
        const uint32_t inputAttachmentIndex   = 0;
        const uint32_t dsInputAttachmentIndex = VK_ATTACHMENT_UNUSED;
        VkRenderingInputAttachmentIndexInfoKHR renderingInputAttachmentIndexInfo{
            VK_STRUCTURE_TYPE_RENDERING_INPUT_ATTACHMENT_INDEX_INFO_KHR,
            nullptr,
            1u,
            &inputAttachmentIndex,
            (isDSFormat ? &dsInputAttachmentIndex : nullptr),
            (isDSFormat ? &dsInputAttachmentIndex : nullptr)};
        ...
        m_renderPipeline.setDefaultMultisampleState()
            ...
            .setupFragmentShaderState(m_pipelineLayout, *m_renderPass, 0u, fragmentShaderModule,
                                      (isDSFormat ? &depthStencilState : nullptr), 0, 0, VK_NULL_HANDLE, {},
                                      renderingInputAttachmentIndexInfoWrapper)
            .setupFragmentOutputState(*m_renderPass, 0u, (isDSFormat ? nullptr : &colorBlendState))
            .setMonolithicPipelineLayout(m_pipelineLayout)
            .buildPipeline();
    }

    // Generate quad vertices
    vector<Vec4> vertexData{tcu::Vec4(-1.0f, -1.0f, 0.5f, 1.0f), tcu::Vec4(-1.0f, 1.0f, 0.5f, 1.0f),
                            tcu::Vec4(1.0f, -1.0f, 0.5f, 1.0f), tcu::Vec4(1.0f, 1.0f, 0.5f, 1.0f)};
...
void SeparateChannelsTestInstance::startDynamicRendering(VkCommandBuffer cmdBuffer, VkRenderingFlagsKHR renderingFlags)
{
    ...
    {
        beginRendering(vkd, cmdBuffer, *m_imageView, renderArea, clearValue, VK_IMAGE_LAYOUT_RENDERING_LOCAL_READ_KHR,
                       VK_ATTACHMENT_LOAD_OP_LOAD, renderingFlags);
    }
}
#endif
...
void SeparateChannelsTestInstance::draw(VkCommandBuffer cmdBuffer)
{
    ...
    if (!isDSFormat)
    {
        vkd.cmdBindDescriptorSets(cmdBuffer, vk::VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayout, 0u, 1u,
        ...
        vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                               VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_DEPENDENCY_BY_REGION_BIT, 1u, &memoryBarrier,
...
void SeparateChannelsTestInstance::postRenderCommands(VkCommandBuffer cmdBuffer)
{
    ...
    if (isDSFormat)
        copyDepthStencilImageToBuffers(vkd, cmdBuffer, *m_dsImage, *m_resultBuffer0, *m_resultBuffer1,
...
tcu::TestStatus SeparateChannelsTestInstance::verifyResults(void)
{
    ...
    if (isDepthStencilFormat(m_format))
    {
        ...
        const float depthThreshold(
            1.0f /
            ((UVec4(1u) << tcu::getTextureFormatMantissaBitDepth(resultDepthAccess.getFormat()).cast<uint32_t>()) - 1u)
                .cast<float>()
                .x());
        ...
        for (uint32_t x = 0; x < m_width; x++)
            ...
            {
                float depthValue =
                    ((x / m_tileSize) % 2 != (y / m_tileSize) % 2) ? m_depthInitValues[0] : m_depthInitValues[1];
    ...
    else
    {
        ...
        for (uint32_t x = 0; x < m_width; x++)
            for (uint32_t y = 0; y < m_height; y++)
            {
                const tcu::Vec4 initValue =
                    ((x / m_tileSize) % 2 != (y / m_tileSize) % 2) ? m_colorInitValues[0] : m_colorInitValues[1];
...
void SeparateChannelsTestInstance::iterateInternal(void)
{
    ...
    const uint32_t queueFamilyIndex = m_context.getUniversalQueueFamilyIndex();
    const Unique<VkCommandPool> commandPool(
        createCommandPool(vkd, device, VK_COMMAND_POOL_CREATE_TRANSIENT_BIT, queueFamilyIndex));
    const Unique<VkCommandBuffer> commandBuffer(
    ...
    invalidateMappedMemoryRange(vkd, device, m_resultBuffer0Memory->getMemory(), m_resultBuffer0Memory->getOffset(),
                                VK_WHOLE_SIZE);
    invalidateMappedMemoryRange(vkd, device, m_resultBuffer1Memory->getMemory(), m_resultBuffer1Memory->getOffset(),
                                VK_WHOLE_SIZE);
}
...
{
    SingleAttachmentTestConfig(VkFormat format_, SharedGroupParams groupParams_)
        : format(format_)
        , groupParams(groupParams_)
    {
    }
...
{
public:
    SingleAttachmentTestInstance(Context &context, SingleAttachmentTestConfig testConfig);
...
SingleAttachmentTestInstance::SingleAttachmentTestInstance(Context &context, SingleAttachmentTestConfig testConfig)
    ...
    , m_height(256u)
    , m_format(testConfig.format)
    , m_pipelineSolidColor(context.getInstanceInterface(), context.getDeviceInterface(), context.getPhysicalDevice(),
                           context.getDevice(), context.getDeviceExtensions(), m_groupParams->pipelineConstructionType)
    , m_pipelineInputAtt(context.getInstanceInterface(), context.getDeviceInterface(), context.getPhysicalDevice(),
                         context.getDevice(), context.getDeviceExtensions(), m_groupParams->pipelineConstructionType)
    , m_pipelineImageSampler(context.getInstanceInterface(), context.getDeviceInterface(), context.getPhysicalDevice(),
...
void SingleAttachmentTestInstance::setup(void)
{
    ...
    // Create image used for final result.
    {
        ...
        m_resultImage           = createImage(vkd, device, &imageCreateInfo, nullptr);
        m_resultImageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vkd, device, *m_resultImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(device, *m_resultImage, m_resultImageAllocation->getMemory(),
                                     m_resultImageAllocation->getOffset()));
    }

    // Initialize color image. This is expected to be cleared later.
    initColorImageChessboardPattern(vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(),
                                    *m_colorImage, m_format, colorInitValues[0], colorInitValues[1], m_width, m_height,
                                    32u, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
                                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    // Initialize result image. This will be overwritten later.
    initColorImageChessboardPattern(vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(),
                                    *m_resultImage, m_format, colorInitValues[0], colorInitValues[1], m_width, m_height,
                                    32u, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    ...
    m_descriptorSetLayoutInput =
        DescriptorSetLayoutBuilder()
            .addSingleBinding(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, VK_SHADER_STAGE_FRAGMENT_BIT)
            .build(vkd, device);

    m_descriptorSetLayoutImageSampler =
        DescriptorSetLayoutBuilder()
    ...
    DescriptorSetUpdateBuilder()
        .writeSingle(*m_descriptorSetInput, DescriptorSetUpdateBuilder::Location::binding(0u),
                     VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, &descIOAttachment)
        .update(vkd, device);

    DescriptorSetUpdateBuilder()
        .writeSingle(*m_descriptorSetImageSampler, DescriptorSetUpdateBuilder::Location::binding(0u),
                     VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, &descImageSampler)
        .update(vkd, device);

    // Create pipeline layouts.
    m_pipelineLayoutInput =
        PipelineLayoutWrapper(m_groupParams->pipelineConstructionType, vkd, device, *m_descriptorSetLayoutInput);
    m_pipelineLayoutImageSampler =
        PipelineLayoutWrapper(m_groupParams->pipelineConstructionType, vkd, device, *m_descriptorSetLayoutImageSampler);

    if (m_groupParams->renderingType != RENDERING_TYPE_DYNAMIC_RENDERING)
    {
        // Create render passes.

        vector<Attachment> attachments{{m_format, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_LOAD,
                                        VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_DONT_CARE,
        ...
        m_renderPass1 = createRenderPass(vkd, device, RenderPass(attachments, subpasses, vector<SubpassDependency>()),
                                         m_groupParams->renderingType);

        attachments.clear();
        attachments.emplace_back(m_format, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_CLEAR,
                                 VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_DONT_CARE,
        ...
        const vector<SubpassDependency> dependencies(
            1, SubpassDependency(0u, 0u, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                                 VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                                 VK_ACCESS_INPUT_ATTACHMENT_READ_BIT, VK_DEPENDENCY_BY_REGION_BIT));

        m_renderPass0 = createRenderPass(vkd, device, RenderPass(attachments, subpasses, dependencies),
                                         m_groupParams->renderingType);
    ...
    {
        const ShaderWrapper vertexShaderModule(vkd, device, m_context.getBinaryCollection().get("vert"), 0u);
        const ShaderWrapper fragmentShaderModuleInputAtt(
            vkd, device, m_context.getBinaryCollection().get("frag_input_attachment"), 0u);
        const ShaderWrapper fragmentShaderModuleSolidColor(vkd, device,
                                                           m_context.getBinaryCollection().get("frag_solid_color"), 0u);
        const ShaderWrapper fragmentShaderModuleSampler(
            vkd, device, m_context.getBinaryCollection().get("frag_combined_image_sampler"), 0u);
        ...
        m_pipelineSolidColor.setDefaultMultisampleState()
            ...
            .setDefaultTopology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP)
            .setupVertexInputState(&vertexInputState)
            .setupPreRasterizationShaderState(viewports, scissors, m_pipelineLayoutInput, *m_renderPass0, 0u,
                                              vertexShaderModule, 0u, ShaderWrapper(), ShaderWrapper(), ShaderWrapper(),
        ...
        m_pipelineImageSampler.setDefaultMultisampleState()
            ...
            .setDefaultTopology(VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP)
            .setupVertexInputState(&vertexInputState)
            .setupPreRasterizationShaderState(viewports, scissors, m_pipelineLayoutImageSampler, *m_renderPass1, 0u,
                                              vertexShaderModule, 0u, ShaderWrapper(), ShaderWrapper(), ShaderWrapper(),
                                              nullptr, nullptr, renderingCreateInfoWrapper)
...
void SingleAttachmentTestInstance::iterateInternal(void)
{
    ...
    endCommandBuffer(vkd, *commandBuffer);
    submitCommandsAndWait(vkd, device, m_context.getUniversalQueue(), *commandBuffer);
    invalidateMappedMemoryRange(vkd, device, m_resultBufferMemory->getMemory(), m_resultBufferMemory->getOffset(),
                                VK_WHOLE_SIZE);
}
    ...
    if (m_groupParams->useSecondaryCmdBuffer)
    {
        ...
        // record first secondary command buffer
        {
            if (m_groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
            {
                ...
                beginRendering(vkd, *secCmdBuffer[0], *m_imageViewInput, renderArea, clearValue,
                               VK_IMAGE_LAYOUT_RENDERING_LOCAL_READ_KHR, VK_ATTACHMENT_LOAD_OP_CLEAR);
            }
            else
        ...
        if (!m_groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
        {
            beginRendering(vkd, *cmdBuffer, *m_imageViewInput, renderArea, clearValue,
                           VK_IMAGE_LAYOUT_RENDERING_LOCAL_READ_KHR, VK_ATTACHMENT_LOAD_OP_CLEAR,
...
void SingleAttachmentTestInstance::preRenderCommands(VkCommandBuffer cmdBuffer)
{
    ...
    vkd.cmdPipelineBarrier(cmdBuffer, 0, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, 0u, 0u, nullptr, 0u, nullptr,
                           1u, &imageBarrier);
}
#endif // CTS_USES_VULKANSC
...
void SingleAttachmentTestInstance::drawThatUsesInputAttachment(VkCommandBuffer cmdBuffer)
{
    ...
    VkImageMemoryBarrier imageBarrier =
        makeImageMemoryBarrier(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_ACCESS_INPUT_ATTACHMENT_READ_BIT,
                               VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_GENERAL, *m_colorImage, subresourceRange);
    // Bind pipeline.
    ...
    // Bind descriptor set.
    vkd.cmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayoutInput, 0u, 1u,
                              &*m_descriptorSetInput, 0u, nullptr);
...
void SingleAttachmentTestInstance::inbetweenDrawsRenderBarriers(VkCommandBuffer cmdBuffer)
{
    ...
    VkImageMemoryBarrier imageBarriers[]{
        makeImageMemoryBarrier(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT |
                                   VK_ACCESS_HOST_WRITE_BIT,
                               VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_GENERAL,
                               VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, *m_colorImage, subresourceRange),
        makeImageMemoryBarrier(VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                               VK_ACCESS_COLOR_ATTACHMENT_READ_BIT | VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                               VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL,
                               *m_resultImage, subresourceRange)};
...
void SingleAttachmentTestInstance::drawThatSamplesPreviousResult(VkCommandBuffer cmdBuffer)
{
    ...
    // Bind descriptor set.
    vkd.cmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *m_pipelineLayoutImageSampler, 0u, 1u,
                              &*m_descriptorSetImageSampler, 0u, nullptr);
...
void SingleAttachmentTestInstance::postRenderCommands(VkCommandBuffer cmdBuffer)
{
    ...
    // Copy results to a buffer.
    copyImageToBuffer(vkd, cmdBuffer, *m_resultImage, *m_resultBuffer, tcu::IVec2(m_width, m_height),
...
struct ExternalPrograms
{
    void init(vk::SourceCollections &dst, ExternalTestConfig testConfig) const
    {
        for (size_t renderPassNdx = 0; renderPassNdx < testConfig.renderPasses.size(); renderPassNdx++)
        {
            ...
            if (renderPassNdx == 0)
            {
                dst.glslSources.add("quad-frag-" + de::toString(renderPassNdx))
                    << glu::FragmentSource("#version 450\n"
                                           ...
                                           "{\n"
                                           "    if (gl_FragCoord.x <= " +
                                           de::toString(testConfig.imageSize.x() / 2) +
                                           " && gl_FragCoord.y <= " + de::toString(testConfig.imageSize.y() / 2) +
                                           ")\n"
                                           "        o_color = vec4(1.0, 0.0, 0.0, 1.0);\n"
                                           "    else if (gl_FragCoord.x > " +
                                           de::toString(testConfig.imageSize.x() / 2) +
                                           " && gl_FragCoord.y <= " + de::toString(testConfig.imageSize.y() / 2) +
                                           ")\n"
                                           "        o_color = vec4(0.0, 1.0, 0.0, 1.0);\n"
                                           "    else if (gl_FragCoord.x <= " +
                                           de::toString(testConfig.imageSize.x() / 2) + " && gl_FragCoord.y > " +
                                           de::toString(testConfig.imageSize.y() / 2) +
                                           ")\n"
            ...
            else
            {
                if (renderPassNdx % 2 == 0)
                {
                    ...
                    dst.glslSources.add("quad-frag-" + de::toString(renderPassNdx)) << glu::FragmentSource(
                        ...
                        "{\n"
                        "    vec2 step = vec2(1.0 / " +
                        de::toString(testConfig.imageSize.x()) + ", 1.0 / " + de::toString(testConfig.imageSize.y()) +
                        ");\n"
                        "    vec2 minCoord = vec2(0.0, 0.0);\n"
                        "    vec2 maxCoord = vec2(1.0, 1.0);\n"
                        "    vec4 blurColor = vec4(0.0);\n"
                        "    for(int sampleNdx = 0; sampleNdx < " +
                        de::toString(testConfig.blurKernel + 1) +
                        "; sampleNdx++)\n"
                        "    {\n"
                        "        vec2 sampleCoord = vec2((vtxTexCoords.x - " +
                        de::toString(testConfig.blurKernel / 2) +
                        " * step.x) + step.x * sampleNdx, vtxTexCoords.y);\n"
                ...
                else
                {
                    ...
                    dst.glslSources.add("quad-frag-" + de::toString(renderPassNdx)) << glu::FragmentSource(
                        ...
                        "; sampleNdx++)\n"
                        "    {\n"
                        "        vec2 sampleCoord = vec2(vtxTexCoords.x, (vtxTexCoords.y - " +
                        de::toString(testConfig.blurKernel / 2) +
                        " * step.y) + step.y * sampleNdx);\n"
                        "        blurColor += 0.12 * texture(previousPass, clamp(sampleCoord, minCoord, maxCoord));\n"
                        "    }\n"
                        "    o_color = blurColor;\n"
                        "}\n");
...
struct SubpassPrograms
{
    void init(vk::SourceCollections &dst, SubpassTestConfig testConfig) const
    {
        ...
        for (size_t subpassNdx = 0; subpassNdx < subpassCount; subpassNdx++)
        {
            if (subpassNdx == 0)
            {
                dst.glslSources.add("subpass-vert-" + de::toString(subpassNdx))
            ...
            if (isDepthStencilFormat(testConfig.format))
            {
                if (subpassNdx == 0)
                {
                    ...
                    dst.glslSources.add("subpass-frag-" + de::toString(subpassNdx))
                        << glu::FragmentSource("#version 450\n"
                                               "void main (void)\n"
                                               "{\n"
                                               "}\n");
...
struct SubpassSelfDependencyBackwardsPrograms
{
    void init(vk::SourceCollections &dst, SubpassSelfDependencyBackwardsTestConfig testConfig) const
    {
        ...
        dst.glslSources.add("geom") << glu::GeometrySource("#version 450\n"
                                                           "layout(points) in;\n"
...
struct SingleAttachmentPrograms
{
    void init(vk::SourceCollections &dst, SingleAttachmentTestConfig testConfig) const
    {
        ...
        dst.glslSources.add("frag_solid_color")
            << glu::FragmentSource("#version 450\n"
                                   "layout(location = 0) out highp vec4 fragColor;\n"
                                   "void main (void)\n"
                                   "{\n"
        ...
        dst.glslSources.add("frag_combined_image_sampler")
            << glu::FragmentSource("#version 450\n"
...
void initTests(tcu::TestCaseGroup *group, const renderpass::SharedGroupParams groupParams)
{
    ...
    {
        const uint32_t renderPassCounts[] = {2u, 3u, 5u};

        const UVec2 renderSizes[] = {UVec2(64, 64), UVec2(128, 128), UVec2(512, 512)};
        ...
        {
            string groupName("render_size_" + de::toString(renderSizes[renderSizeNdx].x()) + "_" +
                             de::toString(renderSizes[renderSizeNdx].y()));
            de::MovePtr<tcu::TestCaseGroup> renderSizeGroup(new tcu::TestCaseGroup(testCtx, groupName.c_str()));

            for (size_t renderPassCountNdx = 0; renderPassCountNdx < DE_LENGTH_OF_ARRAY(renderPassCounts);
                 renderPassCountNdx++)
            {
                vector<RenderPass> renderPasses;
                ...
                ExternalTestConfig testConfig{VK_FORMAT_R8G8B8A8_UNORM,
    ...
    if (groupParams->renderingType != RENDERING_TYPE_DYNAMIC_RENDERING)
    {
             ...
             renderPassCountNdx++)
        {
            ...
            const uint32_t blurKernel(12u);
            const ExternalTestConfig testConfig(VK_FORMAT_R8G8B8A8_UNORM, UVec2(128, 128), renderPasses, groupParams,
    ...
    if (groupParams->renderingType != RENDERING_TYPE_DYNAMIC_RENDERING)
    {
        const UVec2 renderSizes[] = {UVec2(32, 32), UVec2(64, 64), UVec2(128, 128)};

        const uint32_t subpassCounts[] = {2u, 3u, 5u};
        ...
        for (size_t renderSizeNdx = 0; renderSizeNdx < DE_LENGTH_OF_ARRAY(renderSizes); renderSizeNdx++)
        {
            string renderSizeGroupName("render_size_" + de::toString(renderSizes[renderSizeNdx].x()) + "_" +
                                       de::toString(renderSizes[renderSizeNdx].y()));
            de::MovePtr<tcu::TestCaseGroup> renderSizeGroup(
                new tcu::TestCaseGroup(testCtx, renderSizeGroupName.c_str()));
            ...
            for (size_t subpassCountNdx = 0; subpassCountNdx < DE_LENGTH_OF_ARRAY(subpassCounts); subpassCountNdx++)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> subpassCountGroup(
                    new tcu::TestCaseGroup(testCtx, subpassGroupName.c_str()));

                for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                {
                    const uint32_t subpassCount(subpassCounts[subpassCountNdx]);
                    const uint32_t attachmentCount(subpassCount);
                    ...
                    for (size_t subpassNdx = 0; subpassNdx < subpassCount; subpassNdx++)
                    {
                        vector<AttachmentReference> inputAttachmentReferences;
                        const VkImageAspectFlags inputAttachmentAspectMask(
                            (groupParams->renderingType == RENDERING_TYPE_RENDERPASS2) ?
    ...
    // Test using a single attachment with reads and writes using separate channels. This should work without subpass self-dependency.
    {
        ...
        struct TestConfig
        {
            string name;
            VkFormat format;
        } configs[] = {{"r8g8b8a8_unorm", VK_FORMAT_R8G8B8A8_UNORM},
                       {"r16g16b16a16_sfloat", VK_FORMAT_R16G16B16A16_SFLOAT},
                       {"d24_unorm_s8_uint", VK_FORMAT_D24_UNORM_S8_UINT},
                       {"d32_sfloat_s8_uint", VK_FORMAT_D32_SFLOAT_S8_UINT}};

        for (uint32_t configIdx = 0; configIdx < DE_LENGTH_OF_ARRAY(configs); configIdx++)
        {
            const SeparateChannelsTestConfig testConfig(configs[configIdx].format, groupParams);

            separateChannelsGroup->addChild(
    ...
    // Test using a single attachment for input and output.
    {
        ...
        for (uint32_t configIdx = 0; configIdx < DE_LENGTH_OF_ARRAY(configs); configIdx++)
        {
            ...
            singleAttachmentGroup->addChild(
