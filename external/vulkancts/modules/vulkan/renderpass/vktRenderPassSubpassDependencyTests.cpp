...
struct ExternalTestConfig
{
    ExternalTestConfig(VkFormat format_, UVec2 imageSize_, vector<RenderPass> renderPasses_,
                       RenderPassType renderPassType_, SynchronizationType synchronizationType_,
...
ExternalDependencyTestInstance::ExternalDependencyTestInstance(Context &context, ExternalTestConfig testConfig)
    ...
    , m_renderPass2Supported((testConfig.renderPassType == RENDERPASS_TYPE_RENDERPASS2) &&
    ...
    , m_renderPasses(createRenderPasses(context.getDeviceInterface(), context.getDevice(), testConfig.renderPasses,
                                        testConfig.renderPassType, testConfig.synchronizationType))
...
vector<SharedPtrVkRenderPass> ExternalDependencyTestInstance::createRenderPasses(
    const DeviceInterface &vkd, VkDevice device, vector<RenderPass> renderPassInfos,
    const RenderPassType renderPassType, const SynchronizationType synchronizationType)
{
    vector<SharedPtrVkRenderPass> renderPasses;
    ...
    for (const auto &renderPassInfo : renderPassInfos)
        renderPasses.push_back(
            makeSharedPtr(createRenderPass(vkd, device, renderPassInfo, renderPassType, synchronizationType)));
...
{
    SubpassTestConfig(VkFormat format_, UVec2 imageSize_, RenderPass renderPass_, RenderPassType renderPassType_)
        : format(format_)
        , imageSize(imageSize_)
    ...
    uint32_t universalQueueFamilyIndex, RenderPass renderPassInfo, VkFormat format, uint32_t width, uint32_t height)
{
    ...
    {
        const VkFormatFeatureFlags flags    = isDepthStencilFormat(m_format) ?
...
{
    SubpassSelfDependencyBackwardsTestConfig(VkFormat format_, UVec2 imageSize_, RenderPassType renderPassType_)
        : format(format_)
        , imageSize(imageSize_)
        , renderPassType(renderPassType_)
    {
...
{
    SeparateChannelsTestConfig(VkFormat format_, RenderPassType renderPassType_)
        : format(format_)
        , renderPassType(renderPassType_)
    {
    }
...
tcu::TestStatus SeparateChannelsTestInstance::iterateInternal(void)
{
    ...
    const uint32_t stencilInitValues[2] = {2u, 100u};
    const uint32_t stencilRefValue      = 200u;
    const uint32_t tileSize             = 32u;
    vector<Vec4> vertexData;
    ...
    const VkImageLayout colorImageLayout =
        isDSFormat ? VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL : VK_IMAGE_LAYOUT_GENERAL;
    ...
    // Create image used for both input and output in case of color test, and as a color output in depth/stencil test.
    {
        ...
        colorImageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vkd, device, *colorImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(device, *colorImage, colorImageAllocation->getMemory(),
                                     colorImageAllocation->getOffset()));
    }
    ...
    if (isDSFormat)
    {
        ...
        dsImageAllocation = m_context.getDefaultAllocator().allocate(getImageMemoryRequirements(vkd, device, *dsImage),
        ...
        initDepthStencilImageChessboardPattern(
            vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(), *dsImage, m_format,
            depthInitValues[0], depthInitValues[1], stencilInitValues[0], stencilInitValues[1], m_width, m_height,
            tileSize, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
            VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    }

    // Initialize color image
    initColorImageChessboardPattern(vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(), *colorImage,
                                    colorFormat, colorInitValues[0], colorInitValues[1], m_width, m_height, tileSize,
                                    VK_IMAGE_LAYOUT_UNDEFINED, colorImageLayout, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    ...
    // Create color image views
    {
        ...
        if (!isDSFormat)
            inputImageView = createImageView(vkd, device, &imageViewCreateInfo);
        outputImageView = createImageView(vkd, device, &imageViewCreateInfo);
    }
    ...
    if (!isDSFormat)
    {
        VkDescriptorImageInfo descInputAttachment =
            makeDescriptorImageInfo(DE_NULL, *inputImageView, VK_IMAGE_LAYOUT_GENERAL);
    ...
    // Create render pass.
    {
        ...
        const VkImageAspectFlags inputAttachmentAspectMask(
            (m_renderPassType == RENDERPASS_TYPE_RENDERPASS2) ?
        ...
        else
        {
            ...
            inputAttachmentReferences.push_back(
                AttachmentReference(1u, VK_IMAGE_LAYOUT_GENERAL, inputAttachmentAspectMask));
        }
        ...
        renderPass = createRenderPass(vkd, device, RenderPass(attachments, subpasses, vector<SubpassDependency>()),
                                      m_renderPassType);
    }
    ...
    // Create render pipeline.
    {
        ...
        const VkStencilOpState stencilOpState = {
            ...
            stencilRefValue        // uint32_t        reference
        };
    ...
    // Create framebuffer.
    {
        const VkImageView attachments[] = {*outputImageView, isDSFormat ? *dsImageView : *inputImageView};
    ...
    if (isDSFormat)
    {
        copyDepthStencilImageToBuffers(vkd, *commandBuffer, *dsImage, *resultBuffer0, *resultBuffer1,
                                       tcu::IVec2(m_width, m_height), VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT,
                                       VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
    }
    else
    {
        copyImageToBuffer(vkd, *commandBuffer, *colorImage, *resultBuffer0, tcu::IVec2(m_width, m_height),
                          VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_IMAGE_LAYOUT_GENERAL);
    }

    endCommandBuffer(vkd, *commandBuffer);
    submitCommandsAndWait(vkd, m_context.getDevice(), m_context.getUniversalQueue(), *commandBuffer);
    invalidateMappedMemoryRange(vkd, m_context.getDevice(), resultBuffer0Memory->getMemory(),
                                resultBuffer0Memory->getOffset(), VK_WHOLE_SIZE);
    invalidateMappedMemoryRange(vkd, m_context.getDevice(), resultBuffer1Memory->getMemory(),
                                resultBuffer1Memory->getOffset(), VK_WHOLE_SIZE);
    ...
    // Verify result.
    {
        ...
        if (isDSFormat)
        {
            ...
            for (uint32_t x = 0; x < m_width; x++)
                ...
                {
                    float depthValue =
                        ((x / tileSize) % 2 != (y / tileSize) % 2) ? depthInitValues[0] : depthInitValues[1];
            ...
            for (uint32_t x = 0; x < m_width; x++)
                for (uint32_t y = 0; y < m_height; y++)
                {
                    const tcu::Vec4 initValue =
                        ((x / tileSize) % 2 != (y / tileSize) % 2) ? colorInitValues[0] : colorInitValues[1];
                    const tcu::Vec4 refValue =
                        tcu::Vec4(initValue.x(), initValue.y(), initValue.x() + initValue.y(), 1.0f);
...
{
    SingleAttachmentTestConfig(VkFormat format_, RenderPassType renderPassType_)
        : format(format_)
        , renderPassType(renderPassType_)
    {
    }
...
tcu::TestStatus SingleAttachmentTestInstance::iterateInternal(void)
{
    ...
    // Create image used for final result.
    {
        ...
        resultImageAllocation = m_context.getDefaultAllocator().allocate(
            getImageMemoryRequirements(vkd, device, *resultImage), MemoryRequirement::Any);
        VK_CHECK(vkd.bindImageMemory(device, *resultImage, resultImageAllocation->getMemory(),
                                     resultImageAllocation->getOffset()));
    }

    // Initialize color image. This is expected to be cleared later.
    initColorImageChessboardPattern(vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(), *colorImage,
                                    m_format, colorInitValues[0], colorInitValues[1], m_width, m_height, 32u,
                                    VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL,
                                    VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT);
    // Initialize result image. This will be overwritten later.
    initColorImageChessboardPattern(vkd, device, queue, queueFamilyIndex, m_context.getDefaultAllocator(), *resultImage,
                                    m_format, colorInitValues[0], colorInitValues[1], m_width, m_height, 32u,
    ...
    Unique<VkDescriptorSetLayout> descriptorSetLayoutInput(
        DescriptorSetLayoutBuilder()
            .addSingleBinding(VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, VK_SHADER_STAGE_FRAGMENT_BIT)
            .build(vkd, device));

    Unique<VkDescriptorSetLayout> descriptorSetLayoutImageSampler(
        DescriptorSetLayoutBuilder()
    ...
    Unique<VkDescriptorSet> descriptorSetInput(
        makeDescriptorSet(vkd, device, *descriptorPool, *descriptorSetLayoutInput));
    Unique<VkDescriptorSet> descriptorSetImageSampler(
        makeDescriptorSet(vkd, device, *descriptorPool, *descriptorSetLayoutImageSampler));
    ...
    VkDescriptorImageInfo descImageSampler =
        makeDescriptorImageInfo(*sampler, *imageViewInput, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    DescriptorSetUpdateBuilder()
        .writeSingle(*descriptorSetInput, DescriptorSetUpdateBuilder::Location::binding(0u),
                     VK_DESCRIPTOR_TYPE_INPUT_ATTACHMENT, &descIOAttachment)
        .update(vkd, device);

    DescriptorSetUpdateBuilder()
        .writeSingle(*descriptorSetImageSampler, DescriptorSetUpdateBuilder::Location::binding(0u),
    ...
    // Create render passes.
    {
        ...
        attachments.push_back(Attachment(m_format, VK_SAMPLE_COUNT_1_BIT, VK_ATTACHMENT_LOAD_OP_LOAD,
                                         VK_ATTACHMENT_STORE_OP_STORE, VK_ATTACHMENT_LOAD_OP_DONT_CARE,
        ...
        renderPass1 = createRenderPass(vkd, device, RenderPass(attachments, subpasses, vector<SubpassDependency>()),
                                       m_renderPassType);
    }
    {
        vector<Attachment> attachments;
    ...
    // Create pipelines.
    {
        const Unique<VkShaderModule> vertexShaderModule(
            createShaderModule(vkd, device, m_context.getBinaryCollection().get("vert"), 0u));
        const Unique<VkShaderModule> fragmentShaderModuleInputAtt(
            createShaderModule(vkd, device, m_context.getBinaryCollection().get("frag_input_attachment"), 0u));
        const Unique<VkShaderModule> fragmentShaderModuleSolidColor(
            createShaderModule(vkd, device, m_context.getBinaryCollection().get("frag_solid_color"), 0u));
        const Unique<VkShaderModule> fragmentShaderModuleSampler(
            createShaderModule(vkd, device, m_context.getBinaryCollection().get("frag_combined_image_sampler"), 0u));
        ...
        pipelineSolidColor = makeGraphicsPipeline(
            vkd,                  // const DeviceInterface&                            vk
            device,               // const VkDevice                                    device
        ...
        pipelineInputAtt = makeGraphicsPipeline(
            vkd,                  // const DeviceInterface&                            vk
            device,               // const VkDevice                                    device
            *pipelineLayoutInput, // const VkPipelineLayout                            pipelineLayout
            *vertexShaderModule,  // const VkShaderModule                                vertexShaderModule
        ...
        pipelineImageSampler = makeGraphicsPipeline(
            vkd,                         // const DeviceInterface&                            vk
            device,                      // const VkDevice                                    device
    ...
    // Bind descriptor set.
    vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *pipelineLayoutInput, 0u, 1u,
                              &*descriptorSetInput, 0u, DE_NULL);
    ...
    // Bind descriptor set.
    vkd.cmdBindDescriptorSets(*commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *pipelineLayoutImageSampler, 0u, 1u,
                              &*descriptorSetImageSampler, 0u, DE_NULL);
    ...
    // Copy results to a buffer.
    copyImageToBuffer(vkd, *commandBuffer, *resultImage, *resultBuffer, tcu::IVec2(m_width, m_height),
...
void initTests(tcu::TestCaseGroup *group, const RenderPassType renderPassType)
{
    ...
    // Test external subpass dependencies
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> externalGroup(
            new tcu::TestCaseGroup(testCtx, "external_subpass", "external_subpass"));
        ...
        for (size_t renderSizeNdx = 0; renderSizeNdx < DE_LENGTH_OF_ARRAY(renderSizes); renderSizeNdx++)
        {
                 ...
                 renderPassCountNdx++)
            {
                ...
                ExternalTestConfig testConfig{VK_FORMAT_R8G8B8A8_UNORM, renderSizes[renderSizeNdx],  renderPasses,
                                              renderPassType,           SYNCHRONIZATION_TYPE_LEGACY, blurKernel};
    ...
    // Test implicit subpass dependencies
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> implicitGroup(
            new tcu::TestCaseGroup(testCtx, "implicit_dependencies", "implicit_dependencies"));
             ...
             renderPassCountNdx++)
        {
            ...
            const uint32_t blurKernel(12u);
            const ExternalTestConfig testConfig(VK_FORMAT_R8G8B8A8_UNORM, UVec2(128, 128), renderPasses, renderPassType,
    ...
    // Test late fragment operations using depth_stencil attachments in multipass rendering
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> lateFragmentTestsGroup(
            new tcu::TestCaseGroup(testCtx, "late_fragment_tests", "wait for late fragment tests"));
        ...
        for (size_t renderSizeNdx = 0; renderSizeNdx < DE_LENGTH_OF_ARRAY(renderSizes); renderSizeNdx++)
        {
            string renderSizeGroupName("render_size_" + de::toString(renderSizes[renderSizeNdx].x()) + "_" +
                                       de::toString(renderSizes[renderSizeNdx].y()));
            de::MovePtr<tcu::TestCaseGroup> renderSizeGroup(
                new tcu::TestCaseGroup(testCtx, renderSizeGroupName.c_str(), renderSizeGroupName.c_str()));
            ...
            for (size_t subpassCountNdx = 0; subpassCountNdx < DE_LENGTH_OF_ARRAY(subpassCounts); subpassCountNdx++)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> subpassCountGroup(
                    new tcu::TestCaseGroup(testCtx, subpassGroupName.c_str(), subpassGroupName.c_str()));
                ...
                for (size_t formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
                {
                    ...
                    for (size_t subpassNdx = 0; subpassNdx < subpassCount; subpassNdx++)
                    {
                        vector<AttachmentReference> inputAttachmentReferences;
                        const VkImageAspectFlags inputAttachmentAspectMask(
                            (renderPassType == RENDERPASS_TYPE_RENDERPASS2) ?
                    ...
                    subpassCountGroup->addChild(
                        new InstanceFactory1<SubpassDependencyTestInstance, SubpassTestConfig, SubpassPrograms>(
                            testCtx, tcu::NODETYPE_SELF_VALIDATE, format, format, testConfig));
                }
    ...
    // Test subpass self dependency
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> selfDependencyGroup(
            new tcu::TestCaseGroup(testCtx, "self_dependency", "self_dependency"));
        ...
        for (size_t renderSizeNdx = 0; renderSizeNdx < DE_LENGTH_OF_ARRAY(renderSizes); renderSizeNdx++)
        {
            ...
            const SubpassSelfDependencyBackwardsTestConfig testConfig(VK_FORMAT_R8G8B8A8_UNORM,
                                                                      renderSizes[renderSizeNdx], renderPassType);
            renderSizeGroup->addChild(
                new InstanceFactory1<SubpassSelfDependencyBackwardsTestInstance,
                    ...
                    testCtx, tcu::NODETYPE_SELF_VALIDATE, "geometry_to_indirectdraw", "", testConfig));
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> separateChannelsGroup(
            new tcu::TestCaseGroup(testCtx, "separate_channels", "separate_channels"));
        ...
        for (uint32_t configIdx = 0; configIdx < DE_LENGTH_OF_ARRAY(configs); configIdx++)
        {
            ...
            separateChannelsGroup->addChild(new InstanceFactory1<SeparateChannelsTestInstance,
                                                                 SeparateChannelsTestConfig, SeparateChannelsPrograms>(
                testCtx, tcu::NODETYPE_SELF_VALIDATE, configs[configIdx].name, "", testConfig));
        }
    ...
    {
        de::MovePtr<tcu::TestCaseGroup> singleAttachmentGroup(
            new tcu::TestCaseGroup(testCtx, "single_attachment", "single_attachment"));
        ...
        for (uint32_t configIdx = 0; configIdx < DE_LENGTH_OF_ARRAY(configs); configIdx++)
        {
            ...
            singleAttachmentGroup->addChild(new InstanceFactory1<SingleAttachmentTestInstance,
...
{
    return createTestGroup(testCtx, "subpass_dependencies", "Subpass dependency tests", initTests,
