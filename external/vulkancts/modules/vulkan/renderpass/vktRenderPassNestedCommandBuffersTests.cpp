...
private:
	...
	const deUint32						width					= 32u;
	const deUint32						height					= 32u;

	const vk::VkImageSubresourceRange	outputSubresourceRange	= makeImageSubresourceRange(vk::VK_IMAGE_ASPECT_COLOR_BIT, 0u, 1u, 0u, 1u);
	const vk::VkImageSubresourceLayers	outputSubresourceLayers = makeImageSubresourceLayers(vk::VK_IMAGE_ASPECT_COLOR_BIT, 0u, 0u, 1u);
};
...
void NestedCommandBuffersTestInstance::createRenderPass (void)
{
	...
	m_image = de::MovePtr<vk::ImageWithMemory>(new vk::ImageWithMemory(vk, device, alloc, createInfo, vk::MemoryRequirement::Any));
	...
	if (m_testParams.groupParams->renderingType == RENDERING_TYPE_RENDERPASS2 || m_testParams.groupParams->renderingType == RENDERING_TYPE_RENDERPASS_LEGACY)
	{
...
void NestedCommandBuffersTestInstance::beginRenderPass (void)
{
	...
	{
		const auto	preImageBarrier = makeImageMemoryBarrier(vk::VK_ACCESS_NONE, vk::VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, vk::VK_IMAGE_LAYOUT_UNDEFINED, vk::VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, **m_image, outputSubresourceRange);
		vk.cmdPipelineBarrier(*m_cmdBuffer, vk::VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, (vk::VkDependencyFlags)0u, 0u, (const vk::VkMemoryBarrier*)DE_NULL, 0u, (const vk::VkBufferMemoryBarrier*)DE_NULL, 1u, &preImageBarrier);
		...
		vk::VkRenderingInfo renderingInfo =
		{
			...
			DE_NULL,
			vk::VK_RENDERING_CONTENTS_INLINE_BIT_EXT | vk::VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT,	// VkRenderingFlagsKHR					flags;
	...
	else
	{
		...
		vk.cmdBeginRenderPass(*m_cmdBuffer, &renderPassBeginInfo, vk::VK_SUBPASS_CONTENTS_INLINE_AND_SECONDARY_COMMAND_BUFFERS_KHR);
	}
}
...
void NestedCommandBuffersTestInstance::endRenderPass (void)
{
	...
	if (m_testParams.groupParams->renderingType == RENDERING_TYPE_DYNAMIC_RENDERING)
	{
		...
		const auto	postImageBarrier = makeImageMemoryBarrier(vk::VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, vk::VK_ACCESS_TRANSFER_READ_BIT, vk::VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL, vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, **m_image, outputSubresourceRange);
		vk.cmdPipelineBarrier(*m_cmdBuffer, vk::VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, vk::VK_PIPELINE_STAGE_TRANSFER_BIT, (vk::VkDependencyFlags)0u, 0u, (const vk::VkMemoryBarrier*)DE_NULL, 0u, (const vk::VkBufferMemoryBarrier*)DE_NULL, 1u, &postImageBarrier);
	}
	else if (m_testParams.groupParams->renderingType == RENDERING_TYPE_RENDERPASS2)
	{
...
tcu::TestStatus NestedCommandBuffersTestInstance::iterate (void)
{
	...
	vk::GraphicsPipelineWrapper	pipeline(vki, vk, physicalDevice, device, deviceExtensions, vk::PIPELINE_CONSTRUCTION_TYPE_MONOLITHIC);
	...
	for (deUint32 i = 0; i < 3; ++i)
	{
		vk::VkCommandBufferInheritanceRenderingInfo inheritanceRenderingInfo =
		{
			...
			DE_NULL,															// const void*				pNext;
			vk::VK_RENDERING_CONTENTS_INLINE_BIT_EXT | vk::VK_RENDERING_CONTENTS_SECONDARY_COMMAND_BUFFERS_BIT,	// VkRenderingFlags		flags;
			0x0,																// uint32_t				viewMask;
			1u,																	// uint32_t				colorAttachmentCount;
			&format,															// const VkFormat*			pColorAttachmentFormats;
			vk::VK_FORMAT_UNDEFINED,											// VkFormat				depthAttachmentFormat;
		...
		vk::VkCommandBufferBeginInfo commandBufferBeginInfo =
		{
			...
			DE_NULL,											// const void*								pNext;
			(vk::VkCommandBufferUsageFlags)vk::VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT,					// VkCommandBufferUsageFlags				flags;
	...
	vk.cmdCopyImageToBuffer(*m_cmdBuffer, **m_image, vk::VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, **colorOutputBuffer, 1u, &copyRegion);
	vk::endCommandBuffer(vk, *m_cmdBuffer);
	vk::submitCommandsAndWait(vk, device, queue, m_cmdBuffer.get());

	tcu::ConstPixelBufferAccess resultBuffer = tcu::ConstPixelBufferAccess(vk::mapVkFormat(format), width, height, 1, (const void*)colorOutputBuffer->getAllocation().getHostPtr());
	...
	for (deUint32 j = 0; j < height; ++j)
	{
		for (deUint32 i = 0; i < width; ++i)
		{
			...
			if (color != expected)
			{
				log << tcu::TestLog::Message << "Color at (" << i << ", " << j << ") is expected to be (" << expected << "), but was (" << color << ")" << tcu::TestLog::EndMessage;
...
public:
	...
	virtual TestInstance*	createInstance				(Context&			context) const { return new NestedCommandBuffersTestInstance(context, m_testParams); }
...
{
	de::MovePtr<tcu::TestCaseGroup> nestedCommandBuffersGroup(new tcu::TestCaseGroup(testCtx, "nested_command_buffers"));
