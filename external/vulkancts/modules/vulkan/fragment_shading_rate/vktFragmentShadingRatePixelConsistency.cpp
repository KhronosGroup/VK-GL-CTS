...
tcu::TestStatus FSRPixelConsistencyInstance::iterate(void)
{
    ...
    de::MovePtr<vk::DeviceDriver> deviceDriver = de::MovePtr<DeviceDriver>(new DeviceDriver(
        m_context.getPlatformInterface(), m_context.getInstance(), device, m_context.getUsedApiVersion()));
#else
