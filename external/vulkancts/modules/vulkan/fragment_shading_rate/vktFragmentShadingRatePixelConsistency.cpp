...
FSRPixelConsistencyTestCase::FSRPixelConsistencyTestCase(tcu::TestContext &context, const char *name, const char *desc,
                                                         const CaseDef data)
...
void createPixelConsistencyTests(tcu::TestContext &testCtx, tcu::TestCaseGroup *parentGroup)
{
    ...
    TestGroupCase2D shadingRateCases[] = {
        {{1, 1}, "rate_1x1", "1x1 shading rate"}, {{1, 2}, "rate_1x2", "1x2 shading rate"},
        {{1, 4}, "rate_1x4", "1x4 shading rate"}, {{2, 1}, "rate_2x1", "2x1 shading rate"},
        {{2, 2}, "rate_2x2", "2x2 shading rate"}, {{2, 4}, "rate_2x4", "2x4 shading rate"},
        {{4, 1}, "rate_4x1", "4x1 shading rate"}, {{4, 2}, "rate_4x2", "4x2 shading rate"},
    ...
    de::MovePtr<tcu::TestCaseGroup> pixelGroup(
        new tcu::TestCaseGroup(testCtx, "pixel_consistency", "Pixel selection consistency"));

    for (int rateNdx = 0; rateNdx < DE_LENGTH_OF_ARRAY(shadingRateCases); rateNdx++)
    {
        de::MovePtr<tcu::TestCaseGroup> rateGroup(
            new tcu::TestCaseGroup(testCtx, shadingRateCases[rateNdx].name, shadingRateCases[rateNdx].description));

        for (int sampNdx = 0; sampNdx < DE_LENGTH_OF_ARRAY(sampCases); sampNdx++)
        {
            de::MovePtr<tcu::TestCaseGroup> sampleGroup(
                new tcu::TestCaseGroup(testCtx, sampCases[sampNdx].name, sampCases[sampNdx].description));
            for (int extNdx = 0; extNdx < DE_LENGTH_OF_ARRAY(extentCases); extNdx++)
            {
                ...
                sampleGroup->addChild(new FSRPixelConsistencyTestCase(testCtx, extentCases[extNdx].name,
                                                                      extentCases[extNdx].description, caseParams));
                ...
                if ((framebufferExtent.width > 150) && (samples & (VK_SAMPLE_COUNT_1_BIT | VK_SAMPLE_COUNT_4_BIT)))
                {
                    ...
                    sampleGroup->addChild(new FSRPixelConsistencyTestCase(testCtx, caseName.c_str(),
                                                                          extentCases[extNdx].description, caseParams));
                }
            }
