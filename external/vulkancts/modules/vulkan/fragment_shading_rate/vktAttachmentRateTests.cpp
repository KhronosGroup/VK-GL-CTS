...
struct DepthStencilParams
{
    ...
    DepthStencilParams(VkFormat format_, VkImageLayout layout_) : format(format_), layout(layout_)
    {
...
uint32_t calculateRate(uint32_t rateWidth, uint32_t rateHeight)
{
...
private:
    de::MovePtr<ImageWithMemory> buildImageWithMemory(VkDevice device, const DeviceInterface &vk,
                                                      vk::Allocator &allocator, VkFormat format, uint32_t width,
                                                      uint32_t height, VkImageUsageFlags usage,
                                                      VkImageTiling tiling                = VK_IMAGE_TILING_OPTIMAL,
                                                      std::vector<uint32_t> queueFamilies = std::vector<uint32_t>());
    de::MovePtr<BufferWithMemory> buildBufferWithMemory(VkDevice device, const DeviceInterface &vk,
                                                        vk::Allocator &allocator, uint32_t size,
                                                        VkBufferUsageFlags usage);
    Move<VkImageView> buildImageView(VkDevice device, const DeviceInterface &vk, VkFormat format, VkImage image);

    void buildColorBufferObjects(VkDevice device, const DeviceInterface &vk, vk::Allocator &allocator,
                                 uint32_t cbIindex, VkImageUsageFlags cbUsage);
    void buildShadingRateObjects(VkDevice device, const DeviceInterface &vk, vk::Allocator &allocator, uint32_t srIndex,
                                 uint32_t width, uint32_t height, VkImageUsageFlags srUsage,
                                 VkImageTiling srTiling = VK_IMAGE_TILING_OPTIMAL);
    void buildCounterBufferObjects(VkDevice device, const DeviceInterface &vk, vk::Allocator &allocator);

    Move<VkRenderPass> buildRenderPass(VkDevice device, const DeviceInterface &vk, VkFormat cbFormat, VkFormat dsFormat,
                                       uint32_t sr1TileWidth = 0, uint32_t sr1TileHeight = 0, uint32_t sr2TileWidth = 0,
                                       uint32_t sr2TileHeight = 0) const;
    Move<VkFramebuffer> buildFramebuffer(VkDevice device, const DeviceInterface &vk, VkRenderPass renderPass,
                                         const std::vector<FBAttachmentInfo> &attachmentInfo) const;
    Move<VkPipelineLayout> buildPipelineLayout(VkDevice device, const DeviceInterface &vk,
                                               const VkDescriptorSetLayout *setLayouts = nullptr) const;
    Move<VkPipeline> buildGraphicsPipeline(VkDevice device, const DeviceInterface &vk, uint32_t subpass,
                                           VkRenderPass renderPass, VkFormat cbFormat, VkFormat dsFormat,
                                           VkPipelineLayout layout, VkShaderModule vertShader,
                                           VkShaderModule fragShader, bool useShadingRate = VK_TRUE) const;
    Move<VkPipeline> buildComputePipeline(VkDevice device, const DeviceInterface &vk, VkShaderModule compShader,
...
de::MovePtr<ImageWithMemory> AttachmentRateInstance::buildImageWithMemory(VkDevice device, const DeviceInterface &vk,
                                                                          vk::Allocator &allocator, VkFormat format,
                                                                          ...
                                                                          std::vector<uint32_t> queueFamilies)
{
    VkImageCreateInfo imageCreateInfo{
        ...
        queueFamilies.empty() ? VK_SHARING_MODE_EXCLUSIVE : VK_SHARING_MODE_CONCURRENT, // VkSharingMode sharingMode;
    ...
    vk::MemoryRequirement memoryRequirement =
        (tiling == VK_IMAGE_TILING_LINEAR) ? MemoryRequirement::HostVisible : MemoryRequirement::Any;
    return de::MovePtr<ImageWithMemory>(new ImageWithMemory(vk, device, allocator, imageCreateInfo, memoryRequirement));
}

de::MovePtr<BufferWithMemory> AttachmentRateInstance::buildBufferWithMemory(VkDevice device, const DeviceInterface &vk,
                                                                            vk::Allocator &allocator, uint32_t size,
                                                                            VkBufferUsageFlags usage)
{
    ...
    return de::MovePtr<BufferWithMemory>(
        new BufferWithMemory(vk, device, allocator, readBufferInfo, MemoryRequirement::HostVisible));
}

Move<VkImageView> AttachmentRateInstance::buildImageView(VkDevice device, const DeviceInterface &vk, VkFormat format,
                                                         VkImage image)
{
...
void AttachmentRateInstance::buildColorBufferObjects(VkDevice device, const DeviceInterface &vk,
                                                     vk::Allocator &allocator, uint32_t cbIndex,
                                                     VkImageUsageFlags cbUsage)
{
    ...
    m_cbReadBuffer[cbIndex] = buildBufferWithMemory(
        device, vk, allocator, m_cbWidth * m_cbHeight * uint32_t(sizeof(int)) * 4u, VK_BUFFER_USAGE_TRANSFER_DST_BIT);
...
void AttachmentRateInstance::buildShadingRateObjects(VkDevice device, const DeviceInterface &vk,
                                                     vk::Allocator &allocator, uint32_t srIndex, uint32_t width,
                                                     uint32_t height, VkImageUsageFlags srUsage, VkImageTiling srTiling)
{
    DE_ASSERT(srIndex < 2);

    m_srImage[srIndex] =
        buildImageWithMemory(device, vk, allocator, m_params->srFormat, width, height, srUsage, srTiling);
    m_srImageView[srIndex] = buildImageView(device, vk, m_params->srFormat, m_srImage[srIndex]->get());
}

void AttachmentRateInstance::buildCounterBufferObjects(VkDevice device, const vk::DeviceInterface &vk,
                                                       vk::Allocator &allocator)
{
    ...
    const VkDescriptorSetAllocateInfo descriptorSetAllocInfo =
        makeDescriptorSetAllocInfo(*m_counterBufferDescriptorPool, &(*m_counterBufferDescriptorSetLayout));
    m_counterBufferDescriptorSet = allocateDescriptorSet(vk, device, &descriptorSetAllocInfo);

    // create ssbo buffer for atomic counter
    uint32_t ssboSize = uint32_t(sizeof(uint32_t));
    ...
    DescriptorSetUpdateBuilder()
        .writeSingle(*m_counterBufferDescriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u),
                     VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, &descriptorInfo)
        .update(vk, device);

    // reset counter
    *((uint32_t *)m_counterBuffer->getAllocation().getHostPtr()) = 0u;
...
Move<VkRenderPass> AttachmentRateInstance::buildRenderPass(VkDevice device, const vk::DeviceInterface &vk,
                                                           VkFormat cbFormat, VkFormat dsFormat, uint32_t sr0TileWidth,
                                                           uint32_t sr0TileHeight, uint32_t sr1TileWidth,
                                                           uint32_t sr1TileHeight) const
{
    ...
    const uint32_t subpassCount = 1 + useShadingRate1;
...
Move<VkFramebuffer> AttachmentRateInstance::buildFramebuffer(VkDevice device, const vk::DeviceInterface &vk,
                                                             ...
                                                             const std::vector<FBAttachmentInfo> &attachmentInfo) const
{
    ...
    VkFramebufferCreateInfo framebufferParams{
        ...
        (uint32_t)attachmentInfo.size(),               // uint32_t attachmentCount;
    ...
    if (m_params->useImagelessFramebuffer)
    {
        std::vector<VkFramebufferAttachmentImageInfo> framebufferAttachmentImageInfo(
            attachmentInfo.size(),
            {
                ...
                nullptr                                              // const VkFormat* pViewFormats;
            });
...
Move<VkPipelineLayout> AttachmentRateInstance::buildPipelineLayout(VkDevice device, const vk::DeviceInterface &vk,
                                                                   const VkDescriptorSetLayout *setLayouts) const
{
    const VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo{
        ...
        (setLayouts != nullptr),                       // uint32_t setLayoutCount;
...
Move<VkPipeline> AttachmentRateInstance::buildGraphicsPipeline(VkDevice device, const vk::DeviceInterface &vk,
                                                               uint32_t subpass, VkRenderPass renderPass,
                                                               VkFormat cbFormat, VkFormat dsFormat,
                                                               VkPipelineLayout pipelineLayout,
                                                               VkShaderModule vertShader, VkShaderModule fragShader,
                                                               bool useShadingRate) const
{
    const auto dsAspects        = getFormatAspectFlags(dsFormat);
    const auto hasDepth         = ((dsAspects & VK_IMAGE_ASPECT_DEPTH_BIT) != 0);
    const auto hasStencil       = ((dsAspects & VK_IMAGE_ASPECT_STENCIL_BIT) != 0);
    ...
    void *pNext = useShadingRate ? &shadingRateStateCreateInfo : nullptr;
#ifndef CTS_USES_VULKANSC
    VkPipelineRenderingCreateInfoKHR renderingCreateInfo{
        VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR, pNext, 0u, 1u, &cbFormat, dsFormat, VK_FORMAT_UNDEFINED};
...
VkDescriptorSetAllocateInfo AttachmentRateInstance::makeDescriptorSetAllocInfo(
    VkDescriptorPool descriptorPool, const VkDescriptorSetLayout *pSetLayouts) const
{
    return {
                                            ...
                                            const uint32_t srTileWidth, const uint32_t srTileHeight) const
{
    ...
    beginRenderPass(vk, commandBuffer, renderPass, framebuffer, renderArea, (uint32_t)clearColor.size(),
                    clearColor.data(), VK_SUBPASS_CONTENTS_INLINE, pNext);
}
...
tcu::TestStatus AttachmentRateInstance::iterate(void)
{
    ...
    const std::map<TestMode, MethodPtr> modeFuncMap{
        ...
        {TM_SETUP_RATE_WITH_COPYING_FROM_EXCLUSIVE_IMAGE_USING_TRANSFER_QUEUE,
         &AttachmentRateInstance::runCopyModeOnTransferQueue},
        {TM_SETUP_RATE_WITH_COPYING_FROM_CONCURENT_IMAGE_USING_TRANSFER_QUEUE,
         &AttachmentRateInstance::runCopyModeOnTransferQueue},
...
bool AttachmentRateInstance::verifyUsingAtomicChecks(uint32_t tileWidth, uint32_t tileHeight, uint32_t rateWidth,
                                                     uint32_t rateHeight, uint32_t *outBufferPtr) const
{
    tcu::TestLog &log(m_context.getTestContext().getLog());
    tcu::TextureLevel errorMaskStorage(tcu::TextureFormat(tcu::TextureFormat::RGB, tcu::TextureFormat::UNORM_INT8),
                                       m_cbWidth, m_cbHeight, 1u);
    tcu::PixelBufferAccess errorMaskAccess(errorMaskStorage.getAccess());

    uint32_t wrongFragments                          = 0;
    const uint32_t fragmentsWithSameAtomicValueCount = rateWidth * rateHeight;

    // map that uses atomic value as a kay and maps it to all fragments sharing same atomic
    std::map<uint32_t, std::vector<tcu::UVec2>> fragmentsWithSameAtomicValueMap;
    ...
    uint32_t triangleLeftEdgeX = 0;
    uint32_t triangleTopEdgeY  = 0;
    ...
    std::vector<uint32_t> fragmentsOnTheRightTriangleEdgeVect(m_cbHeight, 0);
    std::vector<uint32_t> fragmentsOnTheBottomTriangleEdgeVect(m_cbWidth, 0);

    tcu::clear(errorMaskAccess, tcu::Vec4(0.0f, 0.0f, 0.0f, 1.0));

    // loop over all fragments and validate the output
    for (uint32_t cbFragmentY = 0; cbFragmentY < m_cbHeight; ++cbFragmentY)
        for (uint32_t cbFragmentX = 0; cbFragmentX < m_cbWidth; ++cbFragmentX)
        {
            uint32_t *fragmentColor = &outBufferPtr[4 * (cbFragmentY * m_cbWidth + cbFragmentX)];
            ...
            uint32_t rate          = fragmentColor[0];
            uint32_t fragmentRateX = 1 << ((rate / 4) & 3);
            uint32_t fragmentRateY = 1 << (rate & 3);

            // check if proper rate was used for fragment
            if ((fragmentRateX != rateWidth) || (fragmentRateY != rateHeight))
            {
                ++wrongFragments;
            ...
            uint32_t atomicValue = fragmentColor[2];
            errorMaskAccess.setPixel(tcu::Vec4(0.0f, 1.0f - float(atomicValue % 7) * 0.1f, 0.0f, 1.0f), cbFragmentX,
                                     cbFragmentY, 0u);
    ...
    if ((triangleTopEdgeY + triangleLeftEdgeX) == 0)
    {
        log << tcu::TestLog::Message << "No valid fragments." << tcu::TestLog::EndMessage;
        return false;
    }
    ...
    if (wrongFragments)
    {
        log << tcu::TestLog::Message << "Failed " << wrongFragments << " fragments for tileWidth: " << tileWidth
            << ", tileHeight: " << tileHeight << tcu::TestLog::EndMessage
    ...
    for (auto &fragmentsSetMapIt : fragmentsWithSameAtomicValueMap)
    {
        ...
        if (fragmentSet.size() != fragmentsWithSameAtomicValueCount)
        {
            ...
            uint32_t triangleRightEdgeX  = fragmentsOnTheRightTriangleEdgeVect[topLeftFragment.y()];
            uint32_t triangleBottomEdgeY = fragmentsOnTheBottomTriangleEdgeVect[topLeftFragment.x()];
            ...
            if ((topLeftFragment.y() > (triangleTopEdgeY)) && (topLeftFragment.x() > (triangleLeftEdgeX)) &&
                (topLeftFragment.x() < (triangleRightEdgeX - rateWidth)) &&
                (topLeftFragment.y() < (triangleBottomEdgeY - rateHeight)))
            {
                wrongFragments += (uint32_t)fragmentSet.size();
    ...
    if (wrongFragments)
    {
        log << tcu::TestLog::Message << "Wrong number of fragments with same atomic value (" << wrongFragments
            << ") for tileWidth: " << tileWidth << ", tileHeight: " << tileHeight << tcu::TestLog::EndMessage
            << tcu::TestLog::Image("ErrorMask", "Error mask", errorMaskAccess);
        return false;
    }
...
bool AttachmentRateInstance::runComputeShaderMode(void)
{
    ...
    Move<VkDescriptorPool> computeDescriptorPool =
        DescriptorPoolBuilder()
            ...
            .build(vk, device, VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT, 1u);
    const VkDescriptorSetAllocateInfo computeDescriptorSetAllocInfo =
        makeDescriptorSetAllocInfo(*computeDescriptorPool, &(*computeDescriptorSetLayout));
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            // skip tile sizes that have unsuported aspect ratio
            uint32_t aspectRatio = (tileHeight > tileWidth) ? (tileHeight / tileWidth) : (tileWidth / tileHeight);
            if (aspectRatio > m_maxAspectRatio)
                continue;

            // calculate size of shading rate attachment
            uint32_t srWidth  = (m_cbWidth + tileWidth - 1) / tileWidth;
            uint32_t srHeight = (m_cbHeight + tileHeight - 1) / tileHeight;
            ...
            Move<VkPipelineLayout> computePipelineLayout =
                buildPipelineLayout(device, vk, &(*computeDescriptorSetLayout));
            Move<VkPipelineLayout> graphicsPipelineLayout =
                buildPipelineLayout(device, vk, &(*m_counterBufferDescriptorSetLayout));
            ...
            Move<VkPipeline> graphicsPipeline = buildGraphicsPipeline(
                device, vk, 0, *renderPass, m_cbFormat, dsFormat, *graphicsPipelineLayout, *vertShader, *fragShader);
            ...
            VkImageMemoryBarrier srImageBarrierGeneral =
                makeImageMemoryBarrier(VK_ACCESS_NONE_KHR, VK_ACCESS_NONE_KHR, VK_IMAGE_LAYOUT_UNDEFINED,
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 1,
                                  &srImageBarrierGeneral);

            // fill sr image using compute shader
            vk.cmdBindPipeline(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipeline);
            vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, *computePipelineLayout, 0, 1,
            ...
            VkImageMemoryBarrier cbImageBarrier = makeImageMemoryBarrier(
                VK_ACCESS_NONE_KHR, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                VK_IMAGE_LAYOUT_GENERAL, **m_cbImage[0], m_defaultImageSubresourceRange);
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 1,
                                  &cbImageBarrier);
            ...
            vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *graphicsPipelineLayout, 0, 1,
                                     &(*m_counterBufferDescriptorSet), 0, nullptr);
            ...
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 1, &memoryBarrier, 0, nullptr, 0, nullptr);

            // read back color buffer image
            vk.cmdCopyImageToBuffer(*cmdBuffer, **m_cbImage[0], VK_IMAGE_LAYOUT_GENERAL, **m_cbReadBuffer[0], 1u,
                                    &m_defaultBufferImageCopy);
...
bool AttachmentRateInstance::runFragmentShaderMode(void)
{
    ...
    Move<VkShaderModule> vertSetupShader =
        createShaderModule(vk, device, m_context.getBinaryCollection().get("vert_setup"), 0);
    Move<VkShaderModule> fragSetupShader =
        createShaderModule(vk, device, m_context.getBinaryCollection().get("frag_setup"), 0);
    Move<VkShaderModule> vertShader = createShaderModule(vk, device, m_context.getBinaryCollection().get("vert"), 0);
    Move<VkShaderModule> fragShader = createShaderModule(vk, device, m_context.getBinaryCollection().get("frag"), 0);
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            Move<VkPipelineLayout> ratePipelineLayout =
                buildPipelineLayout(device, vk, &(*m_counterBufferDescriptorSetLayout));
            Move<VkRenderPass> setupRenderPass = buildRenderPass(device, vk, m_params->srFormat, VK_FORMAT_UNDEFINED);
            Move<VkRenderPass> rateRenderPass =
                buildRenderPass(device, vk, m_cbFormat, dsFormat, tileWidth, tileHeight);
            Move<VkPipeline> setupPipeline =
                buildGraphicsPipeline(device, vk, 0, *setupRenderPass, m_params->srFormat, VK_FORMAT_UNDEFINED,
                                      *setupPipelineLayout, *vertSetupShader, *fragSetupShader, false);
            Move<VkPipeline> ratePipeline = buildGraphicsPipeline(device, vk, 0, *rateRenderPass, m_cbFormat, dsFormat,
                                                                  *ratePipelineLayout, *vertShader, *fragShader);
            ...
            if (m_params->useDepthStencil())
                rateAttachmentInfo.push_back(
                    FBAttachmentInfo{dsFormat, kDSUsage, m_cbWidth, m_cbHeight, *m_dsImageView});
            ...
            VkImageMemoryBarrier srImageBarrierGeneral = makeImageMemoryBarrier(
                VK_ACCESS_NONE_KHR, useMemoryAccess ? VK_ACCESS_MEMORY_WRITE_BIT : VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, **m_srImage[0], m_defaultImageSubresourceRange);
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 1,
                                  &srImageBarrierGeneral);
            ...
            if (m_params->useDepthStencil())
            {
                ...
                VkImageMemoryBarrier depthImageReadOnlyBarrier = makeImageMemoryBarrier(
                    VK_ACCESS_NONE_KHR, VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                    m_params->getDSLayout(), **m_dsImage, m_dsImageSubresourceRange);
                vk.cmdPipelineBarrier(*cmdBuffer, srcStage, dstStage, 0, 0, nullptr, 0, nullptr, 1,
                                      &depthImageReadOnlyBarrier);
            }

            // render rate to sr image
            startRendering(*cmdBuffer, *setupRenderPass, *setupFramebuffer, makeRect2D(srWidth, srHeight),
                           setupAttachmentInfo);
            ...
            VkImageMemoryBarrier cbImageBarrier = makeImageMemoryBarrier(
                VK_ACCESS_NONE_KHR, useMemoryAccess ? VK_ACCESS_MEMORY_WRITE_BIT : VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT,
                VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, **m_cbImage[0], m_defaultImageSubresourceRange);
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 1,
                                  &cbImageBarrier);

            startRendering(*cmdBuffer, *rateRenderPass, *rateFramebuffer, makeRect2D(m_cbWidth, m_cbHeight),
                           rateAttachmentInfo, tileWidth, tileHeight);

            // draw single triangle to cb
            vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *ratePipelineLayout, 0, 1,
                                     &(*m_counterBufferDescriptorSet), 0, nullptr);
...
bool AttachmentRateInstance::runCopyMode(void)
{
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            de::MovePtr<ImageWithMemory> srSrcImage =
                buildImageWithMemory(device, vk, m_context.getDefaultAllocator(), m_params->srFormat, srWidth, srHeight,
            ...
            std::vector<VkImageMemoryBarrier> srImageBarrierGeneral(
                2,
                makeImageMemoryBarrier(VK_ACCESS_NONE_KHR, (VK_ACCESS_TRANSFER_WRITE_BIT | VK_ACCESS_TRANSFER_READ_BIT),
                                       VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, **m_srImage[0],
                                       m_defaultImageSubresourceRange));
            srImageBarrierGeneral[1].image = **srSrcImage;
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 2,
                                  srImageBarrierGeneral.data());
            ...
            vk.cmdClearColorImage(*cmdBuffer, **srSrcImage, VK_IMAGE_LAYOUT_GENERAL, &clearValue, 1,
                                  &m_defaultImageSubresourceRange);
            ...
            vk.cmdCopyImage(*cmdBuffer, **srSrcImage, VK_IMAGE_LAYOUT_GENERAL, **m_srImage[0], VK_IMAGE_LAYOUT_GENERAL,
                            1, &imageCopyRegion);
...
bool AttachmentRateInstance::runCopyModeOnTransferQueue(void)
{
    ...
    for (uint32_t queueNdx = 0; queueNdx < queueFamilyProperties.size(); queueNdx++)
    {
        ...
        if ((graphicsQueueFamilyIndex == std::numeric_limits<uint32_t>::max()) && (queueFlags & VK_QUEUE_GRAPHICS_BIT))
...

    {
        ...
        VkPhysicalDeviceFragmentShadingRateFeaturesKHR fsrFeatures{
            VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_SHADING_RATE_FEATURES_KHR, nullptr, false, false, true};
#ifndef CTS_USES_VULKANSC
        VkPhysicalDeviceDynamicRenderingFeaturesKHR drFeatures{
            VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DYNAMIC_RENDERING_FEATURES_KHR, nullptr, true};
#endif // CTS_USES_VULKANSC
        VkPhysicalDeviceImagelessFramebufferFeatures ifbFeatures{
            VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_IMAGELESS_FRAMEBUFFER_FEATURES, nullptr, true};
        VkPhysicalDeviceFeatures2 createPhysicalFeature{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2, &fsrFeatures,
                                                        deviceFeatures};
    ...
    Move<VkCommandPool> transferCmdPool =
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, transferQueueFamilyIndex);
    Move<VkCommandBuffer> transferCmdBuffer =
        allocateCommandBuffer(vk, device, *transferCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
    Move<VkCommandPool> graphicsCmdPool =
        createCommandPool(vk, device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, graphicsQueueFamilyIndex);
    Move<VkCommandBuffer> graphicsCmdBuffer =
        allocateCommandBuffer(vk, device, *graphicsCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            // create image that will be source for shading rate image
            de::MovePtr<ImageWithMemory> srSrcImage =
                buildImageWithMemory(device, vk, *allocator, m_params->srFormat, srWidth, srHeight,
                                     VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT);

            // create buffer that will contain shading rate source data
            tcu::TextureFormat srTextureFormat = mapVkFormat(m_params->srFormat);
            uint32_t srWriteBufferSize =
                srWidth * srHeight * getNumUsedChannels(srTextureFormat.order) * getChannelSize(srTextureFormat.type);
            de::MovePtr<BufferWithMemory> srSrcBuffer =
                buildBufferWithMemory(device, vk, *allocator, srWriteBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);

            // fill buffer with tested shading rate
            uint8_t *srWriteBufferHostPtr = (uint8_t *)srSrcBuffer->getAllocation().getHostPtr();
            uint8_t value                 = (uint8_t)calculateRate(m_params->srRate.width, m_params->srRate.height);
            ...
            std::vector<VkImageMemoryBarrier> srImageBarrierGeneral(
                2,
                makeImageMemoryBarrier(VK_ACCESS_NONE_KHR, (VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT),
                                       VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, **m_srImage[0],
                                       m_defaultImageSubresourceRange));
            srImageBarrierGeneral[1].image = **srSrcImage;
            vk.cmdPipelineBarrier(*transferCmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 2,
                                  srImageBarrierGeneral.data());

            // copy sr data to images
            const VkBufferImageCopy srCopyBuffer =
                makeBufferImageCopy({srWidth, srHeight, 1u}, m_defaultImageSubresourceLayers);
            vk.cmdCopyBufferToImage(*transferCmdBuffer, **srSrcBuffer, **srSrcImage, VK_IMAGE_LAYOUT_GENERAL, 1,
                                    &srCopyBuffer);
            ...
            memoryBarrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
            memoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
            vk.cmdPipelineBarrier(*transferCmdBuffer, srcStageMask, dstStageMask, 0, 1, &memoryBarrier, 0, nullptr, 0,
                                  nullptr);
            ...
            vk.cmdCopyImage(*transferCmdBuffer, **srSrcImage, VK_IMAGE_LAYOUT_GENERAL, **m_srImage[0],
            ...
            vk.cmdPipelineBarrier(*transferCmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 1,
            ...
            memoryBarrier.srcAccessMask = VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT;
            memoryBarrier.dstAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
            vk.cmdPipelineBarrier(*graphicsCmdBuffer, srcStageMask, dstStageMask, 0, 1, &memoryBarrier, 0, nullptr, 0,
                                  nullptr);

            // read back color buffer image
            vk.cmdCopyImageToBuffer(*graphicsCmdBuffer, **m_cbImage[0], VK_IMAGE_LAYOUT_GENERAL, **m_cbReadBuffer[0],
                                    1u, &m_defaultBufferImageCopy);
...
bool AttachmentRateInstance::runFillLinearTiledImage(void)
{
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            uint8_t *imagePtr = reinterpret_cast<uint8_t *>(m_srImage[0]->getAllocation().getHostPtr());
            uint8_t value     = (uint8_t)calculateRate(m_params->srRate.width, m_params->srRate.height);

            // fill sr image on the host row by row
            vk.getImageSubresourceLayout(device, **m_srImage[0], &imageSubresource, &srImageLayout);
            for (uint32_t srTexelRow = 0; srTexelRow < srHeight; srTexelRow++)
            {
                uint8_t *rowDst = imagePtr + srImageLayout.offset + srImageLayout.rowPitch * srTexelRow;
...
bool AttachmentRateInstance::runTwoSubpassMode(void)
{
    ...
    uint32_t supportedFragmentShadingRateCount;
    std::vector<VkPhysicalDeviceFragmentShadingRateKHR> supportedFragmentShadingRates;
    vki.getPhysicalDeviceFragmentShadingRatesKHR(pd, &supportedFragmentShadingRateCount, nullptr);
    supportedFragmentShadingRates.resize(
    ...
    vki.getPhysicalDeviceFragmentShadingRatesKHR(pd, &supportedFragmentShadingRateCount,
                                                 &supportedFragmentShadingRates[0]);

    // grab min and max tile sieze and biggest and smallest rate
    uint32_t sr0Width      = (m_cbWidth + m_minTileSize.width - 1) / m_minTileSize.width;
    uint32_t sr0Height     = (m_cbHeight + m_minTileSize.height - 1) / m_minTileSize.height;
    uint32_t sr1Width      = (m_cbWidth + m_maxTileSize.width - 1) / m_maxTileSize.width;
    uint32_t sr1Height     = (m_cbHeight + m_maxTileSize.height - 1) / m_maxTileSize.height;
    uint32_t sr0RateWidth  = supportedFragmentShadingRates[0].fragmentSize.width; // bigets supported rate
    uint32_t sr0RateHeight = supportedFragmentShadingRates[0].fragmentSize.height;
    uint32_t sr1RateWidth  = supportedFragmentShadingRates[supportedFragmentShadingRateCount - 2]
                                .fragmentSize.width; // smallest supported rate excluding {1, 1}
    uint32_t sr1RateHeight = supportedFragmentShadingRates[supportedFragmentShadingRateCount - 2].fragmentSize.height;
    ...
    const auto dsFormat                   = m_params->getDSFormat();
    Move<VkRenderPass> renderPass         = buildRenderPass(device, vk, m_cbFormat, dsFormat, m_minTileSize.width,
                                                            m_minTileSize.height, m_maxTileSize.width, m_maxTileSize.height);
    Move<VkPipelineLayout> pipelineLayout = buildPipelineLayout(device, vk, &(*m_counterBufferDescriptorSetLayout));
    Move<VkPipeline> graphicsPipeline0    = buildGraphicsPipeline(device, vk, 0, *renderPass, m_cbFormat, dsFormat,
                                                                  *pipelineLayout, *vertShader0, *fragShader);
    Move<VkPipeline> graphicsPipeline1    = buildGraphicsPipeline(device, vk, 1, *renderPass, m_cbFormat, dsFormat,
                                                                  *pipelineLayout, *vertShader1, *fragShader);
    ...
    VkPipelineStageFlags dstStageMask = VK_PIPELINE_STAGE_TRANSFER_BIT;
    std::vector<VkImageMemoryBarrier> srImageBarrierGeneral(
        2, makeImageMemoryBarrier(VK_ACCESS_NONE_KHR, (VK_ACCESS_TRANSFER_READ_BIT | VK_ACCESS_TRANSFER_WRITE_BIT),
                                  VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_GENERAL, **m_srImage[0],
                                  m_defaultImageSubresourceRange));
    ...
    vk.cmdClearColorImage(*cmdBuffer, **m_srImage[0], VK_IMAGE_LAYOUT_GENERAL, &clearValues[0], 1,
                          &m_defaultImageSubresourceRange);
    vk.cmdClearColorImage(*cmdBuffer, **m_srImage[1], VK_IMAGE_LAYOUT_GENERAL, &clearValues[1], 1,
                          &m_defaultImageSubresourceRange);
    ...
    memoryBarrier.srcAccessMask = VK_ACCESS_TRANSFER_WRITE_BIT;
    memoryBarrier.dstAccessMask = VK_ACCESS_FRAGMENT_SHADING_RATE_ATTACHMENT_READ_BIT_KHR;
    std::vector<VkImageMemoryBarrier> srImageBarrierShadingRate(
        2, makeImageMemoryBarrier(VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_FRAGMENT_SHADING_RATE_ATTACHMENT_READ_BIT_KHR,
    ...
    std::vector<VkImageMemoryBarrier> cbImageBarrier(
        2, makeImageMemoryBarrier(VK_ACCESS_NONE_KHR, VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                                  VK_IMAGE_LAYOUT_GENERAL, **m_cbImage[0], m_defaultImageSubresourceRange));
    cbImageBarrier[1].image = **m_cbImage[1];
    vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, nullptr, 0, nullptr, 2, cbImageBarrier.data());

    startRendering(*cmdBuffer, *renderPass, *framebuffer, makeRect2D(m_cbWidth, m_cbHeight), attachmentInfo);

    // draw single triangle to first cb
    vk.cmdBindDescriptorSets(*cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, *pipelineLayout, 0, 1,
                             &(*m_counterBufferDescriptorSet), 0, nullptr);
    ...
    vk.cmdCopyImageToBuffer(*cmdBuffer, **m_cbImage[0], VK_IMAGE_LAYOUT_GENERAL, **m_cbReadBuffer[0], 1u,
                            &m_defaultBufferImageCopy);
    vk.cmdCopyImageToBuffer(*cmdBuffer, **m_cbImage[1], VK_IMAGE_LAYOUT_GENERAL, **m_cbReadBuffer[1], 1u,
                            &m_defaultBufferImageCopy);
    ...
    return (verifyUsingAtomicChecks(m_minTileSize.width, m_minTileSize.height, sr0RateWidth, sr0RateHeight,
                                    (uint32_t *)m_cbReadBuffer[0]->getAllocation().getHostPtr()) &&
            verifyUsingAtomicChecks(m_maxTileSize.width, m_maxTileSize.height, sr1RateWidth, sr1RateHeight,
                                    (uint32_t *)m_cbReadBuffer[1]->getAllocation().getHostPtr()));
}
...
AttachmentRateTestCase::AttachmentRateTestCase(tcu::TestContext &context, const char *name,
                                               de::SharedPtr<TestParams> params)
...
void AttachmentRateTestCase::checkSupport(Context &context) const
{
    ...
    VkResult result = vk.getPhysicalDeviceImageFormatProperties(pd, m_params->srFormat, VK_IMAGE_TYPE_2D,
                                                                VK_IMAGE_TILING_OPTIMAL, srUsage, 0, &imageProperties);
    ...
    if (m_params->mode != TM_TWO_SUBPASS)
    {
        ...
        // fetch information about supported rates
        vk.getPhysicalDeviceFragmentShadingRatesKHR(pd, &supportedFragmentShadingRateCount, nullptr);
        supportedFragmentShadingRates.resize(
            supportedFragmentShadingRateCount,
            {
                VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FRAGMENT_SHADING_RATE_KHR, // VkStructureType sType;
        ...
        vk.getPhysicalDeviceFragmentShadingRatesKHR(pd, &supportedFragmentShadingRateCount,
                                                    &supportedFragmentShadingRates[0]);
        ...
        if (std::none_of(supportedFragmentShadingRates.begin(), supportedFragmentShadingRates.end(),
                         [&testedRate](const VkPhysicalDeviceFragmentShadingRateKHR &r) {
                             return (r.fragmentSize.width == testedRate.width &&
                                     r.fragmentSize.height == testedRate.height);
                         }))
        {
    ...
    if (m_params->mode == TM_SETUP_RATE_WITH_ATOMICS_IN_COMPUTE_SHADER)
        requiredFeatures =
            VK_FORMAT_FEATURE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR | VK_FORMAT_FEATURE_STORAGE_IMAGE_ATOMIC_BIT;
        ...
        requiredFeatures =
            VK_FORMAT_FEATURE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR | VK_FORMAT_FEATURE_COLOR_ATTACHMENT_BIT;

#if DEBUG_USE_STORE_INSTEAD_OF_ATOMICS == 1
    if (m_params->mode == TM_SETUP_RATE_WITH_ATOMICS_IN_COMPUTE_SHADER)
        requiredFeatures =
            VK_FORMAT_FEATURE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR | VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT;
#endif
...
{
    uint32_t rateValue = calculateRate(m_params->srRate.width, m_params->srRate.height);

    if (m_params->mode == TM_SETUP_RATE_WITH_ATOMICS_IN_COMPUTE_SHADER)
    {
        std::stringstream compStream;
        compStream << "#version 450\n"
                      "layout(local_size_x = 1, local_size_y = 1, local_size_z = 1) in;\n"
                      "layout(r32ui, binding = 0) coherent uniform highp uimage2D srImage;\n"
                      "void main (void)\n"
                      "{\n"
#if DEBUG_USE_STORE_INSTEAD_OF_ATOMICS == 1
                      "  imageStore(srImage, ivec2(gl_GlobalInvocationID.xy), uvec4("
                   << rateValue
                   << "));\n"
#else
                      "  imageAtomicAdd(srImage, ivec2(gl_GlobalInvocationID.xy), "
                   << rateValue
                   << ");\n"
#endif
                      "}\n";
    ...
    if ((m_params->mode == TM_SETUP_RATE_WITH_FRAGMENT_SHADER) || (m_params->mode == TM_MEMORY_ACCESS) ||
        (m_params->mode == TM_MAINTENANCE_5))
    {
        // use large triangle that will cover whole color buffer
        specializationMap["SCALE"]     = "9.0";
        specializationMap["TRANSLATE"] = "0.0";
        programCollection.glslSources.add("vert_setup")
            << glu::VertexSource(vertTemplate.specialize(specializationMap));

        std::stringstream fragStream;
        fragStream << "#version 450 core\n"
                               ...
                               SharedGroupParams groupParams)
{
    ...
    const std::vector<SRRate> srRates{
        {{1, 1}, "rate_1x1"}, {{1, 2}, "rate_1x2"}, {{1, 4}, "rate_1x4"}, {{2, 1}, "rate_2x1"}, {{2, 2}, "rate_2x2"},
        {{2, 4}, "rate_2x4"}, {{4, 1}, "rate_4x1"}, {{4, 2}, "rate_4x2"}, {{4, 4}, "rate_4x4"},
    };
        ...
        for (const auto &srFormat : srFormats)
        {
            ...
            for (const auto &srRate : srRates)
            {
                formatGroup->addChild(
                    new AttachmentRateTestCase(testCtx, srRate.name,
    ...
    if (!groupParams->useDynamicRendering)
    {
        miscGroup->addChild(new AttachmentRateTestCase(
            testCtx, "two_subpass",
            de::SharedPtr<TestParams>(new TestParams{
                                                       ...
                                                       })));
        {
            ...
            {
                const auto dsFormat =
                    ((layout == VK_IMAGE_LAYOUT_STENCIL_READ_ONLY_OPTIMAL) ? VK_FORMAT_S8_UINT : VK_FORMAT_D16_UNORM);
                const std::string layoutName = de::toLower(de::toString(layout).substr(skip));
                const std::string testName   = "ro_ds_" + layoutName;

                miscGroup->addChild(
                    new AttachmentRateTestCase(testCtx, testName.c_str(),
                                               de::SharedPtr<TestParams>(new TestParams{
                                                   TM_MEMORY_ACCESS,  // TestMode mode;
                                                   VK_FORMAT_R8_UINT, // VkFormat srFormat;
    ...
    {
#ifndef CTS_USES_VULKANSC
        miscGroup->addChild(new AttachmentRateTestCase(testCtx, "maintenance5",
                                                       de::SharedPtr<TestParams>(new TestParams{
