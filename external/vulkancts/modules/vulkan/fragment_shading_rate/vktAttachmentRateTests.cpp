...
private:
    de::MovePtr<ImageWithMemory> buildImageWithMemory(VkFormat format, uint32_t width, uint32_t height,
                                                      VkImageUsageFlags usage,
                                                      VkImageTiling tiling                = VK_IMAGE_TILING_OPTIMAL,
                                                      std::vector<uint32_t> queueFamilies = std::vector<uint32_t>());
    de::MovePtr<BufferWithMemory> buildBufferWithMemory(uint32_t size, VkBufferUsageFlags usage);
    Move<VkImageView> buildImageView(VkFormat format, VkImage image);

    void buildColorBufferObjects(uint32_t cbIindex, VkImageUsageFlags cbUsage);
    void buildShadingRateObjects(uint32_t srIndex, uint32_t width, uint32_t height, VkImageUsageFlags srUsage,
                                 VkImageTiling srTiling = VK_IMAGE_TILING_OPTIMAL);
    void buildCounterBufferObjects();

    Move<VkRenderPass> buildRenderPass(VkFormat cbFormat, uint32_t sr1TileWidth = 0, uint32_t sr1TileHeight = 0,
                                       uint32_t sr2TileWidth = 0, uint32_t sr2TileHeight = 0) const;
    ...
    Move<VkPipeline> buildGraphicsPipeline(uint32_t subpass, VkRenderPass renderPass, VkFormat cbFormat,
                                           VkPipelineLayout layout, VkShaderModule vertShader,
                                           VkShaderModule fragShader, bool useShadingRate = VK_TRUE) const;
    Move<VkPipeline> buildComputePipeline(VkShaderModule compShader, VkPipelineLayout pipelineLayout) const;
...
de::MovePtr<ImageWithMemory> AttachmentRateInstance::buildImageWithMemory(VkFormat format, uint32_t width,
...
de::MovePtr<BufferWithMemory> AttachmentRateInstance::buildBufferWithMemory(uint32_t size, VkBufferUsageFlags usage)
{
    const DeviceInterface &vk               = m_context.getDeviceInterface();
    VkDevice device                         = m_device.get() ? *m_device : m_context.getDevice();
    vk::Allocator &allocator                = m_allocator.get() ? *m_allocator : m_context.getDefaultAllocator();
...
void AttachmentRateInstance::buildColorBufferObjects(uint32_t cbIndex, VkImageUsageFlags cbUsage)
{
    DE_ASSERT(cbIndex < 2);
    ...
    m_cbReadBuffer[cbIndex] =
        buildBufferWithMemory(m_cbWidth * m_cbHeight * uint32_t(sizeof(int)) * 4u, VK_BUFFER_USAGE_TRANSFER_DST_BIT);
}

void AttachmentRateInstance::buildShadingRateObjects(uint32_t srIndex, uint32_t width, uint32_t height,
                                                     VkImageUsageFlags srUsage, VkImageTiling srTiling)
{
    DE_ASSERT(srIndex < 2);
...
Move<VkRenderPass> AttachmentRateInstance::buildRenderPass(VkFormat cbFormat, uint32_t sr0TileWidth,
...
Move<VkFramebuffer> AttachmentRateInstance::buildFramebuffer(VkRenderPass renderPass,
                                                             const std::vector<FBAttachmentInfo> &attachmentInfo) const
{
    if (m_params->useDynamicRendering)
...
Move<VkPipeline> AttachmentRateInstance::buildGraphicsPipeline(uint32_t subpass, VkRenderPass renderPass,
                                                               VkFormat cbFormat, VkPipelineLayout pipelineLayout,
...
Move<VkPipeline> AttachmentRateInstance::buildComputePipeline(VkShaderModule compShader,
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            Move<VkPipeline> graphicsPipeline =
                buildGraphicsPipeline(0, *renderPass, m_cbFormat, *graphicsPipelineLayout, *vertShader, *fragShader);
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            Move<VkPipeline> setupPipeline =
                buildGraphicsPipeline(0, *setupRenderPass, m_params->srFormat, *setupPipelineLayout, *vertSetupShader,
                                      *fragSetupShader, false);
            Move<VkPipeline> ratePipeline =
                buildGraphicsPipeline(0, *rateRenderPass, m_cbFormat, *ratePipelineLayout, *vertShader, *fragShader);
            ...
            dstStageMask                                   = VK_PIPELINE_STAGE_FRAGMENT_SHADING_RATE_ATTACHMENT_BIT_KHR;
            VkImageMemoryBarrier srImageBarrierShadingRate = makeImageMemoryBarrier(
                VK_ACCESS_COLOR_ATTACHMENT_WRITE_BIT, VK_ACCESS_FRAGMENT_SHADING_RATE_ATTACHMENT_READ_BIT_KHR,
                VK_IMAGE_LAYOUT_GENERAL, VK_IMAGE_LAYOUT_FRAGMENT_SHADING_RATE_ATTACHMENT_OPTIMAL_KHR, **m_srImage[0],
                m_defaultImageSubresourceRange);
            vk.cmdPipelineBarrier(*cmdBuffer, srcStageMask, dstStageMask, 0, 0, DE_NULL, 0, DE_NULL, 1,
                                  &srImageBarrierShadingRate);
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            de::MovePtr<ImageWithMemory> srSrcImage =
                buildImageWithMemory(m_params->srFormat, srWidth, srHeight,
...
bool AttachmentRateInstance::runCopyModeOnTransferQueue(void)
{
    ...
    // create custom device
    {
        ...
        m_allocator = de::MovePtr<Allocator>(
            new SimpleAllocator(m_context.getDeviceInterface(), *m_device, getPhysicalDeviceMemoryProperties(vki, pd)));
    }
    ...
    Move<VkCommandPool> transferCmdPool =
        createCommandPool(vk, *m_device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, transferQueueFamilyIndex);
    Move<VkCommandBuffer> transferCmdBuffer =
        allocateCommandBuffer(vk, *m_device, *transferCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
    Move<VkCommandPool> graphicsCmdPool =
        createCommandPool(vk, *m_device, VK_COMMAND_POOL_CREATE_RESET_COMMAND_BUFFER_BIT, graphicsQueueFamilyIndex);
    Move<VkCommandBuffer> graphicsCmdBuffer =
        allocateCommandBuffer(vk, *m_device, *graphicsCmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
    ...
    for (uint32_t tileWidth = m_minTileSize.width; tileWidth <= m_maxTileSize.width; tileWidth *= 2)
        for (uint32_t tileHeight = m_minTileSize.height; tileHeight <= m_maxTileSize.height; tileHeight *= 2)
        {
            ...
            de::MovePtr<BufferWithMemory> srSrcBuffer =
                buildBufferWithMemory(srWriteBufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT);
            ...
            VK_CHECK(vk.waitForFences(*m_device, 2u, fences, true, ~0ull));
...
bool AttachmentRateInstance::runTwoSubpassMode(void)
{
    ...
    Move<VkRenderPass> renderPass         = buildRenderPass(m_cbFormat, m_minTileSize.width, m_minTileSize.height,
                                                            m_maxTileSize.width, m_maxTileSize.height);
    Move<VkPipelineLayout> pipelineLayout = buildPipelineLayout(&(*m_counterBufferDescriptorSetLayout));
    Move<VkPipeline> graphicsPipeline0 =
        buildGraphicsPipeline(0, *renderPass, m_cbFormat, *pipelineLayout, *vertShader0, *fragShader);
    Move<VkPipeline> graphicsPipeline1 =
        buildGraphicsPipeline(1, *renderPass, m_cbFormat, *pipelineLayout, *vertShader1, *fragShader);
