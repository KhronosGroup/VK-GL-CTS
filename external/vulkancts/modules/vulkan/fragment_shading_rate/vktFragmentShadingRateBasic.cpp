...
protected:
    void preRenderCommands(VkCommandBuffer cmdBuffer, ImageWithMemory *cbImage, ImageWithMemory *dsImage,
                           ImageWithMemory *derivImage, uint32_t derivNumLevels, ImageWithMemory *srImage,
                           VkImageLayout srLayout, BufferWithMemory *srFillBuffer, uint32_t numSRLayers,
                           uint32_t srWidth, uint32_t srHeight, uint32_t srFillBpp, const VkClearValue &clearColor,
                           const VkClearValue &clearDepthStencil);
    void beginLegacyRender(VkCommandBuffer cmdBuffer, VkRenderPass renderPass, VkFramebuffer framebuffer,
    ...
    void drawCommands(VkCommandBuffer cmdBuffer, std::vector<GraphicsPipelineWrapper> &pipelines,
                      ...
                      PipelineRenderingCreateInfoWrapper dynamicRenderingState, const VkShaderModule vertShader,
                      const VkShaderModule geomShader, const VkShaderModule fragShader, VkDescriptorSet descriptorSet,
    ...
    int32_t ShadingRateExtentToClampedMask(VkExtent2D ext, bool allowSwap) const;
...
void FSRTestCase::checkSupport(Context &context) const
{
    ...
    VkResult result =
        vki.getPhysicalDeviceImageFormatProperties(physicalDevice, VK_FORMAT_R32G32B32A32_UINT, VK_IMAGE_TYPE_2D,
...
int32_t FSRTestInstance::ShadingRateExtentToClampedMask(VkExtent2D ext, bool allowSwap) const
{
        ...
        for (uint32_t i = 0; i < m_supportedFragmentShadingRateCount; ++i)
        {
            ...
            if ((supportedRate.sampleCounts & m_data.samples) &&
                ...
                ((supportedRate.fragmentSize.width <= ext.width && supportedRate.fragmentSize.height <= ext.height) ||
                 (supportedRate.fragmentSize.height <= ext.width && supportedRate.fragmentSize.width <= ext.height &&
                  allowSwap)))
            {
...
tcu::TestStatus FSRTestInstance::iterate(void)
{
    ...
    VkImageUsageFlags dsUsage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT |
        ...
        for (uint32_t srTexelWidth = minFragmentShadingRateAttachmentTexelSize.width;
            ...
            for (uint32_t srTexelHeight = minFragmentShadingRateAttachmentTexelSize.height;
                ...
                for (uint32_t formatIdx = 0; formatIdx < numFillFormats; ++formatIdx)
                {
                    ...
                    uint32_t dsAttachmentIdx = 0, srAttachmentIdx = 0;
                    if (m_data.useAttachment())
                    {
                    ...
                    const uint32_t fragSizeWH = m_data.sampleMaskTest ? 2 : 0;
                    ...
                    if (m_data.groupParams->useSecondaryCmdBuffer)
                    {
                        ...
                        drawCommands(*secCmdBuffer, pipelines, viewports, scissors, *pipelineLayout, *renderPass,
                                     ...
                                     renderingCreateInfoWrapper, *vertShader, *geomShader, *fragShader, *descriptorSet,
                                     **vertexBuffer);
                        ...
                        preRenderCommands(*cmdBuffer, cbImage.get(), dsImage.get(), derivImage.get(), derivNumLevels,
                                          srImage.get(), srLayout, srFillBuffer.get(), numSRLayers, srWidth, srHeight,
                                          srFillBpp, clearColor, clearDepthStencil);
                        if (!m_data.groupParams->secondaryCmdBufferCompletelyContainsDynamicRenderpass)
                    ...
                    if (!m_data.groupParams->useDynamicRendering)
                    {
                        ...
                        preRenderCommands(*cmdBuffer, cbImage.get(), dsImage.get(), derivImage.get(), derivNumLevels,
                                          srImage.get(), srLayout, srFillBuffer.get(), numSRLayers, srWidth, srHeight,
                                          srFillBpp, clearColor, clearDepthStencil);
                        beginLegacyRender(*cmdBuffer, *renderPass, *framebuffer, *srImageView, *cbImageView,
                                          *dsImageView, imagelessFB);
                        drawCommands(*cmdBuffer, pipelines, viewports, scissors, *pipelineLayout, *renderPass,
                                     &vertexInputStateCreateInfo, &dynamicStateCreateInfo,
                                     &rasterizationStateCreateInfo, &depthStencilStateParams,
                                     &multisampleStateCreateInfo, &shadingRateStateCreateInfo,
...
void FSRTestInstance::preRenderCommands(VkCommandBuffer cmdBuffer, ImageWithMemory *cbImage, ImageWithMemory *dsImage,
                                        ImageWithMemory *derivImage, uint32_t derivNumLevels, ImageWithMemory *srImage,
                                        VkImageLayout srLayout, BufferWithMemory *srFillBuffer, uint32_t numSRLayers,
                                        uint32_t srWidth, uint32_t srHeight, uint32_t srFillBpp,
                                        const VkClearValue &clearColor, const VkClearValue &clearDepthStencil)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
    ...
    if (m_data.useDepthStencil)
    {
        VkImageSubresourceRange range = makeImageSubresourceRange(
            VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT, 0u, 1u, 0u, VK_REMAINING_ARRAY_LAYERS);
                                        ...
                                        bool imagelessFB) const
{
    ...
    beginRenderPass(vk, cmdBuffer, renderPass, framebuffer, renderArea, 0, DE_NULL, VK_SUBPASS_CONTENTS_INLINE,
...
void FSRTestInstance::drawCommands(VkCommandBuffer cmdBuffer, std::vector<GraphicsPipelineWrapper> &pipelines,
                                   ...
                                   const VkShaderModule vertShader, const VkShaderModule geomShader,
                                   const VkShaderModule fragShader, VkDescriptorSet descriptorSet,
                                   VkBuffer vertexBuffer)
{
    const DeviceInterface &vk = m_context.getDeviceInterface();
    const VkDevice device     = m_context.getDevice();
    ...
    vk.cmdBindDescriptorSets(cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 0,
                             DE_NULL);
    ...
    if (m_data.useDynamicState)
    {
        ...
        pipelines.back()
            ...
            .setupPreRasterizationShaderState(viewports, scissors, pipelineLayout, renderPass, 0u, vertShader,
                                              rasterizationState, DE_NULL, DE_NULL, geomShader, DE_NULL,
                                              dynamicRenderingState)
            .setupFragmentShaderState(pipelineLayout, renderPass, 0u, fragShader, depthStencilState, multisampleState,
                                      shadingRateState)
    ...
    for (int32_t i = 0; i < NUM_TRIANGLES; ++i)
    {
        ...
        int32_t shadingRatePC = PrimIDToPrimitiveShadingRate(i);
        ...
        else
        {
            ...
            pipelines.back()
                ...
                .setupFragmentShaderState(pipelineLayout, renderPass, 0u, fragShader, depthStencilState,
                                          multisampleState, shadingRateState)
                .setupFragmentOutputState(renderPass, 0u, DE_NULL, multisampleState)
                .setMonolithicPipelineLayout(pipelineLayout)
                .buildPipeline();
...
void createBasicTests(tcu::TestContext &testCtx, tcu::TestCaseGroup *parentGroup, SharedGroupParams groupParams)
{
    ...
    for (int groupNdx = 0; groupNdx < DE_LENGTH_OF_ARRAY(groupCases); groupNdx++)
    {
        ...
        de::MovePtr<tcu::TestCaseGroup> group(
            new tcu::TestCaseGroup(testCtx, groupCases[groupNdx].name, groupCases[groupNdx].description));
        for (int dynNdx = 0; dynNdx < DE_LENGTH_OF_ARRAY(dynCases); dynNdx++)
        {
            ...
            de::MovePtr<tcu::TestCaseGroup> dynGroup(
                new tcu::TestCaseGroup(testCtx, dynCases[dynNdx].name, dynCases[dynNdx].description));
            for (int attNdx = 0; attNdx < DE_LENGTH_OF_ARRAY(attCases); attNdx++)
            {
                ...
                de::MovePtr<tcu::TestCaseGroup> attGroup(
                    new tcu::TestCaseGroup(testCtx, attCases[attNdx].name, attCases[attNdx].description));
                for (int shdNdx = 0; shdNdx < DE_LENGTH_OF_ARRAY(shdCases); shdNdx++)
                {
                    de::MovePtr<tcu::TestCaseGroup> shdGroup(
                        new tcu::TestCaseGroup(testCtx, shdCases[shdNdx].name, shdCases[shdNdx].description));
                    for (int cmb0Ndx = 0; cmb0Ndx < DE_LENGTH_OF_ARRAY(combCases); cmb0Ndx++)
                    {
                        de::MovePtr<tcu::TestCaseGroup> cmb0Group(
                            new tcu::TestCaseGroup(testCtx, combCases[cmb0Ndx].name, combCases[cmb0Ndx].description));
                        for (int cmb1Ndx = 0; cmb1Ndx < DE_LENGTH_OF_ARRAY(combCases); cmb1Ndx++)
                        {
                            de::MovePtr<tcu::TestCaseGroup> cmb1Group(new tcu::TestCaseGroup(
                                testCtx, combCases[cmb1Ndx].name, combCases[cmb1Ndx].description));
                            for (int extNdx = 0; extNdx < DE_LENGTH_OF_ARRAY(extentCases); extNdx++)
                            {
                                ...
                                de::MovePtr<tcu::TestCaseGroup> extGroup(new tcu::TestCaseGroup(
                                    testCtx, extentCases[extNdx].name, extentCases[extNdx].description));
                                for (int sampNdx = 0; sampNdx < DE_LENGTH_OF_ARRAY(sampCases); sampNdx++)
                                {
                                    ...
                                    de::MovePtr<tcu::TestCaseGroup> sampGroup(new tcu::TestCaseGroup(
                                        testCtx, sampCases[sampNdx].name, sampCases[sampNdx].description));
                                    for (int geomNdx = 0; geomNdx < DE_LENGTH_OF_ARRAY(geomCases); geomNdx++)
                                    {
                                        ...
                                        if (geomCases[geomNdx].count && (useApiSampleMask || useSampleMaskIn ||
                                                                         consRast || fragDepth || fragStencil))
                                            continue;
                                        ...
                                        sampGroup->addChild(new FSRTestCase(testCtx, geomCases[geomNdx].name,
                                                                            geomCases[geomNdx].description, c));
                                    }
    ...
    if (!groupParams->useSecondaryCmdBuffer)
    {
        de::MovePtr<tcu::TestCaseGroup> group(new tcu::TestCaseGroup(
            testCtx, "misc_tests", "Single tests that don't need to be part of above test matrix"));
