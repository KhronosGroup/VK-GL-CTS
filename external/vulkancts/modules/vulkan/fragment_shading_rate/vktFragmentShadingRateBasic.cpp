...
void FSRTestCase::checkSupport(Context &context) const
{
    ...
    VkResult result = context.getInstanceInterface().getPhysicalDeviceImageFormatProperties(
        context.getPhysicalDevice(), VK_FORMAT_R32G32B32A32_UINT, VK_IMAGE_TYPE_2D, VK_IMAGE_TILING_OPTIMAL, cbUsage, 0,
        &imageProperties);
            ...
            for (uint32_t srTexelHeight = minFragmentShadingRateAttachmentTexelSize.height;
                ...
                for (uint32_t formatIdx = 0; formatIdx < numFillFormats; ++formatIdx)
                {
                    ...
                    const VkPipelineViewportStateCreateInfo viewportStateCreateInfo = {
                        ...
                        (uint32_t)scissors.size(),  // uint32_t                                    scissorCount
                    ...
                    uint32_t numStages = m_data.geometryShader ? 3 : 2u;

                    const VkPipelineShaderStageCreateInfo shaderCreateInfo[3] = {
                        {
                            VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO, DE_NULL,
                            (VkPipelineShaderStageCreateFlags)0,
                    ...
                    for (uint32_t i = 0; i < derivNumLevels; ++i)
                    {
                        VkImageSubresourceRange range =
                            makeImageSubresourceRange(VK_IMAGE_ASPECT_COLOR_BIT, i, 1u, 0u, 1u);
                        VkClearValue clearColor = makeClearValueColorU32(1 << i, 0, 0, 0);
                        vk.cmdClearColorImage(*cmdBuffer, **derivImage, VK_IMAGE_LAYOUT_GENERAL, &clearColor.color, 1,
                                              &range);
                    }
                    ...
                    // Clear color buffer to transparent black
                    {
                        ...
                        vk.cmdClearColorImage(*cmdBuffer, **cbImage, VK_IMAGE_LAYOUT_GENERAL, &clearColor.color, 1,
                                              &range);
                    }
                    ...
                    {
                        VkImageSubresourceRange range =
                            makeImageSubresourceRange(VK_IMAGE_ASPECT_DEPTH_BIT | VK_IMAGE_ASPECT_STENCIL_BIT, 0u, 1u,
                        ...
                        vk.cmdClearDepthStencilImage(*cmdBuffer, **dsImage, VK_IMAGE_LAYOUT_GENERAL,
                                                     &clearColor.depthStencil, 1, &range);
                    }
                    ...
                    if (m_data.useAttachment())
                    {
                        ...
                        vk.cmdCopyBufferToImage(*cmdBuffer, **srFillBuffer, **srImage, VK_IMAGE_LAYOUT_GENERAL, 1,
                                                &copyRegion);
                    ...
                    vk.cmdPipelineBarrier(*cmdBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, allPipelineStages, 0, 1,
                    ...
                    beginRenderPass(vk, *cmdBuffer, *renderPass, *framebuffer,
                                    makeRect2D(m_data.framebufferDim.width, m_data.framebufferDim.height), 0, DE_NULL,
                                    VK_SUBPASS_CONTENTS_INLINE, imagelessFB ? &renderPassAttachmentBeginInfo : DE_NULL);
                    ...
                    for (int32_t i = 0; i < NUM_TRIANGLES; ++i)
                    {
                        ...
                        vk.cmdPushConstants(*cmdBuffer, *pipelineLayout, allShaderStages, 0, sizeof(shadingRatePC),
                                            &shadingRatePC);
                        ...
                        else
                        {
                            ...
                            shadingRateStateCreateInfo.fragmentSize =
                                ShadingRateEnumToExtent(PrimIDToPipelineShadingRate(i));
                            pipelines.push_back(
...
void createBasicTests(tcu::TestContext &testCtx, tcu::TestCaseGroup *parentGroup)
{
    ...
    de::MovePtr<tcu::TestCaseGroup> group(
        new tcu::TestCaseGroup(testCtx, "misc_tests", "Single tests that don't need to be part of above test matrix"));
