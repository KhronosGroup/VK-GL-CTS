...
void MeshShaderSyncCase::initPrograms(vk::SourceCollections &programCollection) const
{
...

    {
        ...
        mesh << "#version 450\n"
             ...
             << "\n"
             << (rwInMesh ? resourceDecl : "") << (rwInMesh ? pcDecl : "") << "\n"
             << "void main ()\n"
             << "{\n"
...

    {
        ...
        frag << "#version 450\n"
             ...
             << "\n"
             << (readFromFrag ? resourceDecl : "") << (readFromFrag ? pcDecl : "") << "\n"
             << "void main ()\n"
             << "{\n"
...
Move<VkRenderPass> createCustomRenderPass(const DeviceInterface &vkd, VkDevice device, VkFormat colorFormat,
                                          const TestParams &params)
{
...
void hostToTransferMemoryBarrier(const DeviceInterface &vkd, VkCommandBuffer cmdBuffer)
{
    ...
    vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_HOST_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0u, 1u, &barrier, 0u,
                           nullptr, 0u, nullptr);
}
...
tcu::TestStatus MeshShaderSyncInstance::iterate(void)
{
    ...
    const auto framebuffer =
        makeFramebuffer(vkd, device, renderPass.get(), colorBufferView.get(), imageExtent.width, imageExtent.height);
            ...
            if (newLayout != currentLayout)
            {
                const auto preWriteBarrier =
                    makeImageMemoryBarrier(0u, (VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT), currentLayout,
                                           newLayout, imageResource->get(), colorSRR);
                vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, fromStageFlags, 0u, 0u, nullptr,
                                       0u, nullptr, 1u, &preWriteBarrier);
                currentLayout = newLayout;
            }
        }
    }
    ...
    if (isShaderStage(m_params.toStage) && !needsTwoDrawCalls)
    {
        if (m_params.barrierType == BarrierType::GENERAL)
        {
            ...
            vkd.cmdPipelineBarrier(cmdBuffer, fromStageFlags, toStageFlags, 0u, 1u, &memoryBarrier, 0u, nullptr, 0u,
                                   nullptr);
        }
        else if (m_params.barrierType == BarrierType::SPECIFIC)
        {
            if (useBufferResource)
            {
                ...
                vkd.cmdPipelineBarrier(cmdBuffer, fromStageFlags, toStageFlags, 0u, 0u, nullptr, 1u, &bufferBarrier, 0u,
                                       nullptr);
            }
            else
            {
                ...
                vkd.cmdPipelineBarrier(cmdBuffer, fromStageFlags, toStageFlags, 0u, 0u, nullptr, 0u, nullptr, 1u,
                                       &imageBarrier);
                currentLayout = newLayout;
            }
        }
    ...
    if (needsTwoDrawCalls)
    {
        ...
        if (useImageResource)
        {
            ...
            const auto imageBarrier =
                makeImageMemoryBarrier(0u, writeAccessFlags, currentLayout, newLayout, imageResource->get(), colorSRR);

            vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, fromStageFlags, 0u, 0u, nullptr, 0u,
...
tcu::TestCaseGroup *createMeshShaderSyncTests(tcu::TestContext &testCtx)
{
    const struct
    {
        ...
        {Stage::HOST, Stage::TASK},     {Stage::HOST, Stage::MESH}, {Stage::TRANSFER, Stage::TASK},
        {Stage::TRANSFER, Stage::MESH}, {Stage::TASK, Stage::MESH}, {Stage::TASK, Stage::FRAG},
        {Stage::TASK, Stage::TRANSFER}, {Stage::TASK, Stage::HOST}, {Stage::MESH, Stage::FRAG},
        {Stage::MESH, Stage::TRANSFER}, {Stage::MESH, Stage::HOST},
    };
    ...
    for (const auto &stageCombination : stageCombinations)
    {
        ...
        for (const auto &resourceCase : resourceTypes)
        {
            ...
            for (const auto &barrierCase : barrierTypes)
            {
                ...
                if (readAndWriteFromShaders(stageCombination.fromStage, stageCombination.toStage) &&
                    barrierCase.barrierType == BarrierType::SPECIFIC)
                    continue;
