                                    ...
                                    const VkPipelineLayout layout)
{
    ...
    else if (m_params->drawCall == DrawCallType::INDIRECT || m_params->drawCall == DrawCallType::INDIRECT_WITH_COUNT)
    {
        ...
        const auto indirectBufferCreateInfo = makeBufferCreateInfo(
            static_cast<VkDeviceSize>((indirectBufferSize + cmdSize) * 2), VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
...
tcu::TestStatus MeshQueryInstance::iterate(void)
{
    ...
    // Verify query results.
    {
        ...
        if (hasStatsQuery)
        {
            ...
            if (hasTaskInvStat)
                verifyQueryCounter(totalTaskInvs, expectedTaskInv, expectedTaskInv * viewCount, *m_params,
                                   "Task invocations");
