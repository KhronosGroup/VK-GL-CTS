...
{
public:
    PayloadReadCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                    ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
    }
...
{
public:
    MixedPipelinesCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                       ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
    }
...
{
public:
    FirstInvocationCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                        ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
    }
...
{
public:
    LocalSizeIdCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                    ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
    }
...
VkPipelineShaderStageCreateInfo makePipelineShaderStageCreateInfo(
...
{
public:
    MultipleTaskPayloadsCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                             ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
    }
...
tcu::TestCaseGroup *createMeshShaderMiscTestsEXT(tcu::TestContext &testCtx)
{
    ...
    for (const auto &dim : dimensionCases)
    {
...

        {
            ...
            const auto desc =
                std::string("Generate a large number of task work groups in the ") + dimChar + " dimension";
...

        {
            ...
            const auto desc =
                std::string("Generate a large number of mesh work groups in the ") + dimChar + " dimension";
...

        {
            ...
            const auto desc =
                std::string("Generate a large number of task and mesh work groups in the ") + dimChar + " dimension";
...

    {
        for (int i = 0; i < 2; ++i)
        {
            ...
            miscTests->addChild(new MixedPipelinesCase(
                testCtx, "mixed_pipelines" + nameSuffix,
                "Test mixing classic and mesh pipelines in the same render pass" + descSuffix, std::move(paramsPtr)));
        }
    }
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        miscTests->addChild(new FirstInvocationCase(
            testCtx, testName, "Check only the first invocation is used in EmitMeshTasksEXT() and SetMeshOutputsEXT()",
            std::move(paramsPtr)));
    }
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        miscTests->addChild(new LocalSizeIdCase(
            testCtx, testName, "Check LocalSizeId can be used with task and mesh shaders", std::move(paramsPtr)));
    }
    ...
    if (false) // Disabled. This may be illegal.
    {
        ...
        miscTests->addChild(new MultipleTaskPayloadsCase(testCtx, "multiple_task_payloads",
                                                         "Check the task payload can be chosen among several ones",
                                                         std::move(paramsPtr)));
    }

    {
        ...
        miscTests->addChild(new PayloadReadCase(testCtx, "payload_read",
                                                "Check the task payload can be read from all task shader instances",
                                                std::move(paramsPtr)));
    }

    {
        ...
        miscTests->addChild(new RebindSetsCase(
            testCtx, "rebind_sets",
            "Use several draw calls binding new descriptor sets and updating push constants between them",
            std::move(paramsPtr)));
    }
