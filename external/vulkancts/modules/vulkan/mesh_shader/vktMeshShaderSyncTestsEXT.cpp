...
{
public:
    MeshShaderSyncCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
...
tcu::TestStatus BarrierAcrossSecondaryInstance::iterate(void)
{
    ...
    const auto computePipeline =
        makeComputePipeline(vkd, device, pipelineLayout.get(), 0u, compModule.get(), 0u, nullptr);
...
tcu::TestCaseGroup *createMeshShaderSyncTestsEXT(tcu::TestContext &testCtx)
{
...

    {
        ...
        otherGroup->addChild(new BarrierAcrossSecondaryCase(
            testCtx, "barrier_across_secondary",
