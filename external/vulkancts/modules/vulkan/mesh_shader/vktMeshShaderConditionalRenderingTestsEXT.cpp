...
void ConditionalRenderingCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    frag << "#version 460\n"
         ...
         << "\n"
         << "void main (void) {\n"
         << "    outColor = vec4(" << (m_params.multiView ? "gl_ViewIndex" : std::to_string(outColor.x())) << ", "
         << outColor.y() << ", " << outColor.z() << ", " << outColor.w() << ");\n"
         << "}\n";
...
tcu::TestStatus ConditionalRenderingInstance::iterate()
{
    ...
    const auto renderPass =
        makeRenderPass(vkd, device, colorFormat, VK_FORMAT_UNDEFINED, VK_ATTACHMENT_LOAD_OP_CLEAR,
    ...
    for (int k = 0; k < imageLayers; ++k)
    {
        ...
        if (!tcu::floatThresholdCompare(log, "Result", "", expectedColor, layerAccess, threshold,
                        ...
                        for (const auto &useTaskCase : useTaskCases)
                        {
                            ...
                            for (const auto &condValue : condValues)
                            {
                                for (const auto multiView : {false, true})
                                {
                                    ...
                                    const auto testName =
                                        "value_" + paddedHex(condValue) + (multiView ? "_multiview" : "");
