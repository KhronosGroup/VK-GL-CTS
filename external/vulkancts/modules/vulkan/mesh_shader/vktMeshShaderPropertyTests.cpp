...
{
    return tcu::StringTemplate("#version 460\n"
...
tcu::TestStatus MeshShaderPropertyInstance::iterate()
{
    ...
    const auto storageBufferSize =
        static_cast<VkDeviceSize>(m_params.bufferElements) * static_cast<VkDeviceSize>(sizeof(uint32_t));
    ...
    setUpdateBuilder.writeSingle(descriptorSet.get(), DescriptorSetUpdateBuilder::Location::binding(0u),
                                 storageBufferType, &storageBufferDescInfo);
    ...
    vkd.cmdPipelineBarrier(cmdBuffer, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_HOST_BIT, 0u, 1u,
                           &shaderToHostBarrier, 0u, nullptr, 0u, nullptr);
    ...
    for (size_t idx = 0u; idx < bufferData.size(); ++idx)
    {
        ...
        if (bufferValue != expected)
            TCU_FAIL("Unexpected value found in buffer position " + de::toString(idx) + ": " +
                     de::toString(bufferValue));
    }
...
{
public:
    MaxTaskWorkGroupInvocationsCase(tcu::TestContext &testCtx, const std::string &name) : vkt::TestCase(testCtx, name)
    {
    }
...
void MaxTaskWorkGroupSizeCase::checkSupport(Context &context) const
{
    ...
    const auto &properties = context.getMeshShaderProperties();
    if (properties.maxTaskWorkGroupSize[0] < minSizeX || properties.maxTaskWorkGroupSize[1] < minSizeY ||
...
{
public:
    MaxTaskOutputCountCase(tcu::TestContext &testCtx, const std::string &name) : vkt::TestCase(testCtx, name)
    {
    }
...
{
public:
    MaxMeshWorkGroupInvocationsCase(tcu::TestContext &testCtx, const std::string &name) : vkt::TestCase(testCtx, name)
    {
    }
...
void MaxMeshWorkGroupSizeCase::checkSupport(Context &context) const
{
    ...
    const auto &properties = context.getMeshShaderProperties();
    if (properties.maxMeshWorkGroupSize[0] < minSizeX || properties.maxMeshWorkGroupSize[1] < minSizeY ||
...
std::string getSharedArrayDecl(uint32_t numElements)
{
    ...
    decl << "const uint arrayElements = " << de::toString(numElements) << ";\n"
...
{
public:
    MaxTaskTotalMemorySizeCase(tcu::TestContext &testCtx, const std::string &name) : vkt::TestCase(testCtx, name)
    {
    }
...
{
public:
    MaxMeshTotalMemorySizeCase(tcu::TestContext &testCtx, const std::string &name) : vkt::TestCase(testCtx, name)
    {
    }
...
tcu::TestCaseGroup *createMeshShaderPropertyTests(tcu::TestContext &testCtx)
{
    ...
    mainGroup->addChild(new MaxDrawMeshTasksCountCase(testCtx, "max_draw_mesh_tasks_count_with_task",
                                                      MaxDrawMeshTasksCountCase::TestType::TASK));
    mainGroup->addChild(new MaxDrawMeshTasksCountCase(testCtx, "max_draw_mesh_tasks_count_with_mesh",
                                                      MaxDrawMeshTasksCountCase::TestType::MESH));
