...
void MeshApiCase::initPrograms(vk::SourceCollections &programCollection) const
{
    ...
    if (m_params.useTask)
    {
        ...
        task << "#version 460\n"
             ...
             << "void main ()\n"
             << "{\n"
             << "    const uint workGroupID = ((pc.dimCoord == 2) ? gl_WorkGroupID.z : ((pc.dimCoord == 1) ? "
...
tcu::TestStatus MeshApiInstance::iterate(void)
{
    ...
    const auto subpassContents =
        (m_params.useSecondaryCmdBuffer ? VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS : VK_SUBPASS_CONTENTS_INLINE);
    ...
    const auto secondaryCmdBuffer =
        (m_params.useSecondaryCmdBuffer ?
             allocateCommandBuffer(vkd, device, cmdPool.get(), VK_COMMAND_BUFFER_LEVEL_SECONDARY) :
             Move<VkCommandBuffer>());
    const auto secondary = secondaryCmdBuffer.get();
    const auto rpCmdBuffer =
        (m_params.useSecondaryCmdBuffer ? secondary : primary); // Holding the contents of the render pass commands.
    ...
    if (m_params.drawType != DrawType::DRAW)
    {
        ...
        DE_ASSERT(indirectArgs.stride % 4u == 0u &&
                  (indirectArgs.stride == 0u ||
                   indirectArgs.stride >= static_cast<uint32_t>(sizeof(VkDrawMeshTasksIndirectCommandEXT))));
    ...
    if (m_params.useSecondaryCmdBuffer)
    {
        ...
        const VkCommandBufferUsageFlags cmdBufferFlags =
    ...
    vkd.cmdBindDescriptorSets(rpCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout.get(), 0u, 1u,
                              &descriptorSet.get(), 0u, nullptr);
    {
        const char *pcDataPtr = reinterpret_cast<const char *>(&pcData);
        for (const auto &range : pcRanges)
            vkd.cmdPushConstants(rpCmdBuffer, pipelineLayout.get(), range.stageFlags, range.offset, range.size,
