...
struct PushConstantData
{
	uint32_t width;
	uint32_t height;
	uint32_t dimMesh;	// Set work group size in the X, Y or Z dimension depending on value (0, 1, 2).
	uint32_t one;
	uint32_t dimTask;	// Same as dimMesh.
	...
	std::vector<VkPushConstantRange> getRanges (bool includeTask) const
	{
		...
		{
			VK_SHADER_STAGE_MESH_BIT_EXT,	//	VkShaderStageFlags	stageFlags;
			offsetMesh,						//	uint32_t			offset;
			sizeMesh,						//	uint32_t			size;
		};
		const VkPushConstantRange taskRange =
		{
			VK_SHADER_STAGE_TASK_BIT_EXT,	//	VkShaderStageFlags	stageFlags;
...
void MeshApiCase::initPrograms (vk::SourceCollections& programCollection) const
{
	...
	if (m_params.useTask)
	{
		...
		task
			...
			<< "    layout (offset=16) uint dimCoord;\n"
			<< "} pc;\n"
			<< "\n"
			<< taskDataDecl
			<< "\n"
			...
			<< "    EmitMeshTasksEXT(pc.one, pc.one, pc.one);"
			<< "}\n"
			;
...
{
	checkTaskMeshShaderSupportEXT(context, m_params.useTask, true);
...
VkDrawMeshTasksIndirectCommandEXT getIndirectCommand (uint32_t blockSize, uint32_t dimCoord)
{
	...
	switch (dimCoord)
	{
		case 0u:		indirectCmd.groupCountX = blockSize; break;
		case 1u:		indirectCmd.groupCountY = blockSize; break;
		case 2u:		indirectCmd.groupCountZ = blockSize; break;
		default:
...
tcu::TestStatus MeshApiInstance::iterate (void)
{
	...
	// Push constants: choose used dimension coordinate pseudorandomly.
	const auto dimCoord = rnd.getUint32() % 3u;

	pcData.width	= extent.width;
	pcData.height	= extent.height;
	pcData.dimMesh	= dimCoord;
	pcData.one		= 1u;
	pcData.dimTask	= dimCoord;
	...
	const auto subpassContents		= (m_params.useSecondaryCmdBuffer ? VK_SUBPASS_CONTENTS_SECONDARY_COMMAND_BUFFERS : VK_SUBPASS_CONTENTS_INLINE);
	...
	const auto secondaryCmdBuffer	= (m_params.useSecondaryCmdBuffer ? allocateCommandBuffer(vkd, device, cmdPool.get(), VK_COMMAND_BUFFER_LEVEL_SECONDARY) : Move<VkCommandBuffer>());
	const auto secondary			= secondaryCmdBuffer.get();
	const auto rpCmdBuffer			= (m_params.useSecondaryCmdBuffer ? secondary : primary); // Holding the contents of the render pass commands.
	...
	if (m_params.drawType != DrawType::DRAW)
	{
		...
		DE_ASSERT(indirectArgs.stride % 4u == 0u && (indirectArgs.stride == 0u || indirectArgs.stride >= static_cast<uint32_t>(sizeof(VkDrawMeshTasksIndirectCommandEXT))));
	...
	beginRenderPass(vkd, primary, renderPass.get(), framebuffer.get(), scissors.at(0), clearColor, subpassContents);
	...
	if (m_params.useSecondaryCmdBuffer)
	{
		...
		const VkCommandBufferUsageFlags	cmdBufferFlags	= (VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT | VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT);
		const VkCommandBufferBeginInfo	beginInfo		=
		{
			VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,	//	VkStructureType							sType;
			nullptr,										//	const void*								pNext;
			cmdBufferFlags,									//	VkCommandBufferUsageFlags				flags;
			&inheritanceInfo,								//	const VkCommandBufferInheritanceInfo*	pInheritanceInfo;
		};

		vkd.beginCommandBuffer(secondary, &beginInfo);
	}

	vkd.cmdBindDescriptorSets(rpCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout.get(), 0u, 1u, &descriptorSet.get(), 0u, nullptr);
	{
		const char* pcDataPtr = reinterpret_cast<const char*>(&pcData);
		for (const auto& range : pcRanges)
			vkd.cmdPushConstants(rpCmdBuffer, pipelineLayout.get(), range.stageFlags, range.offset, range.size, pcDataPtr + range.offset);
	}
	vkd.cmdBindPipeline(rpCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline.get());

	if (m_params.drawType == DrawType::DRAW)
	{
		const auto drawArgs = getIndirectCommand(m_params.drawCount, dimCoord);
		vkd.cmdDrawMeshTasksEXT(rpCmdBuffer, drawArgs.groupCountX, drawArgs.groupCountY, drawArgs.groupCountZ);
	}
	else if (m_params.drawType == DrawType::DRAW_INDIRECT)
	{
		const auto& indirectArgs = m_params.indirectArgs.get();
		vkd.cmdDrawMeshTasksIndirectEXT(rpCmdBuffer, indirectBuffer->get(), indirectArgs.offset, m_params.drawCount, indirectArgs.stride);
	}
	else if (m_params.drawType == DrawType::DRAW_INDIRECT_COUNT)
	{
		...
		const auto maxCount	= ((indirectCountLimit == IndirectCountLimitType::MAX_COUNT)
							? m_params.drawCount
							: largeDrawCount);
		vkd.cmdDrawMeshTasksIndirectCountEXT(rpCmdBuffer, indirectBuffer->get(), indirectArgs.offset, countBuffer->get(), indirectCountOffset, maxCount, indirectArgs.stride);
	}
	else
		DE_ASSERT(false);
	...
	copyImageToBuffer(vkd, primary, colorBuffer->get(), outBuffer->get(), iExtent2D);
...
tcu::TestCaseGroup* createMeshShaderApiTestsEXT (tcu::TestContext& testCtx)
{
	...
	const uint32_t normalStride	= static_cast<uint32_t>(sizeof(VkDrawMeshTasksIndirectCommandEXT));
