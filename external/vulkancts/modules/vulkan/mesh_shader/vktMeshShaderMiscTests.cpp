...
{
public:
    MaximizePrimitivesCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                           ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
...
{
public:
    MaximizeVerticesCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                         ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
        const auto mtParams = dynamic_cast<MaximizeThreadsParams *>(m_params.get());
...
{
public:
    MaximizeInvocationsCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                            ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
        const auto mtParams = dynamic_cast<MaximizeThreadsParams *>(m_params.get());
...
{
public:
    InterfaceVariablesCase(tcu::TestContext &testCtx, const std::string &name, const std::string &description,
                           ParamsPtr params)
        : MeshShaderMiscCase(testCtx, name, description, std::move(params))
    {
    }
...
tcu::TestCaseGroup *createMeshShaderMiscTests(tcu::TestContext &testCtx)
{
...

    {
        ...
        miscTests->addChild(new MaximizePrimitivesCase(testCtx, "maximize_primitives",
...

    {
        ...
        miscTests->addChild(new MaximizeVerticesCase(testCtx, "maximize_vertices",
...

    {
        ...
        for (const auto &invocationCase : kInvocationCases)
        {
            ...
            miscTests->addChild(new MaximizeInvocationsCase(
                testCtx, "maximize_invocations_" + invsStr,
                "Use a large number of invocations compared to other sizes: " + invsStr, std::move(paramsPtr)));
        }
    }
    ...
    if (false) // This test does not work and the spec is not clear that it should.
    {
        ...
        miscTests->addChild(new CountReadCase(
            testCtx, "count_reads", "Attempt to read gl_TaskCountNV and gl_PrimitiveCountNV from multiple invocations",
            std::move(paramsPtr)));
    }

    addFunctionCaseWithPrograms(miscTests.get(), "mixed_pipelines",
                                "Mix classic and mesh shader pipelines in the same render pass", checkMeshSupport,
                                initMixedPipelinesPrograms, testMixedPipelines);
...
{
    GroupPtr inOutTests(
        new tcu::TestCaseGroup(testCtx, "in_out", "Mesh Shader Tests checking Input/Output interfaces"));
