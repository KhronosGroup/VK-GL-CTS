...
tcu::TestStatus parallelPreprocessRun(Context &context, PreprocessParams params)
{
    ...
    std::vector<DGCBufferPtr> sequencesCountBuffers;
    ...
    if (useCountBuffer)
    {
        ...
        {
            sequencesCountBuffers.emplace_back(new DGCBuffer(ctx.vkd, ctx.device, ctx.allocator, u32Size));
    ...
    std::vector<DGCBufferPtr> genCmdBuffers;

    for (uint32_t i = 0; i < kExecuteCount; ++i)
    {
        const auto &cmdData = genCmdDataVecs.at(i);
        genCmdBuffers.emplace_back(new DGCBuffer(ctx.vkd, ctx.device, ctx.allocator, genCmdsBufferSize));
    ...
    for (uint32_t i = 0u; i < kExecuteCount; ++i)
    {
        ...
        preprocessBuffers.emplace_back(new PreprocessBufferExt(ctx.vkd, ctx.device, ctx.allocator, VK_NULL_HANDLE,
                                                               *cmdsLayout, preprocessSequencesCount, 0u, *pipeline));
    }

    std::vector<DGCGenCmdsInfo> cmdInfos;
    cmdInfos.reserve(kExecuteCount);

    for (uint32_t i = 0u; i < kExecuteCount; ++i)
    {
        const auto &preprocessBuffer   = *(preprocessBuffers.at(i).get());
        const auto &genCmdBuffer       = *genCmdBuffers.at(i);
    ...
    // We will preprocess one sequence first, and wait for the preprocessing to finish using a fence.
    {
        ...
        {
            const bool separateState = (params.stateCmdBuffer == StateCmdBuffer::OTHER);
            Move<VkCommandBuffer> separateCmdBufferPtr;
            ...
            if (separateState)
            {
                ...
                ctx.vkd.cmdBindDescriptorSets(*separateCmdBufferPtr, bindPoint, *pipelineLayout, 0u, 1u,
                                              &descriptorSets.at(0u).get(), 0u, nullptr);
                ctx.vkd.cmdBindPipeline(*separateCmdBufferPtr, bindPoint, *pipeline);
            }
            const auto stateCmdBuffer = (separateState ? *separateCmdBufferPtr : cmdBuffer);

            ctx.vkd.cmdPreprocessGeneratedCommandsEXT(cmdBuffer, &cmdInfos.at(0u).get(), stateCmdBuffer);
        }
        ...
        if (queueSwitch)
        {
            ...
            if (preprocessBuffers.at(0u)->get() != VK_NULL_HANDLE)
            {
    ...
    // Separately, execute the preprocessed commands and preprocess+execute the second sequence.
    {
        ...
        VkCommandPool cmdBufferPool{VK_NULL_HANDLE};      // The pool which cmdBuffer was allocated from.
        ...
        std::tie(cmdBuffer, cmdBufferPool) = std::tie(*executeCmdBufferPtr, executeQueueInfo.cmdPool);
        ...
        ctx.vkd.cmdBindDescriptorSets(cmdBuffer, bindPoint, *pipelineLayout, 0u, 1u, &descriptorSets.at(0u).get(), 0u,
                                      nullptr);
        ctx.vkd.cmdBindPipeline(cmdBuffer, bindPoint, *pipeline);
        ctx.vkd.cmdExecuteGeneratedCommandsEXT(cmdBuffer, VK_TRUE, &cmdInfos.at(0u).get());
        ...
        if (queueSwitch)
        {
            ...
            preprocessCmdBufferPtr             = allocateCommandBuffer(ctx.vkd, ctx.device, preprocessQueueInfo.cmdPool,
                                                                       VK_COMMAND_BUFFER_LEVEL_PRIMARY);
            std::tie(cmdBuffer, cmdBufferPool) = std::tie(*preprocessCmdBufferPtr, preprocessQueueInfo.cmdPool);
        ...
        }
        {
            ...
            if (separateState)
            {
                separateCmdBufferPtr =
                    allocateCommandBuffer(ctx.vkd, ctx.device, cmdBufferPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
                beginCommandBuffer(ctx.vkd, *separateCmdBufferPtr);
                ctx.vkd.cmdBindDescriptorSets(*separateCmdBufferPtr, bindPoint, *pipelineLayout, 0u, 1u,
                                              &descriptorSets.at(1u).get(), 0u, nullptr);
                ctx.vkd.cmdBindPipeline(*separateCmdBufferPtr, bindPoint, *pipeline);
            }
            const auto stateCmdBuffer = (separateState ? *separateCmdBufferPtr : cmdBuffer);

            ctx.vkd.cmdPreprocessGeneratedCommandsEXT(cmdBuffer, &cmdInfos.at(1u).get(), stateCmdBuffer);
        }
        preprocessToExecuteBarrierExt(ctx.vkd, cmdBuffer);
        ctx.vkd.cmdExecuteGeneratedCommandsEXT(cmdBuffer, VK_TRUE, &cmdInfos.at(1u).get());
        shaderWriteToHostBarrier(ctx.vkd, cmdBuffer);
        endCommandBuffer(ctx.vkd, cmdBuffer);
...
tcu::TestCaseGroup *createDGCComputePreprocessTestsExt(tcu::TestContext &testCtx)
{
    ...
    const struct
    {
        ...
        {StateCmdBuffer::SAME, ""},
        {StateCmdBuffer::OTHER, "_separate_state_cmd_buffer"},
    };
        ...
        for (const auto &countBufferCase : countBufferCases)
            for (const auto &stateCmdBufferCase : stateCmdBufferCases)
            {
                ...
                const auto testName = std::string("parallel_preprocessing_") + methodCase.name +
                                      countBufferCase.suffix + stateCmdBufferCase.suffix;
                addFunctionCaseWithPrograms(mainGroup.get(), testName, checkDGCComputeAndQueueSupport,
                                            storePushConstantProgram, parallelPreprocessRun, params);
            }
