...
tcu::TestStatus constantPipelineMemoryRequirements (Context& context)
{
	...
	if (deMemCmp(&garbageBufferInfo, &bufferInfoBackup, sizeof(bufferInfoBackup)) != 0)
		return tcu::TestStatus::fail("VkComputePipelineIndirectBufferInfoNV was modified in vkGetPipelineIndirectMemoryRequirementsNV");
	...
	if (!equalMemoryRequirements(baseMemReqs, withBufferInfoReqs) || !equalMemoryRequirements(baseMemReqs, retryMemReqs))
	{
		auto& log = context.getTestContext().getLog();
		log << tcu::TestLog::Message << "Base memory requirements: " << baseMemReqs << tcu::TestLog::EndMessage;
		log << tcu::TestLog::Message << "Memory requirements with buffer info: " << withBufferInfoReqs << tcu::TestLog::EndMessage;
		log << tcu::TestLog::Message << "Memory requirements on retry: " << retryMemReqs << tcu::TestLog::EndMessage;
		return tcu::TestStatus::fail("Indirect memory requirements are not constant for the same pipeline; check log for details");
	}
...
tcu::TestStatus constantPipelineCaptureReplayAddress (Context& context)
{
	...
	DGCComputePipelineMetaDataPool metaDataPool (1u, true);
	DGCComputePipeline pipeline (metaDataPool, ctx.vkd, ctx.device, ctx.allocator, 0u, *pipelineLayout, 0u, *compModule, nullptr, captureAddress);
...
tcu::TestStatus constantCommandsMemReqs (Context& context, ConstantCommandsMemoryReqsCase memReqsCase)
{
	...
	const auto layoutUsageFlags	= (needsPreprocess
								? static_cast<VkIndirectCommandsLayoutUsageFlagsNV>(VK_INDIRECT_COMMANDS_LAYOUT_USAGE_EXPLICIT_PREPROCESS_BIT_NV)
								: 0u);
	...
	{
		const auto extraFlags	= ((i > 0u)
								? static_cast<VkIndirectCommandsLayoutUsageFlagsNV>(VK_INDIRECT_COMMANDS_LAYOUT_USAGE_UNORDERED_SEQUENCES_BIT_NV)
								: 0u);
		...
		{
			currentBuilder.addPushConstantToken(0u, 0u, *pipelineLayout, pcRange.stageFlags, pcRange.offset, pcRange.size);
			currentBuilder.addDispatchToken(0u, currentBuilder.getStreamRange(0u));
		}
	...
	auto memReqsInfo = makeGeneratedCommandsMemoryRequirementsInfoNV(bindPoint, *pipeline, *cmdLayouts.at(0u), maxSequencesCount);
...
tcu::TestCaseGroup* createDGCComputeGetInfoTests (tcu::TestContext& testCtx)
{
	...
	addFunctionCaseWithPrograms(mainGroup.get(), "constant_pipeline_memory_requirements", checkDGCComputePipelineSupport, initBasicProgram, constantPipelineMemoryRequirements);
	addFunctionCaseWithPrograms(mainGroup.get(), "constant_pipeline_device_address", checkDGCComputePipelineSupport, initBasicProgram, constantPipelineDeviceAddress);
	addFunctionCaseWithPrograms(mainGroup.get(), "constant_pipeline_capture_replay_address", checkDGCComputeCaptureReplaySupport, initBasicProgram, constantPipelineCaptureReplayAddress);
	...
	{
		const auto supportCheck = (testCase.needsPipeline ? checkDGCComputePipelineSupportCmd : checkDGCComputeBasicSupportCmd);
		const auto testName = std::string("constant_cmd_memory_requirements_") + testCase.name;
		addFunctionCaseWithPrograms(mainGroup.get(), testName, supportCheck, initBasicProgramCmd, constantCommandsMemReqs, testCase.cmdMemCase);
	}
