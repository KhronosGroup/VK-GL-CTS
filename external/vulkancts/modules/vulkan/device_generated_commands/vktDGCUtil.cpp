...
VkGeneratedCommandsMemoryRequirementsInfoNV makeGeneratedCommandsMemoryRequirementsInfoNV
	(VkPipelineBindPoint         bindPoint,
	 VkPipeline                  pipeline,
	 VkIndirectCommandsLayoutNV  cmdLayout,
...
VkMemoryRequirements getPipelineIndirectMemoryRequirementsNV (const DeviceInterface& vkd, VkDevice device, const VkComputePipelineCreateInfo* createInfo)
{
...
VkMemoryRequirements getGeneratedCommandsMemoryRequirementsNV (const DeviceInterface& vkd, VkDevice device, const VkGeneratedCommandsMemoryRequirementsInfoNV* memReqsInfo)
{
	VkMemoryRequirements2 memReqs = initVulkanStructure();
...
{
	const auto preExecutionBarrier = makeMemoryBarrier(VK_ACCESS_COMMAND_PREPROCESS_WRITE_BIT_NV, VK_ACCESS_INDIRECT_COMMAND_READ_BIT);
	cmdPipelineMemoryBarrier(vkd, cmdBuffer, VK_PIPELINE_STAGE_COMMAND_PREPROCESS_BIT_NV, VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT, &preExecutionBarrier);
}

void metadataUpdateToPreprocessBarrier (const DeviceInterface& vkd, VkCommandBuffer cmdBuffer)
{
	const auto barrier = makeMemoryBarrier2(VK_PIPELINE_STAGE_2_COPY_BIT, VK_ACCESS_2_MEMORY_WRITE_BIT,
...
DGCComputePipelineMetaDataPool::MetaDataLocation DGCComputePipelineMetaDataPool::getMetaDataLocation (const DeviceInterface& vkd, VkDevice device, Allocator& allocator, const VkComputePipelineCreateInfo& createInfo)
{
	...
	if (!bufferInfoPtr)
	{
		...
		const auto bufferUsage		= (VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
		const auto bufferCreateInfo	= makeBufferCreateInfo(bufferInfo.allocationSize, bufferUsage);
		const auto crMemReq			= (m_captureReplay ? MemoryRequirement::DeviceAddressCaptureReplay : MemoryRequirement::Any);
		...
		VK_CHECK(vkd.bindBufferMemory(device, *bufferInfo.buffer, bufferInfo.allocation->getMemory(), bufferInfo.allocation->getOffset()));
...
DGCComputePipeline::DGCComputePipeline (
	...
	VkPipelineShaderStageCreateFlags	shaderFlags,
...
VkPipeline		DGCComputePipeline::get							(void) const { return *m_pipeline;             }
VkPipeline		DGCComputePipeline::operator*					(void) const { return get();                   }
VkDeviceAddress	DGCComputePipeline::getIndirectDeviceAddress	(void) const { return m_indirectDeviceAddress; }

IndirectCommandsLayoutBuilder::IndirectCommandsLayoutBuilder (VkIndirectCommandsLayoutUsageFlagsNV flags, VkPipelineBindPoint bindPoint)
...
void IndirectCommandsLayoutBuilder::addPushConstantToken (uint32_t stream, uint32_t offset, VkPipelineLayout pipelineLayout, VkShaderStageFlags stageFlags, uint32_t pcOffset, uint32_t pcSize)
{
...
namespace
{
	...
	uint32_t tokenDataSize (const VkIndirectCommandsLayoutTokenNV& token)
	{
		...
		{
		case VK_INDIRECT_COMMANDS_TOKEN_TYPE_PIPELINE_NV:      return static_cast<uint32_t>(sizeof(VkBindPipelineIndirectCommandNV));
		case VK_INDIRECT_COMMANDS_TOKEN_TYPE_PUSH_CONSTANT_NV: return token.pushconstantSize;
		case VK_INDIRECT_COMMANDS_TOKEN_TYPE_DISPATCH_NV:      return static_cast<uint32_t>(sizeof(VkDispatchIndirectCommand));
		default: break;
		}
...
Move<VkIndirectCommandsLayoutNV> IndirectCommandsLayoutBuilder::build (const DeviceInterface& vkd, VkDevice device, const vk::VkAllocationCallbacks* pAllocator) const
{
	...
	const auto wpTokenCount = std::count_if(begin(m_tokens), end(m_tokens),
		[](const VkIndirectCommandsLayoutTokenNV& token) { return isWorkProvokingToken(token.tokenType); });
...
PreprocessBuffer::PreprocessBuffer (const DeviceInterface&      vkd,
                                    VkDevice                    device,
                                    Allocator&                  allocator,
                                    VkPipelineBindPoint         bindPoint,
                                    VkPipeline                  pipeline,
	...
	, m_memReqs()
{
	const auto genCmdMemReqsInfo = makeGeneratedCommandsMemoryRequirementsInfoNV(bindPoint, pipeline, cmdLayout, maxSequences);
