...
bool hasPipelineSwitch (TestType testType)
{
	const bool pipelineSwitch =
		(testType == TestType::PIPELINE_DISPATCH
		|| testType == TestType::PIPELINE_PUSH_DISPATCH
...
{
	return (testType == TestType::COMPLEMENTARY_PUSH_DISPATCH || testType == TestType::PIPELINE_COMPLEMENTARY_PUSH_DISPATCH);
}
...
protected:
	...
	void createPipelines (const DeviceInterface& vkd, VkDevice device, vk::Allocator& allocator, VkDescriptorSetLayout setLayout, const std::vector<SpecializationData>& specializationData);
	...
	std::vector<uint32_t> makeIndirectCommands (const std::vector<uint32_t>& wgCounts, const std::vector<SpecializationData>& specializationData) const;
...
void LayoutTestCase::initPrograms (vk::SourceCollections& programCollection) const
{
	...
	if (m_params.testType == TestType::PUSH_DISPATCH || m_params.testType == TestType::PARTIAL_PUSH_DISPATCH || complementaryPush)
	{
		// Push constants only.
		constants
	...
	else if (m_params.testType == TestType::PIPELINE_DISPATCH)
	{
		...
		constants
			<< "layout (constant_id=0) const uint pc_dispatchOffset = 0u;\n"
	...
	comp
		...
		<< "    const uint valueIndex = " << dispatchOffsetPrefix << "dispatchOffset + workGroupIndex * gl_WorkGroupSize.x + gl_LocalInvocationIndex;\n"
		<< "    const uint storageValue = " << valueOffsetPrefix << "valueOffset + (workGroupIndex << 10) + gl_LocalInvocationIndex\n"
		;
...
void LayoutTestInstance::createPipelines (const DeviceInterface& vkd, VkDevice device, vk::Allocator& allocator, VkDescriptorSetLayout setLayout, const std::vector<SpecializationData>& specializationData)
{
	...
	if (pipelineSwitch)
	{
		...
		if (m_params.captureReplay)
		{
			...
			for (size_t i = 0; i < specializationData.size(); ++i)
			{
				...
				const auto specializationInfo = makeSpecializationInfo(de::sizeU32(mapEntries), de::dataOrNull(mapEntries), sizeof(data), &data);
		...
		m_metaDataPool.reset(new DGCComputePipelineMetaDataPool(DGCComputePipelineMetaDataPool::kDefaultMultiplier, m_params.captureReplay));
		for (size_t i = 0; i < specializationData.size(); ++i)
		{
			const auto& data = specializationData.at(i);
			const auto specializationInfo = makeSpecializationInfo(de::sizeU32(mapEntries), de::dataOrNull(mapEntries), sizeof(data), &data);
...
void LayoutTestInstance::makeCommandsLayout (const DeviceInterface& vkd, VkDevice device)
{
	...
	else if (m_params.testType == TestType::PARTIAL_PUSH_DISPATCH)
	{
		...
		cmdsLayoutBuilder.addPushConstantToken(0u, cmdsLayoutBuilder.getStreamRange(0u), *m_pipelineLayout, m_shaderStage, m_constantSize, m_constantSize * 2u);
	}
	else if (m_params.testType == TestType::PIPELINE_DISPATCH)
	{
		cmdsLayoutBuilder.addPipelineToken(0u, 0u);
	}
	else if (m_params.testType == TestType::PIPELINE_PUSH_DISPATCH || m_params.testType == TestType::PIPELINE_COMPLEMENTARY_PUSH_DISPATCH)
	{
		DE_ASSERT(*m_pipelineLayout != VK_NULL_HANDLE);
		DE_ASSERT(m_pcTokenDataSize != 0u);
		const auto pcOffset = ((m_params.testType == TestType::PIPELINE_COMPLEMENTARY_PUSH_DISPATCH) ? m_constantSize : 0u);
		cmdsLayoutBuilder.addPipelineToken(0u, 0u);
		cmdsLayoutBuilder.addPushConstantToken(0u, cmdsLayoutBuilder.getStreamRange(0u), *m_pipelineLayout, m_shaderStage, pcOffset, m_pcTokenDataSize);
	}
	else
		DE_ASSERT(false);
...
std::vector<uint32_t> LayoutTestInstance::makeIndirectCommands (const std::vector<uint32_t>& wgCounts, const std::vector<SpecializationData>& specializationData) const
{
...
tcu::TestStatus LayoutTestInstance::iterate (void)
{
	...
	const VkDeviceSize outputBufferSize = static_cast<VkDeviceSize>(totalInvocations) * static_cast<VkDeviceSize>(sizeof(uint32_t));
	...
	setUpdateBuilder.writeSingle(*descriptorSet, DescriptorSetUpdateBuilder::Location::binding(0u), outputBufferType, &outputBufferDescInfo);
	...
	const auto       indirectCmdsBufferInfo  = makeBufferCreateInfo(indirectCmdsBufferSize, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
	BufferWithMemory indirectCmdsBuffer      (ctx.vkd, ctx.device, ctx.allocator, indirectCmdsBufferInfo, MemoryRequirement::HostVisible);
	...
	PreprocessBuffer preprocessBuffer (ctx.vkd, ctx.device, ctx.allocator, m_bindPoint, *m_singlePipeline, *m_commandsLayout, de::sizeU32(wgCounts));
	...
	for (size_t dispatchIdx = 0u; dispatchIdx < wgCounts.size(); ++dispatchIdx)
	{
		...
		for (uint32_t wgIdx = 0u; wgIdx < wgCount; ++wgIdx)
		{
			for (uint32_t invocationIdx = 0u; invocationIdx < kLocalInvocations; ++invocationIdx)
			{
				...
				const auto expected = ((invocationIdx == skipIndex) ? 0u : (valueOffset + (wgIdx << 10) + invocationIdx + valueOffset2));
				const auto result   = outputBufferValues.at(valueIndex);

				if (expected != result)
				{
					log << tcu::TestLog::Message
						<< "Unexpected value at index " << valueIndex << "; expected " << expected << " but found " << result
						<< " ; dispatchIndex=" << dispatchIdx << " workGroupIndex=" << wgIdx << " invocationIndex=" << invocationIdx
						<< " skipIndex=" << skipIndex << " valueOffset=" << valueOffset
