...
void shaderWriteToHostBarrier (const DeviceInterface& vkd, VkCommandBuffer cmdBuffer)
{
	...
	cmdPipelineMemoryBarrier(vkd, cmdBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_HOST_BIT, &preHostBarrier);
}
...
tcu::TestStatus parallelPreprocessRun (Context& context, PreprocessParams params)
{
	...
	if (useCountBuffer)
	{
		...
		{
			sequencesCountBuffers.emplace_back(new BufferWithMemory(ctx.vkd, ctx.device, ctx.allocator, createInfo, MemoryRequirement::HostVisible));
	...
	const auto descriptorPool = poolBuilder.build(ctx.vkd, ctx.device, VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT, kExecuteCount);
	...
	DescriptorSetUpdateBuilder setUpdateBuilder;
	{
		for (uint32_t i = 0u; i < kExecuteCount; ++i)
		{
			...
			setUpdateBuilder.writeSingle(descriptorSets.at(i).get(), DescriptorSetUpdateBuilder::Location::binding(0u), descType, &outputBufferDescInfo);
		}
	}
	...
	for (uint32_t i = 0; i < kExecuteCount; ++i)
	{
		...
		genCmdBuffers.emplace_back(new BufferWithMemory(ctx.vkd, ctx.device, ctx.allocator, genCmdsBufferCreateInfo, MemoryRequirement::HostVisible));
	...
	for (uint32_t i = 0u; i < kExecuteCount; ++i)
	{
		...
		preprocessBuffers.emplace_back(new PreprocessBuffer(ctx.vkd, ctx.device, ctx.allocator, bindPoint, *pipeline, *cmdsLayout, preprocessSequencesCount));
	}
	...
	const auto& preprocessQueueInfo = ((params.method == Method::UNIVERSAL_QUEUE || params.method == Method::PREPROCESS_UNIVERSAL_EXECUTE_COMPUTE)
	                                ? queueInfoMap[kUniversalKey]
	                                : queueInfoMap[kComputeKey]);

	const auto& executeQueueInfo    = ((params.method == Method::UNIVERSAL_QUEUE || params.method == Method::PREPROCESS_COMPUTE_EXECUTE_UNIVERSAL)
	                                ? queueInfoMap[kUniversalKey]
	                                : queueInfoMap[kComputeKey]);
	...
	{
		const auto cmdBufferPtr = allocateCommandBuffer(ctx.vkd, ctx.device, preprocessQueueInfo.cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
		const auto cmdBuffer    = cmdBufferPtr.get();

		beginCommandBuffer(ctx.vkd, cmdBuffer);
		ctx.vkd.cmdBindDescriptorSets(cmdBuffer, bindPoint, *pipelineLayout, 0u, 1u, &descriptorSets.at(0u).get(), 0u, nullptr);
		...
		if (queueSwitch)
		{
			...
			ownershipBarriers.push_back(makePreprocessToExecuteBarrier(outputBuffers.at(0u)->get(), u32Size, preprocessQueueInfo.qfIndex, executeQueueInfo.qfIndex));
			ownershipBarriers.push_back(makePreprocessToExecuteBarrier(genCmdBuffers.at(0u)->get(), genCmdsBufferSize, preprocessQueueInfo.qfIndex, executeQueueInfo.qfIndex));
			ownershipBarriers.push_back(makePreprocessToExecuteBarrier(preprocessBuffers.at(0u)->get(), preprocessBuffers.at(0u)->getSize(), preprocessQueueInfo.qfIndex, executeQueueInfo.qfIndex));
			if (useCountBuffer)
				ownershipBarriers.push_back(makePreprocessToExecuteBarrier(sequencesCountBuffers.at(0u)->get(), u32Size, preprocessQueueInfo.qfIndex, executeQueueInfo.qfIndex));
	...
	// Separately, execute the preprocessed commands and preprocess+execute the second sequence.
	{
		...
		const auto executeCmdBufferPtr = allocateCommandBuffer(ctx.vkd, ctx.device, executeQueueInfo.cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
		...
		if (queueSwitch)
		{
			...
			preprocessCmdBufferPtr = allocateCommandBuffer(ctx.vkd, ctx.device, preprocessQueueInfo.cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
		...
		ctx.vkd.cmdBindDescriptorSets(cmdBuffer, bindPoint, *pipelineLayout, 0u, 1u, &descriptorSets.at(1u).get(), 0u, nullptr);
		if (queueSwitch)
		{
	...
	for (size_t i = 0u; i < outputBuffers.size(); ++i)
	{
		...
		if (outputBufferValue != reference)
		{
			...
			log << tcu::TestLog::Message
				<< "Unexpected value found in output buffer at position " << i
				<< ": expected " << reference << " but found " << outputBufferValue
...
tcu::TestCaseGroup* createDGCComputePreprocessTests (tcu::TestContext& testCtx)
{
	...
	for (const auto& methodCase : methodCases)
		for (const auto& countBufferCase : countBufferCases)
		{
			...
			addFunctionCaseWithPrograms(mainGroup.get(),
				std::string("parallel_preprocessing_") + methodCase.name + countBufferCase.suffix,
				checkDGCComputeAndQueueSupport,
				storePushConstantProgram,
				parallelPreprocessRun,
				params);
		}
