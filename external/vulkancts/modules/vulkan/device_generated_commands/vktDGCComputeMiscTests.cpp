...
tcu::TestStatus manyExecutesRun(Context &context, ManyExecutesParams params)
{
...

    {
        const auto genCmdMemReqsInfo =
            makeGeneratedCommandsMemoryRequirementsInfoNV(bindPoint, *pipeline, *cmdsLayout, 1u);
        ...
        const auto preprocessBufferCreateInfo =
            makeBufferCreateInfo(preprocessBufferReqs.size, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
    ...
    for (uint32_t i = 0; i < params.executeCount; ++i)
    {
        ...
        {
            log << tcu::TestLog::Message << "Error at execution " << i << ": expected " << kManyexecutesLocalInvocations
...
tcu::TestStatus fullReplayRun(Context &context)
{
    ...
    for (uint32_t iterIdx = 0u; iterIdx < iterCount; ++iterIdx)
    {
        ...
        DGCComputePipelineMetaDataPool metadataPool(DGCComputePipelineMetaDataPool::kDefaultMultiplier,
                                                    true); // Enable capture/replay.
        DGCComputePipeline pipeline(metadataPool, ctx.vkd, ctx.device, ctx.allocator, 0u, *pipelineLayout, 0u,
                                    *compModule, nullptr, capturedAddress);
        ...
        // Generated indirect commands buffer.
        const auto genCmdsBufferSize = de::dataSize(genCmdsData);
        const auto genCmdsBufferCreateInfo =
            makeBufferCreateInfo(genCmdsBufferSize, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
        BufferWithMemory genCmdsBuffer(ctx.vkd, ctx.device, ctx.allocator, genCmdsBufferCreateInfo,
                                       MemoryRequirement::HostVisible);
        auto &genCmdsBufferAlloc = genCmdsBuffer.getAllocation();
        void *genCmdsBufferData  = genCmdsBufferAlloc.getHostPtr();
        ...
        PreprocessBuffer preprocessBuffer(ctx.vkd, ctx.device, ctx.allocator, bindPoint, VK_NULL_HANDLE, *cmdsLayout,
                                          1u);
    ...
    for (size_t i = 0u; i < outputValues.size(); ++i)
    {
        ...
        if (reference != result)
        {
            ...
            msg << "Unexpected value found in output buffer position " << i << ": expected " << reference
                << " but found " << result;
...
tcu::TestStatus ScratchSpaceInstance::iterate()
{
    ...
    const DGCComputePipeline dgcPipeline(metadataPool, ctx.vkd, ctx.device, ctx.allocator, 0u, *pipelineLayout, 0u,
                                         *compModule);
    ...
    for (const auto executeCount : {64u, 1024u, 8192u})
        for (const auto useSecondaries : {false, true})
            for (const auto useComputeQueue : {false, true})
            {
                ...
                const auto testName =
                    std::string("execute_many_") + std::to_string(executeCount) + typeVariant + queueVariant;
                addFunctionCaseWithPrograms(mainGroup.get(), testName, manyExecutesCheckSupport,
                                            manyExecutesInitPrograms, manyExecutesRun, params);
            }

    addFunctionCaseWithPrograms(mainGroup.get(), "full_replay", fullReplayCheckSupport, fullReplayInitPrograms,
                                fullReplayRun);
