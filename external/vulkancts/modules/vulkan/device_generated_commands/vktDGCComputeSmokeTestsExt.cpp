...
{
    NONE = 0,
    STATE_SAME,  // stateCommandBuffer is the same as the main command buffer.
    STATE_OTHER, // stateCommandbuffer is a separate command buffer.
};

struct SmokeTestParams
{
    const uint32_t sequenceCount; // Number of sequences to generate.
    PreprocessType preProcess;    // Do an explicit preprocessing stage.
    ...
    const bool unordered;         // Include the VK_INDIRECT_COMMANDS_LAYOUT_USAGE_UNORDERED_SEQUENCES_BIT_EXT bit?

    bool doPreprocess(void) const
    {
        return (preProcess != PreprocessType::NONE);
    }
};
...
// short range like [1, 256]. The major dimension (x, y or z) of each dispatch will also be chosen pseudorandomly while the rest
// stay at 1. We will have an array of counters accessed by workgroup index (calculated using the number of workgroups and the
// workgroup id). In each dispatch, each local invocation (fixed number: 64) will atomically increase the counter corresponding to
// its workgroup index by 1.
//
...
//
// For example, lets imagine we execute 4 sequences (i.e. 4 dispatches), with 22, 7, 53 and 30 workgroups each (chosen
// pseudorandomly). The result should be:
//
...
// * Counters in the range [30, 53) should end up with value 64*1 (only affected by 1 dispatch).
...
tcu::TestStatus SmokeTestInstance::iterate(void)
{
    ...
    VkIndirectCommandsLayoutUsageFlagsEXT cmdLayoutUsageFlags = 0u;

    if (m_params.doPreprocess())
        cmdLayoutUsageFlags |= static_cast<VkIndirectCommandsLayoutUsageFlagsEXT>(
    ...
    IndirectCommandsLayoutBuilderExt cmdLayoutBuilder(cmdLayoutUsageFlags, stages, *compPipelineLayout);
    cmdLayoutBuilder.addDispatchToken(0u);
    ...
    const auto kMemoryRequirementHostVisibleOnly = MemoryRequirement::HostVisible;
    const auto kMemoryRequirementHostVisibleDeviceAddress =
        (MemoryRequirement::HostVisible | MemoryRequirement::DeviceAddress);
    const auto kMemoryRequirementLocalDeviceAddress = (MemoryRequirement::Local | MemoryRequirement::DeviceAddress);

    bool secondBuffer = false;

    VkBufferUsageFlags firstBufferUsage  = 0u;
    VkBufferUsageFlags secondBufferUsage = 0u;
    VkBufferUsageFlags indirectUsageFlags =
        (VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT);

    const MemoryRequirement *firstBufferMemReq  = &kMemoryRequirementHostVisibleOnly;
    const MemoryRequirement *secondBufferMemReq = &kMemoryRequirementLocalDeviceAddress;

    if (m_params.preCompute)
    {
        secondBuffer      = true;
        firstBufferUsage  = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
        secondBufferUsage = (VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | indirectUsageFlags);

        if (m_params.hostVisible)
            secondBufferMemReq = &kMemoryRequirementHostVisibleDeviceAddress;
    }
    else
    {
        if (m_params.hostVisible)
        {
            firstBufferUsage  = indirectUsageFlags;
            firstBufferMemReq = &kMemoryRequirementHostVisibleDeviceAddress;
        }
        else
        {
            secondBuffer      = true;
            firstBufferUsage  = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
            secondBufferUsage = (indirectUsageFlags | VK_BUFFER_USAGE_TRANSFER_DST_BIT);
        }
    }
    ...
    VkBuffer indirectCmdsBufferHandle = firstCmdsBuffer.get();
    ...
    if (secondBuffer)
    {
        ...
        indirectCmdsBufferHandle = secondCmdsBuffer->get();
    }

    // Create preprocess buffer.
    PreprocessBufferExt preprocessBuffer(ctx.vkd, ctx.device, ctx.allocator, VK_NULL_HANDLE, *cmdLayout,
                                         m_params.sequenceCount, 0u, *compPipeline);
    ...
    if (m_params.doPreprocess())
    {
        Move<VkCommandBuffer> separateCmdBuffer;
        const bool isSeparate = (m_params.preProcess == PreprocessType::STATE_OTHER);
        if (isSeparate)
        {
            separateCmdBuffer =
                allocateCommandBuffer(ctx.vkd, ctx.device, *cmd.cmdPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY);
            beginCommandBuffer(ctx.vkd, *separateCmdBuffer);
            ctx.vkd.cmdBindDescriptorSets(*separateCmdBuffer, bindPoint, *compPipelineLayout, 0u, 1u,
                                          &compDescriptorSet.get(), 0u, nullptr);
            ctx.vkd.cmdBindPipeline(*separateCmdBuffer, bindPoint, *compPipeline);
        }
        const auto stateCommandBuffer = (isSeparate ? *separateCmdBuffer : cmdBuffer);
        ctx.vkd.cmdPreprocessGeneratedCommandsEXT(cmdBuffer, &generatedCommandsInfo.get(), stateCommandBuffer);
