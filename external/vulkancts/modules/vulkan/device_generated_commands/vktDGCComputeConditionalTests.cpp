...
void bindOrPreparePipeline(const DeviceInterface &vkd, VkCommandBuffer cmdBuffer, VkPipelineBindPoint bindPoint,
                           VkPipeline normalPipeline, const DGCComputePipeline *dgcPipeline)
{
    if (dgcPipeline)
    {
...
tcu::TestStatus conditionalDispatchRun(Context &context, TestParams params)
{
    ...
    PreprocessBuffer preprocessBuffer(ctx.vkd, ctx.device, ctx.allocator, bindPoint, *normalPipeline, *cmdsLayout,
                                      potentialSequenceCount);
    ...
    const auto secondary =
        ((params.useSecondaries != UseSecondaries::NO) ?
    ...
    if (params.useSecondaries == UseSecondaries::YES_WITH_INHERITANCE)
    {
        ...
        beginSecondaryCommandBuffer(ctx.vkd, sec, VK_NULL_HANDLE, VK_NULL_HANDLE,
                                    VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT, &icrInfo);
...
tcu::TestStatus conditionalPreprocessRun(Context &context, ConditionalPreprocessParams params)
{
    ...
    if (params.executeOnCompute)
    {
        ...
        ownershipBarriers.push_back(makePreprocessToExecuteBarrier(preprocessBuffer.get(), preprocessBuffer.getSize(),
                                                                   ctx.qfIndex, compQueueIndex));
                    ...
                    for (const auto secondaryUsage : secondaryUsageCases)
                        for (const auto computeQueue : {false, true})
                        {
                            const TestParams params{
                                pipelineToken, indirectCountBuffer,           conditionValue,
                                inverted,      secondaryUsage.useSecondaries, computeQueue,
                            };

                            const std::string testName =
                                std::string() + (pipelineToken ? "pipeline_token" : "classic_bind") +
                                (indirectCountBuffer ? "_with_count_buffer" : "_without_count_buffer") +
                                (conditionValue ? "_condition_true" : "_condition_false") + secondaryUsage.suffix +
