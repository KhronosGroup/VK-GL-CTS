...
tcu::TestStatus maxIndirectCommandsTokenCountRun(Context &context)
{
    ...
    BufferWithMemory genCmdsBuffer(ctx.vkd, ctx.device, ctx.allocator, genCmdsBufferCreateInfo,
                                   MemoryRequirement::HostVisible);
...
tcu::TestStatus maxIndirectCommandsStreamCountRun(Context &context)
{
    ...
    if (pcCmdsCount > 0u)
    {
        ...
        const auto pcStreamBufferCreateInfo =
            makeBufferCreateInfo(pcStreamBufferSize, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);
        ...
        for (uint32_t i = 0u; i < pcCmdsCount; ++i)
        {
            ...
            VK_CHECK(ctx.vkd.bindBufferMemory(ctx.device, genCmdsBuffers.at(i).get(), pcStreamBuffersAlloc->getMemory(),
    ...
    // Indirect dispatch command buffer.
    {
        ...
        const auto dispatchBufferCreateInfo =
            makeBufferCreateInfo(dispatchBufferSize, VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT);

        genCmdsBuffers.push_back(createBuffer(ctx.vkd, ctx.device, &dispatchBufferCreateInfo));
        const auto dispatchBufferMemReqs =
            getBufferMemoryRequirements(ctx.vkd, ctx.device, genCmdsBuffers.back().get());
        dispatchBufferAlloc = ctx.allocator.allocate(dispatchBufferMemReqs, MemoryRequirement::HostVisible);
        VK_CHECK(ctx.vkd.bindBufferMemory(ctx.device, genCmdsBuffers.back().get(), dispatchBufferAlloc->getMemory(),
                                          dispatchBufferAlloc->getOffset()));
...
tcu::TestStatus maxIndirectCommandsStreamStrideRun(Context &context)
{
    ...
    PreprocessBuffer preprocessBuffer(ctx.vkd, ctx.device, ctx.allocator, bindPoint, *pipeline, *cmdsLayout,
                                      maxSequences);
...
tcu::TestStatus minSequencesOffsetAlignmentsRun(Context &context, AlignmentType alignmentType)
{
    ...
    if (alignmentType == AlignmentType::COUNT_BUFFER)
    {
        ...
        log << tcu::TestLog::Message << "minSequencesCountBufferOffsetAlignment: " << offset
        ...
        SimpleAllocator allocatorWithOffset(ctx.vkd, ctx.device, memProperties,
        ...
        countBuffer.reset(
            new BufferWithMemory(ctx.vkd, ctx.device, allocatorWithOffset, createInfo, MemoryRequirement::HostVisible));
    ...
    if (alignmentType == AlignmentType::INDEX_BUFFER)
    {
        ...
        log << tcu::TestLog::Message << "minSequencesIndexBufferOffsetAlignment: " << offset
            << tcu::TestLog::EndMessage;
...
tcu::TestCaseGroup *createDGCPropertyTests(tcu::TestContext &testCtx)
{
    ...
    addFunctionCaseWithPrograms(mainGroup.get(), "maxIndirectCommandsTokenCount", checkBasicDGCComputeSupport,
                                storePushConstantProgram, maxIndirectCommandsTokenCountRun);
    addFunctionCaseWithPrograms(mainGroup.get(), "maxIndirectCommandsStreamCount", checkBasicDGCComputeSupport,
                                storePushConstantProgram, maxIndirectCommandsStreamCountRun);
    addFunctionCaseWithPrograms(mainGroup.get(), "maxIndirectCommandsTokenOffset", checkBasicDGCComputeSupport,
                                storePushConstantProgram, maxIndirectCommandsTokenOffsetRun);
    addFunctionCaseWithPrograms(mainGroup.get(), "maxIndirectCommandsStreamStrideRun", checkBasicDGCComputeSupport,
                                storePushConstantWithIndexProgram, maxIndirectCommandsStreamStrideRun);
    ...
    for (const auto &offset : offsets)
    {
        ...
        addFunctionCaseWithPrograms(mainGroup.get(), testName, checkBufferOffsetAlignmentSupport,
                                    storePushConstantProgramWithOffset, minIndirectCommandsBufferOffsetAlignmentRun,
                                    offset);
    }
    ...
    for (const auto &alignmentCase : alignmentTests)
        addFunctionCaseWithPrograms(mainGroup.get(), alignmentCase.name, checkSequencesOffsetAlignmentSupport,
                                    storePushConstantWithIndexAlignmentProgram, minSequencesOffsetAlignmentsRun,
                                    alignmentCase.alignmentType);
