...
inline void checkRayTracingSupport(Context &context)
{
...
const auto kStages = static_cast<VkShaderStageFlags>(VK_SHADER_STAGE_RAYGEN_BIT_KHR);

void checkConditionalDGCRayTracingSupport(Context &context, TestParams params)
{
    checkRayTracingSupport(context);

    const auto bindStagesPipeline = (params.pipelineToken ? kStages : 0u);
    checkDGCExtSupport(context, kStages, bindStagesPipeline);
...
{
    checkRayTracingSupport(context);
    checkDGCExtSupport(context, kStages);
    checkConditionalRenderingExt(context);
...
tcu::TestStatus conditionalDispatchRun(Context &context, TestParams params)
{
    ...
    BufferWithMemory outputBuffer(ctx.vkd, ctx.device, ctx.allocator, outputBufferCreateInfo, HostIntent::RW);
    ...
    setLayoutBuilder.addSingleBinding(descType, kStages);
    ...
    const auto pcRange = makePushConstantRange(kStages, 0u, pcSize);
    ...
    const auto rgenModule = createShaderModule(ctx.vkd, ctx.device, binaries.get("rgen"));

    // Pipeline.
    RayTracingPipeline pipelineMaker;
    if (params.pipelineToken)
        pipelineMaker.setCreateFlags2(VK_PIPELINE_CREATE_2_INDIRECT_BINDABLE_BIT_EXT);
    pipelineMaker.setMaxAttributeSize(0u);
    pipelineMaker.setMaxPayloadSize(0u);
    pipelineMaker.setMaxRecursionDepth(0u);
    pipelineMaker.addShader(VK_SHADER_STAGE_RAYGEN_BIT_KHR, *rgenModule, 0u);
    const auto pipeline = pipelineMaker.createPipeline(ctx.vkd, ctx.device, *pipelineLayout);
    ...
    const auto rgenSbt                   = pipelineMaker.createShaderBindingTable(
        ctx.vkd, ctx.device, *pipeline, ctx.allocator, shaderGroupHandleSize, shaderGroupBaseAlignment, 0u, 1u);
    const auto rgenAddress = getBufferDeviceAddress(ctx.vkd, ctx.device, rgenSbt->get(), 0u);
    const auto rgenRegion =
        makeStridedDeviceAddressRegionKHR(rgenAddress, shaderGroupHandleSize, shaderGroupHandleSize);

    // Indirect commands layout. Push constant followed by dispatch, optionally preceded by a pipeline bind.
    IndirectCommandsLayoutBuilderExt cmdsLayoutBuilder(0u, kStages, *pipelineLayout);
    if (params.pipelineToken)
        cmdsLayoutBuilder.addExecutionSetToken(0u, VK_INDIRECT_EXECUTION_SET_INFO_TYPE_PIPELINES_EXT, kStages);
    ...
    genCmdsData.reserve(cmdsLayoutBuilder.getStreamStride() / DE_SIZEOF32(uint32_t));
    if (params.pipelineToken)
        genCmdsData.push_back(0u);
    genCmdsData.push_back(pcValue);
    {
        const VkTraceRaysIndirectCommand2KHR rtCmd{
            rgenRegion.deviceAddress, rgenRegion.size, 0ull, 0ull, 0ull, 0ull, 0ull, 0ull, 0ull, 0ull, 0ull, 1u, 1u, 1u,
        };
        pushBackElement(genCmdsData, rtCmd);
    }
    ...
    BufferWithMemory conditionBuffer(ctx.vkd, ctx.device, ctx.allocator, conditionBufferInfo, HostIntent::W);
    ...
    if (params.pipelineToken)
    {
        executionSetManager = makeExecutionSetManagerPipeline(ctx.vkd, ctx.device, *pipeline, 1u);
    ...
    const auto preprocessPipeline     = (params.pipelineToken ? VK_NULL_HANDLE : *pipeline);
    PreprocessBufferExt preprocessBuffer(ctx.vkd, ctx.device, ctx.allocator, executionSetHandle, *cmdsLayout,
                                         potentialSequenceCount, 0u, preprocessPipeline);
    ...
    const DGCGenCmdsInfo cmdsInfo(kStages, executionSetHandle, *cmdsLayout, genCmdsBuffer.getDeviceAddress(),
                                  genCmdsBufferSize, preprocessBuffer.getDeviceAddress(), preprocessBuffer.getSize(),
                                  infoSequencesCount, sequenceCountBufferAddress, 0u, preprocessPipeline);
...
tcu::TestStatus conditionalPreprocessRun(Context &context, ConditionalPreprocessParams params)
{
    ...
    // Generated commands info.
    const DGCGenCmdsInfo cmdsInfo(kStages, VK_NULL_HANDLE, *cmdsLayout, genCmdsBuffer.getDeviceAddress(),
                                  genCmdsBufferSize, preprocessBuffer.getDeviceAddress(), preprocessBuffer.getSize(),
                                  seqCount, 0ull, 0u, *pipeline);
            ...
            for (const auto conditionValue : {false, true})
                for (const auto inverted : {false, true})
                    for (const auto computeQueue : {false, true})
                    {
                        ...
                        addFunctionCaseWithPrograms(generalGroup.get(), testName, checkConditionalDGCRayTracingSupport,
