...
public class RemoteAPI {
    ...
    private ComponentName getDefaultTesterComponent() {
        return new ComponentName(m_context.getPackageName(),
    ...
    private ComponentName getTesterComponent(String testerName) {
        if (testerName != null && !testerName.equals("")) {
            ComponentName component =
                ComponentName.unflattenFromString(testerName);
            if (component == null) {
                Log.e(LOG_TAG, "Invalid component name supplied (" +
                                   testerName + "), using default");
    ...
    public boolean start(String testerName, String cmdLine, String caseList) {
        ...
        cmdLine = testerName + " " + cmdLine +
                  " --deqp-log-filename=" + m_logFileName;
        ...
        boolean isActivity = m_context.getPackageManager().resolveActivity(
                                 testIntent, 0) != null;
        boolean isService =
            m_context.getPackageManager().resolveService(testIntent, 0) != null;

        if (!isActivity && !isService) {
            Log.e(LOG_TAG, "Can't resolve component as activity or service (" +
    ...
    public String getLogFileName() { return m_logFileName; }

    private ActivityManager.RunningAppProcessInfo findProcess(String name) {
        ActivityManager activityMgr =
            (ActivityManager)m_context.getSystemService(
                Context.ACTIVITY_SERVICE);
        List<ActivityManager.RunningAppProcessInfo> processes =
            activityMgr.getRunningAppProcesses();

        for (ActivityManager.RunningAppProcessInfo info : processes) {
            // Log.v(LOG_TAG, "Found proc : " + info.processName + " " +
            // info.pid);
