...
{
    const jclass activityCls = getObjectClass(env, activity);
    const jmethodID getPMID =
        getMethodID(env, activityCls, "getPackageManager", "()Landroid/content/pm/PackageManager;");
...
ScreenClass getScreenClass(const DisplayMetrics &displayMetrics)
{
    ...
    {
        if ((s_screenClasses[ndx].minWidthDp <= widthDp) && (s_screenClasses[ndx].minHeightDp <= heightDp))
...
size_t getCDDRequiredSystemMemory(ANativeActivity *activity)
{
    ...
    if (!is64BitDevice)
        TCU_CHECK_INTERNAL(sizeof(void *) != sizeof(uint64_t));
    ...
    else
    {
        ...
        {
            const DensityClass minClass =
                screenClass == SCREEN_CLASS_EXTRA_LARGE ? s_classes[ndx].extraLargeScreenDensity :
