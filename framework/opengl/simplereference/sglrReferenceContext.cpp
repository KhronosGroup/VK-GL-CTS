    ...
    else
    {
        ...
        else
        {
            ...
            case GL_TEXTURE_2D:
                ...
                break;
            case GL_TEXTURE_CUBE_MAP:
                texObj = new TextureCube(texture);
                break;
            case GL_TEXTURE_2D_ARRAY:
    ...
    // All is ok
    {
        ...
        const void *indicesPtr =
            (vao.m_elementArrayBufferBinding) ?
                (vao.m_elementArrayBufferBinding->getData() + ((const uint8_t *)indices - (const uint8_t *)DE_NULL)) :
                (indices);
...
void ReferenceContext::drawArraysIndirect(uint32_t mode, const void *indirect)
{
    ...
    RC_IF_ERROR((size_t)((const char *)indirect - (const char *)DE_NULL) >
                    (size_t)m_drawIndirectBufferBinding->getSize(),
                GL_INVALID_OPERATION, RC_RET_VOID);
    RC_IF_ERROR((size_t)((const char *)indirect - (const char *)DE_NULL) + sizeof(DrawArraysIndirectCommand) >
                    (size_t)m_drawIndirectBufferBinding->getSize(),
                GL_INVALID_OPERATION, RC_RET_VOID);
    ...
    command = (const DrawArraysIndirectCommand *)(m_drawIndirectBufferBinding->getData() +
                                                  ((const char *)indirect - (const char *)DE_NULL));
...
void ReferenceContext::drawElementsIndirect(uint32_t mode, uint32_t type, const void *indirect)
{
    ...
    RC_IF_ERROR((size_t)((const char *)indirect - (const char *)DE_NULL) >
                    (size_t)m_drawIndirectBufferBinding->getSize(),
                GL_INVALID_OPERATION, RC_RET_VOID);
    RC_IF_ERROR((size_t)((const char *)indirect - (const char *)DE_NULL) + sizeof(DrawElementsIndirectCommand) >
                    (size_t)m_drawIndirectBufferBinding->getSize(),
                GL_INVALID_OPERATION, RC_RET_VOID);
    ...
    // gen attributes
    {
        ...
        for (size_t ndx = 0; ndx < vao.m_arrays.size(); ++ndx)
        {
            ...
            else
            {
                ...
                vertexAttribs[ndx].pointer =
                    (vao.m_arrays[ndx].bufferBinding) ?
                        (vao.m_arrays[ndx].bufferBinding->getData() +
                         ((const uint8_t *)vao.m_arrays[ndx].pointer - (const uint8_t *)DE_NULL)) :
...
Texture::Texture(uint32_t name, Type type)
    : NamedObject(name)
    , m_type(type)
    , m_immutable(false)
...
TextureCube::TextureCube(uint32_t name) : Texture(name, TYPE_CUBE_MAP)
{
}
