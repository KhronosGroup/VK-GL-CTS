    ...
    , m_textureRenderer(log, renderContext)
{
    ...
    if (m_config.useDummyData)
    {
        ...
        m_dummyData   = vector<uint8_t>(dummySize);
    }
    else if (m_config.write)
        m_dummyData = vector<uint8_t>(128);
}
...
MemoryStressCase::MemoryStressCase(tcu::TestContext &ctx, glu::RenderContext &renderContext, uint32_t objectTypes,
                                   ...
                                   bool write, bool use, bool useDummyData, bool clearAfterOOM, const char *name,
                                   const char *desc)
