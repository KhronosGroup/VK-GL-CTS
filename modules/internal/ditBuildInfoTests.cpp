...
static const char *getCpuName(int cpu)
{
    ...
    case DE_CPU_RISCV_32:
        return "DE_CPU_RISCV_32";
...
public:
    ...
    IterateResult iterate(void)
    {
        ...
        m_testCtx.setTestResult(isOk ? QP_TEST_RESULT_PASS : QP_TEST_RESULT_FAIL, isOk ? "Pass" : "No enum name found");
        return STOP;
    }
...
public:
    ...
    IterateResult iterate(void)
    {
        const uint16_t multiByte = (uint16_t)0x0102;

#if DE_ENDIANNESS == DE_BIG_ENDIAN
        const bool isOk = *((const uint8_t *)&multiByte) == (uint8_t)0x01;
#elif DE_ENDIANNESS == DE_LITTLE_ENDIAN
        const bool isOk = *((const uint8_t *)&multiByte) == (uint8_t)0x02;
#endif

        m_testCtx.getLog() << TestLog::Message << "Verifying DE_ENDIANNESS matches actual behavior"
                           << TestLog::EndMessage;
