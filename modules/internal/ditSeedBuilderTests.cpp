...
{
public:
    SeedBuilderTest(tcu::TestContext &testCtx, const T &value, uint32_t seed, const char *name, const char *description)
...
public:
    ...
    {
        TestLog &log     = m_testCtx.getLog();
        const uint32_t a = 77740203u;
        const uint32_t b = 3830824200u;
...
{
public:
    SeedBuilderTests(tcu::TestContext &testCtx) : tcu::TestCaseGroup(testCtx, "seed_builder", "Seed builder tests.")
    {
    }
    ...
    void init(void)
    {
        ...
        addChild(new SeedBuilderTest<int8_t>(m_testCtx, 0, 62533730u, "int8_zero", "Seed from int8 zero."));
        addChild(new SeedBuilderTest<int8_t>(m_testCtx, 1, 93914869u, "int8_one", "Seed from int8 one."));
        addChild(new SeedBuilderTest<int8_t>(m_testCtx, -1, 115002165u, "int8_minus_one", "Seed from int8 minus one."));

        addChild(new SeedBuilderTest<int16_t>(m_testCtx, 0, 133071403u, "int16_zero", "Seed from int16 zero."));
        addChild(new SeedBuilderTest<int16_t>(m_testCtx, 1, 57421642u, "int16_one", "Seed from int16 one."));
        addChild(
            new SeedBuilderTest<int16_t>(m_testCtx, -1, 74389771u, "int16_minus_one", "Seed from int16 minus one."));

        addChild(new SeedBuilderTest<int32_t>(m_testCtx, 0, 75951701u, "int32_zero", "Seed from int32 zero."));
        addChild(new SeedBuilderTest<int32_t>(m_testCtx, 1, 95780822u, "int32_one", "Seed from int32 one."));
        addChild(
            new SeedBuilderTest<int32_t>(m_testCtx, -1, 73949483u, "int32_minus_one", "Seed from int32 minus one."));

        addChild(new SeedBuilderTest<uint8_t>(m_testCtx, 0, 3623298u, "uint8_zero", "Seed from uint8 zero."));
        addChild(new SeedBuilderTest<uint8_t>(m_testCtx, 1, 102006549u, "uint8_one", "Seed from uint8 one."));
        addChild(new SeedBuilderTest<uint8_t>(m_testCtx, 255, 89633493u, "uint8_max", "Seed from uint8 max."));

        addChild(new SeedBuilderTest<uint16_t>(m_testCtx, 0, 78413740u, "uint16_zero", "Seed from uint16 zero."));
        addChild(new SeedBuilderTest<uint16_t>(m_testCtx, 1, 3068621u, "uint16_one", "Seed from uint16 one."));
        addChild(new SeedBuilderTest<uint16_t>(m_testCtx, 65535, 120448140u, "uint16_max", "Seed from uint16 max."));

        addChild(new SeedBuilderTest<uint32_t>(m_testCtx, 0u, 41006057u, "uint32_zero", "Seed from uint32 zero."));
        addChild(new SeedBuilderTest<uint32_t>(m_testCtx, 1u, 54665834u, "uint32_one", "Seed from uint32 one."));
        addChild(
            new SeedBuilderTest<uint32_t>(m_testCtx, 4294967295u, 43990167u, "uint32_max", "Seed from uint32 max."));

        addChild(new SeedBuilderTest<float>(m_testCtx, 0.0f, 41165361u, "float_zero", "Seed from float zero."));
        addChild(new SeedBuilderTest<float>(m_testCtx, -0.0f, 112541574u, "float_negative_zero",
                                            "Seed from float negative zero."));
        addChild(new SeedBuilderTest<float>(m_testCtx, 1.0f, 44355905u, "float_one", "Seed from float one."));
        addChild(new SeedBuilderTest<float>(m_testCtx, -1.0f, 107334902u, "float_negative_one",
                                            "Seed from float negative one."));

        addChild(new SeedBuilderTest<double>(m_testCtx, 0.0, 133470681u, "double_zero", "Seed from double zero."));
        addChild(new SeedBuilderTest<double>(m_testCtx, -0.0, 53838958u, "double_negative_zero",
                                             "Seed from double negative zero."));
        addChild(new SeedBuilderTest<double>(m_testCtx, 1.0, 16975104u, "double_one", "Seed from double one."));
        addChild(new SeedBuilderTest<double>(m_testCtx, -1.0, 96606391u, "double_negative_one",
                                             "Seed from double negative one."));
        ...
        addChild(new SeedBuilderTest<IVec4>(m_testCtx, IVec4(1, 2, 3, 4), 6202236u, "ivec4_1_2_3_4",
                                            "Seed from (1, 2, 3, 4) vector."));
        addChild(new SeedBuilderTest<IVec4>(m_testCtx, IVec4(4, 3, 2, 1), 26964618u, "ivec4_4_3_2_1",
