...
static std::string specializeShader(const std::string &shaderSource, const glu::ContextType &contextType)
{
    ...
    args["GLSL_OES_TEXTURE_STORAGE_MULTISAMPLE"] =
        supportsES32 ? "" : "#extension GL_OES_texture_storage_multisample_2d_array : require\n";
...
LayeredRenderCase::IterateResult LayeredRenderCase::iterate(void)
{
    ...
    if (m_iteration == 1)
    {
        if (m_test == TEST_LAYER_PROVOKING_VERTEX)
        {
            ...
            GLU_EXPECT_NO_ERROR(m_context.getRenderContext().getFunctions().getError(),
                                "getInteger(GL_LAYER_PROVOKING_VERTEX)");
            ...
            if (state != GL_FIRST_VERTEX_CONVENTION && state != GL_LAST_VERTEX_CONVENTION &&
...
LayerProvokingVertexQueryCase::IterateResult LayerProvokingVertexQueryCase::iterate(void)
{
    ...
    if (!state.isUndefined())
    {
        ...
        if (state.getIntAccess() != GL_FIRST_VERTEX_CONVENTION && state.getIntAccess() != GL_LAST_VERTEX_CONVENTION &&
...
void GeometryShaderTests::init(void)
{
    ...
    // query test
    {
        ...
        queryGroup->addChild(
            new GeometryShaderInputQueryCase(m_context, "geometry_linked_input_type", "GL_GEOMETRY_LINKED_INPUT_TYPE"));
