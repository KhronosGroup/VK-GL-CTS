...
void texture_size_invalid_sampler(NegativeTestContext &ctx)
{
    ...
    for (int shaderNdx = 0; shaderNdx < DE_LENGTH_OF_ARRAY(s_shaders); ++shaderNdx)
    {
        if (ctx.isShaderSupported(s_shaders[shaderNdx]))
        {
            ...
            {
                if (contextSupports(ctx.getRenderContext().getType(), glu::ApiType::es(3, 2)) ||
                    ctx.isExtensionSupported(getDataTypeExtension(s_samplerTypes[dataTypeNdx])))
                {
...
void texture_invalid_p(NegativeTestContext &ctx)
{
    ...
    for (int shaderNdx = 0; shaderNdx < DE_LENGTH_OF_ARRAY(s_shaders); ++shaderNdx)
    {
        ...
        // SAMPLER_CUBE_ARRAY
        if (contextSupports(ctx.getRenderContext().getType(), glu::ApiType::es(3, 2)) ||
            ctx.isExtensionSupported(getDataTypeExtension(glu::TYPE_SAMPLER_CUBE_ARRAY)))
        {
            if (s_floatTypes[dataTypeNdx] != glu::TYPE_FLOAT_VEC4)
            {
                {
...
void texture_grad(NegativeTestContext &ctx)
{
    TCU_CHECK_AND_THROW(NotSupportedError,
                        ctx.isExtensionSupported("GL_EXT_texture_cube_map_array") ||
                                         ...
                                         glu::DataType interpolantDataType, glu::DataType secondArgumentDataType)
{
    ...
    source << (supportsES32 ? glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_320_ES) :
                              glu::getGLSLVersionDeclaration(glu::GLSL_VERSION_310_ES))
           << "\n"
...
void interpolate_at_centroid(NegativeTestContext &ctx)
{
    TCU_CHECK_AND_THROW(NotSupportedError,
                        contextSupports(ctx.getRenderContext().getType(), glu::ApiType::es(3, 2)) ||
                            ctx.isExtensionSupported("GL_OES_shader_multisample_interpolation"),
