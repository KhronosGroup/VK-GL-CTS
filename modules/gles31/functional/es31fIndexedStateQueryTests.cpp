...
void IndexedStateQueryTests::init(void)
{
    ...
    static const QueryType vec4Verifiers[] = {QUERY_INDEXED_BOOLEAN_VEC4, QUERY_INDEXED_INTEGER_VEC4,
...
#define FOR_EACH_VEC4_VERIFIER(X)                                                             \
...

    {
        ...
        const char *verifierSuffix = getVerifierSuffix(verifier);
        this->addChild(
            new EnableBlendCase(m_context, (std::string() + "blend_" + verifierSuffix).c_str(), "BLEND", verifier));
    }
