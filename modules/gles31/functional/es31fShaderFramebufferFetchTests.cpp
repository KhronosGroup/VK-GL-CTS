                                       ...
                                       const tcu::TextureFormat &readPixelsFormat)
{
    ...
    return tcu::Vec4(3.0f) / ((tcu::Vector<uint64_t, 4>(1) << (tcu::min(srcBits, readBits).cast<uint64_t>())) -
...
tcu::TextureLevel TextureFormatTestCase::genReferenceTexture(const tcu::Vec4 &fbColor, const tcu::Vec4 &uniformColor)
{
    ...
    {
        tcu::clear(reference.getAccess(),
                   tcu::clamp(fbColor.asInt() + uniformColor.asInt(), formatMinValue.asInt(), formatMaxValue.asInt()));
    }
    else
    {
