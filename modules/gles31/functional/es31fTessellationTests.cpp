...
static void checkTessellationSupport(Context &context)
{
    const bool supportsES32 = glu::contextSupports(context.getRenderContext().getType(), glu::ApiType::es(3, 2));
    TCU_CHECK_AND_THROW(NotSupportedError,
                        supportsES32 || context.getContextInfo().isExtensionSupported("GL_EXT_tessellation_shader"),
...
static std::string specializeShader(Context &context, const char *code)
{
    ...
    specializationMap["TESSELLATION_SHADER_REQUIRE"] =
        supportsES32 ? "" : "#extension GL_EXT_tessellation_shader : require";
...
TessellationTests::TessellationTests(Context &context) : TestCaseGroup(context, "tessellation", "Tessellation Tests")
{
}
