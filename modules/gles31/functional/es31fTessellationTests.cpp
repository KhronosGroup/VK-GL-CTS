...
ReferencedByTessellationQueryCase::IterateResult ReferencedByTessellationQueryCase::iterate(void)
{
    ...
    {
        const uint32_t props[] = {GL_REFERENCED_BY_TESS_CONTROL_SHADER, GL_REFERENCED_BY_TESS_EVALUATION_SHADER};

        {
            ...
            {
                gl.glGetProgramResourceiv(program.getProgram(), GL_UNIFORM, resourcePos, 2, props, 2, &length,
                                          referenced);
                m_testCtx.getLog() << tcu::TestLog::Message << "Query " << glu::getProgramResourcePropertyStr(props[0])
                                   << ", " << glu::getProgramResourcePropertyStr(props[1]) << ", got " << length
                                   << " value(s), value[0] = " << glu::getBooleanStr(referenced[0])
                                   << ", value[1] = " << glu::getBooleanStr(referenced[1]) << tcu::TestLog::EndMessage;
                ...
                {
                    uint32_t count = referenced[0] + referenced[1];
                    if (count < 1)
