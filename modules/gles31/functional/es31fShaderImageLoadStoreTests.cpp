...
static inline IVec3 defaultImageSize(TextureType type)
{
    ...
    case TEXTURETYPE_BUFFER:
        return IVec3(64, 1, 1);
    case TEXTURETYPE_2D:
        return IVec3(64, 64, 1);
    case TEXTURETYPE_CUBE:
        return IVec3(64, 64, 1);
    case TEXTURETYPE_3D:
        return IVec3(64, 64, 8);
...
{
    return type == TextureFormat::UNSIGNED_INT8 || type == TextureFormat::UNSIGNED_INT16 ||
           type == TextureFormat::UNSIGNED_INT32;
}

static inline bool isFormatTypeSignedInteger(TextureFormat::ChannelType type)
{
    return type == TextureFormat::SIGNED_INT8 || type == TextureFormat::SIGNED_INT16 ||
...
{
    return type == TextureFormat::UNORM_INT8 || type == TextureFormat::UNORM_INT16 ||
           type == TextureFormat::UNORM_INT32;
}

static inline bool isFormatTypeSnorm(TextureFormat::ChannelType type)
{
    return type == TextureFormat::SNORM_INT8 || type == TextureFormat::SNORM_INT16 ||
...
static inline bool isFormatSupportedForTextureBuffer(const TextureFormat &format)
{
    ...
    case TextureFormat::RGB:
        return format.type == TextureFormat::FLOAT || format.type == TextureFormat::SIGNED_INT32 ||
    ...
    case TextureFormat::RGBA:
        return format.type == TextureFormat::UNORM_INT8 || format.type == TextureFormat::HALF_FLOAT ||
               format.type == TextureFormat::FLOAT || format.type == TextureFormat::SIGNED_INT8 ||
               format.type == TextureFormat::SIGNED_INT16 || format.type == TextureFormat::SIGNED_INT32 ||
               format.type == TextureFormat::UNSIGNED_INT8 || format.type == TextureFormat::UNSIGNED_INT16 ||
               format.type == TextureFormat::UNSIGNED_INT32;
...
static inline string getShaderSamplerOrImageType(TextureFormat::ChannelType formatType, TextureType textureType,
                                                 bool isSampler)
{
...
static inline uint32_t getGLTextureTarget(TextureType texType)
{
    switch (texType)
    {
    case TEXTURETYPE_BUFFER:
                                              ...
                                              const RenderContext &renderContext)
{
    if (type == TEXTURETYPE_BUFFER && !contextInfo.isExtensionSupported(s_texBufExtString) &&
        !supportsES32orGL45(renderContext))
...
{
    return op == ATOMIC_OPERATION_ADD || op == ATOMIC_OPERATION_MIN || op == ATOMIC_OPERATION_MAX ||
...
{
    static const tcu::CubeFace faces[6] = {tcu::CUBEFACE_POSITIVE_X, tcu::CUBEFACE_NEGATIVE_X,
...
{
public:
    UniformAccessLogger(const glw::Functions &gl, TestLog &log, uint32_t programGL)
        : m_gl(gl)
        , m_log(log)
...
public:
    ...
    TextureType getImageType(void) const
    {
        return m_type;
    }
    ...
    UVec4 getPixelUint(int x, int y, int z) const
    {
        return getPixelInt(x, y, z).asUint();
    }
    ...
    PixelBufferAccess getSliceAccess(int slice)
    {
        return getSliceAccessInternal(slice);
    }
    PixelBufferAccess getCubeFaceAccess(tcu::CubeFace face)
    {
        return getCubeFaceAccessInternal(face);
    }
    ...
    ConstPixelBufferAccess getSliceAccess(int slice) const
    {
        return getSliceAccessInternal(slice);
    }
    ConstPixelBufferAccess getCubeFaceAccess(tcu::CubeFace face) const
    {
        return getCubeFaceAccessInternal(face);
    }
...
LayeredImage::LayeredImage(TextureType type, const TextureFormat &format, int w, int h, int d)
    ...
    , m_size(w, h, d)
    , m_format(format)
    , m_texBuffer(type == TEXTURETYPE_BUFFER ? SharedPtr<tcu::Texture1D>(newOneLevelTexture1D(format, w)) :
                                               SharedPtr<tcu::Texture1D>())
    , m_tex2D(type == TEXTURETYPE_2D ? SharedPtr<tcu::Texture2D>(newOneLevelTexture2D(format, w, h)) :
                                       SharedPtr<tcu::Texture2D>())
    , m_texCube(type == TEXTURETYPE_CUBE ? SharedPtr<tcu::TextureCube>(newOneLevelTextureCube(format, w)) :
                                           SharedPtr<tcu::TextureCube>())
    , m_tex3D(type == TEXTURETYPE_3D ? SharedPtr<tcu::Texture3D>(newOneLevelTexture3D(format, w, h, d)) :
                                       SharedPtr<tcu::Texture3D>())
    , m_tex2DArray(type == TEXTURETYPE_2D_ARRAY ?
                       SharedPtr<tcu::Texture2DArray>(newOneLevelTexture2DArray(format, w, h, d)) :
                       SharedPtr<tcu::Texture2DArray>())
{
    DE_ASSERT(m_size.z() == 1 || m_type == TEXTURETYPE_3D || m_type == TEXTURETYPE_2D_ARRAY);

    DE_ASSERT(m_size.y() == 1 || m_type == TEXTURETYPE_2D || m_type == TEXTURETYPE_CUBE || m_type == TEXTURETYPE_3D ||
...
{
    const PixelBufferAccess access =
        m_type == TEXTURETYPE_BUFFER   ? m_texBuffer->getLevel(0) :
...
{
    const ConstPixelBufferAccess access =
        m_type == TEXTURETYPE_CUBE ? getCubeFaceAccess(glslImageFuncZToCubeFace(z)) : getAccess();
...
{
    DE_ASSERT(m_type == TEXTURETYPE_BUFFER || m_type == TEXTURETYPE_2D || m_type == TEXTURETYPE_3D ||
              m_type == TEXTURETYPE_2D_ARRAY);
...
static void setTextureStorage(glu::CallLogWrapper &glLog, TextureType imageType, uint32_t internalFormat,
                              const IVec3 &imageSize, uint32_t textureBufGL)
{
    const uint32_t textureTarget = getGLTextureTarget(imageType);
...
static void uploadTexture(glu::CallLogWrapper &glLog, const LayeredImage &src, uint32_t textureBufGL)
{
    const uint32_t internalFormat            = glu::getInternalFormat(src.getFormat());
    ...
    {
        glLog.glBindBuffer(GL_TEXTURE_BUFFER, textureBufGL);
        glLog.glBufferData(GL_TEXTURE_BUFFER, src.getFormat().getPixelSize() * imageSize.x(),
                           src.getAccess().getDataPtr(), GL_STATIC_DRAW);
    }
    else if (src.getImageType() == TEXTURETYPE_2D)
        glLog.glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, imageSize.x(), imageSize.y(), transferFormat.format,
                              transferFormat.dataType, src.getAccess().getDataPtr());
    else if (src.getImageType() == TEXTURETYPE_CUBE)
    {
        for (int faceI = 0; faceI < tcu::CUBEFACE_LAST; faceI++)
        {
            ...
            glLog.glTexSubImage2D(cubeFaceToGLFace(face), 0, 0, 0, imageSize.x(), imageSize.y(), transferFormat.format,
                                  transferFormat.dataType, src.getCubeFaceAccess(face).getDataPtr());
        }
    }
    else
    {
        DE_ASSERT(src.getImageType() == TEXTURETYPE_3D || src.getImageType() == TEXTURETYPE_2D_ARRAY);
        const uint32_t textureTarget = getGLTextureTarget(src.getImageType());
        glLog.glTexSubImage3D(textureTarget, 0, 0, 0, 0, imageSize.x(), imageSize.y(), imageSize.z(),
                              transferFormat.format, transferFormat.dataType, src.getAccess().getDataPtr());
    }
}
...
static void readPixelsRGBAInteger32(const PixelBufferAccess &dst, int originX, int originY, glu::CallLogWrapper &glLog)
{
    ...
    if (isFormatTypeUnsignedInteger(dst.getFormat().type))
    {
        ...
        glLog.glReadPixels(originX, originY, dst.getWidth(), dst.getHeight(), GL_RGBA_INTEGER, GL_UNSIGNED_INT,
                           &data[0]);
...
static void setTexParameteri(glu::CallLogWrapper &glLog, uint32_t target)
{
...
static bool readIntegerTextureViaFBOAndVerify(const RenderContext &renderCtx, glu::CallLogWrapper &glLog,
                                              uint32_t textureGL, TextureType textureType,
                                              const TextureFormat &textureFormat, const IVec3 &textureSize,
                                              const ImageLayerVerifier &verifyLayer)
{
    ...
    const tcu::ScopedLogSection section(
        log, "Verification", "Result verification (bind texture layer-by-layer to FBO, read with glReadPixels())");

    const int numSlicesOrFaces     = textureType == TEXTURETYPE_CUBE ? 6 : textureSize.z();
    const uint32_t textureTargetGL = getGLTextureTarget(textureType);
    ...
    for (int sliceOrFaceNdx = 0; sliceOrFaceNdx < numSlicesOrFaces; sliceOrFaceNdx++)
    {
        if (textureType == TEXTURETYPE_CUBE)
            glLog.glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                                         cubeFaceToGLFace(glslImageFuncZToCubeFace(sliceOrFaceNdx)), textureGL, 0);
...
//! \note Not for buffer textures.
static bool readFloatOrNormTextureWithLookupsAndVerify(const RenderContext &renderCtx, glu::CallLogWrapper &glLog,
                                                       uint32_t textureGL, TextureType textureType,
                                                       const TextureFormat &textureFormat, const IVec3 &textureSize,
                                                       const ImageLayerVerifier &verifyLayer)
{
    ...
    const std::string glslVersionDeclaration =
        getGLSLVersionDeclaration(glu::getContextTypeGLSLVersion(renderCtx.getType()));

    const glu::ShaderProgram program(renderCtx,
                                     glu::ProgramSources() << glu::ComputeSource(
                                         ...
                                         "{\n"
                                         "    vec4 color[" +
                                         toString(textureSize.x() * textureSize.y()) +
                                         "];\n"
                                         "} sb_out;\n"
                                         "\n"
                                         "precision highp " +
                                         ...
                                         toString(textureSize.x()) +
                                         ");\n"
                                         "    highp float t = (float(gy) + 0.5) / float(" +
                                         toString(textureType == TEXTURETYPE_CUBE ? textureSize.x() : textureSize.y()) +
                                         ");\n"
...

    {
        ...
        // Setup output buffer.
        {
            const uint32_t blockIndex =
                glLog.glGetProgramResourceIndex(program.getProgram(), GL_SHADER_STORAGE_BLOCK, "Output");
            const int blockSize = glu::getProgramResourceInt(renderCtx.getFunctions(), program.getProgram(),
                                                             GL_SHADER_STORAGE_BLOCK, blockIndex, GL_BUFFER_DATA_SIZE);
        ...
        // Dispatch one layer at a time, read back and verify.
        {
            ...
            const uint32_t valueIndex =
                glLog.glGetProgramResourceIndex(program.getProgram(), GL_BUFFER_VARIABLE, "Output.color");
            const glu::InterfaceVariableInfo valueInfo = glu::getProgramInterfaceVariableInfo(
                renderCtx.getFunctions(), program.getProgram(), GL_BUFFER_VARIABLE, valueIndex);

            TCU_CHECK(valueInfo.arraySize == (uint32_t)(textureSize.x() * textureSize.y()));
            ...
            for (int sliceOrFaceNdx = 0; sliceOrFaceNdx < numSlicesOrFaces; sliceOrFaceNdx++)
            {
                ...
                {
                    log << TestLog::Message << "// Note: mapping buffer and reading color values written"
                        << TestLog::EndMessage;

                    const BufferMemMap bufMap(renderCtx.getFunctions(), GL_SHADER_STORAGE_BUFFER, 0, blockSize,
                                              GL_MAP_READ_BIT);
                    ...
                    for (int y = 0; y < textureSize.y(); y++)
                        for (int x = 0; x < textureSize.x(); x++)
                        {
                            ...
                            const float *const clrData =
                                (const float *)((const uint8_t *)bufMap.getPtr() + valueInfo.offset +
                                                valueInfo.arrayStride * ndx);

                            switch (textureFormat.order)
                            {
                            case TextureFormat::R:
                                resultSliceAccess.setPixel(Vec4(clrData[0]), x, y);
                                break;
                            case TextureFormat::RGBA:
                                resultSliceAccess.setPixel(Vec4(clrData[0], clrData[1], clrData[2], clrData[3]), x, y);
                                break;
                            default:
                                DE_ASSERT(false);
                            }
...
static bool readBufferTextureWithMappingAndVerify(const RenderContext &renderCtx, glu::CallLogWrapper &glLog,
                                                  uint32_t bufferGL, const TextureFormat &textureFormat, int imageSize,
                                                  const ImageLayerVerifier &verifyLayer)
{
    ...
    const tcu::ScopedLogSection section(glLog.getLog(), "Verification",
                                        "Result verification (read texture's buffer with a mapping)");
...
static bool readTextureAndVerify(const RenderContext &renderCtx, glu::CallLogWrapper &glLog, uint32_t textureGL,
                                 uint32_t bufferGL, TextureType textureType, const TextureFormat &textureFormat,
                                 const IVec3 &imageSize, const ImageLayerVerifier &verifyLayer)
{
    if (textureType == TEXTURETYPE_BUFFER)
        return readBufferTextureWithMappingAndVerify(renderCtx, glLog, bufferGL, textureFormat, imageSize.x(),
                                                     verifyLayer);
    else
        return isFormatTypeInteger(textureFormat.type) ?
                   readIntegerTextureViaFBOAndVerify(renderCtx, glLog, textureGL, textureType, textureFormat, imageSize,
                                                     verifyLayer) :
                   readFloatOrNormTextureWithLookupsAndVerify(renderCtx, glLog, textureGL, textureType, textureFormat,
                                                              imageSize, verifyLayer);
}
...
public:
    ImageLayerComparer(const LayeredImage &reference,
        ...
        , m_relevantRegion(relevantRegion.x() > 0 && relevantRegion.y() > 0 ? relevantRegion :
                                                                              reference.getSize().swizzle(0, 1))
    {
    }
    ...
    bool operator()(TestLog &log, const tcu::ConstPixelBufferAccess &resultSlice, int sliceOrFaceNdx) const
    {
        ...
        const ConstPixelBufferAccess referenceSlice =
            tcu::getSubregion(isCube ? m_reference.getCubeFaceAccess(glslImageFuncZToCubeFace(sliceOrFaceNdx)) :
        ...
        const string comparisonDesc =
            "Image Comparison, " +
            (isCube ?
                 "face " + string(glu::getCubeMapFaceName(cubeFaceToGLFace(glslImageFuncZToCubeFace(sliceOrFaceNdx)))) :
                 "slice " + toString(sliceOrFaceNdx));

        if (isFormatTypeInteger(m_reference.getFormat().type))
            return tcu::intThresholdCompare(log, comparisonName.c_str(), comparisonDesc.c_str(), referenceSlice,
                                            resultSlice, UVec4(0), tcu::COMPARE_LOG_RESULT);
        else
            return tcu::floatThresholdCompare(log, comparisonName.c_str(), comparisonDesc.c_str(), referenceSlice,
                                              resultSlice, Vec4(0.01f), tcu::COMPARE_LOG_RESULT);
    }
...
public:
    enum CaseFlag
    {
        CASEFLAG_SINGLE_LAYER_BIND =
            1
            << 0 //!< If given, glBindImageTexture() is called with GL_FALSE <layered> argument, and for each layer the compute shader is separately dispatched.
    };

    ImageStoreCase(Context &context, const char *name, const char *description, const TextureFormat &format,
                   TextureType textureType, uint32_t caseFlags = 0)
    ...
    void init(void)
    {
        checkTextureTypeExtensions(m_context.getContextInfo(), m_textureType, m_context.getRenderContext());
    }
...
ImageStoreCase::IterateResult ImageStoreCase::iterate(void)
{
    ...
    const uint32_t internalFormatGL = glu::getInternalFormat(m_format);
    const uint32_t textureTargetGL  = getGLTextureTarget(m_textureType);
    ...
    if (m_textureType == TEXTURETYPE_BUFFER)
        log << TestLog::Message << "// Created a buffer for the texture (name " << *textureBuf << ")"
            << TestLog::EndMessage;
...

    {
        ...
        const string colorBaseExpr        = string(isUintFormat ? "u" :
                                                   isIntFormat  ? "i" :
                                                                  "") +
                                     "vec4(gx^gy^gz, "
                                     "(" +
                                     toString(imageSize.x() - 1) +
                                     "-gx)^gy^gz, "
                                     "gx^(" +
                                     toString(imageSize.y() - 1) +
                                     "-gy)^gz, "
                                     "(" +
        ...
        const std::string glslVersionDeclaration =
            glu::getGLSLVersionDeclaration(glu::getContextTypeGLSLVersion(renderCtx.getType()));
        ...
        const glu::ShaderProgram program(
            ...
            glu::ProgramSources() << glu::ComputeSource(
                ...
                "layout (local_size_x = 1, local_size_y = 1, local_size_z = 1) in;\n"
                "layout (" +
                shaderImageFormatStr + ", binding=0) writeonly uniform " + shaderImageTypeStr + " u_image;\n" +
                ...
                "    int gy = int(gl_GlobalInvocationID.y);\n"
                "    int gz = " +
                (m_singleLayerBind ? "u_layerNdx" : "int(gl_GlobalInvocationID.z)") + ";\n" +
                (shaderImageType == TEXTURETYPE_BUFFER ? "    imageStore(u_image, gx, " + colorExpr + ");\n" :
                 shaderImageType == TEXTURETYPE_2D ? "    imageStore(u_image, ivec2(gx, gy), " + colorExpr + ");\n" :
                 shaderImageType == TEXTURETYPE_3D || shaderImageType == TEXTURETYPE_CUBE ||
                         shaderImageType == TEXTURETYPE_2D_ARRAY ?
    ...
    // Create reference, read texture and compare to reference.
    {
        ...
        for (int z = 0; z < numSlicesOrFaces; z++)
            for (int y = 0; y < imageSize.y(); y++)
                ...
                {
                    const IVec4 color(x ^ y ^ z, (imageSize.x() - 1 - x) ^ y ^ z, x ^ (imageSize.y() - 1 - y) ^ z,
                                      (imageSize.x() - 1 - x) ^ (imageSize.y() - 1 - y) ^ z);
        ...
        const bool compareOk = readTextureAndVerify(renderCtx, glLog, *texture, *textureBuf, m_textureType, m_format,
                                                    imageSize, ImageLayerComparer(reference));
...
public:
    ...
    ImageLoadAndStoreCase(Context &context, const char *name, const char *description, const TextureFormat &format,
                          TextureType textureType, uint32_t caseFlags = 0)
        : TestCase(context, name, description)
    ...
    ImageLoadAndStoreCase(Context &context, const char *name, const char *description,
                          const TextureFormat &textureFormat, const TextureFormat &imageFormat, TextureType textureType,
                          uint32_t caseFlags = 0)
        : TestCase(context, name, description)
...
void ImageLoadAndStoreCase::replaceBadFloatReinterpretValues(LayeredImage &image, const TextureFormat &imageFormat)
{
    ...
    for (int z = 0; z < numSlicesOrFaces; z++)
    {
        const PixelBufferAccess sliceAccess = image.getImageType() == TEXTURETYPE_CUBE ?
                                                  image.getCubeFaceAccess((tcu::CubeFace)z) :
                                                  image.getSliceAccess(z);
        ...
        for (int y = 0; y < imageSize.y(); y++)
            for (int x = 0; x < imageSize.x(); x++)
            {
                void *const pixelData = (uint8_t *)data + y * rowPitch + x * pixelSize;

                for (int c = 0; c < imageNumChannels; c++)
                {
                    void *const channelData = (uint8_t *)pixelData + c * pixelSize / imageNumChannels;
...
ImageLoadAndStoreCase::IterateResult ImageLoadAndStoreCase::iterate(void)
{
    ...
    TestLog &log(m_testCtx.getLog());
    glu::CallLogWrapper glLog(renderCtx.getFunctions(), log);
    const uint32_t textureInternalFormatGL = glu::getInternalFormat(m_textureFormat);
    const uint32_t imageInternalFormatGL   = glu::getInternalFormat(m_imageFormat);
    const uint32_t textureTargetGL         = getGLTextureTarget(m_textureType);
    const IVec3 &imageSize                 = defaultImageSize(m_textureType);
    const int maxImageDimension            = de::max(imageSize.x(), de::max(imageSize.y(), imageSize.z()));
    ...
    log << TestLog::Message << "// Created 2 textures (names " << *texture0 << " and " << *texture1 << ")"
        << TestLog::EndMessage;
    if (m_textureType == TEXTURETYPE_BUFFER)
        log << TestLog::Message << "// Created buffers for the textures (names " << *texture0Buf << " and "
            << *texture1Buf << ")" << TestLog::EndMessage;
    ...
    if (m_imageFormat.type == TextureFormat::HALF_FLOAT && m_textureFormat.type != TextureFormat::HALF_FLOAT)
        replaceBadFloatReinterpretValues<TextureFormat::HALF_FLOAT, tcu::Float16, uint16_t>(reference, m_imageFormat);
    else if (m_imageFormat.type == TextureFormat::FLOAT && m_textureFormat.type != TextureFormat::FLOAT)
        replaceBadFloatReinterpretValues<TextureFormat::FLOAT, tcu::Float32, uint32_t>(reference, m_imageFormat);
...

    {
        ...
        const glu::ShaderProgram program(
            ...
            glu::ProgramSources() << glu::ComputeSource(
                ...
                "layout (local_size_x = 1, local_size_y = 1, local_size_z = 1) in;\n"
                "layout (" +
                shaderImageFormatStr + ", binding=0) " + maybeRestrict + " readonly uniform " + shaderImageTypeStr +
                " u_image0;\n"
                "layout (" +
                shaderImageFormatStr + ", binding=1) " + maybeRestrict + " writeonly uniform " + shaderImageTypeStr +
                " u_image1;\n"
                "\n"
                "void main (void)\n"
                "{\n" +
                (shaderImageType == TEXTURETYPE_BUFFER ? "    int pos = int(gl_GlobalInvocationID.x);\n"
                                                         "    imageStore(u_image1, pos, imageLoad(u_image0, " +
                                                             toString(imageSize.x() - 1) + "-pos));\n" :
                 shaderImageType == TEXTURETYPE_2D ? "    ivec2 pos = ivec2(gl_GlobalInvocationID.xy);\n"
                                                     "    imageStore(u_image1, pos, imageLoad(u_image0, ivec2(" +
                                                         toString(imageSize.x() - 1) + "-pos.x, pos.y)));\n" :
                 shaderImageType == TEXTURETYPE_3D || shaderImageType == TEXTURETYPE_CUBE ||
                         shaderImageType == TEXTURETYPE_2D_ARRAY ?
                                                     "    ivec3 pos = ivec3(gl_GlobalInvocationID);\n"
                                                     "    imageStore(u_image1, pos, imageLoad(u_image0, ivec3(" +
                                                         toString(imageSize.x() - 1) + "-pos.x, pos.y, pos.z)));\n" :
                                                     deFatalStr("Invalid TextureType")) +
                "}\n"));
        ...
        if (m_textureFormat != m_imageFormat)
        {
            ...
            for (int z = 0; z < numSlicesOrFaces; z++)
            {
                const PixelBufferAccess sliceAccess = m_textureType == TEXTURETYPE_CUBE ?
                                                          reference.getCubeFaceAccess((tcu::CubeFace)z) :
                                                          reference.getSliceAccess(z);
                const int rowPitch                  = sliceAccess.getRowPitch();
                void *const data                    = sliceAccess.getDataPtr();
    ...
    const bool compareOk = readTextureAndVerify(renderCtx, glLog, *texture1, *texture1Buf, m_textureType,
                                                m_textureFormat, imageSize, ImageLayerComparer(reference));
...
enum AtomicOperationCaseType
{
    ATOMIC_OPERATION_CASE_TYPE_END_RESULT =
        0, //!< Atomic case checks the end result of the operations, and not the return values.
...
{
public:
    BinaryAtomicOperationCase(Context &context, const char *name, const char *description, const TextureFormat &format,
                              TextureType imageType, AtomicOperation operation, AtomicOperationCaseType caseType)
        : TestCase(context, name, description)
        , m_format(format)
        ...
        , m_caseType(caseType)
    {
        DE_ASSERT(m_format == TextureFormat(TextureFormat::R, TextureFormat::UNSIGNED_INT32) ||
                  ...
                  (m_format == TextureFormat(TextureFormat::R, TextureFormat::FLOAT) &&
                   m_operation == ATOMIC_OPERATION_EXCHANGE));
...
private:
    ...
    static int getOperationInitialValue(
        AtomicOperation op); //!< Appropriate value with which to initialize the texture.
    ...
    static string getAtomicFuncArgumentShaderStr(AtomicOperation op, const string &x, const string &y, const string &z,
...
int BinaryAtomicOperationCase::getAtomicFuncArgument(AtomicOperation op, const IVec3 &invocationID,
                                                     const IVec2 &dispatchSizeXY)
{
...
string BinaryAtomicOperationCase::getAtomicFuncArgumentShaderStr(AtomicOperation op, const string &x, const string &y,
                                                                 const string &z, const IVec2 &dispatchSizeXY)
{
    switch (op)
    {
    // \note Fall-throughs.
    ...
    case ATOMIC_OPERATION_EXCHANGE:
        return "((" + z + "*" + toString(dispatchSizeXY.x()) + " + " + x + ")*" + toString(dispatchSizeXY.y()) + " + " +
               y + ")";
...
{
public:
    EndResultVerifier(AtomicOperation operation, TextureType imageType, int numInvocationsPerPixel)
        : m_operation(operation)
        , m_imageType(imageType)
        , m_numInvocationsPerPixel(numInvocationsPerPixel)
    {
    }
    ...
    bool operator()(TestLog &log, const ConstPixelBufferAccess &resultSlice, int sliceOrFaceNdx) const
    {
        ...
        log << TestLog::Image("EndResults" + toString(sliceOrFaceNdx),
                              "Result Values, " + (m_imageType == TEXTURETYPE_CUBE ?
                                                       "face " + string(glu::getCubeMapFaceName(cubeFaceToGLFace(
                                                                     glslImageFuncZToCubeFace(sliceOrFaceNdx)))) :
                                                       "slice " + toString(sliceOrFaceNdx)),
                              resultSlice);
        ...
        for (int y = 0; y < resultSlice.getHeight(); y++)
            for (int x = 0; x < resultSlice.getWidth(); x++)
            {
                ...
                if (isOrderIndependentAtomicOperation(m_operation))
                {
                    ...
                    {
                        log << TestLog::Message << "// Failure: end result at pixel " << IVec2(x, y)
                            << " of current layer is " << result.i << TestLog::EndMessage << TestLog::Message
                            << "// Note: relevant shader invocation global IDs are " << arrayStr(invocationGlobalIDs)
                            << TestLog::EndMessage << TestLog::Message
                            << "// Note: data expression values for the IDs are " << arrayStr(atomicArgs)
                            << TestLog::EndMessage << TestLog::Message << "// Note: reference value is " << reference
                            << TestLog::EndMessage;
                        return false;
                    }
                }
                else if (m_operation == ATOMIC_OPERATION_EXCHANGE)
                {
                    ...
                    if (!matchFound)
                    {
                        log << TestLog::Message << "// Failure: invalid value at pixel " << IVec2(x, y) << ": got "
                            << (isIntegerFormat ? toString(result.i) : toString(result.f)) << TestLog::EndMessage
                            << TestLog::Message << "// Note: expected one of " << arrayStr(atomicArgs)
                            << TestLog::EndMessage;
...
public:
    ...
    ReturnValueVerifier(AtomicOperation operation, TextureType imageType, const IVec2 &endResultImageLayerSize,
                        int numInvocationsPerPixel)
        : m_operation(operation)
        , m_imageType(imageType)
        , m_endResultImageLayerSize(endResultImageLayerSize)
        , m_numInvocationsPerPixel(numInvocationsPerPixel)
    {
    }
    ...
    bool operator()(TestLog &log, const ConstPixelBufferAccess &resultSlice, int sliceOrFaceNdx) const
    {
        ...
        DE_ASSERT(resultSlice.getWidth() == m_numInvocationsPerPixel * m_endResultImageLayerSize.x() &&
                  resultSlice.getHeight() == m_endResultImageLayerSize.y() && resultSlice.getDepth() == 1);

        log << TestLog::Image("ReturnValues" + toString(sliceOrFaceNdx),
                              "Per-Invocation Return Values, " +
                                  (m_imageType == TEXTURETYPE_CUBE ?
                                       "face " + string(glu::getCubeMapFaceName(
                                                     cubeFaceToGLFace(glslImageFuncZToCubeFace(sliceOrFaceNdx)))) :
                                       "slice " + toString(sliceOrFaceNdx)),
                              resultSlice);
...
private:
    ...
    template <typename T>
    bool checkPixel(TestLog &log, const ConstPixelBufferAccess &resultSlice, int x, int y, int sliceOrFaceNdx,
                    const IVec2 &dispatchSizeXY) const
    {
        ...
        {
            const bool success = verifyOperationAccumulationIntermediateValues(
                m_operation, static_cast<T>(getOperationInitialValue(m_operation)), atomicArgs, returnValues);

            if (!success)
            {
                log << TestLog::Message << "// Failure: intermediate return values at pixels " << arrayStr(pixelCoords)
                    << " of current layer are " << arrayStr(returnValues) << TestLog::EndMessage << TestLog::Message
                    << "// Note: relevant shader invocation global IDs are " << arrayStr(invocationGlobalIDs)
                    << TestLog::EndMessage << TestLog::Message << "// Note: data expression values for the IDs are "
                    << arrayStr(atomicArgs) << "; return values are not a valid result for any order of operations"
                    << TestLog::EndMessage;
                return false;
            }
        }
    ...
    template <typename T>
    bool verifyOperationAccumulationIntermediateValues(AtomicOperation operation, T init, const std::vector<T>(&args),
                                                       const std::vector<T>(&returnValues)) const
    {
    ...
    {
        return de::abs(a - b) <= 0.01f;
    }

    //! Depth-first search for verifying the return value sequence.
    template <typename T>
    bool verifyRecursive(AtomicOperation operation, int index, T valueSoFar, std::vector<bool>(&argsUsed),
                         const std::vector<T>(&args), const std::vector<T>(&returnValues)) const
    {
        if (index < m_numInvocationsPerPixel)
        {
            for (int i = 0; i < m_numInvocationsPerPixel; i++)
            {
                ...
                {
                    argsUsed[i] = true;
                    if (verifyRecursive(operation, index + 1,
                                        computeBinaryAtomicOperationResult(operation, valueSoFar, args[i]), argsUsed,
                                        args, returnValues))
                        return true;
                    argsUsed[i] = false;
                }
...
BinaryAtomicOperationCase::IterateResult BinaryAtomicOperationCase::iterate(void)
{
    ...
    glu::CallLogWrapper glLog(renderCtx.getFunctions(), log);
    const uint32_t internalFormatGL = glu::getInternalFormat(m_format);
    const uint32_t textureTargetGL  = getGLTextureTarget(m_imageType);
    ...
    const glu::Texture endResultTexture(
        renderCtx); //!< Texture for the final result; i.e. the texture on which the atomic operations are done. Size imageSize.
    const glu::Texture returnValueTexture(
        renderCtx); //!< Texture into which the return values are stored if m_caseType == CASETYPE_RETURN_VALUES.
    ...
    log << TestLog::Message << "// Created a texture (name " << *endResultTexture
        << ") to act as the target of atomic operations" << TestLog::EndMessage;
    if (m_imageType == TEXTURETYPE_BUFFER)
        log << TestLog::Message << "// Created a buffer for the texture (name " << *endResultTextureBuf << ")"
            << TestLog::EndMessage;

    if (m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES)
    {
        log << TestLog::Message << "// Created a texture (name " << *returnValueTexture
            << ") to which the intermediate return values of the atomic operation are stored" << TestLog::EndMessage;
        if (m_imageType == TEXTURETYPE_BUFFER)
            log << TestLog::Message << "// Created a buffer for the texture (name " << *returnValueTextureBuf << ")"
                << TestLog::EndMessage;
    }
...

    {
        ...
        log << TestLog::Message << "// Filling end-result texture with initial pattern (initial value "
            << getOperationInitialValue(m_operation) << ")" << TestLog::EndMessage;
    ...
    if (m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES)
    {
        ...
        setTextureStorage(glLog, m_imageType, internalFormatGL,
                          imageSize * (m_imageType == TEXTURETYPE_CUBE ?
                                           IVec3(m_numInvocationsPerPixel, m_numInvocationsPerPixel, 1) :
...

    {
        ...
        const string atomicArgExpr =
            (isUintFormat ? "uint" :
             isIntFormat  ? "" :
                            "float") +
            getAtomicFuncArgumentShaderStr(m_operation, "gx", "gy", "gz",
                                           IVec2(m_numInvocationsPerPixel * imageSize.x(), imageSize.y()));
        const string atomicInvocation = string() + getAtomicOperationShaderFuncName(m_operation) + "(u_results, " +
                                        atomicCoord + ", " + atomicArgExpr + ")";
        const string shaderImageFormatStr = getShaderImageFormatQualifier(m_format);
        ...
        const glu::ShaderProgram program(
            ...
            glu::ProgramSources() << glu::ComputeSource(
                ...
                "layout (local_size_x = 1, local_size_y = 1, local_size_z = 1) in;\n"
                "layout (" +
                shaderImageFormatStr + ", binding=0) coherent uniform " + shaderImageTypeStr + " u_results;\n" +
                (m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ?
                     "layout (" + shaderImageFormatStr + ", binding=1) writeonly uniform " + shaderImageTypeStr +
                         " u_returnValues;\n" :
                     "") +
                "\n"
                "void main (void)\n"
                ...
                (m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ?
                     "    imageStore(u_returnValues, " + invocationCoord + ", " + colorVecTypeName + "(" +
                         atomicInvocation + "));\n" :
                 m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT ? "    " + atomicInvocation + ";\n" :
    ...
    {
        const uint32_t textureToCheckGL = m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT    ? *endResultTexture :
                                          m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ? *returnValueTexture :
                                                                                                   (uint32_t)-1;
        const uint32_t textureToCheckBufGL =
            m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT    ? *endResultTextureBuf :
            m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ? *returnValueTextureBuf :
                                                                     (uint32_t)-1;

        const IVec3 textureToCheckSize =
            imageSize * IVec3(m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT ? 1 : m_numInvocationsPerPixel, 1, 1);
        const UniquePtr<const ImageLayerVerifier> verifier(
...
{
public:
    AtomicCompSwapCase(Context &context, const char *name, const char *description, const TextureFormat &format,
                       TextureType imageType, AtomicOperationCaseType caseType)
        : TestCase(context, name, description)
        , m_format(format)
        , m_imageType(imageType)
        , m_caseType(caseType)
...
string AtomicCompSwapCase::getCompareArgShaderStr(const string &x, const string &y, const string &z, int imageWidth)
{
    ...
    return "(" + wrapX + "*" + wrapX + " + " + y + "*" + y + " + " + z + "*" + z + " + " + curPixelInvocationNdx +
           "*42)";
}
...
{
public:
    EndResultVerifier(TextureType imageType, int imageWidth, int numInvocationsPerPixel)
        : m_imageType(imageType)
        ...
        for (int y = 0; y < resultSlice.getHeight(); y++)
            for (int x = 0; x < resultSlice.getWidth(); x++)
            {
...

                {
                    ...
                    {
                        log << TestLog::Message << "// Failure: invalid value at pixel " << IVec2(x, y) << ": got "
                            << result << TestLog::EndMessage << TestLog::Message
                            << "// Note: relevant shader invocation global IDs are " << arrayStr(invocationGlobalIDs)
                            << TestLog::EndMessage << TestLog::Message << "// Note: expected one of "
                            << arrayStr(assignArgs)
                            << " (those are the values given as the 'data' argument in the invocations that contribute "
                               "to this pixel)"
                            << TestLog::EndMessage;
                        return false;
                    }
...
public:
    //! \note endResultImageLayerSize is (width, height) of the image operated on by the atomic ops, and not the size of the image where the return values are stored.
    ReturnValueVerifier(TextureType imageType, int endResultImageWidth, int numInvocationsPerPixel)
        : m_imageType(imageType)
        , m_endResultImageWidth(endResultImageWidth)
        , m_numInvocationsPerPixel(numInvocationsPerPixel)
    {
    }
        ...
        for (int y = 0; y < resultSlice.getHeight(); y++)
            for (int x = 0; x < m_endResultImageWidth; x++)
            {
...

                {
                    ...
                    if (failingNdx >= 0)
                    {
                        log << TestLog::Message << "// Failure: intermediate return values at pixels "
                            << arrayStr(pixelCoords) << " of current layer are " << arrayStr(returnValues)
                            << TestLog::EndMessage << TestLog::Message
                            << "// Note: relevant shader invocation global IDs are " << arrayStr(invocationGlobalIDs)
                            << TestLog::EndMessage << TestLog::Message
                            << "// Note: 'compare' argument values for the IDs are " << arrayStr(compareArgs)
                            << TestLog::EndMessage << TestLog::Message
                            << "// Note: expected the return value sequence to fulfill the following conditions:\n"
                            << "// - first value is " << compareArgs[0] << "\n"
                            << "// - each value other than the first is either the same as the one just before it, or "
                               "the smallest value (in the sequence "
                            << arrayStr(compareArgs) << ") bigger than the one just before it" << TestLog::EndMessage;
                        if (failingNdx == 0)
                            log << TestLog::Message << "// Note: the first return value (" << returnValues[0]
                                << ") isn't " << compareArgs[0] << TestLog::EndMessage;
                        else
                            log << TestLog::Message << "// Note: the return value at index " << failingNdx << " (value "
                                << returnValues[failingNdx] << ") "
                                << "is neither " << returnValues[failingNdx - 1] << " (the one just before it) "
                                << "nor " << compareArgs[arrayIndexOf(compareArgs, returnValues[failingNdx - 1]) + 1]
                                << " (the smallest value bigger than the one just before it)" << TestLog::EndMessage;
...
AtomicCompSwapCase::IterateResult AtomicCompSwapCase::iterate(void)
{
...

    {
        ...
        const glu::ShaderProgram program(
            ...
            glu::ProgramSources() << glu::ComputeSource(
                ...
                "void main (void)\n"
                "{\n"
                "    int gx = int(gl_GlobalInvocationID.x) + offset * " +
                std::to_string(imageSize.x()) +
                ";\n"
                "    int gy = int(gl_GlobalInvocationID.y);\n"
                "    int gz = int(gl_GlobalInvocationID.z);\n"
                "    " +
                colorScalarTypeName + " compare = " + colorScalarTypeName +
                getCompareArgShaderStr("gx", "gy", "gz", imageSize.x()) +
                ";\n"
                "    " +
                colorScalarTypeName + " data    = " + colorScalarTypeName +
                getAssignArgShaderStr("gx", "gy", "gz", imageSize.x()) +
                ";\n"
                "    " +
                colorScalarTypeName + " status  = " + colorScalarTypeName +
                "(-1);\n"
                "    status = imageAtomicCompSwap(u_results, " +
                atomicCoord + ", compare, data);\n" +
                (m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ?
                     // Ensure there's an ordered ascending pattern to correctly check values are being stored in order
                     "    if(compare == status) imageStore(u_returnValues, " + invocationCoord + ", " +
                         colorVecTypeName + "(status));\n" :
                     "") +
                "}\n"));
        ...
        {
            uint32_t offsetLocation = glLog.glGetUniformLocation(program.getProgram(), "offset");
...

    {
        ...
        const IVec3 relevantRegion =
            imageSize * (m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT ? IVec3(1, 1, 1) :
                                                                               IVec3(m_numInvocationsPerPixel, 1, 1));
...
public:
    ...
    CoherenceCase(Context &context, const char *name, const char *description, const TextureFormat &format,
    ...
    {
        DE_STATIC_ASSERT(DE_LENGTH_OF_ARRAY(CoherenceCase::SHADER_READ_OFFSETS_Y) ==
                             DE_LENGTH_OF_ARRAY(CoherenceCase::SHADER_READ_OFFSETS_X) &&
                         DE_LENGTH_OF_ARRAY(CoherenceCase::SHADER_READ_OFFSETS_Z) ==
                             DE_LENGTH_OF_ARRAY(CoherenceCase::SHADER_READ_OFFSETS_X));
...
CoherenceCase::IterateResult CoherenceCase::iterate(void)
{
...

    {
        ...
        const glu::ShaderProgram program(
            renderCtx, glu::ProgramSources() << glu::ComputeSource(
                           glslVersionDeclaration + "\n" + textureTypeExtensionShaderRequires(m_imageType, renderCtx) +
                           "\n"
                           "precision highp " +
                           shaderImageTypeStr +
                           ...
                           "\n"
                           "layout (local_size_x = " +
                           localSizeX + ", local_size_y = " + localSizeY + ", local_size_z = " + localSizeZ +
                           ") in;\n"
                           "layout (" +
                           shaderImageFormatStr + ", binding=0) " + qualifierName + " uniform " + shaderImageTypeStr +
                           " u_image;\n"
                           "void main (void)\n"
                           "{\n"
                           "    int gx = int(gl_GlobalInvocationID.x);\n"
                           ...
                           "    int gz = int(gl_GlobalInvocationID.z);\n"
                           "    imageStore(u_image, " +
                           invocationCoord + ", " + colorVecTypeName +
                           "(gx^gy^gz));\n"
                           "\n"
                           "    memoryBarrier();\n"
                           "    barrier();\n"
                           "\n"
                           "    " +
                           colorScalarTypeName + " sum = " + colorScalarTypeName +
                           "(0);\n"
                           "    int groupBaseX = gx/" +
                           ...
                           ";\n"
                           "    int xOffsets[] = " +
                           SHADER_READ_OFFSETS_X_STR +
                           ";\n"
                           "    int yOffsets[] = " +
                           SHADER_READ_OFFSETS_Y_STR +
                           ";\n"
                           "    int zOffsets[] = " +
                           SHADER_READ_OFFSETS_Z_STR +
                           ";\n"
                           "    for (int i = 0; i < " +
                           toString(DE_LENGTH_OF_ARRAY(SHADER_READ_OFFSETS_X)) +
                           "; i++)\n"
                           "    {\n"
                           "        int readX = groupBaseX + (gx + xOffsets[i]) % " +
                           localSizeX +
                           ";\n"
                           "        int readY = groupBaseY + (gy + yOffsets[i]) % " +
                           localSizeY +
                           ";\n"
                           "        int readZ = groupBaseZ + (gz + zOffsets[i]) % " +
                           localSizeZ +
                           ";\n"
                           "        sum += imageLoad(u_image, " +
                           (m_imageType == TEXTURETYPE_BUFFER ? "readX" :
                            m_imageType == TEXTURETYPE_2D     ? "ivec2(readX, readY)" :
                                                                "ivec3(readX, readY, readZ)") +
                           ").x;\n"
                           "    }\n"
                           "\n"
                           "    memoryBarrier();\n"
...

    {
                    ...
                    for (int x = 0; x < imageSize.x(); x++)
                    {
                        ...
                        for (int i = 0; i < DE_LENGTH_OF_ARRAY(SHADER_READ_OFFSETS_X); i++)
                            sum += base.getPixelInt(groupBaseX + (x + SHADER_READ_OFFSETS_X[i]) % localSize.x(),
                                                    ...
                                                    groupBaseZ + (z + SHADER_READ_OFFSETS_Z[i]) % localSize.z())
                                       .x();
        ...
        if (readTextureAndVerify(renderCtx, glLog, *texture, *textureBuf, m_imageType, m_format, imageSize,
                                 ImageLayerComparer(reference)))
            m_testCtx.setTestResult(QP_TEST_RESULT_PASS, "Pass");
        else
...
{
public:
    R32UIImageSingleValueVerifier(const uint32_t value) : m_min(value), m_max(value)
    {
    }
    R32UIImageSingleValueVerifier(const uint32_t min, const uint32_t max) : m_min(min), m_max(max)
    {
    }
    ...
    bool operator()(TestLog &log, const ConstPixelBufferAccess &resultSlice, int) const
    {
        ...
        log << TestLog::Message << "// Note: expecting to get value "
            << (m_min == m_max ? toString(m_min) : "in range [" + toString(m_min) + ", " + toString(m_max) + "]")
            << TestLog::EndMessage;

        const uint32_t resultValue = resultSlice.getPixelUint(0, 0).x();
...
public:
    ...
    ImageSizeCase(Context &context, const char *name, const char *description, const TextureFormat &format,
                  TextureType imageType, const IVec3 &size, ImageAccess imageAccess)
        : TestCase(context, name, description)
        , m_format(format)
        , m_imageType(imageType)
        , m_imageSize(size)
...
ImageSizeCase::IterateResult ImageSizeCase::iterate(void)
{
    ...
    if (m_imageType == TEXTURETYPE_BUFFER)
        log << TestLog::Message << "// Created a buffer for the texture (name " << *mainTextureBuf << ")"
            << TestLog::EndMessage;
    log << TestLog::Message << "// Created a texture (name " << *shaderOutResultTexture
        << ") for storing the shader output" << TestLog::EndMessage;
...

    {
        ...
        const glu::ShaderProgram program(
            ...
            glu::ProgramSources() << glu::ComputeSource(
                ...
                "layout (local_size_x = 1, local_size_y = 1, local_size_z = 1) in;\n"
                "layout (" +
                shaderImageFormatStr + ", binding=0) " + shaderImageAccessStr + " uniform " + shaderImageTypeStr +
                " u_image;\n"
                "layout (r32ui, binding=1) writeonly uniform uimage2D u_result;\n"
                "void main (void)\n"
                "{\n" +
                (m_imageType == TEXTURETYPE_BUFFER ? "    int result = imageSize(u_image);\n" :
    ...
    {
        const uint32_t referenceOutput =
            m_imageType == TEXTURETYPE_BUFFER ?
                (uint32_t)(m_imageSize.x()) :
            m_imageType == TEXTURETYPE_2D || m_imageType == TEXTURETYPE_CUBE ?
                (uint32_t)(m_imageSize.y() * 1000 + m_imageSize.x()) :
            m_imageType == TEXTURETYPE_3D || m_imageType == TEXTURETYPE_2D_ARRAY ?
                (uint32_t)(m_imageSize.z() * 1000000 + m_imageSize.y() * 1000 + m_imageSize.x()) :
                (uint32_t)-1;

        if (readIntegerTextureViaFBOAndVerify(renderCtx, glLog, *shaderOutResultTexture, TEXTURETYPE_2D,
...
public:
    ...
    EarlyFragmentTestsCase(Context &context, const char *name, const char *description, TestType type,
                           bool useEarlyTests, RenderTargetType renderTarget)
        : TestCase(context, name, description)
        , m_type(type)
    ...
    void init(void)
    {
        ...
        if (!m_context.getContextInfo().isExtensionSupported("GL_OES_shader_image_atomic") &&
            !supportsES32orGL45(m_context.getRenderContext()))
            throw tcu::NotSupportedError("Test requires OES_shader_image_atomic extension");

        if (m_type == TESTTYPE_DEPTH && m_renderTarget == RENDERTARGET_DEFAULT &&
            m_context.getRenderTarget().getDepthBits() == 0)
        {
            throw tcu::NotSupportedError("Test requires depth buffer");
        }

        if (m_type == TESTTYPE_STENCIL && m_renderTarget == RENDERTARGET_DEFAULT &&
            m_context.getRenderTarget().getStencilBits() == 0)
        {
            throw tcu::NotSupportedError("Test requires stencil buffer");
        }

        if (m_renderTarget == RENDERTARGET_DEFAULT && (m_context.getRenderTarget().getWidth() < RENDER_SIZE ||
                                                       m_context.getRenderTarget().getHeight() < RENDER_SIZE))
            throw tcu::NotSupportedError("Render target must have at least " + toString(RENDER_SIZE) +
                                         " width and height");
    }
...
EarlyFragmentTestsCase::IterateResult EarlyFragmentTestsCase::iterate(void)
{
    ...
    const int viewportX            = (m_renderTarget == RENDERTARGET_DEFAULT) ?
                                         (rnd.getInt(0, renderCtx.getRenderTarget().getWidth() - viewportWidth)) :
                                         (0);
    const int viewportY            = (m_renderTarget == RENDERTARGET_DEFAULT) ?
                                         (rnd.getInt(0, renderCtx.getRenderTarget().getHeight() - viewportHeight)) :
                                         (0);
    const glu::Texture texture(renderCtx);
    ...
    // Set up framebuffer
    if (m_renderTarget == RENDERTARGET_FBO || m_renderTarget == RENDERTARGET_FBO_WITHOUT_TEST_ATTACHMENT)
    {
...

    {
        ...
        const glu::ShaderProgram program(
            renderCtx, glu::ProgramSources()
                           << glu::VertexSource(glslVersionDeclaration + "\n"
                                                                         "\n"
...

        {
            ...
            const glu::VertexArrayBinding attrBindings[] = {glu::va::Float("a_position", 3, 4, 0, &vertexPositions[0])};
    ...
    // Read counter value and check.
    {
        ...
        const int expectedCounter =
            expectPartialResult ? viewportWidth * viewportHeight / 2 : viewportWidth * viewportHeight;
        ...
        if (readIntegerTextureViaFBOAndVerify(renderCtx, glLog, *texture, TEXTURETYPE_2D,
                                              TextureFormat(TextureFormat::R, TextureFormat::UNSIGNED_INT32),
...
void ShaderImageLoadStoreTests::init(void)
{
    ...
    {
        static const TextureType imageTypes[] = {TEXTURETYPE_2D, TEXTURETYPE_CUBE, TEXTURETYPE_3D, TEXTURETYPE_2D_ARRAY,
                                                 TEXTURETYPE_BUFFER};

        static const TextureFormat formats[] = {TextureFormat(TextureFormat::RGBA, TextureFormat::FLOAT),
        ...
        for (int imageTypeNdx = 0; imageTypeNdx < DE_LENGTH_OF_ARRAY(imageTypes); imageTypeNdx++)
        {
            ...
            TestCaseGroup *const loadStoreGroup =
                new TestCaseGroup(m_context, "load_store", "Cases with imageLoad() followed by imageStore()");
            TestCaseGroup *const atomicGroup = new TestCaseGroup(m_context, "atomic", "Atomic image operation cases");
            TestCaseGroup *const qualifierGroup =
                new TestCaseGroup(m_context, "qualifiers", "Coherent, volatile and restrict");
            TestCaseGroup *const reinterpretGroup =
                new TestCaseGroup(m_context, "format_reinterpret", "Cases with differing texture and image formats");
            ...
            for (int formatNdx = 0; formatNdx < DE_LENGTH_OF_ARRAY(formats); formatNdx++)
            {
                ...
                if (textureLayerType(imageType) != imageType)
                    storeGroup->addChild(new ImageStoreCase(m_context, (formatName + "_single_layer").c_str(), "",
                                                            format, imageType,
                                                            ImageStoreCase::CASEFLAG_SINGLE_LAYER_BIND));

                // Load & store.

                loadStoreGroup->addChild(
                    new ImageLoadAndStoreCase(m_context, formatName.c_str(), "", format, imageType));
                if (textureLayerType(imageType) != imageType)
                    loadStoreGroup->addChild(
                        new ImageLoadAndStoreCase(m_context, (formatName + "_single_layer").c_str(), "", format,
                                                  imageType, ImageLoadAndStoreCase::CASEFLAG_SINGLE_LAYER_BIND));
                ...
                if (format.order == TextureFormat::R)
                {
                    ...
                    {
                        for (int atomicCaseTypeI = 0; atomicCaseTypeI < ATOMIC_OPERATION_CASE_TYPE_LAST;
                             atomicCaseTypeI++)
                        {
                            ...
                            if (operation == ATOMIC_OPERATION_COMP_SWAP)
                                atomicGroup->addChild(new AtomicCompSwapCase(m_context, caseName.c_str(), "", format,
                                                                             imageType, caseType));
                            else
                                atomicGroup->addChild(new BinaryAtomicOperationCase(
                                    m_context, caseName.c_str(), "", format, imageType, operation, caseType));
                        }
                    }

                    // Coherence.

                    for (int coherenceQualifierI = 0; coherenceQualifierI < CoherenceCase::QUALIFIER_LAST;
                         coherenceQualifierI++)
                    {
                        const CoherenceCase::Qualifier coherenceQualifier =
                            (CoherenceCase::Qualifier)coherenceQualifierI;
                        ...
                        qualifierGroup->addChild(
                            new CoherenceCase(m_context, caseName.c_str(), "", format, imageType, coherenceQualifier));
                    }
                }
            }

            // Restrict.
            qualifierGroup->addChild(new ImageLoadAndStoreCase(
                m_context, "restrict", "", TextureFormat(TextureFormat::RGBA, TextureFormat::UNSIGNED_INT32), imageType,
                ImageLoadAndStoreCase::CASEFLAG_RESTRICT_IMAGES));
            ...
            for (int texFmtNdx = 0; texFmtNdx < DE_LENGTH_OF_ARRAY(formats); texFmtNdx++)
                for (int imgFmtNdx = 0; imgFmtNdx < DE_LENGTH_OF_ARRAY(formats); imgFmtNdx++)
                {
                    ...
                    if (texFmt != imgFmt && texFmt.getPixelSize() == imgFmt.getPixelSize())
                        reinterpretGroup->addChild(new ImageLoadAndStoreCase(
                            m_context,
                            (getShaderImageFormatQualifier(texFmt) + "_" + getShaderImageFormatQualifier(imgFmt))
                                .c_str(),
            ...
            {
                static const IVec3 baseImageSizes[] = {IVec3(32, 32, 32), IVec3(12, 34, 56), IVec3(1, 1, 1),
                                                       IVec3(7, 1, 1)};
                ...
                for (int imageAccessI = 0; imageAccessI < ImageSizeCase::IMAGEACCESS_LAST; imageAccessI++)
                {
                    ...
                    const char *const imageAccessStr =
                        imageAccess == ImageSizeCase::IMAGEACCESS_READ_ONLY  ? "readonly" :
                        imageAccess == ImageSizeCase::IMAGEACCESS_WRITE_ONLY ? "writeonly" :
                        imageAccess == ImageSizeCase::IMAGEACCESS_READ_ONLY_WRITE_ONLY ?
                                                                               "readonly_writeonly" :
                    ...
                    for (int imageSizeNdx = 0; imageSizeNdx < DE_LENGTH_OF_ARRAY(baseImageSizes); imageSizeNdx++)
                    {
                        ...
                        const string sizeStr =
                            ...
                            imageType == TEXTURETYPE_3D     ? toString(imageSize.x()) + "x" + toString(imageSize.y()) +
                                                              "x" + toString(imageSize.z()) :
                            imageType == TEXTURETYPE_2D_ARRAY ?
                                                          toString(imageSize.x()) + "x" + toString(imageSize.y()) +
                                                              "x" + toString(imageSize.z()) :
                                                          deFatalStr("Invalid TextureType");

                        const string caseName = string() + imageAccessStr + "_" + sizeStr;

                        imageSizeGroup->addChild(new ImageSizeCase(
                            m_context, caseName.c_str(), "", TextureFormat(TextureFormat::RGBA, TextureFormat::FLOAT),
                            imageType, imageSize, imageAccess));
                    }
                }
            }
        }
...

    {
        ...
        for (int testRenderTargetI = 0; testRenderTargetI < EarlyFragmentTestsCase::RENDERTARGET_LAST;
             testRenderTargetI++)
            for (int useEarlyTestsI = 0; useEarlyTestsI <= 1; useEarlyTestsI++)
                ...
                {
                    const EarlyFragmentTestsCase::RenderTargetType targetType =
                        (EarlyFragmentTestsCase::RenderTargetType)testRenderTargetI;
                    ...
                    const string targetName =
                        targetType == EarlyFragmentTestsCase::RENDERTARGET_FBO ?
                            (std::string("_fbo")) :
                        targetType == EarlyFragmentTestsCase::RENDERTARGET_FBO_WITHOUT_TEST_ATTACHMENT ?
                            (std::string("_fbo_with_no_") + testTypeName) :
                            std::string("");

                    const string caseName =
                        string(useEarlyTests ? "" : "no_") + "early_fragment_tests_" + testTypeName + targetName;

                    const string caseDesc =
                        string(useEarlyTests ? "Specify" : "Don't specify") + " early_fragment_tests, use the " +
                        testTypeName + " test" +
                        ((targetType == EarlyFragmentTestsCase::RENDERTARGET_FBO) ?
                             (", render to fbo") :
                         (targetType == EarlyFragmentTestsCase::RENDERTARGET_FBO_WITHOUT_TEST_ATTACHMENT) ?
                             (", render to fbo without relevant buffer") :
                             (""));

                    earlyTestsGroup->addChild(new EarlyFragmentTestsCase(m_context, caseName.c_str(), caseDesc.c_str(),
                                                                         testType, useEarlyTests, targetType));
                }
    }
}
