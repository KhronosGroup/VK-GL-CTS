        ...
        for (int y = 0; y < m_endResultImageLayerSize.y(); y++)
            for (int x = 0; x < m_endResultImageLayerSize.x(); x++)
            {
...

                {
                    const bool success =
                        isIntegerFormat ?
                            verifyOperationAccumulationIntermediateValues(
                                m_operation, getOperationInitialValue(m_operation), atomicArgs.i, returnValues.i)

                            :
                            verifyOperationAccumulationIntermediateValues(m_operation,
                                                                          (float)getOperationInitialValue(m_operation),
                                                                          atomicArgs.f, returnValues.f);

                    if (!success)
                    {
                        log << TestLog::Message << "// Failure: intermediate return values at pixels "
                            << arrayStr(pixelCoords) << " of current layer are "
                            << (isIntegerFormat ? arrayStr(returnValues.i) : arrayStr(returnValues.f))
                            << TestLog::EndMessage << TestLog::Message
                            << "// Note: relevant shader invocation global IDs are " << arrayStr(invocationGlobalIDs)
                            << TestLog::EndMessage << TestLog::Message
                            << "// Note: data expression values for the IDs are "
...
private:
    ...
    template <typename T>
    static bool verifyOperationAccumulationIntermediateValues(AtomicOperation operation, T init,
                                                              const T (&args)[NUM_INVOCATIONS_PER_PIXEL],
                                                              const T (&returnValues)[NUM_INVOCATIONS_PER_PIXEL])
    {
    ...
    template <typename T>
    static bool verifyRecursive(AtomicOperation operation, int index, T valueSoFar,
                                bool (&argsUsed)[NUM_INVOCATIONS_PER_PIXEL], const T (&args)[NUM_INVOCATIONS_PER_PIXEL],
                                const T (&returnValues)[NUM_INVOCATIONS_PER_PIXEL])
    {
...
BinaryAtomicOperationCase::IterateResult BinaryAtomicOperationCase::iterate(void)
{
    ...
    if (m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES)
    {
        ...
        setTextureStorage(glLog, m_imageType, internalFormatGL,
                          imageSize * (m_imageType == TEXTURETYPE_CUBE ?
                                           IVec3(NUM_INVOCATIONS_PER_PIXEL, NUM_INVOCATIONS_PER_PIXEL, 1) :
...

    {
        ...
        const string atomicArgExpr =
            ...
            getAtomicFuncArgumentShaderStr(m_operation, "gx", "gy", "gz",
                                           IVec2(NUM_INVOCATIONS_PER_PIXEL * imageSize.x(), imageSize.y()));
...

    {
        ...
        const IVec3 textureToCheckSize =
            imageSize *
            IVec3(m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT ? 1 : NUM_INVOCATIONS_PER_PIXEL, 1, 1);
        const UniquePtr<const ImageLayerVerifier> verifier(
            m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT    ? new EndResultVerifier(m_operation, m_imageType) :
            m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ? new ReturnValueVerifier(m_operation, m_imageType,
                                                                                             imageSize.swizzle(0, 1)) :
...
AtomicCompSwapCase::IterateResult AtomicCompSwapCase::iterate(void)
{
...

    {
        ...
        const UniquePtr<const ImageLayerVerifier> verifier(m_caseType == ATOMIC_OPERATION_CASE_TYPE_END_RESULT ?
                                                               new EndResultVerifier(m_imageType, imageSize.x()) :
                                                           m_caseType == ATOMIC_OPERATION_CASE_TYPE_RETURN_VALUES ?
                                                               new ReturnValueVerifier(m_imageType, imageSize.x()) :
                                                               (ImageLayerVerifier *)DE_NULL);
