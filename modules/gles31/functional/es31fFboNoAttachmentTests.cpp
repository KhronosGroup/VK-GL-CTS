...
bool checkFramebufferSize(TestLog &log, const glu::RenderContext &renderCtx, GLuint framebuffer, const IVec2 &size)
{
    ...
    gl.viewport(0, 0, size.x() * 2,
                size.y() * 2); // Oversized viewport so that it will not accidentally limit us to the correct size
    ...
    // Render
    {
        ...
        log << TestLog::Message << "A fragment was not discarded at (" << size.x() - 1 << ", " << size.y() - 1 << "). "
            << "Occlusion query reports it was " << (insidePassed > 0 ? "rendered." : "not rendered")
            << TestLog::EndMessage;
        ...
        log << TestLog::Message << "A fragment was not discarded at (" << size.x() << ", " << size.y() - 1 << "). "
            << "Occlusion query reports it was " << (outsideXPassed > 0 ? "rendered." : "not rendered")
            << TestLog::EndMessage;
        ...
        log << TestLog::Message << "A fragment was not discarded at (" << size.x() - 1 << ", " << size.y() << "). "
            << "Occlusion query reports it was " << (outsideYPassed > 0 ? "rendered." : "not rendered")
            << TestLog::EndMessage;
...
bool checkFramebufferRenderable(TestLog &log, const glu::RenderContext &renderCtx, GLuint framebuffer,
                                const IVec2 &size)
{
    const glw::Functions &gl = renderCtx.getFunctions();
...
{
public:
    FramebufferCompletenessCase(tcu::TestContext &testCtx, const glu::RenderContext &renderCtx, const char *name,
                                const char *desc);
...
FramebufferCompletenessCase::FramebufferCompletenessCase(tcu::TestContext &testCtx, const glu::RenderContext &renderCtx,
...
FramebufferCompletenessCase::IterateResult FramebufferCompletenessCase::iterate(void)
{
    ...
    m_results.check(gl.checkFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE,
                    "Framebuffer was incorrectly reported as complete when it had no width, height or attachments");

    gl.framebufferParameteri(GL_DRAW_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_WIDTH, 16);
    m_results.check(gl.checkFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE,
                    "Framebuffer was incorrectly reported as complete when it only had a width");

    gl.framebufferParameteri(GL_DRAW_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_HEIGHT, 16);
    m_results.check(gl.checkFramebufferStatus(GL_DRAW_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE,
                    "Framebuffer not reported as complete when it had width and height set");

    gl.framebufferParameteri(GL_DRAW_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_WIDTH, 0);
    m_results.check(gl.checkFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE,
                    "Framebuffer was incorrectly reported as complete when it only had a height");
...
{
public:
    SizeCase(tcu::TestContext &testCtx, const glu::RenderContext &renderCtx, const char *name, const char *desc,
             const FboSpec &spec);
...
SizeCase::SizeCase(tcu::TestContext &testCtx, const glu::RenderContext &renderCtx, const char *name, const char *desc,
                   const FboSpec &spec)
    : TestCase(testCtx, name, desc)
    , m_renderCtx(renderCtx)
    , m_spec(spec)
...
SizeCase::IterateResult SizeCase::iterate(void)
{
    ...
    if (!framebuffer)
    {
        m_testCtx.setTestResult(QP_TEST_RESULT_FAIL,
                                "Unable to find a supported no-attachment framebuffer width/height/samples");
        return STOP;
    }

    log << TestLog::Message << "Verifying " << width << "x" << height << " framebuffer with " << samples
        << "x multisampling" << TestLog::EndMessage;

    if (checkFramebufferRenderable(log, m_renderCtx, framebuffer, IVec2(width, height)) &&
        checkFramebufferSize(log, m_renderCtx, framebuffer, IVec2(width, height)))
...
{
public:
    AttachmentInteractionCase(tcu::TestContext &testCtx, const glu::RenderContext &renderCtx, const char *name,
                              const char *desc, const FboSpec &defaultSpec, const FboSpec &attachmentSpec);
...
AttachmentInteractionCase::AttachmentInteractionCase(tcu::TestContext &testCtx, const glu::RenderContext &renderCtx,
                                                     const char *name, const char *desc, const FboSpec &defaultSpec,
...
AttachmentInteractionCase::IterateResult AttachmentInteractionCase::iterate(void)
{
    ...
    gl.renderbufferStorageMultisample(GL_RENDERBUFFER, m_attachmentSpec.samples, GL_RGBA8, m_attachmentSpec.width,
                                      m_attachmentSpec.height);
    gl.framebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, renderbuffer);

    log << TestLog::Message << "Verifying " << m_attachmentSpec.width << "x" << m_attachmentSpec.height
        << " framebuffer with " << m_attachmentSpec.samples << "x multisampling"
        << " and defaults set to " << m_defaultSpec.width << "x" << m_defaultSpec.height << " with "
        << m_defaultSpec.samples << "x multisampling" << TestLog::EndMessage;

    if (checkFramebufferRenderable(log, m_renderCtx, framebuffer,
                                   IVec2(m_attachmentSpec.width, m_attachmentSpec.height)) &&
...
tcu::TestCaseGroup *createFboNoAttachmentTests(Context &context)
{
    ...
    tcu::TestCaseGroup *const root =
        new tcu::TestCaseGroup(testCtx, "no_attachments", "Framebuffer without attachments");

    // Size
    {
        tcu::TestCaseGroup *const group =
            new tcu::TestCaseGroup(testCtx, "size", "Basic functionality tests with varying default size");
    ...
    // NPOT size
    {
        ...
        tcu::TestCaseGroup *const group =
            new tcu::TestCaseGroup(testCtx, "npot_size", "Basic functionality with Non-power-of-two size");
    ...
    {
        tcu::TestCaseGroup *const group =
            new tcu::TestCaseGroup(testCtx, "multisample", "Basic functionality with multisampled fbo");
    ...
    {
        tcu::TestCaseGroup *const group =
            new tcu::TestCaseGroup(testCtx, "interaction", "Interaction of default parameters with normal fbo");

        root->addChild(group);

        const FboSpec specs[][2] = {
            {FboSpec(256, 256, 0), FboSpec(128, 128, 1)}, {FboSpec(256, 256, 1), FboSpec(128, 128, 0)},
            {FboSpec(256, 256, 0), FboSpec(512, 512, 2)}, {FboSpec(256, 256, 2), FboSpec(128, 512, 0)},
            {FboSpec(127, 127, 0), FboSpec(129, 129, 0)}, {FboSpec(17, 512, 4), FboSpec(16, 16, 2)},
            {FboSpec(2048, 2048, 4), FboSpec(1, 1, 0)},   {FboSpec(1, 1, 0), FboSpec(2048, 2048, 4)},
        };
        ...
        for (int specNdx = 0; specNdx < DE_LENGTH_OF_ARRAY(specs); specNdx++)
        {
...

            {
                ...
                group->addChild(
                    new AttachmentInteractionCase(testCtx, renderCtx, name.c_str(), name.c_str(), altSpec, baseSpec));
            }
        }
    }
    ...
    // Maximums
    {
        ...
        root->addChild(group);
        group->addChild(
            new SizeCase(testCtx, renderCtx, "width", "Maximum width", FboSpec(SizeCase::USE_MAXIMUM, 128, 0)));
        group->addChild(
            new SizeCase(testCtx, renderCtx, "height", "Maximum height", FboSpec(128, SizeCase::USE_MAXIMUM, 0)));
        group->addChild(new SizeCase(testCtx, renderCtx, "size", "Maximum size",
                                     FboSpec(SizeCase::USE_MAXIMUM, SizeCase::USE_MAXIMUM, 0)));
        group->addChild(
            new SizeCase(testCtx, renderCtx, "samples", "Maximum samples", FboSpec(128, 128, SizeCase::USE_MAXIMUM)));
        group->addChild(new SizeCase(testCtx, renderCtx, "all", "Maximum size & samples",
                                     FboSpec(SizeCase::USE_MAXIMUM, SizeCase::USE_MAXIMUM, SizeCase::USE_MAXIMUM)));
    }
...
tcu::TestCaseGroup *createFboNoAttachmentCompletenessTests(Context &context)
{
    ...
    group->addChild(new FramebufferCompletenessCase(context.getTestContext(), context.getRenderContext(),
                                                    "no_attachments", "No attachments completeness"));
