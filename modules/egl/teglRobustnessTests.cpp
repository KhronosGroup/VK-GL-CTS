...
public:
    ...
    public:
        ...
        Params(const string &name, const string &description, const ContextResetType &contextResetType,
...
RobustnessTestCase::Params::Params(const string &name, const string &description,
                                   const ContextResetType &contextResetType, const ShaderType &shaderType)
    : m_name(name)
    , m_description(description)
    , m_contextResetType(contextResetType)
...
{
    const char *const computeSource = "#version 310 es\n"
...
glu::ProgramSources InfiniteLoop::genNonComputeSource(void)
{
    ...
    {
        vert << "    mediump vec4 color = "
             << "a_position.xyxy"
             << ";\n";
        vert << loopHeader << "        " << body << "\n";

        frag << "    mediump vec4 color = "
             << "gl_FragCoord"
             << ";\n";
...
void ContextResetCase::execute(glw::Functions &gl)
{
    ...
    if (m_params.getContextResetType() == CONTEXTRESETTYPE_INFINITE_LOOP)
    {
        try
        {
            ...
            {
                m_testCtx.getLog() << tcu::TestLog::Message << "glGetGraphicsResetStatus() returned "
                                   << glu::getGraphicsResetStatusStr(status) << tcu::TestLog::EndMessage;
    ...
    else if (m_params.getContextResetType() == CONTEXTRESETTYPE_SHADER_OOB ||
...
public:
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Check the status of a sync object after a context reset returned by glGetSynciv() "
                              "equals GL_SIGNALED\n\n"
                           << tcu::TestLog::EndMessage;
    ...
    virtual void passAndLog(de::SharedPtr<ContextReset> &contextReset)
    {
        ...
        {
            m_testCtx.getLog() << tcu::TestLog::Message << "Test failed! glGetSynciv() returned wrong value ["
                               << glu::getErrorStr(status) << ", expected " << glu::getErrorStr(GL_SIGNALED) << "]"
                               << tcu::TestLog::EndMessage;
...
public:
    ...
    {
        m_testCtx.getLog() << tcu::TestLog::Message
                           << "Check the status of a query object after a context reset returned by "
                              "glGetQueryObjectuiv() equals GL_TRUE\n\n"
                           << tcu::TestLog::EndMessage;
    ...
    virtual void passAndLog(de::SharedPtr<ContextReset> &contextReset)
    {
        ...
        {
            m_testCtx.getLog() << tcu::TestLog::Message << "Test failed! glGetQueryObjectuiv() returned wrong value ["
                               << glu::getErrorStr(queryReady) << ", expected " << glu::getErrorStr(GL_TRUE) << "]"
                               << tcu::TestLog::EndMessage;
...
public:
    ...
    TestCase::IterateResult iterate(void)
    {
        ...
        const EGLint attribListShared[] = {EGL_CONTEXT_CLIENT_VERSION,
                                           3,
                                           EGL_CONTEXT_MINOR_VERSION_KHR,
                                           0,
                                           EGL_CONTEXT_OPENGL_ROBUST_ACCESS_EXT,
        ...
        catch (const glu::Error &error)
        {
            if (error.getError() == GL_CONTEXT_LOST)
            {
                ...
                if (gl.getError() != GL_CONTEXT_LOST)
                {
                    m_testCtx.setTestResult(
                        QP_TEST_RESULT_FAIL,
                        "Test failed! glGetError() returned wrong value. Expected GL_CONTEXT_LOST in context B");
                    return STOP;
                }
            }
            else
            {
                m_testCtx.setTestResult(
                    QP_TEST_RESULT_FAIL,
                    "Test failed! glGetError() returned wrong value. Expected GL_CONTEXT_LOST in context A");
                return STOP;
            }
        }
    ...
    TestCase::IterateResult iterate(void)
    {
        ...
        catch (const glu::Error &error)
        {
            if (error.getError() == GL_CONTEXT_LOST)
            {
                ...
                if (status == GL_NO_ERROR)
                {
                    log << tcu::TestLog::Message << "Test failed! glGetGraphicsResetStatus() returned wrong value ["
                        << glu::getErrorStr(status) << ", expected " << glu::getErrorStr(GL_GUILTY_CONTEXT_RESET) << "]"
                        << tcu::TestLog::EndMessage;
                ...
                {
                    log << tcu::TestLog::Message
                        << "Test failed! Context did not reset. glGetGraphicsResetStatus() returned wrong value ["
                        << glu::getErrorStr(reset_status) << ", expected " << glu::getErrorStr(GL_NO_ERROR) << "]"
                        << tcu::TestLog::EndMessage;
        ...
        catch (const glu::Error &)
        {
            m_testCtx.setTestResult(
                QP_TEST_RESULT_FAIL,
                "Test failed! Could not create new context. glGetError() returned wrong value. Expected GL_NO_ERROR");
            return STOP;
        }
...
TestCaseGroup *createRobustnessTests(EglTestContext &eglTestCtx)
{
    ...
    tcu::TestCaseGroup *const infiniteLoopTestGroup =
        new TestCaseGroup(eglTestCtx, "infinite_loop", "Infinite loop scenarios");
    ...
    {
        tcu::TestCaseGroup *const infiniteLoopResetTestGroup = new TestCaseGroup(
            eglTestCtx, "reset_status", "Tests that query the reset status after a context reset has occurred");
        tcu::TestCaseGroup *const infiniteLoopSyncTestGroup = new TestCaseGroup(
            eglTestCtx, "sync_status", "Tests that query the sync status after a context reset has occurred");
        tcu::TestCaseGroup *const infiniteLoopQueryTestGroup =
            new TestCaseGroup(eglTestCtx, "query_status",
                              "Tests that query the state of a query object after a context reset has occurred");
        tcu::TestCaseGroup *const infiniteLoopSharedTestGroup =
            new TestCaseGroup(eglTestCtx, "shared_context_status",
                              "Tests that query the state of a shared context after a reset has occurred");
        tcu::TestCaseGroup *const infiniteLoopRecoverTestGroup =
            new TestCaseGroup(eglTestCtx, "recover_from_reset",
        ...
        static const RobustnessTestCase::Params s_infiniteLoopCases[] = {
            RobustnessTestCase::Params("vertex", "Provoke a context reset in vertex shader and ",
                                       CONTEXTRESETTYPE_INFINITE_LOOP, SHADERTYPE_VERT),
            RobustnessTestCase::Params("fragment", "Provoke a context reset in fragment shader and ",
                                       CONTEXTRESETTYPE_INFINITE_LOOP, SHADERTYPE_FRAG),
            RobustnessTestCase::Params("vertex_and_fragment",
                                       "Provoke a context reset in vertex and fragment shader and ",
                                       CONTEXTRESETTYPE_INFINITE_LOOP, SHADERTYPE_VERT_AND_FRAG),
            RobustnessTestCase::Params("compute", "Provoke a context reset in compute shader and ",
                                       CONTEXTRESETTYPE_INFINITE_LOOP, SHADERTYPE_COMPUTE),
        };
        ...
        for (int testNdx = 0; testNdx < DE_LENGTH_OF_ARRAY(s_infiniteLoopCases); ++testNdx)
        {
            ...
            infiniteLoopResetTestGroup->addChild(new BasicResetCase(
                eglTestCtx, test.getName().c_str(), (test.getDescription() + resetScenarioDescription).c_str(), test));
            infiniteLoopSyncTestGroup->addChild(new SyncObjectResetCase(
                eglTestCtx, test.getName().c_str(), (test.getDescription() + syncScenarioDescription).c_str(), test));
            infiniteLoopQueryTestGroup->addChild(new QueryObjectResetCase(
                eglTestCtx, test.getName().c_str(), (test.getDescription() + queryScenarioDescription).c_str(), test));
            infiniteLoopSharedTestGroup->addChild(new SharedContextResetCase(
                eglTestCtx, test.getName().c_str(), (test.getDescription() + sharedScenarioDescription).c_str(), test));
            infiniteLoopRecoverTestGroup->addChild(
                new RecoverFromResetCase(eglTestCtx, test.getName().c_str(),
                                         (test.getDescription() + recoverScenarioDescription).c_str(), test));
        }
