...
struct FunctionNames
{
    ...
    FunctionNames(int numFunctions_, const char *const *functions_) : numFunctions(numFunctions_), functions(functions_)
    {
    }
};
...
void GetProcAddressCase::init(void)
{
    ...
    {
        const std::vector<std::string> displayExtensios =
...
{
public:
    GetProcAddressExtensionCase(EglTestContext &eglTestCtx, const char *name, const char *description,
    ...
    void executeTest(void)
    {
        ...
        log << TestLog::Message << m_extName << ": " << (supported ? "supported" : "not supported")
            << TestLog::EndMessage;
        ...
        for (int funcNdx = 0; funcNdx < funcNames.numFunctions; funcNdx++)
        {
            ...
            {
                log << TestLog::Message << "Fail, received null pointer for supported extension function: " << funcName
                    << TestLog::EndMessage;
...
public:
    ...
    GetProcAddressCoreFunctionsCase(EglTestContext &eglTestCtx, const char *name, const char *description,
    ...
    bool isApiSupported(void)
    {
        ...
        {
        case EGL14:
            if (m_eglVersion >= EGL_MAKE_VERSION(1, 4))
                return true;
            return false;
            break;
        case EGL15:
            ...
            // GetProcAddress.
            if (m_eglVersion >= EGL_MAKE_VERSION(1, 5))
                return true;
            return false;
            break;
        case GLES:
    ...
    FunctionNames getCoreFunctionNames(EGLint apiType)
    {
        ...
        {
        case EGL14:
            return FunctionNames(DE_LENGTH_OF_ARRAY(s_EGL14), s_EGL14);
        case EGL15:
            return FunctionNames(DE_LENGTH_OF_ARRAY(s_EGL15), s_EGL15);
        case GLES:
            return FunctionNames(DE_LENGTH_OF_ARRAY(s_GLES10), s_GLES10);
        case GLES2:
            return FunctionNames(DE_LENGTH_OF_ARRAY(s_GLES20), s_GLES20);
        case GLES3:
            return FunctionNames(DE_LENGTH_OF_ARRAY(s_GLES30), s_GLES30);
        default:
            DE_ASSERT(false);
        }
    ...
    void executeTest(void)
    {
        ...
        log << TestLog::Message
            << "EGL_KHR_get_all_proc_addresses: " << (funcPtrSupported ? "supported" : "not supported")
            << TestLog::EndMessage;
        log << TestLog::Message << TestLog::EndMessage;
        ...
        if (!apiSupported)
        {
            ...
            case EGL14:
                ...
                break;
            case EGL15:
                log << TestLog::Message << " EGL 1.5 not supported by any available configuration."
                    << TestLog::EndMessage;
                break;
            case GLES:
            case GLES2:
            case GLES3:
                log << TestLog::Message << eglu::getConfigAttribValueStr(EGL_RENDERABLE_TYPE, RenderableType(m_apiType))
                    << " not supported by any available configuration." << TestLog::EndMessage;
                break;
            }
            log << TestLog::Message << TestLog::EndMessage;
        }
        ...
        for (int funcNdx = 0; funcNdx < funcNames.numFunctions; funcNdx++)
        {
            ...
            {
                log << TestLog::Message << "Fail, received null pointer for supported function: " << funcName
                    << TestLog::EndMessage;
                m_testCtx.setTestResult(QP_TEST_RESULT_FAIL, "Unexpected null pointer");
            }
            else if (!apiSupported && (funcPtr != 0))
            {
                log << TestLog::Message << "Warning, received non-null value for unsupported function: " << funcName
                    << TestLog::EndMessage;
...
void GetProcAddressTests::init(void)
{
    // extensions
    {
        ...
        for (int extNdx = 0; extNdx < DE_LENGTH_OF_ARRAY(s_extensions); extNdx++)
        {
            ...
            extensionsGroup->addChild(
                new GetProcAddressExtensionCase(m_eglTestCtx, testName.c_str(), ("Test " + extName).c_str(), extName));
        }
    }
    ...
    // core functions
    {
        ...
        coreFuncGroup->addChild(new GetProcAddressCoreFunctionsCase(m_eglTestCtx, "egl", "Test EGL core functions",
                                                                    GetProcAddressCoreFunctionsCase::EGL14));

        coreFuncGroup->addChild(new GetProcAddressCoreFunctionsCase(m_eglTestCtx, "egl15", "Test EGL 1.5 functions",
                                                                    GetProcAddressCoreFunctionsCase::EGL15));
        coreFuncGroup->addChild(new GetProcAddressCoreFunctionsCase(
            m_eglTestCtx, "gles", "Test OpenGL ES core functions", GetProcAddressCoreFunctionsCase::GLES));
        coreFuncGroup->addChild(new GetProcAddressCoreFunctionsCase(
            m_eglTestCtx, "gles2", "Test OpenGL ES 2 core functions", GetProcAddressCoreFunctionsCase::GLES2));
        coreFuncGroup->addChild(new GetProcAddressCoreFunctionsCase(
            m_eglTestCtx, "gles3", "Test OpenGL ES 3 core functions", GetProcAddressCoreFunctionsCase::GLES3));
    }
}
