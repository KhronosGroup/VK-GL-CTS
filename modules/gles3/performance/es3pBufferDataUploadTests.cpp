...
void BasicBufferCase<SampleType>::init(void)
{
    ...
    m_dummyProgram = new glu::ShaderProgram(m_context.getRenderContext(),
                                            glu::ProgramSources() << glu::VertexSource(s_dummyVertexShader)
                                                                  << glu::FragmentSource(s_dummyFragnentShader));
...
void BasicUploadCase<SampleType>::createBuffer(int iteration, int bufferSize)
{
    ...
    {
        const std::vector<uint8_t> dummyData(bufferSize, 0x20);
