...
static bool compareBlockImages(const Surface &reference, const Surface &result, const tcu::RGBA &thresholdRGBA,
                               const IVec2 &blockSize, int numNonDummyBlocks, IVec2 &firstFailedBlockCoordDst,
...
ASTCBlockCase2D::IterateResult ASTCBlockCase2D::iterate(void)
{
    ...
    deMemcpy(compressed.getData(), &m_blockData[m_numBlocksTested * tcu::astc::BLOCK_SIZE_BYTES],
             curNumNonDummyBlocks * tcu::astc::BLOCK_SIZE_BYTES);
    if (curNumDummyBlocks > 1)
        tcu::astc::generateDummyVoidExtentBlocks(
            (uint8_t *)compressed.getData() + curNumNonDummyBlocks * tcu::astc::BLOCK_SIZE_BYTES, curNumDummyBlocks);
...

    {
        ...
        IVec4 maxDiff;
        const bool compareOk = compareBlockImages(referenceFrame, renderedFrame, threshold, blockSize,
                                                  curNumNonDummyBlocks, firstFailedBlockCoord, errorMask, maxDiff);
...

            {
                tcu::ScopedLogSection section(log, "Iteration " + de::toString(m_currentIteration),
                                              "Blocks " + de::toString(m_numBlocksTested) + " to " +
                                                  de::toString(m_numBlocksTested + curNumNonDummyBlocks - 1));

                if (curNumDummyBlocks > 0)
                    log << TestLog::Message << "Note: Only the first " << curNumNonDummyBlocks
                        << " blocks in the image are relevant; rest " << curNumDummyBlocks
...
ASTCBlockSizeRemainderCase2D::IterateResult ASTCBlockSizeRemainderCase2D::iterate(void)
{
    ...
    tcu::astc::generateDummyNormalBlocks((uint8_t *)compressed.getData(), totalNumBlocks, blockSize.x(), blockSize.y());
