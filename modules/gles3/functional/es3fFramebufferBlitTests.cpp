...
{
public:
    BlitDepthStencilCase(Context &context, const char *name, const char *desc, uint32_t format, uint32_t srcBuffers,
                         const IVec2 &srcSize, const IVec4 &srcRect, uint32_t dstBuffers, const IVec2 &dstSize,
                         const IVec4 &dstRect, uint32_t copyBuffers)
        : FboTestCase(context, name, desc)
        , m_format(format)
...
protected:
    ...
    void render(tcu::Surface &dst)
    {
        ...
        uint32_t srcDepthStencilRbo = 0;
        uint32_t dstDepthStencilRbo = 0;
        ...
        for (int ndx = 0; ndx < 2; ndx++)
        {
            ...
            uint32_t &depthStencilRbo = ndx ? dstDepthStencilRbo : srcDepthStencilRbo;
...
void FramebufferBlitTests::init(void)
{
    ...
    // .depth_stencil
    {
        ...
        {
            uint32_t format           = depthStencilFormats[fmtNdx];
            ...
            depthStencilGroup->addChild(new BlitDepthStencilCase(
                m_context, (fmtName + "_basic").c_str(), "", format, buffers, IVec2(128, 128), IVec4(0, 0, 128, 128),
                buffers, IVec2(128, 128), IVec4(0, 0, 128, 128), buffers));
            depthStencilGroup->addChild(new BlitDepthStencilCase(
                m_context, (fmtName + "_scale").c_str(), "", format, buffers, IVec2(127, 119), IVec4(10, 30, 100, 70),
                buffers, IVec2(111, 130), IVec4(20, 5, 80, 130), buffers));

            if (depth && stencil)
            {
                depthStencilGroup->addChild(new BlitDepthStencilCase(
                    m_context, (fmtName + "_depth_only").c_str(), "", format, buffers, IVec2(128, 128),
                    IVec4(0, 0, 128, 128), buffers, IVec2(128, 128), IVec4(0, 0, 128, 128), GL_DEPTH_BUFFER_BIT));
                depthStencilGroup->addChild(new BlitDepthStencilCase(
                    m_context, (fmtName + "_stencil_only").c_str(), "", format, buffers, IVec2(128, 128),
                    IVec4(0, 0, 128, 128), buffers, IVec2(128, 128), IVec4(0, 0, 128, 128), GL_STENCIL_BUFFER_BIT));
            }
        }
    }
