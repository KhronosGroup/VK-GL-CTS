    ...
    for (int y = 0; y < access.getHeight(); y++)
        for (int x = 0; x < access.getWidth(); x++)
        {
            ...
            case tcu::TEXTURECHANNELCLASS_UNSIGNED_FIXED_POINT:
            {
                ...
                IVec4 dstPixel(0, 0, 0, (0x1u << (uint64_t)bitDepth.w()) - 1);
            ...
            for (int vecNdx = 0; vecNdx < numVecs; vecNdx++)
            {
                ...
                else if (isInt)
                {
                    ...
                    if (de::inBounds(output.location + vecNdx, 0, (int)attachments.size()))
                    {
                        ...
                        const IVec4 fmtMinVal =
                            (-(tcu::Vector<int64_t, 4>(1) << (fmtBits - 1).cast<int64_t>())).asInt();
                        const IVec4 fmtMaxVal =
                            ((tcu::Vector<int64_t, 4>(1) << (fmtBits - 1).cast<int64_t>()) - int64_t(1)).asInt();
