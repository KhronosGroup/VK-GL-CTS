...
bool RenderCase::drawTestPattern(bool useTexture)
{
    ...
    // draw pattern
    {
        ...
        if (useTexture)
        {
            ...
            gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, textureSize, textureSize, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                          buffer[0].getPtr());
...
public:
    ...
    TextureCase(Context &context, const char *name, const char *desc, ShaderType type, TextureType texType,
...
TextureCase::TextureCase(Context &context, const char *name, const char *desc, ShaderType type, TextureType texType,
                         UploadType uploadType)
    : RenderCase(context, name, desc)
    , m_type(type)
    ...
    // gen texture
    {
        ...
        if (m_textureType == TEXTURE_FLOAT)
        {
            ...
            for (int x = 0; x < TEST_TEXTURE_SIZE; ++x)
                ...
                {
                    texData[(x * TEST_TEXTURE_SIZE + y) * 2 + 0] =
            ...
            if (m_uploadType == UPLOAD_PBO)
                gl.bufferData(GL_PIXEL_UNPACK_BUFFER, (glw::GLsizeiptr)(texData.size() * sizeof(uint32_t)), &texData[0],
                              GL_STATIC_DRAW);

            gl.texImage2D(GL_TEXTURE_2D, 0, GL_RG32F, TEST_TEXTURE_SIZE, TEST_TEXTURE_SIZE, 0, GL_RG, GL_FLOAT,
                          dataPtr);
        ...
        else if (m_textureType == TEXTURE_DEPTH)
        {
            ...
            m_testCtx.getLog() << tcu::TestLog::Message
                               ...
                               << "  TEXTURE_COMPARE_MODE = COMPARE_REF_TO_TEXTURE" << tcu::TestLog::EndMessage;

            for (int x = 0; x < TEST_TEXTURE_SIZE; ++x)
                for (int y = 0; y < TEST_TEXTURE_SIZE; ++y)
                    texData[x * TEST_TEXTURE_SIZE + y] =
                        rnd.choose<uint32_t>(DE_ARRAY_BEGIN(s_specialFloats), DE_ARRAY_END(s_specialFloats));
            ...
            gl.texImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, TEST_TEXTURE_SIZE, TEST_TEXTURE_SIZE, 0,
                          GL_DEPTH_COMPONENT, GL_FLOAT, dataPtr);
...
TextureCase::IterateResult TextureCase::iterate(void)
{
    ...
    std::vector<uint16_t> indices((gridSize - 1) * (gridSize - 1) * 6);
    tcu::Surface resultImage(TEST_CANVAS_SIZE, TEST_CANVAS_SIZE);
    ...
    for (int x = 0; x < gridSize; ++x)
        ...
        {
            const float posX =
                (float)x / ((float)gridSize - 1.0f) * 2.0f - 1.0f; // map from [0, gridSize - 1] to [-1, 1]
    ...
    for (int x = 0; x < gridSize - 1; ++x)
        for (int y = 0; y < gridSize - 1; ++y)
        {
            ...
            indices[baseNdx + 0] = (uint16_t)((x + 0) * gridSize + (y + 0));
            indices[baseNdx + 1] = (uint16_t)((x + 1) * gridSize + (y + 1));
            indices[baseNdx + 2] = (uint16_t)((x + 1) * gridSize + (y + 0));

            indices[baseNdx + 3] = (uint16_t)((x + 0) * gridSize + (y + 0));
            indices[baseNdx + 4] = (uint16_t)((x + 1) * gridSize + (y + 1));
            indices[baseNdx + 5] = (uint16_t)((x + 0) * gridSize + (y + 1));
        }

    m_testCtx.getLog() << tcu::TestLog::Message << "Drawing a textured grid with the shader."
                       << tcu::TestLog::EndMessage;
...
void TextureSamplerCase::init(void)
{
    ...
    // gen texture
    {
        ...
        if (m_testType == TEST_TEX_COORD)
        {
            ...
            gl.bindTexture(GL_TEXTURE_2D, m_textureID);
            gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, TEST_TEXTURE_SIZE, TEST_TEXTURE_SIZE, 0, GL_RGBA,
        ...
        else if (m_testType == TEST_LOD || m_testType == TEST_GRAD)
        {
            ...
            for (int level = 0; (TEST_TEXTURE_SIZE >> level); ++level)
                gl.texImage2D(GL_TEXTURE_2D, level, GL_RGBA8, TEST_TEXTURE_SIZE >> level, TEST_TEXTURE_SIZE >> level, 0,
        ...
        else if (m_testType == TEST_TEX_COORD_CUBE)
        {
            ...
            for (int faceNdx = 0; faceNdx < DE_LENGTH_OF_ARRAY(faces); ++faceNdx)
                gl.texImage2D(faces[faceNdx], 0, GL_RGBA8, TEST_TEXTURE_CUBE_SIZE, TEST_TEXTURE_CUBE_SIZE, 0, GL_RGBA,
                              GL_UNSIGNED_BYTE, &texData[0]);
...
void OutputCase::testFBO(void)
{
    ...
    tcu::TextureFormat textureFormat(tcu::TextureFormat::RGBA,
                                     (m_fboType == FBO_RGBA_FLOAT16 || m_fboType == FBO_RGBA_FLOAT32) ?
...
void SpecialFloatTests::init(void)
{
    ...
    // .vertex
    {
        ...
        vertexGroup->addChild(new TextureCase(m_context, "texture", "texture with special floating point values",
                                              TextureCase::TYPE_VERTEX, TextureCase::TEXTURE_FLOAT,
                                              TextureCase::UPLOAD_CLIENT));
        vertexGroup->addChild(
            new TextureCase(m_context, "texture_pbo", "texture (via pbo) with special floating point values",
                            TextureCase::TYPE_VERTEX, TextureCase::TEXTURE_FLOAT, TextureCase::UPLOAD_PBO));
        vertexGroup->addChild(
            new TextureCase(m_context, "texture_shadow", "shadow texture with special floating point values",
                            TextureCase::TYPE_VERTEX, TextureCase::TEXTURE_DEPTH, TextureCase::UPLOAD_CLIENT));
        ...
        vertexGroup->addChild(new TextureSamplerCase(m_context, "sampler_grad", "special texture grad",
                                                     TextureSamplerCase::TYPE_VERTEX, TextureSamplerCase::TEST_GRAD));
    ...
    // .fragment
    {
        ...
        fragmentGroup->addChild(new TextureCase(m_context, "texture", "texture with special floating point values",
                                                TextureCase::TYPE_FRAGMENT, TextureCase::TEXTURE_FLOAT,
                                                TextureCase::UPLOAD_CLIENT));
        fragmentGroup->addChild(
            new TextureCase(m_context, "texture_pbo", "texture (via pbo) with special floating point values",
                            TextureCase::TYPE_FRAGMENT, TextureCase::TEXTURE_FLOAT, TextureCase::UPLOAD_PBO));
        fragmentGroup->addChild(
            new TextureCase(m_context, "texture_shadow", "shadow texture with special floating point values",
                            TextureCase::TYPE_FRAGMENT, TextureCase::TEXTURE_DEPTH, TextureCase::UPLOAD_CLIENT));
        ...
        fragmentGroup->addChild(new TextureSamplerCase(m_context, "sampler_grad", "special texture grad",
                                                       TextureSamplerCase::TYPE_FRAGMENT,
                                                       TextureSamplerCase::TEST_GRAD));
    ...
    // .framebuffer
    {
        ...
        framebufferGroup->addChild(new OutputCase(m_context, "write_rgba8",
                                                  "write special floating point values to RGBA8 framebuffer",
                                                  FramebufferRenderCase::FBO_RGBA8));
        framebufferGroup->addChild(new OutputCase(m_context, "write_rgb10_a2",
                                                  "write special floating point values to RGB10_A2 framebuffer",
                                                  FramebufferRenderCase::FBO_RGB10_A2));
        ...
        framebufferGroup->addChild(new OutputCase(m_context, "write_float32",
                                                  "write special floating point values to float32 framebuffer",
                                                  FramebufferRenderCase::FBO_RGBA_FLOAT32));
        ...
        framebufferGroup->addChild(new BlendingCase(m_context, "blend_default",
                                                    ...
                                                    FramebufferRenderCase::FBO_DEFAULT));
        framebufferGroup->addChild(new BlendingCase(m_context, "blend_rgba8",
